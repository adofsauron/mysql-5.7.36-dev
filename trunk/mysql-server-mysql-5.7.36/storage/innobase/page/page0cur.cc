...
UNIV_INLINE
bool page_cur_try_search_shortcut(const buf_block_t *block, const dict_index_t *index, const dtuple_t *tuple,
                                  ulint *iup_matched_fields, ulint *ilow_matched_fields, page_cur_t *cursor)
{
  ...
  if (!page_rec_is_supremum(next_rec))
  {
    ...
    if (cmp_dtuple_rec_with_match(tuple, next_rec, offsets, &up_match) >= 0)
    {
      goto exit_func;
    }
...
@return true on success */
UNIV_INLINE
bool page_cur_try_search_shortcut_bytes(const buf_block_t *block, const dict_index_t *index, const dtuple_t *tuple,
                                        ulint *iup_matched_fields, ulint *iup_matched_bytes, ulint *ilow_matched_fields,
                                        ulint *ilow_matched_bytes, page_cur_t *cursor)
{
  const rec_t *rec;
  const rec_t *next_rec;
  ulint low_match;
  ...
  ut_ad(rec);
  ut_ad(page_rec_is_user_rec(rec));
  if (ut_pair_cmp(*ilow_matched_fields, *ilow_matched_bytes, *iup_matched_fields, *iup_matched_bytes) < 0)
  {
  ...
  if (cmp_dtuple_rec_with_match_bytes(tuple, rec, index, offsets, &low_match, &low_bytes) < 0)
  {
    goto exit_func;
  }
  ...
  if (!page_rec_is_supremum(next_rec))
  {
    ...
    if (cmp_dtuple_rec_with_match_bytes(tuple, next_rec, index, offsets, &up_match, &up_bytes) >= 0)
    {
      goto exit_func;
    }
    ...
    ulint n)               /*!< in: compare nth field */
{
  ...
  if (type->mtype == DATA_VARCHAR || type->mtype == DATA_CHAR || type->mtype == DATA_FIXBINARY ||
      type->mtype == DATA_BINARY || type->mtype == DATA_BLOB || DATA_GEOMETRY_MTYPE(type->mtype) ||
      type->mtype == DATA_VARMYSQL || type->mtype == DATA_MYSQL)
  {
    if (dfield_get_len(dfield) != UNIV_SQL_NULL && rec_f_len != UNIV_SQL_NULL && rec_f_len >= dfield_get_len(dfield) &&
        !cmp_data_data(type->mtype, type->prtype, dfield_get_data(dfield), dfield_get_len(dfield), rec_f,
...
static ulint *populate_offsets(const rec_t *rec, const dtuple_t *tuple, dict_index_t *index, ulint *offsets,
                               mem_heap_t **heap)
{
  ...
  if (!index->rec_cache.offsets_cached ||
      (rec_offs_n_fields(index->rec_cache.offsets) < dtuple_get_n_fields_cmp(tuple)))
  {
    offsets = rec_get_offsets(rec, index, offsets, dtuple_get_n_fields_cmp(tuple), heap);
    ...
    if (sz1 < sz2)
    {
      index->rec_cache.offsets = static_cast<ulint *>(mem_heap_alloc(index->heap, sizeof(ulint) * sz2));
    ...
    rtr_info_t *rtr_info) /*!< in/out: rtree search stack */
{
...
#ifdef PAGE_CUR_ADAPT
  if (page_is_leaf(page) && (mode == PAGE_CUR_LE) && !dict_index_is_spatial(index) &&
      (page_header_get_field(page, PAGE_N_DIRECTION) > 3) && (page_header_get_ptr(page, PAGE_LAST_INSERT)) &&
      (page_header_get_field(page, PAGE_DIRECTION) == PAGE_RIGHT))
  {
    if (page_cur_try_search_shortcut(block, index, tuple, iup_matched_fields, ilow_matched_fields, cursor))
    {
      return;
    }
  }
  ...
  if (dict_index_is_spatial(index) && mode > PAGE_CUR_LE)
  {
    ...
    {
      rtr_cur_search_with_match(block, (dict_index_t *)index, tuple, mode, cursor, rtr_info);
      return;
    }
  }
  ...
  while (up - low > 1)
  {
    ...
    {
      offsets = populate_offsets(mid_rec, tuple, const_cast<dict_index_t *>(index), offsets, &heap);
    }
    else
    {
    ...
    cmp = cmp_dtuple_rec_with_match(tuple, mid_rec, offsets, &cur_matched_fields);
...
first partially matched field in the lower limit record
@param[out]	cursor			page cursor */
void page_cur_search_with_match_bytes(const buf_block_t *block, const dict_index_t *index, const dtuple_t *tuple,
                                      ...
                                      ulint *ilow_matched_fields, ulint *ilow_matched_bytes, page_cur_t *cursor)
{
  ...
  {
    if (page_cur_try_search_shortcut_bytes(block, index, tuple, iup_matched_fields, iup_matched_bytes,
  ...
  while (up - low > 1)
  {
    ...
    offsets = rec_get_offsets(mid_rec, index, offsets_, dtuple_get_n_fields_cmp(tuple), &heap);

    cmp = cmp_dtuple_rec_with_match_bytes(tuple, mid_rec, index, offsets, &cur_matched_fields, &cur_matched_bytes);
    ...
    mtr_t *mtr)          /*!< in: mini-transaction handle */
{
  ...
  if (mtr_get_log_mode(mtr) != MTR_LOG_SHORT_INSERTS)
  {
    ...
    {
      log_ptr =
          mlog_open_and_write_index(mtr, insert_rec, index, MLOG_COMP_REC_INSERT, 2 + 5 + 1 + 5 + 5 + MLOG_BUF_MARGIN);
  ...
  if (page_rec_is_comp(insert_rec))
  {
    if (UNIV_UNLIKELY(rec_get_info_and_status_bits(insert_rec, TRUE) != rec_get_info_and_status_bits(cursor_rec, TRUE)))
    {
    ...
    mtr_t *mtr)          /*!< in: mtr or NULL */
{
  ...
  if (UNIV_UNLIKELY(mismatch_index >= UNIV_PAGE_SIZE))
  {
    ib::fatal() << "is_short " << is_short << ", "
                << "info_and_status_bits " << info_and_status_bits << ", offset " << page_offset(cursor_rec)
                << ","
                   " o_offset "
                << origin_offset << ", mismatch index " << mismatch_index << ", end_seg_len " << end_seg_len
                << " parsed len " << (ptr - ptr2);
  }
  ...
  if (UNIV_UNLIKELY(!page_cur_rec_insert(&cursor, buf + origin_offset, index, offsets, mtr)))
  {
    ...
    mtr_t *mtr)          /*!< in: mini-transaction handle, or NULL */
{
  ...
  ut_ad(mach_read_from_8(page + PAGE_HEADER + PAGE_INDEX_ID) == index->id || recv_recovery_is_on() ||
...
@return pointer to record if succeed, NULL otherwise */
rec_t *page_cur_direct_insert_rec_low(rec_t *current_rec, dict_index_t *index, const dtuple_t *tuple, ulint n_ext,
                                      mtr_t *mtr)
{
  byte *insert_buf;
  ulint rec_size;
    ...
    mtr_t *mtr)          /*!< in: mini-transaction handle, or NULL */
{
  ...
  ut_ad(fil_page_index_page_check(page));
  ut_ad(mach_read_from_8(page + PAGE_HEADER + PAGE_INDEX_ID) == index->id ||
        (mtr ? mtr->is_inside_ibuf() : dict_index_is_ibuf(index)) || recv_recovery_is_on());
  ...
  const bool reorg_before_insert = page_has_garbage(page) && rec_size > page_get_max_insert_size(page, 1) &&
                                   rec_size <= page_get_max_insert_size_after_reorganize(page, 1);

  /* 2. Try to find suitable space from page memory management */
  if (!page_zip_available(page_zip, dict_index_is_clust(index), rec_size, 1) || reorg_before_insert)
  {
    ...
    else if (page_is_empty(page))
    {
      ...
      page_create_zip(page_cur_get_block(cursor), index, page_header_get_field(page, PAGE_LEVEL), 0, NULL, mtr);
    ...
    else if (btr_page_reorganize_low(recv_recovery_is_on(), level, cursor, index, mtr))
    {
      ut_ad(!page_header_get_ptr(page, PAGE_FREE));
    ...
    insert_rec = page_cur_insert_rec_low(cursor->rec, index, rec, offsets, NULL);
    ...
    else
    {
      ...
      if (!log_compressed)
      {
        if (page_zip_compress(page_zip, page, index, level, NULL, NULL))
        {
          page_cur_insert_rec_write_log(insert_rec, rec_size, cursor->rec, index, mtr);
        ...
        ut_ad(cursor->rec == (pos > 1 ? page_rec_get_nth(page, pos - 1) : page + PAGE_NEW_INFIMUM));
      }
      else
      {
  ...
  if (UNIV_LIKELY_NULL(free_rec))
  {
    ...
    extra_size_diff = rec_offs_extra_size(offsets) - rec_offs_extra_size(foffsets);
  ...
  if (!dict_index_is_spatial(index))
  {
    ...
    else if ((last_insert == cursor->rec) && (page_header_get_field(page, PAGE_DIRECTION) != PAGE_LEFT))
    {
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  page_header_set_ptr(new_page, NULL, PAGE_HEAP_TOP, new_page + UNIV_PAGE_SIZE - 1);
#endif
  ...
  if (dict_table_is_temporary(index->table) || index->table->ibd_file_missing /* IMPORT TABLESPACE */)
  {
                               ...
                               or NULL */
{
  ...
  if (page_get_n_recs(page) == 1 && !recv_recovery_is_on())
  {
    ...
    page_cur_move_to_next(cursor);
    ut_ad(page_cur_is_after_last(cursor));
    page_create_empty(page_cur_get_block(cursor), const_cast<dict_index_t *>(index), mtr);
    return;
  }
