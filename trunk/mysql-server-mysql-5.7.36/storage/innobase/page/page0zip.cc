...
#define ASSERT_ZERO(b, s) ut_ad(!memcmp(b, field_ref_zero, ut_min(static_cast<size_t>(s), sizeof field_ref_zero)));
    ...
    ulint zip_size) /*!< in: compressed page size in bytes */
{
  lint size = zip_size
              ...
              - (PAGE_DATA + PAGE_ZIP_CLUST_LEAF_SLOT_SIZE + 1 /* encoded heap_no==2 in page_zip_write_rec() */
...
bool page_zip_is_too_big(const dict_index_t *index, const dtuple_t *entry)
{
  ...
  modification log. */
  ulint free_space_zip = page_zip_empty_size(index->n_fields, page_size.physical());
  ...
  if (entry->n_fields >= n_uniq && (REC_NODE_PTR_SIZE +
                                        rec_get_converted_size_comp_prefix(index, entry->fields, n_uniq, NULL)
    ...
    byte *buf) /*!< out: buffer of (n + 1) * 2 bytes */
{
  ...
  for (i = col = 0; i < n; i++)
  {
    ...
    else if (val)
    {
      ...
      if (fixed_sum && UNIV_UNLIKELY(fixed_sum + field->fixed_len > DICT_MAX_FIXED_COL_LEN))
      {
    ...
    mem_heap_t *heap)                /*!< in: temporary memory heap */
{
  ...
  do
  {
    ...
    UNIV_MEM_ASSERT_RW(rec - rec_offs_extra_size(offsets), rec_offs_extra_size(offsets));

    /* Compress the extra bytes. */
    c_stream->avail_in = static_cast<uInt>(rec - REC_N_NEW_EXTRA_BYTES - c_stream->next_in);
    ...
    c_stream->avail_in = static_cast<uInt>(rec_offs_data_size(offsets) - REC_NODE_PTR_SIZE);
                                     ...
                                     externally stored columns */
{
  ...
  for (i = 0; i < rec_offs_n_fields(offsets); i++)
  {
    ...
    if (UNIV_UNLIKELY(i == trx_id_col))
    {
      ...
      memcpy(storage - (DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN) * (rec_get_heap_no_new(rec) - 1), c_stream->next_in,
    ...
    else if (rec_offs_nth_extern(offsets, i))
    {
      ...
      if (UNIV_LIKELY_NULL(page_zip_dir_find_low(storage, deleted, page_offset(rec))))
      {
        continue;
      }
    ...
    mem_heap_t *heap)                /*!< in: temporary memory heap */
{
  ...
  do
  {
    ...
    if (rec_offs_any_extern(offsets))
    {
      ...
      err =
          page_zip_compress_clust_ext(LOGFILE c_stream, rec, offsets, trx_id_col, deleted, storage, &externs, n_blobs);
    ...
    c_stream->avail_in = static_cast<uInt>(rec + rec_offs_data_size(offsets) - c_stream->next_in);
                ...
                or NULL */
{
  ...
  ut_ad(page_zip_simple_validate(page_zip));
  ut_ad(!index || (index && dict_table_is_comp(index->table) && !dict_index_is_ibuf(index)));
  ...
  ut_a(
      !memcmp(page + (PAGE_NEW_SUPREMUM - REC_N_NEW_EXTRA_BYTES + 1), supremum_extra_data, sizeof supremum_extra_data));
  ...
  if (UNIV_UNLIKELY(page_zip_compress_dbg))
  {
    ib::info() << "compress " << static_cast<void *>(page_zip) << " " << static_cast<const void *>(page) << " "
               << page_is_leaf(page) << " " << n_fields << " " << n_dense;
  }
  ...
  if (UNIV_UNLIKELY(n_dense * PAGE_ZIP_DIR_SLOT_SIZE >= page_zip_get_size(page_zip)))
  {
    goto err_exit;
  }
  ...
  heap = mem_heap_create(page_zip_get_size(page_zip) + n_fields * (2 + sizeof(ulint)) + REC_OFFS_HEADER_SIZE +
                         n_dense * ((sizeof *recs) - PAGE_ZIP_DIR_SLOT_SIZE) + UNIV_PAGE_SIZE * 4 +
                         (512 << MAX_MEM_LEVEL));

  recs = static_cast<const rec_t **>(mem_heap_zalloc(heap, n_dense * sizeof *recs));
  ...
  err = deflateInit2(&c_stream, static_cast<int>(level), Z_DEFLATED, UNIV_PAGE_SIZE_SHIFT, MAX_MEM_LEVEL,
                     Z_DEFAULT_STRATEGY);
  ut_a(err == Z_OK);
  ...
  if (page_is_leaf(page))
  {
    if ((index && dict_index_is_clust(index)) || (page_comp_info && (page_comp_info->type & DICT_CLUSTERED)))
    {
      if (index)
      {
      ...
      else if (page_comp_info && (page_comp_info->type & DICT_CLUSTERED))
      {
  ...
  if (UNIV_UNLIKELY(c_stream.avail_out <= n_dense * slot_size + 6 /* sizeof(zlib header and footer) */))
  {
    goto zlib_error;
  }
  ...
  {
    ut_ad(page_comp_info != NULL);
    c_stream.avail_in = static_cast<uInt>(page_comp_info->field_len);
  ...
  {
    c_stream.avail_in = static_cast<uInt>(page_zip_fields_encode(n_fields, index, trx_id_col, fields));
  }
  ...
  else
  {
    ...
    err = page_zip_compress_clust(LOGFILE & c_stream, recs, n_dense, index, &n_blobs, trx_id_col,
                                  buf_end - PAGE_ZIP_DIR_SLOT_SIZE * page_get_n_recs(page), storage, heap);
    if (UNIV_UNLIKELY(err != Z_OK))
    {
      goto zlib_error;
    }
  ...
  c_stream.avail_in = static_cast<uInt>(page_header_get_field(page, PAGE_HEAP_TOP) - (c_stream.next_in - page));
...
static dict_index_t *page_zip_fields_decode(
    /*===================*/
    ...
    bool is_spatial)   /*< in: is spatial index or not */
{
  ...
  table = dict_mem_table_create("ZIP_DUMMY", DICT_HDR_SPACE, n, 0, DICT_TF_COMPACT, 0);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) ibool page_zip_dir_decode(
    /*================*/
    const page_zip_des_t *page_zip, /*!< in: dense page directory on
                                   compressed page */
                                    ...
                                    size of recs[] */
{
  ...
  for (i = 0; i < n_recs; i++)
  {
    ...
    if (UNIV_UNLIKELY((offs & PAGE_ZIP_DIR_SLOT_MASK) < PAGE_ZIP_START + REC_N_NEW_EXTRA_BYTES))
    {
    ...
    const byte *end)      /*!< in: end of modification log */
{
  ...
  for (i = 0; i < rec_offs_n_fields(offsets); i++)
  {
    ...
    if (UNIV_UNLIKELY(i == trx_id_col))
    {
      ...
      if (UNIV_UNLIKELY(dst - next_out >= end - data) || UNIV_UNLIKELY(len < (DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN)) ||
...
#error "REC_STATUS_NODE_PTR != TRUE"
    ...
    else
    {
      ...
      if (UNIV_UNLIKELY(data + l >= end) || UNIV_UNLIKELY(len < (DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN)))
      {
        page_zip_fail(
            ("page_zip_apply_log:"
             " trx_id %p+%lu >= %p\n",
    ...
    mem_heap_t *heap)         /*!< in: temporary memory heap */
{
  ...
  /* Subtract the space reserved for uncompressed data. */
  d_stream->avail_in -= static_cast<uInt>(n_dense * (PAGE_ZIP_DIR_SLOT_SIZE + REC_NODE_PTR_SIZE));
  ...
  for (slot = 0; slot < n_dense; slot++)
  {
    ...
    d_stream->avail_out = static_cast<uInt>(rec - REC_N_NEW_EXTRA_BYTES - d_stream->next_out);
    ...
    d_stream->avail_out = static_cast<uInt>(rec_offs_data_size(offsets) - REC_NODE_PTR_SIZE);
  ...
  allocated from an originally longer space on the free list. */
  d_stream->avail_out =
      static_cast<uInt>(page_header_get_field(page_zip->data, PAGE_HEAP_TOP) - page_offset(d_stream->next_out));
  if (UNIV_UNLIKELY(d_stream->avail_out > UNIV_PAGE_SIZE - PAGE_ZIP_START - PAGE_DIR))
  {
    page_zip_fail(
        ("page_zip_decompress_node_ptrs:"
         " avail_out = %u\n",
...
zlib_done:
  ...
  /* Apply the modification log. */
  {
    ...
    mod_log_ptr = page_zip_apply_log(d_stream->next_in, d_stream->avail_in + 1, recs, n_dense, ULINT_UNDEFINED,
  ...
  if (UNIV_UNLIKELY(page_zip_get_trailer_len(page_zip, dict_index_is_clust(index)) + page_zip->m_end >=
                    page_zip_get_size(page_zip)))
  {
    page_zip_fail(
        ("page_zip_decompress_node_ptrs:"
         " %lu + %lu >= %lu, %lu\n",
         (ulong)page_zip_get_trailer_len(page_zip, dict_index_is_clust(index)), (ulong)page_zip->m_end,
         (ulong)page_zip_get_size(page_zip), (ulong)dict_index_is_clust(index)));
    return (FALSE);
  }
...
zlib_done:
  ...
  if (UNIV_UNLIKELY(page_zip_get_trailer_len(page_zip, FALSE) + page_zip->m_end >= page_zip_get_size(page_zip)))
  {
    ...
    mem_heap_t *heap)         /*!< in: temporary memory heap */
{
  ...
  /* Subtract the space reserved for uncompressed data. */
  d_stream->avail_in -= static_cast<uInt>(n_dense) * (PAGE_ZIP_CLUST_LEAF_SLOT_SIZE);
  ...
  for (slot = 0; slot < n_dense; slot++)
  {
    ...
    if (rec_offs_any_extern(offsets))
    {
      if (UNIV_UNLIKELY(!page_zip_decompress_clust_ext(d_stream, rec, offsets, trx_id_col)))
      {
        goto zlib_error;
      }
    }
    ...
    d_stream->avail_out = static_cast<uInt>(rec_get_end(rec, offsets) - d_stream->next_out);
...
zlib_done:
  ...
  {
    const byte *mod_log_ptr;
    mod_log_ptr = page_zip_apply_log(d_stream->next_in, d_stream->avail_in + 1, recs, n_dense, trx_id_col, heap_status,
                                     index, offsets);
  ...
  if (UNIV_UNLIKELY(page_zip_get_trailer_len(page_zip, TRUE) + page_zip->m_end >= page_zip_get_size(page_zip)))
  {
...
/**********************************************************************/ /**
 ...
 @return TRUE on success, FALSE on failure */
static ibool page_zip_decompress_low(
    /*====================*/
                              ...
                              after page creation */
{
  ...
  else
  {
    ...
    ut_a(!memcmp(PAGE_HEADER + PAGE_LEVEL + page, PAGE_HEADER + PAGE_LEVEL + page_zip->data,
  ...
  d_stream.avail_in = static_cast<uInt>(page_zip_get_size(page_zip) - (PAGE_DATA + 1));
...

  {
    ...
    ulint n = 1 + 1 /* node ptr */ + REC_OFFS_HEADER_SIZE + dict_index_get_n_fields(index);
  ...
  if (!page_is_leaf(page))
  {
    ...
    if (UNIV_UNLIKELY(!page_zip_decompress_node_ptrs(page_zip, &d_stream, recs, n_dense, index, offsets, heap)))
    {
      goto err_exit;
    }

    info_bits = mach_read_from_4(page + FIL_PAGE_PREV) == FIL_NULL ? REC_INFO_MIN_REC_FLAG : 0;
  ...
  {
    /* This is a leaf page in a secondary index. */
    if (UNIV_UNLIKELY(!page_zip_decompress_sec(page_zip, &d_stream, recs, n_dense, index, offsets)))
    {
      goto err_exit;
    }
  ...
  {
    /* This is a leaf page in a clustered index. */
    if (UNIV_UNLIKELY(!page_zip_decompress_clust(page_zip, &d_stream, recs, n_dense, index, trx_id_col, offsets, heap)))
    {
      goto err_exit;
    }
                                    ...
                                    TRUE=ignore the MIN_REC_FLAG */
{
  ...
  if (memcmp(page_zip->data + FIL_PAGE_PREV, page + FIL_PAGE_PREV, FIL_PAGE_LSN - FIL_PAGE_PREV) ||
      memcmp(page_zip->data + FIL_PAGE_TYPE, page + FIL_PAGE_TYPE, 2) ||
      memcmp(page_zip->data + FIL_PAGE_DATA, page + FIL_PAGE_DATA, PAGE_DATA - FIL_PAGE_DATA))
  {
  ...
  if (memcmp(page + PAGE_HEADER, temp_page + PAGE_HEADER, UNIV_PAGE_SIZE - PAGE_HEADER - FIL_PAGE_DATA_END))
  {
      ...
      if (!memcmp(page + PAGE_HEADER, temp_page + PAGE_HEADER, UNIV_PAGE_SIZE - PAGE_HEADER - FIL_PAGE_DATA_END))
      {
        ...
        page_zip_fail(
            ("page_zip_validate:"
             " min_rec_flag"
             " (%s%lu,%lu,0x%02lx)\n",
             sloppy ? "ignored, " : "", page_get_space_id(page), page_get_page_no(page), (ulong)page[offset]));
    ...
    do
    {
      ...
      if (index)
      {
        ...
        if (memcmp(rec - rec_offs_extra_size(offsets), trec - rec_offs_extra_size(offsets), rec_offs_size(offsets)))
        {
          page_zip_fail(
              ("page_zip_validate:"
    ...
    byte *data)               /*!< in: end of modification log */
{
  ...
  ut_ad(data + rec_offs_data_size(offsets) - (DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN) - n_ext * BTR_EXTERN_FIELD_REF_SIZE <
    ...
    ulint create)             /*!< in: nonzero=insert, zero=update */
{
  ...
  ut_ad(rec_get_end((rec_t *)rec, offsets) <=
        page + UNIV_PAGE_SIZE - PAGE_DIR - PAGE_DIR_SLOT_SIZE * page_dir_get_n_slots(page));
  ...
  if (page_is_leaf(page))
  {
    ...
    if (dict_index_is_clust(index))
    {
      ...
      else
      {
        ...
        /* Store trx_id and roll_ptr. */
        memcpy(storage - (DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN) * (heap_no - 1), src,
               DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN);
                              ...
                              or NULL if no logging is needed */
{
  ...
  blob_no = page_zip_get_n_prev_extern(page_zip, rec, index) + rec_get_n_extern_new(rec, index, n);
  ut_a(blob_no < page_zip->n_blobs);

  externs = page_zip->data + page_zip_get_size(page_zip) -
            (page_dir_get_n_heap(page) - PAGE_HEAP_NO_USER_LOW) * PAGE_ZIP_CLUST_LEAF_SLOT_SIZE;
    ...
    page_zip_des_t *page_zip) /*!< in/out: compressed page */
{
  ...
  if (page)
  {
    ...
    if (UNIV_UNLIKELY((storage_end - storage) % REC_NODE_PTR_SIZE) || UNIV_UNLIKELY(heap_no < PAGE_HEAP_NO_USER_LOW) ||
    ...
    mtr_t *mtr)               /*!< in: mini-transaction, or NULL */
{
  ...
  storage = page_zip_dir_start(page_zip) - (rec_get_heap_no_new(rec) - 1) * REC_NODE_PTR_SIZE;
    ...
    roll_ptr_t roll_ptr)      /*!< in: roll_ptr */
{
  ...
  storage = page_zip_dir_start(page_zip) - (rec_get_heap_no_new(rec) - 1) * (DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN);
    ...
    const ulint *offsets)      /*!< in: rec_get_offsets(rec, index) */
{
  ...
  if (!page_is_leaf(page))
  {
    ...
    storage = page_zip_dir_start(page_zip);
    ut_ad(dict_index_get_n_unique_in_tree_nonleaf(index) == rec_offs_n_fields(offsets) - 1);
  ...
  else if (dict_index_is_clust(index))
  {
    ...
    dense page directory, at the very end of the page. */
    const ulint trx_id_pos = dict_col_get_clust_pos(dict_table_get_sys_col(index->table, DATA_TRX_ID), index);
    storage = page_zip_dir_start(page_zip);
                              ...
                              zero for others */
{
  ...
  dir = page_zip->data + page_zip_get_size(page_zip) - PAGE_ZIP_DIR_SLOT_SIZE * n_dense;
  ...
  else if (is_clustered)
  {
    ...
    ASSERT_ZERO(externs - PAGE_ZIP_CLUST_LEAF_SLOT_SIZE, PAGE_ZIP_CLUST_LEAF_SLOT_SIZE);
  ...
  else
  {
    ...
    ASSERT_ZERO(stored - PAGE_ZIP_DIR_SLOT_SIZE, static_cast<size_t>(PAGE_ZIP_DIR_SLOT_SIZE));
  }
    ...
    mtr_t *mtr)          /*!< in: mini-transaction */
{
  ...
  if (!dict_index_is_clust(index) && !dict_table_is_temporary(index->table) && page_is_leaf(temp_page))
  {
  ...
  if (!page_zip_compress(page_zip, page, index, page_zip_level, NULL, mtr))
  {
#ifndef UNIV_HOTBACKUP
...
void page_zip_copy_recs(
    ...
    mtr_t *mtr)                    /*!< in: mini-transaction */
{
  ut_ad(mtr_memo_contains_page(mtr, page, MTR_MEMO_PAGE_X_FIX) || dict_table_is_intrinsic(index->table));
  ut_ad(mtr_memo_contains_page(mtr, src, MTR_MEMO_PAGE_X_FIX) || dict_table_is_intrinsic(index->table));
  ut_ad(!dict_index_is_ibuf(index));
#ifdef UNIV_ZIP_DEBUG
  ...
  ut_a(dict_index_is_clust(index) || dict_table_is_temporary(index->table) || !page_is_leaf(src) ||
...
#error "PAGE_MAX_TRX_ID + 8 != PAGE_HEADER_PRIV_END"
  ...
  memcpy(PAGE_DATA + page, PAGE_DATA + src, UNIV_PAGE_SIZE - PAGE_DATA - FIL_PAGE_DATA_END);
  ...
  ut_ad(page_zip_get_trailer_len(page_zip, dict_index_is_clust(index)) + page_zip->m_end < page_zip_get_size(page_zip));

  if (!page_is_leaf(src) && UNIV_UNLIKELY(mach_read_from_4(src + FIL_PAGE_PREV) == FIL_NULL) &&
...
uint32_t page_zip_calc_checksum(const void *data, ulint size, srv_checksum_algorithm_t algo,
                                bool use_legacy_big_endian /* = false */)
{
  ...
  switch (algo)
  {
    ...
    case SRV_CHECKSUM_ALGORITHM_STRICT_CRC32:
    {
      ...
      const uint32_t crc32 = crc32_func(s + FIL_PAGE_OFFSET, FIL_PAGE_LSN - FIL_PAGE_OFFSET) ^
                             crc32_func(s + FIL_PAGE_TYPE, 2) ^
                             crc32_func(s + FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID, size - FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID);
...
ibool page_zip_verify_checksum(
    /*=====================*/
...
)
{
  ...
  const uint32_t stored = static_cast<uint32_t>(mach_read_from_4(p));
...
#error "FIL_PAGE_LSN must be 64 bit aligned"
  ...
  if (stored == 0 && *reinterpret_cast<const ib_uint64_t *>(static_cast<const char *>(data) + FIL_PAGE_LSN) == 0)
  {
  ...
  ulint space_id = mach_read_from_4(static_cast<const unsigned char *>(data) + FIL_PAGE_SPACE_ID);
  ...
  if (is_log_enabled)
  {
    fprintf(log_file,
            ...
            " recorded = %u\n",
            page_no, buf_checksum_algorithm_name(static_cast<srv_checksum_algorithm_t>(srv_checksum_algorithm)), calc,
            stored);
  }

  if (!strict_check)
  {
    const uint32_t crc32 = page_zip_calc_checksum(data, size, SRV_CHECKSUM_ALGORITHM_CRC32);
  ...
  switch (curr_algo)
  {
    ...
    case SRV_CHECKSUM_ALGORITHM_CRC32:
      ...
      {
        if (stored == page_zip_calc_checksum(data, size, curr_algo, true))
        {
          return (TRUE);
        }
      ...
      if (stored == page_zip_calc_checksum(data, size, SRV_CHECKSUM_ALGORITHM_INNODB))
      {
#ifndef UNIV_INNOCHECKSUM
        if (curr_algo == SRV_CHECKSUM_ALGORITHM_STRICT_CRC32)
        {
      ...
      if (!legacy_checksum_checked && stored == page_zip_calc_checksum(data, size, curr_algo, true))
      {
    ...
    case SRV_CHECKSUM_ALGORITHM_INNODB:
      ...
      if (stored == page_zip_calc_checksum(data, size, SRV_CHECKSUM_ALGORITHM_CRC32) ||
          stored == page_zip_calc_checksum(data, size, SRV_CHECKSUM_ALGORITHM_CRC32, true))
      {
#ifndef UNIV_INNOCHECKSUM
        if (curr_algo == SRV_CHECKSUM_ALGORITHM_STRICT_INNODB)
        {
