    ...
    const rec_t *rec) /*!< in: the physical record */
{
  ...
  while (UNIV_LIKELY(*(uint16 *)slot != rec_offs_bytes))
  {
    ...
    {
      ib::error() << "Probable data corruption on page " << page_get_page_no(page) << ". Original record on that page;";
...
static const byte infimum_supremum_redundant[] = {
    /* the infimum record */
    0x08 /*end offset*/, 0x01 /*n_owned*/, 0x00, 0x00 /*heap_no=0*/, 0x03 /*n_fields=1, 1-byte offsets*/, 0x00,
    0x74 /* pointer to supremum */, 'i', 'n', 'f', 'i', 'm', 'u', 'm', 0,
    /* the supremum record */
    0x09 /*end offset*/, 0x01 /*n_owned*/, 0x00, 0x08 /*heap_no=1*/, 0x03 /*n_fields=1, 1-byte offsets*/, 0x00,
    0x00 /* end of record list */, 's', 'u', 'p', 'r', 'e', 'm', 'u', 'm', 0};

/** The page infimum and supremum of an empty page in ROW_FORMAT=COMPACT */
static const byte infimum_supremum_compact[] = {
    /* the infimum record */
    0x01 /*n_owned=1*/, 0x00, 0x02 /* heap_no=0, REC_STATUS_INFIMUM */, 0x00, 0x0d /* pointer to supremum */, 'i', 'n',
    'f', 'i', 'm', 'u', 'm', 0,
    /* the supremum record */
    0x01 /*n_owned=1*/, 0x00, 0x0b /* heap_no=1, REC_STATUS_SUPREMUM */, 0x00, 0x00 /* end of record list */, 's', 'u',
    'p', 'r', 'e', 'm', 'u', 'm'};
...
#error "PAGE_BTR_IBUF_FREE_LIST_NODE + FLST_NODE_SIZE > PAGE_DATA"
  ...
  else
  {
    ...
    memcpy(page + PAGE_DATA, infimum_supremum_redundant, sizeof infimum_supremum_redundant);
    memset(page + PAGE_OLD_SUPREMUM_END, 0, UNIV_PAGE_SIZE - PAGE_DIR - PAGE_OLD_SUPREMUM_END);
...
void page_parse_create(buf_block_t *block, ulint comp, bool is_rtree)
{
  if (block != NULL)
  {
                ...
                handle */
{
  ...
  is_spatial = index ? dict_index_is_spatial(index) : page_comp_info->type & DICT_SPATIAL;
  ...
  if (truncate_t::s_fix_up_active)
  {
    ...
    if (!page_zip_compress(page_zip, page, index, page_zip_level, page_comp_info, NULL))
    {
  ...
  else if (!page_zip_compress(page_zip, page, index, page_zip_level, NULL, mtr))
  {
    /* The compression of a newly created
    page should always succeed. */
    ut_error;
    ...
    mtr_t *mtr)          /*!< in/out: mini-transaction */
{
  ...
  for MVCC. */
  if (dict_index_is_sec_or_ibuf(index) && !dict_table_is_temporary(index->table) && page_is_leaf(page))
  {
  ...
  if (page_zip)
  {
    page_create_zip(block, index, page_header_get_field(page, PAGE_LEVEL), max_trx_id, NULL, mtr);
  }
  else
  {
    page_create(block, mtr, page_is_comp(page), dict_index_is_spatial(index));
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
  ...
  else
  {
    if (dict_index_is_spatial(index))
    {
      ...
      rtr_page_copy_rec_list_end_no_locks(new_block, block, rec, index, heap, rec_move, max_to_move, &num_moved, mtr);
    }
    else
    {
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
  ...
  if (dict_index_is_spatial(index))
  {
    ...
    /* For spatial index, we need to insert recs one by one
    to keep recs ordered. */
    rtr_page_copy_rec_list_start_no_locks(new_block, block, rec, index, heap, rec_move, max_to_move, &num_moved, mtr);
  }
  else
  {
  ...
  max_trx_id is ignored for temp tables because it not required
  for MVCC. */
  if (dict_index_is_sec_or_ibuf(index) && page_is_leaf(page_align(rec)) && !dict_table_is_temporary(index->table))
  {
  ...
  if (new_page_zip)
  {
    ...
    DBUG_EXECUTE_IF("page_copy_rec_list_start_compress_fail", goto zip_reorganize;);
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  page_delete_rec_list_write_log(rec, index, page_is_comp(page) ? MLOG_COMP_LIST_END_DELETE : MLOG_LIST_END_DELETE,
                                 mtr);
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
...
#ifdef UNIV_ZIP_DEBUG
  {
    ...
    ut_a(!page_zip || page_zip_validate(page_zip, page_align(split_rec), index));
  }
#endif /* UNIV_ZIP_DEBUG */

  if (UNIV_UNLIKELY(!page_copy_rec_list_end(new_block, block, split_rec, index, mtr)))
  {
    return (FALSE);
  }
...
ibool page_move_rec_list_start(
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
  if (UNIV_UNLIKELY(!page_copy_rec_list_start(new_block, block, split_rec, index, mtr)))
  {
    return (FALSE);
  }
    ...
    const ulint *offsets) /*!< in: record descriptor */
{
  ...
  if (page_rec_is_comp(rec))
  {
    ib::info() << "n_owned: " << rec_get_n_owned_new(rec) << "; heap_no: " << rec_get_heap_no_new(rec)
               << "; next rec: " << rec_get_next_offs(rec, TRUE);
  }
  else
  {
    ib::info() << "n_owned: " << rec_get_n_owned_old(rec) << "; heap_no: " << rec_get_heap_no_old(rec)
    ...
    const ulint *offsets) /*!< in: array returned by rec_get_offsets() */
{
  ...
  {
    ib::warn() << "Dir slot of rec " << page_offset(rec) << ", n owned too big " << n_owned;
    return (FALSE);
  }

  if (UNIV_UNLIKELY(!(heap_no < page_dir_get_n_heap(page))))
  {
    ib::warn() << "Heap no of rec " << page_offset(rec) << " too big " << heap_no << " " << page_dir_get_n_heap(page);
    return (FALSE);
  }
    ...
    const page_t *page) /*!< in: index page in ROW_FORMAT=REDUNDANT */
{
  ...
  {
    ib::error() << "Nonsensical number " << n_slots << " of page dir slots";
  ...
  {
    ib::error() << "Record heap and dir overlap on a page, heap top " << page_header_get_field(page, PAGE_HEAP_TOP)
                << ", dir " << page_offset(page_dir_get_nth_slot(page, n_slots - 1));
  ...
  for (;;)
  {
    ...
    {
      ib::error() << "Record " << (rec - page) << " is above rec heap top " << (rec_heap_top - page);
    ...
    if (UNIV_UNLIKELY(rec_get_n_owned_old(rec)))
    {
      ...
      {
        ib::error() << "Wrong owned count " << rec_get_n_owned_old(rec) << ", " << own_count << ", rec "
                    << (rec - page);
    ...
    if (UNIV_UNLIKELY(rec_get_next_offs(rec, FALSE) < FIL_PAGE_DATA || rec_get_next_offs(rec, FALSE) >= UNIV_PAGE_SIZE))
    {
      ib::error() << "Next record offset nonsensical " << rec_get_next_offs(rec, FALSE) << " for rec " << (rec - page);
  ...
  {
    ib::error() << "n slots wrong " << slot_no << ", " << (n_slots - 1);
    goto func_exit;
  }

  if (UNIV_UNLIKELY(page_header_get_field(page, PAGE_N_RECS) + PAGE_HEAP_NO_USER_LOW != count + 1))
  {
    ib::error() << "n recs wrong " << page_header_get_field(page, PAGE_N_RECS) + PAGE_HEAP_NO_USER_LOW << " "
                << (count + 1);
  ...
  while (rec != NULL)
  {
    if (UNIV_UNLIKELY(rec < page + FIL_PAGE_DATA || rec >= page + UNIV_PAGE_SIZE))
    {
    ...
    if (UNIV_UNLIKELY(rec > rec_heap_top))
    {
      ib::error() << "Free list record " << (rec - page) << " is above rec heap top " << (rec_heap_top - page);
  ...
  {
    ib::error() << "N heap is wrong " << page_dir_get_n_heap(page) << ", " << (count + 1);
    ...
    const page_t *page) /*!< in: index page in ROW_FORMAT!=REDUNDANT */
{
  ...
  for (;;)
  {
    if (UNIV_UNLIKELY(rec > rec_heap_top))
    {
      ib::error() << "Record " << page_offset(rec) << " is above rec heap top " << page_offset(rec_heap_top);
    ...
    if (UNIV_UNLIKELY(rec_get_n_owned_new(rec)))
    {
      ...
      {
        ib::error() << "Wrong owned count " << rec_get_n_owned_new(rec) << ", " << own_count << ", rec "
                    << page_offset(rec);
    ...
    if (UNIV_UNLIKELY(rec_get_next_offs(rec, TRUE) < FIL_PAGE_DATA || rec_get_next_offs(rec, TRUE) >= UNIV_PAGE_SIZE))
    {
      ib::error() << "Next record offset nonsensical " << rec_get_next_offs(rec, TRUE) << " for rec "
                  << page_offset(rec);
  ...
  while (rec != NULL)
  {
    ...
    if (UNIV_UNLIKELY(rec > rec_heap_top))
    {
      ib::error() << "Free list record " << page_offset(rec) << " is above rec heap top " << page_offset(rec_heap_top);
                         ...
                         the page record type definition */
{
  ...
  if (UNIV_UNLIKELY((ibool) !!page_is_comp(page) != dict_table_is_comp(index->table)))
  {
      ...
      !page_is_empty(page))
  {
    ...
    {
      ib::error() << "PAGE_MAX_TRX_ID out of bounds: " << max_trx_id << ", " << sys_max_trx_id;
      goto func_exit2;
    }
  }
  ...
  {
    ib::warn() << "Record heap and dir overlap on space " << page_get_space_id(page) << " page "
               << page_get_page_no(page) << " index " << index->name << ", " << page_header_get_ptr(page, PAGE_HEAP_TOP)
  ...
  for (;;)
  {
    ...
    {
      int ret = cmp_rec_rec(rec, old_rec, offsets, old_offsets, index, page_is_spatial_non_leaf(rec, index));

      /* For spatial index, on nonleaf leavel, we
      allow recs to be equal. */
      bool rtr_equal_nodeptrs = (ret == 0 && dict_index_is_spatial(index) && !page_is_leaf(page));

      if (ret <= 0 && !rtr_equal_nodeptrs)
      {
        ib::error() << "Records in wrong order on"
                       " space "
                    << page_get_space_id(page) << " page " << page_get_page_no(page) << " index " << index->name;
    ...
    if (UNIV_UNLIKELY(rec_own_count))
    {
      ...
      {
        ib::error() << "Wrong owned count " << rec_own_count << ", " << own_count;
        goto func_exit;
      }
  ...
  {
    ib::error() << "Summed data size " << data_size << ", returned by func " << page_get_data_size(page);
    goto func_exit;
  }
...
func_exit:
  ...
  {
  func_exit2:
    ib::error() << "Apparent corruption in space " << page_get_space_id(page) << " page " << page_get_page_no(page)
                << " index " << index->name;
  }
    ...
    const ulint *offsets)      /*!< in: offsets for record */
{
  ...
  if (!rec_offs_any_extern(offsets) &&
      ((page_get_data_size(page) - rec_offs_size(offsets) < BTR_CUR_PAGE_COMPRESS_LIMIT(index)) ||
       (mach_read_from_4(page + FIL_PAGE_NEXT) == FIL_NULL && mach_read_from_4(page + FIL_PAGE_PREV) == FIL_NULL) ||
...
void page_warn_strict_checksum(srv_checksum_algorithm_t curr_algo, srv_checksum_algorithm_t page_checksum,
                               const page_id_t &page_id)
{
  ...
  ib::warn() << "innodb_checksum_algorithm is set to \"" << buf_checksum_algorithm_name(curr_algo) << "\""
             ...
             << " innodb_checksum_algorithm to \"" << buf_checksum_algorithm_name(curr_algo_nonstrict)
             << "\" to silently accept such pages or rewrite all pages"
             << " so that they contain \"" << buf_checksum_algorithm_name(curr_algo_nonstrict) << "\" checksum.";
}
