...
struct row_import
{
  row_import() UNIV_NOTHROW : m_table(NULL),
                              m_version(0),
                              m_hostname(NULL),
                              m_table_name(NULL),
                              m_autoinc(0),
  ...
  bool requires_purge(const char *name) const UNIV_NOTHROW { return (get_n_purge_failed(name) > 0); }
...
class RecIterator
{
 public:
  /** Default constructor */
  RecIterator() UNIV_NOTHROW { memset(&m_cur, 0x0, sizeof(m_cur)); }
...
class IndexPurge
{
 public:
  ...
  IndexPurge(trx_t *trx, dict_index_t *index) UNIV_NOTHROW : m_trx(trx), m_index(index), m_n_rows(0)
  {
...
class AbstractCallback : public PageCallback
{
 public:
  ...
  bool is_compressed_table() const UNIV_NOTHROW { return (get_page_size().is_compressed()); }
 ...
 protected:
  ...
  dberr_t set_current_xdes(ulint page_no, const page_t *page) UNIV_NOTHROW
  {
    ...
    if (state != XDES_FREE)
    {
      ...
      /* Trigger OOM */
      DBUG_EXECUTE_IF("ib_import_OOM_13", UT_DELETE_ARRAY(m_xdes); m_xdes = NULL;);
  ...
  bool is_root_page(const page_t *page) const UNIV_NOTHROW
  {
    ...
    return (mach_read_from_4(page + FIL_PAGE_NEXT) == FIL_NULL && mach_read_from_4(page + FIL_PAGE_PREV) == FIL_NULL);
  }
...
@param block contents of the first page in the tablespace file.
@retval DB_SUCCESS or error code. */
dberr_t AbstractCallback::init(os_offset_t file_size, const buf_block_t *block) UNIV_NOTHROW
{
...
struct FetchIndexRootPages : public AbstractCallback
{
  ...
  FetchIndexRootPages(const dict_table_t *table, trx_t *trx) : AbstractCallback(trx), m_table(table) UNIV_NOTHROW {}
  ...
  virtual ulint get_space_id() const UNIV_NOTHROW { return (m_space); }
  ...
  dberr_t check_row_format(ulint ibd_table_flags) UNIV_NOTHROW
  {
    ...
    {
      ib_errf(m_trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, ".ibd file has invalid table flags: %lx",
              ibd_table_flags);
    ...
    if (table_rec_format != ibd_rec_format)
    {
      ib_errf(m_trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH,
              ...
              dict_tf_to_row_format_string(m_table->flags), dict_tf_to_row_format_string(ibd_table_flags));
...
@param block block to convert, it is not from the buffer pool.
@retval DB_SUCCESS or error code. */
dberr_t FetchIndexRootPages::operator()(os_offset_t offset, buf_block_t *block) UNIV_NOTHROW
{
  dberr_t err;
  ...
  if (block->page.id.page_no() * m_page_size.physical() != offset)
  {
    ib::error() << "Page offset doesn't match file offset:"
                   " page offset: "
                << block->page.id.page_no() << ", file offset: " << (offset / m_page_size.physical());
  ...
  else if (fil_page_index_page_check(page) && !is_free(block->page.id.page_no()) && is_root_page(page))
  {
    ...
    if (m_indexes.size() == 1)
    {
      m_table_flags = fsp_flags_to_dict_tf(m_space_flags, page_is_comp(page) ? true : false);
...
dberr_t FetchIndexRootPages::build_row_import(row_import *cfg) const UNIV_NOTHROW
{
  ...
  /* Trigger OOM */
  DBUG_EXECUTE_IF("ib_import_OOM_11", UT_DELETE_ARRAY(cfg->m_indexes); cfg->m_indexes = NULL;);
  ...
  for (Indexes::const_iterator it = m_indexes.begin(); it != end; ++it, ++cfg_index)
  {
    char name[BUFSIZ];
    ...
    /* Trigger OOM */
    DBUG_EXECUTE_IF("ib_import_OOM_12", UT_DELETE_ARRAY(cfg_index->m_name); cfg_index->m_name = NULL;);
...
class PageConverter : public AbstractCallback
{
 public:
  ...
  virtual ulint get_space_id() const UNIV_NOTHROW { return (m_cfg->m_table->space); }
...
#define trigger_corruption() (false)
  ...
  import_page_status_t validate(os_offset_t offset, buf_block_t *page) UNIV_NOTHROW;
  ...
  @return DB_SUCCESS or error code */
  dberr_t adjust_cluster_index_blob_column(rec_t *rec, const ulint *offsets, ulint i) UNIV_NOTHROW;
  ...
  @param offsets column offsets for the record
  @return DB_SUCCESS or error code */
  dberr_t adjust_cluster_index_blob_columns(rec_t *rec, const ulint *offsets) UNIV_NOTHROW;
  ...
  @param offsets column offsets for the record
  @return DB_SUCCESS or error code */
  dberr_t adjust_cluster_index_blob_ref(rec_t *rec, const ulint *offsets) UNIV_NOTHROW;
  ...
  @return DB_SUCCESS or error code. */
  dberr_t adjust_cluster_record(const dict_index_t *index, rec_t *rec, const ulint *offsets, bool deleted) UNIV_NOTHROW;
...
@return DB_SUCCESS or error code. */
dberr_t row_import::match_index_columns(THD *thd, const dict_index_t *index) UNIV_NOTHROW
{
  ...
  if (cfg_index == 0)
  {
    ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, "Index %s not found in tablespace meta-data file.",
            index->name());
  ...
  for (ulint i = 0; i < index->n_fields; ++i, ++field, ++cfg_field)
  {
    ...
    if (cfg_field->prefix_len != field->prefix_len)
    {
      ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH,
              ...
              " %lu",
              index->name(), field->name(), (ulong)field->prefix_len, (ulong)cfg_field->prefix_len);
    ...
    if (cfg_field->fixed_len != field->fixed_len)
    {
      ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH,
              ...
              " doesn't match metadata file value"
              " %lu",
              index->name(), field->name(), (ulong)field->fixed_len, (ulong)cfg_field->fixed_len);
...
dberr_t row_import::match_table_columns(THD *thd) UNIV_NOTHROW
{
  ...
  for (ulint i = 0; i < m_table->n_cols; ++i, ++col)
  {
    ...
    else
    {
      ...
      {
        ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, "Column %s precise type mismatch.", col_name);
        err = DB_ERROR;
      }

      if (cfg_col->mtype != col->mtype)
      {
        ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, "Column %s main type mismatch.", col_name);
        err = DB_ERROR;
      }

      if (cfg_col->len != col->len)
      {
        ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, "Column %s length mismatch.", col_name);
        err = DB_ERROR;
      }

      if (cfg_col->mbminmaxlen != col->mbminmaxlen)
      {
        ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, "Column %s multi-byte len mismatch.", col_name);
        err = DB_ERROR;
      }
      ...
      {
        ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, "Column %s ordering mismatch.", col_name);
        err = DB_ERROR;
      }

      if (cfg_col->max_prefix != col->max_prefix)
      {
        ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, "Column %s max prefix mismatch.", col_name);
        err = DB_ERROR;
      }
    }
  }
...
dberr_t row_import::match_schema(THD *thd) UNIV_NOTHROW
{
  ...
  {
    if (dict_tf_to_row_format_string(m_flags) != dict_tf_to_row_format_string(m_table->flags))
    {
      ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH,
    ...
    else if (DICT_TF_HAS_DATA_DIR(m_flags) != DICT_TF_HAS_DATA_DIR(m_table->flags))
    {
    ...
    {
      ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH, "Table flags don't match");
    }
    return (DB_ERROR);
  }
...
dberr_t row_import::set_root_by_heuristic() UNIV_NOTHROW
{
  ...
  {
    ib::warn() << "Table " << m_table->name << " should have " << UT_LIST_GET_LEN(m_table->indexes)
               << " indexes but"
                  " the tablespace has "
               << m_n_indexes << " indexes";
  }
  ...
  for (dict_index_t *index = UT_LIST_GET_FIRST(m_table->indexes); index != 0; index = UT_LIST_GET_NEXT(indexes, index))
  {
    ...
    else if (i < m_n_indexes)
    {
      ...
      /* Trigger OOM */
      DBUG_EXECUTE_IF("ib_import_OOM_14", UT_DELETE_ARRAY(cfg_index[i].m_name); cfg_index[i].m_name = NULL;);
...
void IndexPurge::open() UNIV_NOTHROW
{
  ...
  btr_pcur_open_at_index_side(true, m_index, BTR_MODIFY_LEAF, &m_pcur, true, 0, &m_mtr);
}
...
PageConverter::PageConverter(row_import *cfg, trx_t *trx)
    : AbstractCallback(trx), m_cfg(cfg), m_page_zip_ptr(0), m_heap(0) UNIV_NOTHROW
{
...
@param i column ordinal value
@return DB_SUCCESS or error code */
dberr_t PageConverter::adjust_cluster_index_blob_column(rec_t *rec, const ulint *offsets, ulint i) UNIV_NOTHROW
{
  ulint len;
  byte *field;
  ...
  if (is_compressed_table())
  {
    ...
    page_zip_write_blob_ptr(m_page_zip_ptr, rec, m_cluster_index, offsets, i, 0);
  }
  else
  {
...
@param offsets column offsets for the record
@return DB_SUCCESS or error code */
dberr_t PageConverter::adjust_cluster_index_blob_columns(rec_t *rec, const ulint *offsets) UNIV_NOTHROW
{
...
@param offsets column offsets for the record
@return DB_SUCCESS or error code */
dberr_t PageConverter::adjust_cluster_index_blob_ref(rec_t *rec, const ulint *offsets) UNIV_NOTHROW
{
...
@param deleted true if row is delete marked
@return DB_SUCCESS or error code. */
dberr_t PageConverter::adjust_cluster_record(const dict_index_t *index, rec_t *rec, const ulint *offsets,
                                             bool deleted) UNIV_NOTHROW
{
  ...
  if ((err = adjust_cluster_index_blob_ref(rec, offsets)) == DB_SUCCESS)
  {
    ...
    row_upd_rec_sys_fields(rec, m_page_zip_ptr, m_cluster_index, m_offsets, m_trx, 0);
  }
...
dberr_t PageConverter::update_records(buf_block_t *block) UNIV_NOTHROW
{
  ...
  while (!m_rec_iter.end())
  {
    ...
    {
      m_offsets = rec_get_offsets(rec, m_index->m_srv_index, m_offsets, ULINT_UNDEFINED, &m_heap);
    }

    if (clust_index)
    {
      dberr_t err = adjust_cluster_record(m_index->m_srv_index, rec, m_offsets, deleted);
...
dberr_t PageConverter::update_index_page(buf_block_t *block) UNIV_NOTHROW
{
  ...
  ut_a(!is_compressed_table() || page_zip_validate(m_page_zip_ptr, page, m_index->m_srv_index));
...
dberr_t PageConverter::update_header(buf_block_t *block) UNIV_NOTHROW
{
  ...
  mach_write_to_4(get_frame(block) + FSP_HEADER_OFFSET + FSP_SPACE_ID, get_space_id());

  /* This is on every page in the tablespace. */
  mach_write_to_4(get_frame(block) + FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID, get_space_id());
...
@param block block read from file
@retval DB_SUCCESS or error code */
dberr_t PageConverter::update_page(buf_block_t *block, ulint &page_type) UNIV_NOTHROW
{
  dberr_t err = DB_SUCCESS;
...
@param page page read from file.
@return status */
PageConverter::import_page_status_t PageConverter::validate(os_offset_t offset, buf_block_t *block) UNIV_NOTHROW
{
  ...
  if (buf_page_is_corrupted(false, page, get_page_size(), fsp_is_checksum_disabled(block->page.id.space())) ||
      (page_get_page_no(page) != offset / m_page_size.physical() && page_get_page_no(page) != 0))
  {
...
@retval DB_SUCCESS or error code. */
dberr_t PageConverter::operator()(os_offset_t offset, buf_block_t *block) UNIV_NOTHROW
{
  ulint page_type;
  dberr_t err = DB_SUCCESS;
  ...
  switch (validate(offset, block))
  {
    case IMPORT_PAGE_STATUS_OK:
      ...
      {
        buf_flush_init_for_writing(!is_compressed_table() ? block : NULL,
      ...
      else
      {
        ...
        buf_flush_update_zip_checksum(get_frame(block), get_page_size().physical(), m_current_lsn);
      }
    ...
    case IMPORT_PAGE_STATUS_CORRUPTED:
      ...
      ib::warn() << "Page " << (offset / m_page_size.physical()) << " at offset " << offset
...
static MY_ATTRIBUTE((nonnull)) void row_import_discard_changes(
    /*=======================*/
    ...
    dberr_t err)              /*!< in: error code */
{
  ...
  ib::info() << "Discarding tablespace of table " << prebuilt->table->name << ": " << ut_strerr(err);
  ...
  for (dict_index_t *index = UT_LIST_GET_FIRST(table->indexes); index != 0; index = UT_LIST_GET_NEXT(indexes, index))
  {
    index->page = FIL_NULL;
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_cleanup(
    /*===============*/
    row_prebuilt_t *prebuilt, /*!< in/out: prebuilt from handler */
    trx_t *trx,               /*!< in/out: transaction for import */
    dberr_t err)              /*!< in: error code */
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_error(
    /*=============*/
    row_prebuilt_t *prebuilt, /*!< in/out: prebuilt from handler */
    trx_t *trx,               /*!< in/out: transaction for import */
    dberr_t err)              /*!< in: error code */
...
/*****************************************************************/ /**
 ...
 @return error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_adjust_root_pages_of_secondary_indexes(
    /*==============================================*/
    ...
    const row_import &cfg)    /*!< Import context */
{
  ...
  while ((index = dict_table_get_next_index(index)) != NULL)
  {
    ...
    if (!(index->type & DICT_CORRUPT) && index->space != FIL_NULL && index->page != FIL_NULL)
    {
...
/*****************************************************************/ /**
 ...
 @return error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_set_sys_max_row_id(
    /*==========================*/
    row_prebuilt_t *prebuilt,  /*!< in/out: prebuilt from
                               handler */
    const dict_table_t *table) /*!< in: table to import */
{
  ...
  if (!page_rec_is_infimum(rec))
  {
    ...
    field = rec_get_nth_field(rec, offsets, dict_index_get_sys_col_pos(index, DATA_ROW_ID), &len);
  ...
  if (err != DB_SUCCESS)
  {
    ib_errf(prebuilt->trx->mysql_thd, IB_LOG_LEVEL_WARN, ER_INNODB_INDEX_CORRUPT,
...
static dberr_t row_import_cfg_read_string(
    ...
    ulint max_len) /*!< in: maximum length of the output
                   buffer in bytes */
{
  DBUG_EXECUTE_IF("ib_import_string_read_error", errno = EINVAL; return (DB_IO_ERROR););
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code. */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_cfg_read_index_fields(
    /*=============================*/
    ...
    row_import *cfg)    /*!< in/out: meta-data read */
{
  ...
  /* Trigger OOM */
  DBUG_EXECUTE_IF("ib_import_OOM_4", UT_DELETE_ARRAY(index->m_fields); index->m_fields = NULL;);
  ...
  for (ulint i = 0; i < n_fields; ++i, ++field)
  {
    ...
    /* Trigger EOF */
    DBUG_EXECUTE_IF("ib_import_io_read_error_1", (void)fseek(file, 0L, SEEK_END););

    if (fread(row, 1, sizeof(row), file) != sizeof(row))
    {
      ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while reading index fields.");
    ...
    if (err != DB_SUCCESS)
    {
      ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while parsing table name.");
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS or error code. */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_read_index_data(
    /*=======================*/
    ...
    row_import *cfg) /*!< in/out: meta-data read */
{
  ...
  /* Trigger OOM */
  DBUG_EXECUTE_IF("ib_import_OOM_6", UT_DELETE_ARRAY(cfg->m_indexes); cfg->m_indexes = NULL;);
  ...
  {
    /* Trigger EOF */
    DBUG_EXECUTE_IF("ib_import_io_read_error_2", (void)fseek(file, 0L, SEEK_END););

    /* Read the index data. */
    size_t n_bytes = fread(row, 1, sizeof(row), file);

    /* Trigger EOF */
    DBUG_EXECUTE_IF("ib_import_io_read_error", (void)fseek(file, 0L, SEEK_END););
    ...
    if (n_bytes != sizeof(row))
    {
      ...
      ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), msg);
    ...
    /* Trigger OOM */
    DBUG_EXECUTE_IF("ib_import_OOM_7", UT_DELETE_ARRAY(cfg_index->m_name); cfg_index->m_name = NULL;);
    ...
    if (err != DB_SUCCESS)
    {
      ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while parsing index name.");
    ...
    row_import *cfg) /*!< in/out: meta-data read */
{
  ...
  /* Trigger EOF */
  DBUG_EXECUTE_IF("ib_import_io_read_error_3", (void)fseek(file, 0L, SEEK_END););

  /* Read the number of indexes. */
  if (fread(row, 1, sizeof(row), file) != sizeof(row))
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while reading number of indexes.");
  ...
  else if (cfg->m_n_indexes > 1024)
  {
    ...
    ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, "Number of indexes in meta-data file is too high: %lu",
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_read_columns(
    /*====================*/
    FILE *file,      /*!< in: file to write to */
    THD *thd,        /*!< in/out: session */
    row_import *cfg) /*!< in/out: meta-data read */
{
  ...
  /* Trigger OOM */
  DBUG_EXECUTE_IF("ib_import_OOM_8", UT_DELETE_ARRAY(cfg->m_cols); cfg->m_cols = NULL;);
  ...
  /* Trigger OOM */
  DBUG_EXECUTE_IF("ib_import_OOM_9", UT_DELETE_ARRAY(cfg->m_col_names); cfg->m_col_names = NULL;);
  ...
  for (ulint i = 0; i < cfg->m_n_cols; ++i, ++col)
  {
    ...
    /* Trigger EOF */
    DBUG_EXECUTE_IF("ib_import_io_read_error_4", (void)fseek(file, 0L, SEEK_END););

    if (fread(row, 1, sizeof(row), file) != sizeof(row))
    {
      ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno),
    ...
    if (len == 0 || len > 128)
    {
      ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, "Column name length %lu, is invalid", (ulong)len);
    ...
    /* Trigger OOM */
    DBUG_EXECUTE_IF("ib_import_OOM_10", UT_DELETE_ARRAY(cfg->m_col_names[i]); cfg->m_col_names[i] = NULL;);
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS or error code. */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_read_v1(
    /*===============*/
    FILE *file,      /*!< in: File to read from */
    THD *thd,        /*!< in: session */
    row_import *cfg) /*!< out: meta data */
{
  ...
  /* Trigger EOF */
  DBUG_EXECUTE_IF("ib_import_io_read_error_5", (void)fseek(file, 0L, SEEK_END););
  ...
  /* Trigger OOM */
  DBUG_EXECUTE_IF("ib_import_OOM_1", UT_DELETE_ARRAY(cfg->m_hostname); cfg->m_hostname = NULL;);
  ...
  if (err != DB_SUCCESS)
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while parsing export hostname.");

    return (err);
  }

  /* Trigger EOF */
  DBUG_EXECUTE_IF("ib_import_io_read_error_6", (void)fseek(file, 0L, SEEK_END););
  ...
  /* Trigger OOM */
  DBUG_EXECUTE_IF("ib_import_OOM_2", UT_DELETE_ARRAY(cfg->m_table_name); cfg->m_table_name = NULL;);
  ...
  ib::info() << "Importing tablespace for table '" << cfg->m_table_name << "' that was exported from host '"
             << cfg->m_hostname << "'";

  byte row[sizeof(ib_uint32_t) * 3];

  /* Trigger EOF */
  DBUG_EXECUTE_IF("ib_import_io_read_error_7", (void)fseek(file, 0L, SEEK_END););

  /* Read the autoinc value. */
  if (fread(row, 1, sizeof(ib_uint64_t), file) != sizeof(ib_uint64_t))
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while reading autoinc value.");
  ...
  /* Trigger EOF */
  DBUG_EXECUTE_IF("ib_import_io_read_error_8", (void)fseek(file, 0L, SEEK_END););

  /* Read the tablespace page size. */
  if (fread(row, 1, sizeof(row), file) != sizeof(row))
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while reading meta-data header.");
  ...
  else if ((err = row_import_read_columns(file, thd, cfg)) != DB_SUCCESS)
  {
    return (err);
  }
...
Read the contents of the <tablespace>.cfg file.
@return DB_SUCCESS or error code. */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_read_meta_data(
    /*======================*/
    ...
    row_import &cfg)     /*!< out: contents of the .cfg file */
{
  ...
  /* Trigger EOF */
  DBUG_EXECUTE_IF("ib_import_io_read_error_9", (void)fseek(file, 0L, SEEK_END););

  if (fread(&row, 1, sizeof(row), file) != sizeof(row))
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while reading meta-data version.");
...
@return DB_SUCCESS or error code. */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_import_read_cfg(
    /*================*/
    dict_table_t *table, /*!< in: table */
    THD *thd,            /*!< in: session */
    row_import &cfg)     /*!< out: contents of the .cfg file */
{
  ...
  if (file == NULL)
  {
    ...
    ib_senderrf(thd, IB_LOG_LEVEL_WARN, ER_IO_READ_ERROR, errno, strerror(errno), msg);
...
@return DB_SUCCESS or error code. */
static dberr_t row_import_read_encryption_data(dict_table_t *table, FILE *file, THD *thd)
{
  byte row[sizeof(ib_uint32_t)];
  ulint key_size;
  ...
  if (fread(&row, 1, sizeof(row), file) != sizeof(row))
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while reading encrypton key size.");
  ...
  {
    ib_senderrf(thd, IB_LOG_LEVEL_WARN, ER_IO_WRITE_ERROR, errno, strerror(errno), "while reading tranfer key.");

    return (DB_IO_ERROR);
  }

  /* Read the encrypted key. */
  if (fread(encryption_key, 1, ENCRYPTION_KEY_LEN, file) != ENCRYPTION_KEY_LEN)
  {
    ib_senderrf(thd, IB_LOG_LEVEL_WARN, ER_IO_WRITE_ERROR, errno, strerror(errno), "while reading encryption key.");
  ...
  {
    ib_senderrf(thd, IB_LOG_LEVEL_WARN, ER_IO_WRITE_ERROR, errno, strerror(errno), "while reading encryption iv.");

    return (DB_IO_ERROR);
  }

  table->encryption_key = static_cast<byte *>(mem_heap_alloc(table->heap, ENCRYPTION_KEY_LEN));

  table->encryption_iv = static_cast<byte *>(mem_heap_alloc(table->heap, ENCRYPTION_KEY_LEN));
  /* Decrypt tablespace key and iv. */
  elen = my_aes_decrypt(encryption_key, ENCRYPTION_KEY_LEN, table->encryption_key, transfer_key, ENCRYPTION_KEY_LEN,
                        my_aes_256_ecb, NULL, false);

  if (elen == MY_AES_BAD_DATA)
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while decrypt encryption key.");

    return (DB_IO_ERROR);
  }

  elen = my_aes_decrypt(encryption_iv, ENCRYPTION_KEY_LEN, table->encryption_iv, transfer_key, ENCRYPTION_KEY_LEN,
                        my_aes_256_ecb, NULL, false);

  if (elen == MY_AES_BAD_DATA)
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_IO_READ_ERROR, errno, strerror(errno), "while decrypt encryption iv.");
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
dberr_t row_import_update_index_root(
    /*=========================*/
...

{
  ...
  for (index = dict_table_get_first_index(table); index != 0; index = dict_table_get_next_index(index))
  {
    ...
    mach_write_to_4(reinterpret_cast<byte *>(&space), reset ? FIL_NULL : index->space);
...
static ibool row_import_set_discarded(
    /*=====================*/
    ...
    row_prebuilt_t *prebuilt) /*!< in: prebuilt struct in MySQL */
{
  ...
  else if (cfg.m_missing)
  {
    ...
    err = fil_tablespace_iterate(table, IO_BUFFER_SIZE(cfg.m_page_size.physical(), cfg.m_page_size.physical()),
                                 fetchIndexRootPages);
    ...
    if (err == DB_SUCCESS && FSP_FLAGS_HAS_DATA_DIR(space_flags) != DICT_TF_HAS_DATA_DIR(table->flags))
    {
      ib_errf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLE_SCHEMA_MISMATCH,
              "Table location flags do not match. "
              "The source table %s a DATA DIRECTORY "
  ...
  err =
      fil_tablespace_iterate(table, IO_BUFFER_SIZE(cfg.m_page_size.physical(), cfg.m_page_size.physical()), converter);

  DBUG_EXECUTE_IF("ib_import_reset_space_and_lsn_failure", err = DB_TOO_MANY_CONCURRENT_TRXS;);
  ...
  if (err != DB_SUCCESS)
  {
    ...
    ib_errf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_INTERNAL_ERROR, "Cannot reset LSNs in table %s : %s", table_name,
            ut_strerr(err));
  ...
  err = fil_ibd_open(true, true, FIL_TYPE_IMPORT, table->space, fsp_flags, table->name.m_name, filepath);

  DBUG_EXECUTE_IF("ib_import_open_tablespace_failure", err = DB_TABLESPACE_NOT_FOUND;);

  if (err != DB_SUCCESS)
  {
    row_mysql_unlock_data_dictionary(trx);

    ib_senderrf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_FILE_NOT_FOUND, filepath, err, ut_strerr(err));
  ...
  if (dict_table_is_encrypted(table))
  {
    err = fil_set_encryption(table->space, Encryption::AES, table->encryption_key, table->encryption_iv);
  }
  ...
  DBUG_EXECUTE_IF("ib_import_page_corrupt", row_index_t *i_index = cfg.get_index(index->name);
  ...
  err = row_import_adjust_root_pages_of_secondary_indexes(prebuilt, trx, table, cfg);
  ...
  buf_LRU_flush_or_remove_pages(prebuilt->table->space, BUF_REMOVE_FLUSH_WRITE, trx);
