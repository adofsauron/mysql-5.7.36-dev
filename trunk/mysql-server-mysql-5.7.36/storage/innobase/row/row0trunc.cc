...
class IndexIterator
{
 public:
  ...
  explicit IndexIterator(dict_index_t *index) : m_index(index)
  { /* Do nothing */
  }
  ...
  dberr_t search(dtuple_t &key, bool noredo)
  {
    ...
    btr_pcur_open_on_user_rec(m_index, &key, PAGE_CUR_GE, BTR_MODIFY_LEAF, &m_pcur, &m_mtr);
  ...
  dberr_t for_each(Callback &callback)
  {
    ...
    for (;;)
    {
      if (!btr_pcur_is_on_user_rec(&m_pcur) || !callback.match(&m_mtr, &m_pcur))
      {
...
class SysIndexIterator
{
 public:
  ...
  dberr_t for_each(Callback &callback) const
  {
    ...
    dtuple_t *tuple = dtuple_create_from_mem(buf, sizeof(buf), 1, 0);
    dfield_t *dfield = dtuple_get_nth_field(tuple, 0);

    dfield_set_data(dfield, callback.table_id(), sizeof(*callback.table_id()));
...
class Callback
{
 public:
  ...
  Callback(table_id_t table_id, bool noredo) : m_id(), m_noredo(noredo)
  {
  ...
  const table_id_t *table_id() const { return (&m_id); }

  /**
  @return	return if logging needs to be turned off. */
  bool get_logging_status() const { return (m_noredo); }
...
class TruncateLogger : public Callback
{
 public:
  ...
  TruncateLogger(dict_table_t *table, ulint flags, table_id_t new_table_id)
  ...
  dberr_t init()
  {
    ...
    ulint log_file_name_buf_sz = strlen(srv_log_group_home_dir) + 22 + 22 + 1 /* NUL */
                                 + strlen(TruncateLogger::s_log_prefix) + strlen(TruncateLogger::s_log_ext);
    ...
    if (m_log_file_name[log_file_name_len - 1] != OS_PATH_SEPARATOR)
    {
      m_log_file_name[log_file_name_len] = OS_PATH_SEPARATOR;
      log_file_name_len = strlen(m_log_file_name);
    }

    ut_snprintf(m_log_file_name + log_file_name_len, log_file_name_buf_sz - log_file_name_len, "%s%lu_%lu_%s",
                TruncateLogger::s_log_prefix, (ulong)m_table->space, (ulong)m_table->id, TruncateLogger::s_log_ext);
  ...
  ~TruncateLogger()
  {
    ...
    {
      bool exist;
      os_file_delete_if_exists(innodb_log_file_key, m_log_file_name, &exist);
  ...
  bool debug() const
  {
    ...
    return (UT_LIST_GET_LEN(m_table->indexes) == m_truncate.indexes());
  }
  ...
  dberr_t log() const
  {
    ...
    bool ret;
    pfs_os_file_t handle = os_file_create(innodb_log_file_key, m_log_file_name, OS_FILE_CREATE, OS_FILE_NORMAL,
    ...
    do
    {
      ...
      err = m_truncate.write(log_buf + 4, log_buf + sz - 4, m_table->space, m_table->name.m_name, m_flags,
                             m_table->flags, lsn);

      DBUG_EXECUTE_IF("ib_err_trunc_oom_logging", err = DB_FAIL;);
      ...
      if (err != DB_SUCCESS)
      {
        ...
        DBUG_EXECUTE_IF("ib_err_trunc_oom_logging", ut_free(buf); buf = 0;);
        if (buf == 0)
        {
          os_file_close(handle);
          return (DB_OUT_OF_MEMORY);
        }
        log_buf = static_cast<byte *>(ut_align(buf, UNIV_PAGE_SIZE));
      }
    ...
    io_err = os_file_write(request, m_log_file_name, handle, log_buf, 0, sz);

    if (io_err != DB_SUCCESS)
    {
      ib::error() << "IO: Failed to write the file size to '" << m_log_file_name << "'";
  ...
  void done()
  {
    ...
    bool ret;
    pfs_os_file_t handle = os_file_create_simple_no_error_handling(innodb_log_file_key, m_log_file_name, OS_FILE_OPEN,
                                                                   OS_FILE_READ_WRITE, srv_read_only_mode, &ret);
    DBUG_EXECUTE_IF("ib_err_trunc_writing_magic_number", os_file_close(handle); ret = false;);
    if (!ret)
    {
      ib::error() << "Failed to open truncate log file " << m_log_file_name
                  << "."
    ...
    err = os_file_write(request, m_log_file_name, handle, buffer, 0, sizeof(buffer));

    if (err != DB_SUCCESS)
    {
      ib::error() << "IO: Failed to write the magic number to '" << m_log_file_name << "'";
    }

    DBUG_EXECUTE_IF("ib_trunc_crash_after_updating_magic_no", DBUG_SUICIDE(););
    os_file_flush(handle);
    os_file_close(handle);
    DBUG_EXECUTE_IF("ib_trunc_crash_after_logging_complete", log_buffer_flush_to_disk(); os_thread_sleep(1000000);
                    DBUG_SUICIDE(););
    os_file_delete(innodb_log_file_key, m_log_file_name);
  }
 ...
 private:
  ...
  {
    for (const dict_index_t *index = UT_LIST_GET_FIRST(m_table->indexes); index != NULL;
...
@return DB_SUCCESS or error code. */
dberr_t TruncateLogParser::scan(const char *dir_path, trunc_log_files_t &log_files)
{
  os_file_dir_t dir;
  ...
  while (fil_file_readdir_next_file(&err, dir_path, dir, &fileinfo) == 0)
  {
    ...
    if (fileinfo.type == OS_FILE_TYPE_FILE && nm_len > ext_len + prefix_len &&
        (0 == strncmp(fileinfo.name + nm_len - ext_len, TruncateLogger::s_log_ext, ext_len)) &&
        (0 == strncmp(fileinfo.name, TruncateLogger::s_log_prefix, prefix_len)))
    {
      if (fileinfo.size == 0)
      {
        /* Truncate log not written. Remove the file. */
        os_file_delete(innodb_log_file_key, fileinfo.name);
        continue;
      }
      ...
      if (log_file_name[log_file_name_len - 1] != OS_PATH_SEPARATOR)
      {
        log_file_name[log_file_name_len] = OS_PATH_SEPARATOR;
...
@return DB_SUCCESS or error code. */
dberr_t TruncateLogParser::parse(const char *log_file_name)
{
  dberr_t err = DB_SUCCESS;
  ...
  was completed. */
  bool ret;
  pfs_os_file_t handle = os_file_create_simple(innodb_log_file_key, log_file_name, OS_FILE_OPEN, OS_FILE_READ_ONLY,
                                               srv_read_only_mode, &ret);
  if (!ret)
  {
    ib::error() << "Error opening truncate log file: " << log_file_name;
    return (DB_IO_ERROR);
  }
...
@param dir_path		look for log directory in following path
@return DB_SUCCESS or error code. */
dberr_t TruncateLogParser::scan_and_parse(const char *dir_path)
{
  dberr_t err;
  ...
  if (err == DB_SUCCESS)
  {
    for (ulint i = 0; i < log_files.size() && err == DB_SUCCESS; i++)
    {
  ...
  for (trunc_log_files_t::const_iterator it = log_files.begin(); it != end; ++it)
  {
    if (*it != NULL)
    {
      UT_DELETE_ARRAY(*it);
...
class DropIndex : public Callback
{
 public:
  ...
  DropIndex(dict_table_t *table, bool noredo) : Callback(table->id, noredo), m_table(table)
  { /* No op */
  }
...
class CreateIndex : public Callback
{
 public:
  ...
  @param[in,out]	table	Table to truncate
  @param[in]	noredo	whether to disable redo logging */
  CreateIndex(dict_table_t *table, bool noredo) : Callback(table->id, noredo), m_table(table)
  { /* No op */
  }
...
class TableLocator : public Callback
{
 public:
  ...
  explicit TableLocator(table_id_t table_id) : Callback(table_id, false), m_table_found()
  { /* No op */
  }

  /**
  @return true if table is found */
  bool is_table_found() const { return (m_table_found); }
...
@param pcur	persistent cursor used for reading
@return DB_SUCCESS or error code */
dberr_t TruncateLogger::operator()(mtr_t *mtr, btr_pcur_t *pcur)
{
  ulint len;
  const byte *field;
  rec_t *rec = btr_pcur_get_rec(pcur);
  ...
  if (fsp_flags_is_compressed(m_flags))
  {
    ...
    {
      ib::warn() << "Index id " << index.m_id << " not found";
    }
  }
...
@param pcur	persistent cursor used for reading
@return DB_SUCCESS or error code */
dberr_t DropIndex::operator()(mtr_t *mtr, btr_pcur_t *pcur) const
{
  rec_t *rec = btr_pcur_get_rec(pcur);
...
#ifdef UNIV_DEBUG
  {
    ...
    field = rec_get_nth_field_old(btr_pcur_get_rec(pcur), DICT_FLD__SYS_INDEXES__TYPE, &len);
    ut_ad(len == 4);
    ...
    {
      /* Clustered index */
      DBUG_EXECUTE_IF("ib_trunc_crash_on_drop_of_clust_index", log_buffer_flush_to_disk(); os_thread_sleep(2000000);
                      DBUG_SUICIDE(););
    }
    else if (index_type & DICT_UNIQUE)
    {
      /* Unique index */
      DBUG_EXECUTE_IF("ib_trunc_crash_on_drop_of_uniq_index", log_buffer_flush_to_disk(); os_thread_sleep(2000000);
                      DBUG_SUICIDE(););
    }
    else if (index_type == 0)
    {
      /* Secondary index */
      DBUG_EXECUTE_IF("ib_trunc_crash_on_drop_of_sec_index", log_buffer_flush_to_disk(); os_thread_sleep(2000000);
                      DBUG_SUICIDE(););
    }
  }
#endif /* UNIV_DEBUG */

  DBUG_EXECUTE_IF("ib_err_trunc_drop_index", freed = false;);
  ...
  else
  {
    ...
    DBUG_EXECUTE_IF("ib_err_trunc_drop_index", found = false;);
...
@param pcur	persistent cursor used for reading
@return DB_SUCCESS or error code */
dberr_t CreateIndex::operator()(mtr_t *mtr, btr_pcur_t *pcur) const
{
  ulint root_page_no;
...
#ifdef UNIV_DEBUG
  {
    ...
    {
      /* Clustered index */
      DBUG_EXECUTE_IF("ib_trunc_crash_on_create_of_clust_index", log_buffer_flush_to_disk(); os_thread_sleep(2000000);
                      DBUG_SUICIDE(););
    }
    else if (index_type & DICT_UNIQUE)
    {
      /* Unique index */
      DBUG_EXECUTE_IF("ib_trunc_crash_on_create_of_uniq_index", log_buffer_flush_to_disk(); os_thread_sleep(2000000);
                      DBUG_SUICIDE(););
    }
    else if (index_type == 0)
    {
      /* Secondary index */
      DBUG_EXECUTE_IF("ib_trunc_crash_on_create_of_sec_index", log_buffer_flush_to_disk(); os_thread_sleep(2000000);
                      DBUG_SUICIDE(););
    }
  }
#endif /* UNIV_DEBUG */

  DBUG_EXECUTE_IF("ib_err_trunc_create_index", root_page_no = FIL_NULL;);

  if (root_page_no != FIL_NULL)
  {
    rec_t *rec = btr_pcur_get_rec(pcur);

    page_rec_write_field(rec, DICT_FLD__SYS_INDEXES__PAGE_NO, root_page_no, mtr);
  ...
  else
  {
    ...
    DBUG_EXECUTE_IF("ib_err_trunc_create_index", found = false;);
...
@param pcur	persistent cursor used for reading
@return DB_SUCCESS */
dberr_t TableLocator::operator()(mtr_t *mtr, btr_pcur_t *pcur)
{
  m_table_found = true;
...
static void row_truncate_rollback(dict_table_t *table, trx_t *trx, table_id_t new_id, bool has_internal_doc_id,
                                  bool no_redo, bool corrupted, bool unlock_index)
{
  if (unlock_index)
  {
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_truncate_complete(dict_table_t *table, trx_t *trx, ulint fsp_flags, TruncateLogger *&logger, dberr_t err)
{
  ...
  {
    DBUG_EXECUTE_IF("ib_trunc_crash_before_log_removal", log_buffer_flush_to_disk(); os_thread_sleep(500000);
                    DBUG_SUICIDE(););
  ...
  if (is_file_per_table && !dict_table_is_temporary(table) && fsp_flags != ULINT_UNDEFINED)
  {
    /* This function will reset back the stop_new_ops
    and is_being_truncated so that fil-ops can re-start. */
    dberr_t err2 = truncate_t::truncate(table->space, table->data_dir_path, table->name.m_name, fsp_flags, false);
  ...
  DBUG_EXECUTE_IF("ib_trunc_crash_after_truncate_done", DBUG_SUICIDE(););
...
@param trx		transaction covering the truncate
@return DB_SUCCESS or error code. */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_truncate_fts(dict_table_t *table, table_id_t new_id, trx_t *trx)
{
  ...
  creation. */
  if (DICT_TF_HAS_DATA_DIR(table->flags) && table->data_dir_path == NULL)
  {
  ...
  if (DICT_TF_HAS_SHARED_SPACE(table->flags) &&
      (table->tablespace() == NULL || dict_table_has_temp_general_tablespace_name(table->tablespace())))
  {
    dict_get_and_save_space_name(table, true);
    ut_ad(table->tablespace() != NULL);
    ut_ad(!dict_table_has_temp_general_tablespace_name(table->tablespace()));
  }
  ...
  err = fts_create_common_tables(trx, &fts_table, table->name.m_name, TRUE);

  for (ulint i = 0; i < ib_vector_size(table->fts->indexes) && err == DB_SUCCESS; i++)
  {
    dict_index_t *fts_index;

    fts_index = static_cast<dict_index_t *>(ib_vector_getp(table->fts->indexes, i));

    err = fts_create_index_tables_low(trx, fts_index, table->name.m_name, new_id);
  }

  DBUG_EXECUTE_IF("ib_err_trunc_during_fts_trunc", err = DB_ERROR;);
  ...
  if (err != DB_SUCCESS)
  {
    ...
    ib::error() << "Unable to truncate FTS index for table " << table->name;
  }
  else
  {
    ut_ad(trx_is_started(trx));
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_truncate_update_table_id(table_id_t old_table_id, table_id_t new_table_id, ibool reserve_dict_mutex, trx_t *trx)
{
...
static MY_ATTRIBUTE((warn_unused_result)) table_id_t row_truncate_get_trunc_table_id(const truncate_t &truncate)
{
  TableLocator tableLocator(truncate.old_table_id());

  SysIndexIterator().for_each(tableLocator);

  return (tableLocator.is_table_found() ? truncate.old_table_id() : truncate.new_table_id());
}
...
@return error code or DB_SUCCESS */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_truncate_update_sys_tables_during_fix_up(const truncate_t &truncate, table_id_t new_table_id,
                                                 ibool reserve_dict_mutex, bool mark_index_corrupted)
{
  ...
  err = truncate.update_root_page_no(trx, table_id, reserve_dict_mutex, mark_index_corrupted);
  ...
  err = row_truncate_update_table_id(table_id, new_table_id, reserve_dict_mutex, trx);
  ...
  if (err == DB_SUCCESS)
  {
    ...
    dict_table_t *old_table = dict_table_open_on_id(table_id, true, DICT_TABLE_OP_NORMAL);
    ...
    dict_table_t *table = dict_table_open_on_id(new_table_id, true, DICT_TABLE_OP_NORMAL);
    ut_ad(table->id == new_table_id);

    bool has_internal_doc_id = dict_table_has_fts_index(table) || DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_HAS_DOC_ID);
...
@return	error code or DB_SUCCESS */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_truncate_update_system_tables(dict_table_t *table, table_id_t new_id, bool has_internal_doc_id, bool no_redo,
                                      trx_t *trx)
{
  dberr_t err = DB_SUCCESS;
  ...
  DBUG_EXECUTE_IF("ib_err_trunc_during_sys_table_update", err = DB_ERROR;);

  if (err != DB_SUCCESS)
  {
    row_truncate_rollback(table, trx, new_id, has_internal_doc_id, no_redo, true, false);

    ib::error() << "Unable to assign a new identifier to table " << table->name
                << " after truncating it. Marked the"
  ...
  else
  {
    ...
    if (has_internal_doc_id)
    {
      ...
      DBUG_EXECUTE_IF("ib_truncate_crash_while_fts_cleanup", DBUG_SUICIDE(););

      ut_ad(trx_is_started(trx));
    }

    DBUG_EXECUTE_IF("ib_trunc_crash_after_fts_drop", log_buffer_flush_to_disk(); os_thread_sleep(2000000);
                    DBUG_SUICIDE(););
    ...
    {
      DBUG_EXECUTE_IF("ib_trunc_sleep_before_fts_cache_clear", os_thread_sleep(10000000););
...
@return	error code or DB_SUCCESS */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_truncate_prepare(dict_table_t *table, ulint *flags)
{
...
@return DB_SUCCESS or error code */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_truncate_foreign_key_checks(const dict_table_t *table, const trx_t *trx)
{
  ...
  if (!srv_read_only_mode && it != table->referenced_set.end() && trx->check_foreigns)
  {
...
@param table		table being truncated
@return DB_SUCCESS or error code */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_truncate_sanity_checks(const dict_table_t *table)
{
...
dberr_t row_truncate_table_for_mysql(dict_table_t *table, trx_t *trx)
{
  ...
  */
  ...
  {
    ib::error() << "Cannot truncate table " << table->name
  ...
  if (!dict_table_is_temporary(table))
  {
    ...
    DBUG_EXECUTE_IF("ib_err_trunc_assigning_undo_log", err = DB_ERROR;);
    if (err != DB_SUCCESS)
    {
      trx_rollback_to_savepoint(trx, NULL);
      return (row_truncate_complete(table, trx, fsp_flags, logger, err));
  ...
  if (!dict_table_is_temporary(table))
  {
    if (is_file_per_table)
    {
      ...
      DBUG_EXECUTE_IF("ib_err_trunc_preparing_for_truncate", err = DB_ERROR;);

      if (err != DB_SUCCESS)
      {
        row_truncate_rollback(table, trx, new_id, has_internal_doc_id, no_redo, false, true);
        return (row_truncate_complete(table, trx, fsp_flags, logger, err));
      }
    }
    else
    {
      ...
      DBUG_EXECUTE_IF("ib_err_trunc_preparing_for_truncate", fsp_flags = ULINT_UNDEFINED;);

      if (fsp_flags == ULINT_UNDEFINED)
      {
        row_truncate_rollback(table, trx, new_id, has_internal_doc_id, no_redo, false, true);
        return (row_truncate_complete(table, trx, fsp_flags, logger, DB_ERROR));
      }
    }

    logger = UT_NEW_NOKEY(TruncateLogger(table, fsp_flags, new_id));
  ...
  DBUG_EXECUTE_IF("ib_trunc_crash_after_redo_log_write_complete", log_buffer_flush_to_disk(); os_thread_sleep(3000000);
                  DBUG_SUICIDE(););
  ...
  if (!dict_table_is_temporary(table))
  {
    ...
    if (err != DB_SUCCESS)
    {
      row_truncate_rollback(table, trx, new_id, has_internal_doc_id, no_redo, true, true);
  ...
  else
  {
         ...
         index = UT_LIST_GET_NEXT(indexes, index))
    {
      ...
      DBUG_EXECUTE_IF("ib_trunc_crash_during_drop_index_temp_table", log_buffer_flush_to_disk();
  ...
  if (is_file_per_table && !dict_table_is_temporary(table) && fsp_flags != ULINT_UNDEFINED)
  {
    ...
    ulint space_size = table->indexes.count + FIL_IBD_FILE_INITIAL_SIZE + 1;
  ...
  DBUG_EXECUTE_IF("ib_trunc_crash_with_intermediate_log_checkpoint", log_buffer_flush_to_disk();
                  os_thread_sleep(2000000); log_checkpoint(TRUE, TRUE); os_thread_sleep(1000000); DBUG_SUICIDE(););

  DBUG_EXECUTE_IF("ib_trunc_crash_drop_reinit_done_create_to_start", log_buffer_flush_to_disk();
                  os_thread_sleep(2000000); DBUG_SUICIDE(););
  ...
  else
  {
    ...
    err = row_truncate_update_system_tables(table, new_id, has_internal_doc_id, no_redo, trx);
  ...
  DBUG_EXECUTE_IF("ib_trunc_crash_on_updating_dict_sys_info", log_buffer_flush_to_disk(); os_thread_sleep(2000000);
                  DBUG_SUICIDE(););
...
@return	error code or DB_SUCCESS */
dberr_t truncate_t::fixup_tables_in_system_tablespace()
{
  dberr_t err = DB_SUCCESS;
  ...
  for (tables_t::iterator it = s_tables.begin(); it != s_tables.end();)
  {
    if ((*it)->m_space_id == TRX_SYS_SPACE)
    {
      ...
      err = fil_recreate_table((*it)->m_space_id, (*it)->m_format_flags, (*it)->m_tablespace_flags, (*it)->m_tablename,
                               **it);
      ...
      err = row_truncate_update_sys_tables_during_fix_up(**it, new_id, TRUE, (err == DB_SUCCESS) ? false : true);
      ...
      os_file_delete(innodb_log_file_key, (*it)->m_log_file_name);
      UT_DELETE(*it);
...
@return	error code or DB_SUCCESS */
dberr_t truncate_t::fixup_tables_in_non_system_tablespace()
{
  dberr_t err = DB_SUCCESS;
  ...
  for (tables_t::iterator it = s_tables.begin(); it != end; ++it)
  {
    ...
    if (fsp_is_file_per_table((*it)->m_space_id, (*it)->m_tablespace_flags))
    {
      ...
      if (!fil_space_get((*it)->m_space_id))
      {
        ...
        fil_create_directory_for_tablename((*it)->m_tablename);

        err = fil_ibd_create((*it)->m_space_id, (*it)->m_tablename, (*it)->m_dir_path, (*it)->m_tablespace_flags,
      ...
      err = fil_recreate_tablespace((*it)->m_space_id, (*it)->m_format_flags, (*it)->m_tablespace_flags,
                                    (*it)->m_tablename, **it, log_get_lsn());
    }
    else
    {
  ...
  {
    os_file_delete(innodb_log_file_key, s_tables[i]->m_log_file_name);
...
truncate_t::truncate_t(table_id_t old_table_id, table_id_t new_table_id, const char *dir_path)
    : m_space_id(),
...
truncate_t::truncate_t(const char *log_file_name)
    : m_space_id(),
      m_old_table_id(),
      m_new_table_id(),
      m_dir_path(),
...
size_t truncate_t::indexes() const { return (m_indexes.size()); }
...
dberr_t truncate_t::update_root_page_no(trx_t *trx, table_id_t table_id, ibool reserve_dict_mutex,
                                        bool mark_index_corrupted) const
{
  ...
  for (indexes_t::const_iterator it = m_indexes.begin(); it != end; ++it)
  {
    pars_info_t *info = pars_info_create();

    pars_info_add_int4_literal(info, "page_no", it->m_new_root_page_no);

    pars_info_add_ull_literal(info, "table_id", table_id);

    pars_info_add_ull_literal(info, "index_id", (mark_index_corrupted ? -1 : it->m_id));
...
bool truncate_t::is_tablespace_truncated(ulint space_id)
{
  tables_t::iterator end = s_tables.end();
...
bool truncate_t::was_tablespace_truncated(ulint space_id)
{
...
lsn_t truncate_t::get_truncated_tablespace_init_lsn(ulint space_id)
{
...
dberr_t truncate_t::parse(byte *start_ptr, const byte *end_ptr)
{
  ...
  if (fsp_flags_is_compressed(m_tablespace_flags))
  {
    ...
    for (ulint i = 0; i < m_indexes.size(); ++i)
    {
      ...
      index.m_fields.insert(end, start_ptr, &(start_ptr)[len]);
...
byte *truncate_t::parse_redo_entry(byte *start_ptr, const byte *end_ptr, ulint space_id)
{
  lsn_t lsn;
  ...
  {
    truncated_tables_t::iterator it = s_truncated_tables.find(space_id);

    if (it == s_truncated_tables.end())
    {
      s_truncated_tables.insert(std::pair<ulint, lsn_t>(space_id, lsn));
    }
    else
    {
      it->second = lsn;
...
dberr_t truncate_t::index_t::set(const dict_index_t *index)
{
  ...
  ulint len = page_zip_fields_encode(m_n_fields, index, m_trx_id_pos, encoded_buf);
...
ulint truncate_t::create_index(const char *table_name, ulint space_id, const page_size_t &page_size, ulint index_type,
                               index_id_t index_id, const btr_create_t &btr_redo_create_info, mtr_t *mtr) const
{
  ulint root_page_no = btr_create(index_type, space_id, page_size, index_id, NULL, &btr_redo_create_info, mtr);

  if (root_page_no == FIL_NULL)
  {
    ib::info() << "innodb_force_recovery was set to " << srv_force_recovery
               << ". Continuing crash recovery"
                  " even though we failed to create index "
               << index_id << " for compressed table '" << table_name
               << "' with"
                  " tablespace "
               << space_id << " during recovery";
  }
...
bool truncate_t::is_index_modified_since_logged(ulint space_id, ulint root_page_no) const
{
  mtr_t mtr;
  bool found;
  ...
  buf_block_t *block = buf_page_get_gen(page_id_t(space_id, root_page_no), page_size, RW_X_LATCH, NULL,
...
void truncate_t::drop_indexes(ulint space_id) const
{
  mtr_t mtr;
  ...
  for (indexes_t::const_iterator it = m_indexes.begin(); it != end; ++it)
  {
    ...
    if (is_index_modified_since_logged(space_id, root_page_no))
    {
    ...
    if (root_page_no != FIL_NULL)
    {
      ...
      btr_free_if_exists(root_page_id, page_size, it->m_id, &mtr);
    }
...
@return DB_SUCCESS or error code. */
dberr_t truncate_t::create_indexes(const char *table_name, ulint space_id, const page_size_t &page_size, ulint flags,
                                   ulint format_flags)
{
  mtr_t mtr;
  ...
  for (indexes_t::iterator it = m_indexes.begin(); it != end; ++it)
  {
    btr_create_t btr_redo_create_info(fsp_flags_is_compressed(flags) ? &it->m_fields[0] : NULL);
    ...
    if (fsp_flags_is_compressed(flags))
    {
      ...
      btr_redo_create_info.field_len = it->m_fields.size() - 1;
      btr_redo_create_info.trx_id_pos = it->m_trx_id_pos;
    }

    root_page_no = create_index(table_name, space_id, page_size, it->m_type, it->m_id, btr_redo_create_info, &mtr);
...
dberr_t truncate_t::write(byte *start_ptr, byte *end_ptr, ulint space_id, const char *tablename, ulint flags,
                          ulint format_flags, lsn_t lsn) const
{
  ...
  DBUG_EXECUTE_IF("ib_trunc_crash_while_writing_redo_log", DBUG_SUICIDE(););
  ...
  if (fsp_flags_is_compressed(flags))
  {
    for (ulint i = 0; i < m_indexes.size(); ++i)
    {
      ...
      mach_write_to_2(start_ptr, m_indexes[i].m_n_fields);
      start_ptr += 2;
