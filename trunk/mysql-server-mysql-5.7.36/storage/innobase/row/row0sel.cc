    ...
    dict_table_t *table)     /*!< in: table */
{
  ...
  if (!memcmp(clust_field + clust_len - BTR_EXTERN_FIELD_REF_SIZE, field_ref_zero, BTR_EXTERN_FIELD_REF_SIZE))
  {
  ...
  len = btr_copy_externally_stored_field_prefix(buf, prefix_len, dict_tf_get_page_size(table->flags), clust_field,
                                                clust_len);
  ...
  len = dtype_get_at_most_n_mbchars(prtype, mbminmaxlen, prefix_len, len, (const char *)buf);
...
static ibool row_sel_sec_rec_is_for_clust_rec(const rec_t *sec_rec, dict_index_t *sec_index, const rec_t *clust_rec,
                                              dict_index_t *clust_index, que_thr_t *thr)
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (is_virtual)
    {
      ...
      row = row_build(ROW_COPY_POINTERS, clust_index, clust_rec, clust_offs, NULL, NULL, NULL, &ext, heap);

      vfield = innobase_get_computed_value(row, v_col, clust_index, &heap, NULL, NULL, thr_get_trx(thr)->mysql_thd,
                                           thr->prebuilt->m_mysql_table, NULL, NULL, NULL);
    ...
    if (ifield->prefix_len > 0 && len != UNIV_SQL_NULL && sec_len != UNIV_SQL_NULL && !is_virtual)
    {
      ...
      len = dtype_get_at_most_n_mbchars(col->prtype, col->mbminmaxlen, ifield->prefix_len, len, (char *)clust_field);

      if (rec_offs_nth_extern(clust_offs, clust_pos) && len < sec_len)
      {
        if (!row_sel_sec_rec_is_for_blob(col->mtype, col->prtype, col->mbminmaxlen, clust_field, clust_len, sec_field,
                                         sec_len, ifield->prefix_len, clust_index->table))
        {
          goto inequal;
        }
    ...
    if (dict_index_is_spatial(sec_index) && i == 0)
    {
      ...
      if (rec_offs_nth_extern(clust_offs, clust_pos))
      {
        dptr = btr_copy_externally_stored_field(&clust_len, dptr, dict_tf_get_page_size(sec_index->table->flags), len,
                                                heap);
      }

      rtree_mbr_from_wkb(dptr + GEO_DATA_HEADER_SIZE, static_cast<uint>(clust_len - GEO_DATA_HEADER_SIZE), SPDIMS,
    ...
    {
      if (0 != cmp_data_data(col->mtype, col->prtype, clust_field, len, sec_field, sec_len))
      {
      inequal:
        is_equal = FALSE;
        goto func_exit;
    ...
    sel_node_t *node) /*!< in: select node */
{
  ...
  for (exp = node->select_list; var != 0; var = static_cast<sym_node_t *>(que_node_get_next(var)))
  {
    ut_ad(exp);
    ...
    sel_node_t *node) /*!< in: select node */
{
  ...
  for (func_node = static_cast<func_node_t *>(node->select_list); func_node != 0;
  ...
  while (column)
  {
    ...
    if (field_no != ULINT_UNDEFINED)
    {
      if (UNIV_UNLIKELY(rec_offs_nth_extern(offsets, field_no)))
      {
        ...
        data = btr_rec_copy_externally_stored_field(rec, offsets, dict_table_page_size(index->table), field_no, &len,
                                                    heap);
    ...
    plan_t *plan) /*!< in: plan node for a table */
{
  ...
  for (column = UT_LIST_GET_FIRST(plan->columns); column != 0; column = UT_LIST_GET_NEXT(col_var_list, column))
  {
    if (!column->copy_val)
    {
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_sel_build_prev_vers(
    /*====================*/
                  ...
                  comparisons evaluated */
{
  ...
  for (cond = UT_LIST_GET_FIRST(plan->end_conds); cond != 0; cond = UT_LIST_GET_NEXT(cond_list, cond))
  {
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_sel_get_clust_rec(
    /*==================*/
                      ...
                      access the clustered index */
{
  ...
  *out_rec = NULL;
  ...
  if (!node->read_view)
  {
    ...
    err = lock_clust_rec_read_check_and_lock(0, btr_pcur_get_block(&plan->clust_pcur), clust_rec, index, offsets,
  ...
  else
  {
    ...
    {
      err = row_sel_build_prev_vers(node->read_view, index, clust_rec, &offsets, &heap, &plan->old_vers_heap, &old_vers,
                                    mtr);
    ...
    if ((old_vers || rec_get_deleted_flag(rec, dict_table_is_comp(plan->table))) &&
        !row_sel_sec_rec_is_for_clust_rec(rec, plan->index, clust_rec, index, thr))
    {
      goto func_exit;
    }
  }
...
retry:
  ...
  ut_ad(rw_lock_own(&(match->block.lock), RW_LOCK_X) || rw_lock_own(&(match->block.lock), RW_LOCK_S));
  ut_ad(page_is_leaf(buf_block_get_frame(cur_block)));

  err = lock_sec_rec_read_check_and_lock(0, cur_block, rec, index, my_offsets, static_cast<lock_mode>(mode), type, thr);
  ...
  re_scan:
    ...
    if (row_mysql_handle_errors(&err, trx, thr, NULL))
    {
      ...
      ulint page_no = page_get_page_no(btr_pcur_get_page(pcur));
      page_id_t page_id(dict_index_get_space(index), page_no);

      cur_block = buf_page_get_gen(page_id, dict_table_page_size(index->table), RW_X_LATCH, NULL, BUF_GET, __FILE__,
                                   __LINE__, mtr);
    }
    else
    {
    ...
    rtr_cur_search_with_match(cur_block, index, pcur->btr_cur.rtr_info->search_tuple,
                              pcur->btr_cur.rtr_info->search_mode, &pcur->btr_cur.page_cur, pcur->btr_cur.rtr_info);
    ...
    my_offsets = offsets_;
    my_offsets = rec_get_offsets(rec, index, my_offsets, ULINT_UNDEFINED, &heap);
  ...
  for (rtr_rec_vector::iterator it = match_rec->begin(); it != end; ++it)
  {
    ...
    my_offsets = rec_get_offsets(rtr_rec->r_rec, index, my_offsets, ULINT_UNDEFINED, &heap);

    err = lock_sec_rec_read_check_and_lock(0, &match->block, rtr_rec->r_rec, index, my_offsets,
    ...
    mtr_t *mtr)           /*!< in: mtr */
{
  ...
  if (dict_index_is_clust(index))
  {
    err = lock_clust_rec_read_check_and_lock(0, block, rec, index, offsets, static_cast<lock_mode>(mode), type, thr);
  }
  else
  {
    if (dict_index_is_spatial(index))
    {
      ...
      err = sel_set_rtr_rec_lock(pcur, rec, index, offsets, mode, type, thr, mtr);
    }
    else
    {
      err = lock_sec_rec_read_check_and_lock(0, block, rec, index, offsets, static_cast<lock_mode>(mode), type, thr);
    }
  }
    ...
    mtr_t *mtr) /*!< in: mtr */
{
  ...
  else
  {
    ...
    btr_pcur_open_at_index_side(plan->asc, index, BTR_SEARCH_LEAF, &(plan->pcur), false, 0, mtr);
  }
    ...
    mtr_t *mtr) /*!< in: mtr */
{
  ...
  else if (!srv_read_only_mode && !lock_sec_rec_cons_read_sees(rec, index, node->read_view))
  {
    ret = SEL_RETRY;
    goto func_exit;
  }
...
table_loop:
  ...
  if (consistent_read && plan->unique_search && !plan->pcur_is_open && !plan->must_get_clust && !plan->table->big_rows)
  {
    ...
    else if (rw_lock_get_writer(btr_get_search_latch(index)) == RW_LOCK_X_WAIT)
    {
    ...
    found_flag = row_sel_try_search_shortcut(node, plan, search_latch_locked, &mtr);
...
rec_loop:
  ...
  if (!node->asc && cursor_just_opened && !page_rec_is_supremum(rec))
  {
    ...
    if (!consistent_read)
    {
      ...
      err = sel_set_rec_lock(&plan->pcur, next_rec, index, offsets, node->row_lock_mode, lock_type, thr, &mtr);
...
skip_lock:
  ...
  if (!consistent_read)
  {
    ...
    if (srv_locks_unsafe_for_binlog || trx->isolation_level <= TRX_ISO_READ_COMMITTED || dict_index_is_spatial(index))
    {
    ...
    err = sel_set_rec_lock(&plan->pcur, rec, index, offsets, node->row_lock_mode, lock_type, thr, &mtr);
  ...
  if (consistent_read)
  {
    ...
    if (dict_index_is_clust(index))
    {
      if (!lock_clust_rec_cons_read_sees(rec, index, offsets, node->read_view))
      {
        err = row_sel_build_prev_vers(node->read_view, index, rec, &offsets, &heap, &plan->old_vers_heap, &old_vers,
                                      &mtr);
    ...
    void *user_arg) /*!< in:  not used */
{
  ...
  for (exp = node->select_list; exp != 0; exp = que_node_get_next(exp), i++)
  {
    ...
    {
      ut_print_buf(stderr, dfield_get_data(dfield), dfield_get_len(dfield));
      putc('\n', stderr);
    }
    else
    {
  ...
  while (key_ptr < key_end)
  {
    ...
    if (DATA_LARGE_MTYPE(type) || DATA_GEOMETRY_MTYPE(type))
    {
      ...
      else
      {
        ...
        data_field_len = data_offset + 2 + (type == DATA_POINT ? DATA_POINT_LEN : field->prefix_len);
    ...
    if ((dtype_get_mysql_type(dfield_get_type(dfield)) == DATA_MYSQL_TRUE_VARCHAR) && (type != DATA_INT))
    {
    ...
    {
      buf = row_mysql_store_col_in_innobase_format(dfield, buf, FALSE, /* MySQL key value format col */
    ...
    if (UNIV_UNLIKELY(key_ptr > key_end))
    {
      ...
      ib::warn() << "Using a partial-field key prefix in"
                    " search, index "
                 << index->name << " of table " << index->table->name << ". Last data field length " << data_field_len
                 << " bytes, key ptr now"
                    " exceeds key end by "
                               ...
                               (index_rec, index) */
{
  ...
  if (UNIV_UNLIKELY(len != DATA_ROW_ID_LEN))
  {
    ib::error() << "Row id field is wrong length " << len
                << " in"
                   " index "
                << index->name << " of table " << index->table->name << ", Field number "
                << dict_index_get_sys_col_pos(index, DATA_ROW_ID) << ", record:";
...
@param[in]	sec_field	secondary index field no if the secondary index
                                ...
                                range comparison. */
void row_sel_field_store_in_mysql_format_func(byte *dest, const mysql_row_templ_t *templ,
#ifdef UNIV_DEBUG
...
)
{
  byte *ptr;
#ifdef UNIV_DEBUG
  const dict_field_t *field = templ->is_virtual ? NULL : dict_index_get_nth_field(index, field_no);
  ...
  switch (templ->type)
  {
    ...
    case DATA_MYSQL:
      ...
      ut_ad(templ->is_virtual || clust_templ_for_sec || len * templ->mbmaxlen >= templ->mysql_col_len ||
            (field_no == templ->icp_rec_field_no && field->prefix_len > 0));
    ...
    case DATA_DECIMAL:
      ...
      ut_ad((templ->is_virtual && !field) || (field && field->prefix_len ? field->prefix_len == len
...
#define row_sel_store_mysql_field(m, p, r, i, o, f, t, s) row_sel_store_mysql_field_func(m, p, r, i, o, f, t, s)
#else /* UNIV_DEBUG */
/** Convert a field from Innobase format to MySQL format. */
#define row_sel_store_mysql_field(m, p, r, i, o, f, t, s) row_sel_store_mysql_field_func(m, p, r, o, f, t, s)
#endif /* UNIV_DEBUG */
...
static MY_ATTRIBUTE((warn_unused_result)) ibool
    row_sel_store_mysql_field_func(byte *mysql_rec, row_prebuilt_t *prebuilt, const rec_t *rec,
#ifdef UNIV_DEBUG
                                   ...
                                   ulint sec_field_no)
{
  ...
  ut_ad(clust_templ_for_sec || field_no == templ->clust_rec_field_no || field_no == templ->rec_field_no ||
        field_no == templ->icp_rec_field_no);
  ut_ad(rec_offs_validate(rec, clust_templ_for_sec ? prebuilt->index : index, offsets));
  ...
  if (UNIV_UNLIKELY(rec_offs_nth_extern(offsets, field_no)))
  {
    ...
    data =
        btr_rec_copy_externally_stored_field(rec, offsets, dict_table_page_size(prebuilt->table), field_no, &len, heap);
    ...
    if (UNIV_UNLIKELY(!data))
    {
      ...
      ut_a((!prebuilt->idx_cond && prebuilt->m_mysql_handler->end_range != NULL) ||
    ...
    row_sel_field_store_in_mysql_format(mysql_rec + templ->mysql_col_offset, templ, index, field_no, data, len,
                                        ULINT_UNDEFINED);
  ...
  else
  {
    ...
    if (len == UNIV_SQL_NULL)
    {
      ...
      UNIV_MEM_ASSERT_RW(prebuilt->default_rec + templ->mysql_col_offset, templ->mysql_col_len);
      mysql_rec[templ->mysql_null_byte_offset] |= (byte)templ->mysql_null_bit_mask;
      memcpy(mysql_rec + templ->mysql_col_offset, (const byte *)prebuilt->default_rec + templ->mysql_col_offset,
             templ->mysql_col_len);
      DBUG_RETURN(TRUE);
    }
    ...
    if (DATA_LARGE_MTYPE(templ->type) || DATA_GEOMETRY_MTYPE(templ->type))
    {
      ...
      {
        prebuilt->blob_heap = mem_heap_create(UNIV_PAGE_SIZE);
        DBUG_PRINT("anna", ("blob_heap allocated: %p", prebuilt->blob_heap));
      }
...
static MY_ATTRIBUTE((warn_unused_result)) ibool
    row_sel_store_mysql_rec(byte *mysql_rec, row_prebuilt_t *prebuilt, const rec_t *rec, const dtuple_t *vrow,
                            ibool rec_clust, const dict_index_t *index, const ulint *offsets, bool clust_templ_for_sec)
{
  ulint i;
  ...
  if (clust_templ_for_sec)
  {
    ...
    for (i = 0; i < dict_index_get_n_fields(prebuilt->index); i++)
    {
      ...
      const dict_field_t *field = dict_index_get_nth_field(index, sec_field);
  ...
  for (i = 0; i < prebuilt->n_template; i++)
  {
    ...
    if (templ->is_virtual && dict_index_is_clust(index))
    {
      ...
      if (!dict_index_has_virtual(prebuilt->index) || (!prebuilt->read_just_key && !prebuilt->m_read_virtual_key) ||
          !rec_clust)
      {
        continue;
      }
      ...
      if (dfield_get_type(dfield)->mtype == DATA_MISSING)
      {
        ...
        ut_ad(dict_index_get_nth_col_or_prefix_pos(prebuilt->index, col->v_pos, false, true) == ULINT_UNDEFINED);
      ...
      {
        row_sel_field_store_in_mysql_format(mysql_rec + templ->mysql_col_offset, templ, index,
    ...
    ut_ad(dict_index_get_nth_field(index, field_no)->prefix_len == 0);

    if (clust_templ_for_sec)
    {
      std::vector<const dict_col_t *>::iterator it;
      const dict_field_t *field = dict_index_get_nth_field(index, field_no);
      const dict_col_t *col = dict_field_get_col(field);
    ...
    if (!row_sel_store_mysql_field(mysql_rec, prebuilt, rec, index, offsets, field_no, templ, sec_field_no))
    {
      DBUG_RETURN(FALSE);
    }
  }
  ...
  {
    if ((dict_index_is_clust(index) && !clust_templ_for_sec) || prebuilt->fts_doc_id_in_read_set)
    {
      prebuilt->fts_doc_id = fts_get_doc_id_from_rec(prebuilt->table, rec, index, NULL);
    }
  }
...
/*********************************************************************/ /**
 Builds a previous version of a clustered index record for a consistent read
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_sel_build_prev_vers_for_mysql(
    /*==============================*/
    ReadView *read_view,       /*!< in: read view */
    dict_index_t *clust_index, /*!< in: clustered index */
    row_prebuilt_t *prebuilt,  /*!< in: prebuilt struct */
...
/*********************************************************************/ /**
 ...
 interface.
 @return DB_SUCCESS, DB_SUCCESS_LOCKED_REC, or error code */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_sel_get_clust_rec_for_mysql(
    /*============================*/
                              ...
                              access the clustered index */
{
  ...
  *out_rec = NULL;
      ...
      btr_pcur_get_low_match(prebuilt->clust_pcur) < dict_index_get_n_unique(clust_index))
  {
    ...
    if (dict_index_is_spatial(sec_index) && btr_cur->rtr_info->matches &&
        (page_align(rec) == btr_cur->rtr_info->matches->block.frame || rec != btr_pcur_get_rec(prebuilt->pcur)))
    {
#ifdef UNIV_DEBUG
      ...
      ulint page_no = page_get_page_no(btr_pcur_get_page(prebuilt->pcur));

      page_id_t page_id(dict_index_get_space(sec_index), page_no);

      buf_block_t *block = buf_page_get_gen(page_id, dict_table_page_size(sec_index->table), RW_NO_LATCH, NULL, BUF_GET,
                                            __FILE__, __LINE__, mtr);

      mem_heap_t *heap = mem_heap_create(256);
      dtuple_t *tuple =
          dict_index_build_data_tuple(sec_index, const_cast<rec_t *>(rec), dict_index_get_n_fields(sec_index), heap);
      ;
      page_cur_t page_cursor;

      ulint low_match = page_cur_search(block, sec_index, tuple, PAGE_CUR_LE, &page_cursor);
    ...
    else if (!rec_get_deleted_flag(rec, dict_table_is_comp(sec_index->table)) ||
  ...
  *offsets = rec_get_offsets(clust_rec, clust_index, *offsets, ULINT_UNDEFINED, offset_heap);
  ...
  if (prebuilt->select_lock_type != LOCK_NONE)
  {
    ...
    err = lock_clust_rec_read_check_and_lock(0, btr_pcur_get_block(prebuilt->clust_pcur), clust_rec, clust_index,
                                             *offsets, static_cast<lock_mode>(prebuilt->select_lock_type),
  ...
  else
  {
    ...
    if (clust_rec &&
        (old_vers || trx->isolation_level <= TRX_ISO_READ_UNCOMMITTED || dict_index_is_spatial(sec_index) ||
         rec_get_deleted_flag(rec, dict_table_is_comp(sec_index->table))) &&
        !row_sel_sec_rec_is_for_clust_rec(rec, sec_index, clust_rec, clust_index, thr))
    {
      clust_rec = NULL;
    }
    ...
    const mysql_row_templ_t *templ) /*!< in: column template */
{
  ...
  if (templ->mysql_type == DATA_MYSQL_TRUE_VARCHAR && (templ->type != DATA_INT))
  {
    ...
    row_prebuilt_t *prebuilt) /*!< in: prebuilt struct */
{
  ...
  else if (prebuilt->mysql_prefix_len > 63)
  {
    ...
    for (i = 0; i < prebuilt->n_template; i++)
    {
      ...
      /* Skip virtual columns */
      if (templ->is_virtual && !(dict_index_has_virtual(prebuilt->index) && prebuilt->read_just_key))
      {
        continue;
      }
    ...
    row_prebuilt_t *prebuilt) /*!< in/out: prebuilt struct */
{
  ...
  UNIV_MEM_INVALID(prebuilt->fetch_cache[prebuilt->n_fetch_cached], prebuilt->mysql_row_len);
    ...
    const ulint *offsets)     /*!< in: rec_get_offsets() */
{
  ...
  for (i = 0; i < prebuilt->idx_cond_n_cols; i++)
  {
    ...
    if (!row_sel_store_mysql_field(mysql_rec, prebuilt, rec, prebuilt->index, offsets, templ->icp_rec_field_no, templ,
                                   ULINT_UNDEFINED))
    {
      return (ICP_NO_MATCH);
    }
  ...
  switch (result)
  {
    case ICP_MATCH:
      ...
      if (!prebuilt->need_to_access_clustered || dict_index_is_clust(prebuilt->index))
      {
        if (!row_sel_store_mysql_rec(mysql_rec, prebuilt, rec, NULL, FALSE, prebuilt->index, offsets, false))
        {
...
static bool row_search_end_range_check(byte *mysql_rec, const rec_t *rec, row_prebuilt_t *prebuilt,
                                       bool clust_templ_for_sec, const ulint *offsets)
{
  ...
  if (clust_templ_for_sec && handler->m_virt_gcol_in_end_range)
  {
    ...
    for (ulint i = 0; i < prebuilt->n_template; ++i)
    {
      ...
      if (templ->is_virtual && templ->icp_rec_field_no != ULINT_UNDEFINED &&
          !row_sel_store_mysql_field(mysql_rec, prebuilt, rec, prebuilt->index, offsets, templ->icp_rec_field_no, templ,
                                     false))
      {
        return (false);
      }
...
dberr_t row_search_no_mvcc(byte *buf, page_cur_mode_t mode, row_prebuilt_t *prebuilt, ulint match_mode, ulint direction)
{
  ...
  if (direction != 0)
  {
    if (index->last_sel_cur->invalid)
    {
      ...
      tuple = dict_index_build_data_tuple(index, pcur->old_rec, pcur->old_n_fields, heap);

      btr_pcur_open_with_no_init(index, tuple, pcur->search_mode, BTR_SEARCH_LEAF, pcur, 0, mtr);
  ...
  for (/* No op */; err == DB_SUCCESS; err = row_search_traverse(moves_up, match_mode, pcur, mtr))
  {
    ...
    {
      err = row_sel_get_clust_rec_for_mysql(prebuilt, index, rec, thr, &clust_rec, &offsets, &heap, NULL, mtr);
    ...
    captured while SELECT statement started execution. */
    {
      ...
      ulint len;
      ulint trx_id_off = rec_get_nth_field_offs(offsets, clust_index->n_uniq, &len);
    ...
    {
      row_sel_store_row_id_to_prebuilt(prebuilt, result_rec, clust_index, offsets);
    }
    ...
    {
      const rec_t *ret_rec = (index != clust_index && prebuilt->need_to_access_clustered) ? result_rec : rec;

      offsets = rec_get_offsets(ret_rec, index, offsets, ULINT_UNDEFINED, &heap);

      memcpy(buf + 4, ret_rec - rec_offs_extra_size(offsets), rec_offs_size(offsets));

      mach_write_to_4(buf, rec_offs_extra_size(offsets) + 4);
    }
    else if (!row_sel_store_mysql_rec(buf, prebuilt, result_rec, NULL, TRUE, clust_index, offsets, false))
    {
      err = DB_ERROR;
      break;
    }
    ...
    pcur->old_rec =
        dict_index_copy_rec_order_prefix(index, rec, &pcur->old_n_fields, &pcur->old_rec_buf, &pcur->buf_size);
...
static inline rec_t *row_search_debug_copy_rec_order_prefix(const btr_pcur_t *pcur, const dict_index_t *index,
                                                            const rec_t *rec, ulint *n_fields, byte **buf,
                                                            ulint *buf_size)
{
  if (btr_pcur_get_rec(pcur) == rec)
  {
    return dict_index_copy_rec_order_prefix(index, rec, n_fields, buf, buf_size);
  }
  else
  {
    return NULL;
...
@param[in]	direction	0 or ROW_SEL_NEXT or ROW_SEL_PREV;
                                ...
                                cursor 'direction' should be 0.
@return DB_SUCCESS or error code */
dberr_t row_search_mvcc(byte *buf, page_cur_mode_t mode, row_prebuilt_t *prebuilt, ulint match_mode, ulint direction)
{
  ...
  if (trx->has_search_latch
      ...
      && rw_lock_get_writer(btr_get_search_latch(index)) != RW_LOCK_NOT_LOCKED
  ...
  if (UNIV_UNLIKELY(direction == 0) && unique_search && btr_search_enabled && dict_index_is_clust(index) &&
      !prebuilt->templ_contains_blob && !prebuilt->used_in_HANDLER && (prebuilt->mysql_row_len < UNIV_PAGE_SIZE / 8) &&
      !prebuilt->innodb_api)
  {
    mode = PAGE_CUR_GE;

    if (trx->mysql_n_tables_locked == 0 && !prebuilt->ins_sel_stmt && prebuilt->select_lock_type == LOCK_NONE &&
        trx->isolation_level > TRX_ISO_READ_UNCOMMITTED && MVCC::is_view_active(trx->read_view))
    {
      ...
      switch (row_sel_try_search_shortcut_for_mysql(&rec, prebuilt, &offsets, &heap, &mtr))
      {
        case SEL_FOUND:
          ...
          if (!row_sel_store_mysql_rec(buf, prebuilt, rec, NULL, FALSE, index, offsets, false))
          {
  ...
  ut_ad(prebuilt->sql_stat_start || prebuilt->select_lock_type != LOCK_NONE || MVCC::is_view_active(trx->read_view) ||
        srv_read_only_mode);
  ...
  if (!prebuilt->sql_stat_start)
  {
    ...
    if (!MVCC::is_view_active(trx->read_view) && !srv_read_only_mode && prebuilt->select_lock_type == LOCK_NONE)
    {
  ...
  if (UNIV_LIKELY(direction != 0))
  {
    ...
    ibool need_to_process = sel_restore_position_for_mysql(&same_user_rec, BTR_SEARCH_LEAF, pcur, moves_up, &mtr);
    ...
    else if (UNIV_LIKELY(prebuilt->row_read_type != ROW_READ_DID_SEMI_CONSISTENT))
    {
  ...
  else if (dtuple_get_n_fields(search_tuple) > 0)
  {
    ...
    if (dict_index_is_spatial(index))
    {
      ...
      {
        prebuilt->rtr_info = rtr_create_rtr_info(need_pred_lock, true, btr_pcur_get_btr_cur(pcur), index);
      ...
      {
        rtr_info_reinit_in_cursor(btr_pcur_get_btr_cur(pcur), index, need_pred_lock);
        prebuilt->rtr_info->search_tuple = search_tuple;
        prebuilt->rtr_info->search_mode = mode;
      }
    }
    ...
    if (!moves_up && !page_rec_is_supremum(rec) && set_also_gap_locks &&
        ...
        prebuilt->select_lock_type != LOCK_NONE && !dict_index_is_spatial(index))
    {
      ...
      offsets = rec_get_offsets(next_rec, index, offsets, ULINT_UNDEFINED, &heap);
      err = sel_set_rec_lock(pcur, next_rec, index, offsets, prebuilt->select_lock_type, LOCK_GAP, thr, &mtr);
...
rec_loop:
  ...
  if (page_rec_is_supremum(rec))
  {
    ...
    if (prev_rec != NULL && !prebuilt->innodb_api && prebuilt->m_mysql_handler->end_range != NULL &&
        prebuilt->idx_cond == NULL && end_loop >= 100)
    {
      bool clust_templ_for_sec = index != clust_index && prebuilt->need_to_access_clustered;
      dict_index_t *key_index = prebuilt->index;

      if (end_range_cache == NULL)
      {
        end_range_cache = static_cast<byte *>(ut_malloc_nokey(prebuilt->mysql_row_len));
      }
      ...
      offsets = rec_get_offsets(prev_rec, prebuilt->index, offsets, ULINT_UNDEFINED, &heap);

      if (row_sel_store_mysql_rec(end_range_cache, prebuilt, prev_rec, prev_vrow, clust_templ_for_sec, key_index,
                                  offsets, clust_templ_for_sec))
      {
        if (row_search_end_range_check(end_range_cache, prev_rec, prebuilt, clust_templ_for_sec, offsets))
        {
        ...
        prebuilt->select_lock_type != LOCK_NONE && !dict_index_is_spatial(index))
    {
      ...
      offsets = rec_get_offsets(rec, index, offsets, ULINT_UNDEFINED, &heap);
      err = sel_set_rec_lock(pcur, rec, index, offsets, prebuilt->select_lock_type, LOCK_ORDINARY, thr, &mtr);
  ...
  wrong_offs:
    ...
    {
      ib::error() << "Rec address " << static_cast<const void *>(rec) << ", buf block fix count "
                  << btr_cur_get_block(btr_pcur_get_btr_cur(pcur))->page.buf_fix_count;

      ib::error() << "Index corruption: rec offs " << page_offset(rec) << " next offs " << next_offs << ", page no "
                  << page_get_page_no(page_align(rec)) << ", index " << index->name << " of table "
                  << index->table->name
    ...
    else
    {
      ...
      ib::info() << "Index corruption: rec offs " << page_offset(rec) << " next offs " << next_offs << ", page no "
                 << page_get_page_no(page_align(rec)) << ", index " << index->name << " of table " << index->table->name
  ...
  prev_rec = rec;
  ut_d(prev_rec_debug = row_search_debug_copy_rec_order_prefix(pcur, index, prev_rec, &prev_rec_debug_n_fields,
  ...
  if (match_mode == ROW_SEL_EXACT)
  {
    ...
    if (0 != cmp_dtuple_rec(search_tuple, rec, offsets))
    {
          ...
          prebuilt->select_lock_type != LOCK_NONE && !dict_index_is_spatial(index))
      {
        ...
        err = sel_set_rec_lock(pcur, rec, index, offsets, prebuilt->select_lock_type, LOCK_GAP, thr, &mtr);
  ...
  if (prebuilt->select_lock_type != LOCK_NONE)
  {
    ...
    if (!set_also_gap_locks || srv_locks_unsafe_for_binlog || trx->isolation_level <= TRX_ISO_READ_COMMITTED ||
        (unique_search && !rec_get_deleted_flag(rec, comp)) || dict_index_is_spatial(index))
    {
      goto no_gap_lock;
    }
    else
    ...
    err = sel_set_rec_lock(pcur, rec, index, offsets, prebuilt->select_lock_type, lock_type, thr, &mtr);
    ...
    switch (err)
    {
      ...
      case DB_LOCK_WAIT:
        ...
        row_sel_build_committed_vers_for_mysql(clust_index, prebuilt, rec, &offsets, &heap, &old_vers,
  ...
  else
  {
    ...
    else
    {
      ...
      if (!srv_read_only_mode && !lock_sec_rec_cons_read_sees(rec, index, trx->read_view))
      {
...
locks_ok:
  ...
  requires_clust_rec:
    ...
    err = row_sel_get_clust_rec_for_mysql(prebuilt, index, rec, thr, &clust_rec, &offsets, &heap,
                                          need_vrow ? &vrow : NULL, &mtr);
    switch (err)
    {
      case DB_SUCCESS:
        ...
        {
          /* The record did not exist in the read view */
          ut_ad(prebuilt->select_lock_type == LOCK_NONE || dict_index_is_spatial(index));
    ...
    if (prebuilt->idx_cond)
    {
      ...
      if (!row_sel_store_mysql_rec(buf, prebuilt, result_rec, vrow, TRUE, clust_index, offsets, false))
      {
        goto next_rec;
      }
    }
  ...
  if ((match_mode == ROW_SEL_EXACT || prebuilt->n_rows_fetched >= MYSQL_FETCH_CACHE_THRESHOLD) &&
      prebuilt->select_lock_type == LOCK_NONE && !prebuilt->m_no_prefetch && !prebuilt->templ_contains_blob &&
      !prebuilt->templ_contains_fixed_point && !prebuilt->clust_index_was_generated && !prebuilt->used_in_HANDLER &&
      !prebuilt->innodb_api && prebuilt->template_type != ROW_MYSQL_DUMMY_TEMPLATE && !prebuilt->in_fts_query)
  {
    ...
    if (!prebuilt->idx_cond)
    {
      ...
      if (!row_sel_store_mysql_rec(next_buf, prebuilt, result_rec, vrow, result_rec != rec,
  ...
  else
  {
    ...
    else if (!prebuilt->idx_cond && !prebuilt->innodb_api)
    {
      ...
      if (!row_sel_store_mysql_rec(buf, prebuilt, result_rec, vrow, result_rec != rec,
                                   result_rec != rec ? clust_index : index, offsets, false))
      {
        /* Only fresh inserts may contain
        incomplete externally stored
    ...
    if (prebuilt->clust_index_was_generated)
    {
      row_sel_store_row_id_to_prebuilt(prebuilt, result_rec, result_rec == rec ? index : clust_index, offsets);
    }
  }
...
idx_cond_failed:
  if (!unique_search || !dict_index_is_clust(index) || direction != 0 || prebuilt->select_lock_type != LOCK_NONE ||
      prebuilt->used_in_HANDLER || prebuilt->innodb_api)
  {
    ...
    if (prebuilt->innodb_api && (btr_pcur_get_rec(pcur) != result_rec))
    {
      ...
      if (!prebuilt->innodb_api_rec_size || (prebuilt->innodb_api_rec_size < rec_size))
      {
        prebuilt->innodb_api_buf = static_cast<byte *>(mem_heap_alloc(prebuilt->cursor_heap, rec_size));
        prebuilt->innodb_api_rec_size = rec_size;
      }
      prebuilt->innodb_api_rec = rec_copy(prebuilt->innodb_api_buf, result_rec, offsets);
    }
  }
...
next_rec:
  ...
  if (mtr_has_extra_clust_latch || spatial_search)
  {
    ...
    if (!spatial_search)
    {
      const ibool result = sel_restore_position_for_mysql(&same_user_rec, BTR_SEARCH_LEAF, pcur, moves_up, &mtr);
      ...
      if (prev_rec != NULL && prev_rec_debug != NULL)
      {
        ...
        offsets1 = rec_get_offsets(prev_rec_debug, index, NULL, prev_rec_debug_n_fields, &heap_tmp);

        offsets2 = rec_get_offsets(prev_rec, index, NULL, prev_rec_debug_n_fields, &heap_tmp);

        ut_ad(!cmp_rec_rec(prev_rec_debug, prev_rec, offsets1, offsets2, index,
                           page_is_spatial_non_leaf(prev_rec, index)));
  ...
  if (moves_up)
  {
    ...
    if (spatial_search)
    {
      move = rtr_pcur_move_to_next(search_tuple, mode, pcur, 0, &mtr);
    }
    else
    {
...
#ifdef UNIV_DEBUG
  if (dict_index_is_spatial(index) && err != DB_SUCCESS && err != DB_END_OF_INDEX && err != DB_INTERRUPTED)
  {
...
func_exit:
  ...
  ut_ad(prebuilt->row_read_type != ROW_READ_WITH_LOCKS || !did_semi_consistent_read);
                           ...
                           '/' char, table name */
{
  ...
  table = dict_table_open_on_name(norm_name, FALSE, FALSE, DICT_ERR_IGNORE_NONE);
  ...
  if (lock_table_get_n_locks(table) == 0 &&
      ((trx->id != 0 && trx->id >= table->query_cache_inv_id) || !MVCC::is_view_active(trx->read_view) ||
  ...
  *value = 0;
  ...
  else
  {
    ...
    if (rec != NULL)
    {
      ...
      *value = row_search_autoinc_read_column(index, rec, 0, dfield->col->mtype, unsigned_type);
    }
