...
static bool row_vers_non_vc_match(dict_index_t *index, const dtuple_t *row, const row_ext_t *ext,
                                  const dtuple_t *ientry, mem_heap_t *heap, ulint *n_non_v_col);
/*****************************************************************/ /**
    ...
    mtr_t *mtr)                /*!< in/out: mini-transaction */
{
  ...
  if (trx == 0)
  {
    ...
    if (corrupt)
    {
      lock_report_trx_id_insanity(trx_id, clust_rec, clust_index, clust_offsets, trx_sys_get_max_trx_id());
    }
    mem_heap_free(heap);
    DBUG_RETURN(0);
  }
  ...
  for (version = clust_rec;; version = prev_version)
  {
    ...
    trx_undo_prev_version_build(clust_rec, mtr, version, clust_index, clust_offsets, heap, &prev_version, NULL,
    ...
    ut_ad(prev_version || !rec_get_deleted_flag(version, comp) || !trx_rw_is_active(trx_id, NULL, false));
    ...
    row = row_build(ROW_COPY_POINTERS, clust_index, prev_version, clust_offsets, NULL, NULL, NULL, &ext, heap);
    ...
    if (dict_index_has_virtual(index))
    {
      ...
      if (!cur_vrow)
      {
        ...
        to compare */
        if (row_vers_non_vc_match(index, row, ext, ientry, heap, &n_non_v_col) == 0)
        {
          if (rec_del != vers_del)
          {
            break;
    ...
    const ulint *offsets) /*!< in: rec_get_offsets(rec, index) */
{
  ...
  {
    trx = row_vers_impl_x_locked_low(clust_rec, clust_index, rec, index, offsets, &mtr);
...
static void row_vers_build_clust_v_col(dtuple_t *row, dict_index_t *clust_index, dict_index_t *index, mem_heap_t *heap)
{
  ...
  for (ulint i = 0; i < dict_index_get_n_fields(index); i++)
  {
    ...
    if (dict_col_is_virtual(ind_field->col))
    {
      ...
      innobase_get_computed_value(row, col, clust_index, &local_heap, heap, NULL, current_thd, NULL, NULL, NULL, NULL);
    }
  }
...
static void row_vers_build_cur_vrow_low(bool in_purge, const rec_t *rec, dict_index_t *clust_index,
                                        ulint *clust_offsets, dict_index_t *index, roll_ptr_t roll_ptr, trx_id_t trx_id,
                                        mem_heap_t *v_heap, const dtuple_t **vrow, mtr_t *mtr)
{
  ...
  *vrow = dtuple_create_with_vcol(v_heap, 0, num_v);
  ...
  roll_ptr */
  const ulint status = in_purge ? TRX_UNDO_PREV_IN_PURGE | TRX_UNDO_GET_OLD_V_VALUE : TRX_UNDO_GET_OLD_V_VALUE;

  while (!all_filled)
  {
    mem_heap_t *heap2 = heap;
    heap = mem_heap_create(1024);
    roll_ptr_t cur_roll_ptr = row_get_rec_roll_ptr(version, clust_index, clust_offsets);

    trx_undo_prev_version_build(rec, mtr, version, clust_index, clust_offsets, heap, &prev_version, NULL, vrow, status);
...
static bool row_vers_vc_matches_cluster(bool in_purge, const rec_t *rec, const dtuple_t *row, row_ext_t *ext,
                                        ...
                                        mem_heap_t *v_heap, const dtuple_t **vrow, mtr_t *mtr)
{
  ...
  if (!row_vers_non_vc_match(index, row, ext, ientry, tuple_heap, &n_non_v_col))
  {
  ...
  *vrow = dtuple_create_with_vcol(v_heap ? v_heap : tuple_heap, 0, num_v);
  ...
  bit to search the undo log until we hit the current undo log with
  roll_ptr */
  ulint status = (in_purge ? TRX_UNDO_PREV_IN_PURGE : 0) | TRX_UNDO_GET_OLD_V_VALUE;
  ...
  while (n_cmp_v_col < n_fields - n_non_v_col)
  {
    ...
    for (i = 0; i < entry_len; i++)
    {
      ...
      {
        if (ind_field->prefix_len != 0 && !dfield_is_null(field2) && field2->len > ind_field->prefix_len)
        {
          field2->len = ind_field->prefix_len;
        }

        /* The index field mismatch */
        if (v_heap || cmp_dfield_dfield(field2, field1) != 0)
        {
          if (v_heap)
          {
...
static const dtuple_t *row_vers_build_cur_vrow(bool in_purge, const rec_t *rec, dict_index_t *clust_index,
                                               ulint **clust_offsets, dict_index_t *index, const dtuple_t *ientry,
                                               roll_ptr_t roll_ptr, trx_id_t trx_id, mem_heap_t *heap,
                                               mem_heap_t *v_heap, mtr_t *mtr)
{
  const dtuple_t *cur_vrow = NULL;

  roll_ptr_t t_roll_ptr = row_get_rec_roll_ptr(rec, clust_index, *clust_offsets);
  ...
  if (trx_undo_roll_ptr_is_insert(t_roll_ptr))
  {
    ...
    cannot be freed yet. */
    dtuple_t *row = row_build(ROW_COPY_POINTERS, clust_index, rec, *clust_offsets, NULL, NULL, NULL, NULL, heap);
  ...
  else
  {
    ...
    row_vers_build_cur_vrow_low(in_purge, rec, clust_index, *clust_offsets, index, roll_ptr, trx_id, v_heap, &cur_vrow,
                                mtr);
  }
    ...
    trx_id_t trx_id)        /*!< in: transaction ID on the purging record */
{
  ...
  if (also_curr && !rec_get_deleted_flag(rec, comp))
  {
    ...
    row = row_build(ROW_COPY_POINTERS, clust_index, rec, clust_offsets, NULL, NULL, NULL, &ext, heap);
...
#define dbug_v_purge false
      ...
      {
        if (row_vers_vc_matches_cluster(also_curr, rec, row, ext, clust_index, clust_offsets, index, ientry, roll_ptr,
  ...
  else if (dict_index_has_virtual(index))
  {
    ...
    cur_vrow = row_vers_build_cur_vrow(also_curr, rec, clust_index, &clust_offsets, index, ientry, roll_ptr, trx_id,
                                       heap, v_heap, mtr);
  }
  ...
  for (;;)
  {
    ...
    trx_undo_prev_version_build(rec, mtr, version, clust_index, clust_offsets, heap, &prev_version, NULL,
                                dict_index_has_virtual(index) ? &vrow : NULL, 0);
    ...
    const dtuple_t **vrow)    /*!< out: virtual row */
{
  ...
  for (;;)
  {
    ...
    bool purge_sees =
        trx_undo_prev_version_build(rec, mtr, version, index, *offsets, heap, &prev_version, NULL, vrow, 0);
                              ...
                              if it is not updated in the view */
{
  ...
  for (;;)
  {
    ...
    if (version_trx && trx_state_eq(version_trx, TRX_STATE_COMMITTED_IN_MEMORY))
    {
      version_trx = NULL;
    }
    trx_sys_mutex_exit();
    ...
    if (!trx_undo_prev_version_build(rec, mtr, version, index, *offsets, heap, &prev_version, in_heap, vrow, 0))
    {
      mem_heap_free(heap);
      heap = heap2;
      heap2 = NULL;
