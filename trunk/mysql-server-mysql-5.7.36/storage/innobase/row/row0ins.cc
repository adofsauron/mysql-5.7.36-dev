    ...
    ins_node_t *node) /*!< in: row insert node */
{
  ...
  {
    entry = row_build_index_entry_low(node->row, NULL, index, node->entry_sys_heap, ROW_BUILD_FOR_INSERT);
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_ins_sec_index_entry_by_modify(
    /*==============================*/
                           ...
                           latching any further pages */
{
  ...
  update = row_upd_build_sec_rec_difference_binary(rec, cursor->index, *offsets, entry, heap);
  ...
  if (mode == BTR_MODIFY_LEAF)
  {
    ...
    err = btr_cur_optimistic_update(flags | BTR_KEEP_SYS_FLAG, cursor, offsets, &offsets_heap, update, 0, thr,
  ...
  else
  {
    ...
    err = btr_cur_pessimistic_update(flags | BTR_KEEP_SYS_FLAG, cursor, offsets, &offsets_heap, heap, &dummy_big_rec,
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_ins_clust_index_entry_by_modify(
    /*================================*/
                           ...
                           latching any further pages */
{
  ...
  update =
      row_upd_build_difference_binary(cursor->index, entry, rec, NULL, true, thr_get_trx(thr), heap, mysql_table, &err);
  if (err != DB_SUCCESS)
  {
    return (err);
  }
  ...
  if (mode != BTR_MODIFY_TREE)
  {
    ...
    err = btr_cur_optimistic_update(flags, cursor, offsets, offsets_heap, update, 0, thr, thr_get_trx(thr)->id, mtr);
    switch (err)
    {
      case DB_OVERFLOW:
      case DB_UNDERFLOW:
  ...
  else
  {
    ...
    err = btr_cur_pessimistic_update(flags | BTR_KEEP_POS_FLAG, cursor, offsets, offsets_heap, heap, &big_rec, update,
    ...
    if (big_rec)
    {
      ...
      err = btr_store_big_rec_extern_fields(pcur, update, *offsets, big_rec, mtr, BTR_STORE_INSERT_UPDATE);
    ...
    dict_table_t *table) /*!< in: table */
{
  ...
  for (parent = que_node_get_parent(node); que_node_get_type(parent) == QUE_NODE_UPDATE;
  ...
  *fts_col_affected = FALSE;
    ...
    for (j = 0; j < parent_update->n_fields; j++)
    {
      ...
      if (parent_ufield->field_no == parent_field_no)
      {
        ...
        if (!dfield_is_null(&ufield->new_val) &&
            dtype_get_at_most_n_mbchars(col->prtype, col->mbminmaxlen, col->len, ufield_len,
                                        static_cast<char *>(dfield_get_data(&ufield->new_val))) < ufield_len)
        {
          return (ULINT_UNDEFINED);
        }
        ...
        if (min_size > ufield_len)
        {
          ...
          if (mbminlen == 1 && dtype_get_charset_coll(col->prtype) == DATA_MYSQL_BINARY_CHARSET_COLL)
          {
        ...
        FTS index on it */
        if (table->fts && dict_table_is_fts_column(table->fts->indexes, dict_col_get_no(col),
        ...
        if (table->fts && ufield->field_no == doc_id_pos)
        {
          ...
          new_doc_id = fts_read_doc_id(static_cast<const byte *>(dfield_get_data(&ufield->new_val)));
  ...
  if (table->fts && *fts_col_affected)
  {
    if (DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_HAS_DOC_ID))
    {
      ...
      next_doc_id = static_cast<doc_id_t *>(mem_heap_alloc(heap, sizeof(doc_id_t)));
    ...
    dict_foreign_t *foreign) /*!< in: foreign key constraint */
{
  ...
  if (os_file_set_eof(srv_misc_tmpfile))
  {
    ...
    dict_print_info_on_foreign_key_in_create_format(srv_misc_tmpfile, trx, foreign, FALSE);
    ...
    trx_t *trx) /*!< in: transaction */
{
  ...
  trx_print_low(dict_foreign_err_file, trx, 600, n_rec_locks, n_trx_locks, heap_size);
                             ...
                             table */
{
  ...
  putc('\n', ef);
  fputs(errstr, ef);
  fprintf(ef, " in parent table, in index %s", foreign->referenced_index->name());
  if (entry)
  {
    fputs(" tuple:\n", ef);
                             ...
                             child table */
{
  ...
  fputs(":\n", ef);
  dict_print_info_on_foreign_key_in_create_format(ef, trx, foreign, TRUE);
  fprintf(ef, "\nTrying to add in child table, in index %s", foreign->foreign_index->name());
  if (entry)
  {
    fputs(" tuple:\n", ef);
...
static void row_ins_foreign_fill_virtual(upd_node_t *cascade, const rec_t *rec, dict_index_t *index, upd_node_t *node,
                                         dict_foreign_t *foreign, dberr_t *err)
{
  ...
  const ulint *offsets = rec_get_offsets(rec, index, offsets_, ULINT_UNDEFINED, &update->heap);
  ulint n_v_fld = index->table->n_v_def;
  ulint n_diff;
  upd_field_t *upd_field;
  dict_vcol_set *v_cols = foreign->v_cols;

  update->old_vrow = row_build(ROW_COPY_POINTERS, index, rec, offsets, index->table, NULL, NULL, &ext, update->heap);
  ...
  for (ulint i = 0; i < n_v_fld; i++)
  {
    ...
    dfield_t *vfield = innobase_get_computed_value(update->old_vrow, col, index, &v_heap, update->heap, NULL, thd, NULL,
                                                   NULL, NULL, NULL);
    ...
    upd_field->old_v_val = static_cast<dfield_t *>(mem_heap_alloc(update->heap, sizeof *upd_field->old_v_val));
                        ...
                        : (foreign->type & DICT_FOREIGN_ON_UPDATE_SET_NULL))
    {
      ...
      else
      {
        ...
        for (uint32_t j = 0; j < col->num_base; j++)
        {
          ...
          dfield_t *row_field = innobase_get_field_from_update_vector(foreign, node->update, col_no);
          if (row_field != NULL)
          {
            dfield_set_null(row_field);
          }
        }
        dfield_t *new_vfield = innobase_get_computed_value(update->old_vrow, col, index, &v_heap, update->heap, NULL,
                                                           thd, NULL, NULL, node->update, foreign);
    ...
    {
      dfield_t *new_vfield = innobase_get_computed_value(update->old_vrow, col, index, &v_heap, update->heap, NULL, thd,
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_ins_foreign_check_on_constraint(
    /*================================*/
    que_thr_t *thr,          /*!< in: query thread whose run_node
                             is an update node */
    dict_foreign_t *foreign, /*!< in: foreign key constraint whose
                             ...
                             page */
{
  ...
  else
  {
    ...
    {
      ib::error() << "In cascade of a foreign key op index " << index->name << " of table " << index->table->name;
  ...
  if (cascade->is_delete && foreign->v_cols != NULL && foreign->v_cols->size() > 0 && table->vc_templ == NULL)
  {
                      ...
                      : (foreign->type & DICT_FOREIGN_ON_UPDATE_SET_NULL))
  {
    ...
    for (i = 0; i < foreign->n_fields; i++)
    {
      ...
      ulint col_no = dict_index_get_nth_col_no(index, i);

      ufield->field_no = dict_table_get_nth_col_pos(table, col_no);
      dict_col_t *col = dict_table_get_nth_col(table, col_no);
      ...
      if (table->fts &&
          dict_table_is_fts_column(table->fts->indexes, dict_index_get_nth_col_no(index, i),
                                   dict_col_is_virtual(dict_index_get_nth_col(index, i))) != ULINT_UNDEFINED)
      {
    ...
    {
      row_ins_foreign_fill_virtual(cascade, clust_rec, clust_index, node, foreign, &err);
  ...
  if (!node->is_delete && (foreign->type & DICT_FOREIGN_ON_UPDATE_CASCADE))
  {
    ...
    n_to_update = row_ins_cascade_calc_update_vec(node, foreign, tmp_heap, trx, &fts_col_affacted);
    ...
    que_thr_t *thr)          /*!< in: query thread */
{
  ...
  if (check_table == NULL || check_table->ibd_file_missing || check_index == NULL ||
  ...
  do
  {
    ...
    if (cmp == 0)
    {
      ...
      if (rec_get_deleted_flag(rec, rec_offs_comp(offsets)))
      {
        err = row_ins_set_shared_rec_lock(lock_type, block, rec, check_index, offsets, thr);
        switch (err)
        {
          case DB_SUCCESS_LOCKED_REC:
          case DB_SUCCESS:
  ...
  if (check_ref)
  {
    row_ins_foreign_report_add_err(trx, foreign, btr_pcur_get_rec(&pcur), entry);
    err = DB_NO_REFERENCED_ROW;
  }
  else
  {
...
/***************************************************************/ /**
 ...
 a constraint.
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_ins_check_foreign_constraints(
    /*==============================*/
    dict_table_t *table, /*!< in: table */
    dict_index_t *index, /*!< in: index */
    dtuple_t *entry,     /*!< in: index entry for index */
    que_thr_t *thr)      /*!< in: query thread */
{
  ...
  DEBUG_SYNC_C_IF_THD(thr_get_trx(thr)->mysql_thd, "foreign_constraint_check_for_ins");
  ...
  for (dict_foreign_set::iterator it = table->foreign_set.begin(); it != table->foreign_set.end(); ++it)
  {
    ...
    if (foreign->foreign_index == index)
    {
      ...
      {
        ref_table = dict_table_open_on_name(foreign->referenced_table_name_lookup, FALSE, FALSE, DICT_ERR_IGNORE_NONE);
      }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_ins_scan_sec_index_for_duplicate(
    /*=================================*/
    ulint flags,         /*!< in: undo logging and locking flags */
...
/*!< in/out: memory heap that can be emptied */
{
  ...
  btr_pcur_open(index, entry, PAGE_CUR_GE, s_latch ? BTR_SEARCH_LEAF | BTR_ALREADY_S_LATCHED : BTR_SEARCH_LEAF, &pcur,
                mtr);
  ...
  do
  {
    ...
    if (cmp == 0 && !index->allow_duplicates)
    {
      if (row_ins_dupl_error_with_rec(rec, entry, index, offsets))
      {
        ...
        if (index == index->table->fts_doc_id_index && DICT_TF2_FLAG_IS_SET(index->table, DICT_TF2_FTS_HAS_DOC_ID))
        {
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_ins_duplicate_online(
    /*=====================*/
    ...
    ulint *offsets)        /*!< in/out: rec_get_offsets(rec) */
{
  ...
  cmp_dtuple_rec_with_match_low(entry, rec, offsets, n_uniq + 2, &fields);
...
a newer version of entry (the entry should not be inserted)
@retval DB_DUPLICATE_KEY when entry is a duplicate of rec */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_ins_duplicate_error_in_clust_online(
    /*====================================*/
    ulint n_uniq,            /*!< in: offset of DB_TRX_ID */
    const dtuple_t *entry,   /*!< in: entry that is being inserted */
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_ins_duplicate_error_in_clust(
    /*=============================*/
    ulint flags,           /*!< in: undo logging and locking flags */
    btr_cur_t *cursor,     /*!< in: B-tree cursor */
    const dtuple_t *entry, /*!< in: entry to insert */
  ...
  if (cursor->low_match >= n_unique)
  {
    ...
    if (!page_rec_is_infimum(rec))
    {
      ...
      else if (trx->duplicates)
      {
        ...
        err = row_ins_set_exclusive_rec_lock(LOCK_REC_NOT_GAP, btr_cur_get_block(cursor), rec, cursor->index, offsets,
                                             thr);
      }
      else
      {
        err =
            row_ins_set_shared_rec_lock(LOCK_REC_NOT_GAP, btr_cur_get_block(cursor), rec, cursor->index, offsets, thr);
      }
...
and return. don't execute actual insert. */
{
  ...
  ut_ad(!dict_index_is_unique(index) || n_uniq == dict_index_get_n_unique(index));
  ...
  ut_ad(!dict_table_is_intrinsic(index->table) || cursor->page_cur.block->made_dirty_with_no_latch);
...
#ifdef UNIV_DEBUG
  {
    ...
    ut_ad(page_rec_is_supremum(first_rec) || rec_n_fields_is_sane(index, first_rec, entry));
  }
#endif /* UNIV_DEBUG */
  ...
  ut_ad(!index->allow_duplicates || (index->allow_duplicates && dict_table_is_intrinsic(index->table)));

  if (!index->allow_duplicates && n_uniq && (cursor->up_match >= n_uniq || cursor->low_match >= n_uniq))
  {
    if (flags == (BTR_CREATE_FLAG | BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG | BTR_KEEP_SYS_FLAG))
    {
      /* Set no locks when applying log
      in online table rebuild. Only check for duplicates. */
      err = row_ins_duplicate_error_in_clust_online(n_uniq, entry, cursor, &offsets, &offsets_heap);
  ...
  if (!index->allow_duplicates && row_ins_must_modify_rec(cursor))
  {
    ...
    err =
        row_ins_clust_index_entry_by_modify(&pcur, flags, mode, &offsets, &offsets_heap, entry_heap, entry, thr, &mtr);
  ...
  else
  {
    ...
    {
      ut_ad((mode & ~BTR_ALREADY_S_LATCHED) == BTR_MODIFY_LEAF);
      err = btr_cur_optimistic_insert(flags, cursor, &offsets, &offsets_heap, entry, &insert_rec, &big_rec, n_ext, thr,
                                      &mtr);
    }
    else
    {
      ...
      if (err == DB_FAIL)
      {
        err = btr_cur_pessimistic_insert(flags, cursor, &offsets, &offsets_heap, entry, &insert_rec, &big_rec, n_ext,
                                         thr, &mtr);
      }
    }
    ...
    if (big_rec != NULL)
    {
      ...
      DBUG_EXECUTE_IF("row_ins_extern_checkpoint", log_make_checkpoint_at(LSN_MAX, TRUE););
      err = row_ins_index_entry_big_rec(entry, big_rec, offsets, &offsets_heap, index, thr_get_trx(thr)->mysql_thd,
                                        __FILE__, __LINE__);
...
@return error code */
static dberr_t row_ins_sorted_clust_index_entry(ulint mode, dict_index_t *index, dtuple_t *entry, ulint n_ext,
                                                que_thr_t *thr)
{
  dberr_t err;
  mtr_t *mtr;
  ...
  is not cached. */
  if (mode == BTR_MODIFY_TREE || index->last_ins_cur->rec == NULL || index->last_ins_cur->disable_caching)
  {
...
static MY_ATTRIBUTE((warn_unused_result)) bool row_ins_sec_mtr_start_and_check_if_aborted(mtr_t *mtr,
                                                                                          dict_index_t *index,
                                                                                          bool check, ulint search_mode)
{
  ut_ad(!dict_index_is_clust(index));
...
and return. don't execute actual insert. */
{
  ...
  ut_ad(thr_get_trx(thr)->id != 0 || dict_table_is_intrinsic(index->table));
  ...
  if (dict_index_is_spatial(index))
  {
    ...
    btr_cur_search_to_nth_level(index, 0, entry, PAGE_CUR_RTREE_INSERT, search_mode, &cursor, 0, __FILE__, __LINE__,
                                &mtr);
  ...
  if (dict_index_is_unique(index) && (cursor.low_match >= n_unique || cursor.up_match >= n_unique))
  {
    ...
    if (row_ins_sec_mtr_start_and_check_if_aborted(&mtr, index, check, search_mode))
    {
      goto func_exit;
    }
    ...
    {
      btr_cur_search_to_nth_level(index, 0, entry, PAGE_CUR_LE, (search_mode & ~(BTR_INSERT | BTR_IGNORE_SEC_UNIQUE)),
  ...
  if (row_ins_must_modify_rec(&cursor))
  {
    ...
    prefix, we must convert the insert into a modify of an
    existing record */
    offsets = rec_get_offsets(btr_cur_get_rec(&cursor), index, offsets, ULINT_UNDEFINED, &offsets_heap);

    err = row_ins_sec_index_entry_by_modify(flags, mode, &cursor, &offsets, offsets_heap, heap, entry, thr, &mtr);
  ...
  else
  {
    ...
    if (mode == BTR_MODIFY_LEAF)
    {
      err = btr_cur_optimistic_insert(flags, &cursor, &offsets, &offsets_heap, entry, &insert_rec, &big_rec, 0, thr,
                                      &mtr);
      if (err == DB_SUCCESS && dict_index_is_spatial(index) && rtr_info.mbr_adj)
      {
        err = rtr_ins_enlarge_mbr(&cursor, thr, &mtr);
    ...
    else
    {
      ...
      if (err == DB_FAIL)
      {
        err = btr_cur_pessimistic_insert(flags, &cursor, &offsets, &offsets_heap, entry, &insert_rec, &big_rec, 0, thr,
                                         &mtr);
      }
      if (err == DB_SUCCESS && dict_index_is_spatial(index) && rtr_info.mbr_adj)
      {
...
dberr_t row_ins_index_entry_big_rec_func(
    /*=============================*/
    ...
    ulint line)      /*!< in: line number of caller */
{
  ...
  error = btr_store_big_rec_extern_fields(&pcur, 0, offsets, big_rec, &mtr, BTR_STORE_INSERT);
...
and return. don't execute actual insert. */
{
  ...
  {
    log_free_check();
    flags = dict_table_is_temporary(index->table) ? BTR_NO_LOCKING_FLAG : 0;
  }
  else
  {
  ...
  if (dict_table_is_intrinsic(index->table) && dict_index_is_auto_gen_clust(index))
  {
    ...
    err = row_ins_sorted_clust_index_entry(BTR_MODIFY_LEAF, index, entry, n_ext, thr);
  }
  else
  {
    err = row_ins_clust_index_entry_low(flags, BTR_MODIFY_LEAF, index, n_uniq, entry, n_ext, thr, dup_chk_only);
  }

  DEBUG_SYNC_C_IF_THD(thr_get_trx(thr)->mysql_thd, "after_row_ins_clust_index_entry_leaf");
  ...
  if (dict_table_is_intrinsic(index->table) && dict_index_is_auto_gen_clust(index))
  {
    err = row_ins_sorted_clust_index_entry(BTR_MODIFY_TREE, index, entry, n_ext, thr);
  }
  else
  {
    err = row_ins_clust_index_entry_low(flags, BTR_MODIFY_TREE, index, n_uniq, entry, n_ext, thr, dup_chk_only);
  }
...
and return. don't execute actual insert. */
{
  ...
  err = row_ins_sec_index_entry_low(flags, BTR_MODIFY_LEAF, index, offsets_heap, heap, entry, 0, thr, dup_chk_only);
  if (err == DB_FAIL)
  {
    mem_heap_empty(heap);
    ...
    err = row_ins_sec_index_entry_low(flags, BTR_MODIFY_TREE, index, offsets_heap, heap, entry, 0, thr, dup_chk_only);
  }
    ...
    const dfield_t *row_field) /*!< in: row field */
{
  ...
  /* obtain the MBR */
  rtree_mbr_from_wkb(dptr + GEO_DATA_HEADER_SIZE, static_cast<uint>(dlen - GEO_DATA_HEADER_SIZE), SPDIMS, mbr);
...
dberr_t row_ins_index_entry_set_vals(const dict_index_t *index, dtuple_t *entry, const dtuple_t *row)
{
  ...
  for (i = 0; i < n_fields + num_v; i++)
  {
    ...
    if (dict_col_is_virtual(col))
    {
      ...
      ut_ad(dtuple_get_n_fields(row) == dict_table_get_n_cols(index->table));
    ...
    if (ind_field != NULL && ind_field->prefix_len > 0 && dfield_get_len(row_field) != UNIV_SQL_NULL)
    {
      const dict_col_t *col = dict_field_get_col(ind_field);

      len = dtype_get_at_most_n_mbchars(col->prtype, col->mbminmaxlen, ind_field->prefix_len, len,
    ...
    if ((i == 0) && dict_index_is_spatial(index))
    {
      ...
      row_ins_spatial_index_entry_set_mbr_field(field, row_field);
      continue;
    }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_ins_index_entry_step(
    /*=====================*/
    ins_node_t *node, /*!< in: row insert node */
    que_thr_t *thr)   /*!< in: query thread */
{
  ...
  DEBUG_SYNC_C_IF_THD(thr_get_trx(thr)->mysql_thd, "after_row_ins_index_entry_step");
...
/***********************************************************/ /**
 ...
 @return DB_SUCCESS if operation successfully completed, else error
 code or DB_LOCK_WAIT */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_ins(
    /*====*/
    ins_node_t *node, /*!< in: row insert node */
    que_thr_t *thr)   /*!< in: query thread */
{
  ...
  while (node->index != NULL)
  {
    ...
    DBUG_EXECUTE_IF("row_ins_skip_sec", node->index = NULL; node->entry = NULL; break;);
