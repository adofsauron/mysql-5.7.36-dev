...
#ifdef UNIV_DEBUG
  row_log_table_blob_t(ulonglong offset_arg) : old_offset(0), free_offset(offset_arg), offset(BLOB_FREED) {}
#else  /* UNIV_DEBUG */
...
typedef std::map<ulint, row_log_table_blob_t, std::less<ulint>,
...
static MY_ATTRIBUTE((warn_unused_result)) int row_log_tmpfile(row_log_t *log)
{
  ...
  if (log->fd < 0)
  {
    ...
    DBUG_EXECUTE_IF("row_log_tmpfile_fail", if (log->fd > 0) row_merge_file_destroy_low(log->fd); log->fd = -1;);
    if (log->fd >= 0)
    {
...
@return TRUE if success, false if not */
static MY_ATTRIBUTE((warn_unused_result)) bool row_log_block_allocate(row_log_buf_t &log_buf)
{
  ...
  if (log_buf.block == NULL)
  {
    ...
    log_buf.block = ut_allocator<byte>(mem_key_row_log_buf).allocate_large(srv_sort_buf_size, &log_buf.block_pfx);
...
static void row_log_block_free(row_log_buf_t &log_buf)
{
  ...
  if (log_buf.block != NULL)
  {
    ut_allocator<byte>(mem_key_row_log_buf).deallocate_large(log_buf.block, &log_buf.block_pfx);
    log_buf.block = NULL;
  }
  DBUG_VOID_RETURN;
}
                           ...
                           or 0 for delete */
{
  ...
  ut_ad(rw_lock_own(dict_index_get_lock(index), RW_LOCK_S) || rw_lock_own(dict_index_get_lock(index), RW_LOCK_X));
  ...
  size = rec_get_converted_size_temp(index, tuple->fields, tuple->n_fields, NULL, &extra_size);
  ut_ad(size >= extra_size);
  ut_ad(size <= sizeof log->tail.buf);

  mrec_size = ROW_LOG_HEADER_SIZE + (extra_size >= 0x80) + size + (trx_id ? DATA_TRX_ID_LEN : 0);
  ...
  rec_convert_dtuple_to_temp(b + extra_size, index, tuple->fields, tuple->n_fields, NULL);
  b += size;
  ...
  if (mrec_size >= avail_size)
  {
    ...
    const os_offset_t byte_offset = (os_offset_t)log->tail.blocks * srv_sort_buf_size;
    ...
    err = os_file_write_int_fd(request, "(modification log)", log->fd, log->tail.block, byte_offset, srv_sort_buf_size);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) byte *row_log_table_open(
    /*===============*/
...
static MY_ATTRIBUTE((nonnull)) void row_log_table_close_func(
    /*=====================*/
    row_log_t *log, /*!< in/out: online rebuild log */
#ifdef UNIV_DEBUG
...
#ifdef UNIV_DEBUG
#define row_log_table_close(log, b, size, avail) row_log_table_close_func(log, b, size, avail)
#else /* UNIV_DEBUG */
...
{
  return (dict_table_get_nth_v_col(index->online_log->table, v_no)->m_col.ord_part);
}
                            ...
                            be logged, or NULL to use those in rec */
{
  ...
  ut_ad(rw_lock_own_flagged(&index->lock, RW_LOCK_FLAG_S | RW_LOCK_FLAG_X | RW_LOCK_FLAG_SX));

  if (dict_index_is_corrupted(index) || !dict_index_is_online_ddl(index) || index->online_log->error != DB_SUCCESS)
  {
    return;
  }
  ...
  if (index->online_log->same_pk)
  {
    ...
    heap = mem_heap_create(DATA_TRX_ID_LEN + DTUPLE_EST_ALLOC(new_index->n_uniq + 2));
    ...
    if (sys)
    {
      dfield_set_data(dtuple_get_nth_field(tuple, new_index->n_uniq), sys, DATA_TRX_ID_LEN);
      dfield_set_data(dtuple_get_nth_field(tuple, new_index->n_uniq + 1), sys + DATA_TRX_ID_LEN, DATA_ROLL_PTR_LEN);
    }
  }
  else
  {
    /* The PRIMARY KEY has changed. Translate the tuple. */
    old_pk = row_log_table_get_pk(rec, index, offsets, NULL, &heap);
  ...
  ut_ad(DATA_TRX_ID_LEN == dtuple_get_nth_field(old_pk, old_pk->n_fields - 2)->len);
  ut_ad(DATA_ROLL_PTR_LEN == dtuple_get_nth_field(old_pk, old_pk->n_fields - 1)->len);
  old_pk_size = rec_get_converted_size_temp(new_index, old_pk->fields, old_pk->n_fields, NULL, &old_pk_extra_size);
  ...
  if (rec_offs_any_extern(offsets) &&
      (dict_table_get_format(index->table) >= UNIV_FORMAT_B || dict_table_get_format(new_table) >= UNIV_FORMAT_B))
  {
    ...
    row_build(ROW_COPY_DATA, index, rec, offsets, NULL, NULL, NULL, &ext, heap);
    if (ext)
    {
      /* Log the row_ext_t, ext->ext and ext->buf */
      ext_size =
          ext->n_ext * ext->max_len + sizeof(*ext) + ext->n_ext * sizeof(ulint) + (ext->n_ext - 1) * sizeof ext->len;
      mrec_size += ext_size;
    }
  }
  ...
  {
    ulint v_extra;
    mrec_size += rec_get_converted_size_temp(new_index, NULL, 0, ventry, &v_extra);
  }
  ...
  if (byte *b = row_log_table_open(index->online_log, mrec_size, &avail_size))
  {
    ...
    *b++ = static_cast<byte>(old_pk_extra_size);
    ...
    rec_convert_dtuple_to_temp(b + old_pk_extra_size, new_index, old_pk->fields, old_pk->n_fields, NULL);

    b += old_pk_size;

    if (ext_size)
    {
      ulint cur_ext_size = sizeof(*ext) + (ext->n_ext - 1) * sizeof ext->len;
      ...
      if (const ulint *col_map = index->online_log->col_map)
      {
        for (ulint i = 0; i < ext->n_ext; i++)
        {
          const_cast<ulint &>(ext->ext[i]) = col_map[ext->ext[i]];
        }
      }
      ...
      ext_size -= cur_ext_size + ext->n_ext * sizeof(*ext->ext);
...
new table, not latched */
{
  ...
  size = rec_get_converted_size_temp(index, tuple->fields, tuple->n_fields, ventry, &extra_size);
  ...
  if (num_v)
  {
    ...
    {
      ulint v_extra = 0;
      mrec_size += rec_get_converted_size_temp(index, NULL, 0, o_ventry, &v_extra);
    }
  }
    ...
    *b++ = insert ? ROW_T_INSERT : ROW_T_UPDATE;
    ...
    if (old_pk_size)
    {
      *b++ = static_cast<byte>(old_pk_extra_size);
      ...
      rec_convert_dtuple_to_temp(b + old_pk_extra_size, new_index, old_pk->fields, old_pk->n_fields, ventry);
      b += old_pk_size;
    }
    ...
    rec_convert_dtuple_to_temp(b + extra_size, index, tuple->fields, tuple->n_fields, ventry);
    b += size;
                              ...
                              and a PRIMARY KEY is being created) */
{
  ...
  {
    row_log_table_low_redundant(rec, ventry, o_ventry, index, insert, old_pk, new_index);
    return;
  }
  ...
  mrec_size = ROW_LOG_HEADER_SIZE + (extra_size >= 0x80) + rec_offs_size(offsets) - omit_size;

  if (ventry && ventry->n_v_fields > 0)
  {
    ulint v_extra = 0;
    uint64_t rec_size = rec_get_converted_size_temp(new_index, NULL, 0, ventry, &v_extra);
    ...
    {
      mrec_size += rec_get_converted_size_temp(new_index, NULL, 0, o_ventry, &v_extra);
    }
  }
  else if (index->table->n_v_cols)
  {
...
static dberr_t row_log_table_get_pk_col(const dict_col_t *col, const dict_field_t *ifield, dfield_t *dfield,
                                        mem_heap_t *heap, const rec_t *rec, const ulint *offsets, ulint i,
                                        const page_size_t &page_size, ulint max_len)
{
  const byte *field;
  ulint len;
  ...
  if (rec_offs_nth_extern(offsets, i))
  {
    ...
    len = btr_copy_externally_stored_field_prefix(blob_field, field_len, page_size, field, len);
    if (len >= max_len + 1)
    {
    ...
    mem_heap_t **heap)    /*!< in/out: memory heap where allocated */
{
  ...
  if (log->error == DB_SUCCESS)
  {
    ...
    if (!*heap)
    {
      ...
      {
        size += dict_col_get_min_size(dict_index_get_nth_col(new_index, i));
      }
    ...
    for (ulint new_i = 0; new_i < new_n_uniq; new_i++)
    {
      ...
      const ulint col_no = dict_field_get_col(ifield)->ind;

      if (const dict_col_t *col = row_log_table_get_pk_old_col(index->table, log->col_map, col_no))
      {
        ...
        log->error = row_log_table_get_pk_col(col, ifield, dfield, *heap, rec, offsets, i, page_size, max_len);
      ...
      {
        ulint len = dtype_get_at_most_n_mbchars(prtype, mbminmaxlen, ifield->prefix_len, dfield_get_len(dfield),
    ...
    {
      trx_roll = static_cast<const byte *>(mem_heap_dup(*heap, trx_roll, DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN));
    }

    dfield_set_data(dtuple_get_nth_field(tuple, new_n_uniq), trx_roll, DATA_TRX_ID_LEN);
    dfield_set_data(dtuple_get_nth_field(tuple, new_n_uniq + 1), trx_roll + DATA_TRX_ID_LEN, DATA_ROLL_PTR_LEN);
  }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) const dtuple_t *row_log_table_apply_convert_mrec(
    /*=============================*/
                          ...
                          reason of failure */
{
  ...
  *error = DB_SUCCESS;
  ...
  if (log->add_cols)
  {
    ...
    {
      dict_col_copy_type(dict_table_get_nth_col(log->table, i), dfield_get_type(dtuple_get_nth_field(row, i)));
    }
  }
  else
  {
    row = dtuple_create_with_vcol(heap, dict_table_get_n_cols(log->table), num_v);
  ...
  for (ulint i = 0; i < rec_offs_n_fields(offsets); i++)
  {
    ...
    if (rec_offs_nth_extern(offsets, i))
    {
      ...
      if (const page_no_map *blobs = log->blobs)
      {
        ...
        ulint page_no = mach_read_from_4(data + len - (BTR_EXTERN_FIELD_REF_SIZE - BTR_EXTERN_PAGE_NO));
      ...
      data = btr_rec_copy_externally_stored_field(mrec, offsets, dict_table_page_size(index->table), i, &len, heap);
      ut_a(data);
      dfield_set_data(dfield, data, len);
    blob_done:
    ...
    if (len != UNIV_SQL_NULL && col->mtype == DATA_MYSQL && col->len != len && !dict_table_is_comp(log->table))
    {
      ut_ad(col->len >= len);
    ...
    const dict_col_t *new_col = dict_table_get_nth_col(log->table, col_no);
    ...
    ut_ad(!((new_col->prtype ^ dfield_get_type(dfield)->prtype) & ~DATA_NOT_NULL));
    ...
    ut_ad(dict_col_type_assert_equal(new_col, dfield_get_type(dfield)));
  }
  ...
  if (num_v)
  {
    ...
    trx_undo_read_v_cols(log->table, b, row, false, &(log->col_map[log->n_old_col]));
  }
...
/******************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_log_table_apply_insert_low(
    /*===========================*/
                              ...
                              duplicate key errors */
{
  ...
  DBUG_PRINT("ib_alter_table", ("insert table " IB_ID_FMT "(index " IB_ID_FMT "): %s", index->table->id, index->id,
                                rec_printer(row).str().c_str()));

  static const ulint flags = (BTR_CREATE_FLAG | BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG | BTR_KEEP_SYS_FLAG);

  entry = row_build_index_entry(row, NULL, index, heap);

  error = row_ins_clust_index_entry_low(flags, BTR_MODIFY_TREE, index, index->n_uniq, entry, 0, thr, false);
  ...
  do
  {
    ...
    entry = row_build_index_entry(row, NULL, index, heap);
    error = row_ins_sec_index_entry_low(flags, BTR_MODIFY_TREE, index, offsets_heap, heap, entry, trx_id, thr, false);
...
/******************************************************/ /**
 Replays an insert operation on a table that was rebuilt.
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_log_table_apply_insert(
    /*=======================*/
    que_thr_t *thr,           /*!< in: query graph */
    ...
    trx_id_t trx_id)          /*!< in: DB_TRX_ID of mrec */
{
  const row_log_t *log = dup->index->online_log;
  dberr_t error;
  const dtuple_t *row = row_log_table_apply_convert_mrec(mrec, dup->index, offsets, log, heap, trx_id, &error);
  ...
  error = row_log_table_apply_insert_low(thr, row, trx_id, offsets_heap, heap, dup);
  if (error != DB_SUCCESS)
  {
...
/******************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_log_table_apply_delete_low(
    /*===========================*/
                               ...
                               will be committed */
{
  ...
  DBUG_PRINT("ib_alter_table", ("delete table " IB_ID_FMT "(index " IB_ID_FMT "): %s", index->table->id, index->id,
                                rec_printer(btr_pcur_get_rec(pcur), offsets).str().c_str()));

  if (dict_table_get_next_index(index))
  {
    /* Build a row template for purging secondary index entries. */
    row = row_build(ROW_COPY_DATA, index, btr_pcur_get_rec(pcur), offsets, NULL, NULL, NULL, save_ext ? NULL : &ext,
                    heap);
    if (ventry)
    {
  ...
  btr_cur_pessimistic_delete(&error, FALSE, btr_pcur_get_btr_cur(pcur), BTR_CREATE_FLAG, false, mtr);
  mtr_commit(mtr);
  ...
  while ((index = dict_table_get_next_index(index)) != NULL)
  {
    ...
    const dtuple_t *entry = row_build_index_entry(row, save_ext, index, heap);
    mtr_start(mtr);
    mtr->set_named_space(index->space);
    btr_pcur_open(index, entry, PAGE_CUR_LE, BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE, pcur, mtr);
#ifdef UNIV_DEBUG
  ...
  flag_ok:
    ...
    if (page_rec_is_infimum(btr_pcur_get_rec(pcur)) || btr_pcur_get_low_match(pcur) < index->n_uniq)
    {
...
/******************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull(1, 3, 4, 5, 6, 7), warn_unused_result)) dberr_t row_log_table_apply_delete(
    /*=======================*/
    que_thr_t *thr,            /*!< in: query graph */
    ...
    ulint ext_size)            /*!< in: external field size */
{
  ...
  mtr_start(&mtr);
  mtr.set_named_space(index->space);
  btr_pcur_open(index, old_pk, PAGE_CUR_LE, BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE, &pcur, &mtr);
#ifdef UNIV_DEBUG
  switch (btr_pcur_get_btr_cur(&pcur)->flag)
  {
    case BTR_CUR_DELETE_REF:
...
flag_ok:
  ...
  offsets = rec_get_offsets(btr_pcur_get_rec(&pcur), index, NULL, ULINT_UNDEFINED, &offsets_heap);
  ...
  {
    ulint len;
    const byte *mrec_trx_id = rec_get_nth_field(mrec, moffsets, trx_id_col, &len);
    ut_ad(len == DATA_TRX_ID_LEN);
    const byte *rec_trx_id = rec_get_nth_field(btr_pcur_get_rec(&pcur), offsets, trx_id_col, &len);
    ut_ad(len == DATA_TRX_ID_LEN);

    ut_ad(rec_get_nth_field(mrec, moffsets, trx_id_col + 1, &len) == mrec_trx_id + DATA_TRX_ID_LEN);
    ut_ad(len == DATA_ROLL_PTR_LEN);
    ut_ad(rec_get_nth_field(btr_pcur_get_rec(&pcur), offsets, trx_id_col + 1, &len) == rec_trx_id + DATA_TRX_ID_LEN);
    ut_ad(len == DATA_ROLL_PTR_LEN);
  ...
  if (num_v)
  {
    byte *b = (byte *)mrec + rec_offs_data_size(moffsets) + ext_size;
    trx_undo_read_v_cols(log->table, b, old_pk, false, &(log->col_map[log->n_old_col]));
  }

  return (row_log_table_apply_delete_low(&pcur, old_pk, offsets, save_ext, heap, &mtr));
}

/******************************************************/ /**
 Replays an update operation on a table that was rebuilt.
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_log_table_apply_update(
    /*=======================*/
    que_thr_t *thr,           /*!< in: query graph */
                              ...
                              or PRIMARY KEY if same_pk */
{
  ...
  ut_ad(dtuple_get_n_fields(old_pk) == dict_index_get_n_unique(index) + (log->same_pk ? 0 : 2));

  row = row_log_table_apply_convert_mrec(mrec, dup->index, offsets, log, heap, trx_id, &error);
  ...
  ulint *cur_offsets = rec_get_offsets(btr_pcur_get_rec(&pcur), index, NULL, ULINT_UNDEFINED, &offsets_heap);
  ...
  if (!log->same_pk)
  {
    ...
    was buffered. */
    ulint len;
    const void *rec_trx_id = rec_get_nth_field(btr_pcur_get_rec(&pcur), cur_offsets, index->n_uniq, &len);
    ut_ad(len == DATA_TRX_ID_LEN);
    ut_ad(dtuple_get_nth_field(old_pk, index->n_uniq)->len == DATA_TRX_ID_LEN);
    ut_ad(dtuple_get_nth_field(old_pk, index->n_uniq + 1)->len == DATA_ROLL_PTR_LEN);
    ut_ad(DATA_TRX_ID_LEN + static_cast<const char *>(dtuple_get_nth_field(old_pk, index->n_uniq)->data) ==
          dtuple_get_nth_field(old_pk, index->n_uniq + 1)->data);
    if (memcmp(rec_trx_id, dtuple_get_nth_field(old_pk, index->n_uniq)->data, DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN))
    {
  ...
  if (error != DB_SUCCESS)
  {
    ...
    /* Some BLOBs are missing, so we are interpreting
    this ROW_T_UPDATE as ROW_T_DELETE (see *1). */
    error = row_log_table_apply_delete_low(&pcur, old_pk, cur_offsets, NULL, heap, &mtr);
    goto func_exit_committed;
  }

  /** It allows to create tuple with virtual column information. */
  dtuple_t *entry = row_build_index_entry_low(row, NULL, index, heap, ROW_BUILD_FOR_INSERT);
  upd_t *update = row_upd_build_difference_binary(index, entry, btr_pcur_get_rec(&pcur), cur_offsets, false, NULL, heap,
                                                  dup->table, &error);
  if (error != DB_SUCCESS)
  {
    goto func_exit;
  ...
  const bool pk_updated = upd_get_nth_field(update, 0)->field_no < new_trx_id_col;
  ...
  if (dict_table_get_next_index(index) != NULL)
  {
    ...
    the record. */
    old_row = row_build(ROW_COPY_DATA, index, btr_pcur_get_rec(&pcur), cur_offsets, NULL, NULL, NULL, &old_ext, heap);
    ut_ad(old_row);
    DBUG_PRINT("ib_alter_table", ("update table " IB_ID_FMT "(index " IB_ID_FMT "): %s to %s", index->table->id,
                                  index->id, rec_printer(old_row).str().c_str(), rec_printer(row).str().c_str()));
  }
  else
  {
    old_row = NULL;
  ...
  error = btr_cur_pessimistic_update(
      BTR_CREATE_FLAG | BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG | BTR_KEEP_SYS_FLAG | BTR_KEEP_POS_FLAG,
      btr_pcur_get_btr_cur(&pcur), &cur_offsets, &offsets_heap, heap, &big_rec, update, 0, thr, 0, &mtr);

  if (big_rec)
  {
    if (error == DB_SUCCESS)
    {
      error = btr_store_big_rec_extern_fields(&pcur, update, cur_offsets, big_rec, &mtr, BTR_STORE_UPDATE);
    }
  ...
  while ((index = dict_table_get_next_index(index)) != NULL)
  {
    ...
    if (ROW_FOUND != row_search_index_entry(index, entry, BTR_MODIFY_TREE, &pcur, &mtr))
    {
      ut_ad(0);
      error = DB_CORRUPTION;
      break;
    ...
    entry = row_build_index_entry(row, NULL, index, heap);
    error =
        row_ins_sec_index_entry_low(BTR_CREATE_FLAG | BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG | BTR_KEEP_SYS_FLAG,
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) const mrec_t *row_log_table_apply_op(
    /*===================*/
    que_thr_t *thr,           /*!< in: query graph */
    ulint trx_id_col,         /*!< in: position of
  ...
  switch (*mrec++)
  {
    ...
    case ROW_T_INSERT:
      ...
      else
      {
        ...
        ulint len;
        const byte *db_trx_id = rec_get_nth_field(mrec, offsets, trx_id_col, &len);
        ut_ad(len == DATA_TRX_ID_LEN);
        *error = row_log_table_apply_insert(thr, mrec, offsets, offsets_heap, heap, dup, trx_read_trx_id(db_trx_id));
      }
      break;
    ...
    case ROW_T_DELETE:
      ...
      if (ext_size)
      {
        ...
        ext = static_cast<row_ext_t *>(mem_heap_dup(heap, mrec + rec_offs_data_size(offsets), ext_size));

        byte *ext_start = reinterpret_cast<byte *>(ext);

        ulint ext_len = sizeof(*ext) + (ext->n_ext - 1) * sizeof ext->len;
      ...
      *error = row_log_table_apply_delete(thr, new_trx_id_col, mrec, offsets, offsets_heap, heap, log, ext, ext_size);
      break;
    ...
    case ROW_T_UPDATE:
      ...
      if (num_v)
      {
        ...
        if (n_v_size > 2)
        {
          trx_undo_read_v_cols(log->table, const_cast<byte *>(next_mrec), old_pk, false,
...

      {
        ...
        const byte *db_trx_id = rec_get_nth_field(mrec, offsets, trx_id_col, &len);
        ut_ad(len == DATA_TRX_ID_LEN);
        *error = row_log_table_apply_update(thr, new_trx_id_col, mrec, offsets, offsets_heap, heap, dup,
...
inline ulint row_log_progress_inc_per_block()
{
  ...
  (usually 1MiB). */
  const ulint pages_per_block =
      std::max(static_cast<unsigned long>(srv_sort_buf_size / univ_page_size.physical()), 1UL);
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_log_table_apply_ops(que_thr_t *thr, row_merge_dup_t *dup, ut_stage_alter_t *stage)
{
  dberr_t error;
  ...
  dict_index_t *new_index = dict_table_get_first_index(new_table);
  const ulint i =
      1 + REC_OFFS_HEADER_SIZE + ut_max(dict_index_get_n_fields(index), dict_index_get_n_unique(new_index) + 2);
  const ulint trx_id_col = dict_col_get_clust_pos(dict_table_get_sys_col(index->table, DATA_TRX_ID), index);
  const ulint new_trx_id_col = dict_col_get_clust_pos(dict_table_get_sys_col(new_table, DATA_TRX_ID), new_index);
...
next_block:
  ...
  if (UNIV_UNLIKELY(index->online_log->head.blocks > index->online_log->tail.blocks))
  {
  unexpected_eof:
    ib::error() << "Unexpected end of temporary file for table " << index->table->name;
  corruption:
    error = DB_CORRUPTION;
    goto func_exit;
  }
  ...
  if (index->online_log->head.blocks == index->online_log->tail.blocks)
  {
    if (index->online_log->head.blocks)
    {
      ...
      if (index->online_log->fd > 0 && ftruncate(index->online_log->fd, 0) == -1)
      {
        perror("ftruncate");
      }
  ...
  else
  {
    ...
    err = os_file_read_no_error_handling_int_fd(request, index->online_log->fd, index->online_log->head.block, ofs,
    ...
    posix_fadvise(index->online_log->fd, ofs, srv_sort_buf_size, POSIX_FADV_DONTNEED);
  ...
  if (mrec)
  {
    ...
    memcpy((mrec_t *)mrec_end, next_mrec, (&index->online_log->head.buf)[1] - mrec_end);
    mrec = row_log_table_apply_op(thr, trx_id_col, new_trx_id_col, dup, &error, offsets_heap, heap,
                                  index->online_log->head.buf, (&index->online_log->head.buf)[1], offsets);
    if (error != DB_SUCCESS)
    {
      goto func_exit;
    }
  ...
  if (next_mrec_end == index->online_log->head.block + srv_sort_buf_size)
  {
    ...
    {
      ut_ad(next_mrec == index->online_log->head.block + index->online_log->head.bytes);
      ut_ad(index->online_log->tail.blocks > index->online_log->head.blocks);
    }
  }
  else if (next_mrec_end == index->online_log->tail.block + index->online_log->tail.bytes)
  {
    ut_ad(next_mrec == index->online_log->tail.block + index->online_log->head.bytes);
    ut_ad(index->online_log->tail.blocks == 0);
    ut_ad(index->online_log->head.blocks == 0);
  ...
  while (!trx_is_interrupted(trx))
  {
    ...
    else
    {
      ...
      ut_ad(index->online_log->head.blocks == 0);
      ut_ad(index->online_log->tail.blocks == 0);
      ut_ad(mrec_end == index->online_log->tail.block + index->online_log->tail.bytes);
    ...
    next_mrec = row_log_table_apply_op(thr, trx_id_col, new_trx_id_col, dup, &error, offsets_heap, heap, mrec, mrec_end,
                                       offsets);
...
@return DB_SUCCESS, or error code on failure */
dberr_t row_log_table_apply(que_thr_t *thr, dict_table_t *old_table, struct TABLE *table, ut_stage_alter_t *stage)
{
  dberr_t error;
  dict_index_t *clust_index;

  thr_get_trx(thr)->error_key_num = 0;
  DBUG_EXECUTE_IF("innodb_trx_duplicates", thr_get_trx(thr)->duplicates = TRX_DUP_REPLACE;);
  ...
  {
    row_merge_dup_t dup = {clust_index, table, clust_index->online_log->col_map, 0};

    error = row_log_table_apply_ops(thr, &dup, stage);

    ut_ad(error != DB_SUCCESS || clust_index->online_log->head.total == clust_index->online_log->tail.total);
  }
    ...
    dict_index_t *index) /*!< in: index, must be locked */
{
  ...
  ut_ad((rw_lock_own(dict_index_get_lock(index), RW_LOCK_S) && mutex_own(&index->online_log->mutex)) ||
...
static MY_ATTRIBUTE((nonnull)) void row_log_apply_op_low(
    /*=================*/
    dict_index_t *index,      /*!< in/out: index */
    row_merge_dup_t *dup,     /*!< in/out: for reporting
                              duplicate key errors */
    ...
    const dtuple_t *entry)    /*!< in: row */
{
  ...
  DBUG_PRINT("ib_create_index",
             ("%s %s index " IB_ID_FMT "," TRX_ID_FMT ": %s", op == ROW_OP_INSERT ? "insert" : "delete",
              has_index_lock ? "locked" : "unlocked", index->id, trx_id, rec_printer(entry).str().c_str()));
  ...
  scanned. */
  btr_cur_search_to_nth_level(index, 0, entry, PAGE_CUR_LE, has_index_lock ? BTR_MODIFY_TREE : BTR_MODIFY_LEAF, &cursor,
  ...
  if (cursor.low_match >= dict_index_get_n_unique(index) && !page_rec_is_infimum(btr_cur_get_rec(&cursor)))
  {
    ...
    switch (op)
    {
      case ROW_OP_DELETE:
        ...
        if (!has_index_lock)
        {
          ...
          mtr_start(&mtr);
          mtr.set_named_space(index->space);
          btr_cur_search_to_nth_level(index, 0, entry, PAGE_CUR_LE, BTR_MODIFY_TREE, &cursor, 0, __FILE__, __LINE__,
                                      &mtr);
        ...
        btr_cur_pessimistic_delete(error, FALSE, &cursor, BTR_CREATE_FLAG, false, &mtr);
        break;
      case ROW_OP_INSERT:
        if (exists)
        {
    ...
    switch (op)
    {
      ...
      case ROW_OP_INSERT:
        if (dict_index_is_unique(index) &&
            (cursor.up_match >= dict_index_get_n_unique(index) || cursor.low_match >= dict_index_get_n_unique(index)) &&
      ...
      insert_the_rec:
        ...
        *error =
            btr_cur_optimistic_insert(BTR_NO_UNDO_LOG_FLAG | BTR_NO_LOCKING_FLAG | BTR_CREATE_FLAG, &cursor, &offsets,
                                      &offsets_heap, const_cast<dtuple_t *>(entry), &rec, &big_rec, 0, NULL, &mtr);
        ut_ad(!big_rec);
        if (*error != DB_FAIL)
        {
          break;
        ...
        *error =
            btr_cur_pessimistic_insert(BTR_NO_UNDO_LOG_FLAG | BTR_NO_LOCKING_FLAG | BTR_CREATE_FLAG, &cursor, &offsets,
                                       &offsets_heap, const_cast<dtuple_t *>(entry), &rec, &big_rec, 0, NULL, &mtr);
        ut_ad(!big_rec);
        break;
    }
  ...
  {
    page_update_max_trx_id(btr_cur_get_block(&cursor), btr_cur_get_page_zip(&cursor), trx_id, &mtr);
  }
...
/******************************************************/ /**
 ...
 @return NULL on failure (mrec corruption) or when out of data;
 pointer to next record on success */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) const mrec_t *row_log_apply_op(
    /*=============*/
    dict_index_t *index,      /*!< in/out: index */
    row_merge_dup_t *dup,     /*!< in/out: for reporting
                              duplicate key errors */
...

{
  ...
  *error = DB_SUCCESS;
  ...
  entry = row_rec_to_index_entry_low(mrec - data_size, index, offsets, &n_ext, heap);
  /* Online index creation is only implemented for secondary
  indexes, which never contain off-page columns. */
  ut_ad(n_ext == 0);

  row_log_apply_op_low(index, dup, error, offsets_heap, has_index_lock, op, trx_id, entry);
  return (mrec);
}
...
of log that is applied.
@return DB_SUCCESS, or error code on failure */
static dberr_t row_log_apply_ops(const trx_t *trx, dict_index_t *index, row_merge_dup_t *dup, ut_stage_alter_t *stage)
{
  dberr_t error;
  const mrec_t *mrec = NULL;
  const mrec_t *next_mrec;
...
next_block:
  ...
  else
  {
    ...
    IORequest request(IORequest::READ | IORequest::ROW_LOG);
    dberr_t err = os_file_read_no_error_handling_int_fd(request, index->online_log->fd, index->online_log->head.block,
  ...
  if (mrec)
  {
    ...
    memcpy((mrec_t *)mrec_end, next_mrec, (&index->online_log->head.buf)[1] - mrec_end);
    mrec = row_log_apply_op(index, dup, &error, offsets_heap, heap, has_index_lock, index->online_log->head.buf,
  ...
  while (!trx_is_interrupted(trx))
  {
    ...
    next_mrec = row_log_apply_op(index, dup, &error, offsets_heap, heap, has_index_lock, mrec, mrec_end, offsets);
...
func_exit:
  ...
  switch (error)
  {
    ...
    case DB_INDEX_CORRUPT:
      if (((os_offset_t)index->online_log->tail.blocks + 1) * srv_sort_buf_size >= srv_online_max_size)
      {
...
stage->inc() will be called for each block of log that is applied.
@return DB_SUCCESS, or error code on failure */
dberr_t row_log_apply(const trx_t *trx, dict_index_t *index, struct TABLE *table, ut_stage_alter_t *stage)
{
  dberr_t error;
  row_log_t *log;
