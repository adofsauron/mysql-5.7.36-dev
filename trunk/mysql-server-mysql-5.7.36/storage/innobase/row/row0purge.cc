    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  {
    node->found_clust = row_search_on_row_ref(&node->pcur, mode, node->table, node->ref, mtr);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) bool row_purge_remove_clust_if_poss_low(
    /*===============================*/
    ...
    ulint mode)         /*!< in: BTR_MODIFY_LEAF or BTR_MODIFY_TREE */
{
  ...
  else
  {
    ...
    btr_cur_pessimistic_delete(&err, FALSE, btr_pcur_get_btr_cur(&node->pcur), 0, false, &mtr);
...
/***********************************************************/ /**
 ...
 of file space. */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) bool row_purge_remove_clust_if_poss(
    /*===========================*/
    purge_node_t *node) /*!< in/out: row purge node */
{
  ...
  {
    if (row_purge_remove_clust_if_poss_low(node, BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE))
    {
      return (true);
    }
    ...
    const dtuple_t *entry) /*!< in: secondary index entry */
{
  ...
  can_delete = !row_purge_reposition_pcur(BTR_SEARCH_LEAF, node, &mtr) ||
               !row_vers_old_has_index_entry(TRUE, btr_pcur_get_rec(&node->pcur), &mtr, index, entry, node->roll_ptr,
                                             node->trx_id);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) ibool row_purge_remove_sec_if_poss_tree(
    /*==============================*/
    purge_node_t *node,    /*!< in: row purge node */
    ...
    const dtuple_t *entry) /*!< in: index entry */
{
  ...
  search_result = row_search_index_entry(index, entry, BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE, &pcur, &mtr);
  ...
  if (row_purge_poss_sec(node, index, entry))
  {
    ...
    marked for deletion. */
    if (!rec_get_deleted_flag(btr_cur_get_rec(btr_cur), dict_table_is_comp(index->table)))
    {
      ib::error() << "tried to purge non-delete-marked record"
                     " in index "
                  << index->name << " of table " << index->table->name << ": tuple: " << *entry
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) bool row_purge_remove_sec_if_poss_leaf(
    /*==============================*/
    purge_node_t *node,    /*!< in: row purge node */
    dict_index_t *index,   /*!< in: index */
    const dtuple_t *entry) /*!< in: index entry */
{
  ...
  if (!index->is_committed())
  {
    ...
    mode = (dict_table_is_temporary(index->table)) ? BTR_MODIFY_LEAF | BTR_ALREADY_S_LATCHED
  ...
  else
  {
    ...
    /* Change buffering is disabled for temporary tables
    and spatial index. */
    mode = (dict_table_is_temporary(index->table) || dict_index_is_spatial(index)) ? BTR_MODIFY_LEAF
      ...
      if (row_purge_poss_sec(node, index, entry))
      {
        ...
        if (dict_index_is_spatial(index))
        {
          ...
          if (!lock_test_prdt_page_lock(trx, page_get_space_id(page), page_get_page_no(page)) &&
              page_get_n_recs(page) < 2 && page_get_page_no(page) != dict_index_get_page(index))
          {
...
static inline void row_purge_skip_uncommitted_virtual_index(dict_index_t *&index)
{
  ...
  while (index != NULL && dict_index_has_virtual(index) && !index->is_committed() && index->has_new_v_col)
  {
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) bool row_purge_del_mark(
    /*===============*/
    purge_node_t *node) /*!< in/out: row purge node */
{
  mem_heap_t *heap;
  ...
  while (node->index != NULL)
  {
    ...
    {
      dtuple_t *entry = row_build_index_entry_low(node->row, NULL, node->index, heap, ROW_BUILD_FOR_PURGE);
    ...
    trx_undo_rec_t *undo_rec) /*!< in: record to purge */
{
  ...
  if (node->rec_type == TRX_UNDO_UPD_DEL_REC || (node->cmpl_info & UPD_NODE_NO_ORD_CHANGE))
  {
  ...
  while (node->index != NULL)
  {
    ...
    if (row_upd_changes_ord_field_binary(node->index, node->update, thr, NULL, NULL))
    {
...
skip_secondaries:
  ...
  for (ulint i = 0; i < upd_get_n_fields(node->update); i++)
  {
    ...
    if (dfield_is_ext(&ufield->new_val))
    {
      ...
      internal_offset = ((const byte *)dfield_get_data(&ufield->new_val)) - undo_rec;
      ...
      bool is_redo_rseg = dict_table_is_temporary(node->table) ? false : true;
      rseg = trx_sys_get_nth_rseg(trx_sys, rseg_id, is_redo_rseg);
      ...
      block = buf_page_get(page_id_t(rseg->space, page_no), univ_page_size, RW_X_LATCH, &mtr);
      ...
      btr_free_externally_stored_field(index, data_field + dfield_get_len(&ufield->new_val) - BTR_EXTERN_FIELD_REF_SIZE,
...
#ifdef UNIV_DEBUG
#define row_purge_upd_exist_or_extern(thr, node, undo_rec) row_purge_upd_exist_or_extern_func(thr, node, undo_rec)
#else /* UNIV_DEBUG */
...
try_again:
  ...
  node->table = dict_table_open_on_id(table_id, FALSE, DICT_TABLE_OP_NORMAL);
  ...
  {
#if UNIV_DEBUG
    ib::info() << "Record with space id " << node->table->space << " belongs to table which is being truncated"
  ...
  if (node->table->n_v_cols && !node->table->vc_templ && dict_table_has_indexed_v_cols(node->table))
  {
  ...
  if (clust_index == NULL || dict_index_is_corrupted(clust_index))
  {
  ...
  if (type == TRX_UNDO_UPD_EXIST_REC && (node->cmpl_info & UPD_NODE_NO_ORD_CHANGE) && !*updated_extern)
  {
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) bool row_purge_record_func(
    /*==================*/
    purge_node_t *node,       /*!< in: row purge node */
    trx_undo_rec_t *undo_rec, /*!< in: record to purge */
#ifdef UNIV_DEBUG
...
#ifdef UNIV_DEBUG
#define row_purge_record(node, undo_rec, thr, updated_extern) row_purge_record_func(node, undo_rec, thr, updated_extern)
#else /* UNIV_DEBUG */
...
static MY_ATTRIBUTE((nonnull)) void row_purge(
    /*======*/
    purge_node_t *node,       /*!< in: row purge node */
    trx_undo_rec_t *undo_rec, /*!< in: record to purge */
    que_thr_t *thr)           /*!< in: query thread */
    ...
    que_thr_t *thr) /*!< in: query thread */
{
  ...
  if (!(node->undo_recs == NULL || ib_vector_is_empty(node->undo_recs)))
  {
    ...
    purge_rec = static_cast<trx_purge_rec_t *>(ib_vector_pop(node->undo_recs));
...
bool purge_node_t::validate_pcur()
{
  ...
  ulint *offsets = rec_get_offsets(pcur.old_rec, clust_index, NULL, pcur.old_n_fields, &heap);
