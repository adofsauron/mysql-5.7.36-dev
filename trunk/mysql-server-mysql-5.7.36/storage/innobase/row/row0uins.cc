...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_ins_remove_clust_rec(
    /*==========================*/
    undo_node_t *node) /*!< in: undo node */
{
  ...
  success =
      btr_pcur_restore_position(online ? BTR_MODIFY_LEAF | BTR_ALREADY_S_LATCHED : BTR_MODIFY_LEAF, &node->pcur, &mtr);
  ut_a(success);

  btr_cur = btr_pcur_get_btr_cur(&node->pcur);

  ut_ad(rec_get_trx_id(btr_cur_get_rec(btr_cur), btr_cur->index) == node->trx->id);
  ut_ad(!rec_get_deleted_flag(btr_cur_get_rec(btr_cur), dict_table_is_comp(btr_cur->index->table)));

  if (online && dict_index_is_online_ddl(index))
  {
    const rec_t *rec = btr_cur_get_rec(btr_cur);
    mem_heap_t *heap = NULL;
    const ulint *offsets = rec_get_offsets(rec, index, NULL, ULINT_UNDEFINED, &heap);
  ...
  if (node->table->id == DICT_INDEXES_ID)
  {
    ...
    dict_drop_index_tree(btr_pcur_get_rec(&node->pcur), &(node->pcur), &mtr);
...
retry:
  ...
  success = btr_pcur_restore_position(BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE, &node->pcur, &mtr);
  ut_a(success);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_ins_remove_sec_low(
    /*========================*/
    ...
    undo_node_t *node)   /*!< in: undo node */
{
  ...
  if (search_result == ROW_FOUND && dict_index_is_spatial(index))
  {
    if (rec_deleted)
    {
      ib::error() << "Record found in index " << index->name << " is deleted marked on insert rollback.";
    }
  }
...
/***************************************************************/ /**
 ...
 @return DB_SUCCESS or DB_OUT_OF_FILE_SPACE */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_ins_remove_sec(
    /*====================*/
    dict_index_t *index, /*!< in: index */
...
retry:
  err = row_undo_ins_remove_sec_low(BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE, index, entry, thr, node);
    ...
    ibool dict_locked) /*!< in: TRUE if own dict_sys->mutex */
{
  ...
  else
  {
    ...
    if (clust_index != NULL)
    {
      ...
      {
        trx_undo_read_v_cols(node->table, ptr, node->row, false, NULL);
      }
    }
    else
    {
...
/***************************************************************/ /**
 ...
 @return DB_SUCCESS or DB_OUT_OF_FILE_SPACE */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_ins_remove_sec_rec(
    /*========================*/
    undo_node_t *node, /*!< in/out: row undo node */
    que_thr_t *thr)    /*!< in: query thread */
{
    ...
    que_thr_t *thr)    /*!< in: query thread */
{
  ...
  if (err == DB_SUCCESS)
  {
    ...
    if (node->table->id == DICT_INDEXES_ID && !dict_locked)
    {
