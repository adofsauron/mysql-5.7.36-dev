...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_clust_low(
    /*===================*/
    ...
    ulint mode)     /*!< in: BTR_MODIFY_LEAF or BTR_MODIFY_TREE */
{
  ...
  ut_ad(success);
  ut_ad(rec_get_trx_id(btr_cur_get_rec(btr_cur), btr_cur_get_index(btr_cur)) == thr_get_trx(thr)->id);

  if (mode != BTR_MODIFY_LEAF && dict_index_is_online_ddl(btr_cur_get_index(btr_cur)))
  {
    *rebuilt_old_pk = row_log_table_get_pk(btr_cur_get_rec(btr_cur), btr_cur_get_index(btr_cur), NULL, sys, &heap);
  }
  else
  {
    *rebuilt_old_pk = NULL;
  ...
  if (mode != BTR_MODIFY_TREE)
  {
    ...
    err = btr_cur_optimistic_update(BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG | BTR_KEEP_SYS_FLAG, btr_cur, offsets,
                                    offsets_heap, node->update, node->cmpl_info, thr, thr_get_trx(thr)->id, mtr);
  }
  else
  {
    big_rec_t *dummy_big_rec;

    err = btr_cur_pessimistic_update(BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG | BTR_KEEP_SYS_FLAG, btr_cur, offsets,
                                     offsets_heap, heap, &dummy_big_rec, node->update, node->cmpl_info, thr,
...
/***********************************************************/ /**
 ...
 @return	DB_SUCCESS, DB_FAIL, or error code: we may run out of file space */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_remove_clust_low(
    /*==========================*/
    undo_node_t *node, /*!< in: row undo node */
    ...
    ulint mode)        /*!< in: BTR_MODIFY_LEAF or BTR_MODIFY_TREE */
{
  ...
  if (!trx_id_offset)
  {
    ...
    trx_id_col = dict_index_get_sys_col_pos(btr_cur_get_index(btr_cur), DATA_TRX_ID);
    ut_ad(trx_id_col > 0);
    ut_ad(trx_id_col != ULINT_UNDEFINED);

    offsets = rec_get_offsets(btr_cur_get_rec(btr_cur), btr_cur_get_index(btr_cur), NULL, trx_id_col + 1, &heap);
  ...
  if (trx_read_trx_id(btr_cur_get_rec(btr_cur) + trx_id_offset) != node->new_trx_id)
  {
  ...
  ut_ad(rec_get_deleted_flag(btr_cur_get_rec(btr_cur), dict_table_is_comp(node->table)));
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_clust(
    /*===============*/
    undo_node_t *node, /*!< in: row undo node */
    que_thr_t *thr)    /*!< in: query thread */
{
  ...
  ut_ad(rw_lock_own(dict_operation_lock, RW_LOCK_S) || rw_lock_own(dict_operation_lock, RW_LOCK_X));
  ...
  err = row_undo_mod_clust_low(node, &offsets, &offsets_heap, heap, &rebuilt_old_pk, sys, thr, &mtr,
  ...
  if (err != DB_SUCCESS)
  {
    ...
    err = row_undo_mod_clust_low(node, &offsets, &offsets_heap, heap, &rebuilt_old_pk, sys, thr, &mtr, BTR_MODIFY_TREE);
  ...
  if (err == DB_SUCCESS && online)
  {
    ...
    switch (node->rec_type)
    {
      ...
      case TRX_UNDO_UPD_EXIST_REC:
        row_log_table_update(btr_pcur_get_rec(pcur), index, offsets, rebuilt_old_pk, node->undo_row, node->row);
        break;
      case TRX_UNDO_UPD_DEL_REC:
        row_log_table_delete(btr_pcur_get_rec(pcur), node->row, index, offsets, sys);
        break;
      default:
        ut_ad(0);
        break;
  ...
  if (err == DB_SUCCESS && node->rec_type == TRX_UNDO_UPD_DEL_REC)
  {
    ...
    if (err != DB_SUCCESS)
    {
      ...
      err = row_undo_mod_remove_clust_low(node, &mtr, BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_del_mark_or_remove_sec_low(
    /*====================================*/
    undo_node_t *node,   /*!< in: row undo node */
    que_thr_t *thr,      /*!< in: query thread */
                         ...
                         BTR_MODIFY_TREE */
{
  ...
  old_has = row_vers_old_has_index_entry(FALSE, btr_pcur_get_rec(&(node->pcur)), &mtr_vers, index, entry, 0, 0);
  ...
  purgeable.*/
  rec_deleted = rec_get_deleted_flag(btr_pcur_get_rec(&pcur), dict_table_is_comp(index->table));
  if (rec_deleted == 0)
  {
    row_convert_impl_to_expl_if_needed(btr_cur, node);
  }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_del_mark_or_remove_sec(
    /*================================*/
    undo_node_t *node,   /*!< in: row undo node */
    que_thr_t *thr,      /*!< in: query thread */
    dict_index_t *index, /*!< in: index */
    dtuple_t *entry)     /*!< in: index entry */
{
  ...
  err = row_undo_mod_del_mark_or_remove_sec_low(node, thr, index, entry, BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE);
  return (err);
}
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_del_unmark_sec_and_undo_update(
    /*========================================*/
  ...
  switch (search_result)
  {
    ...
    case ROW_NOT_FOUND:
      ...
      if (index->is_committed())
      {
        ...
        ib::warn() << "Record in index " << index->name << " of table " << index->table->name
                   << " was not found on rollback, trying to"
                      " insert: "
                   << *entry << " at: " << rec_index_print(btr_cur_get_rec(btr_cur), index);
      }

      if (btr_cur->up_match >= dict_index_get_n_unique(index) || btr_cur->low_match >= dict_index_get_n_unique(index))
      {
        if (index->is_committed())
        {
      ...
      err = btr_cur_optimistic_insert(flags, btr_cur, &offsets, &offsets_heap, entry, &insert_rec, &big_rec, 0, thr,
                                      &mtr);
      ut_ad(!big_rec);

      if (err == DB_FAIL && mode == BTR_MODIFY_TREE)
      {
        err = btr_cur_pessimistic_insert(flags, btr_cur, &offsets, &offsets_heap, entry, &insert_rec, &big_rec, 0, thr,
                                         &mtr);
      ...
      if (err == DB_SUCCESS)
      {
        page_update_max_trx_id(btr_cur_get_block(btr_cur), btr_cur_get_page_zip(btr_cur), trx->id, &mtr);
      }
    ...
    case ROW_FOUND:
      ...
      ut_a(err == DB_SUCCESS);
      heap = mem_heap_create(sizeof(upd_t) + dtuple_get_n_fields(entry) * sizeof(upd_field_t));
      offsets_heap = NULL;
      offsets = rec_get_offsets(btr_cur_get_rec(btr_cur), index, NULL, ULINT_UNDEFINED, &offsets_heap);
      update = row_upd_build_sec_rec_difference_binary(btr_cur_get_rec(btr_cur), index, offsets, entry, heap);
      ...
      else if (mode != BTR_MODIFY_TREE)
      {
        ...
        err = btr_cur_optimistic_update(flags, btr_cur, &offsets, &offsets_heap, update, 0, thr, thr_get_trx(thr)->id,
                                        &mtr);
        switch (err)
        {
          case DB_OVERFLOW:
      ...
      {
        err = btr_cur_pessimistic_update(flags, btr_cur, &offsets, &offsets_heap, heap, &dummy_big_rec, update, 0, thr,
                                         thr_get_trx(thr)->id, &mtr);
        ut_a(!dummy_big_rec);
      }
...
static MY_ATTRIBUTE((nonnull)) void row_undo_mod_sec_flag_corrupted(
    /*============================*/
...
/***********************************************************/ /**
 ...
 @return DB_SUCCESS or DB_OUT_OF_FILE_SPACE */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_upd_del_sec(
    /*=====================*/
    undo_node_t *node, /*!< in: row undo node */
    que_thr_t *thr)    /*!< in: query thread */
{
...
/***********************************************************/ /**
 ...
 @return DB_SUCCESS or DB_OUT_OF_FILE_SPACE */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_del_mark_sec(
    /*======================*/
    undo_node_t *node, /*!< in: row undo node */
    que_thr_t *thr)    /*!< in: query thread */
{
...
/***********************************************************/ /**
 ...
 @return DB_SUCCESS or DB_OUT_OF_FILE_SPACE */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_undo_mod_upd_exist_sec(
    /*=======================*/
    undo_node_t *node, /*!< in: row undo node */
    que_thr_t *thr)    /*!< in: query thread */
{
  ...
  while (node->index != NULL)
  {
    ...
    {
      if (index->type == DICT_FTS || !row_upd_changes_ord_field_binary(index, node->update, thr, node->row, node->ext))
      {
        dict_table_next_uncorrupted_index(node->index);
        continue;
      }
    ...
    if (UNIV_UNLIKELY(!entry))
    {
      ...
      ut_a(dict_table_get_format(index->table) >= UNIV_FORMAT_B);
    ...
    if (dict_index_is_spatial(index))
    {
      entry = row_build_index_entry_low(node->undo_row, node->undo_ext, index, heap, ROW_BUILD_FOR_UNDO);
    }
    else
    {
...
static MY_ATTRIBUTE((nonnull)) void row_undo_mod_parse_undo_rec(
    /*========================*/
    undo_node_t *node, /*!< in: row undo node */
    ibool dict_locked) /*!< in: TRUE if own dict_sys->mutex */
{
  ...
  node->table = dict_table_open_on_id(table_id, dict_locked, DICT_TABLE_OP_NORMAL);
  ...
  if (node->table->ibd_file_missing || fil_space_is_being_truncated(node->table->space))
  {
  ...
  ptr = trx_undo_update_rec_get_update(ptr, clust_index, type, trx_id, roll_ptr, info_bits, node->trx, node->heap,
                                       &(node->update));
  ...
  {
    row_upd_replace_vcol(node->row, node->table, node->update, false, node->undo_row,
