...
#define ROW_MERGE_READ_GET_NEXT(N)                                                                    \
  do                                                                                                  \
  {                                                                                                   \
    b[N] = row_merge_read_rec(block[N], buf[N], b[N], index, fd[N], &foffs[N], &mrec[N], offsets[N]); \
...
store Doc ID during sort */
{
  ...
  new_index = dict_mem_index_create(index->table->name.m_name, "tmp_fts_idx", 0, DICT_FTS, 3);
  ...
  field->name = NULL;
  field->prefix_len = 0;
  field->col = static_cast<dict_col_t *>(mem_heap_alloc(new_index->heap, sizeof(dict_col_t)));
                         ...
                         to be instantiated */
{
  ...
  *psort = psort_info = static_cast<fts_psort_t *>(ut_zalloc_nokey(fts_sort_pll_degree * sizeof *psort_info));
  ...
  common_info = static_cast<fts_psort_common_t *>(ut_malloc_nokey(sizeof *common_info));
  ...
  for (j = 0; j < fts_sort_pll_degree; j++)
  {
    ...
    {
      psort_info[j].merge_file[i] = static_cast<merge_file_t *>(ut_zalloc_nokey(sizeof(merge_file_t)));
      ...
      psort_info[j].block_alloc[i] = static_cast<row_merge_block_t *>(ut_malloc_nokey(block_size + 1024));

      psort_info[j].merge_block[i] = static_cast<row_merge_block_t *>(ut_align(psort_info[j].block_alloc[i], 1024));
  ...
  *merge = merge_info = static_cast<fts_psort_t *>(ut_malloc_nokey(FTS_NUM_AUX_INDEX * sizeof *merge_info));
    ...
    MYSQL_FTPARSER_BOOLEAN_INFO *boolean_info) /* in: boolean info */
{
  ...
  ptr = static_cast<byte *>(ut_malloc_nokey(sizeof(row_fts_token_t) + sizeof(fts_string_t) + str.f_len));
  fts_token = reinterpret_cast<row_fts_token_t *>(ptr);
  fts_token->text = reinterpret_cast<fts_string_t *>(ptr + sizeof(row_fts_token_t));
  fts_token->text->f_str = static_cast<byte *>(ptr + sizeof(row_fts_token_t) + sizeof(fts_string_t));
    ...
    fts_tokenize_ctx_t *t_ctx)  /*!< in/out: tokenize context */
{
  ...
  while (t_ctx->processed_len < doc->text.f_len)
  {
    ...
    {
      inc = innobase_mysql_fts_get_token(doc->charset, doc->text.f_str + t_ctx->processed_len,
    ...
    t_str.f_len =
        innobase_fts_casedn_str(doc->charset, (char *)str.f_str, str.f_len, (char *)&str_buf, FTS_MAX_WORD_LEN + 1);
    ...
    field = mtuple->fields = static_cast<dfield_t *>(mem_heap_alloc(buf->heap, FTS_NUM_FIELDS_SORT * sizeof *field));
    ...
    if (parser != NULL)
    {
      mach_write_to_4(reinterpret_cast<byte *>(&position), (fts_token->position + t_ctx->init_pos));
    }
    else
    {
      mach_write_to_4(reinterpret_cast<byte *>(&position), (t_ctx->processed_len + inc - str.f_len + t_ctx->init_pos));
    }
    ...
    fts_doc_item_t **doc_item) /*!< in/out: doc item */
{
  ...
  *doc_item = UT_LIST_GET_FIRST(psort_info->fts_doc_list);
  if (*doc_item != NULL)
  {
    ...
    ut_ad(psort_info->memory_used >= sizeof(fts_doc_item_t) + (*doc_item)->field->len);
...
os_thread_ret_t fts_parallel_tokenization(
    /*======================*/
    void *arg) /*!< in: psort_info for the thread */
{
  ...
  const char *path = thd_innodb_tmpdir(psort_info->psort_common->trx->mysql_thd);
  ...
  idx_field = dict_index_get_nth_field(psort_info->psort_common->dup->index, 0);
  word_dtype.prtype = idx_field->col->prtype;
  word_dtype.mbminmaxlen = idx_field->col->mbminmaxlen;
  word_dtype.mtype = (strcmp(doc.charset->name, "latin1_swedish_ci") == 0) ? DATA_VARCHAR : DATA_VARMYSQL;
...
loop:
  while (doc_item)
  {
    ...
    if (processed)
    {
      ...
      {
        doc.text.f_str = btr_copy_externally_stored_field(&doc.text.f_len, data, page_size, data_len, blob_heap);
      }
      else
      {
        doc.text.f_str = data;
    ...
    processed = row_merge_fts_doc_tokenize(buf, doc_item->doc_id, &doc, &word_dtype, merge_file,
    ...
    if (fts_enable_diag_print && num_doc_processed % 10000 == 1)
    {
      ...
      for (i = 0; i < FTS_NUM_AUX_INDEX; i++)
      {
        ib::info() << "ID " << psort_info->psort_id << ", partition " << i << ", word " << mycount[i];
      }
#endif
    }
  ...
  if (t_ctx.rows_added[t_ctx.buf_used] && !processed)
  {
    ...
    row_merge_buf_write(buf[t_ctx.buf_used], merge_file[t_ctx.buf_used], block[t_ctx.buf_used]);

    if (!row_merge_write(merge_file[t_ctx.buf_used]->fd, merge_file[t_ctx.buf_used]->offset++, block[t_ctx.buf_used]))
    {
  ...
  if (psort_info->state == FTS_PARENT_COMPLETE)
  {
    ...
    else if (retried > 10000)
    {
      ...
      ib::error() << "FTS parallel sort processed " << num_doc_processed << " records, the sort queue has "
...
exit:
  ...
  for (i = 0; i < FTS_NUM_AUX_INDEX; i++)
  {
    if (t_ctx.rows_added[i])
    {
      ...
      {
        if (!row_merge_write(merge_file[i]->fd, merge_file[i]->offset++, block[i]))
        {
          error = DB_TEMP_FILE_WRITE_FAIL;
          goto func_exit;
        }
  ...
  for (i = 0; i < FTS_NUM_AUX_INDEX; i++)
  {
    ...
    error = row_merge_sort(psort_info->psort_common->trx, psort_info->psort_common->dup, merge_file[i], block[i],
                           &tmpfd[i]);
    if (error != DB_SUCCESS)
    {
      close(tmpfd[i]);
    ...
    fts_psort_t *psort_info) /*!< parallel sort structure */
{
  ...
  for (i = 0; i < fts_sort_pll_degree; i++)
  {
    ...
    os_thread_create(fts_parallel_tokenization, (void *)&psort_info[i], &thd_id);
  }
}
...
/*********************************************************************/ /**
 ...
 @return OS_THREAD_DUMMY_RETURN */
os_thread_ret_t fts_parallel_merge(
    /*===============*/
    void *arg) /*!< in: parallel merge info */
{
  ...
  row_fts_merge_insert(psort_info->psort_common->dup->index, psort_info->psort_common->new_table,
    ...
    fts_psort_t *merge_info) /*!< in: parallel sort info */
{
  ...
  for (i = 0; i < FTS_NUM_AUX_INDEX; i++)
  {
    ...
    os_thread_create(fts_parallel_merge, (void *)&merge_info[i], &merge_info[i].thread_hdl);
  }
}
...
static dberr_t row_merge_write_fts_node(const fts_psort_insert_t *ins_ctx, const fts_string_t *word,
...
static MY_ATTRIBUTE((nonnull)) dberr_t row_merge_write_fts_word(
    /*=====================*/
                                 ...
                                 word */
{
  ...
  ut_ad(ins_ctx->aux_index_id == fts_select_index(ins_ctx->charset, word->text.f_str, word->text.f_len));
    ...
    dtuple_t *dtuple)            /*!< in: entry to insert */
{
  ...
  {
    doc_id = (doc_id_t)mach_read_from_4(static_cast<byte *>(dfield_get_data(dfield)));
  }
    ...
    dict_index_t *index) /*<! in/out: FTS index */
{
  ...
  else if (cmp_rec_rec_simple(mrec[child_left], mrec[child_right], offsets[child_left], offsets[child_right], index,
                              NULL) < 0)
  {
    selected = child_left;
  }
    ...
    dict_index_t *index) /*<! in: index dictionary */
{
  ...
  {
    propagated =
        static_cast<ulint>(row_fts_sel_tree_propagate(static_cast<int>(propagated), sel_tree, mrec, offsets, index));
  }
    ...
    dict_index_t *index) /*<! in: index dictionary */
{
  ...
  for (i = 0; i < num_item; i++)
  {
    ...
    int cmp =
        cmp_rec_rec_simple(mrec[child_left], mrec[child_right], offsets[child_left], offsets[child_right], index, NULL);
    ...
    dict_index_t *index) /*<! in: index dictionary */
{
  ...
  {
    row_fts_build_sel_tree_level(sel_tree, static_cast<ulint>(i), mrec, offsets, index);
  }
                             ...
                             to insert to */
{
  ...
  offsets = (ulint **)mem_heap_alloc(heap, sizeof(*offsets) * fts_sort_pll_degree);
  ...
  mrec = (const mrec_t **)mem_heap_alloc(heap, sizeof(*mrec) * fts_sort_pll_degree);
  sel_tree = (int *)mem_heap_alloc(heap, sizeof(*sel_tree) * (fts_sort_pll_degree * 2));
  ...
  for (i = 0; i < fts_sort_pll_degree; i++)
  {
    ...
    offsets[i] = static_cast<ulint *>(mem_heap_zalloc(heap, num * sizeof *offsets[i]));
    offsets[i][0] = num;
  ...
  DBUG_EXECUTE_IF("innodb_test_wrong_fts_aux_table_name", index->table->flags2 &= ~DICT_TF2_FTS_AUX_HEX_NAME;);
  ...
  aux_table = dict_table_open_on_name(aux_table_name, FALSE, FALSE, DICT_ERR_IGNORE_NONE);
  ...
  for (i = 0; i < fts_sort_pll_degree; i++)
  {
    ...
    else
    {
      ...
      if (psort_info[i].merge_file[id]->offset > 0 && (!row_merge_read(fd[i], foffs[i], (row_merge_block_t *)block[i])))
      {
        error = DB_CORRUPTION;
        goto exit;
      }
  ...
  height = row_fts_build_sel_tree(sel_tree, (const mrec_t **)mrec, offsets, index);
  ...
  for (;;)
  {
    ...
    if (fts_sort_pll_degree <= 2)
    {
      while (!mrec[min_rec])
      {
        ...
        {
          row_fts_insert_tuple(&ins_ctx, &new_word, positions, &last_doc_id, NULL);
      ...
      for (i = min_rec + 1; i < fts_sort_pll_degree; i++)
      {
        ...
        if (cmp_rec_rec_simple(mrec[i], mrec[min_rec], offsets[i], offsets[min_rec], index, NULL) < 0)
        {
    ...
    dtuple = row_rec_to_index_entry_low(mrec[min_rec], index, offsets[min_rec], &n_ext, tuple_heap);
    ...
    if (fts_sort_pll_degree > 2)
    {
      ...
      row_fts_sel_tree_update(sel_tree, start + min_rec, height, mrec, offsets, index);
    }
