    ...
    trx_t *trx)          /*!< in: transaction */
{
  ...
  dict_foreign_set::iterator it =
      std::find_if(table->referenced_set.begin(), table->referenced_set.end(), dict_foreign_with_index(index));
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_upd_check_references_constraints(
    /*=================================*/
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  DEBUG_SYNC_C_IF_THD(thr_get_trx(thr)->mysql_thd, "foreign_constraint_check_for_insert");
  ...
  for (dict_foreign_set::iterator it = table->referenced_set.begin(); it != table->referenced_set.end(); ++it)
  {
        ...
        (node->is_delete || row_upd_changes_first_fields_binary(entry, index, node->update, foreign->n_fields)))
    {
      ...
      {
        ref_table = dict_table_open_on_name(foreign->foreign_table_name_lookup, FALSE, FALSE, DICT_ERR_IGNORE_NONE);
      }
      ...
      if (foreign_table != NULL &&
          (dict_table_is_discarded(foreign_table) || fil_space_is_being_truncated(foreign_table->space)))
      {
        continue;
      }
    ...
    roll_ptr_t roll_ptr)      /*!< in: roll ptr of the undo log record */
{
  ...
  if (page_zip)
  {
    page_zip_write_trx_id_and_roll_ptr(page_zip, rec, offsets, pos, trx_id, roll_ptr);
  }
  else
  {
    byte *field;
    ...
    const upd_t *update)  /*!< in: update vector */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    a virtual index */
    if (upd_fld_is_virtual_col(upd_field) && dict_index_has_virtual(index) != DICT_VIRTUAL)
    {
      continue;
    }
    ...
    const upd_t *update) /*!< in: update vector */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    field_ref = static_cast<const byte *>(dfield_get_data(new_val)) + new_len - BTR_EXTERN_FIELD_REF_SIZE;
...
#error "MLOG_BUF_MARGIN <= 30"
    ...
    field_no */
    ulint field_no = upd_fld_is_virtual_col(upd_field) ? REC_MAX_N_FIELDS + upd_field->field_no : upd_field->field_no;
    ...
    if (len != UNIV_SQL_NULL)
    {
      ...
      else
      {
        ...
        mlog_catenate_string(mtr, static_cast<byte *>(dfield_get_data(new_val)), len);
...
upd_t *row_upd_build_difference_binary(dict_index_t *index, const dtuple_t *entry, const rec_t *rec,
                                       ...
                                       TABLE *mysql_table, dberr_t *error)
{
  ...
  ut_ad(dict_table_is_intrinsic(index->table) || (dict_index_get_sys_col_pos(index, DATA_ROLL_PTR) == trx_id_pos + 1));
  ...
  for (i = 0; i < n_fld; i++)
  {
    ...
    if (no_sys)
    {
      ...
      /* DB_ROLL_PTR */
      if (i == trx_id_pos + 1 && !dict_table_is_intrinsic(index->table))
      {
        continue;
      }
    }

    if (!dfield_is_ext(dfield) != !rec_offs_nth_extern(offsets, i) || !dfield_data_is_binary_equal(dfield, len, data))
    {
      upd_field = upd_get_nth_field(update, n_diff);
  ...
  if (n_v_fld > 0)
  {
    ...
    for (i = 0; i < n_v_fld; i++)
    {
      ...
      {
        update->old_vrow = row_build(ROW_COPY_POINTERS, index, rec, offsets, index->table, NULL, NULL, &ext, heap);
      }

      dfield = dtuple_get_nth_v_field(entry, i);

      dfield_t *vfield = innobase_get_computed_value(update->old_vrow, col, index, &v_heap, heap, NULL, thd,
                                                     mysql_table, NULL, NULL, NULL);
      if (vfield == NULL)
      {
      ...
      if (!dfield_data_is_binary_equal(dfield, vfield->len, static_cast<byte *>(vfield->data)))
      {
        upd_field = upd_get_nth_field(update, n_diff);

        upd_field->old_v_val = static_cast<dfield_t *>(mem_heap_alloc(heap, sizeof *upd_field->old_v_val));
...
@return BLOB prefix */
static byte *row_upd_ext_fetch(const byte *data, ulint local_len, const page_size_t &page_size, ulint *len,
                               mem_heap_t *heap)
{
  byte *buf = static_cast<byte *>(mem_heap_alloc(heap, *len));

  *len = btr_copy_externally_stored_field_prefix(buf, *len, page_size, data, local_len);
...
static void row_upd_index_replace_new_col_val(dfield_t *dfield, const dict_field_t *field, const dict_col_t *col,
                                              const upd_field_t *uf, mem_heap_t *heap, const page_size_t &page_size)
{
  ulint len;
  const byte *data;
  ...
  if (field->prefix_len > 0)
  {
    ...
    if (fetch_ext)
    {
      ...
      data = row_upd_ext_fetch(data, l, page_size, &len, heap);
    }

    len = dtype_get_at_most_n_mbchars(col->prtype, col->mbminmaxlen, field->prefix_len, len, (const char *)data);
  ...
  switch (uf->orig_len)
  {
    ...
    default:
      ...
      memcpy(buf + uf->orig_len - BTR_EXTERN_FIELD_REF_SIZE, data + len - BTR_EXTERN_FIELD_REF_SIZE,
                      ...
                      copying the new values */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    if (uf)
    {
      row_upd_index_replace_new_col_val(dtuple_get_nth_field(entry, i), field, col, uf, heap, page_size);
    }
  }
}
                         ...
                         copying the new values */
{
  ...
  for (i = 0; i < dict_index_get_n_fields(index); i++)
  {
    ...
    {
      uf = upd_get_field_by_field_no(update, dict_col_get_clust_pos(col, clust_index), false);
    }
...
void row_upd_replace_vcol(dtuple_t *row, const dict_table_t *table, const upd_t *update, bool upd_new,
                          dtuple_t *undo_row, const byte *ptr)
{
  ...
  for (col_no = 0; col_no < n_cols; col_no++)
  {
    ...
    if (!col->m_col.ord_part)
    {
      ...
      in new table */
      if (!dict_index_is_online_ddl(clust_index) || !row_log_col_is_indexed(clust_index, col_no))
      {
        continue;
      }
    }
    ...
    for (i = 0; i < upd_get_n_fields(update); i++)
    {
      ...
      if (!upd_fld_is_virtual_col(upd_field) || upd_field->field_no != col->v_pos)
      {
        continue;
      }
      ...
      dfield_get_type(dfield)->mbminmaxlen = upd_field->new_val.type.mbminmaxlen;
      break;
    }
  }
  ...
  if (ptr != NULL)
  {
    ...
    while (ptr != end_ptr)
    {
      ...
      if (is_v)
      {
        ptr = trx_undo_read_v_idx(table, ptr, first_v_col, &is_undo_log, &field_no);
        first_v_col = false;
      }
    ...
    mem_heap_t *heap)          /*!< in: memory heap */
{
  ...
  for (col_no = 0; col_no < n_cols; col_no++)
  {
    ...
    for (i = 0; i < upd_get_n_fields(update); i++)
    {
      ...
      if (upd_field->field_no != clust_pos || upd_fld_is_virtual_col(upd_field))
      {
        continue;
      }
  ...
  if (n_ext_cols)
  {
    *ext = row_ext_create(n_ext_cols, ext_cols, table->flags, row, heap);
  }
  else
  {
    *ext = NULL;
                          ...
                          ROW_BUILD_FOR_PURGE or ROW_BUILD_FOR_UNDO */
{
  ...
  for (i = 0; i < n_unique; i++)
  {
    ...
    {
      upd_field = upd_get_field_by_field_no(update, dict_col_get_clust_pos(col, clust_index), false);
    }
    ...
    if (dict_index_is_spatial(index) && i == 0)
    {
      ...
      const page_size_t page_size = (ext != NULL) ? ext->page_size : dict_table_page_size(index->table);
      ...
      if (dfield_is_ext(dfield))
      {
        ...
        dptr = btr_copy_externally_stored_field(&dlen, dptr, page_size, flen, temp_heap);
      }
      else
      {
      ...
      rtree_mbr_from_wkb(dptr + GEO_DATA_HEADER_SIZE, static_cast<uint>(dlen - GEO_DATA_HEADER_SIZE), SPDIMS, mbr1);
      old_mbr = reinterpret_cast<rtr_mbr_t *>(mbr1);

      /* Get the new mbr. */
      if (dfield_is_ext(new_field))
      {
        if (flag == ROW_BUILD_FOR_UNDO && dict_table_get_format(index->table) >= UNIV_FORMAT_B)
        {
          ...
          dptr =
              static_cast<byte *>(dfield_get_data(new_field)) + dfield_get_len(new_field) - BTR_EXTERN_FIELD_REF_SIZE;
        }
        else
        {
      ...
      rtree_mbr_from_wkb(dptr + GEO_DATA_HEADER_SIZE, static_cast<uint>(dlen - GEO_DATA_HEADER_SIZE), SPDIMS, mbr2);
    ...
    else if (dfield_is_ext(dfield))
    {
      ...
      ut_a(dict_index_is_clust(index) || ind_field->prefix_len <= dfield_len);
    ...
    const upd_t *update)       /*!< in: update vector for the row */
{
  ...
  for (i = 0; i < upd_get_n_fields(update); i++)
  {
    ...
    {
      if (dict_table_get_nth_v_col(index->table, upd_field->field_no)->m_col.ord_part)
      {
        return (TRUE);
      }
    }
  ...
  for (i = 0; i < n; i++)
  {
    ...
    for (j = 0; j < n_upd_fields; j++)
    {
      ...
      if (col_pos == upd_field->field_no &&
          !dfield_datas_are_binary_equal(dtuple_get_nth_field(entry, i), &upd_field->new_val, 0))
      {
        return (TRUE);
      }
    }
...
static void row_upd_store_v_row(upd_node_t *node, const upd_t *update, THD *thd, TABLE *mysql_table)
{
  ...
  for (ulint col_no = 0; col_no < dict_table_get_n_v_cols(node->table); col_no++)
  {
    ...
    if (col->m_col.ord_part)
    {
      ...
      {
        const upd_field_t *upd_field = upd_get_nth_field(update, i);
        if (!(upd_field->new_val.type.prtype & DATA_VIRTUAL) || upd_field->field_no != col->v_pos)
        {
          continue;
        }
      ...
      if (i >= n_upd)
      {
        ...
        if (update)
        {
          ...
          else
          {
            ...
            if (dfield_is_null(dfield))
            {
              innobase_get_computed_value(node->row, col, index, &heap, node->heap, NULL, thd, mysql_table, NULL, NULL,
                                          NULL);
            }
          }
        }
...
void row_upd_store_row(upd_node_t *node, THD *thd, TABLE *mysql_table)
{
  ...
  {
    row_upd_store_v_row(node, node->is_delete ? NULL : node->update, thd, mysql_table);
  }
    ...
    que_thr_t *thr)   /*!< in: query thread */
{
  ...
  if (!index->is_committed())
  {
    ...
    spatial index. */
    mode = (referenced || dict_table_is_temporary(index->table) || dict_index_is_spatial(index))
  ...
  switch (search_result)
  {
    ...
    case ROW_NOT_FOUND:
      ...
      ib::error() << "Record in index " << index->name << " of table " << index->table->name
                  << " was not found on update: " << *entry << " at: " << rec_index_print(rec, index);
    ...
    const upd_t *update)  /*!< in: update vector */
{
  ...
  for (i = 0; i < dtuple_get_n_fields(entry); i++)
  {
    ...
    ut_ad(!offsets || !rec_offs_nth_extern(offsets, i) == !dfield_is_ext(dfield) ||
    ...
    if (UNIV_LIKELY(rec != NULL))
    {
      ...
      ut_ad(memcmp(rec_data, field_ref_zero, BTR_EXTERN_FIELD_REF_SIZE));
      /* The BLOB must be owned. */
      ut_ad(!(rec_data[BTR_EXTERN_LEN] & BTR_EXTERN_OWNER_FLAG));
    }
#endif /* UNIV_DEBUG */
    ...
    ut_a(rec == NULL || !(data[BTR_EXTERN_LEN] & BTR_EXTERN_OWNER_FLAG));
    ...
    mtr_t *mtr)          /*!< in/out: mtr; gets committed here */
{
  ...
  entry = row_build_index_entry_low(node->upd_row, node->upd_ext, index, heap, ROW_BUILD_FOR_INSERT);
  ...
  switch (node->state)
  {
    ...
    case UPD_NODE_UPDATE_CLUSTERED:
      ...
      if (rec_get_deleted_flag(rec, rec_offs_comp(offsets)))
      {
        ...
        ut_ad(!trx_undo_roll_ptr_is_insert(row_get_rec_roll_ptr(rec, index, offsets)));
        goto check_fk;
      }

      err = btr_cur_del_mark_set_clust_rec(flags, btr_cur_get_block(btr_cur), rec, index, offsets, thr, node->row, mtr);
      if (err != DB_SUCCESS)
      {
      err_exit:
        mtr_commit(mtr);
      ...
      {
        if (row_upd_clust_rec_by_insert_inherit(rec, offsets, entry, node->update))
        {
          ...
          btr_cur_disown_inherited_fields(btr_cur_get_page_zip(btr_cur), rec, index, offsets, node->update, mtr);
        }
      }
    check_fk:
      if (referenced)
    ...
    mtr_t *mtr)     /*!< in: mtr; gets committed here */
{
  ...
  ut_ad(!rec_get_deleted_flag(btr_cur_get_rec(btr_cur), dict_table_is_comp(index->table)));
  ut_ad(rec_offs_validate(btr_cur_get_rec(btr_cur), index, offsets));

  if (dict_index_is_online_ddl(index))
  {
    rebuilt_old_pk = row_log_table_get_pk(btr_cur_get_rec(btr_cur), index, offsets, NULL, &heap);
  }
  ...
  {
    err = btr_cur_update_in_place(flags | BTR_NO_LOCKING_FLAG, btr_cur, offsets, node->update, node->cmpl_info, thr,
                                  thr_get_trx(thr)->id, mtr);
  }
  else
  {
    err = btr_cur_optimistic_update(flags | BTR_NO_LOCKING_FLAG, btr_cur, &offsets, offsets_heap, node->update,
  ...
  err = btr_cur_pessimistic_update(flags | BTR_NO_LOCKING_FLAG | BTR_KEEP_POS_FLAG, btr_cur, &offsets, offsets_heap,
                                   heap, &big_rec, node->update, node->cmpl_info, thr, thr_get_trx(thr)->id, mtr);
  if (big_rec)
  {
    ut_a(err == DB_SUCCESS);

    DEBUG_SYNC_C("before_row_upd_extern");
    err = btr_store_big_rec_extern_fields(pcur, node->update, offsets, big_rec, mtr, BTR_STORE_UPDATE);
  ...
  success:
    if (dict_index_is_online_ddl(index))
    {
      ...
      row_log_table_update(btr_cur_get_rec(btr_cur), index, offsets, rebuilt_old_pk, new_v_row, old_v_row);
    }
  }
    ...
    mtr_t *mtr) /*!< in: mtr; gets committed here */
{
  ...
  row_upd_store_row(node, thr_get_trx(thr)->mysql_thd, thr->prebuilt ? thr->prebuilt->m_mysql_table : NULL);

  /* Mark the clustered index record deleted; we do not have to check
  locks, because we assume that we have an x-lock on the record */

  err = btr_cur_del_mark_set_clust_rec(flags, btr_cur_get_block(btr_cur), btr_cur_get_rec(btr_cur), index, offsets, thr,
                                       node->row, mtr);
    ...
    que_thr_t *thr)   /*!< in: query thread */
{
  ...
  DEBUG_SYNC_C_IF_THD(thr_get_trx(thr)->mysql_thd, "innodb_row_upd_clust_step_enter");
  ...
  if (node->is_delete && node->table->id == DICT_INDEXES_ID)
  {
    ...
    dict_drop_index_tree(btr_pcur_get_rec(pcur), pcur, &mtr);
  ...
  {
    err = lock_clust_rec_modify_check_and_lock(flags, btr_pcur_get_block(pcur), rec, index, offsets, thr);
    if (err != DB_SUCCESS)
    {
      mtr_commit(&mtr);
      goto exit_func;
  ...
  ut_ad(lock_trx_has_rec_x_lock(thr_get_trx(thr), index->table, btr_pcur_get_block(pcur), page_rec_get_heap_no(rec)));

  /* NOTE: the following function calls will also commit mtr */

  if (node->is_delete)
  {
    err = row_upd_del_mark_clust_rec(flags, node, index, offsets, thr, referenced, &mtr);
  ...
  row_upd_store_row(node, trx->mysql_thd, thr->prebuilt ? thr->prebuilt->m_mysql_table : NULL);

  if (row_upd_changes_ord_field_binary(index, node->update, thr, node->row, node->ext))
  {
    ...
    que_thr_t *thr)   /*!< in: query thread */
{
  ...
  DBUG_PRINT("row_upd", ("info bits in update vector: 0x%lx", node->update ? node->update->info_bits : 0));
  DBUG_PRINT("row_upd", ("foreign_id: %s", node->foreign ? node->foreign->id : "NULL"));
  ...
  if (node->index == NULL || (!node->is_delete && (node->cmpl_info & UPD_NODE_NO_ORD_CHANGE)))
  {
    DBUG_RETURN(DB_SUCCESS);
  }
