                          ...
                          or ROW_BUILD_FOR_UNDO */
{
  ...
  for (i = 0; i < entry_len + num_v; i++)
  {
    ...
    if (i >= entry_len)
    {
      ...
      ut_ad(dict_index_is_clust(index) && flag == ROW_BUILD_FOR_INSERT);
...
#error "DATA_MISSING != 0"
    ...
    else
    {
      ...
      ut_ad(dfield_get_type(dfield2)->mtype == DATA_MISSING || (!(dfield_get_type(dfield2)->prtype & DATA_VIRTUAL)));
    }
...
#ifdef UNIV_DEBUG
    if (dfield_get_type(dfield2)->prtype & DATA_VIRTUAL && dict_index_is_clust(index))
    {
    ...
    if (dict_index_is_spatial(index) && i == 0)
    {
      ...
      if (dfield2->data)
      {
        ...
        if (dfield_is_ext(dfield2))
        {
          if (flag == ROW_BUILD_FOR_PURGE)
          {
            ...
            switch (spatial_status)
            {
              ...
              case SPATIAL_MIXED:
                ptr = static_cast<byte *>(dfield_get_data(dfield2)) + dfield_get_len(dfield2);
                break;
          ...
          if (flag == ROW_BUILD_FOR_UNDO && dict_table_get_format(index->table) >= UNIV_FORMAT_B)
          {
            ...
            dptr = static_cast<byte *>(dfield_get_data(dfield2)) + dfield_get_len(dfield2) - BTR_EXTERN_FIELD_REF_SIZE;
          }
          else
          {
        ...
        {
          rtree_mbr_from_wkb(dptr + GEO_DATA_HEADER_SIZE, static_cast<uint>(dlen - GEO_DATA_HEADER_SIZE), SPDIMS,
                             tmp_mbr);
        }
    ...
    if ((!ind_field || ind_field->prefix_len == 0) && (!dfield_is_ext(dfield) || dict_index_is_clust(index)))
    {
...
static inline dtuple_t *row_build_low(ulint type, const dict_index_t *index, const rec_t *rec, const ulint *offsets,
                                      const dict_table_t *col_table, const dtuple_t *add_cols,
                                      const dict_add_v_col_t *add_v, const ulint *col_map, row_ext_t **ext,
                                      mem_heap_t *heap)
{
  const byte *copy;
  dtuple_t *row;
  ...
  if (add_cols)
  {
    ...
    {
      dict_col_copy_type(dict_table_get_nth_col(col_table, i), dfield_get_type(dtuple_get_nth_field(row, i)));
    }
  }
  else if (add_v != NULL)
  {
    row = dtuple_create_with_vcol(heap, dict_table_get_n_cols(col_table),
                                  dict_table_get_n_v_cols(col_table) + add_v->n_v_col);
    dict_table_copy_types(row, col_table);

    for (ulint i = 0; i < add_v->n_v_col; i++)
    {
      dict_col_copy_type(&add_v->v_col[i].m_col, dfield_get_type(dtuple_get_nth_v_field(row, i + col_table->n_v_def)));
    }
  }
  else
  {
    row = dtuple_create_with_vcol(heap, dict_table_get_n_cols(col_table), dict_table_get_n_v_cols(col_table));
    dict_table_copy_types(row, col_table);
  }
  ...
  {
    *ext = row_ext_create(j, ext_cols, index->table->flags, row, heap);
  }
  else
  {
    *ext = NULL;
...
dtuple_t *row_build(
    ...
    mem_heap_t *heap)     /*!< in: memory heap from which
                           the memory needed is allocated */
{
  return (row_build_low(type, index, rec, offsets, col_table, add_cols, NULL, col_map, ext, heap));
}
...
@param[in]	heap		memory heap from which
                                the memory needed is allocated
@return own: row built; */
dtuple_t *row_build_w_add_vcol(ulint type, const dict_index_t *index, const rec_t *rec, const ulint *offsets,
                               const dict_table_t *col_table, const dtuple_t *add_cols, const dict_add_v_col_t *add_v,
                               const ulint *col_map, row_ext_t **ext, mem_heap_t *heap)
{
  return (row_build_low(type, index, rec, offsets, col_table, add_cols, add_v, col_map, ext, heap));
}
                               ...
                               the memory needed is allocated */
{
  ...
  *n_ext = 0;
  ...
  ut_ad(rec_len == dict_index_get_n_fields(index)
        ...
        || (index->table->id == DICT_INDEXES_ID && rec_len == dict_index_get_n_fields(index) - 1));
  ...
  for (i = 0; i < ref_len; i++)
  {
    ...
    if (clust_col_prefix_len > 0)
    {
      if (len != UNIV_SQL_NULL)
      {
        ...
        dfield_set_len(dfield, dtype_get_at_most_n_mbchars(dtype->prtype, dtype->mbminmaxlen, clust_col_prefix_len, len,
                                                           (char *)field));
      }
    }
  }
                          ...
                          formated in hex */
{
  ...
  if (data_len <= sizeof(ib_uint64_t))
  {
    ...
    ret = ut_snprintf(buf, buf_size, unsigned_type ? UINT64PF : "%" PRId64, value) + 1;
  }
  else
  {
    *format_in_hex = TRUE;
                                    ...
                                    in bytes */
{
  ...
  switch (mtype)
  {
    case DATA_INT:
      ...
      ret = row_raw_format_int(data, data_len, prtype, buf, buf_size, &format_in_hex);
      if (format_in_hex)
      {
        goto format_in_hex;
      }
    ...
    case DATA_VARMYSQL:
      ...
      ret = row_raw_format_str(data, data_len, prtype, buf, buf_size, &format_in_hex);
      if (format_in_hex)
      {
        goto format_in_hex;
      }
...
#define CALL_AND_TEST(data, data_len, prtype, buf, buf_size, ret_expected, buf_expected, format_in_hex_expected) \
  do                                                                                                             \
  {                                                                                                              \
    ibool ok = TRUE;                                                                                             \
    ulint i;                                                                                                     \
    ...
    fprintf(stderr, "\", %lu, %lu, %lu\n", (ulint)data_len, (ulint)prtype, (ulint)buf_size);                     \
    ret = row_raw_format_int(data, data_len, prtype, buf, buf_size, &format_in_hex);                             \
    if (ret != ret_expected)                                                                                     \
    {                                                                                                            \
  ...
  CALL_AND_TEST("\x00\x00\x00\x00", 4, 0, buf, sizeof(buf), 12, "-2147483648", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00", 5, 0, buf, sizeof(buf), 14, "-549755813888", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00\x00", 6, 0, buf, sizeof(buf), 17, "-140737488355328", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00\x00\x00", 7, 0, buf, sizeof(buf), 19, "-36028797018963968", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00\x00\x00\x00", 8, 0, buf, sizeof(buf), 21, "-9223372036854775808", 0);
  ...
  CALL_AND_TEST("\x00\x00\x00\x00", 4, DATA_UNSIGNED, buf, sizeof(buf), 2, "0", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00", 5, DATA_UNSIGNED, buf, sizeof(buf), 2, "0", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00\x00", 6, DATA_UNSIGNED, buf, sizeof(buf), 2, "0", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00\x00\x00", 7, DATA_UNSIGNED, buf, sizeof(buf), 2, "0", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00\x00\x00\x00", 8, DATA_UNSIGNED, buf, sizeof(buf), 2, "0", 0);
  ...
  CALL_AND_TEST("\xFF\xFF\xFF\xFF", 4, 0, buf, sizeof(buf), 11, "2147483647", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF\xFF", 5, 0, buf, sizeof(buf), 13, "549755813887", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF\xFF\xFF", 6, 0, buf, sizeof(buf), 16, "140737488355327", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 7, 0, buf, sizeof(buf), 18, "36028797018963967", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 8, 0, buf, sizeof(buf), 20, "9223372036854775807", 0);
  ...
  CALL_AND_TEST("\xFF\xFF\xFF", 3, DATA_UNSIGNED, buf, sizeof(buf), 9, "16777215", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF", 4, DATA_UNSIGNED, buf, sizeof(buf), 11, "4294967295", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF\xFF", 5, DATA_UNSIGNED, buf, sizeof(buf), 14, "1099511627775", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF\xFF\xFF", 6, DATA_UNSIGNED, buf, sizeof(buf), 16, "281474976710655", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 7, DATA_UNSIGNED, buf, sizeof(buf), 18, "72057594037927935", 0);

  CALL_AND_TEST("\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 8, DATA_UNSIGNED, buf, sizeof(buf), 21, "18446744073709551615", 0);
  ...
  CALL_AND_TEST("\x00\xA1\x16", 3, DATA_UNSIGNED, buf, sizeof(buf), 6, "41238", 0);

  CALL_AND_TEST("\x7F\xFF\xFF\xF7", 4, 0, buf, sizeof(buf), 3, "-9", 0);

  CALL_AND_TEST("\x00\x00\x00\x5C", 4, DATA_UNSIGNED, buf, sizeof(buf), 3, "92", 0);

  CALL_AND_TEST("\x7F\xFF\xFF\xFF\xFF\xFF\xDC\x63", 8, 0, buf, sizeof(buf), 6, "-9117", 0);

  CALL_AND_TEST("\x00\x00\x00\x00\x00\x01\x64\x62", 8, DATA_UNSIGNED, buf, sizeof(buf), 6, "91234", 0);
#endif
  ...
  for (i = 0; i < 1000000; i++)
  {
    ...
    row_raw_format_int("\x23", 1, DATA_UNSIGNED, buf, sizeof(buf), &format_in_hex);

    row_raw_format_int("\x00\x00\x00\x00\x00\x01\x64\x62", 8, 0, buf, sizeof(buf), &format_in_hex);
    row_raw_format_int("\x00\x00\x00\x00\x00\x01\x64\x62", 8, DATA_UNSIGNED, buf, sizeof(buf), &format_in_hex);
  }
}
