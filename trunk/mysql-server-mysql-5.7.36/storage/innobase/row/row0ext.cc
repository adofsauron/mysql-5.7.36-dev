...
static void row_ext_cache_fill(row_ext_t *ext, ulint i, const page_size_t &page_size, const dfield_t *dfield)
{
  ...
  if (UNIV_UNLIKELY(!memcmp(field_ref_zero, field + f_len - BTR_EXTERN_FIELD_REF_SIZE, BTR_EXTERN_FIELD_REF_SIZE)))
  {
  ...
  {
    if (ext->max_len == REC_VERSION_56_MAX_INDEX_COL_LEN && f_len > BTR_EXTERN_FIELD_REF_SIZE)
    {
    ...
    else
    {
      ...
      ext->len[i] = btr_copy_externally_stored_field_prefix(buf, ext->max_len, page_size, field, f_len);
    }
  }
}
    ...
    mem_heap_t *heap)      /*!< in: heap where created */
{
  ...
  ret = static_cast<row_ext_t *>(mem_heap_alloc(heap, (sizeof *ret) + (n_ext - 1) * sizeof ret->len));
