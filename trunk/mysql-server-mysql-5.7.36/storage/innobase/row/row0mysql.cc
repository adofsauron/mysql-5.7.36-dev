                              ...
                              ha_innobase:: table handle */
{
  ...
  DBUG_PRINT("row_mysql_prebuilt_free_blob_heap", ("blob_heap freeing: %p", prebuilt->blob_heap));
                     ...
                     the MySQL record header! */
{
  ...
  DBUG_EXECUTE_IF("row_print_geometry_data", {
    ...
    Geometry *g = Geometry::construct(&buffer, (const char *)src, (uint32)src_len);
...
const byte *row_mysql_read_geometry(
    /*====================*/
    ...
    ulint col_len)   /*!< in: MySQL format length */
{
  ...
  *len = mach_read_from_n_little_endian(ref, col_len - 8);
  ...
  DBUG_EXECUTE_IF("row_print_geometry_data", {
    ...
    /** Show the meaning of geometry data. */
    Geometry *g = Geometry::construct(&buffer, (const char *)data, (uint32)*len);
    ...
    else
    {
      ...
      switch (mbminlen)
      {
        ...
        case 4:
          ...
          while (col_len >= 4 && ptr[col_len - 4] == 0x00 && ptr[col_len - 3] == 0x00 && ptr[col_len - 2] == 0x00 &&
                 ptr[col_len - 1] == 0x20)
          {
            col_len -= 4;
          }
                              ...
                              server fixes its issue */
{
  ...
  for (i = 0; i < prebuilt->n_template; i++)
  {
    ...
    row_mysql_store_col_in_innobase_format(
        ...
        mysql_rec + templ->mysql_col_offset, templ->mysql_col_len, dict_table_is_comp(prebuilt->table));
...
handle_new_error:
  ...
  switch (err)
  {
    ...
    case DB_FOREIGN_EXCEED_MAX_CASCADE:
      ...
      break;
    default:
      ib::fatal() << "Unknown error code " << err << ": " << ut_strerr(err);
  }
                         ...
                         the MySQL format */
{
  ...
  search_tuple_n_fields = 2 * (dict_table_get_n_cols(table) + dict_table_get_n_v_cols(table));
...
#define PREBUILT_HEAP_INITIAL_SIZE                                                                                  \
  (sizeof(*prebuilt)                                                     /* allocd in this function */              \
   + DTUPLE_EST_ALLOC(search_tuple_n_fields) + DTUPLE_EST_ALLOC(ref_len) /* allocd in row_prebuild_sel_graph() */   \
   + sizeof(sel_node_t) + sizeof(que_fork_t) + sizeof(que_thr_t) /* allocd in row_get_prebuilt_update_vector() */   \
   + sizeof(upd_node_t) + sizeof(upd_t) + sizeof(upd_field_t) * dict_table_get_n_cols(table) + sizeof(que_fork_t) + \
   ...
   DTUPLE_EST_ALLOC(dict_table_get_n_cols(table) + dict_table_get_n_v_cols(table)) + sizeof(que_fork_t) +           \
   sizeof(que_thr_t) + sizeof(*prebuilt->pcur) + sizeof(*prebuilt->clust_pcur))
  ...
  for (temp_index = dict_table_get_first_index(table); temp_index; temp_index = dict_table_get_next_index(temp_index))
  {
    DBUG_EXECUTE_IF("innodb_srch_key_buffer_max_value", ut_a(temp_index->n_user_defined_cols == MAX_REF_PARTS););
    uint temp_len = 0;
  ...
  DBUG_EXECUTE_IF("innodb_srch_key_buffer_max_value", ut_a(srch_key_len == MAX_SRCH_KEY_VAL_BUFFER););
  ...
  {
    prebuilt->srch_key_val1 = static_cast<byte *>(mem_heap_alloc(prebuilt->heap, 2 * prebuilt->srch_key_val_len));
    prebuilt->srch_key_val2 = prebuilt->srch_key_val1 + prebuilt->srch_key_val_len;
  }
  else
  {
  ...
  prebuilt->pcur = static_cast<btr_pcur_t *>(mem_heap_zalloc(prebuilt->heap, sizeof(btr_pcur_t)));
  prebuilt->clust_pcur = static_cast<btr_pcur_t *>(mem_heap_zalloc(prebuilt->heap, sizeof(btr_pcur_t)));
                              ...
                              handle */
{
  ...
  {
    prebuilt->ins_upd_rec_buff = static_cast<byte *>(mem_heap_alloc(prebuilt->heap, prebuilt->mysql_row_len));
  }

  dtuple_t *row;

  row = dtuple_create_with_vcol(prebuilt->heap, dict_table_get_n_cols(table), dict_table_get_n_v_cols(table));
  ...
  prebuilt->ins_graph = static_cast<que_fork_t *>(
      que_node_get_parent(pars_complete_graph_for_exec(node, prebuilt->trx, prebuilt->heap, prebuilt)));
    ...
    dict_table_t *table) /*!< in: table */
{
  ...
  {
    DBUG_EXECUTE_IF("test_upd_stats_if_needed_not_inited", fprintf(stderr,
...
/*********************************************************************/ /**
 ...
 @return error code or DB_SUCCESS */
dberr_t row_lock_table_for_mysql(
    /*=====================*/
    row_prebuilt_t *prebuilt, /*!< in: prebuilt struct in the MySQL
                              table handle */
...
run_again:
  ...
  {
    err = lock_table(0, prebuilt->table, static_cast<enum lock_mode>(prebuilt->select_lock_type), thr);
  }
...
static dberr_t row_explicit_rollback(dict_index_t *index, const dtuple_t *entry, que_thr_t *thr, mtr_t *mtr)
{
  ...
  btr_cur_search_to_nth_level_with_no_latch(index, 0, entry, PAGE_CUR_LE, &cursor, __FILE__, __LINE__, mtr);

  offsets = rec_get_offsets(btr_cur_get_rec(&cursor), index, offsets_, ULINT_UNDEFINED, &heap);

  if (dict_index_is_clust(index))
  {
    err = btr_cur_del_mark_set_clust_rec(flags, btr_cur_get_block(&cursor), btr_cur_get_rec(&cursor), index, offsets,
                                         thr, entry, mtr);
  }
  else
  {
...
static void row_mysql_to_innobase(dtuple_t *row, row_prebuilt_t *prebuilt, const byte *mysql_rec)
{
  ...
  for (ulint i = 0; i < prebuilt->n_template; i++)
  {
    ...
    ut_ad(dtype->mtype == DATA_INT || dtype->mtype == DATA_CHAR || dtype->mtype == DATA_MYSQL ||
          dtype->mtype == DATA_VARCHAR || dtype->mtype == DATA_VARMYSQL || dtype->mtype == DATA_BINARY ||
          dtype->mtype == DATA_FIXBINARY || dtype->mtype == DATA_FLOAT || dtype->mtype == DATA_DOUBLE ||
          dtype->mtype == DATA_DECIMAL || dtype->mtype == DATA_BLOB || dtype->mtype == DATA_GEOMETRY ||
    ...
    {
      ut_ad(dtype->mtype == DATA_VARCHAR || dtype->mtype == DATA_VARMYSQL || dtype->mtype == DATA_BINARY);
...
static dberr_t row_insert_for_mysql_using_cursor(const byte *mysql_rec, row_prebuilt_t *prebuilt)
{
  ...
  {
    dict_sys_write_row_id(node->row_id_buf, dict_table_get_next_table_sess_row_id(node->table));
  }

  trx_write_trx_id(node->trx_id_buf, dict_table_get_next_table_sess_trx_id(node->table));

  /* Step-4: Iterate over all the indexes and insert entries. */
  dict_index_t *inserted_upto = NULL;
  node->entry = UT_LIST_GET_FIRST(node->entry_list);
  for (dict_index_t *index = UT_LIST_GET_FIRST(node->table->indexes); index != NULL;
       index = UT_LIST_GET_NEXT(indexes, index), node->entry = UT_LIST_GET_NEXT(tuple_list, node->entry))
  {
    node->index = index;
  ...
  if (err != DB_SUCCESS)
  {
    ...
    for (dict_index_t *index = UT_LIST_GET_FIRST(node->table->indexes); inserted_upto != NULL;
         index = UT_LIST_GET_NEXT(indexes, index), node->entry = UT_LIST_GET_NEXT(tuple_list, node->entry))
    {
...
static dberr_t row_insert_for_mysql_using_ins_graph(const byte *mysql_rec, row_prebuilt_t *prebuilt)
{
  ...
  {
    ib::error() << ".ibd file is missing for table " << prebuilt->table->name;
...
run_again:
  ...
  error_exit:
    ...
    if (was_lock_wait)
    {
      ut_ad(node->state == INS_NODE_INSERT_ENTRIES || node->state == INS_NODE_ALLOC_ROW_ID);
      goto run_again;
    }
  ...
  if (dict_table_has_fts_index(table))
  {
    ...
    if (!DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_HAS_DOC_ID))
    {
      ...
      {
        ib::error() << "FTS Doc ID must be large than " << next_doc_id - 1 << " for table " << table->name;
                              ...
                              handle */
{
  ...
  if (prebuilt->sel_graph == NULL)
  {
    ...
    prebuilt->sel_graph = static_cast<que_fork_t *>(que_node_get_parent(
        pars_complete_graph_for_exec(static_cast<sel_node_t *>(node), prebuilt->trx, prebuilt->heap, prebuilt)));
    ...
    mem_heap_t *heap)    /*!< in: mem heap from which allocated */
{
  ...
  node->update = upd_create(dict_table_get_n_cols(table) + dict_table_get_n_v_cols(table), heap);
                              ...
                              handle */
{
  ...
  if (prebuilt->upd_node == NULL)
  {
    ...
    prebuilt->upd_graph = static_cast<que_fork_t *>(que_node_get_parent(
        pars_complete_graph_for_exec(static_cast<upd_node_t *>(node), prebuilt->trx, prebuilt->heap, prebuilt)));
    ...
    ulint *depth)        /*!< in: recusive call depth */
{
  ...
  for (dict_foreign_set::iterator it = table->referenced_set.begin(); it != table->referenced_set.end(); ++it)
  {
    ...
    if (!foreign->foreign_table->referenced_set.empty() && foreign->foreign_table != table)
    {
...
static dberr_t row_update_inplace_for_intrinsic(const upd_node_t *node)
{
  ...
  bool size_changes = row_upd_changes_field_size_or_external(index, offsets, node->update);
...
@param[in]	restore_delete	if true, then restore DELETE records by
                                unmarking delete.
@return error code or DB_SUCCESS */
static dberr_t row_delete_for_mysql_using_cursor(const upd_node_t *node, cursors_t &delete_entries, bool restore_delete)
{
  mtr_t mtr;
  dict_table_t *table = node->table;
  mem_heap_t *heap = node->heap;
  ...
  for (dict_index_t *index = UT_LIST_GET_FIRST(table->indexes); index != NULL && err == DB_SUCCESS && !restore_delete;
       index = UT_LIST_GET_NEXT(indexes, index))
  {
    ...
    offsets = rec_get_offsets(btr_cur_get_rec(btr_pcur_get_btr_cur(&pcur)), index, offsets, ULINT_UNDEFINED, &heap);

    ut_ad(!cmp_dtuple_rec(entry, btr_cur_get_rec(btr_pcur_get_btr_cur(&pcur)), offsets));
#endif /* UNIV_DEBUG */

    ut_ad(!rec_get_deleted_flag(btr_cur_get_rec(btr_pcur_get_btr_cur(&pcur)), dict_table_is_comp(index->table)));

    ut_ad(btr_pcur_get_block(&pcur)->made_dirty_with_no_latch);

    if (page_rec_is_infimum(btr_pcur_get_rec(&pcur)) || page_rec_is_supremum(btr_pcur_get_rec(&pcur)))
    {
      err = DB_ERROR;
    }
    else
    {
      ...
      btr_rec_set_deleted_flag(btr_cur_get_rec(btr_cur), buf_block_get_page_zip(btr_cur_get_block(btr_cur)), TRUE);
  ...
  if (err != DB_SUCCESS || restore_delete)
  {
    ...
    for (cursors_t::iterator it = delete_entries.begin(); it != end; ++it)
    {
      ...
      else
      {
        ...
        btr_rec_set_deleted_flag(btr_cur_get_rec(btr_cur), buf_block_get_page_zip(btr_cur_get_block(btr_cur)), FALSE);
...
static dberr_t row_update_for_mysql_using_cursor(const upd_node_t *node, cursors_t &delete_entries, que_thr_t *thr)
{
  ...
  if (!dict_index_is_unique(dict_table_get_first_index(table)))
  {
    ...
    row_id_field = dtuple_get_nth_field(node->upd_row, dict_table_get_n_cols(table) - 2);

    dict_sys_write_row_id(static_cast<byte *>(row_id_field->data), dict_table_get_next_table_sess_row_id(node->table));
  }

  /* Step-2: Update the trx_id column. */
  trx_id_field = dtuple_get_nth_field(node->upd_row, dict_table_get_n_cols(table) - 1);
  trx_write_trx_id(static_cast<byte *>(trx_id_field->data), dict_table_get_next_table_sess_trx_id(node->table));
  ...
  for (dict_index_t *index = UT_LIST_GET_FIRST(table->indexes); index != NULL && err == DB_SUCCESS;
       index = UT_LIST_GET_NEXT(indexes, index))
  {
    ...
    if (dict_index_is_clust(index))
    {
      ...
      {
        err = row_ins_clust_index_entry(index, entry, thr, entry->get_n_ext(), true);
      }
    }
    else
    {
       ...
       index = UT_LIST_GET_NEXT(indexes, index))
  {
    ...
    if (dict_index_is_clust(index))
    {
      err = row_ins_clust_index_entry(index, entry, thr, entry->get_n_ext(), false);
    ...
    ut_ad(err == DB_SUCCESS || err == DB_TOO_BIG_RECORD || err == DB_OUT_OF_FILE_SPACE);
...
@param[in,out]	prebuilt	prebuilt struct in MySQL handle
@return error code or DB_SUCCESS */
static dberr_t row_del_upd_for_mysql_using_cursor(const byte *mysql_rec, row_prebuilt_t *prebuilt)
{
  dberr_t err = DB_SUCCESS;
  upd_node_t *node;
  ...
  if (!node->is_delete)
  {
    ...
    for (dict_index_t *index = UT_LIST_GET_FIRST(table->indexes); index != NULL;
         index = UT_LIST_GET_NEXT(indexes, index))
    {
      key_changed = row_upd_changes_ord_field_binary(index, node->update, thr, node->upd_row, node->upd_ext);
...
@return error code or DB_SUCCESS */
dberr_t row_delete_all_rows(dict_table_t *table)
{
  dberr_t err = DB_SUCCESS;
  dict_index_t *index;
  ...
  for (index = UT_LIST_GET_FIRST(table->indexes); index != NULL && err == DB_SUCCESS;
       index = UT_LIST_GET_NEXT(indexes, index))
  {
...
void row_unlock_for_mysql(row_prebuilt_t *prebuilt, ibool has_latches_on_recs)
{
  ...
  if (UNIV_UNLIKELY(!srv_locks_unsafe_for_binlog && trx->isolation_level > TRX_ISO_READ_COMMITTED))
  {
    ...
    bool commit) /*!< in: if true, commit the transaction */
{
  ...
  DBUG_EXECUTE_IF("ib_create_table_fail_at_start_of_row_create_table_for_mysql", goto err_exit;);

  trx->op_info = "creating table";

  if (row_mysql_is_system_table(table->name.m_name))
  {
    ib::error() << "Trying to create a MySQL system table " << table->name
                << " of type InnoDB. MySQL system"
  ...
  if (err == DB_SUCCESS && dict_table_is_file_per_table(table))
  {
    ...
    err = dict_replace_tablespace_in_dictionary(table->space, table->name.m_name, fil_space_get_flags(table->space),
                                                path, trx, commit);
  ...
  switch (err)
  {
    ...
    case DB_OUT_OF_FILE_SPACE:
      ...
      ib::warn() << "Cannot create table " << table->name << " because tablespace full";

      if (dict_table_open_on_name(table->name.m_name, TRUE, FALSE, DICT_ERR_IGNORE_NONE))
      {
    ...
    case DB_TOO_MANY_CONCURRENT_TRXS:
      ...
      if (dict_table_is_file_per_table(table) &&
          fil_delete_tablespace(table->space, BUF_REMOVE_FLUSH_NO_WRITE) != DB_SUCCESS)
      {
        ib::error() << "Not able to delete tablespace " << table->space << " of table " << table->name << "!";
      }
      /* fall through */
    ...
    dict_table_t *handler)      /*!< in/out: table handler. */
{
  ...
  if (table == NULL)
  {
    ...
    table = dict_table_open_on_name(table_name, TRUE, TRUE, DICT_ERR_IGNORE_NONE);
  }
  else
  {
    table->acquire();
  ...
  for (i = 0; i < index->n_def; i++)
  {
    ...
    DBUG_EXECUTE_IF("ib_create_table_fail_at_create_index", len = DICT_MAX_FIELD_LEN_BY_FORMAT(table) + 1;);
  ...
  else
  {
    ...
    err = dict_index_add_to_cache(table, index, FIL_NULL, !dict_table_is_intrinsic(table) && trx_is_strict(trx));
...
/*********************************************************************/ /**
 ...
 @return error code or DB_SUCCESS */
dberr_t row_table_add_foreign_constraints(trx_t *trx, const char *sql_string, size_t sql_length, const char *name,
                                          ibool reject_fks)
{
  dberr_t err;
  ...
  err = dict_create_foreign_constraints(trx, sql_string, sql_length, name, reject_fks);
  ...
  if (err == DB_SUCCESS)
  {
    ...
    err = dict_load_foreigns(name, NULL, false, true, DICT_ERR_IGNORE_NONE, fk_tables);

    while (err == DB_SUCCESS && !fk_tables.empty())
    {
      dict_load_table(fk_tables.front(), true, DICT_ERR_IGNORE_NONE);
      fk_tables.pop_front();
    }
  }
...
loop:
  ...
  DBUG_EXECUTE_IF("row_drop_tables_in_background_sleep", os_thread_sleep(5000000););

  table = dict_table_open_on_name(drop->table_name, FALSE, FALSE, DICT_ERR_IGNORE_NONE);
...
already_dropped:
  ...
  ib::info() << "Dropped table " << ut_get_name(NULL, drop->table_name) << " in background drop queue.",
    ...
    const char *name) /*!< in: table name */
{
  ...
  for (drop = UT_LIST_GET_FIRST(row_mysql_drop_list); drop != NULL; drop = UT_LIST_GET_NEXT(row_mysql_drop_list, drop))
  {
  ...
  drop = static_cast<row_mysql_drop_t *>(ut_malloc_nokey(sizeof(row_mysql_drop_t)));
...
@return error code or DB_SUCCESS */
dberr_t row_mysql_table_id_reassign(dict_table_t *table, trx_t *trx, table_id_t *new_id)
{
  dberr_t err;
    ...
    trx_t *trx)       /*!< in: transaction handle */
{
  ...
  table = dict_table_open_on_name(name, TRUE, FALSE, DICT_ERR_IGNORE_NONE);
    ...
    const dict_table_t *table) /*!< in: table to be discarded */
{
  ...
  dict_foreign_set::iterator it =
      std::find_if(table->referenced_set.begin(), table->referenced_set.end(), dict_foreign_different_tables());
...
/*********************************************************************/ /**
 ...
 @return error code. */
static dberr_t row_discard_tablespace_end(
    /*=======================*/
    ...
    dberr_t err)         /*!< in: error code */
{
  ...
  DBUG_EXECUTE_IF("ib_discard_before_commit_crash", log_make_checkpoint_at(LSN_MAX, TRUE); DBUG_SUICIDE(););

  trx_commit_for_mysql(trx);

  DBUG_EXECUTE_IF("ib_discard_after_commit_crash", log_make_checkpoint_at(LSN_MAX, TRUE); DBUG_SUICIDE(););
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code. */
static dberr_t row_discard_tablespace(
    /*===================*/
    trx_t *trx,          /*!< in/out: transaction handle */
    dict_table_t *table) /*!< in/out: table to be discarded */
{
  ...
  if (dict_table_has_fts_index(table) || DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_HAS_DOC_ID))
  {
    ...
    trx_t *trx)       /*!< in: transaction handle */
{
  ...
  {
    ib_senderrf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_CANNOT_DISCARD_TEMPORARY_TABLE);
  ...
  else if (table->space == srv_sys_space.space_id())
  {
    ...
    ib_senderrf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLE_IN_SYSTEM_TABLESPACE, table_name);
  ...
  else if (table->n_foreign_key_checks_running > 0)
  {
    ...
    ib_senderrf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_DISCARD_FK_CHECKS_RUNNING, table_name);
...
@return error code or DB_SUCCESS */
UNIV_INLINE
dberr_t row_drop_ancillary_fts_tables(dict_table_t *table, trx_t *trx)
{
...
@return error code or DB_SUCCESS */
UNIV_INLINE
dberr_t row_drop_table_from_cache(const char *tablename, dict_table_t *table, trx_t *trx)
{
  dberr_t err = DB_SUCCESS;
  ...
  if (!is_temp && dict_load_table(tablename, true, DICT_ERR_IGNORE_NONE) != NULL)
  {
    ib::error() << "Not able to remove table " << ut_get_name(trx, tablename) << " from the dictionary cache!";
    err = DB_ERROR;
  }
...
@return error code or DB_SUCCESS */
UNIV_INLINE
dberr_t row_drop_single_table_tablespace(ulint space_id, const char *tablename, const char *filepath, bool is_temp,
                                         bool is_encrypted, trx_t *trx)
{
  dberr_t err = DB_SUCCESS;
  ...
  if (!fil_space_for_table_exists_in_mem(space_id, tablename, print_msg, false, NULL, 0))
  {
    ...
    if (print_msg)
    {
      ib::info() << "Removed datafile " << filepath << " for table " << tablename;
    }
  }
  else if (fil_delete_tablespace(space_id, BUF_REMOVE_FLUSH_NO_WRITE) != DB_SUCCESS)
  {
    ib::error() << "We removed the InnoDB internal data"
                   " dictionary entry of table "
                << tablename << " but we are not able to delete the tablespace " << space_id << " file " << filepath
                << "!";
...
@return error code or DB_SUCCESS */
dberr_t row_drop_table_for_mysql(const char *name, trx_t *trx, bool drop_db, bool nonatomic, dict_table_t *handler)
{
  dberr_t err;
  dict_foreign_t *foreign;
  dict_table_t *table = NULL;
  ...
  if (table == NULL)
  {
    ...
    table = dict_table_open_on_name(
        name, TRUE, FALSE, static_cast<dict_err_ignore_t>(DICT_ERR_IGNORE_INDEX_ROOT | DICT_ERR_IGNORE_CORRUPT));
  }
  else
  {
    table->acquire();
  ...
  if (!srv_read_only_mode && trx->check_foreigns)
  {
    for (dict_foreign_set::iterator it = table->referenced_set.begin(); it != table->referenced_set.end(); ++it)
    {
      ...
      const bool ref_ok = drop_db && dict_tables_have_same_db(name, foreign->foreign_table_name_lookup);
  ...
  DBUG_EXECUTE_IF("row_drop_table_add_to_background", row_add_table_to_background_drop_list(table->name.m_name);
  ...
  if (table->n_foreign_key_checks_running > 0)
  {
    ...
    if (added)
    {
      ib::info() << "You are trying to drop table " << table->name
  ...
  else if (table->get_ref_count() > 0 || table->n_rec_locks > 0)
  {
    ...
    if (added)
    {
      ib::info() << "MySQL is trying to drop table " << table->name
  ...
  switch (trx_get_dict_operation(trx))
  {
    ...
    case TRX_DICT_OP_INDEX:
      ...
      ut_ad(strstr(table->name.m_name, "/FTS_") != NULL || strstr(table->name.m_name, TEMP_FILE_PREFIX_INNODB) != NULL);
  }
  ...
  unsigned *page_no;
  unsigned *page_nos;
  heap = mem_heap_create(200 + UT_LIST_GET_LEN(table->indexes) * sizeof *page_nos);
  tablename = mem_heap_strdup(heap, name);

  page_no = page_nos = static_cast<unsigned *>(mem_heap_alloc(heap, UT_LIST_GET_LEN(table->indexes) * sizeof *page_no));
  ...
  if (!dict_table_is_temporary(table))
  {
    ...
    std::basic_string<char, std::char_traits<char>, ut_allocator<char>> sql;
    sql.reserve(2000);
  ...
  switch (err)
  {
    ...
    case DB_SUCCESS:
      ...
      {
        filepath = fil_make_filepath(table->dir_path_of_temp_table, NULL, IBD, false);
      }
      ...
      if (is_discarded || ibd_file_missing || shared_tablespace || is_system_tablespace(space_id))
      {
      ...
      err = row_drop_single_table_tablespace(space_id, tablename, filepath, is_temp, is_encrypted, trx);
...
void row_mysql_drop_temp_tables(void)
/*============================*/
{
  trx_t *trx;
  btr_pcur_t pcur;
  ...
  btr_pcur_open_at_index_side(true, dict_table_get_first_index(dict_sys->sys_tables), BTR_SEARCH_LEAF, &pcur, true, 0,
                              &mtr);
  ...
  for (;;)
  {
    ...
    if (len != 4 || !(mach_read_from_4(field) & DICT_N_COLS_COMPACT))
    {
      continue;
    }
...
/*******************************************************************/ /**
 ...
 @return error code or DB_SUCCESS */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t drop_all_foreign_keys_in_db(
    /*========================*/
...
loop:
  ...
  while ((table_name = dict_get_first_table_name_in_db(name)))
  {
    ...
    parent_table_name = fts_get_parent_table_name(table_name, strlen(table_name));
    ...
    table = dict_table_open_on_name(
        table_name, TRUE, FALSE, static_cast<dict_err_ignore_t>(DICT_ERR_IGNORE_INDEX_ROOT | DICT_ERR_IGNORE_CORRUPT));
    ...
    if (!row_is_mysql_tmp_table_name(table->name.m_name))
    {
      ...
      the rest.*/
      if (table->can_be_evicted && (name[namelen - 1] != '#'))
      {
      ...
      {
        ib::warn() << "Missing .ibd file for table " << table->name << ".";
      }
    }
    ...
    if (table->get_ref_count() > 0)
    {
      ...
      ib::warn() << "MySQL is trying to drop database " << ut_get_name(trx, name)
                 << " though"
    ...
    if (err != DB_SUCCESS)
    {
      ib::error() << "DROP DATABASE " << ut_get_name(trx, name)
                  << " failed"
                     " with error ("
                  << ut_strerr(err)
                  << ") for"
                     " table "
                  << ut_get_name(trx, table_name);
      ut_free(table_name);
      break;
    }
...
/****************************************************************/ /**
 ...
 @return error code or DB_SUCCESS */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_delete_constraint_low(
    /*======================*/
...
/****************************************************************/ /**
 Delete a single constraint.
 @return error code or DB_SUCCESS */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t row_delete_constraint(
    /*==================*/
    const char *id,            /*!< in: constraint id */
    ...
    trx_t *trx)                /*!< in: transaction handle */
{
  ...
  err = row_delete_constraint_low(mem_heap_strcat(heap, database_name, id), trx);
...
/*********************************************************************/ /**
 ...
 @return error code or DB_SUCCESS */
dberr_t row_rename_table_for_mysql(
    /*=======================*/
    ...
    bool commit)          /*!< in: whether to commit trx */
{
  ...
  {
    ib::error() << "Trying to create a MySQL system table " << new_name
                << " of type InnoDB. MySQL system tables"
  ...
  {
    my_error(ER_PATH_LENGTH, MYF(0), strchr(new_name, '/') + 1);
  ...
  table = dict_table_open_on_name(old_name, dict_locked, FALSE, DICT_ERR_IGNORE_NONE);

  is_old_part = strstr((char *)old_name, "#p#") || strstr((char *)old_name, "#P");

  is_new_part = strstr((char *)new_name, "#p#") || strstr((char *)new_name, "#P");
  ...
  else if (is_old_part && !is_new_part && table->data_dir_path != NULL && !new_is_tmp)
  {
    ...
    table->data_dir_path = mem_heap_strdup(table->heap, str.c_str());
  ...
  for (retry = 0; retry < 100 && table->n_foreign_key_checks_running > 0; ++retry)
  {
    row_mysql_unlock_data_dictionary(trx);
    os_thread_yield();
    row_mysql_lock_data_dictionary(trx);
  ...
  if (table->n_foreign_key_checks_running > 0)
  {
    ib::error() << "In ALTER TABLE " << ut_get_name(trx, old_name)
  ...
  if (err == DB_SUCCESS && dict_table_is_file_per_table(table) && !table->ibd_file_missing)
  {
  ...
  if (!new_is_tmp)
  {
    ...
    innobase_convert_to_system_charset(strchr(old_table_utf8, '/') + 1, strchr(old_name, '/') + 1, MAX_TABLE_NAME_LEN,
                                       &errors);
    ...
    innobase_convert_to_system_charset(strchr(new_table_name, '/') + 1, strchr(new_name, '/') + 1, MAX_TABLE_NAME_LEN,
                                       &errors);
  ...
  else if (n_constraints_to_drop > 0)
  {
    ...
    char *db_name = mem_heap_strdupl(heap, old_name, db_name_len);
    ulint i;

    for (i = 0; i < n_constraints_to_drop; i++)
    {
      err = row_delete_constraint(constraints_to_drop[i], db_name, heap, trx);
  ...
  if (err != DB_SUCCESS)
  {
    if (err == DB_DUPLICATE_KEY)
    {
      ...
      ib::error() << "(1) Table rename would cause two"
                     ...
                     " comparison.";
      ib::error() << "(2) Table " << ut_get_name(trx, new_name)
      ...
      ib::error() << "If table " << ut_get_name(trx, new_name)
  ...
  else
  {
    ...
    err = dict_table_rename_in_cache(table, new_name, !new_is_tmp);
    if (err != DB_SUCCESS)
    {
      trx->error_state = DB_SUCCESS;
      trx_rollback_to_savepoint(trx, NULL);
    ...
    err =
        dict_load_foreigns(new_name, NULL, false, !old_is_tmp || trx->check_foreigns, DICT_ERR_IGNORE_NONE, fk_tables);

    if (err != DB_SUCCESS)
    {
      if (old_is_tmp)
      {
        ib::error() << "In ALTER TABLE " << ut_get_name(trx, new_name)
      ...
      {
        ib::error() << "In RENAME TABLE table " << ut_get_name(trx, new_name)
                    << " is referenced in foreign key"
                       " constraints which are not compatible"
                       " with the new table definition.";
      }

      ut_a(DB_SUCCESS == dict_table_rename_in_cache(table, old_name, FALSE));
      trx->error_state = DB_SUCCESS;
      trx_rollback_to_savepoint(trx, NULL);
      trx->error_state = DB_SUCCESS;
    }
...
funct_exit:
  if (aux_fts_rename && err != DB_SUCCESS && table != NULL && (table->space != 0))
  {
...
@param[in,out]	trx		Transaction.
@return error code or DB_SUCCESS */
dberr_t row_rename_partitions_for_mysql(THD *thd, const char *old_name, const char *new_name, trx_t *trx)
{
  ...
  while ((table_name = dict_get_first_table_name_in_db(from_name)))
  {
    ...
    if (strlen(table_name) <= (from_len + 3) || table_name[from_len] != '#' || table_name[from_len + 2] != '#' ||
        (table_name[from_len + 1] != 'P' && table_name[from_len + 1] != 'p'))
    {
      ut_ad(0);
      ut_free(table_name);
      continue;
    ...
    memcpy(to_name + to_len, table_name + from_len, strlen(table_name) - from_len + 1);
    error = row_rename_table_for_mysql(table_name, to_name, trx, false);
    if (error == DB_SUCCESS)
    {
  ...
  {
    error = dict_stats_rename_table(true, it->first.c_str(), it->second.c_str(), errstr, sizeof(errstr));
...
loop:
  ...
  switch (ret)
  {
    ...
    default:
    {
      ...
      ib::warn() << doing << " on index " << index->name
                 << " of"
                    " table "
  ...
  *n_rows = *n_rows + 1;
  ...
  if (prev_entry != NULL)
  {
    ...
    cmp = cmp_dtuple_rec_with_match(prev_entry, rec, offsets, &matched_fields);
    contains_null = FALSE;
    ...
    not_ok:
      ib::error() << msg << index->name << " of table " << index->table->name << ": " << *prev_entry << ", "
