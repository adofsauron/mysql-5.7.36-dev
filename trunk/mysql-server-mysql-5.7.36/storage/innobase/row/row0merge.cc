...
class index_tuple_info_t
{
 public:
  ...
  dberr_t insert(trx_id_t trx_id, mem_heap_t *row_heap, btr_pcur_t *pcur, mtr_t *scan_mtr, bool *mtr_committed)
  {
    big_rec_t *big_rec;
    rec_t *rec;
    btr_cur_t ins_cur;
    ...
    dtuple_t *dtuple;
    ulint count = 0;
    const ulint flag = BTR_NO_UNDO_LOG_FLAG | BTR_NO_LOCKING_FLAG | BTR_KEEP_SYS_FLAG | BTR_CREATE_FLAG;

    ut_ad(dict_index_is_spatial(m_index));

    DBUG_EXECUTE_IF("row_merge_instrument_log_check_flush", log_sys->check_flush_or_checkpoint = true;);

    for (idx_tuple_vec::iterator it = m_dtuple_vec->begin(); it != m_dtuple_vec->end(); ++it)
    {
      dtuple = *it;
      ut_ad(dtuple);
      ...
      btr_cur_search_to_nth_level(m_index, 0, dtuple, PAGE_CUR_RTREE_INSERT, BTR_MODIFY_LEAF, &ins_cur, 0, __FILE__,
                                  __LINE__, &mtr);
      ...
      if (rtr_info.mbr_adj)
      {
        ...
        mtr_start(&mtr);
        mtr.set_named_space(m_index->space);
        btr_cur_search_to_nth_level(m_index, 0, dtuple, PAGE_CUR_RTREE_INSERT, BTR_MODIFY_TREE, &ins_cur, 0, __FILE__,
                                    __LINE__, &mtr);
      }
      ...
      if (error == DB_FAIL)
      {
        ...
        error =
            btr_cur_pessimistic_insert(flag, &ins_cur, &ins_offsets, &row_heap, dtuple, &rec, &big_rec, 0, NULL, &mtr);
      }
      ...
      if (error == DB_SUCCESS)
      {
        ...
        if (error == DB_SUCCESS)
        {
          page_update_max_trx_id(btr_cur_get_block(&ins_cur), btr_cur_get_page_zip(&ins_cur), trx_id, &mtr);
        }
      }
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_merge_insert_index_tuples(trx_id_t trx_id, dict_index_t *index, const dict_table_t *old_table, int fd,
                               ...
                               in the entry */
{
  ...
  size = rec_get_converted_size_temp(index, entry->fields, n_fields, NULL, &extra_size);
  ...
  rec_convert_dtuple_to_temp(*b + extra_size, index, entry->fields, n_fields, NULL);
                         ...
                         in bytes */
{
  ...
  buf->tuples = static_cast<mtuple_t *>(ut_malloc_nokey(2 * max_tuples * sizeof *buf->tuples));
    ...
    dict_index_t *index) /*!< in: secondary index */
{
  ...
  max_tuples = static_cast<ulint>(srv_sort_buf_size) / ut_max(static_cast<ulint>(1), dict_index_get_min_size(index));
...
static void row_merge_buf_redundant_convert(const dfield_t *row_field, dfield_t *field, ulint len,
                                            const page_size_t &page_size, mem_heap_t *heap)
{
...
static ulint row_merge_buf_add(row_merge_buf_t *buf, dict_index_t *fts_index, const dict_table_t *old_table,
                               const dict_table_t *new_table, fts_psort_t *psort_info, const dtuple_t *row,
                               const row_ext_t *ext, doc_id_t *doc_id, mem_heap_t *conv_heap, dberr_t *err,
                               mem_heap_t **v_heap, TABLE *my_table, trx_t *trx)
{
  ...
  field = entry->fields = static_cast<dfield_t *>(mem_heap_alloc(buf->heap, n_fields * sizeof *entry->fields));
  ...
  for (i = 0; i < n_fields; i++, field++, ifield++)
  {
    ...
    if (*doc_id > 0 && col_no == index->table->fts->doc_col && !dict_col_is_virtual(col))
    {
    ...
    else
    {
      ...
      if (dict_col_is_virtual(col))
      {
        dict_index_t *clust_index = dict_table_get_first_index(new_table);

        row_field = innobase_get_computed_value(row, v_col, clust_index, v_heap, NULL, ifield, trx->mysql_thd, my_table,
                                                old_table, NULL, NULL);
      ...
      if (index->type & DICT_FTS)
      {
        ...
        if (*doc_id == 0)
        {
          ...
          *doc_id = (doc_id_t)mach_read_from_8(static_cast<byte *>(dfield_get_data(doc_field)));
        ...
        while (psort_info[bucket].memory_used > FTS_PENDING_DOC_MEMORY_LIMIT && trial_count++ < max_trial_count)
        {
      ...
      if (field->len != UNIV_SQL_NULL && col->mtype == DATA_MYSQL && col->len != field->len)
      {
        if (conv_heap != NULL)
        {
          row_merge_buf_redundant_convert(row_field, field, col->len, dict_table_page_size(old_table), conv_heap);
        }
        else
        {
    ...
    {
      len = dtype_get_at_most_n_mbchars(col->prtype, col->mbminmaxlen, ifield->prefix_len, len,
                                        static_cast<char *>(dfield_get_data(field)));
      dfield_set_len(field, len);
    }

    ut_ad(len <= col->len || DATA_LARGE_MTYPE(col->mtype) || (col->mtype == DATA_POINT && len == DATA_MBR_LEN));
...
#ifdef UNIV_DEBUG
  {
    ...
    size = rec_get_converted_size_temp(index, entry->fields, n_fields, NULL, &extra);
                          ...
                          (NULL if non-unique index) */
{
  ...
  row_merge_tuple_sort(dict_index_get_n_unique(buf->index), dict_index_get_n_fields(buf->index), dup, buf->tuples,
    ...
    row_merge_block_t *block) /*!< out: buffer for writing to file */
{
  ...
  for (ulint i = 0; i < buf->n_tuples; i++)
  {
    ...
    DBUG_PRINT("ib_merge_sort", ("%p,fd=%d,%lu %lu: %s", reinterpret_cast<const void *>(b), of->fd, ulong(of->offset),
                                 ulong(i), rec_printer(entry->fields, n_fields).str().c_str()));
  }
  ...
  *b++ = 0;
  ...
  DBUG_PRINT("ib_merge_sort", ("write %p,%d,%lu EOF", reinterpret_cast<const void *>(b), of->fd, ulong(of->offset)));
  DBUG_VOID_RETURN;
}
    ...
    ulint **offsets2)          /*!< out: offsets */
{
  ...
  mem_heap_t *heap = mem_heap_create(2 * i * sizeof **offsets1 + 3 * sizeof **buf);
    ...
    row_merge_block_t *buf) /*!< out: data */
{
  ...
  err = os_file_read_no_error_handling_int_fd(request, fd, buf, ofs, srv_sort_buf_size, NULL);
    ...
    ulint *offsets)            /*!< out: offsets of mrec */
{
  ...
  if (UNIV_UNLIKELY(!extra_size))
  {
    /* End of list */
    *mrec = NULL;
    DBUG_PRINT("ib_merge_sort", ("read %p,%p,%d,%lu EOF\n", reinterpret_cast<const void *>(b),
...
func_exit:
  DBUG_PRINT("ib_merge_sort",
             ("%p,%p,fd=%d,%lu: %s", reinterpret_cast<const void *>(b), reinterpret_cast<const void *>(block), fd,
...
#endif /* NDEBUG */
{
  ...
  DBUG_PRINT("ib_merge_sort", ("%p,fd=%d,%lu: %s", reinterpret_cast<const void *>(b), fd, ulong(foffs),
    ...
    ulint *foffs)             /*!< in/out: file offset */
{
  ...
  DBUG_PRINT("ib_merge_sort", ("%p,%p,fd=%d,%lu", reinterpret_cast<const void *>(b),
                               reinterpret_cast<const void *>(block), fd, ulong(*foffs)));
...
static MY_ATTRIBUTE((warn_unused_result)) int row_merge_tmpfile_if_needed(int *tmpfd, const char *path)
{
  if (*tmpfd < 0)
  {
...
@param[in]	path	location for creating temporary file
@return file descriptor, or -1 on failure */
static MY_ATTRIBUTE((warn_unused_result)) int row_merge_file_create_if_needed(merge_file_t *file, int *tmpfd,
                                                                              ulint nrec, const char *path)
{
...
{
  memcpy(prev_mtuple->fields, mtuple->fields, n_unique * sizeof *mtuple->fields);
...
@return DB_SUCCESS or error number */
static dberr_t row_merge_spatial_rows(trx_id_t trx_id, index_tuple_info_t **sp_tuples, ulint num_spatial,
                                      mem_heap_t *row_heap, mem_heap_t *sp_heap, btr_pcur_t *pcur, mtr_t *mtr,
                                      bool *mtr_committed)
{
...
{
  const dict_field_t *ind_field = dict_index_get_nth_field(index, 0);
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_merge_read_clustered_index(trx_t *trx, struct TABLE *table, const dict_table_t *old_table,
                                   const dict_table_t *new_table, bool online, dict_index_t **index,
                                   dict_index_t *fts_sort_idx, fts_psort_t *psort_info, merge_file_t *files,
                                   const ulint *key_numbers, ulint n_index, const dtuple_t *add_cols,
                                   const dict_add_v_col_t *add_v, const ulint *col_map, ulint add_autoinc,
                                   ib_sequence_t &sequence, row_merge_block_t *block, bool skip_pk_sort, int *tmpfd,
                                   ut_stage_alter_t *stage, struct TABLE *eval_table)
{
  ...
  btr_pcur_open_at_index_side(true, clust_index, BTR_SEARCH_LEAF, &pcur, true, 0, &mtr);
  ...
  if (old_table != new_table)
  {
    ...
    for (ulint i = 0; i < dict_table_get_n_cols(old_table); i++)
    {
      ...
      if (dict_table_get_nth_col(new_table, j)->prtype & DATA_NOT_NULL)
      {
        nonnull[n_nonnull++] = j;
      }
    }
  ...
  if (skip_pk_sort)
  {
    prev_fields = static_cast<dfield_t *>(ut_malloc_nokey(n_uniq * sizeof *prev_fields));
...
#define dbug_run_purge false
...
#endif /* NDEBUG */
      DBUG_EXECUTE_IF("ib_purge_on_create_index_page_switch", dbug_run_purge = true;);

      /* Insert the cached spatial index rows. */
      bool mtr_committed = false;

      err = row_merge_spatial_rows(trx->id, sp_tuples, num_spatial, row_heap, sp_heap, &pcur, &mtr, &mtr_committed);
      ...
      if (dbug_run_purge || rw_lock_get_waiters(dict_index_get_lock(clust_index)))
      {
        ...
        ut_ad(btr_pcur_is_on_user_rec(&pcur) || btr_pcur_get_block(&pcur)->page.id.page_no() == clust_index->page);
      ...
      else
      {
        ...
        block = btr_block_get(page_id_t(block->page.id.space(), next_page_no), block->page.size, BTR_SEARCH_LEAF,
                              clust_index, &mtr);
    ...
    if (online)
    {
      ...
      if (!trx->read_view->changes_visible(row_get_rec_trx_id(rec, clust_index, offsets), old_table->name))
      {
        rec_t *old_vers;

        row_vers_build_for_consistent_read(rec, &mtr, clust_index, &offsets, trx->read_view, &row_heap, row_heap,
                                           &old_vers, NULL);
    ...
    row = row_build_w_add_vcol(ROW_COPY_POINTERS, clust_index, rec, offsets, new_table, add_cols, add_v, col_map, &ext,
                               row_heap);
    ut_ad(row);
  ...
  write_buffers:
    ...
    ulint s_idx_cnt = 0;
    bool skip_sort = skip_pk_sort && dict_index_is_clust(merge_buf[0]->index);
    ...
    for (ulint i = 0; i < n_index; i++, skip_sort = false)
    {
      ...
      if (UNIV_LIKELY(row && (rows_added = row_merge_buf_add(buf, fts_index, old_table, new_table, psort_info, row, ext,
                                                             &doc_id, conv_heap, &err, &v_heap, eval_table, trx))))
      {
        ...
        if (skip_sort)
        {
          ...
          if (prev_mtuple.fields != NULL && (row_mtuple_cmp(&prev_mtuple, curr, &clust_dup) == 0))
          {
      ...
      ut_ad(old_table == new_table ? !dict_index_is_clust(buf->index) : (i == 0) == dict_index_is_clust(buf->index));
      ...
      if (buf->n_tuples)
      {
        if (skip_sort)
        {
          ...
          if (row != NULL)
          {
            ...
            invalid. */
            err =
                row_merge_spatial_rows(trx->id, sp_tuples, num_spatial, row_heap, sp_heap, &pcur, &mtr, &mtr_committed);
          ...
          err = row_merge_insert_index_tuples(trx->id, index[i], old_table, -1, NULL, buf, clust_btr_bulk);
      ...
      if (!skip_sort && !(buf->index->type & DICT_FTS))
      {
        ...
        {
          DBUG_EXECUTE_IF("row_merge_write_failure", err = DB_TEMP_FILE_WRITE_FAIL; trx->error_key_num = i;
                          goto all_done;);
          ...
          err = row_merge_insert_index_tuples(trx->id, index[i], old_table, -1, NULL, buf, &btr_bulk);
        ...
        {
          if (row_merge_file_create_if_needed(file, tmpfd, buf->n_tuples, path) < 0)
          {
      ...
      if (UNIV_LIKELY(row != NULL))
      {
        ...
        if (UNIV_UNLIKELY(!(rows_added = row_merge_buf_add(buf, fts_index, old_table, new_table, psort_info, row, ext,
                                                           &doc_id, conv_heap, &err, &v_heap, table, trx))))
        {
...
all_done:
  ...
  wait_again:
    ...
    for (ulint i = 0; i < fts_sort_pll_degree; i++)
    {
      if (psort_info[i].child_status != FTS_CHILD_COMPLETE && psort_info[i].child_status != FTS_CHILD_EXITING)
      {
  ...
  if (max_doc_id && err == DB_SUCCESS)
  {
    ...
    err = fts_sync_table(const_cast<dict_table_t *>(new_table), false, true, false);
...
#define ROW_MERGE_WRITE_GET_NEXT_LOW(N, INDEX, AT_END)                                                              \
  do                                                                                                                \
  {                                                                                                                 \
    b2 = row_merge_write_rec(&block[2 * srv_sort_buf_size], &buf[2], b2, of->fd, &of->offset, mrec##N, offsets##N); \
    if (UNIV_UNLIKELY(!b2 || ++of->n_rec > file->n_rec))                                                            \
    {                                                                                                               \
      goto corrupt;                                                                                                 \
    }                                                                                                               \
    b##N = row_merge_read_rec(&block[N * srv_sort_buf_size], &buf[N], b##N, INDEX, file->fd, foffs##N, &mrec##N,    \
                              offsets##N);                                                                          \
...
#define ROW_MERGE_WRITE_GET_NEXT(N, INDEX, AT_END) ROW_MERGE_WRITE_GET_NEXT_LOW(N, INDEX, AT_END)
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_merge_blocks(const row_merge_dup_t *dup, const merge_file_t *file, row_merge_block_t *block, ulint *foffs0,
                     ulint *foffs1, merge_file_t *of, ut_stage_alter_t *stage)
{
  ...
  DBUG_PRINT("ib_merge_sort",
             ("fd=%d,%lu+%lu to fd=%d,%lu", file->fd, ulong(*foffs0), ulong(*foffs1), of->fd, ulong(of->offset)));
  ...
  if (!row_merge_read(file->fd, *foffs0, &block[0]) || !row_merge_read(file->fd, *foffs1, &block[srv_sort_buf_size]))
  {
  corrupt:
    mem_heap_free(heap);
  ...
  b0 = row_merge_read_rec(&block[0], &buf[0], b0, dup->index, file->fd, foffs0, &mrec0, offsets0);
  b1 = row_merge_read_rec(&block[srv_sort_buf_size], &buf[srv_sort_buf_size], b1, dup->index, file->fd, foffs1, &mrec1,
                          offsets1);
  ...
  while (mrec0 && mrec1)
  {
    int cmp = cmp_rec_rec_simple(mrec0, mrec1, offsets0, offsets1, dup->index, dup->table);
    if (cmp < 0)
    {
...
static MY_ATTRIBUTE((warn_unused_result)) ibool
    row_merge_blocks_copy(const dict_index_t *index, const merge_file_t *file, row_merge_block_t *block, ulint *foffs0,
                          merge_file_t *of, ut_stage_alter_t *stage)
{
  ...
  DBUG_PRINT("ib_merge_sort", ("fd=%d," ULINTPF " to fd=%d," ULINTPF, file->fd, *foffs0, of->fd, of->offset));
...
done0:
  ...
  mem_heap_free(heap);
  DBUG_RETURN(row_merge_write_eof(&block[2 * srv_sort_buf_size], b2, of->fd, &of->offset) != NULL);
}
...
processed.
@return DB_SUCCESS or error code */
static dberr_t row_merge(trx_t *trx, const row_merge_dup_t *dup, merge_file_t *file, row_merge_block_t *block,
                         int *tmpfd, ulint *num_run, ulint *run_offset, ut_stage_alter_t *stage)
{
...
@return DB_SUCCESS or error code */
dberr_t row_merge_sort(trx_t *trx, const row_merge_dup_t *dup, merge_file_t *file, row_merge_block_t *block, int *tmpfd,
                       ut_stage_alter_t *stage /* = NULL */)
{
  ...
  do
  {
    error = row_merge(trx, dup, file, block, tmpfd, &num_runs, run_offset, stage);
...
static void row_merge_copy_blobs(const mrec_t *mrec, const ulint *offsets, const page_size_t &page_size,
                                 dtuple_t *tuple, mem_heap_t *heap)
{
  ...
  for (ulint i = 0; i < dtuple_get_n_fields(tuple); i++)
  {
    ...
    if (mrec == NULL)
    {
      ...
      data = btr_copy_externally_stored_field(&len, field_data, page_size, field_len, heap);
    }
    else
    {
      data = btr_rec_copy_externally_stored_field(mrec, offsets, page_size, i, &len, heap);
    }
                                  ...
                                  ut_stage_alter_t *stage /* = NULL */)
{
  ...
  if (row_buf != NULL)
  {
    ut_ad(fd == -1);
    ut_ad(block == NULL);
    DBUG_EXECUTE_IF("row_merge_read_failure", error = DB_CORRUPTION; goto err_exit;);
    buf = NULL;
    b = NULL;
  ...
  for (;;)
  {
    ...
    dict_index_t *old_index = dict_table_get_first_index(old_table);

    if (dict_index_is_clust(index) && dict_index_is_online_ddl(old_index))
    {
    ...
    else
    {
      ...
      row_merge_copy_blobs(mrec, offsets, dict_table_page_size(old_table), dtuple, tuple_heap);
    }
...
static void row_merge_drop_index_dict(
    /*======================*/
...
/*********************************************************************/ /**
 ...
 The data dictionary must have been locked exclusively by the caller,
 because the transaction will not be committed. */
void row_merge_drop_indexes_dict(
    /*========================*/
    trx_t *trx,          /*!< in/out: dictionary transaction */
    table_id_t table_id) /*!< in: table identifier */
{
  ...
  switch (error)
  {
    ...
    default:
      ...
      and DEADLOCK free, we could encounter other errors e.g.,
      DB_TOO_MANY_CONCURRENT_TRXS. */
      ib::error() << "row_merge_drop_indexes_dict failed with error " << error;
      /* fall through */
...
void row_merge_drop_temp_indexes(void)
/*=============================*/
{
  static const char sql[] =
...
int row_merge_file_create_low(const char *path)
{
  ...
  locker = PSI_FILE_CALL(get_thread_file_name_locker)(&state, innodb_temp_file_key.m_value, PSI_FILE_OPEN,
                                                      df.filepath(), &locker);
  if (locker != NULL)
  {
    ...
    int fd) /*!< in: merge file descriptor */
{
  ...
  struct PSI_file_locker *locker = NULL;
  PSI_file_locker_state state;
  locker = PSI_FILE_CALL(get_thread_file_descriptor_locker)(&state, fd, PSI_FILE_CLOSE);
  if (locker != NULL)
  {
...
dberr_t row_merge_rename_index_to_add(
    /*==========================*/
    trx_t *trx,          /*!< in/out: transaction */
    table_id_t table_id, /*!< in: table identifier */
    index_id_t index_id) /*!< in: index identifier */
...
/*********************************************************************/ /**
 ...
 the transaction will not be committed.
 @return DB_SUCCESS if all OK */
dberr_t row_merge_rename_index_to_drop(
    /*===========================*/
    trx_t *trx,          /*!< in/out: transaction */
    table_id_t table_id, /*!< in: table identifier */
    index_id_t index_id) /*!< in: index identifier */
    ...
    trx_t *trx)              /*!< in/out: dictionary transaction */
{
  ...
  ut_ad(mutex_own(&dict_sys->mutex));
  ut_a(trx->dict_operation_lock_mode == RW_X_LATCH);
  ut_ad(trx_get_dict_operation(trx) == TRX_DICT_OP_TABLE || trx_get_dict_operation(trx) == TRX_DICT_OP_INDEX);
  ...
  if (err == DB_SUCCESS && dict_table_is_file_per_table(old_table) && !old_table->ibd_file_missing)
  {
    ...
    pars_info_add_int4_literal(info, "old_space", (lint)old_table->space);
  ...
  renamed is a single-table tablespace, which must be implicitly
  renamed along with the table. */
  if (err == DB_SUCCESS && dict_table_is_file_per_table(new_table))
  {
    /* Make pathname to update SYS_DATAFILES. */
    char *old_path = row_make_new_pathname(new_table, old_table->name.m_name);

    info = pars_info_create();

    pars_info_add_str_literal(info, "old_name", old_table->name.m_name);
    pars_info_add_str_literal(info, "old_path", old_path);
    pars_info_add_int4_literal(info, "new_space", (lint)new_table->space);
  ...
  {
    err = row_import_update_discarded_flag(trx, new_table->id, true, true);
  }
...
@return DB_SUCCESS or error code */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_merge_create_index_graph(trx_t *trx, dict_table_t *table, dict_index_t *index, const dict_add_v_col_t *add_v)
{
...
dict_index_t *row_merge_create_index(trx_t *trx, dict_table_t *table, const index_def_t *index_def,
                                     const dict_add_v_col_t *add_v)
{
  ...
  index = dict_mem_index_create(table->name.m_name, index_def->name, 0, index_def->ind_type, n_fields);
  ...
  if (err == DB_SUCCESS)
  {
    index = dict_table_get_index_on_name(table, index_def->name, index_def->rebuild);
...
{
  if (!dict_index_is_clust(index) && dict_index_is_online_ddl(index))
  {
  ...
  return (!dict_index_is_corrupted(index) &&
          (dict_table_is_temporary(index->table) || index->trx_id == 0 || !MVCC::is_view_active(trx->read_view) ||
           trx->read_view->changes_visible(index->trx_id, index->table->name)));
}
    ...
    dict_table_t *table) /*!< in: table to drop */
{
  ...
  return (row_drop_table_for_mysql(table->name.m_name, trx, false, false));
}
...
static void row_merge_write_redo(const dict_index_t *index)
{
  ...
  log_ptr = mlog_write_initial_log_record_low(MLOG_INDEX_LOAD, index->space, index->page, log_ptr, &mtr);
...
@param[in]	eval_table	mysql table used to evaluate virtual column
                                value, see innobase_get_computed_value().
@return DB_SUCCESS or error code */
dberr_t row_merge_build_indexes(trx_t *trx, dict_table_t *old_table, dict_table_t *new_table, bool online,
                                dict_index_t **indexes, const ulint *key_numbers, ulint n_indexes, struct TABLE *table,
                                const dtuple_t *add_cols, const ulint *col_map, ulint add_autoinc,
                                ib_sequence_t &sequence, bool skip_pk_sort, ut_stage_alter_t *stage,
                                const dict_add_v_col_t *add_v, struct TABLE *eval_table)
{
  ...
  for (i = 0; i < n_indexes; i++)
  {
    if (indexes[i]->type & DICT_FTS)
    {
      ...
      fts_sort_idx = row_merge_create_fts_sort_index(indexes[i], old_table, &opt_doc_id_size);

      row_merge_dup_t *dup = static_cast<row_merge_dup_t *>(ut_malloc_nokey(sizeof *dup));
      dup->index = fts_sort_idx;
      dup->table = table;
      dup->col_map = col_map;
      dup->n_dup = 0;

      row_fts_psort_info_init(trx, dup, new_table, opt_doc_id_size, &psort_info, &merge_info);
  ...
  error = row_merge_read_clustered_index(trx, table, old_table, new_table, online, indexes, fts_sort_idx, psort_info,
                                         merge_files, key_numbers, n_indexes, add_cols, add_v, col_map, add_autoinc,
  ...
  for (i = 0; i < n_indexes; i++)
  {
    ...
    if (indexes[i]->type & DICT_FTS)
    {
      ...
      wait_again:
        ...
        {
          if (merge_info[j].child_status != FTS_CHILD_COMPLETE && merge_info[j].child_status != FTS_CHILD_EXITING)
          {
        ...
        else
        {
          for (j = 0; j < FTS_NUM_AUX_INDEX; j++)
          {
            os_thread_join(merge_info[j].thread_hdl);
          }
        }
      }
      else
    ...
    else if (merge_files[i].fd >= 0)
    {
      ...
      if (error == DB_SUCCESS)
      {
        ...
        error = row_merge_insert_index_tuples(trx->id, sort_idx, old_table, merge_files[i].fd, block, NULL, &btr_bulk,
                                              stage);
...
func_exit:
  DBUG_EXECUTE_IF("ib_build_indexes_too_many_concurrent_trxs", error = DB_TOO_MANY_CONCURRENT_TRXS;
  ...
  if (flush_observer != NULL)
  {
    ...
    {
      for (const dict_index_t *index = dict_table_get_first_index(new_table); index != NULL;
