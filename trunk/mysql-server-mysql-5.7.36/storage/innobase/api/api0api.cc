...
/********************************************************************/ /**
 ...
 @return table instance if found */
static dict_table_t *ib_open_table_by_name(
    /*==================*/
    const char *name) /*!< in: table name to lookup */
{
  dict_table_t *table;

  table = dict_table_open_on_name(name, FALSE, FALSE, DICT_ERR_IGNORE_NONE);
    ...
    ulint *len)            /*!< in/out: buffer len */
{
  ...
  for (i = 0; i < n_index_fields; ++i)
  {
    ...
    if (rec_offs_nth_extern(offsets, i))
    {
      ...
      data = btr_rec_copy_externally_stored_field(copy, offsets, page_size, i, &len, tuple->heap);
      ...
      if (data == NULL)
      {
        ...
        ut_ad(ib_cfg_trx_level() == IB_TRX_READ_UNCOMMITTED);
        len = 0;
      }
    }
                                 ...
                                 single DML */
{
  ...
  started = ib_trx_start(static_cast<ib_trx_t>(trx), ib_trx_level, read_write, auto_commit, NULL);
  ut_a(started);
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS or err code */
static ib_err_t ib_create_cursor_with_index_id(
    /*===========================*/
    ib_crsr_t *ib_crsr,   /*!< out: InnoDB cursor */
    dict_table_t *table,  /*!< in: table instance */
    ...
    ib_crsr_t *ib_crsr)   /*!< out,own: InnoDB cursor */
{
  ib_err_t err;
  dict_table_t *table;
  const ib_bool_t locked = ib_trx && ib_schema_lock_is_exclusive(ib_trx);
  ...
  err = ib_create_cursor_with_index_id(ib_crsr, table, 0, (trx_t *)ib_trx);
    ...
    ib_id_u64_t *idx_id)    /*!< out: index id */
{
  ...
  *ib_crsr = NULL;
  ...
  table = dict_table_open_on_id(cursor->prebuilt->table->id, FALSE, DICT_TABLE_OP_NORMAL);
  ut_a(table != NULL);
...
ib_bool_t ib_is_virtual_table(ib_crsr_t crsr) { return (crsr->prebuilt->table->n_v_cols > 0); }
    ...
    ib_crsr_t ib_crsr) /*!< in/out: InnoDB cursor */
{
  ...
  if (cursor->valid_trx && prebuilt->trx != NULL && prebuilt->trx->n_mysql_tables_in_use > 0)
  {
    --prebuilt->trx->n_mysql_tables_in_use;
  }
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS or err code */
ib_err_t ib_cursor_close_table(
    /*==================*/
    ib_crsr_t ib_crsr) /*!< in,own: InnoDB cursor */
{
  ib_cursor_t *cursor = (ib_cursor_t *)ib_crsr;
                          ...
                          in case of an error */
{
  ...
  do
  {
    ...
    if (err != DB_SUCCESS)
    {
      ...
      lock_wait = static_cast<ib_bool_t>(ib_handle_errors(&err, trx, thr, savept));
    ...
    ib_cursor_t *cursor) /*!< in: Cursor instance */
{
  ...
  if (node->ins == NULL)
  {
    ...
    grph->ins =
        static_cast<que_fork_t *>(que_node_get_parent(pars_complete_graph_for_exec(node->ins, trx, heap, NULL)));
    ...
    ib_cursor_t *cursor) /*!< in: current cursor */
{
  ...
  if (node->upd == NULL)
  {
    node->upd = static_cast<upd_node_t *>(row_create_update_node_for_mysql(table, heap));
  }

  ut_ad(!dict_table_have_virtual_index(table));

  grph->upd = static_cast<que_fork_t *>(que_node_get_parent(pars_complete_graph_for_exec(node->upd, trx, heap, NULL)));
    ...
    ib_ulint_t *row_len) /*!< in/out: row buffer len */
{
  ...
  else
  {
    ...
    if (btr_pcur_restore_position(BTR_SEARCH_LEAF, pcur, &mtr))
    {
      ...
      page_format = static_cast<ib_bool_t>(dict_table_is_comp(tuple->index->table));
      rec = btr_pcur_get_rec(pcur);
      ...
      if (!rec_get_deleted_flag(rec, page_format))
      {
        ib_read_tuple(rec, page_format, tuple, row_buf, (ulint *)row_len);
        err = DB_SUCCESS;
      }
      else
      {
    ...
    ib_srch_mode_t mode) /*!< in: Search mode */
{
  ...
  err = static_cast<ib_err_t>(row_search_for_mysql(buf, static_cast<page_cur_mode_t>(mode), prebuilt, 0, 0));
    ...
    ib_crsr_t ib_crsr) /*!< in: InnoDB cursor instance */
{
  ...
  err = static_cast<ib_err_t>(row_search_for_mysql(buf, PAGE_CUR_G, prebuilt, 0, ROW_SEL_NEXT));
    ...
    ib_srch_mode_t ib_srch_mode) /*!< in: search mode */
{
  ...
  err = static_cast<ib_err_t>(
      row_search_for_mysql(buf, static_cast<page_cur_mode_t>(ib_srch_mode), prebuilt, cursor->match_mode, 0));
...
{
  return (static_cast<ib_err_t>((dtype_get_mtype(dtype) == DATA_VARCHAR || dtype_get_mtype(dtype) == DATA_CHAR ||
                                 dtype_get_mtype(dtype) == DATA_MYSQL || dtype_get_mtype(dtype) == DATA_VARMYSQL ||
  ...
  switch (dtype_get_mtype(dtype))
  {
    case DATA_INT:
    {
      if (col_len == len)
      {
        ...
        mach_write_int_type(static_cast<byte *>(dst), static_cast<const byte *>(src), len, usign);
      }
      else
      {
        return (DB_DATA_MISMATCH);
    ...
    case DATA_VARMYSQL:
    {
      ...
      if (cs)
      {
        ...
        {
          true_len = (ulint)cs->cset->well_formed_len(cs, (const char *)src, (const char *)src + len, pos, &error);
    ...
    ib_ulint_t i)    /*!< in: column index in tuple */
{
  ...
  return (static_cast<ib_ulint_t>(data_len == UNIV_SQL_NULL ? IB_SQL_NULL : data_len));
}
  ...
  if (data_len != UNIV_SQL_NULL)
  {
    ...
    switch (dtype_get_mtype(dfield_get_type(dfield)))
    {
      case DATA_INT:
      {
        ...
        usign = dtype_get_prtype(dtype) & DATA_UNSIGNED;
        ret = mach_read_int_type(static_cast<const byte *>(data), data_len, usign);
    ...
    ib_col_meta_t *ib_col_meta) /*!< out: column meta data */
{
  ...
  ib_col_meta->type_len = static_cast<ib_u32_t>(dtype_get_len(dfield_get_type(dfield)));
    ...
    const ib_tpl_t ib_tpl) /*!< in: Tuple for current table */
{
  ...
  if (tuple->type == TPL_TYPE_ROW)
  {
    return (static_cast<ib_ulint_t>(dict_table_get_n_user_cols(tuple->index->table)));
  }

  return (static_cast<ib_ulint_t>(dict_index_get_n_ordering_defined_by_user(tuple->index)));
}
...
ib_bool_t ib_schema_lock_is_shared(
    /*=====================*/
    const ib_trx_t ib_trx) /*!< in: transaction */
{
  const trx_t *trx = (const trx_t *)ib_trx;
    ...
    ib_lck_mode_t ib_lck_mode) /*!< in: InnoDB lock mode */
{
  ...
  err = static_cast<ib_err_t>(lock_table(0, table, (enum lock_mode)ib_lck_mode, thr));
  ...
  *table_id = 0;
  ...
  if (err == DB_SUCCESS)
  {
    ...
    *ib_crsr = NULL;
    ...
    on success. */
    err = static_cast<ib_err_t>(row_truncate_table_for_mysql(table, trx));
    ...
    ib_id_u64_t *table_id)  /*!< out: new table id */
{
  ...
  table = dict_table_open_on_name(table_name, TRUE, FALSE, DICT_ERR_IGNORE_NONE);
  ...
  {
    memcached_sync = static_cast<ib_ulint_t>(table->memcached_sync_count);
...
ib_err_t ib_cursor_set_memcached_sync(
    /*=========================*/
    ib_crsr_t ib_crsr, /*!< in: cursor */
