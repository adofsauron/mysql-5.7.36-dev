    ...
    const char *name) /*!< in: filename to check */
{
  ...
  if (err == DB_FAIL)
  {
    ib::error() << "os_file_get_status() failed on '" << name << "'. Can't determine file permissions.";
    return (false);
  }
...
extern "C" os_thread_ret_t DECLARE_THREAD(io_handler_thread)(
    /*==============================*/
               ...
               the aio array */
{
  ...
  ib::info() << "Io handler thread " << segment << " starts, id " << os_thread_pf(os_thread_get_curr_id());
#endif
  ...
  else if (segment >= start && segment < (start + srv_n_read_io_threads))
  {
    pfs_register_thread(io_read_thread_key);
  }
  else if (segment >= (start + srv_n_read_io_threads) &&
           segment < (start + srv_n_read_io_threads + srv_n_write_io_threads))
  {
  ...
  while (srv_shutdown_state != SRV_SHUTDOWN_EXIT_THREADS || buf_page_cleaner_is_active || !os_aio_all_slots_free())
  {
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t create_log_file(
    /*============*/
    ...
    const char *name)    /*!< in: log file name */
{
  ...
  *file = os_file_create(innodb_log_file_key, name, OS_FILE_CREATE | OS_FILE_ON_ERROR_NO_EXIT, OS_FILE_NORMAL,
  ...
  ib::info() << "Setting log file " << name << " size to " << (srv_log_file_size >> (20 - UNIV_PAGE_SIZE_SHIFT))
             << " MB";

  ret = os_file_set_size(name, *file, (os_offset_t)srv_log_file_size << UNIV_PAGE_SIZE_SHIFT, srv_read_only_mode);
  if (!ret)
  {
    ib::error() << "Cannot set log file " << name << " to size " << (srv_log_file_size >> (20 - UNIV_PAGE_SIZE_SHIFT))
                << " MB";
    return (DB_ERROR);
  }
    ...
    char *&logfile0)   /*!< out: name of the first log file */
{
  ...
  fil_space_t *log_space = fil_space_create("innodb_redo_log", SRV_LOG_SPACE_FIRST_ID,
                                            fsp_flags_set_page_size(0, univ_page_size), FIL_TYPE_LOG);
  ut_a(fil_validate());
  ut_a(log_space != NULL);

  logfile0 = fil_node_create(logfilename, (ulint)srv_log_file_size, log_space, false, false);
  ut_a(logfile0);
  ...
  for (unsigned i = 1; i < srv_n_log_files; i++)
  {
    ...
    if (!fil_node_create(logfilename, (ulint)srv_log_file_size, log_space, false, false))
    {
      ib::error() << "Cannot create file node for log file " << logfilename;

      return (DB_ERROR);
    }
  }

  if (!log_group_init(0, srv_n_log_files, srv_log_file_size * UNIV_PAGE_SIZE, SRV_LOG_SPACE_FIRST_ID))
  {
    return (DB_ERROR);
  }
...
/*********************************************************************/ /**
 Opens a log file.
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t open_log_file(
    /*==========*/
    pfs_os_file_t *file, /*!< out: file handle */
    const char *name,    /*!< in: log file name */
    os_offset_t *size)   /*!< out: file size */
{
  ...
  *file = os_file_create(innodb_log_file_key, name, OS_FILE_OPEN, OS_FILE_AIO, OS_LOG_FILE, srv_read_only_mode, &ret);
  if (!ret)
  {
    ...
    ulint size)       /*!< in: tablespace size in pages */
{
  ...
  fh = os_file_create(innodb_data_file_key, name, srv_read_only_mode ? OS_FILE_OPEN : OS_FILE_CREATE, OS_FILE_NORMAL,
  ...
  else
  {
    ...
    ib::info() << "Setting file " << name << " size to " << (size >> (20 - UNIV_PAGE_SIZE_SHIFT)) << " MB";

    ib::info() << "Database physically writes the file full: "
               << "wait...";

    ret = os_file_set_size(name, fh, size << UNIV_PAGE_SIZE_SHIFT, srv_read_only_mode);

    if (!ret)
    {
      ib::info() << "Error in creating " << name << ": probably out of disk space";
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static dberr_t srv_undo_tablespace_open(
    /*=====================*/
    ...
    ulint space_id)   /*!< in: tablespace id */
{
  ...
  ut_snprintf(undo_name, sizeof(undo_name), "innodb_undo%03u", static_cast<unsigned>(space_id));

  if (!srv_file_check_mode(name))
  {
    ib::error() << "UNDO tablespaces must be " << (srv_read_only_mode ? "writable" : "readable") << "!";

    return (DB_ERROR);
  }

  fh = os_file_create(innodb_data_file_key, name,
                      OS_FILE_OPEN_RETRY | OS_FILE_ON_ERROR_NO_EXIT | OS_FILE_ON_ERROR_SILENT, OS_FILE_NORMAL,
                      OS_DATA_FILE, srv_read_only_mode, &ret);
  ...
  if (ret)
  {
    ...
    32 bits. */
    if (fil_node_create(name, (ulint)n_pages, space, false, atomic_write))
    {
      err = DB_SUCCESS;
    }
  }
...
static dberr_t srv_check_undo_redo_logs_exists()
{
  ...
  {
    ut_snprintf(name, sizeof(name), "%s%cundo%03lu", srv_undo_dir, OS_PATH_SEPARATOR, i);
  ...
  for (unsigned i = 0; i < srv_n_log_files; i++)
  {
    ...
    fh = os_file_create(innodb_log_file_key, logfilename,
                        OS_FILE_OPEN_RETRY | OS_FILE_ON_ERROR_NO_EXIT | OS_FILE_ON_ERROR_SILENT, OS_FILE_NORMAL,
                        OS_LOG_FILE, srv_read_only_mode, &ret);
...
@return DB_SUCCESS or error code */
static dberr_t srv_undo_tablespaces_init(
    /*======================*/
                                    ...
                                    discovered and opened */
{
  ...
  *n_opened = 0;
  ...
  for (i = 0; create_new_db && i < n_conf_tablespaces; ++i)
  {
    ...
    ut_snprintf(name, sizeof(name), "%s%cundo%03lu", srv_undo_dir, OS_PATH_SEPARATOR, space_id);
  ...
  if (!create_new_db)
  {
    n_undo_tablespaces = trx_rseg_get_n_undo_tablespaces(undo_tablespace_ids);
    ...
    for (i = 0; i < n_undo_tablespaces; ++i)
    {
      ...
      if (undo_trunc.needs_fix_up(undo_tablespace_ids[i]))
      {
        ...
        ut_snprintf(name, sizeof(name), "%s%cundo%03lu", srv_undo_dir, OS_PATH_SEPARATOR, undo_tablespace_ids[i]);
        ...
        undo::Truncate::s_fix_up_spaces.push_back(undo_tablespace_ids[i]);
      }
    }
  }
  else
  ...
  for (i = 0; i < n_undo_tablespaces; ++i)
  {
    ...
    if (err != DB_SUCCESS)
    {
      ib::error() << "Unable to open undo tablespace '" << name << "'.";
      return (err);
    }
  ...
  {
    ib::error() << "Expected to open " << n_conf_tablespaces << " undo tablespaces but was able to find only "
                << n_undo_tablespaces
  ...
  {
    ib::info() << "Opened " << n_undo_tablespaces << " undo tablespaces";
  ...
  if (create_new_db)
  {
    ...
    for (i = 0; i < n_undo_tablespaces; ++i)
    {
      fsp_header_init(undo_tablespace_ids[i], SRV_UNDO_TABLESPACE_SIZE_IN_PAGES, &mtr);
    }
  ...
  if (!undo::Truncate::s_fix_up_spaces.empty())
  {
    ...
    for (undo::undo_spaces_t::const_iterator it = undo::Truncate::s_fix_up_spaces.begin();
         it != undo::Truncate::s_fix_up_spaces.end(); ++it)
    {
      undo::Truncate::add_space_to_trunc_list(*it);

      fsp_header_init(*it, SRV_UNDO_TABLESPACE_SIZE_IN_PAGES, &mtr);

      mtr_x_lock(fil_space_get_latch(*it, NULL), &mtr);

      for (ulint i = 0; i < TRX_SYS_N_RSEGS; i++)
      {
        ulint space_id = trx_sysf_rseg_get_space(sys_header, i, &mtr);

        if (space_id == *it)
        {
          trx_rseg_header_create(*it, univ_page_size, ULINT_MAX, i, &mtr);
        }
      }
         ...
         it != undo::Truncate::s_fix_up_spaces.end(); ++it)
    {
      ...
      buf_LRU_flush_or_remove_pages(*it, BUF_REMOVE_FLUSH_WRITE, NULL);
...
/*===============================*/
{
  ...
  while (srv_shutdown_state == SRV_SHUTDOWN_NONE && srv_force_recovery < SRV_FORCE_NO_BACKGROUND &&
...
static dberr_t srv_open_tmp_tablespace(bool create_new_db, SysTablespace *tmp_space)
{
  ...
  dberr_t err = tmp_space->check_file_spec(&create_new_temp_space, 12 * 1024 * 1024);

  if (err == DB_FAIL)
  {
    ib::error() << "The " << tmp_space->name() << " data file must be writable!";

    err = DB_ERROR;
  }
  else if (err != DB_SUCCESS)
  {
    ib::error() << "Could not create the shared " << tmp_space->name() << ".";
  }
  else if ((err = tmp_space->open_or_create(true, create_new_db, &sum_of_new_sizes, NULL)) != DB_SUCCESS)
  {
    ib::error() << "Unable to create the shared " << tmp_space->name();
  }
  else
  {
    mtr_t mtr;
...
static lsn_t srv_prepare_to_delete_redo_log_files(ulint n_files)
{
  ...
  do
  {
...

    {
      ...
      {
        warning << "Resizing redo log from " << n_files << "*" << srv_log_file_size << " to ";
      }
      warning << srv_n_log_files << "*" << srv_log_file_size_requested << " pages, LSN=" << flushed_lsn;
    }
...
@return DB_SUCCESS or error code */
dberr_t innobase_start_or_create_for_mysql(void)
/*====================================*/
{
  ...
  high_level_read_only = srv_read_only_mode || srv_force_recovery > SRV_FORCE_NO_TRX_UNDO;
  ...
  {
    ib::error() << "Size of InnoDB's ulint is " << sizeof(ulint) << ", but size of void* is " << sizeof(void *)
  ...
  {
    ib::error() << "Unrecognized value " << srv_file_flush_method_str << " for innodb_flush_method";
  ...
  srv_max_n_threads = 1     /* io_ibuf_thread */
                      ...
                      + max_connections + srv_n_read_io_threads + srv_n_write_io_threads + srv_n_purge_threads +
                      srv_n_page_cleaners
                      /* FTS Parallel Sort */
  ...
  if (srv_buf_pool_size >= BUF_POOL_SIZE_THRESHOLD)
  {
    if (srv_buf_pool_instances == srv_buf_pool_instances_default)
    {
      ...
      srv_buf_pool_instances =
          ut_min(static_cast<ulong>(MAX_BUFFER_POOLS), static_cast<ulong>(srv_buf_pool_size / (128 * 1024 * 1024)));
  ...
  else
  {
    ...
    if (srv_buf_pool_instances != srv_buf_pool_instances_default && srv_buf_pool_instances != 1)
    {
  ...
  if (srv_buf_pool_chunk_unit * srv_buf_pool_instances > srv_buf_pool_size)
  {
    ...
    srv_buf_pool_chunk_unit = static_cast<ulong>(srv_buf_pool_size) / srv_buf_pool_instances;
  ...
  ib::info() << (ut_crc32_sse2_enabled ? "Using" : "Not using") << " CPU crc32 instructions";
  ...
  if (!srv_read_only_mode)
  {
    ...
    if (srv_innodb_status)
    {
      srv_monitor_file_name =
          static_cast<char *>(ut_malloc_nokey(strlen(fil_path_to_mysql_datadir) + 20 + sizeof "/innodb_status."));

      sprintf(srv_monitor_file_name, "%s/innodb_status." ULINTPF, fil_path_to_mysql_datadir, os_proc_get_number());

      srv_monitor_file = fopen(srv_monitor_file_name, "w+");

      if (!srv_monitor_file)
      {
        ib::error() << "Unable to create " << srv_monitor_file_name << ": " << strerror(errno);
        return (srv_init_abort(DB_ERROR));
      }
    }
    else
  ...
  if (!os_aio_init(srv_n_read_io_threads, srv_n_write_io_threads, SRV_MAX_N_PENDING_SYNC_IOS))
  {
  ...
  ib::info() << "Initializing buffer pool, total size = " << size << unit << ", instances = " << srv_buf_pool_instances
  ...
  if (srv_n_log_files * srv_log_file_size * UNIV_PAGE_SIZE >= 512ULL * 1024ULL * 1024ULL * 1024ULL)
  {
  ...
  if (srv_n_log_files * srv_log_file_size >= ULINT_MAX)
  {
    ...
    ib::error() << "Combined size of log files must be < " << ULINT_MAX / 1073741824 * UNIV_PAGE_SIZE << " GB";
  ...
  err = srv_sys_space.check_file_spec(&create_new_db, MIN_EXPECTED_TABLESPACE_SIZE);
  ...
  else
  {
    for (i = 0; i < SRV_N_LOG_FILES_MAX; i++)
    {
      ...
      err = os_file_get_status(logfilename, &stat_info, false, srv_read_only_mode);
      ...
      if (err == DB_NOT_FOUND)
      {
        if (i == 0)
        {
          ...
          create_log_files_rename(logfilename, dirnamelen, flushed_lsn, logfile0);
      ...
      {
        ib::error() << "Log file " << logfilename << " size " << size
                    << " is not a"
      ...
      {
        ib::error() << "Log file " << logfilename << " is of different size " << (size << UNIV_PAGE_SIZE_SHIFT)
                    << " bytes than other log files " << (srv_log_file_size << UNIV_PAGE_SIZE_SHIFT) << " bytes!";
        return (srv_init_abort(DB_ERROR));
      }
    }
    ...
    if (!log_group_init(0, i, srv_log_file_size * UNIV_PAGE_SIZE, SRV_LOG_SPACE_FIRST_ID))
    {
      return (srv_init_abort(DB_ERROR));
    }
  }
...
files_checked:
  ...
  else
  {
    ...
    process to work. */
    err = trx_sys_file_format_max_check(srv_max_file_format_at_startup);
    ...
    {
      ib::warn() << "The log file may have been corrupt and it"
    ...
    rotation. */
    if (!srv_read_only_mode && srv_force_recovery < SRV_FORCE_NO_LOG_REDO)
    {
    ...
    if (!srv_force_recovery && !recv_sys->found_corrupt_log &&
        (srv_log_file_size_requested != srv_log_file_size || srv_n_log_files_found != srv_n_log_files))
    {
  ...
  srv_available_undo_logs = trx_sys_create_rsegs(srv_undo_tablespaces, srv_rollback_segments, srv_tmp_undo_logs);
  ...
  else if (srv_available_undo_logs < srv_rollback_segments && !srv_force_recovery && !recv_needed_recovery)
  {
  ...
  if (!srv_read_only_mode)
  {
    ...
    for lock waits */
    os_thread_create(lock_wait_timeout_thread, NULL, thread_ids + 2 + SRV_MAX_N_IO_THREADS);
  ...
  {
    os_thread_create(srv_purge_coordinator_thread, NULL, thread_ids + 5 + SRV_MAX_N_IO_THREADS);

    ut_a(UT_ARR_SIZE(thread_ids) > 5 + srv_n_purge_threads + SRV_MAX_N_IO_THREADS);

    /* We've already created the purge coordinator thread above. */
    for (i = 1; i < srv_n_purge_threads; ++i)
    {
      os_thread_create(srv_worker_thread, NULL, thread_ids + 5 + i + SRV_MAX_N_IO_THREADS);
    }
  ...
  {
    ib::error() << "Tablespace size stored in header is " << tablespace_size_in_header
                << " pages, but the sum"
                   " of data file sizes is "
  ...
  {
    ib::info() << INNODB_VERSION_STR << " started; log sequence number " << srv_start_lsn;
  }
...
/****************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
dberr_t innobase_shutdown_for_mysql(void)
/*=============================*/
{
  if (!srv_was_started)
  {
  ...
  {
    ib::warn() << "Query counter shows " << srv_conc_get_active_threads()
               << " queries still"
  ...
  if (srv_print_verbose_log)
  {
    ib::info() << "Shutdown completed; log sequence number " << srv_shutdown_lsn;
  }
...
so. */
void srv_shutdown_table_bg_threads(void)
/*===============================*/
{
  dict_table_t *table;
  dict_table_t *first;
...
void srv_get_meta_data_filename(dict_table_t *table, char *filename, ulint max_len)
{
  ulint len;
  char *path;
  ...
  if (DICT_TF_HAS_DATA_DIR(table->flags))
  {
    ...
    path = fil_make_filepath(table->data_dir_path, table->name.m_name, CFG, true);
  }
  else
  {
...
void srv_get_encryption_data_filename(dict_table_t *table, char *filename, ulint max_len)
{
  ...
  if (DICT_TF_HAS_DATA_DIR(table->flags))
  {
    ...
    path = fil_make_filepath(table->data_dir_path, table->name.m_name, CFP, true);
  }
  else
  {
