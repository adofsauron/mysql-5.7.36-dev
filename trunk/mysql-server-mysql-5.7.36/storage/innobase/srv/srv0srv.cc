...
#define fetch_lock_wait_timeout(trx) ((trx)->lock.allowed_to_wait ? thd_lock_wait_timeout((trx)->mysql_thd) : 0)
...
PSI_stage_info srv_stage_alter_table_end = {0, "alter table (end)", PSI_FLAG_STAGE_PROGRESS};

/** Performance schema stage event for monitoring ALTER TABLE progress
log_make_checkpoint_at(). */
PSI_stage_info srv_stage_alter_table_flush = {0, "alter table (flush)", PSI_FLAG_STAGE_PROGRESS};

/** Performance schema stage event for monitoring ALTER TABLE progress
row_merge_insert_index_tuples(). */
PSI_stage_info srv_stage_alter_table_insert = {0, "alter table (insert)", PSI_FLAG_STAGE_PROGRESS};

/** Performance schema stage event for monitoring ALTER TABLE progress
row_log_apply(). */
PSI_stage_info srv_stage_alter_table_log_index = {0, "alter table (log apply index)", PSI_FLAG_STAGE_PROGRESS};

/** Performance schema stage event for monitoring ALTER TABLE progress
row_log_table_apply(). */
PSI_stage_info srv_stage_alter_table_log_table = {0, "alter table (log apply table)", PSI_FLAG_STAGE_PROGRESS};

/** Performance schema stage event for monitoring ALTER TABLE progress
row_merge_sort(). */
PSI_stage_info srv_stage_alter_table_merge_sort = {0, "alter table (merge sort)", PSI_FLAG_STAGE_PROGRESS};

/** Performance schema stage event for monitoring ALTER TABLE progress
row_merge_read_clustered_index(). */
PSI_stage_info srv_stage_alter_table_read_pk_internal_sort = {0, "alter table (read PK and internal sort)",
                                                              PSI_FLAG_STAGE_PROGRESS};

/** Performance schema stage event for monitoring buffer pool load progress. */
PSI_stage_info srv_stage_buffer_pool_load = {0, "buffer pool load", PSI_FLAG_STAGE_PROGRESS};
...
/*==========*/
{
  ...
  mutex_create(LATCH_ID_PAGE_ZIP_STAT_PER_INDEX, &page_zip_stat_per_index_mutex);
...
static void srv_normalize_init_values(void)
/*===========================*/
{
                          ...
                          the list of active transactions */
{
  ...
  fputs(
      ...
      "--------------\n",
      file);
  fprintf(file, ULINTPF " queries inside InnoDB, " ULINTPF " queries in queue\n", srv_conc_get_active_threads(),
  ...
  fprintf(file, "Process ID=" ULINTPF ", Main thread ID=" ULINTPF ", state: %s\n", srv_main_thread_process_no,
          srv_main_thread_id, srv_main_thread_op_info);
  fprintf(file, "Number of rows inserted " ULINTPF ", updated " ULINTPF ", deleted " ULINTPF ", read " ULINTPF "\n",
          (ulint)srv_stats.n_rows_inserted, (ulint)srv_stats.n_rows_updated, (ulint)srv_stats.n_rows_deleted,
          (ulint)srv_stats.n_rows_read);
  fprintf(file,
          "%.2f inserts/s, %.2f updates/s,"
          " %.2f deletes/s, %.2f reads/s\n",
          ((ulint)srv_stats.n_rows_inserted - srv_n_rows_inserted_old) / time_elapsed,
...
/*==========================*/
{
  ...
  export_vars.innodb_buffer_pool_write_requests = srv_stats.buf_pool_write_requests;
  ...
  export_vars.innodb_buffer_pool_bytes_data = buf_pools_list_size.LRU_bytes + buf_pools_list_size.unzip_LRU_bytes;

  export_vars.innodb_buffer_pool_pages_dirty = flush_list_len;

  export_vars.innodb_buffer_pool_bytes_dirty = buf_pools_list_size.flush_list_bytes;
  ...
  export_vars.innodb_buffer_pool_pages_misc = buf_pool_get_n_pages() - LRU_len - free_len;
  ...
  export_vars.innodb_row_lock_current_waits = srv_stats.n_lock_wait_current_count;

  export_vars.innodb_row_lock_time = srv_stats.n_lock_wait_time / 1000;

  if (srv_stats.n_lock_wait_count > 0)
  {
    export_vars.innodb_row_lock_time_avg = (ulint)(srv_stats.n_lock_wait_time / 1000 / srv_stats.n_lock_wait_count);
  }
  else
  {
  ...
  {
    export_vars.innodb_purge_view_trx_id_age = (ulint)(max_trx_id - up_limit_id);
  }
#endif /* UNIV_DEBUG */
...
extern "C" os_thread_ret_t DECLARE_THREAD(srv_monitor_thread)(
    /*===============================*/
...
os_thread_create */
{
  ...
  ib::info() << "Lock timeout thread starts, id " << os_thread_pf(os_thread_get_curr_id());
...
loop:
  ...
  if (time_elapsed > 15)
  {
    ...
    if (srv_print_innodb_monitor)
    {
      ...
      if (!srv_printf_innodb_monitor(stderr, MUTEX_NOWAIT(mutex_skipped), NULL, NULL))
      {
        mutex_skipped++;
      }
      else
    ...
    if (!srv_read_only_mode && srv_innodb_status)
    {
      ...
      if (!srv_printf_innodb_monitor(srv_monitor_file, MUTEX_NOWAIT(mutex_skipped), NULL, NULL))
      {
        mutex_skipped++;
      }
      else
...
/*********************************************************************/ /**
 ...
 @return a dummy parameter */
extern "C" os_thread_ret_t DECLARE_THREAD(srv_error_monitor_thread)(
    /*=====================================*/
    void *arg MY_ATTRIBUTE((unused)))
/*!< in: a dummy parameter required by
os_thread_create */
...
#ifdef UNIV_DEBUG_THREAD_CREATION
  ib::info() << "Error monitor thread starts, id " << os_thread_pf(os_thread_get_curr_id());
#endif /* UNIV_DEBUG_THREAD_CREATION */
...
loop:
  ...
  if (sync_array_print_long_waits(&waiter, &sema) && sema == old_sema && os_thread_eq(waiter, old_waiter))
  {
    fatal_cnt++;
    if (fatal_cnt > 10)
    {
      ib::fatal() << "Semaphore wait has lasted > " << srv_fatal_semaphore_wait_threshold
...
srv_thread_type srv_get_active_thread_type(void)
/*============================*/
{
  ...
  if (ret == SRV_NONE && srv_shutdown_state != SRV_SHUTDOWN_NONE && trx_purge_state() != PURGE_STATE_DISABLED &&
...
const char *srv_any_background_threads_are_active(void)
/*=======================================*/
{
...
/*=====================================*/
{
  ...
  if (purge_sys->state == PURGE_STATE_RUN && srv_sys->n_threads_active[SRV_PURGE] == 0)
  {
...
/*===================================*/
{
  ...
  if ((current_time - srv_last_log_flush_time) >= srv_flush_log_at_timeout)
  {
                                          ...
                                          just merged */
{
  ...
  if (time_elapsed > 60)
  {
    *last_print_time = ut_time_monotonic();
    ...
    if (n_tables_to_drop)
    {
      ib::info() << "Waiting for " << n_tables_to_drop << " table(s) to be dropped";
    }
...
void srv_master_thread_disabled_debug_update(THD *thd, struct st_mysql_sys_var *var, void *var_ptr, const void *save)
{
...
/*============================*/
{
  ...
  MONITOR_INC_TIME_IN_MICRO_SECS(MONITOR_SRV_BACKGROUND_DROP_TABLE_MICROSECOND, counter_time);
  ...
  MONITOR_INC_TIME_IN_MICRO_SECS(MONITOR_SRV_IBUF_MERGE_MICROSECOND, counter_time);

  /* Flush logs if needed */
  srv_main_thread_op_info = "flushing log";
  srv_sync_log_buffer_in_background();
  MONITOR_INC_TIME_IN_MICRO_SECS(MONITOR_SRV_LOG_FLUSH_MICROSECOND, counter_time);
  ...
  if (cur_time % SRV_MASTER_DICT_LRU_INTERVAL == 0)
  {
    ...
    MONITOR_INC_TIME_IN_MICRO_SECS(MONITOR_SRV_DICT_LRU_MICROSECOND, counter_time);
  }
  ...
  if (cur_time % SRV_MASTER_CHECKPOINT_INTERVAL == 0)
  {
    ...
    MONITOR_INC_TIME_IN_MICRO_SECS(MONITOR_SRV_CHECKPOINT_MICROSECOND, counter_time);
  }
}
...
func_exit:
  ...
  {
    srv_shutdown_print_master_pending(last_print_time, n_tables_to_drop, n_bytes_merged);
  }
...
/*********************************************************************/ /**
 ...
 @return a dummy parameter */
extern "C" os_thread_ret_t DECLARE_THREAD(srv_master_thread)(
    /*==============================*/
    void *arg MY_ATTRIBUTE((unused)))
/*!< in: a dummy parameter required by
os_thread_create */
...
#ifdef UNIV_DEBUG_THREAD_CREATION
  ib::info() << "Master thread starts, id " << os_thread_pf(os_thread_get_curr_id());
#endif /* UNIV_DEBUG_THREAD_CREATION */
...
loop:
  ...
  while (srv_shutdown_state != SRV_SHUTDOWN_EXIT_THREADS && srv_master_do_shutdown_tasks(&last_print_time))
  {
...
static bool srv_purge_should_exit(ulint n_purged) /*!< in: pages purged in last batch */
{
...
/*********************************************************************/ /**
 ...
 @return a dummy parameter */
extern "C" os_thread_ret_t DECLARE_THREAD(srv_worker_thread)(
    /*==============================*/
...
#ifdef UNIV_DEBUG_THREAD_CREATION
  ib::info() << "Worker thread starting, id " << os_thread_pf(os_thread_get_curr_id());
#endif /* UNIV_DEBUG_THREAD_CREATION */
...
#ifdef UNIV_DEBUG_THREAD_CREATION
  ib::info() << "Purge worker thread exiting, id " << os_thread_pf(os_thread_get_curr_id());
#endif /* UNIV_DEBUG_THREAD_CREATION */
    ...
    ulint *n_total_purged) /*!< in/out: total pages purged */
{
  ...
  do
  {
    if (trx_sys->rseg_history_len > rseg_history_len || (srv_max_purge_lag > 0 && rseg_history_len > srv_max_purge_lag))
    {
    ...
    ulint rseg_truncate_frequency = ut_min(static_cast<ulint>(srv_purge_rseg_truncate_frequency), undo_trunc_freq);

    n_pages_purged = trx_purge(n_use_threads, srv_purge_batch_size, (++count % rseg_truncate_frequency) == 0);

    *n_total_purged += n_pages_purged;

  } while (!srv_purge_should_exit(n_pages_purged) && n_pages_purged > 0 && purge_sys->state == PURGE_STATE_RUN);
                            ...
                            before last purge */
{
  ...
  do
  {
    ...
    stop = (srv_shutdown_state == SRV_SHUTDOWN_NONE && purge_sys->state == PURGE_STATE_STOP);
    ...
    if (ret == OS_SYNC_TIME_EXCEEDED)
    {
      ...
      if (rseg_history_len == trx_sys->rseg_history_len && trx_sys->rseg_history_len < 5000)
      {
        stop = true;
      }
    }
...
/*********************************************************************/ /**
 ...
 @return a dummy parameter */
extern "C" os_thread_ret_t DECLARE_THREAD(srv_purge_coordinator_thread)(
    /*=========================================*/
    void *arg MY_ATTRIBUTE((unused))) /*!< in: a dummy parameter
                                      required by os_thread_create */
{
...
#ifdef UNIV_DEBUG_THREAD_CREATION
  ib::info() << "Purge coordinator thread created, id " << os_thread_pf(os_thread_get_curr_id());
#endif /* UNIV_DEBUG_THREAD_CREATION */
  ...
  do
  {
    ...
    if (srv_shutdown_state == SRV_SHUTDOWN_NONE && (purge_sys->state == PURGE_STATE_STOP || n_total_purged == 0))
    {
  ...
  n_pages_purged = trx_purge(1, srv_purge_batch_size <= temp_batch_size ? srv_purge_batch_size : temp_batch_size, true);
...
#ifdef UNIV_DEBUG_THREAD_CREATION
  ib::info() << "Purge coordinator exiting, id " << os_thread_pf(os_thread_get_curr_id());
#endif /* UNIV_DEBUG_THREAD_CREATION */
...
bool srv_is_tablespace_truncated(ulint space_id)
{
  ...
  return (truncate_t::is_tablespace_truncated(space_id) || undo::Truncate::is_tablespace_truncated(space_id));
}
...
bool srv_was_tablespace_truncated(const fil_space_t *space)
{
  if (space == NULL)
  {
    ut_ad(0);
...
bool srv_is_undo_tablespace(ulint space_id)
{
  ...
  return (space_id >= srv_undo_space_id_start && space_id < (srv_undo_space_id_start + srv_undo_tablespaces_open));
}
