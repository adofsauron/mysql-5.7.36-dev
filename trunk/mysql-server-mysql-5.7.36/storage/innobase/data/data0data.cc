...
respectively */
int dtuple_coll_cmp(const dtuple_t *tuple1, const dtuple_t *tuple2)
{
  ulint n_fields;
  ulint i;
  int cmp;
...
{
  if (dfield_get_type(field)->mtype > DATA_MTYPE_CURRENT_MAX || dfield_get_type(field)->mtype < DATA_MTYPE_CURRENT_MIN)
  {
    ib::error() << "Data field type " << dfield_get_type(field)->mtype << ", len " << dfield_get_len(field);
    ...
    const dtuple_t *tuple) /*!< in: tuple */
{
  ...
  {
    ib::error() << "Index entry has " << dtuple_get_n_fields(tuple) << " fields";
  dump:
    ...
    const dfield_t *field) /*!< in: data field */
{
  if (dfield_get_type(field)->mtype > DATA_MTYPE_CURRENT_MAX || dfield_get_type(field)->mtype < DATA_MTYPE_CURRENT_MIN)
  {
    ib::fatal() << "Data field type " << dfield_get_type(field)->mtype << ", len " << dfield_get_len(field);
  }
    ...
    const dfield_t *dfield) /*!< in: dfield */
{
  ...
  if (!dfield_is_null(dfield))
  {
    ...
    if (len != print_len)
    {
      fprintf(f, "(total %lu bytes%s)", (ulong)len, dfield_is_ext(dfield) ? ", external" : "");
    }
  }
  else
  {
...
void dfield_print(std::ostream &o, const dfield_t *field, ulint n)
{
  for (ulint i = 0; i < n; i++, field++)
  {
    ...
    else if (dfield_is_ext(field))
    {
      ...
      ut_print_buf_hex(o, static_cast<const byte *>(data) + local_len, BTR_EXTERN_FIELD_REF_SIZE);
    }
    else
    {
      o << '[' << len << ']';
...
void dtuple_print(std::ostream &o, const dtuple_t *tuple)
{
  ...
  o << "TUPLE (info_bits=" << dtuple_get_info_bits(tuple) << ", " << n << " fields): {";
                         ...
                         externally stored columns */
{
  ...
  while (page_zip_rec_needs_ext(rec_get_converted_size(index, entry, *n_ext), dict_table_is_comp(index->table),
                                dict_index_get_n_fields(index), dict_table_page_size(index->table)))
  {
    ...
    for (i = dict_index_get_n_unique_in_tree(index); i < dtuple_get_n_fields(entry); i++)
    {
      ...
      if (ifield->fixed_len || dfield_is_null(dfield) || dfield_is_ext(dfield) || dfield_get_len(dfield) <= local_len ||
    ...
    vector->append(big_rec_field_t(longest_i, dfield_get_len(dfield) - local_prefix_len,
...
{
  big_rec_t *rec = static_cast<big_rec_t *>(mem_heap_alloc(heap, sizeof(big_rec_t)));

  new (rec) big_rec_t(n_fld);

  rec->heap = heap;
  rec->fields = static_cast<big_rec_field_t *>(mem_heap_alloc(heap, n_fld * sizeof(big_rec_field_t)));
...
dfield_t *dfield_t::clone(mem_heap_t *heap)
{
  ...
  dfield_t *obj = static_cast<dfield_t *>(mem_heap_alloc(heap, sizeof(dfield_t) + size));
