...
UNIV_INLINE MY_ATTRIBUTE((nonnull)) void rec_init_offsets_comp_ordinary(
    /*===========================*/
                               ...
                               in: n=rec_offs_n_fields(offsets) */
{
  ...
  do
  {
    ...
    if (!field->fixed_len || (temp && !dict_col_get_fixed_size(col, temp)))
    {
...
/**********************************************************/ /**
 ...
 @return total size */
UNIV_INLINE MY_ATTRIBUTE((warn_unused_result, nonnull(1, 2))) ulint rec_get_converted_size_comp_prefix_low(
    /*===================================*/
    const dict_index_t *index, /*!< in: record descriptor;
                               ...
                               temporary file record */
{
  ...
  extra_size = temp ? UT_BITS_IN_BYTES(n_null) : REC_N_NEW_EXTRA_BYTES + UT_BITS_IN_BYTES(n_null);
  data_size = 0;
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    if (dict_index_is_spatial(index))
    {
      ...
      {
        ut_ad(type->mtype == DATA_SYS_CHILD || dict_col_type_assert_equal(col, type));
      }
    }
    else
    {
    ...
    ut_ad(len <= col->len || DATA_LARGE_MTYPE(col->mtype) || (DATA_POINT_MTYPE(col->mtype) && len == DATA_MBR_LEN) ||
          (col->len == 0 && col->mtype == DATA_VARCHAR));

    fixed_len = field->fixed_len;
    if (temp && fixed_len && !dict_col_get_fixed_size(col, temp))
    {
      fixed_len = 0;
    }
    ...
    if (fixed_len)
    {
      ...
      if (dict_index_is_spatial(index))
      {
        ut_ad(type->mtype == DATA_SYS_CHILD || !mbmaxlen || len >= mbminlen * (fixed_len / mbmaxlen));
      }
      else
      {
  ...
  if (n_v_fields != 0)
  {
    ...
    for (i = 0; i < n_v_fields; i++)
    {
      ...
      if (col->m_col.ord_part)
      {
        ...
        {
          flen = ut_min(flen, static_cast<ulint>(DICT_MAX_FIELD_LEN_BY_FORMAT(index->table)));
          data_size += flen;
        }
...
{
  ut_ad(dict_table_is_comp(index->table));
  return (rec_get_converted_size_comp_prefix_low(index, fields, n_fields, NULL, extra, false));
}
    ...
    ulint *extra)              /*!< out: extra size */
{
  ...
  return (size + rec_get_converted_size_comp_prefix_low(index, fields, n_fields, NULL, extra, false));
}
                               ...
                               index creation */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    else
    {
      ...
      ut_ad(len <= dtype_get_len(type) || DATA_LARGE_MTYPE(dtype_get_mtype(type)) ||
    ...
    const dtuple_t *dtuple)    /*!< in: data tuple */
{
  ...
  rec_convert_dtuple_to_rec_comp(rec, index, dtuple->fields, dtuple->n_fields, NULL, status, false);
...
ulint rec_get_converted_size_temp(
    ...
    ulint *extra)              /*!< out: extra size */
{
  return (rec_get_converted_size_comp_prefix_low(index, fields, n_fields, v_entry, extra, true));
}
...
void rec_convert_dtuple_to_temp(
    ...
    const dtuple_t *v_entry)   /*!< in: dtuple contains
                               virtual column data */
{
  rec_convert_dtuple_to_rec_comp(rec, index, fields, n_fields, v_entry, REC_STATUS_ORDINARY, true);
}
    ...
    const rec_t *rec) /*!< in: physical record */
{
  ...
  {
    ib::error() << "Record len should be " << len_sum << ", len " << rec_get_data_size_old(rec);
    return (FALSE);
  }
    ...
    const ulint *offsets) /*!< in: array returned by rec_get_offsets() */
{
  ...
  {
    ib::error() << "Record len should be " << len_sum << ", len " << rec_offs_data_size(offsets);
    return (FALSE);
  }
    ...
    const rec_t *rec) /*!< in: physical record */
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    {
      fprintf(file, " SQL NULL, size " ULINTPF " ", rec_get_nth_field_size(rec, i));
    }
  ...
  for (i = 0; i < rec_offs_n_fields(offsets); i++)
  {
    ...
    if (len != UNIV_SQL_NULL)
    {
      ...
      else if (rec_offs_nth_extern(offsets, i))
      {
        ...
        ut_print_buf(file, data + len - BTR_EXTERN_FIELD_REF_SIZE, BTR_EXTERN_FIELD_REF_SIZE);
      }
      else
      {
        ut_print_buf(file, data, 30);
...
void rec_print(std::ostream &o, const rec_t *rec, ulint info, const ulint *offsets)
{
  ...
  o << (comp ? "COMPACT RECORD" : "RECORD") << "(info_bits=" << info << ", " << n << " fields): {";
...
{
  mem_heap_t *heap = NULL;
  ulint *offsets = rec_get_offsets(r.m_rec, r.m_index, NULL, ULINT_UNDEFINED, &heap);
  rec_print(o, r.m_rec, rec_get_info_bits(r.m_rec, rec_offs_comp(offsets)), offsets);
  mem_heap_free(heap);
  return (o);
}
...
{
  rec_print(o, r.m_rec, rec_get_info_bits(r.m_rec, rec_offs_comp(r.m_offsets)), r.m_offsets);
  return (o);
}
