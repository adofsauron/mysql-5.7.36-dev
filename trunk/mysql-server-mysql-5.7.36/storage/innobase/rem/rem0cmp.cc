...
respectively */
UNIV_INLINE
int innobase_mysql_cmp(ulint prtype, const byte *a, unsigned int a_length, const byte *b, unsigned int b_length)
{
#ifdef UNIV_DEBUG
  ...
  {
    return (cs->coll->strnncollsp(cs, a, a_length, b, b_length, 0));
  }
...
@return positive, 0, negative, if a is greater, equal, less than b,
respectively */
static UNIV_COLD int cmp_decimal(const byte *a, unsigned int a_length, const byte *b, unsigned int b_length)
{
  int swap_flag;
                           ...
                           not UNIV_SQL_NULL */
{
  if (mode == PAGE_CUR_MBR_EQUAL)
  {
    ...
    return (cmp_geometry_field(DATA_GEOMETRY, DATA_GIS_MBR, a, a_length, b, b_length));
  }
  else
  {
...
@return positive, 0, negative, if a is greater, equal, less than b,
respectively */
static int cmp_whole_field(ulint mtype, ulint prtype, const byte *a, unsigned int a_length, const byte *b,
                           unsigned int b_length)
{
  ...
  switch (mtype)
  {
    ...
    case DATA_VARCHAR:
    case DATA_CHAR:
      return (my_charset_latin1.coll->strnncollsp(&my_charset_latin1, a, a_length, b, b_length, 0));
    case DATA_BLOB:
    ...
    case DATA_VARMYSQL:
    case DATA_MYSQL:
      return (innobase_mysql_cmp(prtype, a, a_length, b, b_length));
    case DATA_POINT:
    case DATA_VAR_POINT:
    case DATA_GEOMETRY:
...
inline int cmp_data(ulint mtype, ulint prtype, const byte *data1, ulint len1, const byte *data2, ulint len2)
{
  ...
  switch (mtype)
  {
    ...
    case DATA_GEOMETRY:
      ...
      {
        return (cmp_whole_field(mtype, prtype, data1, (unsigned)len1, data2, (unsigned)len2));
      }
      break;
    case DATA_BLOB:
      if (prtype & DATA_BINARY_TYPE)
    ...
    page_cur_mode_t mode)   /*!< in: compare mode */
{
  ...
  ret = cmp_gis_field(mode, static_cast<const byte *>(dfield_get_data(dtuple_field)), (unsigned)dtuple_f_len, rec_b_ptr,
                      (unsigned)rec_f_len);
...
@param[in]	offsets		rec_get_offsets(rec)
@retval negative if dtuple is less than rec */
int cmp_dtuple_rec_with_gis_internal(const dtuple_t *dtuple, const rec_t *rec, const ulint *offsets)
{
  const dfield_t *dtuple_field; /* current field in logical record */
  ulint dtuple_f_len;           /* the length of the current field
                                in the logical record */
  ...
  rec_b_ptr = rec_get_nth_field(rec, offsets, 0, &rec_f_len);
  ret = cmp_gis_field(PAGE_CUR_WITHIN, static_cast<const byte *>(dfield_get_data(dtuple_field)), (unsigned)dtuple_f_len,
  ...
  return (cmp_data(dtuple_field->type.mtype, dtuple_field->type.prtype, static_cast<const byte *>(dtuple_field->data),
...
@retval negative if data1 is less than data2
@retval positive if data1 is greater than data2 */
int cmp_data_data(ulint mtype, ulint prtype, const byte *data1, ulint len1, const byte *data2, ulint len2)
{
...
int cmp_dtuple_rec_with_match_low(const dtuple_t *dtuple, const rec_t *rec, const ulint *offsets, ulint n_cmp,
                                  ulint *matched_fields)
{
  ...
  for (; cur_field < n_cmp; cur_field++)
  {
    ...
    const byte *dtuple_b_ptr = static_cast<const byte *>(dfield_get_data(dtuple_field));
...
ulint cmp_get_pad_char(const dtype_t *type)
{
  switch (type->mtype)
  {
    case DATA_FIXBINARY:
    case DATA_BINARY:
      if (dtype_get_charset_coll(type->prtype) == DATA_MYSQL_BINARY_CHARSET_COLL)
      {
...
@retval negative if dtuple is less than rec
@retval positive if dtuple is greater than rec */
int cmp_dtuple_rec_with_match_bytes(const dtuple_t *dtuple, const rec_t *rec, const dict_index_t *index,
                                    const ulint *offsets, ulint *matched_fields, ulint *matched_bytes)
{
  ...
  ut_ad(!(REC_INFO_MIN_REC_FLAG & rec_get_info_bits(rec, rec_offs_comp(offsets))));
...
@retval negative if dtuple is less than rec
@retval positive if dtuple is greater than rec */
int cmp_dtuple_rec(const dtuple_t *dtuple, const rec_t *rec, const ulint *offsets)
{
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) int cmp_rec_rec_simple_field(
    /*=====================*/
    ...
    ulint n)                   /*!< in: field to compare */
{
  ...
  return (cmp_data(col->mtype, col->prtype, rec1_b_ptr, rec1_f_len, rec2_b_ptr, rec2_f_len));
}
                               ...
                               or NULL */
{
  ...
  for (n = 0; n < n_uniq; n++)
  {
    int cmp = cmp_rec_rec_simple_field(rec1, rec2, offsets1, offsets2, index, n);
    ...
    be NULL or non-NULL. */
    ut_ad(rec_offs_nth_sql_null(offsets1, n) == rec_offs_nth_sql_null(offsets2, n));

    if (rec_offs_nth_sql_null(offsets1, n))
    {
      ut_ad(!(dict_index_get_nth_col(index, n)->prtype & DATA_NOT_NULL));
      null_eq = true;
    }
  }
...
int cmp_rec_rec_with_match(const rec_t *rec1, const rec_t *rec2, const ulint *offsets1, const ulint *offsets2,
                           const dict_index_t *index, bool spatial_index_non_leaf, bool nulls_unequal,
                           ulint *matched_fields)
{
  ...
  else if (UNIV_UNLIKELY(rec_get_info_bits(rec2, comp) & REC_INFO_MIN_REC_FLAG))
  {
    ret = 1;
    goto order_resolved;
  }
  ...
  for (; cur_field < rec1_n_fields && cur_field < rec2_n_fields; cur_field++)
  {
    ...
    if (nulls_unequal && rec1_f_len == UNIV_SQL_NULL && rec2_f_len == UNIV_SQL_NULL)
    {
      ret = -1;
      goto order_resolved;
    }

    ret = cmp_data(mtype, prtype, rec1_b_ptr, rec1_f_len, rec2_b_ptr, rec2_f_len);
    if (ret)
    {
      goto order_resolved;
    }
