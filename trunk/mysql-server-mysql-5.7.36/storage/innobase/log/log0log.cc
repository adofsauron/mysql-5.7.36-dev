...
void log_buffer_extend(ulint len)
{
  ...
  if (len >= log_sys->buf_size / 2)
  {
    ...
    ib::warn() << "The transaction log size is too large"
               ...
               << len << " >= " << LOG_BUFFER_SIZE << " / 2). Trying to extend it.";
  }

  log_sys->is_extending = true;

  while (ut_calc_align_down(log_sys->buf_free, OS_FILE_LOG_BLOCK_SIZE) !=
  ...
  log_sys->buf_ptr = static_cast<byte *>(ut_zalloc_nokey(log_sys->buf_size * 2 + OS_FILE_LOG_BLOCK_SIZE));
...
static inline ulint log_calculate_actual_len(ulint len)
{
  ...
  const ulint len_per_blk = OS_FILE_LOG_BLOCK_SIZE - (LOG_BLOCK_HDR_SIZE + LOG_BLOCK_TRL_SIZE);
  ...
  extra_len = ((len + extra_len) / len_per_blk) * (LOG_BLOCK_HDR_SIZE + LOG_BLOCK_TRL_SIZE);
...
void log_margin_checkpoint_age(ulint len)
{
  ...
  if (margin > log_sys->log_group_capacity)
  {
    ...
    if (!log_has_printed_chkp_margine_warning || difftime(time(NULL), log_last_margine_warning_time) > 15)
    {
  ...
  if (log_sys->lsn - log_sys->last_checkpoint_lsn + margin > log_sys->log_group_capacity)
  {
    ...
    if (log_sys->lsn - log_buf_pool_get_oldest_modification() + margin <= log_sys->log_group_capacity)
    {
      flushed_enough = true;
    }
...
loop:
  ...
  len_upper_limit = LOG_BUF_WRITE_MARGIN + srv_log_write_ahead_size + (5 * len) / 4;
...
/*===========*/
{
  ...
  {
    DBUG_EXECUTE_IF("print_all_chkp_warnings", log_has_printed_chkp_warning = false;);
  ...
  if (!oldest_lsn || lsn - oldest_lsn > log->max_modified_age_sync || checkpoint_age > log->max_checkpoint_age_async)
  {
    log->check_flush_or_checkpoint = true;
  }
function_exit:
                                  ...
                                  (including the header) */
{
  ...
  {
    add_this_many = 1 + (first_header_lsn - lsn) / (capacity * static_cast<int64_t>(n_log_files));
    lsn += add_this_many * capacity * static_cast<int64_t>(n_log_files);
  }
...
/*==========*/
{
  ...
  rw_lock_create(checkpoint_lock_key, &log_sys->checkpoint_lock, SYNC_NO_ORDER_CHECK);

  log_sys->checkpoint_buf_ptr = static_cast<byte *>(ut_zalloc_nokey(2 * OS_FILE_LOG_BLOCK_SIZE));

  log_sys->checkpoint_buf = static_cast<byte *>(ut_align(log_sys->checkpoint_buf_ptr, OS_FILE_LOG_BLOCK_SIZE));
                     ...
                     group */
{
  ...
  group->file_header_bufs_ptr = static_cast<byte **>(ut_zalloc_nokey(sizeof(byte *) * n_files));

  group->file_header_bufs = static_cast<byte **>(ut_zalloc_nokey(sizeof(byte **) * n_files));

  for (i = 0; i < n_files; i++)
  {
    group->file_header_bufs_ptr[i] = static_cast<byte *>(ut_zalloc_nokey(LOG_FILE_HDR_SIZE + OS_FILE_LOG_BLOCK_SIZE));

    group->file_header_bufs[i] = static_cast<byte *>(ut_align(group->file_header_bufs_ptr[i], OS_FILE_LOG_BLOCK_SIZE));
  }

  group->checkpoint_buf_ptr = static_cast<byte *>(ut_zalloc_nokey(2 * OS_FILE_LOG_BLOCK_SIZE));

  group->checkpoint_buf = static_cast<byte *>(ut_align(group->checkpoint_buf_ptr, OS_FILE_LOG_BLOCK_SIZE));
    ...
    log_group_t *group) /*!< in: log group or a dummy pointer */
{
  if ((ulint)group & 0x1UL)
  {
    ...
    DBUG_PRINT("ib_log", ("checkpoint info written to group %u", unsigned(group->id)));
                        ...
                        lsn */
{
  ...
  strcpy(reinterpret_cast<char *>(buf) + LOG_HEADER_CREATOR, LOG_HEADER_CREATOR_CURRENT);
  ut_ad(LOG_HEADER_CREATOR_END - LOG_HEADER_CREATOR >= sizeof LOG_HEADER_CREATOR_CURRENT);
  log_block_set_checksum(buf, log_block_calc_checksum_crc32(buf));

  dest_offset = nth_file * group->file_size;

  DBUG_PRINT("ib_log", ("write " LSN_PF " group " ULINTPF " file " ULINTPF " header", start_lsn, group->id, nth_file));
  ...
  const ulint page_no = (ulint)(dest_offset / univ_page_size.physical());

  fil_io(IORequestLogWrite, true, page_id_t(group->space_id, page_no), univ_page_size,
         (ulint)(dest_offset % univ_page_size.physical()), OS_FILE_LOG_BLOCK_SIZE, buf, group);
...
loop:
  ...
  if (write_header && next_offset % group->file_size == LOG_FILE_HDR_SIZE)
  {
    ...
    log_group_file_header_flush(group, (ulint)(next_offset / group->file_size), start_lsn);
  ...
  DBUG_PRINT("ib_log",
             ("write " LSN_PF " to " LSN_PF ": group " ULINTPF " len " ULINTPF " blocks " ULINTPF ".." ULINTPF,
  ...
  ut_ad(pad_len >= len || log_block_get_hdr_no(buf) == log_block_convert_lsn_to_no(start_lsn));
  ...
  for (i = 0; i < write_len / OS_FILE_LOG_BLOCK_SIZE; i++)
  {
    ut_ad(pad_len >= len || i * OS_FILE_LOG_BLOCK_SIZE >= len - pad_len ||
          log_block_get_hdr_no(buf + i * OS_FILE_LOG_BLOCK_SIZE) == log_block_get_hdr_no(buf) + i);
  ...
  const ulint page_no = (ulint)(next_offset / univ_page_size.physical());

  fil_io(IORequestLogWrite, true, page_id_t(group->space_id, page_no), univ_page_size,
         (ulint)(next_offset % UNIV_PAGE_SIZE), write_len, buf, group);
...
static inline void log_buffer_switch()
{
  ...
  ut_memcpy(log_sys->buf, old_buf + area_end - OS_FILE_LOG_BLOCK_SIZE, OS_FILE_LOG_BLOCK_SIZE);
...
loop:
  ...
  lsn_t limit_lsn = flush_to_disk ? log_sys->flushed_to_disk_lsn : log_sys->write_lsn;
  ...
  if (log_sys->n_pending_flushes > 0 || !os_event_is_set(log_sys->flush_event))
  {
    log_write_mutex_exit();
  ...
  if (flush_to_disk && (log_sys->n_pending_flushes > 0 || !os_event_is_set(log_sys->flush_event)))
  {
  ...
  DBUG_PRINT("ib_log", ("write " LSN_PF " to " LSN_PF, log_sys->write_lsn, log_sys->lsn));
  ...
  if (write_ahead_size > OS_FILE_LOG_BLOCK_SIZE)
  {
    ...
    end_offset = log_group_calc_lsn_offset(ut_uint64_align_up(write_lsn, OS_FILE_LOG_BLOCK_SIZE), group);
  ...
  log_group_write_buf(group, write_buf + area_start, area_end - area_start + pad_size,
#ifdef UNIV_DEBUG
                      pad_size,
#endif /* UNIV_DEBUG */
                      ut_uint64_align_down(log_sys->write_lsn, OS_FILE_LOG_BLOCK_SIZE), start_offset - area_start);
    ...
    bool flush) /*!< in: flush the logs to disk */
{
  ...
  if (flush && log_sys->n_pending_flushes > 0 && log_sys->current_flush_lsn >= lsn)
  {
...
static bool log_preflush_pool_modified_pages(lsn_t new_oldest)
{
  ...
  if (new_oldest == LSN_MAX || !buf_page_cleaner_is_active || srv_is_being_started)
  {
    ulint n_pages;
...
/*=========================*/
{
  ...
  DBUG_PRINT("ib_log", ("checkpoint ended at " LSN_PF ", flushed to " LSN_PF, log_sys->last_checkpoint_lsn,
...
#error "LOG_CHECKPOINT_SIZE > OS_FILE_LOG_BLOCK_SIZE"
  ...
  DBUG_PRINT("ib_log", ("checkpoint " UINT64PF " at " LSN_PF " written to group " ULINTPF, log_sys->next_checkpoint_no,
                        log_sys->next_checkpoint_lsn, group->id));
  ...
  fil_io(IORequestLogWrite, false, page_id_t(group->space_id, 0), univ_page_size,
         (log_sys->next_checkpoint_no & 1) ? LOG_CHECKPOINT_2 : LOG_CHECKPOINT_1, OS_FILE_LOG_BLOCK_SIZE, buf,
         (byte *)group + 1);
                       ...
                       start + LOG_BLOCK_HDR_SIZE */
{
  ...
  mach_write_to_4(hdr_buf + LOG_HEADER_FORMAT, LOG_HEADER_FORMAT_CURRENT);
  ...
  ut_sprintf_timestamp((char *)hdr_buf + (LOG_HEADER_CREATOR + (sizeof LOG_HEADER_CREATOR_CURRENT) - 1));
  ...
  mach_write_to_8(buf + LOG_CHECKPOINT_OFFSET, LOG_FILE_HDR_SIZE + LOG_BLOCK_HDR_SIZE);
...
void log_group_header_read(const log_group_t *group, ulint header)
{
  ...
  fil_io(IORequestLogRead, true, page_id_t(group->space_id, header / univ_page_size.physical()), univ_page_size,
         header % univ_page_size.physical(), OS_FILE_LOG_BLOCK_SIZE, log_sys->checkpoint_buf, NULL);
}
...
void log_write_checkpoint_info(bool sync)
{
  ...
  {
    for (group = UT_LIST_GET_FIRST(log_sys->log_groups); group; group = UT_LIST_GET_NEXT(log_groups, group))
    {
...
mtr_buf_t *log_append_on_checkpoint(mtr_buf_t *buf)
{
  log_mutex_enter();
...
bool log_checkpoint(bool sync, bool write_always)
{
  ...
  if (!write_always && oldest_lsn <= log_sys->last_checkpoint_lsn + SIZE_OF_MLOG_CHECKPOINT)
  {
  ...
  const bool do_write = srv_shutdown_state == SRV_SHUTDOWN_NONE || flush_lsn != log_sys->lsn;
  ...
  DBUG_EXECUTE_IF(
      "using_wa_checkpoint_middle", if (write_always) {
...
loop:
  ...
  fil_io(IORequestLogRead, true, page_id_t(group->space_id, page_no), univ_page_size,
         (ulint)(source_offset % univ_page_size.physical()), len, buf, NULL);
...
loop:
  ...
  if (active_thd != SRV_NONE)
  {
    ...
    if (srv_print_verbose_log && count > 600)
    {
      ...
      ib::info() << "Waiting for " << thread_type << " to be suspended";
  ...
  if (n_write != 0 || n_flush != 0)
  {
    if (srv_print_verbose_log && count > 600)
    {
      ib::info() << "Pending checkpoint_writes: " << n_write << ". Pending log flush writes: " << n_flush;
      count = 0;
    }
    goto loop;
  }
  ...
  const bool is_last =
      ((srv_force_recovery == SRV_FORCE_NO_LOG_REDO && lsn == log_sys->last_checkpoint_lsn + LOG_BLOCK_HDR_SIZE) ||
  ...
  {
    ib::error() << "Log sequence number at shutdown " << lsn << " is lower than at startup " << srv_start_lsn << "!";
  }
    ...
    FILE *file) /*!< in: file where to print */
{
  ...
  fprintf(file,
          ...
          log_sys->lsn, log_sys->flushed_to_disk_lsn, log_buf_pool_get_oldest_modification(),
  ...
  fprintf(file,
          ...
          log_sys->n_pending_flushes, log_sys->n_pending_checkpoint_writes, log_sys->n_log_ios,
