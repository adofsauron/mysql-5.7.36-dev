...
static void recv_init_crash_recovery(void);
/*===========================*/
#endif /* !UNIV_HOTBACKUP */
...
struct file_name_t
{
  ...
  /** Constructor */
  file_name_t(std::string name_, bool deleted_) : name(name_), space(NULL), deleted(deleted_) {}
};

/** Map of dirty tablespaces during recovery */
typedef std::map<ulint, file_name_t, std::less<ulint>, ut_allocator<std::pair<const ulint, file_name_t>>> recv_spaces_t;
...
static bool fil_name_process(char *name, ulint len, ulint space_id, bool deleted)
{
  bool processed = true;
  ...
  std::pair<recv_spaces_t::iterator, bool> p = recv_spaces.insert(std::make_pair(space_id, fname));
           ...
           || f.name != fname.name)
  {
    ...
    switch (fil_ibd_load(space_id, name, space))
    {
      case FIL_LOAD_OK:
        ...
        if (FSP_FLAGS_GET_ENCRYPTION(space->flags) && recv_sys->encryption_list != NULL)
        {
          dberr_t err;
          encryption_list_t::iterator it;

          for (it = recv_sys->encryption_list->begin(); it != recv_sys->encryption_list->end(); it++)
          {
            if (it->space_id == space->id)
            {
              err = fil_set_encryption(space->id, Encryption::AES, it->key, it->iv);
              if (err != DB_SUCCESS)
              {
                ib::error() << "Can't set"
                               " encryption"
        ...
        {
          ib::error() << "Tablespace " << space_id << " has been found in two places: '" << f.name << "' and '" << name
                      << "'."
      ...
      case FIL_LOAD_NOT_FOUND:
        ...
        if (srv_force_recovery)
        {
          ...
          ib::info() << "At LSN: " << recv_sys->recovered_lsn << ": unable to open file " << name << " for tablespace "
                     << space_id;
        }
        break;
...
static byte *fil_name_parse(byte *ptr, const byte *end, ulint space_id, ulint first_page_no, mlog_id_t type)
{
  ...
  and end in .ibd. */
  bool corrupt = is_predefined_tablespace(space_id) || first_page_no != 0  // TODO: multi-file user tablespaces
                 || len < sizeof "/a.ibd\0" || memcmp(ptr + len - 5, DOT_IBD, 5) != 0 ||
  ...
  switch (type)
  {
    ...
    case MLOG_FILE_NAME:
      ...
      fil_name_process(reinterpret_cast<char *>(ptr), len, space_id, false);
      break;
    ...
    case MLOG_FILE_RENAME2:
      ...
      corrupt = corrupt || new_len < sizeof "/a.ibd\0" || memcmp(new_name + new_len - 5, DOT_IBD, 5) != 0 ||
      ...
      fil_name_process(reinterpret_cast<char *>(ptr), len, space_id, false);
      fil_name_process(reinterpret_cast<char *>(new_name), new_len, space_id, false);

      if (!fil_op_replay_rename(space_id, first_page_no, reinterpret_cast<const char *>(ptr),
...
static void make_abs_file_path(const std::string &name, std::string &absolute_path, std::string &tablespace_name)
{
...
@retval		true		if able to process file successfully.
@retval		false		if unable to process the file */
bool fil_name_process(const char *name, ulint space_id)
{
...
static byte *fil_name_parse(byte *ptr, const byte *end, ulint space_id, ulint first_page_no, mlog_id_t type)
{
  ...
  {
    name = static_cast<char *>(ut_malloc_nokey((abs_file_path.length() + 1)));
  ...
  switch (type)
  {
    ...
    case MLOG_FILE_NAME:
      ...
      if (!recv_is_making_a_backup)
      {
        recv_spaces.insert(std::make_pair(space_id, file_name_t(abs_file_path, false)));
      }
      break;
    case MLOG_FILE_DELETE:
      /* Don't validate tablespaces while copying redo logs
      ...
      if (recv_replay_file_ops && fil_space_get(space_id))
      {
        dberr_t err = fil_delete_tablespace(space_id, BUF_REMOVE_FLUSH_NO_WRITE);
        ut_a(err == DB_SUCCESS);
      }

      break;
    case MLOG_FILE_CREATE2:
      if (recv_is_making_a_backup || (!recv_replay_file_ops) || (is_intermediate_file(abs_file_path.c_str())) ||
          (fil_space_get(space_id)) || (fil_space_get_id_by_name(tablespace_name.c_str()) != ULINT_UNDEFINED))
      {
      ...
      else
      {
        ...
        if (itr == recv_spaces.end() || (itr->second.name != abs_file_path))
        {
          dberr_t ret = fil_ibd_create(space_id, tablespace_name.c_str(), abs_file_path.c_str(), flags,
                                       FIL_IBD_FILE_INITIAL_SIZE);

          if (ret != DB_SUCCESS)
          {
            ib::fatal() << "Could not create the"
                        << " tablespace : " << abs_file_path << " with space Id : " << space_id;
          }
        }
      }
      break;
    case MLOG_FILE_RENAME2:
      ...
      corrupt = corrupt || new_len < sizeof "/a.ibd\0" || memcmp(new_table_name + new_len - 5, DOT_IBD, 5) != 0 ||
      ...
      if (recv_is_making_a_backup || (!recv_replay_file_ops) || (is_intermediate_file(name)) ||
      ...
      {
        make_abs_file_path(new_table, abs_file_path, tablespace_name);

        new_name = static_cast<char *>(ut_malloc_nokey((abs_file_path.length() + 1)));
      ...
      if (!fil_op_replay_rename(space_id, first_page_no, name, new_name))
      {
        recv_sys->found_corrupt_fs = true;
      }
  }
...
extern "C" os_thread_ret_t DECLARE_THREAD(recv_writer_thread)(
    /*===============================*/
...
os_thread_create */
{
  ...
  ib::info() << "recv_writer thread running, id " << os_thread_pf(os_thread_get_curr_id());
...
static void recv_synchronize_groups(void)
/*=========================*/
{
  lsn_t start_lsn;
  lsn_t end_lsn;
  ...
  log_group_read_log_seg(recv_sys->last_block, UT_LIST_GET_FIRST(log_sys->log_groups), start_lsn, end_lsn);

  for (log_group_t *group = UT_LIST_GET_FIRST(log_sys->log_groups); group; group = UT_LIST_GET_NEXT(log_groups, group))
  {
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t recv_find_max_checkpoint_0(log_group_t **max_group, ulint *max_field)
{
  ...
  for (ulint field = LOG_CHECKPOINT_1; field <= LOG_CHECKPOINT_2; field += LOG_CHECKPOINT_2 - LOG_CHECKPOINT_1)
  {
    log_group_header_read(group, field);

    if (static_cast<uint32_t>(ut_fold_binary(buf, CHECKSUM_1)) != mach_read_from_4(buf + CHECKSUM_1) ||
        static_cast<uint32_t>(ut_fold_binary(buf + LOG_CHECKPOINT_LSN, CHECKSUM_2 - LOG_CHECKPOINT_LSN)) !=
            mach_read_from_4(buf + CHECKSUM_2))
    {
      DBUG_PRINT("ib_log", ("invalid pre-5.7.9 checkpoint " ULINTPF, field));
      continue;
    }
    ...
    group->lsn_offset =
        static_cast<ib_uint64_t>(mach_read_from_4(buf + OFFSET_HIGH32)) << 32 | mach_read_from_4(buf + OFFSET_LOW32);
    checkpoint_no = mach_read_from_8(buf + LOG_CHECKPOINT_NO);

    DBUG_PRINT("ib_log",
               ("checkpoint " UINT64PF " at " LSN_PF " found in group " ULINTPF, checkpoint_no, group->lsn, group->id));
...
static dberr_t recv_log_format_0_recover(lsn_t lsn)
{
  log_mutex_enter();
  log_group_t *group = UT_LIST_GET_FIRST(log_sys->log_groups);
  const lsn_t source_offset = log_group_calc_lsn_offset(lsn, group);
  log_mutex_exit();
  const ulint page_no = (ulint)(source_offset / univ_page_size.physical());
  byte *buf = log_sys->buf;

  static const char *NO_UPGRADE_RECOVERY_MSG =
      "Upgrade after a crash is not supported."
      " This redo log was created before MySQL 5.7.9";
  static const char *NO_UPGRADE_RTFM_MSG = ". Please follow the instructions at " REFMAN "upgrading.html";

  fil_io(IORequestLogRead, true, page_id_t(group->space_id, page_no), univ_page_size,
         (ulint)((source_offset & ~(OS_FILE_LOG_BLOCK_SIZE - 1)) % univ_page_size.physical()), OS_FILE_LOG_BLOCK_SIZE,
         buf, NULL);

  if (log_block_calc_checksum_format_0(buf) != log_block_get_checksum(buf))
  {
    ib::error() << NO_UPGRADE_RECOVERY_MSG << ", and it appears corrupted" << NO_UPGRADE_RTFM_MSG;
    return (DB_CORRUPTION);
  }

  if (log_block_get_data_len(buf) != (source_offset & (OS_FILE_LOG_BLOCK_SIZE - 1)))
  {
    ib::error() << NO_UPGRADE_RECOVERY_MSG << NO_UPGRADE_RTFM_MSG;
    return (DB_ERROR);
  }

  /* Mark the redo log for upgrading. */
  srv_log_file_size = 0;
  recv_sys->parse_start_lsn = recv_sys->recovered_lsn = recv_sys->scanned_lsn = recv_sys->mlog_checkpoint_lsn = lsn;
  log_sys->last_checkpoint_lsn = log_sys->next_checkpoint_lsn = log_sys->lsn = log_sys->write_lsn =
      log_sys->current_flush_lsn = log_sys->flushed_to_disk_lsn = lsn;
...
@param[out]	max_field	LOG_CHECKPOINT_1 or LOG_CHECKPOINT_2
@return error code or DB_SUCCESS */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t recv_find_max_checkpoint(log_group_t **max_group, ulint *max_field)
{
  log_group_t *group;
  ib_uint64_t max_no;
  ib_uint64_t checkpoint_no;
  ...
  *max_field = 0;
  ...
  while (group)
  {
    ...
    {
      case 0:
        return (recv_find_max_checkpoint_0(max_group, max_field));
      ...
      default:
        ...
        ib::error() << "Unsupported redo log format."
                       ...
                       " with "
                    << buf + LOG_HEADER_CREATOR
                    << ". Please follow the instructions at " REFMAN "upgrading-downgrading.html";
    ...
    for (field = LOG_CHECKPOINT_1; field <= LOG_CHECKPOINT_2; field += LOG_CHECKPOINT_2 - LOG_CHECKPOINT_1)
    {
      ...
      {
        DBUG_PRINT("ib_log", ("invalid checkpoint,"
                              " group " ULINTPF " at " ULINTPF ", checksum %x",
                              group->id, field, (unsigned)log_block_get_checksum(buf)));
        continue;
      }
      ...
      DBUG_PRINT("ib_log", ("checkpoint " UINT64PF " at " LSN_PF " found in group " ULINTPF, checkpoint_no, group->lsn,
                            group->id));
...
ibool recv_read_checkpoint_info_for_backup(
    /*=================================*/
...
static bool log_block_checksum_is_ok(const byte *block) /*!< in: pointer to a log block */
{
  return (!innodb_log_checksums || log_block_get_checksum(block) == log_block_calc_checksum(block));
}
                            ...
                            data ended here */
{
  ...
  *n_bytes_scanned = 0;
  ...
  for (log_block = buf; log_block < buf + buf_len; log_block += OS_FILE_LOG_BLOCK_SIZE)
  {
    ...
    if (no != log_block_convert_lsn_to_no(*scanned_lsn) || !log_block_checksum_is_ok(log_block))
    {
#if 0
			fprintf(stderr,
...
static byte *fil_write_encryption_parse(byte *ptr, const byte *end, ulint space_id)
{
  fil_space_t *space;
  ulint offset;
  ulint len;
  ...
  if (space == NULL)
  {
    ...
    if (recv_sys->encryption_list == NULL)
    {
      recv_sys->encryption_list = UT_NEW_NOKEY(encryption_list_t());
    }
  ...
  if (offset >= UNIV_PAGE_SIZE || len + offset > UNIV_PAGE_SIZE ||
      (len != ENCRYPTION_INFO_SIZE_V1 && len != ENCRYPTION_INFO_SIZE_V2))
  {
    recv_sys->found_corrupt_log = TRUE;
    return (NULL);
  }
...
#endif
  if (!fsp_header_decode_encryption_info(key, iv, ptr))
  {
    recv_sys->found_corrupt_log = TRUE;
  ...
  ut_ad(len == ENCRYPTION_INFO_SIZE_V1 || len == ENCRYPTION_INFO_SIZE_V2);
...
a page log record should not be applied
@return log record end, NULL if not a complete record */
static byte *recv_parse_or_apply_log_rec_body(mlog_id_t type, byte *ptr, byte *end_ptr, ulint space_id, ulint page_no,
                                              buf_block_t *block, mtr_t *mtr)
{
  ...
  switch (type)
  {
      ...
      */
      if (!recv_recovery_on)
      {
        if (is_online_redo_copy)
        {
          ...
          {
            ib::trace() << "Last flushed lsn: " << backup_redo_log_flushed_lsn << " load_index lsn "
    ...
    case MLOG_WRITE_STRING:
      ...
      {
        return (fil_write_encryption_parse(ptr, end_ptr, space_id));
      }
      break;
  ...
  switch (type)
  {
      ...
      if (page && page_type == FIL_PAGE_TYPE_ALLOCATED && end_ptr >= ptr + 2)
      {
        ...
        switch (type)
        {
          ...
          case MLOG_2BYTES:
            ...
            ut_ad(offs == FIL_PAGE_TYPE || offs == IBUF_TREE_SEG_HEADER + IBUF_HEADER + FSEG_HDR_OFFSET ||
                  offs == PAGE_BTR_IBUF_FREE_LIST + PAGE_HEADER + FIL_ADDR_BYTE ||
                  offs == PAGE_BTR_IBUF_FREE_LIST + PAGE_HEADER + FIL_ADDR_BYTE + FIL_ADDR_SIZE ||
                  offs == PAGE_BTR_SEG_LEAF + PAGE_HEADER + FSEG_HDR_OFFSET ||
                  offs == PAGE_BTR_SEG_TOP + PAGE_HEADER + FSEG_HDR_OFFSET ||
                  offs == PAGE_BTR_IBUF_FREE_LIST_NODE + PAGE_HEADER + FIL_ADDR_BYTE + 0 /*FLST_PREV*/
                  || offs == PAGE_BTR_IBUF_FREE_LIST_NODE + PAGE_HEADER + FIL_ADDR_BYTE + FIL_ADDR_SIZE /*FLST_NEXT*/);
            break;
          case MLOG_4BYTES:
            /* Note that this can fail when the
            redo log been written with something
            older than InnoDB Plugin 1.0.4. */
            ut_ad(0 || offs == IBUF_TREE_SEG_HEADER + IBUF_HEADER + FSEG_HDR_SPACE ||
                  ...
                  offs == PAGE_BTR_IBUF_FREE_LIST + PAGE_HEADER + FIL_ADDR_PAGE + FIL_ADDR_SIZE ||
                  ...
                  offs == PAGE_BTR_IBUF_FREE_LIST_NODE + PAGE_HEADER + FIL_ADDR_PAGE + 0 /*FLST_PREV*/
                  || offs == PAGE_BTR_IBUF_FREE_LIST_NODE + PAGE_HEADER + FIL_ADDR_PAGE + FIL_ADDR_SIZE /*FLST_NEXT*/);
            break;
        }
      }
#endif /* UNIV_DEBUG */
      ptr = mlog_parse_nbytes(type, ptr, end_ptr, page, page_zip);
      if (ptr != NULL && page != NULL && page_no == 0 && type == MLOG_4BYTES)
      {
    ...
    case MLOG_ZIP_PAGE_REORGANIZE:
      ...
      if (NULL != (ptr = mlog_parse_index(ptr, end_ptr, type != MLOG_PAGE_REORGANIZE, &index)))
      {
        ut_a(!page || (ibool) !!page_is_comp(page) == dict_table_is_comp(index->table));
        ptr = btr_parse_page_reorganize(ptr, end_ptr, index, type == MLOG_ZIP_PAGE_REORGANIZE, block, mtr);
      }
      break;
    ...
    case MLOG_ZIP_PAGE_COMPRESS_NO_DATA:
      ...
      {
        ut_a(!page || ((ibool) !!page_is_comp(page) == dict_table_is_comp(index->table)));
        ptr = page_zip_parse_compress_no_data(ptr, end_ptr, page, page_zip, index);
      }
      break;
    default:
      ptr = NULL;
    ...
    buf_block_t *block) /*!< in/out: buffer block */
{
  ...
  recv_addr = recv_get_fil_addr_struct(block->page.id.space(), block->page.id.page_no());
  ...
  while (recv)
  {
    ...
    ut_ad(end_lsn <= UT_LIST_GET_FIRST(log_sys->log_groups)->scanned_lsn);
    ...
    {
      lsn_t init_lsn = truncate_t::get_truncated_tablespace_init_lsn(recv_addr->space);
    ...
    such action. */
    if (recv->start_lsn >= page_lsn && !srv_is_tablespace_truncated(recv_addr->space) && !skip_recv)
    {
      lsn_t end_lsn;
      ...
      DBUG_PRINT("ib_log",
                 ("apply " LSN_PF ":"
                  ...
                  recv->start_lsn, get_mlog_string(recv->type), recv->len, recv_addr->space, recv_addr->page_no));

      recv_parse_or_apply_log_rec_body(recv->type, buf, buf + recv->len, recv_addr->space, recv_addr->page_no, block,
                                       &mtr);
  ...
  if (fil_page_index_page_check(page))
  {
    ...
    ut_a(!page_zip || page_zip_validate_low(page_zip, page, NULL, FALSE));
  }
...
static ulint recv_read_in_area(const page_id_t &page_id)
{
  ...
  for (ulint page_no = low_limit; page_no < low_limit + RECV_READ_AHEAD_AREA; page_no++)
  {
...
loop:
  ...
  {
    for (recv_addr = static_cast<recv_addr_t *>(HASH_GET_FIRST(recv_sys->addr_hash, i)); recv_addr != 0;
...
/*================================*/
{
  ...
  for (i = 0; i < n_hash_cells; i++)
  {
    ...
    recv_addr = static_cast<recv_addr_t *>(hash_get_nth_cell(recv_sys->addr_hash, i)->node);

    while (recv_addr != NULL)
    {
      ib::trace() << "recv_addr {State: " << recv_addr->state << ", Space id: " << recv_addr->space
                  << "Page no: " << recv_addr->page_no << ". index i: " << i << "\n";
      ...
      buf_page_init_for_backup_restore(page_id_t(recv_addr->space, recv_addr->page_no), page_size, block);
      ...
      if (!success)
      {
        ib::fatal() << "Cannot extend tablespace " << recv_addr->space << " to hold " << recv_addr->page_no << " pages";
      }
      ...
      {
        error = fil_io(IORequestRead, true, page_id, page_size, 0, page_size.physical(), block->page.zip.data, NULL);
      ...
      {
        error = fil_io(IORequestRead, true, page_id, page_size, 0, page_size.logical(), block->frame, NULL);
      }

      if (error != DB_SUCCESS)
      {
        ib::fatal() << "Cannot read from tablespace " << recv_addr->space << " page number " << recv_addr->page_no;
      }
      ...
      if (page_size.is_compressed())
      {
        error = fil_io(IORequestWrite, true, page_id, page_size, 0, page_size.physical(), block->page.zip.data, NULL);
      }
      else
      {
        error = fil_io(IORequestWrite, true, page_id, page_size, 0, page_size.logical(), block->frame, NULL);
      }
    skip_this_recv_addr:
      recv_addr = static_cast<recv_addr_t *>(HASH_GET_NEXT(addr_hash, recv_addr));
    }
...
static ulint recv_parse_log_rec(mlog_id_t *type, byte *ptr, byte *end_ptr, ulint *space, ulint *page_no, bool apply,
                                byte **body)
{
  byte *new_ptr;
  ...
  *body = NULL;
  ...
  switch (*ptr)
  {
    ...
    case MLOG_LSN:
      ...
      if (new_ptr != NULL)
      {
        const lsn_t lsn = static_cast<lsn_t>(*space) << 32 | *page_no;
...
static lsn_t recv_calc_lsn_on_data_add(
    /*======================*/
                     ...
                     added, log block headers not included */
{
  ...
  lsn_len = len;
  lsn_len += (lsn_len + frag_len) / (OS_FILE_LOG_BLOCK_SIZE - LOG_BLOCK_HDR_SIZE - LOG_BLOCK_TRL_SIZE) *
...
static bool recv_report_corrupt_log(const byte *ptr, int type, ulint space, ulint page_no)
{
  ib::error() << "############### CORRUPT LOG RECORD FOUND ##################";

  ib::info() << "Log record type " << type << ", page " << space << ":" << page_no
             << ". Log parsing proceeded successfully up to " << recv_sys->recovered_lsn
             << ". Previous log record type " << recv_previous_parsed_rec_type << ", is multi "
             << recv_previous_parsed_rec_is_multi << " Recv offset " << (ptr - recv_sys->buf) << ", prev "
  ...
  ut_print_buf(stderr, recv_sys->buf + recv_previous_parsed_rec_offset - before,
...
static MY_ATTRIBUTE((warn_unused_result)) bool recv_parse_log_recs(lsn_t checkpoint_lsn, store_t store)
{
  byte *ptr;
  byte *end_ptr;
  bool single_rec;
...
#error SIZE_OF_MLOG_CHECKPOINT != 1 + 8
        ...
        DBUG_PRINT("ib_log", ("MLOG_CHECKPOINT(" LSN_PF ") %s at " LSN_PF, lsn,
                              lsn != checkpoint_lsn           ? "ignored"
                              : recv_sys->mlog_checkpoint_lsn ? "reread"
                                                              : "read",
                              recv_sys->recovered_lsn));
      ...
      default:
        ...
        /* fall through */
      case MLOG_INDEX_LOAD:
        DBUG_PRINT("ib_log", ("scan " LSN_PF ": log rec %s"
                              " len " ULINTPF " page " ULINTPF ":" ULINTPF,
                              old_lsn, get_mlog_string(type), len, space, page_no));
    }
  }
  else
  {
    ...
    for (;;)
    {
      ...
      if (recv_sys->found_corrupt_log || type == MLOG_CHECKPOINT || (*ptr & MLOG_SINGLE_REC_FLAG))
      {
        recv_sys->found_corrupt_log = true;
        recv_report_corrupt_log(ptr, type, space, page_no);
        return (true);
      ...
      if (only_mlog_file)
      {
        new_recovered_lsn = recv_calc_lsn_on_data_add(recv_sys->recovered_lsn, len);
        mlog_rec_len += len;
        recv_sys->recovered_offset += len;
        recv_sys->recovered_lsn = new_recovered_lsn;
      }
      ...
      {
        DBUG_PRINT("ib_log", ("scan " LSN_PF ": multi-log end"
                              " total_len " ULINTPF " n=" ULINTPF,
                              recv_sys->recovered_lsn, total_len, n_recs));
        total_len -= mlog_rec_len;
        break;
      }

      DBUG_PRINT("ib_log", ("scan " LSN_PF ": multi-log rec %s"
                            " len " ULINTPF " page " ULINTPF ":" ULINTPF,
                            recv_sys->recovered_lsn, get_mlog_string(type), len, space, page_no));
    }
    ...
    for (;;)
    {
      ...
      if (recv_sys->found_corrupt_log && !recv_report_corrupt_log(ptr, type, space, page_no))
      {
        return (true);
      }
                            ...
                            this lsn */
{
  ...
  do
  {
    ...
    if (!log_block_checksum_is_ok(log_block))
    {
      ib::error() << "Log block " << no << " at lsn " << scanned_lsn
                  ...
                  << log_block_get_checksum(log_block) << ", should be " << log_block_calc_checksum(log_block);
      /* Garbage or an incompletely written log block.
    ...
    if (scanned_lsn > recv_sys->scanned_lsn)
    {
      ...
      DBUG_EXECUTE_IF("reduce_recv_parsing_buf", recv_parsing_buf_size = (70 * 1024););
  ...
  *group_scanned_lsn = scanned_lsn;
  ...
  if (more_data && !recv_sys->found_corrupt_log)
  {
    ...
    if (recv_parse_log_recs(checkpoint_lsn, *store_to_hash))
    {
      ut_ad(recv_sys->found_corrupt_log || recv_sys->found_corrupt_fs ||
            recv_sys->mlog_checkpoint_lsn == recv_sys->recovered_lsn);
      return (true);
    }

    if (*store_to_hash != STORE_NO && mem_heap_get_size(recv_sys->heap) > available_memory)
    {
...
static bool recv_group_scan_log_recs(log_group_t *group, lsn_t *contiguous_lsn, bool last_phase)
{
  ...
  ulint available_mem = UNIV_PAGE_SIZE * (buf_pool_get_n_pages() - (recv_n_pool_free_frames * srv_buf_pool_instances));
  ...
  } while (!recv_scan_log_recs(available_mem, &store_to_hash, log_sys->buf, RECV_SCAN_SIZE, checkpoint_lsn, start_lsn,
  ...
  DBUG_PRINT("ib_log", ("%s " LSN_PF " completed for log group " ULINTPF, last_phase ? "rescan" : "scan",
...
static dberr_t recv_init_missing_space(dberr_t err, const recv_spaces_t::const_iterator &i)
{
  if (srv_force_recovery == 0)
  {
    ib::error() << "Tablespace " << i->first
                << " was not"
                   " found at "
                << i->second.name << ".";
...
static void recv_init_missing_mlog(recv_addr_t *recv_addr)
{
  ...
  ib::fatal() << "Missing MLOG_FILE_NAME or MLOG_FILE_DELETE "
                 "for redo log record "
              << type << " (page " << space_id << ":" << page_no << ") at " << start_lsn;
}

/** Check if all tablespaces were found for crash recovery.
@return error code or DB_SUCCESS */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t recv_init_crash_recovery_spaces(void)
{
  ...
  for (recv_spaces_t::iterator i = recv_spaces.begin(); i != recv_spaces.end(); i++)
  {
  ...
  if (flag_deleted)
  {
    ...
    {
      for (recv_addr_t *recv_addr = static_cast<recv_addr_t *>(HASH_GET_FIRST(recv_sys->addr_hash, h)); recv_addr != 0;
           recv_addr = static_cast<recv_addr_t *>(HASH_GET_NEXT(addr_hash, recv_addr)))
      {
        ...
        if (i->second.deleted)
        {
          ut_ad(missing_spaces.find(space) == missing_spaces.end());
          recv_addr->state = RECV_DISCARDED;
          continue;
        }

        space_set_t::iterator m = missing_spaces.find(space);
  ...
  for (space_set_t::const_iterator m = missing_spaces.begin(); m != missing_spaces.end(); m++)
  {
    recv_spaces_t::iterator i = recv_spaces.find(*m);
    ut_ad(i != recv_spaces.end());

    ib::info() << "Tablespace " << i->first << " was not found at '" << i->second.name
...
dberr_t recv_recovery_from_checkpoint_start(lsn_t flush_lsn)
{
  ...
  fil_io(IORequestLogRead, true, page_id, univ_page_size, 0, LOG_FILE_HDR_SIZE, log_hdr_buf, max_cp_group);

  if (0 == ut_memcmp(log_hdr_buf + LOG_HEADER_CREATOR, (byte *)"ibbackup", (sizeof "ibbackup") - 1))
  {
    if (srv_read_only_mode)
    {
      log_mutex_exit();
    ...
    fil_io(IORequestLogWrite, true, page_id, univ_page_size, 0, OS_FILE_LOG_BLOCK_SIZE, log_hdr_buf, max_cp_group);
  }
  ...
  if ((recv_sys->found_corrupt_log && !srv_force_recovery) || recv_sys->found_corrupt_fs)
  {
    log_mutex_exit();
    return (DB_ERROR);
  }
  ...
  {
    if (!srv_read_only_mode && group->scanned_lsn != checkpoint_lsn)
    {
      ib::error() << "Ignoring the redo log due to missing"
                     " MLOG_CHECKPOINT between the checkpoint "
                  << checkpoint_lsn << " and the end " << group->scanned_lsn << ".";
      if (srv_force_recovery < SRV_FORCE_NO_LOG_REDO)
      {
        log_mutex_exit();
        return (DB_ERROR);
  ...
  if (group->scanned_lsn < checkpoint_lsn || group->scanned_lsn < recv_max_page_lsn)
  {
    ib::error() << "We scanned the log up to " << group->scanned_lsn << ". A checkpoint was at " << checkpoint_lsn
                << " and"
  ...
  MONITOR_SET(MONITOR_LSN_CHECKPOINT_AGE, log_sys->lsn - log_sys->last_checkpoint_lsn);
                         ...
                         divisible by OS_FILE_LOG_BLOCK_SIZE */
{
  ...
  */
  ...
  buf = (byte *)ut_zalloc_nokey(LOG_FILE_HDR_SIZE + OS_FILE_LOG_BLOCK_SIZE);

  for (i = 0; i < n_log_files; i++)
  {
    sprintf(name, "%s%s%lu", log_dir, ib_logfile_basename, (ulong)i);

    log_file = os_file_create_simple(innodb_log_file_key, name, OS_FILE_CREATE, OS_FILE_READ_WRITE, srv_read_only_mode,
                                     &success);
    if (!success)
    {
      ib::fatal() << "Cannot create " << name
                  << ". Check that"
    ...
    success = os_file_set_size(name, log_file, log_file_size, srv_read_only_mode);

    if (!success)
    {
      ib::fatal() << "Cannot set " << name << " size to " << (long long unsigned)log_file_size;
    }
  ...
  log_block_set_checksum(buf + LOG_FILE_HDR_SIZE, log_block_calc_checksum_crc32(buf + LOG_FILE_HDR_SIZE));

  log_block_set_checksum(buf, log_block_calc_checksum_crc32(buf));
  sprintf(name, "%s%s%lu", log_dir, ib_logfile_basename, (ulong)0);

  log_file =
      os_file_create_simple(innodb_log_file_key, name, OS_FILE_OPEN, OS_FILE_READ_WRITE, srv_read_only_mode, &success);
  if (!success)
  {
  ...
  dberr_t err = os_file_write(request, name, log_file, buf, 0, LOG_FILE_HDR_SIZE + OS_FILE_LOG_BLOCK_SIZE);
...
const byte *recv_dblwr_t::find_page(ulint space_id, ulint page_no)
{
