...
ReadView::ReadView() : m_low_limit_id(), m_up_limit_id(), m_creator_trx_id(), m_ids(), m_low_limit_no()
{
...
MVCC::~MVCC()
{
  for (ReadView *view = UT_LIST_GET_FIRST(m_free); view != NULL; view = UT_LIST_GET_FIRST(m_free))
  {
...
void ReadView::copy_trx_ids(const trx_ids_t &trx_ids)
{
  ...
  for (trx_ids_t::const_iterator it = trx_ids.begin(); it != trx_ids.end(); ++it)
  {
...
ReadView *MVCC::get_oldest_view() const
{
  ...
  for (view = UT_LIST_GET_LAST(m_views); view != NULL; view = UT_LIST_GET_PREV(m_view_list, view))
  {
    if (!view->is_closed())
    {
      break;
...
ulint MVCC::size() const
{
  ...
  for (const ReadView *view = UT_LIST_GET_FIRST(m_views); view != NULL; view = UT_LIST_GET_NEXT(m_view_list, view))
  {
    if (!view->is_closed())
    {
      ++size;
