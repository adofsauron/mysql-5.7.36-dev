...
inline bool is_intermediate_file(const std::string &filepath)
{
  ...
  std::vector<std::string> file_name_patterns = {"#sql-", "#sql2-", "#tmp#", "#ren#"};

  /* search for the unsupported patterns */
  for (auto itr = file_name_patterns.begin(); itr != file_name_patterns.end(); itr++)
  {
    if (0 == std::strncmp(file_name.c_str(), itr->c_str(), itr->length()))
    {
      return true;
    }
  }
...
{
  return (type == FIL_TYPE_TEMPORARY || type == FIL_TYPE_IMPORT || type == FIL_TYPE_TABLESPACE);
}
...
#define fil_page_type_is_index(page_type) (page_type == FIL_PAGE_INDEX || page_type == FIL_PAGE_RTREE)

/** Check whether the page is index page (either regular Btree index or Rtree
index */
#define fil_page_index_page_check(page) fil_page_type_is_index(fil_page_get_type(page))
...
fil_space_t *fil_space_get(ulint id) MY_ATTRIBUTE((warn_unused_result));
#ifndef UNIV_HOTBACKUP
...
bool fsp_is_temporary(ulint id) MY_ATTRIBUTE((warn_unused_result, pure));
#endif /* UNIV_DEBUG */
#endif /* !UNIV_HOTBACKUP */
...
char *fil_node_create(const char *name, ulint size, fil_space_t *space, bool is_raw, bool atomic_write,
                      ulint max_pages = ULINT_MAX) MY_ATTRIBUTE((warn_unused_result));
...
fil_space_t *fil_space_acquire(ulint id) MY_ATTRIBUTE((warn_unused_result));
...
@param[in]	id	tablespace ID
@return the tablespace, or NULL if missing or being deleted */
fil_space_t *fil_space_acquire_silent(ulint id) MY_ATTRIBUTE((warn_unused_result));
...
class FilSpace
{
 public:
  ...
  explicit FilSpace(ulint space_id) : m_space(fil_space_acquire(space_id)) {}
...
char *fil_make_filepath(const char *path, const char *name, ib_extention suffix, bool strip_name);
...
/********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
dberr_t fil_ibd_open(bool validate, bool fix_dict, fil_type_t purpose, ulint id, ulint flags, const char *tablename,
                     const char *path_in) MY_ATTRIBUTE((warn_unused_result));
...
@return page type */
inline ulint fil_page_get_type(const byte *page) { return (mach_read_from_2(page + FIL_PAGE_TYPE)); }
...
@param[in]	type	expected page type
@param[in,out]	mtr	mini-transaction */
inline void fil_page_check_type(const page_id_t &page_id, byte *page, ulint type, mtr_t *mtr)
{
...
bool fil_space_is_redo_skipped(
    /*======================*/
    ulint id); /*!< in: space id */
#endif
...
dberr_t fil_tablespace_iterate(
    /*===================*/
    dict_table_t *table, ulint n_io_buffers, PageCallback &callback) MY_ATTRIBUTE((warn_unused_result));
...
@return DB_SUCCESS or error code */
dberr_t fil_set_compression(dict_table_t *table, const char *algorithm) MY_ATTRIBUTE((warn_unused_result));

/** Get the compression type for the tablespace
@param[in]	space_id	Space ID to check
@return the compression algorithm */
Compression::Type fil_get_compression(ulint space_id) MY_ATTRIBUTE((warn_unused_result));
...
inline MY_ATTRIBUTE((warn_unused_result)) bool fil_names_write_if_was_clean(fil_space_t *space, mtr_t *mtr)
{
  ut_ad(log_mutex_own());
