...
class SysTablespace : public Tablespace
{
 public:
  ...
  void set_tablespace_full_status(bool is_full) { m_is_tablespace_full = is_full; }
  ...
  dberr_t check_file_spec(bool *create_new_db, ulint min_expected_tablespace_size);
  ...
  {
    return (sys_tablespace_auto_extend_increment * ((1024 * 1024) / UNIV_PAGE_SIZE));
  }
  ...
  {
    return (ulint)((size / (1024 * 1024)) * ((1024 * 1024) / UNIV_PAGE_SIZE));
  }
  ...
  dberr_t open_or_create(bool is_temp, bool create_new_db, ulint *sum_new_sizes, lsn_t *flush_lsn)
 ...
 private:
  ...
  bool is_valid_size() const { return (m_last_file_size_max >= last_file_size()); }
...
UNIV_INLINE
bool is_system_tablespace(ulint id) { return (id == srv_sys_space.space_id() || id == srv_tmp_space.space_id()); }

/** Check if shared-system or undo tablespace.
@return true if shared-system or undo tablespace */
UNIV_INLINE
bool is_system_or_undo_tablespace(ulint id) { return (id == srv_sys_space.space_id() || srv_is_undo_tablespace(id)); }

/** Check if predefined shared tablespace.
@return true if predefined shared tablespace */
UNIV_INLINE
bool is_predefined_tablespace(ulint id)
{
  ut_ad(srv_sys_space.space_id() == TRX_SYS_SPACE);
  ut_ad(TRX_SYS_SPACE == 0);
  return (is_system_or_undo_tablespace(id) || id == srv_tmp_space.space_id());
}
