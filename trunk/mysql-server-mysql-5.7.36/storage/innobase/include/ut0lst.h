...
{
  ut_list_append(list, elem, GenericGetNode<typename List::elem_type>(list.node));
}
...
{
  ut_list_remove(list, elem->*list.node, GenericGetNode<typename List::elem_type>(list.node));
}
...
void ut_list_map(const List &list, Functor &functor)
{
  ...
  for (typename List::elem_type *elem = list.start; elem != 0; elem = (elem->*list.node).next, ++count)
  {
    functor(elem);
  }
...
void ut_list_reverse(List &list)
{
  ...
  for (typename List::elem_type *elem = list.start; elem != 0; elem = (elem->*list.node).prev)
  {
...
void ut_list_validate(const List &list, Functor &functor)
{
  ...
  for (typename List::elem_type *elem = list.end; elem != 0; elem = (elem->*list.node).prev)
  {
    ++count;
  }
