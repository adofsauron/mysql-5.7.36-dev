...
@return the given output stream */
inline std::ostream &operator<<(std::ostream &out, const lock_table_t &lock) { return (lock.print(out)); }
...
{
  out << "[lock_rec_t: space=" << space << ", page_no=" << page_no << ", n_bits=" << n_bits << "]";
  return (out);
}

inline std::ostream &operator<<(std::ostream &out, const lock_rec_t &lock) { return (lock.print(out)); }
...
struct lock_t
{
  ...
  bool is_predicate() const { return (type_mode & (LOCK_PREDICATE | LOCK_PRDT_PAGE)); }
  ...
  bool is_insert_intention() const { return (type_mode & LOCK_INSERT_INTENTION); }

  ulint type() const { return (type_mode & LOCK_TYPE_MASK); }

  enum lock_mode mode() const { return (static_cast<enum lock_mode>(type_mode & LOCK_MODE_MASK)); }
  ...
  @return space ID */
  ulint space() const { return (un_member.rec_lock.space); }

  /** Get page number of the lock
  @return page number */
  ulint page_number() const { return (un_member.rec_lock.page_no); }
...
inline std::ostream &operator<<(std::ostream &out, const lock_t &lock) { return (lock.print(out)); }
...
class RecLock
{
 public:
  ...
  RecLock(que_thr_t *thr, dict_index_t *index, const buf_block_t *block, ulint heap_no, ulint mode,
      ...
      : m_thr(thr), m_trx(thr_get_trx(thr)), m_mode(mode), m_index(index), m_rec_id(block, heap_no)
  {
  ...
  RecLock(dict_index_t *index, const buf_block_t *block, ulint heap_no, ulint mode)
      : m_thr(), m_trx(), m_mode(mode), m_index(index), m_rec_id(block, heap_no)
  {
    btr_assert_not_corrupted(block, index);
  ...
  lock_t *create(trx_t *trx, bool owns_trx_mutex, bool add_to_hash, const lock_prdt_t *prdt = NULL);
  ...
  static lock_t *lock_alloc(trx_t *trx, dict_index_t *index, ulint mode, const RecID &rec_id, ulint size);
 ...
 private:
  ...
  bool lock_add_priority(lock_t *lock, const lock_t *conflict_lock, bool *high_priority);
  ...
  void init(const page_t *page)
  {
    ...
    ut_ad(dict_index_is_clust(m_index) || !dict_index_is_online_ddl(m_index));
  ...
  @return true if the requested lock mode is for a predicate
          or page lock */
  static bool is_predicate_lock(ulint mode) { return (mode & (LOCK_PREDICATE | LOCK_PRDT_PAGE)); }
...
UNIV_INLINE
ulint lock_mode_stronger_or_eq(
    /*=====================*/
    enum lock_mode mode1,  /*!< in: lock mode */
    enum lock_mode mode2); /*!< in: lock mode */
