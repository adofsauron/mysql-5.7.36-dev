...
#define mem_heap_create(size) mem_heap_create_func((size), __FILE__, __LINE__, MEM_HEAP_DYNAMIC)

/** Macro for memory heap creation.
@param[in]	size		Desired start block size.
@param[in]	type		Heap type */
#define mem_heap_create_typed(size, type) mem_heap_create_func((size), __FILE__, __LINE__, (type))
...
UNIV_INLINE
bool mem_heap_is_top(mem_heap_t *heap, const void *buf, ulint buf_sz) MY_ATTRIBUTE((warn_unused_result));
...
class mem_heap_allocator
{
 public:
  ...
  template <typename U>
  mem_heap_allocator(const mem_heap_allocator<U> &other) : m_heap(other.m_heap)
  {
    // Do nothing
  }
  ...
  void construct(pointer p, const_reference t) { new (reinterpret_cast<void *>(p)) T(t); }
