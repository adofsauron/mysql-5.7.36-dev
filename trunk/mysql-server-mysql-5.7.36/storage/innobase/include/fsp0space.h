...
class Tablespace
{
 public:
  ...
  Tablespace() : m_files(), m_name(), m_space_id(ULINT_UNDEFINED), m_path(), m_flags(), m_ignore_read_only(false)
  {
    /* No op */
  }
  ...
  void set_ignore_read_only(bool read_only_status) { m_ignore_read_only = read_only_status; }
  ...
  ulint get_sum_of_sizes() const
  {
    ...
    for (files_t::const_iterator it = m_files.begin(); it != m_files.end(); ++it)
    {
      sum += it->m_size;
    }
  ...
  dberr_t add_datafile(const char *datafile_path);
