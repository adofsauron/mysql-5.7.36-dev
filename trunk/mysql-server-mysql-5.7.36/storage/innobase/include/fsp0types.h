...
#define FSP_EXTENT_SIZE              \
  ((UNIV_PAGE_SIZE <= (16384)        \
        ...
        : ((UNIV_PAGE_SIZE <= (32768)) ? (2097152 / UNIV_PAGE_SIZE) : (4194304 / UNIV_PAGE_SIZE))))
...
class fseg_header
{
 public:
  ...
  @param[in]	mtr	the mini-transaction.  No redo logs are
                          ...
                          mini-transaction */
  fseg_header(const fseg_header_t *header, mtr_t *mtr) : m_header(header), m_mtr(mtr) {}
...
inline std::ostream &operator<<(std::ostream &out, const fseg_header &header) { return (header.to_stream(out)); }
#endif /* UNIV_DEBUG */
...
bool fsp_is_system_temporary(ulint space_id);

/** Check if checksum is disabled for the given space.
@param[in]	space_id	verify is checksum is enabled for given space.
@return true if checksum is disabled for given space. */
bool fsp_is_checksum_disabled(ulint space_id);
...
#define FSP_FLAGS_WIDTH                                                                                         \
  (FSP_FLAGS_WIDTH_POST_ANTELOPE + FSP_FLAGS_WIDTH_ZIP_SSIZE + FSP_FLAGS_WIDTH_ATOMIC_BLOBS +                   \
   FSP_FLAGS_WIDTH_PAGE_SSIZE + FSP_FLAGS_WIDTH_DATA_DIR + FSP_FLAGS_WIDTH_SHARED + FSP_FLAGS_WIDTH_TEMPORARY + \
...
#define FSP_FLAGS_POS_UNUSED (FSP_FLAGS_POS_ENCRYPTION + FSP_FLAGS_WIDTH_ENCRYPTION)
