...
UNIV_INLINE
lock_word_t os_atomic_test_and_set(volatile lock_word_t *ptr, lock_word_t new_val);
...
UNIV_INLINE
lock_word_t os_atomic_val_compare_and_swap(volatile lock_word_t *ptr, lock_word_t old_val, lock_word_t new_val);
...
#define os_compare_and_swap_lint(ptr, old_val, new_val) (win_cmp_and_xchg_lint(ptr, new_val, old_val) == old_val)

#define os_compare_and_swap_ulint(ptr, old_val, new_val) (win_cmp_and_xchg_ulint(ptr, new_val, old_val) == old_val)

#define os_compare_and_swap_uint32(ptr, old_val, new_val) (InterlockedCompareExchange(ptr, new_val, old_val) == old_val)

/* windows thread objects can always be passed to windows atomic functions */
#define os_compare_and_swap_thread_id(ptr, old_val, new_val) (win_cmp_and_xchg_dword(ptr, new_val, old_val) == old_val)
...
#define os_atomic_increment_ulint(ptr, amount)                                                               \
  (static_cast<ulint>(win_xchg_and_add(reinterpret_cast<volatile lint *>(ptr), static_cast<lint>(amount))) + \
   static_cast<ulint>(amount))

#define os_atomic_increment_uint32(ptr, amount)                                                           \
  (static_cast<ulint>(InterlockedExchangeAdd(reinterpret_cast<long *>(ptr), static_cast<long>(amount))) + \
   static_cast<ulint>(amount))

#define os_atomic_increment_uint64(ptr, amount)                                                      \
  (static_cast<ib_uint64_t>(                                                                         \
       InterlockedExchangeAdd64(reinterpret_cast<LONGLONG *>(ptr), static_cast<LONGLONG>(amount))) + \
...
#define os_atomic_decrement_lint(ptr, amount) (win_xchg_and_add(ptr, -(static_cast<lint>(amount))) - amount)
...
#define os_atomic_decrement_uint32(ptr, amount)                                                                    \
  (static_cast<ib_uint32_t>(InterlockedExchangeAdd(reinterpret_cast<long *>(ptr), -(static_cast<long>(amount)))) - \
...
#define os_compare_and_swap_uint32(ptr, old_val, new_val) os_compare_and_swap(ptr, old_val, new_val)

#else

UNIV_INLINE
bool os_compare_and_swap_ulint(volatile ulint *ptr, ulint old_val, ulint new_val)
{
...
UNIV_INLINE
bool os_compare_and_swap_uint32(volatile ib_uint32_t *ptr, ib_uint32_t old_val, ib_uint32_t new_val)
{
  return __atomic_compare_exchange_n(ptr, &old_val, new_val, 0, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
}
...
#else
UNIV_INLINE
bool os_compare_and_swap_thread_id(volatile os_thread_id_t *ptr, os_thread_id_t old_val, os_thread_id_t new_val)
{
  return __atomic_compare_exchange_n(ptr, &old_val, new_val, 0, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
}
#endif /* HAVE_GCC_SYNC_BUILTINS */
...
#else
#define os_atomic_increment(ptr, amount) __atomic_add_fetch(ptr, amount, __ATOMIC_SEQ_CST)
#endif /* HAVE_GCC_SYNC_BUILTINS */
...
#else
#define os_atomic_decrement(ptr, amount) __atomic_sub_fetch(ptr, amount, __ATOMIC_SEQ_CST)
#endif /* HAVE_GCC_SYNC_BUILTINS */
...
#define IB_MEMORY_BARRIER_STARTUP_MSG "Solaris memory ordering functions are used for memory barrier"

#elif defined HAVE_IB_GCC_ATOMIC_THREAD_FENCE
#define HAVE_MEMORY_BARRIER
#define os_rmb __atomic_thread_fence(__ATOMIC_ACQUIRE)
#define os_wmb __atomic_thread_fence(__ATOMIC_RELEASE)
#define IB_MEMORY_BARRIER_STARTUP_MSG "GCC builtin __atomic_thread_fence() is used for memory barrier"

#elif defined(HAVE_IB_GCC_SYNC_SYNCHRONISE)
#define HAVE_MEMORY_BARRIER
#define os_rmb __sync_synchronize()
#define os_wmb __sync_synchronize()
#define IB_MEMORY_BARRIER_STARTUP_MSG "GCC builtin __sync_synchronize() is used for memory barrier"
...
#define IB_MEMORY_BARRIER_STARTUP_MSG "_mm_lfence() and _mm_sfence() are used for memory barrier"
