...
class PageBulk
{
 public:
  ...
  PageBulk(dict_index_t *index, trx_id_t trx_id, ulint page_no, ulint level, FlushObserver *observer)
      : m_heap(NULL),
        m_index(index),
        m_mtr(NULL),
        m_trx_id(trx_id),
  ...
  /** Get page no */
  ulint getPageNo() { return (m_page_no); }
...
class BtrBulk
{
 public:
  ...
  BtrBulk(dict_index_t *index, trx_id_t trx_id, FlushObserver *observer)
      : m_heap(NULL), m_index(index), m_trx_id(trx_id), m_flush_observer(observer)
  {
