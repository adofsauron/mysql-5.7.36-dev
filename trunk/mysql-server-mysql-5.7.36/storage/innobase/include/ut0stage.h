...
inline ut_stage_alter_t::~ut_stage_alter_t()
{
  ...
  mysql_stage_set_work_completed(m_progress, mysql_stage_get_work_estimated(m_progress));
...
inline void ut_stage_alter_t::begin_phase_read_pk(ulint n_sort_indexes)
{
  ...
  m_progress = mysql_set_stage(srv_stage_alter_table_read_pk_internal_sort.m_key);
...
inline void ut_stage_alter_t::end_phase_read_pk()
{
  ...
  {
    m_n_recs_per_page = std::max(static_cast<double>(m_n_pk_recs) / m_n_pk_pages, 1.0);
  }
}
...
/** Flag the beginning of the insert phase. */
inline void ut_stage_alter_t::begin_phase_insert() { change_phase(&srv_stage_alter_table_insert); }
...
/** Flag the beginning of the log index phase. */
inline void ut_stage_alter_t::begin_phase_log_index() { change_phase(&srv_stage_alter_table_log_index); }

/** Flag the beginning of the log table phase. */
inline void ut_stage_alter_t::begin_phase_log_table() { change_phase(&srv_stage_alter_table_log_table); }

/** Flag the beginning of the end phase. */
inline void ut_stage_alter_t::begin_phase_end() { change_phase(&srv_stage_alter_table_end); }
...
inline void ut_stage_alter_t::reestimate()
{
  ...
  if (m_cur_phase == LOG_TABLE)
  {
    mysql_stage_set_work_estimated(m_progress,
                                   mysql_stage_get_work_completed(m_progress) + row_log_estimate_work(m_pk));
    return;
  }
  ...
  const ulint n_pk_pages = m_cur_phase != READ_PK ? m_n_pk_pages : m_pk->stat_n_leaf_pages;
  ...
  ulonglong estimate = n_pk_pages * (1                  /* read PK */
