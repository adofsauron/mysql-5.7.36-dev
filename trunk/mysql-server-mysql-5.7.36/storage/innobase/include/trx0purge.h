...
struct purge_iter_t
{
  purge_iter_t() : trx_no(), undo_no(), undo_rseg_space(ULINT_UNDEFINED)
  {
    // Do nothing
  }
 ...
 public:
  Truncate()
      : m_undo_for_trunc(ULINT_UNDEFINED),
        ...
        m_purge_rseg_truncate_frequency(static_cast<ulint>(srv_purge_rseg_truncate_frequency))
  {
    /* Do Nothing. */
  }
  ...
  bool is_marked() const { return (!(m_undo_for_trunc == ULINT_UNDEFINED)); }
  ...
  ulint get_marked_space_id() const { return (m_undo_for_trunc); }

  /** Add rseg to truncate vector.
  @param[in,out]	rseg	rseg for truncate */
  void add_rseg_to_trunc(trx_rseg_t *rseg) { m_rseg_for_trunc.push_back(rseg); }

  /** Get number of rsegs registered for truncate.
  @return return number of rseg that belongs to tablespace mark
  for truncate. */
  ulint rsegs_size() const { return (m_rseg_for_trunc.size()); }
  ...
  ulint get_scan_start() const { return (m_scan_start); }

  /** Set the tablespace id to start scanning from. */
  void set_scan_start(ulint space_id) { m_scan_start = space_id; }
  ...
  bool needs_fix_up(ulint space_id) const { return (is_log_present(space_id)); }

  /** Add undo tablespace to truncate vector.
  @param[in]	space_id	space id of tablespace to
                                  truncate */
  static void add_space_to_trunc_list(ulint space_id) { s_spaces_to_truncate.push_back(space_id); }

  /** Clear the truncate vector. */
  static void clear_trunc_list() { s_spaces_to_truncate.clear(); }
  ...
  {
    return (std::find(s_spaces_to_truncate.begin(), s_spaces_to_truncate.end(), space_id) !=
  ...
  {
    return (std::find(s_fix_up_spaces.begin(), s_fix_up_spaces.end(), space_id) != s_fix_up_spaces.end());
  }

  /** Get local rseg purge truncate frequency
  @return rseg purge truncate frequency. */
  ulint get_rseg_truncate_frequency() const { return (m_purge_rseg_truncate_frequency); }
  ...
  @return DB_SUCCESS or error code. */
  dberr_t start_logging(ulint space_id) { return (init(space_id)); }

  /* Mark completion of logging./
  @param	space_id	space id of undo tablespace */
  void done_logging(ulint space_id) { return (done(space_id)); }
