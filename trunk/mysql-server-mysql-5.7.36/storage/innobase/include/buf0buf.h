...
class page_id_t
{
 public:
  ...
  page_id_t(ulint space, ulint page_no)
      : m_space(static_cast<ib_uint32_t>(space)), m_page_no(static_cast<ib_uint32_t>(page_no)), m_fold(ULINT_UNDEFINED)
  {
  ...
  inline ib_uint32_t space() const { return (m_space); }

  /** Retrieve the page number.
  @return page number */
  inline ib_uint32_t page_no() const { return (m_page_no); }
  ...
  inline bool equals_to(const page_id_t &a) const { return (a.space() == m_space && a.page_no() == m_page_no); }
...
UNIV_INLINE
void buf_pool_mutex_enter_all(void);
/*===========================*/

/********************************************************************/ /**
 Release mutex on all buffer pool instances */
UNIV_INLINE
void buf_pool_mutex_exit_all(void);
/*==========================*/
...
*/
extern "C" os_thread_ret_t DECLARE_THREAD(buf_resize_thread)(
    /*==============================*/
...
/**************************************************************/ /**
 ...
 in LA! */
#define buf_page_get(ID, SIZE, LA, MTR) buf_page_get_gen(ID, SIZE, LA, NULL, BUF_GET, __FILE__, __LINE__, MTR)
/**************************************************************/ /**
...
#define buf_page_get_with_no_latch(ID, SIZE, MTR) \
  buf_page_get_gen(ID, SIZE, RW_NO_LATCH, NULL, BUF_GET_NO_LATCH, __FILE__, __LINE__, MTR)
/********************************************************************/ /**
...
/********************************************************************/ /**
 ...
 @return value */
UNIV_INLINE
ib_uint64_t buf_block_get_modify_clock(
    /*=======================*/
    buf_block_t *block);                                              /*!< in: block */
/*******************************************************************/ /**
...
UNIV_INLINE
buf_block_t *buf_page_get_block(
    /*===============*/
...
a watch sentinel. */
UNIV_INLINE
buf_page_t *buf_page_hash_get_locked(buf_pool_t *buf_pool, const page_id_t &page_id, rw_lock_t **lock, ulint lock_mode,
                                     bool watch = false);
...
UNIV_INLINE
buf_block_t *buf_block_hash_get_locked(buf_pool_t *buf_pool, const page_id_t &page_id, rw_lock_t **lock,
                                       ulint lock_mode);
...
buf_page_t *buf_pool_watch_set(const page_id_t &page_id, rw_lock_t **hash_lock) MY_ATTRIBUTE((warn_unused_result));
...
ibool buf_pool_watch_occurred(const page_id_t &page_id) MY_ATTRIBUTE((warn_unused_result));
...
#define assert_block_ahi_valid(block) ut_a((block)->index || os_atomic_increment_ulint(&(block)->n_pointers, 0) == 0)
...
class FlushHp : public HazardPointer
{
 public:
  ...
  @param buf_pool buffer pool instance
  @param mutex	mutex that is protecting the hp. */
  FlushHp(const buf_pool_t *buf_pool, const ib_mutex_t *mutex) : HazardPointer(buf_pool, mutex) {}
...
class LRUHp : public HazardPointer
{
 public:
  ...
  @param buf_pool buffer pool instance
  @param mutex	mutex that is protecting the hp. */
  LRUHp(const buf_pool_t *buf_pool, const ib_mutex_t *mutex) : HazardPointer(buf_pool, mutex) {}
...
class LRUItr : public LRUHp
{
 public:
  ...
  @param buf_pool buffer pool instance
  @param mutex	mutex that is protecting the hp. */
  LRUItr(const buf_pool_t *buf_pool, const ib_mutex_t *mutex) : LRUHp(buf_pool, mutex) {}
...
#define buf_page_hash_lock_held_s_or_x(buf_pool, bpage) \
  (buf_page_hash_lock_held_s((buf_pool), (bpage)) || buf_page_hash_lock_held_x((buf_pool), (bpage)))
