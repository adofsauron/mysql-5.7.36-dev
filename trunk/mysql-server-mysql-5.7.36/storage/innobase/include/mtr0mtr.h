...
#define mtr_is_block_fix(m, o, t, table) (mtr_memo_contains(m, o, t) || dict_table_is_intrinsic(table))

/** Check if memo contains the given page ignore if table is intrinsic
@return TRUE if contains or table is intrinsic. */
#define mtr_is_page_fix(m, p, t, table) (mtr_memo_contains_page(m, p, t) || dict_table_is_intrinsic(table))
...
struct mtr_t
{
  ...
  @param[in]	write_mlog_checkpoint	Write MLOG_CHECKPOINT marker
                                          if it is enabled. */
  void commit_checkpoint(lsn_t checkpoint_lsn, bool write_mlog_checkpoint);
  ...
  @return	value read */
  inline ulint read_ulint(const byte *ptr, mlog_id_t type) const MY_ATTRIBUTE((warn_unused_result));
  ...
  fil_space_t *x_lock_space(ulint space_id, const char *file, ulint line);
  ...
  bool is_inside_ibuf() const { return (m_impl.m_inside_ibuf); }

  /*
  @return true if the mini-transaction is active */
  bool is_active() const { return (m_impl.m_state == MTR_STATE_ACTIVE); }

  /** Get flush observer
  @return flush observer */
  FlushObserver *get_flush_observer() const { return (m_impl.m_flush_observer); }
  ...
  @return	true if contains */
  static bool memo_contains(mtr_buf_t *memo, const void *object, ulint type) MY_ATTRIBUTE((warn_unused_result));
  ...
  bool has_committed() const { return (m_impl.m_state == MTR_STATE_COMMITTED); }

  /** @return true if the mini-transaction is committing */
  bool is_committing() const { return (m_impl.m_state == MTR_STATE_COMMITTING); }

  /** @return true if mini-transaction contains modifications. */
  bool has_modifications() const { return (m_impl.m_modifications); }

  /** @return the memo stack */
  const mtr_buf_t *get_memo() const { return (&m_impl.m_memo); }

  /** @return the memo stack */
  mtr_buf_t *get_memo() { return (&m_impl.m_memo); }
#endif /* UNIV_DEBUG */

  /** @return true if a record was added to the mini-transaction */
  bool is_dirty() const { return (m_impl.m_made_dirty); }
  ...
  @return true if the mtr is dirtying a clean page. */
  static bool is_block_dirtied(const buf_block_t *block) MY_ATTRIBUTE((warn_unused_result));
