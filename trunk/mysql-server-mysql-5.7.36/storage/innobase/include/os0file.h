...
struct Compression
{
  ...
  static bool is_compressed_page(const byte *page) MY_ATTRIBUTE((warn_unused_result));

  /** Check the page header type field.
  @param[in]   page            Page contents
  @return true if it is a compressed and encrypted page */
  static bool is_compressed_encrypted_page(const byte *page) MY_ATTRIBUTE((warn_unused_result));
  ...
  static dberr_t check(const char *algorithm, Compression *type) MY_ATTRIBUTE((warn_unused_result));

  /** Validate the algorithm string.
  @param[in]      algorithm       Compression algorithm to check
  @return DB_SUCCESS or error code */
  static dberr_t validate(const char *algorithm) MY_ATTRIBUTE((warn_unused_result));
  ...
  @return the string representation */
  static std::string to_string(const meta_t &meta) MY_ATTRIBUTE((warn_unused_result));
...
key + iv + checksum */
static const ulint ENCRYPTION_INFO_SIZE_V1 = (ENCRYPTION_MAGIC_SIZE + (ENCRYPTION_KEY_LEN * 2) + 2 * sizeof(ulint));
...
static const ulint ENCRYPTION_INFO_SIZE_V2 =
    (ENCRYPTION_MAGIC_SIZE + (ENCRYPTION_KEY_LEN * 2) + ENCRYPTION_SERVER_UUID_LEN + 2 * sizeof(ulint));
...
struct Encryption
{
  ...
  Encryption(const Encryption &other)
      : m_type(other.m_type), m_key(other.m_key), m_klen(other.m_klen), m_iv(other.m_iv){};

  /** Check if page is encrypted page or not
  @param[in]	page	page which need to check
  @return true if it is a encrypted page */
  static bool is_encrypted_page(const byte *page) MY_ATTRIBUTE((warn_unused_result));

  /** Check the encryption option and set it
  @param[in]	option		encryption option
  @param[in/out]	encryption	The encryption type
  @return DB_SUCCESS or DB_UNSUPPORTED */
  dberr_t set_algorithm(const char *option, Encryption *type) MY_ATTRIBUTE((warn_unused_result));
  ...
  static void get_master_key(ulint *master_key_id, byte **master_key, Encryption::Version *version);
...
class IORequest
{
 public:
  ...
  /** Default constructor */
  IORequest() : m_block_size(UNIV_SECTOR_SIZE), m_type(READ), m_compression(), m_encryption() { /* No op */ }

  /**
  @param[in]	type		Request type, can be a value that is
                                  ORed from the above enum */
  explicit IORequest(ulint type)
      : m_block_size(UNIV_SECTOR_SIZE), m_type(static_cast<uint16_t>(type)), m_compression(), m_encryption()
  {
  ...
  bool is_read() const MY_ATTRIBUTE((warn_unused_result)) { return ((m_type & READ) == READ); }

  /** @return true if it is a write request */
  bool is_write() const MY_ATTRIBUTE((warn_unused_result)) { return ((m_type & WRITE) == WRITE); }

  /** @return true if it is a redo log write */
  bool is_log() const MY_ATTRIBUTE((warn_unused_result)) { return ((m_type & LOG) == LOG); }

  /** @return true if it is a row log entry used in online DDL */
  bool is_row_log() const MY_ATTRIBUTE((warn_unused_result)) { return ((m_type & ROW_LOG) == ROW_LOG); }

  /** @return true if the simulated AIO thread should be woken up */
  bool is_wake() const MY_ATTRIBUTE((warn_unused_result)) { return ((m_type & DO_NOT_WAKE) == 0); }

  /** @return true if partial read warning disabled */
  bool is_partial_io_warning_disabled() const MY_ATTRIBUTE((warn_unused_result))
  {
    return ((m_type & DISABLE_PARTIAL_IO_WARNINGS) == DISABLE_PARTIAL_IO_WARNINGS);
  }
  ...
  bool ignore_missing() const MY_ATTRIBUTE((warn_unused_result)) { return (ignore_missing(m_type)); }

  /** @return true if punch hole should be used */
  bool punch_hole() const MY_ATTRIBUTE((warn_unused_result)) { return ((m_type & PUNCH_HOLE) == PUNCH_HOLE); }
  ...
  ulint block_size() const MY_ATTRIBUTE((warn_unused_result)) { return (m_block_size); }

  /** Set the block size for IO
  @param[in] block_size		Block size to set */
  void block_size(ulint block_size) { m_block_size = static_cast<uint32_t>(block_size); }
  ...
  Compression compression_algorithm() const MY_ATTRIBUTE((warn_unused_result)) { return (m_compression); }
  ...
  bool is_compression_enabled() const MY_ATTRIBUTE((warn_unused_result)) { return ((m_type & NO_COMPRESSION) == 0); }
  ...
  Encryption encryption_algorithm() const MY_ATTRIBUTE((warn_unused_result)) { return (m_encryption); }

  /** @return true if the page should be encrypted. */
  bool is_encrypted() const MY_ATTRIBUTE((warn_unused_result)) { return (m_encryption.m_type != Encryption::NONE); }
...
os_file_dir_t os_file_opendir(const char *dirname, bool is_fatal);
...
#define register_pfs_file_open_begin(state, locker, key, op, name, src_file, src_line)          \
  do                                                                                            \
  {                                                                                             \
    locker = PSI_FILE_CALL(get_thread_file_name_locker)(state, key.m_value, op, name, &locker); \
    if (locker != NULL)                                                                         \
    {                                                                                           \
      PSI_FILE_CALL(start_file_open_wait)(locker, src_file, static_cast<uint>(src_line));       \
    }                                                                                           \
  } while (0)
...
#define register_pfs_file_rename_begin(state, locker, key, op, name, src_file, src_line) \
  register_pfs_file_open_begin(state, locker, key, op, name, src_file, static_cast<uint>(src_line))
...
#define register_pfs_file_close_begin(state, locker, key, op, name, src_file, src_line)         \
  do                                                                                            \
  {                                                                                             \
    locker = PSI_FILE_CALL(get_thread_file_name_locker)(state, key.m_value, op, name, &locker); \
    if (locker != NULL)                                                                         \
    {                                                                                           \
      PSI_FILE_CALL(start_file_close_wait)(locker, src_file, static_cast<uint>(src_line));      \
    }                                                                                           \
  } while (0)
...
#define register_pfs_file_io_begin(state, locker, file, count, op, src_file, src_line)      \
  do                                                                                        \
  {                                                                                         \
    locker = PSI_FILE_CALL(get_thread_file_stream_locker)(state, file.m_psi, op);           \
    if (locker != NULL)                                                                     \
    {                                                                                       \
      PSI_FILE_CALL(start_file_wait)(locker, count, src_file, static_cast<uint>(src_line)); \
    }                                                                                       \
  } while (0)
...
#define os_file_create(key, name, create, purpose, type, read_only, success) \
  pfs_os_file_create_func(key, name, create, purpose, type, read_only, success, __FILE__, __LINE__)

#define os_file_create_simple(key, name, create, access, read_only, success) \
  pfs_os_file_create_simple_func(key, name, create, access, read_only, success, __FILE__, __LINE__)

#define os_file_create_simple_no_error_handling(key, name, create_mode, access, read_only, success)              \
  pfs_os_file_create_simple_no_error_handling_func(key, name, create_mode, access, read_only, success, __FILE__, \
                                                   __LINE__)
...
#define os_aio(type, mode, name, file, buf, offset, n, read_only, message1, message2) \
  pfs_os_aio_func(type, mode, name, file, buf, offset, n, read_only, message1, message2, __FILE__, __LINE__)
...
#define os_file_read_no_error_handling_pfs(type, file, buf, offset, n, o) \
  pfs_os_file_read_no_error_handling_func(type, file, buf, offset, n, o, __FILE__, __LINE__)

#define os_file_read_no_error_handling_int_fd(type, file, buf, offset, n, o) \
  pfs_os_file_read_no_error_handling_int_fd_func(type, file, buf, offset, n, o, __FILE__, __LINE__)
...
#define os_file_write_int_fd(type, name, file, buf, offset, n) \
  pfs_os_file_write_int_fd_func(type, name, file, buf, offset, n, __FILE__, __LINE__)

#define os_file_flush_pfs(file) pfs_os_file_flush_func(file, __FILE__, __LINE__)

#define os_file_rename(key, oldpath, newpath) pfs_os_file_rename_func(key, oldpath, newpath, __FILE__, __LINE__)

#define os_file_delete(key, name) pfs_os_file_delete_func(key, name, __FILE__, __LINE__)
...
@return own: handle to the file, not defined if error, error number
        can be retrieved with os_file_get_last_error */
UNIV_INLINE
pfs_os_file_t pfs_os_file_create_simple_func(mysql_pfs_key_t key, const char *name, ulint create_mode,
                                             ulint access_type, bool read_only, bool *success, const char *src_file,
                                             ulint src_line) MY_ATTRIBUTE((warn_unused_result));
...
@return own: handle to the file, not defined if error, error number
        can be retrieved with os_file_get_last_error */
UNIV_INLINE
pfs_os_file_t pfs_os_file_create_simple_no_error_handling_func(mysql_pfs_key_t key, const char *name, ulint create_mode,
                                                               ulint access_type, bool read_only, bool *success,
...
@return own: handle to the file, not defined if error, error number
        can be retrieved with os_file_get_last_error */
UNIV_INLINE
pfs_os_file_t pfs_os_file_create_func(mysql_pfs_key_t key, const char *name, ulint create_mode, ulint purpose,
                                      ulint type, bool read_only, bool *success, const char *src_file, ulint src_line)
    MY_ATTRIBUTE((warn_unused_result));
...
@return true if success */
UNIV_INLINE
bool pfs_os_file_close_func(pfs_os_file_t file, const char *src_file, ulint src_line);
...
UNIV_INLINE
dberr_t pfs_os_file_read_func(IORequest &type, pfs_os_file_t file, void *buf, os_offset_t offset, ulint n,
                              const char *src_file, ulint src_line);
...
@return DB_SUCCESS if request was successful */
UNIV_INLINE
dberr_t pfs_os_file_read_no_error_handling_func(IORequest &type, pfs_os_file_t file, void *buf, os_offset_t offset,
                                                ulint n, ulint *o, const char *src_file, ulint src_line);
...
UNIV_INLINE
dberr_t pfs_os_file_read_no_error_handling_int_fd_func(IORequest &type, int file, void *buf, os_offset_t offset,
                                                       ulint n, ulint *o, const char *src_file, ulint src_line);
...
UNIV_INLINE
dberr_t pfs_os_aio_func(IORequest &type, ulint mode, const char *name, pfs_os_file_t file, void *buf,
                        os_offset_t offset, ulint n, bool read_only, fil_node_t *m1, void *m2, const char *src_file,
                        ulint src_line);
...
@return DB_SUCCESS if request was successful */
UNIV_INLINE
dberr_t pfs_os_file_write_func(IORequest &type, const char *name, pfs_os_file_t file, const void *buf,
...
@return DB_SUCCESS if request was successful */
UNIV_INLINE
dberr_t pfs_os_file_write_int_fd_func(IORequest &type, const char *name, int file, const void *buf, os_offset_t offset,
                                      ulint n, const char *src_file, ulint src_line);
...
@return TRUE if success */
UNIV_INLINE
bool pfs_os_file_flush_func(pfs_os_file_t file, const char *src_file, ulint src_line);
...
@return true if success */
UNIV_INLINE
bool pfs_os_file_rename_func(mysql_pfs_key_t key, const char *oldpath, const char *newpath, const char *src_file,
                             ulint src_line);
...
@return true if success */
UNIV_INLINE
bool pfs_os_file_delete_func(mysql_pfs_key_t key, const char *name, const char *src_file, ulint src_line);
...
@return true if success */
UNIV_INLINE
bool pfs_os_file_delete_if_exists_func(mysql_pfs_key_t key, const char *name, bool *exist, const char *src_file,
                                       ulint src_line);
...
#define os_file_create_simple(key, name, create_mode, access, read_only, success) \
  os_file_create_simple_func(name, create_mode, access, read_only, success)

#define os_file_create_simple_no_error_handling(key, name, create_mode, access, read_only, success) \
  os_file_create_simple_no_error_handling_func(name, create_mode, access, read_only, success)

#define os_file_close_pfs(file) os_file_close_func(file)

#define os_aio(type, mode, name, file, buf, offset, n, read_only, message1, message2) \
  os_aio_func(type, mode, name, file, buf, offset, n, read_only, message1, message2)

#define os_file_read_pfs(type, file, buf, offset, n) os_file_read_func(type, file, buf, offset, n)
...
#define os_file_write_pfs(type, name, file, buf, offset, n) os_file_write_func(type, name, file, buf, offset, n)

#define os_file_write_int_fd(type, name, file, buf, offset, n) os_file_write_func(type, name, file, buf, offset, n)
...
#define os_file_delete_if_exists(key, name, exist) os_file_delete_if_exists_func(name, exist)
...
#ifdef UNIV_PFS_IO
#define os_file_read(type, file, buf, offset, n) os_file_read_pfs(type, file, buf, offset, n)
#else
#define os_file_read(type, file, buf, offset, n) os_file_read_pfs(type, file.m_file, buf, offset, n)
#endif
...
#else
#define os_file_write(type, name, file, buf, offset, n) os_file_write_pfs(type, name, file.m_file, buf, offset, n)
#endif
...
ulint os_file_get_last_error(bool report);
...
dberr_t os_file_read_no_error_handling_func(IORequest &type, os_file_t file, void *buf, os_offset_t offset, ulint n,
                                            ulint *o) MY_ATTRIBUTE((warn_unused_result));
...
char *os_file_make_new_pathname(const char *old_path, const char *new_name);
...
bool os_aio_init(ulint n_read_segs, ulint n_write_segs, ulint n_slots_sync);
...
@return DB_SUCCESS or error code */
dberr_t os_aio_handler(ulint segment, fil_node_t **m1, void **m2, IORequest *type);
...
@return DB_SUCCESS or error code */
dberr_t os_file_punch_hole(os_file_t fh, os_offset_t off, os_offset_t len) MY_ATTRIBUTE((warn_unused_result));
...
bool os_is_sparse_file_supported(const char *path, pfs_os_file_t fh) MY_ATTRIBUTE((warn_unused_result));
