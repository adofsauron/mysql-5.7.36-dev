...
class MutexDebug
{
 public:
  ...
  {
    /** Constructor */
    Context() : m_mutex(), m_filename(), m_line(), m_thread_id(os_thread_id_t(ULINT_UNDEFINED)) { /* No op */ }
    ...
    void locked(const Mutex *mutex, const char *filename, ulint line) UNIV_NOTHROW
    {
      m_mutex = mutex;
  ...
  bool is_owned() const UNIV_NOTHROW { return (os_thread_eq(m_context.m_thread_id, os_thread_get_curr_id())); }
  ...
  os_thread_id_t get_thread_id() const UNIV_NOTHROW { return (m_context.m_thread_id); }
...
#endif /* UNIV_DEBUG */
{
 public:
  ...
  void init(const MutexType &mutex, latch_id_t id, const char *filename, uint32_t line) UNIV_NOTHROW
  {
    m_id = id;
  ...
  @param[in]	filename	Filename from where it was called
  @param[in]	line		Line number from where it was called */
  void enter(const MutexType &mutex, const char *filename, ulint line) UNIV_NOTHROW
  {
  ...
  @param[in]	filename	Filename from where it was called
  @param[in]	line		Line number from where it was called */
  void locked(const MutexType &mutex, const char *filename, ulint line) UNIV_NOTHROW
  {
  ...
  void release(const MutexType &mutex) UNIV_NOTHROW { ut_d(MutexDebug<MutexType>::release(&mutex)); }
