...
class dict_intrinsic_table_t
{
 public:
  ...
  dict_intrinsic_table_t(dict_table_t *handler) : m_handler(handler) { /* Do nothing. */ }
...
typedef std::map<std::string, dict_intrinsic_table_t *, std::less<std::string>,
...
class innodb_session_t
{
 public:
  ...
  ~innodb_session_t()
  {
    ...
    for (table_cache_t::iterator it = m_open_tables.begin(); it != m_open_tables.end(); ++it)
    {
      delete (it->second);
    }
  }
  ...
  void register_table_handler(const char *table_name, dict_table_t *table)
  {
    ...
    m_open_tables.insert(table_cache_t::value_type(table_name, new dict_intrinsic_table_t(table)));
  }
  ...
  uint count_register_table_handler() const { return (static_cast<uint>(m_open_tables.size())); }
