...
UNIV_INLINE
bool rec_n_fields_is_sane(dict_index_t *index, const rec_t *rec, const dtuple_t *entry)
    MY_ATTRIBUTE((warn_unused_result));
...
#else /* UNIV_DEBUG */
#define rec_get_offsets(rec, index, offsets, n, heap) rec_get_offsets_func(rec, index, offsets, n, heap)
#endif /* UNIV_DEBUG */
...
UNIV_INLINE
ulint rec_fold(const rec_t *rec, const ulint *offsets, ulint n_fields, ulint n_bytes, index_id_t tree_id)
    MY_ATTRIBUTE((warn_unused_result));
...
{
  /** Constructor */
  rec_index_print(const rec_t *rec, const dict_index_t *index) : m_rec(rec), m_index(index) {}
...
{
  /** Constructor */
  rec_offsets_print(const rec_t *rec, const ulint *offsets) : m_rec(rec), m_offsets(offsets) {}
...
class rec_printer : public std::ostringstream
{
 public:
  ...
  {
    rec_print(*this, rec, rec_get_info_bits(rec, rec_offs_comp(offsets)), offsets);
  }
  ...
  @param offsets rec_get_offsets(rec, ...) */
  rec_printer(const rec_t *rec, ulint info, const ulint *offsets) : std::ostringstream()
  {
  ...
  rec_printer(const dtuple_t *tuple) : std::ostringstream() { dtuple_print(*this, tuple); }

  /** Construct a pretty-printed tuple.
  @param field	array of data tuple fields
  @param n	number of fields */
  rec_printer(const dfield_t *field, ulint n) : std::ostringstream() { dfield_print(*this, field, n); }
