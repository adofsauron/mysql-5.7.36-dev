...
inline uchar fts_get_word(const CHARSET_INFO *cs, uchar **start, uchar *end, FT_WORD *word,
                          MYSQL_FTPARSER_BOOLEAN_INFO *info)
{
  ...
  while (doc < end)
  {
      ...
      if (!info->quot)
      {
        if (*doc == FTB_LBR || *doc == FTB_RBR || *doc == FTB_LQUOT)
        {
          ...
          *start = doc + 1;
          ...
          info->type = (*doc == FTB_RBR ? FT_TOKEN_RIGHT_PAREN : FT_TOKEN_LEFT_PAREN);
    ...
    mwc = length = 0;
    for (word->pos = doc; doc < end; length++, doc += (mbl > 0 ? mbl : (mbl < 0 ? -mbl : 1)))
    {
      mbl = cs->cset->ctype(cs, &ctype, doc, end);
