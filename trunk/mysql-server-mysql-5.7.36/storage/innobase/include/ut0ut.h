...
#define UT_WAIT_FOR(cond, max_wait_us)                          \
  ...
  {                                                             \
    ...
    {                                                           \
      ...
      if (limit <= 0 || (diff > 0 && ((uint64_t)diff > limit))) \
      {                                                         \
        break;                                                  \
      }                                                         \
...
UNIV_INLINE
void ut_pair_min(ulint *min_hi, ulint *min_lo, ulint a_hi, ulint a_lo, ulint b_hi, ulint b_lo);
/******************************************************/ /**
 Compares two ulints.
...
UNIV_INLINE
int ut_pair_cmp(ulint a_h, ulint a_l, ulint b_h, ulint b_l) MY_ATTRIBUTE((warn_unused_result));
...
class logger
{
 public:
  ...
  std::ostream &write(const char *buf, std::streamsize count) { return (m_oss.write(buf, count)); }
