...
class Datafile
{
 ...
 public:
  ...
  @param[in]	read_only_mode	if true, then readonly mode checks
                                  are enforced.
  @return DB_SUCCESS or error code */
  virtual dberr_t open_read_write(bool read_only_mode) MY_ATTRIBUTE((warn_unused_result));
  ...
  void make_filepath(const char *dirpath, const char *filename, ib_extention ext);
  ...
  dberr_t validate_to_dd(ulint space_id, ulint flags, bool for_import) MY_ATTRIBUTE((warn_unused_result));
  ...
  @retval DB_SUCCESS if tablespace is valid, DB_ERROR if not.
  m_is_valid is also set true on success, else false. */
  dberr_t validate_for_recovery() MY_ATTRIBUTE((warn_unused_result));
  ...
  dberr_t validate_first_page(lsn_t *flush_lsn, bool for_import) MY_ATTRIBUTE((warn_unused_result));
 ...
 private:
  ...
  @param[in]	read_only_mode	if true, then readonly mode checks
                                  are enforced.
  @return DB_SUCCESS or error code */
  dberr_t open_or_create(bool read_only_mode) MY_ATTRIBUTE((warn_unused_result));
  ...
  dberr_t read_first_page(bool read_first_page) MY_ATTRIBUTE((warn_unused_result));
  ...
  void set_open_flags(os_file_create_t open_flags) { m_open_flags = open_flags; };

  /** Determine if this datafile is on a Raw Device
  @return true if it is a RAW device. */
  bool is_raw_device() { return (m_type != SRV_NOT_RAW); }
 ...
 private:
  ...
  dberr_t restore_from_doublewrite(ulint restore_page_no);
...
class RemoteDatafile : public Datafile
{
 ...
 public:
  RemoteDatafile() : m_link_filepath() { /* No op - base constructor is called. */ }

  RemoteDatafile(const char *name, ulint size, ulint order) : m_link_filepath()
  {
  ...
  @param[in]	read_only_mode	If true, then readonly mode checks
                                  are enforced.
  @return DB_SUCCESS or error code */
  dberr_t open_read_write(bool read_only_mode) MY_ATTRIBUTE((warn_unused_result));
  ...
  ******************************************************************/
  ...
  @return DB_SUCCESS or error code */
  static dberr_t create_link_file(const char *name, const char *filepath, bool is_shared = false);
