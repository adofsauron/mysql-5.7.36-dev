...
PSI_memory_key ut_new_get_key_by_file(const char *file);
...
class ut_allocator
{
 public:
  ...
  template <class U>
  ut_allocator(const ut_allocator<U> &other) : m_oom_fatal(other.is_oom_fatal())
  {
#ifdef UNIV_PFS_MEMORY
  ...
  pointer allocate(size_type n_elements, const_pointer hint = NULL, const char *file = NULL, bool set_to_zero = false,
                   bool throw_on_error = true)
  {
    ...
    if (ptr == NULL)
    {
      ib::fatal_or_error(m_oom_fatal) << "Cannot allocate " << total_bytes << " bytes of memory after "
  ...
  pointer reallocate(void *ptr, size_type n_elements, const char *file)
  {
    if (n_elements == 0)
    {
    ...
    if (pfx_new == NULL)
    {
      ib::fatal_or_error(m_oom_fatal) << "Cannot reallocate " << total_bytes << " bytes of memory after "
                                      << alloc_max_retries << " retries over " << alloc_max_retries
                                      << " seconds. OS error: " << strerror(errno) << " (" << errno << "). "
                                      << OUT_OF_MEMORY_MSG;
      /* not reached */
  ...
  pointer new_array(size_type n_elements, const char *file)
  {
  ...
  PSI_memory_key get_mem_key(const char *file) const
  {
    ...
    const size_t len MY_ATTRIBUTE((unused)) = ut_basename_noext(file, keyname, sizeof(keyname));
 ...
 private:
  ...
  void allocate_trace(size_t size, const char *file, ut_new_pfx_t *pfx)
  {
...
#define UT_NEW(expr, key)                                           \
  ...
  ::new (ut_allocator<byte>(key).allocate(sizeof expr, NULL, __FILE__, false, false)) expr
...
@param[in]	key		performance schema memory tracing key
@return pointer to the first allocated object or NULL */
#define UT_NEW_ARRAY(type, n_elements, key) ut_allocator<type>(key).new_array(n_elements, __FILE__)
...
#define ut_malloc(n_bytes, key) \
  static_cast<void *>(ut_allocator<byte>(key).allocate(n_bytes, NULL, __FILE__, false, false))

#define ut_zalloc(n_bytes, key) \
  static_cast<void *>(ut_allocator<byte>(key).allocate(n_bytes, NULL, __FILE__, true, false))

#define ut_malloc_nokey(n_bytes) \
  static_cast<void *>(ut_allocator<byte>(PSI_NOT_INSTRUMENTED).allocate(n_bytes, NULL, __FILE__, false, false))

#define ut_zalloc_nokey(n_bytes) \
  static_cast<void *>(ut_allocator<byte>(PSI_NOT_INSTRUMENTED).allocate(n_bytes, NULL, __FILE__, true, false))

#define ut_zalloc_nokey_nofatal(n_bytes) \
  static_cast<void *>(                   \
      ut_allocator<byte>(PSI_NOT_INSTRUMENTED).set_oom_not_fatal().allocate(n_bytes, NULL, __FILE__, true, false))

#define ut_realloc(ptr, n_bytes) \
  static_cast<void *>(ut_allocator<byte>(PSI_NOT_INSTRUMENTED).reallocate(ptr, n_bytes, __FILE__))

#define ut_free(ptr) ut_allocator<byte>(PSI_NOT_INSTRUMENTED).deallocate(reinterpret_cast<byte *>(ptr))
...
#define UT_NEW_ARRAY(type, n_elements, key) ::new (std::nothrow) type[n_elements]

#define UT_NEW_ARRAY_NOKEY(type, n_elements) ::new (std::nothrow) type[n_elements]
