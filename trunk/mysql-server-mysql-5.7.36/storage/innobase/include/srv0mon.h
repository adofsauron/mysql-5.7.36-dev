...
counter option. */
#define MONITOR_ON(monitor) (monitor_set_tbl[monitor / NUM_BITS_ULINT] |= ((ulint)1 << (monitor % NUM_BITS_ULINT)))

#define MONITOR_OFF(monitor) (monitor_set_tbl[monitor / NUM_BITS_ULINT] &= ~((ulint)1 << (monitor % NUM_BITS_ULINT)))

/** Check whether the requested monitor is turned on/off */
#define MONITOR_IS_ON(monitor) (monitor_set_tbl[monitor / NUM_BITS_ULINT] & ((ulint)1 << (monitor % NUM_BITS_ULINT)))
...
#define MONITOR_MAX_MIN_NOT_INIT(monitor)                                                                         \
  (MONITOR_STATUS(monitor) == MONITOR_INIT_ZERO_VALUE && MONITOR_MIN_VALUE(monitor) == MONITOR_INIT_ZERO_VALUE && \
...
#define MONITOR_ATOMIC_INC(monitor)                                                \
  ...
  {                                                                                \
    ib_uint64_t value;                                                             \
    value = os_atomic_increment_uint64((ib_uint64_t *)&MONITOR_VALUE(monitor), 1); \
...
#define MONITOR_ATOMIC_DEC(monitor)                                                \
  ...
  {                                                                                \
    ib_uint64_t value;                                                             \
    value = os_atomic_decrement_uint64((ib_uint64_t *)&MONITOR_VALUE(monitor), 1); \
    /* Note: This is not 100% accurate because of the                              \
    inherent race, we ignore it due to performance. */                             \
...
#define MONITOR_INC_VALUE_CUMULATIVE(monitor, monitor_n_calls, monitor_per_call, value) \
  ...
  {                                                                                     \
    ...
    if (MONITOR_VALUE(monitor_per_call) > MONITOR_MAX_VALUE(monitor_per_call))          \
    {                                                                                   \
...
#define MONITOR_SAVE_START(monitor, value)                                             \
  ...
  {                                                                                    \
    MONITOR_CHECK_DEFINED(value);                                                      \
    (MONITOR_START_VALUE(monitor) = (mon_type_t)(value)-MONITOR_VALUE_RESET(monitor)); \
  } while (0)
...
#define MONITOR_SET_DIFF(monitor, value)                                                                              \
  MONITOR_SET_UPD_MAX_ONLY(monitor, ((value)-MONITOR_VALUE_RESET(monitor) - MONITOR_FIELD(monitor, mon_start_value) + \
