...
#define DICT_TF_BITS                                                                                       \
  (DICT_TF_WIDTH_COMPACT + DICT_TF_WIDTH_ZIP_SSIZE + DICT_TF_WIDTH_ATOMIC_BLOBS + DICT_TF_WIDTH_DATA_DIR + \
...
#define DICT_MAX_FIELD_LEN_BY_FORMAT(table)                                              \
  ((dict_table_get_format(table) < UNIV_FORMAT_B) ? (REC_ANTELOPE_MAX_INDEX_COL_LEN - 1) \
                                                  : REC_VERSION_56_MAX_INDEX_COL_LEN)

#define DICT_MAX_FIELD_LEN_BY_FORMAT_FLAG(flags)                                            \
  ((DICT_TF_HAS_ATOMIC_BLOBS(flags) < UNIV_FORMAT_B) ? (REC_ANTELOPE_MAX_INDEX_COL_LEN - 1) \
                                                     : REC_VERSION_56_MAX_INDEX_COL_LEN)
...
class last_ops_cur_t
{
 public:
  /** Constructor */
  last_ops_cur_t() : rec(), block(), mtr(), disable_caching(), invalid() { /* Do Nothing. */ }
...
struct dict_foreign_with_index
{
  ...
  bool operator()(const dict_foreign_t *foreign) const { return (foreign->referenced_index == m_index); }
...
{
  bool operator()(const dict_foreign_t *foreign) const { return (foreign->foreign_table != foreign->referenced_table); }
};
...
struct dict_foreign_not_exists
{
  ...
  bool operator()(dict_foreign_t *const &foreign) const { return (m_foreigns.find(foreign) == m_foreigns.end()); }
...
{
  dict_foreign_set_free(const dict_foreign_set &foreign_set) : m_foreign_set(foreign_set) {}

  ~dict_foreign_set_free() { std::for_each(m_foreign_set.begin(), m_foreign_set.end(), dict_foreign_free); }
...
struct dict_foreign_add_to_referenced_table
{
  void operator()(dict_foreign_t *foreign) const
  {
    ...
    {
      std::pair<dict_foreign_set::iterator, bool> ret = table->referenced_set.insert(foreign);
      ut_a(ret.second);
    }
  }
};
...
inline void dict_table_analyze_index_create_lazy(dict_table_t *table)
{
...
inline void dict_table_analyze_index_destroy(dict_table_t *table)
{
  if (table->analyze_index_mutex_created == os_once::DONE && table->analyze_index_mutex != NULL)
  {
...
{
  if (table->autoinc_mutex_created == os_once::DONE && table->autoinc_mutex != NULL)
  {
...
{
  if (index->zip_pad.mutex_created == os_once::DONE && index->zip_pad.mutex != NULL)
  {
...
inline void dict_index_zip_pad_unlock(dict_index_t *index) { mutex_exit(index->zip_pad.mutex); }
...
inline bool dict_table_autoinc_own(const dict_table_t *table) { return (mutex_own(table->autoinc_mutex)); }
#endif /* UNIV_DEBUG */

/** Check whether the col is used in spatial index or regular index.
@param[in]	col	column to check
@return spatial status */
inline spatial_status_t dict_col_get_spatial_status(const dict_col_t *col)
{
