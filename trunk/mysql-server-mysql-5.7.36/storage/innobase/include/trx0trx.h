...
#define trx_start_if_not_started_xa(t, rw) trx_start_if_not_started_xa_low((t), (rw))
#endif /* UNIV_DEBUG */
...
#define assert_trx_is_free(t)                                                                        \
  do                                                                                                 \
  {                                                                                                  \
    ut_ad(trx_state_eq((t), TRX_STATE_NOT_STARTED) || trx_state_eq((t), TRX_STATE_FORCED_ROLLBACK)); \
...
#define assert_trx_nonlocking_or_in_list(t)                                                                           \
  ...
  {                                                                                                                   \
    ...
    {                                                                                                                 \
      ...
      ut_ad(t_state == TRX_STATE_NOT_STARTED || t_state == TRX_STATE_FORCED_ROLLBACK || t_state == TRX_STATE_ACTIVE); \
    }                                                                                                                 \
    else                                                                                                              \
    {                                                                                                                 \
      check_trx_state(t);                                                                                             \
...
{
  return (trx->state != TRX_STATE_NOT_STARTED && trx->state != TRX_STATE_FORCED_ROLLBACK);
}
...
class TrxInInnoDB
{
 public:
  ...
  TrxInInnoDB(trx_t *trx, bool disable = false) : m_trx(trx) { enter(trx, disable); }
  ...
  bool is_rollback_disabled() const { return ((m_trx->in_innodb & TRX_FORCE_ROLLBACK_DISABLE) > 0); }
  ...
  static bool is_async_rollback(const trx_t *trx) { return (trx->killed_by == os_thread_get_curr_id()); }
 ...
 private:
  ...
  static void enter(trx_t *trx, bool disable)
  {
    ...
    {
      trx_mutex_enter(trx);
      if (!is_forced_rollback(trx) && is_started(trx) && !trx_is_autocommit_non_locking(trx))
      {
