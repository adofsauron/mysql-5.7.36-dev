...
class dyn_buf_t
{
 public:
  ...
  class block_t
  {
   public:
    ...
    byte *end() MY_ATTRIBUTE((warn_unused_result)) { return (begin() + m_used); }

    /**
    @return start of data - const version */
    const byte *begin() const MY_ATTRIBUTE((warn_unused_result)) { return (m_data); }

    /**
    @return end of used data - const version */
    const byte *end() const MY_ATTRIBUTE((warn_unused_result)) { return (begin() + m_used); }
   ...
   private:
    ...
    enum
    {
      MAX_DATA_SIZE = SIZE - sizeof(block_node_t) + sizeof(ib_uint32_t)
    };
  ...
  enum
  {
    MAX_DATA_SIZE = block_t::MAX_DATA_SIZE
  };
  ...
  {
    block_t *block = const_cast<block_t *>(const_cast<dyn_buf_t *>(this)->find(pos));
  ...
  ulint size() const MY_ATTRIBUTE((warn_unused_result))
  {
    ...
    for (const block_t *block = UT_LIST_GET_FIRST(m_list); block != NULL; block = UT_LIST_GET_NEXT(m_node, block))
    {
  ...
  {
    for (block_t *block = UT_LIST_GET_LAST(m_list); block != NULL; block = UT_LIST_GET_PREV(m_node, block))
    {
      if (!functor(block))
      {
        return (false);
  ...
  bool is_small() const MY_ATTRIBUTE((warn_unused_result)) { return (m_heap == NULL); }
 ...
 private:
  ...
  bool has_space(ulint size) const { return (back()->m_used + size <= MAX_DATA_SIZE); }

  /*
  @return true if request can be fullfilled */
  bool has_space(ulint size) { return (back()->m_used + size <= MAX_DATA_SIZE); }
  ...
  block_t *find(ulint &pos)
  {
    ...
    for (block = UT_LIST_GET_FIRST(m_list); block != NULL; block = UT_LIST_GET_NEXT(m_node, block))
    {
      if (pos < block->used())
      {
        break;
