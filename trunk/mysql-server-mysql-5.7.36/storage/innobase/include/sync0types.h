...
#define LATCH_ADD_MUTEX(id, level, key) \
  latch_meta[LATCH_ID_##id] = UT_NEW_NOKEY(latch_meta_t(LATCH_ID_##id, #id, level, #level, key))
...
#else
#define LATCH_ADD_RWLOCK(id, level, key) \
  latch_meta[LATCH_ID_##id] = UT_NEW_NOKEY(latch_meta_t(LATCH_ID_##id, #id, level, #level, PSI_NOT_INSTRUMENTED))
#endif /* UNIV_PFS_RWLOCK */

#else
#define LATCH_ADD_MUTEX(id, level, key) \
  latch_meta[LATCH_ID_##id] = UT_NEW_NOKEY(latch_meta_t(LATCH_ID_##id, #id, level, #level))
#define LATCH_ADD_RWLOCK(id, level, key) \
  latch_meta[LATCH_ID_##id] = UT_NEW_NOKEY(latch_meta_t(LATCH_ID_##id, #id, level, #level))
...
class LatchCounter
{
 public:
  ...
  {
    /** Constructor */
    Count() UNIV_NOTHROW : m_spins(), m_waits(), m_calls(), m_enabled() { /* No op */ }
  ...
  void single_deregister(Count *count) UNIV_NOTHROW
  {
    ...
    m_counters.erase(std::remove(m_counters.begin(), m_counters.end(), count), m_counters.end());
...
class LatchMeta
{
 public:
  ...
  @param[in]	key		PFS key */
  LatchMeta(latch_id_t id, const char *name, latch_level_t level, const char *level_name
#ifdef UNIV_PFS_MUTEX
            ,
            pfs_key_t key
#endif /* UNIV_PFS_MUTEX */
  ...
  bool operator<(const LatchMeta &rhs) const { return (get_id() < rhs.get_id()); }
...
struct latch_t
{
  ...
  explicit latch_t(latch_id_t id = LATCH_ID_NONE) UNIV_NOTHROW : m_id(id), m_rw_lock(), m_temp_fsp() {}
...
struct btrsea_sync_check : public sync_check_functor_t
{
  ...
  explicit btrsea_sync_check(bool has_search_latch) : m_result(), m_has_search_latch(has_search_latch) {}
  ...
  virtual bool operator()(const latch_level_t level)
  {
    ...
    if (!m_has_search_latch && (level != SYNC_SEARCH_SYS && level != SYNC_FTS_CACHE && level != SYNC_TRX_I_S_RWLOCK &&
...
struct dict_sync_check : public sync_check_functor_t
{
  ...
  @param[in]	dict_mutex_allow	true if the dict mutex
                                          is allowed */
  explicit dict_sync_check(bool dict_mutex_allowed) : m_result(), m_dict_mutex_allowed(dict_mutex_allowed) {}
  ...
  virtual bool operator()(const latch_level_t level)
  {
    if (!m_dict_mutex_allowed || (level != SYNC_DICT && level != SYNC_DICT_OPERATION &&
...
struct sync_allowed_latches : public sync_check_functor_t
{
  ...
  sync_allowed_latches(const latch_level_t *from, const latch_level_t *to) : m_result(), m_latches(from, to) {}
  ...
  virtual bool operator()(const latch_level_t level)
  {
    for (latches_t::const_iterator it = m_latches.begin(); it != m_latches.end(); ++it)
    {
      if (level == *it)
      {
        m_result = false;
