...
{
  if (page_id.space() < IBUF_COUNT_N_SPACES && page_id.page_no() < IBUF_COUNT_N_PAGES)
  {
    return;
  }
  ...
  ib::fatal() << "UNIV_IBUF_COUNT_DEBUG limits space_id and page_no"
              ...
              << ". page_no=" << page_id.page_no() << ", should be 0<=page_no<" << IBUF_COUNT_N_PAGES;
}
#endif
    ...
    mtr_t *mtr) /*!< in: mtr */
{
  ...
  block = buf_page_get(page_id_t(IBUF_SPACE_ID, FSP_IBUF_TREE_ROOT_PAGE_NO), univ_page_size, RW_SX_LATCH, mtr);
    ...
    const page_t *root) /*!< in: ibuf tree root */
{
  ...
  ibuf->free_list_len = flst_get_len(root + PAGE_HEADER + PAGE_BTR_IBUF_FREE_LIST);
...
/*=======================*/
{
...

  {
    ...
    block = buf_page_get(page_id_t(IBUF_SPACE_ID, FSP_IBUF_TREE_ROOT_PAGE_NO), univ_page_size, RW_X_LATCH, &mtr);
  ...
  ibuf->index =
      dict_mem_index_create("innodb_change_buffer", "CLUST_IND", IBUF_SPACE_ID, DICT_CLUSTERED | DICT_IBUF, 1);
  ibuf->index->id = DICT_IBUF_ID_MIN + IBUF_SPACE_ID;
  ibuf->index->table = dict_mem_table_create("innodb_change_buffer", IBUF_SPACE_ID, 1, 0, 0, 0);
  ibuf->index->n_uniq = REC_MAX_N_FIELDS;
  rw_lock_create(index_tree_rw_lock_key, &ibuf->index->lock, SYNC_IBUF_INDEX_TREE);
...
#define ibuf_bitmap_page_get_bits(page, page_id, page_size, bit, mtr) \
  ibuf_bitmap_page_get_bits_low(page, page_id, page_size, MTR_MEMO_PAGE_X_FIX, mtr, bit)
#else /* UNIV_DEBUG */
/** Gets the desired bits for a given page from a bitmap page.
@param[in]	page		bitmap page
@param[in]	page_id		page id whose bits to get
...
@return value of bits */
UNIV_INLINE
ulint ibuf_bitmap_page_get_bits_low(const page_t *page, const page_id_t &page_id, const page_size_t &page_size,
#ifdef UNIV_DEBUG
...
static void ibuf_bitmap_page_set_bits(page_t *page, const page_id_t &page_id, const page_size_t &page_size, ulint bit,
                                      ulint val, mtr_t *mtr)
{
  ulint byte_offset;
  ulint bit_offset;
...
#ifdef UNIV_IBUF_COUNT_DEBUG
  ut_a((bit != IBUF_BITMAP_BUFFERED) || (val != FALSE) || (0 == ibuf_count_get(page_id)));
#endif
...
UNIV_INLINE
const page_id_t ibuf_bitmap_page_no_calc(const page_id_t &page_id, const page_size_t &page_size)
{
  ulint bitmap_page_no;

  bitmap_page_no = FSP_IBUF_BITMAP_OFFSET + (page_id.page_no() & ~(page_size.physical() - 1));
...
is x-latched */
static page_t *ibuf_bitmap_get_map_page_func(const page_id_t &page_id, const page_size_t &page_size, const char *file,
                                             ulint line, mtr_t *mtr)
{
  buf_block_t *block;

  block = buf_page_get_gen(ibuf_bitmap_page_no_calc(page_id, page_size), page_size, RW_X_LATCH, NULL, BUF_GET, file,
                           line, mtr);
    ...
    mtr_t *mtr)               /*!< in/out: mtr */
{
  ...
  ibuf_bitmap_page_set_bits(bitmap_page, block->page.id, block->page.size, IBUF_BITMAP_FREE, val, mtr);
}
    ...
    ulint val)     /*!< in: value to set: < 4 */
{
  ...
  if (max_val != ULINT_UNDEFINED)
  {
    ...
    old_val = ibuf_bitmap_page_get_bits(bitmap_page, block->page.id, IBUF_BITMAP_FREE, &mtr);
#if 0
		if (old_val != max_val) {
			fprintf(stderr,
    ...
    mtr_t *mtr)               /*!< in/out: mtr */
{
  ...
  before = ibuf_index_page_calc_free_bits(block->page.size.logical(), max_ins_size);
    ...
    mtr_t *mtr)         /*!< in/out: mtr */
{
  ...
  ibuf_bitmap_page_set_bits(bitmap_page, block->page.id, block->page.size, IBUF_BITMAP_FREE, after, mtr);
}
...
UNIV_INLINE
ibool ibuf_fixed_addr_page(const page_id_t &page_id, const page_size_t &page_size)
{
  return ((page_id.space() == IBUF_SPACE_ID && page_id.page_no() == IBUF_TREE_ROOT_PAGE_NO) ||
                    ...
                    const char *file, ulint line, mtr_t *mtr)
{
  ...
  if (!x_latch)
  {
    ...
    bitmap_page = buf_block_get_frame(buf_page_get_gen(ibuf_bitmap_page_no_calc(page_id, page_size), page_size,
                                                       RW_NO_LATCH, NULL, BUF_GET_NO_LATCH, file, line, &local_mtr));

    ret =
        ibuf_bitmap_page_get_bits_low(bitmap_page, page_id, page_size, MTR_MEMO_BUF_FIX, &local_mtr, IBUF_BITMAP_IBUF);
  ...
  bitmap_page = ibuf_bitmap_get_map_page_func(page_id, page_size, file, line, mtr);

  ret = ibuf_bitmap_page_get_bits(bitmap_page, page_id, page_size, IBUF_BITMAP_IBUF, mtr);
...
/********************************************************************/ /**
 ...
 @return page number */
static ulint ibuf_rec_get_page_no_func(
/*======================*/
#ifdef UNIV_DEBUG
...
/********************************************************************/ /**
 ...
 returns 0.
 @return space id */
static ulint ibuf_rec_get_space_func(
/*====================*/
#ifdef UNIV_DEBUG
    mtr_t *mtr,       /*!< in: mini-transaction owning rec */
#endif                /* UNIV_DEBUG */
    ...
    ulint *counter)   /*!< in: counter value, or NULL */
{
  ...
  ut_a((len - info_len_local) == (fields - IBUF_REC_FIELD_USER) * DATA_NEW_ORDER_NULL_TYPE_BUF_SIZE);
    ...
    FILE *file)       /*!< in: file where to print */
{
  ...
  for (i = 0; i < IBUF_OP_COUNT; i++)
  {
    fprintf(file, "%s %lu%s", op_names[i], (ulong)ops[i], (i < (IBUF_OP_COUNT - 1)) ? ", " : "");
  }
    ...
    ibool comp) /*!< in: TRUE=use compact record format */
{
  ...
  table = dict_mem_table_create("IBUF_DUMMY", DICT_HDR_SPACE, n, 0, comp ? DICT_TF_COMPACT : 0, 0);
...
/*====================*/
{
  ...
  block = fseg_alloc_free_page(header_page + IBUF_HEADER + IBUF_TREE_SEG_HEADER, 0, FSP_UP, &mtr);
  ...
  ibuf_bitmap_page_set_bits(bitmap_page, page_id, page_size, IBUF_BITMAP_IBUF, TRUE, &mtr);
...
/*=======================*/
{
  ...
  fseg_free_page(header_page + IBUF_HEADER + IBUF_TREE_SEG_HEADER, IBUF_SPACE_ID, page_no, false, &mtr);
  ...
  ibuf_bitmap_page_set_bits(bitmap_page, page_id, page_size, IBUF_BITMAP_IBUF, FALSE, &mtr);
                     ...
                     page_nos in this function */
{
  ...
  *n_stored = 0;
  ...
  while (*n_stored < limit)
  {
    ...
    {
      if (contract || (prev_page_no == first_page_no && prev_space_id == first_space_id) ||
          (volume_for_page >
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) const rec_t *ibuf_get_user_rec(
    /*===============*/
...
/*********************************************************************/ /**
 ...
 @return a lower limit for the combined volume of records which will be
 merged */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) ulint ibuf_get_merge_pages(
    /*=================*/
    ...
    mtr_t *mtr)       /*!< in: mini transaction */
{
  ...
  *n_pages = 0;
  ...
  while ((rec = ibuf_get_user_rec(pcur, mtr)) != 0 && ibuf_rec_get_space(mtr, rec) == space && *n_pages < limit)
  {
                    ...
                    address to complete */
{
  ...
  *n_pages = 0;
  ...
  available = btr_pcur_open_at_rnd_pos(ibuf->index, BTR_SEARCH_LEAF, &pcur, &mtr);
  ...
  if (page_is_empty(btr_pcur_get_page(&pcur)))
  {
    ...
    ut_ad(page_get_page_no(btr_pcur_get_page(&pcur)) == FSP_IBUF_TREE_ROOT_PAGE_NO);
  ...
  sum_sizes = ibuf_get_merge_page_nos(TRUE, btr_pcur_get_rec(&pcur), &mtr, space_ids, page_nos, n_pages);
    ...
    ulint space) /*!< in: tablespace id to merge */
{
  ...
  {
    sum_sizes = ibuf_get_merge_pages(&pcur, space, IBUF_MAX_N_PAGES_MERGED, &pages[0], &spaces[0], &n_pages, &mtr);
...
will be merged from ibuf trees to the pages read, 0 if ibuf is
empty */
static MY_ATTRIBUTE((warn_unused_result)) ulint ibuf_merge(ulint *n_pages, bool sync)
{
  *n_pages = 0;
    ...
    ulint size)        /*!< in: number of elements in hash array */
{
  ...
  len = ibuf_rec_get_size(rec, types, rec_get_n_fields_old(rec) - IBUF_REC_FIELD_USER, comp);
                      ...
                      on the page that rec points to */
{
  ...
  switch (UNIV_EXPECT(len % DATA_NEW_ORDER_NULL_TYPE_BUF_SIZE, IBUF_REC_INFO_SIZE))
  {
    default:
      ut_error;
    case 0:
      ...
      return (len + rec_get_converted_extra_size(len, n_fields, 0) + page_dir_calc_reserved_space(1));
    case 1:
  ...
  switch (ibuf_op)
  {
    ...
    case IBUF_OP_DELETE_MARK:
      ...
      if (n_recs && ibuf_get_volume_buffered_hash(rec, types + IBUF_REC_INFO_SIZE, types + len,
    ...
    mtr_t *mtr)             /*!< in: mini-transaction of pcur */
{
  ...
  for (; !page_rec_is_infimum(rec); rec = page_rec_get_prev_const(rec))
  {
    ...
    if (page_no != ibuf_rec_get_page_no(mtr, rec) || space != ibuf_rec_get_space(mtr, rec))
    {
      goto count_later;
    }

    volume += ibuf_get_volume_buffered_count(mtr, rec, hash_bitmap, UT_ARR_SIZE(hash_bitmap), n_recs);
  }
...

  {
    ...
    block = buf_page_get(page_id_t(IBUF_SPACE_ID, prev_page_no), univ_page_size, RW_X_LATCH, mtr);
...
count_later:
...

  {
    ...
    block = buf_page_get(page_id_t(IBUF_SPACE_ID, next_page_no), univ_page_size, RW_X_LATCH, mtr);
...
/*===============================*/
{
  ...
  btr_pcur_open_at_index_side(false, ibuf->index, BTR_SEARCH_LEAF, &pcur, true, 0, &mtr);
...
#ifdef UNIV_DEBUG
#define ibuf_get_entry_counter_low(mtr, rec, space, page_no) ibuf_get_entry_counter_low_func(mtr, rec, space, page_no)
#else /* UNIV_DEBUG */
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    ibuf_insert_low(ulint mode, ibuf_op_t op, ibool no_counter, const dtuple_t *entry, ulint entry_size,
                    dict_index_t *index, const page_id_t &page_id, const page_size_t &page_size, que_thr_t *thr)
{
  ...
  if (buf_page_peek(page_id) || lock_rec_expl_exist_on_page(page_id.space(), page_id.page_no()))
  {
  ...
  if (!no_counter)
  {
    ...
    ulint counter = ibuf_get_entry_counter(page_id.space(), page_id.page_no(), btr_pcur_get_rec(&pcur), &mtr,
    ...
    mach_write_to_2((byte *)dfield_get_data(field) + IBUF_REC_OFFSET_COUNTER, counter);
  }

  /* Set the bitmap bit denoting that the insert buffer contains
  buffered entries for this index page, if the bit is not set yet */

  old_bit_value = ibuf_bitmap_page_get_bits(bitmap_page, page_id, page_size, IBUF_BITMAP_BUFFERED, &bitmap_mtr);
  ...
  {
    err = btr_cur_optimistic_insert(BTR_NO_LOCKING_FLAG, cursor, &offsets, &offsets_heap, ibuf_entry, &ins_rec,
  ...
  else
  {
    ...
    err = btr_cur_optimistic_insert(BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG, cursor, &offsets, &offsets_heap,
                                    ibuf_entry, &ins_rec, &dummy_big_rec, 0, thr, &mtr);

    if (err == DB_FAIL)
    {
      err = btr_cur_pessimistic_insert(BTR_NO_LOCKING_FLAG | BTR_NO_UNDO_LOG_FLAG, cursor, &offsets, &offsets_heap,
                                       ibuf_entry, &ins_rec, &dummy_big_rec, 0, thr, &mtr);
    }
...
func_exit:
  ...
  if (err == DB_SUCCESS)
  {
    ib::info() << "Incrementing ibuf count of page " << page_id << " from " << ibuf_count_get(space, page_no)
               << " by 1";
  ...
  if (err == DB_SUCCESS && BTR_LATCH_MODE_WITHOUT_INTENTION(mode) == BTR_MODIFY_TREE)
  {
...
ibool ibuf_insert(ibuf_op_t op, const dtuple_t *entry, dict_index_t *index, const page_id_t &page_id,
                  const page_size_t &page_size, que_thr_t *thr)
{
  ...
  DBUG_PRINT("ibuf", ("op: %d, space: " UINT32PF ", page_no: " UINT32PF, op, page_id.space(), page_id.page_no()));
...
skip_watch:
  ...
  if (entry_size >= page_get_free_space_of_empty(dict_table_is_comp(index->table)) / 2)
  {
    DBUG_RETURN(FALSE);
  }

  err = ibuf_insert_low(BTR_MODIFY_PREV, op, no_counter, entry, entry_size, index, page_id, page_size, thr);
  if (err == DB_FAIL)
  {
    err = ibuf_insert_low(BTR_MODIFY_TREE | BTR_LATCH_FOR_INSERT, op, no_counter, entry, entry_size, index, page_id,
                          page_size, thr);
  }
...
static MY_ATTRIBUTE((nonnull)) rec_t *ibuf_insert_to_index_page_low(
    /*==========================*/
                          ...
                          after which to insert the buffered entry */
{
  ...
  ib::error() << "Insert buffer insert fails; page free " << page_get_max_insert_size(page, 1) << ", dtuple size "
  ...
  bitmap_page = ibuf_bitmap_get_map_page(block->page.id, block->page.size, mtr);
  old_bits = ibuf_bitmap_page_get_bits(bitmap_page, block->page.id, block->page.size, IBUF_BITMAP_FREE, mtr);

  ib::error() << "page " << block->page.id << ", size " << block->page.size.physical() << ", bitmap bits " << old_bits;
    ...
    mtr_t *mtr)            /*!< in: mtr */
{
  ...
  DBUG_PRINT("ibuf", ("page " UINT32PF ":" UINT32PF, block->page.id.space(), block->page.id.page_no()));
  ...
  if (UNIV_UNLIKELY(dict_table_is_comp(index->table) != (ibool) !!page_is_comp(page)))
  {
  ...
  heap = mem_heap_create(sizeof(upd_t) + REC_OFFS_HEADER_SIZE * sizeof(*offsets) +
  ...
  if (UNIV_UNLIKELY(low_match == dtuple_get_n_fields(entry)))
  {
    ...
    update = row_upd_build_sec_rec_difference_binary(rec, index, offsets, entry, heap);
    ...
    if (!row_upd_changes_field_size_or_external(index, offsets, update) &&
        (!page_zip ||
         btr_cur_update_alloc_zip(page_zip, &page_cur, index, offsets, rec_offs_size(offsets), false, mtr)))
    {
      ...
      btr_cur_update_in_place_log(BTR_KEEP_SYS_FLAG, rec, index, update, 0, 0, mtr);

      DBUG_EXECUTE_IF("crash_after_log_ibuf_upd_inplace", log_buffer_flush_to_disk();
    ...
    */
    ...
    rec = ibuf_insert_to_index_page_low(entry, block, index, &offsets, heap, mtr, &page_cur);
  ...
  {
    offsets = NULL;
    ibuf_insert_to_index_page_low(entry, block, index, &offsets, heap, mtr, &page_cur);
  }
updated_in_place:
  mem_heap_free(heap);
    ...
    mtr_t *mtr)                /*!< in: mtr */
{
  ...
  if (low_match == dtuple_get_n_fields(entry))
  {
    ...
    if (UNIV_LIKELY(!rec_get_deleted_flag(rec, dict_table_is_comp(index->table))))
    {
  ...
  else
  {
    ...
    ib::error() << "page " << block->page.id << " (" << page_get_n_recs(page) << " records, index id "
                           ...
                           before latching any further pages */
{
  ...
  if (low_match == dtuple_get_n_fields(entry))
  {
    ...
    if (page_get_n_recs(page) <= 1 || !(REC_INFO_DELETED_FLAG & rec_get_info_bits(rec, page_is_comp(page))))
    {
      ...
      fprintf(stderr,
              ...
              block->page.id.space(), block->page.id.page_no(), (unsigned)page_get_n_recs(page),
...
static MY_ATTRIBUTE((nonnull)) ibool ibuf_restore_pos(
    /*=============*/
    ulint space,   /*!< in: space id */
    ...
    mtr_t *mtr)       /*!< in/out: mini-transaction */
{
  ut_ad(mode == BTR_MODIFY_LEAF || BTR_LATCH_MODE_WITHOUT_INTENTION(mode) == BTR_MODIFY_TREE);
  ...
  if (fil_space_get_flags(space) == ULINT_UNDEFINED || fil_space_is_being_truncated(space))
  {
    ...
    mtr_t *mtr) /*!< in: mtr */
{
...
#ifdef UNIV_IBUF_COUNT_DEBUG
    ib::info() << "Decrementing ibuf count of space " << space << " page " << page_no << " from "
  ...
  if (!ibuf_restore_pos(space, page_no, search_tuple, BTR_MODIFY_TREE | BTR_LATCH_FOR_DELETE, pcur, mtr))
  {
    mutex_exit(&ibuf_mutex);
  ...
  btr_cur_pessimistic_delete(&err, TRUE, btr_pcur_get_btr_cur(pcur), 0, false, mtr);
  ut_a(err == DB_SUCCESS);
...
void ibuf_merge_or_delete_for_page(buf_block_t *block, const page_id_t &page_id, const page_size_t *page_size,
                                   ibool update_ibuf_bitmap)
{
  ...
  if (srv_force_recovery >= SRV_FORCE_NO_IBUF_MERGE || trx_sys_hdr_page(page_id) ||
  ...
  if (ibuf_fixed_addr_page(page_id, univ_page_size) || fsp_descr_page(page_id, univ_page_size))
  {
    return;
  }
  ...
  if (update_ibuf_bitmap)
  {
    ...
    if (ibuf_fixed_addr_page(page_id, *page_size) || fsp_descr_page(page_id, *page_size))
    {
      return;
    }
    ...
    else
    {
      ...
      bitmap_bits = ibuf_bitmap_page_get_bits(bitmap_page, page_id, *page_size, IBUF_BITMAP_BUFFERED, &mtr);
  ...
  else if (block != NULL && (ibuf_fixed_addr_page(page_id, *page_size) || fsp_descr_page(page_id, *page_size)))
  {
    return;
  }
  ...
  if (block != NULL)
  {
    ...
    if (!fil_page_index_page_check(block->frame) || !page_is_leaf(block->frame))
    {
      corruption_noticed = true;

      ib::error() << "Corruption in the tablespace. Bitmap"
                     " shows insert buffer records to page "
                  << page_id << " though the page type is " << fil_page_get_type(block->frame)
...
loop:
  ...
  for (;;)
  {
    ...
    if (ibuf_rec_get_page_no(&mtr, rec) != page_id.page_no() || ibuf_rec_get_space(&mtr, rec) != page_id.space())
    {
      if (block != NULL)
      {
    ...
    else if (block != NULL && !rec_get_deleted_flag(rec, 0))
    {
      ...
      switch (op)
      {
        ...
        case IBUF_OP_DELETE:
          ...
          if (!ibuf_restore_pos(page_id.space(), page_id.page_no(), search_tuple, BTR_MODIFY_LEAF, &pcur, &mtr))
          {
            ut_ad(mtr.has_committed());
            mops[op]++;
            ibuf_dummy_index_free(dummy_index);
    ...
    if (ibuf_delete_rec(page_id.space(), page_id.page_no(), &pcur, search_tuple, &mtr))
    {
...
reset_bit:
  if (update_ibuf_bitmap)
  {
    ...
    ibuf_bitmap_page_set_bits(bitmap_page, page_id, *page_size, IBUF_BITMAP_BUFFERED, FALSE, &mtr);

    if (block != NULL)
    {
      ulint old_bits = ibuf_bitmap_page_get_bits(bitmap_page, page_id, *page_size, IBUF_BITMAP_FREE, &mtr);

      ulint new_bits = ibuf_index_page_calc_free(block);

      if (old_bits != new_bits)
      {
        ibuf_bitmap_page_set_bits(bitmap_page, page_id, *page_size, IBUF_BITMAP_FREE, new_bits, &mtr);
      }
    }
  }
    ...
    FILE *file) /*!< in: file where to print */
{
  ...
  fprintf(file,
          ...
          (ulong)ibuf->size, (ulong)ibuf->free_list_len, (ulong)ibuf->seg_size, (ulong)ibuf->n_merges);
    ...
    ulint space_id)   /*!< in: tablespace identifier */
{
  ...
  for (page_no = 0; page_no < size; page_no += page_size.physical())
  {
    ...
    bitmap_page = ibuf_bitmap_get_map_page(page_id_t(space_id, page_no), page_size, &mtr);
    ...
    if (buf_page_is_zeroes(bitmap_page, page_size))
    {
      ...
      all-zero pages. */
#ifdef UNIV_DEBUG
      for (ulint curr_page = page_no + 1; curr_page < page_size.physical(); curr_page++)
      {
        buf_block_t *block = buf_page_get(page_id_t(space_id, curr_page), page_size, RW_S_LATCH, &mtr);
        page_t *page = buf_block_get_frame(block);
    ...
    for (i = FSP_IBUF_BITMAP_OFFSET + 1; i < page_size.physical(); i++)
    {
      const ulint offset = page_no + i;

      const page_id_t cur_page_id(space_id, offset);

      if (ibuf_bitmap_page_get_bits(bitmap_page, cur_page_id, page_size, IBUF_BITMAP_IBUF, &mtr))
      {
        mutex_exit(&ibuf_mutex);
        ibuf_exit(&mtr);
        mtr_commit(&mtr);
      ...
      if (ibuf_bitmap_page_get_bits(bitmap_page, cur_page_id, page_size, IBUF_BITMAP_BUFFERED, &mtr))
      {
        ...
        ibuf_bitmap_page_set_bits(bitmap_page, cur_page_id, page_size, IBUF_BITMAP_BUFFERED, FALSE, &mtr);
      }
    }
...
void ibuf_set_bitmap_for_bulk_load(buf_block_t *block, bool reset)
{
  ...
  ibuf_bitmap_page_set_bits(bitmap_page, block->page.id, block->page.size, IBUF_BITMAP_FREE, free_val, &mtr);

  ibuf_bitmap_page_set_bits(bitmap_page, block->page.id, block->page.size, IBUF_BITMAP_BUFFERED, FALSE, &mtr);
