    ...
    ulint flags2)     /*!< in: table flags2 */
{
  ...
  table->cols = static_cast<dict_col_t *>(mem_heap_alloc(heap, table->n_cols * sizeof(dict_col_t)));
  table->v_cols = static_cast<dict_v_col_t *>(mem_heap_alloc(heap, n_v_cols * sizeof(*table->v_cols)));
...
#ifndef UNIV_HOTBACKUP
  table->autoinc_lock = static_cast<ib_lock_t *>(mem_heap_alloc(heap, lock_get_size()));
...
static const char *dict_add_col_name(
    /*==============*/
...
dict_v_col_t *dict_mem_table_add_v_col(dict_table_t *table, mem_heap_t *heap, const char *name, ulint mtype,
                                       ulint prtype, ulint len, ulint pos, ulint num_base)
{
  dict_v_col_t *v_col;
  ulint i;
  ...
  if (num_base != 0)
  {
    v_col->base_col = static_cast<dict_col_t **>(mem_heap_zalloc(table->heap, num_base * sizeof(*v_col->base_col)));
  }
  else
  {
    v_col->base_col = NULL;
...
void dict_mem_table_add_s_col(dict_table_t *table, ulint num_base)
{
  ...
  if (num_base != 0)
  {
    s_col.base_col = static_cast<dict_col_t **>(mem_heap_zalloc(table->heap, num_base * sizeof(dict_col_t *)));
  }
  else
  {
    s_col.base_col = NULL;
...
static MY_ATTRIBUTE((nonnull)) void dict_mem_table_col_rename_low(
    /*==========================*/
...
{
  char *t_col_names = const_cast<char *>(is_virtual ? table->v_col_names : table->col_names);
  ...
  else
  {
    ...
    if (to_len > from_len)
    {
      ...
      col_names = static_cast<char *>(mem_heap_alloc(table->heap, full_len + to_len - from_len));
      ...
      {
        dict_sys->size += table_size_after_rename_col - table_size_before_rename_col;
      }
    ...
    memmove(col_names + prefix_len + to_len, t_col_names + (prefix_len + from_len), full_len - (prefix_len + from_len));
         ...
         index = dict_table_get_next_index(index))
    {
      ...
      for (ulint i = 0; i < n_fields; i++)
      {
        ...
        not match, continue */
        if ((!is_virtual) != (!dict_col_is_virtual(field->col)))
        {
          continue;
        }
  ...
  for (dict_foreign_set::iterator it = table->foreign_set.begin(); it != table->foreign_set.end(); ++it)
  {
    ...
    for (unsigned f = 0; f < foreign->n_fields; f++)
    {
      ...
      foreign->foreign_col_names[f] = dict_index_get_nth_field(foreign->foreign_index, f)->name;
    }
  }
  ...
  for (dict_foreign_set::iterator it = table->referenced_set.begin(); it != table->referenced_set.end(); ++it)
  {
    ...
    for (unsigned f = 0; f < foreign->n_fields; f++)
    {
      ...
      const char *col_name = dict_index_get_nth_field(foreign->referenced_index, f)->name;
      ...
      if (strcmp(foreign->referenced_col_names[f], col_name))
      {
        ...
        {
          *rc = static_cast<char *>(mem_heap_dup(foreign->heap, col_name, col_name_len_1));
        }
      }
    }
  }
...
/*!< in: if this is a virtual column */
{
  ...
  ut_ad((!is_virtual && nth_col < table->n_def) || (is_virtual && nth_col < table->n_v_def));
  ...
  dict_mem_table_col_rename_low(table, static_cast<unsigned>(nth_col), to, s, is_virtual);
}
    ...
    ulint n_fields)         /*!< in: number of fields */
{
  ...
  dict_mem_fill_index_struct(index, heap, table_name, index_name, space, type, n_fields);
  ...
  if (type & DICT_SPATIAL)
  {
    ...
    index->rtr_track = static_cast<rtr_info_track_t *>(mem_heap_alloc(heap, sizeof(*index->rtr_track)));
    mutex_create(LATCH_ID_RTR_ACTIVE_MUTEX, &index->rtr_track->rtr_active_mutex);
    ...
    ibool do_alloc)          /*!< in: is an alloc needed */
{
  if (innobase_get_lower_case_table_names() == 2)
  {
    if (do_alloc)
    {
      ...
      foreign->foreign_table_name_lookup = static_cast<char *>(mem_heap_alloc(foreign->heap, len));
    }
    ...
    ibool do_alloc)          /*!< in: is an alloc needed */
{
  if (innobase_get_lower_case_table_names() == 2)
  {
    if (do_alloc)
    {
      ...
      foreign->referenced_table_name_lookup = static_cast<char *>(mem_heap_alloc(foreign->heap, len));
    }
    strcpy(foreign->referenced_table_name_lookup, foreign->referenced_table_name);
...
static void dict_mem_fill_vcol_has_index(const dict_index_t *index, dict_vcol_set **v_cols)
{
  for (ulint i = 0; i < index->table->n_v_cols; i++)
  {
    ...
    for (it = v_col->v_indexes->begin(); it != v_col->v_indexes->end(); ++it)
    {
...
void dict_mem_foreign_fill_vcol_set(dict_foreign_t *foreign)
{
  ...
  for (ulint i = 0; i < foreign->n_fields; i++)
  {
    ...
    of virtual columns. */
    dict_mem_fill_vcol_set_for_base_col(foreign->foreign_col_names[i], foreign->foreign_table, &foreign->v_cols);

    /** FK can be present on the columns
    which can be a part of virtual index. */
    dict_mem_fill_vcol_from_v_indexes(foreign->foreign_col_names[i], foreign->foreign_table, &foreign->v_cols);
  }
}
...
char *dict_mem_create_temporary_tablename(mem_heap_t *heap, const char *dbtab, table_id_t id)
{
  ...
  ut_snprintf(name + dblen, size - dblen, TEMP_FILE_PREFIX_INNODB UINT64PF "-" UINT32PF, id, dict_temp_file_num);
...
void dict_mem_init(void)
{
  ...
  DBUG_PRINT("dict_mem_init", ("Starting Temporary file number is " UINT32PF, dict_temp_file_num));
}
...
bool dict_foreign_set_validate(const dict_foreign_set &fk_set)
{
  ...
  dict_foreign_set::iterator it = std::find_if(fk_set.begin(), fk_set.end(), not_exists);
...
{
  return (dict_foreign_set_validate(table.foreign_set) && dict_foreign_set_validate(table.referenced_set));
}
