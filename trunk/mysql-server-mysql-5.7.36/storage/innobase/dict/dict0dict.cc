...
static ibool dict_index_find_cols(const dict_table_t *table, dict_index_t *index, const dict_add_v_col_t *add_v);
/*******************************************************************/ /**
...
static void dict_table_stats_latch_alloc(void *table_void)
{
  ...
  table->stats_latch = static_cast<rw_lock_t *>(ut_malloc_nokey(sizeof(rw_lock_t)));
...
{
  if (table->stats_latch_created == os_once::DONE && table->stats_latch != NULL)
  {
...
void dict_table_stats_lock(dict_table_t *table, ulint latch_mode)
{
  ...
  os_once::do_or_wait_for_done(&table->stats_latch_created, dict_table_stats_latch_alloc, table);
    ...
    ulint ref_count)     /*!< in: expected table->n_ref_count */
{
  ...
  if (table == NULL)
  {
    table = dict_table_open_on_id_low(table_id, DICT_ERR_IGNORE_NONE);
  }
  else
  {
...
static void dict_table_try_drop_aborted_and_mutex_exit(
    ...
    ibool try_drop)      /*!< in: FALSE if should try to
                         ...
                         was aborted */
{
  if (try_drop && table != NULL && table->drop_aborted && table->get_ref_count() == 1 &&
                         ...
                         index creation */
{
  ...
  drop_aborted = try_drop && table->drop_aborted && table->get_ref_count() == 1 && dict_table_get_first_index(table);
  table->release();
  ...
  if (strchr(table->name.m_name, '/') != NULL && table->get_ref_count() == 0 && dict_stats_is_persistent_enabled(table))
  {
...
ulint dict_table_has_column(const dict_table_t *table, const char *col_name, ulint col_nr)
{
  ...
  if (col_nr < col_max && innobase_strcasecmp(col_name, dict_table_get_col_name(table, col_nr)) == 0)
  {
    return (col_nr);
  }
  ...
  {
    if (i != col_nr && innobase_strcasecmp(col_name, dict_table_get_col_name(table, i)) == 0)
    {
      return (i);
    }
  }
...
@return column name. */
static const char *dict_table_get_v_col_name_mysql(const dict_table_t *table, ulint col_nr)
{
...
void dict_table_autoinc_lock(
    ...
    dict_table_t *table) /*!< in/out: table */
{
  os_once::do_or_wait_for_done(&table->autoinc_mutex_created, dict_table_autoinc_alloc, table);
...
void dict_index_zip_pad_lock(dict_index_t *index)
{
  os_once::do_or_wait_for_done(&index->zip_pad.mutex_created, dict_index_zip_pad_alloc, index);
...
ulint dict_table_get_all_fts_indexes(const dict_table_t *table, ib_vector_t *indexes)
{
  dict_index_t *index;
...
void dict_table_autoinc_store(const dict_table_t *table)
{
  ut_ad(mutex_own(&dict_sys->mutex));

  if (table->autoinc != 0)
  {
    ut_ad(dict_sys->autoinc_map->find(table->id) == dict_sys->autoinc_map->end());

    dict_sys->autoinc_map->insert(std::pair<table_id_t, ib_uint64_t>(table->id, table->autoinc));
  }
}

/** Restore autoinc value when the table is loaded.
@param[in]	table	table loaded */
void dict_table_autoinc_restore(dict_table_t *table)
{
  ut_ad(mutex_own(&dict_sys->mutex));
...
static void dict_table_analyze_index_alloc(void *table_void)
{
  dict_table_t *table = static_cast<dict_table_t *>(table_void);
...
void dict_table_analyze_index_lock(dict_table_t *table)
{
  os_once::do_or_wait_for_done(&table->analyze_index_mutex_created, dict_table_analyze_index_alloc, table);
...
void dict_table_analyze_index_unlock(dict_table_t *table)
{
  ut_a(table->analyze_index_mutex != NULL);
...
ulint dict_index_get_nth_col_or_prefix_pos(const dict_index_t *index, ulint n, bool inc_prefix, bool is_virtual)
{
...
dict_table_t *dict_table_open_on_id(
    /*==================*/
    ...
    dict_table_op_t table_op) /*!< in: operation to perform */
{
  ...
  table = dict_table_open_on_id_low(
      table_id, table_op == DICT_TABLE_OP_LOAD_TABLESPACE ? DICT_ERR_IGNORE_RECOVER_LOCK : DICT_ERR_IGNORE_NONE);
  ...
  if (!dict_locked)
  {
    dict_table_try_drop_aborted_and_mutex_exit(table, table_op == DICT_TABLE_OP_DROP_ORPHAN);
  }
...
/*===========*/
{
  dict_operation_lock = static_cast<rw_lock_t *>(ut_zalloc_nokey(sizeof(*dict_operation_lock)));
                                  ...
                                  loading a table definition */
{
  ...
  if (table != NULL)
  {
    ...
    if (ignore_err == DICT_ERR_IGNORE_NONE && table->corrupted)
    {
...
{
  ut_ad(table);
  ut_ad(table->n_def == (table->n_cols - dict_table_get_n_sys_cols(table)));
  ut_ad(table->magic_n == DICT_TABLE_MAGIC_N);
  ut_ad(!table->cached);
    ...
    mem_heap_t *heap)     /*!< in: temporary heap */
{
  ...
  {
    dict_table_t *table2;
    HASH_SEARCH(name_hash, dict_sys->table_hash, fold, dict_table_t *, table2, ut_ad(table2->cached),
  ...
  {
    dict_table_t *table2;
    HASH_SEARCH(id_hash, dict_sys->table_id_hash, id_fold, dict_table_t *, table2, ut_ad(table2->cached),
                table2->id == table->id);
    ut_a(table2 == NULL);
  ...
  dict_sys->size += mem_heap_get_size(table->heap) + strlen(table->name.m_name) + 1;
}
    ...
    ulint pct_check)  /*!< in: max percent to check */
{
  ...
  for (table = UT_LIST_GET_LAST(dict_sys->table_LRU);
       table != NULL && i > check_up_to && (len - n_evicted) > max_tables; --i)
  {
...
dict_index_t *dict_table_find_index_on_id(const dict_table_t *table, index_id_t id)
{
  dict_index_t *index;
    ...
    index_id_t id) /*!< in: index id */
{
  ...
  for (table = UT_LIST_GET_FIRST(dict_sys->table_LRU); table != NULL; table = UT_LIST_GET_NEXT(table_LRU, table))
  {
                           ...
                           in constraints which reference it */
{
  ...
  {
    ib::fatal() << "Too long table name: " << table->name << ", max length is " << MAX_FULL_NAME_LEN;
  }
  ...
  DBUG_EXECUTE_IF(
      "dict_table_rename_in_cache_failure", if (table2 == NULL) { table2 = (dict_table_t *)-1; });
  if (table2)
  {
  ...
  else if (dict_table_is_file_per_table(table))
  {
    ...
    {
      ib::error() << "Trying to rename a TEMPORARY TABLE " << old_name << " ( " << table->dir_path_of_temp_table
                  << " )";
      return (DB_ERROR);
    }
    ...
    if (DICT_TF_HAS_DATA_DIR(table->flags))
    {
      ...
      dberr_t err = RemoteDatafile::create_link_file(new_name, new_path);
  ...
  if (strlen(new_name) > strlen(table->name.m_name))
  {
    ...
    table->name.m_name = static_cast<char *>(ut_realloc(table->name.m_name, MAX_FULL_NAME_LEN + 1));
  }
  ...
  if (!rename_also_foreigns)
  {
    ...
    std::for_each(table->foreign_set.begin(), table->foreign_set.end(), dict_foreign_remove_partial());
    table->foreign_set.clear();

    /* Reset table field in referencing constraints */
    for (dict_foreign_set::iterator it = table->referenced_set.begin(); it != table->referenced_set.end(); ++it)
    {
      foreign = *it;
  ...
  for (;;)
  {
    ...
    if (ut_strlen(foreign->foreign_table_name) < ut_strlen(table->name.m_name))
    {
    ...
    if (strchr(foreign->id, '/'))
    {
      ...
      {
        innobase_convert_to_system_charset(strchr(old_name_cs_filename, '/') + 1, strchr(old_name, '/') + 1,
        ...
        if (errors)
        {
          ...
          actually in UTF-8. */
          innobase_convert_to_filename_charset(strchr(old_name_cs_filename, '/') + 1, strchr(old_name, '/') + 1,
                                               MAX_TABLE_NAME_LEN);
        }
        else
        {
      ...
      {
        innobase_convert_to_filename_charset(strchr(fkid, '/') + 1, strchr(foreign->id, '/') + 1,
      ...
      if (ut_strlen(fkid) > ut_strlen(old_name_cs_filename) + ((sizeof dict_ibfk) - 1) &&
          !memcmp(fkid, old_name_cs_filename, ut_strlen(old_name_cs_filename)) &&
          !memcmp(fkid + ut_strlen(old_name_cs_filename), dict_ibfk, (sizeof dict_ibfk) - 1))
      {
        ...
        {
          foreign->id =
              static_cast<char *>(mem_heap_alloc(foreign->heap, strlen(table->name.m_name) + strlen(old_id) + 1));
        }

        /* Convert the table name to UTF-8 */
        strncpy(table_name, table->name.m_name, MAX_TABLE_NAME_LEN);
        innobase_convert_to_system_charset(strchr(table_name, '/') + 1, strchr(table->name.m_name, '/') + 1,
                                           MAX_TABLE_NAME_LEN, &errors);
        ...
        {
          sprintf(strchr(foreign->id, '/') + 1, "%s%s", strchr(table_name, '/') + 1, strstr(old_id, "_ibfk_"));
        }
      }
      else
      {
        ...
        {
          foreign->id = static_cast<char *>(mem_heap_alloc(foreign->heap, db_len + strlen(old_id) + 1));
        }
  ...
  for (dict_foreign_set::iterator it = table->referenced_set.begin(); it != table->referenced_set.end(); ++it)
  {
    ...
    if (ut_strlen(foreign->referenced_table_name) < ut_strlen(table->name.m_name))
    {
      /* Allocate a longer name buffer;
      TODO: store buf len to save memory */

      foreign->referenced_table_name = mem_heap_strdup(foreign->heap, table->name.m_name);
    ...
    table_id_t new_id)   /*!< in: new id to set */
{
  ...
  HASH_DELETE(dict_table_t, id_hash, dict_sys->table_id_hash, ut_fold_ull(table->id), table);
  table->id = new_id;

  /* Add the table back to the hash table */
  HASH_INSERT(dict_table_t, id_hash, dict_sys->table_id_hash, ut_fold_ull(table->id), table);
}

/**********************************************************************/ /**
 Removes a table object from the dictionary cache. */
void dict_table_remove_from_cache_low(
    /*=============================*/
                         ...
                         to make room in the table LRU list */
{
  ...
  for (index = UT_LIST_GET_LAST(table->indexes); index != NULL; index = UT_LIST_GET_LAST(table->indexes))
  {
  ...
  HASH_DELETE(dict_table_t, name_hash, dict_sys->table_hash, ut_fold_string(table->name.m_name), table);
    ...
    const dict_index_t *index) /*!< in: index */
{
  ...
  rec_max_size = comp ? REC_NODE_PTR_SIZE + 1 + REC_N_NEW_EXTRA_BYTES : REC_NODE_PTR_SIZE + 2 + REC_N_OLD_EXTRA_BYTES;
  ...
  for (i = 0; i < dict_index_get_n_unique_in_tree(index); i++)
  {
    ...
    {
      ulint page_rec_max =
          srv_page_size == UNIV_PAGE_SIZE_MAX ? REC_MAX_DATA_SIZE - 1 : page_get_free_space_of_empty(comp) / 2;
...
static ibool dict_index_too_big_for_tree(
    /*========================*/
    const dict_table_t *table,     /*!< in: table */
                                   ...
                                   fit in an B-tree page */
{
  ...
  if (page_size.is_compressed() && page_size.physical() < univ_page_size.physical())
  {
    ...
    page_rec_max = page_zip_empty_size(new_index->n_fields, page_size.physical());
    if (page_rec_max)
    {
      page_rec_max--;
    }
  ...
  else
  {
    ...
    few user records. */
    page_rec_max = srv_page_size == UNIV_PAGE_SIZE_MAX ? REC_MAX_DATA_SIZE - 1 : page_get_free_space_of_empty(comp) / 2;
  ...
  for (i = 0; i < new_index->n_fields; i++)
  {
    ...
    else if (field_max_size > BTR_EXTERN_LOCAL_STORED_MAX_SIZE && dict_index_is_clust(new_index))
    {
  ...
  add_field_size:
    ...
    {
      ib::error_or_warn(strict) << "Cannot add field " << field->name << " in table " << table->name
...
dberr_t dict_index_add_to_cache(dict_table_t *table, dict_index_t *index, ulint page_no, ibool strict)
{
...
@param[in]	strict	TRUE=refuse to create the index
                        ...
                        an B-tree page
@return DB_SUCCESS, DB_TOO_BIG_RECORD, or DB_CORRUPTION */
dberr_t dict_index_add_to_cache_w_vcol(dict_table_t *table, dict_index_t *index, const dict_add_v_col_t *add_v,
                                       ulint page_no, ibool strict)
{
  ...
  for (i = 0; i < n_ord; i++)
  {
    ...
    else if (field->col->max_prefix != 0 && field->prefix_len > field->col->max_prefix)
    {
  ...
  new_index->stat_n_diff_key_vals = static_cast<ib_uint64_t *>(
      mem_heap_zalloc(new_index->heap, dict_index_get_n_unique(new_index) * sizeof(*new_index->stat_n_diff_key_vals)));

  new_index->stat_n_sample_sizes = static_cast<ib_uint64_t *>(
      mem_heap_zalloc(new_index->heap, dict_index_get_n_unique(new_index) * sizeof(*new_index->stat_n_sample_sizes)));

  new_index->stat_n_non_null_key_vals = static_cast<ib_uint64_t *>(mem_heap_zalloc(
      new_index->heap, dict_index_get_n_unique(new_index) * sizeof(*new_index->stat_n_non_null_key_vals)));
...
static ibool dict_index_find_cols(const dict_table_t *table, dict_index_t *index, const dict_add_v_col_t *add_v)
{
  ...
  for (ulint i = 0; i < index->n_fields; i++)
  {
    ...
    for (j = 0; j < table->n_cols; j++)
    {
      if (!strcmp(dict_table_get_col_name(table, j), field->name))
      {
        ...
        bool exists = std::find(col_added.begin(), col_added.end(), j) != col_added.end();
    ...
    for (j = 0; j < table->n_v_cols; j++)
    {
      if (!strcmp(dict_table_get_v_col_name(table, j), field->name))
      {
        /* Check if same column is being assigned again
        which suggest that column has duplicate name. */
        bool exists = std::find(v_col_added.begin(), v_col_added.end(), j) != v_col_added.end();
        ...
        field->col = reinterpret_cast<dict_col_t *>(dict_table_get_nth_v_col(table, j));
  ...
  dup_err:
    ...
    ib::error() << "No matching column for " << field->name << " in index " << index->name << " of table "
                << table->name;
#endif /* UNIV_DEBUG */
    return (FALSE);
    ...
    ulint prefix_len)          /*!< in: column prefix length */
{
  ...
  if (dict_index_is_spatial(index) && DATA_POINT_MTYPE(col->mtype) && index->n_def == 1)
  {
    field->fixed_len = DATA_MBR_LEN;
  }
  else
  {
    field->fixed_len = static_cast<unsigned int>(dict_col_get_fixed_size(col, dict_table_is_comp(table)));
  }
                               ...
                               field types to copy */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    if (dict_index_is_spatial(index) && DATA_GEOMETRY_MTYPE(dfield_type->mtype))
    {
...
void dict_table_copy_v_types(dtuple_t *tuple, const dict_table_t *table)
{
  ...
  virtual columns */
  ulint n_fields = ut_min(dtuple_get_n_v_fields(tuple), static_cast<ulint>(table->n_v_def));
  ...
  for (ulint i = 0; i < n_fields; i++)
  {
    ...
    dict_col_copy_type(&(dict_table_get_nth_v_col(table, i)->m_col), dtype);
  }
}
/*******************************************************************/ /**
                               ...
                               a clustered index */
{
  ...
  new_index = dict_mem_index_create(table->name.m_name, index->name, table->space, index->type,
                               ...
                               a non-clustered index */
{
  ...
  /* Create a new index */
  new_index = dict_mem_index_create(table->name.m_name, index->name, index->space, index->type,
    ...
    dict_index_t *index) /*!< in: user representation of an FTS index */
{
  ...
  /* Create a new index */
  new_index = dict_mem_index_create(table->name.m_name, index->name, index->space, index->type, index->n_fields);
...
NOT NULL */
{
  ...
  while (index != NULL)
  {
    if (types_idx != index && !(index->type & DICT_FTS) && !dict_index_is_spatial(index) && !index->to_be_dropped &&
        (!(index->uncommitted && ((index->online_status == ONLINE_INDEX_ABORTED_DROPPED) ||
                                  (index->online_status == ONLINE_INDEX_ABORTED)))) &&
        dict_foreign_qualify_index(table, col_names, columns, n_cols, index, types_idx, check_charsets, check_null))
    {
      return (index);
    }
...
/*!< in: error to be ignored */
{
  ...
  for_table = dict_table_check_if_in_cache_low(foreign->foreign_table_name_lookup);

  ref_table = dict_table_check_if_in_cache_low(foreign->referenced_table_name_lookup);
  ...
  {
    index = dict_foreign_find_index(ref_table, NULL, for_in_cache->referenced_col_names, for_in_cache->n_fields,
    ...
    std::pair<dict_foreign_set::iterator, bool> ret = ref_table->referenced_set.insert(for_in_cache);
  ...
  if (for_table && !for_in_cache->foreign_table)
  {
    index = dict_foreign_find_index(
        for_table, col_names, for_in_cache->foreign_col_names, for_in_cache->n_fields, for_in_cache->referenced_index,
        check_charsets, for_in_cache->type & (DICT_FOREIGN_ON_DELETE_SET_NULL | DICT_FOREIGN_ON_UPDATE_SET_NULL));
    ...
    if (index == NULL && !(ignore_err & DICT_ERR_IGNORE_FK_NOKEY))
    {
      ...
      if (for_in_cache == foreign)
      {
        ...
        {
          const dict_foreign_set::size_type n = ref_table->referenced_set.erase(for_in_cache);
    ...
    std::pair<dict_foreign_set::iterator, bool> ret = for_table->foreign_set.insert(for_in_cache);
...
always */
{
  ...
  *id = NULL;
  ...
  {
    while (!my_isspace(cs, *ptr) && *ptr != '(' && *ptr != ')' && (accept_also_dot || *ptr != '.') && *ptr != ',' &&
           *ptr != '\0')
    {
      ptr++;
    }
  ...
  else if (!strncmp(str, srv_mysql50_table_name_prefix, sizeof(srv_mysql50_table_name_prefix) - 1))
  {
                          ...
                          NULL if no name was scannable */
{
  ...
  *ref_name =
      dict_get_referenced_table(name, database_name, database_name_len, table_name, strlen(table_name), table, heap);
    ...
    dict_table_t *table) /*!< in: table in the dictionary memory cache */
{
  ...
  for (dict_foreign_set::iterator it = table->foreign_set.begin(); it != table->foreign_set.end(); ++it)
  {
    foreign = *it;
...
static dberr_t dict_create_foreign_constraints_low(trx_t *trx, mem_heap_t *heap, CHARSET_INFO *cs,
                                                   const char *sql_string, const char *name, ibool reject_fks)
{
...
loop:
  ...
  if (*ptr == '\0')
  {
    ...
    error = dict_create_add_foreigns_to_dictionary(local_fk_set, table, trx);

    if (error == DB_SUCCESS)
    {
      table->foreign_set.insert(local_fk_set.begin(), local_fk_set.end());
      std::for_each(local_fk_set.begin(), local_fk_set.end(), dict_foreign_add_to_referenced_table());
      local_fk_set.clear();
...
col_loop1:
  ...
  index = dict_foreign_find_index(table, NULL, column_names, i, NULL, TRUE, FALSE);
  ...
  std::pair<dict_foreign_set::iterator, bool> ret = local_fk_set.insert(foreign);
  ...
  foreign->foreign_col_names = static_cast<const char **>(mem_heap_alloc(foreign->heap, i * sizeof(void *)));

  for (i = 0; i < foreign->n_fields; i++)
  {
    foreign->foreign_col_names[i] =
        mem_heap_strdup(foreign->heap, dict_table_get_col_name(table, dict_col_get_no(columns[i])));
  }
...
try_find_index:
  ...
  if (referenced_table)
  {
    index = dict_foreign_find_index(referenced_table, NULL, column_names, i, foreign->foreign_index, TRUE, FALSE);
    if (!index)
    {
      mutex_enter(&dict_foreign_err_mutex);
      dict_foreign_error_report_low(ef, name);
  ...
  foreign->referenced_col_names = static_cast<const char **>(mem_heap_alloc(foreign->heap, i * sizeof(void *)));
...
ibool dict_str_starts_with_keyword(
    /*=========================*/
...
@param[in]	reject_fks	if TRUE, fail with error code
                                ...
                                foreign keys are found.
@return error code or DB_SUCCESS */
dberr_t dict_create_foreign_constraints(trx_t *trx, const char *sql_string, size_t sql_length, const char *name,
                                        ibool reject_fks)
{
  char *str;
  dberr_t err;
  ...
  err = dict_create_foreign_constraints_low(trx, heap, innobase_get_charset(trx->mysql_thd), str, name, reject_fks);
...
loop:
  ...
  if (std::find_if(table->foreign_set.begin(), table->foreign_set.end(), dict_foreign_matches_id(id)) ==
...
dict_index_t *dict_index_get_if_in_cache_low(
    /*===========================*/
    ...
    ibool add_newline)       /*!< in: whether to add a newline */
{
  ...
  if (dict_tables_have_same_db(foreign->foreign_table_name_lookup, foreign->referenced_table_name_lookup))
  {
...
static dict_table_t *dict_find_single_table_by_space(ulint space_id)
{
  dict_table_t *table;
  ulint num_item;
  ulint count = 0;
...
ibool dict_set_corrupted_by_space(
    /*========================*/
    ...
    const char *ctx)     /*!< in: context */
{
  ...
  heap = mem_heap_create(sizeof(dtuple_t) +
                         2 * (sizeof(dfield_t) + sizeof(que_fork_t) + sizeof(upd_node_t) + sizeof(upd_t) + 12));
  mtr_start(&mtr);
  index->type |= DICT_CORRUPT;
  ...
  btr_cur_search_to_nth_level(sys_index, 0, tuple, PAGE_CUR_LE, BTR_MODIFY_LEAF, &cursor, 0, __FILE__, __LINE__, &mtr);
  ...
  if (cursor.low_match == dtuple_get_n_fields(tuple))
  {
    ...
    ulint len;
    byte *field = rec_get_nth_field_old(btr_cur_get_rec(&cursor), DICT_FLD__SYS_INDEXES__TYPE, &len);
    if (len != 4)
    {
      goto fail;
    }
  ...
  mtr_commit(&mtr);
  mem_heap_empty(heap);
  ib::error() << status << " corruption of " << index->name << " in table " << index->table->name << " in " << ctx;
  mem_heap_free(heap);
...
void dict_set_corrupted_index_cache_only(dict_index_t *index)
{
  ut_ad(index != NULL);
...
void dict_index_set_merge_threshold(dict_index_t *index, ulint merge_threshold)
{
  ...
  btr_cur_search_to_nth_level(sys_index, 0, tuple, PAGE_CUR_GE, BTR_MODIFY_LEAF, &cursor, 0, __FILE__, __LINE__, &mtr);

  if (cursor.up_match == dtuple_get_n_fields(tuple) &&
      rec_get_n_fields_old(btr_cur_get_rec(&cursor)) == DICT_NUM_FIELDS__SYS_INDEXES)
  {
    ulint len;
    byte *field = rec_get_nth_field_old(btr_cur_get_rec(&cursor), DICT_FLD__SYS_INDEXES__MERGE_THRESHOLD, &len);

    ut_ad(len == 4);

    if (len == 4)
    {
      mlog_write_ulint(field, merge_threshold, MLOG_4BYTES, &mtr);
    }
  }
...
inline void dict_set_merge_threshold_list_debug(UT_LIST_BASE_NODE_T(dict_table_t) * list, uint merge_threshold_all)
{
  for (dict_table_t *table = UT_LIST_GET_FIRST(*list); table != NULL; table = UT_LIST_GET_NEXT(table_LRU, table))
  {
...
void dict_set_merge_threshold_all_debug(uint merge_threshold_all)
{
  ...
  dict_set_merge_threshold_list_debug(&dict_sys->table_LRU, merge_threshold_all);
  dict_set_merge_threshold_list_debug(&dict_sys->table_non_LRU, merge_threshold_all);
...
dict_index_t *dict_table_get_index_on_name(dict_table_t *table, const char *name, bool committed)
{
  ...
  while (index != NULL)
  {
    if (index->is_committed() == committed && innobase_strcasecmp(index->name, name) == 0)
    {
      return (index);
    }
    ...
    const dict_index_t *index) /*!< in: index to be replaced */
{
  ...
  for (dict_foreign_set::iterator it = table->foreign_set.begin(); it != table->foreign_set.end(); ++it)
  {
    ...
    if (foreign->foreign_index == index)
    {
      ...
      dict_index_t *new_index = dict_foreign_find_index(foreign->foreign_table, col_names, foreign->foreign_col_names,
  ...
  for (dict_foreign_set::iterator it = table->referenced_set.begin(); it != table->referenced_set.end(); ++it)
  {
    ...
    if (foreign->referenced_index == index)
    {
      ...
      dict_index_t *new_index = dict_foreign_find_index(foreign->referenced_table, NULL, foreign->referenced_col_names,
                                                        foreign->n_fields, index,
                                                        /*check_charsets=*/TRUE, /*check_null=*/FALSE);
                               ...
                               temporary index names */
{
  ...
  do
  {
    ...
    for (index2 = UT_LIST_GET_NEXT(indexes, index1); index2 != NULL; index2 = UT_LIST_GET_NEXT(indexes, index2))
    {
      ut_ad(index1->is_committed() != index2->is_committed() || strcmp(index1->name, index2->name) != 0);
    }
...
#define CREATE_TYPES_NAMES()                                                                           \
  dtype_sql_name((unsigned)req_schema->columns[i].mtype, (unsigned)req_schema->columns[i].prtype_mask, \
                 (unsigned)req_schema->columns[i].len, req_type, sizeof(req_type));                    \
  dtype_sql_name(table->cols[j].mtype, table->cols[j].prtype, table->cols[j].len, actual_type, sizeof(actual_type))
    ...
    size_t errstr_sz)                /*!< in: errstr size */
{
  ...
  if (table == NULL)
  {
    ...
    ut_snprintf(errstr, errstr_sz, "Table %s not found.", ut_format_name(req_schema->table_name, buf, sizeof(buf)));
  ...
  if (table->ibd_file_missing)
  {
    ...
    ut_snprintf(errstr, errstr_sz, "Tablespace for table %s is missing.",
                ut_format_name(req_schema->table_name, buf, sizeof(buf)));
  ...
  if ((ulint)table->n_def - n_sys_cols != req_schema->n_cols)
  {
    ...
    ut_snprintf(errstr, errstr_sz, "%s has %lu columns but should have %lu.",
                ut_format_name(req_schema->table_name, buf, sizeof(buf)), table->n_def - n_sys_cols,
                req_schema->n_cols);
  ...
  {
    ulint j = dict_table_has_column(table, req_schema->columns[i].name, i);
    ...
    if (j == table->n_def)
    {
      ut_snprintf(errstr, errstr_sz,
                  ...
                  req_schema->columns[i].name, ut_format_name(req_schema->table_name, buf, sizeof(buf)));
    ...
    {
      if (!strcmp(req_schema->table_name, TABLE_STATS_NAME) || !strcmp(req_schema->table_name, INDEX_STATS_NAME))
      {
        ut_ad(table->cols[j].len < req_schema->columns[i].len);
        ib::warn() << "Table " << req_schema->table_name << " has length mismatch in the"
                   << " column name " << req_schema->columns[i].name << ".  Please run mysql_upgrade";
      }
      else
      {
        CREATE_TYPES_NAMES();
        ...
        ut_snprintf(errstr, errstr_sz,
                    ...
                    req_schema->columns[i].name, ut_format_name(req_schema->table_name, buf, sizeof(buf)), actual_type,
                    req_type);
    ...
    if (req_schema->columns[i].prtype_mask != 0 &&
        (table->cols[j].prtype & req_schema->columns[i].prtype_mask) != req_schema->columns[i].prtype_mask)
    {
      CREATE_TYPES_NAMES();
  ...
  if (req_schema->n_foreign != table->foreign_set.size())
  {
    ut_snprintf(errstr, errstr_sz,
                ...
                ut_format_name(req_schema->table_name, buf, sizeof(buf)), static_cast<ulint>(table->foreign_set.size()),
  ...
  if (req_schema->n_referenced != table->referenced_set.size())
  {
    ut_snprintf(errstr, errstr_sz,
                ...
                ut_format_name(req_schema->table_name, buf, sizeof(buf)), req_schema->n_referenced);
    return (DB_ERROR);
  }
    ...
    size_t table_utf8_size)   /*!< in: table_utf8 size */
{
  ...
  strconvert(&my_charset_filename, db, system_charset_info, db_utf8, db_utf8_size, &errors);
  ...
  errors = 0;
  strconvert(&my_charset_filename, buf, system_charset_info, table_utf8, table_utf8_size, &errors);

  if (errors != 0)
  {
    ut_snprintf(table_utf8, table_utf8_size, "%s%s", srv_mysql50_table_name_prefix, table);
  }
}
...
void dict_resize()
{
  ...
  for (table = UT_LIST_GET_FIRST(dict_sys->table_LRU); table; table = UT_LIST_GET_NEXT(table_LRU, table))
  {
  ...
  for (table = UT_LIST_GET_FIRST(dict_sys->table_non_LRU); table; table = UT_LIST_GET_NEXT(table_LRU, table))
  {
    ulint fold = ut_fold_string(table->name.m_name);
    ulint id_fold = ut_fold_ull(table->id);
...
NOT NULL */
{
  ...
  for (ulint i = 0; i < n_cols; i++)
  {
    ...
    col_name = col_names ? col_names[col_no] : dict_table_get_col_name(table, col_no);
...
ulint dict_tf_to_fsp_flags(ulint table_flags, bool is_temp, bool is_encrypted)
{
  ...
  ulint fsp_flags = fsp_flags_init(page_size, has_atomic_blobs, has_data_dir, is_shared, is_temp, is_encrypted);
...
bool dict_space_is_empty(ulint space_id)
{
  btr_pcur_t pcur;
  const rec_t *rec;
  mtr_t mtr;
  ...
  for (rec = dict_startscan_system(&pcur, &mtr, SYS_TABLES); rec != NULL; rec = dict_getnext_system(&pcur, &mtr))
  {
    const byte *field;
    ulint len;
...
ulint dict_space_get_id(const char *name)
{
  btr_pcur_t pcur;
  const rec_t *rec;
  mtr_t mtr;
...
ulint dict_table_extent_size(const dict_table_t *table)
{
...
uint32_t dict_vcol_base_is_foreign_key(dict_v_col_t *vcol, dict_foreign_t *foreign)
{
  ...
  for (uint32_t i = 0; i < foreign->n_fields; i++)
  {
    ...
    {
      if (innobase_strcasecmp(foreign_col_name, dict_table_get_col_name(table, vcol->base_col[j]->ind)) == 0)
      {
        foreign_col_count++;
      }
    }
