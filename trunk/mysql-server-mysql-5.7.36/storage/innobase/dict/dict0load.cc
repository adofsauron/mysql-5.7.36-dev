...
static const char *SYSTEM_TABLE_NAME[] = {"SYS_TABLES",      "SYS_INDEXES",   "SYS_COLUMNS",
...
static dict_table_t *dict_load_table_one(table_name_t &name, bool cached, dict_err_ignore_t ignore_err,
...
/********************************************************************/ /**
 ...
 @return error message, or NULL on success */
const char *dict_process_sys_tables_rec_and_mtr_commit(
    /*=======================================*/
                              ...
                              will be committed */
{
  ...
  field = (const char *)rec_get_nth_field_old(rec, DICT_FLD__SYS_TABLES__NAME, &len);
...
/********************************************************************/ /**
 ...
 @return error message, or NULL on success */
const char *dict_process_sys_indexes_rec(
    /*=========================*/
    ...
    table_id_t *table_id) /*!< out: index table id */
{
  ...
  err_msg = dict_load_index_low(buf, NULL, heap, rec, FALSE, &index);
...
/********************************************************************/ /**
 ...
 @return error message, or NULL on success */
const char *dict_process_sys_columns_rec(
    /*=========================*/
    mem_heap_t *heap,      /*!< in/out: heap memory */
                           ...
                           record's sequence number */
{
  ...
  err_msg = dict_load_column_low(NULL, heap, column, table_id, col_name, rec, nth_v_col);
...
@return error message, or NULL on success */
const char *dict_process_sys_virtual_rec(mem_heap_t *heap, const rec_t *rec, table_id_t *table_id, ulint *pos,
                                         ulint *base_pos)
{
  const char *err_msg;

  /* Parse the record, and get "dict_col_t" struct filled */
  err_msg = dict_load_virtual_low(NULL, heap, NULL, table_id, pos, base_pos, rec);
...
/********************************************************************/ /**
 ...
 structure with the information from the record.
 @return error message, or NULL on success */
const char *dict_process_sys_fields_rec(
    /*========================*/
    mem_heap_t *heap,        /*!< in/out: heap memory */
    ...
    index_id_t last_id)      /*!< in: previous index id */
{
  ...
  err_msg = dict_load_field_low(buf, NULL, sys_field, pos, last_index_id, heap, rec);
...
/********************************************************************/ /**
 ...
 @return error message, or NULL on success */
const char *dict_process_sys_foreign_rec(
    /*=========================*/
    mem_heap_t *heap,        /*!< in/out: heap memory */
                             ...
                             to be filled */
{
  ...
  rec_get_nth_field_offs_old(rec, DICT_FLD__SYS_FOREIGN__DB_TRX_ID, &len);
  if (len != DATA_TRX_ID_LEN && len != UNIV_SQL_NULL)
  {
    goto err_len;
  }
  rec_get_nth_field_offs_old(rec, DICT_FLD__SYS_FOREIGN__DB_ROLL_PTR, &len);
  ...
  foreign->foreign_table_name = mem_heap_strdupl(heap, (const char *)field, len);
  ...
  foreign->referenced_table_name = mem_heap_strdupl(heap, (const char *)field, len);

  field = rec_get_nth_field_old(rec, DICT_FLD__SYS_FOREIGN__N_COLS, &len);
  if (len != 4)
  {
    goto err_len;
  }
...
/********************************************************************/ /**
 ...
 @return error message, or NULL on success */
const char *dict_process_sys_foreign_col_rec(
    /*=============================*/
    mem_heap_t *heap,          /*!< in/out: heap memory */
    ...
    ulint *pos)                /*!< out: column position */
{
  ...
  *pos = mach_read_from_4(field);
  ...
  rec_get_nth_field_offs_old(rec, DICT_FLD__SYS_FOREIGN_COLS__DB_TRX_ID, &len);
  if (len != DATA_TRX_ID_LEN && len != UNIV_SQL_NULL)
  {
    goto err_len;
  }
  rec_get_nth_field_offs_old(rec, DICT_FLD__SYS_FOREIGN_COLS__DB_ROLL_PTR, &len);
  if (len != DATA_ROLL_PTR_LEN && len != UNIV_SQL_NULL)
  {
    goto err_len;
  }

  field = rec_get_nth_field_old(rec, DICT_FLD__SYS_FOREIGN_COLS__FOR_COL_NAME, &len);
  if (len == 0 || len == UNIV_SQL_NULL)
  {
    goto err_len;
  }
  *for_col_name = mem_heap_strdupl(heap, (char *)field, len);
  ...
  field = rec_get_nth_field_old(rec, DICT_FLD__SYS_FOREIGN_COLS__REF_COL_NAME, &len);
  if (len == 0 || len == UNIV_SQL_NULL)
  {
    goto err_len;
  }
...
/********************************************************************/ /**
 ...
 @return error message, or NULL on success */
const char *dict_process_sys_datafiles(
    /*=======================*/
    mem_heap_t *heap,  /*!< in/out: heap memory */
    ...
    const char **path) /*!< out: datafile paths */
{
  ...
  *space = mach_read_from_4(field);
  ...
  rec_get_nth_field_offs_old(rec, DICT_FLD__SYS_DATAFILES__DB_TRX_ID, &len);
  if (len != DATA_TRX_ID_LEN && len != UNIV_SQL_NULL)
  {
    goto err_len;
  }

  rec_get_nth_field_offs_old(rec, DICT_FLD__SYS_DATAFILES__DB_ROLL_PTR, &len);
  if (len != DATA_ROLL_PTR_LEN && len != UNIV_SQL_NULL)
  {
    goto err_len;
  }
...
char *dict_get_first_path(ulint space_id)
{
  ...
  ut_ad(name_of_col_is(sys_datafiles, sys_index, DICT_FLD__SYS_DATAFILES__SPACE, "SPACE"));
  ut_ad(name_of_col_is(sys_datafiles, sys_index, DICT_FLD__SYS_DATAFILES__PATH, "PATH"));
...
static char *dict_space_get_name(ulint space_id, mem_heap_t *callers_heap)
{
  mtr_t mtr;
  ...
  ut_ad(name_of_col_is(sys_tablespaces, sys_index, DICT_FLD__SYS_TABLESPACES__SPACE, "SPACE"));
  ut_ad(name_of_col_is(sys_tablespaces, sys_index, DICT_FLD__SYS_TABLESPACES__NAME, "NAME"));
  ...
  if (btr_pcur_is_on_user_rec(&pcur))
  {
    ...
    if (space_id == mach_read_from_4(field))
    {
      ...
      if (len > 0 && len != UNIV_SQL_NULL)
      {
        ...
        {
          space_name = mem_heap_strdupl(callers_heap, reinterpret_cast<const char *>(field), len);
        }
        ut_ad(space_name);
      }
    }
...
dberr_t dict_update_filepath(ulint space_id, const char *filepath)
{
...
@return DB_SUCCESS if OK, dberr_t if the insert failed */
dberr_t dict_replace_tablespace_and_filepath(ulint space_id, const char *name, const char *filepath, ulint fsp_flags)
{
  if (!srv_sys_tablespaces_open)
  {
    /* Startup procedure is not yet ready for updates.
  ...
  DBUG_EXECUTE_IF("innodb_fail_to_update_tablespace_dict", return (DB_INTERRUPTED););
  ...
  err = dict_replace_tablespace_in_dictionary(space_id, name, fsp_flags, filepath, trx, false);
...
bool dict_sys_tablespaces_rec_read(const rec_t *rec, ulint *id, char *name, ulint *flags)
{
  const byte *field;
  ulint len;
...
UNIV_INLINE
ulint dict_check_sys_tablespaces(bool validate)
{
  ...
  for (rec = dict_startscan_system(&pcur, &mtr, SYS_TABLESPACES); rec != NULL; rec = dict_getnext_system(&pcur, &mtr))
  {
    ...
    if (!dict_sys_tablespaces_rec_read(rec, &space_id, space_name, &fsp_flags))
    {
      continue;
    }

    /* Ignore system and file-per-table tablespaces. */
    if (is_system_tablespace(space_id) || !fsp_is_shared_tablespace(fsp_flags))
    {
      continue;
    }
    ...
    cache. */
    if (fil_space_for_table_exists_in_mem(space_id, space_name, false, true, NULL, 0))
    {
      ...
      if (dict_path && fil_path && strcmp(dict_path, fil_path))
      {
    ...
    dberr_t err = fil_ibd_open(validate, !srv_read_only_mode && srv_log_file_size != 0, FIL_TYPE_TABLESPACE, space_id,
                               fsp_flags, space_name, filepath);

    if (err != DB_SUCCESS)
    {
      ib::warn() << "Ignoring tablespace " << id_name_t(space_name) << " because it could not be opened.";
    }
...
@return true if the record was read correctly, false if not. */
static bool dict_sys_tables_rec_read(const rec_t *rec, const table_name_t &table_name, table_id_t *table_id,
                                     ulint *space_id, ulint *n_cols, ulint *flags, ulint *flags2)
{
  const byte *field;
  ulint len;
  ulint type;
...
@return the highest space ID found. */
UNIV_INLINE
ulint dict_check_sys_tables(bool validate)
{
  ulint max_space_id = 0;
  btr_pcur_t pcur;
  const rec_t *rec;
  ...
  for (rec = dict_startscan_system(&pcur, &mtr, SYS_TABLES); rec != NULL; rec = dict_getnext_system(&pcur, &mtr))
  {
    ...
    DBUG_PRINT("dict_check_sys_tables", ("name: %p, '%s'", table_name.m_name, table_name.m_name));

    dict_sys_tables_rec_read(rec, table_name, &table_id, &space_id, &n_cols, &flags, &flags2);
    if (flags == ULINT_UNDEFINED || is_system_tablespace(space_id))
    {
    ...
    {
      ib::info() << "Ignoring tablespace " << table_name << " because the DISCARD flag is set .";
      ut_free(table_name.m_name);
      continue;
    }
    ...
    ut_ad(DICT_TF_HAS_SHARED_SPACE(flags) || !DICT_TF_GET_COMPACT(flags) || flags2 & DICT_TF2_USE_FILE_PER_TABLE);
    ...
    space_name = shared_space_name == NULL ? table_name.m_name : shared_space_name;
    ...
    ulint fsp_flags = dict_tf_to_fsp_flags(flags, is_temp, is_encrypted);
...
void dict_check_tablespaces_and_store_max_id(bool validate)
{
  mtr_t mtr;
  ...
  mtr_start(&mtr);
  ulint max_space_id = mtr_read_ulint(dict_hdr_get(&mtr) + DICT_HDR_MAX_SPACE_ID, MLOG_4BYTES, &mtr);
  mtr_commit(&mtr);
...
/********************************************************************/ /**
 ...
 dict_table_t.
 @return error message, or NULL on success */
const char *dict_load_column_low(
    /*=================*/
                           ...
                           column */
{
  ...
  if (column == NULL)
  {
    if (prtype & DATA_VIRTUAL)
    {
      ...
      dict_v_col_t *vcol =
#endif
          dict_mem_table_add_v_col(table, heap, name, mtype, prtype, col_len, dict_get_v_col_mysql_pos(pos), num_base);
...
@return error message, or NULL on success */
const char *dict_load_virtual_low(dict_table_t *table, mem_heap_t *heap, dict_col_t **column, table_id_t *table_id,
                                  ulint *pos, ulint *base_pos, const rec_t *rec)
{
  const byte *field;
  ulint len;
  ulint base;
                         ...
                         for temporary storage */
{
  ...
  ut_ad(name_of_col_is(sys_columns, sys_index, DICT_FLD__SYS_COLUMNS__NAME, "NAME"));
  ut_ad(name_of_col_is(sys_columns, sys_index, DICT_FLD__SYS_COLUMNS__PREC, "PREC"));
  ...
  ut_ad(table->n_t_cols == static_cast<ulint>(table->n_cols) + static_cast<ulint>(table->n_v_cols));
  ...
  for (i = 0; i + DATA_N_SYS_COLS < table->n_t_cols + n_skipped; i++)
  {
    ...
    if (innobase_strcasecmp(name, FTS_DOC_ID_COL_NAME) == 0 && nth_v_col == ULINT_UNDEFINED)
    {
      dict_col_t *col;
...
static void dict_load_virtual_one_col(dict_table_t *table, ulint nth_v_col, dict_v_col_t *v_col, mem_heap_t *heap)
{
  ...
  ut_ad(name_of_col_is(sys_virtual, sys_virtual_index, DICT_FLD__SYS_VIRTUAL__POS, "POS"));
  ...
  btr_pcur_open_on_user_rec(sys_virtual_index, tuple, PAGE_CUR_GE, BTR_SEARCH_LEAF, &pcur, &mtr);
  ...
  for (i = 0; i < v_col->num_base + skipped; i++)
  {
    ...
    err_msg = dict_load_virtual_low(table, heap, &v_col->base_col[i - skipped], NULL, &pos, NULL, rec);
...
/********************************************************************/ /**
 ...
 dict_index_t.
 @return error message, or NULL on success */
const char *dict_load_field_low(
    /*================*/
    ...
    const rec_t *rec)        /*!< in: SYS_FIELDS record */
{
  ...
  if (index && UNIV_UNLIKELY((pos_and_prefix_len & 0xFFFFUL) != index->n_def &&
  ...
  if (index)
  {
    dict_mem_index_add_field(index, mem_heap_strdupl(heap, (const char *)field, len), prefix_len);
  }
  else
  {
    ut_a(sys_field);
    ...
    mem_heap_t *heap)    /*!< in: memory heap for temporary storage */
{
  ...
  ut_ad(name_of_col_is(sys_fields, sys_index, DICT_FLD__SYS_FIELDS__COL_NAME, "COL_NAME"));
...
const char *dict_load_index_low(
    /*================*/
    ...
    dict_index_t **index)   /*!< out,own: index, or NULL */
{
  ...
  if (rec_get_n_fields_old(rec) == DICT_NUM_FIELDS__SYS_INDEXES)
  {
    ...
    field = rec_get_nth_field_old(rec, DICT_FLD__SYS_INDEXES__MERGE_THRESHOLD, &len);
    switch (len)
    {
      case 4:
  ...
  else
  {
    ...
    dict_mem_fill_index_struct(*index, NULL, NULL, name_buf, space, type, n_fields);
  }
...
static MY_ATTRIBUTE((nonnull)) dberr_t dict_load_indexes(
    /*==============*/
    dict_table_t *table, /*!< in/out: table */
    mem_heap_t *heap,    /*!< in: memory heap for temporary storage */
...
loading the index definition */
{
  ...
  ut_ad(name_of_col_is(sys_indexes, sys_index, DICT_FLD__SYS_INDEXES__NAME, "NAME"));
  ut_ad(name_of_col_is(sys_indexes, sys_index, DICT_FLD__SYS_INDEXES__PAGE_NO, "PAGE_NO"));
  ...
  for (;;)
  {
    ...
    if (!btr_pcur_is_on_user_rec(&pcur))
    {
      ...
      for drop table */
      if (dict_table_get_first_index(table) == NULL && !(ignore_err & DICT_ERR_IGNORE_CORRUPT))
      {
         ...
         || rec_get_n_fields_old(rec) == DICT_NUM_FIELDS__SYS_INDEXES - 1))
    {
      ...
      if (len != UNIV_SQL_NULL && static_cast<char>(*field) == static_cast<char>(*TEMP_INDEX_PREFIX_STR))
      {
    ...
    err_msg = dict_load_index_low(buf, table->name.m_name, heap, rec, TRUE, &index);
    ut_ad((index == NULL && err_msg != NULL) || (index != NULL && err_msg == NULL));
    ...
    {
      ib::error() << "Index " << index->name << " of table " << table->name << " is corrupted";

      if (!srv_load_corrupted && !(ignore_err & DICT_ERR_IGNORE_CORRUPT) && dict_index_is_clust(index))
      {
      ...
      else
      {
        ...
        index */
        ib::info() << "Load corrupted index " << index->name << " of table " << table->name;
      }
    }
    ...
    if (index->type & ~(DICT_CLUSTERED | DICT_UNIQUE | DICT_CORRUPT | DICT_FTS | DICT_SPATIAL | DICT_VIRTUAL))
    {
      ib::error() << "Unknown type " << index->type << " of index " << index->name << " of table " << table->name;

      error = DB_UNSUPPORTED;
      dict_mem_index_free(index);
      goto func_exit;
    }
    else if (index->page == FIL_NULL && !table->ibd_file_missing && (!(index->type & DICT_FTS)))
    {
      ib::error() << "Trying to load index " << index->name << " for table " << table->name
    ...
    else if (!dict_index_is_clust(index) && NULL == dict_table_get_first_index(table))
    {
      ib::error() << "Trying to load index " << index->name << " for table " << table->name
      ...
      goto corrupted;
             ...
             (dict_index_is_clust(index) || ((table == dict_sys->sys_tables) && !strcmp("ID_IND", index->name))))
    {
  ...
  if (table->fts != NULL)
  {
    table->fts_doc_id_index = dict_table_get_index_on_name(table, FTS_DOC_ID_INDEX_NAME);
  }
...
static const char *dict_load_table_low(table_name_t &name, const rec_t *rec, dict_table_t **table)
{
  ...
  dict_sys_tables_rec_read(rec, name, &table_id, &space_id, &t_num, &flags, &flags2);
  ...
  *table = dict_mem_table_create(name.m_name, space_id, n_cols + n_v_col, n_v_col, flags, flags2);
  (*table)->id = table_id;
    ...
    char *filepath)      /*!< in: filepath of tablespace */
{
  ...
  char *default_filepath = fil_make_filepath(NULL, table->name.m_name, IBD, false);
  if (default_filepath)
  {
...
{
  if (DICT_TF_HAS_DATA_DIR(table->flags) && (!table->data_dir_path))
  {
...
void dict_get_and_save_space_name(dict_table_t *table, bool dict_mutex_own)
{
  ...
  {
    if (srv_sys_tablespaces_open && dict_table_has_temp_general_tablespace_name(table->tablespace))
    {
  ...
  if (use_cache)
  {
    ...
    if (space != NULL)
    {
      ...
      if (!srv_sys_tablespaces_open || !dict_table_has_temp_general_tablespace_name(space->name))
      {
  ...
  if (srv_sys_tablespaces_open)
  {
    ...
    table->tablespace = dict_space_get_name(table->space, table->heap);
...
void dict_load_tablespace(dict_table_t *table, mem_heap_t *heap, dict_err_ignore_t ignore_err)
{
  ...
  {
    ib::warn() << "Tablespace for table " << table->name << " is set as discarded.";
  ...
  if (DICT_TF_HAS_SHARED_SPACE(table->flags))
  {
    ...
    else
    {
      ...
      shared_space_name = static_cast<char *>(ut_malloc_nokey(strlen(general_space_name) + 20));

      sprintf(shared_space_name, "%s_" ULINTPF, general_space_name, static_cast<ulint>(table->space));
    }
  ...
  if (fil_space_for_table_exists_in_mem(table->space, space_name, false, true, heap, table->id))
  {
    ut_free(shared_space_name);
    return;
  }
  ...
  ulint fsp_flags = dict_tf_to_fsp_flags(table->flags, false, dict_table_is_encrypted(table));
  dberr_t err = fil_ibd_open(true, false, FIL_TYPE_TABLESPACE, table->space, fsp_flags, space_name, filepath);
                                         ...
                                         dict_names_t &fk_tables)
{
  ...
  ut_ad(name_of_col_is(sys_tables, sys_index, DICT_FLD__SYS_TABLES__N_COLS, "N_COLS"));
  ut_ad(name_of_col_is(sys_tables, sys_index, DICT_FLD__SYS_TABLES__TYPE, "TYPE"));
  ut_ad(name_of_col_is(sys_tables, sys_index, DICT_FLD__SYS_TABLES__MIX_LEN, "MIX_LEN"));
  ut_ad(name_of_col_is(sys_tables, sys_index, DICT_FLD__SYS_TABLES__SPACE, "SPACE"));
  ...
  if (len != ut_strlen(name.m_name) || 0 != ut_memcmp(name.m_name, field, len))
  {
    goto err_exit;
  }
  ...
  dict_err_ignore_t index_load_err =
      !(ignore_err & DICT_ERR_IGNORE_RECOVER_LOCK) && table->ibd_file_missing ? DICT_ERR_IGNORE_ALL : ignore_err;
  ...
  if (!DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_HAS_DOC_ID) && table->space == srv_sys_space.space_id() &&
      table->flags == 0)
  {
    table->flags2 = 0;
  }
  ...
  DBUG_EXECUTE_IF(
      "ib_table_invalid_flags", if (strcmp(table->name.m_name, "test/t1") == 0) {
        table->flags2 = 255;
        table->flags = 255;
      });
  ...
  {
    err = dict_load_foreigns(table->name.m_name, NULL, true, true, ignore_err, fk_tables);
  ...
  else
  {
    ...
    if (!srv_force_recovery || !index || !dict_index_is_clust(index))
    {
      dict_table_remove_from_cache(table);
      table = NULL;
    }
    else if (dict_index_is_corrupted(index) && !table->ibd_file_missing)
    {
...
func_exit:
  ...
  ut_ad(!table || ignore_err != DICT_ERR_IGNORE_NONE || table->ibd_file_missing || !table->corrupted);
    ...
    dict_foreign_t *foreign) /*!< in/out: foreign constraint object */
{
  ...
  foreign->foreign_col_names =
      static_cast<const char **>(mem_heap_alloc(foreign->heap, foreign->n_fields * sizeof(void *)));
  ...
  for (i = 0; i < foreign->n_fields; i++)
  {
    ...
    if (len != id_len || ut_memcmp(foreign->id, field, len) != 0)
    {
      ...
      pos = rec_get_nth_field_old(rec, DICT_FLD__SYS_FOREIGN_COLS__POS, &pos_len);

      for_col_name = rec_get_nth_field_old(rec, DICT_FLD__SYS_FOREIGN_COLS__FOR_COL_NAME, &for_col_name_len);

      ref_col_name = rec_get_nth_field_old(rec, DICT_FLD__SYS_FOREIGN_COLS__REF_COL_NAME, &ref_col_name_len);
...
static MY_ATTRIBUTE((nonnull(1), warn_unused_result)) dberr_t dict_load_foreign(
    /*==============*/
    const char *id,
...
stack. */
{
  ...
  DBUG_PRINT("dict_load_foreign", ("id: '%s', check_recursive: %d", id, check_recursive));
  ...
  if (!btr_pcur_is_on_user_rec(&pcur) || rec_get_deleted_flag(rec, 0))
  {
    ...
    ib::error() << "Cannot load foreign constraint " << id << ": could not find the relevant record in "
                << "SYS_FOREIGN";
  ...
  n_fields_and_type = mach_read_from_4(rec_get_nth_field_old(rec, DICT_FLD__SYS_FOREIGN__N_COLS, &len));
  ...
  if (!for_table)
  {
    ...
    ut_a(ref_table != NULL);
    fk_tables.push_back(mem_heap_strdupl(ref_table->heap, foreign->foreign_table_name_lookup, foreign_table_name_len));
  ...
  DBUG_RETURN(dict_foreign_add_to_cache(foreign, col_names, check_charsets, ignore_err));
}
...
loop:
  ...
  field = rec_get_nth_field_old(rec, DICT_FLD__SYS_FOREIGN_FOR_NAME__NAME, &len);

  /* Check if the table name in the record is the one searched for; the
  following call does the comparison in the latin1_swedish_ci
  charset-collation, in a case-insensitive way. */

  if (0 != cmp_data_data(dfield_get_type(dfield)->mtype, dfield_get_type(dfield)->prtype,
                         static_cast<const byte *>(dfield_get_data(dfield)), dfield_get_len(dfield), field, len))
  {
    goto load_next_index;
  }
  ...
  if ((innobase_get_lower_case_table_names() != 2) && (0 != ut_memcmp(field, table_name, len)))
  {
    goto next_rec;
  }
  ...
  err = dict_load_foreign(fk_id, col_names, check_recursive, check_charsets, ignore_err, fk_tables);
