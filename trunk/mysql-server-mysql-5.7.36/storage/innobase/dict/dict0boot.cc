    ...
    mtr_t *mtr) /*!< in: mtr */
{
  ...
  block = buf_page_get(page_id_t(DICT_HDR_SPACE, DICT_HDR_PAGE_NO), univ_page_size, RW_X_LATCH, mtr);
                               ...
                               then disable-redo */
{
  ...
  else if (disable_redo)
  {
    ...
    mtr_set_log_mode(&mtr, (srv_read_only_mode ? MTR_LOG_NONE : MTR_LOG_NO_REDO));
  }
  ...
  if (space_id)
  {
    *space_id = mtr_read_ulint(dict_hdr + DICT_HDR_MAX_SPACE_ID, MLOG_4BYTES, &mtr);
    if (fil_assign_new_space_id(space_id))
    {
      mlog_write_ulint(dict_hdr + DICT_HDR_MAX_SPACE_ID, *space_id, MLOG_4BYTES, &mtr);
    }
  }
    ...
    mtr_t *mtr) /*!< in: mtr */
{
  ...
  mlog_write_ull(dict_header + DICT_HDR_INDEX_ID, DICT_HDR_FIRST_ID, mtr);

  mlog_write_ulint(dict_header + DICT_HDR_MAX_SPACE_ID, 0, MLOG_4BYTES, mtr);

  /* Obsolete, but we must initialize it anyway. */
  mlog_write_ulint(dict_header + DICT_HDR_MIX_ID_LOW, DICT_HDR_FIRST_ID, MLOG_4BYTES, mtr);

  /* Create the B-tree roots for the clustered indexes of the basic
  system tables */

  /*--------------------------*/
  root_page_no = btr_create(DICT_CLUSTERED | DICT_UNIQUE, DICT_HDR_SPACE, univ_page_size, DICT_TABLES_ID,
  ...
  /*--------------------------*/
  root_page_no =
      btr_create(DICT_UNIQUE, DICT_HDR_SPACE, univ_page_size, DICT_TABLE_IDS_ID, dict_ind_redundant, NULL, mtr);
  if (root_page_no == FIL_NULL)
  {
    return (FALSE);
  }
  ...
  /*--------------------------*/
  root_page_no = btr_create(DICT_CLUSTERED | DICT_UNIQUE, DICT_HDR_SPACE, univ_page_size, DICT_COLUMNS_ID,
                            dict_ind_redundant, NULL, mtr);
  if (root_page_no == FIL_NULL)
  {
    return (FALSE);
  ...
  /*--------------------------*/
  root_page_no = btr_create(DICT_CLUSTERED | DICT_UNIQUE, DICT_HDR_SPACE, univ_page_size, DICT_INDEXES_ID,
                            dict_ind_redundant, NULL, mtr);
  if (root_page_no == FIL_NULL)
  {
    return (FALSE);
  ...
  /*--------------------------*/
  root_page_no = btr_create(DICT_CLUSTERED | DICT_UNIQUE, DICT_HDR_SPACE, univ_page_size, DICT_FIELDS_ID,
                            dict_ind_redundant, NULL, mtr);
  if (root_page_no == FIL_NULL)
  {
    return (FALSE);
...
/*===========*/
{
  ...
  dict_sys->row_id = DICT_HDR_ROW_ID_WRITE_MARGIN +
                     ut_uint64_align_up(mach_read_from_8(dict_hdr + DICT_HDR_ROW_ID), DICT_HDR_ROW_ID_WRITE_MARGIN);
  ...
  error =
      dict_index_add_to_cache(table, index, mtr_read_ulint(dict_hdr + DICT_HDR_TABLE_IDS, MLOG_4BYTES, &mtr), FALSE);
  ut_a(error == DB_SUCCESS);
  ...
  empty. */
  if (srv_force_recovery != SRV_FORCE_NO_LOG_REDO && srv_read_only_mode && !ibuf_is_empty())
  {
