                               ...
                               tuple is allocated */
{
  ...
  /* 0: NAME -----------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_TABLES__NAME);

  dfield_set_data(dfield, table->name.m_name, strlen(table->name.m_name));

  /* 1: DB_TRX_ID added later */
  /* 2: DB_ROLL_PTR added later */
  /* 3: ID -------------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_TABLES__ID);
  ...
  /* 4: N_COLS ---------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_TABLES__N_COLS);

  ptr = static_cast<byte *>(mem_heap_alloc(heap, 4));

  /* If there is any virtual column, encode it in N_COLS */
  mach_write_to_4(ptr, dict_table_encode_n_col(static_cast<ulint>(table->n_def), static_cast<ulint>(table->n_v_def)) |
                           ((table->flags & DICT_TF_COMPACT) << 31));
  dfield_set_data(dfield, ptr, 4);

  /* 5: TYPE (table flags) -----------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_TABLES__TYPE);
  ...
  /* 6: MIX_ID (obsolete) ---------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_TABLES__MIX_ID);
  ...
  /* 8: CLUSTER_NAME ---------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_TABLES__CLUSTER_ID);
  dfield_set_null(dfield); /* not supported */

  /* 9: SPACE ----------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_TABLES__SPACE);
                               ...
                               tuple is allocated */
{
  ...
  if (i >= table->n_def)
  {
    dict_v_col_t *v_col = dict_table_get_nth_v_col(table, i - table->n_def);
    column = &v_col->m_col;
  ...
  if (v_col_no != ULINT_UNDEFINED)
  {
    ...
    table in "POS" */
    mach_write_to_4(ptr, dict_create_v_col_pos(i - table->n_def, v_col_no));
  }
  else
  {
                                               ...
                                               mem_heap_t *heap)
{
  ...
  entry = dtuple_create(heap, DICT_NUM_COLS__SYS_VIRTUAL + DATA_N_SYS_COLS);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t dict_build_table_def_step(
    /*======================*/
...
@return DB_SUCCESS or error code. */
dberr_t dict_build_tablespace(Tablespace *tablespace)
{
  dberr_t err = DB_SUCCESS;
  mtr_t mtr;
  ulint space = 0;
  ...
  err = fil_ibd_create(space, tablespace->name(), datafile->filepath(), tablespace->flags(), FIL_IBD_FILE_INITIAL_SIZE);
  if (err != DB_SUCCESS)
  {
    return (err);
  }
  ...
  err = dict_replace_tablespace_and_filepath(tablespace->space_id(), tablespace->name(), datafile->filepath(),
                                             tablespace->flags());
  if (err != DB_SUCCESS)
  {
...
dberr_t dict_build_tablespace_for_table(dict_table_t *table)
{
  ...
  DBUG_EXECUTE_IF("innodb_test_wrong_fts_aux_table_name", DICT_TF2_FLAG_UNSET(table, DICT_TF2_FTS_AUX_HEX_NAME););
  ...
  if (needs_file_per_table)
  {
    ...
    ut_ad(DICT_TF_GET_ZIP_SSIZE(table->flags) == 0 || dict_table_get_format(table) >= UNIV_FORMAT_B);

    /* Get a new tablespace ID */
    dict_hdr_get_new_id(NULL, NULL, &space, table, false);

    DBUG_EXECUTE_IF("ib_create_table_fail_out_of_space_ids", space = ULINT_UNDEFINED;);
    ...
    ulint fsp_flags = dict_tf_to_fsp_flags(table->flags, is_temp, is_encrypted);
    ...
    err = fil_ibd_create(space, table->name.m_name, filepath, fsp_flags, FIL_IBD_FILE_INITIAL_SIZE);
    ...
    bool ret = fsp_header_init(table->space, FIL_IBD_FILE_INITIAL_SIZE, &mtr);
...
static void dict_build_col_def_step(
    /*====================*/
    tab_node_t *node) /*!< in: table create node */
{
  dtuple_t *row;
...
static void dict_build_v_col_def_step(tab_node_t *node)
{
  dtuple_t *row;

  row = dict_create_sys_virtual_tuple(node->table, node->col_no, node->base_col_no, node->heap);
                               ...
                               tuple is allocated */
{
  ...
  entry = dtuple_create(heap, DICT_NUM_COLS__SYS_INDEXES + DATA_N_SYS_COLS);

  dict_table_copy_types(entry, sys_indexes);

  /* 0: TABLE_ID -----------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_INDEXES__TABLE_ID);
  ...
  /* 1: ID ----------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_INDEXES__ID);
  ...
  /* 3: DB_ROLL_PTR added later */
  /* 4: NAME --------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_INDEXES__NAME);

  if (!index->is_committed())
  {
    ulint len = strlen(index->name) + 1;
    char *name = static_cast<char *>(mem_heap_alloc(heap, len));
  ...
  /* 5: N_FIELDS ----------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_INDEXES__N_FIELDS);
  ...
  /* 6: TYPE --------------------------*/
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_INDEXES__TYPE);
  ...
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_INDEXES__SPACE);
  ...
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_INDEXES__PAGE_NO);
  ...
  dfield = dtuple_get_nth_field(entry, DICT_COL__SYS_INDEXES__MERGE_THRESHOLD);
...
/***************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t dict_build_index_def_step(
    /*======================*/
    que_thr_t *thr,   /*!< in: query thread */
...
static void dict_build_field_def_step(
    /*======================*/
    ind_node_t *node) /*!< in: index create node */
{
  dict_index_t *index;
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t dict_create_index_tree_step(
    /*========================*/
    ind_node_t *node) /*!< in: index create node */
{
  mtr_t mtr;
  ...
  const bool missing = index->table->ibd_file_missing || dict_table_is_discarded(index->table);
  ...
  {
    node->page_no =
        btr_create(index->type, index->space, dict_table_page_size(index->table), index->id, index, NULL, &mtr);
    ...
    DBUG_EXECUTE_IF("ib_import_create_index_failure_1", node->page_no = FIL_NULL; err = DB_OUT_OF_FILE_SPACE;);
  }

  page_rec_write_field(btr_pcur_get_rec(&pcur), DICT_FLD__SYS_INDEXES__PAGE_NO, node->page_no, &mtr);
...
/***************************************************************/ /**
 ...
 @return DB_SUCCESS or DB_OUT_OF_FILE_SPACE */
dberr_t dict_create_index_tree_in_mem(
    /*==========================*/
    dict_index_t *index, /*!< in/out: index */
    const trx_t *trx)    /*!< in: InnoDB transaction handle */
{
  ...
  ut_ad(mutex_own(&dict_sys->mutex) || dict_table_is_intrinsic(index->table));
  ...
  ut_ad(index->table->ibd_file_missing == 0 && !dict_table_is_discarded(index->table));

  page_no = btr_create(index->type, index->space, dict_table_page_size(index->table), index->id, index, NULL, &mtr);
...
bool dict_drop_index_tree(rec_t *rec, btr_pcur_t *pcur, mtr_t *mtr)
{
  const byte *ptr;
  ulint len;
  ulint space;
  ...
  btr_free_if_exists(page_id_t(space, root_page_no), page_size, mach_read_from_8(ptr), mtr);
...
void dict_drop_index_tree_in_mem(
    /*========================*/
    const dict_index_t *index, /*!< in: index */
...
ulint dict_recreate_index_tree(
    /*=====================*/
    const dict_table_t *table, /*!< in/out: the table the index belongs to */
                               ...
                               on the record page. */
{
  ...
  const byte *ptr = rec_get_nth_field_old(rec, DICT_FLD__SYS_INDEXES__PAGE_NO, &len);
  ...
  if (!found)
  {
    ...
    ib::warn() << "Trying to TRUNCATE a missing .ibd file of table " << table->name << "!";
  ...
  for (dict_index_t *index = UT_LIST_GET_FIRST(table->indexes); index != NULL; index = UT_LIST_GET_NEXT(indexes, index))
  {
    if (index->id == index_id)
    {
      ...
      {
        root_page_no = btr_create(type, space, page_size, index_id, index, NULL, mtr);
  ...
  ib::error() << "Failed to create index with index id " << index_id << " of table " << table->name;
...
dberr_t dict_truncate_index_tree_in_mem(
    /*============================*/
    dict_index_t *index) /*!< in/out: index */
{
  mtr_t mtr;
  ...
  if (root_page_no == FIL_NULL)
  {
    ...
    ib::warn() << "Trying to TRUNCATE a missing index of table " << index->table->name << "!";
  ...
  if (!found)
  {
    ...
    ib::warn() << "Trying to TRUNCATE a missing .ibd file of table " << index->table->name << "!";
  }
  ...
  root_page_no = btr_create(type, space, page_size, index->id, index, NULL, &mtr);

  DBUG_EXECUTE_IF("ib_err_trunc_temp_recreate_index", root_page_no = FIL_NULL;);
...
ind_node_t *ind_create_graph_create(dict_index_t *index, mem_heap_t *heap, const dict_add_v_col_t *add_v)
{
  ind_node_t *node;
    ...
    que_thr_t *thr) /*!< in: query thread */
{
  ...
  {
    if (node->col_no < (static_cast<ulint>(node->table->n_def) + static_cast<ulint>(node->table->n_v_def)))
    {
  ...
  if (node->state == TABLE_BUILD_V_COL_DEF)
  {
    ...
    {
      dict_v_col_t *v_col = dict_table_get_nth_v_col(node->table, node->col_no);
      ...
      while (v_col->num_base == 0)
      {
        ...
        v_col = dict_table_get_nth_v_col(node->table, node->col_no);
        node->base_col_no = 0;
      }
    ...
    que_thr_t *thr) /*!< in: query thread */
{
  ...
  if (node->state == INDEX_ADD_TO_CACHE)
  {
    ...
    err = dict_index_add_to_cache_w_vcol(node->table, node->index, node->add_v, FIL_NULL, trx_is_strict(trx));
  ...
  if (node->state == INDEX_CREATE_INDEX_TREE)
  {
    ...
    DBUG_EXECUTE_IF("ib_dict_create_index_tree_fail", err = DB_OUT_OF_MEMORY;);

    if (err != DB_SUCCESS)
    {
      /* If this is a FTS index, we will need to remove
      it from fts->cache->indexes list as well */
      if ((node->index->type & DICT_FTS) && node->table->fts)
      {
        fts_index_cache_t *index_cache;

        rw_lock_x_lock(&node->table->fts->cache->init_lock);

        index_cache = (fts_index_cache_t *)fts_find_index_cache(node->table->fts->cache, node->index);
        ...
        ib_vector_remove(node->table->fts->cache->indexes, *reinterpret_cast<void **>(index_cache));

        rw_lock_x_unlock(&node->table->fts->cache->init_lock);
      }
...
static dberr_t dict_check_if_system_table_exists(
    /*==============================*/
    ...
    ulint num_indexes)     /*!< in: number of indexes */
{
  ...
  else if (UT_LIST_GET_LEN(sys_table->indexes) != num_indexes || sys_table->n_cols != num_fields)
  {
    error = DB_CORRUPTION;
  }
  else
...
/****************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
dberr_t dict_create_or_check_foreign_constraint_tables(void)
/*================================================*/
{
  trx_t *trx;
  ...
  sys_foreign_err = dict_check_if_system_table_exists("SYS_FOREIGN", DICT_NUM_FIELDS__SYS_FOREIGN + 1, 3);
  sys_foreign_cols_err =
      dict_check_if_system_table_exists("SYS_FOREIGN_COLS", DICT_NUM_FIELDS__SYS_FOREIGN_COLS + 1, 1);

  if (sys_foreign_err == DB_SUCCESS && sys_foreign_cols_err == DB_SUCCESS)
  {
    return (DB_SUCCESS);
  }
  ...
  if (err != DB_SUCCESS)
  {
    ...
    ut_ad(err == DB_OUT_OF_FILE_SPACE || err == DB_TOO_MANY_CONCURRENT_TRXS);
...
not of the right form.
@return DB_SUCCESS or error code */
dberr_t dict_create_or_check_sys_virtual()
{
  trx_t *trx;
  my_bool srv_file_per_table_backup;
  dberr_t err;
  ...
  /* Note: The master thread has not been started at this point. */
  err = dict_check_if_system_table_exists("SYS_VIRTUAL", DICT_NUM_FIELDS__SYS_VIRTUAL + 1, 1);
  ...
  /* Note: The master thread has not been started at this point. */
  /* Confirm and move to the non-LRU part of the table LRU list. */
  dberr_t sys_virtual_err = dict_check_if_system_table_exists("SYS_VIRTUAL", DICT_NUM_FIELDS__SYS_VIRTUAL + 1, 1);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t dict_foreign_eval_sql(
    /*==================*/
    ...
    trx_t *trx)        /*!< in/out: transaction */
{
  ...
  if (error != DB_SUCCESS)
  {
    ib::error() << "Foreign key constraint creation failed: " << ut_strerr(error);
...
/********************************************************************/ /**
 ...
 the database.
 @return error code or DB_SUCCESS */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t dict_create_add_foreign_field_to_dictionary(
    /*========================================*/
    ...
    trx_t *trx)                    /*!< in/out: transaction */
{
  ...
  DBUG_RETURN(dict_foreign_eval_sql(info,
                                    "PROCEDURE P () IS\n"
                                    "BEGIN\n"
...
/********************************************************************/ /**
 ...
 @return error code or DB_SUCCESS */
dberr_t dict_create_add_foreign_to_dictionary(
    /*==================================*/
    ...
    trx_t *trx)                    /*!< in/out: dictionary transaction */
{
  ...
  DBUG_PRINT("dict_create_add_foreign_to_dictionary",
             ("'%s', '%s', '%s', %d", foreign->id, name, foreign->referenced_table_name,
  ...
  {
    error = dict_create_add_foreign_field_to_dictionary(i, name, foreign, trx);
...
bool dict_foreign_has_col_in_v_index(const char *fk_col_name, const dict_table_t *table)
{
  ...
  for (dict_index_t *index = dict_table_get_next_index(dict_table_get_first_index(table)); index;
...
bool dict_foreign_has_col_as_base_col(const char *col_name, const dict_table_t *table)
{
  ...
  for (ulint i = 0; i < table->n_v_cols; i++)
  {
    ...
    {
      if (strcmp(col_name, dict_table_get_col_name(table, v_col->base_col[j]->ind)) == 0)
      {
        return (true);
      }
    }
...
bool dict_foreigns_has_s_base_col(const dict_foreign_set &local_fk_set, const dict_table_t *table)
{
  ...
  for (dict_foreign_set::const_iterator it = local_fk_set.begin(); it != local_fk_set.end(); ++it)
  {
    foreign = *it;
...
@return error code or DB_SUCCESS */
dberr_t dict_create_add_foreigns_to_dictionary(
    /*===================================*/
    const dict_foreign_set &local_fk_set, const dict_table_t *table, trx_t *trx)
{
  dict_foreign_t *foreign;
  dberr_t error;
  ...
  for (dict_foreign_set::const_iterator it = local_fk_set.begin(); it != local_fk_set.end(); ++it)
  {
    ...
    error = dict_create_add_foreign_to_dictionary(table->name.m_name, foreign, trx);
...
/****************************************************************/ /**
 ...
 not of the right form.
 @return DB_SUCCESS or error code */
dberr_t dict_create_or_check_sys_tablespace(void)
/*=====================================*/
{
  trx_t *trx;
  my_bool srv_file_per_table_backup;
  ...
  sys_tablespaces_err = dict_check_if_system_table_exists("SYS_TABLESPACES", DICT_NUM_FIELDS__SYS_TABLESPACES + 1, 1);
  sys_datafiles_err = dict_check_if_system_table_exists("SYS_DATAFILES", DICT_NUM_FIELDS__SYS_DATAFILES + 1, 1);

  if (sys_tablespaces_err == DB_SUCCESS && sys_datafiles_err == DB_SUCCESS)
  {
    return (DB_SUCCESS);
  }
  ...
  if (err != DB_SUCCESS)
  {
    ib::error() << "Creation of SYS_TABLESPACES and SYS_DATAFILES"
                ...
                << ut_strerr(err) << ". Dropping incompletely created tables.";
...
@return error code or DB_SUCCESS */
dberr_t dict_replace_tablespace_in_dictionary(ulint space_id, const char *name, ulint flags, const char *path,
                                              trx_t *trx, bool commit)
{
...
dberr_t dict_delete_tablespace_and_datafiles(ulint space, trx_t *trx)
{
  dberr_t err = DB_SUCCESS;
  ...
  if (err != DB_SUCCESS)
  {
    ib::warn() << "Could not delete space_id " << space << " from data dictionary";
  }
