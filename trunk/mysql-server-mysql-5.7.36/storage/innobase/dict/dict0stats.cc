...
#define N_SAMPLE_PAGES(index)                                                                           \
  static_cast<ib_uint64_t>((index)->table->stats_sample_pages != 0 ? (index)->table->stats_sample_pages \
...
typedef ut_allocator<std::pair<const char *const, dict_index_t *>> index_map_t_allocator;
...
{
  return ((index->type & DICT_FTS) || dict_index_is_corrupted(index) || dict_index_is_spatial(index) ||
...
/*********************************************************************/ /**
 ...
 @return true if exists and all tables are ok */
static bool dict_stats_persistent_storage_check(
    /*================================*/
    bool caller_has_dict_sys_mutex) /*!< in: true if the caller
                                    owns dict_sys->mutex */
{
  /* definition for the table TABLE_STATS_NAME */
  dict_col_meta_t table_stats_columns[] = {{"database_name", DATA_VARMYSQL, DATA_NOT_NULL, 192},

                                           {"table_name", DATA_VARMYSQL, DATA_NOT_NULL, 597},

                                           {"last_update", DATA_FIXBINARY, DATA_NOT_NULL, 4},
  ...
  dict_table_schema_t table_stats_schema = {
      TABLE_STATS_NAME, UT_ARR_SIZE(table_stats_columns), table_stats_columns, 0 /* n_foreign */, 0 /* n_referenced */
  };

  /* definition for the table INDEX_STATS_NAME */
  dict_col_meta_t index_stats_columns[] = {{"database_name", DATA_VARMYSQL, DATA_NOT_NULL, 192},
  ...
  dict_table_schema_t index_stats_schema = {
      INDEX_STATS_NAME, UT_ARR_SIZE(index_stats_columns), index_stats_columns, 0 /* n_foreign */, 0 /* n_referenced */
  };
  ...
  ret = dict_table_schema_check(&table_stats_schema, errstr, sizeof(errstr));
  if (ret == DB_SUCCESS)
  {
    ...
    const dict_table_t *table) /*!< in: table whose stats to copy */
{
  ...
  for (index = dict_table_get_first_index(table); index != NULL; index = dict_table_get_next_index(index))
  {
    ...
    idx->fields = (dict_field_t *)mem_heap_alloc(heap, idx->n_uniq * sizeof(idx->fields[0]));
    ...
    idx->stat_n_diff_key_vals = (ib_uint64_t *)mem_heap_alloc(heap, idx->n_uniq * sizeof(idx->stat_n_diff_key_vals[0]));

    idx->stat_n_sample_sizes = (ib_uint64_t *)mem_heap_alloc(heap, idx->n_uniq * sizeof(idx->stat_n_sample_sizes[0]));

    idx->stat_n_non_null_key_vals =
        (ib_uint64_t *)mem_heap_alloc(heap, idx->n_uniq * sizeof(idx->stat_n_non_null_key_vals[0]));
...
static void dict_stats_assert_initialized_index(
    ...
    const dict_index_t *index) /*!< in: index */
{
  UNIV_MEM_ASSERT_RW_ABORT(index->stat_n_diff_key_vals, index->n_uniq * sizeof(index->stat_n_diff_key_vals[0]));

  UNIV_MEM_ASSERT_RW_ABORT(index->stat_n_sample_sizes, index->n_uniq * sizeof(index->stat_n_sample_sizes[0]));

  UNIV_MEM_ASSERT_RW_ABORT(index->stat_n_non_null_key_vals, index->n_uniq * sizeof(index->stat_n_non_null_key_vals[0]));

  UNIV_MEM_ASSERT_RW_ABORT(&index->stat_index_size, sizeof(index->stat_index_size));

  UNIV_MEM_ASSERT_RW_ABORT(&index->stat_n_leaf_pages, sizeof(index->stat_n_leaf_pages));
}
    ...
    const dict_table_t *table) /*!< in: table */
{
  ...
  UNIV_MEM_ASSERT_RW_ABORT(&table->stats_last_recalc, sizeof(table->stats_last_recalc));

  UNIV_MEM_ASSERT_RW_ABORT(&table->stat_persistent, sizeof(table->stat_persistent));

  UNIV_MEM_ASSERT_RW_ABORT(&table->stats_auto_recalc, sizeof(table->stats_auto_recalc));

  UNIV_MEM_ASSERT_RW_ABORT(&table->stats_sample_pages, sizeof(table->stats_sample_pages));

  UNIV_MEM_ASSERT_RW_ABORT(&table->stat_n_rows, sizeof(table->stat_n_rows));

  UNIV_MEM_ASSERT_RW_ABORT(&table->stat_clustered_index_size, sizeof(table->stat_clustered_index_size));

  UNIV_MEM_ASSERT_RW_ABORT(&table->stat_sum_of_other_index_sizes, sizeof(table->stat_sum_of_other_index_sizes));

  UNIV_MEM_ASSERT_RW_ABORT(&table->stat_modified_counter, sizeof(table->stat_modified_counter));
...
#define INDEX_EQ(i1, i2) ((i1) != NULL && (i2) != NULL && (i1)->id == (i2)->id && strcmp((i1)->name, (i2)->name) == 0)
    ...
    const dict_table_t *src) /*!< in: source table */
{
  ...
  for (dst_idx = dict_table_get_first_index(dst), src_idx = dict_table_get_first_index(src); dst_idx != NULL;
       dst_idx = dict_table_get_next_index(dst_idx),
      (src_idx != NULL && (src_idx = dict_table_get_next_index(src_idx))))
  {
    ...
    {
      for (src_idx = dict_table_get_first_index(src); src_idx != NULL; src_idx = dict_table_get_next_index(src_idx))
      {
        if (INDEX_EQ(src_idx, dst_idx))
        {
          break;
    ...
    memmove(dst_idx->stat_n_non_null_key_vals, src_idx->stat_n_non_null_key_vals,
    ...
    dict_index_t *index) /*!< in/out: index */
{
  if (srv_force_recovery >= SRV_FORCE_NO_TRX_UNDO &&
      (srv_force_recovery >= SRV_FORCE_NO_LOG_REDO || !dict_index_is_clust(index)))
  {
    ...
    dict_table_t *table) /*!< in/out: table */
{
  ...
  else if (index == NULL)
  {
    ...
    ib::warn() << "Table " << table->name << " has no indexes. Cannot calculate statistics.";
    dict_stats_empty_table(table);
    dict_table_analyze_index_unlock(table);
    return;
  }
  ...
  table->stat_n_rows = index->stat_n_diff_key_vals[dict_index_get_n_unique(index) - 1];
    ...
    mtr_t *mtr)                      /*!< in/out: mini-transaction */
{
  ...
  rec_offsets = static_cast<ulint *>(mem_heap_alloc(heap, i * sizeof *rec_offsets));
  prev_rec_offsets = static_cast<ulint *>(mem_heap_alloc(heap, i * sizeof *prev_rec_offsets));
  ...
  if (n_diff_boundaries != NULL)
  {
    for (i = 0; i < n_uniq; i++)
    {
      n_diff_boundaries[i].erase(n_diff_boundaries[i].begin(), n_diff_boundaries[i].end());
    }
  }
  ...
  btr_pcur_open_at_index_side(true, index, BTR_SEARCH_TREE | BTR_ALREADY_S_LATCHED, &pcur, true, level, mtr);
  ...
  ut_ad(btr_pcur_get_rec(&pcur) == page_rec_get_next_const(page_get_infimum_rec(page)));
  ...
  ut_a((level == 0) == !(REC_INFO_MIN_REC_FLAG & rec_get_info_bits(btr_pcur_get_rec(&pcur), page_is_comp(page))));
  ...
  *total_pages = 0;
  ...
  for (; btr_pcur_is_on_user_rec(&pcur); btr_pcur_move_to_next_user_rec(&pcur, mtr))
  {
    ...
    if (level == 0 &&
        (srv_stats_include_delete_marked ? 0 : rec_get_deleted_flag(rec, page_is_comp(btr_pcur_get_page(&pcur)))))
    {
      if (rec_is_last_on_page && !prev_rec_is_copied && prev_rec != NULL)
      {
        /* copy prev_rec */

        prev_rec_offsets = rec_get_offsets(prev_rec, index, prev_rec_offsets, n_uniq, &heap);

        prev_rec = rec_copy_prefix_to_buf(prev_rec, index, rec_offs_n_fields(prev_rec_offsets), &prev_rec_buf,
                                          &prev_rec_buf_size);
    ...
    if (prev_rec != NULL)
    {
      ...
      cmp_rec_rec_with_match(rec, prev_rec, rec_offsets, prev_rec_offsets, index, false, false, &matched_fields);
    ...
    if (rec_is_last_on_page)
    {
      ...
      prev_rec = rec_copy_prefix_to_buf(rec, index, rec_offs_n_fields(rec_offsets), &prev_rec_buf, &prev_rec_buf_size);
      prev_rec_is_copied = true;
    }
    else
    {
  ...
  for (i = 0; i < n_uniq; i++)
  {
    DEBUG_PRINTF("    %s(): total recs: " UINT64PF ", total pages: " UINT64PF ", n_diff[%lu]: " UINT64PF "\n", __func__,
                 *total_recs, *total_pages, i, n_diff[i]);
...
UNIV_INLINE
ulint *dict_stats_scan_page(const rec_t **out_rec, ulint *offsets1, ulint *offsets2, const dict_index_t *index,
                            const page_t *page, ulint n_prefix, page_scan_method_t scan_method, ib_uint64_t *n_diff,
                            ib_uint64_t *n_external_pages)
{
  ...
  if (should_count_external_pages)
  {
    *n_external_pages += btr_rec_get_externally_stored_len(rec, offsets_rec);
  }
  ...
  *n_diff = 1;
  ...
  while (!page_rec_is_supremum(next_rec))
  {
    ...
    cmp_rec_rec_with_match(rec, next_rec, offsets_rec, offsets_next_rec, index, false, false, &matched_fields);
...
static void dict_stats_analyze_index_below_cur(const btr_cur_t *cur, ulint n_prefix, ib_uint64_t *n_diff,
                                               ib_uint64_t *n_external_pages)
{
  dict_index_t *index;
  buf_block_t *block;
  ...
  offsets1 = static_cast<ulint *>(mem_heap_alloc(heap, size * sizeof *offsets1));

  offsets2 = static_cast<ulint *>(mem_heap_alloc(heap, size * sizeof *offsets2));
  ...
  page_id_t page_id(dict_index_get_space(index), btr_node_ptr_get_child_page_no(rec, offsets_rec));
  ...
  *n_external_pages = 0;
  ...
  for (;;)
  {
    block = buf_page_get_gen(page_id, page_size, RW_S_LATCH, NULL /* no guessed block */, BUF_GET, __FILE__, __LINE__,
                             &mtr);
    ...
    offsets_rec =
        dict_stats_scan_page(&rec, offsets1, offsets2, index, page, n_prefix, QUIT_ON_FIRST_NON_BORING, n_diff, NULL);
  ...
  offsets_rec = dict_stats_scan_page(&rec, offsets1, offsets2, index, page, n_prefix,
                                     srv_stats_include_delete_marked ? COUNT_ALL_NON_BORING_INCLUDE_DEL_MARKED
...
static void dict_stats_analyze_index_for_n_prefix(dict_index_t *index, ulint n_prefix, const boundaries_t *boundaries,
                                                  n_diff_data_t *n_diff_data, mtr_t *mtr)
{
  ...
  btr_pcur_open_at_index_side(true, index, BTR_SEARCH_TREE | BTR_ALREADY_S_LATCHED, &pcur, true, n_diff_data->level,
                              mtr);
  btr_pcur_move_to_next_on_page(&pcur);
  ...
  ut_a(rec_get_info_bits(first_rec, page_is_comp(page)) & REC_INFO_MIN_REC_FLAG);

  const ib_uint64_t last_idx_on_level = boundaries->at(static_cast<unsigned>(n_diff_data->n_diff_on_level - 1));
  ...
  for (i = 0; i < n_diff_data->n_leaf_pages_to_analyze; i++)
  {
    ...
    const ib_uint64_t dive_below_idx = boundaries->at(static_cast<unsigned>(left + rnd));
    ...
    dict_stats_analyze_index_below_cur(btr_pcur_get_btr_cur(&pcur), n_prefix, &n_diff_on_leaf_page, &n_external_pages);
...
UNIV_INLINE
void dict_stats_index_set_n_diff(const n_diff_data_t *n_diff_data, dict_index_t *index)
{
  for (ulint n_prefix = dict_index_get_n_unique(index); n_prefix >= 1; n_prefix--)
  {
    ...
    index->stat_n_diff_key_vals[n_prefix - 1] = n_ordinary_leaf_pages
    ...
    DEBUG_PRINTF("    %s(): n_diff=" UINT64PF
                 ...
                 __func__, index->stat_n_diff_key_vals[n_prefix - 1], n_prefix, index->stat_n_leaf_pages,
                 data->n_diff_on_level, data->n_recs_on_level, data->n_diff_all_analyzed_pages,
    ...
    dict_index_t *index) /*!< in/out: index to analyze */
{
  ...
  ut_ad(!(index->table->stats_latch_created) || !rw_lock_own(index->table->stats_latch, RW_X_LATCH));

  DBUG_PRINT("info", ("index: %s, online status: %d", index->name(), dict_index_get_online_status(index)));
  ...
  if (root_level == 0 || N_SAMPLE_PAGES(index) * n_uniq > index->stat_n_leaf_pages)
  {
    if (root_level == 0)
    {
      DEBUG_PRINTF(
    ...
    dict_stats_analyze_index_level(index, 0 /* leaf level */, index->stat_n_diff_key_vals, &total_recs, &total_pages,
                                   NULL /* boundaries not needed */, &mtr);
  ...
  ib_uint64_t *n_diff_on_level = UT_NEW_ARRAY(ib_uint64_t, n_uniq, mem_key_dict_stats_n_diff_on_level);
  ...
  for (n_prefix = n_uniq; n_prefix >= 1; n_prefix--)
  {
    ...
    if (level_is_analyzed && (n_diff_on_level[n_prefix - 1] >= N_DIFF_REQUIRED(index) || level == 1))
    {
      goto found_level;
    }
    ...
    for (;;)
    {
      ...
      if (level == 1 || n_diff_on_level[n_prefix - 1] >= N_DIFF_REQUIRED(index))
      {
  ...
  found_level:
    ...
    DEBUG_PRINTF("  %s(): found level %lu that has " UINT64PF " distinct records for n_prefix=%lu\n", __func__, level,
                 n_diff_on_level[n_prefix - 1], n_prefix);
    ...
    data->n_leaf_pages_to_analyze = std::min(N_SAMPLE_PAGES(index), n_diff_on_level[n_prefix - 1]);

    /* pick some records from this level and dive below them for
    the given n_prefix */

    dict_stats_analyze_index_for_n_prefix(index, n_prefix, &n_diff_boundaries[n_prefix - 1], data, &mtr);
  }
    ...
    dict_table_t *table) /*!< in/out: table */
{
  ...
  if (index == NULL || dict_index_is_corrupted(index) || (index->type | DICT_UNIQUE) != (DICT_CLUSTERED | DICT_UNIQUE))
  {
    /* Table definition is corrupt */
    dict_stats_empty_table(table);
    dict_table_analyze_index_unlock(table);
  ...
  for (index = dict_table_get_next_index(index); index != NULL; index = dict_table_get_next_index(index))
  {
    ut_ad(!dict_index_is_ibuf(index));
    ...
    stat_sum_of_other_index_sizes_tmp += index->stat_index_size;
  }
...
@return DB_SUCCESS or error code */
static dberr_t dict_stats_save_index_stat(dict_index_t *index, lint last_update, const char *stat_name,
                                          ib_uint64_t stat_value, ib_uint64_t *sample_size,
                                          const char *stat_description, trx_t *trx)
{
  dberr_t ret;
  pars_info_t *pinfo;
  ...
  dict_fs2utf8(index->table->name.m_name, db_utf8, sizeof(db_utf8), table_utf8, sizeof(table_utf8));
  ...
  {
    pars_info_add_literal(pinfo, "sample_size", NULL, UNIV_SQL_NULL, DATA_FIXBINARY, 0);
  }
  ...
  if (ret != DB_SUCCESS)
  {
    ib::error() << "Cannot save index statistics for table " << index->table->name << ", index " << index->name
...
are saved
@return DB_SUCCESS or error code */
static dberr_t dict_stats_save(dict_table_t *table_orig, const index_id_t *only_for_index)
{
  pars_info_t *pinfo;
  lint now;
  dberr_t ret;
  ...
  pars_info_add_ull_literal(pinfo, "clustered_index_size", table->stat_clustered_index_size);
  pars_info_add_ull_literal(pinfo, "sum_of_other_index_sizes", table->stat_sum_of_other_index_sizes);
  ...
  if (ret != DB_SUCCESS)
  {
    ib::error() << "Cannot save table statistics for table " << table->name << ": " << ut_strerr(ret);
  ...
  dict_index_t *index;
  index_map_t indexes((ut_strcmp_functor()), index_map_t_allocator(mem_key_dict_stats_index_map_t));
  ...
  for (it = indexes.begin(); it != indexes.end(); ++it)
  {
    ...
    for (ulint i = 0; i < index->n_uniq; i++)
    {
      ...
      ut_snprintf(stat_name, sizeof(stat_name), "n_diff_pfx%02lu", i + 1);

      /* craft a string that contains the column names */
      ut_snprintf(stat_description, sizeof(stat_description), "%s", index->fields[0].name());
      for (j = 1; j <= i; j++)
      {
        size_t len;
        ...
        ut_snprintf(stat_description + len, sizeof(stat_description) - len, ",%s", index->fields[j].name());
      }

      ret = dict_stats_save_index_stat(index, now, stat_name, index->stat_n_diff_key_vals[i],
    ...
    ret = dict_stats_save_index_stat(index, now, "n_leaf_pages", index->stat_n_leaf_pages, NULL,
    ...
    ret = dict_stats_save_index_stat(index, now, "size", index->stat_index_size, NULL,
                                     "Number of pages "
                                     "in the index",
                                     trx);
    if (ret != DB_SUCCESS)
    ...
    void *table_void) /*!< out: table */
{
  ...
  for (cnode = static_cast<que_common_t *>(node->select_list), i = 0; cnode != NULL;
       ...
       cnode = static_cast<que_common_t *>(que_node_get_next(cnode)), i++)
  {
    ...
    switch (i)
    {
      case 0: /* mysql.innodb_index_stats.index_name */
        ...
        for (index = dict_table_get_first_index(table); index != NULL; index = dict_table_get_next_index(index))
        {
          if (index->is_committed() && strlen(index->name) == len && memcmp(index->name, data, len) == 0)
          {
...
#define PFX_LEN 10
      ...
      && native_strncasecmp("size", stat_name, stat_name_len) == 0)
  {
    ...
    arg->stats_were_modified = true;
           ...
           && native_strncasecmp("n_leaf_pages", stat_name, stat_name_len) == 0)
  {
           ...
           && native_strncasecmp(PFX, stat_name, PFX_LEN) == 0)
  {
    ...
    if (stat_name_len != PFX_LEN + 2 || num_ptr[0] < '0' || num_ptr[0] > '9' || num_ptr[1] < '0' || num_ptr[1] > '9')
    {
      char db_utf8[MAX_DB_UTF8_LEN];
      char table_utf8[MAX_TABLE_UTF8_LEN];
      ...
      out << "Ignoring strange row from " << INDEX_STATS_NAME_PRINT
          << " WHERE"
             " database_name = '"
          << db_utf8 << "' AND table_name = '" << table_utf8 << "' AND index_name = '" << index->name()
          << "' AND stat_name = '";
    ...
    dict_table_t *table) /*!< in/out: table */
{
  ...
  pars_info_bind_function(pinfo, "fetch_table_stats_step", dict_stats_fetch_table_stats_step, table);

  index_fetch_arg.table = table;
  index_fetch_arg.stats_were_modified = false;
  pars_info_bind_function(pinfo, "fetch_index_stats_step", dict_stats_fetch_index_stats_step, &index_fetch_arg);
...
storage */
{
  ...
  {
    ib::warn() << "Cannot calculate statistics for table " << table->name << " because the .ibd file is missing. "
               << TROUBLESHOOTING_MSG;
    ...
    case DICT_STATS_FETCH_ONLY_IF_NOT_IN_MEMORY:
      ...
      if (!dict_stats_persistent_storage_check(false))
      {
        ...
        ib::error() << "Fetch of persistent statistics"
                       " requested for table "
                    << table->name << " but the required system tables " << TABLE_STATS_NAME_PRINT << " and "
      ...
      switch (err)
      {
        ...
        case DB_STATS_DO_NOT_EXIST:
          ...
          ib::info() << "Trying to use table " << table->name
                     << " which has persistent statistics enabled,"
                        ...
                        " statistics do not exist in " TABLE_STATS_NAME_PRINT " and " INDEX_STATS_NAME_PRINT
        ...
        default:
          ...
          ib::error() << "Error fetching persistent statistics"
                         " for table "
                      << table->name << " from " TABLE_STATS_NAME_PRINT " and " INDEX_STATS_NAME_PRINT ": "
    ...
    ulint errstr_sz)          /*!< in: size of the errstr buffer */
{
  ...
  dict_fs2utf8(db_and_table, db_utf8, sizeof(db_utf8), table_utf8, sizeof(table_utf8));
  ...
  if (ret != DB_SUCCESS)
  {
    ut_snprintf(errstr, errstr_sz,
                ...
                " table_name = '%s' AND"
                " index_name = '%s';",
                iname, INDEX_STATS_NAME_PRINT, (ret == DB_LOCK_WAIT_TIMEOUT ? " because the rows are locked" : ""),
    ...
    ulint errstr_sz)          /*!< in: size of errstr buffer */
{
  ...
  if (strcmp(db_and_table, TABLE_STATS_NAME) == 0 || strcmp(db_and_table, INDEX_STATS_NAME) == 0)
  {
    return (DB_SUCCESS);
  }
    ...
    size_t errstr_sz)     /*!< in: errstr size */
{
  ...
  /* skip innodb_table_stats and innodb_index_stats themselves */
  if (strcmp(old_name, TABLE_STATS_NAME) == 0 || strcmp(old_name, INDEX_STATS_NAME) == 0 ||
      strcmp(new_name, TABLE_STATS_NAME) == 0 || strcmp(new_name, INDEX_STATS_NAME) == 0)
  {
    return (DB_SUCCESS);
  }

  dict_fs2utf8(old_name, old_db_utf8, sizeof(old_db_utf8), old_table_utf8, sizeof(old_table_utf8));

  dict_fs2utf8(new_name, new_db_utf8, sizeof(new_db_utf8), new_table_utf8, sizeof(new_table_utf8));
  ...
  do
  {
    ...
    ret = dict_stats_rename_table_in_table_stats(old_db_utf8, old_table_utf8, new_db_utf8, new_table_utf8);
    ...
    DBUG_EXECUTE_IF("rename_stats", mutex_exit(&dict_sys->mutex); rw_lock_x_unlock(dict_operation_lock);
                    os_thread_sleep(20000000); DEBUG_SYNC_C("rename_stats"); rw_lock_x_lock(dict_operation_lock);
                    mutex_enter(&dict_sys->mutex););
  ...
  } while ((ret == DB_DEADLOCK || ret == DB_DUPLICATE_KEY || ret == DB_LOCK_WAIT_TIMEOUT) && n_attempts < 5);
  ...
  if (ret != DB_SUCCESS)
  {
    ut_snprintf(errstr, errstr_sz,
                ...
                old_db_utf8, old_table_utf8, new_db_utf8, new_table_utf8, TABLE_STATS_NAME_PRINT, ut_strerr(ret),

                TABLE_STATS_NAME_PRINT, new_db_utf8, new_table_utf8, old_db_utf8, old_table_utf8);
    mutex_exit(&dict_sys->mutex);
    rw_lock_x_unlock(dict_operation_lock);
    return (ret);
  }
  ...
  if (ret != DB_SUCCESS)
  {
    ut_snprintf(errstr, errstr_sz,
                ...
                old_db_utf8, old_table_utf8, new_db_utf8, new_table_utf8, INDEX_STATS_NAME_PRINT, ut_strerr(ret),

                INDEX_STATS_NAME_PRINT, new_db_utf8, new_table_utf8, old_db_utf8, old_table_utf8);
  }
    ...
    const char *new_index_name) /*!< in: new index name */
{
  ...
  dict_fs2utf8(table->name.m_name, dbname_utf8, sizeof(dbname_utf8), tablename_utf8, sizeof(tablename_utf8));
...
void test_dict_table_schema_check()
{
  /*
  CREATE TABLE tcheck (
          c01 VARCHAR(123),
  ...
  */
  ...
  dict_col_meta_t columns[] = {{"c01", DATA_VARCHAR, 0, 123},
                               {"c02", DATA_INT, 0, 4},
  ...
  dict_table_schema_t schema = {"test/tcheck", 0 /* will be set individually for each test below */, columns};
  char errstr[512];
  ...
  schema.n_cols = 7;
  if (dict_table_schema_check(&schema, errstr, sizeof(errstr)) == DB_SUCCESS)
  {
...
void test_dict_stats_save()
{
  ...
  printf(
      ...
      "\n",
      TABLE_STATS_NAME_PRINT, TEST_DATABASE_NAME, TEST_TABLE_NAME, TEST_N_ROWS, TEST_CLUSTERED_INDEX_SIZE,
  ...
  printf(
      ...
      ");\n"
      "\n",
      INDEX_STATS_NAME_PRINT, TEST_DATABASE_NAME, TEST_TABLE_NAME, TEST_IDX1_NAME, TEST_IDX1_INDEX_SIZE,
      TEST_IDX1_N_LEAF_PAGES, TEST_IDX1_N_DIFF1, TEST_IDX1_N_DIFF1_SAMPLE_SIZE, TEST_IDX1_COL1_NAME);
  ...
  printf(
      ...
      TEST_IDX2_N_LEAF_PAGES, TEST_IDX2_N_DIFF1, TEST_IDX2_N_DIFF1_SAMPLE_SIZE, TEST_IDX2_COL1_NAME, TEST_IDX2_N_DIFF2,
      TEST_IDX2_N_DIFF2_SAMPLE_SIZE, TEST_IDX2_COL1_NAME, TEST_IDX2_COL2_NAME, TEST_IDX2_N_DIFF3,
      TEST_IDX2_N_DIFF3_SAMPLE_SIZE, TEST_IDX2_COL1_NAME, TEST_IDX2_COL2_NAME, TEST_IDX2_COL3_NAME, TEST_IDX2_N_DIFF4,
      TEST_IDX2_N_DIFF4_SAMPLE_SIZE, TEST_IDX2_COL1_NAME, TEST_IDX2_COL2_NAME, TEST_IDX2_COL3_NAME,
      TEST_IDX2_COL4_NAME);
}
/* @} */
