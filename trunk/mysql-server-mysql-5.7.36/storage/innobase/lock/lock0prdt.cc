...
{
  lock_prdt_t *prdt =
      reinterpret_cast<lock_prdt_t *>(&((reinterpret_cast<byte *>(const_cast<lock_t *>(&lock[1])))[UNIV_WORD_SIZE]));
    ...
    const trx_t *trx)         /*!< in: transaction */
{
  ...
  for (lock = lock_rec_get_first(lock_hash_get(type_mode), block, PRDT_HEAPNO); lock != NULL;
       lock = lock_rec_get_next(PRDT_HEAPNO, lock))
  {
    ut_ad(lock->type_mode & (LOCK_PREDICATE | LOCK_PRDT_PAGE));

    if (lock->trx == trx && !(lock->type_mode & LOCK_INSERT_INTENTION) && !lock_get_wait(lock) &&
        lock_mode_stronger_or_eq(lock_get_mode(lock), static_cast<lock_mode>(precise_mode & LOCK_MODE_MASK)))
    {
      ...
      then we find a lock */
      if (cur_prdt->op == prdt->op && lock_prdt_consistent(cur_prdt, prdt, 0))
      {
        return (lock);
      }
    }
    ...
    const trx_t *trx)         /*!< in: our transaction */
{
  ...
  for (const lock_t *lock = lock_rec_get_first(lock_hash_get(mode), block, PRDT_HEAPNO); lock != NULL;
    ...
    const trx_t *trx)         /*!< in: transaction */
{
  ...
  for (lock = lock_rec_get_first_on_page(lock_hash_get(type_mode), block); lock != NULL;
                         ...
                         Rectangle */
{
  ...
  const lock_t *wait_for = lock_prdt_other_has_conflicting(mode, block, prdt, trx);
    ...
    ulint page_no)            /*!< in: parent page number */
{
  ...
  for (lock = lock_rec_get_first_on_page_addr(lock_sys->prdt_hash, space, page_no); lock;
       lock = lock_rec_get_next_on_page(lock))
  {
    ...
    if (!(lock->type_mode & LOCK_PREDICATE) || (lock->type_mode & LOCK_MODE_MASK) == LOCK_X)
    {
      continue;
    }
    ...
    if (!lock_prdt_consistent(lock_prdt, left_prdt, op) &&
        !lock_prdt_find_on_page(lock->type_mode, left_block, lock_prdt, lock->trx))
    {
      lock_prdt_add_to_queue(lock->type_mode, left_block, lock->index, lock->trx, lock_prdt, FALSE);
    }

    if (!lock_prdt_consistent(lock_prdt, right_prdt, op) &&
        !lock_prdt_find_on_page(lock->type_mode, right_block, lock_prdt, lock->trx))
    {
      lock_prdt_add_to_queue(lock->type_mode, right_block, lock->index, lock->trx, lock_prdt, FALSE);
    }
  }
                            ...
                            LOCK_PRDT_PAGE */
{
  ...
  for (lock = lock_rec_get_first_on_page_addr(lock_hash_get(type_mode), space, page_no); lock;
       lock = lock_rec_get_next_on_page(lock))
  {
    ut_ad(lock);
    ...
    if (lock->type_mode & LOCK_PRDT_PAGE)
    {
      ...
      lock_prdt_add_to_queue(lock->type_mode, new_block, lock->index, lock->trx, NULL, TRUE);
    ...
    if (lock_prdt_consistent(lock_prdt, prdt, op))
    {
      if (!lock_prdt_consistent(lock_prdt, new_prdt, op))
      {
        ...
        trx_mutex_enter(lock->trx);
        lock_prdt_add_to_queue(lock->type_mode, new_block, lock->index, lock->trx, lock_prdt, TRUE);
        trx_mutex_exit(lock->trx);
      }
    }
...
void lock_prdt_update_split(
    ...
    ulint page_no)          /*!< in: page number */
{
  lock_prdt_update_split_low(block, new_block, prdt, new_prdt, space, page_no, LOCK_PREDICATE);

  lock_prdt_update_split_low(block, new_block, NULL, NULL, space, page_no, LOCK_PRDT_PAGE);
}
    ...
    mtr_t *mtr)     /*!< in/out: mini-transaction */
{
  ...
  hash_table_t *hash = type_mode == LOCK_PREDICATE ? lock_sys->prdt_hash : lock_sys->prdt_page_hash;
  ...
  else
  {
    ...
    if (lock_rec_get_next_on_page(lock) || lock->trx != trx || lock->type_mode != (LOCK_REC | prdt_mode) ||
        lock_rec_get_n_bits(lock) == 0 ||
        ((type_mode & LOCK_PREDICATE) && (!lock_prdt_consistent(lock_get_prdt_from_lock(lock), prdt, 0))))
    {
    ...
    que_thr_t *thr)      /*!< in: query thread */
{
  ...
  const lock_t *lock = lock_rec_get_first_on_page_addr(lock_sys->prdt_page_hash, space, page_no);
...
bool lock_test_prdt_page_lock(const trx_t *trx, ulint space, ulint page_no)
{
  lock_t *lock;

  lock_mutex_enter();

  lock = lock_rec_get_first_on_page_addr(lock_sys->prdt_page_hash, space, page_no);
                                 ...
                                 the donating record */
{
  ...
  for (lock = lock_rec_get_first(lock_sys->prdt_hash, donator, PRDT_HEAPNO); lock != NULL;
       lock = lock_rec_get_next(PRDT_HEAPNO, lock))
  {
    ...
    lock_prdt_add_to_queue(type_mode, receiver, lock->index, lock->trx, lock_prdt, FALSE);
  }
