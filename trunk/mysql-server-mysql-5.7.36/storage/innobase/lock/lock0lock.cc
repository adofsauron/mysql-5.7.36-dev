...
class DeadlockChecker
{
 ...
 private:
  ...
  DeadlockChecker(const trx_t *trx, const lock_t *wait_lock, ib_uint64_t mark_start)
      : m_cost(), m_start(trx), m_too_deep(), m_wait_lock(wait_lock), m_mark_start(mark_start), m_n_elems()
  {
  }
  ...
  {
    return (m_n_elems > LOCK_MAX_DEPTH_IN_DEADLOCK_CHECK || m_cost > LOCK_MAX_N_STEPS_IN_DEADLOCK_CHECK);
  }
  ...
  {
    ut_ad((lock_get_type_low(lock) & LOCK_REC) || (lock_get_type_low(lock) & LOCK_TABLE));

    ut_ad(((lock_get_type_low(lock) & LOCK_TABLE) != 0) == (heap_no == ULINT_UNDEFINED));
  ...
  bool is_visited(const lock_t *lock) const { return (lock->trx->lock.deadlock_mark > m_mark_start); }
...
{
  ib::error() << "Transaction id " << trx_id << " associated with record" << rec_offsets_print(rec, offsets)
              << " in index " << index->name << " of table " << index->table->name
              << " is greater than the global counter " << max_trx_id << "! The table is corrupted.";
}
...
{
  return (lock_rec_fold(lock->un_member.rec_lock.space, lock->un_member.rec_lock.page_no));
}
...
void lock_sys_resize(ulint n_cells)
{
  ...
  lock_sys->prdt_page_hash = hash_create(n_cells);
  HASH_MIGRATE(old_hash, lock_sys->prdt_page_hash, lock_t, hash, lock_rec_lock_fold);
  hash_table_free(old_hash);
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; ++i)
  {
    ...
    while (bpage != NULL)
    {
      if (buf_page_get_state(bpage) == BUF_BLOCK_FILE_PAGE)
      {
        ...
        block->lock_hash_val = lock_rec_hash(bpage->id.space(), bpage->id.page_no());
      }
    ...
    lock_mode *mode)    /*!< out: lock mode of the source table */
{
  ...
  *mode = LOCK_NONE;
  ...
  for (lock = UT_LIST_GET_FIRST(trx->lock.trx_locks); lock != NULL; lock = UT_LIST_GET_NEXT(trx_locks, lock))
  {
    ...
    const trx_t *trx)          /*!< in: transaction */
{
  ...
  for (lock = UT_LIST_GET_FIRST(table->locks); lock != NULL; lock = UT_LIST_GET_NEXT(locks, &lock->un_member.tab_lock))
  {
    if (lock->trx != trx)
    {
                         ...
                         locks are record locks */
{
  ...
  if (lock1->trx != lock2->trx && !lock_mode_compatible(lock_get_mode(lock1), lock_get_mode(lock2)))
  {
    if (lock_get_type_low(lock1) == LOCK_REC)
    {
      ...
      {
        return (lock_prdt_has_to_wait(lock1->trx, lock1->type_mode, lock_get_prdt_from_lock(lock1), lock2));
      }
      else
      {
        return (lock_rec_has_to_wait(lock1->trx, lock1->type_mode, lock2, lock_rec_get_nth_bit(lock1, true)));
      }
    }
    ...
    const trx_t *trx)         /*!< in: transaction */
{
  ...
  for (lock = lock_rec_get_first(lock_sys->rec_hash, block, heap_no); lock != NULL;
                              ...
                              are taken into account */
{
  ...
  for (const lock_t *lock = lock_rec_get_first(lock_sys->rec_hash, block, heap_no); lock != NULL;
       lock = lock_rec_get_next_const(heap_no, lock))
  {
    if (lock->trx != trx && !lock_rec_get_gap(lock) && (wait || !lock_get_wait(lock)) &&
                              ...
                              containing the record */
{
  ...
  if (trx_t *impl_trx = trx_rw_is_active(trx->id, NULL, false))
  {
    ...
    for (trx_t *t = UT_LIST_GET_FIRST(trx_sys->rw_trx_list); t != NULL; t = UT_LIST_GET_NEXT(trx_list, t))
    {
      lock_t *expl_lock = lock_rec_has_expl(precise_mode, block, heap_no, t);
...
bool RecLock::is_on_row(const lock_t *lock) const
{
  ...
  return (other.space == m_rec_id.m_space_id && other.page_no == m_rec_id.m_page_no &&
  ...
  switch (trx_get_dict_operation(m_trx))
  {
    ...
    case TRX_DICT_OP_INDEX:
      ib::error() << "A record lock wait happens in a dictionary"
                     " operation. index "
                  << m_index->name << " of table " << m_index->table->name << ". " << BUG_REPORT_MSG;
      ut_ad(0);
  }
...
lock_t *RecLock::create(trx_t *trx, bool owns_trx_mutex, bool add_to_hash, const lock_prdt_t *prdt)
{
  ut_ad(lock_mutex_own());
...
transaction mutex */
{
  ...
  ut_ad(dict_index_is_clust(index) || dict_index_get_online_status(index) != ONLINE_INDEX_CREATION);
  ...
  if (!(type_mode & (LOCK_WAIT | LOCK_GAP)))
  {
    ...
    const lock_t *other_lock = lock_rec_other_has_expl_req(mode, block, false, heap_no, trx);
    ut_a(!other_lock);
  }
#endif /* UNIV_DEBUG */
  ...
  for (first_lock = lock = lock_rec_get_first_on_page(hash, block); lock != NULL;
    ...
    que_thr_t *thr)           /*!< in: query thread */
{
  ...
  else
  {
    ...
    if (lock_rec_get_next_on_page(lock) || lock->trx != trx || lock->type_mode != (mode | LOCK_REC) ||
    ...
    const lock_t *wait_lock) /*!< in: waiting record lock */
{
  ...
  for (lock = lock_rec_get_first_on_page_addr(hash, space, page_no); lock != wait_lock;
       lock = lock_rec_get_next_on_page_const(lock))
  {
    const byte *p = (const byte *)&lock[1];

    if (heap_no < lock_rec_get_n_bits(lock) && (p[bit_offset] & bit_mask) && lock_has_to_wait(wait_lock, lock))
    {
      return (lock);
    }
  }
    ...
    lock_t *lock) /*!< in/out: waiting lock request */
{
  ...
  DBUG_PRINT("ib_lock", ("wait for trx " TRX_ID_FMT " ends", trx_get_id_for_print(lock->trx)));
...
#ifdef UNIV_DEBUG
    ib::info() << "Granting High Priority Transaction (ID): " << lock->trx->id << " the lock jumping over"
...
bool RecLock::lock_add_priority(lock_t *lock, const lock_t *conflict_lock, bool *high_priority)
{
  ut_ad(high_priority);
  ...
  *high_priority = false;
  ...
  HASH_SEARCH(hash, lock_hash, m_rec_id.fold(), lock_t *, lock_head, ut_ad(lock_head->is_record_lock()), true);
...
void RecLock::make_trx_hit_list(lock_t *lock, const lock_t *conflict_lock)
{
  ...
  for (next = conflict_lock; next != NULL; next = next->hash)
  {
    ...
    if (trx == lock->trx || !is_on_row(next) || next->is_waiting() || trx->read_only || trx->mysql_thd == NULL ||
    ...
    if (trx_is_high_priority(trx) || (trx->in_innodb & TRX_FORCE_ROLLBACK_DISABLE) != 0 || trx->abort)
    {
      trx_mutex_exit(trx);
      continue;
    }
...
#ifdef UNIV_DEBUG
      ib::info() << "High Priority Transaction (ID): " << lock->trx->id << " waking up blocking"
...
static void lock_rec_grant(lock_t *in_lock)
{
  ...
  for (lock = lock_rec_get_first_on_page_addr(lock_hash, space, page_no); lock != NULL;
       lock = lock_rec_get_next_on_page(lock))
  {
                     ...
                     in this lock object are removed */
{
  ...
  HASH_DELETE(lock_t, hash, lock_hash_get(in_lock->type_mode), lock_rec_fold(space, page_no), in_lock);
...
static void lock_rec_free_all_from_discard_page_low(
    /*====================================*/
    ulint space, ulint page_no, hash_table_t *lock_hash)
{
  lock_t *lock;
  lock_t *next_lock;
    ...
    const buf_block_t *block) /*!< in: page to be discarded */
{
  ...
  lock_rec_free_all_from_discard_page_low(space, page_no, lock_sys->prdt_page_hash);
}
    ...
    ulint heap_no)            /*!< in: heap number of record */
{
  ...
  for (lock = lock_rec_get_first(hash, block, heap_no); lock != NULL; lock = lock_rec_get_next(heap_no, lock))
  {
    ...
    ulint heap_no)            /*!< in: heap number of record */
{
  ...
  lock_rec_reset_and_release_wait_low(lock_sys->prdt_hash, block, PAGE_HEAP_NO_INFIMUM);
  lock_rec_reset_and_release_wait_low(lock_sys->prdt_page_hash, block, PAGE_HEAP_NO_INFIMUM);
}
                                 ...
                                 which gives the locks */
{
  ...
  ut_ad(lock_rec_get_first(lock_hash, receiver, receiver_heap_no) == NULL || lock_hash == lock_sys->prdt_hash ||
        lock_hash == lock_sys->prdt_page_hash);

  for (lock = lock_rec_get_first(lock_hash, donator, donator_heap_no); lock != NULL;
       lock = lock_rec_get_next(donator_heap_no, lock))
  {
    ...
    lock_rec_add_to_queue(type_mode, receiver, receiver_heap_no, lock->index, lock->trx, FALSE);
  }

  ut_ad(lock_rec_get_first(lock_sys->rec_hash, donator, donator_heap_no) == NULL);
}
...
static void lock_move_granted_locks_to_front(UT_LIST_BASE_NODE_T(lock_t) & lock_list)
{
  ...
  for (lock = UT_LIST_GET_FIRST(lock_list); lock; lock = UT_LIST_GET_NEXT(trx_locks, lock))
  {
                               ...
                               reorganized page */
{
  ...
  DBUG_EXECUTE_IF("do_lock_reverse_page_reorganize", UT_LIST_REVERSE(old_locks););
  ...
  for (lock = UT_LIST_GET_FIRST(old_locks); lock; lock = UT_LIST_GET_NEXT(trx_locks, lock))
  {
    ...
    for (;;)
    {
      ...
      if (old_heap_no < lock->un_member.rec_lock.n_bits && lock_rec_reset_nth_bit(lock, old_heap_no))
      {
        /* NOTE that the old lock bitmap could be too
        small for the new heap number! */

        lock_rec_add_to_queue(lock->type_mode, block, new_heap_no, lock->index, lock->trx, FALSE);
      }
...
#ifdef UNIV_DEBUG
    {
      ...
      {
        ib::fatal() << "lock_move_reorganize_page(): " << i << " not moved in " << (void *)lock;
      }
    }
#endif /* UNIV_DEBUG */
  }
                                  ...
                                  is the first record moved */
{
  ...
  for (lock = lock_rec_get_first_on_page(lock_sys->rec_hash, block); lock; lock = lock_rec_get_next_on_page(lock))
  {
    ...
    if (comp)
    {
      ...
      rec2 = page_rec_get_next_low(buf_block_get_frame(new_block) + PAGE_NEW_INFIMUM, TRUE);
    }
    else
    {
      ...
      rec2 = page_rec_get_next_low(buf_block_get_frame(new_block) + PAGE_OLD_INFIMUM, FALSE);
    }
    ...
    for (;;)
    {
      ...
      if (rec1_heap_no < lock->un_member.rec_lock.n_bits && lock_rec_reset_nth_bit(lock, rec1_heap_no))
      {
        if (type_mode & LOCK_WAIT)
        {
          lock_reset_lock_and_trx_wait(lock);
        ...
        lock_rec_add_to_queue(type_mode, new_block, rec2_heap_no, lock->index, lock->trx, FALSE);
      }
    }
  }
                                  ...
                                  were copied */
{
  ...
  for (lock = lock_rec_get_first_on_page(lock_sys->rec_hash, block); lock; lock = lock_rec_get_next_on_page(lock))
  {
    ...
    if (comp)
    {
      rec1 = page_rec_get_next_low(buf_block_get_frame(block) + PAGE_NEW_INFIMUM, TRUE);
      rec2 = page_rec_get_next_low(old_end, TRUE);
    }
    else
    {
      rec1 = page_rec_get_next_low(buf_block_get_frame(block) + PAGE_OLD_INFIMUM, FALSE);
    ...
    if (page_rec_is_supremum(rec))
    {
      ...
      for (i = PAGE_HEAP_NO_USER_LOW; i < lock_rec_get_n_bits(lock); i++)
      {
        if (lock_rec_get_nth_bit(lock, i))
        {
          ib::fatal() << "lock_move_rec_list_start():" << i << " not moved in " << (void *)lock;
        }
      }
    }
#endif /* UNIV_DEBUG */
                                    ...
                                    discarded */
{
  ...
  lock_rec_reset_and_release_wait_low(lock_sys->rec_hash, left_block, PAGE_HEAP_NO_SUPREMUM);
  ...
  ut_ad(lock_rec_get_first_on_page_addr(lock_sys->prdt_page_hash, space, page_no) == NULL);
#endif /* UNIV_DEBUG */
                                    ...
                                    which will be discarded */
{
  ...
  lock_t *lock_test = lock_rec_get_first_on_page_addr(lock_sys->prdt_page_hash, space, page_no);
  ut_ad(!lock_test);
#endif /* UNIV_DEBUG */
...
{
  ut_list_node<lock_t> &operator()(lock_t &elem) { return (elem.un_member.tab_lock.locks); }
};
    ...
    trx_t *trx)          /*!< in: trx */
{
  ...
  {
    lock = static_cast<lock_t *>(mem_heap_alloc(trx->lock.lock_heap, sizeof(*lock)));
  }
    ...
    lock_mode mode)            /*!< in: lock mode */
{
  ...
  for (lock = UT_LIST_GET_LAST(table->locks); lock != NULL; lock = UT_LIST_GET_PREV(un_member.tab_lock.locks, lock))
  {
    ...
    que_thr_t *thr)      /*!< in: query thread */
{
  ...
  locking overhead */
  if ((flags & BTR_NO_LOCKING_FLAG) || srv_read_only_mode || dict_table_is_temporary(table))
  {
    return (DB_SUCCESS);
  }
                     ...
                     they are now qualified to it */
{
  ...
  for (/* No op */; lock != NULL; lock = UT_LIST_GET_NEXT(un_member.tab_lock.locks, lock))
  {
...
dberr_t lock_table_for_trx(dict_table_t *table, trx_t *trx, enum lock_mode mode)
{
  ...
  thr = static_cast<que_thr_t *>(que_fork_get_first_thr(static_cast<que_fork_t *>(que_node_get_parent(thr))));
    ...
    lock_mode lock_mode)      /*!< in: LOCK_S or LOCK_X */
{
  ...
  for (lock = first_lock; lock != NULL; lock = lock_rec_get_next(heap_no, lock))
  {
  ...
  {
    ib::error err;
    err << "Unlock row could not find a " << lock_mode << " mode lock on the record. Current statement: ";
...
/*********************************************************************/ /**
 ...
 code correctly. */
static void lock_check_dict_lock(
    /*==================*/
    const lock_t *lock) /*!< in: lock to check */
{
  if (lock_get_type_low(lock) == LOCK_REC)
  {
    ...
    if (lock->index->table->id < DICT_HDR_FIRST_ID && lock_get_mode(lock) == LOCK_X)
    {
  ...
  else
  {
    ...
    correctly if it did. */
    if (table->id < DICT_HDR_FIRST_ID && (lock_get_mode(lock) == LOCK_X || lock_get_mode(lock) == LOCK_IX))
    {
      ut_ad(lock->trx->dict_operation != TRX_DICT_OP_NONE);
    }
  }
...
void lock_trx_release_read_locks(trx_t *trx, bool only_gap)
{
  ...
  while (lock != NULL)
  {
    ...
    if (!lock->is_record_lock() || lock->is_insert_intention() || lock->is_predicate())
    {
      lock = next_lock;
      continue;
    }
    ...
    trx_t *trx) /*!< in/out: transaction */
{
  ...
  for (lock = UT_LIST_GET_LAST(trx->lock.trx_locks); lock != NULL; lock = UT_LIST_GET_LAST(trx->lock.trx_locks))
  {
    ...
    else
    {
      ...
      if (lock_get_mode(lock) != LOCK_IS && trx->undo_no != 0)
      {
                                   ...
                                   table S and X locks */
{
  ...
  for (lock = UT_LIST_GET_LAST(trx->lock.trx_locks); lock != NULL; lock = prev_lock)
  {
                         ...
                         table itself */
{
  ...
  for (trx_t *trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list); trx != NULL; trx = UT_LIST_GET_NEXT(trx_list, trx))
  {
    ...
    for (lock_t *lock = UT_LIST_GET_FIRST(trx->lock.trx_locks); lock != NULL; lock = next_lock)
    {
      ut_a(lock->trx == trx);
                                   ...
                                   table S and X locks */
{
  ...
  if (!lock_sys->rollback_complete && lock_remove_recovered_trx_record_locks(table) == 0)
  {
    ...
    const lock_t *lock) /*!< in: table type lock */
{
  ...
  ut_print_name(file, lock->trx, lock->un_member.tab_lock.table->name.m_name);
    ...
    const lock_t *lock) /*!< in: record type lock */
{
  ...
  fprintf(file,
          ...
          "index %s of table ",
          (ulong)space, (ulong)page_no, (ulong)lock_rec_get_n_bits(lock), lock->index->name());
...
/*======================*/
{
  ...
  for (i = 0; i < hash_get_n_cells(lock_sys->rec_hash); i++)
  {
    ...
    for (lock = static_cast<const lock_t *>(HASH_GET_FIRST(lock_sys->rec_hash, i)); lock != 0;
    ...
    ibool nowait) /*!< in: whether to wait for the lock mutex */
{
  ...
  fprintf(file, "Purge done for trx's n:o < " TRX_ID_FMT " undo n:o < " TRX_ID_FMT " state: ", purge_sys->iter.trx_no,
...
class TrxLockIterator
{
 public:
  ...
  const lock_t *current(const trx_t *trx) const
  {
    ...
    for (lock = UT_LIST_GET_FIRST(trx->lock.trx_locks); lock != NULL && i < m_index;
...
class TrxListIterator
{
 ...
 private:
  ...
  const trx_t *reposition() const
  {
    ...
    for (i = 0, trx = UT_LIST_GET_FIRST(*m_trx_list); trx != NULL && (i < m_index);
    ...
    const lock_t *lock) /*!< in: record lock */
{
  ...
  if (found)
  {
    ...
    {
      mtr_start(&mtr);
      buf_page_get_gen(page_id_t(space_id, page_no), page_size, RW_NO_LATCH, NULL, BUF_GET_POSSIBLY_FREED, __FILE__,
                       __LINE__, &mtr);
      mtr_commit(&mtr);
    ...
    const dict_table_t *table) /*!< in: table */
{
  ...
  for (lock = UT_LIST_GET_FIRST(table->locks); lock != NULL; lock = UT_LIST_GET_NEXT(un_member.tab_lock.locks, lock))
  {
    ...
    const ulint *offsets)      /*!< in: rec_get_offsets(rec, index) */
{
  ...
  ut_ad(!index || dict_index_is_clust(index) || !dict_index_is_online_ddl(index));
  ...
  else if (dict_index_is_clust(index))
  {
    ...
    if (impl_trx != NULL)
    {
      const lock_t *other_lock = lock_rec_other_has_expl_req(LOCK_S, block, true, heap_no, impl_trx);
       ...
       lock = lock_rec_get_next_const(heap_no, lock))
  {
    ...
    if (!lock_rec_get_gap(lock) && !lock_get_wait(lock))
    {
      ...
      const lock_t *other_lock = lock_rec_other_has_expl_req(mode, block, false, heap_no, lock->trx);
      ut_a(!other_lock);
    }
    ...
    const buf_block_t *block) /*!< in: buffer block */
{
  ...
  mutex_enter(&trx_sys->mutex);
loop:
  lock = lock_rec_get_first_on_page_addr(lock_sys->rec_hash, block->page.id.space(), block->page.id.page_no());
...
/*********************************************************************/ /**
 ...
 @return TRUE if ok */
static ibool lock_validate_table_locks(
    /*======================*/
    const trx_ut_list_t *trx_list) /*!< in: trx list */
{
  ...
  for (trx = UT_LIST_GET_FIRST(*trx_list); trx != NULL; trx = UT_LIST_GET_NEXT(trx_list, trx))
  {
    const lock_t *lock;
                        ...
                        (space, page_no) */
{
  ...
  for (const lock_t *lock = static_cast<const lock_t *>(HASH_GET_FIRST(lock_sys->rec_hash, start)); lock != NULL;
       lock = static_cast<const lock_t *>(HASH_GET_NEXT(hash, lock)))
  {
    ib_uint64_t current;
    ...
    current = ut_ull_create(lock->un_member.rec_lock.space, lock->un_member.rec_lock.page_no);
    ...
    ulint space_id, ulint page_no)
{
  ...
  if (fil_space_t *space = fil_space_acquire(space_id))
  {
    ...
    block = buf_page_get_gen(page_id_t(space_id, page_no), page_size_t(space->flags), RW_X_LATCH, NULL,
...
/*===========*/
{
  ...
  typedef std::set<page_addr_t, std::less<page_addr_t>, ut_allocator<page_addr_t>> page_addr_set;
  ...
  for (page_addr_set::const_iterator it = pages.begin(); it != pages.end(); ++it)
  {
    ...
    ulint heap_no)            /*!< in: rec heap number to lock */
{
      ...
      !lock_rec_has_expl(LOCK_X | LOCK_REC_NOT_GAP, block, heap_no, trx))
  {
    ...
    lock_rec_add_to_queue(type_mode, block, heap_no, index, trx, FALSE);
  }
    ...
    const ulint *offsets)     /*!< in: rec_get_offsets(rec, index) */
{
  ...
  else
  {
    ...
    ut_ad(!trx || !lock_rec_other_trx_holds_expl(LOCK_S | LOCK_REC_NOT_GAP, trx, rec, block));
  }
...
void lock_rec_convert_active_impl_to_expl(const buf_block_t *block, /*!< in: buffer block of rec */
                                          const rec_t *rec,         /*!< in: user record on page */
                                          dict_index_t *index,      /*!< in: index of record */
                                          const ulint *offsets,     /*!< in: rec_get_offsets(rec, index) */
                                          trx_t *trx,               /*!< in/out: active transaction */
    ...
    que_thr_t *thr)           /*!< in: query thread */
{
  ...
  if ((flags & BTR_NO_LOCKING_FLAG) || srv_read_only_mode || dict_table_is_temporary(index->table))
  {
    return (DB_SUCCESS);
  }
...
static ibool lock_trx_holds_autoinc_locks(
    /*=========================*/
    const trx_t *trx) /*!< in: transaction */
{
    ...
    const lock_t *lock) /*!< in: lock */
{
  ...
  {
    case LOCK_REC:
      ut_ad(dict_index_is_clust(lock->index) || !dict_index_is_online_ddl(lock->index));
...
const table_name_t &lock_get_table_name(const lock_t *lock) { return (lock_get_table(lock)->name); }
    ...
    const trx_ut_list_t *trx_list) /*!< in: trx list to check */
{
  ...
  for (trx = UT_LIST_GET_FIRST(*trx_list); trx != NULL; trx = UT_LIST_GET_NEXT(trx_list, trx))
  {
    ...
    for (lock = UT_LIST_GET_FIRST(trx->lock.trx_locks); lock != NULL; lock = UT_LIST_GET_NEXT(trx_locks, lock))
    {
      ...
      if (lock_get_type_low(lock) == LOCK_REC)
      {
        ut_ad(!dict_index_is_online_ddl(lock->index) || dict_index_is_clust(lock->index));
        if (lock->index->table == table)
        {
          return (lock);
        }
...
const lock_t *lock_trx_has_sys_table_locks(
    /*=========================*/
    const trx_t *trx) /*!< in: transaction to check */
{
  ...
  for (/* No op */; it != end; ++it)
  {
    ...
    if (lock != NULL && dict_is_sys_table(lock->un_member.tab_lock.table->id))
    {
  ...
  for (/* No op */; it != end; ++it)
  {
    ...
    if (dict_is_sys_table(lock->un_member.tab_lock.table->id) && lock_mode_stronger_or_eq(mode, strongest))
    {
      strongest = mode;
      strongest_lock = lock;
    }
    ...
    ulint heap_no)             /*!< in: record heap number */
{
  ...
  lock_mutex_enter();
  ut_a(lock_table_has(trx, table, LOCK_IX) || dict_table_is_temporary(table));
  ut_a(lock_rec_has_expl(LOCK_X | LOCK_REC_NOT_GAP, block, heap_no, trx) || dict_table_is_temporary(table));
  lock_mutex_exit();
  return (true);
}
#endif /* UNIV_DEBUG */
...
void DeadlockChecker::print(const trx_t *trx, ulint max_query_len)
{
  ...
  trx_print_low(lock_latest_err_file, trx, max_query_len, n_rec_locks, n_trx_locks, heap_size);

  if (srv_print_all_deadlocks)
  {
    trx_print_low(stderr, trx, max_query_len, n_rec_locks, n_trx_locks, heap_size);
  }
...
@return next lock or NULL if at end of queue */
const lock_t *DeadlockChecker::get_next_lock(const lock_t *lock, ulint heap_no) const
{
  ut_ad(lock_mutex_own());
  ...
  ut_ad(lock == NULL || lock_get_type_low(lock) == lock_get_type_low(m_wait_lock));
...
const lock_t *DeadlockChecker::get_first_lock(ulint *heap_no) const
{
  ...
  if (lock_get_type_low(lock) == LOCK_REC)
  {
    ...
    lock_hash = lock->type_mode & LOCK_PREDICATE ? lock_sys->prdt_hash : lock_sys->rec_hash;
...
const trx_t *DeadlockChecker::select_victim() const
{
  ...
  if (thd_trx_priority(m_start->mysql_thd) > 0 || thd_trx_priority(m_wait_lock->trx->mysql_thd) > 0)
  {
    const trx_t *victim;
...
const trx_t *DeadlockChecker::check_and_resolve(const lock_t *lock, trx_t *trx)
{
  ut_ad(lock_mutex_own());
  ut_ad(trx_mutex_own(trx));
  check_trx_state(trx);
