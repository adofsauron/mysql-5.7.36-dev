...
/*=======================*/
{
  ...
  for (ulint i = 0; i < OS_THREAD_MAX_N; i++, ++slot)
  {
    fprintf(stderr,
            ...
            (ulong)i, (ulong)slot->type, (ulong)slot->in_use, (ulong)slot->suspended, slot->wait_timeout,
    ...
    srv_slot_t *slot) /*!< in: slot to release */
{
  ...
  for (slot = lock_sys->last_slot; slot > lock_sys->waiting_threads && !slot->in_use; --slot)
  {
    /* No op */
  }
    ...
    ulong wait_timeout) /*!< in: lock wait timeout value */
{
  ...
  ib::error() << "There appear to be " << OS_THREAD_MAX_N
              << " user"
                    ...
                    user OS thread */
{
  ...
  if (thr->lock_state == QUE_THR_LOCK_ROW)
  {
    ...
    diff_time = (finish_time > start_time) ? (uint64_t)(finish_time - start_time) : 0;
  ...
  if (lock_wait_timeout < 100000000 && wait_time > (double)lock_wait_timeout && !trx_is_high_priority(trx))
  {
                            ...
                            thread when the wait started */
{
  ...
  if (trx_is_interrupted(trx) ||
      (slot->wait_timeout < 100000000 && (wait_time > (int64_t)slot->wait_timeout || wait_time < 0)))
  {
...
extern "C" os_thread_ret_t DECLARE_THREAD(lock_wait_timeout_thread)(
    /*=====================================*/
