...
size_t Ha_innopart_share::append_sep_and_name(char *to, const char *from, const char *sep, size_t len)
{
  size_t ret;
  ...
  if (strcmp(sep, part_sep_nix) != 0 && strcmp(sep, sub_sep_nix) != 0)
  {
...
bool Ha_innopart_share::open_one_table_part(uint part_id, const char *partition_name)
{
  char norm_name[FN_REFLEN];

  normalize_table_name(norm_name, partition_name);
  m_table_parts[part_id] = ha_innobase::open_dict_table(partition_name, norm_name, TRUE, DICT_ERR_IGNORE_NONE);
  ...
  if ((!DICT_TF2_FLAG_IS_SET(ib_table, DICT_TF2_FTS_HAS_DOC_ID) &&
       m_table_share->fields != (dict_table_get_n_user_cols(ib_table) + dict_table_get_n_v_cols(ib_table))) ||
      (DICT_TF2_FLAG_IS_SET(ib_table, DICT_TF2_FTS_HAS_DOC_ID) &&
       (m_table_share->fields != dict_table_get_n_user_cols(ib_table) + dict_table_get_n_v_cols(ib_table) - 1)))
  {
    ib::warn() << "Partition `" << get_partition_name(part_id) << "` contains " << dict_table_get_n_user_cols(ib_table)
               << " user defined columns in InnoDB, but " << m_table_share->fields
...
void Ha_innopart_share::set_v_templ(TABLE *table, dict_table_t *ib_table, const char *name)
{
  ...
  if (ib_table->n_v_cols > 0)
  {
    for (ulint i = 0; i < m_tot_parts; i++)
    {
      ...
      {
        innobase_build_v_templ(table, ib_table, m_table_parts[i]->vc_templ, NULL, true, name);
      }
    }
  }
}
...
@param[in]	table_name	Table name (db/table_name).
@return	false on success else true. */
bool Ha_innopart_share::open_table_parts(partition_info *part_info, const char *table_name)
{
  ...
  m_table_parts = static_cast<dict_table_t **>(ut_zalloc(table_parts_size, mem_key_partitioning));
  if (m_table_parts == NULL)
  {
    m_ref_count--;
    return (true);
  ...
  {
    len = append_sep_and_name(partition_name + table_name_len, part_elem->partition_name, part_sep_nix,
    ...
    if (part_info->is_sub_partitioned())
    {
      ...
      {
        append_sep_and_name(partition_name + table_name_len + len, sub_elem->partition_name, sub_sep_nix,
  ...
  {
    size_t alloc_size = mysql_num_index * m_tot_parts * sizeof(*m_index_mapping);
    m_index_mapping = static_cast<dict_index_t **>(ut_zalloc(alloc_size, mem_key_partitioning));
    if (m_index_mapping == NULL)
    {
  ...
  for (ulint idx = 0; idx < mysql_num_index; idx++)
  {
    for (ulint part = 0; part < m_tot_parts; part++)
    {
      ...
      m_index_mapping[count] = dict_table_get_index_on_name(m_table_parts[part], part_info->table->key_info[idx].name);

      if (m_index_mapping[count] == NULL)
      {
        ib::error() << "Cannot find index `" << part_info->table->key_info[idx].name
      ...
      if (!innobase_match_index_columns(&part_info->table->key_info[idx], m_index_mapping[count]))
      {
...
void Ha_innopart_share::close_table_parts()
{
#ifndef NDEBUG
  if (m_table_share->tmp_table == NO_TMP_TABLE)
  {
...
inline dict_index_t *Ha_innopart_share::get_index(uint part_id, uint keynr)
{
  ...
  if (m_index_mapping == NULL || keynr >= m_index_count)
  {
    if (keynr == MAX_KEY)
    {
      return (dict_table_get_first_index(get_table_part(part_id)));
    }
    return (NULL);
  }
...
inline uint Ha_innopart_share::get_mysql_key(uint part_id, const dict_index_t *index)
{
  ut_ad(index != NULL);
  ...
  if (index != NULL && m_index_mapping != NULL)
  {
    ...
    {
      ib::error() << "Cannot find index " << index->name << " in InnoDB index translation table.";
    }
  }
...
static inline void set_bit(byte *buf, size_t pos) { buf[pos / 8] |= (0x1 << (pos & 0x7)); }

/** Helper function for clear bit in bitmap.
@param[in,out]	buf	Bitmap buffer to update bit in.
@param[in]	bit_pos	Bit number (index starts at 0). */
static inline void clear_bit(byte *buf, size_t pos) { buf[pos / 8] &= ~(0x1 << (pos & 0x7)); }
...
@retval	0x1 bet set. */
static inline byte get_bit(byte *buf, size_t pos) { return ((buf[pos / 8] >> (pos & 0x7)) & 0x1); }
...
class Altered_partitions
{
 ...
 public:
  Altered_partitions(uint n_partitions, bool only_create);

  ~Altered_partitions();

  bool initialize();

  bool only_create() const { return (m_only_create); }

  /** Set currently used partition.
  @param[in]	new_part_id	Partition id to set.
  @param[in]	part	InnoDB table to use. */
  inline void set_part(ulint new_part_id, dict_table_t *part)
  {
  ...
  inline dict_table_t *part(uint part_id) const
  {
  ...
  inline void get_prebuilt(row_prebuilt_t *prebuilt, uint new_part_id) const
  {
    ...
    prebuilt->sql_stat_start = get_bit(m_sql_stat_start, new_part_id);
  }
  ...
  inline void set_from_prebuilt(row_prebuilt_t *prebuilt, uint new_part_id)
  {
...
Altered_partitions::Altered_partitions(uint n_partitions, bool only_create)
    : m_new_table_parts(),
      m_ins_nodes(),
      m_sql_stat_start(),
      m_trx_ids(),
...
Altered_partitions::~Altered_partitions()
{
  if (m_new_table_parts != NULL)
  {
    for (ulint i = 0; i < m_num_new_parts; i++)
    {
      ...
      {
        dict_table_close(m_new_table_parts[i], false, true);
      }
    }
...
bool Altered_partitions::initialize()
{
  ...
  m_new_table_parts = static_cast<dict_table_t **>(ut_zalloc(alloc_size, mem_key_partitioning));
  if (m_new_table_parts == NULL)
  {
    return (true);
  }
  ...
  m_ins_nodes = static_cast<ins_node_t **>(ut_zalloc(alloc_size, mem_key_partitioning));
  if (m_ins_nodes == NULL)
  {
    ut_free(m_new_table_parts);
    m_new_table_parts = NULL;
  ...
  alloc_size = sizeof(*m_sql_stat_start) * UT_BITS_IN_BYTES(m_num_new_parts);
  m_sql_stat_start = static_cast<byte *>(ut_zalloc(alloc_size, mem_key_partitioning));
  if (m_sql_stat_start == NULL)
  {
    ut_free(m_new_table_parts);
    m_new_table_parts = NULL;
  ...
  m_trx_ids = static_cast<trx_id_t *>(ut_zalloc(alloc_size, mem_key_partitioning));
  if (m_trx_ids == NULL)
  {
    ut_free(m_new_table_parts);
    m_new_table_parts = NULL;
...
uint ha_innopart::alter_table_flags(uint flags) { return (HA_PARTITION_FUNCTION_SUPPORTED | HA_FAST_CHANGE_PARTITION); }
      ...
      switch (err)
      {
        ...
        case DB_RECORD_NOT_FOUND:
          ib::error() << "MySQL and InnoDB data"
                      ...
                      << col_name << " in the InnoDB table " << index->table->name
                      << ". We set the"
...
int ha_innopart::open(const char *name, int /*mode*/, uint /*test_if_locked*/)
{
  dict_table_t *ib_table;
  char norm_name[FN_REFLEN];
  THD *thd;
  ...
  if (m_part_share->open_table_parts(m_part_info, name) || m_part_share->populate_partition_name_hash(m_part_info))
  {
    goto share_error;
  }
  if (m_part_share->auto_inc_mutex == NULL && table->found_next_number_field != NULL)
  {
  ...
  {
    innobase_copy_frm_flags_from_table_share(m_part_share->get_table_part(part_id), table->s);
  ...
  else
  {
    ...
    if (key_used_on_scan != MAX_KEY)
    {
      table_name_t table_name;
      table_name.m_name = const_cast<char *>(name);
      ib::warn() << "Table " << table_name << " key_used_on_scan is " << key_used_on_scan
  ...
  m_ins_node_parts = static_cast<ins_node_t **>(ut_zalloc(alloc_size, mem_key_partitioning));

  alloc_size = sizeof(*m_upd_node_parts) * m_tot_parts;
  m_upd_node_parts = static_cast<upd_node_t **>(ut_zalloc(alloc_size, mem_key_partitioning));

  alloc_blob_heap_array();

  alloc_size = sizeof(*m_trx_id_parts) * m_tot_parts;
  m_trx_id_parts = static_cast<trx_id_t *>(ut_zalloc(alloc_size, mem_key_partitioning));

  alloc_size = sizeof(*m_row_read_type_parts) * m_tot_parts;
  m_row_read_type_parts = static_cast<ulint *>(ut_zalloc(alloc_size, mem_key_partitioning));

  alloc_size = UT_BITS_IN_BYTES(m_tot_parts);
  m_sql_stat_start_parts = static_cast<uchar *>(ut_zalloc(alloc_size, mem_key_partitioning));
  if (m_ins_node_parts == NULL || m_upd_node_parts == NULL || m_blob_heap_parts == NULL || m_trx_id_parts == NULL ||
      m_row_read_type_parts == NULL || m_sql_stat_start_parts == NULL)
  {
...
void ha_innopart::set_partition(uint part_id)
{
  ...
  {
    DBUG_PRINT("ha_innopart", ("validating blob_heap: %p", m_prebuilt->blob_heap));
...
void ha_innopart::try_semi_consistent_read(bool yes)
{
  ...
  for (uint i = m_part_info->get_first_used_partition(); i < m_tot_parts; i = m_part_info->get_next_used_partition(i))
  {
...
@return	0 or error number. */
int ha_innopart::update_row_in_part(uint part_id, const uchar *old_row, uchar *new_row)
{
  int error;
...
int ha_innopart::init_record_priority_queue_for_parts(uint used_parts)
{
  ...
  bool need_clust_index = m_curr_key_info[1] != NULL || get_lock_type() != F_RDLCK;
...
{
  if (ha_innobase::is_ignorable_error(error) || error == HA_ERR_NO_PARTITION_FOUND ||
...
inline dict_index_t *ha_innopart::innopart_get_index(uint part_id, uint keynr)
{
  ...
  if (keynr != MAX_KEY && table->s->keys > 0)
  {
    ...
    else
    {
      ...
      ib::warn() << "InnoDB could not find index " << (key ? key->name : "NULL") << " key no " << keynr << " for table "
                 << m_prebuilt->table->name << " through its index translation table";
  ...
  else
  {
    ...
    ut_ad(keynr == MAX_KEY);
    index = dict_table_get_first_index(m_part_share->get_table_part(part_id));
  }

  if (index == NULL)
  {
    ib::error() << "InnoDB could not find key n:o " << keynr << " with name " << (key ? key->name : "NULL")
                << " from dict cache for table " << m_prebuilt->table->name << " partition n:o " << part_id;
  }
...
int ha_innopart::change_active_index(uint part_id, uint keynr)
{
  ...
  {
    ib::warn() << "change_active_index(" << part_id << "," << keynr << ") failed";
...
int ha_innopart::index_next_in_part(uint part, uchar *record)
{
  ...
  ut_ad(m_ordered_scan_ongoing || m_ordered_rec_buffer == NULL || m_prebuilt->used_in_HANDLER ||
...
@return	error number or 0. */
int ha_innopart::index_next_same_in_part(uint part, uchar *record, const uchar *key, uint length)
{
  int error;
...
@return	error number or 0. */
int ha_innopart::index_read_map_in_part(uint part, uchar *record, const uchar *key, key_part_map keypart_map,
...
@param[in]	find_flag	Key condition/direction to use.
@return	error number or 0. */
int ha_innopart::index_read_idx_map_in_part(uint part, uchar *record, uint index, const uchar *key,
                                            key_part_map keypart_map, enum ha_rkey_function find_flag)
{
  int error;

  set_partition(part);
  error = ha_innobase::index_read_idx_map(record, index, key, keypart_map, find_flag);
  update_partition(part);
  return (error);
}
...
@param[in]	keypart_map	Which part of the key to use.
@return	error number or 0. */
int ha_innopart::index_read_last_map_in_part(uint part, uchar *record, const uchar *key, key_part_map keypart_map)
{
  int error;
  set_partition(part);
...
@return	error number or 0. */
int ha_innopart::read_range_first_in_part(uint part, uchar *record, const key_range *start_key,
                                          const key_range *end_key, bool eq_range, bool sorted)
{
  int error;
  ...
  {
    error = ha_innobase::index_read(read_record, m_start_key.key, m_start_key.length, m_start_key.flag);
  }
  else
  {
...
@return	0 or error number. */
int ha_innopart::rnd_end_in_part(uint part_id, bool scan) { return (index_end()); }
...
int ha_innopart::rnd_pos(uchar *buf, uchar *pos)
{
  ...
  error = ha_innobase::index_read(buf, pos + PARTITION_BYTES_IN_POS, ref_length - PARTITION_BYTES_IN_POS,
                                  HA_READ_KEY_EXACT);
...
void ha_innopart::update_part_elem(partition_element *part_elem, dict_table_t *ib_table)
{
  dict_get_and_save_data_dir_path(ib_table, false);
  if (ib_table->data_dir_path != NULL)
  {
    if (part_elem->data_file_name == NULL || strcmp(ib_table->data_dir_path, part_elem->data_file_name) != 0)
    {
      /* Play safe and allocate memory from TABLE and copy
      instead of expose the internal data dictionary. */
      part_elem->data_file_name = strdup_root(&table->mem_root, ib_table->data_dir_path);
    }
  }
  else
  {
  ...
  {
    ut_ad(part_elem->tablespace_name == NULL || 0 == strcmp(part_elem->tablespace_name, ib_table->tablespace));
    if (part_elem->tablespace_name == NULL || strcmp(ib_table->tablespace, part_elem->tablespace_name) != 0)
    {
      /* Play safe and allocate memory from TABLE and copy
      instead of expose the internal data dictionary. */
      part_elem->tablespace_name = strdup_root(&table->mem_root, ib_table->tablespace);
    }
  }
  else
  {
    ut_ad(part_elem->tablespace_name == NULL || 0 == strcmp(part_elem->tablespace_name, "innodb_file_per_table"));
    if (part_elem->tablespace_name != NULL && 0 != strcmp(part_elem->tablespace_name, "innodb_file_per_table"))
    {
...
static void set_create_info_dir(partition_element *part_elem, HA_CREATE_INFO *info)
{
  if (part_elem->data_file_name != NULL && part_elem->data_file_name[0] != '\0')
  {
    info->data_file_name = part_elem->data_file_name;
    /* Also implies non-default tablespace. */
    info->tablespace = NULL;
  }
  if (part_elem->index_file_name != NULL && part_elem->index_file_name[0] != '\0')
  {
    info->index_file_name = part_elem->index_file_name;
  }
  if (part_elem->tablespace_name != NULL && part_elem->tablespace_name[0] != '\0')
  {
...
@return	0 or error number. */
int ha_innopart::create(const char *name, TABLE *form, HA_CREATE_INFO *create_info)
{
  int error;
  /** {database}/{tablename} */
  ...
  create_table_info_t info(ha_thd(), form, create_info, table_name, temp_path, remote_path, tablespace_name);
  ...
  if (create_info != NULL && (create_info->options & HA_LEX_CREATE_TMP_TABLE) != 0)
  {
  ...
  while ((part_elem = part_it++))
  {
    ...
    len = Ha_innopart_share::append_sep_and_name(partition_name_start, part_elem->partition_name, part_sep,
                                                 FN_REFLEN - table_name_len);
    ...
    if ((table_name_len + len + sizeof "/") >= FN_REFLEN)
    {
      ...
      my_error(ER_IDENT_CAUSES_TOO_LONG_PATH, MYF(0), FN_REFLEN, partition_name);
      goto cleanup;
    }

    /* Report error if table name with partition name exceeds
    maximum file name length */
    if ((len + table_name_len - db_name_length - 1) > stat_table_name_length)
    {
      error = HA_ERR_INTERNAL_ERROR;
      my_error(ER_PATH_LENGTH, MYF(0), partition_name + db_name_length + 1);
      goto cleanup;
    }
    ...
    {
      size_t part_name_len = strlen(partition_name_start) + table_name_len;
      ...
      while ((sub_elem = sub_it++))
      {
        ...
        len = Ha_innopart_share::append_sep_and_name(part_name_end, sub_elem->partition_name, sub_sep,
        ...
        if ((len + part_name_len - db_name_length - 1) > stat_table_name_length)
        {
          error = HA_ERR_INTERNAL_ERROR;
          ;
          my_error(ER_PATH_LENGTH, MYF(0), partition_name + db_name_length + 1);
        ...
        Ha_innopart_share::partition_name_casedn_str(part_name_end + 4);
        error = info.prepare_create_table(partition_name);
        if (error != 0)
        {
          goto cleanup;
  ...
  while ((part_elem = part_it++))
  {
    len = Ha_innopart_share::append_sep_and_name(table_name_end, part_elem->partition_name, part_sep,
                                                 FN_REFLEN - table_name_len);
    ...
    else
    {
      ...
      {
        len = Ha_innopart_share::append_sep_and_name(part_name_end, sub_elem->partition_name, sub_sep,
                                                     FN_REFLEN - table_name_len - part_name_len);
...
@return	0 or error number. */
int ha_innopart::discard_or_import_tablespace(my_bool discard)
{
  int error = 0;
  uint i;
  DBUG_ENTER("ha_innopart::discard_or_import_tablespace");

  for (i = m_part_info->get_first_used_partition(); i < m_tot_parts; i = m_part_info->get_next_used_partition(i))
  {
...
int ha_innopart::extra(enum ha_extra_function operation)
{
  if (operation == HA_EXTRA_SECONDARY_SORT_ROWID)
  {
    ...
    ut_ad(m_curr_key_info[1] != NULL || m_prebuilt->clust_index_was_generated != 0 ||
...
@return	0 or error number. */
int ha_innopart::truncate_partition_low() { return (truncate()); }

/** Deletes all rows of a partitioned InnoDB table.
@return	0 or error number. */
int ha_innopart::truncate()
{
...
ha_rows ha_innopart::records_in_range(uint keynr, key_range *min_key, key_range *max_key)
{
  KEY *key;
  dict_index_t *index;
  ...
  if (index == NULL || dict_table_is_discarded(m_prebuilt->table) || !row_merge_is_index_usable(m_prebuilt->trx, index))
  {
    n_rows = HA_POS_ERROR;
    goto func_exit;
  }
  ...
  ut_ad(min_key != NULL ? range_start->n_fields > 0 : range_start->n_fields == 0);
  ...
  if (mode1 != PAGE_CUR_UNSUPP && mode2 != PAGE_CUR_UNSUPP)
  {
    ...
    for (part_id = m_part_info->get_next_used_partition(part_id); part_id < m_tot_parts;
         part_id = m_part_info->get_next_used_partition(part_id))
    {
      index = m_part_share->get_index(part_id, keynr);
      /* Individual partitions can be discarded
      we need to check each partition */
      if (index == NULL || dict_table_is_discarded(index->table) || !row_merge_is_index_usable(m_prebuilt->trx, index))
      {
        n_rows = HA_POS_ERROR;
        mem_heap_free(heap);
        goto func_exit;
      }
      int64_t n = btr_estimate_n_rows_in_range(index, range_start, mode1, range_end, mode2);
      n_rows += n;
      DBUG_PRINT("info", ("part_id %u rows %ld (%ld)", part_id, (long int)n, (long int)n_rows));
    }
  }
  else
  {
...
int ha_innopart::info_low(uint flag, bool is_analyze)
{
  ...
  if ((flag & HA_STATUS_VARIABLE) != 0)
  {
    ...
    for (uint i = m_part_info->get_first_used_partition(); i < m_tot_parts; i = m_part_info->get_next_used_partition(i))
    {
      ...
      if ((flag & HA_STATUS_VARIABLE_EXTRA) != 0 && (flag & HA_STATUS_NO_LOCK) == 0 &&
          srv_force_recovery < SRV_FORCE_NO_IBUF_MERGE && avail_space != ULINT_UNDEFINED)
      {
        ...
        uintmax_t space = fsp_get_available_space_in_free_extents(ib_table->space);
    ...
    if ((flag & HA_STATUS_NO_LOCK) == 0 && (flag & HA_STATUS_VARIABLE_EXTRA) != 0 &&
    ...
    stats.check_time = 0;
    stats.mrr_length_per_rec = ref_length + sizeof(void *) - PARTITION_BYTES_IN_POS;
  ...
  if ((flag & HA_STATUS_CONST) != 0)
  {
    ...
    for (uint i = 0; i < m_tot_parts; i++)
    {
      ...
      if ((flag & HA_STATUS_VARIABLE) == 0 || !bitmap_is_set(&(m_part_info->read_partitions), i))
      {
        ib_table = m_part_share->get_table_part(i);
        if (ib_table->stat_n_rows > max_rows)
        {
    ...
    if (table->s->keys != num_innodb_index)
    {
      ib::error() << "Table " << ib_table->name << " contains " << num_innodb_index
    ...
    for (ulong i = 0; i < table->s->keys; i++)
    {
      ...
      for (j = 0; j < key->actual_key_parts; j++)
      {
        ...
        if ((j + 1) > index->n_uniq)
        {
          ib::error() << "Index " << index->name << " of " << ib_table->name << " has " << index->n_uniq
        ...
        ulong rec_per_key_int = static_cast<ulong>(innodb_rec_per_key(index, j, max_rows));
  ...
  if ((flag & HA_STATUS_ERRKEY) != 0)
  {
    ...
    {
      errkey = (unsigned int)((m_prebuilt->trx->error_key_num == ULINT_UNDEFINED) ? UINT_MAX
...
int ha_innopart::optimize(THD *thd, HA_CHECK_OPT *check_opt) { return (HA_ADMIN_TRY_ALTER); }
...
int ha_innopart::check(THD *thd, HA_CHECK_OPT *check_opt)
{
  ...
  if (error != 0)
  {
    print_admin_msg(thd, 256, "error", table_share->db.str, table->alias, "check",
                    m_is_sub_partitioned ? "Subpartition %s returned error" : "Partition %s returned error",
...
int ha_innopart::repair(THD *thd, HA_CHECK_OPT *repair_opt)
{
  ...
  for (uint i = m_part_info->get_first_used_partition(); i < m_tot_parts; i = m_part_info->get_next_used_partition(i))
  {
    ...
    if (error != 0)
    {
      print_admin_msg(thd, 256, "error", table_share->db.str, table->alias, "repair",
                      m_is_sub_partitioned ? "Subpartition %s returned error" : "Partition %s returned error",
                      m_part_share->get_partition_name(i));
      break;
    }
...
int ha_innopart::start_stmt(THD *thd, thr_lock_type lock_type)
{
  ...
  {
    memset(m_sql_stat_start_parts, 0xff, UT_BITS_IN_BYTES(m_tot_parts));
  }
  else
  {
    memset(m_sql_stat_start_parts, 0, UT_BITS_IN_BYTES(m_tot_parts));
  }
  return (error);
}
...
THR_LOCK_DATA **ha_innopart::store_lock(THD *thd, THR_LOCK_DATA **to, thr_lock_type lock_type)
{
...
int ha_innopart::external_lock(THD *thd, int lock_type)
{
  ...
  if (m_part_info->get_first_used_partition() == MY_BIT_NONE && !(m_mysql_has_locked && lock_type == F_UNLCK))
  {
    /* All partitions pruned away, do nothing! */
    ...
    switch (table->quiesce)
    {
      case QUIESCE_START:
        ...
        if (!srv_read_only_mode && thd_sql_command(thd) == SQLCOM_FLUSH && lock_type == F_RDLCK)
        {
          ...
          {
            ib_senderrf(m_prebuilt->trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_DISCARDED, table->name.m_name);
      ...
      case QUIESCE_COMPLETE:
        ...
        or trx interruption. */
        if (m_prebuilt->trx->flush_tables > 0 && (lock_type == F_UNLCK || trx_is_interrupted(m_prebuilt->trx)))
        {
...
void ha_innopart::get_auto_increment(ulonglong offset, ulonglong increment, ulonglong nb_desired_values,
                                     ulonglong *first_value, ulonglong *nb_reserved_values)
{
  ...
  get_auto_increment_first_field(increment, nb_desired_values, first_value, nb_reserved_values);
  DBUG_VOID_RETURN;
}
...
enum row_type ha_innopart::get_partition_row_type(uint part_id)
{
  set_partition(part_id);
  return get_row_type();
}
...
int ha_innopart::cmp_ref(const uchar *ref1, const uchar *ref2)
{
  int cmp;

  cmp = ha_innobase::cmp_ref(ref1 + PARTITION_BYTES_IN_POS, ref2 + PARTITION_BYTES_IN_POS);
...
@return	0 for success else error code. */
int ha_innopart::prepare_for_new_partitions(uint num_partitions, bool only_create)
{
  m_new_partitions = UT_NEW(Altered_partitions(num_partitions, only_create), mem_key_partitioning);
...
@return	0 for success else error code. */
int ha_innopart::create_new_partition(TABLE *table, HA_CREATE_INFO *create_info, const char *part_name,
                                      uint new_part_id, partition_element *part_elem)
{
  int error;
  char norm_name[FN_REFLEN];
  ...
  if (tablespace_is_shared_space(create_info) && create_info->data_file_name != NULL &&
  ...
  {
    dict_table_t *part;
    part = dict_table_open_on_name(norm_name, false, true, DICT_ERR_IGNORE_NONE);
    if (part == NULL)
    {
      DBUG_RETURN(HA_ERR_INTERNAL_ERROR);
    }
...
void ha_innopart::close_new_partitions()
{
  if (m_new_partitions != NULL)
  {
    UT_DELETE(m_new_partitions);
...
mem_heap_t **ha_innopart::alloc_blob_heap_array()
{
  ...
  m_blob_heap_parts = static_cast<mem_heap_t **>(ut_zalloc(len, mem_key_partitioning));
...
void ha_innopart::clear_blob_heaps()
{
  ...
  for (uint i = 0; i < m_tot_parts; i++)
  {
    ...
    {
      DBUG_PRINT("ha_innopart", ("freeing blob_heap: %p", m_blob_heap_parts[i]));
...
int ha_innopart::rnd_pos_by_record(uchar *record)
{
  ...
  if (unlikely(get_part_for_delete(record, m_table->record[0], m_part_info, &m_last_part)))
  {
    DBUG_RETURN(HA_ERR_INTERNAL_ERROR);
  }
