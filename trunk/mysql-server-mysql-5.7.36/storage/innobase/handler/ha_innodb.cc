...
static const char *innodb_debug_compress_names[] = {"none", "zlib", "lz4", "lz4hc", NullS};

/** Enumeration of --innodb-debug-compress */
static TYPELIB innodb_debug_compress_typelib = {array_elements(innodb_debug_compress_names) - 1,
...
static const char *innodb_stats_method_names[] = {"nulls_equal", "nulls_unequal", "nulls_ignored", NullS};

/** Used to define an enumerate type of the system variable innodb_stats_method.
This is the same as "myisam_stats_method_typelib" */
static TYPELIB innodb_stats_method_typelib = {array_elements(innodb_stats_method_names) - 1,
                                              "innodb_stats_method_typelib", innodb_stats_method_names, NULL};

/** Possible values of the parameter innodb_checksum_algorithm */
static const char *innodb_checksum_algorithm_names[] = {"crc32", "strict_crc32", "innodb", "strict_innodb",
                                                        "none",  "strict_none",  NullS};

/** Used to define an enumerate type of the system variable
innodb_checksum_algorithm. */
static TYPELIB innodb_checksum_algorithm_typelib = {array_elements(innodb_checksum_algorithm_names) - 1,
                                                    "innodb_checksum_algorithm_typelib",
                                                    innodb_checksum_algorithm_names, NULL};

/** Possible values for system variable "innodb_default_row_format". */
static const char *innodb_default_row_format_names[] = {"redundant", "compact", "dynamic", NullS};

/** Used to define an enumerate type of the system variable
innodb_default_row_format. */
static TYPELIB innodb_default_row_format_typelib = {array_elements(innodb_default_row_format_names) - 1,
                                                    "innodb_default_row_format_typelib",
                                                    innodb_default_row_format_names, NULL};
...
const char PARTITION_IN_SHARED_TABLESPACE_WARNING[] = "InnoDB : A table partition in a shared tablespace";
...
const struct _ft_vft ft_vft_result = {NULL, innobase_fts_find_ranking, innobase_fts_close_ranking,
                                      innobase_fts_retrieve_ranking, NULL};

const struct _ft_vft_ext ft_vft_ext_result = {innobase_fts_get_version, innobase_fts_flags, innobase_fts_retrieve_docid,
...
#define PSI_RWLOCK_KEY(n)                    \
  {                                          \
    &n##_key.m_value, #n, PSI_RWLOCK_FLAG_SX \
  }
...
static PSI_mutex_info all_pthread_mutexes[] = {PSI_KEY(commit_cond_mutex), PSI_KEY(innobase_share_mutex)};

static PSI_cond_info all_innodb_conds[] = {PSI_KEY(commit_cond)};
...
#endif /* UNIV_DEBUG */
    PSI_RWLOCK_KEY(dict_operation_lock),   PSI_RWLOCK_KEY(fil_space_latch),        PSI_RWLOCK_KEY(checkpoint_lock),
    PSI_RWLOCK_KEY(fts_cache_rw_lock),     PSI_RWLOCK_KEY(fts_cache_init_rw_lock), PSI_RWLOCK_KEY(trx_i_s_cache_lock),
    PSI_RWLOCK_KEY(trx_purge_latch),       PSI_RWLOCK_KEY(index_tree_rw_lock),     PSI_RWLOCK_KEY(index_online_log),
    PSI_RWLOCK_KEY(dict_table_stats),      PSI_RWLOCK_KEY(hash_table_locks),
};
...
static PSI_thread_info all_innodb_threads[] = {
    PSI_KEY(buf_dump_thread),         PSI_KEY(dict_stats_thread),
    PSI_KEY(io_handler_thread),       PSI_KEY(io_ibuf_thread),
    PSI_KEY(io_log_thread),           PSI_KEY(io_read_thread),
    PSI_KEY(io_write_thread),         PSI_KEY(page_cleaner_thread),
    PSI_KEY(recv_writer_thread),      PSI_KEY(srv_error_monitor_thread),
    PSI_KEY(srv_lock_timeout_thread), PSI_KEY(srv_master_thread),
    PSI_KEY(srv_monitor_thread),      PSI_KEY(srv_purge_thread),
    PSI_KEY(srv_worker_thread),       PSI_KEY(trx_rollback_clean_thread),
};
...
static PSI_file_info all_innodb_files[] = {PSI_KEY(innodb_data_file), PSI_KEY(innodb_log_file),
                                           PSI_KEY(innodb_temp_file)};
#endif /* UNIV_PFS_IO */
#endif /* HAVE_PSI_INTERFACE */

/** Set up InnoDB API callback function array */
ib_cb_t innodb_api_cb[] = {(ib_cb_t)ib_cursor_open_table,
...
#ifdef UNIV_DEBUG
void srv_mbr_debug(const byte *data)
{
  double a, b, c, d;
...
static int innodb_stopword_table_validate(
    /*===========================*/
    THD *thd, struct st_mysql_sys_var *var, void *save, struct st_mysql_value *value);
...
static int innodb_tmpdir_validate(THD *thd, struct st_mysql_sys_var *var, void *save, struct st_mysql_value *value)
{
  char *alter_tmp_dir;
  char *innodb_tmp_dir;
  ...
  {
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, "InnoDB: FILE Permissions required");
  ...
  {
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, "Path length should not exceed %d bytes",
                        FN_REFLEN);
    *static_cast<const char **>(save) = NULL;
    return (1);
  }
  ...
  {
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, "InnoDB: Path doesn't exist.");
    *static_cast<const char **>(save) = NULL;
    return (1);
  }
  ...
  if (my_stat(tmp_abs_path, &stat_info_dir, MYF(0)))
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, "Given path is not a directory. ");
      *static_cast<const char **>(save) = NULL;
      return (1);
    }
  }
  ...
  innodb_tmp_dir = static_cast<char *>(thd_memdup(thd, tmp_abs_path, tmp_abs_len + 1));
...
static void innodb_support_xa_update(THD *thd, struct st_mysql_sys_var *var, void *var_ptr, const void *save)
{
  my_bool innodb_support_xa = *static_cast<const my_bool *>(save);
  push_warning(thd, Sql_condition::SL_WARNING, HA_ERR_WRONG_COMMAND,
               innodb_support_xa ? deprecated_innodb_support_xa : deprecated_innodb_support_xa_off);
}

static MYSQL_THDVAR_BOOL(support_xa, PLUGIN_VAR_OPCMDARG, "Enable InnoDB support for the XA two-phase commit",
                         /* check_func */ NULL, innodb_support_xa_update,
                         /* default */ TRUE);

static MYSQL_THDVAR_BOOL(table_locks, PLUGIN_VAR_OPCMDARG, "Enable InnoDB locking in LOCK TABLES",
                         /* check_func */ NULL, /* update_func */ NULL,
                         /* default */ TRUE);

static MYSQL_THDVAR_BOOL(strict_mode, PLUGIN_VAR_OPCMDARG, "Use strict mode when evaluating create options.", NULL,
                         NULL, TRUE);

static MYSQL_THDVAR_BOOL(ft_enable_stopword, PLUGIN_VAR_OPCMDARG, "Create FTS index with stopword.", NULL, NULL,
                         /* default */ TRUE);

static MYSQL_THDVAR_ULONG(lock_wait_timeout, PLUGIN_VAR_RQCMDARG,
                          "Timeout in seconds an InnoDB transaction may wait for a lock before being rolled back. "
                          "Values above 100000000 disable the timeout.",
                          NULL, NULL, 50, 1, 1024 * 1024 * 1024, 0);

static MYSQL_THDVAR_STR(ft_user_stopword_table, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_MEMALLOC,
...
static MYSQL_THDVAR_STR(tmpdir, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_MEMALLOC,
                        "Directory for temporary non-tablespace files.", innodb_tmpdir_validate, NULL, NULL);

static SHOW_VAR innodb_status_variables[] = {
    {"buffer_pool_dump_status", (char *)&export_vars.innodb_buffer_pool_dump_status, SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_load_status", (char *)&export_vars.innodb_buffer_pool_load_status, SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_resize_status", (char *)&export_vars.innodb_buffer_pool_resize_status, SHOW_CHAR, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_pages_data", (char *)&export_vars.innodb_buffer_pool_pages_data, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_bytes_data", (char *)&export_vars.innodb_buffer_pool_bytes_data, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_pages_dirty", (char *)&export_vars.innodb_buffer_pool_pages_dirty, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_bytes_dirty", (char *)&export_vars.innodb_buffer_pool_bytes_dirty, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_pages_flushed", (char *)&export_vars.innodb_buffer_pool_pages_flushed, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_pages_free", (char *)&export_vars.innodb_buffer_pool_pages_free, SHOW_LONG, SHOW_SCOPE_GLOBAL},
#ifdef UNIV_DEBUG
    {"buffer_pool_pages_latched", (char *)&export_vars.innodb_buffer_pool_pages_latched, SHOW_LONG, SHOW_SCOPE_GLOBAL},
#endif /* UNIV_DEBUG */
    {"buffer_pool_pages_misc", (char *)&export_vars.innodb_buffer_pool_pages_misc, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_pages_total", (char *)&export_vars.innodb_buffer_pool_pages_total, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_read_ahead_rnd", (char *)&export_vars.innodb_buffer_pool_read_ahead_rnd, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"buffer_pool_read_ahead", (char *)&export_vars.innodb_buffer_pool_read_ahead, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_read_ahead_evicted", (char *)&export_vars.innodb_buffer_pool_read_ahead_evicted, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"buffer_pool_read_requests", (char *)&export_vars.innodb_buffer_pool_read_requests, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_reads", (char *)&export_vars.innodb_buffer_pool_reads, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_wait_free", (char *)&export_vars.innodb_buffer_pool_wait_free, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"buffer_pool_write_requests", (char *)&export_vars.innodb_buffer_pool_write_requests, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"data_fsyncs", (char *)&export_vars.innodb_data_fsyncs, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"data_pending_fsyncs", (char *)&export_vars.innodb_data_pending_fsyncs, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"data_pending_reads", (char *)&export_vars.innodb_data_pending_reads, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"data_pending_writes", (char *)&export_vars.innodb_data_pending_writes, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"data_read", (char *)&export_vars.innodb_data_read, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"data_reads", (char *)&export_vars.innodb_data_reads, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"data_writes", (char *)&export_vars.innodb_data_writes, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"data_written", (char *)&export_vars.innodb_data_written, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"dblwr_pages_written", (char *)&export_vars.innodb_dblwr_pages_written, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"dblwr_writes", (char *)&export_vars.innodb_dblwr_writes, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"log_waits", (char *)&export_vars.innodb_log_waits, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"log_write_requests", (char *)&export_vars.innodb_log_write_requests, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"log_writes", (char *)&export_vars.innodb_log_writes, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"os_log_fsyncs", (char *)&export_vars.innodb_os_log_fsyncs, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"os_log_pending_fsyncs", (char *)&export_vars.innodb_os_log_pending_fsyncs, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"os_log_pending_writes", (char *)&export_vars.innodb_os_log_pending_writes, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"os_log_written", (char *)&export_vars.innodb_os_log_written, SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"page_size", (char *)&export_vars.innodb_page_size, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"pages_created", (char *)&export_vars.innodb_pages_created, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"pages_read", (char *)&export_vars.innodb_pages_read, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"pages_written", (char *)&export_vars.innodb_pages_written, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"row_lock_current_waits", (char *)&export_vars.innodb_row_lock_current_waits, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"row_lock_time", (char *)&export_vars.innodb_row_lock_time, SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"row_lock_time_avg", (char *)&export_vars.innodb_row_lock_time_avg, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"row_lock_time_max", (char *)&export_vars.innodb_row_lock_time_max, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"row_lock_waits", (char *)&export_vars.innodb_row_lock_waits, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"rows_deleted", (char *)&export_vars.innodb_rows_deleted, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"rows_inserted", (char *)&export_vars.innodb_rows_inserted, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"rows_read", (char *)&export_vars.innodb_rows_read, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"rows_updated", (char *)&export_vars.innodb_rows_updated, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"num_open_files", (char *)&export_vars.innodb_num_open_files, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"truncated_status_writes", (char *)&export_vars.innodb_truncated_status_writes, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"available_undo_logs", (char *)&export_vars.innodb_available_undo_logs, SHOW_LONG, SHOW_SCOPE_GLOBAL},
#ifdef UNIV_DEBUG
    {"purge_trx_id_age", (char *)&export_vars.innodb_purge_trx_id_age, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"purge_view_trx_id_age", (char *)&export_vars.innodb_purge_view_trx_id_age, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"ahi_drop_lookups", (char *)&export_vars.innodb_ahi_drop_lookups, SHOW_LONG, SHOW_SCOPE_GLOBAL},
#endif /* UNIV_DEBUG */
...
static handler *innobase_create_handler(
    /*====================*/
    handlerton *hton, /*!< in/out: handlerton for InnoDB */
...
static void innobase_undo_logs_init_default_max();
/*==================================*/

/************************************************************/ /**
 Validate the file format name and return its corresponding id.
 @return valid file format id */
static uint innobase_file_format_name_lookup(
    /*=============================*/
...
static int innobase_file_format_validate_and_set(
    /*==================================*/
...
/*******************************************************************/ /**
 ...
 @return 0 or error number */
static int innobase_commit_by_xid(
    /*===================*/
    handlerton *hton,                                                 /*!< in: InnoDB handlerton */
...
/*******************************************************************/ /**
 ...
 which is in the prepared state
 @return 0 or error number */
static int innobase_rollback_by_xid(
    /*=====================*/
    handlerton *hton, /*!< in: InnoDB handlerton */
    XID *xid);        /*!< in: X/Open XA transaction
                      identification */
...
static int innobase_alter_tablespace(handlerton *hton, THD *thd, st_alter_tablespace *alter_info);
...
static void innobase_drop_database(handlerton *hton, char *path);

/*******************************************************************/ /**
 Closes an InnoDB database. */
static int innobase_end(
    /*=========*/
    handlerton *hton, /* in: InnoDB handlerton */
...
static bool innobase_show_status(
    /*=================*/
    handlerton *hton, /*!< in: the innodb handlerton */
    THD *thd,         /*!< in: the MySQL query thread of
                      the caller */
    ...
    TABLE_SHARE *table, MEM_ROOT *mem_root)
{
  ...
  if (table && table->db_type() == innodb_hton_ptr  // 1)
...
int thd_trx_priority(THD *thd) { return (thd == NULL ? 0 : thd_tx_priority(thd)); }
...
@param[in] thd			Thread handle
@return true if the thd is marked as read-only */
bool thd_trx_is_dd_trx(THD *thd) { return (thd != NULL && thd_tx_is_dd_trx(thd)); }
#endif /* UNIV_DEBUG */
...
{
  return (thd != NULL && !thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN) && thd_is_select(thd));
}
...
static inline void innobase_srv_conc_enter_innodb(row_prebuilt_t *prebuilt)
{
  ...
  if (srv_thread_concurrency)
  {
    ...
    else if (trx->mysql_thd != NULL && thd_is_replication_slave_thread(trx->mysql_thd))
    {
      UT_WAIT_FOR(srv_conc_get_active_threads() < srv_thread_concurrency, srv_replication_delay * 1000);
    }
    else
    {
...
ibool skip_concurrency_ticket(row_prebuilt_t *prebuilt)
{
  ...
  if (thd && (thd_has_active_attachable_trx(thd) || thd_is_operating_gtid_table_implicitly(thd)))
  {
    return true;
  }
...
static innodb_session_t *&thd_to_innodb_session(THD *thd)
{
  innodb_session_t *&innodb_session = *(innodb_session_t **)thd_ha_data(thd, innodb_hton_ptr);
...
@param[in,out]	thd		MySQL thread handler */
static inline void add_table_to_thread_cache(dict_table_t *table, mem_heap_t *heap, THD *thd)
{
...
/********************************************************************/ /**
 ...
 @return 0 */
inline int innobase_release_temporary_latches(
    /*===============================*/
  ...
  switch (error)
  {
    ...
    case DB_TOO_BIG_RECORD:
    {
      ...
      my_printf_error(ER_TOO_BIG_ROWSIZE,
                      ...
                      MYF(0),
                      srv_page_size == UNIV_PAGE_SIZE_MAX ? REC_MAX_DATA_SIZE - 1
    ...
    case DB_TOO_BIG_INDEX_COL:
      my_error(ER_INDEX_COLUMN_TOO_LONG, MYF(0), DICT_MAX_FIELD_LEN_BY_FORMAT_FLAG(flags));
    ...
    ulint *mbmaxlen) /*!< out: maximum length of a char (in bytes) */
{
  ...
  else
  {
    ...
    if (thd && thd_sql_command(thd) == SQLCOM_DROP_TABLE)
    {
      ...
      if (cset != 0)
      {
        sql_print_warning("Unknown collation #%lu.", cset);
      }
    }
    else
    {
...
my_bool innobase_check_identifier_length(
    /*=============================*/
                    ...
                    database portion. */
{
  ...
  size_t len = cs->cset->well_formed_len(cs, id, id + strlen(id), NAME_CHAR_LEN, &well_formed_error);
...
CHARSET_INFO *innobase_get_charset(
    /*=================*/
...
int innobase_mysql_tmpfile(const char *path)
{
  ...
  if (fd >= 0)
  {
    ...
    to C runtime filedescriptor. */
    {
      ...
      HANDLE hDup;
      BOOL bOK =
          DuplicateHandle(GetCurrentProcess(), hFile, GetCurrentProcess(), &hDup, 0, FALSE, DUPLICATE_SAME_ACCESS);
      if (bOK)
      {
    ...
    if (fd2 < 0)
    {
      ...
      set_my_errno(errno);
      my_error(EE_OUT_OF_FILERESOURCES, MYF(0), "ib*", my_errno(), my_strerror(errbuf, sizeof(errbuf), my_errno()));
    }
...
{
  return (
      copy_and_convert((char *)to, (uint32)to_length, to_cs, (const char *)from, (uint32)from_length, from_cs, errors));
}
                        ...
                        in bytes */
{
  ...
  buf_tmp_used =
      innobase_convert_string(buf_tmp, sizeof(buf_tmp), system_charset_info, data, data_len, data_cs, &num_errors);
...
bool Compression::is_none(const char *algorithm)
{
  ...
  if (algorithm == NULL || *algorithm == 0 || innobase_strcasecmp(algorithm, "none") == 0)
  {
    return (true);
  }
...
{
  /* NULL is the same as NONE */
  if (algorithm == NULL || innobase_strcasecmp(algorithm, "n") == 0 || innobase_strcasecmp(algorithm, "") == 0)
  {
    return (true);
  }
    ...
    ulonglong max_value) /*!< in: max value for type */
{
  ...
  if (block >= max_value || offset > max_value || current >= max_value || max_value - offset <= offset)
  {
...
/*********************************************************************/ /**
 ...
 lacks one.
 @return InnoDB transaction handle */
static inline trx_t *check_trx_exists(
    /*=============*/
    THD *thd) /*!< in: user thread handle */
{
  trx_t *&trx = thd_to_trx(thd);
...
static void innodb_replace_trx_in_thd(THD *thd, void *new_trx_arg, void **ptr_trx_arg)
{
  ...
  ut_ad(new_trx_arg == NULL || (((trx_t *)new_trx_arg)->mysql_thd == thd && !((trx_t *)new_trx_arg)->is_recovered));
...
static inline bool trx_is_registered_for_2pc(
    /*=========================*/
...
/*********************************************************************/ /**
 ...
 InnoDB table object. */
void innobase_copy_frm_flags_from_create_info(
    /*=====================================*/
    ...
    const HA_CREATE_INFO *create_info) /*!< in: create info */
{
  ...
  dict_stats_auto_recalc_set(innodb_table, create_info->stats_auto_recalc == HA_STATS_AUTO_RECALC_ON,
    ...
    const TABLE_SHARE *table_share) /*!< in: table share */
{
  ...
  dict_stats_auto_recalc_set(innodb_table, table_share->stats_auto_recalc == HA_STATS_AUTO_RECALC_ON,
...
ha_innobase::ha_innobase(
    ...
    : handler(hton, table_arg),
      ...
      m_user_thd(),
      m_int_table_flags(HA_REC_NOT_IN_SEQ | HA_NULL_IN_KEY | HA_CAN_INDEX_BLOBS | HA_CAN_SQL_HANDLER |
                        HA_PRIMARY_KEY_REQUIRED_FOR_POSITION | HA_PRIMARY_KEY_IN_READ_INDEX | HA_BINLOG_ROW_CAPABLE |
                        HA_CAN_GEOMETRY | HA_PARTIAL_COLUMN_READ | HA_TABLE_SCAN_ON_INDEX | HA_CAN_FULLTEXT |
                        HA_CAN_FULLTEXT_EXT | HA_CAN_FULLTEXT_HINTS
#ifdef WL6742
                        | HA_HAS_RECORDS
#endif
                        | HA_CAN_EXPORT | HA_CAN_RTREEKEYS | HA_NO_READ_LOCAL_LOCK | HA_GENERATED_COLUMNS |
    ...
    THD *thd) /*!< in: thd to use the handle */
{
  ...
  DBUG_PRINT("ha_innobase::update_thd", ("user_thd: %p -> %p", m_user_thd, thd));
  ...
  ut_ad(dict_table_is_intrinsic(m_prebuilt->table) || trx_in_innodb.is_aborted() ||
        (trx->dict_operation_lock_mode == 0 && trx->dict_operation == TRX_DICT_OP_NONE));
...
static inline void innobase_register_trx(
    /*==================*/
...
/******************************************************************/ /**
 ...
 called explicitly */
static my_bool innobase_query_caching_of_table_permitted(
    /*======================================*/
...
void innobase_invalidate_query_cache(
    /*============================*/
                           ...
                           also the null chars count */
{
  ...
  mysql_query_cache_invalidate4(trx->mysql_thd, full_name, (uint32)full_name_len, TRUE);
}
...
{
  const int q =
      trx != NULL && trx->mysql_thd != NULL ? get_quote_char_for_identifier(trx->mysql_thd, id, strlen(id)) : '`';
...
char *innobase_convert_identifier(char *buf, ulint buflen, const char *id, ulint idlen, THD *thd)
{
  const char *s = id;
  ...
  s = nz2;
  idlen = explain_filename(thd, nz, nz2, sizeof nz2, EXPLAIN_PARTITIONS_AS_COMMENT);
  if (idlen > buflen)
  {
    idlen = buflen;
  }
    ...
    THD *thd)       /*!< in: MySQL connection thread, or NULL */
{
  ...
  {
    *s++ = '.';
    s = innobase_convert_identifier(s, bufend - s, slash + 1, idlen - (slash - id) - 1, thd);
  }
...
/*=============================*/
{
  ...
  DBUG_EXECUTE_IF(
      "free_table_in_fts_query", if (m_prebuilt->in_fts_query) { table->m_needs_reopen = true; });
...
static void innobase_space_shutdown()
/*=====================*/
{
...
static uint innobase_partition_flags() { return (HA_CAN_EXCHANGE_PARTITION | HA_CANNOT_PARTITION_FK); }

/** Deprecation message about InnoDB file format related parameters */
#define DEPRECATED_FORMAT_PARAMETER(x)  \
  "Using " x                            \
  " is deprecated and the parameter"    \
...
static const char *deprecated_large_prefix = DEPRECATED_FORMAT_PARAMETER("innodb_large_prefix");
/** Deprecation message about innodb_file_format_check */
static const char *deprecated_file_format_check = DEPRECATED_FORMAT_PARAMETER("innodb_file_format_check");
/** Deprecation message about innodb_file_format_max */
static const char *deprecated_file_format_max = DEPRECATED_FORMAT_PARAMETER("innodb_file_format_max");
...
{
  log_checksum_algorithm_ptr = check ? log_block_calc_checksum_crc32 : log_block_calc_checksum_none;
}
    ...
    void *p) /*!< in: InnoDB handlerton */
{
  ...
  innobase_hton->savepoint_rollback_can_release_mdl = innobase_rollback_to_savepoint_can_release_mdl;
  ...
  innobase_hton->flags = HTON_SUPPORTS_EXTENDED_KEYS | HTON_SUPPORTS_FOREIGN_KEYS | HTON_SUPPORTS_TABLE_ENCRYPTION;
  ...
  innobase_hton->rotate_encryption_master_key = innobase_encryption_key_rotation;

  ut_a(DATA_MYSQL_TRUE_VARCHAR == (ulint)MYSQL_TYPE_VARCHAR);

#ifndef NDEBUG
  static const char test_filename[] = "-@";
  char test_tablename[sizeof test_filename + sizeof(srv_mysql50_table_name_prefix) - 1];
  if ((sizeof(test_tablename)) - 1 !=
          filename_to_tablename(test_filename, test_tablename, sizeof(test_tablename), true) ||
      strncmp(test_tablename, srv_mysql50_table_name_prefix, sizeof(srv_mysql50_table_name_prefix) - 1) ||
      strcmp(test_tablename + sizeof(srv_mysql50_table_name_prefix) - 1, test_filename))
  {
  ...
  ulint fsp_flags = fsp_flags_init(univ_page_size, false, false, false, false);
  ...
  {
    ib::error() << "Unable to parse innodb_data_file_path=" << innobase_data_file_path;
    DBUG_RETURN(innobase_init_abort());
  }
  ...
  {
    ib::error() << "Unable to parse innodb_temp_data_file_path=" << innobase_temp_data_file_path;
    DBUG_RETURN(innobase_init_abort());
  }
  ...
  {
    sql_print_error("%s and %s file names seem to be the same.", srv_tmp_space.name(), srv_sys_space.name());
    DBUG_RETURN(innobase_init_abort());
  }
  ...
  if (innobase_change_buffering)
  {
    ...
    for (use = 0; use < UT_ARR_SIZE(innobase_change_buffering_values); use++)
    {
      if (!innobase_strcasecmp(innobase_change_buffering, innobase_change_buffering_values[use]))
      {
...
innobase_change_buffering_inited_ok:
  ...
  innobase_change_buffering = (char *)innobase_change_buffering_values[ibuf_use];
  ...
  if (!is_filename_allowed(srv_buf_dump_filename, strlen(srv_buf_dump_filename), FALSE))
  {
    sql_print_error(
  ...
  for (num_pll_degree = 1; num_pll_degree < fts_sort_pll_degree; num_pll_degree <<= 1)
  {
    /* No op */
  }
  ...
  {
    ib::error() << "You have created new InnoDB PFS key(s) but " << mysql_pfs_key_t::get_count() - global_count
  ...
  if (srv_buf_pool_size_org != 0)
  {
    ...
    innobase_buffer_pool_size = static_cast<long long>(srv_buf_pool_size_org);
  }
  else
  {
  ...
  innobase_old_blocks_pct = static_cast<uint>(buf_LRU_old_ratio_update(innobase_old_blocks_pct, TRUE));

  ibuf_max_size_update(srv_change_buffer_max_size);

  innobase_open_tables = hash_create(200);
  mysql_mutex_init(innobase_share_mutex_key.m_value, &innobase_share_mutex, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(commit_cond_mutex_key.m_value, &commit_cond_m, MY_MUTEX_INIT_FAST);
                      ...
                      whom the transaction should be committed */
{
  ...
  level. */
  trx->isolation_level = innobase_map_isolation_level(thd_get_trx_isolation(thd));
                       ...
                       statement only */
{
  ...
  ut_ad(trx_in_innodb.is_aborted() || (trx->dict_operation_lock_mode == 0 && trx->dict_operation == TRX_DICT_OP_NONE));
  ...
  if (rollback_trx || !thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))
  {
    ...
    if (trx->state == TRX_STATE_FORCED_ROLLBACK)
    {
      ...
      ib::info() << "Forced rollback : " << thd_security_context(thd, buffer, sizeof(buffer), 512);
#endif /* UNIV_DEBUG */
    ...
    void *savepoint)  /*!< in: savepoint data */
{
  ...
  dberr_t error = trx_rollback_to_savepoint_for_mysql(trx, name, &mysql_binlog_cache_pos);
                      ...
                      whose resources should be free'd */
{
  ...
  if (trx != NULL)
  {
    ...
    {
      sql_print_error(
          "Transaction not registered for MySQL"
    ...
    if (trx_is_started(trx))
    {
      ...
      else
      {
        sql_print_warning(
            ...
            "has an active InnoDB transaction.  " TRX_ID_FMT " row modifications will roll back.",
            trx->undo_no);
        ut_d(ib::warn() << "trx: " << trx << " started on: " << innobase_basename(trx->start_file) << ":"
                        << trx->start_line);
...
static const char *ha_innobase_exts[] = {dot_ext[IBD], dot_ext[ISL], NullS};
...
const char *ha_innobase::index_type(
    /*====================*/
    ...
    uint key, uint, bool) const
{
  ...
  {
    flags = HA_READ_NEXT | HA_READ_ORDER | HA_READ_RANGE | HA_KEYREAD_ONLY | HA_KEY_SCAN_NOT_ROR;
  }
...
const key_map *ha_innobase::keys_to_use_for_scanning()
/*===================================*/
{
  return (&key_map_full);
}
...
uint8 ha_innobase::table_cache_type()
/*===========================*/
{
  return (HA_CACHE_TBL_ASKTRANSACT);
}
...
void create_table_info_t::normalize_table_name_low(char *norm_name, const char *name, ibool set_lower_case)
{
  char *name_ptr;
  ulint name_len;
  char *db_ptr;
...
/*===========================*/
{
  ...
  for (size_t i = 0; i < UT_ARR_SIZE(test_data); i++)
  {
    ...
    create_table_info_t::normalize_table_name_low(norm_name, test_data[i][0], FALSE);
...
/*=================*/
{
  ...
  for (size_t i = 0; i < UT_ARR_SIZE(test_data); i++)
  {
    ...
    {
      ib::info() << "ut_format_name(" << test_data[i].name << ", buf, " << test_data[i].buf_size
                 << "),"
                    " expected "
    ...
    {
      ib::error() << "ut_format_name(" << test_data[i].name << ", buf, " << test_data[i].buf_size
                  << "),"
                     " expected "
  ...
  for (; key_part != key_end; ++key_part)
  {
    ...
    if (col_type != mtype)
    {
      ...
      switch (col_type)
      {
        case DATA_POINT:
        case DATA_VAR_POINT:
          if (DATA_POINT_MTYPE(mtype) || mtype == DATA_GEOMETRY || mtype == DATA_BLOB)
          {
            break;
          }
          /* Fall through */
...
*/
static void innobase_vcol_build_templ(const TABLE *table, dict_index_t *clust_index, Field *field,
                                      const dict_col_t *col, mysql_row_templ_t *templ, ulint col_no)
{
  ...
  {
    templ->mysql_length_bytes = static_cast<ulint>(((Field_varstring *)field)->length_bytes);
  }
...
void innobase_build_v_templ(const TABLE *table, const dict_table_t *ib_table, dict_vcol_templ_t *s_templ,
                            const dict_add_v_col_t *add_v, bool locked, const char *share_tbl_name)
{
  ...
  s_templ->vtempl = static_cast<mysql_row_templ_t **>(ut_zalloc_nokey((ncol + n_v_col) * sizeof *s_templ->vtempl));
  s_templ->n_col = ncol;
  s_templ->n_v_col = n_v_col;
  s_templ->rec_len = table->s->reclength;
  s_templ->default_rec = static_cast<byte *>(ut_malloc_nokey(table->s->reclength));
  ...
  for (ulint i = 0; i < table->s->fields; i++)
  {
    ...
    if (innobase_is_v_fld(field))
    {
      ...
      s_templ->vtempl[z + s_templ->n_col] =
          static_cast<mysql_row_templ_t *>(ut_malloc_nokey(sizeof *s_templ->vtempl[j]));

      innobase_vcol_build_templ(table, clust_index, field, &vcol->m_col, s_templ->vtempl[z + s_templ->n_col], z);
      z++;
      continue;
    }
    ...
    if (marker[j])
    {
      dict_col_t *col = dict_table_get_nth_col(ib_table, j);

#ifdef UNIV_DEBUG
      const char *name = dict_table_get_col_name(ib_table, j);

      ut_ad(!ut_strcmp(name, field->field_name));
#endif

      s_templ->vtempl[j] = static_cast<mysql_row_templ_t *>(ut_malloc_nokey(sizeof *s_templ->vtempl[j]));

      innobase_vcol_build_templ(table, clust_index, field, col, s_templ->vtempl[j], j);
    }
                            ...
                            will be constructed in. */
{
  ...
  {
    index_mapping =
        reinterpret_cast<dict_index_t **>(ut_realloc(index_mapping, mysql_num_index * sizeof(*index_mapping)));
    ...
    if (index_mapping == NULL)
    {
      ...
      sql_print_error(
          ...
          mysql_num_index, share->idx_trans_tbl.array_size);
      ret = false;
      goto func_exit;
    }
  ...
  for (ulint count = 0; count < mysql_num_index; count++)
  {
    ...
    index sequence */
    index_mapping[count] = dict_table_get_index_on_name(ib_table, table->key_info[count].name);
...
static dict_index_t *innobase_index_lookup(
    ...
    uint keynr)            /*!< in: index number for the requested
                           index */
{
  if (share->idx_trans_tbl.index_mapping == NULL || keynr >= share->idx_trans_tbl.index_count)
  {
    return (NULL);
  }
...
/*======================================*/
{
  ...
  else
  {
    ...
    if (m_prebuilt->table != NULL && dict_table_is_intrinsic(m_prebuilt->table))
    {
      ulint col_no = dict_col_get_no(dict_table_get_nth_col(m_prebuilt->table, field->field_index));

      col_name = dict_table_get_col_name(m_prebuilt->table, col_no);
    }
    ...
    uint test_if_locked) /*!< in: not used */
{
  ...
  if (ib_table != NULL && ((!DICT_TF2_FLAG_IS_SET(ib_table, DICT_TF2_FTS_HAS_DOC_ID) &&
                            table->s->fields != dict_table_get_n_tot_u_cols(ib_table)) ||
                           (DICT_TF2_FLAG_IS_SET(ib_table, DICT_TF2_FTS_HAS_DOC_ID) &&
                            (table->s->fields != dict_table_get_n_tot_u_cols(ib_table) - 1))))
  {
    ib::warn() << "Table " << norm_name << " contains " << dict_table_get_n_user_cols(ib_table)
               << " user"
  ...
  if (ib_table != NULL && dict_table_is_encrypted(ib_table) && ib_table->ibd_file_missing &&
  ...
  {
    ib_senderrf(thd, IB_LOG_LEVEL_WARN, ER_TABLESPACE_DISCARDED, table->s->table_name.str);
  ...
  if (ib_table->n_v_cols)
  {
    ...
    {
      innobase_build_v_templ(table, ib_table, ib_table->vc_templ, NULL, true, m_share->table_name);
    }
  ...
  if (m_prebuilt->table != NULL)
  {
    ...
    trx_sys_file_format_max_upgrade((const char **)&innobase_file_format_max, dict_table_get_format(m_prebuilt->table));
  }

  /* Only if the table has an AUTOINC column. */
  if (m_prebuilt->table != NULL && !m_prebuilt->table->ibd_file_missing && table->found_next_number_field != NULL)
  {
  ...
  for (uint i = 0; i < table->s->keys; i++)
  {
    if (table->key_info[i].flags & HA_USES_PARSER)
    {
      ...
      index->parser = static_cast<st_mysql_ftparser *>(plugin_decl(parser)->info);

      index->is_ngram = strncmp(plugin_name(parser)->str, FTS_NGRAM_PARSER_NAME, plugin_name(parser)->length) == 0;

      DBUG_EXECUTE_IF("fts_instrument_use_default_parser", index->parser = &fts_default_parser;);
    }
  }
...
dict_table_t *ha_innobase::open_dict_table(const char *table_name, const char *norm_name, bool is_partition,
                                           dict_err_ignore_t ignore_err)
{
  DBUG_ENTER("ha_innobase::open_dict_table");
  dict_table_t *ib_table = dict_table_open_on_name(norm_name, FALSE, TRUE, ignore_err);
  ...
  if (NULL == ib_table && is_partition)
  {
    ...
    if (innobase_get_lower_case_table_names() == 1)
    {
      ...
      create_table_info_t::normalize_table_name_low(par_case_name, table_name, FALSE);
#endif
      ib_table = dict_table_open_on_name(par_case_name, FALSE, TRUE, ignore_err);
    }
    ...
    MEM_ROOT *mem_root) /*!< in: memory context */
{
  ...
  ha_innobase *new_handler = dynamic_cast<ha_innobase *>(handler::clone(name, mem_root));
    ...
    const void *p2) /*!< in: node */
{
  ...
  return (
      ha_compare_text(charset, s1->f_str, static_cast<uint>(s1->f_len), s2->f_str, static_cast<uint>(s2->f_len), 0, 0));
}
    ...
    const void *p2) /*!< in: node */
{
  ...
  return (
      ha_compare_text(charset, s1->f_str, static_cast<uint>(s1->f_len), s2->f_str, static_cast<uint>(newlen), 0, 0));
}
    ...
    const void *p2) /*!< in: value to compare */
{
  ...
  result =
      ha_compare_text(charset, s2->f_str, static_cast<uint>(s2->f_len), s1->f_str, static_cast<uint>(s1->f_len), 1, 0);
    ...
    ulint num_v)         /*!< in: num virtual column so far */
{
  ...
  if (bitmap_is_set(table->read_set, static_cast<uint>(i)) || bitmap_is_set(table->write_set, static_cast<uint>(i)))
  {
    /* This field is needed in the query */
  ...
  ut_ad(i >= num_v);
  if (fetch_primary_key_cols && dict_table_col_in_clustered_key(index->table, i - num_v))
  {
    /* This field is needed in the query */
...
/*!< in: a virtual column or not */
{
  ...
  return (index == prebuilt->index ? contains : dict_index_contains_col_or_prefix(prebuilt->index, i, is_virtual));
}
    ...
    ulint v_no)                /*!< in: field index for virtual col */
{
  ...
  else
  {
    ...
    {
      templ->rec_field_no = dict_index_get_nth_col_or_prefix_pos(index, v_no, FALSE, true);
      ...
      templ->icp_rec_field_no = templ->rec_field_no != ULINT_UNDEFINED
                                    ? templ->rec_field_no
                                    : dict_index_get_nth_col_or_prefix_pos(index, v_no, true, true);
    }
  }
  ...
  if (!dict_index_is_clust(index) && templ->rec_field_no == ULINT_UNDEFINED)
  {
  ...
  if (prebuilt->mysql_prefix_len < templ->mysql_col_offset + templ->mysql_col_len)
  {
                    ...
                    false=ROW_MYSQL_REC_FIELDS */
{
  ...
  {
    m_prebuilt->mysql_template = (mysql_row_templ_t *)ut_malloc_nokey(n_fields * sizeof(mysql_row_templ_t));
  }
  ...
  if (active_index != MAX_KEY && active_index == pushed_idx_cond_keyno)
  {
    ...
    for (i = 0; i < n_fields; i++)
    {
      ...
      {
        index_contains = dict_index_contains_col_or_prefix(index, i - num_v, false);
      }
      ...
      if (build_template_needs_field_in_icp(index, m_prebuilt, index_contains, is_v ? num_v : i - num_v, is_v))
      {
        /* Needed in ICP */
        const Field *field;
        mysql_row_templ_t *templ;
        ...
        {
          field = build_template_needs_field(index_contains, m_prebuilt->read_just_key, fetch_all_in_key,
        ...
        templ = build_template_field(m_prebuilt, clust_index, index, table, field, i - num_v, 0);
        ...
        {
          templ->icp_rec_field_no = dict_index_get_nth_col_pos(m_prebuilt->index, i - num_v);
        }
        ...
        templ->icp_rec_field_no = dict_index_get_nth_col_or_prefix_pos(m_prebuilt->index, i - num_v, true, false);
        ut_ad(templ->icp_rec_field_no != ULINT_UNDEFINED);
    ...
    for (i = 0; i < n_fields; i++)
    {
      ...
      if (!build_template_needs_field_in_icp(index, m_prebuilt, index_contains, is_v ? num_v : i - num_v, is_v))
      {
        ...
        templ = build_template_field(m_prebuilt, clust_index, index, table, field, i - num_v, num_v);
    ...
    for (i = 0; i < n_fields; i++)
    {
      ...
      if (whole_row)
      {
        ...
        if (innobase_is_v_fld(table->field[i]) && m_prebuilt->read_just_key &&
            !dict_index_contains_col_or_prefix(m_prebuilt->index, num_v, true))
        {
      ...
      else
      {
        ...
        field = build_template_needs_field(contain, m_prebuilt->read_just_key, fetch_all_in_key, fetch_primary_key_cols,
                                           index, table, i, num_v);
        if (!field)
        {
          if (innobase_is_v_fld(table->field[i]))
...
/*====================================*/
{
  ...
  switch (lock_mode)
  {
    ...
    case AUTOINC_NEW_STYLE_LOCKING:
      ...
      if (thd_sql_command(m_user_thd) == SQLCOM_INSERT || thd_sql_command(m_user_thd) == SQLCOM_REPLACE)
      {
...
int ha_innobase::intrinsic_table_write_row(uchar *record)
{
  ...
  if (m_prebuilt->mysql_template == NULL || m_prebuilt->template_type != ROW_MYSQL_WHOLE_ROW)
  {
    ...
    uchar *record) /*!< in: a row in MySQL format */
{
  ...
  if (!dict_table_is_intrinsic(m_prebuilt->table) && trx_in_innodb.is_aborted())
  {
  ...
  else if (m_prebuilt->trx != trx)
  {
    ib::error() << "The transaction object for the table handle is"
                   " at "
                << static_cast<const void *>(m_prebuilt->trx) << ", but for the current thread it is at "
      ...
      m_num_write_row >= 10000)
  {
    ...
    src_table = lock_get_src_table(m_prebuilt->trx, m_prebuilt->table, &mode);
    if (!src_table)
    {
    no_commit:
    ...
    else
    {
      ...
      if (!lock_is_table_exclusive(m_prebuilt->table, m_prebuilt->trx))
      {
        goto no_commit;
      }
...
report_error:
  ...
  {
    ib_senderrf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_DISCARDED, table->s->table_name.str);
  }

  error_result = convert_error_code_to_mysql(error, m_prebuilt->table->flags, m_user_thd);
...
static byte *innodb_fill_old_vcol_val(row_prebuilt_t *prebuilt, dfield_t *vfield, ulint o_len, dict_col_t *col,
                                      const byte *old_mysql_row_col, ulint col_pack_len, byte *buf)
{
  dict_col_copy_type(col, dfield_get_type(vfield));
  if (o_len != UNIV_SQL_NULL)
  {
    buf = row_mysql_store_col_in_innobase_format(vfield, buf, TRUE, old_mysql_row_col, col_pack_len,
    ...
    THD *thd)                 /*!< in: user thread */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    if (is_virtual)
    {
      ...
      if (!col->ord_part)
      {
        ...
        if (dict_index_is_online_ddl(clust_index) && row_log_col_is_indexed(clust_index, num_v))
        {
#ifdef UNIV_DEBUG
      ...
      if (!uvect->old_vrow)
      {
        uvect->old_vrow = dtuple_create_with_vcol(uvect->heap, 0, prebuilt->table->n_v_cols);
      }
      ...
      if (o_len != UNIV_SQL_NULL && n_len != UNIV_SQL_NULL && o_len >= max_field_len && n_len >= max_field_len &&
          memcmp(o_ptr, n_ptr, max_field_len) == 0)
      {
        dfield_t *vfield = dtuple_get_nth_v_field(uvect->old_vrow, num_v);
        buf = innodb_fill_old_vcol_val(prebuilt, vfield, o_len, col, old_mysql_row_col, col_pack_len, buf);
        num_v++;
        continue;
      }
    }
    ...
    if (o_len != n_len || (o_len != UNIV_SQL_NULL && 0 != memcmp(o_ptr, n_ptr, o_len)))
    {
      ...
      if (is_virtual)
      {
        ...
        ufield->old_v_val = static_cast<dfield_t *>(mem_heap_alloc(uvect->heap, sizeof *ufield->old_v_val));
        ...
        if (!field->is_null_in_record(old_row))
        {
          ...
          buf = row_mysql_store_col_in_innobase_format(&dfield, (byte *)buf, TRUE, old_mysql_row_col, col_pack_len,
                                                       dict_table_is_comp(prebuilt->table));
        ...
        {
          dict_col_copy_type(col, dfield_get_type(ufield->old_v_val));
      ...
      {
        ufield->field_no = dict_col_get_clust_pos(&prebuilt->table->cols[i - num_v], clust_index);
  ...
  else if (changes_fts_column || changes_fts_doc_col)
  {
    ...
    if (!DICT_TF2_FLAG_IS_SET(innodb_table, DICT_TF2_FTS_HAS_DOC_ID))
    {
      ...
      {
        ib::warn() << "FTS Doc ID must be larger than " << innodb_table->fts->cache->next_doc_id - 1 << " for table "
                   << innodb_table->name;

        return (DB_FTS_INVALID_DOCID);
      }
      else if ((doc_id - prebuilt->table->fts->cache->next_doc_id) >= FTS_DOC_ID_MAX_STEP)
      {
        ib::warn() << "Doc ID " << doc_id
                   << " is too"
                      " big. Its difference with largest"
                      " Doc ID used "
                   << prebuilt->table->fts->cache->next_doc_id - 1 << " cannot exceed or equal to "
                   << FTS_DOC_ID_MAX_STEP;
      }
...
int ha_innobase::update_row(const uchar *old_row, uchar *new_row)
{
  ...
  if (m_upd_buf == NULL)
  {
    ...
    m_upd_buf_size = table->s->reclength + table->s->max_key_length + MAX_REF_PARTS * 3;

    m_upd_buf = reinterpret_cast<uchar *>(my_malloc(PSI_INSTRUMENT_ME, m_upd_buf_size, MYF(MY_WME)));
  ...
  error = calc_row_difference(uvect, old_row, new_row, table, m_upd_buf, m_upd_buf_size, m_prebuilt, m_user_thd);
  ...
  if (!dict_table_is_intrinsic(m_prebuilt->table) && TrxInInnoDB::is_aborted(trx))
  {
    innobase_rollback(ht, m_user_thd, false);
...
func_exit:
  ...
  err = convert_error_code_to_mysql(error, m_prebuilt->table->flags, m_user_thd);
    ...
    const uchar *record) /*!< in: a row in MySQL format */
{
  ...
  DBUG_RETURN(convert_error_code_to_mysql(error, m_prebuilt->table->flags, m_user_thd));
}
...
/*=========================*/
{
  ...
  if (m_prebuilt->select_lock_type == LOCK_NONE || dict_table_is_intrinsic(m_prebuilt->table))
  {
    DBUG_VOID_RETURN;
  }
  ...
  ut_ad(trx_state_eq(m_prebuilt->trx, TRX_STATE_ACTIVE) || trx_state_eq(m_prebuilt->trx, TRX_STATE_FORCED_ROLLBACK));

  switch (m_prebuilt->row_read_type)
  {
    case ROW_READ_WITH_LOCKS:
      if (!srv_locks_unsafe_for_binlog && m_prebuilt->trx->isolation_level > TRX_ISO_READ_COMMITTED)
      {
        break;
      }
      /* fall through */
...
/*===========================================*/
{
  ...
  if (yes && (srv_locks_unsafe_for_binlog || m_prebuilt->trx->isolation_level <= TRX_ISO_READ_COMMITTED))
  {
    m_prebuilt->row_read_type = ROW_READ_TRY_SEMI_CONSISTENT;
  }
  else
    ...
    enum ha_rkey_function find_flag) /*!< in: search flags from my_base.h */
{
  ...
  {
    DBUG_RETURN(dict_index_is_corrupted(index) ? HA_ERR_INDEX_CORRUPT : HA_ERR_TABLE_DEF_CHANGED);
  }
  ...
  if (key_ptr != NULL)
  {
    ...
    row_sel_convert_mysql_key_to_innobase(m_prebuilt->search_tuple, m_prebuilt->srch_key_val1,
  ...
  switch (ret)
  {
    case DB_SUCCESS:
      error = 0;
      table->status = 0;
      srv_stats.n_rows_read.add(thd_get_thread_id(m_prebuilt->trx->mysql_thd), 1);
      break;
    ...
    case DB_TABLESPACE_DELETED:
      ib_senderrf(m_prebuilt->trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_DISCARDED, table->s->table_name.str);
    ...
    case DB_TABLESPACE_NOT_FOUND:
      ...
      ib_senderrf(m_prebuilt->trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_MISSING, table->s->table_name.str);
                ...
                generated by InnoDB */
{
    ...
    else
    {
      ...
      if (m_share->idx_trans_tbl.index_mapping != NULL)
      {
        sql_print_warning(
            ...
            key ? key->name : "NULL", keynr, m_prebuilt->table->name.m_name);
      }
  ...
  if (index == NULL)
  {
    sql_print_error(
        ...
        keynr, key ? key->name : "NULL", m_prebuilt->table->name.m_name);
  }
                ...
                InnoDB */
{
  ...
  {
    sql_print_warning("InnoDB: change_active_index(%u) failed", keynr);
    m_prebuilt->index_usable = FALSE;
    DBUG_RETURN(1);
  }

  m_prebuilt->index_usable = row_merge_is_index_usable(m_prebuilt->trx, m_prebuilt->index);
  ...
  if (!m_prebuilt->index_usable)
  {
    if (dict_index_is_corrupted(m_prebuilt->index))
    {
      ...
      innobase_format_name(table_name, sizeof table_name, m_prebuilt->index->table->name.m_name);

      if (dict_index_is_clust(m_prebuilt->index))
      {
        ut_ad(m_prebuilt->index->table->corrupted);
        push_warning_printf(m_user_thd, Sql_condition::SL_WARNING, HA_ERR_TABLE_CORRUPT,
                            "InnoDB: Table %s is corrupted.", table_name);
        DBUG_RETURN(HA_ERR_TABLE_CORRUPT);
      }
      else
      {
        push_warning_printf(m_user_thd, Sql_condition::SL_WARNING, HA_ERR_INDEX_CORRUPT,
    ...
    {
      push_warning_printf(m_user_thd, Sql_condition::SL_WARNING, HA_ERR_TABLE_DEF_CHANGED,
  ...
  if ((m_prebuilt->index->type & DICT_FTS))
  {
    if (table->fts_doc_id_field &&
        bitmap_is_set(table->read_set, table->fts_doc_id_field->field_index && m_prebuilt->read_just_key))
    {
  ...
  else
  {
    ...
    dict_index_copy_types(m_prebuilt->search_tuple, m_prebuilt->index, m_prebuilt->index->n_fields);

    /* If it's FTS query and FTS_DOC_ID exists FTS_DOC_ID field is
    always added to read_set. */
    m_prebuilt->fts_doc_id_in_read_set =
        (m_prebuilt->read_just_key && table->fts_doc_id_field && m_prebuilt->in_fts_query);
  }
                     ...
                     ROW_SEL_EXACT_PREFIX */
{
  ...
  {
    ret = row_search_no_mvcc(buf, PAGE_CUR_UNSUPP, m_prebuilt, match_mode, direction);
  }
  ...
  switch (ret)
  {
    ...
    case DB_TABLESPACE_NOT_FOUND:
      ...
      ib_senderrf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_MISSING, table->s->table_name.str);
    ...
    String *key) /* in: */
{
  ...
  we proceed. */
  if (strcmp(char_set->csname, "utf32") == 0 || strcmp(char_set->csname, "utf16") == 0)
  {
    buf_tmp_used = innobase_convert_string(buf_tmp, sizeof(buf_tmp) - 1, &my_charset_utf8_general_ci, query, query_len,
                                           (CHARSET_INFO *)char_set, &num_errors);
  ...
  if (keynr == NO_SUCH_KEY)
  {
    ...
    index = reinterpret_cast<dict_index_t *>(ib_vector_getp(ft_table->fts->indexes, 0));
  }
  else
  {
  ...
  dberr_t error = fts_query(trx, index, flags, q, query_len, &result, m_prebuilt->m_fts_limit);
  ...
  fts_hdl = reinterpret_cast<NEW_FT_INFO *>(my_malloc(PSI_INSTRUMENT_ME, sizeof(NEW_FT_INFO), MYF(0)));
    ...
    uchar *buf) /*!< in/out: buf contain result row */
{
  ...
  if (result->current == NULL)
  {
    ...
    if (result->rankings_by_id != NULL)
    {
      ...
      result->current = const_cast<ib_rbt_node_t *>(rbt_first(result->rankings_by_rank));
    }
    else
    {
  ...
  {
    result->current = const_cast<ib_rbt_node_t *>(rbt_next(result->rankings_by_rank, result->current));
  }
...
next_record:
  ...
  if (result->current != NULL)
  {
    ...
    dberr_t ret = row_search_for_mysql((byte *)buf, PAGE_CUR_GE, m_prebuilt, ROW_SEL_EXACT, 0);
...
static bool create_table_check_doc_id_col(
    /*==========================*/
                       ...
                       wrong type/name/size */
{
  for (ulint i = 0; i < form->s->fields; i++)
  {
    ...
    if (innobase_strcasecmp(field->field_name, FTS_DOC_ID_COL_NAME) == 0)
    {
      ...
      if (col_type == DATA_INT && !field->real_maybe_null() && col_len == sizeof(doc_id_t) &&
      ...
      {
        push_warning_printf(trx->mysql_thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
...
void innodb_base_col_setup(dict_table_t *table, const Field *field, dict_v_col_t *v_col)
{
  ...
  for (uint i = 0; i < field->table->s->fields; ++i)
  {
    ...
    if (!base_field->is_virtual_gcol() && bitmap_is_set(&field->gcol_info->base_columns_map, i))
    {
      ulint z;
...
void innodb_base_col_setup_for_stored(const dict_table_t *table, const Field *field, dict_s_col_t *s_col)
{
  ...
  for (uint i = 0; i < field->table->s->fields; ++i)
  {
    ...
    if (!innobase_is_v_fld(base_field) && bitmap_is_set(&field->gcol_info->base_columns_map, i))
    {
      ulint z;
      for (z = 0; z < table->n_cols; z++)
      {
...
inline MY_ATTRIBUTE((warn_unused_result)) int create_table_info_t::create_table_def()
{
  dict_table_t *table;
  ulint n_cols;
  dberr_t err;
  ...
  {
    push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_TABLE_NAME, "InnoDB: Table name is empty");
  ...
  table = dict_mem_table_create(m_table_name, space_id, actual_n_cols, num_v, m_flags, m_flags2);
  ...
  {
    table->dir_path_of_temp_table = mem_heap_strdup(table->heap, m_temp_path);
  }
  ...
  for (i = 0; i < n_cols; i++)
  {
    ...
    {
      ut_snprintf(field_name, sizeof(field_name), "%s_%s_%lu", field->orig_table->alias, field->field_name, i);
    }
    else
    {
      ut_snprintf(field_name, sizeof(field_name), "%s", field->field_name);
    }
    ...
    if (!col_type)
    {
      push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_CANT_CREATE_TABLE,
    ...
    if (dtype_is_string_type(col_type))
    {
      ...
      DBUG_EXECUTE_IF("simulate_max_char_col", charset_no = MAX_CHAR_COLL_NUM + 1;);
    ...
    {
      dict_mem_table_add_v_col(table, heap, field_name, col_type,
                               dtype_form_prtype((ulint)field->type() | nulls_allowed | unsigned_type | binary_type |
    ...
    if (is_stored)
    {
      ...
      dict_mem_table_add_s_col(table, field->gcol_info->non_virtual_base_columns());
    }
  }
  ...
  if (dict_table_is_temporary(table))
  {
    ...
    else if (m_create_info->encrypt_type.length > 0 && !Encryption::is_none(m_create_info->encrypt_type.str))
    {
  ...
  else
  {
    ...
    if (!(m_flags2 & DICT_TF2_USE_FILE_PER_TABLE) && m_create_info->compress.length > 0 &&
    ...
    else if (Compression::validate(algorithm) != DB_SUCCESS || m_form->s->row_type == ROW_TYPE_COMPRESSED ||
    ...
    if (!(m_flags2 & DICT_TF2_USE_FILE_PER_TABLE) && m_create_info->encrypt_type.length > 0 &&
    ...
    if (err == DB_SUCCESS)
    {
      err = row_create_table_for_mysql(table, algorithm, m_trx, false);
    }
  ...
  DBUG_EXECUTE_IF("ib_create_err_tablespace_exist", err = DB_TABLESPACE_EXISTS;);

  if (err == DB_DUPLICATE_KEY || err == DB_TABLESPACE_EXISTS)
  {
    char display_name[FN_REFLEN];
    char *buf_end =
        innobase_convert_identifier(display_name, sizeof(display_name) - 1, m_table_name, strlen(m_table_name), m_thd);

    *buf_end = '\0';

    my_error(err == DB_DUPLICATE_KEY ? ER_TABLE_EXISTS_ERROR : ER_TABLESPACE_EXISTS, MYF(0), display_name);
  }
    ...
    uint key_num)           /*!< in: index number */
{
  ...
  if (ind_type != 0)
  {
    ...
    for (ulint i = 0; i < key->user_defined_key_parts; i++)
    {
      ...
      dict_mem_index_add_field(index, key_part->field->field_name, 0);
    }

    DBUG_RETURN(convert_error_code_to_mysql(row_create_index_for_mysql(index, trx, NULL, NULL), flags, NULL));
  }
  ...
  field_lengths =
      (ulint *)my_malloc(PSI_INSTRUMENT_ME, key->user_defined_key_parts * sizeof *field_lengths, MYF(MY_FAE));
  ...
  for (ulint i = 0; i < key->user_defined_key_parts; i++)
  {
    ...
    if (handler != NULL && dict_table_is_intrinsic(handler))
    {
      ...
      ulint col_no = dict_col_get_no(dict_table_get_nth_col(handler, key_part->field->field_index));
  ...
  error = convert_error_code_to_mysql(row_create_index_for_mysql(index, trx, field_lengths, handler), flags, NULL);
    ...
    const char *table_name) /*!< in: table name */
{
  ...
  /* We pass 0 as the space id, and determine at a lower level the space
  id where to store the table */
  index = dict_mem_index_create(table_name, innobase_index_reserve_name, 0, DICT_CLUSTERED, 0);
...
bool create_table_info_t::create_option_data_directory_is_valid()
{
  ...
  ut_ad(m_create_info->data_file_name && m_create_info->data_file_name[0] != '\0');
...
static int validate_tablespace_name(const char *name, bool for_table)
{
  ...
  if (strlen(name) >= sizeof(reserved_space_name_prefix) - 1 &&
      0 == memcmp(name, reserved_space_name_prefix, sizeof(reserved_space_name_prefix) - 1))
  {
    /* Use a different message for reserved names */
    if (0 == strcmp(name, reserved_file_per_table_space_name) || 0 == strcmp(name, reserved_system_space_name) ||
...
bool create_table_info_t::create_option_tablespace_is_valid()
{
  if (!m_use_shared_space)
  {
    ...
    if ((m_create_info->options & HA_LEX_CREATE_TMP_TABLE) && tablespace_is_file_per_table(m_create_info))
    {
      ...
      push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
  ...
  ulint space_id = fil_space_get_id_by_name(m_create_info->tablespace);
  ...
  if (is_create_table && m_create_info->data_file_name != NULL && m_create_info->data_file_name[0] != '\0')
  {
    my_printf_error(ER_ILLEGAL_HA_CREATE_OPTION,
                    "InnoDB: DATA DIRECTORY cannot be used"
  ...
  if (m_create_info->options & HA_LEX_CREATE_TMP_TABLE)
  {
    ...
    if (m_create_info->key_block_size || m_create_info->row_type == ROW_TYPE_COMPRESSED)
    {
      my_printf_error(ER_ILLEGAL_HA_CREATE_OPTION,
  ...
  {
    block_size_needed = ut_min(UNIV_PAGE_SIZE / 2, static_cast<ulint>(UNIV_ZIP_SIZE_MAX));
    table_is_compressed = true;
  }
  else
  {
  ...
  if (block_size_needed != page_size.physical())
  {
    my_printf_error(ER_ILLEGAL_HA_CREATE_OPTION,
                    ...
                    " page size " ULINTPF,
                    MYF(0), m_create_info->tablespace, page_size.physical(), block_size_needed);
    return (false);
  }
...
bool create_table_info_t::create_option_compression_is_valid()
{
  ...
  {
    push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_UNSUPPORTED_EXTENSION,
                        "InnoDB: Unsupported compression algorithm '%s'", m_create_info->compress.str);
    return (false);
  }
  ...
  if (m_create_info->key_block_size != 0 || m_create_info->row_type == ROW_TYPE_COMPRESSED)
  {
    push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_UNSUPPORTED_EXTENSION,
  ...
  if (m_create_info->options & HA_LEX_CREATE_TMP_TABLE)
  {
    push_warning_printf(m_thd, Sql_condition::SL_WARNING, HA_ERR_UNSUPPORTED, "%s for temporary tables", intro);
    return (false);
  }

  if (tablespace_is_general_space(m_create_info))
  {
    push_warning_printf(m_thd, Sql_condition::SL_WARNING, HA_ERR_UNSUPPORTED, "%s for shared general tablespaces",
                        intro);
    return (false);
  }
  ...
  {
    push_warning_printf(m_thd, Sql_condition::SL_WARNING, HA_ERR_UNSUPPORTED, "%s for the system tablespace", intro);
    return (false);
  }
...
const char *create_table_info_t::create_options_are_invalid()
{
  ...
  if (has_key_block_size)
  {
    switch (m_create_info->key_block_size)
    {
      ...
      case 16:
        ...
        KBS is also smaller. */
        kbs_max = ut_min(1 << (UNIV_PAGE_SSIZE_MAX - 1), 1 << (PAGE_ZIP_SSIZE_MAX - 1));
  ...
  if (m_create_info->data_file_name && m_create_info->data_file_name[0] != '\0' &&
  ...
  if ((has_key_block_size || row_format == ROW_TYPE_COMPRESSED) && UNIV_PAGE_SIZE > UNIV_PAGE_SIZE_DEF)
  {
    push_warning(m_thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
    ...
    HA_CREATE_INFO *create_info) /*!< in/out: create info */
{
  ...
  if (m_prebuilt->table->tablespace != NULL && create_info->tablespace == NULL)
  {
...
{
  return (fts_load_stopword(table, trx, innobase_server_stopword_table, THDVAR(thd, ft_user_stopword_table),
...
int create_table_info_t::parse_table_name(const char *name)
{
  ...
  if (m_innodb_file_per_table && !mysqld_embedded && !(m_create_info->options & HA_LEX_CREATE_TMP_TABLE))
  {
  ...
  if ((m_create_info->options & HA_LEX_CREATE_TMP_TABLE) && !m_use_shared_space)
  {
  ...
  if (m_create_info->data_file_name && m_create_info->data_file_name[0] != '\0')
  {
    ...
    {
      push_warning_printf(m_thd, Sql_condition::SL_WARNING, WARN_OPTION_IGNORED, ER_DEFAULT(WARN_OPTION_IGNORED),
                          "DATA DIRECTORY");
...
bool create_table_info_t::innobase_table_flags()
{
  ...
  const ulint zip_ssize_max = ut_min(static_cast<ulint>(UNIV_PAGE_SSIZE_MAX), static_cast<ulint>(PAGE_ZIP_SSIZE_MAX));
  ...
  if (m_create_info->encrypt_type.length > 0)
  {
    ...
    if (m_use_shared_space || (m_create_info->options & HA_LEX_CREATE_TMP_TABLE) || !m_use_file_per_table)
    {
  ...
  for (uint i = 0; i < m_form->s->keys; i++)
  {
    ...
    {
      if (m_create_info->options & HA_LEX_CREATE_TMP_TABLE &&
          m_create_info->options & HA_LEX_CREATE_INTERNAL_TMP_TABLE && !m_use_file_per_table)
      {
    ...
    if (!(key->flags & HA_NOSAME) || strcmp(key->name, FTS_DOC_ID_INDEX_NAME) ||
  ...
  if (m_create_info->key_block_size > 0)
  {
    ...
    if (file_format_allowed < UNIV_FORMAT_B && !m_use_shared_space)
    {
      push_warning(m_thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
                   "InnoDB: KEY_BLOCK_SIZE requires"
                   " innodb_file_format > Antelope.");
    ...
    if (!zip_allowed || zssize > zip_ssize_max)
    {
      push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
                          "InnoDB: ignoring KEY_BLOCK_SIZE=%lu.", m_create_info->key_block_size);
    }
  }
  ...
  switch (row_type)
  {
    ...
    case ROW_TYPE_COMPRESSED:
      ...
      else if (file_format_allowed == UNIV_FORMAT_A && !m_use_shared_space)
      {
        push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
                            "InnoDB: ROW_FORMAT=COMPRESSED requires"
                            " innodb_file_format > Antelope.");
  ...
  if (m_create_info->options & HA_LEX_CREATE_TMP_TABLE)
  {
    ...
    if ((m_create_info->options & HA_LEX_CREATE_INTERNAL_TMP_TABLE) && !m_use_file_per_table)
    {
    ...
    if (m_create_info->tablespace != NULL && strcmp(m_create_info->tablespace, reserved_temporary_space_name) == 0)
    {
      push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
  ...
  /* Set the table flags */
  dict_tf_set(&m_flags, innodb_row_format, zip_ssize, m_use_data_dir, m_use_shared_space);
  ...
  DBUG_EXECUTE_IF("innodb_test_wrong_fts_aux_table_name", m_flags2 &= ~DICT_TF2_FTS_AUX_HEX_NAME;);
...
static ulint innobase_parse_merge_threshold(THD *thd, const char *str)
{
  ...
  push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
...
void innobase_parse_hint_from_comment(THD *thd, dict_table_t *table, const TABLE_SHARE *table_share)
{
  ...
  {
    merge_threshold_table = innobase_parse_merge_threshold(thd, table_share->comment.str);
  }
  else
  {
  ...
  for (uint i = 0; i < table_share->keys; i++)
  {
    ...
    {
      merge_threshold_index[i] = innobase_parse_merge_threshold(thd, key_info->comment.str);
    }
    else
    {
  ...
  if (!dict_table_is_temporary(table))
  {
         ...
         index = UT_LIST_GET_NEXT(indexes, index))
    {
      if (dict_index_is_auto_gen_clust(index))
      {
        ...
        dict_index_set_merge_threshold(index, merge_threshold_table);
        continue;
      }
      ...
      for (uint i = 0; i < table_share->keys; i++)
      {
        ...
        {
          dict_index_set_merge_threshold(index, merge_threshold_index[i]);
          is_found[i] = true;
          break;
        }
      }
  ...
  /* update in memory */
  for (dict_index_t *index = UT_LIST_GET_FIRST(table->indexes); index != NULL; index = UT_LIST_GET_NEXT(indexes, index))
  {
    if (dict_index_is_auto_gen_clust(index))
    {
...
/** Set m_use_* flags. */
void create_table_info_t::set_tablespace_type(bool table_being_altered_is_file_per_table)
{
  ...
  m_allow_file_per_table =
      m_innodb_file_per_table || table_being_altered_is_file_per_table || tablespace_is_file_per_table(m_create_info);
  ...
  bool is_noncompressed_temporary =
      ...
      !(m_create_info->row_type == ROW_TYPE_COMPRESSED || m_create_info->key_block_size > 0);
  ...
  m_use_file_per_table = m_allow_file_per_table && !is_noncompressed_temporary && !m_use_shared_space;

  /* DATA DIRECTORY must have m_use_file_per_table but cannot be
  used with TEMPORARY tables. */
  m_use_data_dir = m_use_file_per_table && !(m_create_info->options & HA_LEX_CREATE_TMP_TABLE) &&
                   (m_create_info->data_file_name != NULL) && (m_create_info->data_file_name[0] != '\0');
...
int create_table_info_t::initialize()
{
  ...
  if (innobase_index_name_is_reserved(m_thd, m_form->key_info, m_form->s->keys))
  {
...
int create_table_info_t::create_table()
{
  ...
  if (primary_key_no != -1)
  {
    ...
    first */
    if ((error = create_index(m_trx, m_form, m_flags, m_table_name, (uint)primary_key_no)))
    {
      DBUG_RETURN(error);
    }
  }
  ...
  if (m_flags2 & DICT_TF2_FTS)
  {
    ...
    innobase_table = dict_table_open_on_name(m_table_name, TRUE, FALSE, DICT_ERR_IGNORE_NONE);

    ut_a(innobase_table);

    /* Check whether there already exists FTS_DOC_ID_INDEX */
    ret = innobase_fts_check_doc_id_index_in_def(m_form->s->keys, m_form->key_info);

    switch (ret)
    {
      case FTS_INCORRECT_DOC_ID_INDEX:
        push_warning_printf(m_thd, Sql_condition::SL_WARNING, ER_WRONG_NAME_FOR_INDEX,
                            ...
                            " type\n",
                            FTS_DOC_ID_INDEX_NAME, innobase_table->name.m_name);
    ...
    dberr_t err = fts_create_common_tables(m_trx, innobase_table, m_table_name, (ret == FTS_EXIST_DOC_ID_INDEX));
  ...
  ut_ad(handler == NULL || (handler != NULL && dict_table_is_intrinsic(handler)));

  /* There is no concept of foreign key for intrinsic tables. */
  if (stmt && (handler == NULL))
  {
    dberr_t err = row_table_add_foreign_constraints(m_trx, stmt, stmt_len, m_table_name,
                                                    m_create_info->options & HA_LEX_CREATE_TMP_TABLE);

    switch (err)
    {
      case DB_PARENT_NO_INDEX:
        push_warning_printf(m_thd, Sql_condition::SL_WARNING, HA_ERR_CANNOT_ADD_FOREIGN,
...
int create_table_info_t::create_table_update_dict()
{
  ...
  innobase_table = thd_to_innodb_session(m_thd)->lookup_table_handler(m_table_name);

  if (innobase_table == NULL)
  {
    innobase_table = dict_table_open_on_name(m_table_name, FALSE, FALSE, DICT_ERR_IGNORE_NONE);
  }
  else
  {
  ...
  if (innobase_table->fts != NULL)
  {
    ...
    {
      innobase_table->fts_doc_id_index = dict_table_get_index_on_name(innobase_table, FTS_DOC_ID_INDEX_NAME);
      assert(innobase_table->fts_doc_id_index != NULL);
    }
    else
    {
      assert(innobase_table->fts_doc_id_index == dict_table_get_index_on_name(innobase_table, FTS_DOC_ID_INDEX_NAME));
    }
  }

  assert((innobase_table->fts == NULL) == (innobase_table->fts_doc_id_index == NULL));
  ...
  if (innobase_table)
  {
    ...
    trx_sys_file_format_max_upgrade((const char **)&innobase_file_format_max, dict_table_get_format(innobase_table));
  }
  ...
  if (((m_create_info->used_fields & HA_CREATE_USED_AUTO) || thd_sql_command(m_thd) == SQLCOM_ALTER_TABLE ||
       thd_sql_command(m_thd) == SQLCOM_OPTIMIZE || thd_sql_command(m_thd) == SQLCOM_CREATE_INDEX) &&
...
/** Allocate a new trx. */
void create_table_info_t::allocate_trx()
{
...
int ha_innobase::create(const char *name, TABLE *form, HA_CREATE_INFO *create_info)
{
  int error;
  ...
  create_table_info_t info(ha_thd(), form, create_info, norm_name, temp_path, remote_path, tablespace);
...
cleanup:
  ...
  else
  {
    ...
    dict_table_t *intrinsic_table = thd_to_innodb_session(thd)->lookup_table_handler(info.table_name());
    ...
    my_bool discard) /*!< in: TRUE if discard, else import */
{
  ...
  {
    ib_senderrf(m_prebuilt->trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_CANNOT_DISCARD_TEMPORARY_TABLE);
  ...
  {
    ib_senderrf(m_prebuilt->trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLE_IN_SYSTEM_TABLESPACE, dict_table->name.m_name);
  ...
  if (DICT_TF_HAS_SHARED_SPACE(dict_table->flags))
  {
    my_printf_error(ER_NOT_ALLOWED_COMMAND,
                    ...
                    MYF(0), discard ? "discard" : "import", dict_table->name.m_name);
  ...
  dberr_t err = row_mysql_lock_table(
      ...
      discard ? "setting table lock for DISCARD TABLESPACE" : "setting table lock for IMPORT TABLESPACE");
  ...
  else if (discard)
  {
    ...
    {
      ib_senderrf(m_prebuilt->trx->mysql_thd, IB_LOG_LEVEL_WARN, ER_TABLESPACE_MISSING, dict_table->name.m_name);
    }

    err = row_discard_tablespace_for_mysql(dict_table->name.m_name, m_prebuilt->trx);
  }
  else if (!dict_table->ibd_file_missing)
  {
    ...
    ib::error() << "Unable to import tablespace " << dict_table->name
                << " because it already"
                   ...
                   " before IMPORT.";
    ib_senderrf(m_prebuilt->trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_EXISTS, dict_table->name.m_name);
  ...
  else
  {
    ...
    if (err == DB_SUCCESS)
    {
      info(HA_STATUS_TIME | HA_STATUS_CONST | HA_STATUS_VARIABLE | HA_STATUS_AUTO);
    }
  }
  ...
  if (err == DB_SUCCESS && !discard && dict_stats_is_persistent_enabled(dict_table))
  {
    dberr_t ret;
...
/*===================*/
{
  ...
  switch (err)
  {
    ...
    default:
      error = convert_error_code_to_mysql(err, m_prebuilt->table->flags, m_prebuilt->trx->mysql_thd);
    ...
    const char *name) /*!< in: table name */
{
  ...
  if (handler != NULL)
  {
    for (dict_index_t *index = UT_LIST_GET_FIRST(handler->indexes); index != NULL && index->last_ins_cur;
         index = UT_LIST_GET_NEXT(indexes, index))
    {
  ...
  else if (srv_read_only_mode || srv_force_recovery >= SRV_FORCE_NO_UNDO_LOG_SCAN)
  {
    DBUG_RETURN(HA_ERR_TABLE_READONLY);
  }
  ...
  err = row_drop_table_for_mysql(norm_name, trx, thd_sql_command(thd) == SQLCOM_DROP_DB, true, handler);
  ...
  if (err == DB_TABLE_NOT_FOUND)
  {
    ...
    norm_name[len] = '#';
    norm_name[len + 1] = 0;
    err = row_drop_database_for_mysql(norm_name, trx, &num_partitions);
    norm_name[len] = 0;
    if (num_partitions == 0 && !row_is_mysql_tmp_table_name(norm_name))
    {
  ...
  if (err == DB_TABLE_NOT_FOUND && innobase_get_lower_case_table_names() == 1)
  {
#ifdef _WIN32
    ...
    if (is_part != NULL)
    {
      ...
      system table whose name is
      not being normalized to lower case */
      create_table_info_t::normalize_table_name_low(par_case_name, name, FALSE);
#endif /* _WIN32 */
      err = row_drop_table_for_mysql(par_case_name, trx, thd_sql_command(thd) == SQLCOM_DROP_DB, true, handler);
    }
  }
...
@return MySQL handler error code like HA_... */
static int validate_create_tablespace_info(THD *thd, st_alter_tablespace *alter_info)
{
  ulint space_id;
  ...
  int error = validate_tablespace_name(alter_info->tablespace_name, false);
  ...
  if (alter_info->file_block_size)
  {
    ...
    if (!ut_is_2pow(alter_info->file_block_size) || alter_info->file_block_size < UNIV_ZIP_SIZE_MIN ||
    ...
    else if (UNIV_PAGE_SIZE > UNIV_PAGE_SIZE_DEF && alter_info->file_block_size != UNIV_PAGE_SIZE)
    {
      my_printf_error(ER_ILLEGAL_HA_CREATE_OPTION,
  ...
  if (basename_len < 5)
  {
    my_error(ER_WRONG_FILE_NAME, MYF(0), alter_info->data_file_name);
    ut_free(filepath);
  ...
  {
    my_error(ER_WRONG_FILE_NAME, MYF(0), alter_info->data_file_name);
    my_printf_error(ER_WRONG_FILE_NAME, "An IBD filepath must end with `.ibd`.", MYF(0));
    ut_free(filepath);
    return (HA_WRONG_CREATE_OPTION);
  }
  ...
  if (colon != NULL)
  {
    ...
    if (colon != &filepath[1] || (colon[1] != OS_PATH_SEPARATOR) || NULL != strchr(&colon[1], ':'))
    {
#endif /* _WIN32 */
      my_error(ER_WRONG_FILE_NAME, MYF(0), alter_info->data_file_name);
  ...
  {
    my_error(ER_WRONG_FILE_NAME, MYF(0), alter_info->data_file_name);
    my_printf_error(ER_WRONG_FILE_NAME, "The directory does not exist.", MYF(0));
    return (HA_WRONG_CREATE_OPTION);
  }
...
@param[in]	alter_info	How to do the command
@return MySQL error code*/
static int innobase_create_tablespace(handlerton *hton, THD *thd, st_alter_tablespace *alter_info)
{
  trx_t *trx;
  int error;
  ...
  /* Create the filespace flags */
  ulint fsp_flags = fsp_flags_init(page_size,    /* page sizes and a flag if compressed */
...
@param[in]	alter_info	How to do the command
@return MySQL error code*/
static int innobase_drop_tablespace(handlerton *hton, THD *thd, st_alter_tablespace *alter_info)
{
  trx_t *trx;
  dberr_t err;
  int error = 0;
  ...
  if (err != DB_SUCCESS)
  {
    ib::error() << "Unable to delete the dictionary entries"
                   " for tablespace `"
                << alter_info->tablespace_name << "`, Space ID " << space_id;
    goto have_error;
  }
  ...
  switch (err)
  {
    ...
    case DB_SUCCESS:
      ...
      break;
    default:
      ib::error() << "Unable to delete the tablespace `" << alter_info->tablespace_name << "`, Space ID " << space_id;
    have_error:
      error = convert_error_code_to_mysql(err, 0, NULL);
      trx_rollback_for_mysql(trx);
  }
...
static int innobase_alter_tablespace(handlerton *hton, THD *thd, st_alter_tablespace *alter_info)
{
  ...
  if (error)
  {
    ...
    {
      case DROP_TABLESPACE:
        ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_DROP_FILEGROUP_FAILED, "%s %s", object_type, object);
        break;
      case CREATE_TABLESPACE:
        ib_errf(thd, IB_LOG_LEVEL_ERROR, ER_CREATE_FILEGROUP_FAILED, "%s %s", object_type, object);
        break;
      case CREATE_LOGFILE_GROUP:
        my_error(ER_ILLEGAL_HA_CREATE_OPTION, MYF(0), innobase_hton_name, "LOGFILE GROUP");
        break;
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
inline MY_ATTRIBUTE((warn_unused_result)) dberr_t innobase_rename_table(
    /*==================*/
    ...
    const char *to)   /*!< in: new name of the table */
{
  ...
  if (table != NULL)
  {
         ...
         index = dict_table_get_next_index(index))
    {
      ...
      {
        /* Found */
        while (index->index_fts_syncing && !trx_is_interrupted(trx) && (lock_wait_timeout--) > 0)
        {
          DICT_BG_YIELD(trx);
        }
      }
  ...
  if (error == DB_TABLE_NOT_FOUND)
  {
    ...
    trx_start_if_not_started(trx, true);
    error = row_rename_partitions_for_mysql(thd, norm_from, norm_to, trx);
    rename_parts = true;
    if (error == DB_TABLE_NOT_FOUND)
    {
  ...
  if (error != DB_SUCCESS)
  {
    if (error == DB_TABLE_NOT_FOUND && innobase_get_lower_case_table_names() == 1)
    {
      char *is_part = NULL;
#ifdef _WIN32
      ...
      if (is_part)
      {
        ...
        system table whose name is
        not being normalized to lower case */
        create_table_info_t::normalize_table_name_low(par_case_name, from, FALSE);
#endif /* _WIN32 */
        trx_start_if_not_started(trx, true);
        error = row_rename_table_for_mysql(par_case_name, norm_to, trx, TRUE);
      }
    }
  ...
  {
    char errstr[512];
    error = dict_stats_rename_table(false, norm_from, norm_to, errstr, sizeof(errstr));
    if (error != DB_SUCCESS)
    {
      ib::error() << errstr;
      push_warning(thd, Sql_condition::SL_WARNING, ER_LOCK_WAIT_TIMEOUT, errstr);
    }
  }
    ...
    ha_rows *num_rows) /*!< out: number of rows */
{
  ...
  {
    ib_senderrf(m_user_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_DISCARDED, table->s->table_name.str);
  ...
  {
    ib_senderrf(m_user_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_MISSING, table->s->table_name.str);
  ...
  {
    ib_errf(m_user_thd, IB_LOG_LEVEL_WARN, ER_INNODB_INDEX_CORRUPT, "Table '%s' is corrupt.", table->s->table_name.str);
  ...
  m_prebuilt->index_usable = row_merge_is_index_usable(m_prebuilt->trx, index);
                        ...
                        also be 0 */
{
  ...
  heap = mem_heap_create(2 * (key->actual_key_parts * sizeof(dfield_t) + sizeof(dtuple_t)));
  ...
  row_sel_convert_mysql_key_to_innobase(range_start, m_prebuilt->srch_key_val1, m_prebuilt->srch_key_val_len, index,
  ...
  row_sel_convert_mysql_key_to_innobase(range_end, m_prebuilt->srch_key_val2, m_prebuilt->srch_key_val_len, index,
  ...
  DBUG_EXECUTE_IF("print_btr_estimate_n_rows_in_range_return_value",
...
/*====================================*/
{
  ...
  DBUG_EXECUTE_IF("set_num_rows_lt_MERGEBUFF", estimate = 2; DBUG_SET("-d,set_num_rows_lt_MERGEBUFF"););
...
void innodb_set_buf_pool_size(ulonglong buf_pool_size) { innobase_buffer_pool_size = buf_pool_size; }
    ...
    const dict_index_t *index) /*!< in: index */
{
  ...
  for (ind = dict_table_get_first_index(ib_table); ind != NULL; ind = dict_table_get_next_index(ind))
  {
    if (index == ind)
    {
...
rec_per_key_t innodb_rec_per_key(
    /*===============*/
    ...
    ha_rows records)     /*!< in: estimated total records */
{
  ...
  else if (srv_innodb_stats_method == SRV_STATS_NULLS_IGNORED)
  {
    ...
    else
    {
      ...
      rec_per_key = static_cast<rec_per_key_t>(records - n_null) / (n_diff - n_null);
    }
  }
  else
  {
    ...
    bool is_analyze)
{
  ...
  if (flag & HA_STATUS_VARIABLE)
  {
    ...
    if (n_rows == 0 && !(flag & HA_STATUS_TIME) && table_share->table_category != TABLE_CATEGORY_TEMPORARY)
    {
      n_rows++;
    }
    ...
    stats.deleted = 0;
    stats.data_file_length = ((ulonglong)stat_clustered_index_size) * page_size.physical();
    stats.index_file_length = ((ulonglong)stat_sum_of_other_index_sizes) * page_size.physical();
    ...
    else
    {
      ...
      if (avail_space == UINTMAX_MAX)
      {
        ...
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_CANT_GET_STAT,
                            ...
                            " (errno: %d - %s)",
                            ib_table->name.m_name, errno, my_strerror(errbuf, sizeof(errbuf), errno));
  ...
  if (flag & HA_STATUS_CONST)
  {
    ...
    ulint num_innodb_index = UT_LIST_GET_LEN(ib_table->indexes) - m_prebuilt->clust_index_was_generated;
    if (table->s->keys < num_innodb_index)
    {
      ...
      for (const dict_index_t *index = UT_LIST_GET_FIRST(ib_table->indexes); index != NULL;
           index = UT_LIST_GET_NEXT(indexes, index))
      {
      ...
      if (table->s->keys < num_innodb_index &&
          innobase_fts_check_doc_id_index(ib_table, NULL, NULL) == FTS_EXIST_DOC_ID_INDEX)
      {
        num_innodb_index--;
      }
    }
      ...
      for (j = 0; j < key->actual_key_parts; j++)
      {
        ...
        if (j + 1 > index->n_uniq)
        {
          sql_print_error(
              ...
              " installations? %s",
              index->name(), ib_table->name.m_name, (unsigned long)index->n_uniq, j + 1, TROUBLESHOOTING_MSG);
          break;
        }
        ...
        ulong rec_per_key_int = static_cast<ulong>(innodb_rec_per_key(index, j, stats.records));
    ...
    my_snprintf(path, sizeof(path), "%s/%s%s", mysql_data_home, table->s->normalized_path.str, reg_ext);
    ...
    if (os_file_get_status(path, &stat_info, false, (dict_table_is_intrinsic(ib_table) ? false : srv_read_only_mode)) ==
        DB_SUCCESS)
    {
  ...
  else if (flag & HA_STATUS_ERRKEY)
  {
    ...
    if (err_index)
    {
      errkey = innobase_get_mysql_key_number_for_index(m_share, table, ib_table, err_index);
    }
    else
    {
      errkey =
          (unsigned int)((m_prebuilt->trx->error_key_num == ULINT_UNDEFINED) ? ~0 : m_prebuilt->trx->error_key_num);
    }
  }
...
int ha_innobase::enable_indexes(uint mode)
{
  ...
  if (dict_table_is_intrinsic(m_prebuilt->table))
  {
    ...
    for (dict_index_t *index = UT_LIST_GET_FIRST(m_prebuilt->table->indexes); index != NULL;
         index = UT_LIST_GET_NEXT(indexes, index))
    {
    ...
    HA_CHECK_OPT *check_opt) /*!< in: currently ignored */
{
  ...
  {
    if (m_prebuilt->table->fts && m_prebuilt->table->fts->cache && !dict_table_is_discarded(m_prebuilt->table))
    {
    ...
    HA_CHECK_OPT *check_opt) /*!< in: check options */
{
  ...
  if (dict_table_is_discarded(m_prebuilt->table))
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_DISCARDED, table->s->table_name.str);

    DBUG_RETURN(HA_ADMIN_CORRUPT);
  }
  else if (m_prebuilt->table->ibd_file_missing)
  {
    ib_senderrf(thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_MISSING, table->s->table_name.str);
  ...
  if (m_prebuilt->table->corrupted)
  {
    ...
    if (!dict_index_is_corrupted(index))
    {
      dict_set_corrupted(index, m_prebuilt->trx, "CHECK TABLE");
    }
  ...
  for (index = dict_table_get_first_index(m_prebuilt->table); index != NULL; index = dict_table_get_next_index(index))
  {
    ...
    if (!(check_opt->flags & T_QUICK) && !dict_index_is_corrupted(index))
    {
      /* Enlarge the fatal lock wait timeout during
      CHECK TABLE. */
      os_atomic_increment_ulint(&srv_fatal_semaphore_wait_threshold, SRV_SEMAPHORE_WAIT_EXTENSION);

      bool valid = btr_validate_index(index, m_prebuilt->trx, false);

      /* Restore the fatal lock wait timeout after
      CHECK TABLE. */
      os_atomic_decrement_ulint(&srv_fatal_semaphore_wait_threshold, SRV_SEMAPHORE_WAIT_EXTENSION);
    ...
    if (!m_prebuilt->index_usable)
    {
      ...
      {
        push_warning_printf(thd, Sql_condition::SL_WARNING, HA_ERR_TABLE_DEF_CHANGED,
    ...
    {
      ret = row_scan_index_for_mysql(m_prebuilt, index, &n_rows);
    }

    DBUG_EXECUTE_IF(
        "dict_set_index_corrupted", if (!dict_index_is_clust(index)) { ret = DB_CORRUPTION; });
    ...
    if (ret != DB_SUCCESS)
    {
      ...
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_NOT_KEYFILE,
                          ...
                          index->name());
      is_ok = false;
      dict_set_corrupted(index, m_prebuilt->trx, "CHECK TABLE-check index");
    }
    ...
    else if (!(index->type & DICT_FTS) && (n_rows != n_rows_in_table))
    {
      ...
      is_ok = false;
      dict_set_corrupted(index, m_prebuilt->trx, "CHECK TABLE; Wrong count");
    }
  }
  ...
  {
    push_warning(thd, Sql_condition::SL_WARNING, ER_NOT_KEYFILE, "InnoDB: The adaptive hash index is corrupted.");
    is_ok = false;
  }
...
/*==========================================*/
{
  ...
  if (!srv_read_only_mode)
  {
    ...
    dict_print_info_on_foreign_keys(TRUE, srv_dict_tmpfile, m_prebuilt->trx, m_prebuilt->table);
    ...
    str = reinterpret_cast<char *>(my_malloc(PSI_INSTRUMENT_ME, flen + 1, MYF(0)));
    ...
    dict_foreign_t *foreign) /*!< in: foreign key constraint */
{
  ...
  f_key_info.foreign_id = thd_make_lex_string(thd, 0, ptr, (uint)strlen(ptr), 1);
  ...
  f_key_info.referenced_db = thd_make_lex_string(thd, 0, name_buff, static_cast<unsigned int>(len), 1);

  /* Referenced (parent) table name */
  ptr = dict_remove_db_name(foreign->referenced_table_name);
  len = filename_to_tablename(ptr, name_buff, sizeof(name_buff));
  f_key_info.referenced_table = thd_make_lex_string(thd, 0, name_buff, static_cast<unsigned int>(len), 1);
  ...
  len = filename_to_tablename(tmp_buff, name_buff, sizeof(name_buff));
  f_key_info.foreign_db = thd_make_lex_string(thd, 0, name_buff, static_cast<unsigned int>(len), 1);

  /* Dependent (child) table name */
  ptr = dict_remove_db_name(foreign->foreign_table_name);
  len = filename_to_tablename(ptr, name_buff, sizeof(name_buff));
  f_key_info.foreign_table = thd_make_lex_string(thd, 0, name_buff, static_cast<unsigned int>(len), 1);
  ...
  f_key_info.delete_method = thd_make_lex_string(thd, f_key_info.delete_method, ptr, static_cast<unsigned int>(len), 1);
  ...
  f_key_info.update_method = thd_make_lex_string(thd, f_key_info.update_method, ptr, static_cast<unsigned int>(len), 1);
  ...
  if (foreign->referenced_table == NULL)
  {
    ...
    ut_ad(mutex_own(&dict_sys->mutex));
    ref_table = dict_table_open_on_name(foreign->referenced_table_name_lookup, TRUE, FALSE, DICT_ERR_IGNORE_NONE);

    if (ref_table == NULL)
    {
      if (!thd_test_options(thd, OPTION_NO_FOREIGN_KEY_CHECKS))
      {
        ib::info() << "Foreign Key referenced table " << foreign->referenced_table_name
  ...
  {
    referenced_key_name = thd_make_lex_string(thd, f_key_info.referenced_key_name, foreign->referenced_index->name,
  ...
  pf_key_info = (FOREIGN_KEY_INFO *)thd_memdup(thd, &f_key_info, sizeof(FOREIGN_KEY_INFO));
    ...
    List<FOREIGN_KEY_INFO> *f_key_list) /*!< out: foreign key list */
{
  ...
  for (dict_foreign_set::iterator it = m_prebuilt->table->referenced_set.begin();
...
{
  bool operator()(const st_handler_tablename lhs, const st_handler_tablename rhs) const
  {
...
int ha_innobase::get_cascade_foreign_key_table_list(THD *thd, List<st_handler_tablename> *fk_table_list)
{
  TrxInInnoDB trx_in_innodb(m_prebuilt->trx);
  ...
  typedef std::set<st_handler_tablename, tablename_compare, ut_allocator<st_handler_tablename>> cascade_fk_set;
  ...
  struct table_list_item item = {m_prebuilt->table, m_prebuilt->table->name.m_name};
  ...
  do
  {
    ...
    if (parent_table == NULL)
    {
      ...
      parent_table = parent = dict_table_open_on_name(item.name, TRUE, FALSE, DICT_ERR_IGNORE_NONE);
    ...
    for (dict_foreign_set::const_iterator it = parent_table->foreign_set.begin(); it != parent_table->foreign_set.end();
         ++it)
    {
      ...
      cascading operation. */
      if (0 == (foreign->type & ~(DICT_FOREIGN_ON_DELETE_NO_ACTION | DICT_FOREIGN_ON_UPDATE_NO_ACTION)))
      {
        continue;
      }
      ...
      if (foreign->referenced_table_name_lookup != NULL)
      {
        ...
        struct table_list_item item1 = {foreign->referenced_table, foreign->referenced_table_name_lookup};

        table_list.push_back(item1);

        st_handler_tablename *fk_table = (st_handler_tablename *)thd_memdup(thd, &f1, sizeof(*fk_table));
...
/*=================================*/
{
  ...
  bool can_switch = m_prebuilt->table->referenced_set.empty() && m_prebuilt->table->foreign_set.empty();
...
int ha_innobase::end_stmt()
{
  ...
  trx_t *trx = m_prebuilt->trx;
  if (!dict_table_is_temporary(m_prebuilt->table) && trx != thd_to_trx(ha_thd()))
  {
    ut_ad(false);
    return (0);
  }
    ...
    thr_lock_type lock_type)
{
  ...
  if (dict_table_is_temporary(m_prebuilt->table) && m_mysql_has_locked && m_prebuilt->select_lock_type == LOCK_NONE)
  {
    dberr_t error;
  ...
  else if (trx->isolation_level != TRX_ISO_SERIALIZABLE && thd_sql_command(thd) == SQLCOM_SELECT &&
           lock_type == TL_READ)
  {
  ...
  else
  {
    ...
    m_prebuilt->select_lock_type = m_prebuilt->stored_select_lock_type;
  }
    ...
    int lock_type) /*!< in: lock type */
{
  ...
  if (lock_type == F_WRLCK && !(table_flags() & HA_BINLOG_STMT_CAPABLE) &&
      thd_binlog_format(thd) == BINLOG_FORMAT_STMT && thd_binlog_filter_ok(thd) &&
  ...
  if (srv_read_only_mode && (thd_sql_command(thd) == SQLCOM_UPDATE || thd_sql_command(thd) == SQLCOM_INSERT ||
                             thd_sql_command(thd) == SQLCOM_REPLACE || thd_sql_command(thd) == SQLCOM_DROP_TABLE ||
                             thd_sql_command(thd) == SQLCOM_ALTER_TABLE || thd_sql_command(thd) == SQLCOM_OPTIMIZE ||
                             (thd_sql_command(thd) == SQLCOM_CREATE_TABLE && lock_type == F_WRLCK) ||
                             thd_sql_command(thd) == SQLCOM_CREATE_INDEX || thd_sql_command(thd) == SQLCOM_DROP_INDEX ||
  ...
  switch (m_prebuilt->table->quiesce)
  {
    case QUIESCE_START:
      ...
      if (!srv_read_only_mode && thd_sql_command(thd) == SQLCOM_FLUSH && lock_type == F_RDLCK)
      {
        if (dict_table_is_discarded(m_prebuilt->table))
        {
          ib_senderrf(trx->mysql_thd, IB_LOG_LEVEL_ERROR, ER_TABLESPACE_DISCARDED, table->s->table_name.str);
    ...
    case QUIESCE_COMPLETE:
      ...
      or trx interruption. */
      if (trx->flush_tables > 0 && (lock_type == F_UNLCK || trx_is_interrupted(trx)))
      {
  ...
  if (lock_type != F_UNLCK)
  {
    ...
    *trx->detailed_error = 0;
    ...
    if (trx->isolation_level == TRX_ISO_SERIALIZABLE && m_prebuilt->select_lock_type == LOCK_NONE &&
    ...
    {
      if (thd_sql_command(thd) == SQLCOM_LOCK_TABLES && THDVAR(thd, table_locks) &&
          thd_test_options(thd, OPTION_NOT_AUTOCOMMIT) && thd_in_lock_tables(thd))
      {
        dberr_t error = row_lock_table_for_mysql(m_prebuilt, NULL, 0);
    ...
    if (!trx_is_started(trx) &&
        (m_prebuilt->select_lock_type != LOCK_NONE || m_prebuilt->stored_select_lock_type != LOCK_NONE))
    {
      ++trx->will_lock;
    }
  ...
  if (trx->n_mysql_tables_in_use == 0)
  {
    ...
    else if (trx->isolation_level <= TRX_ISO_READ_COMMITTED && MVCC::is_view_active(trx->read_view))
    {
    ...
    stat_print_fn *stat_print)
{
  ...
  srv_printf_innodb_monitor(srv_monitor_file, FALSE, &trx_list_start, &trx_list_end);
  ...
  ret_val = stat_print(thd, innobase_hton_name, static_cast<uint>(strlen(innobase_hton_name)), STRING_WITH_LEN(""), str,
                       static_cast<uint>(flen));
...
struct ShowStatus
{
  ...
  struct OrderByWaits : public std::binary_function<Value, Value, bool>
  {
    ...
    bool operator()(const Value &lhs, const Value &rhs) const UNIV_NOTHROW { return (rhs.m_waits < lhs.m_waits); }
  };
  ...
  struct GetCount
  {
    ...
    GetCount(const char *name, Values *values) UNIV_NOTHROW : m_name(name), m_values(values)
    { /* No op */
    }
    ...
    void operator()(Count *count) UNIV_NOTHROW
    {
      ...
      {
        m_values->push_back(Value(m_name, count->m_spins, count->m_waits, count->m_calls));
      }
    }
  ...
  bool to_string(handlerton *hton, THD *thd, stat_print_fn *stat_print) UNIV_NOTHROW;
...
@param[in,out]	stat_print	function for printing statistics
@return true on success. */
bool ShowStatus::to_string(handlerton *hton, THD *thd, stat_print_fn *stat_print) UNIV_NOTHROW
{
  ...
  for (Values::iterator it = m_values.begin(); it != end; ++it)
  {
    ...
    name_len = ut_snprintf(name_buf, sizeof(name_buf), "%s", it->m_name.c_str());

    int status_len;
    char status_buf[IO_SIZE];

    status_len = ut_snprintf(status_buf, sizeof(status_buf), "spins=%lu,waits=%lu,calls=" TRX_ID_FMT,
                             static_cast<ulong>(it->m_spins), static_cast<long>(it->m_waits), it->m_calls);

    if (stat_print(thd, innobase_hton_name, hton_name_len, name_buf, static_cast<uint>(name_len), status_buf,
...
static int innodb_show_rwlock_status(handlerton *hton, THD *thd, stat_print_fn *stat_print)
{
  ...
  for (rw_lock_t *rw_lock = UT_LIST_GET_FIRST(rw_lock_list); rw_lock != NULL; rw_lock = UT_LIST_GET_NEXT(list, rw_lock))
  {
    ...
    buf1len = ut_snprintf(buf1, sizeof buf1, "rwlock: %s:%lu", innobase_basename(rw_lock->cfile_name),
                          static_cast<ulong>(rw_lock->cline));

    int buf2len;
    char buf2[IO_SIZE];

    buf2len = ut_snprintf(buf2, sizeof buf2, "waits=%lu", static_cast<ulong>(rw_lock->count_os_wait));

    if (stat_print(thd, innobase_hton_name, hton_name_len, buf1, static_cast<uint>(buf1len), buf2,
  ...
  if (block_rwlock != NULL)
  {
    ...
    buf1len = ut_snprintf(buf1, sizeof buf1, "sum rwlock: %s:%lu", innobase_basename(block_rwlock->cfile_name),
                          static_cast<ulong>(block_rwlock->cline));

    int buf2len;
    char buf2[IO_SIZE];

    buf2len = ut_snprintf(buf2, sizeof buf2, "waits=%lu", static_cast<ulong>(block_rwlock_oswait_count));
    ...
    const char *table_name)
{
  ...
  HASH_SEARCH(table_name_hash, innobase_open_tables, fold, INNOBASE_SHARE *, share, ut_ad(share->use_count > 0),
  ...
  if (share == NULL)
  {
    ...
    share = reinterpret_cast<INNOBASE_SHARE *>(
        my_malloc(PSI_INSTRUMENT_ME, sizeof(*share) + length + 1, MYF(MY_FAE | MY_ZEROFILL)));

    share->table_name = reinterpret_cast<char *>(memcpy(share + 1, table_name, length + 1));
    ...
    INNOBASE_SHARE *share) /*!< in/own: table share to free */
{
  ...
  HASH_SEARCH(table_name_hash, innobase_open_tables, fold, INNOBASE_SHARE *, share2, ut_ad(share->use_count > 0),
  ...
  if (share->use_count == 0)
  {
    ...
    HASH_DELETE(INNOBASE_SHARE, table_name_hash, innobase_open_tables, fold, share);
                             ...
                             TL_IGNORE */
{
  ...
  if (srv_read_only_mode && !dict_table_is_intrinsic(m_prebuilt->table) &&
      (sql_command == SQLCOM_UPDATE || sql_command == SQLCOM_INSERT || sql_command == SQLCOM_REPLACE ||
       sql_command == SQLCOM_DROP_TABLE || sql_command == SQLCOM_ALTER_TABLE || sql_command == SQLCOM_OPTIMIZE ||
       (sql_command == SQLCOM_CREATE_TABLE && (lock_type >= TL_WRITE_CONCURRENT_INSERT && lock_type <= TL_WRITE)) ||
       sql_command == SQLCOM_CREATE_INDEX || sql_command == SQLCOM_DROP_INDEX || sql_command == SQLCOM_DELETE))
  {
  ...
  else if ((lock_type == TL_READ && in_lock_tables) || (lock_type == TL_READ_HIGH_PRIORITY && in_lock_tables) ||
           ...
           (lock_type != TL_IGNORE && sql_command != SQLCOM_SELECT))
  {
    ...
    if (sql_command == SQLCOM_CHECKSUM ||
        ((srv_locks_unsafe_for_binlog || trx->isolation_level <= TRX_ISO_READ_COMMITTED) &&
         trx->isolation_level != TRX_ISO_SERIALIZABLE && (lock_type == TL_READ || lock_type == TL_READ_NO_INSERT) &&
         (sql_command == SQLCOM_INSERT_SELECT || sql_command == SQLCOM_REPLACE_SELECT || sql_command == SQLCOM_UPDATE ||
...
ulonglong ha_innobase::innobase_peek_autoinc(void)
/*====================================*/
{
  ulonglong auto_inc;
  dict_table_t *innodb_table;
                                   ...
                                   values */
{
  ...
  */
  ...
  if (increment > 1 && thd_sql_command(m_user_thd) != SQLCOM_ALTER_TABLE && autoinc < col_max_value)
  {
  ...
  *nb_reserved_values = trx->n_autoinc_rows;
  ...
  if (innobase_autoinc_lock_mode != AUTOINC_OLD_STYLE_LOCKING)
  {
    ...
    else
    {
      ...
      dict_table_autoinc_update_if_greater(m_prebuilt->table, m_prebuilt->autoinc_last_value);
    }
  }
  else
  {
...
bool ha_innobase::get_foreign_dup_key(
    /*=============================*/
    char *child_table_name, uint child_table_name_len, char *child_key_name, uint child_key_name_len)
{
  const dict_index_t *err_index;
                       ...
                       MySQL key value format */
{
  ...
  for (; key_part != key_part_end; ++key_part)
  {
    ...
    if (mysql_type == MYSQL_TYPE_TINY_BLOB || mysql_type == MYSQL_TYPE_MEDIUM_BLOB || mysql_type == MYSQL_TYPE_BLOB ||
...
my_bool ha_innobase::register_query_cache_table(
    /*====================================*/
    THD *thd,                      /*!< in: user thread handle */
    ...
    size_t key_length,             /*!< in: length of the normalized
                                   path to the table */
    qc_engine_callback *call_back, /*!< out: pointer to function for
    ...
    ulonglong *engine_data)        /*!< in/out: data to call_back */
{
  ...
  *call_back = innobase_query_caching_of_table_permitted;
  ...
  return (innobase_query_caching_of_table_permitted(thd, table_key, static_cast<uint>(key_length), engine_data));
}
                      ...
                      ended */
{
  ...
  if (trx_in_innodb.is_aborted() || DBUG_EVALUATE_IF("simulate_xa_failure_prepare_in_engine", 1, 0))
  {
    ...
    HA_CREATE_INFO *info, uint table_changes)
{
  ...
  if ((info->used_fields & HA_CREATE_USED_ROW_FORMAT) && info->row_type != get_row_type())
  {
    return (COMPATIBLE_DATA_NO);
  }
                                  ...
                                  from check function */
{
  ...
  if (in_val < srv_io_capacity)
  {
    ...
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, "Setting innodb_io_capacity_max to %lu",
                        srv_io_capacity);
  }
                                  ...
                                  from check function */
{
  ...
  if (in_val > srv_max_io_capacity)
  {
    ...
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, "Setting innodb_io_capacity to %lu",
                        srv_max_io_capacity);
  }
    ...
    const char *format_name) /*!< in: pointer to file format name */
{
  ...
  {
    for (format_id = 0; format_id <= UNIV_FORMAT_MAX; format_id++)
    {
      const char *name;
...
static int innodb_file_format_name_validate(
    /*=============================*/
    THD *thd,                     /*!< in: thread handle */
    struct st_mysql_sys_var *var, /*!< in: pointer to system
                                  variable */
    ...
    struct st_mysql_value *value) /*!< in: incoming string */
{
  ...
  if (file_format_input != NULL)
  {
    ...
    if (format_id <= UNIV_FORMAT_MAX)
    {
      ...
      *static_cast<const char **>(save) = trx_sys_file_format_id_to_name(format_id);
...
/****************************************************************/ /**
 ...
 value. This function is registered as a callback with MySQL. */
static void innodb_file_format_name_update(
    /*===========================*/
    THD *thd,                     /*!< in: thread handle */
    struct st_mysql_sys_var *var, /*!< in: pointer to
                                  system variable */
                                  ...
                                  from check function */
{
  ...
  push_warning(thd, Sql_condition::SL_WARNING, HA_ERR_WRONG_COMMAND, deprecated_file_format);
...
/*************************************************************/ /**
 ...
 @return 0 for valid file format */
static int innodb_file_format_max_validate(
    /*============================*/
    THD *thd,                     /*!< in: thread handle */
    struct st_mysql_sys_var *var, /*!< in: pointer to system
                                  variable */
    ...
    struct st_mysql_value *value) /*!< in: incoming string */
{
  ...
  if (file_format_input != NULL)
  {
    ...
    if (format_id >= 0)
    {
      /* Save a pointer to the name in the
      'file_format_name_map' constant array. */
      *static_cast<const char **>(save) = trx_sys_file_format_id_to_name((uint)format_id);
    ...
    else
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS,
                          ...
                          trx_sys_file_format_id_to_name(UNIV_FORMAT_MAX), UNIV_FORMAT_MAX);
    }
  }
...
/****************************************************************/ /**
 ...
 value. This function is registered as a callback with MySQL. */
static void innodb_file_format_max_update(
    /*==========================*/
    THD *thd,                     /*!< in: thread handle */
    struct st_mysql_sys_var *var, /*!< in: pointer to
                                  system variable */
                                  ...
                                  from check function */
{
  ...
  push_warning(thd, Sql_condition::SL_WARNING, HA_ERR_WRONG_COMMAND, deprecated_file_format_max);
  ...
  if (format_id > UNIV_FORMAT_MAX)
  {
    ...
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, "Ignoring SET innodb_file_format=%s",
                        format_name_in);
    return;
  }
...
static void innodb_large_prefix_update(THD *thd, st_mysql_sys_var *, void *var_ptr, const void *save)
{
  push_warning(thd, Sql_condition::SL_WARNING, HA_ERR_WRONG_COMMAND, deprecated_large_prefix);
...
@param[out]	var_ptr	current value
@param[in]	save	to-be-assigned value */
static void innodb_undo_logs_update(THD *thd, st_mysql_sys_var *, void *var_ptr, const void *save)
{
  ib::warn() << deprecated_undo_logs;

  push_warning(thd, Sql_condition::SL_WARNING, HA_ERR_WRONG_COMMAND, deprecated_undo_logs);
...
@param[in]	value		incoming string
@return 0 for valid stopword table */
static int innodb_stopword_table_validate(THD *thd, struct st_mysql_sys_var *var, void *save,
...
static void innodb_buffer_pool_size_update(THD *thd, struct st_mysql_sys_var *var, void *var_ptr, const void *save)
{
  longlong in_val = *static_cast<const longlong *>(save);

  ut_snprintf(export_vars.innodb_buffer_pool_resize_status, sizeof(export_vars.innodb_buffer_pool_resize_status),
              "Requested to resize buffer pool.");

  os_event_set(srv_buf_resize_event);

  ib::info() << export_vars.innodb_buffer_pool_resize_status << " (new size: " << in_val << " bytes)";
}
...
@param[in]	value		incoming string
@return 0 for valid stopword table */
static int innodb_internal_table_validate(THD *thd, struct st_mysql_sys_var *var, void *save,
                                          struct st_mysql_value *value)
{
  const char *table_name;
  char buff[STRING_BUFFER_USUAL_SIZE];
  ...
  user_table = dict_table_open_on_name(table_name, FALSE, TRUE, DICT_ERR_IGNORE_NONE);
  ...
  if (user_table)
  {
    ...
    DBUG_EXECUTE_IF("innodb_evict_autoinc_table", mutex_enter(&dict_sys->mutex);
                    dict_table_remove_from_cache_low(user_table, TRUE); mutex_exit(&dict_sys->mutex););
  }
...
static void innodb_old_blocks_pct_update(
    ...
    const void *save)             /*!< in: immediate result
                                  from check function */
{
  innobase_old_blocks_pct = static_cast<uint>(buf_LRU_old_ratio_update(*static_cast<const uint *>(save), TRUE));
}
                                  ...
                                  from check function */
{
  ...
  buf_block_t *block =
      buf_page_get(page_id_t(space_id, srv_saved_page_number_debug), page_size_t(space->flags), RW_X_LATCH, &mtr);

  if (block != NULL)
  {
    byte *page = block->frame;

    ib::info() << "Dirtying page: " << page_id_t(page_get_space_id(page), page_get_page_no(page));

    mlog_write_ulint(page + FIL_PAGE_TYPE, fil_page_get_type(page), MLOG_2BYTES, &mtr);
  }
  mtr.commit();
  fil_space_release(space);
}
...
static ibuf_use_t innodb_find_change_buffering_value(
    /*===============================*/
    const char *input_name) /*!< in: input change buffering
                            option name */
{
  for (ulint i = 0; i < UT_ARR_SIZE(innobase_change_buffering_values); ++i)
  {
    /* found a match */
    if (!innobase_strcasecmp(input_name, innobase_change_buffering_values[i]))
    {
...
static int innodb_change_buffering_validate(
    /*=============================*/
    THD *thd,                     /*!< in: thread handle */
    struct st_mysql_sys_var *var, /*!< in: pointer to system
                                  variable */
    ...
    struct st_mysql_value *value) /*!< in: incoming string */
{
  ...
  if (change_buffering_input != NULL)
  {
    ...
    if (use != IBUF_USE_COUNT)
    {
      ...
      *static_cast<const char **>(save) = innobase_change_buffering_values[use];
...
/****************************************************************/ /**
 ...
 value. This function is registered as a callback with MySQL. */
static void innodb_change_buffering_update(
    /*===========================*/
    THD *thd,                     /*!< in: thread handle */
    struct st_mysql_sys_var *var, /*!< in: pointer to
                                  system variable */
                                  ...
                                  from check function */
{
  ...
  use = innodb_find_change_buffering_value(*static_cast<const char *const *>(save));
...
/*************************************************************/ /**
 ...
 @return 0 */
static void innodb_stats_sample_pages_update(
    /*=============================*/
    THD *thd,                     /*!< in: thread handle */
    struct st_mysql_sys_var *var, /*!< in: pointer to
                                  system variable */
                                  ...
                                  from check function */
{
  ...
  push_warning(thd, Sql_condition::SL_WARNING, HA_ERR_WRONG_COMMAND, STATS_SAMPLE_PAGES_DEPRECATED_MSG);

  ib::warn() << STATS_SAMPLE_PAGES_DEPRECATED_MSG;

  srv_stats_transient_sample_pages = *static_cast<const unsigned long long *>(save);
}
                             ...
                             to turn on/off or reset the counter */
{
  ...
  for (ulint use = 0; use < NUM_MONITOR; use++)
  {
    ...
    if (!innobase_wildcasecmp(srv_mon_get_name(monitor_id), name))
    {
      ...
      {
        if ((monitor_id >= MONITOR_MODULE_BUF_PAGE) && (monitor_id < MONITOR_MODULE_OS))
        {
          if (set_option == MONITOR_TURN_ON && MONITOR_IS_ON(MONITOR_MODULE_BUF_PAGE))
          {
            continue;
          }
    ...
    const char *name) /*!< in: monitor counter namer */
{
  ...
  {
    if (!innobase_strcasecmp(name, srv_mon_get_name(static_cast<monitor_id_t>(i))))
    {
      return (i);
    }
  }
    ...
    const char *name) /*!< in: monitor counter namer */
{
  for (ulint i = 0; i < NUM_MONITOR; i++)
  {
    if (!innobase_wildcasecmp(srv_mon_get_name(static_cast<monitor_id_t>(i)), name))
    {
      return (TRUE);
    }
  }
    ...
    const char *name) /*!< in: incoming monitor name */
{
  ...
  if (use < NUM_MONITOR)
  {
    ...
    its module name */
    if ((monitor_info->monitor_type & MONITOR_GROUP_MODULE) && (!(monitor_info->monitor_type & MONITOR_MODULE)))
    {
      sql_print_warning(
                             ...
                             need to free the memory */
{
  ...
  else
  {
    ...
    {
      srv_mon_set_module_control(static_cast<monitor_id_t>(monitor_id), set_option);
    }
    else
    {
      innodb_monitor_set_option(monitor_info, set_option);
...
exit:
  ...
  if (err_monitor)
  {
    sql_print_warning("InnoDB: Monitor %s is already enabled.", srv_mon_get_name((monitor_id_t)err_monitor));
  }
...
static MY_ATTRIBUTE((warn_unused_result)) bool innodb_buffer_pool_evict_uncompressed(void)
/*=======================================*/
{
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; i++)
  {
    ...
    for (buf_block_t *block = UT_LIST_GET_LAST(buf_pool->unzip_LRU); block != NULL;)
    {
    ...
    char *str) /*!< in/out: monitor counter enable list */
{
  ...
  for (char *option = my_strtok_r(str, sep, &last); option; option = my_strtok_r(NULL, sep, &last))
  {
    ulint ret;
    char *option_name;
                         ...
                         be created. */
{
  ...
  for (key_num = 0; key_num < num_of_keys; key_num++)
  {
    ...
    if (innobase_strcasecmp(key->name, innobase_index_reserve_name) == 0)
    {
      ...
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_NAME_FOR_INDEX,
...
float innobase_fts_retrieve_ranking(
    /*============================*/
...
void innobase_fts_close_ranking(
    /*=======================*/
    FT_INFO *fts_hdl)
{
  fts_result_t *result;
...
of m_prebuilt->fts_doc_id
@return the relevance ranking value */
float innobase_fts_find_ranking(
    /*======================*/
    FT_INFO *fts_hdl, /*!< in: FTS handler */
...
static uint innodb_merge_threshold_set_all_debug = DICT_INDEX_MERGE_THRESHOLD_DEFAULT;

/** Wait for the background drop list to become empty. */
static void wait_background_drop_list_empty(THD *thd /*!< in: thread handle */
                                                MY_ATTRIBUTE((unused)),
                                            struct st_mysql_sys_var *var /*!< in: pointer to system
                                                                         variable */
                                                MY_ATTRIBUTE((unused)),
                      ...
                      check function */
{
  if (*(my_bool *)save)
  {
    while (log_sys->last_checkpoint_lsn + SIZE_OF_MLOG_CHECKPOINT +
               (log_sys->append_on_checkpoint != NULL ? log_sys->append_on_checkpoint->size() : 0) <
           log_sys->lsn)
    {
...
@param[out]	var_ptr	where the formal string goes
@param[in]	save	immediate result from check function */
static void innodb_merge_threshold_set_all_debug_update(THD *thd, struct st_mysql_sys_var *var, void *var_ptr,
                                                        const void *save)
{
...
ulonglong innobase_fts_retrieve_docid(
    /*========================*/
...
ulonglong innobase_fts_count_matches(
    /*=======================*/
    FT_INFO_EXT *fts_hdl) /*!< in: FTS handler */
{
...
@param[out]	var_ptr	current value
@param[in]	save	to-be-assigned value */
static void innodb_status_output_update(THD *, struct st_mysql_sys_var *, void *var_ptr, const void *save)
{
  *static_cast<my_bool *>(var_ptr) = *static_cast<const my_bool *>(save);
...
@param[out]	var_ptr	current value
@param[in]	save	immediate result from check function */
static void innodb_log_checksums_update(THD *thd, struct st_mysql_sys_var *var, void *var_ptr, const void *save)
{
  my_bool check = *static_cast<my_bool *>(var_ptr) = *static_cast<const my_bool *>(save);
...
static SHOW_VAR innodb_status_variables_export[] = {{"Innodb", (char *)&show_innodb_vars, SHOW_FUNC, SHOW_SCOPE_GLOBAL},
                                                    {NullS, NullS, SHOW_LONG, SHOW_SCOPE_GLOBAL}};
...
static MYSQL_SYSVAR_ENUM(checksum_algorithm, srv_checksum_algorithm, PLUGIN_VAR_RQCMDARG,
                         ...
                         NULL, NULL, SRV_CHECKSUM_ALGORITHM_CRC32, &innodb_checksum_algorithm_typelib);

static MYSQL_SYSVAR_BOOL(log_checksums, innodb_log_checksums, PLUGIN_VAR_RQCMDARG,
                         "Whether to compute and require checksums for InnoDB redo log blocks", NULL,
                         innodb_log_checksums_update, TRUE);

static MYSQL_SYSVAR_BOOL(checksums, innobase_use_checksums, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_STR(data_home_dir, innobase_data_home_dir, PLUGIN_VAR_READONLY,
                        "The common part for InnoDB table spaces.", NULL, NULL, NULL);

static MYSQL_SYSVAR_BOOL(doublewrite, innobase_use_doublewrite, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
                         "Enable InnoDB doublewrite buffer (enabled by default)."
                         " Disable with --skip-innodb-doublewrite.",
                         NULL, NULL, TRUE);

static MYSQL_SYSVAR_BOOL(stats_include_delete_marked, srv_stats_include_delete_marked, PLUGIN_VAR_OPCMDARG,
                         "Include delete marked records when calculating persistent statistics", NULL, NULL, FALSE);
...
static MYSQL_SYSVAR_ULONG(io_capacity_max, srv_max_io_capacity, PLUGIN_VAR_RQCMDARG,
                          "Limit to which innodb_io_capacity can be inflated.", NULL, innodb_io_capacity_max_update,
                          SRV_MAX_IO_CAPACITY_DUMMY_DEFAULT, 100, SRV_MAX_IO_CAPACITY_LIMIT, 0);

#ifdef UNIV_DEBUG
static MYSQL_SYSVAR_BOOL(background_drop_list_empty, innodb_background_drop_list_empty, PLUGIN_VAR_OPCMDARG,
                         "Wait for the background drop list to become empty", NULL, wait_background_drop_list_empty,
                         FALSE);

static MYSQL_SYSVAR_BOOL(purge_run_now, innodb_purge_run_now, PLUGIN_VAR_OPCMDARG, "Set purge state to RUN", NULL,
                         purge_run_now_set, FALSE);

static MYSQL_SYSVAR_BOOL(purge_stop_now, innodb_purge_stop_now, PLUGIN_VAR_OPCMDARG, "Set purge state to STOP", NULL,
                         purge_stop_now_set, FALSE);

static MYSQL_SYSVAR_BOOL(log_checkpoint_now, innodb_log_checkpoint_now, PLUGIN_VAR_OPCMDARG, "Force checkpoint now",
                         NULL, checkpoint_now_set, FALSE);

static MYSQL_SYSVAR_BOOL(buf_flush_list_now, innodb_buf_flush_list_now, PLUGIN_VAR_OPCMDARG,
                         "Force dirty page flush now", NULL, buf_flush_list_now_set, FALSE);

static MYSQL_SYSVAR_UINT(merge_threshold_set_all_debug, innodb_merge_threshold_set_all_debug, PLUGIN_VAR_RQCMDARG,
                         "Override current MERGE_THRESHOLD setting for all indexes at dictionary"
                         " cache by the specified value dynamically, at the time.",
                         NULL, innodb_merge_threshold_set_all_debug_update, DICT_INDEX_MERGE_THRESHOLD_DEFAULT, 1, 50,
                         0);

static MYSQL_SYSVAR_ULONG(semaphore_wait_timeout_debug, srv_fatal_semaphore_wait_threshold, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_ULONG(purge_batch_size, srv_purge_batch_size, PLUGIN_VAR_OPCMDARG,
                          "Number of UNDO log pages to purge in one batch from the history list.", NULL, NULL,
                          300,      /* Default setting */
                          1,        /* Minimum value */
                          5000, 0); /* Maximum value */

static MYSQL_SYSVAR_ULONG(purge_threads, srv_n_purge_threads, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                          "Purge threads can be from 1 to 32. Default is 4.", NULL, NULL, 4, /* Default setting */
                          1,                                                                 /* Minimum value */
                          32, 0);                                                            /* Maximum value */

static MYSQL_SYSVAR_ULONG(sync_array_size, srv_sync_array_size, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                          "Size of the mutex/lock wait array.", NULL, NULL, 1, /* Default setting */
                          1,                                                   /* Minimum value */
                          1024, 0);                                            /* Maximum value */

static MYSQL_SYSVAR_ULONG(fast_shutdown, innobase_fast_shutdown, PLUGIN_VAR_OPCMDARG,
                          "Speeds up the shutdown process of the InnoDB storage engine. Possible"
                          " values are 0, 1 (faster) or 2 (fastest - crash-like).",
                          NULL, NULL, 1, 0, 2, 0);

static MYSQL_SYSVAR_BOOL(file_per_table, srv_file_per_table, PLUGIN_VAR_NOCMDARG,
                         "Stores each InnoDB table to an .ibd file in the database dir.", NULL, NULL, TRUE);

static MYSQL_SYSVAR_STR(file_format, innobase_file_format_name, PLUGIN_VAR_RQCMDARG,
...
server startup */
static MYSQL_SYSVAR_BOOL(file_format_check, innobase_file_format_check, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
                         "Whether to perform system file format check.", NULL, NULL, TRUE);

/* If a new file format is introduced, the file format
name needs to be updated accordingly. Please refer to
file_format_name_map[] defined in trx0sys.cc for the next
file format name. */
static MYSQL_SYSVAR_STR(file_format_max, innobase_file_format_max, PLUGIN_VAR_OPCMDARG,
                        "The highest file format in the tablespace.", innodb_file_format_max_validate,
                        innodb_file_format_max_update, innodb_file_format_max_default);

static MYSQL_SYSVAR_STR(ft_server_stopword_table, innobase_server_stopword_table,
                        PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_MEMALLOC, "The user supplied stopword table name.",
                        innodb_stopword_table_validate, NULL, NULL);

static MYSQL_SYSVAR_UINT(flush_log_at_timeout, srv_flush_log_at_timeout, PLUGIN_VAR_OPCMDARG,
                         "Write and flush logs every (n) second.", NULL, NULL, 1, 0, 2700, 0);

static MYSQL_SYSVAR_ULONG(flush_log_at_trx_commit, srv_flush_log_at_trx_commit, PLUGIN_VAR_OPCMDARG,
                          "Set to 0 (write and flush once per second),"
                          " 1 (write and flush at each commit)"
                          " or 2 (write at commit, flush once per second).",
                          NULL, NULL, 1, 0, 2, 0);

static MYSQL_SYSVAR_STR(flush_method, innobase_file_flush_method, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                        "With which method to flush data.", NULL, NULL, NULL);

static MYSQL_SYSVAR_BOOL(large_prefix, innobase_large_prefix, PLUGIN_VAR_NOCMDARG,
                         "Support large index prefix length of REC_VERSION_56_MAX_INDEX_COL_LEN (3072) bytes.", NULL,
                         innodb_large_prefix_update, TRUE);

static MYSQL_SYSVAR_BOOL(force_load_corrupted, srv_load_corrupted, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
                         "Force InnoDB to load metadata of corrupted table.", NULL, NULL, FALSE);
...
static MYSQL_SYSVAR_STR(log_group_home_dir, srv_log_group_home_dir, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                        "Path to InnoDB log files.", NULL, NULL, NULL);

static MYSQL_SYSVAR_ULONG(page_cleaners, srv_n_page_cleaners, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                          "Page cleaner threads can be from 1 to 64. Default is 4.", NULL, NULL, 4, 1, 64, 0);

static MYSQL_SYSVAR_DOUBLE(max_dirty_pages_pct, srv_max_buf_pool_modified_pct, PLUGIN_VAR_RQCMDARG,
                           "Percentage of dirty pages allowed in bufferpool.", NULL, innodb_max_dirty_pages_pct_update,
                           75.0, 0, 99.999, 0);

static MYSQL_SYSVAR_DOUBLE(max_dirty_pages_pct_lwm, srv_max_dirty_pages_pct_lwm, PLUGIN_VAR_RQCMDARG,
                           "Percentage of dirty pages at which flushing kicks in.", NULL,
                           innodb_max_dirty_pages_pct_lwm_update, 0, 0, 99.999, 0);

static MYSQL_SYSVAR_ULONG(adaptive_flushing_lwm, srv_adaptive_flushing_lwm, PLUGIN_VAR_RQCMDARG,
                          "Percentage of log capacity below which no adaptive flushing happens.", NULL, NULL, 10, 0, 70,
                          0);

static MYSQL_SYSVAR_BOOL(adaptive_flushing, srv_adaptive_flushing, PLUGIN_VAR_NOCMDARG,
                         "Attempt flushing dirty pages to avoid IO bursts at checkpoints.", NULL, NULL, TRUE);

static MYSQL_SYSVAR_BOOL(flush_sync, srv_flush_sync, PLUGIN_VAR_NOCMDARG,
                         "Allow IO bursts at the checkpoints ignoring io_capacity setting.", NULL, NULL, TRUE);

static MYSQL_SYSVAR_ULONG(flushing_avg_loops, srv_flushing_avg_loops, PLUGIN_VAR_RQCMDARG,
                          "Number of iterations over which the background flushing is averaged.", NULL, NULL, 30, 1,
                          1000, 0);

static MYSQL_SYSVAR_ULONG(max_purge_lag, srv_max_purge_lag, PLUGIN_VAR_RQCMDARG,
                          "Desired maximum length of the purge queue (0 = no limit)", NULL, NULL, 0, 0, ~0UL, 0);

static MYSQL_SYSVAR_ULONG(max_purge_lag_delay, srv_max_purge_lag_delay, PLUGIN_VAR_RQCMDARG,
                          "Maximum delay of user threads in micro-seconds", NULL, NULL, 0L, /* Default seting */
                          0L,                                                               /* Minimum value */
                          10000000UL, 0);                                                   /* Maximum value */

static MYSQL_SYSVAR_BOOL(
    rollback_on_timeout, innobase_rollback_on_timeout, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
    "Roll back the complete transaction on lock wait timeout, for 4.x compatibility (disabled by default)", NULL, NULL,
    FALSE);

static MYSQL_SYSVAR_BOOL(status_file, innobase_create_status_file, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_NOSYSVAR,
                         "Enable SHOW ENGINE INNODB STATUS output in the innodb_status.<pid> file", NULL, NULL, FALSE);

static MYSQL_SYSVAR_BOOL(stats_on_metadata, innobase_stats_on_metadata, PLUGIN_VAR_OPCMDARG,
                         "Enable statistics gathering for metadata commands such as"
                         " SHOW TABLE STATUS for tables that use transient statistics (off by default)",
                         NULL, NULL, FALSE);

static MYSQL_SYSVAR_ULONGLONG(stats_sample_pages, srv_stats_transient_sample_pages, PLUGIN_VAR_RQCMDARG,
                              "Deprecated, use innodb_stats_transient_sample_pages instead", NULL,
                              innodb_stats_sample_pages_update, 8, 1, ~0ULL, 0);

static MYSQL_SYSVAR_ULONGLONG(stats_transient_sample_pages, srv_stats_transient_sample_pages, PLUGIN_VAR_RQCMDARG,
                              "The number of leaf index pages to sample when calculating transient"
                              " statistics (if persistent statistics are not used, default 8)",
                              NULL, NULL, 8, 1, ~0ULL, 0);

static MYSQL_SYSVAR_BOOL(stats_persistent, srv_stats_persistent, PLUGIN_VAR_OPCMDARG,
                         "InnoDB persistent statistics enabled for all tables unless overridden"
                         " at table level",
                         NULL, NULL, TRUE);

static MYSQL_SYSVAR_BOOL(stats_auto_recalc, srv_stats_auto_recalc, PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_ULONGLONG(stats_persistent_sample_pages, srv_stats_persistent_sample_pages, PLUGIN_VAR_RQCMDARG,
                              "The number of leaf index pages to sample when calculating persistent"
                              " statistics (by ANALYZE, default 20)",
                              NULL, NULL, 20, 1, ~0ULL, 0);

static MYSQL_SYSVAR_BOOL(adaptive_hash_index, btr_search_enabled, PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_ULONG(adaptive_hash_index_parts, btr_ahi_parts, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                          "Number of InnoDB Adapative Hash Index Partitions. (default = 8). ", NULL, NULL, 8, 1, 512,
                          0);

static MYSQL_SYSVAR_ULONG(replication_delay, srv_replication_delay, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_BOOL(log_compressed_pages, page_zip_log_pages, PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_ULONG(autoextend_increment, sys_tablespace_auto_extend_increment, PLUGIN_VAR_RQCMDARG,
                          "Data file autoextend increment in megabytes", NULL, NULL, 64L, 1L, 1000L, 0);
...
@return 0 on success, 1 on failure.
*/
static int innodb_buffer_pool_size_validate(THD *thd, struct st_mysql_sys_var *var, void *save,
                                            struct st_mysql_value *value);
...
static MYSQL_SYSVAR_LONGLONG(buffer_pool_size, innobase_buffer_pool_size, PLUGIN_VAR_RQCMDARG,
                             "The size of the memory buffer InnoDB uses to cache data and indexes of its tables.",
                             innodb_buffer_pool_size_validate, innodb_buffer_pool_size_update,
                             static_cast<longlong>(srv_buf_pool_def_size), static_cast<longlong>(srv_buf_pool_min_size),
...
static MYSQL_SYSVAR_ULONG(page_hash_locks, srv_n_page_hash_locks, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
...
static MYSQL_SYSVAR_ULONG(doublewrite_batch_size, srv_doublewrite_batch_size, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                          "Number of pages reserved in doublewrite buffer for batch flushing", NULL, NULL, 120, 1, 127,
                          0);
#endif /* defined UNIV_DEBUG || defined UNIV_PERF_DEBUG */

static MYSQL_SYSVAR_ULONG(
    buffer_pool_instances, srv_buf_pool_instances, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
    "Number of buffer pool instances, set to higher value on high-end machines to increase scalability", NULL, NULL,
    srv_buf_pool_instances_default, 0, MAX_BUFFER_POOLS, 0);

static MYSQL_SYSVAR_STR(buffer_pool_filename, srv_buf_dump_filename, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_MEMALLOC,
                        "Filename to/from which to dump/load the InnoDB buffer pool",
                        innodb_srv_buf_dump_filename_validate, NULL, SRV_BUF_DUMP_FILENAME_DEFAULT);

static MYSQL_SYSVAR_BOOL(buffer_pool_dump_now, innodb_buffer_pool_dump_now, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_BOOL(buffer_pool_dump_at_shutdown, srv_buffer_pool_dump_at_shutdown, PLUGIN_VAR_RQCMDARG,
                         "Dump the buffer pool into a file named @@innodb_buffer_pool_filename", NULL, NULL, TRUE);

static MYSQL_SYSVAR_ULONG(buffer_pool_dump_pct, srv_buf_pool_dump_pct, PLUGIN_VAR_RQCMDARG,
                          "Dump only the hottest N% of each buffer pool, defaults to 25", NULL, NULL, 25, 1, 100, 0);

#ifdef UNIV_DEBUG
static MYSQL_SYSVAR_STR(buffer_pool_evict, srv_buffer_pool_evict, PLUGIN_VAR_RQCMDARG,
                        "Evict pages from the buffer pool", NULL, innodb_buffer_pool_evict_update, "");
#endif /* UNIV_DEBUG */

static MYSQL_SYSVAR_BOOL(buffer_pool_load_now, innodb_buffer_pool_load_now, PLUGIN_VAR_RQCMDARG,
                         "Trigger an immediate load of the buffer pool from a file named @@innodb_buffer_pool_filename",
                         NULL, buffer_pool_load_now, FALSE);

static MYSQL_SYSVAR_BOOL(buffer_pool_load_abort, innodb_buffer_pool_load_abort, PLUGIN_VAR_RQCMDARG,
                         "Abort a currently running load of the buffer pool", NULL, buffer_pool_load_abort, FALSE);

/* there is no point in changing this during runtime, thus readonly */
static MYSQL_SYSVAR_BOOL(buffer_pool_load_at_startup, srv_buffer_pool_load_at_startup,
                         PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                         "Load the buffer pool from a file named @@innodb_buffer_pool_filename", NULL, NULL, TRUE);

static MYSQL_SYSVAR_ULONG(lru_scan_depth, srv_LRU_scan_depth, PLUGIN_VAR_RQCMDARG,
                          "How deep to scan LRU to keep it clean", NULL, NULL, 1024, 100, ~0UL, 0);

static MYSQL_SYSVAR_ULONG(flush_neighbors, srv_flush_neighbors, PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_ULONG(commit_concurrency, innobase_commit_concurrency, PLUGIN_VAR_RQCMDARG,
                          "Helps in performance tuning in heavily concurrent environments.",
                          innobase_commit_concurrency_validate, NULL, 0, 0, 1000, 0);

static MYSQL_SYSVAR_ULONG(
    concurrency_tickets, srv_n_free_tickets_to_enter, PLUGIN_VAR_RQCMDARG,
    "Number of times a thread is allowed to enter InnoDB within the same SQL query after it has once got the ticket",
    NULL, NULL, 5000L, 1L, ~0UL, 0);

static MYSQL_SYSVAR_BOOL(deadlock_detect, innobase_deadlock_detect, PLUGIN_VAR_NOCMDARG,
...
static MYSQL_SYSVAR_LONG(fill_factor, innobase_fill_factor, PLUGIN_VAR_RQCMDARG,
                         "Percentage of B-tree page filled during bulk insert", NULL, NULL, 100, 10, 100, 0);

static MYSQL_SYSVAR_BOOL(ft_enable_diag_print, fts_enable_diag_print, PLUGIN_VAR_OPCMDARG,
                         "Whether to enable additional FTS diagnostic printout ", NULL, NULL, FALSE);

static MYSQL_SYSVAR_BOOL(disable_sort_file_cache, srv_disable_sort_file_cache, PLUGIN_VAR_OPCMDARG,
                         "Whether to disable OS system file cache for sort I/O", NULL, NULL, FALSE);

static MYSQL_SYSVAR_STR(ft_aux_table, fts_internal_tbl_name, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_MEMALLOC,
                        "FTS internal auxiliary table to be checked", innodb_internal_table_validate, NULL, NULL);

static MYSQL_SYSVAR_ULONG(ft_cache_size, fts_max_cache_size, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "InnoDB Fulltext search cache size in bytes", NULL, NULL, 8000000, 1600000, 80000000, 0);

static MYSQL_SYSVAR_ULONG(ft_total_cache_size, fts_max_total_cache_size, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "Total memory allocated for InnoDB Fulltext Search cache", NULL, NULL, 640000000, 32000000,
                          1600000000, 0);

static MYSQL_SYSVAR_ULONG(ft_result_cache_limit, fts_result_cache_limit, PLUGIN_VAR_RQCMDARG,
                          "InnoDB Fulltext search query result cache limit in bytes", NULL, NULL, 2000000000L, 1000000L,
                          4294967295UL, 0);

static MYSQL_SYSVAR_ULONG(ft_min_token_size, fts_min_token_size, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "InnoDB Fulltext search minimum token size in characters", NULL, NULL, 3, 0, 16, 0);

static MYSQL_SYSVAR_ULONG(ft_max_token_size, fts_max_token_size, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "InnoDB Fulltext search maximum token size in characters", NULL, NULL,
                          FTS_MAX_WORD_LEN_IN_CHAR, 10, FTS_MAX_WORD_LEN_IN_CHAR, 0);

static MYSQL_SYSVAR_ULONG(ft_num_word_optimize, fts_num_word_optimize, PLUGIN_VAR_OPCMDARG,
                          "InnoDB Fulltext search number of words to optimize for each optimize table call ", NULL,
                          NULL, 2000, 1000, 10000, 0);

static MYSQL_SYSVAR_ULONG(ft_sort_pll_degree, fts_sort_pll_degree, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "InnoDB Fulltext search parallel sort degree, will round up to nearest power of 2 number",
                          NULL, NULL, 2, 1, 16, 0);

static MYSQL_SYSVAR_ULONG(sort_buffer_size, srv_sort_buf_size, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "Memory buffer size for index creation", NULL, NULL, 1048576, 65536, 64 << 20, 0);

static MYSQL_SYSVAR_ULONGLONG(online_alter_log_max_size, srv_online_max_size, PLUGIN_VAR_RQCMDARG,
                              "Maximum modification log file size for online index creation", NULL, NULL, 128 << 20,
                              65536, ~0ULL, 0);

static MYSQL_SYSVAR_BOOL(optimize_fulltext_only, innodb_optimize_fulltext_only, PLUGIN_VAR_NOCMDARG,
                         "Only optimize the Fulltext index of the table", NULL, NULL, FALSE);

static MYSQL_SYSVAR_ULONG(read_io_threads, innobase_read_io_threads, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "Number of background read I/O threads in InnoDB.", NULL, NULL, 4, 1, 64, 0);

static MYSQL_SYSVAR_ULONG(write_io_threads, innobase_write_io_threads, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "Number of background write I/O threads in InnoDB.", NULL, NULL, 4, 1, 64, 0);

static MYSQL_SYSVAR_ULONG(force_recovery, srv_force_recovery, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "Helps to save your data in case the disk image of the database becomes corrupt.", NULL, NULL,
                          0, 0, 6, 0);

#ifndef NDEBUG
static MYSQL_SYSVAR_ULONG(force_recovery_crash, srv_force_recovery_crash, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "Kills the server during crash recovery.", NULL, NULL, 0, 0, 100, 0);
#endif /* !NDEBUG */

static MYSQL_SYSVAR_ULONG(page_size, srv_page_size, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                          "Page size to use for all InnoDB tablespaces.", NULL, NULL, UNIV_PAGE_SIZE_DEF,
                          UNIV_PAGE_SIZE_MIN, UNIV_PAGE_SIZE_MAX, 0);

static MYSQL_SYSVAR_LONG(log_buffer_size, innobase_log_buffer_size, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                         "The size of the buffer which InnoDB uses to write log to the log files on disk.", NULL, NULL,
                         16 * 1024 * 1024L, 256 * 1024L, LONG_MAX, 1024);

static MYSQL_SYSVAR_LONGLONG(log_file_size, innobase_log_file_size, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                             "Size of each log file in a log group.", NULL, NULL, 48 * 1024 * 1024L, 4 * 1024 * 1024L,
                             LLONG_MAX, 1024 * 1024L);

static MYSQL_SYSVAR_ULONG(log_files_in_group, srv_n_log_files, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "Number of log files in the log group. InnoDB writes to the files in a circular fashion.",
                          NULL, NULL, 2, 2, SRV_N_LOG_FILES_MAX, 0);

static MYSQL_SYSVAR_ULONG(log_write_ahead_size, srv_log_write_ahead_size, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_UINT(old_blocks_pct, innobase_old_blocks_pct, PLUGIN_VAR_RQCMDARG,
                         "Percentage of the buffer pool to reserve for 'old' blocks.", NULL,
                         innodb_old_blocks_pct_update, 100 * 3 / 8, 5, 95, 0);

static MYSQL_SYSVAR_UINT(old_blocks_time, buf_LRU_old_threshold_ms, PLUGIN_VAR_RQCMDARG,
                         "Move blocks to the 'new' end of the buffer pool if the first access"
                         " was at least this many milliseconds ago."
                         " The timeout is disabled if 0.",
                         NULL, NULL, 1000, 0, UINT_MAX32, 0);

static MYSQL_SYSVAR_LONG(open_files, innobase_open_files, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                         "How many files at the maximum InnoDB keeps open at the same time.", NULL, NULL, 0L, 0L,
                         LONG_MAX, 0);

static MYSQL_SYSVAR_ULONG(sync_spin_loops, srv_n_spin_wait_rounds, PLUGIN_VAR_RQCMDARG,
                          "Count of spin-loop rounds in InnoDB mutexes (30 by default)", NULL, NULL, 30L, 0L, ~0UL, 0);

static MYSQL_SYSVAR_ULONG(spin_wait_delay, srv_spin_wait_delay, PLUGIN_VAR_OPCMDARG,
                          "Maximum delay between polling for a spin lock (6 by default)", NULL, NULL, 6L, 0L, ~0UL, 0);

static MYSQL_SYSVAR_ULONG(thread_concurrency, srv_thread_concurrency, PLUGIN_VAR_RQCMDARG,
                          "Helps in performance tuning in heavily concurrent environments. Sets the maximum number of "
                          "threads allowed inside InnoDB. Value 0 will disable the thread throttling.",
                          NULL, NULL, 0, 0, 1000, 0);

static MYSQL_SYSVAR_ULONG(adaptive_max_sleep_delay, srv_adaptive_max_sleep_delay, PLUGIN_VAR_RQCMDARG,
                          "The upper limit of the sleep delay in usec. Value of 0 disables it.", NULL, NULL,
                          150000,      /* Default setting */
                          0,           /* Minimum value */
                          1000000, 0); /* Maximum value */

static MYSQL_SYSVAR_ULONG(thread_sleep_delay, srv_thread_sleep_delay, PLUGIN_VAR_RQCMDARG,
                          "Time of innodb thread sleeping before joining InnoDB queue (usec)."
                          " Value 0 disable a sleep",
                          NULL, NULL, 10000L, 0L, 1000000L, 0);

static MYSQL_SYSVAR_STR(data_file_path, innobase_data_file_path, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                        "Path to individual files and their sizes.", NULL, NULL, NULL);

static MYSQL_SYSVAR_STR(temp_data_file_path, innobase_temp_data_file_path, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                        "Path to files and their sizes making temp-tablespace.", NULL, NULL, NULL);

static MYSQL_SYSVAR_STR(undo_directory, srv_undo_dir, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                        "Directory where undo tablespace files live, this path can be absolute.", NULL, NULL, NULL);

static MYSQL_SYSVAR_ULONG(undo_tablespaces, srv_undo_tablespaces, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                          "Number of undo tablespaces to use. (deprecated)", NULL, NULL, 0L, /* Default seting */
                          0L,                                                                /* Minimum value */
                          95L, 0);                                                           /* Maximum value */

static MYSQL_SYSVAR_ULONG(undo_logs, srv_undo_logs, PLUGIN_VAR_OPCMDARG,
                          "Number of rollback segments to use for storing undo logs. (deprecated)", NULL,
                          innodb_undo_logs_update, TRX_SYS_N_RSEGS, /* Default setting */
                          1,                                        /* Minimum value */
                          TRX_SYS_N_RSEGS, 0);                      /* Maximum value */

static MYSQL_SYSVAR_ULONGLONG(max_undo_log_size, srv_max_undo_log_size, PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_ULONG(purge_rseg_truncate_frequency, srv_purge_rseg_truncate_frequency, PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_BOOL(undo_log_truncate, srv_undo_log_truncate, PLUGIN_VAR_OPCMDARG,
                         "Enable or Disable Truncate of UNDO tablespace.", NULL, NULL, FALSE);

/* Alias for innodb_undo_logs, this config variable is deprecated. */
static MYSQL_SYSVAR_ULONG(rollback_segments, srv_rollback_segments, PLUGIN_VAR_OPCMDARG,
                          "Number of rollback segments to use for storing undo logs.", NULL, NULL,
                          TRX_SYS_N_RSEGS,     /* Default setting */
                          1,                   /* Minimum value */
                          TRX_SYS_N_RSEGS, 0); /* Maximum value */

static MYSQL_SYSVAR_LONG(autoinc_lock_mode, innobase_autoinc_lock_mode, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                         "The AUTOINC lock modes supported by InnoDB:"
                         " 0 => Old style AUTOINC locking (for backward compatibility);"
                         " 1 => New style AUTOINC locking;"
                         " 2 => No AUTOINC locking (unsafe for SBR)",
                         NULL, NULL, AUTOINC_NEW_STYLE_LOCKING, /* Default setting */
                         AUTOINC_OLD_STYLE_LOCKING,             /* Minimum value */
                         AUTOINC_NO_LOCKING, 0);                /* Maximum value */

static MYSQL_SYSVAR_STR(version, innodb_version_str, PLUGIN_VAR_NOCMDOPT | PLUGIN_VAR_READONLY, "InnoDB version", NULL,
                        NULL, INNODB_VERSION_STR);

static MYSQL_SYSVAR_BOOL(use_native_aio, srv_use_native_aio, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
                         "Use native AIO if supported on this platform.", NULL, NULL, TRUE);

#ifdef HAVE_LIBNUMA
static MYSQL_SYSVAR_BOOL(numa_interleave, srv_numa_interleave, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
                         "Use NUMA interleave memory policy to allocate InnoDB buffer pool.", NULL, NULL, FALSE);
#endif /* HAVE_LIBNUMA */

static MYSQL_SYSVAR_BOOL(api_enable_binlog, ib_binlog_enabled, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
                         "Enable binlog for applications direct access InnoDB through InnoDB APIs", NULL, NULL, FALSE);

static MYSQL_SYSVAR_BOOL(api_enable_mdl, ib_mdl_enabled, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
                         "Enable MDL for applications direct access InnoDB through InnoDB APIs", NULL, NULL, FALSE);

static MYSQL_SYSVAR_BOOL(api_disable_rowlock, ib_disable_row_lock, PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_READONLY,
                         "Disable row lock when direct access InnoDB through InnoDB APIs", NULL, NULL, FALSE);

static MYSQL_SYSVAR_ULONG(api_trx_level, ib_trx_level_setting, PLUGIN_VAR_OPCMDARG,
                          "InnoDB API transaction isolation level", NULL, NULL, 0, /* Default setting */
                          0,                                                       /* Minimum value */
                          3, 0);                                                   /* Maximum value */

static MYSQL_SYSVAR_ULONG(api_bk_commit_interval, ib_bk_commit_interval, PLUGIN_VAR_OPCMDARG,
                          "Background commit interval in seconds", NULL, NULL, 5, /* Default setting */
                          1,                                                      /* Minimum value */
                          1024 * 1024 * 1024, 0);                                 /* Maximum value */

static MYSQL_SYSVAR_STR(change_buffering, innobase_change_buffering, PLUGIN_VAR_RQCMDARG,
                        "Buffer changes to reduce random access:"
                        " OFF, ON, inserting, deleting, changing, or purging.",
                        innodb_change_buffering_validate, innodb_change_buffering_update, "all");

static MYSQL_SYSVAR_UINT(change_buffer_max_size, srv_change_buffer_max_size, PLUGIN_VAR_RQCMDARG,
                         "Maximum on-disk size of change buffer in terms of percentage"
                         " of the buffer pool.",
                         NULL, innodb_change_buffer_max_size_update, CHANGE_BUFFER_DEFAULT_SIZE, 0, 50, 0);

static MYSQL_SYSVAR_ENUM(stats_method, srv_innodb_stats_method, PLUGIN_VAR_RQCMDARG,
...
static MYSQL_SYSVAR_UINT(change_buffering_debug, ibuf_debug, PLUGIN_VAR_RQCMDARG,
                         "Debug flags for InnoDB change buffering (0=none, 2=crash at merge)", NULL, NULL, 0, 0, 2, 0);

static MYSQL_SYSVAR_BOOL(disable_background_merge, srv_ibuf_disable_background_merge,
                         PLUGIN_VAR_NOCMDARG | PLUGIN_VAR_RQCMDARG,
                         "Disable change buffering merges by the master thread", NULL, NULL, FALSE);

static MYSQL_SYSVAR_ENUM(compress_debug, srv_debug_compress, PLUGIN_VAR_RQCMDARG,
                         "Compress all tables, without specifying the COMPRESS table attribute", NULL, NULL,
                         Compression::NONE, &innodb_debug_compress_typelib);
#endif /* UNIV_DEBUG || UNIV_IBUF_DEBUG */

static MYSQL_SYSVAR_BOOL(random_read_ahead, srv_random_read_ahead, PLUGIN_VAR_NOCMDARG,
                         "Whether to use read ahead for random access within an extent.", NULL, NULL, FALSE);

static MYSQL_SYSVAR_ULONG(read_ahead_threshold, srv_read_ahead_threshold, PLUGIN_VAR_RQCMDARG,
                          "Number of pages that must be accessed sequentially for InnoDB to"
                          " trigger a readahead.",
                          NULL, NULL, 56, 0, 64, 0);

static MYSQL_SYSVAR_STR(monitor_enable, innobase_enable_monitor_counter, PLUGIN_VAR_RQCMDARG,
                        "Turn on a monitor counter", innodb_monitor_validate, innodb_enable_monitor_update, NULL);

static MYSQL_SYSVAR_STR(monitor_disable, innobase_disable_monitor_counter, PLUGIN_VAR_RQCMDARG,
                        "Turn off a monitor counter", innodb_monitor_validate, innodb_disable_monitor_update, NULL);

static MYSQL_SYSVAR_STR(monitor_reset, innobase_reset_monitor_counter, PLUGIN_VAR_RQCMDARG, "Reset a monitor counter",
                        innodb_monitor_validate, innodb_reset_monitor_update, NULL);

static MYSQL_SYSVAR_STR(monitor_reset_all, innobase_reset_all_monitor_counter, PLUGIN_VAR_RQCMDARG,
                        "Reset all values for a monitor counter", innodb_monitor_validate,
                        innodb_reset_all_monitor_update, NULL);

static MYSQL_SYSVAR_BOOL(status_output, srv_print_innodb_monitor, PLUGIN_VAR_OPCMDARG,
                         "Enable InnoDB monitor output to the error log.", NULL, innodb_status_output_update, FALSE);

static MYSQL_SYSVAR_BOOL(status_output_locks, srv_print_innodb_lock_monitor, PLUGIN_VAR_OPCMDARG,
                         "Enable InnoDB lock monitor output to the error log."
                         " Requires innodb_status_output=ON.",
                         NULL, innodb_status_output_update, FALSE);

static MYSQL_SYSVAR_BOOL(print_all_deadlocks, srv_print_all_deadlocks, PLUGIN_VAR_OPCMDARG,
                         "Print all deadlocks to MySQL error log (off by default)", NULL, NULL, FALSE);

static MYSQL_SYSVAR_ULONG(compression_failure_threshold_pct, zip_failure_threshold_pct, PLUGIN_VAR_OPCMDARG,
                          "If the compression failure rate of a table is greater than this number"
                          " more padding is added to the pages to reduce the failures. A value of"
                          " zero implies no padding",
                          NULL, NULL, 5, 0, 100, 0);

static MYSQL_SYSVAR_ULONG(compression_pad_pct_max, zip_pad_max, PLUGIN_VAR_OPCMDARG,
                          "Percentage of empty space on a data page that can be reserved"
                          " to make the page compressible.",
                          NULL, NULL, 50, 0, 75, 0);

static MYSQL_SYSVAR_BOOL(read_only, srv_read_only_mode, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                         "Start InnoDB in read only mode (off by default)", NULL, NULL, FALSE);

static MYSQL_SYSVAR_BOOL(cmp_per_index_enabled, srv_cmp_per_index_enabled, PLUGIN_VAR_OPCMDARG,
                         "Enable INFORMATION_SCHEMA.innodb_cmp_per_index,"
                         " may have negative impact on performance (off by default)",
                         NULL, innodb_cmp_per_index_update, FALSE);

static MYSQL_SYSVAR_ENUM(default_row_format, innodb_default_row_format, PLUGIN_VAR_RQCMDARG,
...
#ifdef UNIV_DEBUG
static MYSQL_SYSVAR_UINT(trx_rseg_n_slots_debug, trx_rseg_n_slots_debug, PLUGIN_VAR_RQCMDARG,
                         "Debug flags for InnoDB to limit TRX_RSEG_N_SLOTS for trx_rsegf_undo_find_free()", NULL, NULL,
                         0, 0, 1024, 0);

static MYSQL_SYSVAR_UINT(limit_optimistic_insert_debug, btr_cur_limit_optimistic_insert_debug, PLUGIN_VAR_RQCMDARG,
                         "Artificially limit the number of records per B-tree page (0=unlimited).", NULL, NULL, 0, 0,
                         UINT_MAX32, 0);

static MYSQL_SYSVAR_BOOL(trx_purge_view_update_only_debug, srv_purge_view_update_only_debug, PLUGIN_VAR_NOCMDARG,
                         "Pause actual purging any delete-marked records, but merely update the purge view."
                         " It is to create artificially the situation the purge view have been updated"
                         " but the each purges were not done yet.",
                         NULL, NULL, FALSE);

static MYSQL_SYSVAR_ULONG(fil_make_page_dirty_debug, srv_fil_make_page_dirty_debug, PLUGIN_VAR_OPCMDARG,
                          "Make the first page of the given tablespace dirty.", NULL, innodb_make_page_dirty, 0, 0,
                          UINT_MAX32, 0);

static MYSQL_SYSVAR_ULONG(saved_page_number_debug, srv_saved_page_number_debug, PLUGIN_VAR_OPCMDARG,
                          "An InnoDB page number.", NULL, innodb_save_page_no, 0, 0, UINT_MAX32, 0);

static MYSQL_SYSVAR_BOOL(disable_resize_buffer_pool_debug, buf_disable_resize_buffer_pool_debug, PLUGIN_VAR_NOCMDARG,
                         "Disable resizing buffer pool to make assertion code not expensive.", NULL, NULL, TRUE);

static MYSQL_SYSVAR_BOOL(page_cleaner_disabled_debug, innodb_page_cleaner_disabled_debug, PLUGIN_VAR_OPCMDARG,
                         "Disable page cleaner", NULL, buf_flush_page_cleaner_disabled_debug_update, FALSE);

static MYSQL_SYSVAR_BOOL(dict_stats_disabled_debug, innodb_dict_stats_disabled_debug, PLUGIN_VAR_OPCMDARG,
                         "Disable dict_stats thread", NULL, dict_stats_disabled_debug_update, FALSE);

static MYSQL_SYSVAR_BOOL(master_thread_disabled_debug, srv_master_thread_disabled_debug, PLUGIN_VAR_OPCMDARG,
                         "Disable master thread", NULL, srv_master_thread_disabled_debug_update, FALSE);

static MYSQL_SYSVAR_BOOL(sync_debug, srv_sync_debug, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                         "Enable the sync debug checks", NULL, NULL, FALSE);
#endif /* UNIV_DEBUG */

static struct st_mysql_sys_var *innobase_system_variables[] = {MYSQL_SYSVAR(api_trx_level),
...
mysql_declare_plugin(innobase){
    ...
    i_s_innodb_trx, i_s_innodb_locks, i_s_innodb_lock_waits, i_s_innodb_cmp, i_s_innodb_cmp_reset, i_s_innodb_cmpmem,
    i_s_innodb_cmpmem_reset, i_s_innodb_cmp_per_index, i_s_innodb_cmp_per_index_reset, i_s_innodb_buffer_page,
    i_s_innodb_buffer_page_lru, i_s_innodb_buffer_stats, i_s_innodb_temp_table_info, i_s_innodb_metrics,
    i_s_innodb_ft_default_stopword, i_s_innodb_ft_deleted, i_s_innodb_ft_being_deleted, i_s_innodb_ft_config,
    i_s_innodb_ft_index_cache, i_s_innodb_ft_index_table, i_s_innodb_sys_tables, i_s_innodb_sys_tablestats,
    i_s_innodb_sys_indexes, i_s_innodb_sys_columns, i_s_innodb_sys_fields, i_s_innodb_sys_foreign,
    i_s_innodb_sys_foreign_cols, i_s_innodb_sys_tablespaces, i_s_innodb_sys_datafiles,
...
/*=================================*/
{
  MYSQL_SYSVAR_NAME(undo_logs).max_val = MYSQL_SYSVAR_NAME(undo_logs).def_val =
...
int ha_innobase::multi_range_read_init(RANGE_SEQ_IF *seq, void *seq_init_param, uint n_ranges, uint mode,
                                       HANDLER_BUFFER *buf)
{
  return (m_ds_mrr.dsmrr_init(this, seq, seq_init_param, n_ranges, mode, buf));
}

int ha_innobase::multi_range_read_next(char **range_info) { return (m_ds_mrr.dsmrr_next(range_info)); }

ha_rows ha_innobase::multi_range_read_info_const(uint keyno, RANGE_SEQ_IF *seq, void *seq_init_param, uint n_ranges,
                                                 uint *bufsz, uint *flags, Cost_estimate *cost)
{
  /* See comments in ha_myisam::multi_range_read_info_const */
  m_ds_mrr.init(this, table);

  return (m_ds_mrr.dsmrr_info_const(keyno, seq, seq_init_param, n_ranges, bufsz, flags, cost));
}

ha_rows ha_innobase::multi_range_read_info(uint keyno, uint n_ranges, uint keys, uint *bufsz, uint *flags,
                                           Cost_estimate *cost)
{
  m_ds_mrr.init(this, table);
...
void innobase_init_vc_templ(dict_table_t *table)
{
  ...
  dbnamelen = filename_to_tablename(dbname, t_dbname, MAX_DATABASE_NAME_LEN + 1);
  tbnamelen = filename_to_tablename(tbname, t_tbname, MAX_TABLE_NAME_LEN + 1);

#ifdef UNIV_DEBUG
  bool ret =
#endif /* UNIV_DEBUG */
      handler::my_prepare_gcolumn_template(thd, t_dbname, t_tbname, &innobase_build_v_templ_callback,
...
dfield_t *innobase_get_field_from_update_vector(dict_foreign_t *foreign, upd_t *update, uint32_t col_no)
{
  ...
  {
    child_col_no = dict_index_get_nth_col_no(foreign->foreign_index, i);
...
dfield_t *innobase_get_computed_value(const dtuple_t *row, const dict_v_col_t *col, const dict_index_t *index,
                                      mem_heap_t **local_heap, mem_heap_t *heap, const dict_field_t *ifield, THD *thd,
                                      TABLE *mysql_table, const dict_table_t *old_table, upd_t *parent_update,
                                      dict_foreign_t *foreign)
{
  ...
  const page_size_t page_size =
      (old_table == NULL) ? dict_table_page_size(index->table) : dict_table_page_size(old_table);
  ...
  const mysql_row_templ_t *vctempl = index->table->vc_templ->vtempl[index->table->vc_templ->n_col + col->v_pos];
  ...
  if (!heap || index->table->vc_templ->rec_len >= REC_VERSION_56_MAX_INDEX_COL_LEN)
  {
    ...
    mysql_rec = static_cast<byte *>(mem_heap_alloc(*local_heap, index->table->vc_templ->rec_len));
    buf = static_cast<byte *>(mem_heap_alloc(*local_heap, index->table->vc_templ->rec_len));
  }
  else
  {
    mysql_rec = rec_buf1;
  ...
  for (ulint i = 0; i < col->num_base; i++)
  {
    ...
    if (row_field->ext)
    {
      ...
      data = btr_copy_externally_stored_field(&len, data, page_size, dfield_get_len(row_field), *local_heap);
    }
    ...
    if (len == UNIV_SQL_NULL)
    {
      ...
      memcpy(mysql_rec + templ->mysql_col_offset,
             static_cast<const byte *>(index->table->vc_templ->default_rec + templ->mysql_col_offset),
             templ->mysql_col_len);
    }
    else
    {
      row_sel_field_store_in_mysql_format(mysql_rec + templ->mysql_col_offset, templ, index, templ->clust_rec_field_no,
  ...
  if (mysql_table == NULL)
  {
    if (vctempl->type == DATA_BLOB)
    {
      ...
      row_mysql_store_blob_ref(mysql_rec + vctempl->mysql_col_offset, vctempl->mysql_col_len, blob_mem, max_len);
    }

    ret = handler::my_eval_gcolumn_expr_with_open(thd, index->table->vc_templ->db_name.c_str(),
                                                  index->table->vc_templ->tb_name.c_str(), &column_map,
                                                  (uchar *)mysql_rec);
  }
  else
  {
    ret = handler::my_eval_gcolumn_expr(thd, mysql_table, &column_map, (uchar *)mysql_rec);
  }
  ...
  if (vctempl->mysql_null_bit_mask && (mysql_rec[vctempl->mysql_null_byte_offset] & vctempl->mysql_null_bit_mask))
  {
  ...
  row_mysql_store_col_in_innobase_format(field, buf, TRUE, mysql_rec + vctempl->mysql_col_offset,
                                         vctempl->mysql_col_len, dict_table_is_comp(index->table));
  field->type.prtype |= DATA_VIRTUAL;

  ulint max_prefix = col->m_col.max_prefix;

  if (max_prefix && ifield && (ifield->prefix_len == 0 || ifield->prefix_len > col->m_col.max_prefix))
  {
  ...
  if (max_prefix)
  {
    len = dtype_get_at_most_n_mbchars(col->m_col.prtype, col->m_col.mbminmaxlen, max_prefix, field->len,
...
const char *BUG_REPORT_MSG = "Submit a detailed bug report to http://bugs.mysql.com";

const char *FORCE_RECOVERY_MSG = "Please refer to " REFMAN
                                 "forcing-innodb-recovery.html"
                                 " for information about forcing recovery.";

const char *ERROR_CREATING_MSG = "Please refer to " REFMAN "error-creating-innodb.html";

const char *OPERATING_SYSTEM_ERROR_MSG =
    "Some operating system error numbers are described at"
    " " REFMAN "operating-system-error-codes.html";

const char *FOREIGN_KEY_CONSTRAINTS_MSG = "Please refer to " REFMAN
                                          "innodb-foreign-key-constraints.html"
                                          " for correct foreign key definition.";

const char *SET_TRANSACTION_MSG = "Please refer to " REFMAN "set-transaction.html";

const char *INNODB_PARAMETERS_MSG = "Please refer to " REFMAN "innodb-parameters.html";
    ...
    ulint len)        /* in: length of 'to', in bytes */
{
  ...
  return (static_cast<uint>(strconvert(cs_from, from, cs_to, to, static_cast<size_t>(len), &errors)));
}
    ...
    uint *errors)     /* out: error return */
{
  ...
  return (static_cast<uint>(strconvert(cs1, from, cs2, to, static_cast<size_t>(len), errors)));
}
...
void ib_warn_row_too_big(const dict_table_t *table)
{
  ...
  const ulint free_space = page_get_free_space_of_empty(table->flags & DICT_TF_COMPACT) / 2;

  THD *thd = current_thd;

  push_warning_printf(thd, Sql_condition::SL_WARNING, HA_ERR_TOO_BIG_ROW,
                                            ...
                                            struct st_mysql_value *value)
{
  ...
  ulint requested_buf_pool_size = buf_pool_size_align(static_cast<ulint>(intbuf));
  ...
  *static_cast<longlong *>(save) = requested_buf_pool_size;
  ...
  if (intbuf != static_cast<longlong>(requested_buf_pool_size))
  {
    ...
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TRUNCATED_WRONG_VALUE, ER(ER_TRUNCATED_WRONG_VALUE),
