...
class ha_innobase : public handler
{
 public:
  ...
  static dict_table_t *open_dict_table(const char *table_name, const char *norm_name, bool is_partition,
  ...
  int get_cascade_foreign_key_table_list(THD *thd, List<st_handler_tablename> *fk_table_list);
...
bool innobase_index_name_is_reserved(THD *thd,                          /*!< in/out: MySQL connection */
...
bool tablespace_is_file_per_table(const HA_CREATE_INFO *create_info)
{
  return (create_info->tablespace != NULL &&
          (0 == strcmp(create_info->tablespace, reserved_file_per_table_space_name)));
}
...
{
  return (create_info->tablespace != NULL && create_info->tablespace[0] != '\0' &&
          (0 != strcmp(create_info->tablespace, reserved_file_per_table_space_name)));
}
...
bool tablespace_is_general_space(const HA_CREATE_INFO *create_info)
{
  return (create_info->tablespace != NULL && create_info->tablespace[0] != '\0' &&
          (0 != strcmp(create_info->tablespace, reserved_file_per_table_space_name)) &&
          (0 != strcmp(create_info->tablespace, reserved_temporary_space_name)) &&
          (0 != strcmp(create_info->tablespace, reserved_system_space_name)));
}
...
class create_table_info_t
{
 public:
  ...
  create_table_info_t(THD *thd, TABLE *form, HA_CREATE_INFO *create_info, char *table_name, char *temp_path,
                      char *remote_path, char *tablespace)
      : m_thd(thd),
        m_form(form),
...
float innobase_fts_retrieve_ranking(FT_INFO *fts_hdl); /*!< in: FTS handler */

/**
Find and Retrieve the FTS Relevance Ranking result for doc with doc_id
of prebuilt->fts_doc_id
@return the relevance ranking value */
float innobase_fts_find_ranking(FT_INFO *fts_hdl, /*!< in: FTS handler */
                                uchar *record,    /*!< in: Unused */
                                uint len);        /*!< in: Unused */

/**
Free the memory for the FTS handler */
void innobase_fts_close_ranking(FT_INFO *fts_hdl); /*!< in: FTS handler */
...
fts_doc_id_index_enum innobase_fts_check_doc_id_index(const dict_table_t *table,  /*!< in: table definition */
...
fts_doc_id_index_enum innobase_fts_check_doc_id_index_in_def(ulint n_key,         /*!< in: Number of keys */
...
ulonglong innobase_fts_retrieve_docid(FT_INFO_EXT *fts_hdl); /*!< in: FTS handler */

/**
Find and retrieve the size of the current result
@return number of matching rows */
ulonglong innobase_fts_count_matches(FT_INFO_EXT *fts_hdl); /*!< in: FTS handler */
...
InnoDB table object. */
void innobase_copy_frm_flags_from_create_info(dict_table_t *innodb_table,         /*!< in/out: InnoDB table */
...
but are frequently used inside InnoDB so we keep their copies into the
InnoDB table object. */
void innobase_copy_frm_flags_from_table_share(dict_table_t *innodb_table,      /*!< in/out: InnoDB table */
...
#ifdef _WIN32
#define normalize_table_name(norm_name, name) create_table_info_t::normalize_table_name_low(norm_name, name, TRUE)
#else
...
rec_per_key_t innodb_rec_per_key(dict_index_t *index, ulint i, ha_rows records);
