...
class Ha_innopart_share : public Partition_share
{
 ...
 public:
  ...
  bool open_table_parts(partition_info *part_info, const char *table_name);
  ...
  static size_t append_sep_and_name(char *to, const char *from, const char *sep, size_t len);
 ...
 private:
  ...
  @return false on success else true. */
  bool open_one_table_part(uint part_id, const char *partition_name);
};
...
class ha_innopart : public ha_innobase, public Partition_helper, public Partition_handler
{
 public:
  ...
  my_bool register_query_cache_table(THD *thd, char *table_key, size_t key_length, qc_engine_callback *call_back,
  ...
  bool get_foreign_dup_key(char *child_table_name, uint child_table_name_len, char *child_key_name,
  ...
  // TODO: not yet supporting FK.
  int get_foreign_key_list(THD *thd, List<FOREIGN_KEY_INFO> *f_key_list) { return (0); }

  // TODO: not yet supporting FK.
  int get_parent_foreign_key_list(THD *thd, List<FOREIGN_KEY_INFO> *f_key_list) { return (0); }

  // TODO: not yet supporting FK.
  int get_cascade_foreign_key_table_list(THD *thd, List<st_handler_tablename> *fk_table_list) { return (0); }
  ...
  Table_flags table_flags() const { return (ha_innobase::table_flags() | HA_CAN_REPAIR); }

  void release_auto_increment() { Partition_helper::ph_release_auto_increment(); }
  ...
  {
    Partition_helper::get_dynamic_partition_info_low(stat_info, check_sum, part_id);
  }
  ...
  void set_part_info(partition_info *part_info, bool early) { Partition_helper::set_part_info_low(part_info, early); }
 ...
 private:
  ...
  @return 0 for success else error code. */
  int create_new_partition(TABLE *table, HA_CREATE_INFO *create_info, const char *part_name, uint new_part_id,
  ...
  @return error number or 0. */
  int index_next_same_in_part(uint part, uchar *record, const uchar *key, uint length);
  ...
  @return error number or 0. */
  int index_read_map_in_part(uint part, uchar *record, const uchar *key, key_part_map keypart_map,
  ...
  @param[in]	keypart_map	Which part of the key to use.
  @return error number or 0. */
  int index_read_last_map_in_part(uint part, uchar *record, const uchar *key, key_part_map keypart_map);
  ...
  @return error number or 0. */
  int read_range_first_in_part(uint part, uchar *record, const key_range *start_key, const key_range *end_key,
  ...
  @param[in]	find_flag	Key condition/direction to use.
  @return error number or 0. */
  int index_read_idx_map_in_part(uint part, uchar *record, uint index, const uchar *key, key_part_map keypart_map,
                                 enum ha_rkey_function find_flag);
  ...
  void copy_cached_row(uchar *to_record, const uchar *from_record);
  /** @} */
  ...
  int write_row(uchar *record) { return (Partition_helper::ph_write_row(record)); }
  ...
  int delete_row(const uchar *record) { return (Partition_helper::ph_delete_row(record)); }
  /** @} */
  ...
  int change_partitions_low(HA_CREATE_INFO *create_info, const char *path, ulonglong *const copied,
                            ulonglong *const deleted)
  {
    return (Partition_helper::change_partitions(create_info, path, copied, deleted));
  }
  ...
  void set_range_key_part(KEY_PART_INFO *key_part) { range_key_part = key_part; }
  /** @} */
 ...
 protected:
  ...
  int rnd_next(uchar *record) { return (Partition_helper::ph_rnd_next(record)); }
  ...
  int index_next(uchar *record) { return (Partition_helper::ph_index_next(record)); }

  int index_next_same(uchar *record, const uchar *key, uint keylen)
  {
    return (Partition_helper::ph_index_next_same(record, key, keylen));
  }

  int index_prev(uchar *record) { return (Partition_helper::ph_index_prev(record)); }

  int index_first(uchar *record) { return (Partition_helper::ph_index_first(record)); }

  int index_last(uchar *record) { return (Partition_helper::ph_index_last(record)); }

  int index_read_last_map(uchar *record, const uchar *key, key_part_map keypart_map)
  {
