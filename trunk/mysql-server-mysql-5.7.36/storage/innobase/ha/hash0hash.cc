    ...
    ulint n) /*!< in: number of array cells */
{
  ...
  array = static_cast<hash_cell_t *>(ut_malloc_nokey(sizeof(hash_cell_t) * prime));
                              ...
                              must be a power of 2 */
{
  ...
  switch (table->type)
  {
    case HASH_TABLE_SYNC_MUTEX:
      table->sync_obj.mutexes = static_cast<ib_mutex_t *>(ut_malloc_nokey(n_sync_obj * sizeof(ib_mutex_t)));
    ...
    case HASH_TABLE_SYNC_RW_LOCK:
    {
      ...
      table->sync_obj.rw_locks = static_cast<rw_lock_t *>(ut_malloc_nokey(n_sync_obj * sizeof(rw_lock_t)));

      for (ulint i = 0; i < n_sync_obj; i++)
      {
        rw_lock_create(hash_table_locks_key, table->sync_obj.rw_locks + i, level);
      }
