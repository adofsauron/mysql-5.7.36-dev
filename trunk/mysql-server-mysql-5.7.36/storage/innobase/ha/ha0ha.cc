                ...
                MEM_HEAP_FOR_PAGE_HASH */
{
  ...
  if (n_sync_obj == 0)
  {
    table->heap = mem_heap_create_typed(
        ut_min(static_cast<ulint>(4096), MEM_MAX_ALLOC_IN_BUF / 2 - MEM_BLOCK_HEADER_SIZE - MEM_SPACE_NEEDED(0)), type);
    ut_a(table->heap);
  ...
  table->heaps = static_cast<mem_heap_t **>(ut_malloc_nokey(n_sync_obj * sizeof(void *)));
...
hash_table_t *ib_recreate(hash_table_t *table, ulint n)
{
  ...
  new_table->heaps = static_cast<mem_heap_t **>(ut_malloc_nokey(new_table->n_sync_obj * sizeof(void *)));

  for (ulint i = 0; i < new_table->n_sync_obj; i++)
  {
    new_table->heaps[i] = mem_heap_create_typed(
        ut_min(static_cast<ulint>(4096), MEM_MAX_ALLOC_IN_BUF / 2 - MEM_BLOCK_HEADER_SIZE - MEM_SPACE_NEEDED(0)),
        MEM_HEAP_FOR_PAGE_HASH);
  ...
  while (prev_node != NULL)
  {
    if (prev_node->fold == fold)
    {
      ...
      if (table->adaptive)
      {
        ...
        ut_a(os_atomic_decrement_ulint(&prev_block->n_pointers, 1) < MAX_N_POINTERS);
    ...
    ha_node_t *del_node) /*!< in: node to be deleted */
{
  ...
  if (table->adaptive)
  {
    ...
    ut_a(os_atomic_decrement_ulint(&del_node->block->n_pointers, 1) < MAX_N_POINTERS);
  }
#endif /* UNIV_AHI_DEBUG || UNIV_DEBUG */
    ...
    const rec_t *new_data)  /*!< in: new pointer to the data */
{
  ...
  if (node)
  {
    ...
    if (table->adaptive)
    {
      ut_a(os_atomic_decrement_ulint(&node->block->n_pointers, 1) < MAX_N_POINTERS);
      ut_a(os_atomic_increment_ulint(&new_block->n_pointers, 1) < MAX_N_POINTERS);
    }
    ...
    ulint end_index)     /*!< in: end index */
{
  ...
  for (i = start_index; i <= end_index; i++)
  {
    ...
    for (node = static_cast<ha_node_t *>(cell->node); node != 0; node = node->next)
    {
