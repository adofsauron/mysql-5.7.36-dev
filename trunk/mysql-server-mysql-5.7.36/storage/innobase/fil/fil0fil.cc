...
@param[in] print_info	if true, prints information why it
                        cannot close a file */
static bool fil_try_to_close_file_in_LRU(bool print_info);
...
#define fil_buffering_disabled(s) \
  ((s)->purpose == FIL_TYPE_TABLESPACE && srv_unix_file_flush_method == SRV_UNIX_O_DIRECT_NO_FSYNC)
#else /* _WIN32 */
...
UNIV_INLINE
bool fil_is_user_tablespace_id(ulint space_id)
{
  return (!srv_is_undo_tablespace(space_id) && space_id != srv_tmp_space.space_id());
}
...
static bool fil_validate_skip(void)
/*===================*/
{
...
static bool fil_node_prepare_for_io(
    /*====================*/
...
static void fil_node_complete_io(fil_node_t *node, fil_system_t *system, const IORequest &type);
...
UNIV_INLINE
dberr_t fil_read(const page_id_t &page_id, const page_size_t &page_size, ulint byte_offset, ulint len, void *buf)
{
  return (fil_io(IORequestRead, true, page_id, page_size, byte_offset, len, buf, NULL));
}
...
i/o on a tablespace which does not exist */
UNIV_INLINE
dberr_t fil_write(const page_id_t &page_id, const page_size_t &page_size, ulint byte_offset, ulint len, void *buf)
{
  ut_ad(!srv_read_only_mode);

  return (fil_io(IORequestWrite, true, page_id, page_size, byte_offset, len, buf, NULL));
}

/*******************************************************************/ /**
 Returns the table space by a given id, NULL if not found. */
UNIV_INLINE
fil_space_t *fil_space_get_by_id(
    /*================*/
    ulint id) /*!< in: space id */
{
  ...
  HASH_SEARCH(hash, fil_system->spaces, id, fil_space_t *, space, ut_ad(space->magic_n == FIL_SPACE_MAGIC_N),
              space->id == id);
...
UNIV_INLINE
fil_space_t *fil_space_get_by_name(
    /*==================*/
    const char *name) /*!< in: space name */
{
  ...
  HASH_SEARCH(name_hash, fil_system->name_hash, fold, fil_space_t *, space, ut_ad(space->magic_n == FIL_SPACE_MAGIC_N),
...
fil_space_t *fil_space_get(ulint id)
{
...
@return file type */
fil_type_t fil_space_get_type(ulint id)
{
  fil_space_t *space;
...
@param[in]	id	tablespace identifier */
void fil_space_set_imported(ulint id)
{
...
static bool fil_space_is_flushed(
    /*=================*/
    fil_space_t *space) /*!< in: space */
{
  ut_ad(mutex_own(&fil_system->mutex));

  for (const fil_node_t *node = UT_LIST_GET_FIRST(space->chain); node != NULL; node = UT_LIST_GET_NEXT(chain, node))
  {
...
static fil_node_t *fil_node_create_low(const char *name, ulint size, fil_space_t *space, bool is_raw, bool punch_hole,
                                       bool atomic_write, ulint max_pages = ULINT_MAX)
{
  fil_node_t *node;
  ...
  dberr_t err =
      ...
      os_file_get_status(node->name, &stat_info, false, fsp_is_system_temporary(space->id) ? true : srv_read_only_mode);
  ...
  DBUG_EXECUTE_IF("ignore_punch_hole",
                  node->block_size = ut_min(stat_info.block_size, static_cast<size_t>(UNIV_PAGE_SIZE / 2)););

  if (!IORequest::is_punch_hole_supported() || !punch_hole || node->block_size >= srv_page_size)
  {
...
@return pointer to the file name
@retval NULL if error */
char *fil_node_create(const char *name, ulint size, fil_space_t *space, bool is_raw, bool atomic_write, ulint max_pages)
{
  fil_node_t *node;

  node = fil_node_create_low(name, size, space, is_raw, IORequest::is_punch_hole_supported(), atomic_write, max_pages);
...
static bool fil_node_open_file(fil_node_t *node)
{
  ...
  if (node->size == 0 ||
      (space->purpose == FIL_TYPE_TABLESPACE && node == UT_LIST_GET_FIRST(space->chain) &&
  ...
  retry:
    node->handle = os_file_create_simple_no_error_handling(innodb_data_file_key, node->name, OS_FILE_OPEN,
    ...
    const ulint buf2_size = recv_recovery_is_on() ? (2 * UNIV_PAGE_SIZE) : UNIV_PAGE_SIZE;
    buf2 = static_cast<byte *>(ut_malloc_nokey(buf2_size + UNIV_PAGE_SIZE));
    ...
    success = os_file_read(request, node->handle, page, 0, buf2_size);
    ...
    {
      ib::error() << "The size of tablespace file " << node->name << " is only " << size_bytes
    ...
    {
      ib::fatal() << "Tablespace id is " << space->id << " in the data dictionary but in file " << node->name
                  << " it is " << space_id << "!";
    }
    ...
    {
      ib::fatal() << "Tablespace file " << node->name << " has page size " << page_size << " (flags=" << ib::hex(flags)
                  << ") but the"
                     " data dictionary expects page size "
                  << space_page_size << " (flags=" << ib::hex(space->flags) << ")!";
    }
    ...
    {
      ulint size = fsp_header_get_field(page, FSP_SIZE);
      ulint free_limit = fsp_header_get_field(page, FSP_FREE_LIMIT);
      ulint free_len = flst_get_len(FSP_HEADER_OFFSET + FSP_FREE + page);
      ...
      if (recv_recovery_is_on() && (Compression::is_compressed_page(page + page_size.physical()) ||
    ...
    if (FSP_FLAGS_GET_ENCRYPTION(flags) && !recv_recovery_is_on())
    {
      ...
      {
        ib::error() << "Can't read encryption"
                    << " key from file " << node->name << "!";
        return (false);
      }
    }
  ...
  {
    node->handle = os_file_create(innodb_log_file_key, node->name, OS_FILE_OPEN, OS_FILE_AIO, OS_LOG_FILE,
  ...
  {
    node->handle = os_file_create(innodb_data_file_key, node->name, OS_FILE_OPEN, OS_FILE_AIO, OS_DATA_FILE,
                                  read_only_mode, &success);
  }
...
#ifndef UNIV_HOTBACKUP
  ut_a(node->modification_counter == node->flush_counter || node->space->purpose == FIL_TYPE_TEMPORARY ||
       srv_fast_shutdown == 2);
#endif /* !UNIV_HOTBACKUP */
    ...
    bool print_info)
{
  ...
  if (print_info)
  {
    ib::info() << "fil_sys open file LRU len " << UT_LIST_GET_LEN(fil_system->LRU);
  }

  for (node = UT_LIST_GET_LAST(fil_system->LRU); node != NULL; node = UT_LIST_GET_PREV(LRU, node))
  {
    if (node->modification_counter == node->flush_counter && node->n_pending_flushes == 0 && !node->being_extended)
    {
    ...
    {
      ib::info() << "Cannot close file " << node->name << ", because n_pending_flushes " << node->n_pending_flushes;
    }

    if (node->modification_counter != node->flush_counter)
    {
      ib::warn() << "Cannot close file " << node->name << ", because modification count " << node->modification_counter
                 << " != flush count " << node->flush_counter;
    }

    if (node->being_extended)
    {
      ib::info() << "Cannot close file " << node->name << ", because it is being extended";
    }
  }
...
static void fil_node_close_to_free(fil_node_t *node, fil_space_t *space)
{
  ut_ad(mutex_own(&fil_system->mutex));
...
static void fil_space_detach(fil_space_t *space)
{
  ut_ad(mutex_own(&fil_system->mutex));
  ...
  for (fil_node_t *fil_node = UT_LIST_GET_FIRST(space->chain); fil_node != NULL;
...
There must not be any pending i/o's or flushes on the files.
@param[in,out]	space		tablespace */
static void fil_space_free_low(fil_space_t *space)
{
...
bool fil_space_free(ulint id, bool x_latched)
{
...
fil_space_t *fil_space_create(const char *name, ulint id, ulint flags, fil_type_t purpose)
{
  fil_space_t *space;
  ...
  if (space != NULL)
  {
    ib::error() << "Trying to add tablespace '" << name << "' with id " << id
                << " to the tablespace memory cache, but tablespace '" << space->name
...
#ifndef UNIV_HOTBACKUP
  if (fil_type_is_data(purpose) && !recv_recovery_on && id > fil_system->max_assigned_id)
  {
    if (!fil_system->space_id_reuse_warned)
    {
      fil_system->space_id_reuse_warned = true;

      ib::warn() << "Allocated tablespace ID " << id << " for " << name << ", old maximum was "
    ...
    ulint *space_id) /*!< in/out: space id */
{
  ...
  if (id > (SRV_LOG_SPACE_FIRST_ID / 2) && (id % 1000000UL == 0))
  {
    ib::warn() << "You are running out of new single-table"
               ...
               << id << " and it must not exceed" << SRV_LOG_SPACE_FIRST_ID
...
UNIV_INLINE
fil_space_t *fil_space_get_space(
    /*================*/
    ulint id) /*!< in: space id */
{
  fil_space_t *space;
...
or not found. */
char *fil_space_get_first_path(ulint id)
{
  fil_space_t *space;
  fil_node_t *node;
  char *path;
...
bool fil_space_is_being_truncated(ulint id)
{
...
bool fil_space_open(const char *name)
{
  ut_ad(fil_system != NULL);
  ...
  for (node = UT_LIST_GET_FIRST(space->chain); node != NULL; node = UT_LIST_GET_NEXT(chain, node))
  {
...
@param[in]	name	Tablespace name */
void fil_space_close(const char *name)
{
  if (fil_system == NULL)
  {
    return;
  ...
  for (fil_node_t *node = UT_LIST_GET_FIRST(space->chain); node != NULL; node = UT_LIST_GET_NEXT(chain, node))
  {
    if (node->is_open)
    {
      fil_node_close_file(node);
...
@return page size */
const page_size_t fil_space_get_page_size(ulint id, bool *found)
{
    ...
    ulint max_n_open) /*!< in: max number of open files */
{
  ...
  fil_system = static_cast<fil_system_t *>(ut_zalloc_nokey(sizeof(*fil_system)));
  ...
  UT_LIST_INIT(fil_system->unflushed_spaces, &fil_space_t::unflushed_spaces);
...
/*==========================================*/
{
  ...
  for (space = UT_LIST_GET_FIRST(fil_system->space_list); space != NULL; space = UT_LIST_GET_NEXT(space_list, space))
  {
    fil_node_t *node;
    ...
    for (node = UT_LIST_GET_FIRST(space->chain); node != NULL; node = UT_LIST_GET_NEXT(chain, node))
    {
      ...
      if (fil_system->max_n_open < 10 + fil_system->n_open)
      {
        ib::warn() << "You must raise the value of"
                      ...
                      " Current open files "
                   << fil_system->n_open << ", max allowed open files " << fil_system->max_n_open << ".";
      }
    }
  }
...
/*=====================*/
{
  ...
  ut_ad(srv_fast_shutdown == 2 || UT_LIST_GET_LEN(fil_system->named_spaces) == 0);

  mutex_enter(&fil_system->mutex);

  for (space = UT_LIST_GET_FIRST(fil_system->space_list); space != NULL;)
  {
    fil_node_t *node;
...
inline fil_space_t *fil_space_acquire_low(ulint id, bool silent)
{
  fil_space_t *space;
...
@param[in]	id	tablespace ID
@return the tablespace, or NULL if missing or being deleted */
fil_space_t *fil_space_acquire(ulint id) { return (fil_space_acquire_low(id, false)); }
...
@param[in]	id	tablespace ID
@return the tablespace, or NULL if missing or being deleted */
fil_space_t *fil_space_acquire_silent(ulint id) { return (fil_space_acquire_low(id, true)); }
...
static void fil_op_write_log(mlog_id_t type, ulint space_id, ulint first_page_no, const char *path,
                             const char *new_path, ulint flags, mtr_t *mtr)
{
  byte *log_ptr;
  ulint len;
  ...
  log_ptr = mlog_write_initial_log_record_low(type, space_id, first_page_no, log_ptr, mtr);
...
@param[in,out]	mtr		mini-transaction */
static void fil_name_write_rename(ulint space_id, ulint first_page_no, const char *old_name, const char *new_name,
                                  mtr_t *mtr)
{
  ut_ad(!is_predefined_tablespace(space_id));

  fil_op_write_log(MLOG_FILE_RENAME2, space_id, first_page_no, old_name, new_name, 0, mtr);
}
#endif /* !UNIV_HOTBACKUP */
...
@param[in,out]	mtr		mini-transaction */
static void fil_name_write(ulint space_id, ulint first_page_no, const char *name, mtr_t *mtr)
{
  fil_op_write_log(MLOG_FILE_NAME, space_id, first_page_no, name, NULL, 0, mtr);
}
...
@param[in,out]	mtr		mini-transaction */
static void fil_name_write(const fil_space_t *space, ulint first_page_no, const fil_node_t *file, mtr_t *mtr)
{
                          ...
                          TRUNCATE log record */
{
  ...
  if (!found)
  {
    ib::info() << "Missing .ibd file for table '" << name << "' with tablespace " << space_id;
    return (DB_ERROR);
  }
  ...
  err = truncate.create_indexes(name, space_id, page_size, flags, format_flags);
  if (err != DB_SUCCESS)
  {
    ib::info() << "Failed to create indexes for the table '" << name << "' with tablespace " << space_id
...
/********************************************************/ /**
 ...
 TRUNCATE log record during recovery.
 @return DB_SUCCESS or error code */
dberr_t fil_recreate_tablespace(
    /*====================*/
    ulint space_id,       /*!< in: space id */
    ulint format_flags,   /*!< in: page format */
    ulint flags,          /*!< in: tablespace flags */
                                  ...
                                  the log record */
{
  ...
  err = truncate.truncate(space_id, truncate.get_dir_path(), name, flags, true);

  if (err != DB_SUCCESS)
  {
    ib::info() << "Cannot access .ibd file for table '" << name << "' with tablespace " << space_id
               << " while truncating";
    return (DB_ERROR);
  }
  ...
  if (page_size.is_compressed())
  {
    ...
    buf_flush_init_for_writing(NULL, page, &page_zip, 0, fsp_is_checksum_disabled(space_id));

    err = fil_write(page_id_t(space_id, 0), page_size, 0, page_size.physical(), page_zip.data);
  ...
  for (ulint page_no = 0; page_no < node->size; ++page_no)
  {
    ...
    buf_block_t *block = buf_page_get(cur_page_id, page_size, RW_X_LATCH, &mtr);
    ...
    if (!fsp_flags_is_compressed(flags))
    {
      ...
      buf_flush_init_for_writing(block, page, NULL, recv_lsn, fsp_is_checksum_disabled(space_id));

      err = fil_write(cur_page_id, page_size, 0, page_size.physical(), page);
    }
    else
    {
      ut_ad(page_size.is_compressed());
      ...
      if (fil_page_get_type(page) != 0)
      {
        ...
        buf_flush_init_for_writing(block, page, page_zip, recv_lsn, fsp_is_checksum_disabled(space_id));

        err = fil_write(cur_page_id, page_size, 0, page_size.physical(), page_zip->data);
      }
      else
      {
#ifdef UNIV_DEBUG
        ...
        for (ulint i = 0; i < page_size.physical(); ++i)
        {
          ut_a(data[i] == 0);
        }
#endif /* UNIV_DEBUG */
    ...
    if (err != DB_SUCCESS)
    {
      ib::info() << "Cannot write page " << page_no << " into a .ibd file for table '" << name << "' with tablespace "
                 << space_id;
    }
  }
...
bool fil_op_replay_rename(ulint space_id, ulint first_page_no, const char *name, const char *new_name)
{
#ifdef UNIV_HOTBACKUP
  ...
  * The space ID for that tablepace matches this log entry.
  ...
  const bool name_match = strcmp(name, UT_LIST_GET_FIRST(space->chain)->name) == 0;
  ...
  char *dir = static_cast<char *>(ut_malloc_nokey(namend - new_name + 1));
  ...
  dberr_t err = fil_rename_tablespace_check(space_id, name, new_name, false);
  if (err != DB_SUCCESS)
  {
  ...
  char *new_table = mem_strdupl(new_name + dirlen, strlen(new_name + dirlen) - 4 /* remove ".ibd" */);
...
static ulint fil_check_pending_ops(fil_space_t *space, ulint count)
{
  ut_ad(mutex_own(&fil_system->mutex));
  ...
  if (n_pending_ops)
  {
    if (count > 5000)
    {
      ib::warn() << "Trying to close/delete/truncate"
                    " tablespace '"
                 << space->name << "' but there are " << n_pending_ops << " pending operations on it.";
    }
...
static ulint fil_check_pending_io(
    /*=================*/
  ...
  if (space->n_pending_flushes > 0 || (*node)->n_pending > 0)
  {
    ...
    if (count > 1000)
    {
      ib::warn() << "Trying to delete/close/truncate"
                    " tablespace '"
                 << space->name << "' but there are " << space->n_pending_flushes << " flushes and "
                 << (*node)->n_pending << " pending i/o's on it.";
    }
...
static dberr_t fil_check_pending_operations(
    /*=========================*/
    ulint id,                  /*!< in: space id */
    fil_operation_t operation, /*!< in: File operation */
...
@return DB_SUCCESS or error */
dberr_t fil_delete_tablespace(ulint id, buf_remove_t buf_remove)
{
  char *path = 0;
  fil_space_t *space = 0;
  ...
  if (const fil_space_t *s = fil_space_get_by_id(id))
  {
    ...
    if (!os_file_delete(innodb_data_file_key, path) && !os_file_delete_if_exists(innodb_data_file_key, path, NULL))
    {
...
bool fil_truncate_tablespace(ulint space_id, ulint size_in_pages)
{
  ...
  if (success)
  {
    ...
    success = os_file_set_size(node->name, node->handle, size, srv_read_only_mode);
...
/*******************************************************************/ /**
 ...
 @return DB_SUCCESS or error */
dberr_t fil_prepare_for_truncate(
    /*=====================*/
    ulint id) /*!< in: space id */
{
  char *path = 0;
  ...
  dberr_t err = fil_check_pending_operations(id, FIL_OPERATION_TRUNCATE, &space, &path);
...
void fil_reinit_space_header_for_table(dict_table_t *table, ulint size, trx_t *trx)
{
...
@param[in]	id	space id */
void fil_space_inc_redo_skipped_count(ulint id)
{
  fil_space_t *space;
...
@param[in]	id	space id */
void fil_space_dec_redo_skipped_count(ulint id)
{
  fil_space_t *space;
...
bool fil_space_is_redo_skipped(ulint id)
{
  fil_space_t *space;
...
/*******************************************************************/ /**
 ...
 @return DB_SUCCESS or error */
dberr_t fil_discard_tablespace(
    /*===================*/
    ulint id) /*!< in: space id */
{
  dberr_t err;
  ...
  switch (err = fil_delete_tablespace(id, BUF_REMOVE_ALL_NO_WRITE))
  {
    ...
    case DB_TABLESPACE_NOT_FOUND:
      ib::warn() << "Cannot delete tablespace " << id << " in DISCARD TABLESPACE: " << ut_strerr(err);
      break;
...
char *fil_make_filepath(const char *path, const char *name, ib_extention ext, bool trim_name)
{
  ...
  if (path[0] == '.' && (path[1] == '\0' || path[1] == OS_PATH_SEPARATOR) && name != NULL && name[0] == '.')
  {
    path = NULL;
    path_len = 0;
  }
...
dberr_t fil_rename_tablespace_check(ulint space_id, const char *old_path, const char *new_path, bool is_discarded)
{
  bool exists = false;
  ...
  {
    ib::error() << "Cannot rename '" << old_path << "' to '" << new_path << "' for space ID " << space_id
...
@return true if success */
bool fil_rename_tablespace(ulint id, const char *old_path, const char *new_name, const char *new_path_in)
{
  bool sleep = false;
  bool flush = false;
  fil_space_t *space;
...
retry:
  ...
  if (!(count % 1000))
  {
    ib::warn() << "Cannot rename file " << old_path << " (space id " << id << "), retried " << count
               << " times."
  ...
  if (space == NULL)
  {
    ib::error() << "Cannot find space id " << id << " in the tablespace memory cache, though the file '" << old_path
  ...
  {
    ib::error() << "Cannot find " << space->name << " in tablespace memory cache";
    space->stop_ios = false;
    goto func_exit;
  }
  ...
  {
    ib::error() << new_name << " is already in tablespace memory cache";
    space->stop_ios = false;
    goto func_exit;
  }
  ...
  if (node->n_pending > 0 || node->n_pending_flushes > 0 || node->being_extended)
  {
  ...
  char *new_file_name = new_path_in == NULL ? fil_make_filepath(NULL, new_name, IBD, false) : mem_strdup(new_path_in);
  ...
  if (!recv_recovery_on)
  {
    ...
    mtr.start();
    fil_name_write_rename(id, 0, old_file_name, new_file_name, &mtr);
    mtr.commit();
    log_mutex_enter();
  }
#endif /* !UNIV_HOTBACKUP */
  ...
  DBUG_EXECUTE_IF("fil_rename_tablespace_failure_2", skip_rename : success = false;);
  ...
  if (success)
  {
    HASH_DELETE(fil_space_t, name_hash, fil_system->name_hash, old_fold, space);
    space->name = new_space_name;
    HASH_INSERT(fil_space_t, name_hash, fil_system->name_hash, new_fold, space);
  }
  else
  {
...
@return DB_SUCCESS or error code */
dberr_t fil_ibd_create(ulint space_id, const char *name, const char *path, ulint flags, ulint size)
{
  pfs_os_file_t file;
  dberr_t err;
  byte *buf2;
  ...
  file = os_file_create(innodb_data_file_key, path, OS_FILE_CREATE | OS_FILE_ON_ERROR_NO_EXIT, OS_FILE_NORMAL,
  ...
  if (fil_fusionio_enable_atomic_write(file))
  {
    ...
    if (ret != 0)
    {
      ib::error() << "posix_fallocate(): Failed to preallocate"
                     " data for file "
                  << path << ", desired size " << size * UNIV_PAGE_SIZE << " Operating system error number " << ret
                  << ". Check"
                     " that the disk is not full or a disk quota"
                     " exceeded. Make sure the file system supports"
                     " this function. Some operating system error"
                     " numbers are described at " REFMAN " operating-system-error-codes.html";
  ...
  else
  {
    ...
    success = os_file_set_size(path, file, size * UNIV_PAGE_SIZE, srv_read_only_mode);
  }
#else
  atomic_write = false;
  ...
  if (!page_size.is_compressed())
  {
    buf_flush_init_for_writing(NULL, page, NULL, 0, fsp_is_checksum_disabled(space_id));
  ...
  else
  {
    ...
    err = os_file_write(request, path, file, page_zip.data, 0, page_size.physical());
  ...
  if (has_data_dir || has_shared_space)
  {
    ...
    err = RemoteDatafile::create_link_file(name, path, has_shared_space);
    if (err != DB_SUCCESS)
    {
      os_file_close(file);
      os_file_delete(innodb_data_file_key, path);
  ...
  if (!fil_node_create_low(path, size, space, false, punch_hole, atomic_write))
  {
    err = DB_ERROR;
    goto error_exit_1;
  }
  ...
  {
    err = fil_set_encryption(space->id, Encryption::AES, NULL, NULL);
    ut_ad(err == DB_SUCCESS);
  }
  ...
  if (!is_temp)
  {
    ...
    mtr_start(&mtr);
    fil_op_write_log(MLOG_FILE_CREATE2, space_id, 0, file->name, NULL, space->flags, &mtr);
...
@return DB_SUCCESS or error code */
dberr_t fil_ibd_open(bool validate, bool fix_dict, fil_type_t purpose, ulint id, ulint flags, const char *space_name,
                     const char *path_in)
{
  dberr_t err = DB_SUCCESS;
  ...
  {
    atomic_write = fil_fusionio_enable_atomic_write(df_default.handle());
  }
  else
  {
    atomic_write = false;
  ...
  locations, if found. */
  valid_tablespaces_found += (df_remote.validate_to_dd(id, flags, for_import) == DB_SUCCESS) ? 1 : 0;

  valid_tablespaces_found += (df_default.validate_to_dd(id, flags, for_import) == DB_SUCCESS) ? 1 : 0;

  valid_tablespaces_found += (df_dict.validate_to_dd(id, flags, for_import) == DB_SUCCESS) ? 1 : 0;
  ...
  if (valid_tablespaces_found == 0)
  {
    if (!is_encrypted)
    {
      ...
      ib::error() << "Could not find a valid tablespace file for `" << space_name << "`. " << TROUBLESHOOT_DATADICT_MSG;
    }
  ...
  {
    ib::error() << "A tablespace for `" << space_name << "` has been found in multiple places;";

    if (df_default.is_open())
    {
      ib::error() << "Default location: " << df_default.filepath() << ", Space ID=" << df_default.space_id()
                  << ", Flags=" << df_default.flags();
    }
    if (df_remote.is_open())
    {
      ib::error() << "Remote location: " << df_remote.filepath() << ", Space ID=" << df_remote.space_id()
                  << ", Flags=" << df_remote.flags();
    }
    if (df_dict.is_open())
    {
      ib::error() << "Dictionary location: " << df_dict.filepath() << ", Space ID=" << df_dict.space_id()
                  << ", Flags=" << df_dict.flags();
    }
    ...
    {
      ib::error() << "Will not open tablespace `" << space_name << "`";
      ...
      if (df_default.is_open() != df_default.is_valid() || df_dict.is_open() != df_dict.is_valid() ||
  ...
  if (df_dict.filepath())
  {
    ...
    else if (!is_shared && (!link_file_found || link_file_is_bad))
    {
      ut_ad(df_dict.is_open());
      ...
      RemoteDatafile::delete_link_file(space_name);
      RemoteDatafile::create_link_file(space_name, df_dict.filepath());
    }
  }
  else if (df_remote.is_open())
  {
    ...
    else if (path_in == NULL)
    {
      ...
      was not found. */
      dict_replace_tablespace_and_filepath(id, space_name, df_remote.filepath(), flags);
    }
  }
  else if (df_default.is_open())
  {
    ...
    if ((path_in != NULL && !dict_filepath_same_as_default) ||
        (path_in == NULL && (DICT_TF_HAS_DATA_DIR(flags) || DICT_TF_HAS_SHARED_SPACE(flags))) ||
        df_remote.filepath() != NULL)
    {
      dict_replace_tablespace_and_filepath(id, space_name, df_default.filepath(), flags);
    }
  }
...
skip_validate:
  if (err == DB_SUCCESS)
  {
    fil_space_t *space = fil_space_create(space_name, id, flags, purpose);

    /* We do not measure the size of the file, that is why
    we pass the 0 below */

    if (fil_node_create_low(df_remote.is_open() ? df_remote.filepath()
                            : df_dict.is_open() ? df_dict.filepath()
                                                : df_default.filepath(),
                            0, space, false, true, atomic_write) == NULL)
    {
      err = DB_ERROR;
    }
    ...
    {
      Datafile &df_current = df_remote.is_open() ? df_remote : df_dict.is_open() ? df_dict : df_default;
    ...
    const char *name) /*!< in: original file name */
{
  ...
  ut_sprintf_timestamp_without_extra_chars(path + len + sizeof(suffix) - 1);
  return (path);
}
#endif /* UNIV_HOTBACKUP */
...
bool fil_space_read_name_and_filepath(ulint space_id, char **name, char **filepath)
{
  bool success = false;
  *name = NULL;
  *filepath = NULL;
...
#ifdef HAVE_MEMRCHR
  const char *tablename = 1 + static_cast<const char *>(memrchr(filename, OS_PATH_SEPARATOR, filename_len));
  const char *dbname = 1 + static_cast<const char *>(memrchr(filename, OS_PATH_SEPARATOR, tablename - filename - 1));
#else  /* HAVE_MEMRCHR */
  const char *tablename = filename;
  const char *dbname = NULL;

  while (const char *t = static_cast<const char *>(memchr(tablename, OS_PATH_SEPARATOR, end - tablename)))
  {
    dbname = tablename;
    tablename = t + 1;
  }
...
bool fil_ibd_discover(ulint space_id, Datafile &df)
{
  ...
  if (df_def_gen.open_read_only(false) == DB_SUCCESS && df_def_gen.validate_for_recovery() == DB_SUCCESS &&
  ...
  if (sep_found == 2)
  {
    ...
    if (df_def_per.open_read_only(false) == DB_SUCCESS && df_def_per.validate_for_recovery() == DB_SUCCESS &&
  ...
  if (df_rem_gen.open_link_file() == DB_SUCCESS)
  {
    ...
    if (df_rem_gen.open_read_only(false) != DB_SUCCESS || df_rem_gen.validate_for_recovery() != DB_SUCCESS)
    {
      ...
      ib::error() << "ISL file '" << df_rem_gen.link_filepath() << "' was found but the linked file '"
                  << df_rem_gen.filepath() << "' could not be opened or is not correct.";
      return (false);
    }
  ...
  if (sep_found == 2)
  {
    ...
    {
      /* An ISL file was found with contents. */
      if (df_rem_per.open_read_only(false) != DB_SUCCESS || df_rem_per.validate_for_recovery() != DB_SUCCESS)
      {
        ...
        a valid IBD file. */
        ib::error() << "ISL file '" << df_rem_per.link_filepath() << "' was found but the linked file '"
                    << df_rem_per.filepath()
  ...
  if (df.open_read_only(false) == DB_SUCCESS && df.validate_for_recovery() == DB_SUCCESS && df.space_id() == space_id)
  {
    return (true);
  }
...
enum fil_load_status fil_ibd_load(ulint space_id, const char *filename, fil_space_t *&space)
{
  ...
  if (space != NULL)
  {
    ...
    if (0 != strcmp(innobase_basename(filename), innobase_basename(node->name)))
    {
#ifdef UNIV_HOTBACKUP
      ib::trace()
#else
      ib::info()
#endif /* UNIV_HOTBACKUP */
          << "Ignoring data file '" << filename << "' with space ID " << space->id << ". Another data file called "
          << node->name << " exists with the same space ID.";
  ...
  if (!file.is_open())
  {
    ...
    if (!fil_ibd_discover(space_id, file) || !file.is_open())
    {
  ...
  switch (file.validate_for_recovery())
  {
    ...
    case DB_SUCCESS:
      if (file.space_id() != space_id)
      {
        ...
        ib::info()
#endif /* UNIV_HOTBACKUP */
            << "Ignoring data file '" << file.filepath() << "' with space ID " << file.space_id()
            << ", since the redo log references " << file.filepath() << " with space ID " << space_id << ".";
        return (FIL_LOAD_ID_CHANGED);
      }
      ...
      {
#ifndef UNIV_HOTBACKUP
        ib::error() << "The size of tablespace file '" << file.filepath() << "' is only " << size
  ...
  if (file.space_id() == ULINT_UNDEFINED || file.space_id() == 0)
  {
    ...
    ib::info() << "Renaming tablespace file '" << file.filepath() << "' with space ID " << file.space_id() << " to "
               << file.name()
    ...
    bool success = os_file_rename(innodb_data_file_key, file.filepath(), new_path);
  ...
  if (space != NULL)
  {
    ib::info() << "Renaming data file '" << file.filepath() << "' with space ID " << space_id << " to " << file.name()
  ...
  space = fil_space_create(file.name(), space_id, file.flags(), is_temp ? FIL_TYPE_TEMPORARY : FIL_TYPE_TABLESPACE);
  ...
  if (!fil_node_create_low(file.filepath(), 0, space, false, true, false))
  {
    ut_error;
  }

  /* For encryption tablespace, initial encryption information. */
  if (FSP_FLAGS_GET_ENCRYPTION(space->flags) && file.m_encryption_key != NULL)
  {
    dberr_t err = fil_set_encryption(space->id, Encryption::AES, file.m_encryption_key, file.m_encryption_iv);
    if (err != DB_SUCCESS)
    {
...
/***********************************************************************/ /**
 ...
 of the directory */
int fil_file_readdir_next_file(
    /*=======================*/
...
static void fil_report_missing_tablespace(
    /*===========================*/
    const char *name, /*!< in: table name */
    ulint space_id)   /*!< in: table's space id */
{
  ib::error() << "Table " << name << " in the InnoDB data dictionary has tablespace id " << space_id
              << ","
...
bool fil_space_for_table_exists_in_mem(ulint id, const char *name, bool print_error_if_does_not_exist,
                                       bool adjust_space, mem_heap_t *heap, table_id_t table_id)
{
  ...
  if (space != NULL && FSP_FLAGS_GET_SHARED(space->flags) && adjust_space && srv_sys_tablespaces_open &&
      0 == strncmp(space->name, general_space_name, strlen(general_space_name)))
  {
    ...
    ut_free(space->name);
    space->name = mem_strdup(name);
    HASH_INSERT(fil_space_t, name_hash, fil_system->name_hash, ut_fold_string(space->name), space);
  ...
  if (adjust_space && space != NULL && row_is_mysql_tmp_table_name(space->name) && !row_is_mysql_tmp_table_name(name))
  {
    mutex_exit(&fil_system->mutex);

    DBUG_EXECUTE_IF("ib_crash_before_adjust_fil_space", DBUG_SUICIDE(););

    if (fnamespace)
    {
      const char *tmp_name;

      tmp_name = dict_mem_create_temporary_tablename(heap, name, table_id);

      fil_rename_tablespace(fnamespace->id, UT_LIST_GET_FIRST(fnamespace->chain)->name, tmp_name, NULL);
    }

    DBUG_EXECUTE_IF("ib_crash_after_adjust_one_fil_space", DBUG_SUICIDE(););

    fil_rename_tablespace(id, UT_LIST_GET_FIRST(space->chain)->name, name, NULL);

    DBUG_EXECUTE_IF("ib_crash_after_adjust_fil_space", DBUG_SUICIDE(););
...
ulint fil_space_get_id_by_name(const char *tablespace)
{
  mutex_enter(&fil_system->mutex);
...
@return DB_SUCCESS or error code */
static dberr_t fil_write_zeros(const fil_node_t *node, ulint page_size, os_offset_t start, ulint len,
                               bool read_only_mode)
{
  ut_a(len > 0);
  ...
  {
#ifdef UNIV_HOTBACKUP
    err = os_file_write(request, node->name, node->handle, buf, offset, n_bytes);
#else
    err = os_aio_func(request, OS_AIO_SYNC, node->name, node->handle, buf, offset, n_bytes, read_only_mode, NULL, NULL);
#endif /* UNIV_HOTBACKUP */
...
bool fil_space_extend(fil_space_t *space, ulint size)
{
...
retry:
  ...
  ib::trace() << "space id : " << space->id << ", space name : " << space->name << ", space size : " << actual_size
              << " pages,"
  ...
  ulint n_node_physical_pages = static_cast<ulint>(node_start) / page_size;
  ...
  if (node->size + n_node_extend > n_node_physical_pages)
  {
    ...
    if (ret != 0)
    {
      ...
      {
        ib::error() << "posix_fallocate(): Failed to"
                       " preallocate data for file "
                    << node->name << ", desired size " << len
                    << " bytes."
    ...
    if (!node->atomic_write || err == DB_IO_ERROR)
    {
      ...
      read_only_mode = (space->purpose != FIL_TYPE_TEMPORARY ? false : srv_read_only_mode);

      err = fil_write_zeros(node, page_size, node_start, static_cast<ulint>(len), read_only_mode);

      if (err != DB_SUCCESS)
      {
        ib::warn() << "Error while writing " << len << " zeroes to " << node->name << " starting at offset "
                   << node_start;
      }
    }
...
#else
  ib::trace() << "extended space : " << space->name << " from " << actual_size << " pages to " << space->size
              << " pages "
              << ", desired space size : " << size << " pages.";
#endif /* !UNIV_HOTBACKUP */
...
/*======================================*/
{
  ...
  for (fil_space_t *space = UT_LIST_GET_FIRST(fil_system->space_list); space != NULL;
       space = UT_LIST_GET_NEXT(space_list, space))
  {
    ...
    error = fil_read(page_id_t(space->id, 0), page_size_t(space->flags), 0, univ_page_size.physical(), buf);
    ...
    fil_space_t *space)   /*!< in: space */
{
  ...
  {
    ib::warn() << "Open files " << system->n_open << " exceeds the limit " << system->max_n_open;
  }
                           ...
                           modified if TYPE_IS_WRITE() */
{
  ...
  if (type.is_write())
  {
    ut_ad(!srv_read_only_mode || fsp_is_system_temporary(node->space->id));
...
static void fil_report_invalid_page_access(ulint block_offset,     /*!< in: block offset */
                                           ulint space_id,         /*!< in: space id */
                                           ...
                                           bool is_read)           /*!< in: I/O type */
{
  ib::error() << "Trying to access page number " << block_offset
              ...
              << byte_offset << ", len " << len << ", i/o type " << (is_read ? "read" : "write")
...
inline void fil_io_set_encryption(IORequest &req_type, const page_id_t &page_id, fil_space_t *space)
{
  /* Don't encrypt the log, page 0 of all tablespaces, all pages
  from the system tablespace. */
  if (!req_type.is_log() && page_id.page_no() > 0 && space->encryption_type != Encryption::NONE)
  {
    req_type.encryption_key(space->encryption_key, space->encryption_klen, space->encryption_iv);
...
dberr_t fil_io(const IORequest &type, bool sync, const page_id_t &page_id, const page_size_t &page_size,
...
#error "(1 << UNIV_PAGE_SIZE_SHIFT_MIN) != UNIV_PAGE_SIZE_MIN"
  ...
  ut_ad(recv_no_ibuf_operations || req_type.is_write() || !ibuf_bitmap_page(page_id, page_size) || sync ||
        req_type.is_log());
  ...
  else if (req_type.is_read() && !recv_no_ibuf_operations && ibuf_page(page_id, page_size, NULL))
  {
    mode = OS_AIO_IBUF;
  ...
  if (space == NULL || (req_type.is_read() && !sync && space->stop_new_ops && !space->is_being_truncated))
  {
    mutex_exit(&fil_system->mutex);
    ...
    if (!req_type.ignore_missing())
    {
      ib::error() << "Trying to do I/O to a tablespace which"
                  ...
                  << (req_type.is_read() ? "read" : "write") << ", page: " << page_id << ", I/O length: " << len
                  << " bytes";
    }
  ...
  for (;;)
  {
    if (node == NULL)
    {
      ...
      fil_report_invalid_page_access(page_id.page_no(), page_id.space(), space->name, byte_offset, len,
                                     req_type.is_read());
    }
    else if (fil_is_user_tablespace_id(space->id) && node->size == 0)
    {
    ...
    {
      if (space->id != srv_sys_space.space_id() && UT_LIST_GET_LEN(space->chain) == 1 &&
          (srv_is_tablespace_truncated(space->id) || space->is_being_truncated ||
  ...
  if (!fil_node_prepare_for_io(node, fil_system, space))
  {
    if (fil_type_is_data(space->purpose) && fil_is_user_tablespace_id(space->id))
    {
      mutex_exit(&fil_system->mutex);
      ...
      if (!req_type.ignore_missing())
      {
        ib::error() << "Trying to do I/O to a tablespace"
                       ...
                       " I/O type: "
                    << (req_type.is_read() ? "read" : "write") << ", page: " << page_id_t(page_id.space(), cur_page_no)
  ...
  if (node->size <= cur_page_no && space->id != srv_sys_space.space_id() && fil_type_is_data(space->purpose))
  {
    if (req_type.ignore_missing())
    {
  ...
  if (!page_size.is_compressed())
  {
    offset = ((os_offset_t)cur_page_no << UNIV_PAGE_SIZE_SHIFT) + byte_offset;

    ut_a(node->size - cur_page_no >= ((byte_offset + len + (UNIV_PAGE_SIZE - 1)) / UNIV_PAGE_SIZE));
  }
  else
  {
    ulint size_shift;
    ...
    offset = ((os_offset_t)cur_page_no << size_shift) + byte_offset;

    ut_a(node->size - cur_page_no >= (len + (page_size.physical() - 1)) / page_size.physical());
  }
  ...
  if (req_type.is_write() && !req_type.is_log() && !page_size.is_compressed() && page_id.page_no() > 0 &&
  ...
  else
  {
    ...
    err = os_file_write(req_type, node->name, node->handle, buf, offset, len);
  }
#else /* UNIV_HOTBACKUP */
  /* Queue the aio request */
  err = os_aio(req_type, mode, node->name, node->handle, buf, offset, len,
               fsp_is_system_temporary(page_id.space()) ? false : srv_read_only_mode, node, message);
  ...
  if (err == DB_IO_NO_PUNCH_HOLE)
  {
    ...
    if (node->punch_hole)
    {
      ib::warn() << "Punch hole failed for '" << node->name << "'";
    }
                    ...
                    log files or a tablespace of the database) */
{
  ...
  if (space == NULL || space->purpose == FIL_TYPE_TEMPORARY || space->stop_new_ops || space->is_being_truncated)
  {
    mutex_exit(&fil_system->mutex);
  ...
  if (fbd)
  {
    ...
    {
#ifdef UNIV_DEBUG
      ut_ad(node->modification_counter == node->flush_counter);
#endif /* UNIV_DEBUG */
...
void fil_flush_file_spaces(fil_type_t purpose)
{
  fil_space_t *space;
  ulint *space_ids;
  ulint n_space_ids;
  ...
  space_ids = static_cast<ulint *>(ut_malloc_nokey(n_space_ids * sizeof(*space_ids)));
  ...
  {
    if (space->purpose == purpose && !space->stop_new_ops && !space->is_being_truncated)
    {
...
/*==============*/
{
  ...
  {
    for (space = static_cast<fil_space_t *>(HASH_GET_FIRST(fil_system->spaces, i)); space != 0;
  ...
  for (fil_node = UT_LIST_GET_FIRST(fil_system->LRU); fil_node != 0; fil_node = UT_LIST_GET_NEXT(LRU, fil_node))
  {
...
{
  ib::info() << "Resetting invalid page " << page_id << " type " << fil_page_get_type(page) << " to " << type << ".";
...
static void fil_buf_block_init(
    /*===============*/
...
static dberr_t fil_iterate(
    /*========*/
    const fil_iterator_t &iter, buf_block_t *block, PageCallback &callback)
{
  os_offset_t offset;
  ulint page_no = 0;
  ...
  for (offset = iter.start; offset < iter.end; offset += n_bytes)
  {
    ...
    if (callback.get_page_size().is_compressed())
    {
      ...
      block->page.size.copy_from(page_size_t(iter.page_size, univ_page_size.logical(), true));
    ...
    n_bytes = static_cast<ulint>(ut_min(static_cast<os_offset_t>(n_bytes), iter.end - offset));
    ...
    {
      read_request.encryption_key(iter.encryption_key, ENCRYPTION_KEY_LEN, iter.encryption_iv);
      read_request.encryption_algorithm(Encryption::AES);
    }

    err = os_file_read(read_request, iter.file, io_buffer, offset, (ulint)n_bytes);
    ...
    if (iter.encryption_key != NULL && offset != 0)
    {
      write_request.encryption_key(iter.encryption_key, ENCRYPTION_KEY_LEN, iter.encryption_iv);
    ...
    if (updated &&
        (err = os_file_write(write_request, iter.filepath, iter.file, io_buffer, offset, (ulint)n_bytes)) != DB_SUCCESS)
    {
...
/********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
dberr_t fil_tablespace_iterate(
    /*===================*/
    dict_table_t *table, ulint n_io_buffers, PageCallback &callback)
{
  dberr_t err;
  pfs_os_file_t file;
  char *filepath;
  ...
  if (DICT_TF_HAS_DATA_DIR(table->flags))
  {
    ...
    filepath = fil_make_filepath(table->data_dir_path, table->name.m_name, IBD, true);
  }
  else
  {
    filepath = fil_make_filepath(NULL, table->name.m_name, IBD, false);
  }
  ...
  file = os_file_create_simple_no_error_handling(innodb_data_file_key, filepath, OS_FILE_OPEN, OS_FILE_READ_WRITE,
  ...
  else if ((err = callback.init(file_size, block)) == DB_SUCCESS)
  {
    ...
    if (FSP_FLAGS_GET_ENCRYPTION(space_flags))
    {
      ...
      else
      {
        ...
        if (table->encryption_key == NULL || table->encryption_iv == NULL)
        {
          err = DB_ERROR;
        }
      }
    ...
    if (err == DB_SUCCESS)
    {
      ...
      {
        iter.n_io_buffers = 1;
        ut_a(iter.page_size == callback.get_page_size().physical());
      }

      /** Add an extra page for compressed page scratch
      area. */
      void *io_buffer = ut_malloc_nokey((2 + iter.n_io_buffers) * UNIV_PAGE_SIZE);

      iter.io_buffer = static_cast<byte *>(ut_align(io_buffer, UNIV_PAGE_SIZE));
    ...
    const char *ibd_filepath)
{
  ...
  char *cfg_filepath = fil_make_filepath(ibd_filepath, NULL, CFG, false);
  if (cfg_filepath != NULL)
  {
    os_file_delete_if_exists(innodb_data_file_key, cfg_filepath, NULL);
    ut_free(cfg_filepath);
  }

  char *cfp_filepath = fil_make_filepath(ibd_filepath, NULL, CFP, false);
  if (cfp_filepath != NULL)
  {
    os_file_delete_if_exists(innodb_data_file_key, cfp_filepath, NULL);
...
const fil_node_t *fil_node_next(const fil_node_t *prev_node)
{
  fil_space_t *space;
  ...
  else
  {
    ...
    if (node == NULL)
    {
      ...
      while (space != NULL && (UT_LIST_GET_LEN(space->chain) == 0 || space->stop_new_ops || space->is_being_truncated))
      {
        space = UT_LIST_GET_NEXT(space_list, space);
      }
...
@param[in,out]	mtr		mini-transaction
@return innodb error code */
dberr_t fil_mtr_rename_log(const dict_table_t *old_table, const dict_table_t *new_table, const char *tmp_name,
                           mtr_t *mtr)
{
  dberr_t err;

  bool old_is_file_per_table = !is_system_tablespace(old_table->space) && !DICT_TF_HAS_SHARED_SPACE(old_table->flags);

  bool new_is_file_per_table = !is_system_tablespace(new_table->space) && !DICT_TF_HAS_SHARED_SPACE(new_table->flags);
  ...
  const char *old_dir = DICT_TF_HAS_DATA_DIR(old_table->flags) ? old_table->data_dir_path : NULL;

  char *old_path = fil_make_filepath(old_dir, old_table->name.m_name, IBD, (old_dir != NULL));
  if (old_path == NULL)
  {
    return (DB_OUT_OF_MEMORY);
  }
  ...
  {
    char *tmp_path = fil_make_filepath(old_dir, tmp_name, IBD, (old_dir != NULL));
    if (tmp_path == NULL)
    {
      ut_free(old_path);
      return (DB_OUT_OF_MEMORY);
    ...
    err = fil_rename_tablespace_check(old_table->space, old_path, tmp_path, dict_table_is_discarded(old_table));
    if (err != DB_SUCCESS)
    {
      ut_free(old_path);
      ut_free(tmp_path);
    ...
    fil_name_write_rename(old_table->space, 0, old_path, tmp_path, mtr);
  ...
  {
    const char *new_dir = DICT_TF_HAS_DATA_DIR(new_table->flags) ? new_table->data_dir_path : NULL;
    char *new_path = fil_make_filepath(new_dir, new_table->name.m_name, IBD, (new_dir != NULL));
    if (new_path == NULL)
    {
      ut_free(old_path);
      return (DB_OUT_OF_MEMORY);
    ...
    if (!old_is_file_per_table)
    {
      err = fil_rename_tablespace_check(new_table->space, new_path, old_path, dict_table_is_discarded(new_table));
      if (err != DB_SUCCESS)
      {
        ut_free(old_path);
        ut_free(new_path);
    ...
    fil_name_write_rename(new_table->space, 0, new_path, old_path, mtr);
...
static void fil_space_validate_for_mtr_commit(const fil_space_t *space)
{
  ut_ad(!mutex_own(&fil_system->mutex));
  ut_ad(space != NULL);
  ...
  ut_ad(!space->stop_new_ops || space->is_being_truncated /* TRUNCATE sets stop_new_ops */
...
@param[in]	space	tablespace
@param[in,out]	mtr	mini-transaction */
static void fil_names_write(const fil_space_t *space, mtr_t *mtr)
{
...
by redo log.
@param[in,out]	space	tablespace */
void fil_names_dirty(fil_space_t *space)
{
  ut_ad(log_mutex_own());
...
@param[in,out]	space	tablespace
@param[in,out]	mtr	mini-transaction */
void fil_names_dirty_and_write(fil_space_t *space, mtr_t *mtr)
{
  ut_ad(log_mutex_own());
  ut_d(fil_space_validate_for_mtr_commit(space));
  ...
  DBUG_EXECUTE_IF("fil_names_write_bogus", {
    ...
    fil_name_write(SRV_LOG_SPACE_FIRST_ID, 0, bogus_name, mtr);
  });
}
#ifndef UNIV_HOTBACKUP
...
bool fil_names_clear(lsn_t lsn, bool do_write)
{
  mtr_t mtr;
  ulint mtr_checkpoint_size = LOG_CHECKPOINT_FREE_PER_THREAD;

  DBUG_EXECUTE_IF("increase_mtr_checkpoint_size", mtr_checkpoint_size = 75 * 1024;);
  ...
  for (fil_space_t *space = UT_LIST_GET_FIRST(fil_system->named_spaces); space != NULL;)
  {
...
@return DB_SUCCESS or error */
dberr_t truncate_t::truncate(
    /*=================*/
    ulint space_id, const char *dir_path, const char *tablename, ulint flags, bool trunc_to_default)
{
  dberr_t err = DB_SUCCESS;
  char *path;
  bool has_data_dir = FSP_FLAGS_HAS_DATA_DIR(flags);
  ...
  if (!already_open)
  {
    ...
    node->handle =
        os_file_create_simple_no_error_handling(innodb_data_file_key, path, OS_FILE_OPEN, OS_FILE_READ_WRITE,
                                                fsp_is_system_temporary(space_id) ? false : srv_read_only_mode, &ret);

    if (!ret)
    {
      ib::error() << "Failed to open tablespace file " << path << ".";
  ...
  os_offset_t trunc_size = trunc_to_default ? FIL_IBD_FILE_INITIAL_SIZE : space->size;

  const bool success = os_file_truncate(path, node->handle, trunc_size * UNIV_PAGE_SIZE);

  if (!success)
  {
    ib::error() << "Cannot truncate file " << path << " in TRUNCATE TABLESPACE.";
    err = DB_ERROR;
  }
  ...
  if (!already_open)
  {
    ...
    if (!closed)
    {
      ib::error() << "Failed to close tablespace file " << path << ".";
...
void fil_no_punch_hole(fil_node_t *node) { node->punch_hole = false; }
...
dberr_t fil_set_compression(dict_table_t *table, const char *algorithm)
{
  ...
  if (table->ibd_file_missing || !DICT_TF2_FLAG_IS_SET(table, DICT_TF2_USE_FILE_PER_TABLE) ||
      DICT_TF2_FLAG_IS_SET(table, DICT_TF2_TEMPORARY) || page_size_t(table->flags).is_compressed())
  {
...
Compression::Type fil_get_compression(ulint space_id)
{
  fil_space_t *space = fil_space_get(space_id);
...
@return DB_SUCCESS or error code */
dberr_t fil_set_encryption(ulint space_id, Encryption::Type algorithm, byte *key, byte *iv)
{
...
bool fil_encryption_rotate()
{
  ...
  for (space = UT_LIST_GET_FIRST(fil_system->space_list); space != NULL;)
  {
    ...
    if (is_system_or_undo_tablespace(space->id) || fsp_is_system_temporary(space->id) || space->purpose == FIL_TYPE_LOG)
    {
      space = UT_LIST_GET_NEXT(space_list, space);
      continue;
    }
...
void Folder::make_path(const char *path, size_t len)
{
  ...
  else
  {
    ...
    if (path != fil_path_to_mysql_datadir)
    {
      ...
      memcpy(m_folder, fil_path_to_mysql_datadir, m_folder_len);
...
class Folder &Folder::operator=(const char *path)
{
  ut_free(m_folder);
...
{
  return (m_abs_len == other.m_abs_len && !memcmp(m_abs_path, other.m_abs_path, m_abs_len));
}
...
bool Folder::operator>=(const Folder &other) const
{
  return (m_abs_len <= other.m_abs_len && (!memcmp(other.m_abs_path, m_abs_path, m_abs_len)));
}
...
bool Folder::exists()
{
  ...
  if (m_abs_path[m_abs_len - 1] == OS_PATH_SEPARATOR && m_abs_path[m_abs_len - 2] != ':')
  {
...
void test_make_filepath()
{
  ...
  const char *long_path =
      ...
      "/folder/name";
  path = MF("/this/is/a/path/with/a/filename", NULL, IBD, false);
  DISPLAY;
  path = MF("/this/is/a/path/with/a/filename", NULL, ISL, false);
  DISPLAY;
  path = MF("/this/is/a/path/with/a/filename", NULL, CFG, false);
  DISPLAY;
  path = MF("/this/is/a/path/with/a/filename", NULL, CFP, false);
  DISPLAY;
  path = MF("/this/is/a/path/with/a/filename.ibd", NULL, IBD, false);
  DISPLAY;
  path = MF("/this/is/a/path/with/a/filename.ibd", NULL, IBD, false);
  DISPLAY;
  path = MF("/this/is/a/path/with/a/filename.dat", NULL, IBD, false);
  DISPLAY;
  path = MF(NULL, "tablespacename", NO_EXT, false);
  DISPLAY;
  path = MF(NULL, "tablespacename", IBD, false);
  DISPLAY;
  path = MF(NULL, "dbname/tablespacename", NO_EXT, false);
  DISPLAY;
  path = MF(NULL, "dbname/tablespacename", IBD, false);
  DISPLAY;
  path = MF(NULL, "dbname/tablespacename", ISL, false);
  DISPLAY;
  path = MF(NULL, "dbname/tablespacename", CFG, false);
  DISPLAY;
  path = MF(NULL, "dbname/tablespacename", CFP, false);
  DISPLAY;
  path = MF(NULL, "dbname\\tablespacename", NO_EXT, false);
  DISPLAY;
  path = MF(NULL, "dbname\\tablespacename", IBD, false);
  DISPLAY;
  ...
  DISPLAY;
  path = MF(long_path, NULL, IBD, false);
  DISPLAY;
