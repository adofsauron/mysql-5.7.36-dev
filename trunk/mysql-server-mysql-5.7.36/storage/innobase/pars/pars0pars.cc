    ...
    ulint ptr_len)    /* in: length of literal to (re) bind*/
{
  ...
  op_check = static_cast<ib_like_t>(mach_read_from_4(static_cast<byte *>(dfield_get_data(dfield))));
    ...
    que_node_t *arg) /* in: LIKE comparison string.*/
{
  ...
  ut_a(dtype_get_mtype(dtype) == DATA_CHAR || dtype_get_mtype(dtype) == DATA_VARCHAR);
  ...
  if (ptr_len)
  {
    func = pars_like_rebind(static_cast<sym_node_t *>(arg), (byte *)ptr, ptr_len);
  }
                      ...
                      operator */
{
  ...
  if (func == PARS_LIKE_TOKEN)
  {
    ...
    ut_a(func == PARS_LIKE_TOKEN_EXACT || func == PARS_LIKE_TOKEN_PREFIX || func == PARS_LIKE_TOKEN_SUFFIX ||
    ...
    que_node_t *exp_node)   /*!< in: expression */
{
  ...
  while (t_node)
  {
    ...
    for (i = 0; i < n_cols; i++)
    {
      ...
      if ((sym_node->name_len == ut_strlen(col_name)) && (0 == ut_memcmp(sym_node->name, col_name, sym_node->name_len)))
      {
        /* Found */
        sym_node->resolved = TRUE;
    ...
    sym_node_t *sym_node) /*!< in: table node */
{
  ...
  if (sym_node->token_type != SYM_TABLE_REF_COUNTED)
  {
    ...
    sym_node->table = dict_table_open_on_name(sym_node->name, TRUE, FALSE, DICT_ERR_IGNORE_NONE);
                             ...
                             the table list */
{
  ...
  while (table_node)
  {
    ...
    for (i = 0; i < dict_table_get_n_user_cols(table); i++)
    {
      ...
      col_node = sym_tab_add_id(pars_sym_tab_global, (byte *)col_name, ut_strlen(col_name));
    ...
    que_node_t *exp)    /*!< in: value to assign */
{
  ...
  node = static_cast<col_assign_node_t *>(mem_heap_alloc(pars_sym_tab_global->heap, sizeof(col_assign_node_t)));
    ...
    que_node_t *search_cond) /*!< in: search condition or NULL */
{
  ...
  else
  {
    ...
    pars_select_statement(sel_node, table_sym, search_cond, NULL, &pars_share_token, NULL);
                           ...
                           NOT NULL. */
{
  ...
  else if (type == &pars_binary_token)
  {
    ...
    dtype_set(dfield_get_type(dfield), DATA_FIXBINARY, DATA_BINARY_TYPE | flags, len);
  }
...
/*=======================*/
{
  ...
  node = static_cast<return_node_t *>(mem_heap_alloc(pars_sym_tab_global->heap, sizeof(return_node_t)));
    ...
    que_node_t *val) /*!< in: value to assign */
{
  ...
  node = static_cast<assign_node_t *>(mem_heap_alloc(pars_sym_tab_global->heap, sizeof(assign_node_t)));
    ...
    sym_node_t *user_func) /*!< in: user function name, or NULL */
{
  ...
  else
  {
    ...
    node->func = pars_info_lookup_user_func(pars_sym_tab_global->info, user_func->name);
    ...
    sel_node_t *sel_node) /*!< in: select node */
{
  ...
  node = static_cast<row_printf_node_t *>(mem_heap_alloc(pars_sym_tab_global->heap, sizeof(row_printf_node_t)));
                           ...
                           is of type NOT NULL. */
{
  ...
  pars_set_dfield_type(que_node_get_val(sym_node), type, len2, is_unsigned != NULL, is_not_null != NULL);
...
from disk */
{
  ...
  if (block_size != NULL)
  {
    ...
    size = mach_read_from_4(static_cast<byte *>(dfield_get_data(dfield)));
  ...
  DBUG_EXECUTE_IF("innodb_test_wrong_fts_aux_table_name", flags2 &= ~DICT_TF2_FTS_AUX_HEX_NAME;);

  n_cols = que_node_list_get_len(column_defs);

  table = dict_mem_table_create(table_sym->name, 0, n_cols, 0, flags, flags2);
  ...
  while (column)
  {
    ...
    dict_mem_table_add_col(table, table->heap, column->name, dtype->mtype, dtype->prtype, dtype->len);
...
If the literal value already exists then it rebinds otherwise it
creates a new entry. */
void pars_info_bind_int4_literal(
    /*========================*/
    pars_info_t *info,      /* in: info struct */
    const char *name,       /* in: name */
...
If the literal value already exists then it rebinds otherwise it
creates a new entry. */
void pars_info_bind_int8_literal(
    /*========================*/
    pars_info_t *info,      /* in: info struct */
    const char *name,       /* in: name */
