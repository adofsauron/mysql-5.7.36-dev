                              ...
                              side */
{
  ...
  ut_a((search_cond->func == '<') || (search_cond->func == '>') || (search_cond->func == '=') ||
       ...
       (search_cond->func == PARS_LIKE_TOKEN_EXACT) || (search_cond->func == PARS_LIKE_TOKEN_PREFIX) ||
       (search_cond->func == PARS_LIKE_TOKEN_SUFFIX) || (search_cond->func == PARS_LIKE_TOKEN_SUBSTR));

  table = sel_node_get_nth_plan(sel_node, nth_table)->table;

  if ((cmp_type == OPT_EQUAL) && (search_cond->func != '=') && (search_cond->func != PARS_LIKE_TOKEN_EXACT) &&
      (search_cond->func != PARS_LIKE_TOKEN_PREFIX))
  {
    return (NULL);
           ...
           (search_cond->func != PARS_GE_TOKEN) && (search_cond->func != PARS_LE_TOKEN) &&
           (search_cond->func != PARS_LIKE_TOKEN_PREFIX) && (search_cond->func != PARS_LIKE_TOKEN_SUFFIX))
  {
    return (NULL);
  }
                             ...
                             goodness > 1 */
{
  ...
  for (j = 0; j < n_fields; j++)
  {
    ...
    exp = opt_look_for_col_in_cond_before(OPT_EQUAL, col_no, static_cast<func_node_t *>(sel_node->search_cond),
    ...
    else
    {
      ...
      exp = opt_look_for_col_in_cond_before(OPT_COMPARISON, col_no, static_cast<func_node_t *>(sel_node->search_cond),
                                            sel_node, nth_table, &op);
      if (exp)
      {
        index_plan[j] = exp;
...
{
  if (op == '=' || op == PARS_LIKE_TOKEN_EXACT || op == PARS_LIKE_TOKEN_PREFIX || op == PARS_LIKE_TOKEN_SUFFIX ||
    ...
    dict_table_t *table)  /*!< in: table */
{
  ...
  else
  {
    ...
    plan->tuple_exps = static_cast<que_node_t **>(mem_heap_alloc(pars_sym_tab_global->heap, n_fields * sizeof(void *)));

    ut_memcpy(plan->tuple_exps, best_index_plan, n_fields * sizeof(void *));
    if (best_last_op == '=' || best_last_op == PARS_LIKE_TOKEN_EXACT || best_last_op == PARS_LIKE_TOKEN_PREFIX ||
        best_last_op == PARS_LIKE_TOKEN_SUFFIX || best_last_op == PARS_LIKE_TOKEN_SUBSTR)
    {
  ...
  if (dict_index_is_clust(best_index) && (plan->n_exact_match >= dict_index_get_n_unique(best_index)))
  {
    ...
    ulint i)              /*!< in: ith table in the join */
{
  ...
  opt_find_all_cols(FALSE, plan->index, &plan->columns, plan, static_cast<func_node_t *>(sel_node->search_cond));
}
    ...
    ulint n)              /*!< in: nth table in select */
{
  ...
  plan->clust_map = static_cast<ulint *>(mem_heap_alloc(heap, n_fields * sizeof(ulint)));
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    if (dict_is_sys_table(index->table->id) && (dict_index_get_nth_field(index, pos)->prefix_len != 0 ||
                                                dict_index_get_nth_field(clust_index, i)->prefix_len != 0))
    {
      ib::error() << "Error in pars0opt.cc: table " << index->table->name << " has prefix_len != 0";
    }
    ...
    sel_node_t *sel_node) /*!< in: parsed select node */
{
  ...
  sel_node->plans =
      static_cast<plan_t *>(mem_heap_alloc(pars_sym_tab_global->heap, sel_node->n_tables * sizeof(plan_t)));
    ...
    sel_node_t *sel_node) /*!< in: select node */
{
  ...
  for (i = 0; i < sel_node->n_tables; i++)
  {
    ...
    fprintf(stderr,
            ...
            plan->index->name(), plan->index->table_name, (unsigned long)plan->n_exact_match, (unsigned long)n_fields,
