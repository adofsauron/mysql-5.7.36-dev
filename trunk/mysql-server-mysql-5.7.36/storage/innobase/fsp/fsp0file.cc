...
dberr_t Datafile::open_or_create(bool read_only_mode)
{
  ...
  m_handle = os_file_create(innodb_data_file_key, m_filepath, m_open_flags, OS_FILE_NORMAL, OS_DATA_FILE,
...
dberr_t Datafile::open_read_only(bool strict)
{
  ...
  m_handle = os_file_create_simple_no_error_handling(innodb_data_file_key, m_filepath, m_open_flags, OS_FILE_READ_ONLY,
                                                     true, &success);
  ...
  {
    m_last_os_error = os_file_get_last_error(true);
    ib::error() << "Cannot open datafile for read-only: '" << m_filepath << "' OS error: " << m_last_os_error;
  }
...
dberr_t Datafile::open_read_write(bool read_only_mode)
{
  ...
  set_open_flags(OS_FILE_OPEN);
  m_handle = os_file_create_simple_no_error_handling(innodb_data_file_key, m_filepath, m_open_flags, OS_FILE_READ_WRITE,
                                                     read_only_mode, &success);
...
void Datafile::make_filepath(const char *dirpath, const char *filename, ib_extention ext)
{
...
bool Datafile::same_filepath_as(const char *other) const { return (0 == strcmp(m_filepath, other)); }
...
{
#ifdef _WIN32
  return (m_file_info.dwVolumeSerialNumber == other.m_file_info.dwVolumeSerialNumber &&
          m_file_info.nFileIndexHigh == other.m_file_info.nFileIndexHigh &&
          m_file_info.nFileIndexLow == other.m_file_info.nFileIndexLow);
#else
  return (m_file_info.st_ino == other.m_file_info.st_ino && m_file_info.st_dev == other.m_file_info.st_dev);
#endif /* WIN32 */
}
...
void Datafile::set_name(const char *name)
{
  ...
  else
  {
    ...
    m_name = static_cast<char *>(ut_malloc_nokey(strlen(general_space_name) + 20));
...
dberr_t Datafile::read_first_page(bool read_only_mode)
{
  ...
  while (page_size >= UNIV_PAGE_SIZE_MIN)
  {
    ...
    err = os_file_read_no_error_handling(request, m_handle, m_first_page, 0, page_size, &n_read);
    ...
    {
      ib::error() << "Cannot read first page of '" << m_filepath << "' " << ut_strerr(err);
      break;
    }
  }
...
dberr_t Datafile::validate_to_dd(ulint space_id, ulint flags, bool for_import)
{
  dberr_t err;
  ...
  if (m_space_id == space_id &&
      (m_flags & FSP_FLAGS_MASK_SHARED || (m_flags & ~FSP_FLAGS_MASK_DATA_DIR) == (flags & ~FSP_FLAGS_MASK_DATA_DIR)))
  {
...
@retval DB_SUCCESS if tablespace is valid, DB_ERROR if not.
m_is_valid is also set true on success, else false. */
dberr_t Datafile::validate_for_recovery()
{
  dberr_t err;
  ...
  switch (err)
  {
    ...
    case DB_TABLESPACE_EXISTS:
      ...
      if (err == DB_SUCCESS)
      {
        std::string filepath = fil_space_get_first_path(m_space_id);
...
dberr_t Datafile::validate_first_page(lsn_t *flush_lsn, bool for_import)
{
  char *prev_name;
  char *prev_filepath;
  ...
  if (m_first_page == NULL && read_first_page(srv_read_only_mode) != DB_SUCCESS)
  {
  ...
  if (error_txt == NULL && m_space_id == srv_sys_space.space_id() && !m_flags)
  {
  ...
  else if (univ_page_size.logical() != page_size.logical())
  {
    ...
    ib::error() << "Data file '" << m_filepath << "' uses page size " << page_size.logical()
  ...
  else if (buf_page_is_corrupted(false, m_first_page, page_size, fsp_is_checksum_disabled(m_space_id)))
  {
  ...
  if (error_txt != NULL)
  {
    ib::error() << error_txt << " in datafile: " << m_filepath << ", Space ID:" << m_space_id << ", Flags: " << m_flags
                << ". " << TROUBLESHOOT_DATADICT_MSG;
    m_is_valid = false;
  ...
  if (FSP_FLAGS_GET_ENCRYPTION(m_flags) && !for_import)
  {
...
#endif
    if (!fsp_header_get_encryption_key(m_flags, m_encryption_key, m_encryption_iv, m_first_page))
    {
    ...
    if (recv_recovery_is_on() && memcmp(m_encryption_key, m_encryption_iv, ENCRYPTION_KEY_LEN) == 0)
    {
      ut_free(m_encryption_key);
      ut_free(m_encryption_iv);
      m_encryption_key = NULL;
  ...
  if (fil_space_read_name_and_filepath(m_space_id, &prev_name, &prev_filepath))
  {
    ...
    ib::error() << "Attempted to open a previously opened"
                ...
                << prev_name << " at filepath: " << prev_filepath << " uses space ID: " << m_space_id
                << ". Cannot open filepath: " << m_filepath << " which uses the same space ID.";
    ...
    return (is_predefined_tablespace(m_space_id) ? DB_CORRUPTION : DB_TABLESPACE_EXISTS);
  }
...
dberr_t Datafile::find_space_id()
{
  ...
  for (ulint page_size = UNIV_ZIP_SIZE_MIN; page_size <= UNIV_PAGE_SIZE_MAX; page_size <<= 1)
  {
    /* map[space_id] = count of pages */
    typedef std::map<ulint, ulint, std::less<ulint>, ut_allocator<std::pair<const ulint, ulint>>> Pages;
    ...
    ib::info() << "Page size:" << page_size << ". Pages to analyze:" << page_count;
    ...
    for (ulint j = 0; j < page_count; ++j)
    {
      ...
      if (err == DB_IO_DECOMPRESS_FAIL)
      {
        ...
        {
          err = os_file_read(request, m_handle, page, page_size, UNIV_PAGE_SIZE_MAX);
      ...
      {
        noncompressed_ok = !buf_page_is_corrupted(false, page, univ_page_size, false);
      }
      ...
      univ_page_size.logical(). */
      if (univ_page_size.logical() <= UNIV_PAGE_SIZE_DEF && page_size <= univ_page_size.logical())
      {
        const page_size_t compr_page_size(page_size, univ_page_size.logical(), true);

        compressed_ok = !buf_page_is_corrupted(false, page, compr_page_size, false);
      }
      ...
      if (noncompressed_ok || compressed_ok)
      {
        ...
        if (space_id > 0)
        {
          ib::info() << "VALID: space:" << space_id << " page_no:" << j << " page_size:" << page_size;
    ...
    ib::info() << "Page size: " << page_size << ". Possible space_id count:" << verify.size();

    const ulint pages_corrupted = 3;

    for (ulint missed = 0; missed <= pages_corrupted; ++missed)
    {
      for (Pages::const_iterator it = verify.begin(); it != verify.end(); ++it)
      {
        ib::info() << "space_id:" << it->first << ", Number of pages matched: " << it->second << "/" << valid_pages
                   << " (" << page_size << ")";
...
dberr_t Datafile::restore_from_doublewrite(ulint restore_page_no)
{
  ...
  if (page == NULL)
  {
    ...
    ib::error() << "Corrupted page " << page_id_t(m_space_id, restore_page_no) << " of datafile '" << m_filepath
                << "' could not be found in the doublewrite buffer.";

    return (DB_CORRUPTION);
  }

  const ulint flags = mach_read_from_4(FSP_HEADER_OFFSET + FSP_SPACE_FLAGS + page);

  const page_size_t page_size(flags);

  ut_a(page_get_page_no(page) == restore_page_no);

  ib::info() << "Restoring page " << page_id_t(m_space_id, restore_page_no) << " of datafile '" << m_filepath
             << "' from the doublewrite buffer. Writing " << page_size.physical() << " bytes into file '" << m_filepath
             << "'";
  ...
  return (os_file_write(request, m_filepath, m_handle, page, 0, page_size.physical()));
}
...
dberr_t RemoteDatafile::open_link_file()
{
...
dberr_t RemoteDatafile::open_read_only(bool strict)
{
  ...
  if (err != DB_SUCCESS && strict)
  {
    ...
    ib::error() << "A link file was found named '" << m_link_filepath << "' but the linked tablespace '" << m_filepath
...
@param[in]	read_only_mode	If true, then readonly mode checks are enforced.
@return DB_SUCCESS if remote linked tablespace file is found and opened. */
dberr_t RemoteDatafile::open_read_write(bool read_only_mode)
{
  if (m_filepath == NULL && open_link_file() == DB_CANNOT_OPEN_FILE)
  {
    return (DB_ERROR);
  ...
  if (err != DB_SUCCESS)
  {
    /* The following call prints an error message */
    m_last_os_error = os_file_get_last_error(true);
    ib::error() << "A link file was found named '" << m_link_filepath << "' but the linked data file '" << m_filepath
...
void RemoteDatafile::set_link_filepath(const char *path)
{
  ...
  if (path != NULL && FSP_FLAGS_GET_SHARED(flags()))
  {
    ...
    m_link_filepath = fil_make_filepath(NULL, base_name(path), ISL, false);
  }
  else
  {
...
@return DB_SUCCESS or error code */
dberr_t RemoteDatafile::create_link_file(const char *name, const char *filepath, bool is_shared)
{
  ...
  if (is_shared)
  {
    ...
    link_filepath = fil_make_filepath(NULL, base_name(filepath), ISL, false);
  }
  else
  {
  ...
  if (error != 0)
  {
    ...
    {
      ib::error() << "The link file: " << link_filepath << " already exists.";
      err = DB_TABLESPACE_EXISTS;
    }
  ...
  {
    os_file_get_last_error(true);
    ib::error() << "Cannot write link file " << link_filepath << ".";
    err = DB_ERROR;
  }
...
void RemoteDatafile::delete_link_file(void)
{
  ut_ad(m_link_filepath != NULL);

  if (m_link_filepath != NULL)
  {
    os_file_delete_if_exists(innodb_data_file_key, m_link_filepath, NULL);
  }
}

/** Delete an InnoDB Symbolic Link (ISL) file by name.
@param[in]	name	tablespace name */
void RemoteDatafile::delete_link_file(const char *name)
{
...
char *RemoteDatafile::read_link_file(const char *link_filepath)
{
  char *filepath = NULL;
  FILE *file = NULL;
  ...
  filepath = static_cast<char *>(ut_malloc_nokey(OS_FILE_MAX_PATH));
