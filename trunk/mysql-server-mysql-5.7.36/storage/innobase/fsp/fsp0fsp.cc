...
static xdes_t *fseg_get_first_extent(fseg_inode_t *inode, ulint space_id, const page_size_t &page_size, mtr_t *mtr);
...
static buf_block_t *fseg_alloc_free_page_low(fil_space_t *space, const page_size_t &page_size, fseg_inode_t *seg_inode,
                                             ulint hint, byte direction, rw_lock_type_t rw_latch, mtr_t *mtr,
                                             mtr_t *init_mtr
#ifdef UNIV_DEBUG
                                             ,
...
UNIV_INLINE
fsp_header_t *fsp_get_space_header(ulint id, const page_size_t &page_size, mtr_t *mtr)
{
  buf_block_t *block;
  fsp_header_t *header;
...
ulint fsp_flags_to_dict_tf(ulint fsp_flags, bool compact)
{
  ...
  ulint flags = dict_tf_init(post_antelope | compact, zip_ssize, atomic_blobs, data_dir, shared_space);
...
bool fsp_flags_is_valid(ulint flags)
{
  ...
  if (page_ssize != 0 && (page_ssize < UNIV_PAGE_SSIZE_MIN || page_ssize > UNIV_PAGE_SSIZE_MAX))
  {
    return (false);
  }
...
bool fsp_is_system_temporary(ulint space_id) { return (space_id == srv_tmp_space.space_id()); }

/** Check if checksum is disabled for the given space.
@param[in]	space_id	tablespace ID
@return true if checksum is disabled for given space. */
bool fsp_is_checksum_disabled(ulint space_id) { return (fsp_is_system_temporary(space_id)); }

/** Check if tablespace is file-per-table.
@param[in]	space_id	tablespace ID
@param[in]	fsp_flags	tablespace flags
@return true if tablespace is file-per-table. */
bool fsp_is_file_per_table(ulint space_id, ulint fsp_flags)
{
  return (!is_system_tablespace(space_id) && !fsp_is_shared_tablespace(fsp_flags));
}
...
bool fsp_skip_sanity_check(ulint space_id)
{
  return (srv_skip_temp_table_checks_debug && fsp_is_system_temporary(space_id));
}
...
UNIV_INLINE MY_ATTRIBUTE((warn_unused_result)) xdes_t *xdes_get_descriptor_with_space_hdr(
    fsp_header_t *sp_header, ulint space, ulint offset, mtr_t *mtr, bool init_space = false,
    buf_block_t **desc_block = NULL)
{
  ...
  ut_ad(
      limit == fspace->free_limit ||
      (fspace->free_limit == 0 && (init_space || fspace->purpose == FIL_TYPE_TEMPORARY ||
                                   (srv_startup_is_before_trx_rollback_phase && fspace->id <= srv_undo_tablespaces))));
  ...
  {
    block = buf_page_get(page_id_t(space, descr_page_no), page_size, RW_SX_LATCH, mtr);
  ...
  return (descr_page + XDES_ARR_OFFSET + XDES_SIZE * xdes_calc_descriptor_index(page_size, offset));
}
...
xdes_t *xdes_get_descriptor(ulint space_id, ulint offset, const page_size_t &page_size, mtr_t *mtr)
{
  buf_block_t *block;
...
xdes_t *xdes_lst_get_descriptor(
    ...
    const page_size_t &page_size, fil_addr_t lst_node, /*!< in: file address of the list node
    ...
    mtr_t *mtr)                                        /*!< in/out: mini-transaction */
{
  ...
  descr = fut_get_ptr(space, page_size, lst_node, RW_SX_LATCH, mtr) - XDES_FLST_NODE;
    ...
    buf_block_t *block) /*!< in: pointer to a page */
{
  ...
  mach_write_to_4(page + FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID, block->page.id.space());

  if (page_zip)
  {
    memset(page_zip->data, 0, page_zip_get_size(page_zip));
    memcpy(page_zip->data + FIL_PAGE_OFFSET, page + FIL_PAGE_OFFSET, 4);
    memcpy(page_zip->data + FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID, page + FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID, 4);
  }
}
...
static void fsp_space_modify_check(ulint id, const mtr_t *mtr)
{
  switch (mtr->get_log_mode())
  {
...
#ifdef UNIV_DEBUG
    {
      ...
      ut_a(id == srv_tmp_space.space_id() || srv_is_tablespace_truncated(id) || fil_space_is_being_truncated(id) ||
           fil_space_get_flags(id) == ULINT_UNDEFINED || type == FIL_TYPE_TEMPORARY || type == FIL_TYPE_IMPORT ||
...
static ulint fsp_header_get_encryption_offset(const page_size_t &page_size)
{
  ulint offset;
#ifdef UNIV_DEBUG
  ulint left_size;
...
#ifdef UNIV_DEBUG
  left_size = page_size.physical() - FSP_HEADER_OFFSET - offset - FIL_PAGE_DATA_END;
...
bool fsp_header_fill_encryption_info(fil_space_t *space, byte *encrypt_info)
{
  byte *ptr;
  lint elen;
  ulint master_key_id;
  ...
  memcpy(key_info + ENCRYPTION_KEY_LEN, space->encryption_iv, ENCRYPTION_KEY_LEN);

#ifdef UNIV_ENCRYPT_DEBUG
  fprintf(stderr, "Set %lu:%lu ", space->id, Encryption::master_key_id);
  for (data = (const byte *)master_key, i = 0; i < ENCRYPTION_KEY_LEN; i++) fprintf(stderr, "%02lx", (ulong)*data++);
  fprintf(stderr, " ");
  for (data = (const byte *)space->encryption_key, i = 0; i < ENCRYPTION_KEY_LEN; i++)
    fprintf(stderr, "%02lx", (ulong)*data++);
  fprintf(stderr, " ");
  for (data = (const byte *)space->encryption_iv, i = 0; i < ENCRYPTION_KEY_LEN; i++)
    fprintf(stderr, "%02lx", (ulong)*data++);
  fprintf(stderr, "\n");
#endif
  /* Encrypt tablespace key and iv. */
  elen = my_aes_encrypt(key_info, ENCRYPTION_KEY_LEN * 2, ptr, master_key, ENCRYPTION_KEY_LEN, my_aes_256_ecb, NULL,
                        false);
...
bool fsp_header_rotate_encryption(fil_space_t *space, byte *encrypt_info, mtr_t *mtr)
{
  ...
  block = buf_page_get(page_id_t(space->id, 0), page_size, RW_SX_LATCH, mtr);
  buf_block_dbg_add_level(block, SYNC_FSP_PAGE);
  ...
  if (recv_recovery_is_on() && master_key_id == Encryption::master_key_id)
  {
    ut_ad(memcmp(page + offset, ENCRYPTION_KEY_MAGIC_V1, ENCRYPTION_MAGIC_SIZE) == 0 ||
          memcmp(page + offset, ENCRYPTION_KEY_MAGIC_V2, ENCRYPTION_MAGIC_SIZE) == 0);
    return (true);
  }

  mlog_write_string(page + offset, encrypt_info, ENCRYPTION_INFO_SIZE_V2, mtr);
...
bool fsp_header_init(ulint space_id, ulint size, mtr_t *mtr)
{
  fsp_header_t *header;
  buf_block_t *block;
  page_t *page;
  ...
  if (FSP_FLAGS_GET_ENCRYPTION(space->flags))
  {
    ...
    if (!fsp_header_fill_encryption_info(space, encryption_info))
    {
    ...
    mlog_write_string(page + offset, encryption_info, ENCRYPTION_INFO_SIZE_V2, mtr);
  }

  if (space_id == srv_sys_space.space_id())
  {
    if (btr_create(DICT_CLUSTERED | DICT_IBUF, 0, univ_page_size, DICT_IBUF_ID_MIN + space_id, dict_ind_redundant, NULL,
                   mtr) == FIL_NULL)
    {
      return (false);
    }
    ...
    const page_t *page) /*!< in: first page of a tablespace */
{
  ...
  DBUG_EXECUTE_IF("fsp_header_get_space_id_failure", id = ULINT_UNDEFINED;);

  if (id != fsp_id)
  {
    ib::error() << "Space ID in fsp header is " << fsp_id << ", but in the page header it is " << id << ".";
    return (ULINT_UNDEFINED);
  }
...
@return page size */
page_size_t fsp_header_get_page_size(const page_t *page) { return (page_size_t(fsp_header_get_flags(page))); }
...
@return true if success */
bool fsp_header_decode_encryption_info(byte *key, byte *iv, byte *encryption_info)
{
  byte *ptr;
  ulint master_key_id;
  ...
  /* Check magic. */
  if (version == Encryption::ENCRYPTION_VERSION_2 && memcmp(ptr, ENCRYPTION_KEY_MAGIC_V2, ENCRYPTION_MAGIC_SIZE) != 0)
  {
  ...
  elen = my_aes_decrypt(ptr, ENCRYPTION_KEY_LEN * 2, key_info, master_key, ENCRYPTION_KEY_LEN, my_aes_256_ecb, NULL,
                        false);
...
#ifdef UNIV_ENCRYPT_DEBUG
  fprintf(stderr, " ");
  for (data = (const byte *)key, i = 0; i < ENCRYPTION_KEY_LEN; i++) fprintf(stderr, "%02lx", (ulong)*data++);
  fprintf(stderr, " ");
  for (data = (const byte *)iv, i = 0; i < ENCRYPTION_KEY_LEN; i++) fprintf(stderr, "%02lx", (ulong)*data++);
  fprintf(stderr, "\n");
#endif
...
@param[in]	page	first page of a tablespace
@return true if success */
bool fsp_header_get_encryption_key(ulint fsp_flags, byte *key, byte *iv, page_t *page)
{
  ulint offset;
...
@param[in,out]	mtr	mini-transaction
@return true if success */
static UNIV_COLD MY_ATTRIBUTE((warn_unused_result)) bool fsp_try_extend_data_file_with_pages(fil_space_t *space,
                                                                                             ulint page_no,
                                                                                             fsp_header_t *header,
                                                                                             mtr_t *mtr)
{
...
static UNIV_COLD ulint fsp_try_extend_data_file(fil_space_t *space, fsp_header_t *header, mtr_t *mtr)
{
  ...
  if (space->id == srv_sys_space.space_id() && !srv_sys_space.can_auto_extend_last_file())
  {
    ...
    {
      ib::error() << "Tablespace " << srv_sys_space.name() << " " << OUT_OF_SPACE_MSG << " innodb_data_file_path.";
  ...
  else if (fsp_is_system_temporary(space->id) && !srv_tmp_space.can_auto_extend_last_file())
  {
    ...
    {
      ib::error() << "Tablespace " << srv_tmp_space.name() << " " << OUT_OF_SPACE_MSG << " innodb_temp_data_file_path.";
  ...
  {
    ulint extent_pages = fsp_get_extent_size_in_pages(page_size);
  ...
  space->size_in_header = ut_calc_align_down(space->size, (1024 * 1024) / page_size.physical());

  mlog_write_ulint(header + FSP_SIZE, space->size_in_header, MLOG_4BYTES, mtr);
...
ulint fsp_get_pages_to_extend_ibd(const page_size_t &page_size, ulint size)
{
  ulint size_increase; /* number of pages to extend this file */
...
@param[in,out]	header		tablespace header
@param[in,out]	mtr		mini-transaction */
static void fsp_fill_free_list(bool init_space, fil_space_t *space, fsp_header_t *header, mtr_t *mtr)
{
  ulint limit;
  ulint size;
  ulint flags;
  ...
  if (size < limit + FSP_EXTENT_SIZE * FSP_FREE_ADD)
  {
    if ((!init_space && !is_system_tablespace(space->id)) ||
        (space->id == srv_sys_space.space_id() && srv_sys_space.can_auto_extend_last_file()) ||
        (space->id == srv_tmp_space.space_id() && srv_tmp_space.can_auto_extend_last_file()))
    {
...
static xdes_t *fsp_alloc_free_extent(ulint space_id, const page_size_t &page_size, ulint hint, mtr_t *mtr)
{
  fsp_header_t *header;
  fil_addr_t first;
  xdes_t *descr;
  ...
  descr = xdes_get_descriptor_with_space_hdr(header, space_id, hint, mtr, false, &desc_block);
...
static buf_block_t *fsp_page_create(const page_id_t &page_id, const page_size_t &page_size, rw_lock_type_t rw_latch,
                                    mtr_t *mtr, mtr_t *init_mtr)
{
  buf_block_t *block = buf_page_create(page_id, page_size, init_mtr);

  ut_ad(mtr_memo_contains(mtr, block, MTR_MEMO_PAGE_X_FIX) == rw_lock_own(&block->lock, RW_LOCK_X));

  ut_ad(mtr_memo_contains(mtr, block, MTR_MEMO_PAGE_SX_FIX) == rw_lock_own(&block->lock, RW_LOCK_SX));
  ...
  mtr_memo_push(init_mtr, block, rw_latch == RW_X_LATCH ? MTR_MEMO_PAGE_X_FIX : MTR_MEMO_PAGE_SX_FIX);

  if (init_mtr == mtr || (rw_latch == RW_X_LATCH ? rw_lock_get_x_lock_count(&block->lock) == 1
                                                 : rw_lock_get_sx_lock_count(&block->lock) == 1))
  {
    ...
    ut_ad(init_mtr == mtr || !mtr_memo_contains_flagged(mtr, block, MTR_MEMO_PAGE_X_FIX | MTR_MEMO_PAGE_SX_FIX));
...
(init_mtr == mtr, or the page was not previously freed in mtr)
@retval block	(not allocated or initialized) otherwise */
static MY_ATTRIBUTE((warn_unused_result)) buf_block_t *fsp_alloc_free_page(ulint space, const page_size_t &page_size,
                                                                           ulint hint, rw_lock_type_t rw_latch,
                                                                           mtr_t *mtr, mtr_t *init_mtr)
{
  fsp_header_t *header;
  fil_addr_t first;
...
static void fsp_free_page(const page_id_t &page_id, const page_size_t &page_size, mtr_t *mtr)
{
  ...
  {
    ib::error() << "File space extent descriptor of page " << page_id << " has state " << state;
  ...
  {
    ib::error() << "File space extent descriptor of page " << page_id << " says it is free. Dump of descriptor: ";
    ut_print_buf(stderr, ((byte *)descr) - 50, 200);
    putc('\n', stderr);
    /* Crash in debug version, so that we get a core dump
    of this corruption. */
...
UNIV_INLINE
fseg_inode_t *fsp_seg_inode_page_get_nth_inode(page_t *page, ulint i, const page_size_t &page_size, mtr_t *mtr)
{
...
static ulint fsp_seg_inode_page_find_used(page_t *page, const page_size_t &page_size, mtr_t *mtr)
{
  ulint i;
  fseg_inode_t *inode;
...
@param[in,out]	mtr		mini-transaction
@return segment inode index, or ULINT_UNDEFINED if not found */
static ulint fsp_seg_inode_page_find_free(page_t *page, ulint i, const page_size_t &page_size, mtr_t *mtr)
{
    ...
    mtr_t *mtr)                 /*!< in/out: mini-transaction */
{
  ...
  if (flst_get_len(space_header + FSP_SEG_INODES_FREE) == 0 && !fsp_alloc_seg_inode_page(space_header, mtr))
  {
    return (NULL);
  }
  ...
  const page_id_t page_id(page_get_space_id(page_align(space_header)),
  ...
  if (ULINT_UNDEFINED == fsp_seg_inode_page_find_free(page, n + 1, page_size, mtr))
  {
...
@param[in,out]	mtr		mini-transaction */
static void fsp_free_seg_inode(ulint space, const page_size_t &page_size, fseg_inode_t *inode, mtr_t *mtr)
{
  page_t *page;
  ...
  if (ULINT_UNDEFINED == fsp_seg_inode_page_find_free(page, 0, page_size, mtr))
  {
  ...
  if (ULINT_UNDEFINED == fsp_seg_inode_page_find_used(page, page_size, mtr))
  {
...
static fseg_inode_t *fseg_inode_try_get(fseg_header_t *header, ulint space, const page_size_t &page_size, mtr_t *mtr,
                                        buf_block_t **block)
{
...
static fseg_inode_t *fseg_inode_get(fseg_header_t *header, ulint space, const page_size_t &page_size, mtr_t *mtr,
    ...
    mtr_t *mtr)                 /*!< in/out: mini-transaction */
{
  ...
  if (page != 0)
  {
    ...
    const ulint type = space_id == TRX_SYS_SPACE && page == TRX_SYS_PAGE_NO ? FIL_PAGE_TYPE_TRX_SYS : FIL_PAGE_TYPE_SYS;
  ...
  if (!has_done_reservation && !fsp_reserve_free_extents(&n_reserved, space_id, 2, FSP_NORMAL, mtr))
  {
    DBUG_RETURN(NULL);
  }
  ...
  if (page == 0)
  {
    block = fseg_alloc_free_page_low(space, page_size, inode, 0, FSP_UP, RW_SX_LATCH, mtr, mtr
#ifdef UNIV_DEBUG
                                     ,
    ...
    header = byte_offset + buf_block_get_frame(block);
    mlog_write_ulint(buf_block_get_frame(block) + FIL_PAGE_TYPE, FIL_PAGE_TYPE_SYS, MLOG_2BYTES, mtr);
  }
    ...
    mtr_t *mtr)          /*!< in/out: mini-transaction */
{
  ...
  *used = mach_read_from_4(inode + FSEG_NOT_FULL_N_USED) + FSP_EXTENT_SIZE * flst_get_len(inode + FSEG_FULL) +
          fseg_get_n_frag_pages(inode, mtr);

  ret = fseg_get_n_frag_pages(inode, mtr) + FSP_EXTENT_SIZE * flst_get_len(inode + FSEG_FREE) +
        FSP_EXTENT_SIZE * flst_get_len(inode + FSEG_NOT_FULL) + FSP_EXTENT_SIZE * flst_get_len(inode + FSEG_FULL);
...
extent
@param[in,out]	mtr		mini-transaction */
static void fseg_fill_free_list(fseg_inode_t *inode, ulint space, const page_size_t &page_size, ulint hint, mtr_t *mtr)
{
  xdes_t *descr;
  ulint i;
  ib_id_t seg_id;
...
(init_mtr == mtr, or the page was not previously freed in mtr)
@retval block	(not allocated or initialized) otherwise */
static xdes_t *fseg_alloc_free_extent(fseg_inode_t *inode, ulint space, const page_size_t &page_size, mtr_t *mtr)
{
  xdes_t *descr;
  ib_id_t seg_id;
  fil_addr_t first;
  ...
  else
  {
    ...
    fseg_fill_free_list(inode, space, page_size, xdes_get_offset(descr) + FSP_EXTENT_SIZE, mtr);
  }
...
)
{
  ...
  ut_ad(space->purpose == FIL_TYPE_TEMPORARY || space->purpose == FIL_TYPE_TABLESPACE);
  ...
  /*-------------------------------------------------------------*/
  if ((xdes_get_state(descr, mtr) == XDES_FSEG) && mach_read_from_8(descr + XDES_ID) == seg_id &&
      (xdes_mtr_get_bit(descr, XDES_FREE_BIT, hint % FSP_EXTENT_SIZE, mtr) == TRUE))
  {
  take_hinted_page:
  ...
  else if ((xdes_get_state(descr, mtr) == XDES_FSEG) && mach_read_from_8(descr + XDES_ID) == seg_id &&
  ...
  else if (used < FSEG_FRAG_LIMIT)
  {
    ...
    ===================================================*/
    buf_block_t *block = fsp_alloc_free_page(space_id, page_size, hint, rw_latch, mtr, init_mtr);
  ...
  if (space->size <= ret_page && !is_system_tablespace(space_id))
  {
    ...
    if (!fsp_try_extend_data_file_with_pages(space, ret_page, space_header, mtr))
    {
      /* No disk space left */
      ut_ad(!has_done_reservation);
      return (NULL);
...
got_hinted_page:
  ...
  return (fsp_page_create(page_id_t(space_id, ret_page), page_size, rw_latch, mtr, init_mtr));
}
                               ...
                               latched in mtr, do not initialize the page. */
{
  ...
  block = fseg_alloc_free_page_low(space, page_size, inode, hint, direction, RW_X_LATCH, mtr, init_mtr
#ifdef UNIV_DEBUG
                                   ,
                                   has_done_reservation
#endif /* UNIV_DEBUG */
...
to extend */
static bool fsp_reserve_free_pages(fil_space_t *space, fsp_header_t *space_header, ulint size, mtr_t *mtr,
                                   ulint n_pages)
{
  xdes_t *descr;
  ulint n_used;
  ...
  return (size >= n_used + n_pages ||
          fsp_try_extend_data_file_with_pages(space, n_used + n_pages - 1, space_header, mtr));
}
...
bool fsp_reserve_free_extents(ulint *n_reserved, ulint space_id, ulint n_ext, fsp_reserve_t alloc_type, mtr_t *mtr,
                              ulint n_pages)
{
  fsp_header_t *space_header;
...
uintmax_t fsp_get_available_space_in_free_extents(ulint space_id)
{
...
uintmax_t fsp_get_available_space_in_free_extents(const fil_space_t *space)
{
  ...
  return (static_cast<uintmax_t>(n_free - reserve) * FSP_EXTENT_SIZE * (page_size.physical() / 1024));
}
...
hash index
@param[in,out]	mtr		mini-transaction */
static void fseg_free_page_low(fseg_inode_t *seg_inode, const page_id_t &page_id, const page_size_t &page_size,
                               bool ahi, mtr_t *mtr)
{
  xdes_t *descr;
  ulint not_full_n_used;
  ...
  if (UNIV_UNLIKELY(descr_id != seg_id))
  {
    ...
    ib::error() << "InnoDB is trying to free page " << page_id << ", which does not belong to segment " << descr_id
    ...
    ulint page)                /*!< in: page offset */
{
  ...
  is_free = xdes_mtr_get_bit(descr, XDES_FREE_BIT, page % FSP_EXTENT_SIZE, &mtr);
...
static void fseg_free_extent(
    ...
    fseg_inode_t *seg_inode,                  /*!< in: segment inode */
    ulint space,                              /*!< in: space id */
    const page_size_t &page_size, ulint page, /*!< in: a page in the extent */
    bool ahi,                                 /*!< in: whether we may need to drop
                                              the adaptive hash index */
    mtr_t *mtr)                               /*!< in/out: mini-transaction */
{
  ...
  for (i = 0; i < FSP_EXTENT_SIZE; i++)
  {
    buf_page_set_file_page_was_freed(page_id_t(space, first_page_in_extent + i));
  }
#endif /* UNIV_DEBUG */
}
    ...
    mtr_t *mtr)            /*!< in/out: mini-transaction */
{
  ...
  ut_a(xdes_mtr_get_bit(descr, XDES_FREE_BIT, header_page % FSP_EXTENT_SIZE, mtr) == FALSE);
  buf_block_t *iblock;
  ...
  if (inode == NULL)
  {
    ib::info() << "Double free of inode from " << page_id_t(space_id, header_page);
    DBUG_RETURN(TRUE);
  }
  ...
  fseg_free_page_low(inode, page_id_t(space_id, fseg_get_nth_frag_page_no(inode, n, mtr)), page_size, ahi, mtr);
...
{
  const ulint space = mtr_read_ulint(m_header + FSEG_HDR_SPACE, MLOG_4BYTES, m_mtr);

  const ulint page_no = mtr_read_ulint(m_header + FSEG_HDR_PAGE_NO, MLOG_4BYTES, m_mtr);

  const ulint offset = mtr_read_ulint(m_header + FSEG_HDR_OFFSET, MLOG_2BYTES, m_mtr);

  out << "[fseg_header_t: space=" << space << ", page=" << page_no << ", offset=" << offset << "]";
