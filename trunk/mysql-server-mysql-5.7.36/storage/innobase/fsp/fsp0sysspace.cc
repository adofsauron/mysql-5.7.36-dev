...
char *SysTablespace::parse_units(char *ptr, ulint *megs)
{
  char *endp;
...
bool SysTablespace::parse_params(const char *filepath_spec, bool supports_raw)
{
  ...
  while (*str != '\0')
  {
    ...
    if (::strlen(str) >= 6 && *str == 'n' && *(str + 1) == 'e' && *(str + 2) == 'w')
    {
      ...
      m_files.back().m_type = opt_initialize ? SRV_NEW_RAW : SRV_OLD_RAW;
    }
...
dberr_t SysTablespace::check_size(Datafile &file)
{
  ...
  {
    if (file.m_size > rounded_size_pages || (m_last_file_size_max > 0 && m_last_file_size_max < rounded_size_pages))
    {
      ib::error() << "The Auto-extending " << name() << " data file '" << file.filepath()
                  << "' is"
                     " of a different size "
                  << rounded_size_pages
                  << " pages (rounded down to MB) than specified"
                     " in the .cnf file: initial "
                  << file.m_size << " pages, max " << m_last_file_size_max << " (relevant if non-zero) pages!";
      return (DB_ERROR);
    }
  ...
  {
    ib::error() << "The " << name() << " data file '" << file.filepath() << "' is of a different size "
                << rounded_size_pages
...
dberr_t SysTablespace::set_size(Datafile &file)
{
  ...
  ib::info() << "Setting file '" << file.filepath() << "' size to " << (file.m_size >> (20 - UNIV_PAGE_SIZE_SHIFT))
             << " MB."
                " Physically writing the file full; Please wait ...";

  bool success =
      os_file_set_size(file.m_filepath, file.m_handle, static_cast<os_offset_t>(file.m_size << UNIV_PAGE_SIZE_SHIFT),
                       m_ignore_read_only ? false : srv_read_only_mode);

  if (success)
  {
    ib::info() << "File '" << file.filepath() << "' size is now " << (file.m_size >> (20 - UNIV_PAGE_SIZE_SHIFT))
               << " MB.";
  }
  else
  {
    ib::error() << "Could not set the file size of '" << file.filepath() << "'. Probably out of disk space";
...
dberr_t SysTablespace::create_file(Datafile &file)
{
  ...
  switch (file.m_type)
  {
    ...
    case SRV_NOT_RAW:
      err = file.open_or_create(m_ignore_read_only ? false : srv_read_only_mode);
      break;
  }
...
dberr_t SysTablespace::read_lsn_and_check_flags(lsn_t *flushed_lsn)
{
  ...
  {
    ib::error() << "The " << name() << " data file '" << it->name() << "' has the wrong space ID. It should be "
                << space_id() << ", but " << it->m_space_id << " was found";
...
@return DB_SUCCESS or error code. */
dberr_t SysTablespace::check_file_status(const Datafile &file, file_status_t &reason)
{
  os_file_stat_t stat;

  memset(&stat, 0x0, sizeof(stat));

  dberr_t err = os_file_get_status(file.m_filepath, &stat, true, m_ignore_read_only ? false : srv_read_only_mode);
  ...
  {
    case DB_FAIL:
      ib::error() << "os_file_get_status() failed on '" << file.filepath() << "'. Can't determine file permissions";
      err = DB_ERROR;
    ...
    case DB_SUCCESS:
      ...
      if (stat.type == OS_FILE_TYPE_FILE)
      {
        if (!stat.rw_perm)
        {
          const char *p = (!srv_read_only_mode || m_ignore_read_only) ? "writable" : "readable";
...
dberr_t SysTablespace::file_not_found(Datafile &file, bool *create_new_db)
{
  ...
  if (srv_read_only_mode && !m_ignore_read_only)
  {
    ib::error() << "Can't create file '" << file.filepath() << "' when --innodb-read-only is set";
  ...
  {
    ib::info() << "Need to create a new " << name() << " data file '" << file.name() << "'.";
  }
...
bool SysTablespace::file_found(Datafile &file)
{
  ...
  {
    case SRV_NOT_RAW:
      file.set_open_flags(&file == &m_files.front() ? OS_FILE_OPEN_RETRY : OS_FILE_OPEN);
      break;
...
dberr_t SysTablespace::check_file_spec(bool *create_new_db, ulint min_expected_size)
{
  *create_new_db = FALSE;

  if (m_files.size() >= 1000)
  {
    ib::error() << "There must be < 1000 data files in " << name() << " but " << m_files.size()
                << " have been"
                   " defined.";
  ...
  {
    ib::error() << "Tablespace size must be at least " << min_expected_size / (1024 * 1024) << " MB";
  ...
  if (err == DB_SUCCESS && *create_new_db && begin->m_size < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 3)
  {
    ib::error() << "The " << name() << " data file "
                << "'" << begin->name() << "' must be at least "
                << TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 3 * UNIV_PAGE_SIZE / (1024 * 1024) << " MB";
...
@return DB_SUCCESS or error code */
dberr_t SysTablespace::open_or_create(bool is_temp, bool create_new_db, ulint *sum_new_sizes, lsn_t *flush_lsn)
{
  dberr_t err = DB_SUCCESS;
  ...
  for (files_t::iterator it = begin; it != end; ++it)
  {
    ...
    ulint max_size = (++node_counter == m_files.size() ? (m_last_file_size_max == 0 ? ULINT_MAX : m_last_file_size_max)
                                                       : it->m_size);
...
ulint SysTablespace::get_increment() const
{
  ulint increment;
  ...
  else
  {
    if (!is_valid_size())
    {
      ib::error() << "The last data file in " << name() << " has a size of " << last_file_size()
                  << " but the max size allowed is " << m_last_file_size_max;
    }
...
bool SysTablespace::has_raw_device()
{
  files_t::iterator end = m_files.end();
