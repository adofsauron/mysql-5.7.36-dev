...
bool Tablespace::intersection(const Tablespace *other_space)
{
  ...
  for (files_t::const_iterator it = other_space->m_files.begin(); it != end; ++it)
  {
...
dberr_t Tablespace::open_or_create(bool is_temp)
{
  ...
  for (files_t::iterator it = begin; it != end; ++it)
  {
    ...
    if (it == begin)
    {
      ...
      space = fil_space_create(m_name, m_space_id, flags, is_temp ? FIL_TYPE_TEMPORARY : FIL_TYPE_TABLESPACE);
    }

    ut_a(fil_validate());

    /* Create the tablespace node entry for this data file. */
    if (!fil_node_create(it->m_filepath, it->m_size, space, false, atomic_write))
    {
      err = DB_ERROR;
      break;
    }
...
void Tablespace::delete_files()
{
  ...
  for (files_t::iterator it = m_files.begin(); it != end; ++it)
  {
    ...
    bool success = os_file_delete_if_exists(innodb_data_file_key, it->m_filepath, &file_pre_exists);
...
bool Tablespace::is_undo_tablespace(ulint id) { return (srv_is_undo_tablespace(id)); }
...
dberr_t Tablespace::add_datafile(const char *datafile_added)
{
  ...
  m_files.push_back(Datafile(m_name, m_flags, FIL_IBD_FILE_INITIAL_SIZE, 0));
