...
class AIO
{
 public:
  ...
  Slot *reserve_slot(IORequest &type, fil_node_t *m1, void *m2, pfs_os_file_t file, const char *name, void *buf,
                     os_offset_t offset, ulint len) MY_ATTRIBUTE((warn_unused_result));
  ...
  ulint slots_per_segment() const MY_ATTRIBUTE((warn_unused_result)) { return (m_slots.size() / m_n_segments); }

  /** @return accessor for n_segments */
  ulint get_n_segments() const MY_ATTRIBUTE((warn_unused_result)) { return (m_n_segments); }

#ifdef UNIV_DEBUG
  /** @return true if the thread owns the mutex */
  bool is_mutex_owned() const MY_ATTRIBUTE((warn_unused_result)) { return (mutex_own(&m_mutex)); }
#endif /* UNIV_DEBUG */
  ...
  @return true on success. */
  static bool linux_create_io_ctx(ulint max_events, io_context_t *io_ctx) MY_ATTRIBUTE((warn_unused_result));
  ...
  static bool is_linux_native_aio_supported() MY_ATTRIBUTE((warn_unused_result));
#endif /* LINUX_NATIVE_AIO */
  ...
  static AIO *create(latch_id_t id, ulint n_slots, ulint segments) MY_ATTRIBUTE((warn_unused_result));
  ...
  static bool start(ulint n_per_seg, ulint n_readers, ulint n_writers, ulint n_slots_sync)
  ...
  static ulint get_array_and_local_segment(AIO **array, ulint segment) MY_ATTRIBUTE((warn_unused_result));
  ...
  static AIO *select_slot_array(IORequest &type, bool read_only, ulint mode) MY_ATTRIBUTE((warn_unused_result));
  ...
  @return segment number (which is the number used by, for example,
          I/O handler threads) */
  static ulint get_segment_no_from_slot(const AIO *array, const Slot *slot) MY_ATTRIBUTE((warn_unused_result));
  ...
  static bool is_read(const AIO *aio) MY_ATTRIBUTE((warn_unused_result)) { return (s_reads == aio); }

  /** Wait on an event until no pending writes */
  static void wait_until_no_pending_writes() { os_event_wait(AIO::s_writes->m_is_empty); }
...
@return true if we should retry the operation */
static bool os_file_handle_error_no_exit(const char *name, const char *operation, bool silent);
...
@param[in]	len		Compressed buffer length for write and size
                                of buf len for read
@return DB_SUCCESS or error code */
static dberr_t os_file_io_complete(const IORequest &type, os_file_t fh, byte *buf, byte *scratch, ulint src_len,
...
@return DB_SUCCESS or error code */
static dberr_t os_aio_windows_handler(ulint segment, ulint pos, fil_node_t **m1, void **m2, IORequest *type);
#endif /* WIN_ASYNC_IO */
...
static Block *os_alloc_block()
{
  ...
  for (;;)
  {
    ...
    if (retry == MAX_BLOCKS * 3)
    {
      ...
      ptr = static_cast<byte *>(ut_malloc_nokey(sizeof(*block) + BUFFER_BLOCK_SIZE));
...
static void os_free_block(Block *block)
{
  ...
  if (std::less<Block *>()(block, &block_cache->front()) || std::greater<Block *>()(block, &block_cache->back()))
  {
    ut_free(block);
  }
}
...
class AIOHandler
{
 public:
  ...
  static dberr_t io_complete(const Slot *slot)
  {
    ...
    return (os_file_io_complete(slot->type, slot->file.m_file, slot->buf, NULL, slot->original_len, slot->offset,
                                slot->len));
  }
 ...
 private:
  ...
  static bool is_encrypted_page(const Slot *slot) { return (Encryption::is_encrypted_page(slot->buf)); }
...
dberr_t AIOHandler::post_io_processing(Slot *slot)
{
  ...
  if (n_bytes == slot->original_len ||
      (slot->type.is_write() && slot->type.is_compressed() && slot->len == static_cast<ulint>(slot->n_bytes)))
  {
    if (!slot->type.is_log() && (is_compressed_page(slot) || is_encrypted_page(slot)))
    {
      ut_a(slot->offset > 0);
      ...
      ut_ad(err == DB_SUCCESS || err == DB_UNSUPPORTED || err == DB_CORRUPTION || err == DB_IO_DECOMPRESS_FAIL);
    }
    else
    {
      err = DB_SUCCESS;
...
static byte *os_file_compress_page(Compression compression, ulint block_size, byte *src, ulint src_len, byte *dst,
                                   ulint *dst_len)
{
  ulint len = 0;
  ...
  if (page_type == FIL_PAGE_RTREE || block_size == ULINT_UNDEFINED || compression.m_type == Compression::NONE ||
  ...
  switch (compression.m_type)
  {
    ...
    case Compression::ZLIB:
    {
      ...
      if (compress2(dst + FIL_PAGE_DATA, &zlen, src + FIL_PAGE_DATA, static_cast<uLong>(content_len),
    ...
    case Compression::LZ4:
      ...
      len = LZ4_compress_default(reinterpret_cast<char *>(src) + FIL_PAGE_DATA,
                                 reinterpret_cast<char *>(dst) + FIL_PAGE_DATA, static_cast<int>(content_len),
  ...
  ut_ad(memcmp(src + FIL_PAGE_LSN + 4, src + src_len - FIL_PAGE_END_LSN_OLD_CHKSUM + 4, 4) == 0);
                                   ...
                                   os_offset_t offset, ulint len)
{
  ...
  else if (type.is_read())
  {
    ...
    if (ret == DB_SUCCESS)
    {
      return (os_file_decompress_page(type.is_dblwr_recover(), buf, scratch, len));
    }
    else
    {
      return (ret);
...
char *os_file_make_new_pathname(const char *old_path, const char *tablename)
{
  ...
  ut_snprintf(new_path + dir_len, new_path_len - dir_len, "%c%s.ibd", OS_PATH_SEPARATOR, base_name);
...
void os_file_make_data_dir_path(char *data_dir_path)
{
...
void test_os_file_get_parent_dir(const char *child_dir, const char *expected_dir)
{
  ...
  bool unexpected = (expected == NULL ? (parent != NULL) : (0 != strcmp(parent, expected)));
  if (unexpected)
  {
    ib::fatal() << "os_file_get_parent_dir('" << child << "') returned '" << parent << "', instead of '" << expected
                << "'.";
  }
  ut_free(parent);
  ut_free(child);
...
static Block *os_file_compress_page(IORequest &type, void *&buf, ulint *n)
{
  ...
  ut_a(type.compression_algorithm().m_type != Compression::LZ4 || static_cast<ulint>(LZ4_COMPRESSBOUND(*n)) < n_alloc);
  ...
  old_compressed_len = mach_read_from_2(reinterpret_cast<byte *>(buf) + FIL_PAGE_COMPRESS_SIZE_V1);

  if (old_compressed_len > 0)
  {
    old_compressed_len = ut_calc_align(old_compressed_len + FIL_PAGE_DATA, type.block_size());
  }
  else
  {
  ...
  compressed_page = static_cast<byte *>(ut_align(block->m_ptr, os_io_ptr_align));

  byte *buf_ptr;

  buf_ptr = os_file_compress_page(type.compression_algorithm(), type.block_size(), reinterpret_cast<byte *>(buf), *n,
                                  compressed_page, &compressed_len);

  if (buf_ptr != buf)
  {
    /* Set new compressed size to uncompressed page. */
    memcpy(reinterpret_cast<byte *>(buf) + FIL_PAGE_COMPRESS_SIZE_V1, buf_ptr + FIL_PAGE_COMPRESS_SIZE_V1, 2);
...
static Block *os_file_encrypt_page(const IORequest &type, void *&buf, ulint *n)
{
  byte *encrypted_page;
  ...
  buf_ptr = encryption.encrypt(type, reinterpret_cast<byte *>(buf), *n, encrypted_page, &encrypted_len);
...
static dberr_t os_file_punch_hole_posix(os_file_t fh, os_offset_t off, os_offset_t len)
{
  ...
  ib::warn() << "fallocate(" << fh << ", FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, " << off << ", " << len
...
class LinuxAIOHandler
{
 ...
 private:
  ...
  bool is_shutdown() const { return (srv_shutdown_state == SRV_SHUTDOWN_EXIT_THREADS && !buf_page_cleaner_is_active); }
...
dberr_t LinuxAIOHandler::check_state(Slot *slot)
{
  ...
  srv_set_io_thread_op_info(m_global_segment, "processing completed aio requests");
...
void LinuxAIOHandler::collect()
{
  ...
  for (;;)
  {
    ...
    for (int i = 0; i < ret; ++i)
    {
      ...
      if (slot->offset > 0 && !slot->skip_punch_hole && slot->type.is_compression_enabled() && !slot->type.is_log() &&
          slot->type.is_write() && slot->type.is_compressed() && slot->type.punch_hole())
      {
    ...
    if (srv_shutdown_state == SRV_SHUTDOWN_EXIT_THREADS || !buf_page_cleaner_is_active || ret > 0)
    {
      break;
    }
...
dberr_t LinuxAIOHandler::poll(fil_node_t **m1, void **m2, IORequest *request)
{
  ...
  for (;;)
  {
    ...
    else
    {
      ...
      srv_set_io_thread_op_info(m_global_segment, "waiting for completed aio requests");
...
static dberr_t os_aio_linux_handler(ulint global_segment, fil_node_t **m1, void **m2, IORequest *request)
{
      ...
      case -EAGAIN:
        if (n_retries == 0)
        {
          ...
          ib::warn() << "io_setup() failed with EAGAIN."
                        " Will make "
                     << OS_AIO_IO_SETUP_RETRY_ATTEMPTS << " attempts before giving up.";
        }
        ...
        ib::error() << "io_setup() failed with EAGAIN after " << OS_AIO_IO_SETUP_RETRY_ATTEMPTS << " attempts.";
        break;
...
bool AIO::is_linux_native_aio_supported()
{
  ...
  switch (err)
  {
    ...
    case -ENOSYS:
      ...
      /* fall through. */
    default:
      ib::error() << "Linux Native AIO check on " << (srv_read_only_mode ? name : "tmpdir") << "returned error[" << -err
                  << "]";
  }
...
static ulint os_file_get_last_error_low(bool report_all_errors, bool on_error_silent)
{
  ...
  if (report_all_errors || (err != ENOSPC && err != EEXIST && !on_error_silent))
  {
...
pfs_os_file_t os_file_create_simple_func(const char *name, ulint create_mode, ulint access_type, bool read_only,
                                         bool *success)
{
  pfs_os_file_t file;
  ...
  *success = false;
  ...
  {
    ib::error() << "Unknown file create mode (" << create_mode << " for file '" << name << "'";
...
#ifdef USE_FILE_LOCK
  if (!read_only && *success && access_type == OS_FILE_READ_WRITE && os_file_lock(file.m_file, name))
  {
    *success = false;
    close(file.m_file);
    file.m_file = -1;
...
next_file:
  ...
  if (ret != 0)
  {
    ib::error() << "Cannot read directory " << dirname << " error: " << ret;
  ...
  full_path = static_cast<char *>(ut_malloc_nokey(strlen(dirname) + strlen(ent->d_name) + 10));
...
@return handle to the file, not defined if error, error number
        can be retrieved with os_file_get_last_error */
pfs_os_file_t os_file_create_func(const char *name, ulint create_mode, ulint purpose, ulint type, bool read_only,
                                  bool *success)
{
  ...
  *success = false;
  ...
  DBUG_EXECUTE_IF("ib_create_table_fail_disk_full", *success = false; errno = ENOSPC; file.m_file = OS_FILE_CLOSED;
                  return (file););
  ...
  ut_a(type == OS_LOG_FILE || type == OS_DATA_FILE || type == OS_DATA_TEMP_FILE);
  ...
  if (!read_only && type == OS_LOG_FILE && srv_unix_file_flush_method == SRV_UNIX_O_DSYNC)
  {
    create_flag |= O_SYNC;
  }
#endif /* O_SYNC */
  ...
  do
  {
    ...
    if (file.m_file == -1)
    {
      ...
      operation = (create_mode == OS_FILE_CREATE && !read_only) ? "create" : "open";
  ...
  if (!read_only && *success && (type != OS_LOG_FILE && type != OS_DATA_TEMP_FILE) &&
...
#ifdef USE_FILE_LOCK
  if (!read_only && *success && create_mode != OS_FILE_OPEN_RAW && os_file_lock(file.m_file, name))
  {
...
@return own: handle to the file, not defined if error, error number
        can be retrieved with os_file_get_last_error */
pfs_os_file_t os_file_create_simple_no_error_handling_func(const char *name, ulint create_mode, ulint access_type,
                                                           bool read_only, bool *success)
{
  ...
  *success = false;
  ...
  if (create_mode == OS_FILE_OPEN)
  {
    ...
    {
      ut_a(access_type == OS_FILE_READ_WRITE || access_type == OS_FILE_READ_ALLOW_DELETE);
...
static dberr_t os_file_get_status_posix(const char *path, os_file_stat_t *stat_info, struct stat *statinfo,
...
@return true if success */
bool os_file_set_eof(FILE *file) /*!< in: file to be truncated */ { return (!ftruncate(fileno(file), ftell(file))); }

#ifdef UNIV_HOTBACKUP
/** Closes a file handle.
@param[in]	file		Handle to a file
@return true if success */
bool os_file_close_no_error_handling(os_file_t file) { return (close(file) != -1); }
#endif /* UNIV_HOTBACKUP */
...
ssize_t SyncFileIO::execute(Slot *slot)
{
  ...
  if (slot->type.is_read())
  {
    ret = ReadFile(slot->file.m_file, slot->ptr, slot->len, &slot->n_bytes, &slot->control);
  }
  else
  {
    ut_ad(slot->type.is_write());
    ret = WriteFile(slot->file.m_file, slot->ptr, slot->len, &slot->n_bytes, &slot->control);
  }
...
static bool os_is_sparse_file_supported_win32(const char *filename)
{
  ...
  if (!result)
  {
    ib::error() << "os_is_sparse_file_supported: "
                << "Failed to get the volume path name for: " << filename << "- OS error number " << GetLastError();
  ...
  GetVolumeInformation(volname, NULL, MAX_PATH, NULL, NULL, &flags, NULL, MAX_PATH);
...
static dberr_t os_file_punch_hole_win32(os_file_t fh, os_offset_t off, os_offset_t len)
{
  ...
  BOOL result = DeviceIoControl(fh, FSCTL_SET_ZERO_DATA, &punch, sizeof(punch), NULL, 0, &temp, NULL);
...
@param[out]	type		Type of the file, if it exists
@return true if call succeeded */
bool os_file_status_win32(const char *path, bool *exists, os_file_type_t *type)
{
  int ret;
...
static ulint os_file_get_last_error_low(bool report_all_errors, bool on_error_silent)
{
  ...
  if (report_all_errors || (!on_error_silent && err != ERROR_DISK_FULL && err != ERROR_FILE_EXISTS))
  {
    ib::error() << "Operating system error number " << err << " in a file operation.";
    ...
    else if (err == ERROR_WORKING_SET_QUOTA || err == ERROR_NO_SYSTEM_RESOURCES)
    {
                                         ...
                                         bool *success)
{
  ...
  *success = false;
  ...
  do
  {
    ...
    file.m_file = CreateFile((LPCTSTR)name, access, FILE_SHARE_READ, NULL, create_flag, attributes, NULL);
    ...
    else
    {
      ...
      *success = true;
      ...
      DeviceIoControl(file.m_file, FSCTL_SET_SPARSE, NULL, 0, NULL, 0, &temp, NULL);
    }
...
os_file_dir_t os_file_opendir(const char *dirname, bool error_is_fatal)
{
  ...
  lpFindFileData = static_cast<LPWIN32_FIND_DATA>(ut_malloc_nokey(sizeof(WIN32_FIND_DATA)));
                                  ...
                                  bool *success)
{
  ...
  *success = false;
  ...
  DBUG_EXECUTE_IF("ib_create_table_fail_disk_full", *success = false; SetLastError(ERROR_DISK_FULL);
                  file.m_file = OS_FILE_CLOSED; return (file););
  ...
  {
    /* Use default security attributes and no template file. */
    file.m_file = CreateFile((LPCTSTR)name, access, share_mode, NULL, create_flag, attributes, NULL);
...
bool os_file_delete_func(const char *name)
{
  ...
  for (;;)
  {
    ...
    if (count > 100 && 0 == (count % 10))
    {
      ...
      ib::warn() << "Cannot delete file '" << name << "'. Are "
...
@return DB_SUCCESS if all OK */
static dberr_t os_file_get_status_win32(const char *path, os_file_stat_t *stat_info, struct _stat64 *statinfo,
                                        bool check_rw_perm, bool read_only)
{
  ...
  else if (_S_IFREG & statinfo->st_mode)
  {
    ...
    if (!result)
    {
      ib::error() << "os_file_get_status_win32: "
                  << "Failed to get the volume path name for: " << path << "- OS error number " << GetLastError();
    ...
    result = GetDiskFreeSpace((LPCSTR)volname, &sectorsPerCluster, &bytesPerSector, &numberOfFreeClusters,
    ...
    stat_info->block_size = (stat_info->block_size <= 4096) ? stat_info->block_size * 16 : ULINT_UNDEFINED;
  }
  else
  {
    stat_info->type = OS_FILE_TYPE_UNKNOWN;
...
@param[in]	size		size to preserve in bytes
@return true if success */
static bool os_file_truncate_win32(const char *pathname, pfs_os_file_t file, os_offset_t size)
{
...
@param[in]	file		Handle to close
@return true if success */
bool os_file_close_no_error_handling(os_file_t file) { return (CloseHandle(file) ? true : false); }
#endif /* UNIV_HOTBACKUP */
...
call os_aio_simulated_wake_handler_threads later to ensure the threads
are not left sleeping! */
void os_aio_simulated_put_read_threads_to_sleep() { AIO::simulated_put_read_threads_to_sleep(); }
...
static MY_ATTRIBUTE((warn_unused_result)) ssize_t
    os_file_io(const IORequest &in_type, os_file_t file, void *buf, ulint n, os_offset_t offset, dberr_t *err)
{
  Block *block;
  ulint original_n = n;
  ...
  for (ulint i = 0; i < NUM_RETRIES_ON_PARTIAL_IO; ++i)
  {
    ...
    else if ((ulint)n_bytes + bytes_returned == n)
    {
      ...
      {
        *err = os_file_io_complete(type, file, reinterpret_cast<byte *>(buf), NULL, original_n, offset, n);
      }
      else
      {
        *err = DB_SUCCESS;
    ...
    if (!type.is_partial_io_warning_disabled())
    {
      ...
      ib::warn() << n << " bytes should have been " << op << ". Only " << bytes_returned << " bytes " << op
                 << ". Retrying"
  ...
  *err = DB_IO_ERROR;
  ...
  if (!type.is_partial_io_warning_disabled())
  {
    ib::warn() << "Retry attempts for " << (type.is_read() ? "reading" : "writing") << " partial data failed.";
  }
...
static MY_ATTRIBUTE((warn_unused_result)) ssize_t
    os_file_pwrite(IORequest &type, os_file_t file, const byte *buf, ulint n, os_offset_t offset, dberr_t *err)
{
  ut_ad(type.validate());
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    os_file_write_page(IORequest &type, const char *name, os_file_t file, const byte *buf, os_offset_t offset, ulint n)
{
  dberr_t err;
  ut_ad(type.validate());
  ut_ad(n > 0);
  ...
  {
    ib::error() << "Write to file " << name << "failed at offset " << offset << ", " << n
...
@return DB_SUCCESS or error code */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    os_file_read_page(IORequest &type, os_file_t file, void *buf, os_offset_t offset, ulint n, ulint *o,
                      bool exit_on_err)
{
  dberr_t err;
  ...
  for (;;)
  {
    ...
    ib::error() << "Tried to read " << n << " bytes at offset " << offset << ", but was only able to read " << n_bytes;
...
@param[in]	report_all_errors	true if we want an error printed
                                        for all errors
@return error number, or OS error number + 100 */
ulint os_file_get_last_error(bool report_all_errors) { return (os_file_get_last_error_low(report_all_errors, false)); }
...
@return true if we should retry the operation */
static MY_ATTRIBUTE((warn_unused_result)) bool os_file_handle_error_cond_exit(const char *name, const char *operation,
                                                                              bool should_exit, bool on_error_silent)
{
  ulint err;
  ...
  switch (err)
  {
    ...
    default:
      ...
      {
        ib::error() << "File " << (name != NULL ? name : "(unknown)") << ": '" << operation
                    << "'"
                       " returned OS error "
...
@param[in]	name		"open" or "create"; used in the diagnostic
                                message */
void os_file_set_nocache(int fd MY_ATTRIBUTE((unused)), const char *file_name MY_ATTRIBUTE((unused)),
                         const char *operation_name MY_ATTRIBUTE((unused)))
{
  ...
  if (directio(fd, DIRECTIO_ON) == -1)
  {
    ...
    ib::error() << "Failed to set DIRECTIO_ON on file " << file_name << ": " << operation_name << strerror(errno_save)
                << ","
                   " continuing anyway.";
  }
...
#ifdef UNIV_LINUX
        ib::warn() << "Failed to set O_DIRECT on file" << file_name << ";" << operation_name << ": "
                   << strerror(errno_save) << ", "
...
@return true if success */
bool os_file_set_size(const char *name, pfs_os_file_t file, os_offset_t size, bool read_only)
{
  /* Write up to 1 megabyte at a time. */
  ulint buf_size = ut_min(static_cast<ulint>(64), static_cast<ulint>(size / UNIV_PAGE_SIZE));
  ...
  while (current_size < size)
  {
    ...
    err = os_aio(request, OS_AIO_SYNC, name, file, buf, current_size, n_bytes, read_only, NULL, NULL);
#endif /* UNIV_HOTBACKUP */
    ...
    {
      fprintf(stderr, " %lu00", (ulong)((current_size + n_bytes) / (100 << 20)));
    }
...
@param[in]	n		number of bytes to read, starting from offset
@return DB_SUCCESS or error code */
dberr_t os_file_read_func(IORequest &type, os_file_t file, void *buf, os_offset_t offset, ulint n)
{
  ut_ad(type.is_read());
...
@param[out]	o		number of bytes actually read
@return DB_SUCCESS or error code */
dberr_t os_file_read_no_error_handling_func(IORequest &type, os_file_t file, void *buf, os_offset_t offset, ulint n,
                                            ulint *o)
{
  ut_ad(type.is_read());
...
@param[in]	n		number of bytes to read, starting from offset
@return DB_SUCCESS if request was successful, false if fail */
dberr_t os_file_write_func(IORequest &type, const char *name, os_file_t file, const void *buf, os_offset_t offset,
                           ulint n)
{
  ut_ad(type.validate());
  ut_ad(type.is_write());
...
dberr_t AIO::init_linux_native_aio()
{
  ...
  m_aio_ctx = static_cast<io_context **>(ut_zalloc_nokey(m_n_segments * sizeof(*m_aio_ctx)));
...
bool AIO::start(ulint n_per_seg, ulint n_readers, ulint n_writers, ulint n_slots_sync)
{
#if defined(LINUX_NATIVE_AIO)
  ...
  s_writes = create(LATCH_ID_OS_AIO_WRITE_MUTEX, n_writers * n_per_seg, n_writers);
  ...
  os_aio_segment_wait_events =
      static_cast<os_event_t *>(ut_zalloc_nokey(n_segments * sizeof *os_aio_segment_wait_events));
...
void os_fusionio_get_sector_size()
{
  if (srv_unix_file_flush_method == SRV_UNIX_O_DIRECT || srv_unix_file_flush_method == SRV_UNIX_O_DIRECT_NO_FSYNC)
  {
    ...
    if (check_file == -1)
    {
      ib::error() << "Failed to create check sector file, errno:" << errno << " Please confirm O_DIRECT is"
                  << " supported and remove the file " << check_file_name << " if it exists.";
...
bool os_aio_init(ulint n_readers, ulint n_writers, ulint n_slots_sync)
{
  ...
  for (Blocks::iterator it = block_cache->begin(); it != block_cache->end(); ++it)
  {
    ut_a(it->m_in_use == 0);
    ut_a(it->m_ptr == NULL);
...
void os_aio_wait_until_no_pending_writes() { AIO::wait_until_no_pending_writes(); }
...
ulint AIO::get_segment_no_from_slot(const AIO *array, const Slot *slot)
{
  ...
  else
  {
    ...
    segment = s_reads->m_n_segments + (srv_read_only_mode ? 0 : 2) + slot->pos / seg_len;
  }
...
@param[in]	len		length of the block to read or write
@return pointer to slot */
Slot *AIO::reserve_slot(IORequest &type, fil_node_t *m1, void *m2, pfs_os_file_t file, const char *name, void *buf,
                        os_offset_t offset, ulint len)
{
#ifdef WIN_ASYNC_IO
  ...
  for (ulint i = local_seg * slots_per_seg; counter < m_slots.size(); ++i, ++counter)
  {
    i %= m_slots.size();
  ...
  if (srv_use_native_aio && offset > 0 && type.is_write() && type.is_compressed())
  {
  ...
  before compression, the encrypted data will cause compression fail
  or low compression rate. */
  if (srv_use_native_aio && offset > 0 && type.is_write() && type.is_encrypted())
  {
...
@param[in]	mode		IO mode
@return slot array or NULL if invalid mode specified */
AIO *AIO::select_slot_array(IORequest &type, bool read_only, ulint mode)
{
  AIO *array;
...
static dberr_t os_aio_windows_handler(ulint segment, ulint pos, fil_node_t **m1, void **m2, IORequest *type)
{
  ...
  else
  {
    ...
    pos = WaitForMultipleObjects((DWORD)array->slots_per_segment(), array->handles(segment), FALSE, INFINITE);
  }

  array->acquire();

  if (srv_shutdown_state == SRV_SHUTDOWN_EXIT_THREADS && array->is_empty() && !buf_page_cleaner_is_active)
  {
    *m1 = NULL;
    *m2 = NULL;
  ...
  BOOL ret;
  ret = GetOverlappedResult(slot->file.m_file, &slot->control, &slot->n_bytes, TRUE);
  *m1 = slot->m1;
  *m2 = slot->m2;
  ...
  *type = slot->type;
  ...
  if (retry)
  {
    ...
    register_pfs_file_io_begin(&state, locker, slot->file, slot->len,
                               slot->type.is_write() ? PSI_FILE_WRITE : PSI_FILE_READ, __FILE__, __LINE__);
#endif /* UNIV_PFS_IO */
...
@param[in,out]	m2		message for the AIO handler (can be used to
                                ...
                                if mode is OS_AIO_SYNC
@return DB_SUCCESS or error code */
dberr_t os_aio_func(IORequest &type, ulint mode, const char *name, pfs_os_file_t file, void *buf, os_offset_t offset,
...
try_again:
  ...
  if (type.is_read())
  {
    if (srv_use_native_aio)
    {
      ...
      os_bytes_read_since_printout += n;
#ifdef WIN_ASYNC_IO
      ret = ReadFile(file.m_file, slot->ptr, slot->len, &slot->n_bytes, &slot->control);
#elif defined(LINUX_NATIVE_AIO)
...
#ifdef WIN_ASYNC_IO
      ret = WriteFile(file.m_file, slot->ptr, slot->len, &slot->n_bytes, &slot->control);
#elif defined(LINUX_NATIVE_AIO)
      if (!array->linux_dispatch(slot))
      {
        goto err_exit;
  ...
  if (srv_use_native_aio)
  {
    if ((ret && slot->len == slot->n_bytes) || (!ret && GetLastError() == ERROR_IO_PENDING))
    {
      /* aio was queued successfully! */
      ...
      if (mode == OS_AIO_SYNC)
      {
        ...
        return (os_aio_windows_handler(ULINT_UNDEFINED, slot->pos, &dummy_mess1, &dummy_mess2, &dummy_type));
      }
...
class SimulatedAIOHandler
{
 public:
  ...
  ulint check_pending(ulint global_segment, os_event_t event) MY_ATTRIBUTE((warn_unused_result));
 ...
 private:
  ...
  void read(Slot *slot)
  {
    dberr_t err = os_file_read_func(slot->type, slot->file.m_file, slot->ptr, slot->offset, slot->len);
    ut_a(err == DB_SUCCESS);
  }
  ...
  {
    dberr_t err = os_file_write_func(slot->type, slot->name, slot->file.m_file, slot->ptr, slot->offset, slot->len);
  ...
  {
    return (s1 != s2 && s1->file.m_file == s2->file.m_file && s2->offset == s1->offset + s1->len &&
            s1->type == s2->type);
  }
  ...
  {
    int64_t time_diff = ut_time_monotonic() - slot->reservation_time;

    const uint64_t age = time_diff > 0 ? (uint64_t)time_diff : 0;

    if ((age >= 2 && age > m_oldest) || (age >= 2 && age == m_oldest && slot->offset < m_lowest_offset))
    {
      /* Found an i/o request */
      m_slots[0] = slot;
...
/** Wait for I/O requests
@return the number of slots */
ulint SimulatedAIOHandler::check_pending(ulint global_segment, os_event_t event)
{
...
static dberr_t os_aio_simulated_handler(ulint global_segment, fil_node_t **m1, void **m2, IORequest *type)
{
  ...
  for (;;)
  {
    ...
    else if (n_reserved == 0 && !buf_page_cleaner_is_active && srv_shutdown_state == SRV_SHUTDOWN_EXIT_THREADS)
    {
...
void os_aio_print(FILE *file)
{
  ...
  fprintf(file,
          ...
          fil_n_pending_log_flushes, fil_n_pending_tablespace_flushes, os_n_file_reads, os_n_file_writes, os_n_fsyncs);
...
/** Allocate a page for sync IO
@return pointer to page */
static Block *os_alloc_block() { return (reinterpret_cast<byte *>(malloc(UNIV_PAGE_SIZE_MAX * 2))); }

/** Free a page after sync IO
@param[in,own]	block		The block to free/release */
static void os_free_block(Block *block) { ut_free(block); }
...
{
  return (mach_read_from_2(page + FIL_PAGE_TYPE) == FIL_PAGE_COMPRESSED_AND_ENCRYPTED);
}
...
void Compression::deserialize_header(const byte *page, Compression::meta_t *control)
{
  ...
  control->m_algorithm = static_cast<Type>(mach_read_from_1(page + FIL_PAGE_ALGORITHM_V1));
}
...
@return DB_SUCCESS or error code */
dberr_t Compression::deserialize(bool dblwr_recover, byte *src, byte *dst, ulint dst_len)
{
  ...
  if (!is_valid_page_version(header.m_version) || header.m_original_size < UNIV_PAGE_SIZE_MIN - (FIL_PAGE_DATA + 8) ||
      header.m_original_size > UNIV_PAGE_SIZE_MAX - FIL_PAGE_DATA || dst_len < header.m_original_size + FIL_PAGE_DATA)
  {
  ...
  switch (compression.m_type)
  {
    ...
    case Compression::LZ4:
      ...
      ret = LZ4_decompress_safe(reinterpret_cast<char *>(ptr), reinterpret_cast<char *>(dst), header.m_compressed_size,
    ...
    default:
      ...
      ib::error() << "Compression algorithm support missing: " << Compression::to_string(compression.m_type);
#else
  ...
  ut_ad(dblwr_recover ||
        memcmp(src + FIL_PAGE_LSN + 4, src + (header.m_original_size + FIL_PAGE_DATA) - FIL_PAGE_END_LSN_OLD_CHKSUM + 4,
               4) == 0);
...
void Encryption::create_master_key(byte **master_key)
{
  ...
  ut_snprintf(key_name, ENCRYPTION_MASTER_KEY_NAME_MAX_LEN, "%s-%s-%lu", ENCRYPTION_MASTER_KEY_PRIFIX, uuid,
              master_key_id + 1);
  ...
  ret = my_key_fetch(key_name, &key_type, NULL, reinterpret_cast<void **>(master_key), &key_len);
...
@param[in,out]	master_key	master key */
void Encryption::get_master_key(ulint master_key_id, char *srv_uuid, byte **master_key)
{
#ifndef UNIV_INNOCHECKSUM
  char *key_type = NULL;
  size_t key_len;
  ...
  if (srv_uuid != NULL)
  {
    ut_snprintf(key_name, ENCRYPTION_MASTER_KEY_NAME_MAX_LEN, "%s-%s-%lu", ENCRYPTION_MASTER_KEY_PRIFIX, srv_uuid,
                master_key_id);
  }
  else
  {
    ...
    server id. */
    memset(key_name, 0, ENCRYPTION_MASTER_KEY_NAME_MAX_LEN);
    ut_snprintf(key_name, ENCRYPTION_MASTER_KEY_NAME_MAX_LEN, "%s-%lu-%lu", ENCRYPTION_MASTER_KEY_PRIFIX, server_id,
                master_key_id);
  }
...
void Encryption::get_master_key(ulint *master_key_id, byte **master_key, Encryption::Version *version)
{
#ifndef UNIV_INNOCHECKSUM
  char *key_type = NULL;
  size_t key_len;
  ...
  *version = Encryption::ENCRYPTION_VERSION_2;
  ...
  if (Encryption::master_key_id == 0)
  {
    ...
    ut_snprintf(key_name, ENCRYPTION_MASTER_KEY_NAME_MAX_LEN, "%s-%s-1", ENCRYPTION_MASTER_KEY_PRIFIX, uuid);
...
bool Encryption::is_encrypted_page(const byte *page)
{
  ...
  return (page_type == FIL_PAGE_ENCRYPTED || page_type == FIL_PAGE_COMPRESSED_AND_ENCRYPTED ||
...
@return buffer data, dst_len will have the length of the data */
byte *Encryption::encrypt(const IORequest &type, byte *src, ulint src_len, byte *dst, ulint *dst_len)
{
  ut_ad(m_type != NONE);
  ut_ad(!type.is_log());
#ifdef UNIV_ENCRYPT_DEBUG
  ...
  {
    src_enc_len = mach_read_from_2(src + FIL_PAGE_COMPRESS_SIZE_V1) + FIL_PAGE_DATA;
  ...
  switch (m_type)
  {
    ...
    case Encryption::AES:
    {
      ...
      lint elen = my_aes_encrypt(src + FIL_PAGE_DATA, static_cast<uint32>(chunk_len), dst + FIL_PAGE_DATA,
                                 reinterpret_cast<byte *>(m_key), static_cast<uint32>(m_klen), my_aes_256_cbc,
      ...
      if (elen == MY_AES_BAD_DATA)
      {
        ...
        *dst_len = src_len;
#ifndef UNIV_INNOCHECKSUM
        ib::warn() << " Can't encrypt data of page,"
                   << " page no:" << page_no << " space id:" << space_id;
#else
        fprintf(stderr,
                " Can't encrypt data of page,"
                " page no:" ULINTPF " space id:" ULINTPF,
                page_no, space_id);
#endif /* !UNIV_INNOCHECKSUM */
        return (src);
      }
      ...
      if (remain_len != 0)
      {
        ...
        memcpy(dst + FIL_PAGE_DATA + len, src + FIL_PAGE_DATA + len, remain_len);

        const ulint trailer_len = MY_AES_BLOCK_SIZE * 2;
        byte buf[trailer_len];

        elen = my_aes_encrypt(dst + FIL_PAGE_DATA + data_len - trailer_len, static_cast<uint32>(trailer_len), buf,
                              reinterpret_cast<unsigned char *>(m_key), static_cast<uint32>(m_klen), my_aes_256_cbc,
                              reinterpret_cast<byte *>(m_iv), false);
  ...
  if (page_type == FIL_PAGE_COMPRESSED)
  {
    ...
    ut_ad(memcmp(src + FIL_PAGE_TYPE + 2, dst + FIL_PAGE_TYPE + 2, FIL_PAGE_DATA - FIL_PAGE_TYPE - 2) == 0);
  }
...
@param[in]	dst_len		Size of the scratch area in bytes
@return DB_SUCCESS or error code */
dberr_t Encryption::decrypt(const IORequest &type, byte *src, ulint src_len, byte *dst, ulint dst_len)
{
  ulint data_len;
  ulint main_len;
  ulint remain_len;
  ...
  {
    src_len = static_cast<uint16_t>(mach_read_from_2(src + FIL_PAGE_COMPRESS_SIZE_V1)) + FIL_PAGE_DATA;
#ifndef UNIV_INNOCHECKSUM
  ...
  original_type = static_cast<uint16_t>(mach_read_from_2(src + FIL_PAGE_ORIGINAL_TYPE_V1));
  ...
  switch (m_type)
  {
    case Encryption::AES:
    {
      ...
      if (remain_len != 0)
      {
        ...
        elen = my_aes_decrypt(remain_buf, static_cast<uint32>(remain_len), dst + data_len - remain_len,
                              reinterpret_cast<unsigned char *>(m_key), static_cast<uint32>(m_klen), my_aes_256_cbc,
      ...
      elen =
          my_aes_decrypt(dst, static_cast<uint32>(main_len), ptr, reinterpret_cast<unsigned char *>(m_key),
                         static_cast<uint32>(m_klen), my_aes_256_cbc, reinterpret_cast<unsigned char *>(m_iv), false);
      if (elen == MY_AES_BAD_DATA)
      {
        if (block != NULL)
        {
    ...
    default:
      ...
      {
#if !defined(UNIV_INNOCHECKSUM)
        ib::error() << "Encryption algorithm support missing: " << Encryption::to_string(m_type);
#else
