...
)
{
  ...
  switch (ret)
  {
    ...
    default:
      ib::error() << "pthread_cond_timedwait() returned: " << ret << ": abstime={" << abstime->tv_sec << ","
                  << abstime->tv_nsec << "}";
      ut_error;
  }
  ...
  for (int i = 0;; i++)
  {
    ...
    if (cond_attr_has_monotonic_clock)
    {
      ...
      if (clock_gettime(CLOCK_MONOTONIC, &tp) == -1)
      {
        ...
        ib::error() << "clock_gettime() failed: " << strerror(errno_clock_gettime);
      ...
      {
        const uint64_t increased = tp.tv_nsec + time_in_usec * 1000;
...
#ifndef UNIV_NO_ERR_MSGS
        ib::error() << "clock_gettime() failed: " << strerror(errno_gettimeofday);
#endif /* !UNIV_NO_ERR_MSGS */
...
@param reset_sig_count - zero or the value returned by previous call
        of os_event_reset().
@return	0 if success, OS_SYNC_TIME_EXCEEDED if timeout was exceeded */
ulint os_event::wait_time_low(ulint time_in_usec, int64_t reset_sig_count) UNIV_NOTHROW
{
