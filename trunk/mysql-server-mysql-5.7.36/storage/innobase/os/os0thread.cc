...
#ifdef _WIN32
typedef std::map<DWORD, HANDLE, std::less<DWORD>, ut_allocator<std::pair<const DWORD, HANDLE>>> WinThreadMap;
                               ...
                               thread, or NULL */
{
  ...
  handle = CreateThread(NULL,         /* no security attributes */
                        ...
                        func, arg, 0, /* thread runs immediately */
                        &new_thread_id);
  ...
  ret = win_thread_map.insert(std::pair<DWORD, HANDLE>(new_thread_id, handle));
...
void os_thread_join(os_thread_id_t thread)
{
#ifdef _WIN32
  /* Do nothing. */
#else
...
void os_thread_exit(bool detach)
{
