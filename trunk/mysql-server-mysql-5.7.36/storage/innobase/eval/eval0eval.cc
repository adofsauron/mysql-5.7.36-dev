    ...
    que_node_t *arg2) /* !< in: right operand */
{
  ...
  op = static_cast<ib_like_t>(mach_read_from_4(static_cast<const byte *>(dfield_get_data(dfield))));

  switch (op)
  {
    case IB_LIKE_PREFIX:
      arg4 = que_node_get_next(arg3);
      return (!cmp_dfield_dfield_like_prefix(que_node_get_val(arg1), que_node_get_val(arg4)));
    case IB_LIKE_EXACT:
    ...
    func_node_t *func_node) /*!< in: function node */
{
  ...
  while (arg)
  {
    ...
    if (dfield_is_null(que_node_get_val(arg)) && (fclass != PARS_FUNC_CMP) && (func != PARS_NOTFOUND_TOKEN) &&
