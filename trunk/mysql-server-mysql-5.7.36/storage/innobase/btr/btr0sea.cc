...
inline MY_ATTRIBUTE((warn_unused_result)) ulint btr_search_get_n_fields(ulint n_fields, ulint n_bytes)
{
...
@return	number of complete or incomplete fields */
inline MY_ATTRIBUTE((warn_unused_result)) ulint btr_search_get_n_fields(const btr_cur_t *cursor)
{
...
void btr_search_sys_create(ulint hash_size)
{
  ...
  btr_search_latches = reinterpret_cast<rw_lock_t **>(ut_malloc(sizeof(rw_lock_t *) * btr_ahi_parts, mem_key_ahi));

  for (ulint i = 0; i < btr_ahi_parts; ++i)
  {
    btr_search_latches[i] = reinterpret_cast<rw_lock_t *>(ut_malloc(sizeof(rw_lock_t), mem_key_ahi));

    rw_lock_create(btr_search_latch_key, btr_search_latches[i], SYNC_SEARCH_SYS);
  }

  /* Step-2: Allocate hash tablees. */
  btr_search_sys = reinterpret_cast<btr_search_sys_t *>(ut_malloc(sizeof(btr_search_sys_t), mem_key_ahi));

  btr_search_sys->hash_tables =
      reinterpret_cast<hash_table_t **>(ut_malloc(sizeof(hash_table_t *) * btr_ahi_parts, mem_key_ahi));

  for (ulint i = 0; i < btr_ahi_parts; ++i)
  {
    btr_search_sys->hash_tables[i] =
        ib_create((hash_size / btr_ahi_parts), LATCH_ID_HASH_TABLE_MUTEX, 0, MEM_HEAP_FOR_BTR_SEARCH);
...
ulint btr_search_info_get_ref_count(btr_search_t *info, dict_index_t *index)
{
  ulint ret = 0;
...
static ibool btr_search_update_block_hash_info(btr_search_t *info, buf_block_t *block, const btr_cur_t *cursor)
{
  ut_ad(!rw_lock_own(btr_get_search_latch(cursor->index), RW_LOCK_S));
  ut_ad(!rw_lock_own(btr_get_search_latch(cursor->index), RW_LOCK_X));
  ut_ad(rw_lock_own(&block->lock, RW_LOCK_S) || rw_lock_own(&block->lock, RW_LOCK_X));
  ...
  if ((block->n_hash_helps > 0) && (info->n_hash_potential > 0) && (block->n_fields == info->n_fields) &&
      (block->n_bytes == info->n_bytes) && (block->left_side == info->left_side))
  {
    if ((block->index) && (block->curr_n_fields == info->n_fields) && (block->curr_n_bytes == info->n_bytes) &&
  ...
  {
    if ((!block->index) || (block->n_hash_helps > 2 * page_get_n_recs(block->frame)) ||
...
@param[in]	cursor	cursor */
static void btr_search_update_hash_ref(const btr_search_t *info, buf_block_t *block, const btr_cur_t *cursor)
{
  dict_index_t *index;
  ulint fold;
  const rec_t *rec;
  ...
  if ((info->n_hash_potential > 0) && (block->curr_n_fields == info->n_fields) &&
      (block->curr_n_bytes == info->n_bytes) && (block->curr_left_side == info->left_side))
  {
    ...
    fold = rec_fold(rec, rec_get_offsets(rec, index, offsets_, ULINT_UNDEFINED, &heap), block->curr_n_fields,
...
void btr_search_info_update_slow(btr_search_t *info, btr_cur_t *cursor)
{
  ...
  if (build_index)
  {
    ...
    btr_search_build_page_hash_index(cursor->index, block, block->n_fields, block->n_bytes, block->left_side);
  }
}
...
static ibool btr_search_check_guess(btr_cur_t *cursor, ibool can_only_compare_to_cursor_rec, const dtuple_t *tuple,
                                    ulint mode, mtr_t *mtr)
{
  ...
  if ((mode == PAGE_CUR_G) || (mode == PAGE_CUR_GE))
  {
    ...
    cmp = cmp_dtuple_rec_with_match(tuple, prev_rec, offsets, &match);
    if (mode == PAGE_CUR_GE)
    {
      success = cmp > 0;
    }
  ...
  else
  {
    ...
    cmp = cmp_dtuple_rec_with_match(tuple, next_rec, offsets, &match);
    if (mode == PAGE_CUR_LE)
    {
      success = cmp < 0;
      cursor->up_match = match;
...
ibool btr_search_guess_on_hash(dict_index_t *index, btr_search_t *info, const dtuple_t *tuple, ulint mode,
                               ulint latch_mode, btr_cur_t *cursor, ulint has_search_latch, mtr_t *mtr)
{
  const rec_t *rec;
  ulint fold;
  index_id_t index_id;
...
retry:
  ...
  ut_ad(block->page.buf_fix_count == 0 || buf_block_get_state(block) == BUF_BLOCK_REMOVE_HASH ||
        rw_lock_own(&block->lock, RW_LOCK_S) || rw_lock_own(&block->lock, RW_LOCK_X));
  ...
  const ulint ahi_slot =
      ut_fold_ulint_pair(static_cast<ulint>(index_id), static_cast<ulint>(block->page.id.space())) % btr_ahi_parts;
  ...
  {
    offsets = rec_get_offsets(rec, index, offsets, btr_search_get_n_fields(n_fields, n_bytes), &heap);
...
void btr_search_drop_page_hash_when_freed(const page_id_t &page_id, const page_size_t &page_size)
{
  ...
  block = buf_page_get_gen(page_id, page_size, RW_X_LATCH, NULL, BUF_PEEK_IF_IN_POOL, __FILE__, __LINE__, &mtr);
...
static void btr_search_build_page_hash_index(dict_index_t *index, buf_block_t *block, ulint n_fields, ulint n_bytes,
                                             ibool left_side)
{
  hash_table_t *table;
  page_t *page;
  ...
  if (block->index &&
      ((block->curr_n_fields != n_fields) || (block->curr_n_bytes != n_bytes) || (block->curr_left_side != left_side)))
  {
    btr_search_s_unlock(index);
  ...
  if (dict_index_get_n_unique_in_tree(index) < btr_search_get_n_fields(n_fields, n_bytes))
  {
    return;
  }
  ...
  offsets = rec_get_offsets(rec, index, offsets, btr_search_get_n_fields(n_fields, n_bytes), &heap);
  ut_ad(page_rec_is_supremum(rec) || n_fields + (n_bytes > 0) == rec_offs_n_fields(offsets));
  ...
  for (;;)
  {
    ...
    offsets = rec_get_offsets(next_rec, index, offsets, btr_search_get_n_fields(n_fields, n_bytes), &heap);
    next_fold = rec_fold(next_rec, offsets, n_fields, n_bytes, index->id);
...
void btr_search_update_hash_on_insert(btr_cur_t *cursor)
{
  ...
  offsets = rec_get_offsets(ins_rec, index, offsets, ULINT_UNDEFINED, &heap);
...
static ibool btr_search_hash_table_validate(ulint hash_table_id)
{
  ...
  for (i = 0; i < cell_count; i++)
  {
    ...
    if ((i != 0) && ((i % chunk_size) == 0))
    {
      ...
      ulint curr_cell_count = hash_get_n_cells(btr_search_sys->hash_tables[hash_table_id]);
    ...
    node = (ha_node_t *)hash_get_nth_cell(btr_search_sys->hash_tables[hash_table_id], i)->node;

    for (; node != NULL; node = node->next)
    {
      const buf_block_t *block = buf_block_from_ahi((byte *)node->data);
      ...
      offsets = rec_get_offsets(node->data, block->index, offsets,
                                btr_search_get_n_fields(block->curr_n_fields, block->curr_n_bytes), &heap);

      const ulint fold = rec_fold(node->data, offsets, block->curr_n_fields, block->curr_n_bytes, page_index_id);
      ...
      if (node->fold != fold)
      {
        ...
        ib::error() << "Error in an adaptive hash"
                    << " index pointer to page " << page_id_t(page_get_space_id(page), page_get_page_no(page))
                    << ", ptr mem address " << reinterpret_cast<const void *>(node->data) << ", index id "
                    << page_index_id << ", node fold " << node->fold << ", rec fold " << fold;
        ...
        fprintf(stderr,
                ...
                " n fields %lu\n"
                "InnoDB: side %lu\n",
                (void *)page, (void *)block->index, (ulong)block->curr_n_fields, (ulong)block->curr_left_side);
        ut_ad(0);
      }
    }
  }
  ...
  for (i = 0; i < cell_count; i += chunk_size)
  {
    ...
    if (!ha_validate(btr_search_sys->hash_tables[hash_table_id], i, end_index))
    {
      ok = FALSE;
    }
  }
