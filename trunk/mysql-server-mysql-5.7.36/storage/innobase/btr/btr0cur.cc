...
#define BTR_TABLE_STATS_FROM_SAMPLE(value, index, sample, ext_size, not_empty)                          \
  (((value) * static_cast<int64_t>(index->stat_n_leaf_pages) + (sample)-1 + (ext_size) + (not_empty)) / \
   ((sample) + (ext_size)))
...
btr_latch_leaves_t btr_cur_latch_leaves(buf_block_t *block, const page_id_t &page_id, const page_size_t &page_size,
                                        ulint latch_mode, btr_cur_t *cursor, mtr_t *mtr)
{
  ...
  switch (latch_mode)
  {
    ...
    case BTR_SEARCH_TREE:
      if (spatial)
      {
        cursor->rtr_info->tree_savepoints[RTR_MAX_LEVELS] = mtr_set_savepoint(mtr);
      }
    ...
    case BTR_MODIFY_TREE:
      ...
      btr_page_set_prev() */
      ut_ad(mtr_memo_contains_flagged(mtr, dict_index_get_lock(cursor->index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK) ||
      ...
      if (left_page_no != FIL_NULL)
      {
        ...
        get_block = btr_block_get(page_id_t(page_id.space(), left_page_no), page_size, RW_X_LATCH, cursor->index, mtr);
      ...
      if (spatial)
      {
        cursor->rtr_info->tree_savepoints[RTR_MAX_LEVELS + 1] = mtr_set_savepoint(mtr);
      }

      latch_leaves.savepoints[1] = mtr_set_savepoint(mtr);
      get_block = btr_block_get(page_id, page_size, RW_X_LATCH, cursor->index, mtr);
      latch_leaves.blocks[1] = get_block;
      ...
      if (latch_leaves.blocks[0] != NULL)
      {
        ...
        ut_a(btr_page_get_next(latch_leaves.blocks[0]->frame, mtr) == page_get_page_no(page));
      }
      ut_a(page_is_comp(get_block->frame) == page_is_comp(page));
#endif /* UNIV_BTR_DEBUG */
      ...
      if (right_page_no != FIL_NULL)
      {
        if (spatial)
        {
          cursor->rtr_info->tree_savepoints[RTR_MAX_LEVELS + 2] = mtr_set_savepoint(mtr);
        }
        latch_leaves.savepoints[2] = mtr_set_savepoint(mtr);
        get_block = btr_block_get(page_id_t(page_id.space(), right_page_no), page_size, RW_X_LATCH, cursor->index, mtr);
    ...
    case BTR_MODIFY_PREV:
      ...
      {
        latch_leaves.savepoints[0] = mtr_set_savepoint(mtr);
        get_block = btr_block_get(page_id_t(page_id.space(), left_page_no), page_size, mode, cursor->index, mtr);
        latch_leaves.blocks[0] = get_block;
...
bool btr_cur_optimistic_latch_leaves(buf_block_t *block, ib_uint64_t modify_clock, ulint *latch_mode, btr_cur_t *cursor,
                                     const char *file, ulint line, mtr_t *mtr)
{
  ...
  switch (*latch_mode)
  {
    case BTR_SEARCH_LEAF:
    case BTR_MODIFY_LEAF:
      return (buf_page_optimistic_get(*latch_mode, block, modify_clock, file, line, mtr));
    case BTR_SEARCH_PREV:
    case BTR_MODIFY_PREV:
      mode = *latch_mode == BTR_SEARCH_PREV ? RW_S_LATCH : RW_X_LATCH;
      ...
      if (left_page_no != FIL_NULL)
      {
        ...
        cursor->left_block =
            btr_block_get(page_id, dict_table_page_size(cursor->index->table), mode, cursor->index, mtr);
      }
      else
      {
...
static bool btr_cur_will_modify_tree(dict_index_t *index, const page_t *page, btr_intention_t lock_intention,
                                     const rec_t *rec, ulint rec_size, const page_size_t &page_size, mtr_t *mtr)
{
  ut_ad(!page_is_leaf(page));
  ut_ad(mtr_memo_contains_flagged(mtr, dict_index_get_lock(index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK) ||
  ...
  if (lock_intention >= BTR_INTENTION_BOTH)
  {
    ...
    if (page_size.is_compressed() &&
        page_zip_empty_size(index->n_fields, page_size.physical()) <
            rec_size * 2 + page_get_data_size(page) + page_dir_calc_reserved_space(page_get_n_recs(page) + 2) + 1)
    {
      return (true);
    }
  }
    ...
    mtr_t *mtr)       /*!< in: mtr */
{
  ...
  ut_ad(!s_latch_by_caller || srv_read_only_mode ||
        mtr_memo_contains_flagged(mtr, dict_index_get_lock(index), MTR_MEMO_S_LOCK | MTR_MEMO_SX_LOCK));
  ...
  switch (UNIV_EXPECT(latch_mode & (BTR_INSERT | BTR_DELETE | BTR_DELETE_MARK), 0))
  {
    case 0:
      ...
      break;
    case BTR_INSERT:
      btr_op = (latch_mode & BTR_IGNORE_SEC_UNIQUE) ? BTR_INSERT_IGNORE_UNIQUE_OP : BTR_INSERT_OP;
      break;
    case BTR_DELETE:
  ...
  ut_ad(!s_latch_by_caller || latch_mode == BTR_SEARCH_LEAF || latch_mode == BTR_SEARCH_TREE ||
  ...
  if (rw_lock_get_writer(btr_get_search_latch(index)) == RW_LOCK_NOT_LOCKED && latch_mode <= BTR_MODIFY_LEAF &&
      info->last_hash_succ && !index->disable_ahi && !estimate
  ...
  switch (latch_mode)
  {
    case BTR_MODIFY_TREE:
      ...
      if (lock_intention == BTR_INTENTION_DELETE && trx_sys->rseg_history_len > BTR_CUR_FINE_HISTORY_LENGTH &&
    ...
    case BTR_CONT_SEARCH_TREE:
      ...
      ut_ad(srv_read_only_mode ||
            mtr_memo_contains_flagged(mtr, dict_index_get_lock(index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK));
  ...
  switch (mode)
  {
    ...
    default:
#ifdef PAGE_CUR_LE_OR_EXTENDS
      ut_ad(mode == PAGE_CUR_L || mode == PAGE_CUR_LE || RTREE_SEARCH_MODE(mode) || mode == PAGE_CUR_LE_OR_EXTENDS);
#else  /* PAGE_CUR_LE_OR_EXTENDS */
      ut_ad(mode == PAGE_CUR_L || mode == PAGE_CUR_LE || RTREE_SEARCH_MODE(mode));
#endif /* PAGE_CUR_LE_OR_EXTENDS */
      page_mode = mode;
      break;
  }
...
search_loop:
  ...
  if (height != 0)
  {
    ...
    if ((latch_mode != BTR_MODIFY_TREE || height == level) && !retrying_for_search_prev)
    {
  ...
  else if (latch_mode <= BTR_MODIFY_LEAF)
  {
    ...
    if (btr_op != BTR_NO_OP && ibuf_should_try(index, btr_op != BTR_INSERT_OP))
    {
      ...
      buf_mode = btr_op == BTR_DELETE_OP ? BUF_GET_IF_IN_POOL_OR_WATCH : BUF_GET_IF_IN_POOL;
    }
  }
...
retry_page_get:
  ...
  block = buf_page_get_gen(page_id, page_size, rw_latch, (height == ULINT_UNDEFINED ? info->root_guess : NULL),
  ...
  if (block == NULL)
  {
    ...
    switch (btr_op)
    {
      ...
      case BTR_DELMARK_OP:
        ...
        if (ibuf_insert(IBUF_OP_DELETE_MARK, tuple, index, page_id, page_size, cursor->thr))
        {
  ...
  if (retrying_for_search_prev && height != 0)
  {
    ...
    if (left_page_no != FIL_NULL)
    {
      ...
      get_block = buf_page_get_gen(page_id_t(page_id.space(), left_page_no), page_size, rw_latch, NULL, buf_mode, file,
                                   line, mtr);
    ...
    tree_savepoints[n_blocks] = mtr_set_savepoint(mtr);
    block = buf_page_get_gen(page_id, page_size, rw_latch, NULL, buf_mode, file, line, mtr);
    tree_blocks[n_blocks] = block;
  }

  page = buf_block_get_frame(block);

  if (height == ULINT_UNDEFINED && page_is_leaf(page) && rw_latch != RW_NO_LATCH && rw_latch != root_leaf_rw_latch)
  {
  ...
  if (rw_latch != RW_NO_LATCH)
  {
    ...
    buf_block_dbg_add_level(block, dict_index_is_ibuf(index) ? SYNC_IBUF_TREE_NODE : SYNC_TREE_NODE);
  }
    ...
    switch (latch_mode)
    {
      ...
      default:
        ...
        if (retrying_for_search_prev)
        {
          ...
          {
            mtr_release_block_at_savepoint(mtr, prev_tree_savepoints[prev_n_releases],
        ...
        for (; n_releases < n_blocks; n_releases++)
        {
          if (n_releases == 0 && modify_external)
          {
            ...
            ut_ad(mtr_memo_contains_flagged(mtr, tree_blocks[n_releases], MTR_MEMO_PAGE_SX_FIX | MTR_MEMO_PAGE_X_FIX));
            continue;
          }
  ...
  if (dict_index_is_spatial(index))
  {
    ...
    if (latch_mode == BTR_MODIFY_TREE || latch_mode == BTR_CONT_MODIFY_TREE || latch_mode == BTR_CONT_SEARCH_TREE)
    {
  ...
  {
    ut_ad(need_path);
    found = rtr_cur_search_with_match(block, index, tuple, page_mode, page_cursor, cursor->rtr_info);
  ...
  else if (height == 0 && btr_search_enabled && !dict_index_is_spatial(index))
  {
    /* The adaptive hash index is only used when searching
    for leaf pages (height==0), but not in r-trees.
    We only need the byte prefix comparison for the purpose
    of updating the adaptive hash index. */
    page_cur_search_with_match_bytes(block, index, tuple, page_mode, &up_match, &up_bytes, &low_match, &low_bytes,
                                     page_cursor);
  }
  else
  {
    ...
    page_cur_search_with_match(block, index, tuple, page_mode, &up_match, &low_match, page_cursor,
  ...
  if (dict_index_is_spatial(index) && cursor->rtr_info->need_prdt_lock && mode != PAGE_CUR_RTREE_INSERT &&
      mode != PAGE_CUR_RTREE_LOCATE && mode >= PAGE_CUR_CONTAIN)
  {
    trx_t *trx = thr_get_trx(cursor->thr);
    lock_prdt_t prdt;

    lock_mutex_enter();
    lock_init_prdt_from_mbr(&prdt, &cursor->rtr_info->mbr, mode, trx->lock.lock_heap);
    lock_mutex_exit();
  ...
  if (level != height)
  {
    ...
    */
    if (latch_mode == BTR_MODIFY_TREE && btr_cur_need_opposite_intention(page, lock_intention, node_ptr))
    {
    ...
    if (dict_index_is_spatial(index))
    {
      ...
      if (page_mode == PAGE_CUR_RTREE_INSERT ||
          (search_mode == PAGE_CUR_RTREE_LOCATE && (latch_mode != BTR_MODIFY_LEAF)))
      {
...
#ifdef UNIV_DEBUG
        ulint num_stored = rtr_store_parent_path(block, cursor, latch_mode, height + 1, mtr);
#else
        ...
        if (page_mode == PAGE_CUR_RTREE_INSERT)
        {
          btr_pcur_t *r_cursor = rtr_get_parent_cursor(cursor, height + 1, true);
      ...
      ut_ad(page_mode == search_mode || (page_mode == PAGE_CUR_WITHIN && search_mode == PAGE_CUR_RTREE_LOCATE));
    ...
    if (!detected_same_key_root && lock_intention == BTR_INTENTION_BOTH && !dict_index_is_unique(index) &&
        latch_mode == BTR_MODIFY_TREE &&
        (up_match >= rec_offs_n_fields(offsets) - 1 || low_match >= rec_offs_n_fields(offsets) - 1))
    {
      const rec_t *first_rec = page_rec_get_next_const(page_get_infimum_rec(page));
      ...
      else
      {
        ...
        offsets2 = rec_get_offsets(first_rec, index, offsets2, ULINT_UNDEFINED, &heap);
        cmp_rec_rec_with_match(node_ptr, first_rec, offsets, offsets2, index,
                               page_is_spatial_non_leaf(first_rec, index), false, &matched_fields);
        ...
        else
        {
          ...
          offsets2 = rec_get_offsets(last_rec, index, offsets2, ULINT_UNDEFINED, &heap);
          cmp_rec_rec_with_match(node_ptr, last_rec, offsets, offsets2, index,
                                 page_is_spatial_non_leaf(last_rec, index), false, &matched_fields);
          if (matched_fields >= rec_offs_n_fields(offsets) - 1)
          {
            detected_same_key_root = true;
          }
    ...
    if (!detected_same_key_root && latch_mode == BTR_MODIFY_TREE &&
        !btr_cur_will_modify_tree(index, page, lock_intention, node_ptr, node_ptr_max_size, page_size, mtr) &&
    ...
    if ((latch_mode == BTR_SEARCH_PREV || latch_mode == BTR_MODIFY_PREV) && !retrying_for_search_prev)
    {
      /* block should be latched for consistent
         btr_page_get_prev() */
      ut_ad(mtr_memo_contains_flagged(mtr, block, MTR_MEMO_PAGE_S_FIX | MTR_MEMO_PAGE_X_FIX));
      ...
      if (height == 0 && leftmost_from_level > 0)
      {
        ...
        prev_tree_blocks = static_cast<buf_block_t **>(ut_malloc_nokey(sizeof(buf_block_t *) * leftmost_from_level));

        prev_tree_savepoints = static_cast<ulint *>(ut_malloc_nokey(sizeof(ulint) * leftmost_from_level));
        ...
        for (ulint i = n_blocks - (leftmost_from_level - 1); i <= n_blocks; i++)
        {
          mtr_release_block_at_savepoint(mtr, tree_savepoints[i], tree_blocks[i]);
        }
        ...
        {
          page_cur_search_with_match(tree_blocks[i], index, tuple, page_mode, &up_match, &low_match, page_cursor,
                                     rtr_info);
        }
    ...
    if (dict_index_is_spatial(index) && page_mode >= PAGE_CUR_CONTAIN && page_mode != PAGE_CUR_RTREE_INSERT)
    {
      ut_ad(need_path);
...
#ifdef UNIV_DEBUG
        if (page_mode == PAGE_CUR_RTREE_LOCATE && (latch_mode != BTR_MODIFY_LEAF))
        {
          btr_pcur_t *cur = cursor->rtr_info->parent_path->back().cursor;
          rec_t *my_node_ptr = btr_pcur_get_rec(cur);

          offsets = rec_get_offsets(my_node_ptr, index, offsets, ULINT_UNDEFINED, &heap);

          ulint my_page_no = btr_node_ptr_get_child_page_no(my_node_ptr, offsets);
  ...
  else if (!dict_index_is_spatial(index) && latch_mode == BTR_MODIFY_TREE && lock_intention == BTR_INTENTION_INSERT &&
           mach_read_from_4(page + FIL_PAGE_NEXT) != FIL_NULL && page_rec_is_last(page_cur_get_rec(page_cursor), page))
  {
    ...
    if (height == 0)
    {
      ...
      for (uint i = 0; i < 3; i++)
      {
        ...
        {
          mtr_release_block_at_savepoint(mtr, latch_leaves.savepoints[i], latch_leaves.blocks[i]);
  ...
  if (level != 0)
  {
    if (upper_rw_latch == RW_NO_LATCH)
    {
      ...
      else
      {
        ...
        child_block = btr_block_get(page_id, page_size, RW_SX_LATCH, index, mtr);
      }
  ...
  if (dict_index_is_spatial(index) && (latch_mode == BTR_MODIFY_TREE || latch_mode == BTR_MODIFY_LEAF))
  {
...
void btr_cur_search_to_nth_level_with_no_latch(dict_index_t *index, ulint level, const dtuple_t *tuple,
                                               ...
                                               mtr_t *mtr, bool mark_dirty)
{
  ...
  while (!at_desired_level)
  {
    ...
    block = buf_page_get_gen(page_id, page_size, rw_latch, NULL, buf_mode, file, line, mtr, mark_dirty);
    ...
    page_cur_search_with_match(block, index, tuple, page_mode, &up_match, &low_match, page_cursor, NULL);
    ...
    mtr_t *mtr)          /*!< in/out: mini-transaction */
{
  ...
  switch (latch_mode)
  {
    ...
    default:
      ut_ad(!s_latch_by_caller ||
            mtr_memo_contains_flagged(mtr, dict_index_get_lock(index), MTR_MEMO_SX_LOCK | MTR_MEMO_S_LOCK));
      if (!srv_read_only_mode)
      {
        if (!s_latch_by_caller)
        {
  ...
  for (;;)
  {
    ...
    tree_savepoints[n_blocks] = mtr_set_savepoint(mtr);
    block = buf_page_get_gen(page_id, page_size, rw_latch, NULL, BUF_GET, file, line, mtr);
    tree_blocks[n_blocks] = block;

    page = buf_block_get_frame(block);

    if (height == ULINT_UNDEFINED && btr_page_get_level(page, mtr) == 0 && rw_latch != RW_NO_LATCH &&
    ...
    if (height == level)
    {
      if (srv_read_only_mode)
      {
        btr_cur_latch_leaves(block, page_id, page_size, latch_mode, cursor, mtr);
      }
      else if (height == 0)
      {
        if (rw_latch == RW_NO_LATCH)
    ...
    */
    ...
    if (latch_mode == BTR_MODIFY_TREE &&
        !btr_cur_will_modify_tree(cursor->index, page, lock_intention, node_ptr, node_ptr_max_size, page_size, mtr))
    {
      ut_ad(upper_rw_latch == RW_X_LATCH);
      ut_ad(n_releases <= n_blocks);
...
*/
void btr_cur_open_at_index_side_with_no_latch_func(bool from_left, dict_index_t *index, btr_cur_t *cursor, ulint level,
                                                   const char *file, ulint line, mtr_t *mtr)
{
  page_cur_t *page_cursor;
  ulint height;
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) rec_t *btr_cur_insert_if_possible(
    /*=======================*/
    ...
    mtr_t *mtr)            /*!< in/out: mini-transaction */
{
  ...
  ut_ad(mtr_is_block_fix(mtr, btr_cur_get_block(cursor), MTR_MEMO_PAGE_X_FIX, cursor->index->table));
  page_cursor = btr_cur_get_page_cur(cursor);

  /* Now, try the insert */
  rec = page_cur_tuple_insert(page_cursor, tuple, cursor->index, offsets, heap, n_ext, mtr);

  /* If the record did not fit, reorganize.
  For compressed pages, page_cur_tuple_insert()
  attempted this already. */
  if (!rec && !page_cur_get_page_zip(page_cursor) && btr_page_reorganize(page_cursor, cursor->index, mtr))
  {
    rec = page_cur_tuple_insert(page_cursor, tuple, cursor->index, offsets, heap, n_ext, mtr);
  }
...
UNIV_INLINE MY_ATTRIBUTE((warn_unused_result, nonnull(2, 3, 5, 6))) dberr_t btr_cur_ins_lock_and_undo(
    /*======================*/
                       ...
                       successor record */
{
  ...
  ut_ad(!dict_index_is_online_ddl(index) || dict_index_is_clust(index) || (flags & BTR_CREATE_FLAG));
  ...
  if (!(flags & BTR_NO_LOCKING_FLAG))
  {
    if (dict_index_is_spatial(index))
    {
      ...
      err = lock_prdt_insert_check_and_lock(flags, rec, btr_cur_get_block(cursor), index, thr, mtr, &prdt);
      *inherit = false;
    }
    else
    {
      err = lock_rec_insert_check_and_lock(flags, rec, btr_cur_get_block(cursor), index, thr, mtr, inherit);
    }
  }

  if (err != DB_SUCCESS || !dict_index_is_clust(index) || dict_index_is_ibuf(index))
  {
    return (err);
  }

  err = trx_undo_report_row_operation(flags, TRX_UNDO_INSERT_OP, thr, index, entry, NULL, 0, NULL, NULL, &roll_ptr);
  if (err != DB_SUCCESS)
  {
    return (err);
  }
  ...
  if (!(flags & BTR_KEEP_SYS_FLAG) && !dict_table_is_intrinsic(index->table))
  {
...
static void btr_cur_prefetch_siblings(buf_block_t *block)
{
  ...
  if (left_page_no != FIL_NULL)
  {
    buf_read_page_background(page_id_t(block->page.id.space(), left_page_no), block->page.size, false);
  }
  if (right_page_no != FIL_NULL)
  {
    buf_read_page_background(page_id_t(block->page.id.space(), right_page_no), block->page.size, false);
  }
                         ...
                         any further pages */
{
  ...
  *big_rec = NULL;
  ...
  if (page_zip_rec_needs_ext(rec_size, page_is_comp(page), dtuple_get_n_fields(entry), page_size))
  {
  ...
  if (leaf && page_size.is_compressed() &&
      (page_get_data_size(page) + rec_size >= dict_index_zip_pad_optimal_page_size(index)))
  {
  ...
  {
    if ((max_size < rec_size || max_size < BTR_CUR_PAGE_REORGANIZE_LIMIT) && page_get_n_recs(page) > 1 &&
  ...
  if (leaf && !page_size.is_compressed() && dict_index_is_clust(index) && page_get_n_recs(page) >= 2 &&
      dict_index_get_space_reserve() + rec_size > max_size &&
      (btr_page_get_split_rec_to_right(cursor, &dummy) || btr_page_get_split_rec_to_left(cursor, &dummy)))
  {
    goto fail;
  }
  ...
  DBUG_PRINT("ib_cur", ("insert %s (" IB_ID_FMT ") by " TRX_ID_FMT ": %s", index->name(), index->id,
                        thr != NULL ? trx_get_id_for_print(thr_get_trx(thr)) : 0, rec_printer(entry).str().c_str()));

  DBUG_EXECUTE_IF("do_page_reorganize", btr_page_reorganize(page_cursor, index, mtr););
  ...
  /* Now, try the insert */
  {
    ...
    if (dict_table_is_intrinsic(index->table))
    {
      ...
      *rec = page_cur_tuple_direct_insert(page_cursor, entry, index, n_ext, mtr);
    }
    else
    {
      ...
      *rec = page_cur_tuple_insert(page_cursor, entry, index, offsets, heap, n_ext, mtr);
    }
  ...
  else if (page_size.is_compressed())
  {
    ...
    if (leaf && !dict_index_is_clust(index) && !dict_table_is_temporary(index->table))
    {
  ...
  else
  {
    ...
    *rec = page_cur_tuple_insert(page_cursor, entry, index, offsets, heap, n_ext, mtr);
    ...
    {
      ib::fatal() << "Cannot insert tuple " << *entry << "into index " << index->name << " of table "
                  << index->table->name << ". Max size: " << max_size;
    }
  }
    ...
    mtr_t *mtr)          /*!< in/out: mini-transaction */
{
  ...
  *big_rec = NULL;
  ...
  ut_ad(mtr_memo_contains_flagged(mtr, dict_index_get_lock(btr_cur_get_index(cursor)),
  ...
  if (!(flags & BTR_NO_UNDO_LOG_FLAG) || dict_table_is_intrinsic(index->table))
  {
  ...
  if (page_zip_rec_needs_ext(rec_get_converted_size(index, entry, n_ext), dict_table_is_comp(index->table),
                             dtuple_get_n_fields(entry), dict_table_page_size(index->table)))
  {
    /* The record is so big that we have to store some fields
    externally on separate database pages */
  ...
  if (dict_index_get_page(index) == btr_cur_get_block(cursor)->page.id.page_no())
  {
    ...
    *rec = btr_root_raise_and_insert(flags, cursor, offsets, heap, entry, n_ext, mtr);
  }
  else
  {
    *rec = btr_page_split_and_insert(flags, cursor, offsets, heap, entry, n_ext, mtr);
  }

  ut_ad(page_rec_get_next(btr_cur_get_rec(cursor)) == *rec || dict_index_is_spatial(index));
  ...
  if (!(flags & BTR_NO_LOCKING_FLAG))
  {
    ...
    else
    {
      ...
      {
        page_update_max_trx_id(btr_cur_get_block(cursor), btr_cur_get_page_zip(cursor), thr_get_trx(thr)->id, mtr);
      }
      if (!page_rec_is_infimum(btr_cur_get_rec(cursor)) ||
          btr_page_get_prev(buf_block_get_frame(btr_cur_get_block(cursor)), mtr) == FIL_NULL)
      {
...
UNIV_INLINE MY_ATTRIBUTE((warn_unused_result)) dberr_t btr_cur_upd_lock_and_undo(
    /*======================*/
    ...
    roll_ptr_t *roll_ptr) /*!< out: roll pointer */
{
  ...
  if (!dict_index_is_clust(index))
  {
    ...
    record */
    return (lock_sec_rec_modify_check_and_lock(flags, btr_cur_get_block(cursor), rec, index, thr, mtr));
  }
  ...
  if (!(flags & BTR_NO_LOCKING_FLAG))
  {
    err = lock_clust_rec_modify_check_and_lock(flags, btr_cur_get_block(cursor), rec, index, offsets, thr);
    if (err != DB_SUCCESS)
    {
      return (err);
    }
  ...
  return (trx_undo_report_row_operation(flags, TRX_UNDO_MODIFY_OP, thr, index, NULL, update, cmpl_info, rec, offsets,
                                        roll_ptr));
}
    ...
    mtr_t *mtr)     /*!< in/out: mini-transaction */
{
  ...
  if (page_zip_available(page_zip, dict_index_is_clust(index), length, create))
  {
    return (true);
  }
  ...
  if (create && page_is_leaf(page) &&
      (length + page_get_data_size(page) >= dict_index_zip_pad_optimal_page_size(index)))
  {
    return (false);
  }
...
out_of_space:
  ...
  if (!dict_index_is_clust(index) && !dict_table_is_temporary(index->table) && page_is_leaf(page))
  {
                         ...
                         further pages */
{
  ...
  ut_ad(trx_id > 0 || (flags & BTR_KEEP_SYS_FLAG) || dict_table_is_intrinsic(index->table));
  /* The insert buffer tree should never be updated in place. */
  ut_ad(!dict_index_is_ibuf(index));
  ut_ad(dict_index_is_online_ddl(index) == !!(flags & BTR_CREATE_FLAG) || dict_index_is_clust(index));
  ut_ad(thr_get_trx(thr)->id == trx_id ||
        (flags & ~(BTR_KEEP_POS_FLAG | BTR_KEEP_IBUF_BITMAP)) ==
            (BTR_NO_UNDO_LOG_FLAG | BTR_NO_LOCKING_FLAG | BTR_CREATE_FLAG | BTR_KEEP_SYS_FLAG));
  ut_ad(fil_page_index_page_check(btr_cur_get_page(cursor)));
  ut_ad(btr_page_get_index_id(btr_cur_get_page(cursor)) == index->id);

  DBUG_PRINT("ib_cur", ("update-in-place %s (" IB_ID_FMT ") by " TRX_ID_FMT ": %s", index->name(), index->id, trx_id,
  ...
  if (page_zip)
  {
    if (!btr_cur_update_alloc_zip(page_zip, btr_cur_get_page_cur(cursor), index, offsets, rec_offs_size(offsets), false,
                                  mtr))
    {
  ...
  err = btr_cur_upd_lock_and_undo(flags, cursor, offsets, update, cmpl_info, thr, mtr, &roll_ptr);
  ...
  if (!(flags & BTR_KEEP_SYS_FLAG) && !dict_table_is_intrinsic(index->table))
  {
    row_upd_rec_sys_fields(rec, NULL, index, offsets, thr_get_trx(thr), roll_ptr);
  }
  ...
  if (is_hashed)
  {
    ...
    if (!dict_index_is_clust(index) || row_upd_changes_ord_field_binary(index, update, thr, NULL, NULL))
    {
...
func_exit:
  if (page_zip && !(flags & BTR_KEEP_IBUF_BITMAP) && !dict_index_is_clust(index) &&
      !dict_table_is_temporary(index->table) && page_is_leaf(buf_block_get_frame(block)))
  {
                         ...
                         further pages */
{
  ...
  *offsets = rec_get_offsets(rec, index, *offsets, ULINT_UNDEFINED, heap);
  ...
  ut_a(!rec_offs_any_null_extern(rec, *offsets) || trx_is_recv(thr_get_trx(thr)));
  ...
  if (!row_upd_changes_field_size_or_external(index, *offsets, update))
  {
    ...
    return (btr_cur_update_in_place(flags, cursor, *offsets, update, cmpl_info, thr, trx_id, mtr));
  }
  ...
  DBUG_PRINT("ib_cur", ("update %s (" IB_ID_FMT ") by " TRX_ID_FMT ": %s", index->name(), index->id, trx_id,
                        rec_printer(rec, *offsets).str().c_str()));
  ...
  if (!*heap)
  {
    *heap = mem_heap_create(rec_offs_size(*offsets) + DTUPLE_EST_ALLOC(rec_offs_n_fields(*offsets)));
  }
  ...
  if (page_zip)
  {
    if (!btr_cur_update_alloc_zip(page_zip, page_cursor, index, *offsets, new_rec_size, true, mtr))
    {
      return (DB_ZIP_OVERFLOW);
    }
  ...
  if (UNIV_UNLIKELY(page_get_data_size(page) - old_rec_size + new_rec_size < BTR_CUR_PAGE_COMPRESS_LIMIT(index)))
  {
    /* We may need to update the IBUF_BITMAP_FREE
    bits after a reorganize that was done in
    btr_cur_update_alloc_zip(). */
  ...
  rec = btr_cur_insert_if_possible(cursor, new_entry, offsets, heap, 0 /*n_ext*/, mtr);
...
func_exit:
  if (!(flags & BTR_KEEP_IBUF_BITMAP) && !dict_index_is_clust(index) && !dict_table_is_temporary(index->table))
  {
    /* Update the free bits in the insert buffer. */
    if (page_zip)
    {
                         ...
                         committed before latching any further pages */
{
  ...
  *big_rec = NULL;
  ...
  ut_ad(trx_id > 0 || (flags & BTR_KEEP_SYS_FLAG) || dict_table_is_intrinsic(index->table));
  ut_ad(dict_index_is_online_ddl(index) == !!(flags & BTR_CREATE_FLAG) || dict_index_is_clust(index));
  ut_ad(thr_get_trx(thr)->id == trx_id || (flags & ~BTR_KEEP_POS_FLAG) == (BTR_NO_UNDO_LOG_FLAG | BTR_NO_LOCKING_FLAG |
                                                                           BTR_CREATE_FLAG | BTR_KEEP_SYS_FLAG));

  err = optim_err = btr_cur_optimistic_update(flags | BTR_KEEP_IBUF_BITMAP, cursor, offsets, offsets_heap, update,
  ...
  switch (err)
  {
    ...
    err_exit:
      ...
      if (page_zip && optim_err != DB_ZIP_OVERFLOW && !dict_index_is_clust(index) &&
          !dict_table_is_temporary(index->table) && page_is_leaf(page))
      {
        ibuf_update_free_bits_zip(block, mtr);
      }
  ...
  *offsets = rec_get_offsets(rec, index, *offsets, ULINT_UNDEFINED, offsets_heap);

  dtuple_t *new_entry = row_rec_to_index_entry(rec, index, *offsets, &n_ext, entry_heap);
  ...
  if ((flags & BTR_NO_UNDO_LOG_FLAG) && rec_offs_any_extern(*offsets) && !dict_table_is_intrinsic(index->table))
  {
    ...
    btr_rec_free_updated_extern_fields(index, rec, page_zip, *offsets, update, true, mtr);
  }

  if (page_zip_rec_needs_ext(rec_get_converted_size(index, new_entry, n_ext), page_is_comp(page),
                             dict_index_get_n_fields(index), block->page.size))
  {
  ...
  rec = btr_cur_insert_if_possible(cursor, new_entry, offsets, offsets_heap, n_ext, mtr);
  ...
  if (rec)
  {
    ...
    else if (!dict_index_is_clust(index) && !dict_table_is_temporary(index->table) && page_is_leaf(page))
    {
      /* Update the free bits in the insert buffer.
    ...
    if (!srv_read_only_mode && !big_rec_vec && page_is_leaf(page) && !dict_index_is_online_ddl(index))
    {
  ...
  err = btr_cur_pessimistic_insert(BTR_NO_UNDO_LOG_FLAG | BTR_NO_LOCKING_FLAG | BTR_KEEP_SYS_FLAG, cursor, offsets,
                                   offsets_heap, new_entry, &rec, &dummy_big_rec, n_ext, NULL, mtr);
  ut_a(rec);
  ut_a(err == DB_SUCCESS);
  ...
  for MVCC. */
  if (dict_index_is_sec_or_ibuf(index) && !dict_table_is_temporary(index->table))
  {
    ...
    page_update_max_trx_id(rec_block, buf_block_get_page_zip(rec_block), trx_id, mtr);
  }
    ...
    dict_index_t *index)      /*!< in: index corresponding to page */
{
  ...
  if (page)
  {
    ...
    if (!(flags & BTR_KEEP_SYS_FLAG))
    {
      ...
      row_upd_rec_sys_fields_in_recovery(rec, page_zip, rec_get_offsets(rec, index, offsets_, ULINT_UNDEFINED, &heap),
                                         pos, trx_id, roll_ptr);
      if (UNIV_LIKELY_NULL(heap))
      {
        mem_heap_free(heap);
    ...
    mtr_t *mtr)            /*!< in/out: mini-transaction */
{
  ...
  err = trx_undo_report_row_operation(flags, TRX_UNDO_MODIFY_OP, thr, index, entry, NULL, 0, rec, offsets, &roll_ptr);
  if (err != DB_SUCCESS)
  {
    return (err);
  }
  ...
  DBUG_PRINT("ib_cur", ("delete-mark clust %s (" IB_ID_FMT ") by " TRX_ID_FMT ": %s", index->table_name, index->id,
                        trx_get_id_for_print(trx), rec_printer(rec, offsets).str().c_str()));
    ...
    mtr_t *mtr)        /*!< in/out: mini-transaction */
{
  ...
  DBUG_PRINT("ib_cur", ("delete-mark=%u sec %u:%u:%u in %s(" IB_ID_FMT ") by " TRX_ID_FMT, unsigned(val),
                        block->page.id.space(), block->page.id.page_no(), unsigned(page_rec_get_heap_no(rec)),
                        cursor->index->name(), cursor->index->id, trx_get_id_for_print(thr_get_trx(thr))));
    ...
    mtr_t *mtr)        /*!< in/out: mini-transaction */
{
  ...
  if (dict_index_is_spatial(cursor->index))
  {
    ...
    if (!lock_test_prdt_page_lock(trx, page_get_space_id(page), page_get_page_no(page)))
    {
      return (false);
    }
  }
                 ...
                 before latching any further pages */
{
  ...
  ut_ad(!dict_index_is_online_ddl(cursor->index) || dict_index_is_clust(cursor->index) || (flags & BTR_CREATE_FLAG));
  ...
  if (no_compress_needed)
  {
    ...
    else
    {
      ...
      if (!dict_index_is_clust(cursor->index) && !dict_table_is_temporary(cursor->index->table) &&
    ...
    mtr_t *mtr)                 /*!< in: mtr */
{
  ...
  if (!has_reserved_extents)
  {
    ...
    success = fsp_reserve_free_extents(&n_reserved, index->space, n_extents, FSP_CLEANING, mtr);
    if (!success)
    {
      *err = DB_OUT_OF_FILE_SPACE;
  ...
  if (rec_offs_any_extern(offsets))
  {
    btr_rec_free_externally_stored_fields(index, rec, offsets, page_zip, rollback, mtr);
#ifdef UNIV_ZIP_DEBUG
    ut_a(!page_zip || page_zip_validate(page_zip, page, index));
#endif /* UNIV_ZIP_DEBUG */
  }

  if (UNIV_UNLIKELY(page_get_n_recs(page) < 2) && UNIV_UNLIKELY(dict_index_get_page(index) != block->page.id.page_no()))
  {
  ...
  if (level > 0 && UNIV_UNLIKELY(rec == page_rec_get_next(page_get_infimum_rec(page))))
  {
    ...
    else if (dict_index_is_spatial(index))
    {
      ...
      rtr_page_get_father_block(NULL, heap, index, block, mtr, NULL, &father_cursor);
      offsets = rec_get_offsets(btr_cur_get_rec(&father_cursor), index, NULL, ULINT_UNDEFINED, &heap);

      father_rec = btr_cur_get_rec(&father_cursor);
      rtr_read_mbr(rec_get_nth_field(father_rec, offsets, 0, &len), &father_mbr);

      upd_ret = rtr_update_mbr_field(&father_cursor, offsets, NULL, page, &father_mbr, next_rec, mtr);
    ...
    else
    {
      ...
      dtuple_t *node_ptr = dict_index_build_node_ptr(index, next_rec, block->page.id.page_no(), heap, level);
  ...
  {
    allow_merge = btr_cur_will_modify_tree(index, page, BTR_INTENTION_DELETE, rec, rec_size_est, page_size, mtr);
  }
...
#define N_PAGES_READ_LIMIT 10
  ...
  do
  {
    ...
    block = buf_page_get_gen(page_id, page_size, RW_S_LATCH, NULL, BUF_GET_POSSIBLY_FREED, __FILE__, __LINE__, &mtr);
...
the two dives). */
static int64_t btr_estimate_n_rows_in_range_low(dict_index_t *index, const dtuple_t *tuple1, page_cur_mode_t mode1,
                                                const dtuple_t *tuple2, page_cur_mode_t mode2, unsigned nth_attempt)
{
  ...
  {
    btr_cur_search_to_nth_level(index, 0, tuple1, mode1, BTR_SEARCH_LEAF | BTR_ESTIMATE, &cursor, 0, __FILE__, __LINE__,
                                &mtr);
    ...
    should_count_the_left_border = !page_rec_is_supremum(btr_cur_get_rec(&cursor));
  }
  else
  {
  ...
  {
    btr_cur_search_to_nth_level(index, 0, tuple2, mode2, BTR_SEARCH_LEAF | BTR_ESTIMATE, &cursor, 0, __FILE__, __LINE__,
                                &mtr);
    ...
    should_count_the_right_border = (mode2 == PAGE_CUR_LE /* if the range is '<=' */
  ...
  for (i = 0;; i++)
  {
    ...
    if (slot1->nth_rec == ULINT_UNDEFINED || slot2->nth_rec == ULINT_UNDEFINED)
    {
      ...
      if (is_n_rows_exact)
      {
        ...
        the leaf level. */
        if (last1->page_no == last2->page_no && last1->nth_rec == last2->nth_rec)
        {
    ...
    if (!diverged && slot1->nth_rec != slot2->nth_rec)
    {
      ...
      if (slot1->page_no != slot2->page_no || slot1->page_level != slot2->page_level)
      {
        ...
        const int64_t ret = btr_estimate_n_rows_in_range_low(index, tuple1, mode1, tuple2, mode2, nth_attempt + 1);
    ...
    {
      n_rows = btr_estimate_n_rows_in_range_on_level(index, slot1, slot2, n_rows, &is_n_rows_exact);
    }
  }
}
...
int64_t btr_estimate_n_rows_in_range(dict_index_t *index, const dtuple_t *tuple1, page_cur_mode_t mode1,
                                     const dtuple_t *tuple2, page_cur_mode_t mode2)
{
  const int64_t ret = btr_estimate_n_rows_in_range_low(index, tuple1, mode1, tuple2, mode2, 1 /* first attempt */);
    ...
    dict_index_t *index) /*!< in: index */
{
  ...
  heap = mem_heap_create((sizeof *n_diff + sizeof *n_not_null) * n_cols +
                         dict_index_get_n_fields(index) * (sizeof *offsets_rec + sizeof *offsets_next_rec));
  ...
  for (i = 0; i < n_sample_pages; i++)
  {
    ...
    while (!page_rec_is_supremum(rec))
    {
      ...
      cmp_rec_rec_with_match(rec, next_rec, offsets_rec, offsets_next_rec, index,
                             page_is_spatial_non_leaf(next_rec, index), stats_null_not_equal, &matched_fields);
  ...
  for (j = 0; j < n_cols; j++)
  {
    index->stat_n_diff_key_vals[j] =
        BTR_TABLE_STATS_FROM_SAMPLE(n_diff[j], index, n_sample_pages, total_external_size, not_empty_flag);
    ...
    if (n_not_null != NULL)
    {
      index->stat_n_non_null_key_vals[j] =
          BTR_TABLE_STATS_FROM_SAMPLE(n_not_null[j], index, n_sample_pages, total_external_size, not_empty_flag);
    }
  }
...
static ulint btr_rec_get_field_ref_offs(
    /*=======================*/
    const ulint *offsets, /*!< in: array returned by rec_get_offsets() */
...
ulint btr_rec_get_externally_stored_len(const rec_t *rec, const ulint *offsets)
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    {
      ulint extern_len = mach_read_from_4(btr_rec_get_field_ref(rec, offsets, i) + BTR_EXTERN_LEN + 4);
    ...
    mtr_t *mtr)               /*!< in/out: mini-transaction */
{
  ...
  {
    if (rec_offs_nth_extern(offsets, i) && !upd_get_field_by_field_no(update, i, false))
    {
    ...
    mtr_t *mtr)          /*!< in: mini-transaction to commit */
{
  ...
  if (buf_block_get_state(block) == BUF_BLOCK_FILE_PAGE && block->page.id.space() == space &&
...
struct btr_blob_log_check_t
{
  ...
  btr_blob_log_check_t(btr_pcur_t *pcur, mtr_t *mtr, const ulint *offsets, buf_block_t **block, rec_t **rec,
                       enum blob_op op)
      : m_pcur(pcur), m_mtr(mtr), m_offsets(offsets), m_block(block), m_rec(rec), m_op(op)
  {
  ...
  void check()
  {
    ...
    if (m_op == BTR_STORE_INSERT_BULK)
    {
      ...
      page_no = page_get_page_no(buf_block_get_frame(*m_block));
    ...
    if (m_op == BTR_STORE_INSERT_BULK)
    {
      ...
      page_cur->block = btr_block_get(page_id, page_size, RW_X_LATCH, index, m_mtr);
      page_cur->rec = buf_block_get_frame(page_cur->block) + offs;
    ...
    else
    {
      ...
      bool ret = btr_pcur_restore_position(BTR_MODIFY_LEAF | BTR_MODIFY_EXTERNAL, m_pcur, m_mtr);
    ...
    *m_rec = btr_pcur_get_rec(m_pcur);
    ...
    ut_d(rec_offs_make_valid(*m_rec, index, const_cast<ulint *>(m_offsets)));

    ut_ad(m_mtr->memo_contains_page_flagged(*m_rec, MTR_MEMO_PAGE_X_FIX | MTR_MEMO_PAGE_SX_FIX) ||
          dict_table_is_intrinsic(index->table));

    ut_ad(mtr_memo_contains_flagged(m_mtr, dict_index_get_lock(index), MTR_MEMO_SX_LOCK | MTR_MEMO_X_LOCK) ||
          dict_table_is_intrinsic(index->table));
  }
};
    ...
    enum blob_op op)              /*! in: operation code */
{
  ...
  ut_ad(rec_offs_any_extern(offsets));
  ut_ad(btr_mtr);
  ut_ad(mtr_memo_contains_flagged(btr_mtr, dict_index_get_lock(index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK) ||
        dict_table_is_intrinsic(index->table) || !index->is_committed());
  ut_ad(mtr_is_block_fix(btr_mtr, rec_block, MTR_MEMO_PAGE_X_FIX, index->table));
  ut_ad(buf_block_get_frame(rec_block) == page_align(rec));
  ut_a(dict_index_is_clust(index));

  ut_a(dict_table_page_size(index->table).equals_to(rec_block->page.size));

  btr_blob_log_check_t redo_log(pcur, btr_mtr, offsets, &rec_block, &rec, op);
  page_zip = buf_block_get_page_zip(rec_block);
  space_id = rec_block->page.id.space();
  rec_page_no = rec_block->page.id.page_no();
  ut_a(fil_page_index_page_check(page_align(rec)) || op == BTR_STORE_INSERT_BULK);
  ...
  if (page_zip)
  {
    ...
    err = deflateInit2(&c_stream, page_zip_level, Z_DEFLATED, 15, 7, Z_DEFAULT_STRATEGY);
    ut_a(err == Z_OK);
  }
  ...
  {
    field_ref = btr_rec_get_field_ref(rec, offsets, big_rec_vec->fields[i].field_no);

    ut_a(!(field_ref[BTR_EXTERN_LEN] & BTR_EXTERN_OWNER_FLAG));
    /* Either this must be an update in place,
    or the BLOB must be inherited, or the BLOB pointer
    must be zero (will be written in this function). */
    ut_a(op == BTR_STORE_UPDATE || (field_ref[BTR_EXTERN_LEN] & BTR_EXTERN_INHERITED_FLAG) ||
  ...
  const ulint payload_size = page_size.physical() - FIL_PAGE_DATA - BTR_BLOB_HDR_SIZE - FIL_PAGE_DATA_END;
  ...
  for (i = 0; i < big_rec_vec->n_fields; i++)
  {
    ...
    UNIV_MEM_ASSERT_RW(big_rec_vec->fields[i].data, extern_len);
    ...
    for (ulint blob_npages = 0;; ++blob_npages)
    {
      ...
      if (!fsp_reserve_free_extents(&r_extents, space_id, 1, FSP_BLOB, alloc_mtr, 1))
      {
      ...
      block = btr_page_alloc(index, hint_page_no, FSP_NO_DIR, 0, alloc_mtr, &mtr);
      ...
      if (prev_page_no != FIL_NULL)
      {
        ...
        prev_block = buf_page_get(page_id_t(space_id, prev_page_no), rec_block->page.size, RW_X_LATCH, &mtr);
      ...
      if (page_zip)
      {
        ...
        mlog_write_ulint(page + FIL_PAGE_TYPE, prev_page_no == FIL_NULL ? FIL_PAGE_TYPE_ZBLOB : FIL_PAGE_TYPE_ZBLOB2,
                         MLOG_2BYTES, &mtr);

        c_stream.next_out = page + FIL_PAGE_DATA;
        c_stream.avail_out = static_cast<uInt>(payload_size_zip);
        ...
        if (op != BTR_STORE_INSERT_BULK)
        {
          page_zip_write_blob_ptr(page_zip, rec, index, offsets, field_no, &mtr);
        }
    ...
    DBUG_EXECUTE_IF("btr_store_big_rec_extern", error = DB_OUT_OF_FILE_SPACE; goto func_exit;);
...
func_exit:
  ...
  for (i = 0; i < rec_offs_n_fields(offsets); i++)
  {
    ...
    succeeded. */
    ut_a(0 != memcmp(field_ref, field_ref_zero, BTR_EXTERN_FIELD_REF_SIZE) || error != DB_SUCCESS);
    ...
    ibool read)         /*!< in: TRUE=read, FALSE=purge */
{
  ...
  if (UNIV_UNLIKELY(type != FIL_PAGE_TYPE_BLOB))
  {
    ...
    ib::fatal() << "FIL_PAGE_TYPE=" << type << " on BLOB " << (read ? "read" : "purge") << " space " << space_id
...
{
  page_t *page;
  const ulint space_id = mach_read_from_4(field_ref + BTR_EXTERN_SPACE_ID);
  const ulint start_page = mach_read_from_4(field_ref + BTR_EXTERN_PAGE_NO);
  ulint page_no;
  ulint next_page_no;
  mtr_t mtr;

  ut_ad(dict_index_is_clust(index));
  ut_ad(mtr_memo_contains_flagged(local_mtr, dict_index_get_lock(index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK) ||
        dict_table_is_intrinsic(index->table));
  ut_ad(mtr_is_page_fix(local_mtr, field_ref, MTR_MEMO_PAGE_X_FIX, index->table));
  ut_ad(!rec || rec_offs_validate(rec, index, offsets));
  ut_ad(!rec || field_ref == btr_rec_get_field_ref(rec, offsets, i));
  ut_ad(local_mtr->is_named_space(page_get_space_id(page_align(field_ref))));

  if (UNIV_UNLIKELY(!memcmp(field_ref, field_ref_zero, BTR_EXTERN_FIELD_REF_SIZE)))
  {
  ...
  ut_ad(!(mach_read_from_4(field_ref + BTR_EXTERN_LEN) & ~((BTR_EXTERN_OWNER_FLAG | BTR_EXTERN_INHERITED_FLAG) << 24)));
  ut_ad(space_id == index->space);

  const page_size_t ext_page_size(dict_table_page_size(index->table));
  const page_size_t &rec_page_size(rec == NULL ? univ_page_size : ext_page_size);
  if (rec == NULL)
  {
  ...
  for (;;)
  {
    ...
    ut_ad(!dict_table_is_temporary(index->table) || local_mtr->get_log_mode() == MTR_LOG_NO_REDO);

    const page_t *p = page_align(field_ref);

    const page_id_t page_id(page_get_space_id(p), page_get_page_no(p));
    ...
    if (/* There is no external storage data */
        ...
        || (rollback && (mach_read_from_1(field_ref + BTR_EXTERN_LEN) & BTR_EXTERN_INHERITED_FLAG)))
    {
      /* Do not free */
      mtr_commit(&mtr);
    ...
    ext_block = buf_page_get(page_id_t(space_id, page_no), ext_page_size, RW_X_LATCH, &mtr);
    ...
    if (ext_page_size.is_compressed())
    {
      ...
      btr_page_free_low(index, ext_block, ULINT_UNDEFINED, &mtr);
                              ...
                              tree */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    if (rec_offs_nth_extern(offsets, i))
    {
      btr_free_externally_stored_field(index, btr_rec_get_field_ref(rec, offsets, i), rec, offsets, page_zip, i,
                                       rollback, mtr);
    }
  }
}
                              ...
                              an X-latch to record page and to the tree */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    if (rec_offs_nth_extern(offsets, ufield->field_no))
    {
      ...
      btr_free_externally_stored_field(index, data + len - BTR_EXTERN_FIELD_REF_SIZE, rec, offsets, page_zip,
    ...
    ulint offset)   /*!< in: offset on the first BLOB page */
{
  ...
  for (;;)
  {
    ...
    block = buf_page_get(page_id_t(space_id, page_no), univ_page_size, RW_S_LATCH, &mtr);
...
@return number of bytes written to buf */
static ulint btr_copy_zblob_prefix(byte *buf, ulint len, const page_size_t &page_size, ulint space_id, ulint page_no,
                                   ulint offset)
{
  ...
  for (;;)
  {
    ...
    bpage = buf_page_get_zip(page_id_t(space_id, page_no), page_size);

    if (UNIV_UNLIKELY(!bpage))
    {
      ib::error() << "Cannot load compressed BLOB " << page_id_t(space_id, page_no);
      goto func_exit;
    }

    if (UNIV_UNLIKELY(fil_page_get_type(bpage->zip.data) != page_type))
    {
      ib::error() << "Unexpected type " << fil_page_get_type(bpage->zip.data) << " of compressed BLOB page "
    ...
    d_stream.avail_in = static_cast<uInt>(page_size.physical() - offset);
    ...
    switch (err)
    {
      ...
      default:
      inflate_error:
        ib::error() << "inflate() of compressed BLOB page " << page_id_t(space_id, page_no) << " returned " << err
                    << " (" << d_stream.msg << ")";
    ...
    if (next_page_no == FIL_NULL)
    {
      if (!d_stream.avail_in)
      {
        ib::error() << "Unexpected end of compressed "
                    << "BLOB page " << page_id_t(space_id, page_no);
      }
      else
      {
...
@param[in]	offset		offset on the first BLOB page
@return number of bytes written to buf */
static ulint btr_copy_externally_stored_field_prefix_low(byte *buf, ulint len, const page_size_t &page_size,
                                                         ulint space_id, ulint page_no, ulint offset)
{
  if (len == 0)
  {
    return (0);
  ...
  if (page_size.is_compressed())
  {
    return (btr_copy_zblob_prefix(buf, len, page_size, space_id, page_no, offset));
  }
  else
  {
...
or has been deleted */
ulint btr_copy_externally_stored_field_prefix(byte *buf, ulint len, const page_size_t &page_size, const byte *data,
                                              ulint local_len)
{
  ulint space_id;
  ulint page_no;
  ...
  return (local_len + btr_copy_externally_stored_field_prefix_low(buf + local_len, len - local_len, page_size, space_id,
                                                                  page_no, offset));
}
...
byte *btr_copy_externally_stored_field(ulint *len, const byte *data, const page_size_t &page_size, ulint local_len,
                                       mem_heap_t *heap)
{
  ulint space_id;
  ulint page_no;
  ...
  memcpy(buf, data, local_len);
  *len = local_len +
         btr_copy_externally_stored_field_prefix_low(buf + local_len, extern_len, page_size, space_id, page_no, offset);
...
byte *btr_rec_copy_externally_stored_field(const rec_t *rec, const ulint *offsets, const page_size_t &page_size,
                                           ulint no, ulint *len, mem_heap_t *heap)
{
  ulint local_len;
  const byte *data;
  ...
  if (UNIV_UNLIKELY(!memcmp(data + local_len - BTR_EXTERN_FIELD_REF_SIZE, field_ref_zero, BTR_EXTERN_FIELD_REF_SIZE)))
  {
  ...
  return (btr_copy_externally_stored_field(len, data, page_size, local_len, heap));
}
#endif /* !UNIV_HOTBACKUP */
