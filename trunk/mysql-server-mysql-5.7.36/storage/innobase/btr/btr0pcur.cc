    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  if (dict_index_is_spatial(index))
  {
    ...
    ut_ad((mtr_memo_contains_flagged(mtr, dict_index_get_lock(index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK) ||
           mtr_memo_contains(mtr, block, MTR_MEMO_PAGE_S_FIX) || mtr_memo_contains(mtr, block, MTR_MEMO_PAGE_X_FIX)) &&
  ...
  cursor->old_stored = true;
  cursor->old_rec =
      dict_index_copy_rec_order_prefix(index, rec, &cursor->old_n_fields, &cursor->old_rec_buf, &cursor->buf_size);
...
struct Btr_cur_optimistic_latch_leaves_functor_t
{
  ...
  {
    return hint != NULL && btr_cur_optimistic_latch_leaves(hint, cursor->modify_clock, &latch_mode,
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  if (UNIV_UNLIKELY(cursor->rel_pos == BTR_PCUR_AFTER_LAST_IN_TREE || cursor->rel_pos == BTR_PCUR_BEFORE_FIRST_IN_TREE))
  {
    ...
    btr_cur_open_at_index_side(cursor->rel_pos == BTR_PCUR_BEFORE_FIRST_IN_TREE, index, latch_mode,
  ...
  if ((latch_mode == BTR_SEARCH_LEAF || latch_mode == BTR_MODIFY_LEAF || latch_mode == BTR_SEARCH_PREV ||
      ...
      !dict_table_is_intrinsic(cursor->btr_cur.index->table))
  {
    ...
    if (cursor->block_when_stored.run_with_hint(functor))
    {
      ...
      if (cursor->rel_pos == BTR_PCUR_ON)
      {
        ...
        ut_ad(!cmp_rec_rec(cursor->old_rec, rec, offsets1, offsets2, index, page_is_spatial_non_leaf(rec, index)));
        mem_heap_free(heap);
#endif /* UNIV_DEBUG */
        return (TRUE);
      }
  ...
  ut_ad(cursor->rel_pos == BTR_PCUR_ON || cursor->rel_pos == BTR_PCUR_BEFORE || cursor->rel_pos == BTR_PCUR_AFTER);
  if (cursor->rel_pos == BTR_PCUR_ON && btr_pcur_is_on_user_rec(cursor) &&
      !cmp_dtuple_rec(tuple, btr_pcur_get_rec(cursor),
                      rec_get_offsets(btr_pcur_get_rec(cursor), index, NULL, ULINT_UNDEFINED, &heap)))
  {
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  next_block = btr_block_get(page_id_t(block->page.id.space(), next_page_no), block->page.size, mode,
  ...
  if (!cursor->import_ctx)
  {
    ...
    ut_a(btr_page_get_prev(next_page, mtr) == btr_pcur_get_block(cursor)->page.id.page_no());
  }
  else
  {
    ...
    DBUG_EXECUTE_IF("ib_import_page_corrupt", cursor->import_ctx->is_error = true;);
  }
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  if (!dict_table_is_intrinsic(btr_cur_get_index(btr_pcur_get_btr_cur(cursor))->table))
  {
