...
{
  ib::error() << "Flag mismatch in page " << block->page.id << " index " << index->name << " of table "
              << index->table->name;
}
    ...
    const dict_index_t *index) /*!< in: index tree */
{
  ...
  else if (dict_index_is_clust(index))
  {
    ...
    else
    {
      ...
      flags match. */
      ulint flags = dict_tf_to_fsp_flags(table->flags, false, dict_table_is_encrypted(table));
  ...
  if (err == DB_SUCCESS &&
      (!btr_root_fseg_adjust_on_import(FIL_PAGE_DATA + PAGE_BTR_SEG_LEAF + page, page_zip, space_id, &mtr) ||
       !btr_root_fseg_adjust_on_import(FIL_PAGE_DATA + PAGE_BTR_SEG_TOP + page, page_zip, space_id, &mtr)))
  {
    err = DB_CORRUPTION;
  }
    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  {
    page_create(block, mtr, dict_table_is_comp(index->table), dict_index_is_spatial(index));
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  new_block = buf_page_get(page_id_t(dict_index_get_space(index), node_addr.page), dict_table_page_size(index->table),
                           RW_X_LATCH, mtr);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) buf_block_t *btr_page_alloc_low(
    /*===============*/
    dict_index_t *index, /*!< in: index */
                         ...
                         not initialize the page. */
{
  ...
  return (fseg_alloc_free_page_general(seg_header, hint_page_no, file_direction, TRUE, mtr, init_mtr));
}
                         ...
                         is s-latched */
{
  ...
  ut_ad(srv_read_only_mode || mtr_memo_contains(mtr, dict_index_get_lock(index), MTR_MEMO_S_LOCK) ||
        dict_table_is_intrinsic(index->table));
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  flst_add_first(root + PAGE_HEADER + PAGE_BTR_IBUF_FREE_LIST,
                 buf_block_get_frame(block) + PAGE_HEADER + PAGE_BTR_IBUF_FREE_LIST_NODE, mtr);
    ...
    mtr_t *mtr)            /*!< in: mtr */
{
  ...
  const page_id_t page_id(page_get_space_id(page_align(node_ptr)), btr_node_ptr_get_child_page_no(node_ptr, offsets));

  return (btr_block_get(page_id, dict_table_page_size(index->table), RW_SX_LATCH, index, mtr));
}
    ...
    mtr_t *mtr)        /*!< in: mtr */
{
  ...
  ut_ad(latch_mode == BTR_CONT_MODIFY_TREE || latch_mode == BTR_CONT_SEARCH_TREE);
  ...
  {
    btr_cur_search_to_nth_level_with_no_latch(index, level + 1, tuple, PAGE_CUR_LE, cursor, file, line, mtr);
  }
  else
  {
    btr_cur_search_to_nth_level(index, level + 1, tuple, PAGE_CUR_LE, latch_mode, cursor, 0, file, line, mtr);
  }
  ...
  if (btr_node_ptr_get_child_page_no(node_ptr, offsets) != page_no)
  {
    ...
    ib::error() << "Corruption of an index tree: table " << index->table->name << " index " << index->name
                << ", father ptr page no " << btr_node_ptr_get_child_page_no(node_ptr, offsets) << ", child page no "
                << page_no;
    ...
    ib::fatal() << "You should dump + drop + reimport the table to"
                ...
                << " database startup. " << FORCE_RECOVERY_MSG << " Then dump + drop + reimport.";
  }
...
#define btr_page_get_father_node_ptr(of, heap, cur, mtr) \
  btr_page_get_father_node_ptr_func(of, heap, cur, BTR_CONT_MODIFY_TREE, __FILE__, __LINE__, mtr)

#define btr_page_get_father_node_ptr_for_validate(of, heap, cur, mtr) \
  btr_page_get_father_node_ptr_func(of, heap, cur, BTR_CONT_SEARCH_TREE, __FILE__, __LINE__, mtr)
...
static void btr_free_root_invalidate(buf_block_t *block, mtr_t *mtr)
{
  ...
  btr_page_set_index_id(buf_block_get_frame(block), buf_block_get_page_zip(block), BTR_FREED_INDEX_ID, mtr);
}
...
static MY_ATTRIBUTE((warn_unused_result)) buf_block_t *btr_free_root_check(const page_id_t &page_id,
...
ulint btr_create(ulint type, ulint space, const page_size_t &page_size, index_id_t index_id, dict_index_t *index,
                 const btr_create_t *btr_redo_create_info, mtr_t *mtr)
{
  ulint page_no;
  buf_block_t *block;
  buf_frame_t *frame;
  ...
  if (type & DICT_IBUF)
  {
    ...
    block = fseg_alloc_free_page(buf_block_get_frame(ibuf_hdr_block) + IBUF_HEADER + IBUF_TREE_SEG_HEADER,
  ...
  if (page_zip)
  {
    ...
    else
    {
      ...
      page_comp_info.n_fields = btr_redo_create_info->n_fields;

      page_comp_info.field_len = btr_redo_create_info->field_len;

      page_comp_info.fields = btr_redo_create_info->fields;

      page_comp_info.trx_id_pos = btr_redo_create_info->trx_id_pos;

      page = page_create_zip(block, NULL, 0, 0, &page_comp_info, mtr);
    }
  }
  else
  {
    if (index != NULL)
    {
      page = page_create(block, mtr, dict_table_is_comp(index->table), dict_index_is_spatial(index));
    }
    else
    {
      ut_ad(btr_redo_create_info != NULL);
      page = page_create(block, mtr, btr_redo_create_info->format_flags, type == DICT_SPATIAL);
    }
    /* Set the level of the new index page */
  ...
  if (!(type & DICT_CLUSTERED) && (index == NULL || !dict_table_is_temporary(index->table)))
  {
...
#ifdef UNIV_BTR_DEBUG
  ut_a(btr_root_fseg_validate(FIL_PAGE_DATA + PAGE_BTR_SEG_LEAF + root, block->page.id.space()));
  ut_a(btr_root_fseg_validate(FIL_PAGE_DATA + PAGE_BTR_SEG_TOP + root, block->page.id.space()));
#endif /* UNIV_BTR_DEBUG */
...
top_loop:
  ...
  finished = fseg_free_step_not_header(root + PAGE_HEADER + PAGE_BTR_SEG_TOP, true, &mtr);
  mtr_commit(&mtr);
...
@param[in]	index_id	PAGE_INDEX_ID contents
@param[in,out]	mtr		mini-transaction */
void btr_free_if_exists(const page_id_t &page_id, const page_size_t &page_size, index_id_t index_id, mtr_t *mtr)
{
    ...
    mtr_t *mtr)          /*!< in/out: mini-transaction */
{
  ...
  is_spatial = (fil_page_get_type(page) == FIL_PAGE_RTREE || dict_index_is_spatial(index));
  ...
  for MVCC. */
  if (dict_index_is_sec_or_ibuf(index) && page_is_leaf(page) && !dict_table_is_temporary(index->table))
  {
  ...
  if (page_zip && !page_zip_compress(page_zip, page, index, z_level, NULL, mtr))
  {
    /* Restore the old page and exit. */
#if defined UNIV_DEBUG || defined UNIV_ZIP_DEBUG
    /* Check that the bytes that we skip are identical. */
    ut_a(!memcmp(page, temp_page, PAGE_HEADER));
    ut_a(!memcmp(PAGE_HEADER + PAGE_N_RECS + page, PAGE_HEADER + PAGE_N_RECS + temp_page,
                 PAGE_DATA - (PAGE_HEADER + PAGE_N_RECS)));
    ut_a(!memcmp(UNIV_PAGE_SIZE - FIL_PAGE_DATA_END + page, UNIV_PAGE_SIZE - FIL_PAGE_DATA_END + temp_page,
                 FIL_PAGE_DATA_END));
  ...
  {
    ib::error() << "Page old data size " << data_size1 << " new data size " << data_size2 << ", page old max ins size "
                << max_ins_size1 << " new max ins size " << max_ins_size2;
...
func_exit:
  ...
  if (success)
  {
    ...
    log_ptr = log_compressed ? NULL : mlog_open_and_write_index(mtr, page, index, type, page_zip ? 1 : 0);
...
/*************************************************************/ /**
 ...
 @retval true if the operation was successful
 @retval false if it is a compressed page, and recompression failed */
static MY_ATTRIBUTE((nonnull)) bool btr_page_reorganize_block(
    /*======================*/
    bool recovery,       /*!< in: true if called in recovery:
                        locks should not be updated, i.e.,
                        there cannot exist locks on the
    ...
    mtr_t *mtr)            /*!< in: mtr */
{
  ...
  if (0
      ...
      || new_page_zip
#endif /* UNIV_ZIP_COPY */
      || !page_copy_rec_list_end(new_block, root_block, page_get_infimum_rec(root), index, mtr))
  {
    ut_a(new_page_zip);
  ...
  if (dict_index_is_spatial(index))
  {
    ...
    node_ptr = rtr_index_build_node_ptr(index, &new_mbr, rec, new_page_no, *heap, level);
  }
  else
  {
  ...
  node_ptr_rec = page_cur_tuple_insert(page_cursor, node_ptr, index, offsets, heap, 0, mtr);
  ...
  if (!dict_index_is_clust(index) && !dict_table_is_temporary(index->table))
  {
  ...
  if (dict_index_is_spatial(index))
  {
    ...
    return (rtr_page_split_and_insert(flags, cursor, offsets, heap, tuple, n_ext, mtr));
  }
  else
  {
    return (btr_page_split_and_insert(flags, cursor, offsets, heap, tuple, n_ext, mtr));
  }
}
    ...
    ulint n_ext)           /*!< in: number of externally stored columns */
{
  ...
  if (page_zip)
  {
    ...
    ulint free_space_zip = page_zip_empty_size(cursor->index->n_fields, page_zip_get_size(page_zip));
...
/*************************************************************/ /**
 ...
 chosen split_rec.
 @return true if fits */
static MY_ATTRIBUTE((nonnull(1, 3, 4, 6), warn_unused_result)) bool btr_page_insert_fits(
    /*=================*/
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  if (!dict_index_is_spatial(index))
  {
    if (dict_table_is_intrinsic(index->table))
    {
      btr_cur_search_to_nth_level_with_no_latch(index, level, tuple, PAGE_CUR_LE, &cursor, __FILE__, __LINE__, mtr);
    }
    else
    {
      btr_cur_search_to_nth_level(index, level, tuple, PAGE_CUR_LE, BTR_CONT_MODIFY_TREE, &cursor, 0, file, line, mtr);
    }
  }
  else
  {
    ...
    btr_cur_search_to_nth_level(index, level, tuple, PAGE_CUR_RTREE_INSERT, BTR_CONT_MODIFY_TREE, &cursor, 0, file,
                                line, mtr);
  }

  ut_ad(cursor.flag == BTR_CUR_BINARY);

  err = btr_cur_optimistic_insert(flags | BTR_NO_LOCKING_FLAG | BTR_KEEP_SYS_FLAG | BTR_NO_UNDO_LOG_FLAG, &cursor,
                                  &offsets, &heap, tuple, &rec, &dummy_big_rec, 0, NULL, mtr);

  if (err == DB_FAIL)
  {
    err = btr_cur_pessimistic_insert(flags | BTR_NO_LOCKING_FLAG | BTR_KEEP_SYS_FLAG | BTR_NO_UNDO_LOG_FLAG, &cursor,
                                     &offsets, &heap, tuple, &rec, &dummy_big_rec, 0, NULL, mtr);
    ut_a(err == DB_SUCCESS);
  }
...
/**************************************************************/ /**
 ...
 index tree. */
static MY_ATTRIBUTE((nonnull)) void btr_attach_half_pages(
    /*==================*/
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
  ...
  if (direction == FSP_DOWN)
  {
    ...
    btr_node_ptr_set_child_page_no(btr_cur_get_rec(&cursor), btr_cur_get_page_zip(&cursor), offsets, lower_page_no,
                                   mtr);
    mem_heap_empty(heap);
  }
  else
  ...
  {
    prev_block = btr_block_get(page_id_t(space, prev_page_no), block->page.size, RW_X_LATCH, index, mtr);
  }
  if (next_page_no != FIL_NULL && direction != FSP_DOWN)
  {
    next_block = btr_block_get(page_id_t(space, next_page_no), block->page.size, RW_X_LATCH, index, mtr);
  }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) bool btr_page_tuple_smaller(
    /*===================*/
...
static rec_t *btr_insert_into_right_sibling(ulint flags, btr_cur_t *cursor, ulint **offsets, mem_heap_t *heap,
                                            const dtuple_t *tuple, ulint n_ext, mtr_t *mtr)
{
  ...
  ut_ad(dict_table_is_intrinsic(cursor->index->table) ||
        mtr_memo_contains_flagged(mtr, dict_index_get_lock(cursor->index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK));
  ut_ad(mtr_is_block_fix(mtr, block, MTR_MEMO_PAGE_X_FIX, cursor->index->table));
  ut_ad(heap);

  if (next_page_no == FIL_NULL || !page_rec_is_supremum(page_rec_get_next(btr_cur_get_rec(cursor))))
  {
    return (NULL);
  }
  ...
  next_block = btr_block_get(page_id_t(space, next_page_no), block->page.size, RW_X_LATCH, cursor->index, mtr);
  ...
  rec = page_cur_tuple_insert(&next_page_cursor, tuple, cursor->index, offsets, &heap, n_ext, mtr);

  if (rec == NULL)
  {
    if (is_leaf && next_block->page.size.is_compressed() && !dict_index_is_clust(cursor->index) &&
  ...
  *btr_cur_get_page_cur(cursor) = next_page_cursor;
  ...
  compressed = btr_cur_pessimistic_delete(&err, TRUE, &next_father_cursor, BTR_CREATE_FLAG, false, mtr);
  ...
  dtuple_t *node_ptr = dict_index_build_node_ptr(cursor->index, rec, next_block->page.id.page_no(), heap, level);

  btr_insert_on_non_leaf_level(flags, cursor->index, level + 1, node_ptr, mtr);

  ut_ad(rec_offs_validate(rec, cursor->index, *offsets));

  if (is_leaf && !dict_index_is_clust(cursor->index) && !dict_table_is_temporary(cursor->index->table))
  {
    ...
    {
      ibuf_update_free_bits_if_full(next_block, max_size, rec_offs_size(*offsets) + PAGE_DIR_SLOT_SIZE);
    }
  }
...
func_start:
  ...
  *offsets = NULL;
  ...
  ut_ad(!dict_index_is_online_ddl(cursor->index) || (flags & BTR_CREATE_FLAG) || dict_index_is_clust(cursor->index));
  ut_ad(rw_lock_own_flagged(dict_index_get_lock(cursor->index), RW_LOCK_FLAG_X | RW_LOCK_FLAG_SX) ||
        dict_table_is_intrinsic(cursor->index->table));
  ...
  rec = btr_insert_into_right_sibling(flags, cursor, offsets, *heap, tuple, n_ext, mtr);
  ...
  new_block = btr_page_alloc(cursor->index, hint_page_no, direction, btr_page_get_level(page, mtr), mtr, mtr);
  ...
  insert_empty:
    ut_ad(!split_rec);
    ut_ad(!insert_left);
    buf = UT_NEW_ARRAY_NOKEY(byte, rec_get_converted_size(cursor->index, tuple, n_ext));
  ...
  else
  {
    ...
    insert_will_fit = !new_page_zip && btr_page_insert_fits(cursor, NULL, offsets, tuple, n_ext, heap);
  }

  if (!srv_read_only_mode && !dict_table_is_intrinsic(cursor->index->table) && insert_will_fit && page_is_leaf(page) &&
  ...
  if (direction == FSP_DOWN)
  {
        ...
        || !page_move_rec_list_start(new_block, block, move_limit, cursor->index, mtr))
    {
      ...
      page_delete_rec_list_end(move_limit - page + new_page, new_block, cursor->index, ULINT_UNDEFINED, ULINT_UNDEFINED,
                               mtr);
...
#ifdef UNIV_ZIP_DEBUG
  {
    ...
    ut_a(!insert_page_zip || page_zip_validate(insert_page_zip, insert_page, cursor->index));
  }
#endif /* UNIV_ZIP_DEBUG */
...
func_exit:
  ...
  if (!dict_index_is_clust(cursor->index) && !dict_table_is_temporary(cursor->index->table) && page_is_leaf(page))
  {
...
@param[in]	index		index tree
@param[in,out]	mtr		mini-transaction */
static void btr_level_list_remove_func(ulint space, const page_size_t &page_size, page_t *page,
                                       const dict_index_t *index, mtr_t *mtr)
{
  ...
  {
    buf_block_t *prev_block = btr_block_get(page_id_t(space, prev_page_no), page_size, RW_X_LATCH, index, mtr);
  ...
  {
    buf_block_t *next_block = btr_block_get(page_id_t(space, next_page_no), page_size, RW_X_LATCH, index, mtr);
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  compressed = btr_cur_pessimistic_delete(&err, TRUE, &cursor, BTR_CREATE_FLAG, false, mtr);
  ut_a(err == DB_SUCCESS);
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
...

  {
    btr_cur_t cursor;
    ulint *offsets = NULL;
    mem_heap_t *heap = mem_heap_create(sizeof(*offsets) * (REC_OFFS_HEADER_SIZE + 1 + 1 + index->n_fields));
    buf_block_t *b;

    if (dict_index_is_spatial(index))
    {
      offsets = rtr_page_get_father_block(NULL, heap, index, block, mtr, NULL, &cursor);
    }
    else
    {
    ...
    for (b = father_block; b->page.id.page_no() != root_page_no;)
    {
      ...
      if (dict_index_is_spatial(index))
      {
        offsets = rtr_page_get_father_block(NULL, heap, index, b, mtr, NULL, &cursor);
      }
      else
      {
  ...
  if (0
      ...
      || father_page_zip
#endif /* UNIV_ZIP_COPY */
      || !page_copy_rec_list_end(father_block, block, page_get_infimum_rec(page), index, mtr))
  {
    ...
    mtr_t *mtr)        /*!< in/out: mini-transaction */
{
  ...
  if (dict_index_is_spatial(index))
  {
    ut_ad(mtr_memo_contains_flagged(mtr, dict_index_get_lock(index), MTR_MEMO_X_LOCK));
  }
  else
  {
    ut_ad(mtr_memo_contains_flagged(mtr, dict_index_get_lock(index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK) ||
  ...
  if (dict_index_is_spatial(index))
  {
    offsets = rtr_page_get_father_block(NULL, heap, index, block, mtr, cursor, &father_cursor);
    ut_ad(cursor->page_cur.block->page.id.page_no() == block->page.id.page_no());
    ...
    {
      ib::info() << "father positioned on page " << page_no << "instead of " << block->page.id.page_no();
      offsets = btr_page_get_father_block(NULL, heap, index, block, mtr, &father_cursor);
    }
  }
  else
  {
  ...
  ut_d(leftmost_child =
           left_page_no != FIL_NULL && (page_rec_get_next(page_get_infimum_rec(btr_cur_get_page(&father_cursor))) ==
...
retry:
  ...
  if (is_left)
  {
    ...
    if (dict_index_is_spatial(index))
    {
      ...
      page */
      if (!rtr_check_same_block(index, &cursor2, btr_cur_get_block(&father_cursor), merge_block, heap))
      {
        is_left = false;
        goto retry;
      }
      ...
      offsets2 = rec_get_offsets(btr_cur_get_rec(&cursor2), index, NULL, ULINT_UNDEFINED, &heap);

      /* Check if parent entry needs to be updated */
      mbr_changed =
          rtr_merge_mbr_changed(&cursor2, &father_cursor, offsets2, offsets, &new_mbr, merge_block, block, index);
    }

    rec_t *orig_pred = page_copy_rec_list_start(merge_block, block, page_get_supremum_rec(page), index, mtr);
    ...
    if (dict_index_is_spatial(index))
    {
      ...
      if (page_no != block->page.id.page_no())
      {
        ib::fatal() << "father positioned on " << page_no << " instead of " << block->page.id.page_no();
      ...
      {
#ifdef UNIV_DEBUG
        bool success = rtr_update_mbr_field(&cursor2, offsets2, &father_cursor, merge_page, &new_mbr, NULL, mtr);

        ut_ad(success);
#else
        rtr_update_mbr_field(&cursor2, offsets2, &father_cursor, merge_page, &new_mbr, NULL, mtr);
#endif
      }
      else
      {
...
#error "FIL_NULL != 0xffffffff"
    ...
    ut_ad(btr_node_ptr_get_child_page_no(btr_cur_get_rec(&father_cursor), offsets) == block->page.id.page_no());

    /* Replace the address of the old child node (= page) with the
    address of the merge page to the right */
    btr_node_ptr_set_child_page_no(btr_cur_get_rec(&father_cursor), btr_cur_get_page_zip(&father_cursor), offsets,
                                   right_page_no, mtr);
    ...
    if (!page_is_leaf(page) && left_page_no == FIL_NULL)
    {
      ut_ad(REC_INFO_MIN_REC_FLAG &
            rec_get_info_bits(page_rec_get_next(page_get_infimum_rec(buf_block_get_frame(merge_block))),
                              page_is_comp(page)));
    }
#endif /* UNIV_DEBUG */
    ...
    if (dict_index_is_spatial(index))
    {
      ...
      ut_ad(btr_node_ptr_get_child_page_no(btr_cur_get_rec(&cursor2), offsets2) == right_page_no);

      rec_info = rec_get_info_bits(btr_cur_get_rec(&father_cursor), rec_offs_comp(offsets));
      if (rec_info & REC_INFO_MIN_REC_FLAG)
      {
        /* When the father node ptr is minimal rec,
        we will keep it and delete the node ptr of
        merge page. */
        rtr_merge_and_update_mbr(&father_cursor, &cursor2, offsets, offsets2, merge_page, merge_block, block, index,
                                 mtr);
      }
      else
      {
        ...
        level. */
        rtr_merge_and_update_mbr(&cursor2, &father_cursor, offsets2, offsets, merge_page, merge_block, block, index,
                                 mtr);
      }
      lock_mutex_enter();
      lock_prdt_page_free_from_discard(block, lock_sys->prdt_page_hash);
    ...
    {
      compressed = btr_cur_pessimistic_delete(&err, TRUE, &cursor2, BTR_CREATE_FLAG, false, mtr);
      ut_a(err == DB_SUCCESS);
  ...
  if (!dict_index_is_clust(index) && !dict_table_is_temporary(index->table) && page_is_leaf(merge_page))
  {
    /* Update the free bits of the B-tree page in the
...
func_exit:
  ...
  {
    ut_ad(nth_rec > 0);
    btr_cur_position(index, page_rec_get_nth(merge_block->frame, nth_rec), merge_block, cursor);
  }
...
err_exit:
  ...
  if (page_size.is_compressed() && merge_page && page_is_leaf(merge_page) && !dict_index_is_clust(index))
  {
    ibuf_reset_free_bits(merge_block);
  }
    ...
    mtr_t *mtr)        /*!< in: mtr */
{
  ...
  if (left_page_no != FIL_NULL)
  {
    merge_block = btr_block_get(page_id_t(space, left_page_no), page_size, RW_X_LATCH, index, mtr);

    merge_page = buf_block_get_frame(merge_block);
#ifdef UNIV_BTR_DEBUG
    ut_a(btr_page_get_next(merge_page, mtr) == block->page.id.page_no());
#endif /* UNIV_BTR_DEBUG */
    ut_d(parent_is_different = (page_rec_get_next(page_get_infimum_rec(btr_cur_get_page(&parent_cursor))) ==
                                btr_cur_get_rec(&parent_cursor)));
  }
  else if (right_page_no != FIL_NULL)
  {
    merge_block = btr_block_get(page_id_t(space, right_page_no), page_size, RW_X_LATCH, index, mtr);

    merge_page = buf_block_get_frame(merge_block);
#ifdef UNIV_BTR_DEBUG
    ut_a(btr_page_get_prev(merge_page, mtr) == block->page.id.page_no());
#endif /* UNIV_BTR_DEBUG */
    ut_d(parent_is_different = page_rec_is_supremum(page_rec_get_next(btr_cur_get_rec(&parent_cursor))));
  }
  else
  {
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  ib::info() << "NODE ON LEVEL " << btr_page_get_level(page, mtr) << " page " << block->page.id;
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  tuple = dict_index_build_node_ptr(index, page_rec_get_next(page_get_infimum_rec(page)), 0, heap,
  ...
  if (dict_index_is_spatial(index))
  {
    ut_a(!cmp_dtuple_rec_with_gis(tuple, btr_cur_get_rec(&cursor), offsets, PAGE_CUR_WITHIN));
  }
  else
  {
...
{
  ib::info() << "Record in index " << index->name << " of table " << index->table->name << ", page "
             << page_id_t(page_get_space_id(page), page_get_page_no(page)) << ", at offset " << page_offset(rec);
}
                               ...
                               and page on error */
{
  ...
  if ((ibool) !!page_is_comp(page) != dict_table_is_comp(index->table))
  {
    ...
    ib::error() << "Compact flag=" << !!page_is_comp(page) << ", should be " << dict_table_is_comp(index->table);
  ...
  if (!page_is_comp(page) && (rec_get_n_fields_old(rec) != n
                              ...
                              && !(index->id == DICT_INDEXES_ID && rec_get_n_fields_old(rec) == n - 1)))
  {
    btr_index_rec_validate_report(page, rec, index);

    ib::error() << "Has " << rec_get_n_fields_old(rec) << " fields, should have " << n;
  ...
  for (i = 0; i < n; i++)
  {
    ...
    ulint fixed_size = dict_col_get_fixed_size(dict_field_get_col(field), page_is_comp(page));
    ...
    if (dict_field_get_col(field)->mtype == DATA_POINT)
    {
      ...
      if (dict_index_is_spatial(index))
      {
        ...
        ut_ad((field->fixed_len == DATA_MBR_LEN && i == 0) || (field->fixed_len == DATA_POINT_LEN && i != 0));
        fixed_size = field->fixed_len;
      }
    }

    if ((field->prefix_len == 0 && len != UNIV_SQL_NULL && fixed_size && len != fixed_size) ||
        (field->prefix_len > 0 && len != UNIV_SQL_NULL && len > field->prefix_len))
    {
      btr_index_rec_validate_report(page, rec, index);

      ib::error error;

      error << "Field " << i << " len is " << len << ", should be " << fixed_size;

      if (dump_on_error)
      {
        error << "; ";
        rec_print(error.m_oss, rec, rec_get_info_bits(rec, rec_offs_comp(offsets)), offsets);
      }
      if (heap)
      {
        mem_heap_free(heap);
    ...
    dict_index_t *index) /*!< in: index */
{
  ...
  DBUG_EXECUTE_IF("check_table_rec_next", ut_a(page_rec_get_nth_const(page_cur_get_page(&cur), 0) == cur.rec);
                  ut_a(page_dir_slot_get_n_owned(page_dir_get_nth_slot(page_cur_get_page(&cur), 0)) == 1););
  ...
  for (;;)
  {
    ...
    DBUG_EXECUTE_IF(
        ...
        ut_a(cur.rec == page_rec_get_nth_const(page_cur_get_page(&cur), page_rec_get_n_recs_before(cur.rec)));
...
{
  ib::error error;
  error << "In page " << block->page.id.page_no() << " of index " << index->name << " of table " << index->table->name;
...
{
  ib::error error;
  error << "In pages " << block1->page.id << " and " << block2->page.id << " of index " << index->name << " of table "
        << index->table->name;
    ...
    bool lockout)        /*!< in: true if X-latch index is intended */
{
  ...
  {
    ib::warn() << "Flags mismatch: table=" << index->table->flags << ", tablespace=" << space->flags;
  ...
  while (level != btr_page_get_level(page, &mtr))
  {
    ...
    if (fseg_page_is_free(seg, block->page.id.space(), block->page.id.page_no()))
    {
    ...
    if (dict_index_is_spatial(index))
    {
      ...
      while (left_page_no != FIL_NULL)
      {
        ...
        savepoint2 = mtr_set_savepoint(&mtr);
        block = btr_block_get(left_page_id, table_page_size, RW_SX_LATCH, index, &mtr);
        page = buf_block_get_frame(block);
        left_page_no = btr_page_get_prev(page, &mtr);
      }
    }
...
loop:
  ...
  {
    ib::error() << "Page index id " << btr_page_get_index_id(page) << " != data dictionary index id " << index->id;
  ...
  ut_a(!page_is_empty(page) || (level == 0 && page_get_page_no(page) == dict_index_get_page(index)));
  ...
  if (right_page_no != FIL_NULL)
  {
    ...
    right_block = btr_block_get(page_id_t(index->space, right_page_no), table_page_size, RW_SX_LATCH, index, &mtr);
    ...
    if (!dict_index_is_spatial(index) && cmp_rec_rec(rec, right_rec, offsets, offsets2, index, false) >= 0)
    {
      btr_validate_report2(index, level, block, right_block);
  ...
  if (!dict_index_is_spatial(index) && block->page.id.page_no() != dict_index_get_page(index))
  {
    /* Check father node pointers */
    rec_t *node_ptr;

    btr_cur_position(index, page_rec_get_next(page_get_infimum_rec(page)), block, &node_cur);
    offsets = btr_page_get_father_node_ptr_for_validate(offsets, heap, &node_cur, &mtr);
    ...
    btr_cur_position(index, page_rec_get_prev(page_get_supremum_rec(page)), block, &node_cur);

    offsets = btr_page_get_father_node_ptr_for_validate(offsets, heap, &node_cur, &mtr);

    if (node_ptr != btr_cur_get_rec(&node_cur) ||
        btr_node_ptr_get_child_page_no(node_ptr, offsets) != block->page.id.page_no())
    {
      btr_validate_report1(index, level, block);
    ...
    if (!page_is_leaf(page))
    {
      node_ptr_tuple = dict_index_build_node_ptr(index, page_rec_get_next(page_get_infimum_rec(page)), 0, heap,
                                                 btr_page_get_level(page, &mtr));
    ...
    else
    {
      ...
      if (!lockout && rightmost_child)
      {
        ...
        btr_block_get(page_id_t(index->space, parent_right_page_no), table_page_size, RW_SX_LATCH, index, &mtr);

        right_block = btr_block_get(page_id_t(index->space, right_page_no), table_page_size, RW_SX_LATCH, index, &mtr);
      }

      btr_cur_position(index, page_rec_get_next(page_get_infimum_rec(buf_block_get_frame(right_block))), right_block,
                       &right_node_cur);

      offsets = btr_page_get_father_node_ptr_for_validate(offsets, heap, &right_node_cur, &mtr);
...
node_ptr_fails:
  ...
  else if (right_page_no != FIL_NULL)
  {
    ...
    if (!lockout)
    {
      ...
      {
        btr_block_get(page_id_t(index->space, parent_page_no), table_page_size, RW_SX_LATCH, index, &mtr);
      }
    }

    block = btr_block_get(page_id_t(index->space, right_page_no), table_page_size, RW_SX_LATCH, index, &mtr);
...
bool btr_validate_spatial_index(
    /*=======================*/
    dict_index_t *index, /*!< in: index */
    const trx_t *trx)    /*!< in: transaction or NULL */
{
    ...
    mtr_t *mtr)                /*!< in: mini-transaction */
{
  ...
  if (page_size.is_compressed() && page_is_leaf(mpage) &&
      (page_get_data_size(mpage) + data_size >= dict_index_zip_pad_optimal_page_size(index)))
  {
    goto error;
  }
