...
dberr_t PageBulk::init()
{
  ...
  if (m_page_no == FIL_NULL)
  {
    ...
    ulint n_reserved;
    bool success;
    success = fsp_reserve_free_extents(&n_reserved, m_index->space, 1, FSP_NORMAL, &alloc_mtr);
    if (!success)
    {
    ...
    if (new_page_zip)
    {
      page_create_zip(new_block, m_index, m_level, 0, NULL, mtr);
    }
    else
    {
      ut_ad(!dict_index_is_spatial(m_index));
      page_create(new_block, mtr, dict_table_is_comp(m_index->table), false);
  ...
  if (dict_index_is_sec_or_ibuf(m_index) && !dict_table_is_temporary(m_index->table) && page_is_leaf(new_page))
  {
  ...
  m_padding_space = UNIV_PAGE_SIZE - dict_index_zip_pad_optimal_page_size(m_index);
...
void PageBulk::insert(const rec_t *rec, ulint *offsets)
{
  ...
  if (!page_rec_is_infimum(m_cur_rec))
  {
    ...
    ulint *old_offsets = rec_get_offsets(old_rec, m_index, NULL, ULINT_UNDEFINED, &m_heap);

    ut_ad(cmp_rec_rec(rec, old_rec, offsets, old_offsets, m_index, page_is_spatial_non_leaf(old_rec, m_index)) > 0);
  }
  ...
  ulint slot_size;
  slot_size = page_dir_calc_reserved_space(m_rec_no + 1) - page_dir_calc_reserved_space(m_rec_no);
...
#ifdef UNIV_DEBUG
  ut_ad(m_total_data + page_dir_calc_reserved_space(m_rec_no) <= page_get_free_space_of_empty(m_is_comp));
...
void PageBulk::commit(bool success)
{
  if (success)
  {
    ...
    if (!dict_index_is_clust(m_index) && !dict_table_is_temporary(m_index->table) && page_is_leaf(m_page))
    {
...
bool PageBulk::compress()
{
  ...
  return (page_zip_compress(m_page_zip, m_page, m_index, page_zip_level, NULL, m_mtr));
}
...
dtuple_t *PageBulk::getNodePtr()
{
  ...
  node_ptr = dict_index_build_node_ptr(m_index, first_rec, m_page_no, m_heap, m_level);
...
rec_t *PageBulk::getSplitRec()
{
  ...
  do
  {
    ...
    n_recs++;
  } while (total_recs_size + page_dir_calc_reserved_space(n_recs) < total_used_size / 2);
...
void PageBulk::copyOut(rec_t *split_rec)
{
  ...
  offsets = rec_get_offsets(last_rec, m_index, offsets, ULINT_UNDEFINED, &(m_heap));

  m_free_space += rec_get_end(last_rec, offsets) - m_heap_top + page_dir_calc_reserved_space(m_rec_no) -
...
void PageBulk::setNext(ulint next_page_no) { btr_page_set_next(m_page, NULL, next_page_no, m_mtr); }

/** Set previous page
@param[in]	prev_page_no	previous page no */
void PageBulk::setPrev(ulint prev_page_no) { btr_page_set_prev(m_page, NULL, prev_page_no, m_mtr); }
...
bool PageBulk::isSpaceAvailable(ulint rec_size)
{
  ...
  growing too high. */
  if (m_rec_no >= 2 && ((m_page_zip == NULL && m_free_space - required_space < m_reserved_space) ||
...
{
  return (page_zip_rec_needs_ext(rec_size, m_is_comp, dtuple_get_n_fields(tuple), m_block->page.size));
}
...
dberr_t PageBulk::storeExt(const big_rec_t *big_rec, ulint *offsets)
{
  ...
  dberr_t err = btr_store_big_rec_extern_fields(&btr_pcur, NULL, offsets, big_rec, m_mtr, BTR_STORE_INSERT_BULK);
...
void PageBulk::latch()
{
  ...
  ret = buf_page_optimistic_get(RW_X_LATCH, m_block, m_modify_clock, __FILE__, __LINE__, m_mtr);
  ...
  if (!ret)
  {
    ...
    m_block = buf_page_get_gen(page_id, page_size, RW_X_LATCH, m_block, BUF_GET_IF_IN_POOL, __FILE__, __LINE__, m_mtr);
    ut_ad(m_block != NULL);
  }
...
bool PageBulk::isIndexXLocked()
{
  return (dict_index_is_online_ddl(m_index) &&
          mtr_memo_contains_flagged(m_mtr, dict_index_get_lock(m_index), MTR_MEMO_X_LOCK | MTR_MEMO_SX_LOCK));
}
#endif  // UNIV_DEBUG
...
dberr_t BtrBulk::pageSplit(PageBulk *page_bulk, PageBulk *next_page_bulk)
{
  ...
  /* 2. create a new page. */
  PageBulk new_page_bulk(m_index, m_trx_id, FIL_NULL, page_bulk->getLevel(), m_flush_observer);
...
@return	error code */
dberr_t BtrBulk::pageCommit(PageBulk *page_bulk, PageBulk *next_page_bulk, bool insert_father)
{
  page_bulk->finish();
...
dberr_t BtrBulk::insert(dtuple_t *tuple, ulint level)
{
  ...
  {
    PageBulk *new_page_bulk = UT_NEW_NOKEY(PageBulk(m_index, m_trx_id, FIL_NULL, level, m_flush_observer));
  ...
  if (is_left_most && level > 0 && page_bulk->getRecNo() == 0)
  {
    ...
    dtuple_set_info_bits(tuple, dtuple_get_info_bits(tuple) | REC_INFO_MIN_REC_FLAG);
  }
  ...
  if (!page_bulk->isSpaceAvailable(rec_size))
  {
    ...
    sibling_page_bulk = UT_NEW_NOKEY(PageBulk(m_index, m_trx_id, FIL_NULL, level, m_flush_observer));
  ...
  rec = rec_convert_dtuple_to_rec(static_cast<byte *>(mem_heap_alloc(page_bulk->m_heap, rec_size)), m_index, tuple,
                                  n_ext);
  offsets = rec_get_offsets(rec, m_index, offsets, ULINT_UNDEFINED, &(page_bulk->m_heap));
