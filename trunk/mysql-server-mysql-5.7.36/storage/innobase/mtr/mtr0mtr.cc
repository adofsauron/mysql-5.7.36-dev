...
{
  /** Constructor */
  Find(const void *object, ulint type) : m_slot(), m_type(type), m_object(object) { ut_a(object != NULL); }
...
struct FindPage
{
  ...
  FindPage(const void *ptr, ulint flags) : m_ptr(ptr), m_flags(flags), m_slot(NULL)
  {
    /* We can only look for page-related flags. */
    ut_ad(!(flags &
            ~(MTR_MEMO_PAGE_S_FIX | MTR_MEMO_PAGE_X_FIX | MTR_MEMO_PAGE_SX_FIX | MTR_MEMO_BUF_FIX | MTR_MEMO_MODIFY)));
  }
  ...
  bool operator()(mtr_memo_slot_t *slot)
  {
    ...
    if (m_ptr < block->frame || m_ptr >= block->frame + block->page.size.logical())
    {
      return (true);
    }
  ...
  buf_block_t *get_block() const { return (reinterpret_cast<buf_block_t *>(get_slot()->object)); }
...
void mtr_write_log(const mtr_buf_t *log)
{
  ...
  DBUG_PRINT("ib_log", (ULINTPF " extra bytes written at " LSN_PF, len, log_sys->lsn));
...
@param[in]	write_mlog_checkpoint	Write MLOG_CHECKPOINT marker
                                        if it is enabled. */
void mtr_t::commit_checkpoint(lsn_t checkpoint_lsn, bool write_mlog_checkpoint)
{
  ut_ad(log_mutex_own());
  ut_ad(is_active());
  ut_ad(!is_inside_ibuf());
...
#error SIZE_OF_MLOG_CHECKPOINT != 9
  ...
  if (write_mlog_checkpoint)
  {
    DBUG_PRINT("ib_log", ("MLOG_CHECKPOINT(" LSN_PF ") written at " LSN_PF, checkpoint_lsn, log_sys->lsn));
  }
}
...
bool mtr_t::is_named_space(ulint space) const
{
  ut_ad(!m_impl.m_sys_space || m_impl.m_sys_space->id == TRX_SYS_SPACE);
  ut_ad(!m_impl.m_undo_space || m_impl.m_undo_space->id != TRX_SYS_SPACE);
  ut_ad(!m_impl.m_user_space || m_impl.m_user_space->id != TRX_SYS_SPACE);
  ut_ad(!m_impl.m_sys_space || m_impl.m_sys_space != m_impl.m_user_space);
  ut_ad(!m_impl.m_sys_space || m_impl.m_sys_space != m_impl.m_undo_space);
  ut_ad(!m_impl.m_user_space || m_impl.m_user_space != m_impl.m_undo_space);
  ...
  switch (get_log_mode())
  {
    ...
    case MTR_LOG_SHORT_INSERTS:
      return (m_impl.m_user_space_id == space || is_predefined_tablespace(space));
  }
...
fil_space_t *mtr_t::x_lock_space(ulint space_id, const char *file, ulint line)
{
  fil_space_t *space;
  ...
  else if (get_log_mode() == MTR_LOG_NO_REDO)
  {
    ...
    ut_ad(space->purpose == FIL_TYPE_TEMPORARY || space->purpose == FIL_TYPE_IMPORT || space->redo_skipped_count > 0 ||
  ...
  ut_ad(space->purpose == FIL_TYPE_TEMPORARY || space->purpose == FIL_TYPE_IMPORT ||
        space->purpose == FIL_TYPE_TABLESPACE);
  return (space);
}

/** Look up the system tablespace. */
void mtr_t::lookup_sys_space()
{
...
@param[in]	space_id	tablespace ID */
void mtr_t::lookup_user_space(ulint space_id)
{
...
void mtr_t::set_named_space(fil_space_t *space)
{
  ut_ad(m_impl.m_user_space_id == TRX_SYS_SPACE);
  ut_d(m_impl.m_user_space_id = space->id);
  if (space->id == TRX_SYS_SPACE)
  {
    ut_ad(m_impl.m_sys_space == NULL || m_impl.m_sys_space == space);
...
bool mtr_t::memo_release(const void *object, ulint type)
{
  ut_ad(m_impl.m_magic_n == MTR_MAGIC_N);
  ut_ad(is_active());
...
/** Release the blocks used in this mini-transaction */
void mtr_t::Command::release_blocks()
{
...
void mtr_t::release_free_extents(ulint n_reserved)
{
  fil_space_t *space;
  ...
  {
    ut_ad(m_impl.m_user_space->id == m_impl.m_user_space_id);
    ut_ad(memo_contains(get_memo(), &m_impl.m_user_space->latch, MTR_MEMO_X_LOCK));
  ...
  else
  {
    ...
    ut_ad(memo_contains(get_memo(), &m_impl.m_sys_space->latch, MTR_MEMO_X_LOCK));
...
buf_block_t *mtr_t::memo_contains_page_flagged(const byte *ptr, ulint flags) const
{
  FindPage check(ptr, flags);
  Iterate<FindPage> iterator(check);

  return (m_impl.m_memo.for_each_block_in_reverse(iterator) ? NULL : check.get_block());
}
...
{
  buf_block_t *block = memo_contains_page_flagged(ptr, MTR_MEMO_PAGE_X_FIX | MTR_MEMO_PAGE_SX_FIX);
  ut_ad(block != NULL);
...
{
  ib::info() << "Mini-transaction handle: memo size " << m_impl.m_memo.size() << " bytes log size " << get_log()->size()
             << " bytes";
}
