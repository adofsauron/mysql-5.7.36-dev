...
int ut_win_init_time()
{
  ...
  sql_print_error("LoadLibrary(\"kernel32.dll\") failed: GetLastError returns %lu", error);
  return (-1);
}
    ...
    FILE *file) /*!< in: file where to print */
{
  ...
  fprintf(file, "%d-%02d-%02d %02d:%02d:%02d %#llx", (int)cal_tm.wYear, (int)cal_tm.wMonth, (int)cal_tm.wDay,
          (int)cal_tm.wHour, (int)cal_tm.wMinute, (int)cal_tm.wSecond, static_cast<ulonglong>(thread_id));
#else
  ...
  cal_tm_ptr = &cal_tm;
  fprintf(file, "%d-%02d-%02d %02d:%02d:%02d %#lx", cal_tm_ptr->tm_year + 1900, cal_tm_ptr->tm_mon + 1,
          cal_tm_ptr->tm_mday, cal_tm_ptr->tm_hour, cal_tm_ptr->tm_min, cal_tm_ptr->tm_sec, thread_id);
#endif
}
...
std::string ut_get_name(const trx_t *trx, const char *name)
{
  ...
  bufend = innobase_convert_name(buf, sizeof buf, name, strlen(name), trx ? trx->mysql_thd : NULL);
  buf[bufend - buf] = '\0';
...
char *ut_format_name(const char *name, char *formatted, ulint formatted_size)
{
  ...
  end = innobase_convert_name(formatted, formatted_size, name, strlen(name), NULL);
...
info::~info() { sql_print_information("InnoDB: %s", m_oss.str().c_str()); }

warn::~warn() { sql_print_warning("InnoDB: %s", m_oss.str().c_str()); }

error::~error() { sql_print_error("InnoDB: %s", m_oss.str().c_str()); }
