...
typedef std::map<const char *, PSI_memory_key *, ut_strcmp_functor> mem_keys_auto_t;
...
void ut_new_boot()
{
  ...
  static const char *auto_event_names[] = {
      /* Keep this list alphabetically sorted. */
      "api0api",  "btr0btr",    "btr0bulk",  "btr0cur",      "btr0pcur",  "btr0sea",    "buf0buf",       "buf0dblwr",
      "buf0dump", "buf0flu",    "buf0lru",   "dict0dict",    "dict0mem",  "dict0stats", "dict0stats_bg", "eval0eval",
      "fil0fil",  "fsp0file",   "fsp0space", "fsp0sysspace", "fts0ast",   "fts0config", "fts0fts",       "fts0opt",
      "fts0pars", "fts0que",    "fts0sql",   "gis0sea",      "ha0ha",     "ha_innodb",  "handler0alter", "hash0hash",
      "i_s",      "ibuf0ibuf",  "lexyy",     "lock0lock",    "log0log",   "log0recv",   "mem0mem",       "os0event",
      "os0file",  "page0cur",   "page0zip",  "pars0lex",     "read0read", "rem0rec",    "row0ftsort",    "row0import",
      "row0log",  "row0merge",  "row0mysql", "row0sel",      "row0trunc", "srv0conc",   "srv0srv",       "srv0start",
      "sync0arr", "sync0debug", "sync0rw",   "sync0types",   "trx0i_s",   "trx0purge",  "trx0roll",      "trx0rseg",
      "trx0sys",  "trx0trx",    "trx0undo",  "usr0sess",     "ut0list",   "ut0mem",     "ut0mutex",      "ut0pool",
      "ut0rbt",   "ut0wqueue",
  };
  ...
  {
    const std::pair<mem_keys_auto_t::iterator, bool> ret MY_ATTRIBUTE((unused)) =
        mem_keys_auto.insert(mem_keys_auto_t::value_type(auto_event_names[i], &auto_event_keys[i]));
