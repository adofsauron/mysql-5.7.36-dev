    ...
    ulint page_no) /*!< in: page number */
{
  ...
  if (page_no >= buf_dblwr->block1 && page_no < buf_dblwr->block1 + TRX_SYS_DOUBLEWRITE_BLOCK_SIZE)
  {
    return (TRUE);
  }

  if (page_no >= buf_dblwr->block2 && page_no < buf_dblwr->block2 + TRX_SYS_DOUBLEWRITE_BLOCK_SIZE)
  {
    return (TRUE);
  }
                       ...
                       header on trx sys page */
{
  ...
  buf_dblwr->block1 = mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_BLOCK1);
  buf_dblwr->block2 = mach_read_from_4(doublewrite + TRX_SYS_DOUBLEWRITE_BLOCK2);

  buf_dblwr->in_use = static_cast<bool *>(ut_zalloc_nokey(buf_size * sizeof(bool)));

  buf_dblwr->write_buf_unaligned = static_cast<byte *>(ut_malloc_nokey((1 + buf_size) * UNIV_PAGE_SIZE));

  buf_dblwr->write_buf = static_cast<byte *>(ut_align(buf_dblwr->write_buf_unaligned, UNIV_PAGE_SIZE));

  buf_dblwr->buf_block_arr = static_cast<buf_page_t **>(ut_zalloc_nokey(buf_size * sizeof(void *)));
}
...
start_again:
  ...
  ulint min_doublewrite_size = ((2 * TRX_SYS_DOUBLEWRITE_BLOCK_SIZE + FSP_EXTENT_SIZE / 2 + 100) * UNIV_PAGE_SIZE);
  ...
  {
    new_block = fseg_alloc_free_page(fseg_header, prev_page_no + 1, FSP_UP, &mtr);
    if (new_block == NULL)
    {
...
dberr_t buf_dblwr_init_or_load_pages(pfs_os_file_t file, const char *path)
{
  ...
  err = os_file_read(read_request, file, read_buf, TRX_SYS_PAGE_NO * UNIV_PAGE_SIZE, UNIV_PAGE_SIZE);
  ...
  err = os_file_read(read_request, file, buf, block1 * UNIV_PAGE_SIZE, TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * UNIV_PAGE_SIZE);
  ...
  for (ulint i = 0; i < TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * 2; i++)
  {
    if (reset_space_ids)
    {
      ...
      err = os_file_write(write_request, path, file, page, source_page_no * UNIV_PAGE_SIZE, UNIV_PAGE_SIZE);
...
void buf_dblwr_process(void)
{
  ...
  for (recv_dblwr_t::list::iterator i = recv_dblwr.pages.begin(); i != recv_dblwr.pages.end(); ++i, ++page_no_dblwr)
  {
    const byte *page = *i;
    ...
    if (page_no >= space->size)
    {
      ...
      bool skip_warning = srv_is_tablespace_truncated(space_id) || srv_was_tablespace_truncated(space);
    ...
    else
    {
      ...
      dberr_t err = fil_io(request, true, page_id, page_size, 0, page_size.physical(), read_buf, NULL);

      if (err != DB_SUCCESS)
      {
        ib::warn() << "Double write buffer recovery: " << page_id << " read failed with "
                   << "error: " << ut_strerr(err);
      }

      /* Check if the page is corrupt */
      if (buf_page_is_corrupted(true, read_buf, page_size, fsp_is_checksum_disabled(space_id)))
      {
        ib::warn() << "Database page corruption or"
                   << " a failed file read of page " << page_id << ". Trying to recover it from the"
                   << " doublewrite buffer.";

        if (buf_page_is_corrupted(true, page, page_size, fsp_is_checksum_disabled(space_id)))
        {
      ...
      else if (buf_page_is_zeroes(read_buf, page_size) && !buf_page_is_zeroes(page, page_size) &&
               !buf_page_is_corrupted(true, page, page_size, fsp_is_checksum_disabled(space_id)))
      {
      ...
      else
      {
        ...
        bool t3 = buf_page_is_corrupted(true, page, page_size, fsp_is_checksum_disabled(space_id));
      ...
      fil_io(write_request, true, page_id, page_size, 0, page_size.physical(), const_cast<byte *>(page), NULL);

      ib::info() << "Recovered page " << page_id << " from the doublewrite buffer.";
    }
  }
...
{
  if (!srv_use_doublewrite_buf || buf_dblwr == NULL || fsp_is_system_temporary(bpage->id.space()))
  {
    return;
  }
    ...
    const page_t *page) /*!< in: page to check */
{
  if (memcmp(page + (FIL_PAGE_LSN + 4), page + (UNIV_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + 4), 4))
  {
    ...
    const ulint lsn2 = mach_read_from_4(page + UNIV_PAGE_SIZE - FIL_PAGE_END_LSN_OLD_CHKSUM + 4);
                             ...
                             is requested */
{
  ...
  if (bpage->zip.data != NULL)
  {
    ...
    fil_io(request, sync, bpage->id, bpage->size, 0, bpage->size.physical(), (void *)bpage->zip.data, (void *)bpage);
  }
  else
  {
    ut_ad(!bpage->size.is_compressed());
    ...
    buf_block_t *block = reinterpret_cast<buf_block_t *>(const_cast<buf_page_t *>(bpage));

    ut_a(buf_block_get_state(block) == BUF_BLOCK_FILE_PAGE);
    buf_dblwr_check_page_lsn(block->frame);

    fil_io(request, sync, bpage->id, bpage->size, 0, bpage->size.physical(), block->frame, block);
  }
}
...
try_again:
  ...
  for (ulint len2 = 0, i = 0; i < buf_dblwr->first_free; len2 += UNIV_PAGE_SIZE, i++)
  {
    ...
    if (buf_block_get_state(block) != BUF_BLOCK_FILE_PAGE || block->page.zip.data)
    {
  ...
  len = ut_min(TRX_SYS_DOUBLEWRITE_BLOCK_SIZE, buf_dblwr->first_free) * UNIV_PAGE_SIZE;

  fil_io(IORequestWrite, true, page_id_t(TRX_SYS_SPACE, buf_dblwr->block1), univ_page_size, 0, len, (void *)write_buf,
         NULL);
  ...
  len = (buf_dblwr->first_free - TRX_SYS_DOUBLEWRITE_BLOCK_SIZE) * UNIV_PAGE_SIZE;

  write_buf = buf_dblwr->write_buf + TRX_SYS_DOUBLEWRITE_BLOCK_SIZE * UNIV_PAGE_SIZE;

  fil_io(IORequestWrite, true, page_id_t(TRX_SYS_SPACE, buf_dblwr->block2), univ_page_size, 0, len, (void *)write_buf,
         NULL);
...
try_again:
  ...
  byte *p = buf_dblwr->write_buf + univ_page_size.physical() * buf_dblwr->first_free;
  ...
  if (bpage->size.is_compressed())
  {
    ...
    memset(p + bpage->size.physical(), 0x0, univ_page_size.physical() - bpage->size.physical());
  }
  else
  {
...
retry:
  ...
  if (bpage->size.is_compressed())
  {
    memcpy(buf_dblwr->write_buf + univ_page_size.physical() * i, bpage->zip.data, bpage->size.physical());

    memset(buf_dblwr->write_buf + univ_page_size.physical() * i + bpage->size.physical(), 0x0,
           univ_page_size.physical() - bpage->size.physical());

    fil_io(IORequestWrite, true, page_id_t(TRX_SYS_SPACE, offset), univ_page_size, 0, univ_page_size.physical(),
           (void *)(buf_dblwr->write_buf + univ_page_size.physical() * i), NULL);
  }
  else
  {
