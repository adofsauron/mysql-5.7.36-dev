    ...
    ulint i)               /*!< in: block size */
{
  ...
  mach_write_to_4(buf->stamp.bytes + BUF_BUDDY_STAMP_OFFSET, BUF_BUDDY_STAMP_FREE);
  buf->stamp.size = i;
}
    ...
    ulint i)                     /*!< in: index of buf_pool->zip_free[] */
{
  ...
  for (itr = UT_LIST_GET_FIRST(buf_pool->zip_free[i]); itr && itr != buf; itr = UT_LIST_GET_NEXT(list, itr))
  {
  }
                           ...
                           buf_pool->zip_free[] */
{
  ...
  * A valid space_id of a compressed tablespace
  ...
  return (buf->stamp.size == i ? BUF_BUDDY_STATE_FREE : BUF_BUDDY_STATE_PARTIALLY_USED);
}

/**********************************************************************/ /**
 Add a block to the head of the appropriate buddy free list. */
UNIV_INLINE
void buf_buddy_add_to_free(
    /*==================*/
    buf_pool_t *buf_pool,  /*!< in: buffer pool instance */
...
static buf_buddy_free_t *buf_buddy_alloc_zip(
    /*================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
    ulint i)              /*!< in: index of buf_pool->zip_free[] */
{
  ...
  {
    while (buf != NULL && buf_frame_will_withdrawn(buf_pool, reinterpret_cast<byte *>(buf)))
    {
  ...
  else if (i + 1 < BUF_BUDDY_SIZES)
  {
    ...
    if (buf)
    {
      buf_buddy_free_t *buddy = reinterpret_cast<buf_buddy_free_t *>(buf->stamp.bytes + (BUF_BUDDY_LOW << i));
  ...
  if (buf)
  {
    ...
    ut_ad(mach_read_from_4(buf->stamp.bytes + BUF_BUDDY_STAMP_OFFSET) == BUF_BUDDY_STAMP_NONFREE);
  }
...
static void buf_buddy_block_free(
    /*=================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
    void *buf)            /*!< in: buffer frame to deallocate */
{
  ...
  HASH_SEARCH(hash, buf_pool->zip_hash, fold, buf_page_t *, bpage,
              ut_ad(buf_page_get_state(bpage) == BUF_BLOCK_MEMORY && bpage->in_zip_hash && !bpage->in_page_hash),
...
static void *buf_buddy_alloc_from(
    /*=================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
                          ...
                          of buf_pool->zip_free[] */
{
  ...
  while (j > i)
  {
    ...
    zip_buf = reinterpret_cast<buf_buddy_free_t *>(reinterpret_cast<byte *>(buf) + offs);
...
static bool buf_buddy_relocate(
    /*===============*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
...
recombine:
  ...
  if (UT_LIST_GET_LEN(buf_pool->zip_free[i]) < 16 && buf_pool->curr_size >= buf_pool->old_size)
  {
    goto func_exit;
  }

  /* Try to combine adjacent blocks. */
  buddy = reinterpret_cast<buf_buddy_free_t *>(buf_buddy_get(reinterpret_cast<byte *>(buf), BUF_BUDDY_LOW << i));
  ...
  switch (buf_buddy_is_free(buddy, i))
  {
    ...
    case BUF_BUDDY_STATE_USED:
      ...
      this size? */
      if (buf_buddy_free_t *zip_buf = UT_LIST_GET_FIRST(buf_pool->zip_free[i]))
      {
...
bool buf_buddy_realloc(buf_pool_t *buf_pool, void *buf, ulint size)
{
  ...
  if (block == NULL)
  {
    ...
    block = reinterpret_cast<buf_block_t *>(buf_buddy_alloc_from(buf_pool, block->frame, i, BUF_BUDDY_SIZES));
  }
...
void buf_buddy_condense_free(buf_pool_t *buf_pool)
{
  ...
  for (ulint i = 0; i < UT_ARR_SIZE(buf_pool->zip_free); ++i)
  {
    ...
    while (buf != NULL)
    {
      ...
      buf_buddy_free_t *buddy =
          reinterpret_cast<buf_buddy_free_t *>(buf_buddy_get(reinterpret_cast<byte *>(buf), BUF_BUDDY_LOW << i));

      /* seek to the next withdraw target */
      while (true)
      {
        while (next != NULL && !buf_frame_will_withdrawn(buf_pool, reinterpret_cast<byte *>(next)))
        {
