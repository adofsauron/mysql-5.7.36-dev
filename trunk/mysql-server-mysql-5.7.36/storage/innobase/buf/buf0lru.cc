    ...
    buf_pool_t *buf_pool)
{
  ...
  if (UT_LIST_GET_LEN(buf_pool->unzip_LRU) <= UT_LIST_GET_LEN(buf_pool->LRU) / 10)
  {
    return (FALSE);
  }
  ...
  ulint io_avg = buf_LRU_stat_sum.io / BUF_LRU_STAT_N_INTERVAL + buf_LRU_stat_cur.io;

  ulint unzip_avg = buf_LRU_stat_sum.unzip / BUF_LRU_STAT_N_INTERVAL + buf_LRU_stat_cur.unzip;
...
static void buf_LRU_drop_page_hash_batch(ulint space_id, const page_size_t &page_size, const ulint *arr, ulint count)
{
...
static void buf_LRU_drop_page_hash_for_tablespace(
    /*==================================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
    ulint id)             /*!< in: space id */
{
  ...
  ulint *page_arr = static_cast<ulint *>(ut_malloc_nokey(sizeof(ulint) * BUF_LRU_DROP_SEARCH_SIZE));
...
scan_again:
       ...
       /* No op */)
  {
    ...
    if (buf_page_get_state(bpage) != BUF_BLOCK_FILE_PAGE || bpage->id.space() != id || bpage->io_fix != BUF_IO_NONE)
    {
    ...
    ulint processed)      /*!< in: number of pages processed */
{
  ...
  if (bpage != NULL && processed >= BUF_LRU_DROP_SEARCH_SIZE && buf_page_get_io_fix(bpage) == BUF_IO_NONE)
  {
...
static MY_ATTRIBUTE((warn_unused_result)) dberr_t buf_flush_or_remove_pages(
    /*======================*/
...
rescan:
  ...
  for (bpage = UT_LIST_GET_LAST(buf_pool->flush_list); bpage != NULL; bpage = prev)
  {
    ...
    {
      DBUG_EXECUTE_IF(
          "ib_export_flush_crash", static ulint n_pages; if (++n_pages == 4) { DBUG_SUICIDE(); });
    }
#endif /* NDEBUG */
...
static void buf_flush_dirty_pages(
    /*==================*/
    buf_pool_t *buf_pool,    /*!< buffer pool instance */
    ulint id,                /*!< in: space id */
    FlushObserver *observer, /*!< in: flush observer */
                             ...
                             be interrupted */
{
  ...
  ut_ad(err == DB_INTERRUPTED || buf_pool_get_dirty_pages_count(buf_pool, id, observer) == 0);
}

/******************************************************************/ /**
 Remove all pages that belong to a given tablespace inside a specific
 buffer pool instance when we are DISCARDing the tablespace. */
static void buf_LRU_remove_all_pages(
    /*=====================*/
    buf_pool_t *buf_pool, /*!< buffer pool instance */
    ulint id)             /*!< in: space id */
{
...
scan_again:
       ...
       /* No op */)
  {
    ...
    DBUG_PRINT("ib_buf",
               ("evict page " UINT32PF ":" UINT32PF " state %u", bpage->id.space(), bpage->id.page_no(), bpage->state));
...
static bool buf_LRU_free_from_unzip_LRU_list(
    /*=============================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
                          ...
                          srv_LRU_scan_depth / 2 blocks. */
{
  ...
  for (buf_block_t *block = UT_LIST_GET_LAST(buf_pool->unzip_LRU);
       block != NULL && !freed && (scan_all || scanned < srv_LRU_scan_depth); ++scanned)
  {
  ...
  if (scanned)
  {
    MONITOR_INC_VALUE_CUMULATIVE(MONITOR_LRU_UNZIP_SEARCH_SCANNED, MONITOR_LRU_UNZIP_SEARCH_SCANNED_NUM_CALL,
                                 MONITOR_LRU_UNZIP_SEARCH_SCANNED_PER_CALL, scanned);
  }
                          ...
                          up to BUF_LRU_SEARCH_SCAN_THRESHOLD */
{
  ...
  if (scanned)
  {
    MONITOR_INC_VALUE_CUMULATIVE(MONITOR_LRU_SEARCH_SCANNED, MONITOR_LRU_SEARCH_SCANNED_NUM_CALL,
...
bool buf_LRU_scan_and_free_block(
    /*========================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
    bool scan_all)        /*!< in: scan whole LRU list
                          if true, otherwise scan only
...
/*==============================*/
{
  ...
  for (ulint i = 0; i < srv_buf_pool_instances && !ret; i++)
  {
    ...
    if (!recv_recovery_is_on() && UT_LIST_GET_LEN(buf_pool->free) + UT_LIST_GET_LEN(buf_pool->LRU) <
    ...
    buf_pool_t *buf_pool)
{
  ...
  while (block != NULL)
  {
    ...
    if (buf_pool->curr_size >= buf_pool->old_size || UT_LIST_GET_LEN(buf_pool->withdraw) >= buf_pool->withdraw_target ||
    ...
    const buf_pool_t *buf_pool) /*!< in: buffer pool instance */
{
  ...
  if (!recv_recovery_is_on() && buf_pool->curr_size == buf_pool->old_size &&
      UT_LIST_GET_LEN(buf_pool->free) + UT_LIST_GET_LEN(buf_pool->LRU) < buf_pool->curr_size / 20)
  {
  ...
  else if (!recv_recovery_is_on() && buf_pool->curr_size == buf_pool->old_size &&
           (UT_LIST_GET_LEN(buf_pool->free) + UT_LIST_GET_LEN(buf_pool->LRU)) < buf_pool->curr_size / 3)
  {
...
loop:
  ...
  if (block != NULL)
  {
    ...
    if (started_monitor)
    {
      srv_print_innodb_monitor = static_cast<my_bool>(mon_value_was);
    }
  ...
  if (n_iterations > 20 && srv_buf_pool_old_size == srv_buf_pool_size)
  {
    ib::warn() << "Difficult to find free blocks in the buffer pool"
               ...
               << fil_n_pending_log_flushes << "; buffer pool: " << fil_n_pending_tablespace_flushes << ". "
               << os_n_file_reads << " OS file reads, " << os_n_file_writes << " OS file writes, " << os_n_fsyncs
    ...
    buf_pool_t *buf_pool)
{
  ...
  for (buf_page_t *bpage = UT_LIST_GET_LAST(buf_pool->LRU); bpage != NULL; bpage = UT_LIST_GET_PREV(LRU, bpage))
  {
    ut_ad(bpage->in_LRU_list);
    ut_ad(buf_page_in_file(bpage));
    ...
    buf_page_t *bpage) /*!< in: control block */
{
  ...
  {
    for (buf_page_t *bpage = UT_LIST_GET_FIRST(buf_pool->LRU); bpage != NULL; bpage = UT_LIST_GET_NEXT(LRU, bpage))
    {
      /* This loop temporarily violates the
      assertions of buf_page_set_old(). */
      bpage->old = FALSE;
...
static void buf_LRU_add_block_to_end_low(
    /*=========================*/
    buf_page_t *bpage) /*!< in: control block */
{
  buf_pool_t *buf_pool = buf_pool_from_bpage(bpage);
                       ...
                       compressed page of an uncompressed page */
{
  ...
  DBUG_PRINT("ib_buf", ("free page " UINT32PF ":" UINT32PF, bpage->id.space(), bpage->id.page_no()));
  ...
  if (b != NULL)
  {
    ...
    bpage->size.copy_from(page_size_t(bpage->size.logical(), bpage->size.logical(), false));
  ...
  if (b != NULL)
  {
    ...
    const uint32_t checksum = page_zip_calc_checksum(b->zip.data, b->size.physical(),
    ...
    buf_block_t *block) /*!< in: block, must not contain a file page */
{
  ...
  if (data != NULL)
  {
    ...
    block->page.size.copy_from(page_size_t(block->page.size.logical(), block->page.size.logical(), false));
  }
...
/******************************************************************/ /**
 ...
 @retval false if BUF_BLOCK_ZIP_PAGE was removed from page_hash. In
 this case the block is already returned to the buddy allocator. */
static bool buf_LRU_block_remove_hashed(
    /*========================*/
    buf_page_t *bpage, /*!< in: block, must contain a file page and
                       be in a state where it can be freed; there
                       may or may not be a hash index to the page */
                       ...
                       compressed page of an uncompressed page */
{
      ...
      if (bpage->zip.data)
      {
        ...
        switch (fil_page_get_type(page))
        {
          ...
          case FIL_PAGE_RTREE:
            ...
            ut_a(page_zip_validate(&bpage->zip, page, ((buf_block_t *)bpage)->index));
  ...
  if (bpage != hashed_bpage)
  {
    ...
    if (hashed_bpage)
    {
      ib::error() << "In hash table we find block " << hashed_bpage << " of " << hashed_bpage->id << " which is not "
                  << bpage;
    }
...
static uint buf_LRU_old_ratio_update_instance(
    /*==============================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
...
static void buf_LRU_validate_instance(
    /*======================*/
    buf_pool_t *buf_pool)
{
  ulint old_len;
  ...
  for (buf_page_t *bpage = UT_LIST_GET_FIRST(buf_pool->LRU); bpage != NULL; bpage = UT_LIST_GET_NEXT(LRU, bpage))
  {
    switch (buf_page_get_state(bpage))
    {
      ...
      case BUF_BLOCK_REMOVE_HASH:
        ...
        break;
      case BUF_BLOCK_FILE_PAGE:
        ut_ad(((buf_block_t *)bpage)->in_unzip_LRU_list == buf_page_belongs_to_unzip_LRU(bpage));
      case BUF_BLOCK_ZIP_PAGE:
      case BUF_BLOCK_ZIP_DIRTY:
        break;
    }
  ...
  for (buf_page_t *bpage = UT_LIST_GET_FIRST(buf_pool->free); bpage != NULL; bpage = UT_LIST_GET_NEXT(list, bpage))
  {
  ...
  for (buf_block_t *block = UT_LIST_GET_FIRST(buf_pool->unzip_LRU); block != NULL;
...
void buf_LRU_print_instance(
    /*===================*/
    buf_pool_t *buf_pool)
{
  buf_pool_mutex_enter(buf_pool);

  for (const buf_page_t *bpage = UT_LIST_GET_FIRST(buf_pool->LRU); bpage != NULL; bpage = UT_LIST_GET_NEXT(LRU, bpage))
  {
    mutex_enter(buf_page_get_mutex(bpage));

    fprintf(stderr, "BLOCK space " UINT32PF " page " UINT32PF " ", bpage->id.space(), bpage->id.page_no());
    ...
    switch (buf_page_get_state(bpage))
    {
      ...
      case BUF_BLOCK_ZIP_PAGE:
        ...
        fprintf(stderr,
                "\ntype %lu size %lu"
                " index id " IB_ID_FMT "\n",
                (ulong)fil_page_get_type(frame), (ulong)bpage->size.physical(), btr_page_get_index_id(frame));
        break;
