    ...
    lsn_t lsn)            /*!< in: oldest modification */
{
  ...
  else
  {
    ...
    while (b != NULL && b->oldest_modification > block->page.oldest_modification)
    {
                       ...
                       buf_page_in_file(bpage) and in the LRU list */
{
  ...
  ib::fatal() << "Buffer block " << bpage << " state " << bpage->state << " in the LRU list!";
...
bool buf_flush_ready_for_flush(
    /*======================*/
    buf_page_t *bpage,      /*!< in: buffer control block, must be
...
void buf_flush_update_zip_checksum(buf_frame_t *page, ulint size, lsn_t lsn)
{
  ...
  const uint32_t checksum =
      page_zip_calc_checksum(page, size, static_cast<srv_checksum_algorithm_t>(srv_checksum_algorithm));
...
void buf_flush_init_for_writing(const buf_block_t *block, byte *page, void *page_zip_, lsn_t newest_lsn,
                                bool skip_checksum)
{
  ...
  else
  {
    if (block != NULL && UNIV_PAGE_SIZE == 16384)
    {
      ...
      {
        case 0:
          reset_type = block->page.id.page_no() == 0 ? FIL_PAGE_TYPE_FSP_HDR : FIL_PAGE_TYPE_XDES;
          break;
        case 1:
      ...
      {
        ib::info() << "Resetting invalid page " << block->page.id << " type " << page_type << " to " << reset_type
                   << " when flushing.";
    ...
    bool sync)              /*!< in: true if sync IO request */
{
  ...
  DBUG_PRINT("ib_buf", ("flush %s %u page " UINT32PF ":" UINT32PF, sync ? "sync" : "async", (unsigned)flush_type,
  ...
  switch (buf_page_get_state(bpage))
  {
    ...
    case BUF_BLOCK_FILE_PAGE:
      ...
      buf_flush_init_for_writing(reinterpret_cast<const buf_block_t *>(bpage),
  ...
  if (!srv_use_doublewrite_buf || buf_dblwr == NULL || srv_read_only_mode || fsp_is_system_temporary(bpage->id.space()))
  {
    ...
    bool sync)              /*!< in: true if sync IO request */
{
  ...
  if (!is_uncompressed)
  {
    ...
    rw_lock = NULL;
           ...
           (!no_fix_count && srv_shutdown_state <= SRV_SHUTDOWN_CLEANUP && fsp_is_system_temporary(bpage->id.space())))
  {
...
static ulint buf_flush_try_neighbors(const page_id_t &page_id, buf_flush_t flush_type, ulint n_flushed,
                                     ulint n_to_flush)
{
  ulint i;
  ulint low;
  ...
  if (UT_LIST_GET_LEN(buf_pool->LRU) < BUF_LRU_OLD_MIN_LEN || srv_flush_neighbors == 0)
  {
  ...
  else
  {
    ...
    buf_flush_area = ut_min(BUF_READ_AHEAD_AREA(buf_pool), buf_pool->curr_size / 16);
    ...
    if (srv_flush_neighbors == 1)
    {
      ...
      if (page_id.page_no() > low)
      {
        ...
        {
          if (!buf_flush_check_neighbor(page_id_t(page_id.space(), i), flush_type))
          {
            break;
          }
      ...
      for (i = page_id.page_no() + 1; i < high && buf_flush_check_neighbor(page_id_t(page_id.space(), i), flush_type);
           i++)
      {
        /* do nothing */
      }
  ...
  DBUG_PRINT("ib_buf", ("flush " UINT32PF ":%u..%u", page_id.space(), (unsigned)low, (unsigned)high));
  ...
  if (count > 1)
  {
    MONITOR_INC_VALUE_CUMULATIVE(MONITOR_FLUSH_NEIGHBOR_TOTAL_PAGE, MONITOR_FLUSH_NEIGHBOR_COUNT,
                          ...
                          blocks in the free_list */
{
  ...
  while (block != NULL && count < max && free_len < srv_LRU_scan_depth && lru_len > UT_LIST_GET_LEN(buf_pool->LRU) / 10)
  {
    ++scanned;
  ...
  if (scanned)
  {
    MONITOR_INC_VALUE_CUMULATIVE(MONITOR_LRU_BATCH_SCANNED, MONITOR_LRU_BATCH_SCANNED_NUM_CALL,
...
static ulint buf_flush_LRU_list_batch(
    /*=====================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
    ulint max)            /*!< in: desired number of
                          blocks in the free_list */
{
  ...
  if (buf_pool->curr_size < buf_pool->old_size && buf_pool->withdraw_target > 0)
  {
    withdraw_depth = buf_pool->withdraw_target - UT_LIST_GET_LEN(buf_pool->withdraw);
  }

  for (bpage = UT_LIST_GET_LAST(buf_pool->LRU);
       bpage != NULL && count + evict_count < max && free_len < srv_LRU_scan_depth + withdraw_depth &&
  ...
  if (evict_count)
  {
    MONITOR_INC_VALUE_CUMULATIVE(MONITOR_LRU_BATCH_EVICT_TOTAL_PAGE, MONITOR_LRU_BATCH_EVICT_COUNT,
...
static ulint buf_do_flush_list_batch(buf_pool_t *buf_pool, ulint min_n, lsn_t lsn_limit)
{
  ...
  for (buf_page_t *bpage = UT_LIST_GET_LAST(buf_pool->flush_list);
       count < min_n && bpage != NULL && len > 0 && bpage->oldest_modification < lsn_limit;
  ...
  if (scanned)
  {
    MONITOR_INC_VALUE_CUMULATIVE(MONITOR_FLUSH_BATCH_SCANNED, MONITOR_FLUSH_BATCH_SCANNED_NUM_CALL,
                                 MONITOR_FLUSH_BATCH_SCANNED_PER_CALL, scanned);
  }

  if (count)
  {
    MONITOR_INC_VALUE_CUMULATIVE(MONITOR_FLUSH_BATCH_TOTAL_PAGE, MONITOR_FLUSH_BATCH_COUNT, MONITOR_FLUSH_BATCH_PAGES,
                                 count);
  }
...
static ulint buf_flush_batch(buf_pool_t *buf_pool, buf_flush_t flush_type, ulint min_n, lsn_t lsn_limit)
{
  ...
  DBUG_PRINT("ib_buf", ("flush %u completed, %u pages", unsigned(flush_type), unsigned(count)));
...
void buf_flush_wait_flushed(lsn_t new_oldest)
{
  for (ulint i = 0; i < srv_buf_pool_instances; ++i)
  {
    ...
    for (;;)
    {
      ...
      for (bpage = UT_LIST_GET_LAST(buf_pool->flush_list); bpage != NULL && fsp_is_system_temporary(bpage->id.space());
...
bool buf_flush_lists(ulint min_n, lsn_t lsn_limit, ulint *n_processed)
{
  ...
  for (i = 0; i < srv_buf_pool_instances; i++)
  {
    ...
    if (!buf_flush_do_batch(buf_pool, BUF_FLUSH_LIST, min_n, lsn_limit, &page_count))
    {
    ...
    buf_pool_t *buf_pool) /*!< in/out: buffer pool instance */
{
  ...
  for (bpage = buf_pool->single_scan_itr.start(), scanned = 0, freed = false; bpage != NULL;
  ...
  if (scanned)
  {
    MONITOR_INC_VALUE_CUMULATIVE(MONITOR_LRU_SINGLE_FLUSH_SCANNED, MONITOR_LRU_SINGLE_FLUSH_SCANNED_NUM_CALL,
 ...
 * Flush dirty pages at the tail of LRU to the disk
 ...
 config parameter innodb_LRU_scan_depth.
 @return total pages flushed */
ulint buf_flush_LRU_lists(void)
/*=====================*/
{
  ulint n_flushed = 0;
...
/*==============================*/
{
  for (ulint i = 0; i < srv_buf_pool_instances; i++)
  {
    ...
    if (buf_pool->n_flush[BUF_FLUSH_LRU] > 0 || buf_pool->init_flush[BUF_FLUSH_LRU])
    {
      buf_pool_mutex_exit(buf_pool);
    ...
    lsn_t age) /*!< in: current age of LSN. */
{
  ...
  return (static_cast<ulint>(
      ((srv_max_io_capacity / srv_io_capacity) * (lsn_age_factor * sqrt((double)lsn_age_factor))) / 7.5));
}
    ...
    lsn_t *lsn_limit, ulint last_pages_in)
{
  ...
  if (++n_iterations >= avg_loop || time_elapsed >= (uint64_t)avg_loop)
  {
    if (time_elapsed < 1)
    {
      time_elapsed = 1;
    ...
    MONITOR_SET(MONITOR_FLUSH_ADAPTIVE_AVG_TIME_THREAD, list_tm / (srv_n_page_cleaners * flush_pass));
    MONITOR_SET(MONITOR_LRU_BATCH_FLUSH_AVG_TIME_THREAD, lru_tm / (srv_n_page_cleaners * flush_pass));
    MONITOR_SET(MONITOR_FLUSH_ADAPTIVE_AVG_TIME_EST, flush_tm * list_tm / flush_pass / (list_tm + lru_tm));
    MONITOR_SET(MONITOR_LRU_BATCH_FLUSH_AVG_TIME_EST, flush_tm * lru_tm / flush_pass / (list_tm + lru_tm));
    MONITOR_SET(MONITOR_FLUSH_AVG_TIME, flush_tm / flush_pass);

    MONITOR_SET(MONITOR_FLUSH_ADAPTIVE_AVG_PASS, list_pass / page_cleaner->n_slots);
    MONITOR_SET(MONITOR_LRU_BATCH_FLUSH_AVG_PASS, lru_pass / page_cleaner->n_slots);
  ...
  const ulint pages_for_lsn_max = (sum_pages_max / srv_buf_pool_instances) * buf_flush_lsn_scan_factor * 2;
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; i++)
  {
    ...
    buf_flush_list_mutex_enter(buf_pool);
    for (buf_page_t *b = UT_LIST_GET_LAST(buf_pool->flush_list); b != NULL; b = UT_LIST_GET_PREV(list, b))
    {
    ...
    page_cleaner->slots[i].n_pages_requested = pages_for_lsn / buf_flush_lsn_scan_factor + 1;
    mutex_exit(&page_cleaner->mutex);
  }
...
static ulint pc_sleep_if_needed(
    /*===============*/
    ib_time_monotonic_ms_t next_loop_time, int64_t sig_count)
{
  ...
  if (next_loop_time > cur_time)
  {
    ...
    sleep_us = ut_min(int64_t(1000000), (next_loop_time - cur_time) * int64_t(1000));
    ut_a(sleep_us > 0);
...
void buf_flush_page_cleaner_init(void)
/*=============================*/
{
  ut_ad(page_cleaner == NULL);

  page_cleaner = static_cast<page_cleaner_t *>(ut_zalloc_nokey(sizeof(*page_cleaner)));
  ...
  page_cleaner->slots =
      static_cast<page_cleaner_slot_t *>(ut_zalloc_nokey(page_cleaner->n_slots * sizeof(*page_cleaner->slots)));
...
static ulint pc_flush_slot(void)
{
  ...
  if (page_cleaner->n_slots_requested > 0)
  {
    ...
    if (page_cleaner->requested)
    {
      ...
      slot->succeeded_list = buf_flush_do_batch(buf_pool, BUF_FLUSH_LIST, slot->n_pages_requested,
  ...
  finish_mutex:
    ...
    if (page_cleaner->n_slots_requested == 0 && page_cleaner->n_slots_flushing == 0)
    {
...
static void buf_flush_page_cleaner_disabled_loop(void)
{
  ...
  while (innodb_page_cleaner_disabled_debug && srv_shutdown_state == SRV_SHUTDOWN_NONE && page_cleaner->is_running)
  {
...
void buf_flush_page_cleaner_disabled_debug_update(THD *thd, struct st_mysql_sys_var *var, void *var_ptr,
                                                  const void *save)
{
...
extern "C" os_thread_ret_t DECLARE_THREAD(buf_flush_page_cleaner_coordinator)(
    /*===============================================*/
...
os_thread_create */
{
  ...
  ib::info() << "page_cleaner thread running, id " << os_thread_pf(os_thread_get_curr_id());
  ...
  {
    ib::info() << "page_cleaner coordinator priority: " << buf_flush_page_cleaner_priority;
  }
  else
  {
  ...
  while (!srv_read_only_mode && srv_shutdown_state == SRV_SHUTDOWN_NONE && recv_sys->heap != NULL)
  {
  ...
  while (srv_shutdown_state == SRV_SHUTDOWN_NONE)
  {
        ...
        if (warn_count == 0)
        {
          ib::info() << "page_cleaner: 1000ms"
                     ...
                     << "ms. The settings might not"
                        " be optimal. (flushed="
                     << n_flushed_last << " and evicted=" << n_evicted << ", during the time.)";
    ...
    if (ret_sleep != OS_SYNC_TIME_EXCEEDED && srv_flush_sync && buf_flush_sync_lsn > 0)
    {
      ...
      if (n_flushed_list > 0 || n_flushed_lru > 0)
      {
        ...
        MONITOR_INC_VALUE_CUMULATIVE(MONITOR_FLUSH_SYNC_TOTAL_PAGE, MONITOR_FLUSH_SYNC_COUNT, MONITOR_FLUSH_SYNC_PAGES,
    ...
    else if (srv_check_activity(last_activity))
    {
      ...
      if (n_flushed_list)
      {
        MONITOR_INC_VALUE_CUMULATIVE(MONITOR_FLUSH_ADAPTIVE_TOTAL_PAGE, MONITOR_FLUSH_ADAPTIVE_COUNT,
    ...
    else if (ret_sleep == OS_SYNC_TIME_EXCEEDED)
    {
      ...
      if (n_flushed)
      {
        MONITOR_INC_VALUE_CUMULATIVE(MONITOR_FLUSH_BACKGROUND_TOTAL_PAGE, MONITOR_FLUSH_BACKGROUND_COUNT,
  ...
  if (srv_fast_shutdown == 2 || srv_shutdown_state == SRV_SHUTDOWN_EXIT_THREADS)
  {
...
/******************************************************************/ /**
 ...
 @return a dummy parameter */
extern "C" os_thread_ret_t DECLARE_THREAD(buf_flush_page_cleaner_worker)(
    /*==========================================*/
    void *arg MY_ATTRIBUTE((unused)))
/*!< in: a dummy parameter required by
os_thread_create */
{
  ...
  if (buf_flush_page_cleaner_set_priority(buf_flush_page_cleaner_priority))
  {
    ib::info() << "page_cleaner worker priority: " << buf_flush_page_cleaner_priority;
  }
#endif /* UNIV_LINUX */
...

{
  ...
  for (bpage = UT_LIST_GET_FIRST(buf_pool->flush_list); bpage != 0; bpage = UT_LIST_GET_NEXT(list, bpage))
  {
    ...
    if ((observer != NULL && observer == bpage->flush_observer) || (observer == NULL && id == bpage->id.space()))
    {
      ++count;
    }
  }
...
ulint buf_flush_get_dirty_pages_count(
    /*============================*/
    ulint id,                /*!< in: space id to check */
    FlushObserver *observer) /*!< in: flush observer to check */
{
...
for accounting. */
FlushObserver::FlushObserver(ulint space_id, trx_t *trx, ut_stage_alter_t *stage)
...
#ifdef FLUSH_LIST_OBSERVER_DEBUG
  ib::info() << "Flush <" << bpage->id.space() << ", " << bpage->id.page_no() << ">";
#endif /* FLUSH_LIST_OBSERVER_DEBUG */
}
...
#ifdef FLUSH_LIST_OBSERVER_DEBUG
  ib::info() << "Remove <" << bpage->id.space() << ", " << bpage->id.page_no() << ">";
#endif /* FLUSH_LIST_OBSERVER_DEBUG */
}
