...
/*==================================*/
{
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; i++)
  {
    ...
    for (; bpage != NULL && fsp_is_system_temporary(bpage->id.space()); bpage = UT_LIST_GET_PREV(list, bpage))
    {
      /* Do nothing. */
    }
                                                ...
                                                in all buffer pools */
{
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; i++)
  {
    ...
    buf_pools_list_size->unzip_LRU_bytes += UT_LIST_GET_LEN(buf_pool->unzip_LRU) * UNIV_PAGE_SIZE;
...
UNIV_INLINE
bool buf_page_is_checksum_valid_crc32(const byte *read_buf, ulint checksum_field1, ulint checksum_field2,
#ifdef UNIV_INNOCHECKSUM
                                      uintmax_t page_no, bool is_log_enabled, FILE *log_file,
                                      const srv_checksum_algorithm_t curr_algo,
#endif /* UNIV_INNOCHECKSUM */
                                      bool use_legacy_big_endian)
{
  const uint32_t crc32 = buf_calc_page_crc32(read_buf, use_legacy_big_endian);

#ifdef UNIV_INNOCHECKSUM
  if (is_log_enabled && curr_algo == SRV_CHECKSUM_ALGORITHM_STRICT_CRC32)
  {
    fprintf(log_file,
            "page::%" PRIuMAX
            ";"
            " crc32 calculated = %u;"
            " recorded checksum field1 = " ULINTPF " recorded checksum field2 = " ULINTPF "\n",
            page_no, crc32, checksum_field1, checksum_field2);
  }
#endif /* UNIV_INNOCHECKSUM */
...
UNIV_INLINE
bool buf_page_is_checksum_valid_innodb(const byte *read_buf, ulint checksum_field1, ulint checksum_field2
#ifdef UNIV_INNOCHECKSUM
                                       ,
                                       uintmax_t page_no, bool is_log_enabled, FILE *log_file,
                                       const srv_checksum_algorithm_t curr_algo
...
#ifdef UNIV_INNOCHECKSUM
  if (is_log_enabled && curr_algo == SRV_CHECKSUM_ALGORITHM_INNODB)
  {
    fprintf(log_file,
            "page::%" PRIuMAX
            ";"
            " old style: calculated = " ULINTPF "; recorded = " ULINTPF "\n",
            page_no, old_checksum, checksum_field2);
    fprintf(log_file,
            "page::%" PRIuMAX
            ";"
            " new style: calculated = " ULINTPF "; crc32 = %u; recorded = " ULINTPF "\n",
            page_no, new_checksum, buf_calc_page_crc32(read_buf), checksum_field1);
  }

  if (is_log_enabled && curr_algo == SRV_CHECKSUM_ALGORITHM_STRICT_INNODB)
  {
    fprintf(log_file,
            "page::%" PRIuMAX
            ";"
    ...
    fprintf(log_file,
            ...
            page_no, new_checksum, checksum_field1);
  }
#endif /* UNIV_INNOCHECKSUM */

  if (checksum_field2 != mach_read_from_4(read_buf + FIL_PAGE_LSN) && checksum_field2 != old_checksum)
  {
    return (false);
  }
...
UNIV_INLINE
bool buf_page_is_checksum_valid_none(const byte *read_buf, ulint checksum_field1, ulint checksum_field2
#ifdef UNIV_INNOCHECKSUM
                                     ,
                                     uintmax_t page_no, bool is_log_enabled, FILE *log_file,
                                     const srv_checksum_algorithm_t curr_algo
...
{
#ifdef UNIV_INNOCHECKSUM
  if (is_log_enabled && curr_algo == SRV_CHECKSUM_ALGORITHM_STRICT_NONE)
  {
    fprintf(log_file,
            "page::%" PRIuMAX
            "; none checksum: calculated = %lu;"
            " recorded checksum_field1 = " ULINTPF " recorded checksum_field2 = " ULINTPF "\n",
            page_no, BUF_NO_CHECKSUM_MAGIC, checksum_field1, checksum_field2);
  }
#endif /* UNIV_INNOCHECKSUM */

  return (checksum_field1 == checksum_field2 && checksum_field1 == BUF_NO_CHECKSUM_MAGIC);
}
...
ibool buf_page_is_corrupted(bool check_lsn, const byte *read_buf, const page_size_t &page_size, bool skip_checksum
#ifdef UNIV_INNOCHECKSUM
                            ,
                            uintmax_t page_no, bool strict_check, bool is_log_enabled, FILE *log_file
#endif /* UNIV_INNOCHECKSUM */
)
{
  ...
  if (!page_size.is_compressed() &&
      memcmp(read_buf + FIL_PAGE_LSN + 4, read_buf + page_size.logical() - FIL_PAGE_END_LSN_OLD_CHKSUM + 4, 4))
  {
  ...
  if (check_lsn && recv_lsn_checks_on)
  {
    ...
    {
      const ulint space_id = mach_read_from_4(read_buf + FIL_PAGE_SPACE_ID);
      const ulint page_no = mach_read_from_4(read_buf + FIL_PAGE_OFFSET);

      ib::error() << "Page " << page_id_t(space_id, page_no) << " log sequence number " << page_lsn
  ...
  if (srv_checksum_algorithm == SRV_CHECKSUM_ALGORITHM_NONE || skip_checksum)
  {
    return (FALSE);
  }
  ...
  {
#ifdef UNIV_INNOCHECKSUM
    return (!page_zip_verify_checksum(read_buf, page_size.physical(), page_no, strict_check, is_log_enabled, log_file));
#else
    return (!page_zip_verify_checksum(read_buf, page_size.physical()));
#endif /* UNIV_INNOCHECKSUM */
  }

  checksum_field1 = mach_read_from_4(read_buf + FIL_PAGE_SPACE_OR_CHKSUM);

  checksum_field2 = mach_read_from_4(read_buf + page_size.logical() - FIL_PAGE_END_LSN_OLD_CHKSUM);
...
#error "FIL_PAGE_LSN must be 64 bit aligned"
  ...
  if (checksum_field1 == 0 && checksum_field2 == 0 &&
      *reinterpret_cast<const ib_uint64_t *>(read_buf + FIL_PAGE_LSN) == 0)
  {
    ...
    for (i = 0; i < page_size.logical(); ++i)
    {
      ...
      if ((i < FIL_PAGE_FILE_FLUSH_LSN || i >= FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID) && read_buf[i] != 0)
      {
        break;
      }
    }
    ...
    if (i >= page_size.logical())
    {
      if (is_log_enabled)
      {
        fprintf(log_file, "Page::%" PRIuMAX " is empty and uncorrupted\n", page_no);
      }
      return (FALSE);
    }
#else
...
#ifndef UNIV_INNOCHECKSUM
  const page_id_t page_id(mach_read_from_4(read_buf + FIL_PAGE_SPACE_ID), mach_read_from_4(read_buf + FIL_PAGE_OFFSET));
#endif /* UNIV_INNOCHECKSUM */
  ...
  switch (curr_algo)
  {
    ...
    case SRV_CHECKSUM_ALGORITHM_STRICT_CRC32:
      ...
      if (buf_page_is_checksum_valid_crc32(read_buf, checksum_field1, checksum_field2,
#ifdef UNIV_INNOCHECKSUM
      ...
      if (buf_page_is_checksum_valid_none(read_buf, checksum_field1, checksum_field2
#ifdef UNIV_INNOCHECKSUM
                                          ,
                                          page_no, is_log_enabled, log_file, curr_algo))
      {
                                          ...
                                          ))
      {
        ...
        {
          page_warn_strict_checksum(curr_algo, SRV_CHECKSUM_ALGORITHM_NONE, page_id);
        }
#endif /* UNIV_INNOCHECKSUM */
        ...
        if (is_log_enabled)
        {
          fprintf(log_file,
                  ...
                  ";"
                  " recorded = " ULINTPF "\n",
                  page_no, buf_calc_page_old_checksum(read_buf), checksum_field2);
          fprintf(log_file,
                  "page::%" PRIuMAX
                  ";"
                  ...
                  page_no, buf_calc_page_new_checksum(read_buf), buf_calc_page_crc32(read_buf), checksum_field1);
        }
#endif /* UNIV_INNOCHECKSUM */
        return (FALSE);
      }
      ...
      if (buf_page_is_checksum_valid_innodb(read_buf, checksum_field1, checksum_field2
#ifdef UNIV_INNOCHECKSUM
                                            ,
                                            page_no, is_log_enabled, log_file, curr_algo))
      {
                                            ...
                                            ))
      {
        if (curr_algo == SRV_CHECKSUM_ALGORITHM_STRICT_CRC32)
        {
          page_warn_strict_checksum(curr_algo, SRV_CHECKSUM_ALGORITHM_INNODB, page_id);
        }
#endif /* UNIV_INNOCHECKSUM */
        return (FALSE);
      }
      ...
      if (!legacy_checksum_checked && buf_page_is_checksum_valid_crc32(read_buf, checksum_field1, checksum_field2,
#ifdef UNIV_INNOCHECKSUM
                                                                       page_no, is_log_enabled, log_file, curr_algo,
#endif /* UNIV_INNOCHECKSUM */
                                                                       true))
      ...
      if (is_log_enabled)
      {
        fprintf(log_file, "Fail; page %" PRIuMAX " invalid (fails crc32 checksum)\n", page_no);
      }
#endif /* UNIV_INNOCHECKSUM */
      return (TRUE);
    ...
    case SRV_CHECKSUM_ALGORITHM_STRICT_INNODB:
...
#ifdef UNIV_INNOCHECKSUM
      if (buf_page_is_checksum_valid_crc32(read_buf, checksum_field1, checksum_field2, page_no, is_log_enabled,
                                           log_file, curr_algo, false) ||
          buf_page_is_checksum_valid_crc32(read_buf, checksum_field1, checksum_field2, page_no, is_log_enabled,
                                           log_file, curr_algo, true))
      {
#else  /* UNIV_INNOCHECKSUM */
      if (buf_page_is_checksum_valid_crc32(read_buf, checksum_field1, checksum_field2, false) ||
          buf_page_is_checksum_valid_crc32(read_buf, checksum_field1, checksum_field2, true))
      {
        if (curr_algo == SRV_CHECKSUM_ALGORITHM_STRICT_INNODB)
        {
          page_warn_strict_checksum(curr_algo, SRV_CHECKSUM_ALGORITHM_CRC32, page_id);
        }
#endif /* UNIV_INNOCHECKSUM */
      ...
      if (is_log_enabled)
      {
        fprintf(log_file, "Fail; page %" PRIuMAX " invalid (fails innodb checksum)\n", page_no);
      }
#endif /* UNIV_INNOCHECKSUM */
      return (TRUE);
    ...
    case SRV_CHECKSUM_ALGORITHM_STRICT_NONE:
      ...
      if (is_log_enabled)
      {
        fprintf(log_file, "Fail; page %" PRIuMAX " invalid (fails none checksum)\n", page_no);
      }
#endif /* UNIV_INNOCHECKSUM */
      return (TRUE);
...
void buf_page_print(const byte *read_buf, const page_size_t &page_size, ulint flags)
{
  ...
  {
    ib::info() << "Page dump in ascii and hex (" << page_size.physical() << " bytes):";
  ...
  if (page_size.is_compressed())
  {
    ...
    ib::info() << "Compressed page type (" << fil_page_get_type(read_buf) << "); stored checksum in field1 "
               << mach_read_from_4(read_buf + FIL_PAGE_SPACE_OR_CHKSUM)
               << "; calculated checksums for field1: " << buf_checksum_algorithm_name(SRV_CHECKSUM_ALGORITHM_CRC32)
               << " " << page_zip_calc_checksum(read_buf, page_size.physical(), SRV_CHECKSUM_ALGORITHM_CRC32) << "/"
               << page_zip_calc_checksum(read_buf, page_size.physical(), SRV_CHECKSUM_ALGORITHM_CRC32, true) << ", "
               << buf_checksum_algorithm_name(SRV_CHECKSUM_ALGORITHM_INNODB) << " "
               << page_zip_calc_checksum(read_buf, page_size.physical(), SRV_CHECKSUM_ALGORITHM_INNODB) << ", "
               << buf_checksum_algorithm_name(SRV_CHECKSUM_ALGORITHM_NONE) << " "
               << page_zip_calc_checksum(read_buf, page_size.physical(), SRV_CHECKSUM_ALGORITHM_NONE) << "; page LSN "
               << mach_read_from_8(read_buf + FIL_PAGE_LSN) << "; page number (if stored to page"
  ...
  else
  {
    ...
    ib::info() << "Uncompressed page, stored checksum in field1 "
               ...
               << buf_checksum_algorithm_name(SRV_CHECKSUM_ALGORITHM_NONE) << " " << BUF_NO_CHECKSUM_MAGIC
               << ", stored checksum in field2 "
               << mach_read_from_4(read_buf + page_size.logical() - FIL_PAGE_END_LSN_OLD_CHKSUM)
               << ", calculated checksums for field2: " << buf_checksum_algorithm_name(SRV_CHECKSUM_ALGORITHM_CRC32)
               << " " << crc32 << "/" << crc32_legacy << ", "
               << buf_checksum_algorithm_name(SRV_CHECKSUM_ALGORITHM_INNODB) << " "
               << buf_calc_page_old_checksum(read_buf) << ", "
               << buf_checksum_algorithm_name(SRV_CHECKSUM_ALGORITHM_NONE) << " " << BUF_NO_CHECKSUM_MAGIC
               << ",  page LSN " << mach_read_from_4(read_buf + FIL_PAGE_LSN) << " "
               << mach_read_from_4(read_buf + FIL_PAGE_LSN + 4) << ", low 4 bytes of LSN at page end "
               << mach_read_from_4(read_buf + page_size.logical() - FIL_PAGE_END_LSN_OLD_CHKSUM + 4)
               << ", page number (if stored to page already) " << mach_read_from_4(read_buf + FIL_PAGE_OFFSET)
  ...
  switch (fil_page_get_type(read_buf))
  {
    ...
    case FIL_PAGE_RTREE:
      ...
      if (index)
      {
        ib::info() << "Index " << index_id << " is " << index->name << " in table " << index->table->name;
      }
#endif /* !UNIV_HOTBACKUP */
      break;
    case FIL_PAGE_INODE:
    ...
    buf_chunk_t *chunk) /*!< in/out: chunk of buffers */
{
  ...
  for (ulint i = 0; i < num_to_register; i++)
  {
    ...
    rwlock = &block->lock;
    ut_a(!rwlock->pfs_psi);
    rwlock->pfs_psi = (PSI_server) ? PSI_server->init_rwlock(buf_block_lock_key.m_value, rwlock) : NULL;

#ifdef UNIV_DEBUG
    rwlock = &block->debug_latch;
    ut_a(!rwlock->pfs_psi);
    rwlock->pfs_psi = (PSI_server) ? PSI_server->init_rwlock(buf_block_debug_latch_key.m_value, rwlock) : NULL;
#endif /* UNIV_DEBUG */
    ...
    byte *frame)          /*!< in: pointer to buffer frame */
{
  ...
  ut_d(rw_lock_create(PFS_NOT_INSTRUMENTED, &block->debug_latch, SYNC_NO_ORDER_CHECK));
...
/*********************************************************************/ /**
 ...
 given compressed page.
 @return buffer block pointing to the compressed page, or NULL */
buf_block_t *buf_pool_contains_zip(
    /*==================*/
    buf_pool_t *buf_pool, /*!< in: buffer pool instance */
    const void *data)     /*!< in: pointer to compressed page */
{
    ...
    ulint instance_no)    /*!< in: id of the instance */
{
  ...
  if (buf_pool_size > 0)
  {
    ...
    buf_pool->chunks = reinterpret_cast<buf_chunk_t *>(ut_zalloc_nokey(buf_pool->n_chunks * sizeof(*chunk)));
    ...
    buf_pool->read_ahead_area =
        ut_min(BUF_READ_AHEAD_PAGES, ut_2_power_up(buf_pool->curr_size / BUF_READ_AHEAD_PORTION));
    ...
    power of two */
    srv_n_page_hash_locks = static_cast<ulong>(ut_2_power_up(srv_n_page_hash_locks));
    ut_a(srv_n_page_hash_locks != 0);
    ut_a(srv_n_page_hash_locks <= MAX_PAGE_HASH_LOCKS);

    buf_pool->page_hash =
        ib_create(2 * buf_pool->curr_size, LATCH_ID_HASH_TABLE_RW_LOCK, srv_n_page_hash_locks, MEM_HEAP_FOR_PAGE_HASH);
...
static void buf_pool_free_instance(
    /*===================*/
...
static bool buf_page_realloc(buf_pool_t *buf_pool, buf_block_t *block)
{
  ...
  if (buf_page_can_relocate(&block->page))
  {
    ...
    ut_ad(new_block->lock_hash_val == lock_rec_hash(new_block->page.id.space(), new_block->page.id.page_no()));
...
static void buf_resize_status(const char *fmt, ...)
{
  ...
  ut_vsnprintf(export_vars.innodb_buffer_pool_resize_status, sizeof(export_vars.innodb_buffer_pool_resize_status), fmt,
               ap);
...
bool buf_frame_will_withdrawn(buf_pool_t *buf_pool, const byte *ptr)
{
  ...
  while (chunk < echunk)
  {
    if (ptr >= chunk->blocks->frame && ptr < (chunk->blocks + chunk->size - 1)->frame + UNIV_PAGE_SIZE)
    {
      return (true);
    }
    ++chunk;
...
static bool buf_pool_withdraw_blocks(buf_pool_t *buf_pool)
{
  ...
  ib::info() << "buffer pool " << i << " : start to withdraw the last " << buf_pool->withdraw_target << " blocks.";
  ...
  while (UT_LIST_GET_LEN(buf_pool->withdraw) < buf_pool->withdraw_target)
  {
    ...
    while (block != NULL && UT_LIST_GET_LEN(buf_pool->withdraw) < buf_pool->withdraw_target)
    {
      ...
      next_block = reinterpret_cast<buf_block_t *>(UT_LIST_GET_NEXT(list, &block->page));
    ...
    if (UT_LIST_GET_LEN(buf_pool->withdraw) < buf_pool->withdraw_target)
    {
      ...
      scan_depth = ut_min(ut_max(buf_pool->withdraw_target - UT_LIST_GET_LEN(buf_pool->withdraw),
      ...
      if (n_flushed)
      {
        MONITOR_INC_VALUE_CUMULATIVE(MONITOR_LRU_BATCH_FLUSH_TOTAL_PAGE, MONITOR_LRU_BATCH_FLUSH_COUNT,
    ...
    while (bpage != NULL)
    {
      ...
      if (bpage->zip.data != NULL && buf_frame_will_withdrawn(buf_pool, static_cast<byte *>(bpage->zip.data)))
      {
        if (buf_page_can_relocate(bpage))
        {
          mutex_exit(block_mutex);
          buf_pool_mutex_exit_forbid(buf_pool);
          if (!buf_buddy_realloc(buf_pool, bpage->zip.data, page_zip_get_size(&bpage->zip)))
          {
      ...
      if (buf_page_get_state(bpage) == BUF_BLOCK_FILE_PAGE &&
          buf_block_will_withdrawn(buf_pool, reinterpret_cast<buf_block_t *>(bpage)))
      {
        if (buf_page_can_relocate(bpage))
        {
          mutex_exit(block_mutex);
          buf_pool_mutex_exit_forbid(buf_pool);
          if (!buf_page_realloc(buf_pool, reinterpret_cast<buf_block_t *>(bpage)))
          {
            /* failed to allocate block */
            buf_pool_mutex_exit_allow(buf_pool);
            break;
    ...
    buf_resize_status("buffer pool %lu : withdrawing blocks. (%lu/%lu)", i, UT_LIST_GET_LEN(buf_pool->withdraw),
                      buf_pool->withdraw_target);

    ib::info() << "buffer pool " << i << " : withdrew " << count1 << " blocks from free list."
               << " Tried to relocate " << count2 << " pages (" << UT_LIST_GET_LEN(buf_pool->withdraw) << "/"
               << buf_pool->withdraw_target << ").";
  ...
  ib::info() << "buffer pool " << i << " : withdrawn target " << UT_LIST_GET_LEN(buf_pool->withdraw) << " blocks.";
...
void buf_pool_resize()
{
  ...
  buf_resize_status("Resizing buffer pool from " ULINTPF " to " ULINTPF " (unit=" ULINTPF ").", srv_buf_pool_old_size,
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; i++)
  {
    ...
    buf_pool->n_chunks_new = new_instance_size * UNIV_PAGE_SIZE / srv_buf_pool_chunk_unit;
...
withdraw_retry:
  ...
  if (should_retry_withdraw && ut_difftime(ut_time(), withdraw_started) >= message_interval)
  {
  ...
  {
    ib::info() << "Will retry to withdraw " << retry_interval << " seconds later.";
...
#ifndef NDEBUG
  {
    ...
    {
      should_wait = false;
      DBUG_EXECUTE_IF("ib_buf_pool_resize_wait_before_resize", should_wait = true; os_thread_sleep(10000););
    }
  }
#endif /* !NDEBUG */
  ...
  for (ulint i = 0; i < srv_buf_pool_instances; ++i)
  {
    ...
    if (buf_pool->n_chunks_new < buf_pool->n_chunks)
    {
      ...
      ib::info() << "buffer pool " << i << " : " << buf_pool->n_chunks - buf_pool->n_chunks_new << " chunks ("
...

    {
      ...
      buf_chunk_t *new_chunks = reinterpret_cast<buf_chunk_t *>(ut_zalloc_nokey_nofatal(new_chunks_size));

      DBUG_EXECUTE_IF("buf_pool_resize_chunk_null", buf_pool_resize_chunk_make_null(&new_chunks););
    ...
    if (buf_pool->n_chunks_new > buf_pool->n_chunks)
    {
      ...
      ib::info() << "buffer pool " << i << " : " << buf_pool->n_chunks_new - buf_pool->n_chunks << " chunks ("
  ...
  const bool new_size_too_diff =
      srv_buf_pool_base_size > srv_buf_pool_size * 2 || srv_buf_pool_base_size * 2 < srv_buf_pool_size;
  ...
  {
    ib::info() << "Completed to resize buffer pool from " << srv_buf_pool_old_size << " to " << srv_buf_pool_size
               << ".";
    srv_buf_pool_old_size = srv_buf_pool_size;
  }
...
*/
extern "C" os_thread_ret_t DECLARE_THREAD(buf_resize_thread)(void *arg MY_ATTRIBUTE((unused)))
{
  my_thread_init();
  ...
  while (srv_shutdown_state == SRV_SHUTDOWN_NONE)
  {
    ...
    if (srv_buf_pool_old_size == srv_buf_pool_size)
    {
      ...
      sout << "Size did not change (old size = new size = " << srv_buf_pool_size << ". Nothing to do.";
...
buf_page_t *buf_pool_watch_set(const page_id_t &page_id, rw_lock_t **hash_lock)
{
  buf_page_t *bpage;
  ulint i;
...
void buf_pool_watch_unset(const page_id_t &page_id)
{
  ...
  if (buf_block_unfix(bpage) == 0 && buf_pool_watch_is_sentinel(buf_pool, bpage))
  {
...
static void buf_page_make_young_if_needed(
    /*==========================*/
    ...
    ibool check)        /*!< in: TRUE=verify the page checksum */
{
  ...
  if (UNIV_UNLIKELY(check && !page_zip_verify_checksum(frame, size)))
  {
    ib::error() << "Compressed page checksum mismatch " << block->page.id
                ...
                << ", crc32: " << page_zip_calc_checksum(frame, size, SRV_CHECKSUM_ALGORITHM_CRC32) << "/"
                << page_zip_calc_checksum(frame, size, SRV_CHECKSUM_ALGORITHM_CRC32, true)
                << " innodb: " << page_zip_calc_checksum(frame, size, SRV_CHECKSUM_ALGORITHM_INNODB)
                << ", none: " << page_zip_calc_checksum(frame, size, SRV_CHECKSUM_ALGORITHM_NONE);
  ...
  switch (fil_page_get_type(frame))
  {
    ...
    case FIL_PAGE_RTREE:
      ...
      ib::error() << "Unable to decompress space " << block->page.id.space() << " page " << block->page.id.page_no();
...
ibool buf_pointer_is_block_field_instance(const buf_pool_t *buf_pool, const void *ptr)
{
...
buf_block_t *buf_page_get_gen(const page_id_t &page_id, const page_size_t &page_size, ulint rw_latch,
                              ...
                              bool dirty_with_no_latch)
{
  ...
  ut_ad(!ibuf_inside(mtr) || ibuf_page_low(page_id, page_size, FALSE, file, line, NULL));
...
loop:
  ...
  if (block != NULL)
  {
    ...
    if (!buf_pointer_is_block_field_instance(buf_pool, block) || !page_id.equals_to(block->page.id) ||
  ...
  if (block == NULL)
  {
    ...
    {
      ++retries;
      DBUG_EXECUTE_IF("innodb_page_corruption_retries", retries = BUF_PAGE_READ_MAX_RETRIES;);
    }
    else
    {
      ib::fatal() << "Unable to read page " << page_id << " into the buffer pool after " << BUF_PAGE_READ_MAX_RETRIES
                  << " attempts."
...
#if defined UNIV_DEBUG || defined UNIV_BUF_DEBUG
    ut_a(fsp_skip_sanity_check(page_id.space()) || ++buf_dbg_counter % 5771 || buf_validate());
#endif /* UNIV_DEBUG || UNIV_BUF_DEBUG */
    goto loop;
  }
  else
...
got_block:
  ...
  if (mode == BUF_GET_IF_IN_POOL || mode == BUF_PEEK_IF_IN_POOL)
  {
    ...
    mutex_enter(fix_mutex);
    const bool must_read = (buf_page_get_io_fix(fix_page) == BUF_IO_READ);
    mutex_exit(fix_mutex);
  ...
  switch (buf_block_get_state(fix_block))
  {
    ...
    case BUF_BLOCK_FILE_PAGE:
      bpage = &block->page;
      if (fsp_is_system_temporary(page_id.space()) && buf_page_get_io_fix(bpage) != BUF_IO_NONE)
      {
    ...
    case BUF_BLOCK_ZIP_DIRTY:
      ...
      if (bpage->buf_fix_count > 1 || buf_page_get_io_fix(bpage) != BUF_IO_NONE)
      {
  ...
  ut_ad(mode == BUF_GET_POSSIBLY_FREED || mode == BUF_PEEK_IF_IN_POOL || !fix_block->page.file_page_was_freed);
...
#if defined UNIV_DEBUG || defined UNIV_BUF_DEBUG
  ut_a(fsp_skip_sanity_check(block->page.id.space()) || ++buf_dbg_counter % 5771 || buf_validate());
...
static void buf_page_init(buf_pool_t *buf_pool, const page_id_t &page_id, const page_size_t &page_size,
                          buf_block_t *block)
{
  ...
  {
    ib::error() << "Page " << page_id << " already found in the hash table: " << hash_page << ", " << block;
...
@return pointer to the block or NULL */
buf_page_t *buf_page_init_for_read(dberr_t *err, ulint mode, const page_id_t &page_id, const page_size_t &page_size,
                                   ibool unzip)
{
  buf_block_t *block;
  ...
  *err = DB_SUCCESS;
  ...
  else
  {
    ...
    if (UNIV_UNLIKELY(lru))
    {
      ...
      if (UNIV_UNLIKELY(watch_page && !buf_pool_watch_is_sentinel(buf_pool, watch_page)))
      {
...
buf_block_t *buf_page_create(const page_id_t &page_id, const page_size_t &page_size, mtr_t *mtr)
{
  buf_frame_t *frame;
  buf_block_t *block;
  ...
  DBUG_PRINT("ib_buf", ("create page " UINT32PF ":" UINT32PF, page_id.space(), page_id.page_no()));
    ...
    enum buf_io_fix io_type) /*!< in: io_fix types */
{
  ...
  switch (fil_page_get_type(frame))
  {
    ...
    case FIL_PAGE_RTREE:
      ...
      if (btr_page_get_index_id(frame) == (index_id_t)(DICT_IBUF_ID_MIN + IBUF_SPACE_ID))
      {
        if (level == 0)
        {
...
/********************************************************************/ /**
 ...
 @return TRUE if successful */
static ibool buf_mark_space_corrupt(
    /*===================*/
...

{
  ...
  if (io_type == BUF_IO_READ)
  {
    ...
    if (bpage->id.space() == TRX_SYS_SPACE && buf_dblwr_page_inside(bpage->id.page_no()))
    {
      ib::error() << "Reading page " << bpage->id << ", which is in the doublewrite buffer!";
    }
    else if (read_space_id == 0 && read_page_no == 0)
    {
      /* This is likely an uninitialized page. */
    }
    else if ((bpage->id.space() != 0 && bpage->id.space() != read_space_id) || bpage->id.page_no() != read_page_no)
    {
      ...
      ib::error() << "Space id and page no stored in "
                  ...
                  << page_id_t(read_space_id, read_page_no) << ", should be " << bpage->id;
    }
    ...
    if (compressed_page || buf_page_is_corrupted(true, frame, bpage->size, fsp_is_checksum_disabled(bpage->id.space())))
    {
      /* Not a real corruption if it was triggered by
      error injection */
      DBUG_EXECUTE_IF(
          "buf_page_import_corrupt_failure",
          if (bpage->id.space() > TRX_SYS_SPACE && !Tablespace::is_undo_tablespace(bpage->id.space()) &&
    ...
    corrupt:
      ...
      if (srv_force_recovery < SRV_FORCE_IGNORE_CORRUPT)
      {
        ...
        if (bpage->id.space() > TRX_SYS_SPACE && buf_mark_space_corrupt(bpage))
        {
          return (false);
        }
        else
    ...
    DBUG_EXECUTE_IF("buf_page_import_corrupt_failure", page_not_corrupt : bpage = bpage;);
    ...
    if (uncompressed && !Compression::is_compressed_page(frame) && !recv_no_ibuf_operations &&
        !Tablespace::is_undo_tablespace(bpage->id.space()) && bpage->id.space() != srv_tmp_space.space_id() &&
        !srv_is_tablespace_truncated(bpage->id.space()) && fil_page_get_type(frame) == FIL_PAGE_INDEX &&
        page_is_leaf(frame))
    {
  ...
  DBUG_PRINT("ib_buf", ("%s page " UINT32PF ":" UINT32PF, io_type == BUF_IO_READ ? "read" : "wrote", bpage->id.space(),
                        bpage->id.page_no()));
...
/*********************************************************************/ /**
 ...
 @return TRUE */
static ibool buf_all_freed_instance(
    /*===================*/
...
static void buf_pool_invalidate_instance(
    /*=========================*/
    buf_pool_t *buf_pool) /*!< in: buffer pool instance */
{
  ulint i;
...
/*********************************************************************/ /**
 ...
 @return TRUE */
static ibool buf_pool_validate_instance(
    /*=======================*/
    buf_pool_t *buf_pool) /*!< in: buffer pool instance */
{
  buf_page_t *b;
...
#ifdef UNIV_IBUF_COUNT_DEBUG
          ut_a(buf_page_get_io_fix(&block->page) == BUF_IO_READ || !ibuf_count_get(block->page.id));
#endif
            ...
            case BUF_IO_WRITE:
              switch (buf_page_get_flush_type(&block->page))
              {
                ...
                case BUF_FLUSH_SINGLE_PAGE:
                  n_page_flush++;
                assert_s_latched:
                  ut_a(rw_lock_is_locked(&block->lock, RW_LOCK_S) || rw_lock_is_locked(&block->lock, RW_LOCK_SX));
                  break;
                case BUF_FLUSH_LIST:
                  n_list_flush++;
                  break;
  ...
  for (b = UT_LIST_GET_FIRST(buf_pool->zip_clean); b; b = UT_LIST_GET_NEXT(list, b))
  {
  ...
  buf_flush_list_mutex_enter(buf_pool);
  for (b = UT_LIST_GET_FIRST(buf_pool->flush_list); b; b = UT_LIST_GET_NEXT(list, b))
  {
  ...
  if (buf_pool->curr_size == buf_pool->old_size && n_lru + n_free > buf_pool->curr_size + n_zip)
  {
    ib::fatal() << "n_LRU " << n_lru << ", n_free " << n_free << ", pool " << buf_pool->curr_size << " zip " << n_zip
                << ". Aborting...";
  }

  ut_a(UT_LIST_GET_LEN(buf_pool->LRU) == n_lru);
  if (buf_pool->curr_size == buf_pool->old_size && UT_LIST_GET_LEN(buf_pool->free) != n_free)
  {
    ib::fatal() << "Free list len " << UT_LIST_GET_LEN(buf_pool->free) << ", free blocks " << n_free << ". Aborting...";
  }
...
static void buf_print_instance(
    /*===============*/
    buf_pool_t *buf_pool)
{
  ...
  index_ids = static_cast<index_id_t *>(ut_malloc_nokey(size * sizeof *index_ids));
  ...
  for (i = 0; i < n_found; i++)
  {
    ...
    if (!index)
    {
      ib::info() << "Block count for index " << index_ids[i] << " in buffer is about " << counts[i];
    }
    else
    {
      ib::info() << "Block count for index " << index_ids[i] << " in buffer is about " << counts[i] << ", index "
...
ulint buf_get_latched_pages_number_instance(
    /*==================================*/
    buf_pool_t *buf_pool) /*!< in: buffer pool instance */
{
  buf_page_t *b;
                                    ...
                                    to fill */
{
  ...
  pool_info->n_pending_flush_lru = (buf_pool->n_flush[BUF_FLUSH_LRU] + buf_pool->init_flush[BUF_FLUSH_LRU]);

  pool_info->n_pending_flush_list = (buf_pool->n_flush[BUF_FLUSH_LIST] + buf_pool->init_flush[BUF_FLUSH_LIST]);

  pool_info->n_pending_flush_single_page =
      (buf_pool->n_flush[BUF_FLUSH_SINGLE_PAGE] + buf_pool->init_flush[BUF_FLUSH_SINGLE_PAGE]);
  ...
  pool_info->pages_read_rate = (buf_pool->stat.n_pages_read - buf_pool->old_stat.n_pages_read) / time_elapsed;

  pool_info->pages_created_rate = (buf_pool->stat.n_pages_created - buf_pool->old_stat.n_pages_created) / time_elapsed;

  pool_info->pages_written_rate = (buf_pool->stat.n_pages_written - buf_pool->old_stat.n_pages_written) / time_elapsed;

  pool_info->n_page_get_delta = buf_pool->stat.n_page_gets - buf_pool->old_stat.n_page_gets;

  if (pool_info->n_page_get_delta)
  {
    pool_info->page_read_delta = buf_pool->stat.n_pages_read - buf_pool->old_stat.n_pages_read;

    pool_info->young_making_delta = buf_pool->stat.n_pages_made_young - buf_pool->old_stat.n_pages_made_young;

    pool_info->not_young_making_delta =
        buf_pool->stat.n_pages_not_made_young - buf_pool->old_stat.n_pages_not_made_young;
  }
  pool_info->pages_readahead_rnd_rate =
      (buf_pool->stat.n_ra_pages_read_rnd - buf_pool->old_stat.n_ra_pages_read_rnd) / time_elapsed;
    ...
    FILE *file)                 /*!< in/out: buffer where to print */
{
  ...
  fprintf(file,
          ...
          pool_info->pool_size, pool_info->free_list_len, pool_info->lru_len, pool_info->old_lru_len,
          pool_info->flush_list_len, pool_info->n_pend_reads, pool_info->n_pending_flush_lru,
          pool_info->n_pending_flush_list, pool_info->n_pending_flush_single_page);
  ...
  fprintf(file,
          ...
          pool_info->n_pages_made_young, pool_info->n_pages_not_made_young, pool_info->page_made_young_rate,
          pool_info->page_not_made_young_rate, pool_info->n_pages_read, pool_info->n_pages_created,
          pool_info->n_pages_written, pool_info->pages_read_rate, pool_info->pages_created_rate,
  ...
  if (pool_info->n_page_get_delta)
  {
    fprintf(file,
            ...
            (ulong)(1000 - (1000 * pool_info->page_read_delta / pool_info->n_page_get_delta)),
            (ulong)(1000 * pool_info->young_making_delta / pool_info->n_page_get_delta),
            (ulong)(1000 * pool_info->not_young_making_delta / pool_info->n_page_get_delta));
  }
  else
  {
  ...
  fprintf(file,
          ...
          pool_info->pages_readahead_rate, pool_info->pages_evicted_rate, pool_info->pages_readahead_rnd_rate);
  ...
  fprintf(file,
          ...
          pool_info->lru_len, pool_info->unzip_lru_len, pool_info->io_sum, pool_info->io_cur, pool_info->unzip_sum,
          pool_info->unzip_cur);
}
    ...
    FILE *file) /*!< in/out: buffer where to print */
{
  ...
  {
    pool_info = (buf_pool_info_t *)ut_zalloc_nokey((srv_buf_pool_instances + 1) * sizeof *pool_info);
  ...
  else
  {
    ...
    pool_info_total = pool_info = static_cast<buf_pool_info_t *>(ut_zalloc_nokey(sizeof *pool_info));
  }
...
/*==============================*/
{
  ...
  for (i = 0; i < srv_buf_pool_instances; i++)
  {
    ...
    pending_io += buf_pool->n_pend_reads + buf_pool->n_flush[BUF_FLUSH_LRU] + buf_pool->n_flush[BUF_FLUSH_SINGLE_PAGE] +
...
@param[in]	page_size	page size
@param[in,out]	block		block to init */
void buf_page_init_for_backup_restore(const page_id_t &page_id, const page_size_t &page_size, buf_block_t *block)
{
  block->page.state = BUF_BLOCK_FILE_PAGE;
...
{
  out << "[page id: space=" << page_id.m_space << ", page number=" << page_id.m_page_no << "]";
  return (out);
}
...
std::ostream &operator<<(std::ostream &out, const buf_pool_t &buf_pool)
{
  out << "[buffer pool instance: "
      << "buf_pool size=" << buf_pool.curr_size << ", database pages=" << UT_LIST_GET_LEN(buf_pool.LRU)
      << ", free pages=" << UT_LIST_GET_LEN(buf_pool.free)
      << ", modified database pages=" << UT_LIST_GET_LEN(buf_pool.flush_list)
      << ", n pending decompressions=" << buf_pool.n_pend_unzip << ", n pending reads=" << buf_pool.n_pend_reads
      << ", n pending flush LRU=" << buf_pool.n_flush[BUF_FLUSH_LRU] << " list=" << buf_pool.n_flush[BUF_FLUSH_LIST]
      << " single page=" << buf_pool.n_flush[BUF_FLUSH_SINGLE_PAGE]
      << ", pages made young=" << buf_pool.stat.n_pages_made_young
      << ", not young=" << buf_pool.stat.n_pages_not_made_young << ", pages read=" << buf_pool.stat.n_pages_read
      << ", created=" << buf_pool.stat.n_pages_created << ", written=" << buf_pool.stat.n_pages_written << "]";
  return (out);
}
#endif /* !UNIV_INNOCHECKSUM */
