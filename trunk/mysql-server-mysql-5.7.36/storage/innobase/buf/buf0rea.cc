...
static void buf_read_page_handle_error(
    /*=======================*/
...
static ulint buf_read_page_low(dberr_t *err, bool sync, ulint type, ulint mode, const page_id_t &page_id,
                               const page_size_t &page_size, bool unzip)
{
  ...
  *err = DB_SUCCESS;
  ...
  if (page_id.space() == TRX_SYS_SPACE && buf_dblwr_page_inside(page_id.page_no()))
  {
  ...
  DBUG_PRINT("ib_buf", ("read page %u:%u size=%u unzip=%u,%s", (unsigned)page_id.space(), (unsigned)page_id.page_no(),
                        (unsigned)page_size.physical(), (unsigned)unzip, sync ? "sync" : "async"));
  ...
  DBUG_EXECUTE_IF(
      "innodb_invalid_read_after_truncate", fil_space_t *space = fil_space_get(page_id.space());

      if (space != NULL && strcmp(space->name, "test/t1") == 0 && page_id.page_no() == space->size - 1) {
        type = IORequest::READ;
        sync = true;
      });
  ...
  *err = fil_io(request, sync, page_id, page_size, 0, page_size.physical(), dst, bpage);
  ...
  if (*err != DB_SUCCESS)
  {
    ...
    else if (IORequest::ignore_missing(type) || *err == DB_TABLESPACE_DELETED)
    {
      buf_read_page_handle_error(bpage);
      return (0);
    }
...
ulint buf_read_ahead_random(const page_id_t &page_id, const page_size_t &page_size, ibool inside_ibuf)
{
  ...
  dberr_t err;
  ulint i;
  const ulint buf_read_ahead_random_area = BUF_READ_AHEAD_AREA(buf_pool);
  ...
  low = (page_id.page_no() / buf_read_ahead_random_area) * buf_read_ahead_random_area;

  high = (page_id.page_no() / buf_read_ahead_random_area + 1) * buf_read_ahead_random_area;
  ...
  if (fil_space_t *space = fil_space_acquire(page_id.space()))
  {
    ...
    if (srv_file_per_table)
    {
      ...
      {
        size += os_file_get_size(node->handle) / page_size.physical();
      }
    }
#endif /* UNIV_DEBUG */
  ...
  for (i = low; i < high; i++)
  {
    ...
    const buf_page_t *bpage = buf_page_hash_get(buf_pool, page_id_t(page_id.space(), i));

    if (bpage != NULL && buf_page_is_accessed(bpage) && buf_page_peek_if_young(bpage))
    {
      recent_blocks++;
...
read_ahead:
  ...
  for (i = low; i < high; i++)
  {
    ...
    {
      count += buf_read_page_low(&err, false, IORequest::DO_NOT_WAKE, ibuf_mode, cur_page_id, page_size, false);
  ...
  if (count)
  {
    DBUG_PRINT("ib_buf", ("random read-ahead %u pages, %u:%u", (unsigned)count, (unsigned)page_id.space(),
...
ibool buf_read_page(const page_id_t &page_id, const page_size_t &page_size)
{
  ...
  if (err == DB_TABLESPACE_DELETED)
  {
    ib::error() << "trying to read page " << page_id << " in nonexisting or being-dropped tablespace";
  }
...
@param[in]	sync		true if synchronous aio is desired
@return TRUE if page has been read in, FALSE in case of failure */
ibool buf_read_page_background(const page_id_t &page_id, const page_size_t &page_size, bool sync)
{
  ulint count;
  dberr_t err;

  count = buf_read_page_low(&err, sync, IORequest::DO_NOT_WAKE | IORequest::IGNORE_MISSING, BUF_READ_ANY_PAGE, page_id,
                            page_size, false);
...
ulint buf_read_ahead_linear(const page_id_t &page_id, const page_size_t &page_size, ibool inside_ibuf)
{
  buf_pool_t *buf_pool = buf_pool_get(page_id);
  buf_page_t *bpage;
  buf_frame_t *frame;
  ...
  dberr_t err;
  ulint i;
  const ulint buf_read_ahead_linear_area = BUF_READ_AHEAD_AREA(buf_pool);
  ulint threshold;
  ...
  low = (page_id.page_no() / buf_read_ahead_linear_area) * buf_read_ahead_linear_area;
  high = (page_id.page_no() / buf_read_ahead_linear_area + 1) * buf_read_ahead_linear_area;
  ...
  threshold = ut_min(static_cast<ulint>(64 - srv_read_ahead_threshold), BUF_READ_AHEAD_AREA(buf_pool));

  fail_count = 0;

  for (i = low; i < high; i++)
  {
    bpage = buf_page_hash_get(buf_pool, page_id_t(page_id.space(), i));
  ...
  else if ((page_id.page_no() == high - 1) && (pred_offset == page_id.page_no() - 1))
  {
    /* This is ok, we can continue */
  ...
  {
    DBUG_PRINT("ib_buf",
               ("linear read-ahead %lu pages, " UINT32PF ":" UINT32PF, count, page_id.space(), page_id.page_no()));
  }
                            ...
                            in the arrays */
{
  ...
  for (ulint i = 0; i < n_stored; i++)
  {
    ...
    buf_read_page_low(&err, sync && (i + 1 == n_stored), 0, BUF_READ_ANY_PAGE, page_id, page_size, true);
  ...
  if (n_stored)
  {
    DBUG_PRINT("ib_buf", ("ibuf merge read-ahead %u pages, space %u", unsigned(n_stored), unsigned(space_ids[0])));
  }
}
...
void buf_read_recv_pages(bool sync, ulint space_id, const ulint *page_nos, ulint n_stored)
{
  ulint count;
  dberr_t err;
  ulint i;
  ...
  for (i = 0; i < n_stored; i++)
  {
    ...
    while (buf_pool->n_pend_reads >= recv_n_pool_free_frames / 2)
    {
      ...
      if (!(count % 1000))
      {
        ib::error() << "Waited for " << count / 100 << " seconds for " << buf_pool->n_pend_reads << " pending reads";
      }
    }

    if ((i + 1 == n_stored) && sync)
    {
      buf_read_page_low(&err, true, 0, BUF_READ_ANY_PAGE, cur_page_id, page_size, true);
    }
    else
    {
      buf_read_page_low(&err, false, IORequest::DO_NOT_WAKE, BUF_READ_ANY_PAGE, cur_page_id, page_size, true);
    }
  }
