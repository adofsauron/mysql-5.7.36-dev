    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  buf_block_t *block = buf_page_get(page_id_t(space, prev_page_no), page_size, shared ? RW_S_LATCH : RW_X_LATCH, mtr);
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  return (trx_undo_get_prev_rec_from_prev_page(rec, page_no, offset, shared, mtr));
}
...
static trx_undo_rec_t *trx_undo_get_next_rec_from_next_page(ulint space, const page_size_t &page_size,
                                                            const page_t *undo_page, ulint page_no, ulint offset,
    ...
    mtr_t *mtr)          /*!< in: mtr */
{
  ...
  return (trx_undo_get_next_rec_from_next_page(space, page_size, page_align(rec), page_no, offset, RW_S_LATCH, mtr));
}
...
@param[in,out]	mtr		mini-transaction
@return undo log record, the page latched, NULL if none */
trx_undo_rec_t *trx_undo_get_first_rec(ulint space, const page_size_t &page_size, ulint page_no, ulint offset,
                                       ulint mode, mtr_t *mtr)
{
  page_t *undo_page;
  trx_undo_rec_t *rec;
  ...
  return (trx_undo_get_next_rec_from_next_page(space, page_size, undo_page, page_no, offset, mode, mtr));
}
...
static void trx_undo_write_xid(
    ...
    mtr_t *mtr)           /*!< in: mtr */
{
  mlog_write_ulint(log_hdr + TRX_UNDO_XA_FORMAT, static_cast<ulint>(xid->get_format_id()), MLOG_4BYTES, mtr);

  mlog_write_ulint(log_hdr + TRX_UNDO_XA_TRID_LEN, static_cast<ulint>(xid->get_gtrid_length()), MLOG_4BYTES, mtr);

  mlog_write_ulint(log_hdr + TRX_UNDO_XA_BQUAL_LEN, static_cast<ulint>(xid->get_bqual_length()), MLOG_4BYTES, mtr);

  mlog_write_string(log_hdr + TRX_UNDO_XA_XID, reinterpret_cast<const byte *>(xid->get_data()), XIDDATASIZE, mtr);
}
...
{
  xid->set_format_id(static_cast<long>(mach_read_from_4(log_hdr + TRX_UNDO_XA_FORMAT)));

  xid->set_gtrid_length(static_cast<long>(mach_read_from_4(log_hdr + TRX_UNDO_XA_TRID_LEN)));

  xid->set_bqual_length(static_cast<long>(mach_read_from_4(log_hdr + TRX_UNDO_XA_BQUAL_LEN)));
                              ...
                              the rollback segment mutex */
{
  ...
  header_page = trx_undo_page_get(page_id_t(undo->space, undo->hdr_page_no), undo->page_size, mtr);
  ...
  new_block = fseg_alloc_free_page_general(TRX_UNDO_SEG_HDR + TRX_UNDO_FSEG_HEADER + header_page, undo->top_page_no + 1,
                                           FSP_UP, TRUE, mtr, mtr);
                       ...
                       the rollback segment mutex */
{
  ...
  undo_page = trx_undo_page_get(page_id_t(space, page_no), rseg->page_size, mtr);

  header_page = trx_undo_page_get(page_id_t(space, hdr_page_no), rseg->page_size, mtr);

  flst_remove(header_page + TRX_UNDO_SEG_HDR + TRX_UNDO_PAGE_LIST, undo_page + TRX_UNDO_PAGE_HDR + TRX_UNDO_PAGE_NODE,
              mtr);

  fseg_free_page(header_page + TRX_UNDO_SEG_HDR + TRX_UNDO_FSEG_HEADER, space, page_no, false, mtr);
                      ...
                      has allocated the undo log page */
{
  ...
  undo->last_page_no = trx_undo_free_page(undo->rseg, FALSE, undo->space, undo->hdr_page_no, undo->last_page_no, mtr);
...
@param[in,out]	mtr		mini-transaction */
static void trx_undo_empty_header_page(ulint space, const page_size_t &page_size, ulint hdr_page_no, ulint hdr_offset,
                                       mtr_t *mtr)
{
  page_t *header_page;
  trx_ulogf_t *log_hdr;
  ...
  header_page = trx_undo_page_get(page_id_t(space, hdr_page_no), page_size, mtr);
                      ...
                      >= this value should be truncated */
{
  ...
  for (;;)
  {
    ...
    undo_page = trx_undo_page_get(page_id_t(undo->space, last_page_no), undo->page_size, &mtr);
...
void trx_undo_truncate_start(trx_rseg_t *rseg, ulint hdr_page_no, ulint hdr_offset, undo_no_t limit)
{
  page_t *undo_page;
  trx_undo_rec_t *rec;
...
loop:
  ...
  rec = trx_undo_get_first_rec(rseg->space, rseg->page_size, hdr_page_no, hdr_offset, RW_X_LATCH, &mtr);
  if (rec == NULL)
  {
    /* Already empty */
  ...
  {
    trx_undo_empty_header_page(rseg->space, rseg->page_size, hdr_page_no, hdr_offset, &mtr);
  }
  else
  {
    trx_undo_free_page(rseg, TRUE, rseg->space, hdr_page_no, page_no, &mtr);
  }
...
static void trx_undo_seg_free(const trx_undo_t *undo, bool noredo)
{
  ...
  do
  {
    ...
    seg_header = trx_undo_page_get(page_id_t(undo->space, undo->hdr_page_no), undo->page_size, &mtr) + TRX_UNDO_SEG_HDR;
    ...
    mtr_t *mtr)       /*!< in: mtr */
{
  ...
  undo_page = trx_undo_page_get(page_id_t(rseg->space, page_no), rseg->page_size, mtr);
  ...
  last_page = trx_undo_page_get(page_id_t(rseg->space, undo->last_page_no), rseg->page_size, mtr);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t trx_undo_create(
    /*============*/
    trx_t *trx,        /*!< in: transaction */
    ...
    mtr_t *mtr)       /*!< in: mtr */
{
  ...
  undo_page = trx_undo_page_get(page_id_t(undo->space, undo->hdr_page_no), undo->page_size, mtr);
    ...
    mtr_t *mtr)       /*!< in: mtr */
{
  ...
  hdr_page = trx_undo_page_get(page_id_t(undo->space, undo->hdr_page_no), undo->page_size, mtr);
  ...
  mlog_write_ull(hdr_page + undo->hdr_offset + TRX_UNDO_TABLE_ID, undo->table_id, mtr);
                              ...
                              TRX_UNDO_UPDATE */
{
  ...
  DBUG_EXECUTE_IF("ib_create_table_fail_too_many_trx", err = DB_TOO_MANY_CONCURRENT_TRXS; goto func_exit;);
...
page_t *trx_undo_set_state_at_prepare(trx_t *trx, trx_undo_t *undo, bool rollback, mtr_t *mtr)
{
  ...
  if (rollback)
  {
    ...
    mlog_write_ulint(seg_hdr + TRX_UNDO_STATE, TRX_UNDO_ACTIVE, MLOG_2BYTES, mtr);
    return (undo_page);
  }
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  trx_purge_add_update_undo_to_history(trx, undo_ptr, undo_page, update_rseg_history_len, n_added_logs, mtr);
    ...
    trx_t *trx) /*!< in/out: PREPARED transaction */
{
  ...
  if (trx->rsegs.m_redo.update_undo)
  {
    ...
    UT_LIST_REMOVE(trx->rsegs.m_redo.rseg->update_undo_list, trx->rsegs.m_redo.update_undo);
  ...
  if (trx->rsegs.m_redo.insert_undo)
  {
    ...
    UT_LIST_REMOVE(trx->rsegs.m_redo.rseg->insert_undo_list, trx->rsegs.m_redo.insert_undo);
  ...
  if (trx->rsegs.m_noredo.update_undo)
  {
    ...
    UT_LIST_REMOVE(trx->rsegs.m_noredo.rseg->update_undo_list, trx->rsegs.m_noredo.update_undo);
  ...
  if (trx->rsegs.m_noredo.insert_undo)
  {
    ...
    UT_LIST_REMOVE(trx->rsegs.m_noredo.rseg->insert_undo_list, trx->rsegs.m_noredo.insert_undo);
...

{
  ...
  for (ulint i = 0; i < undo_trunc->rsegs_size(); ++i)
  {
    ...
    rseg->page_no = trx_rseg_header_create(space_id, univ_page_size, ULINT_MAX, rseg->id, &mtr);
    ...
    for (trx_undo_t *undo = UT_LIST_GET_FIRST(rseg->update_undo_cached); undo != NULL; undo = next_undo)
    {
    ...
    for (trx_undo_t *undo = UT_LIST_GET_FIRST(rseg->insert_undo_cached); undo != NULL; undo = next_undo)
    {
      next_undo = UT_LIST_GET_NEXT(undo_list, undo);
      UT_LIST_REMOVE(rseg->insert_undo_cached, undo);
      MONITOR_DEC(MONITOR_NUM_UNDO_SLOT_CACHED);
    ...
    rseg->curr_size = mtr_read_ulint(rseg_header + TRX_RSEG_HISTORY_SIZE, MLOG_4BYTES, &mtr) + 1;
  ...
  for (ulint i = 0; i < TRX_SYS_N_RSEGS; ++i)
  {
    ...
    if (rseg != NULL && rseg->space == undo_trunc->get_marked_space_id())
    {
      /* Reset the rollback segment slot in the trx
      system header */
      trx_sysf_rseg_set_page_no(sys_header, rseg->id, FIL_NULL, &mtr);
      /* Free a pending rollback segment instance in memory */
      trx_rseg_mem_free(rseg, trx_sys->pending_purge_rseg_array);
    }
  }
  mtr_commit(&mtr);
