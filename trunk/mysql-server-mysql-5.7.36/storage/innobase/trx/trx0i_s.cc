...
@param cache hash storage
@return maximum allowed allocation size */
#define MAX_ALLOWED_FOR_ALLOC(cache) (TRX_I_S_MEM_LIMIT - (cache)->mem_allocd - ha_storage_get_size((cache)->storage))
...
static ulint wait_lock_get_heap_no(
    /*==================*/
                                    ...
                                    allocated */
{
  ...
  else
  {
    ...
    for (i = 0; i < MEM_CHUNKS_IN_TABLE_CACHE; i++)
    {
      if (table_cache->chunks[i].offset + table_cache->chunks[i].rows_allocd > table_cache->rows_used)
      {
        break;
      }
    }
                                               ...
                                               strings */
{
  ...
  ut_ad(requested_lock_row == NULL || i_s_locks_row_validate(requested_lock_row));
  ...
  if (stmt_len > 0)
  {
    row->trx_query = static_cast<const char *>(
        ha_storage_put_memlim(cache->storage, query, stmt_len + 1, MAX_ALLOWED_FOR_STORAGE(cache)));
...
thd_done:
  ...
  if (s != NULL && s[0] != '\0')
  {
    TRX_I_S_STRING_COPY(s, row->trx_operation_state, TRX_I_S_TRX_OP_STATE_MAX_LEN, cache);
  ...
  if (s != NULL && s[0] != '\0')
  {
    TRX_I_S_STRING_COPY(s, row->trx_foreign_key_error, TRX_I_S_TRX_FK_ERROR_MAX_LEN, cache);
                               ...
                               by rec_get_offsets() */
{
  ...
  ret += row_raw_format((const char *)data, data_len, dict_field, buf, buf_size);
                            ...
                            volatile data */
{
  ...
  switch (heap_no)
  {
    ...
    case PAGE_HEAP_NO_SUPREMUM:
      *lock_data = ha_storage_put_str_memlim(
          cache->storage, heap_no == PAGE_HEAP_NO_INFIMUM ? "infimum pseudo-record" : "supremum pseudo-record",
  ...
  block = buf_page_try_get(page_id_t(lock_rec_get_space_id(lock), lock_rec_get_page_no(lock)), &mtr);
  ...
  {
    buf_used += put_nth_field(buf + buf_used, sizeof(buf) - buf_used, i, index, rec, offsets) - 1;
  }

  *lock_data = (const char *)ha_storage_put_memlim(cache->storage, buf, buf_used + 1, MAX_ALLOWED_FOR_STORAGE(cache));
                            ...
                            volatile strings */
{
  ...
  row->lock_table =
      ha_storage_put_str_memlim(cache->storage, lock_get_table_name(lock).m_name, MAX_ALLOWED_FOR_STORAGE(cache));
  ...
  switch (lock_get_type(lock))
  {
    case LOCK_REC:
      row->lock_index =
          ha_storage_put_str_memlim(cache->storage, lock_rec_get_index_name(lock), MAX_ALLOWED_FOR_STORAGE(cache));
...
static i_s_lock_waits_row_t *fill_lock_waits_row(
    /*================*/
                        ...
                        is a table lock */
{
  ...
  switch (lock_get_type(lock))
  {
    case LOCK_REC:
      ...
      ret = ut_fold_ulint_pair((ulint)lock_get_trx_id(lock), lock_rec_get_space_id(lock));

      ret = ut_fold_ulint_pair(ret, lock_rec_get_page_no(lock));
...
static ibool locks_row_eq_lock(
    /*==============*/
                                ...
                                is a table lock */
{
  ...
  switch (lock_get_type(lock))
  {
    case LOCK_REC:
      ...
      return (row->lock_trx_id == lock_get_trx_id(lock) && row->lock_space == lock_rec_get_space_id(lock) &&
              row->lock_page == lock_rec_get_page_no(lock) && row->lock_rec == heap_no);
    ...
    case LOCK_TABLE:
      ...
      return (row->lock_trx_id == lock_get_trx_id(lock) && row->lock_table_id == lock_get_table_id(lock));
...
/*******************************************************************/ /**
 ...
 @return row or NULL */
static i_s_locks_row_t *search_innodb_locks(
    /*================*/
...
/*******************************************************************/ /**
 ...
 If memory can not be allocated then FALSE is returned.
 @return FALSE if allocation fails */
static ibool add_lock_wait_to_cache(
    /*===================*/
    trx_i_s_cache_t *cache,                    /*!< in/out: cache */
    const i_s_locks_row_t *requested_lock_row, /*!< in: pointer to the
                                            relevant requested lock
    ...
    *requested_lock_row = add_lock_to_cache(cache, trx->lock.wait_lock, wait_lock_heap_no);
         ...
         curr_lock = lock_queue_iterator_get_prev(&iter))
    {
      if (lock_has_to_wait(trx->lock.wait_lock, curr_lock))
      {
        ...
        if (!add_lock_wait_to_cache(cache, *requested_lock_row, blocking_lock_row))
        {
          /* memory could not be allocated */
          return (FALSE);
        }
...
static ibool can_cache_be_updated(
    /*=================*/
    trx_i_s_cache_t *cache) /*!< in: cache */
{
    ...
    trx_ut_list_t *trx_list) /*!< in: trx list */
{
  ...
  for (trx = UT_LIST_GET_FIRST(*trx_list); trx != NULL;
       trx = (rw_trx_list ? UT_LIST_GET_NEXT(trx_list, trx) : UT_LIST_GET_NEXT(mysql_trx_list, trx)))
  {
...
/*******************************************************************/ /**
 Fetches the data needed to fill the 3 INFORMATION SCHEMA tables into the
 table cache buffer. Cache must be locked for write. */
static void fetch_data_into_cache(
    /*==================*/
    trx_i_s_cache_t *cache) /*!< in/out: cache */
{
  ut_ad(lock_mutex_own());
    ...
    enum i_s_table table)   /*!< in: which table */
{
  ...
  ut_ad(rw_lock_own(cache->rw_lock, RW_LOCK_S) || rw_lock_own(cache->rw_lock, RW_LOCK_X));
    ...
    ulint n)                /*!< in: row number */
{
  ...
  for (i = 0; i < MEM_CHUNKS_IN_TABLE_CACHE; i++)
  {
    if (table_cache->chunks[i].offset + table_cache->chunks[i].rows_allocd > n)
    {
      row = (char *)table_cache->chunks[i].base + (n - table_cache->chunks[i].offset) * table_cache->row_size;
      break;
    }
  }
                           ...
                           buffer */
{
  ...
  {
    /* record lock */
    res_len = ut_snprintf(lock_id, lock_id_size, TRX_ID_FMT ":%lu:%lu:%lu", row->lock_trx_id, row->lock_space,
                          row->lock_page, row->lock_rec);
  }
  else
  {
    /* table lock */
    res_len = ut_snprintf(lock_id, lock_id_size, TRX_ID_FMT ":" UINT64PF, row->lock_trx_id, row->lock_table_id);
  }
