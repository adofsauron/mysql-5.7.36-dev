...
/** Set of table_id */
typedef std::set<table_id_t, std::less<table_id_t>, ut_allocator<table_id_t>> table_id_set;

/** Dummy session used currently in MySQL interface */
sess_t *trx_dummy_sess = NULL;

/** Constructor */
TrxVersion::TrxVersion(trx_t *trx) : m_trx(trx), m_version(trx->version) { /* No op */ }

/** Set flush observer for the transaction
@param[in/out]	trx		transaction struct
@param[in]	observer	flush observer */
void trx_set_flush_observer(trx_t *trx, FlushObserver *observer) { trx->flush_observer = observer; }
...
struct TrxFactory
{
  ...
  static void init(trx_t *trx)
  {
    ...
    trx->detailed_error = reinterpret_cast<char *>(ut_zalloc_nokey(MAX_DETAILED_ERROR_LEN));
  ...
  static bool debug(const trx_t *trx)
  {
    ...
    ut_ad(trx->state == TRX_STATE_NOT_STARTED || trx->state == TRX_STATE_FORCED_ROLLBACK);
...
static void trx_validate_state_before_free(trx_t *trx)
{
  ...
  if (trx->n_mysql_tables_in_use != 0 || trx->mysql_n_tables_locked != 0)
  {
    ib::error() << "MySQL is freeing a thd though"
                ...
                << trx->n_mysql_tables_in_use << " and trx->mysql_n_tables_locked is " << trx->mysql_n_tables_locked
                << ".";
...
@param[in,out]	trx	transaction */
inline void trx_disconnect_plain(trx_t *trx) { trx_disconnect_from_mysql(trx, false); }

/** Disconnect a prepared transaction from MySQL.
@param[in,out]	trx	transaction */
void trx_disconnect_prepared(trx_t *trx) { trx_disconnect_from_mysql(trx, true); }
...
static void trx_resurrect_table_locks(
    /*======================*/
    ...
    const trx_undo_t *undo) /*!< in: undo log */
{
  ...
  undo_page = trx_undo_page_get(page_id_t(undo->space, undo->top_page_no), undo->page_size, &mtr);
  ...
  do
  {
    ...
    trx_undo_rec_get_pars(undo_rec, &type, &cmpl_info, &updated_extern, &undo_no, &table_id);
    tables.insert(table_id);

    undo_rec = trx_undo_get_prev_rec(undo_rec, undo->hdr_page_no, undo->hdr_offset, false, &mtr);
  } while (undo_rec);

  mtr_commit(&mtr);

  for (table_id_set::const_iterator i = tables.begin(); i != tables.end(); i++)
  {
    if (dict_table_t *table = dict_table_open_on_id(*i, FALSE, DICT_TABLE_OP_LOAD_TABLESPACE))
    {
      if (table->ibd_file_missing || dict_table_is_temporary(table))
      {
      ...
      DBUG_PRINT("ib_trx", ("resurrect" TRX_ID_FMT "  table '%s' IX lock from %s undo", trx_get_id_for_print(trx),
                            table->name.m_name, undo == undo_ptr->insert_undo ? "insert" : "update"));
    ...
    trx_rseg_t *rseg) /*!< in: rollback segment */
{
  ...
  *trx->xid = undo->xid;
  ...
  if (undo->state != TRX_UNDO_ACTIVE)
  {
    ...
    {
      ib::info() << "Transaction " << trx_get_id_for_print(trx) << " was in the XA prepared state.";
...
/*============================*/
{
  ...
  for (ulint i = 0; i < TRX_SYS_N_RSEGS; ++i)
  {
    ...
    for (undo = UT_LIST_GET_FIRST(rseg->insert_undo_list); undo != NULL; undo = UT_LIST_GET_NEXT(undo_list, undo))
    {
      trx_t *trx;
      ...
      trx_resurrect_table_locks(trx, &trx->rsegs.m_redo, undo);
    }

    /* Ressurrect transactions that were doing updates. */
    for (undo = UT_LIST_GET_FIRST(rseg->update_undo_list); undo != NULL; undo = UT_LIST_GET_NEXT(undo_list, undo))
    {
  ...
  for (TrxIdSet::iterator it = trx_sys->rw_trx_set.begin(); it != end; ++it)
  {
    ...
    if (it->m_trx->state == TRX_STATE_ACTIVE || it->m_trx->state == TRX_STATE_PREPARED)
    {
    ...
    ulint n_tablespaces) /*!< in: number of rollback tablespaces */
{
  ...
  while (!allocated)
  {
    for (;;)
    {
      ...
      else if (rseg->space == srv_sys_space.space_id() && n_tablespaces > 0 && trx_sys->rseg_array[slot] != NULL &&
               trx_sys->rseg_array[slot]->space != srv_sys_space.space_id())
      {
...
/******************************************************************/ /**
 ...
 @return assigned rollback segment instance */
static trx_rseg_t *get_next_noredo_rseg(
    /*=================*/
    ulong max_undo_logs) /*!< in: maximum number of UNDO logs to use */
{
  trx_rseg_t *rseg;
    ...
    trx_rseg_type_t rseg_type) /*!< in: type of rseg to assign. */
{
  ...
  ut_ad(rseg_type == TRX_RSEG_TYPE_REDO || trx_sys->rseg_array[1] != NULL);
  ...
  ut_ad(rseg_type == TRX_RSEG_TYPE_REDO || fsp_is_system_temporary(trx_sys->rseg_array[1]->space));
                ...
                to temp-table. */
{
  ...
  trx->rsegs.m_noredo.rseg = trx_assign_rseg_low(srv_rollback_segments, srv_undo_tablespaces, TRX_RSEG_TYPE_NOREDO);
    ...
    bool read_write) /*!< in: true if read-write transaction */
{
  ...
  trx->read_only = (trx->api_trx && !trx->read_write) ||
                   (!trx->ddl && !trx->internal && thd_trx_is_read_only(trx->mysql_thd)) || srv_read_only_mode;
  ...
  if (trx->is_dd_trx)
  {
    ut_ad(trx->read_only && trx->auto_commit && trx->isolation_level == TRX_ISO_READ_COMMITTED);
  }
#endif /* UNIV_DEBUG */
  ...
  {
    trx->rsegs.m_redo.rseg = trx_assign_rseg_low(srv_rollback_segments, srv_undo_tablespaces, TRX_RSEG_TYPE_REDO);
    ...
    ut_ad(trx->rsegs.m_redo.rseg != 0 || srv_read_only_mode || srv_force_recovery >= SRV_FORCE_NO_TRX_UNDO);
                                          ...
                                          referred undo rseg. */
{
      ...
      (noredo_rseg != NULL && noredo_rseg->last_page_no == FIL_NULL))
  {
    ...
    if (noredo_rseg != NULL && noredo_rseg->last_page_no == FIL_NULL)
    {
    ...
    mtr_t *mtr) /*!< in/out: mini-transaction */
{
  ...
  to purge queue. */
  if (trx->rsegs.m_redo.update_undo != NULL || trx->rsegs.m_noredo.update_undo != NULL)
  {
    ...
    trx_undo_ptr_t *redo_rseg_undo_ptr = trx->rsegs.m_redo.update_undo != NULL ? &trx->rsegs.m_redo : NULL;

    trx_undo_ptr_t *noredo_rseg_undo_ptr = trx->rsegs.m_noredo.update_undo != NULL ? &trx->rsegs.m_noredo : NULL;

    /* Will set trx->no and will add rseg to purge queue. */
    serialised = trx_serialisation_number_get(trx, redo_rseg_undo_ptr, noredo_rseg_undo_ptr);
    ...
    if (trx->rsegs.m_redo.update_undo != NULL)
    {
      ...
      undo_hdr_page = trx_undo_set_state_at_finish(trx->rsegs.m_redo.update_undo, mtr);
      ...
      trx_undo_update_cleanup(trx, &trx->rsegs.m_redo, undo_hdr_page, update_rseg_len, (update_rseg_len ? 1 : 0), mtr);
    }
    ...
    if (trx->rsegs.m_noredo.update_undo != NULL)
    {
      ...
      undo_hdr_page = trx_undo_set_state_at_finish(trx->rsegs.m_noredo.update_undo, &temp_mtr);

      ulint n_added_logs = (redo_rseg_undo_ptr != NULL) ? 2 : 1;

      trx_undo_update_cleanup(trx, &trx->rsegs.m_noredo, undo_hdr_page, true, n_added_logs, &temp_mtr);
    }
  }
    ...
    trx_t *trx) /*!< in: transaction */
{
  ...
  for (trx_mod_tables_t::const_iterator it = trx->mod_tables.begin(); it != end; ++it)
  {
...
static void trx_erase_lists(trx_t *trx, bool serialised)
{
  ...
  trx_ids_t::iterator it = std::lower_bound(trx_sys->rw_trx_ids.begin(), trx_sys->rw_trx_ids.end(), trx->id);
  ut_ad(*it == trx->id);
...
written */
{
  ...
  if (mtr != NULL)
  {
    ...
    else if (srv_flush_log_at_trx_commit == 0 || thd_requested_durability(trx->mysql_thd) == HA_IGNORE_DURABILITY)
    {
      /* Do nothing */
    }
    else
    ...
    trx_t *trx) /*!< in/out: transaction */
{
  ...
  DBUG_EXECUTE_IF("ib_trx_commit_crash_before_trx_commit_start", DBUG_SUICIDE(););
...
void trx_commit_complete_for_mysql(
    ...
    trx_t *trx) /*!< in/out: transaction */
{
  if (trx->id != 0 || !trx->must_flush_log_later || thd_requested_durability(trx->mysql_thd) == HA_IGNORE_DURABILITY)
  {
    return;
  }
...
state_ok:
  ...
  {
    innobase_mysql_print_thd(f, trx->mysql_thd, static_cast<uint>(max_query_len));
  }
}
                         ...
                         or 0 to use the default max length */
{
  ...
  trx_print_low(f, trx, max_query_len, lock_number_of_rows_locked(&trx->lock), UT_LIST_GET_LEN(trx->lock.trx_locks),
    ...
    const trx_t *b) /*!< in: transaction to be compared */
{
  ...
  a_notrans_edit = a->mysql_thd != NULL && thd_has_edited_nontrans_tables(a->mysql_thd);

  b_notrans_edit = b->mysql_thd != NULL && thd_has_edited_nontrans_tables(b->mysql_thd);
    ...
    ulint len)     /*!< in: number of slots in xid_list */
{
  ...
  for (trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list); trx != NULL; trx = UT_LIST_GET_NEXT(trx_list, trx))
  {
    assert_trx_in_rw_list(trx);
    ...
    if (trx_state_eq(trx, TRX_STATE_PREPARED))
    {
      ...
      ib::info() << "Transaction " << trx_get_id_for_print(trx) << " in prepared state after recovery";

      ib::info() << "Transaction contains changes to " << trx->undo_no << " rows";
                    ...
                    identifier */
{
  ...
  for (trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list); trx != NULL; trx = UT_LIST_GET_NEXT(trx_list, trx))
  {
    ...
    if (trx->is_recovered && trx_state_eq(trx, TRX_STATE_PREPARED) && xid->eq(trx->xid))
    {
...
void trx_kill_blocking(trx_t *trx)
{
  ...
  for (hit_list_t::reverse_iterator it = trx->hit_list.rbegin(); it != end; ++it)
  {
    ...
    while ((victim_trx->in_innodb & TRX_FORCE_ROLLBACK_MASK) > 0 && victim_trx->version == version)
    {
    ...
    thr_text = thd_security_context(victim_trx->mysql_thd, buffer, sizeof(buffer), 512);
    id = victim_trx->id;
#endif /* UNIV_DEBUG */
    trx_rollback_for_mysql(victim_trx);

#ifdef UNIV_DEBUG
    ib::info() << "High Priority Transaction (ID): " << trx->id << " killed transaction (ID): " << id << " in hit list"
               << " - " << thr_text;
#endif /* UNIV_DEBUG */
    trx_mutex_enter(victim_trx);
