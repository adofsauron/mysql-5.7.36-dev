...
TrxUndoRsegsIterator::TrxUndoRsegsIterator(trx_purge_t *purge_sys)
    : m_purge_sys(purge_sys), m_trx_undo_rsegs(NullElement), m_iter(m_trx_undo_rsegs.end())
{
}
...
const page_size_t TrxUndoRsegsIterator::set_next()
{
  ...
  else if (!m_purge_sys->purge_queue->empty())
  {
    ...
    while (!m_purge_sys->purge_queue->empty())
    {
      ...
      else if (purge_sys->purge_queue->top().get_trx_no() == m_trx_undo_rsegs.get_trx_no())
      {
  ...
  ut_a(srv_is_undo_tablespace(m_purge_sys->rseg->space) || is_system_tablespace(m_purge_sys->rseg->space));
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  rseg_header = trx_rsegf_get(undo->rseg->space, undo->rseg->page_no, undo->rseg->page_size, mtr);
  ...
  if (update_rseg_history_len)
  {
    ...
    if (trx_sys->rseg_history_len > srv_n_purge_threads * srv_purge_batch_size)
    {
...
{
  flst_remove(rseg_hdr + TRX_RSEG_HISTORY, log_hdr + TRX_UNDO_HISTORY_NODE, mtr);
...
static void trx_purge_free_segment(trx_rseg_t *rseg, fil_addr_t hdr_addr, bool noredo)
{
  ...
  for (;;)
  {
    ...
    rseg_hdr = trx_rsegf_get(rseg->space, rseg->page_no, rseg->page_size, &mtr);

    undo_page = trx_undo_page_get(page_id_t(rseg->space, hdr_addr.page), rseg->page_size, &mtr);
    ...
    if (fseg_free_step_not_header(seg_hdr + TRX_UNDO_FSEG_HEADER, false, &mtr))
    {
      break;
    }
    ...
    const purge_iter_t *limit) /*!< in: truncate offset */
{
  ...
  const bool noredo = trx_sys_is_noredo_rseg_slot(rseg->id);
...
loop:
  ...
  if (undo_trx_no >= limit->trx_no)
  {
    ...
    if (undo_trx_no == limit->trx_no && rseg->space == limit->undo_rseg_space)
    {
...
dberr_t populate_log_file_name(ulint space_id, char *&log_file_name)
{
  ulint log_file_name_sz = strlen(srv_log_group_home_dir) + 22 + 1 /* NUL */
  ...
  ut_snprintf(log_file_name + log_file_name_len, log_file_name_sz - log_file_name_len, "%s%lu_%s", undo::s_log_prefix,
...
dberr_t init(ulint space_id)
{
  ...
  indicate init phase. */
  bool ret;
  pfs_os_file_t handle = os_file_create(innodb_log_file_key, log_file_name, OS_FILE_CREATE, OS_FILE_NORMAL, OS_LOG_FILE,
...
void done(ulint space_id)
{
  ...
  indicate done phase. */
  bool ret;
  pfs_os_file_t handle = os_file_create_simple_no_error_handling(innodb_log_file_key, log_file_name, OS_FILE_OPEN,
...
bool is_log_present(ulint space_id)
{
  ...
  if (exist)
  {
    ...
    if (err != DB_SUCCESS)
    {
      ib::info() << "Unable to read '" << log_file_name << "' : " << ut_strerr(err);
...
static void trx_purge_mark_undo_for_truncate(undo::Truncate *undo_trunc)
{
  ...
  if (srv_undo_tablespaces_active < 2 || (srv_rollback_segments < (1 + srv_tmp_undo_logs + 2)))
  {
    return;
  }
  ...
  for (ulint i = 1; i <= srv_undo_tablespaces_active; i++)
  {
    ...
    if (fil_space_get_size(space_id) > (srv_max_undo_log_size / srv_page_size))
    {
    ...
    if (space_id >= (srv_undo_space_id_start + srv_undo_tablespaces_active))
    {
...
#ifdef UNIV_DEBUG
  ib::info() << "UNDO tablespace with space identifier " << undo_trunc->get_marked_space_id() << " marked for truncate";
#endif /* UNIV_DEBUG */
...
static void trx_purge_cleanse_purge_queue(undo::Truncate *undo_trunc)
{
  ...
  for (purge_elem_list_t::iterator it = purge_elem_list.begin(); it != purge_elem_list.end(); ++it)
  {
    for (TrxUndoRsegs::iterator it2 = it->begin(); it2 != it->end(); ++it2)
    {
      if ((*it2)->space == undo_trunc->get_marked_space_id())
      {
        it->erase(it2);
        break;
      }
...
@param[in,out]	undo_trunc	undo truncate tracker */
static void trx_purge_initiate_truncate(purge_iter_t *limit, undo::Truncate *undo_trunc)
{
  ...
  for (ulint i = 0; i < undo_trunc->rsegs_size() && all_free; ++i)
  {
    ...
    else
    {
      ...
      for (trx_undo_t *undo = UT_LIST_GET_FIRST(rseg->update_undo_cached); undo != NULL && all_free;
      ...
      for (trx_undo_t *undo = UT_LIST_GET_FIRST(rseg->insert_undo_cached); undo != NULL && all_free;
           undo = UT_LIST_GET_NEXT(undo_list, undo))
      {
        if (limit->trx_no < undo->trx_id)
        {
  ...
  DBUG_EXECUTE_IF("ib_undo_trunc_before_checkpoint", ib::info() << "ib_undo_trunc_before_checkpoint"; DBUG_SUICIDE(););
  ...
  ib::info() << "Truncating UNDO tablespace with space identifier " << undo_trunc->get_marked_space_id();

  DBUG_EXECUTE_IF("ib_undo_trunc_before_ddl_log_start", ib::info() << "ib_undo_trunc_before_ddl_log_start";
                  DBUG_SUICIDE(););

#ifdef UNIV_DEBUG
  dberr_t err =
#endif /* UNIV_DEBUG */
      undo_trunc->start_logging(undo_trunc->get_marked_space_id());
  ut_ad(err == DB_SUCCESS);

  DBUG_EXECUTE_IF("ib_undo_trunc_before_truncate", ib::info() << "ib_undo_trunc_before_truncate"; DBUG_SUICIDE(););
  ...
  DBUG_EXECUTE_IF("ib_undo_trunc_before_ddl_log_end", ib::info() << "ib_undo_trunc_before_ddl_log_end";
                  DBUG_SUICIDE(););
  ...
  DBUG_EXECUTE_IF("ib_undo_trunc_trunc_done", ib::info() << "ib_undo_trunc_trunc_done"; DBUG_SUICIDE(););
}
                            ...
                            handled */
{
  ...
  undo_page = trx_undo_page_get_s_latched(page_id_t(rseg->space, rseg->last_page_no), rseg->page_size, &mtr);
  ...
  log_hdr = trx_undo_page_get_s_latched(page_id_t(rseg->space, prev_log_addr.page), rseg->page_size, &mtr) +
...
static void trx_purge_read_undo_rec(trx_purge_t *purge_sys, const page_size_t &page_size)
{
  ...
  if (purge_sys->rseg->last_del_marks)
  {
    ...
    undo_rec = trx_undo_get_first_rec(purge_sys->rseg->space, page_size, purge_sys->hdr_page_no, purge_sys->hdr_offset,
                                      RW_S_LATCH, &mtr);
    ...
    mem_heap_t *heap)       /*!< in: memory heap where copied */
{
  ...
  undo_page = trx_undo_page_get_s_latched(page_id_t(space, page_no), page_size, &mtr);
...
static MY_ATTRIBUTE((warn_unused_result)) trx_undo_rec_t *trx_purge_fetch_next_rec(
    /*=====================*/
    ...
    ulint batch_size)       /*!< in: no. of pages to purge */
{
  ...
  for (thr = UT_LIST_GET_FIRST(purge_sys->query->thrs); thr != NULL && i < n_purge_threads;
  ...
  for (;;)
  {
    ...
    purge_rec = static_cast<trx_purge_rec_t *>(mem_heap_zalloc(node->heap, sizeof(*purge_rec)));
    ...
    purge_rec->undo_rec = trx_purge_fetch_next_rec(&purge_rec->roll_ptr, &n_pages_handled, node->heap);

    if (purge_rec->undo_rec != NULL)
    {
      if (node->undo_recs == NULL)
      {
        node->undo_recs = ib_vector_create(ib_heap_allocator_create(node->heap), sizeof(trx_purge_rec_t), batch_size);
      }
      else
      {
...
/*=====================*/
{
  ...
  if (srv_max_purge_lag > 0 && trx_sys->rseg_history_len > srv_n_purge_threads * srv_purge_batch_size)
  {
    float ratio;
    ...
    bool truncate)         /*!< in: truncate history if true */
{
  ...
  n_pages_handled = trx_purge_attach_undo_recs(n_purge_threads, purge_sys, batch_size);
