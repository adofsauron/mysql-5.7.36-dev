...
static const char *file_format_name_map[] = {
    "Antelope", "Barracuda", "Cheetah", "Dragon", "Elk",      "Fox",    "Gazelle",  "Hornet", "Impala",
    "Jaguar",   "Kangaroo",  "Leopard", "Moose",  "Nautilus", "Ocelot", "Porpoise", "Quail",  "Rabbit",
    "Shark",    "Tiger",     "Urchin",  "Viper",  "Whale",    "Xenops", "Yak",      "Zebra"};
    ...
    const trx_t *in_trx) /*!< in: transaction */
{
  ...
  for (trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list); trx != NULL && trx != in_trx;
...
/*===================================*/
{
  ...
  ib::info() << "Last MySQL binlog file position " << trx_sys_mysql_bin_log_pos_high << " "
                            ...
                            0 means next free slot. */
{
  ...
  for (i = 0; i < TRX_SYS_N_RSEGS; i++)
  {
    ...
    if (page_no == FIL_NULL || (include_tmp_slots && trx_sys_is_noredo_rseg_slot(i)))
    {
...
static ulint trx_sysf_used_slots_for_redo_rseg(
    /*==============================*/
    mtr_t *mtr) /*!< in: mtr */
{
  trx_sysf_t *sys_header;
    ...
    mtr_t *mtr) /*!< in: mtr */
{
  ...
  page_no = trx_rseg_header_create(TRX_SYS_SPACE, univ_page_size, ULINT_MAX, slot_no, mtr);
...
/*==========================*/
{
  ...
  trx_sys->max_trx_id =
      ...
      ut_uint64_align_up(mach_read_from_8(sys_header + TRX_SYS_TRX_ID_STORE), TRX_SYS_TRX_ID_WRITE_MARGIN);
...
/*================*/
{
  ...
  new (&trx_sys->rw_trx_ids) trx_ids_t(ut_allocator<trx_id_t>(mem_key_trx_sys_t_rw_trx_ids));
...
dberr_t trx_sys_file_format_max_check(
    /*==========================*/
    ulint max_format_id) /*!< in: max format id to check */
{
  ...
  ib::info() << "Highest supported file format is " << trx_sys_file_format_id_to_name(UNIV_FORMAT_MAX) << ".";
...
static ulint trx_sys_create_noredo_rsegs(
    /*========================*/
                       ...
                       temp-tables. */
{
  ...
  if (!srv_force_recovery && !recv_needed_recovery && n_used < n_rsegs)
  {
    ...
    {
      ulint space_id;
      space_id = (n_spaces == 0) ? 0 : (srv_undo_space_id_start + i % n_spaces);
      ...
      if (trx_rseg_create(space_id, 0) != NULL)
      {
        ...
        ulint last_undo_space = srv_undo_space_id_start + (srv_undo_tablespaces_active - 1);
  ...
  ib::info() << n_used - srv_tmp_undo_logs << " redo rollback segment(s) found. " << n_redo_active
             << " redo rollback segment(s) are active.";

  ib::info() << n_noredo_created
             << " non-redo rollback segment(s) are"
                " active.";
                      ...
                      TRX_SYS_PAGE_NO in the tablespace */
{
      ...
      TRX_SYS_MYSQL_LOG_MAGIC_N)
  {
    ib::info() << "mysqlbackup: Last MySQL binlog file position "
               << mach_read_from_4(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_HIGH) << " "
               << mach_read_from_4(sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_OFFSET_LOW) << ", file name "
               << sys_header + TRX_SYS_MYSQL_LOG_INFO + TRX_SYS_MYSQL_LOG_NAME;
  }
}
                                    ...
                                    space */
{
  ...
  *format_id = ULINT_UNDEFINED;
  ...
  file = os_file_create_simple_no_error_handling(innodb_data_file_key, pathname, OS_FILE_OPEN, OS_FILE_READ_ONLY,
  ...
  IORequest read_req(IORequest::READ)
      ...
      dberr_t err =
          os_file_read_no_error_handling(read_req, file, page, TRX_SYS_PAGE_NO * UNIV_PAGE_SIZE, UNIV_PAGE_SIZE, NULL);
                                    ...
                                    data file */
{
  ...
  *format_id = ULINT_UNDEFINED;
  ...
  dberr_t err = os_file_read_no_error_handling(read_req, file, page, 0, UNIV_PAGE_SIZE, NULL);
...
/*===============*/
{
  ...
  for (trx_t *trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list); trx != NULL; trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list))
  {
  ...
  trx_rseg_t **rseg_array = static_cast<trx_rseg_t **>(trx_sys->rseg_array);
...
/*=================================*/
{
  ...
  if (total_trx == 0)
  {
    ...
    if (total_trx > trx_sys->n_prepared_trx && srv_force_recovery >= SRV_FORCE_NO_TRX_UNDO)
    {
    ...
    trx_ut_list_t *trx_list) /*!< in: &trx_sys->rw_trx_list */
{
  ...
  for (trx = UT_LIST_GET_FIRST(*trx_list); trx != NULL; prev_trx = trx, trx = UT_LIST_GET_NEXT(trx_list, prev_trx))
  {
    check_trx_state(trx);
