...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t trx_rollback_to_savepoint_for_mysql_low(
    /*====================================*/
    trx_t *trx,                /*!< in/out: transaction */
                                     ...
                                     executed after the savepoint */
{
  ...
  switch (trx->state)
  {
    ...
    case TRX_STATE_FORCED_ROLLBACK:
      ...
      ib::error() << "Transaction has a savepoint " << savep->name << " though it is not started";

      return (DB_ERROR);

    case TRX_STATE_ACTIVE:

      return (trx_rollback_to_savepoint_for_mysql_low(trx, savep, mysql_binlog_cache_pos));
    ...
    trx_t *trx) /*!< in/out: transaction */
{
  ...
  ib::info() << "Rolling back trx with id " << trx_id << ", " << rows_to_undo << unit << " rows to undo";
  ...
  if (trx_get_dict_operation(trx) != TRX_DICT_OP_NONE && trx->table_id != 0)
  {
    ...
    table = dict_table_open_on_id(trx->table_id, TRUE, DICT_TABLE_OP_NORMAL);

    if (table && !dict_table_is_discarded(table))
    {
      ib::warn() << "Dropping table '" << table->name << "', with id " << trx->table_id << " in recovery";
...
extern "C" os_thread_ret_t DECLARE_THREAD(trx_rollback_or_clean_all_recovered)(
    /*================================================*/
...
static page_t *trx_roll_pop_top_rec(
    /*=================*/
    trx_t *trx,         /*!< in: transaction */
    ...
    ulint *undo_offset) /*!< out: offset of undo record in the page */
{
  ...
  page_t *undo_page = trx_undo_page_get_s_latched(page_id_t(undo->space, undo->top_page_no), undo->page_size, mtr);

  *undo_offset = undo->top_offset;

  trx_undo_rec_t *prev_rec = trx_undo_get_prev_rec((trx_undo_rec_t *)(undo_page + *undo_offset), undo->hdr_page_no,
...
trx_undo_rec_t *trx_roll_pop_top_rec_of_trx_low(
    /*============================*/
    trx_t *trx,               /*!< in/out: transaction */
    trx_undo_ptr_t *undo_ptr, /*!< in: rollback segment to look
                              for next undo log record. */
    ...
    mem_heap_t *heap)         /*!< in/out: memory heap where copied */
{
  ...
  *roll_ptr = trx_undo_build_roll_ptr(is_insert, undo->rseg->id, undo->top_page_no, undo->top_offset);
  ...
  ut_ad(trx_roll_check_undo_rec_ordering(undo_no, undo->rseg->space, trx));
    ...
    mem_heap_t *heap)     /*!< in: memory heap where copied */
{
  ...
  {
    undo_rec = trx_roll_pop_top_rec_of_trx_low(trx, &trx->rsegs.m_redo, limit, roll_ptr, heap);
  }

  if (undo_rec == 0 && trx_is_noredo_rseg_updated(trx))
  {
    undo_rec = trx_roll_pop_top_rec_of_trx_low(trx, &trx->rsegs.m_noredo, limit, roll_ptr, heap);
  }
