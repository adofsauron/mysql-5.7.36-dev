...
@return remaining part of undo log record after reading these values */
const byte *trx_undo_read_v_idx(const dict_table_t *table, const byte *ptr, bool first_v_col, bool *is_undo_log,
                                ulint *field_no)
{
    ...
    mtr_t *mtr)                  /*!< in: mtr */
{
  ...
  *ptr++ = TRX_UNDO_INSERT_REC;
  ...
  {
    if (!trx_undo_report_insert_virtual(undo_page, index->table, clust_entry, &ptr))
    {
      return (0);
    }
  }
...
ext_buf
@return ext_buf */
static byte *trx_undo_page_fetch_ext(byte *ext_buf, ulint prefix_len, const page_size_t &page_size, const byte *field,
                                     ulint *len)
{
  /* Fetch the BLOB. */
  ulint ext_len = btr_copy_externally_stored_field_prefix(ext_buf, prefix_len, page_size, field, *len);
...
static byte *trx_undo_page_report_modify_ext(byte *ptr, byte *ext_buf, ulint prefix_len, const page_size_t &page_size,
                                             const byte **field, ulint *len, spatial_status_t spatial_status)
{
  ulint spatial_len = 0;
  ...
  if (ext_buf)
  {
    ...
    *field = trx_undo_page_fetch_ext(ext_buf, prefix_len, page_size, *field, len);

    ptr += mach_write_compressed(ptr, *len + spatial_len);
  }
  else
  {
    ptr += mach_write_compressed(ptr, UNIV_EXTERN_STORAGE_FIELD + *len + spatial_len);
  }
...
static void trx_undo_get_mbr_from_ext(
    /*======================*/
    double *mbr, const page_size_t &page_size, const byte *field, ulint *len)
{
  uchar *dptr = NULL;
  ulint dlen;
  mem_heap_t *heap = mem_heap_create(100);

  dptr = btr_copy_externally_stored_field(&dlen, field, page_size, *len, heap);
    ...
    mtr_t *mtr)           /*!< in: mtr */
{
  ...
  undo_ptr = dict_table_is_temporary(index->table) ? &trx->rsegs.m_noredo : &trx->rsegs.m_redo;
  ...
  if (update)
  {
    ...
    if (dict_index_is_online_ddl(index) && index->table->n_v_cols > 0)
    {
      for (i = 0; i < upd_get_n_fields(update); i++)
      {
        ...
        on them */
        if (upd_fld_is_virtual_col(fld) && dict_table_get_nth_v_col(table, pos)->v_indexes->empty())
        {
          n_updated--;
        }
      }
    ...
    for (i = 0; i < upd_get_n_fields(update); i++)
    {
      ...
      if (is_virtual)
      {
        ...
        if (dict_index_is_online_ddl(index) && dict_table_get_nth_v_col(table, pos)->v_indexes->empty())
        {
          continue;
        }
      ...
      if (!is_virtual && rec_offs_nth_extern(offsets, pos))
      {
        ...
        ptr = trx_undo_page_report_modify_ext(
            ptr, col->ord_part && !ignore_prefix && flen < REC_ANTELOPE_MAX_INDEX_COL_LEN ? ext_buf : NULL, prefix_len,
            dict_table_page_size(table), &field, &flen, SPATIAL_UNKNOWN);
      ...
      if (col->ord_part)
      {
        ...
        if (rec_offs_nth_extern(offsets, pos))
        {
          ...
          if (spatial_status != SPATIAL_NONE)
          {
            ...
            trx_undo_get_mbr_from_ext(mbr, dict_table_page_size(table), field, &flen);
          }

          ptr = trx_undo_page_report_modify_ext(
              ptr, flen < REC_ANTELOPE_MAX_INDEX_COL_LEN && !ignore_prefix ? ext_buf : NULL, prefix_len,
              dict_table_page_size(table), &field, &flen, spatial_status);
        }
        else
        {
          ptr += mach_write_compressed(ptr, flen);
    ...
    upd_t **upd)         /*!< out, own: update vector */
{
  ...
  for (i = 0; i < n_fields; i++)
  {
    ...
    if (is_virtual)
    {
      ...
      ptr = trx_undo_read_v_idx(index->table, ptr, first_v_col, &is_undo_log, &field_no);
      first_v_col = false;
    }
    else if (field_no >= dict_index_get_n_fields(index))
    {
      ib::error() << "Trying to access update undo rec"
                     " field "
                  << field_no << " in index " << index->name << " of table " << index->table->name
                  << " but index has only " << dict_index_get_n_fields(index) << " fields " << BUG_REPORT_MSG
                         ...
                         needed is allocated */
{
  ...
  *row = dtuple_create_with_vcol(heap, dict_table_get_n_cols(index->table), dict_table_get_n_v_cols(index->table));
  ...
  while (ptr != end_ptr)
  {
    ...
    else
    {
      ...
      dict_col_copy_type(dict_table_get_nth_col(index->table, col_no), dfield_get_type(dfield));
    }
    ...
    if (len != UNIV_SQL_NULL && len >= UNIV_EXTERN_STORAGE_FIELD)
    {
      ...
      spatial_status = static_cast<spatial_status_t>((len & SPATIAL_STATUS_MASK) >> SPATIAL_STATUS_SHIFT);
...
static MY_ATTRIBUTE((nonnull)) ibool trx_undo_erase_page_end(
    /*====================*/
                                 ...
                                 flag was specified */
{
  ...
  undo_block = buf_page_get_gen(page_id_t(undo->space, page_no), undo->page_size, RW_X_LATCH, undo->guess_block,
  ...
  do
  {
    ...
    switch (op_type)
    {
      ...
      default:
        ut_ad(op_type == TRX_UNDO_MODIFY_OP);
        offset = trx_undo_page_report_modify(undo_page, trx, index, rec, offsets, update, cmpl_info, clust_entry, &mtr);
    }
    ...
    else
    {
      ...
      *roll_ptr = trx_undo_build_roll_ptr(op_type == TRX_UNDO_INSERT_OP, undo_ptr->rseg->id, page_no, offset);
      return (DB_SUCCESS);
    }
  ...
  ib_errf(
      ...
      " the tablespace",
      ((undo->space == srv_sys_space.space_id()) ? "system"
    ...
    bool is_redo_rseg)   /*!< in: true if redo rseg. */
{
  ...
  undo_page = trx_undo_page_get_s_latched(page_id_t(rseg->space, page_no), rseg->page_size, &mtr);
...
static MY_ATTRIBUTE((warn_unused_result)) bool trx_undo_get_undo_rec(
    /*==================*/
    roll_ptr_t roll_ptr, trx_id_t trx_id, mem_heap_t *heap, bool is_redo_rseg, const table_name_t &name,
    trx_undo_rec_t **undo_rec)
{
  ...
  if (!missing_history)
  {
    *undo_rec = trx_undo_get_undo_rec_low(roll_ptr, heap, is_redo_rseg);
  }
...

{
  ...
  *old_vers = NULL;
  ...
  bool is_redo_rseg = dict_table_is_temporary(index->table) ? false : true;
  if (trx_undo_get_undo_rec(roll_ptr, rec_trx_id, heap, is_redo_rseg, index->table->name, &undo_rec))
  {
  ...
  if (row_upd_changes_field_size_or_external(index, offsets, update))
  {
    ...
    if ((update->info_bits & REC_INFO_DELETED_FLAG) && row_upd_changes_disowned_external(update))
    {
      bool missing_extern;
    ...
    buf = static_cast<byte *>(mem_heap_alloc(heap, rec_get_converted_size(index, entry, n_ext)));
  ...
  {
    row_upd_replace_vcol((dtuple_t *)*vrow, index->table, update, false, NULL, NULL);
  }

#if defined UNIV_DEBUG || defined UNIV_BLOB_LIGHT_DEBUG
  ut_a(!rec_offs_any_null_extern(*old_vers, rec_get_offsets(*old_vers, index, NULL, ULINT_UNDEFINED, &heap)));
  ...
  if (vrow && !(cmpl_info & UPD_NODE_NO_ORD_CHANGE))
  {
    if (!(*vrow))
    {
      *vrow = dtuple_create_with_vcol(v_heap ? v_heap : heap, dict_table_get_n_cols(index->table),
                                      dict_table_get_n_v_cols(index->table));
      dtuple_init_v_fld(*vrow);
    }

    ut_ad(index->table->n_v_cols);
    trx_undo_read_v_cols(index->table, ptr, *vrow, v_status & TRX_UNDO_PREV_IN_PURGE, NULL);
  }
...
void trx_undo_read_v_cols(const dict_table_t *table, const byte *ptr, const dtuple_t *row, bool in_purge,
                          const ulint *col_map)
{
  const byte *end_ptr;
  bool first_v_col = true;
