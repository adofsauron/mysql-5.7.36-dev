...
ulint trx_rseg_header_create(ulint space, const page_size_t &page_size, ulint max_size, ulint rseg_slot_no, mtr_t *mtr)
{
  ulint page_no;
  trx_rsegf_t *rsegf;
                             ...
                             central array. */
{
  ...
  for (undo = UT_LIST_GET_FIRST(rseg->update_undo_cached); undo != NULL; undo = next_undo)
  {
  ...
  for (undo = UT_LIST_GET_FIRST(rseg->insert_undo_cached); undo != NULL; undo = next_undo)
  {
    next_undo = UT_LIST_GET_NEXT(undo_list, undo);
...
static trx_rseg_t *trx_rseg_mem_create(ulint id, ulint space, ulint page_no, const page_size_t &page_size,
                                       purge_pq_t *purge_queue, trx_rseg_t **rseg_array, mtr_t *mtr)
{
  ulint len;
  trx_rseg_t *rseg;
  ...
  *((trx_rseg_t **)rseg_array + rseg->id) = rseg;
  ...
  if (len > 0)
  {
    ...
    undo_log_hdr = trx_undo_page_get(page_id_t(rseg->space, node_addr.page), rseg->page_size, mtr) + node_addr.boffset;
...
static const page_size_t read_sys_rseg_info(ulint rseg_id, mtr_t *mtr, ulint &space, ulint &page_no)
{
  ...
  const page_size_t &page_size = is_system_tablespace(space) ? univ_page_size : fil_space_get_page_size(space, &found);
...
static void trx_rseg_initialize(ulint rseg_id, trx_rseg_t **rseg_array, purge_pq_t *purge_queue)
{
  ...
  const page_size_t &page_size = read_sys_rseg_info(rseg_id, &mtr, space, page_no);
  ...
  trx_rseg_t *rseg = trx_rseg_mem_create(rseg_id, space, page_no, page_size, purge_queue, rseg_array, &mtr);
...
static bool check_duplicate_rseg(ulint space, ulint page_no)
{
  for (ulint rseg_id = 0; rseg_id < TRX_SYS_N_RSEGS; rseg_id++)
  {
    ...
    if (rseg != NULL && rseg->space == space && rseg->page_no == page_no)
    {
      ib::info() << "Found duplicate reference rseg: " << rseg_id << " space: " << space << " page: " << page_no;
      return (true);
    }
  }
  return (false);
...
*/
static bool is_purge_pending(ulint rseg_id, bool &reset_rseg)
{
  ...
  {
    mtr.commit();
    ib::info() << "Reset pre-5.7.2 rseg: " << rseg_id << " after duplicate is found.";
    return (false);
  }

  trx_rsegf_t *rseg_header = trx_rsegf_get_new(space, page_no, page_size, &mtr);
  ...
  if (len > 0)
  {
    ib::info() << "pre-5.7.2 rseg: " << rseg_id << " holds data to be purged. History length: " << len
               << ". Recommend slow shutdown with"
...
@param[in,out]	purge_queue	queue for rollback segments that need purging
*/
static void trx_rseg_create_instance(purge_pq_t *purge_queue)
{
  ...
  for (ulint rseg_id = 0; rseg_id < TRX_SYS_N_RSEGS; rseg_id++)
  {
    ...
    {
      trx_rseg_initialize(rseg_id, trx_sys->pending_purge_rseg_array, purge_queue);
      continue;
    }
...
@param[in]	rseg_id	rollback segment ID */
static void trx_rseg_reset_slot(ulint rseg_id)
{
  ...
  trx_sysf_rseg_set_page_no(sys_header, rseg_id, FIL_NULL, &mtr);
  mtr.commit();
}
...
void trx_rseg_reset_pending()
{
  ...
  std::for_each(s_pending_reset_rseg_ids.begin(), s_pending_reset_rseg_ids.end(), trx_rseg_reset_slot);

  ib::info() << "Successfully reset " << s_pending_reset_rseg_ids.size() << " pre-5.7.2 rseg slots.";
                         ...
                         0 means next free slots. */
{
  ...
  slot_no = trx_sysf_rseg_find_free(&mtr, space->purpose == FIL_TYPE_TEMPORARY, nth_free_slot);
  ...
  if (slot_no != ULINT_UNDEFINED)
  {
    ...
    page_no = trx_rseg_header_create(space_id, page_size, ULINT_MAX, slot_no, &mtr);
    ...
    rseg = trx_rseg_mem_create(slot_no, space_id, page_no, page_size, purge_sys->purge_queue, rseg_array, &mtr);
  }
