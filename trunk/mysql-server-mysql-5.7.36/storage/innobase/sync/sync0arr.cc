    ...
    sync_cell_t *cell) /*!< in: non-empty sync array cell */
{
  ...
  { /* RW_LOCK_S and RW_LOCK_X wait on the same event */
    ...
    sync_cell_t *cell) /*!< in: sync cell */
{
  ...
  fprintf(file,
          ...
          (ulong)os_thread_pf(cell->thread_id), innobase_basename(cell->file), (ulong)cell->line,
  ...
  else if (type == RW_LOCK_X || type == RW_LOCK_X_WAIT || type == RW_LOCK_SX || type == RW_LOCK_S)
  {
    ...
    fprintf(file, " RW-latch at %p created in file %s line %lu\n", (void *)rwlock,
            innobase_basename(rwlock->cfile_name), (ulong)rwlock->cline);
    ...
    fprintf(file,
            ...
            rw_lock_get_reader_count(rwlock), rwlock->waiters, static_cast<ulong>(rwlock->lock_word),
            innobase_basename(rwlock->last_s_file_name), static_cast<ulong>(rwlock->last_s_line),
            rwlock->last_x_file_name, static_cast<ulong>(rwlock->last_x_line));
  }
  else
  {
    ut_error;
    ...
    ulint depth)        /*!< in: recursion depth */
{
  ...
  switch (cell->request_type)
  {
      ...
      if (mutex->state() != MUTEX_STATE_UNLOCKED)
      {
        ...
        if (ret)
        {
          ...
          ib::info() << "Mutex " << mutex
                     << " owned by"
                        " thread "
                     << os_thread_pf(thread) << " file " << name << " line " << policy.get_enter_line();
    ...
    case RW_LOCK_X_WAIT:
      ...
      for (debug = UT_LIST_GET_FIRST(lock->debug_list); debug != NULL; debug = UT_LIST_GET_NEXT(list, debug))
      {
    ...
    case RW_LOCK_SX:
      ...
      for (debug = UT_LIST_GET_FIRST(lock->debug_list); debug != 0; debug = UT_LIST_GET_NEXT(list, debug))
      {
        thread = debug->thread_id;
    ...
    case RW_LOCK_S:
      ...
      for (debug = UT_LIST_GET_FIRST(lock->debug_list); debug != 0; debug = UT_LIST_GET_NEXT(list, debug))
      {
        ...
        if (debug->lock_type == RW_LOCK_X || debug->lock_type == RW_LOCK_X_WAIT)
        {
...
#define SYNC_ARRAY_TIMEOUT 240
  ...
  for (ulint i = 0; i < arr->n_cells; i++)
  {
    ...
    int64_t time_diff = ut_time_monotonic() - cell->reservation_time;
