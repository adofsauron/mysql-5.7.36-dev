...
struct Latched
{
  ...
  Latched(const latch_t *latch, latch_level_t level) : m_latch(latch), m_level(level) { /* No op */ }
  ...
  bool operator==(const Latched &rhs) const { return (m_latch == rhs.m_latch && m_level == rhs.m_level); }
...
struct LatchDebug
{
  ...
  struct os_thread_id_less : public std::binary_function<os_thread_id_t, os_thread_id_t, bool>
  {
    /** @return true if lhs < rhs */
    bool operator()(const os_thread_id_t &lhs, const os_thread_id_t &rhs) const UNIV_NOTHROW
    {
  ...
  const Latched *less(const Latches *latches, latch_level_t limit) const UNIV_NOTHROW;

  /** Checks if the level value exists in the thread's acquired latches.
  @param[in]	latches		the thread's existing (acquired) latches
  @param[in]	level		to lookup
  @return	latch if found or 0 */
  const latch_t *find(const Latches *Latches, latch_level_t level) const UNIV_NOTHROW;
  ...
  @param[in]	level		The new level request that breaks
                                  the order */
  void crash(const Latches *latches, const Latched *latched, latch_level_t level) const UNIV_NOTHROW;
  ...
  bool basic_check(const Latches *latches, latch_level_t requested_level, ulint level) const UNIV_NOTHROW;
  ...
  void lock_validate(const latch_t *latch, latch_level_t level) UNIV_NOTHROW
  {
    ...
    if (*latch->get_name() != '.' && latch->get_level() != SYNC_LEVEL_VARYING)
    {
      ...
      ut_a(latches->empty() || level == SYNC_LEVEL_VARYING || level == SYNC_NO_ORDER_CHECK ||
           latches->back().get_level() == SYNC_NO_ORDER_CHECK ||
           latches->back().m_latch->get_level() == SYNC_LEVEL_VARYING || latches->back().get_level() >= level);
    }
  }
  ...
  void relock(const latch_t *latch) UNIV_NOTHROW
  {
    ...
    if (*latch->get_name() != '.' && latch->get_level() != SYNC_LEVEL_VARYING)
    {
      ...
      Latches::iterator it = std::find(latches->begin(), latches->end(), Latched(latch, level));

      ut_a(latches->empty() || level == SYNC_LEVEL_VARYING || level == SYNC_NO_ORDER_CHECK ||
           latches->back().m_latch->get_level() == SYNC_LEVEL_VARYING ||
           latches->back().m_latch->get_level() == SYNC_NO_ORDER_CHECK || latches->back().get_level() >= level ||
           it != latches->end());
 ...
 private:
  ...
  {
    return (latch->get_id() == LATCH_ID_RTR_ACTIVE_MUTEX || latch->get_id() == LATCH_ID_RTR_PATH_MUTEX ||
            latch->get_id() == LATCH_ID_RTR_MATCH_MUTEX || latch->get_id() == LATCH_ID_RTR_SSN_MUTEX);
  }

 private:
  /** Comparator for the Levels . */
  struct latch_level_less : public std::binary_function<latch_level_t, latch_level_t, bool>
  {
    /** @return true if lhs < rhs */
    bool operator()(const latch_level_t &lhs, const latch_level_t &rhs) const UNIV_NOTHROW { return (lhs < rhs); }
  };
...
#define LEVEL_MAP_INSERT(T)                                                                \
  do                                                                                       \
  {                                                                                        \
    std::pair<Levels::iterator, bool> result = m_levels.insert(Levels::value_type(T, #T)); \
    ut_ad(result.second);                                                                  \
  } while (0)
...
void LatchDebug::print_latches(const Latches *latches) const UNIV_NOTHROW
{
  ...
  for (Latches::const_iterator it = latches->begin(); it != end; ++it)
  {
    ib::error() << sync_latch_get_name(it->m_latch->get_id()) << " -> " << it->m_level << " "
...
void LatchDebug::crash(const Latches *latches, const Latched *latched, latch_level_t level) const UNIV_NOTHROW
{
  ...
  ib::error() << "Thread " << os_thread_pf(os_thread_get_curr_id()) << " already owns a latch "
              << sync_latch_get_name(latch->m_id) << " at level"
              << " " << latched->m_level << " (" << latch_level_name << " ), which is at a lower/same level than the"
              << " requested latch: " << level << " (" << in_level_name << "). " << latch->to_string();
...
const Latched *LatchDebug::less(const Latches *latches, latch_level_t limit) const UNIV_NOTHROW
{
  Latches::const_iterator end = latches->end();
...
@param[in]	in_level	declared ulint so that we can do level - 1.
                                ...
                                trying to acquire
@return true if passes, else crash with error message. */
bool LatchDebug::basic_check(const Latches *latches, latch_level_t requested_level, ulint in_level) const UNIV_NOTHROW
{
...
@param[in]	level		to lookup
@return	latch if found or 0 */
const latch_t *LatchDebug::find(const Latches *latches, latch_level_t level) const UNIV_NOTHROW
{
  Latches::const_iterator end = latches->end();
...
const latch_t *LatchDebug::find(latch_level_t level) UNIV_NOTHROW { return (find(thread_latches(), level)); }
...
@param[in]	level	level in the latching order
@return the thread's latches */
Latches *LatchDebug::check_order(const latch_t *latch, latch_level_t level) UNIV_NOTHROW
{
  ...
  switch (level)
  {
    ...
    case SYNC_ANY_LATCH:
      ...
      if (is_rtr_mutex(latch))
      {
        ...
        if (latched == NULL || (latched != NULL && is_rtr_mutex(latched->m_latch)))
        {
          /* No violation */
          break;
        }
    ...
    case SYNC_FSP:
      ...
      ut_a(find(latches, SYNC_FSP) != 0 || basic_check(latches, level, SYNC_FSP));
      break;
    ...
    case SYNC_TRX_UNDO_PAGE:
      ...
      ut_a(find(latches, SYNC_TRX_UNDO) != 0 || find(latches, SYNC_REDO_RSEG) != 0 ||
           find(latches, SYNC_NOREDO_RSEG) != 0 || basic_check(latches, level, level - 1));
      break;

    case SYNC_RSEG_HEADER:

      ut_a(find(latches, SYNC_REDO_RSEG) != 0 || find(latches, SYNC_NOREDO_RSEG) != 0);
      break;
...

    {
      ...
      ut_a((fsp_latch != NULL && fsp_latch->is_temp_fsp()) || find(latches, SYNC_INDEX_TREE) != 0 ||
           find(latches, SYNC_DICT_OPERATION) || basic_check(latches, level, SYNC_TREE_NODE - 1));
    }
    ...
    case SYNC_IBUF_TREE_NODE:
      ...
      ut_a(find(latches, SYNC_IBUF_INDEX_TREE) != 0 || basic_check(latches, level, SYNC_IBUF_TREE_NODE - 1));
      break;
...
void LatchDebug::unlock(const latch_t *latch) UNIV_NOTHROW
{
  ...
  else if ((latches = thread_latches()) != NULL)
  {
    ...
    if (latch->get_level() != SYNC_LEVEL_VARYING)
    {
      ib::error() << "Couldn't find latch " << sync_latch_get_name(latch->get_id());
...
static void sync_latch_meta_init() UNIV_NOTHROW
{
  ...
  LATCH_ADD_MUTEX(CACHE_LAST_READ, SYNC_TRX_I_S_LAST_READ, cache_last_read_mutex_key);

  LATCH_ADD_MUTEX(DICT_FOREIGN_ERR, SYNC_NO_ORDER_CHECK, dict_foreign_err_mutex_key);

  LATCH_ADD_MUTEX(DICT_SYS, SYNC_DICT, dict_sys_mutex_key);

  LATCH_ADD_MUTEX(FILE_FORMAT_MAX, SYNC_FILE_FORMAT_TAG, file_format_max_mutex_key);
  ...
  LATCH_ADD_MUTEX(FTS_BG_THREADS, SYNC_FTS_BG_THREADS, fts_bg_threads_mutex_key);
  ...
  LATCH_ADD_MUTEX(IBUF_PESSIMISTIC_INSERT, SYNC_IBUF_PESS_INSERT_MUTEX, ibuf_pessimistic_insert_mutex_key);

  LATCH_ADD_MUTEX(LOG_SYS, SYNC_LOG, log_sys_mutex_key);

  LATCH_ADD_MUTEX(LOG_WRITE, SYNC_LOG_WRITE, log_sys_write_mutex_key);

  LATCH_ADD_MUTEX(LOG_FLUSH_ORDER, SYNC_LOG_FLUSH_ORDER, log_flush_order_mutex_key);
  ...
  latch_meta[LATCH_ID_RW_LOCK_DEBUG] = LATCH_ADD_MUTEX(RW_LOCK_DEBUG, SYNC_NO_ORDER_CHECK, rw_lock_debug_mutex_key);
#endif /* UNIV_DEBUG */
  ...
  LATCH_ADD_MUTEX(SRV_DICT_TMPFILE, SYNC_DICT_OPERATION, srv_dict_tmpfile_mutex_key);
  ...
  LATCH_ADD_MUTEX(SRV_MONITOR_FILE, SYNC_NO_ORDER_CHECK, srv_monitor_file_mutex_key);
  ...
  LATCH_ADD_MUTEX(TRX_POOL_MANAGER, SYNC_POOL_MANAGER, trx_pool_manager_mutex_key);
  ...
  LATCH_ADD_MUTEX(PAGE_ZIP_STAT_PER_INDEX, SYNC_ANY_LATCH, page_zip_stat_per_index_mutex_key);
  ...
  LATCH_ADD_MUTEX(OS_AIO_WRITE_MUTEX, SYNC_NO_ORDER_CHECK, PFS_NOT_INSTRUMENTED);
  ...
  LATCH_ADD_RWLOCK(INDEX_ONLINE_LOG, SYNC_INDEX_ONLINE_LOG, index_online_log_key);
  ...
  LATCH_ADD_RWLOCK(FTS_CACHE_INIT, SYNC_FTS_CACHE_INIT, fts_cache_init_rw_lock_key);
  ...
  LATCH_ADD_RWLOCK(IBUF_INDEX_TREE, SYNC_IBUF_INDEX_TREE, index_tree_rw_lock_key);
  ...
  LATCH_ADD_RWLOCK(HASH_TABLE_RW_LOCK, SYNC_BUF_PAGE_HASH, hash_table_locks_key);

  LATCH_ADD_RWLOCK(SYNC_DEBUG_MUTEX, SYNC_NO_ORDER_CHECK, PFS_NOT_INSTRUMENTED);

  LATCH_ADD_MUTEX(MASTER_KEY_ID_MUTEX, SYNC_NO_ORDER_CHECK, master_key_id_mutex_key);

  LATCH_ADD_MUTEX(ANALYZE_INDEX_MUTEX, SYNC_ANALYZE_INDEX, analyze_index_mutex_key);
  ...
  for (LatchMetaData::iterator it = latch_meta.begin(); it != latch_meta.end(); ++it)
  {
...
struct CreateTracker
{
  ...
  void register_latch(const void *ptr, const char *filename, uint16_t line) UNIV_NOTHROW
  {
    m_mutex.enter();
 ...
 private:
  ...
  struct File
  {
    ...
    File(const char *name, uint16_t line) UNIV_NOTHROW : m_name(sync_basename(name)), m_line(line) { /* No op */ }
  ...
  typedef std::map<const void *, File, std::less<const void *>, ut_allocator<std::pair<const void *const, File>>> Files;
...
void sync_file_created_deregister(const void *ptr) { create_tracker->deregister_latch(ptr); }

/** Get the string where the file was created. Its format is "name:line"
@param[in]	ptr		Latch instance
@return created information or "" if can't be found */
std::string sync_file_created_get(const void *ptr) { return (create_tracker->get(ptr)); }
