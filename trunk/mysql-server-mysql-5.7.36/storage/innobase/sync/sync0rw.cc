    ...
    ulint line)            /*!< in: line where requested */
{
  ...
  const size_t counter_index = (size_t)ut_rnd_gen_next_ulint((ulint)curr_thread);
...
lock_loop:
  ...
  if (rw_lock_s_lock_low(lock, pass, file_name, line))
  {
    if (count_os_wait > 0)
    {
      ...
      rw_lock_stats.rw_s_os_wait_count.add(counter_index, count_os_wait);
    }

    rw_lock_stats.rw_s_spin_round_count.add(counter_index, spin_count);
  ...
  else
  {
    ...
    sync_arr = sync_array_get_and_reserve_cell(lock, RW_LOCK_S, file_name, line, &cell);
    ...
    ulint line)                       /*!< in: line where requested */
{
  ...
  while (lock->lock_word < threshold)
  {
    ...
    sync_arr = sync_array_get_and_reserve_cell(lock, RW_LOCK_X_WAIT, file_name, line, &cell);
  ...
  {
    lock->count_os_wait += static_cast<uint32_t>(count_os_wait);
    rw_lock_stats.rw_x_os_wait_count.add(counter_index, count_os_wait);
  }
}

#ifdef UNIV_DEBUG
#define rw_lock_x_lock_wait(L, P, T, C, F, O) rw_lock_x_lock_wait_func(L, P, T, C, F, O)
#else
#define rw_lock_x_lock_wait(L, P, T, C, F, O) rw_lock_x_lock_wait_func(L, T, C, F, O)
#endif /* UNIV_DBEUG */
    ...
    ulint line)                       /*!< in: line where requested */
{
  if (rw_lock_lock_word_decr(lock, X_LOCK_DECR, X_LOCK_HALF_DECR))
  {
    ...
    rw_lock_set_writer_id_and_recursion_flag(lock, !pass, curr_thread);

    rw_lock_x_lock_wait(lock, pass, 0, curr_thread, file_name, line);
  }
  else
  {
    bool recursive;
    ...
    if (!pass && recursive && os_thread_eq(writer_thread, curr_thread))
    {
      ...
      if (rw_lock_lock_word_decr(lock, X_LOCK_DECR, 0))
      {
        ...
        released. */
        rw_lock_x_lock_wait(lock, pass, -X_LOCK_HALF_DECR, curr_thread, file_name, line);
      }
      else
      {
      ...
      if (lock->sx_recursive++ == 0)
      {
        ...
        * Any other X-lock thread cannot exist because
        ...
        ut_ad((lock->lock_word == 0) ||
              ((lock->lock_word <= -X_LOCK_DECR) && (lock->lock_word > -(X_LOCK_DECR + X_LOCK_HALF_DECR))));
...
lock_loop:
  ...
  if (rw_lock_x_lock_low(lock, pass, curr_thread, file_name, line))
  {
    ...
    rw_lock_stats.rw_x_spin_round_count.add(counter_index, spin_count);
  ...
  sync_arr = sync_array_get_and_reserve_cell(lock, RW_LOCK_X, file_name, line, &cell);
...
lock_loop:
  ...
  if (rw_lock_sx_lock_low(lock, pass, curr_thread, file_name, line))
  {
    ...
    {
      lock->count_os_wait += static_cast<uint32_t>(count_os_wait);
      rw_lock_stats.rw_sx_os_wait_count.add(counter_index, count_os_wait);
    }

    rw_lock_stats.rw_sx_spin_round_count.add(counter_index, spin_count);
    rw_lock_stats.rw_sx_spin_wait_count.add(counter_index, spin_wait_count);
  ...
  sync_arr = sync_array_get_and_reserve_cell(lock, RW_LOCK_SX, file_name, line, &cell);
...
/******************************************************************/ /**
 ...
 @return FALSE if did not succeed, TRUE if success. */
ibool rw_lock_sx_lock_func_nowait(
    /*========================*/
    rw_lock_t *lock,       /*!< in: pointer to rw-lock */
    ulint pass,            /*!< in: pass value; != 0, if the lock will
                           be passed to another thread to unlock */
    ...
    ulint line)            /*!< in: line where requested */
{
  ...
  if (pass == 0 && lock_type != RW_LOCK_X_WAIT)
  {
    ...
    if ((lock_type == RW_LOCK_X && lock->lock_word < -X_LOCK_HALF_DECR) ||
        (lock_type == RW_LOCK_SX && (lock->lock_word < 0 || lock->sx_recursive == 1)))
    {
    ...
    ulint lock_type) /*!< in: lock type */
{
  ...
  for (info = UT_LIST_GET_FIRST(lock->debug_list); info != 0; info = UT_LIST_GET_NEXT(list, info))
  {
                     ...
                     RW_LOCK_X */
{
  ...
  for (const rw_lock_debug_t *info = UT_LIST_GET_FIRST(lock->debug_list); info != NULL;
...
void rw_lock_get_debug_info(const rw_lock_t *lock, Infos *infos)
{
  ...
  for (info = UT_LIST_GET_FIRST(lock->debug_list); info != NULL; info = UT_LIST_GET_NEXT(list, info))
  {
    ...
    FILE *file) /*!< in: file where to print */
{
  ...
  for (const rw_lock_t *lock = UT_LIST_GET_FIRST(rw_lock_list); lock != NULL; lock = UT_LIST_GET_NEXT(list, lock))
  {
    count++;
    ...
    const rw_lock_debug_t *info) /*!< in: debug struct */
{
  ...
  fprintf(f, "Locked: thread %lu file %s line %lu  ", static_cast<ulong>(os_thread_pf(info->thread_id)),
          sync_basename(info->file_name), static_cast<ulong>(info->line));
...
std::string rw_lock_t::to_string() const
{
  ...
  msg << "RW-LATCH: "
      << "thread id " << os_thread_pf(os_thread_get_curr_id()) << " addr: " << this
