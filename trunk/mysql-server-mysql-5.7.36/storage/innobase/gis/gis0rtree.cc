    ...
    double **buf_pos)      /*!< in/out: current buffer position */
{
  ...
  buf = static_cast<double *>(
      mem_heap_alloc(heap, DATA_MBR_LEN * (n_recs + 3) + sizeof(rtr_split_node_t) * (n_recs + 1)));
  ...
  *offsets = rec_get_offsets(rec, cursor->index, *offsets, n_uniq, &heap);
  ...
  source_cur = static_cast<byte *>(dfield_get_data(dtuple_get_nth_field(tuple, 0)));
  cur->coords = reserve_coords(buf_pos, SPDIMS);
  rec = (byte *)mem_heap_alloc(heap, rec_get_converted_size(cursor->index, tuple, 0));
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  dtuple_t *node_ptr = rtr_index_build_node_ptr(index, mbr, child_rec, child, heap, level);
  ...
  if (rec_info & REC_INFO_MIN_REC_FLAG)
  {
    ...
    if (page_zip)
    {
      ...
      update the zip page. */
      if (!btr_cur_update_alloc_zip(page_zip, btr_cur_get_page_cur(cursor), index, offsets, rec_offs_size(offsets),
                                    false, mtr))
      {
    ...
    if (cursor2)
    {
      ...
      offsets2 = rec_get_offsets(btr_cur_get_rec(cursor2), index, NULL, ULINT_UNDEFINED, &heap);
      ut_ad(del_page_no == btr_node_ptr_get_child_page_no(cursor2->page_cur.rec, offsets2));
  ...
  else if (page_get_n_recs(page) == 1)
  {
    ...
    err = btr_cur_optimistic_insert(flags, cursor, &insert_offsets, &heap, node_ptr, &insert_rec, &dummy_big_rec, 0,
                                    NULL, mtr);
  ...
  update_mbr:
    ...
    /* Insert the new rec. */
    page_cur_search_with_match(block, index, node_ptr, PAGE_CUR_LE, &up_match, &low_match, btr_cur_get_page_cur(cursor),
                               NULL);
    ...
    if (!ins_suc)
    {
      ...
      err = btr_cur_pessimistic_insert(flags, cursor, &insert_offsets, &new_heap, node_ptr, &insert_rec, &dummy_big_rec,
                                       0, NULL, mtr);
...
static MY_ATTRIBUTE((nonnull)) void rtr_adjust_upper_level(
    /*===================*/
    ...
    mtr_t *mtr)             /*!< in: mtr */
{
  ...
  offsets = rtr_page_get_father_block(NULL, heap, index, block, mtr, sea_cur, &cursor);
  ...
  node_ptr_upper = rtr_index_build_node_ptr(index, new_mbr, page_rec_get_next(page_get_infimum_rec(new_page)),
  ...
  page_cur_search_with_match(father_block, index, node_ptr_upper, PAGE_CUR_LE, &up_match, &low_match,
  ...
  err = btr_cur_optimistic_insert(flags | BTR_NO_LOCKING_FLAG | BTR_KEEP_SYS_FLAG | BTR_NO_UNDO_LOG_FLAG, &cursor,
                                  &offsets, &heap, node_ptr_upper, &rec, &dummy_big_rec, 0, NULL, mtr);
  ...
  if (err == DB_FAIL)
  {
    ...
    DBUG_EXECUTE_IF("rtr_page_need_first_split", { DBUG_SET("+d,rtr_page_need_second_split"); });

    err = btr_cur_pessimistic_insert(flags | BTR_NO_LOCKING_FLAG | BTR_KEEP_SYS_FLAG | BTR_NO_UNDO_LOG_FLAG, &cursor,
                                     &offsets, &new_heap, node_ptr_upper, &rec, &dummy_big_rec, 0, NULL, mtr);
  ...
  lock_prdt_update_parent(block, new_block, &prdt, &new_prdt, &parent_prdt, dict_index_get_space(index),
  ...
  if (prev_page_no != FIL_NULL)
  {
    ...
    buf_block_t *prev_block = btr_block_get(prev_page_id, page_size, RW_X_LATCH, index, mtr);
#ifdef UNIV_BTR_DEBUG
    ut_a(page_is_comp(prev_block->frame) == page_is_comp(page));
    ut_a(btr_page_get_next(prev_block->frame, mtr) == block->page.id.page_no());
#endif /* UNIV_BTR_DEBUG */

    btr_page_set_next(buf_block_get_frame(prev_block), buf_block_get_page_zip(prev_block), page_no, mtr);
  }
  ...
  if (next_page_no != FIL_NULL)
  {
    ...
    buf_block_t *next_block = btr_block_get(next_page_id, page_size, RW_X_LATCH, index, mtr);
#ifdef UNIV_BTR_DEBUG
    ut_a(page_is_comp(next_block->frame) == page_is_comp(page));
    ut_a(btr_page_get_prev(next_block->frame, mtr) == page_get_page_no(page));
#endif /* UNIV_BTR_DEBUG */

    btr_page_set_prev(buf_block_get_frame(next_block), buf_block_get_page_zip(next_block), new_page_no, mtr);
  }
...
ibool rtr_split_page_move_rec_list(
    /*=========================*/
    ...
    mtr_t *mtr)                   /*!< in: mtr */
{
  ...
  rec_move = static_cast<rtr_rec_move_t *>(mem_heap_alloc(heap, sizeof(*rec_move) * max_to_move));
  ...
  for (cur_split_node = node_array; cur_split_node < end_split_node; ++cur_split_node)
  {
    if (cur_split_node->n_node != first_rec_group)
    {
      ...
      offsets = rec_get_offsets(cur_split_node->key, index, offsets, ULINT_UNDEFINED, &heap);

      ut_ad(cur_split_node->key != first_rec || !page_is_leaf(page));

      rec = page_cur_insert_rec_low(page_cur_get_rec(&new_page_cursor), index, cur_split_node->key, offsets, mtr);
  ...
  if (new_page_zip)
  {
    ...
    if (!page_zip_compress(new_page_zip, new_page, index, page_zip_level, NULL, mtr))
    {
      ulint ret_pos;
      ...
      {
        if (UNIV_UNLIKELY(!page_zip_decompress(new_page_zip, new_page, FALSE)))
        {
          ut_error;
        }
  ...
  for (cur_split_node = node_array; cur_split_node < end_split_node; ++cur_split_node)
  {
    if (cur_split_node->n_node != first_rec_group)
    {
      ...
      offsets = rec_get_offsets(page_cur_get_rec(&page_cursor), index, offsets, ULINT_UNDEFINED, &heap);
...
func_start:
  ...
  *offsets = NULL;
  ...
  {
    first_rec = page_rec_get_next(page_get_infimum_rec(buf_block_get_frame(block)));
  }

  /* Initial split nodes array. */
  rtr_split_node_array = rtr_page_split_initialize_nodes(*heap, cursor, offsets, tuple, &buf_pos);
  ...
  for (cur_split_node = rtr_split_node_array; cur_split_node < end_split_node; ++cur_split_node)
  {
  ...
  first_rec_group =
      split_rtree_node(rtr_split_node_array, static_cast<int>(n_recs), static_cast<int>(total_data),
                       static_cast<int>(insert_size), 0, 2, 2, &buf_pos, SPDIMS, static_cast<uchar *>(first_rec));

  /* Allocate a new page to the index */
  direction = FSP_UP;
  hint_page_no = page_no + 1;
  new_block = btr_page_alloc(cursor->index, hint_page_no, direction, page_level, mtr, mtr);
  new_page_zip = buf_block_get_page_zip(new_block);
  ...
  if (0
      ...
      || page_zip
#endif
      || !rtr_split_page_move_rec_list(rtr_split_node_array, first_rec_group, new_block, block, first_rec,
                                       cursor->index, *heap, mtr))
  {
    ...
    for (cur_split_node = rtr_split_node_array; cur_split_node < end_split_node - 1; ++cur_split_node)
    {
      if (cur_split_node->n_node != first_rec_group)
      {
    ...
    for (cur_split_node = rtr_split_node_array; cur_split_node < end_split_node - 1; ++cur_split_node)
    {
      if (cur_split_node->n_node == first_rec_group)
      {
        ...
        *offsets = rec_get_offsets(page_cur_get_rec(page_cursor), cursor->index, *offsets, ULINT_UNDEFINED, heap);
...
after_insert:
  ...
  new page */
  lock_prdt_update_split(block, new_block, &prdt, &new_prdt, dict_index_get_space(cursor->index), page_no);

  /* Adjust the upper level. */
  rtr_adjust_upper_level(cursor, flags, block, new_block, &mbr, &new_mbr, direction, mtr);
  ...
  if (!rec)
  {
    ...
    rec_t *i_rec = page_rec_get_next(page_get_infimum_rec(buf_block_get_frame(block)));
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  for (ulint i = 1; i < btr_cur->tree_height; i++)
  {
    ...
    /* Get father block. */
    btr_cur_t cursor;
    offsets = rtr_page_get_father_block(NULL, heap, index, block, mtr, btr_cur, &cursor);

    page = buf_block_get_frame(block);

    /* Update the mbr field of the rec. */
    if (!rtr_update_mbr_field(&cursor, offsets, NULL, page, &new_mbr, NULL, mtr))
    {
      err = DB_ERROR;
      break;
    }
    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  cur_rec = page_rec_get_next(page_get_infimum_rec(buf_block_get_frame(new_block)));
  ...
  while (!page_cur_is_after_last(&cur1))
  {
    ...
    offsets1 = rec_get_offsets(cur1_rec, index, offsets1, ULINT_UNDEFINED, &heap);
    while (!page_rec_is_supremum(cur_rec))
    {
      ulint cur_matched_fields = 0;
      int cmp;

      offsets2 = rec_get_offsets(cur_rec, index, offsets2, ULINT_UNDEFINED, &heap);
      cmp = cmp_rec_rec_with_match(cur1_rec, cur_rec, offsets1, offsets2, index,
                                   page_is_spatial_non_leaf(cur1_rec, index), false, &cur_matched_fields);
      if (cmp < 0)
      {
    ...
    {
      fprintf(stderr, "page number %ld and %ld\n", (long)new_block->page.id.page_no(), (long)block->page.id.page_no());

      ib::fatal() << "rec offset " << page_offset(rec) << ", cur1 offset " << page_offset(page_cur_get_rec(&cur1))
                  << ", cur_rec offset " << page_offset(cur_rec);
    }
    ...
    dict_index_t *index)      /*!< in: index */
{
  ...
  rtr_read_mbr(rec_get_nth_field(rec, offsets, 0, &len), reinterpret_cast<rtr_mbr_t *>(mbr1));

  rec = btr_cur_get_rec(cursor2);

  rtr_read_mbr(rec_get_nth_field(rec, offsets2, 0, &len), reinterpret_cast<rtr_mbr_t *>(mbr2));
    ...
    mtr_t *mtr)               /*!< in: mtr */
{
  ...
  changed = rtr_merge_mbr_changed(cursor, cursor2, offsets, offsets2, &new_mbr, merge_block, block, index);

  /* Update the mbr field of the rec. And will delete the record
  pointed by cursor2 */
  if (changed)
  {
    if (!rtr_update_mbr_field(cursor, offsets, cursor2, child_page, &new_mbr, NULL, mtr))
    {
      err = DB_ERROR;
    }
  }
...

{
  ...
  ulint *offsets;
  rec_t *rec = page_rec_get_next(page_get_infimum_rec(buf_block_get_frame(parentb)));
    ...
    double *area)           /*!< out: increased area */
{
  ...
  ret = rtree_area_increase(rec_b_ptr, static_cast<const byte *>(dfield_get_data(dtuple_field)),
...
int64_t rtr_estimate_n_rows_in_range(dict_index_t *index, const dtuple_t *tuple, page_cur_mode_t mode)
{
  ...
  range_area = (range_mbr.xmax - range_mbr.xmin) * (range_mbr.ymax - range_mbr.ymin);
  ...
  while (!page_rec_is_supremum(rec))
  {
    ...
    if (rec_area == 0)
    {
      switch (mode)
      {
        ...
        case PAGE_CUR_WITHIN:
        case PAGE_CUR_MBR_EQUAL:
          if (rtree_key_cmp(PAGE_CUR_WITHIN, range_mbr_ptr, DATA_MBR_LEN, field, DATA_MBR_LEN) == 0)
          {
            area += 1;
          }
    ...
    else
    {
      switch (mode)
      {
        case PAGE_CUR_CONTAIN:
        case PAGE_CUR_INTERSECT:
          area += rtree_area_overlapping(range_mbr_ptr, field, DATA_MBR_LEN) / rec_area;
          break;
  ...
  return (static_cast<int64_t>(dict_table_get_n_rows(index->table) * area / n_recs));
}
