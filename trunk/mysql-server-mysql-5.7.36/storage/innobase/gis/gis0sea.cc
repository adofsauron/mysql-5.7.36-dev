    ...
    mtr_t *mtr) /*!< in: mtr */
{
  ...
  ut_ad(my_latch_mode == BTR_SEARCH_LEAF || my_latch_mode == BTR_MODIFY_LEAF || my_latch_mode == BTR_MODIFY_TREE ||
  ...
  need_parent =
      ((my_latch_mode == BTR_MODIFY_TREE || my_latch_mode == BTR_CONT_MODIFY_TREE) && mode == PAGE_CUR_RTREE_LOCATE);
  ...
  do
  {
    ...
    if (level == 0)
    {
      ...
      if (my_latch_mode == BTR_CONT_MODIFY_TREE || my_latch_mode == BTR_MODIFY_TREE)
      {
    ...
    if (my_latch_mode != BTR_SEARCH_LEAF)
    {
      for (ulint idx = 0; idx < btr_cur->tree_height; idx++)
      {
        ...
        {
          mtr_release_block_at_savepoint(mtr, rtr_info->tree_savepoints[idx], rtr_info->tree_blocks[idx]);
    ...
    ut_ad(!(rw_lock_own(&btr_cur->page_cur.block->lock, RW_LOCK_X) ||
          ...
          my_latch_mode == BTR_MODIFY_TREE || my_latch_mode == BTR_CONT_MODIFY_TREE ||
          !page_is_leaf(buf_block_get_frame(btr_cur->page_cur.block)));
#endif /* UNIV_RTR_DEBUG */

    page_id_t page_id(space, next_rec.page_no);

    block = buf_page_get_gen(page_id, page_size, rw_latch, NULL, BUF_GET, __FILE__, __LINE__, mtr);
    ...
    if (page_ssn > path_ssn)
    {
      ...
      rtr_non_leaf_stack_push(rtr_info->path, next_page_no, path_ssn, level, 0, NULL, 0);

      if (!srv_read_only_mode && mode != PAGE_CUR_RTREE_INSERT && mode != PAGE_CUR_RTREE_LOCATE)
      {
        ut_ad(rtr_info->thr);
    ...
    if (mode == PAGE_CUR_RTREE_LOCATE)
    {
      if (level == target_level && level == 0)
      {
        ...
        low_match = page_cur_search(block, index, tuple, PAGE_CUR_LE, btr_cur_get_page_cur(btr_cur));

        if (low_match == dtuple_get_n_fields_cmp(tuple))
        {
          rec_t *rec = btr_cur_get_rec(btr_cur);

          if (!rec_get_deleted_flag(rec, dict_table_is_comp(index->table)) || (!for_delete && !for_undo_ins))
          {
            found = true;
      ...
      else
      {
        ...
        found = rtr_cur_search_with_match(block, index, tuple, page_mode, page_cursor, btr_cur->rtr_info);
...
#ifdef UNIV_DEBUG
          ulint num_stored = rtr_store_parent_path(block, btr_cur, rw_latch, level, mtr);
          ut_ad(num_stored > 0);
#else
    ...
    {
      found = rtr_cur_search_with_match(block, index, tuple, mode, page_cursor, btr_cur->rtr_info);
    }

    /* Attach predicate lock if needed, no matter whether
    there are matched records */
    if (mode != PAGE_CUR_RTREE_INSERT && mode != PAGE_CUR_RTREE_LOCATE && mode >= PAGE_CUR_CONTAIN &&
        btr_cur->rtr_info->need_prdt_lock)
    {
      ...
      lock_mutex_enter();
      lock_init_prdt_from_mbr(&prdt, &btr_cur->rtr_info->mbr, mode, trx->lock.lock_heap);
      lock_mutex_exit();
      ...
      lock_prdt_lock(block, &prdt, index, LOCK_S, LOCK_PREDICATE, btr_cur->rtr_info->thr, mtr);
    ...
    if (found)
    {
      if (level == target_level)
      {
        ...
        if (my_latch_mode == BTR_MODIFY_TREE && level == 0)
        {
          ...
          btr_cur_latch_leaves(block, my_page_id, page_size, BTR_MODIFY_TREE, btr_cur, mtr);
        }

        r_cur = btr_cur_get_page_cur(btr_cur);

        page_cur_position(page_cur_get_rec(page_cursor), page_cur_get_block(page_cursor), r_cur);

        btr_cur->low_match = level != 0 ? DICT_INDEX_SPATIAL_NODEPTR_SIZE + 1 : btr_cur->low_match;
        break;
      }
    ...
    mtr_t *mtr)            /*!< in: mtr */
{
  ...
  /* Fetch the next page */
  return (rtr_pcur_getnext_from_path(tuple, mode, &cursor->btr_cur, level, cursor->latch_mode, false, mtr));
}
    ...
    mtr_t *mtr)            /*!< in: mtr */
{
  ...
  if (page_rec_is_infimum(rec) || low_match != n_fields ||
      (rec_get_deleted_flag(rec, dict_table_is_comp(index->table)) && (for_delete || for_undo_ins)))
  {
    if (rec_get_deleted_flag(rec, dict_table_is_comp(index->table)) && for_delete)
    {
    ...
    bool ret = rtr_pcur_getnext_from_path(tuple, mode, btr_cursor, level, latch_mode, tree_latched, mtr);
...
/************************************************************/ /**
 ...
 @return	rec_get_offsets() of the node pointer record */
ulint *rtr_page_get_father_node_ptr_func(
    /*==============================*/
    ulint *offsets,     /*!< in: work area for the return value */
    mem_heap_t *heap,   /*!< in: memory heap to use */
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  if (child_page != page_no)
  {
    ...
    error << "Corruption of index " << index->name << " of table " << index->table->name << " parent page " << page_no
    ...
    error << "; child ";
    rec_print(error.m_oss, print_rec, rec_get_info_bits(print_rec, rec_offs_comp(offsets)), offsets);
    offsets = rec_get_offsets(node_ptr, index, offsets, ULINT_UNDEFINED, &heap);
    error << "; parent ";
    rec_print(error.m_oss, print_rec, rec_get_info_bits(print_rec, rec_offs_comp(offsets)), offsets);
    ...
    mtr_t *mtr)            /*!< in: mtr */
{
  ...
  if (sea_cur && sea_cur->tree_height > level)
  {
    ...
    if (ret)
    {
      ...
      page_cur_position(rec, btr_pcur_get_block(r_cursor), btr_cur_get_page_cur(btr_cur));
  ...
  if (sea_cur && sea_cur->tree_height == level)
  {
    ...
    btr_cur_search_to_nth_level(index, level, tuple, PAGE_CUR_RTREE_LOCATE, BTR_CONT_MODIFY_TREE, btr_cur, 0, __FILE__,
                                __LINE__, mtr);
  }
  else
  {
    ...
    {
      ret = rtr_pcur_getnext_from_path(tuple, PAGE_CUR_RTREE_LOCATE, btr_cur, level, BTR_CONT_MODIFY_TREE, true, mtr);
    ...
    dict_index_t *index) /*!< in: index struct */
{
  ...
  if (init_matches)
  {
    ...
    rtr_info->matches = static_cast<matched_rec_t *>(mem_heap_zalloc(rtr_info->heap, sizeof(*rtr_info->matches)));

    rtr_info->matches->matched_recs = UT_NEW_NOKEY(rtr_rec_vector());

    rtr_info->matches->bufp = page_align(rtr_info->matches->rec_buf + UNIV_PAGE_SIZE_MAX + 1);
    mutex_create(LATCH_ID_RTR_MATCH_MUTEX, &rtr_info->matches->rtr_match_mutex);
    rw_lock_create(PFS_NOT_INSTRUMENTED, &(rtr_info->matches->block.lock), SYNC_LEVEL_VARYING);
  }
    ...
    ulint page_no)        /*!< in: need to free rtr_info itself */
{
  ...
  if (!rtr_info->parent_path->empty())
  {
    ...
    for (rit = rtr_info->parent_path->begin(); rit != rtr_info->parent_path->end(); ++rit)
    {
      node_visit_t next_rec = *rit;
    ...
    buf_block_t *block)  /*!< in: block of page to be discarded */
{
  ...
  for (it = index->rtr_track->rtr_active->begin(); it != index->rtr_track->rtr_active->end(); ++it)
  {
    rtr_info = *it;
    rtr_node_path_t::iterator rit;
    bool found = false;
...
struct Buf_page_optimistic_get_functor_t
{
  ...
  {
    return hint != NULL && buf_page_optimistic_get(RW_X_LATCH, hint, r_cursor->modify_clock, file, line, mtr);
  }
};
/**************************************************************/ /**
 Restores the stored position of a persistent cursor bufferfixing the page */
bool rtr_cur_restore_position_func(
    /*==========================*/
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  if (r_cursor->rel_pos == BTR_PCUR_AFTER_LAST_IN_TREE || r_cursor->rel_pos == BTR_PCUR_BEFORE_FIRST_IN_TREE)
  {
    return (false);
  }
  ...
  if (r_cursor->block_when_stored.run_with_hint(functor))
  {
    ...
    do
    {
      ...
      offsets1 = rec_get_offsets(r_cursor->old_rec, index, NULL, r_cursor->old_n_fields, &heap);
      ...
      {
        ut_ad(!cmp_rec_rec(r_cursor->old_rec, rec, offsets1, offsets2, index, page_is_spatial_non_leaf(rec, index)));
      }
  ...
  tuple = dict_index_build_data_tuple(index, r_cursor->old_rec, r_cursor->old_n_fields, heap);
...
search_again:
  ...
  block = buf_page_get_gen(page_id, page_size, RW_X_LATCH, NULL, BUF_GET, __FILE__, __LINE__, mtr);
  ...
  ulint low_match = page_cur_search(block, index, tuple, PAGE_CUR_LE, page_cursor);
  ...
  if (low_match == r_cursor->old_n_fields)
  {
    ...
    else if (!cmp_rec_rec(r_cursor->old_rec, rec, offsets1, offsets2, index, page_is_spatial_non_leaf(rec, index)))
    {
      r_cursor->pos_state = BTR_PCUR_IS_POSITIONED;
      ret = true;
    }
    ...
    double mbr_inc)           /*!< in: MBR needs to be enlarged */
{
  ...
  rtr_non_leaf_stack_push(path, page_no, new_seq, level, child_no, my_cursor, mbr_inc);
}
    ...
    const page_t *page)       /*!< in: buffer page */
{
  ...
  memcpy(matches->block.frame, page, page_is_comp(page) ? PAGE_NEW_SUPREMUM_END : PAGE_OLD_SUPREMUM_END);
  matches->used = page_is_comp(page) ? PAGE_NEW_SUPREMUM_END : PAGE_OLD_SUPREMUM_END;
    ...
    rtr_info_t *rtr_info)     /*!< in/out: search stack */
{
  ...
  if (mode == PAGE_CUR_RTREE_INSERT && !page_rec_is_supremum(rec))
  {
    ...
    offsets = rec_get_offsets(rec, index, offsets, dtuple_get_n_fields_cmp(tuple), &heap);
    ...
    if (buf_block_get_page_zip(block) && mach_read_from_4(page + FIL_PAGE_PREV) == FIL_NULL &&
    ...
    if (!is_leaf)
    {
      switch (mode)
      {
        ...
        case PAGE_CUR_MBR_EQUAL:
          ...
          if (cmp != 0)
          {
            cmp = cmp_dtuple_rec_with_gis(tuple, rec, offsets, PAGE_CUR_INTERSECT);
          }
          break;
    ...
    if (cmp == 0)
    {
      ...
      if (rtr_info && mode != PAGE_CUR_RTREE_INSERT)
      {
        if (!is_leaf)
        {
          ...
          is_loc = (orig_mode == PAGE_CUR_RTREE_LOCATE || orig_mode == PAGE_CUR_RTREE_GET_FATHER);
          ...
          rtr_non_leaf_stack_push(rtr_info->path, page_no, new_seq, level - 1, 0, NULL, 0);

          if (is_loc)
          {
            rtr_non_leaf_insert_stack_push(index, rtr_info->parent_path, level, page_no, block, rec, 0);
          }
  ...
  if (page_rec_is_supremum(rec))
  {
    if (!is_leaf)
    {
      if (!found)
      {
        ...
        if (mode == PAGE_CUR_RTREE_INSERT)
        {
          ...
          offsets = rec_get_offsets(best_rec, index, offsets, ULINT_UNDEFINED, &heap);
          child_no = btr_node_ptr_get_child_page_no(best_rec, offsets);

          rtr_non_leaf_insert_stack_push(index, rtr_info->parent_path, level, child_no, block, best_rec, least_inc);
    ...
    else if (rtr_info)
    {
      ...
      offsets2 = rec_get_offsets(test_rec.r_rec, index, offsets2, ULINT_UNDEFINED, &heap);

      offsets = rec_get_offsets(last_match_rec, index, offsets, ULINT_UNDEFINED, &heap);

      ut_ad(cmp_rec_rec(test_rec.r_rec, last_match_rec, offsets2, offsets, index, false) == 0);
#endif /* UNIV_DEBUG */
  ...
  else
  {
    if (mode == PAGE_CUR_RTREE_INSERT)
    {
      ...
      rtr_non_leaf_insert_stack_push(index, rtr_info->parent_path, level, child_no, block, rec, 0);
    }
  ...
  the path stack */
  if (!is_leaf && (!page_rec_is_supremum(rec) || found) && mode != PAGE_CUR_RTREE_INSERT)
  {
    ulint page_no;
