...
static int rtree_add_point_to_mbr(
    /*===================*/
...
static int rtree_get_point_mbr(
    /*================*/
    uchar **wkb,      /*!< in: pointer to wkb,
                      where point is stored. */
    uchar *end,       /*!< in: end of wkb. */
...
static int rtree_get_linestring_mbr(
    /*=====================*/
    uchar **wkb,      /*!< in: pointer to wkb,
                      where point is stored. */
    uchar *end,       /*!< in: end of wkb. */
                      ...
                      must be of length n_dims * 2. */
{
  ...
  for (; n_points > 0; --n_points)
  {
    ...
    if (rtree_add_point_to_mbr(wkb, end, n_dims, byte_order, mbr))
    {
      return (-1);
    }
  }
...
static int rtree_get_polygon_mbr(
    /*==================*/
    uchar **wkb,      /*!< in: pointer to wkb,
                      where point is stored. */
    uchar *end,       /*!< in: end of wkb. */
...
static int rtree_get_geometry_mbr(
    /*===================*/
    uchar **wkb, /*!< in: pointer to wkb,
                 where point is stored. */
    uchar *end,  /*!< in: end of wkb. */
                 ...
                 by itself. */
{
  ...
  switch ((enum wkbType)wkb_type)
  {
    case wkbPoint:
      ...
      break;
    case wkbLineString:
      res = rtree_get_linestring_mbr(wkb, end, n_dims, byte_order, mbr);
      break;
    case wkbPolygon:
    ...
    case wkbMultiPoint:
      ...
      for (; n_items > 0; --n_items)
      {
        ...
        ++(*wkb);
        (*wkb) += 4;
        if (rtree_get_point_mbr(wkb, end, n_dims, byte_order, mbr))
        {
          return (-1);
        }
      }
    ...
    case wkbMultiLineString:
      ...
      for (; n_items > 0; --n_items)
      {
        ...
        ++(*wkb);
        (*wkb) += 4;
        if (rtree_get_linestring_mbr(wkb, end, n_dims, byte_order, mbr))
        {
          return (-1);
        }
      }
    ...
    case wkbMultiPolygon:
      ...
      for (; n_items > 0; --n_items)
      {
        ...
        ++(*wkb);
        (*wkb) += 4;
        if (rtree_get_polygon_mbr(wkb, end, n_dims, byte_order, mbr))
        {
          return (-1);
        }
      }
    ...
    case wkbGeometryCollection:
      ...
      for (; n_items > 0; --n_items)
      {
        if (rtree_get_geometry_mbr(wkb, end, n_dims, mbr, 0))
        {
          return (-1);
        }
      }
    ...
    int n_dim)                 /*!< in: dimensions. */
{
  ...
  *seed_b = node + 1;
  ...
  for (cur1 = node; cur1 < lim1; ++cur1)
  {
    ...
    {
      d = mbr_join_square(cur1->coords, cur2->coords, n_dim) - cur1->square - cur2->square;
      if (d > max_d)
      {
        max_d = d;
        *seed_a = cur1;
    ...
    int n_dim)                 /*!< in: dimensions. */
{
  ...
  for (; cur < end; ++cur)
  {
    ...
    diff = mbr_join_square(g1, cur->coords, n_dim) - mbr_join_square(g2, cur->coords, n_dim);
...
double rtree_area_increase(const uchar *a,                      /*!< in: original mbr. */
                           ...
                           double *ab_area)                     /*!< out: increased area. */
{
  ...
  for (key_len = mbr_len; key_len > 0; key_len -= keyseg_len)
  {
    ...
    if (loc_ab_area == a_area)
    {
      ...
      {
        data_round *= ((double)std::max(amax, bmax) - amax + (amin - (double)std::min(amin, bmin)));
      }
      else
      {
        data_round *= area;
...
double rtree_area_overlapping(const uchar *a, const uchar *b, int mbr_len)
{
  double area = 1.0;
  double amin;
  double amax;
...
uint get_wkb_of_default_point(uint n_dims, uchar *wkb, uint len)
{
