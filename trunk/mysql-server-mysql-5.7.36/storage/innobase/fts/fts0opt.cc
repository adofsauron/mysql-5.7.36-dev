    ...
    ulint max_words)  /*!< in: max words to read */
{
  ...
  zip->word.f_str = static_cast<byte *>(mem_heap_zalloc(heap, FTS_MAX_WORD_LEN + 1));
    ...
    ulint len)        /*!< in: length of string in bytes */
{
  ...
  word->nodes = ib_vector_create(word->heap_alloc, sizeof(fts_node_t), FTS_WORD_NODES_INIT_SIZE);
    ...
    void *user_arg) /*!< in: pointer to ib_vector_t */
{
  ...
  if (dfield_len != word->text.f_len || memcmp(word->text.f_str, data, dfield_len))
  {
    word = static_cast<fts_word_t *>(ib_vector_push(words, NULL));
    fts_word_init(word, (byte *)data, dfield_len);
    is_word_init = true;
  ...
  if (is_word_init)
  {
    fetch->total_memory += sizeof(fts_word_t) + sizeof(ib_alloc_t) + sizeof(ib_vector_t) + dfield_len +
    ...
    fts_string_t *word) /*!< out: uncompressed word */
{
  ...
  while (zip->status == Z_OK && zip->zp->avail_out > 0)
  {
    ...
    if (zip->zp->avail_in == 0)
    {
      ...
      {
        zip->zp->next_in = static_cast<byte *>(ib_vector_getp(zip->blocks, zip->pos));
    ...
    void *user_arg) /*!< in: pointer to ib_vector_t */
{
  ...
  if (zip->word.f_len == static_cast<ulint>(len) && !memcmp(zip->word.f_str, data, len))
  {
    return (TRUE);
  }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_index_fetch_words(
    /*==================*/
    ...
    ulint n_words)            /*!< in: max words to read */
{
  ...
  for (selected = fts_select_index(optim->fts_index_table.charset, word->f_str, word->f_len);
    ...
    void *user_arg) /*!< in: pointer to ib_vector_t */
{
  ...
  fts_update_t *update = static_cast<fts_update_t *>(ib_vector_push(fts_doc_ids->doc_ids, NULL));
    ...
    doc_id_t last_doc_id)  /*!< in: doc id to lookup */
{
  ...
  if (pos < 0)
  {
    ...
    double check */
    if (i == 1 && array[0].doc_id <= last_doc_id && first_doc_id < array[0].doc_id)
    {
      pos = 0;
    }
...
/**********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code*/
static MY_ATTRIBUTE((nonnull)) dberr_t fts_optimize_encode_node(
    /*=====================*/
...
/**********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code*/
static MY_ATTRIBUTE((nonnull)) dberr_t fts_optimize_node(
    /*==============*/
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) int fts_optimize_deleted_pos(
    /*=====================*/
...
/**********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_write_word(
    /*====================*/
    trx_t *trx,             /*!< in: transaction */
    ...
    ib_vector_t *nodes)     /*!< in: the nodes to write */
{
  ...
  graph = fts_parse_sql(fts_table, info, "BEGIN DELETE FROM $table_name WHERE word = :word;");
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_compact(
    /*=================*/
    fts_optimize_t *optim,          /*!< in: optimize state data */
    ...
    ib_time_monotonic_t start_time) /*!< in: optimize start time */
{
  ...
  for (i = 0; i < size && error == DB_SUCCESS && !optim->done; ++i)
  {
    ...
    error = fts_optimize_write_word(trx, &optim->fts_index_table, &word->text, nodes);

    if (error == DB_SUCCESS)
    {
      /* Write the last word optimized to the config table,
      we use this value for restarting optimize. */
      error = fts_config_set_index_value(optim->trx, index, FTS_LAST_OPTIMIZED_WORD, &word->text);
    }

    /* Free the word that was optimized. */
    fts_word_free(word);

    if (fts_optimize_time_limit > 0 && (ut_time_monotonic() - start_time) > fts_optimize_time_limit)
    {
      optim->done = TRUE;
    }
  }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_get_index_start_time(
    /*==============================*/
    trx_t *trx,            /*!< in: transaction */
    dict_index_t *index,   /*!< in: FTS index */
    ib_time_t *start_time) /*!< out: time in secs */
{
  return (fts_config_get_index_ulint(trx, index, FTS_OPTIMIZE_START_TIME, (ulint *)start_time));
}

/**********************************************************************/ /**
 Set the optimize start time of an FTS index.
 @return DB_SUCCESS if all OK else error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_set_index_start_time(
    /*==============================*/
    trx_t *trx,           /*!< in: transaction */
    dict_index_t *index,  /*!< in: FTS index */
    ib_time_t start_time) /*!< in: start time */
{
  return (fts_config_set_index_ulint(trx, index, FTS_OPTIMIZE_START_TIME, (ulint)start_time));
}

/**********************************************************************/ /**
 Get optimize end time of an FTS index.
 @return DB_SUCCESS if all OK else error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_get_index_end_time(
    /*============================*/
    trx_t *trx,          /*!< in: transaction */
    dict_index_t *index, /*!< in: FTS index */
    ib_time_t *end_time) /*!< out: time in secs */
{
  return (fts_config_get_index_ulint(trx, index, FTS_OPTIMIZE_END_TIME, (ulint *)end_time));
}

/**********************************************************************/ /**
 Set the optimize end time of an FTS index.
 @return DB_SUCCESS if all OK else error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_set_index_end_time(
    /*============================*/
    trx_t *trx,          /*!< in: transaction */
    dict_index_t *index, /*!< in: FTS index */
    ib_time_t end_time)  /*!< in: end time */
{
  return (fts_config_set_index_ulint(trx, index, FTS_OPTIMIZE_END_TIME, (ulint)end_time));
}
#endif
    ...
    fts_table_t *fts_table) /*!< in: aux table */
{
  ...
  fts_config_get_ulint(trx, fts_table, FTS_OPTIMIZE_LIMIT_IN_SECS, (ulint *)&time_limit);
    ...
    fts_string_t *word)    /*!< in: the starting word to optimize */
{
  ...
  fts_optimize_time_limit = fts_optimize_get_time_limit(optim->trx, &optim->fts_common_table);
  ...
  while (!optim->done)
  {
    ...
    error = fts_index_fetch_nodes(trx, &graph, &optim->fts_index_table, word, &fetch);
    ...
    if (error == DB_SUCCESS)
    {
      if (!optim->done)
      {
        ...
        else if (selected != fts_select_index(charset, word->f_str, word->f_len) && graph)
        {
          fts_que_graph_free(graph);
          graph = NULL;
        }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_index_completed(
    /*=========================*/
    fts_optimize_t *optim, /*!< in: optimize instance */
    dict_index_t *index)   /*!< in: table with one FTS index */
{
  ...
  *word.f_str = '\0';
  ...
  error = fts_config_set_index_value(optim->trx, index, FTS_LAST_OPTIMIZED_WORD, &word);
...
/**********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_index_read_words(
    /*==========================*/
    fts_optimize_t *optim, /*!< in: optimize instance */
    dict_index_t *index,   /*!< in: table with one FTS index */
    fts_string_t *word)    /*!< in: buffer to use */
{
  ...
  else
  {
    ...
    the config table. */
    error = fts_config_get_index_value(optim->trx, index, FTS_LAST_OPTIMIZED_WORD, word);
  }
...
/**********************************************************************/ /**
 ...
 time (hours).
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_index(
    /*===============*/
    fts_optimize_t *optim, /*!< in: optimize instance */
    dict_index_t *index)   /*!< in: table with one FTS index */
{
...
/**********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_purge_deleted_doc_ids(
    /*===============================*/
    fts_optimize_t *optim) /*!< in: optimize instance */
{
  ulint i;
  ...
  update = static_cast<fts_update_t *>(ib_vector_get(optim->to_delete->doc_ids, 0));
  ...
  {
    update = static_cast<fts_update_t *>(ib_vector_get(optim->to_delete->doc_ids, i));
...
/**********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_purge_deleted_doc_id_snapshot(
    /*=======================================*/
    fts_optimize_t *optim) /*!< in: optimize instance */
{
  dberr_t error;
  ...
  pars_info_bind_id(info, true, fts_common_tables[1], being_deleted_cache);
    ...
    fts_optimize_t *optim) /*!< in: optimize instance */
{
  ...
  FTS_INIT_FTS_TABLE(&fts_table, "BEING_DELETED", FTS_COMMON_TABLE, optim->table);
...
/*********************************************************************/ /**
 ...
 upon successfull copy and rolled back on DB_DUPLICATE_KEY error.
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_create_deleted_doc_id_snapshot(
    /*========================================*/
    fts_optimize_t *optim) /*!< in: optimize instance */
{
  dberr_t error;
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_read_deleted_doc_id_snapshot(
    /*======================================*/
    fts_optimize_t *optim) /*!< in: optimize instance */
{
  dberr_t error;
  ...
  error = fts_table_fetch_doc_ids(optim->trx, &optim->fts_common_table, optim->to_delete);
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_indexes(
    /*=================*/
    fts_optimize_t *optim) /*!< in: optimize instance */
{
  ulint i;
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_purge_snapshot(
    /*========================*/
    fts_optimize_t *optim) /*!< in: optimize instance */
{
  dberr_t error;
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_optimize_reset_start_time(
    /*==========================*/
    fts_optimize_t *optim) /*!< in: optimize instance */
{
  dberr_t error = DB_SUCCESS;
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull)) dberr_t fts_optimize_table_bk(
    /*==================*/
    fts_slot_t *slot) /*!< in: table to optimiza */
{
  ...
  if (slot->last_run > 0 && (ut_time_monotonic() - slot->last_run) < slot->interval_time)
  {
    return (DB_SUCCESS);
  }
  else if (fts && fts->cache && fts->cache->deleted >= FTS_OPTIMIZE_THRESHOLD)
  {
    ...
    dict_table_t *table) /*!< in: table to optimiza */
{
  ...
  if (error == DB_SUCCESS)
  {
    ...
    if (error == DB_SUCCESS && optim->n_completed == ib_vector_size(fts->indexes))
    {
      if (fts_enable_diag_print)
      {
    ...
    dict_table_t *table) /*!< in: table to remove */
{
  ...
  {
    ib::info() << "Try to remove table " << table->name << " after FTS optimize thread exiting.";
    return;
  }
  ...
  remove = static_cast<fts_msg_del_t *>(mem_heap_alloc(msg->heap, sizeof(*remove)));
...
void fts_optimize_request_sync_table(dict_table_t *table)
{
  ...
  if (fts_opt_start_shutdown)
  {
    ib::info() << "Try to sync table " << table->name << " after FTS optimize thread exiting.";
    return;
  }

  msg = fts_optimize_create_msg(FTS_MSG_SYNC_TABLE, NULL);

  table_id = static_cast<table_id_t *>(mem_heap_alloc(msg->heap, sizeof(table_id_t)));
  *table_id = table->id;
  msg->ptr = table_id;

  ib_wqueue_add(fts_optimize_wq, msg, msg->heap);
  DBUG_EXECUTE_IF(
      "fts_optimize_wq_count_check", if (ib_wqueue_get_count(fts_optimize_wq) > 1000) { DBUG_SUICIDE(); });
}
    ...
    fts_msg_del_t *msg)  /*!< in: table to delete */
{
  ...
  for (i = 0; i < ib_vector_size(tables); ++i)
  {
    ...
    if (slot->state != FTS_STATE_EMPTY && slot->table == table)
    {
      if (fts_enable_diag_print)
      {
                               ...
                               vector*/
{
  ...
  for (ulint i = 0; i < ib_vector_size(tables); ++i)
  {
    ...
    if (slot->state != FTS_STATE_EMPTY && slot->table && slot->table->fts && slot->table->fts->cache)
    {
...
/**********************************************************************/ /**
 ...
 @return Dummy return */
os_thread_ret_t fts_optimize_thread(
    /*================*/
    void *arg) /*!< in: work queue*/
{
  ...
  while (!done && srv_shutdown_state == SRV_SHUTDOWN_NONE)
  {
    ...
    else if (n_optimize == 0 || !ib_wqueue_is_empty(wq))
    {
      ...
      msg = static_cast<fts_msg_t *>(ib_wqueue_timedwait(wq, FTS_QUEUE_WAIT_IN_USECS));
      ...
      switch (msg->type)
      {
        ...
        case FTS_MSG_ADD_TABLE:
          ut_a(!done);
          if (fts_optimize_new_table(fts_slots, static_cast<dict_table_t *>(msg->ptr)))
          {
            ++n_tables;
          }
          break;
        ...
        case FTS_MSG_DEL_TABLE:
          if (fts_optimize_del_table(fts_slots, static_cast<fts_msg_del_t *>(msg->ptr)))
          {
            --n_tables;
          }
        ...
        case FTS_MSG_SYNC_TABLE:
          DBUG_EXECUTE_IF("fts_instrument_msg_sync_sleep", os_thread_sleep(300000););
...
/*===================*/
{
  ...
  for (table = UT_LIST_GET_FIRST(dict_sys->table_LRU); table != NULL; table = UT_LIST_GET_NEXT(table_LRU, table))
  {
    ...
    {
      if (fts_optimize_new_table(fts_slots, table))
      {
...
ibool fts_optimize_is_init(void)
/*======================*/
{
