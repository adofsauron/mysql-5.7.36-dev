    ...
    const dict_index_t *index) /*!< in: index for config */
{
  ...
  name = static_cast<char *>(ut_malloc_nokey(len + FTS_AUX_MIN_TABLE_ID_LENGTH + 2));
  ::strcpy(name, param);
  name[len] = '_';

  fts_write_object_id(index->id, name + len + 1, DICT_TF2_FLAG_IS_SET(index->table, DICT_TF2_FTS_AUX_HEX_NAME));
                         ...
                         config table */
{
  ...
  FTS_INIT_FTS_TABLE(&fts_table, "CONFIG", FTS_COMMON_TABLE, index->table);
    ...
    ulint int_value)     /*!< in: value */
{
  ...
  value.f_len = ut_snprintf((char *)value.f_str, FTS_MAX_INT_LEN, "%lu", int_value);
    ...
    ulint int_value)        /*!< in: value */
{
  ...
  value.f_len = my_snprintf((char *)value.f_str, FTS_MAX_INT_LEN, "%lu", int_value);
