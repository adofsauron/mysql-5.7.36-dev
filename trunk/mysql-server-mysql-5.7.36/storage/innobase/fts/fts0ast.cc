    ...
    const fts_ast_string_t *ptr) /*!< in: ast term string */
{
  ...
  while (cur_pos < len)
  {
    ...
    cur_len = innobase_mysql_fts_get_token(state->charset, reinterpret_cast<const byte *>(ptr->str) + cur_pos,
    ...
    if (str.f_n_char > 0)
    {
      ...
      if ((first_node && (str.f_n_char < fts_min_token_size)) || str.f_n_char > fts_max_token_size)
      {
        continue;
      }
      ...
      if (first_node)
      {
        ...
        if (!node_list)
        {
          node_list = fts_ast_create_node_list(static_cast<fts_ast_state_t *>(arg), first_node);
        }
    ...
    const ulint len) /*!< in: term string length */
{
  ...
  if (len == 0 || len > FTS_MAX_WORD_LEN || ptr[0] == '%' || ptr[len - 1] == '%')
  {
    return (NULL);
  }
  ...
  node->term.ptr = fts_ast_string_create(reinterpret_cast<const byte *>(ptr), len);
    ...
    const fts_ast_string_t *ptr) /*!< in: ast text string */
{
  ...
  node->text.ptr = fts_ast_string_create(reinterpret_cast<const byte *>(ptr->str + 1), len);
...
{
  ut_a(node->type == FTS_AST_LIST || node->type == FTS_AST_SUBEXP_LIST || node->type == FTS_AST_PARSER_PHRASE_LIST);
...
bool fts_ast_node_check_union(fts_ast_node_t *node)
{
  ...
  else if (node->type == FTS_AST_OPER && (node->oper == FTS_IGNORE || node->oper == FTS_EXIST))
  {
    return (false);
  }
                              ...
                              and FTS_IGNORE operators */
{
  ...
  for (node = node->list.head; node && (error == DB_SUCCESS); node = node->next)
  {
    switch (node->type)
    {
      case FTS_AST_LIST:
      ...
      default:
        ...
        ut_a(oper == FTS_NONE || !oper_node || oper_node->oper == oper || oper_node->oper == FTS_EXIST_SKIP ||
             oper_node->oper == FTS_IGNORE_SKIP);
  ...
  if (revisit)
  {
    ...
    for (node = start_node; node && error == DB_SUCCESS; node = node->next)
    {
      ...
      {
        error = fts_ast_visit(FTS_EXIST_SKIP, node, visitor, arg, &will_be_ignored);
      }
    }
    ...
    for (node = start_node; node && error == DB_SUCCESS; node = node->next)
    {
      if (node->type == FTS_AST_LIST)
      {
        error = fts_ast_visit(FTS_IGNORE_SKIP, node, visitor, arg, &will_be_ignored);
      }
    }
  }
...
fts_ast_string_t *fts_ast_string_create(const byte *str, ulint len)
{
  ...
  ast_str = static_cast<fts_ast_string_t *>(ut_malloc_nokey(sizeof(fts_ast_string_t)));
...
void fts_ast_string_print(const fts_ast_string_t *ast_str)
{
