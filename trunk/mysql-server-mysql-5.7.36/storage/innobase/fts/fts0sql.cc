...
{
  int len;
  bool hex_name = DICT_TF2_FLAG_IS_SET(fts_table->table, DICT_TF2_FTS_AUX_HEX_NAME);
  ...
  switch (fts_table->type)
  {
    case FTS_COMMON_TABLE:
      len = fts_write_object_id(fts_table->table_id, table_id, hex_name);
      break;
    ...
    case FTS_INDEX_TABLE:
      ...
      len += fts_write_object_id(fts_table->index_id, table_id, hex_name);
      break;
    ...
    const fts_table_t *fts_table) /*!< in: Auxiliary table type */
{
  ...
  slash = static_cast<const char *>(memchr(fts_table->parent, '/', strlen(fts_table->parent)));
  ...
  len = sprintf(prefix_name, "%.*sFTS_%s", dbname_len, fts_table->parent, table_id);
...
/*!< in/out: aux table name */
{
  ...
  ut_a(len > 0);
  ut_a(strlen(prefix_name) + 1 + strlen(fts_table->suffix) == static_cast<uint>(len));
    ...
    const char *sql)        /*!< in: SQL string to evaluate */
{
  ...
  dict_locked = (fts_table && fts_table->table->fts && (fts_table->table->fts->fts_status & TABLE_DICT_LOCKED));
...
/******************************************************************/ /**
 Parse an SQL string.
 @return query graph */
que_t *fts_parse_sql_no_dict_lock(
    /*=======================*/
