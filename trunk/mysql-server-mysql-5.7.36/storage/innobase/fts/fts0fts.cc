...
#define FTS_IS_OBSOLETE_AUX_TABLE(table_name)                                         \
  (strstr((table_name), "DOC_ID") != NULL || strstr((table_name), "ADDED") != NULL || \
...
for decision */
const char *fts_default_stopword[] = {"a",   "about", "an",   "are", "as",   "at",  "be",  "by",   "com",  "de",
...
const char *fts_common_tables[] = {"BEING_DELETED", "BEING_DELETED_CACHE", "CONFIG", "DELETED", "DELETED_CACHE", NULL};

/** FTS auxiliary INDEX split intervals. */
const fts_index_selector_t fts_index_selector[] = {{9, "INDEX_1"},  {65, "INDEX_2"}, {70, "INDEX_3"}, {75, "INDEX_4"},
...
static dberr_t fts_sync(fts_sync_t *sync, bool unlock_cache, bool wait, bool has_dict_lock);
...
static ulint fts_add_doc_by_id(
    /*==============*/
...
static dberr_t fts_is_word_in_index(
    /*=================*/
    ...
    fts_stopword_t *stopword_info) /*!< in: stopword info */
{
  ...
  if (!stopword_info->cached_stopword)
  {
    stopword_info->cached_stopword =
        rbt_create_arg_cmp(sizeof(fts_tokenizer_word_t), innobase_fts_text_cmp, &my_charset_latin1);
  }
    ...
    void *user_arg) /*!< in: pointer to ib_vector_t */
{
  ...
  if (str.f_len != UNIV_SQL_NULL && rbt_search(stop_words, &parent, &str) != 0)
  {
    fts_tokenizer_word_t new_word;

    new_word.nodes = ib_vector_create(allocator, sizeof(fts_node_t), 4);

    new_word.text.f_str = static_cast<byte *>(mem_heap_alloc(heap, str.f_len + 1));
    ...
    fts_stopword_t *stopword_info)   /*!< in: Stopword info */
{
  ...
  else if (!stopword_info->cached_stopword)
  {
    ...
    stopword_info->cached_stopword =
        rbt_create_arg_cmp(sizeof(fts_tokenizer_word_t), innobase_fts_text_cmp, stopword_info->charset);
  }
    ...
    fts_index_cache_t *index_cache) /*!< in: index cache */
{
  ...
  index_cache->words = rbt_create_arg_cmp(sizeof(fts_tokenizer_word_t), innobase_fts_text_cmp, index_cache->charset);

  ut_a(index_cache->doc_stats == NULL);

  index_cache->doc_stats = ib_vector_create(allocator, sizeof(fts_doc_stats_t), 4);
    ...
    fts_cache_t *cache) /*!< in: cache to initialize */
{
  ...
  cache->deleted_doc_ids = ib_vector_create(cache->sync_heap, sizeof(fts_update_t), 4);
    ...
    dict_table_t *table) /*!< in: table owns the FTS cache */
{
  ...
  rw_lock_create(fts_cache_init_rw_lock_key, &cache->init_lock, SYNC_FTS_CACHE_INIT);
  ...
  cache->sync = static_cast<fts_sync_t *>(mem_heap_zalloc(heap, sizeof(fts_sync_t)));

  cache->sync->table = table;
  cache->sync->event = os_event_create(0);

  /* Create the index cache vector that will hold the inverted indexes. */
  cache->indexes = ib_vector_create(cache->self_heap, sizeof(fts_index_cache_t), 2);
    ...
    fts_cache_t *cache) /*!< in: FTS index cache */
{
  ...
  for (i = 0; i < ib_vector_size(cache->indexes); i++)
  {
    ...
    ind_cache = static_cast<fts_index_cache_t *>(ib_vector_get(cache->indexes, i));

    get_doc = static_cast<fts_get_doc_t *>(ib_vector_push(cache->get_docs, NULL));
    ...
    dict_index_t *index_check) /*!< in: index to be checked */
{
  ...
  for (index = dict_table_get_first_index(table); index != NULL; index = dict_table_get_next_index(index))
  {
    ...
    dict_index_t *index) /*!< in: index to be checked */
{
  ...
  for (i = 0; i < ib_vector_size(table->fts->cache->indexes); i++)
  {
    ...
    index_cache = static_cast<fts_index_cache_t *>(ib_vector_get(table->fts->cache->indexes, i));
    ...
    dict_table_t *table) /*!< in: Table where indexes are dropped */
{
  ...
  ut_a(ib_vector_size(table->fts->indexes) == ib_vector_size(table->fts->cache->indexes));
    ...
    trx_t *trx)          /*!< in: Transaction for the drop */
{
  ...
  if ((ib_vector_size(indexes) == 1 &&
       (index == static_cast<dict_index_t *>(ib_vector_getp(table->fts->indexes, 0)))) ||
      ib_vector_is_empty(indexes))
  {
    ...
    if (!DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_HAS_DOC_ID))
    {
      ...
      while (index->index_fts_syncing && !trx_is_interrupted(trx))
      {
        DICT_BG_YIELD(trx);
      }
...
/****************************************************************/ /**
 ...
 held */
void fts_que_graph_free_check_lock(
    /*==========================*/
    ...
    que_t *graph)                         /*!< in: query graph */
{
  ...
  if (fts_table && fts_table->table)
  {
    ...
    has_dict = fts_table->table->fts->fts_status & TABLE_DICT_LOCKED;
  }
  else if (index_cache)
  {
    ut_ad(index_cache->index->table->fts);

    has_dict = index_cache->index->table->fts->fts_status & TABLE_DICT_LOCKED;
  }
    ...
    dict_index_t *index) /*!< in: FTS index */
{
  ...
  index_cache = static_cast<fts_index_cache_t *>(ib_vector_push(cache->indexes, NULL));
  ...
  index_cache->ins_graph =
      static_cast<que_t **>(mem_heap_zalloc(static_cast<mem_heap_t *>(cache->self_heap->arg), n_bytes));

  index_cache->sel_graph =
      static_cast<que_t **>(mem_heap_zalloc(static_cast<mem_heap_t *>(cache->self_heap->arg), n_bytes));
    ...
    ib_rbt_t *words) /*!< in: rb tree of words */
{
  ...
  for (rbt_node = rbt_first(words); rbt_node != NULL; rbt_node = rbt_first(words))
  {
    ulint i;
    ...
    {
      fts_node_t *fts_node = static_cast<fts_node_t *>(ib_vector_get(word->nodes, i));
...
void fts_cache_clear(fts_cache_t *cache)
{
  ...
  for (i = 0; i < ib_vector_size(cache->indexes); ++i)
  {
    ...
    for (j = 0; j < FTS_NUM_AUX_INDEX; ++j)
    {
      ...
      {
        fts_que_graph_free_check_lock(NULL, index_cache, index_cache->ins_graph[j]);
      ...
      {
        fts_que_graph_free_check_lock(NULL, index_cache, index_cache->sel_graph[j]);
    ...
    const dict_index_t *index) /*!< in: index to search for */
{
  ...
  ut_ad(rw_lock_own((rw_lock_t *)&cache->lock, RW_LOCK_X) || rw_lock_own((rw_lock_t *)&cache->init_lock, RW_LOCK_X));
    ...
    fts_string_t *text)             /*!< in: node text */
{
  ...
  if (!fts_check_token(text, cache->stopword_info.cached_stopword, index_cache->index->is_ngram, index_cache->charset))
  {
    return (NULL);
  }
  ...
  if (rbt_search(index_cache->words, &parent, text) != 0)
  {
    ...
    cache->total_size +=
        sizeof(new_word) + sizeof(ib_rbt_node_t) + text->f_len + (sizeof(fts_node_t) * 4) + sizeof(*new_word.nodes);
    ...
    ib_rbt_t *tokens)               /*!< in: document tokens */
{
  ...
  for (node = rbt_first(tokens); node; node = rbt_first(tokens))
  {
    ...
    if (fts_node == NULL || fts_node->synced || fts_node->ilist_size > FTS_ILIST_MAX_SIZE ||
  ...
  doc_stats = static_cast<fts_doc_stats_t *>(ib_vector_push(index_cache->doc_stats, NULL));
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_drop_table(
    /*===========*/
    ...
    const char *table_name) /*!< in: table to drop */
{
  ...
  table = dict_table_open_on_name(table_name, TRUE, FALSE,
                                  static_cast<dict_err_ignore_t>(DICT_ERR_IGNORE_INDEX_ROOT | DICT_ERR_IGNORE_CORRUPT));
  ...
  if (table != 0)
  {
    ...
    if (error != DB_SUCCESS)
    {
      ib::error() << "Unable to drop FTS index aux table " << table_name << ": " << ut_strerr(error);
    }
  }
  else
  {
...
/****************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_rename_one_aux_table(
    /*=====================*/
    ...
    trx_t *trx)                     /*!< in: transaction */
{
  ...
  ulint table_new_name_len = strlen(fts_table_old_name) + new_db_name_len - old_db_name_len;

  /* Check if the new and old database names are the same, if so,
  nothing to do */
  ut_ad((new_db_name_len != old_db_name_len) || strncmp(new_name, fts_table_old_name, old_db_name_len) != 0);
  ...
  strncpy(fts_table_new_name + new_db_name_len, strchr(fts_table_old_name, '/'), table_new_name_len - new_db_name_len);
  fts_table_new_name[table_new_name_len] = 0;

  return (row_rename_table_for_mysql(fts_table_old_name, fts_table_new_name, trx, false));
}
    ...
    trx_t *trx)           /*!< in: transaction */
{
  ...
  for (i = 0; fts->indexes != 0 && i < ib_vector_size(fts->indexes); ++i)
  {
    ...
    for (ulint j = 0; j < FTS_NUM_AUX_INDEX; ++j)
    {
      ...
      DBUG_EXECUTE_IF("fts_rename_failure", err = DB_DEADLOCK; fts_sql_rollback(trx););
...
/****************************************************************/ /**
 ...
 before this.
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_drop_common_tables(
    /*===================*/
    trx_t *trx,             /*!< in: transaction */
...
/****************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
dberr_t fts_drop_index_split_tables(
    /*========================*/
    trx_t *trx,          /*!< in: transaction */
...
#ifdef FTS_DOC_STATS_DEBUG
  fts_table_t fts_table;
  static const char *index_tables[] = {"DOC_ID", NULL};
#endif /* FTS_DOC_STATS_DEBUG */
...
/****************************************************************/ /**
 ...
 before this.
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_drop_all_index_tables(
    /*======================*/
    trx_t *trx, /*!< in: transaction */
    fts_t *fts) /*!< in: fts instance */
{
  ...
  for (ulint i = 0; fts->indexes != 0 && i < ib_vector_size(fts->indexes); ++i)
  {
    dberr_t err;
    dict_index_t *index;
...
static inline ulint fts_get_table_flags2_for_aux_tables(ulint flags2)
{
  ...
  return ((flags2 & DICT_TF2_USE_FILE_PER_TABLE) | (flags2 & DICT_TF2_ENCRYPTION) | (flags2 & DICT_TF2_TEMPORARY));
}
...
{
  dict_table_t *new_table = dict_mem_table_create(aux_table_name, table->space, n_cols, 0, table->flags,
  ...
  if (DICT_TF_HAS_DATA_DIR(table->flags))
  {
    ...
    new_table->data_dir_path = mem_heap_strdup(new_table->heap, table->data_dir_path);
  }
...
static dict_table_t *fts_create_one_common_table(trx_t *trx, const dict_table_t *table, const char *fts_table_name,
                                                 const char *fts_suffix, mem_heap_t *heap)
{
  ...
  if (!is_config)
  {
    new_table = fts_create_in_mem_aux_table(fts_table_name, table, FTS_DELETED_TABLE_NUM_COLS);

    dict_mem_table_add_col(new_table, heap, "doc_id", DATA_INT, DATA_UNSIGNED, FTS_DELETED_TABLE_COL_LEN);
  }
  else
  {
    /* Config table has different schema. */
    new_table = fts_create_in_mem_aux_table(fts_table_name, table, FTS_CONFIG_TABLE_NUM_COLS);

    dict_mem_table_add_col(new_table, heap, "key", DATA_VARCHAR, 0, FTS_CONFIG_TABLE_KEY_COL_LEN);

    dict_mem_table_add_col(new_table, heap, "value", DATA_VARCHAR, DATA_NOT_NULL, FTS_CONFIG_TABLE_VALUE_COL_LEN);
  }
  ...
  if (error == DB_SUCCESS)
  {
    dict_index_t *index = dict_mem_index_create(fts_table_name, "FTS_COMMON_TABLE_IND", new_table->space,
...
dberr_t fts_create_common_tables(trx_t *trx, const dict_table_t *table, const char *name, bool skip_doc_id_index)
{
  ...
  for (ulint i = 0; fts_common_tables[i] != NULL; ++i)
  {
    ...
    dict_table_t *common_table = fts_create_one_common_table(trx, table, full_name[i], fts_table.suffix, heap);
  ...
  graph = fts_parse_sql_no_dict_lock(&fts_table, info, fts_config_table_insert_values_sql);
  ...
  index = dict_mem_index_create(name, FTS_DOC_ID_INDEX_NAME, table->space, DICT_UNIQUE, 1);
...
func_exit:
  if (error != DB_SUCCESS)
  {
    for (it = common_tables.begin(); it != common_tables.end(); ++it)
    {
      row_drop_table_for_mysql((*it)->name.m_name, trx, FALSE);
    }
  }
...
@return DB_SUCCESS or error code */
static dict_table_t *fts_create_one_index_table(trx_t *trx, const dict_index_t *index, fts_table_t *fts_table,
                                                mem_heap_t *heap)
{
  dict_field_t *field;
  dict_table_t *new_table = NULL;
  ...
  new_table = fts_create_in_mem_aux_table(table_name, fts_table->table, FTS_AUX_INDEX_TABLE_NUM_COLS);

  field = dict_index_get_nth_field(index, 0);
  charset = fts_get_charset(field->col->prtype);

  dict_mem_table_add_col(new_table, heap, "word", charset == &my_charset_latin1 ? DATA_VARCHAR : DATA_VARMYSQL,
                         field->col->prtype, FTS_INDEX_WORD_LEN);

  dict_mem_table_add_col(new_table, heap, "first_doc_id", DATA_INT, DATA_NOT_NULL | DATA_UNSIGNED,
                         FTS_INDEX_FIRST_DOC_ID_LEN);

  dict_mem_table_add_col(new_table, heap, "last_doc_id", DATA_INT, DATA_NOT_NULL | DATA_UNSIGNED,
                         FTS_INDEX_LAST_DOC_ID_LEN);

  dict_mem_table_add_col(new_table, heap, "doc_count", DATA_INT, DATA_NOT_NULL | DATA_UNSIGNED,
  ...
  dict_mem_table_add_col(new_table, heap, "ilist", DATA_BLOB, (DATA_MTYPE_MAX << 16) | DATA_UNSIGNED | DATA_NOT_NULL,
                         FTS_INDEX_ILIST_LEN);
  ...
  {
    dict_index_t *index =
        dict_mem_index_create(table_name, "FTS_INDEX_TABLE_IND", new_table->space, DICT_UNIQUE | DICT_CLUSTERED, 2);
...
@return DB_SUCCESS or error code */
dberr_t fts_create_index_tables_low(trx_t *trx, const dict_index_t *index, const char *table_name, table_id_t table_id)
{
  ulint i;
  fts_table_t fts_table;
  dberr_t error = DB_SUCCESS;
  ...
  graph = fts_parse_sql_no_dict_lock(NULL, info, fts_create_index_tables_sql);
  ...
  if (error != DB_SUCCESS)
  {
    for (it = aux_idx_tables.begin(); it != aux_idx_tables.end(); ++it)
    {
      row_drop_table_for_mysql((*it)->name.m_name, trx, FALSE);
    }
  }
...
@return DB_SUCCESS or error code */
dberr_t fts_create_index_tables(trx_t *trx, const dict_index_t *index)
{
  dberr_t err;
  dict_table_t *table;
    ...
    fts_row_state event)     /*!< in: new event */
{
  ...
  static const fts_row_state table[4][4] = {/*    I            M            D            N */
...
/******************************************************************/ /**
 Create an FTS trx.
 @return FTS trx */
static fts_trx_t *fts_trx_create(
    /*===========*/
                ...
                transaction */
{
  ...
  ftt->savepoints = static_cast<ib_vector_t *>(ib_vector_create(heap_alloc, sizeof(fts_savepoint_t), 4));

  ftt->last_stmt = static_cast<ib_vector_t *>(ib_vector_create(heap_alloc, sizeof(fts_savepoint_t), 4));
  ...
  for (savep = UT_LIST_GET_FIRST(trx->trx_savepoints); savep != NULL; savep = UT_LIST_GET_NEXT(trx_savepoints, savep))
  {
    ...
    dict_table_t *table) /*!< in: table */
{
  ...
  ftt = static_cast<fts_trx_table_t *>(mem_heap_alloc(fts_trx->heap, sizeof(*ftt)));
    ...
    const fts_trx_table_t *ftt_src) /*!< in: FTS trx */
{
  ...
  ftt = static_cast<fts_trx_table_t *>(mem_heap_alloc(ftt_src->fts_trx->heap, sizeof(*ftt)));
    ...
    ib_vector_t *fts_indexes) /*!< in: FTS indexes affected */
{
  ...
  { /* Row-id not found, create a new one. */
    fts_trx_row_t row;
    ...
    fts_table_t *fts_table) /*!< in: table instance */
{
  ...
  error = fts_config_get_value(trx, fts_table, FTS_MAX_CACHE_SIZE_IN_MB, &value);
  ...
  if (error == DB_SUCCESS)
  {
    ...
    if (cache_size_in_mb > FTS_CACHE_SIZE_UPPER_LIMIT_IN_MB)
    {
      ib::warn() << "FTS max cache size (" << cache_size_in_mb
                 ...
                 << FTS_CACHE_SIZE_UPPER_LIMIT_IN_MB << "MB, setting cache size to upper limit";
    ...
    else if (cache_size_in_mb < FTS_CACHE_SIZE_LOWER_LIMIT_IN_MB)
    {
      ib::warn() << "FTS max cache size (" << cache_size_in_mb
                 << ") out of range."
                    " Minimum value is "
                 << FTS_CACHE_SIZE_LOWER_LIMIT_IN_MB << "MB and the maximum value is"
                 << FTS_CACHE_SIZE_UPPER_LIMIT_IN_MB << "MB, setting cache size to lower limit";
...
dberr_t fts_get_total_word_count(
    /*=====================*/
    trx_t *trx,          /*!< in: transaction */
    ...
    ulint *total)        /* out: total words */
{
  ...
  *total = 0;
  ...
  error = fts_config_get_index_value(trx, index, FTS_TOTAL_WORD_COUNT, &value);
    ...
    doc_id_t doc_id)           /*!< in: DOC ID to set */
{
  ...
  fts_update_sync_doc_id(table, table_name, table->fts->cache->synced_doc_id, trx);
}
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if OK */
static MY_ATTRIBUTE((nonnull)) dberr_t fts_cmp_set_sync_doc_id(
    /*====================*/
    const dict_table_t *table, /*!< in: table */
...
retry:
  ...
  *doc_id = 0;
  ...
  {
    error = fts_update_sync_doc_id(table, table->name.m_name, cache->synced_doc_id, trx);
  }
    ...
    trx_t *trx)                /*!< in: update trx, or NULL */
{
  ...
  id_len = ut_snprintf((char *)id, sizeof(id), FTS_DOC_ID_FORMAT, doc_id + 1);
...
/*====================*/
{
  ...
  fts_doc_ids = static_cast<fts_doc_ids_t *>(mem_heap_alloc(heap, sizeof(*fts_doc_ids)));

  fts_doc_ids->self_heap = ib_heap_allocator_create(heap);

  fts_doc_ids->doc_ids = static_cast<ib_vector_t *>(ib_vector_create(fts_doc_ids->self_heap, sizeof(fts_update_t), 32));
    ...
    fts_trx_row_t *row)   /*!< in: row */
{
  ...
  if (!DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_HAS_DOC_ID) && doc_id >= table->fts->cache->next_doc_id)
  {
    table->fts->cache->next_doc_id = doc_id + 1;
  }
}
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_delete(
    /*=======*/
    fts_trx_table_t *ftt, /*!< in: FTS trx table */
    fts_trx_row_t *row)   /*!< in: row */
{
  ...
  if (table->fts->fts_status & ADDED_TABLE_SYNCED && doc_id > cache->synced_doc_id)
  {
    mutex_enter(&table->fts->cache->deleted_lock);
    ...
    if (doc_id >= table->fts->cache->first_doc_id && table->fts->cache->added > 0)
    {
      --table->fts->cache->added;
    }
  ...
  if (error == DB_SUCCESS)
  {
    ...
    graph = fts_parse_sql(&fts_table, info, "BEGIN INSERT INTO $deleted VALUES (:doc_id);");
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_modify(
    /*=======*/
    fts_trx_table_t *ftt, /*!< in: FTS trx table */
    fts_trx_row_t *row)   /*!< in: row */
{
    ...
    mem_heap_t *heap)    /*!< in: heap */
{
  ...
  if (error == DB_SUCCESS)
  {
    ...
    write_doc_id = static_cast<doc_id_t *>(mem_heap_alloc(heap, sizeof(*write_doc_id)));
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_commit_table(
    /*=============*/
    fts_trx_table_t *ftt) /*!< in: FTS table to commit*/
{
  ...
  for (node = rbt_first(rows); node != NULL && error == DB_SUCCESS; node = rbt_next(rows, node))
  {
    ...
    trx_t *trx) /*!< in: transaction */
{
  ...
  savepoint = static_cast<fts_savepoint_t *>(ib_vector_last(trx->fts_trx->savepoints));
  tables = savepoint->tables;

  for (node = rbt_first(tables), error = DB_SUCCESS; node != NULL && error == DB_SUCCESS; node = rbt_next(tables, node))
  {
    fts_trx_table_t **ftt;
                               ...
                               documents */
{
  ...
  for (i = 0; i < num_field; i++)
  {
    ...
    {
      doc->text.f_str =
          btr_rec_copy_externally_stored_field(clust_rec, offsets, dict_table_page_size(table), clust_pos,
                                               &doc->text.f_len, static_cast<mem_heap_t *>(doc->self_heap->arg));
    }
    else
    {
      doc->text.f_str = (byte *)rec_get_nth_field(clust_rec, offsets, clust_pos, &doc->text.f_len);
    }
...
/*!< in: affected fts indexes */
{
  ...
  btr_pcur_open_with_no_init(fts_id_index, tuple, PAGE_CUR_LE, BTR_SEARCH_LEAF, &pcur, 0, &mtr);
  ...
  if (btr_pcur_get_low_match(&pcur) == 1)
  {
    ...
    if (page_rec_is_infimum(rec) || rec_get_deleted_flag(rec, dict_table_is_comp(table)))
    {
      goto func_exit;
    }
    ...
    else
    {
      ...
      btr_pcur_open_with_no_init(clust_index, clust_ref, PAGE_CUR_LE, BTR_SEARCH_LEAF, &clust_pcur, 0, &mtr);

      doc_pcur = &clust_pcur;
      clust_rec = btr_pcur_get_rec(&clust_pcur);
    }

    offsets = rec_get_offsets(clust_rec, clust_index, NULL, ULINT_UNDEFINED, &heap);
    ...
    for (ulint i = 0; i < num_idx; ++i)
    {
      ...
      if (doc.found)
      {
        ...
        DBUG_EXECUTE_IF("fts_instrument_sync_cache_wait", srv_fatal_semaphore_wait_threshold = 25;
                        fts_max_cache_size = 100; fts_sync(cache->sync, true, true, false););

        DBUG_EXECUTE_IF("fts_instrument_sync", fts_optimize_request_sync_table(table);
                        os_event_wait(cache->sync->event););

        DBUG_EXECUTE_IF("fts_instrument_sync_debug", fts_sync(cache->sync, true, true, false););

        DEBUG_SYNC_C("fts_instrument_sync_request");
        DBUG_EXECUTE_IF("fts_instrument_sync_request", fts_optimize_request_sync_table(table););
    ...
    void *user_arg) /*!< in: pointer to ulint */
{
  ...
  *value = static_cast<ulint>(mach_read_from_4(static_cast<const byte *>(data)));
    ...
    dict_table_t *table) /*!< in: user table */
{
  ...
  btr_pcur_open_at_index_side(false, index, BTR_SEARCH_LEAF, &pcur, true, 0, &mtr);
  ...
  if (!page_is_empty(btr_pcur_get_page(&pcur)))
  {
    ...
    doc_id = static_cast<doc_id_t>(fts_read_doc_id(static_cast<const byte *>(data)));
  }
    ...
    else
    {
      ...
      graph = fts_parse_sql(NULL, info,
                            mem_heap_printf(info->heap,
                                            ...
                                            "END LOOP;\n"
                                            "CLOSE c;",
                                            FTS_DOC_ID_COL_NAME, select_str, FTS_DOC_ID_COL_NAME));
    }
    if (get_doc)
    {
    ...
    fts_node_t *node)       /*!< in: node columns */
{
  ...
  pars_info_bind_int4_literal(info, "doc_count", (const ib_uint32_t *)&doc_count);

  /* Set copy_name to FALSE since it's a static. */
  pars_info_bind_literal(info, "ilist", node->ilist, node->ilist_size, DATA_BLOB, DATA_BINARY_TYPE);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_sync_add_deleted_cache(
    /*=======================*/
    fts_sync_t *sync,     /*!< in: sync state */
    ib_vector_t *doc_ids) /*!< in: doc ids to add */
{
  ...
  FTS_INIT_FTS_TABLE(&fts_table, "DELETED_CACHE", FTS_COMMON_TABLE, sync->table);

  fts_get_table_name(&fts_table, table_name);
  pars_info_bind_id(info, true, "table_name", table_name);

  graph = fts_parse_sql(&fts_table, info, "BEGIN INSERT INTO $table_name VALUES (:doc_id);");
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t
    fts_sync_write_words(trx_t *trx, fts_index_cache_t *index_cache, bool unlock_cache, ib_time_t sync_start_time)
{
  fts_table_t fts_table;
  ulint n_nodes = 0;
  ulint n_words = 0;
  ...
  ulint lock_threshold = (srv_fatal_semaphore_wait_threshold % SRV_SEMAPHORE_WAIT_EXTENSION) * cutoff;
  ...
  for (rbt_node = rbt_first(index_cache->words); rbt_node; rbt_node = rbt_next(index_cache->words, rbt_node))
  {
    ...
    DBUG_EXECUTE_IF("fts_instrument_write_words_before_select_index", os_thread_sleep(300000););
    ...
    if (error == DB_SUCCESS && fts_enable_diag_print)
    {
      ...
      error = fts_is_word_in_index(trx, &index_cache->sel_graph[selected], &fts_table, &word->text, &found);
    ...
    for (i = 0; i < ib_vector_size(word->nodes); ++i)
    {
      ...
      if (error == DB_SUCCESS)
      {
        ...
        DBUG_EXECUTE_IF("fts_instrument_sync", os_thread_sleep(10000000););
        if (!unlock_cache)
        {
        ...
        DBUG_EXECUTE_IF("fts_instrument_sync", os_thread_sleep(15000000););
  ...
  if (error == DB_SUCCESS && n_new_words > 0 && fts_enable_diag_print)
  {
    ...
    error = fts_config_increment_index_value(trx, index_cache->index, FTS_TOTAL_WORD_COUNT, n_new_words);
  }
#endif /* FTS_DOC_STATS_DEBUG */
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if all went well else error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_sync_write_doc_stat(
    /*====================*/
    trx_t *trx,                      /*!< in: transaction */
    ...
    const fts_doc_stats_t *doc_stat) /*!< in: doc stats to write */
{
  ...
  pars_info_bind_int4_literal(info, "count", (const ib_uint32_t *)&word_count);
  ...
  if (!*graph)
  {
    ...
    FTS_INIT_INDEX_TABLE(&fts_table, "DOC_ID", FTS_INDEX_TABLE, index);
  ...
  for (;;)
  {
    ...
    else
    {
      ...
      {
        ib::error() << "(" << ut_strerr(error) << ") while writing to FTS doc_id.";
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static ulint fts_sync_write_doc_stats(
    /*=====================*/
    trx_t *trx,                           /*!< in: transaction */
    const fts_index_cache_t *index_cache) /*!< in: index cache */
{
  ...
  doc_stat = static_cast<ts_doc_stats_t *>(ib_vector_pop(index_cache->doc_stats));

  while (doc_stat)
  {
    error = fts_sync_write_doc_stat(trx, index_cache->index, &graph, doc_stat);
    ...
    fts_sync_t *sync) /*!< in: sync state */
{
  ...
  if (fts_enable_diag_print)
  {
    ib::info() << "FTS SYNC for table " << sync->table->name
               << ", deleted count: " << ib_vector_size(cache->deleted_doc_ids) << " size: " << cache->total_size
               << " bytes";
  }
}
...
/*********************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_sync_index(
    /*===========*/
    fts_sync_t *sync,               /*!< in: sync state */
    fts_index_cache_t *index_cache) /*!< in: index cache */
{
  ...
  error = fts_sync_write_words(trx, index_cache, sync->unlock_cache, sync->start_time);
...
static bool fts_sync_index_check(fts_index_cache_t *index_cache)
{
  ...
  for (rbt_node = rbt_first(index_cache->words); rbt_node != NULL; rbt_node = rbt_next(index_cache->words, rbt_node))
  {
    fts_tokenizer_word_t *word;
    word = rbt_value(fts_tokenizer_word_t, rbt_node);
...
@param[in,out]	sync	sync state
@return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_sync_commit(fts_sync_t *sync)
{
  dberr_t error;
  trx_t *trx = sync->trx;
  fts_cache_t *cache = sync->table->fts->cache;
  ...
  error = fts_cmp_set_sync_doc_id(sync->table, sync->max_doc_id, FALSE, &last_doc_id);
  ...
  {
    ib::info() << "SYNC for table " << sync->table->name << ": SYNC time: " << (ut_time_monotonic() - sync->start_time)
...
static bool fts_check_all_indexes_synced(fts_sync_t *sync)
{
  ulint i;
  fts_cache_t *cache = sync->table->fts->cache;
  ...
  for (i = 0; i < ib_vector_size(cache->indexes); ++i)
  {
    ...
    if (index_cache->index->to_be_dropped || index_cache->index->table->to_be_dropped ||
...
static dberr_t fts_sync(fts_sync_t *sync, bool unlock_cache, bool wait, bool has_dict_lock)
{
  ...
  do
  {
    ...
    for (i = 0; i < ib_vector_size(cache->indexes); ++i)
    {
      ...
      DBUG_EXECUTE_IF("fts_instrument_sync_before_syncing", os_thread_sleep(300000););
    ...
    DBUG_EXECUTE_IF("fts_instrument_sync_interrupted", sync->interrupted = true; error = DB_INTERRUPTED;);
...
dberr_t fts_sync_table(dict_table_t *table, bool unlock_cache, bool wait, bool has_dict)
{
  ...
  if (!dict_table_is_discarded(table) && table->fts->cache && !dict_table_is_corrupted(table))
  {
...
bool fts_check_token(const fts_string_t *token, const ib_rbt_t *stopwords, bool is_ngram, const CHARSET_INFO *cs)
{
  ...
  if (!is_ngram)
  {
    ...
    if (token->f_n_char < fts_min_token_size || token->f_n_char > fts_max_token_size ||
  ...
  for (ulint ngram_token_size = 1; ngram_token_size <= token->f_n_char; ngram_token_size++)
  {
    const char *start;
    const char *next;
    const char *end;
...
static void fts_add_token(fts_doc_t *result_doc, fts_string_t str, ulint position)
{
  ...
  if (fts_check_token(&str, NULL, result_doc->is_ngram, result_doc->charset))
  {
    ...
    {
      newlen =
          innobase_fts_casedn_str(result_doc->charset, (char *)str.f_str, str.f_len, (char *)t_str.f_str, t_str.f_len);
    }
    ...
    if (rbt_search(result_doc->tokens, &parent, &t_str) != 0)
    {
      ...
      new_token.positions = ib_vector_create(result_doc->self_heap, sizeof(ulint), 32);
...
static ulint fts_process_token(
    /*==============*/
                       ...
                       tokens from this tokenization */
{
  ...
  ret =
      innobase_mysql_fts_get_token(doc->charset, doc->text.f_str + start_pos, doc->text.f_str + doc->text.f_len, &str);
    ...
    ulint len)              /*!< in: token length */
{
  ...
  while (start < end)
  {
    ...
    mbl = cs->cset->ctype(cs, &ctype, reinterpret_cast<uchar *>(start), reinterpret_cast<uchar *>(end));
    ...
    int len)                     /*!< in: document length */
{
  fts_string_t str;
  byte buf[FTS_MAX_WORD_LEN + 1];
  MYSQL_FTPARSER_BOOLEAN_INFO bool_info = {FT_TOKEN_WORD, 0, 0, 0, 0, 0, ' ', 0};
  ...
  {
    inc = innobase_mysql_fts_get_token(const_cast<CHARSET_INFO *>(param->cs), reinterpret_cast<byte *>(doc) + i,
    ...
    if (str.f_len > 0)
    {
      ...
      if (param->mysql_add_word(param, reinterpret_cast<char *>(str.f_str), static_cast<int>(str.f_len), &bool_info))
      {
        break;
      }
    }
    ...
    MYSQL_FTPARSER_BOOLEAN_INFO *boolean_info) /* in: word boolean info */
{
  ...
  str.f_str = reinterpret_cast<byte *>(word);
  str.f_len = word_len;
  str.f_n_char = fts_get_token_size(const_cast<CHARSET_INFO *>(param->cs), word, word_len);
    ...
    st_mysql_ftparser *parser) /* in: plugin fts parser */
{
  ...
  doc->tokens = rbt_create_arg_cmp(sizeof(fts_token_t), innobase_fts_text_cmp, doc->charset);
...
ib_vector_t *fts_get_docs_create(
    /*================*/
    /* out: vector of
    ...
    ib_vector_t *get_docs) /*!< in: Doc retrieval vector */
{
  ...
  {
    fts_get_doc_t *get_doc = static_cast<fts_get_doc_t *>(ib_vector_get(get_docs, i));
                                    ...
                                    document */
{
  ...
  for (i = 0; i < ib_vector_size(fts_indexes); ++i)
  {
    ...
    updated_fts_index = static_cast<const dict_index_t *>(ib_vector_getp_const(fts_indexes, i));
    ...
    fts_trx_t *fts_trx) /* in, own: FTS trx */
{
  ...
  for (i = 0; i < ib_vector_size(fts_trx->savepoints); ++i)
  {
    ...
    savepoint = static_cast<fts_savepoint_t *>(ib_vector_get(fts_trx->savepoints, i));
  ...
  for (i = 0; i < ib_vector_size(fts_trx->last_stmt); ++i)
  {
    ...
    savepoint = static_cast<fts_savepoint_t *>(ib_vector_get(fts_trx->last_stmt, i));
...
doc_id_t fts_get_doc_id_from_rec(dict_table_t *table, const rec_t *rec, const dict_index_t *index, mem_heap_t *heap)
{
  ...
  col_no = dict_col_get_index_pos(&table->cols[table->fts->doc_col], index);
...
ibool fts_cache_is_deleted_doc_id(
    /*========================*/
    ...
    doc_id_t doc_id)          /*!< in: doc id to search for */
{
  ...
  for (ulint i = 0; i < ib_vector_size(cache->deleted_doc_ids); ++i)
  {
    ...
    update = static_cast<const fts_update_t *>(ib_vector_get_const(cache->deleted_doc_ids, i));
    ...
    ib_vector_t *vector)      /*!< in: append to this vector */
{
  ...
  for (ulint i = 0; i < ib_vector_size(cache->deleted_doc_ids); ++i)
  {
    ...
    update = static_cast<fts_update_t *>(ib_vector_get(cache->deleted_doc_ids, i));
...
{
  dict_mem_table_add_col(table, heap, FTS_DOC_ID_COL_NAME, DATA_INT,
                         dtype_form_prtype(DATA_NOT_NULL | DATA_UNSIGNED | DATA_BINARY_TYPE | DATA_FTS_DOC_ID, 0),
                         sizeof(doc_id_t));
...
doc_id_t fts_update_doc_id(dict_table_t *table, upd_field_t *ufield, doc_id_t *next_doc_id)
{
  ...
  {
    dict_index_t *clust_index;
    dict_col_t *col = dict_table_get_nth_col(table, table->fts->doc_col);
...
ibool fts_dict_table_has_fts_index(
    /*=========================*/
    dict_table_t *table) /*!< in: table */
{
...
fts_t::fts_t(const dict_table_t *table, mem_heap_t *heap)
    : bg_threads(0), fts_status(0), add_wq(NULL), cache(NULL), doc_col(ULINT_UNDEFINED), fts_heap(heap)
{
  ut_a(table->fts == NULL);
    ...
    const char *name)   /*!< in: savepoint name */
{
  ...
  last_savepoint = static_cast<fts_savepoint_t *>(ib_vector_last(fts_trx->savepoints));
    ...
    const char *name) /*!< in: savepoint name */
{
  ...
  if (i != ULINT_UNDEFINED)
  {
    ...
    if (i == ib_vector_size(savepoints) - 1)
    {
      ...
      prev_savepoint = static_cast<fts_savepoint_t *>(ib_vector_get(savepoints, i - 1));
    ...
    trx_t *trx) /*!< in: transaction */
{
  ...
  savepoint = static_cast<fts_savepoint_t *>(ib_vector_last(savepoints));
  last_stmt = static_cast<fts_savepoint_t *>(ib_vector_last(fts_trx->last_stmt));
  ...
  for (node = rbt_first(l_tables); node; node = rbt_next(l_tables, node))
  {
    ...
    rbt_search_cmp(s_tables, &parent, &(*l_ftt)->table->id, fts_trx_table_id_cmp, NULL);
...
static bool fts_is_aux_table_name(fts_aux_table_t *table, const char *name, ulint len)
{
  const char *ptr;
  char *end;
  ...
  if (ptr != NULL && len > 20 && strncmp(ptr, "FTS_", 4) == 0)
  {
    ...
    if (strncmp(ptr, "ADDED", len) == 0 || strncmp(ptr, "STOPWORDS", len) == 0)
    {
      return (true);
    }
  ...
  for (i = 0; exp && !done; exp = que_node_get_next(exp), ++i)
  {
    ...
    switch (i)
    {
      case 0: /* NAME */
        ...
        if (!fts_is_aux_table_name(table, static_cast<const char *>(data), len))
        {
        ...
        table->name = static_cast<char *>(mem_heap_alloc(heap, len + 1));
      ...
      case 1: /* ID */
        ut_a(len == 8);
        table->id = mach_read_from_8(static_cast<const byte *>(data));
        break;
...
static ibool fts_set_hex_format(
    /*===============*/
    void *row,      /*!< in: sel_node_t* */
    void *user_arg) /*!< in: bool set/unset flag */
{
  ...
  ut_ad(mach_read_from_4(static_cast<byte *>(user_arg)) == ULINT32_UNDEFINED);
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS or error code. */
dberr_t fts_update_hex_format_flag(
    /*=======================*/
                         ...
                         dict_sys_t::mutex. */
{
  ...
  pars_info_bind_function(info, "my_func", fts_set_hex_format, &flags2);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_rename_one_aux_table_to_hex_format(
    /*===================================*/
    trx_t *trx,                       /*!< in: transaction */
    ...
    const dict_table_t *parent_table) /*!< in: parent table name */
{
  ...
  error = row_rename_table_for_mysql(aux_table->name, new_name, trx, FALSE);

  if (error != DB_SUCCESS)
  {
    ib::warn() << "Failed to rename aux table '" << aux_table->name << "' to new format '" << new_name << "'.";
  }
  else
  {
    ib::info() << "Renamed aux table '" << aux_table->name << "' to '" << new_name << "'.";
  }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_rename_aux_tables_to_hex_format_low(
    /*====================================*/
    trx_t *trx,                 /*!< in: transaction */
    ...
    ib_vector_t *tables)        /*!< in: aux tables to rename. */
{
  ...
  if (error != DB_SUCCESS)
  {
    ib::warn() << "Setting parent table " << parent_table->name << " to hex format failed.";
    fts_sql_rollback(trx);
    return (error);
  }
  ...
  for (count = 0; count < ib_vector_size(tables); ++count)
  {
    ...
    aux_table = static_cast<fts_aux_table_t *>(ib_vector_get(tables, count));

    table = dict_table_open_on_id(aux_table->id, TRUE, DICT_TABLE_OP_NORMAL);
    ...
    error = fts_rename_one_aux_table_to_hex_format(trx, aux_table, parent_table);
    ...
    if (error != DB_SUCCESS)
    {
      ...
      ib::warn() << "Failed to rename one aux table " << aux_table->name
                 << ". Will revert"
    ...
    if (error != DB_SUCCESS)
    {
      ib::warn() << "Setting aux table " << aux_table->name << " to hex format failed.";
  ...
  if (error != DB_SUCCESS)
  {
    ...
    for (ulint i = 0; i <= count; ++i)
    {
      ...
      aux_table = static_cast<fts_aux_table_t *>(ib_vector_get(tables, i));
      ...
      if (!DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_AUX_HEX_NAME))
      {
        dict_table_close(table, TRUE, FALSE);
        continue;
      }
      ...
      DICT_TF2_FLAG_UNSET(table, DICT_TF2_FTS_AUX_HEX_NAME);
      err = row_rename_table_for_mysql(table->name.m_name, aux_table->name, trx_bg, FALSE);
      ...
      if (err != DB_SUCCESS)
      {
        ib::warn() << "Failed to revert table " << table->name
                   << ". Please revert"
                      " manually.";
...
static ib_id_t fts_fake_hex_to_dec(
    /*================*/
...
UNIV_INLINE
int fts_check_aux_table_parent_id_cmp(
    /*==============================*/
...
static void fts_parent_all_index_set_corrupt(trx_t *trx, dict_table_t *parent_table)
{
  ...
  {
    dict_index_t *index = static_cast<dict_index_t *>(ib_vector_getp_const(fts->indexes, j));
    dict_set_corrupted(index, trx, "DROP ORPHANED TABLE");
  }
}
...
@param[in, out]	parent_table	parent table to check with all
                                the index. */
static void fts_set_index_corrupt(trx_t *trx, index_id_t id, dict_table_t *table)
{
  fts_t *fts = table->fts;
...
static ulint fts_check_corrupt_index(fts_aux_table_t *aux_table)
{
  dict_table_t *table;
  dict_index_t *index;
  table = dict_table_open_on_id(aux_table->parent_id, TRUE, DICT_TABLE_OP_NORMAL);
...
char *fts_get_parent_table_name(const char *aux_table_name, ulint aux_table_len)
{
  ...
  if (fts_is_aux_table_name(&aux_table, aux_table_name, aux_table_len))
  {
    ...
    parent_table = dict_table_open_on_id(aux_table.parent_id, TRUE, DICT_TABLE_OP_NORMAL);

    if (parent_table != NULL)
    {
      parent_table_name = mem_strdupl(parent_table->name.m_name, strlen(parent_table->name.m_name));
...
static bool fts_valid_parent_table(const fts_aux_table_t *aux_table)
{
  dict_table_t *parent_table;
  bool valid = false;
  ...
  if (parent_table != NULL && parent_table->fts != NULL)
  {
    ...
    else
    {
      ...
      for (index = UT_LIST_GET_FIRST(parent_table->indexes); index; index = UT_LIST_GET_NEXT(indexes, index))
      {
        if (index->id == id)
        {
          valid = true;
...
static void fts_rename_aux_tables_to_hex_format(ib_vector_t *aux_tables, dict_table_t *parent_table)
{
  dberr_t err;
  ...
  err = fts_rename_aux_tables_to_hex_format_low(trx_rename, parent_table, aux_tables);
...
@param[in]	trx		transaction */
static void fts_set_parent_hex_format_flag(dict_table_t *parent_table, trx_t *trx)
{
  if (!DICT_TF2_FLAG_IS_SET(parent_table, DICT_TF2_FTS_AUX_HEX_NAME))
  {
    DBUG_EXECUTE_IF("parent_table_flag_fail", DBUG_SUICIDE(););

    dberr_t err = fts_update_hex_format_flag(trx, parent_table->id, true);

    if (err != DB_SUCCESS)
    {
      ib::fatal() << "Setting parent table " << parent_table->name << "to hex format failed. Please try "
...
static void fts_drop_obsolete_aux_table_from_vector(ib_vector_t *tables)
{
  dberr_t err;

  for (ulint count = 0; count < ib_vector_size(tables); ++count)
  {
    fts_aux_table_t *aux_drop_table;
    aux_drop_table = static_cast<fts_aux_table_t *>(ib_vector_get(tables, count));
    ...
    err = row_drop_table_for_mysql(aux_drop_table->name, trx_drop, false, true);
    ...
    if (err != DB_SUCCESS)
    {
      ...
      ib::warn() << "Failed to drop obsolete aux table " << aux_drop_table->name << ", which is "
...
@param[in]	trx	transaction
@param[in]	tables	tables to be dropped */
static void fts_drop_aux_table_from_vector(trx_t *trx, ib_vector_t *tables)
{
  for (ulint count = 0; count < ib_vector_size(tables); ++count)
  {
    fts_aux_table_t *aux_drop_table;
    ...
    {
      ib::warn() << "Parent table of FTS auxiliary table " << aux_drop_table->name << " not found.";

      dberr_t err = fts_drop_table(trx, aux_drop_table->name);
      if (err == DB_FAIL)
      {
        char *path = fil_make_filepath(NULL, aux_drop_table->name, IBD, false);
...
/**********************************************************************/ /**
 ...
 @return DB_SUCCESS or error code */
static MY_ATTRIBUTE((nonnull)) void fts_check_and_drop_orphaned_tables(
    /*===============================*/
    trx_t *trx,          /*!< in: transaction */
    ib_vector_t *tables) /*!< in: tables to check */
{
  ...
  aux_tables_to_rename = ib_vector_create(heap_alloc, sizeof(fts_aux_table_t), 128);

  /* We store all fake auxiliary table and orphaned table here. */
  invalid_aux_tables = ib_vector_create(heap_alloc, sizeof(fts_aux_table_t), 128);

  /* We store all valid aux tables. We use this to filter the
  fake auxiliary table from invalid auxiliary tables. */
  valid_aux_tables = ib_vector_create(heap_alloc, sizeof(fts_aux_table_t), 128);

  /* We store all auxiliary tables to be dropped. */
  drop_aux_tables = ib_vector_create(heap_alloc, sizeof(fts_aux_table_t), 128);

  /* We store all obsolete auxiliary tables to be dropped. */
  obsolete_aux_tables = ib_vector_create(heap_alloc, sizeof(fts_aux_table_t), 128);
  ...
  for (ulint i = 0; i < ib_vector_size(tables); ++i)
  {
    ...
    if (table == NULL || strcmp(table->name.m_name, aux_table->name))
    {
      bool fake_aux = false;
      ...
      {
        next_aux_table = static_cast<fts_aux_table_t *>(ib_vector_get(tables, i + 1));
      }

      /* To know whether aux table is fake fts or
      orphan fts table. */
      for (ulint count = 0; count < ib_vector_size(valid_aux_tables); count++)
      {
        fts_aux_table_t *valid_aux;
        valid_aux = static_cast<fts_aux_table_t *>(ib_vector_get(valid_aux_tables, count));
        if (strcmp(valid_aux->name, aux_table->name) == 0)
        {
          fake_aux = true;
          break;
        }
      ...
      them if necessary. */
      if ((next_aux_table == NULL || orig_parent_id != next_aux_table->parent_id) &&
          (!ib_vector_is_empty(aux_tables_to_rename)))
      {
        ib_id_t parent_id = fts_fake_hex_to_dec(aux_table->parent_id);

        parent_table = dict_table_open_on_id(parent_id, TRUE, DICT_TABLE_OP_NORMAL);

        fts_rename_aux_tables_to_hex_format(aux_tables_to_rename, parent_table);
    ...
    else if (!DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_AUX_HEX_NAME))
    {
      aux_table->parent_id = fts_fake_hex_to_dec(aux_table->parent_id);

      if (aux_table->index_id != 0)
      {
        aux_table->index_id = fts_fake_hex_to_dec(aux_table->index_id);
      }
    ...
    for (ulint count = 0; count < ib_vector_size(invalid_aux_tables); count++)
    {
      fts_aux_table_t *invalid_aux;
      invalid_aux = static_cast<fts_aux_table_t *>(ib_vector_get(invalid_aux_tables, count));
      if (strcmp(invalid_aux->name, aux_table->name) == 0)
      {
        ib_vector_remove(invalid_aux_tables, *reinterpret_cast<void **>(invalid_aux));
        break;
      }
    }
    ...
    if (fts_check_corrupt_index(aux_table) > 0)
    {
      ...
      if (next_aux_table == NULL || orig_parent_id != next_aux_table->parent_id)
      {
        ...
        {
          fts_set_parent_hex_format_flag(parent_table, trx);
        }
    ...
    if (!drop && rename)
    {
      ...
      for (ulint count = 0; count < ib_vector_size(aux_tables_to_rename); count++)
      {
        fts_aux_table_t *rename_aux = static_cast<fts_aux_table_t *>(ib_vector_get(aux_tables_to_rename, count));
        if (strcmp(rename_aux->name, aux_table->name) == 0)
        {
          rename_table = false;
          break;
        }
    ...
    if (!drop && !rename)
    {
      ...
      if (table != NULL && !DICT_TF2_FLAG_IS_SET(table, DICT_TF2_FTS_AUX_HEX_NAME))
      {
        DBUG_EXECUTE_IF("aux_table_flag_fail", ib::warn() << "Setting aux table " << table->name
                                                          << " to hex "
                                                             "format failed.";
                        fts_set_index_corrupt(trx, aux_table->index_id, parent_table); goto table_exit;);

        dberr_t err = fts_update_hex_format_flag(trx, table->id, true);

        if (err != DB_SUCCESS)
        {
          ib::warn() << "Setting aux table " << table->name
                     << " to hex "
                        "format failed.";
...
void fts_drop_orphaned_tables(void)
/*==========================*/
{
  trx_t *trx;
  pars_info_t *info;
  ...
  for (space_name_list_t::iterator it = space_name_list.begin(); it != space_name_list.end(); ++it)
  {
    fts_aux_table_t *fts_aux_table;

    fts_aux_table = static_cast<fts_aux_table_t *>(ib_vector_push(tables, NULL));
    ...
    else
    {
      ...
      fts_aux_table->name = static_cast<char *>(mem_heap_dup(heap, *it, len + 1));
  ...
  for (;;)
  {
    ...
    else
    {
      ...
      {
        ib::error() << "(" << ut_strerr(error) << ") while reading SYS_TABLES.";
                                     ...
                                     name */
{
  ...
  if (!table)
  {
    ib::error() << "User stopword table " << stopword_table_name << " does not exist.";
                                        ...
                                        for reloading FTS table */
{
  ...
  if (reload)
  {
    error = fts_config_get_ulint(trx, &fts_table, FTS_USE_STOPWORD, &use_stopword);
  }
  else
  {
    use_stopword = (ulint)stopword_is_on;

    error = fts_config_set_ulint(trx, &fts_table, FTS_USE_STOPWORD, use_stopword);
  }
  ...
  if (reload)
  {
    ...
    error = fts_config_get_value(trx, &fts_table, FTS_STOPWORD_TABLE_NAME, &str);
  ...
  {
    stopword_to_use = (session_stopword_table) ? session_stopword_table : global_stopword_table;
  }

  if (stopword_to_use && fts_load_user_stopword(table->fts, stopword_to_use, &cache->stopword_info))
  {
    ...
    if (!reload)
    {
      ...
      error = fts_config_set_value(trx, &fts_table, FTS_STOPWORD_TABLE_NAME, &str);
    }
  }
  else
  {
    ...
    void *user_arg) /*!< in: fts cache */
{
  ...
  if (exp)
  {
    ...
    doc_id = static_cast<doc_id_t>(mach_read_from_8(static_cast<const byte *>(data)));
    ...
    void *user_arg) /*!< in: fts cache */
{
  ...
  while (exp)
  {
    ...
    if (dfield_is_ext(dfield))
    {
      ...
      doc.text.f_str = btr_copy_externally_stored_field(&doc.text.f_len, static_cast<byte *>(dfield_get_data(dfield)),
                          ...
                          cache lock */
{
  ...
  if (ib_vector_is_empty(cache->get_docs))
  {
    ...
    fts_doc_fetch_by_doc_id(NULL, start_doc, index, FTS_FETCH_DOC_BY_ID_LARGE, fts_init_get_doc_id, cache);
  }
  else
  {
    if (table->fts->cache->stopword_info.status & STOPWORD_NOT_INIT)
    ...
    for (ulint i = 0; i < ib_vector_size(cache->get_docs); ++i)
    {
      ...
      fts_doc_fetch_by_doc_id(NULL, start_doc, index, FTS_FETCH_DOC_BY_ID_LARGE, fts_init_recover_doc, get_doc);
    }
  }
...
void fts_check_corrupt(dict_table_t *base_table, trx_t *trx)
{
  bool sane = true;
  fts_table_t fts_table;
  ...
  for (ulint i = 0; fts_common_tables[i] != NULL && sane; ++i)
  {
    ...
    dict_table_t *aux_table = dict_table_open_on_name(table_name, true, FALSE, DICT_ERR_IGNORE_NONE);

    if (aux_table == NULL)
    {
      dict_set_corrupted(dict_table_get_first_index(base_table), trx, "FTS_SANITY_CHECK");
      ut_ad(base_table->corrupted == TRUE);
      sane = false;
      continue;
    }

    for (dict_index_t *aux_table_index = UT_LIST_GET_FIRST(aux_table->indexes); aux_table_index != NULL;
         aux_table_index = UT_LIST_GET_NEXT(indexes, aux_table_index))
    {
