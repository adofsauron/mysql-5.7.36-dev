...
{
  ranking->words = static_cast<byte *>(mem_heap_zalloc(query->heap, RANKING_WORDS_INIT_LEN));
    ...
    const fts_string_t *word) /*!< in: term/word to add */
{
  ...
  if (byte_offset >= ranking->words_len)
  {
    ...
    ranking->words = static_cast<byte *>(mem_heap_zalloc(query->heap, words_len));
    ...
    const fts_string_t *word) /*!< in: term/word to add */
{
  ...
  if (rbt_search(query->word_freqs, &parent, word) != 0)
  {
    ...
    word_freq.doc_freqs = rbt_create(sizeof(fts_doc_freq_t), fts_freq_doc_id_cmp);

    parent.last = rbt_add_node(query->word_freqs, &parent, &word_freq);

    query->total_size += word->f_len + SIZEOF_RBT_CREATE + SIZEOF_RBT_NODE_ADD + sizeof(fts_word_freq_t);
  }
                        ...
                        rank associated with the doc_id */
{
  ...
  if (fts_bsearch(array, 0, static_cast<int>(size), doc_id) < 0 && rbt_search(query->doc_ids, &parent, &doc_id) != 0)
  {
    ...
    query->total_size += SIZEOF_RBT_NODE_ADD + sizeof(fts_ranking_t) + RANKING_WORDS_INIT_LEN;
  }
}
    ...
    const fts_string_t *word) /*!< in: the token to add */
{
  ...
  if (query->intersection && rbt_search(query->intersection, &parent, &doc_id) == 0)
  {
    ranking = rbt_value(fts_ranking_t, parent.last);
  }
    ...
    const fts_node_t *node)    /*!< in: node to check */
{
  ...
  if (query->oper == FTS_EXIST && ((query->upper_doc_id > 0 && node->first_doc_id > query->upper_doc_id) ||
  ...
  else
  {
    ...
    query->error = fts_query_filter_doc_ids(query, token, word_freqs, node, node->ilist, ilist_size, TRUE);
  }
}
    ...
    const fts_string_t *token)            /*!< in: token to search */
{
  ...
  srch_text.f_len = (token->f_str[token->f_len - 1] == '%') ? token->f_len - 1 : token->f_len;
  ...
  if (rbt_search_cmp(index_cache->words, &parent, &srch_text, NULL, innobase_fts_text_cmp_prefix) == 0)
  {
    ...
    while (innobase_fts_text_cmp_prefix(index_cache->charset, &srch_text, &word->text) == 0)
    {
      nodes = word->nodes;
      ...
      for (i = 0; nodes && i < ib_vector_size(nodes); ++i)
      {
        ...
        query->error =
            fts_query_filter_doc_ids(query, &srch_text, word_freqs, node, node->ilist, node->ilist_size, TRUE);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_query_difference(
    /*=================*/
    fts_query_t *query,        /*!< in: query instance */
    const fts_string_t *token) /*!< in: token to search */
{
  ...
  if (query->doc_ids && !rbt_empty(query->doc_ids))
  {
    ...
    if (query->cur_node->term.wildcard && query->flags != FTS_PROXIMITY && query->flags != FTS_PHRASE)
    {
    ...
    else
    {
      ...
      for (i = 0; nodes && i < ib_vector_size(nodes) && query->error == DB_SUCCESS; ++i)
      {
        const fts_node_t *node;
    ...
    error = fts_index_fetch_nodes(trx, &graph, &query->fts_index_table, token, &fetch);
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS if all go well */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_query_intersect(
    /*================*/
    fts_query_t *query,        /*!< in: query instance */
    const fts_string_t *token) /*!< in: the token to search */
{
  ...
  if (!(rbt_empty(query->doc_ids) && query->multi_exist))
  {
    ...
    the intersection. */
    query->intersection = rbt_create(sizeof(fts_ranking_t), fts_ranking_doc_id_cmp);
    ...
    if (query->error == DB_SUCCESS)
    {
      ...
      ut_a(!query->multi_exist || (query->multi_exist && rbt_size(query->doc_ids) <= n_doc_ids));
    }
  }
...
/*****************************************************************/ /**
 Set union.
 @return DB_SUCCESS if all go well */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_query_union(
    /*============*/
    fts_query_t *query,  /*!< in: query instance */
    fts_string_t *token) /*!< in: token to search */
{
  ...
  ut_a(query->oper == FTS_NONE || query->oper == FTS_DECR_RATING || query->oper == FTS_NEGATE ||
       query->oper == FTS_INCR_RATING);
    ...
    const ib_rbt_t *doc_ids) /*!< in: result set to merge */
{
  ...
  for (node = rbt_first(doc_ids); node; node = rbt_next(doc_ids, node))
  {
    ...
    query->error = fts_query_process_doc_id(query, ranking->doc_id, ranking->rank);
    ...
    mem_heap_t *heap)     /*!< in: heap */
{
  ...
  for (i = 1; ptr < end && i < ib_vector_size(tokens); /* No op */)
  {
    ...
    ret = innobase_mysql_fts_get_token(phrase->charset, ptr, const_cast<byte *>(end), &match);
    ...
    ulint total_len)            /*!< in: length of text */
{
  ...
  for (ulint i = 0; i < proximity_pos->n_pos; i++)
  {
    ...
    while (cur_pos <= proximity_pos->max_pos[i])
    {
      ...
      len = innobase_mysql_fts_get_token(phrase->charset, start + cur_pos, start + total_len, &str);
    ...
    MYSQL_FTPARSER_BOOLEAN_INFO *info) /*!< in: token info */
{
  ...
  if (match.f_len > 0)
  {
    ...
    token = static_cast<const fts_string_t *>(ib_vector_get_const(tokens, phrase_param->token_index));
...
/*****************************************************************/ /**
 ...
 @return TRUE if matched else FALSE */
static ibool fts_query_match_phrase(
    /*===================*/
    fts_phrase_t *phrase, /*!< in: phrase to match */
    ...
    mem_heap_t *heap)     /* heap */
{
  ...
  for (i = phrase->match->start; i < ib_vector_size(positions); ++i)
  {
    ...
    if (phrase->parser)
    {
      ...
      if (fts_query_match_phrase_terms_by_parser(&phrase_param, phrase->parser, ptr, (end - ptr)))
      {
        break;
      }
    }
    else
    {
      ...
      match.f_str = ptr;
      ret = innobase_mysql_fts_get_token(phrase->charset, start + pos, const_cast<byte *>(end), &match);
    ...
    void *user_arg) /*!< in:  fts_doc_t* */
{
  ...
  if (phrase->proximity_pos)
  {
    while (exp)
    {
      ...
      if (dfield_is_ext(dfield))
      {
        ...
        field_len = mach_read_from_4(data + local_len + BTR_EXTERN_LEN + 4);
      }
      else
      {
    ...
    document_text = static_cast<byte *>(mem_heap_zalloc(phrase->heap, total_len));
  ...
  while (exp)
  {
    ...
    if (dfield_is_ext(dfield))
    {
      data = btr_copy_externally_stored_field(&cur_len, data, phrase->page_size, dfield_get_len(dfield), phrase->heap);
    }
    else
    {
    ...
    if (cur_len != UNIV_SQL_NULL && cur_len != 0)
    {
      ...
      {
        /* For phrase search */
        phrase->found = fts_query_match_phrase(phrase, static_cast<byte *>(data), cur_len, prev_len, phrase->heap);
      }
  ...
  if (phrase->proximity_pos)
  {
    ...
    phrase->found = fts_proximity_is_word_in_range(phrase, document_text, total_len);
  }
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_query_match_document(
    /*=====================*/
    ...
    ibool *found)              /*!< out: TRUE if phrase found */
{
  ...
  *found = phrase.found = FALSE;
  ...
  error = fts_doc_fetch_by_doc_id(get_doc, match->doc_id, NULL, FTS_FETCH_DOC_BY_ID_EQUAL, fts_query_fetch_document,
                                  &phrase);
...
static MY_ATTRIBUTE((nonnull, warn_unused_result)) bool fts_query_is_in_proximity_range(
    /*============================*/
    const fts_query_t *query,       /*!< in:  query instance */
    fts_match_t **match,            /*!< in: query instance */
                                    ...
                                    qualified ranges */
{
  ...
  err = fts_doc_fetch_by_doc_id(&get_doc, match[0]->doc_id, NULL, FTS_FETCH_DOC_BY_ID_EQUAL, fts_query_fetch_document,
                                &phrase);
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS if all OK */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_query_search_phrase(
    /*====================*/
    fts_query_t *query,       /*!< in: query instance */
                              ...
                              ranking */
{
  ...
  for (i = 0; i < n_matched && query->error == DB_SUCCESS; ++i)
  {
    ...
    if (match->doc_id != 0)
    {
      query->error = fts_query_match_document(orig_tokens, &get_doc, match, query->distance, query->parser, &found);
...
static void fts_query_phrase_split(fts_query_t *query, const fts_ast_node_t *node, ib_vector_t *tokens,
                                   ib_vector_t *orig_tokens, mem_heap_t *heap)
{
  ...
  while (true)
  {
    ...
    if (node->type == FTS_AST_TEXT)
    {
      ...
      cur_len = innobase_mysql_fts_get_token(query->fts_index_table.charset,
    ...
    else
    {
      ...
      result_str.f_n_char = fts_get_token_size(query->fts_index_table.charset,
    ...
    fts_string_t *token = static_cast<fts_string_t *>(ib_vector_push(tokens, NULL));
    fts_string_dup(token, &result_str, heap);

    if (fts_check_token(&result_str, cache->stopword_info.cached_stopword, query->index->is_ngram,
    ...
    {
      fts_string_t *orig_token = static_cast<fts_string_t *>(ib_vector_push(orig_tokens, NULL));
  ...
  if (num_token > 0)
  {
    ...
    if (!query->matched)
    {
      ...
      else
      {
        ...
        query->match_array = (ib_vector_t **)mem_heap_alloc(heap, num_token * sizeof(query->matched));

        for (i = 0; i < num_token; i++)
        {
          query->match_array[i] = ib_vector_create(heap_alloc, sizeof(fts_match_t), 64);
        }
    ...
    for (i = 0; i < num_token; i++)
    {
      ...
      if (ib_vector_is_empty(query->match_array[i]) || query->error != DB_SUCCESS)
      {
        goto func_exit;
      }
    }
    ...
    if (ib_vector_size(orig_tokens) == 1 && !ib_vector_is_empty(query->match_array[0]))
    {
      fts_match_t *match;
      ulint n_matched;
      ...
      {
        match = static_cast<fts_match_t *>(ib_vector_get(query->match_array[0], i));
...
/*****************************************************************/ /**
 ...
 @return DB_SUCCESS if all go well */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_query_execute(
    /*==============*/
    fts_query_t *query,  /*!< in: query instance */
    fts_string_t *token) /*!< in: token to search */
{
    ...
    ibool calc_doc_count)       /*!< in: whether to remember doc count */
{
  ...
  while (decoded < len)
  {
    ...
    if (query->collect_positions)
    {
      ...
      query->total_size += sizeof(fts_match_t) + sizeof(ib_vector_t) + sizeof(ulint) * 64;
    }
    ...
    que_node_t *exp)          /*!< in: query graph node */
{
  ...
  ut_a(query->cur_node->type == FTS_AST_TERM || query->cur_node->type == FTS_AST_TEXT ||
  ...
  for (i = 1; exp && !skip; exp = que_node_get_next(exp), ++i)
  {
    ...
    switch (i)
    {
      ...
      case 3: /* LAST_DOC_ID */
        ...
        /* Skip nodes whose doc ids are out range. */
        if (query->oper == FTS_EXIST && query->lower_doc_id > 0 && node.last_doc_id < query->lower_doc_id)
        {
          skip = TRUE;
        }
        break;
      ...
      case 4: /* ILIST */
        ...
        error = fts_query_filter_doc_ids(query, &word_freq->word, word_freq, &node, data, len, FALSE);
    ...
    fts_query_t *query) /*!< in: Query state */
{
  ...
  may have allocated. */
  for (node = rbt_first(query->word_freqs); node; node = rbt_next(query->word_freqs, node))
  {
    fts_word_freq_t *word_freq;
    ...
    {
      ib::info() << "'" << word_freq->word.f_str << "' -> " << query->total_docs << "/" << word_freq->doc_count << " "
                 << std::setw(6) << std::setprecision(5) << word_freq->idf;
    }
  }
}
                          ...
                          another FTS index */
{
  ...
  if (result == NULL)
  {
    ...
    result->rankings_by_id = rbt_create(sizeof(fts_ranking_t), fts_ranking_doc_id_cmp);
  ...
  if (query->flags == FTS_OPT_RANKING)
  {
    ...
    for (node = rbt_first(word_freq->doc_freqs); node; node = rbt_next(word_freq->doc_freqs, node))
    {
      fts_doc_freq_t *doc_freq;
      fts_ranking_t ranking;
      ...
      if (fts_bsearch(array, 0, static_cast<int>(size), doc_freq->doc_id) >= 0)
      {
    ...
    for (node = rbt_first(result->rankings_by_id); node != NULL; node = rbt_next(result->rankings_by_id, node))
    {
      fts_ranking_t *ranking;

      ranking = rbt_value(fts_ranking_t, node);

      ranking->rank = static_cast<fts_rank_t>(ranking->rank * word_freq->idf * word_freq->idf);
    }
  ...
  for (node = rbt_first(query->doc_ids); node; node = rbt_next(query->doc_ids, node))
  {
    fts_ranking_t *ranking;
    ...
    ulint query_len)    /*!< in: query string length */
{
  ...
  if (query->parser)
  {
    ...
    error = fts_parse_by_parser(mode, query_str, query_len, query->parser, &state);
  }
  else
  {
...
dberr_t fts_query(trx_t *trx, dict_index_t *index, uint flags, const byte *query_str, ulint query_len,
                  fts_result_t **result, ulonglong limit)
{
  ...
  *result = NULL;
  ...
  query.word_map = rbt_create_arg_cmp(sizeof(fts_string_t), innobase_fts_text_cmp, charset);
  ...
  statistics. */
  query.word_freqs = rbt_create_arg_cmp(sizeof(fts_word_freq_t), innobase_fts_text_cmp, charset);

  if (flags & FTS_EXPAND)
  {
    query.wildcard_words = rbt_create_arg_cmp(sizeof(fts_string_t), innobase_fts_text_cmp, charset);
  }
  ...
  if (ft_enable_diag_print)
  {
    ...
    ib::info() << "Total docs: " << query.total_docs << " Total words: " << query.total_words;
  }
  ...
  fts_cache_append_deleted_doc_ids(index->table->fts->cache, query.deleted->doc_ids);
  ...
  {
    result_len =
        innobase_fts_casedn_str(charset, (char *)(query_str), query_len, (char *)(lc_query_str), lc_query_str_len);
  }
  ...
  if (fts_query_parse(&query, lc_query_str, result_len))
  {
    ...
    DBUG_EXECUTE_IF("fts_instrument_result_cache_limit", fts_result_cache_limit = 2048;);
    ...
    the query. */
    query.error = fts_ast_visit(FTS_NONE, ast, fts_query_visitor, &query, &will_be_ignored);
  ...
  if (fts_enable_diag_print && (*result))
  {
    ...
    ib::info() << "FTS Search Processing time: " << diff_time / 1000 << " secs: " << diff_time % 1000
               << " millisec: row(s) " << ((*result)->rankings_by_id ? rbt_size((*result)->rankings_by_id) : -1);

    /* Log memory consumption & result size */
    ib::info() << "Full Search Memory: " << query.total_size
               << " (bytes),  Row: " << ((*result)->rankings_by_id ? rbt_size((*result)->rankings_by_id) : 0) << ".";
  }
    ...
    fts_result_t *result) /*!< out: result instance to sort.*/
{
  ...
  /* We need to free any instances of fts_doc_freq_t that we
  may have allocated. */
  for (node = rbt_first(result->rankings_by_id); node; node = rbt_next(result->rankings_by_id, node))
  {
    fts_ranking_t *ranking;
...
/*************************************************************/ /**
 ...
 the search result set.
 @return DB_SUCCESS if success, otherwise the error code */
static MY_ATTRIBUTE((nonnull, warn_unused_result)) dberr_t fts_expand_query(
    /*=============*/
    dict_index_t *index, /*!< in: FTS index to search */
    fts_query_t *query)  /*!< in: FTS query instance */
{
  ...
  result_doc.tokens = rbt_create_arg_cmp(sizeof(fts_token_t), innobase_fts_text_cmp, index_cache->charset);
  ...
  for (node = rbt_first(query->doc_ids); node; node = rbt_next(query->doc_ids, node))
  {
    ...
    fts_doc_fetch_by_doc_id(NULL, ranking->doc_id, index, FTS_FETCH_DOC_BY_ID_EQUAL, fts_query_expansion_fetch_doc,
                            &result_doc);
    ...
    estimate_size = (rbt_size(result_doc.tokens) - prev_token_size) *
                    (SIZEOF_RBT_NODE_ADD + sizeof(fts_token_t) + sizeof(ib_vector_t) + sizeof(ulint) * 32);
  ...
  for (ulint i = 0; i < query->word_vector->size(); i++)
  {
    ...
    if (query->wildcard_words && rbt_search(query->wildcard_words, &parent, &word) == 0)
    {
      /* If it's a wildcard word, remove words having
      it as prefix. */
      while (rbt_search_cmp(result_doc.tokens, &parent, &word, NULL, innobase_fts_text_cmp_prefix) == 0)
      {
  ...
  for (token_node = rbt_first(result_doc.tokens); token_node; token_node = rbt_next(result_doc.tokens, token_node))
  {
    fts_token_t *mytoken;
    ...
    ib_vector_t *tokens) /*!< in: Tokens contain words */
{
  ...
  for (i = 0; i < n_matched; i++)
  {
    ...
    match[0] = static_cast<fts_match_t *>(ib_vector_get(query->match_array[0], i));
    ...
    {
      match[j] = static_cast<fts_match_t *>(ib_vector_get(query->match_array[j], k));

      while (match[j]->doc_id < match[0]->doc_id && k < ib_vector_size(query->match_array[j]))
      {
        match[j] = static_cast<fts_match_t *>(ib_vector_get(query->match_array[j], k));
        k++;
      }
      ...
      if (k == ib_vector_size(query->match_array[j]))
      {
        ...
        if (query->flags & FTS_PHRASE)
        {
          ...
          {
            match_temp = static_cast<fts_match_t *>(ib_vector_get(query->match_array[0], s));
            match_temp->doc_id = 0;
          }
    ...
    else if (fts_proximity_get_positions(match, num_token, ULINT_MAX, &qualified_pos))
    {
                                    ...
                                    all matching words. */
{
  ...
  while (idx[min_idx] < num_pos[min_idx])
  {
    ...
    find a good match */
    if (max_pos - min_pos <= distance && (i >= num_match || position[i] != ULINT_UNDEFINED))
    {
