...
struct st_mysql_ftparser fts_default_parser = {MYSQL_FTPARSER_INTERFACE_VERSION, fts_default_parser_parse,
...
int fts_query_add_word_for_parser(
    /*==========================*/
    ...
    MYSQL_FTPARSER_BOOLEAN_INFO *info) /*!< in: token info */
{
  ...
  switch (info->type)
  {
    ...
    case FT_TOKEN_WORD:
      ...
      {
        ut_ad(cur_node->type == FTS_AST_LIST || cur_node->type == FTS_AST_SUBEXP_LIST);
    ...
    case FT_TOKEN_LEFT_PAREN:
      /* Check parse error */
      if (cur_node->type != FTS_AST_LIST && cur_node->type != FTS_AST_SUBEXP_LIST)
      {
        return (1);
      }
    ...
    int len)                     /*!< in: query length */
{
  ...
  {
    int ret = param->mysql_add_word(param, reinterpret_cast<char *>(w.pos), w.len, &info);
    if (ret)
    {
      return (ret);
    }
    ...
    fts_ast_state_t *state)    /*!< in/out: parser state */
{
  ...
  param.flags = 0;
  param.mode = mode ? MYSQL_FTPARSER_FULL_BOOLEAN_INFO : MYSQL_FTPARSER_SIMPLE_MODE;
