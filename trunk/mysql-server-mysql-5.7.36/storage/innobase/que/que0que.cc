    ...
    que_fork_t *fork) /*!< in: a query fork */
{
  ...
  for (thr = UT_LIST_GET_FIRST(fork->thrs); thr != NULL; thr = UT_LIST_GET_NEXT(thrs, thr))
  {
    switch (thr->state)
    {
      case QUE_THR_COMMAND_WAIT:
    ...
    que_node_t *node) /*!< in: query graph node */
{
  ...
  DBUG_PRINT("que_graph_free_recursive", ("node: %p, type: %lu", node, que_node_get_type(node)));
...
static MY_ATTRIBUTE((warn_unused_result, nonnull)) const char *que_node_type_string(
    /*=================*/
    ...
    que_thr_t *thr) /*!< in: query thread */
{
  ...
  DBUG_PRINT("ib_que", ("Execute %u (%s) at %p", unsigned(type), que_node_type_string(node), (const void *)node));
