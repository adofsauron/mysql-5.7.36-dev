...
#define mi_set_key_active(_keymap_, _keyno_) \
  (_keymap_) |= (((_keyno_) < MI_KEYMAP_BITS) ? (1ULL << (_keyno_)) : MI_KEYMAP_HIGH_MASK)
#define mi_clear_key_active(_keymap_, _keyno_) \
  (_keymap_) &= (((_keyno_) < MI_KEYMAP_BITS) ? (~(1ULL << (_keyno_))) : (~(0ULL)) /*ignore*/)

#else

#define mi_is_key_active(_keymap_, _keyno_) MY_TEST((_keymap_) & (1ULL << (_keyno_)))
...
#define mi_close(file) mi_close_share(file, NULL)
  ...
  extern struct st_myisam_info *mi_open_share(const char *name, struct st_mi_isam_share *old_share, int mode,
                                              uint wait_if_locked);
#define mi_open(name, mode, wait_if_locked) mi_open_share(name, NULL, mode, wait_if_locked)
  ...
  extern int mi_rkey(MI_INFO *info, uchar *buf, int inx, const uchar *key, key_part_map keypart_map,
  ...
  extern int mi_rsame_with_pos(struct st_myisam_info *file, uchar *record, int inx, my_off_t pos);
  extern int mi_update(struct st_myisam_info *file, const uchar *old, uchar *new_record);
  ...
  extern ha_rows mi_records_in_range(MI_INFO *info, int inx, key_range *min_key, key_range *max_key);
...
#define O_DATA_LOST 4
  ...
  */
  ...
  int mi_repair(MI_CHECK *param, MI_INFO *info, char *name, int rep_quick, my_bool no_copy_stat);
  int mi_sort_index(MI_CHECK *param, MI_INFO *info, char *name, my_bool no_copy_stat);
  int mi_repair_by_sort(MI_CHECK *param, MI_INFO *info, const char *name, int rep_quick, my_bool no_copy_stat);
  int mi_repair_parallel(MI_CHECK *param, MI_INFO *info, const char *name, int rep_quick, my_bool no_copy_stat);
  ...
  void update_key_parts(MI_KEYDEF *keyinfo, ulong *rec_per_key_part, ulonglong *unique, ulonglong *notnull,
                        ulonglong records);
