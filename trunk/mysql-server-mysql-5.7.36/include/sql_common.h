...
#define MYSQL_EXTENSION_PTR(H) \
  ((struct st_mysql_extension *)((H)->extension ? (H)->extension : ((H)->extension = mysql_extension_init(H))))
  ...
  typedef struct st_mysql_methods
  {
    ...
    my_bool (*advanced_command)(MYSQL *mysql, enum enum_server_command command, const unsigned char *header,
                                size_t header_length, const unsigned char *arg, size_t arg_length, my_bool skip_check,
                                MYSQL_STMT *stmt);
...
#define simple_command(mysql, command, arg, length, skip_check)                                                  \
  ((mysql)->methods ? (*(mysql)->methods->advanced_command)(mysql, command, 0, 0, arg, length, skip_check, NULL) \
                    : (set_mysql_error(mysql, CR_COMMANDS_OUT_OF_SYNC, unknown_sqlstate), 1))
#define stmt_command(mysql, command, arg, length, stmt)                                                 \
  ((mysql)->methods ? (*(mysql)->methods->advanced_command)(mysql, command, 0, 0, arg, length, 1, stmt) \
                    : (set_mysql_error(mysql, CR_COMMANDS_OUT_OF_SYNC, unknown_sqlstate), 1))

  extern CHARSET_INFO *default_client_charset_info;
  MYSQL_FIELD *unpack_fields(MYSQL *mysql, MYSQL_ROWS *data, MEM_ROOT *alloc, uint fields, my_bool default_value,
  ...
  my_bool cli_advanced_command(MYSQL *mysql, enum enum_server_command command, const unsigned char *header,
                               size_t header_length, const unsigned char *arg, size_t arg_length, my_bool skip_check,
                               MYSQL_STMT *stmt);
