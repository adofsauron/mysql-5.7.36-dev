...
*/
#if SIZEOF_TIME_T > 4
#define IS_TIME_T_VALID_FOR_TIMESTAMP(x) ((x) <= TIMESTAMP_MAX_VALUE && (x) >= TIMESTAMP_MIN_VALUE)
#else
...
my_bool str_to_datetime(const char *str, size_t length, MYSQL_TIME *l_time, my_time_flags_t flags,
...
my_bool str_to_time(const char *str, size_t length, MYSQL_TIME *l_time, MYSQL_TIME_STATUS *status);
