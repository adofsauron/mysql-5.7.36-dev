...
class Simple_cstring
{
 ...
 public:
  ...
  Simple_cstring(const char *str_arg, size_t length_arg) { set(str_arg, length_arg); }
  ...
  */
  ...
  {
    return m_length == other.m_length && memcmp(m_str, other.m_str, m_length) == 0;
  }
  /**
...
inline size_t copy_and_convert(char *to, size_t to_length, const CHARSET_INFO *to_cs, const char *from,
...
size_t well_formed_copy_nchars(const CHARSET_INFO *to_cs, char *to, size_t to_length, const CHARSET_INFO *from_cs,
                               const char *from, size_t from_length, size_t nchars, const char **well_formed_error_pos,
                               const char **cannot_convert_error_pos, const char **from_end_pos);
size_t convert_to_printable(char *to, size_t to_len, const char *from, size_t from_len, const CHARSET_INFO *from_cs,
                            size_t nbytes = 0);
...
class String
{
 ...
 public:
  String() : m_ptr(NULL), m_length(0), m_charset(&my_charset_bin), m_alloced_length(0), m_is_alloced(false) {}
  String(size_t length_arg)
      : m_ptr(NULL), m_length(0), m_charset(&my_charset_bin), m_alloced_length(0), m_is_alloced(false)
  {
    (void)real_alloc(length_arg);
  }
  String(const char *str, const CHARSET_INFO *cs)
      : m_ptr(const_cast<char *>(str)), m_length(strlen(str)), m_charset(cs), m_alloced_length(0), m_is_alloced(false)
  {
  }
  String(const char *str, size_t len, const CHARSET_INFO *cs)
      : m_ptr(const_cast<char *>(str)), m_length(len), m_charset(cs), m_alloced_length(0), m_is_alloced(false)
  {
  }
  String(char *str, size_t len, const CHARSET_INFO *cs)
      : m_ptr(str), m_length(len), m_charset(cs), m_alloced_length(static_cast<uint32>(len)), m_is_alloced(false)
  {
  }
  ...
  char *c_ptr()
  {
    assert(!m_is_alloced || !m_ptr || !m_alloced_length || (m_alloced_length >= (m_length + 1)));
  ...
  */
  ...
  bool set(longlong num, const CHARSET_INFO *cs) { return set_int(num, false, cs); }
  bool set(ulonglong num, const CHARSET_INFO *cs) { return set_int((longlong)num, true, cs); }
 ...
 public:
  ...
  void shrink(size_t arg_length)
  {
    ...
    {
      char *new_ptr;
      if (!(new_ptr = static_cast<char *>(my_realloc(STRING_PSI_MEMORY_KEY, m_ptr, arg_length, MYF(0)))))
      {
        m_alloced_length = 0;
  ...
  */
  ...
  static bool needs_conversion(size_t arg_length, const CHARSET_INFO *cs_from, const CHARSET_INFO *cs_to,
                               size_t *offset);
  ...
  bool copy(const char *s, size_t arg_length, const CHARSET_INFO *csfrom, const CHARSET_INFO *csto, uint *errors);
  ...
  int reserve(size_t space_needed) { return mem_realloc(m_length + space_needed); }
  ...
  */
  ...
  void write_at_position(int position, uint32 value) { int4store(m_ptr + position, value); }
...
class StringBuffer : public String
{
 ...
 public:
  ...
  explicit StringBuffer(const CHARSET_INFO *cs) : String(buff, buff_sz, cs) { length(0); }
  StringBuffer(const char *str, size_t length, const CHARSET_INFO *cs) : String(buff, buff_sz, cs)
  {
    set(str, length, cs);
  }
};

static inline bool check_if_only_end_space(const CHARSET_INFO *cs, char *str, char *end)
{
...
bool validate_string(const CHARSET_INFO *cs, const char *str, uint32 length, size_t *valid_length, bool *length_error);
