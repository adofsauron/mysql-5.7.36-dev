...
bool Rpl_filter::tables_ok(const char *db, TABLE_LIST *tables)
{
  ...
  */
  DBUG_RETURN(some_tables_updating && !do_table_hash_inited && !wild_do_table_inited);
}
...
int Rpl_filter::build_do_table_hash()
{
  ...
  if (build_table_hash_from_array(&do_table_array, &do_table_hash, do_table_array_inited, &do_table_hash_inited))
    DBUG_RETURN(1);
...
int Rpl_filter::build_ignore_table_hash()
{
  ...
  if (build_table_hash_from_array(&ignore_table_array, &ignore_table_hash, ignore_table_array_inited,
...
/**
  ...
  @return
             ...
             1           error
*/
int Rpl_filter::build_table_hash_from_array(Table_rule_array *table_array, HASH *table_hash, bool array_inited,
                                            bool *hash_inited)
{
...
int Rpl_filter::add_table_rule_to_array(Table_rule_array *a, const char *table_spec)
{
  const char *dot = strchr(table_spec, '.');
  if (!dot)
    return 1;
...
int Rpl_filter::add_string_pair_list(I_List<i_string_pair> *list, char *key, char *val)
{
...
uchar *get_table_key(const uchar *a, size_t *len, my_bool MY_ATTRIBUTE((unused)))
{
...
{
  my_hash_init(h, table_alias_charset, TABLE_RULE_HASH_SIZE, 0, 0, get_table_key, free_table_ent, 0,
...
TABLE_RULE_ENT *Rpl_filter::find_wild(Table_rule_array *a, const char *key, size_t len)
{
  ...
  for (size_t i = 0; i < a->size(); i++)
  {
    ...
    /*
      ...
      Otherwise they will be case insensitive but accent sensitive.
    */
    if (!my_wildcmp(table_alias_charset, key, key_end, (const char *)e->db, (const char *)(e->db + e->key_len), '\\',
                    wild_one, wild_many))
      return e;
  }
...
void Rpl_filter::table_rule_ent_dynamic_array_to_str(String *s, Table_rule_array *a, bool inited)
{
  s->length(0);
  if (inited)
  {
...
void Sql_cmd_change_repl_filter::set_filter_value(List<Item> *item_list, options_mysqld filter_type)
{
...
bool Sql_cmd_change_repl_filter::change_rpl_filter(THD *thd)
{
  ...
  */
  ...
  if (!ret)
  {
    if (!rpl_filter->set_do_db(do_db_list) && !rpl_filter->set_ignore_db(ignore_db_list) &&
        !rpl_filter->set_do_table(do_table_list) && !rpl_filter->set_ignore_table(ignore_table_list) &&
        !rpl_filter->set_wild_do_table(wild_do_table_list) &&
        !rpl_filter->set_wild_ignore_table(wild_ignore_table_list) && rpl_filter->set_db_rewrite(rewrite_db_pair_list))
    {
      /* purecov: begin inspected */
