...
extern "C"
{
  static uchar *hash_get_key_for_sp_head(const uchar *ptr, size_t *plen, my_bool first)
  {
...
class sp_cache
{
 public:
  sp_cache()
  {
    my_hash_init(&m_hashtable, system_charset_info, 0, 0, 0, hash_get_key_for_sp_head, hash_free_sp_head, 0,
                 key_memory_sp_cache);
  }
...
void sp_cache_insert(sp_cache **cp, sp_head *sp)
{
  ...
  DBUG_PRINT("info", ("sp_cache: inserting: %.*s", (int)sp->m_qname.length, sp->m_qname.str));
  c->insert(sp);
