      ...
      m_transaction_write_set_ctx()
{
  ...
  init_sql_alloc(key_memory_thd_transactions, &m_mem_root, global_system_variables.trans_alloc_block_size,
...
void Transaction_ctx::push_unsafe_rollback_warnings(THD *thd)
{
  if (m_scope_info[SESSION].has_modified_non_trans_table())
    push_warning(thd, Sql_condition::SL_WARNING, ER_WARNING_NOT_COMPLETE_ROLLBACK,
                 ER(ER_WARNING_NOT_COMPLETE_ROLLBACK));

  if (m_scope_info[SESSION].has_created_temp_table())
    push_warning(thd, Sql_condition::SL_WARNING, ER_WARNING_NOT_COMPLETE_ROLLBACK_WITH_CREATED_TEMP_TABLE,
                 ER(ER_WARNING_NOT_COMPLETE_ROLLBACK_WITH_CREATED_TEMP_TABLE));

  if (m_scope_info[SESSION].has_dropped_temp_table())
    push_warning(thd, Sql_condition::SL_WARNING, ER_WARNING_NOT_COMPLETE_ROLLBACK_WITH_DROPPED_TEMP_TABLE,
...
bool Transaction_ctx::add_changed_table(const char *key, long key_length)
{
  ...
  for (; curr; prev_changed = &(curr->next), curr = curr->next)
  {
    ...
    if (cmp < 0)
    {
      ...
      DBUG_PRINT("info", ("key_length: %ld  %u", key_length, (*prev_changed)->key_length));
      DBUG_RETURN(false);
    }
    else if (cmp == 0)
    {
...
CHANGED_TABLE_LIST *Transaction_ctx::changed_table_dup(const char *key, long key_length)
{
  CHANGED_TABLE_LIST *new_table =
      (CHANGED_TABLE_LIST *)allocate_memory(ALIGN_SIZE(sizeof(CHANGED_TABLE_LIST)) + key_length + 1);
  if (!new_table)
  {
    my_error(EE_OUTOFMEMORY, MYF(ME_FATALERROR), ALIGN_SIZE(sizeof(TABLE_LIST)) + key_length + 1);
    return 0;
  }
