                                    ...
                                    Opt_trace_context::feature_value feature)
{
  ...
  empty = true;
  if (likely(!stmt->open_struct(key, this, has_disabled_I_S, opening_bracket(requires_key))))
    started = true;
}
...
void Opt_trace_struct::do_destruct()
{
  ...
  assert(started);
  stmt->close_struct(saved_key, has_disabled_I_S, closing_bracket(requires_key));
  started = false;
}
...
*/
Opt_trace_struct &Opt_trace_struct::do_add(const char *key, const char *val, size_t val_length, bool escape)
{
  assert(started);
...
LEX_CSTRING bool_as_text[] = {{STRING_WITH_LEN("false")}, {STRING_WITH_LEN("true")}};
}  // namespace
...
Opt_trace_struct &Opt_trace_struct::do_add(const char *key, Item *item)
{
  ...
  if (item != NULL)
  {
    ...
    item->print(&str, enum_query_type(QT_TO_SYSTEM_CHARSET | QT_SHOW_SELECT_NUMBER | QT_NO_DEFAULT_DB));
    /* needs escaping */
...
Opt_trace_struct &Opt_trace_struct::do_add_utf8_table(const TABLE_LIST *tl)
{
  if (tl != NULL)
  {
    ...
    tl->print(
        current_thd, &str,
        enum_query_type(QT_TO_SYSTEM_CHARSET | QT_SHOW_SELECT_NUMBER | QT_NO_DEFAULT_DB | QT_DERIVED_TABLE_ONLY_ALIAS));
...
void Opt_trace_stmt::set_query(const char *query, size_t length, const CHARSET_INFO *charset)
{
  ...
  size_t available = (trace_buffer.alloced_length() >= trace_buffer.get_allowed_mem_size())
                         ? 0
                         : (trace_buffer.get_allowed_mem_size() - trace_buffer.alloced_length());
  ...
  const size_t new_allowed_mem_size = (query_buffer.alloced_length() >= trace_buffer.get_allowed_mem_size())
                                          ? 0
                                          : (trace_buffer.get_allowed_mem_size() - query_buffer.alloced_length());
  trace_buffer.set_allowed_mem_size(new_allowed_mem_size);
}

bool Opt_trace_stmt::open_struct(const char *key, Opt_trace_struct *ots, bool wants_disable_I_S, char opening_bracket)
{
  if (support_I_S())
  {
  ...
  {
    DBUG_EXECUTE_IF("opt_trace_oom_in_open_struct", DBUG_SET("+d,simulate_out_of_memory"););
...
void Opt_trace_stmt::close_struct(const char *saved_key, bool has_disabled_I_S, char closing_bracket)
{
  /*
...
{
  my_snprintf(unknown_key, sizeof(unknown_key), "unknown_key_%u", ++unknown_key_count);
  return unknown_key;
}

void Opt_trace_stmt::add(const char *key, const char *val, size_t val_length, bool quotes, bool escape)
{
  if (!support_I_S())
    return;
  separator();
...
void Opt_trace_stmt::fill_info(Opt_trace_info *info) const
{
  ...
  else
  {
    ...
    info->missing_bytes = trace_buffer.get_missing_bytes() + query_buffer.get_missing_bytes();
  }
}
...
void Buffer::append(const char *str, size_t length)
{
  ...
  DBUG_EXECUTE_IF("opt_trace_oom_in_buffers", DBUG_SET("+d,simulate_out_of_memory"););
...
const char *Opt_trace_context::flag_names[] = {"enabled", "one_line", "default", NullS};

const char *Opt_trace_context::feature_names[] = {"greedy_search",      "range_optimizer", "dynamic_range",
                                                  "repeated_subselect", "default",         NullS};

const Opt_trace_context::feature_value Opt_trace_context::default_features =
    Opt_trace_context::feature_value(Opt_trace_context::GREEDY_SEARCH | Opt_trace_context::RANGE_OPTIMIZER |
                                     Opt_trace_context::DYNAMIC_RANGE | Opt_trace_context::REPEATED_SUBSELECT);
...
bool Opt_trace_context::start(bool support_I_S_arg, bool support_dbug_or_missing_priv, bool end_marker_arg,
                              bool one_line_arg, long offset_arg, long limit_arg, ulong max_mem_size_arg,
                              ulonglong features_arg)
{
  ...
  */
  ...
  if (pimpl == NULL && ((pimpl = new_nothrow_w_my_error<Opt_trace_context_impl>()) == NULL))
    DBUG_RETURN(true);
  ...
  */
  if (I_S_disabled == 0)
  {
    ...
    */
    ...
    // MISC always on
    pimpl->features = Opt_trace_context::feature_value(features_arg | Opt_trace_context::MISC);
  }
  if (support_I_S_arg && pimpl->offset >= 0)
  {
    ...
    {
      DBUG_PRINT("opt", ("disabled: since_offset_0(%ld) < offset(%ld)", pimpl->since_offset_0, pimpl->offset));
      support_I_S_arg = false;
    }
  ...
  }
  {
    ...
    */
    ...
    if (unlikely(stmt == NULL || pimpl->stack_of_current_stmts.push_back(pimpl->current_stmt_in_gen)))
...
{
  return (pimpl != NULL) && (pimpl->current_stmt_in_gen != NULL) && pimpl->current_stmt_in_gen->support_I_S();
}
...
void Opt_trace_context::purge_stmts(bool purge_all)
{
  ...
  long idx;
  compile_time_assert(static_cast<long>(static_cast<size_t>(LONG_MAX)) == LONG_MAX);
  /*
  ...
  */
  for (idx = (pimpl->all_stmts_for_I_S.size() - 1); idx >= 0; idx--)
  {
    ...
    if (!purge_all && ((static_cast<long>(pimpl->all_stmts_for_I_S.size()) + pimpl->offset) <= idx))
    {
    ...
    else
    {
      ...
      */
      DBUG_EXECUTE_IF("opt_trace_oom_in_purge", DBUG_SET("+d,simulate_out_of_memory"););
      if (likely(!pimpl->all_stmts_to_del.push_back(pimpl->all_stmts_for_I_S.at(idx))))
...
size_t Opt_trace_context::allowed_mem_size_for_current_stmt() const
{
  ...
  size_t rc = (mem_size <= pimpl->max_mem_size) ? (pimpl->max_mem_size - mem_size) : 0;
  DBUG_PRINT("opt", ("rc %llu max_mem_size %llu", (ulonglong)rc, (ulonglong)pimpl->max_mem_size));
  return rc;
}
