 ...
 protected:
      ...
      switch (peek_class())
      {
        ...
        case HINT_CHR_ASTERISK:
          if (peek_class2() == HINT_CHR_SLASH)
          {
            ...
            return get_byte();
        case HINT_CHR_AT:
          if (prev_token == '(' || (prev_token == HINT_ARG_IDENT && whitespaces == 0))
      ...
      switch (chr_class)
      {
        ...
        case Quote:
          ...
          else
          {
            ...
            {
              if (thd->convert_string(&s, system_charset_info, yytext, yyleng, thd->charset()))
  ...
  int scan_query_block_name()
  {
    ...
    switch (peek_class())
    {
      ...
      case HINT_CHR_BACKQUOTE:
        return scan_quoted_ident<HINT_CHR_BACKQUOTE>() == HINT_ARG_IDENT ? HINT_ARG_QB_NAME : HINT_ERROR;
      case HINT_CHR_DOUBLEQUOTE:
        return scan_quoted_ident<HINT_CHR_DOUBLEQUOTE>() == HINT_ARG_IDENT ? HINT_ARG_QB_NAME : HINT_ERROR;
      default:
        return HINT_ERROR;
    }
  }
  ...
  int scan_ident_or_keyword()
  {
    for (;;)
    {
      switch (peek_class())
      {
        ...
        case HINT_CHR_EOF:
        default:
          const SYMBOL *symbol = Lex_hash::hint_keywords.get_hash_symbol(yytext, yyleng);
          if (symbol)  // keyword
          {
            /*
  ...
  hint_lex_char_classes peek_class2() const
  {
    ...
    return ptr + 1 >= input_buf_end ? HINT_CHR_EOF : char_classes[static_cast<uchar>(ptr[1])];
  }
