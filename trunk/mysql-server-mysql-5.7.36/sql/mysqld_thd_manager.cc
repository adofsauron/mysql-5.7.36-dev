...
static PSI_mutex_info all_thd_manager_mutexes[] = {{&key_LOCK_thd_list, "LOCK_thd_list", PSI_FLAG_GLOBAL},
...
static PSI_cond_info all_thd_manager_conds[] = {{&key_COND_thd_list, "COND_thd_list", PSI_FLAG_GLOBAL}};
      ...
      unit_test(false)
{
  ...
  mysql_mutex_init(key_LOCK_thd_list, &LOCK_thd_list, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_LOCK_thd_remove, &LOCK_thd_remove, MY_MUTEX_INIT_FAST);
...
void Global_THD_manager::add_thd(THD *thd)
{
  ...
  std::pair<THD_array::iterator, bool> insert_result = thd_list.insert_unique(thd);
...
void Global_THD_manager::release_thread_id(my_thread_id thread_id)
{
  ...
  Mutex_lock lock(&LOCK_thread_ids);
  const size_t num_erased MY_ATTRIBUTE((unused)) = thread_ids.erase_unique(thread_id);
...
THD *Global_THD_manager::find_thd(Find_THD_Impl *func)
{
  ...
  mysql_mutex_lock(&LOCK_thd_list);
  THD_array::const_iterator it = std::find_if(thd_list.begin(), thd_list.end(), find_thd);
  THD *ret = NULL;
  if (it != thd_list.end())
    ret = *it;
  mysql_mutex_unlock(&LOCK_thd_list);
