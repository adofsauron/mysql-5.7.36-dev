...
static inline void do_retry_sleep(unsigned milli_sleep)
{
...
*/
static NdbTransaction *gsl_lock_ext(THD *thd, Ndb *ndb, NdbError &ndb_error, int retry_time = 10)
{
...
static bool gsl_unlock_ext(Ndb *ndb, NdbTransaction *trans, NdbError &ndb_error)
{
...
static int ndbcluster_global_schema_lock(THD *thd, bool no_lock_queue, bool report_cluster_disconnected)
{
  if (!gsl_initialized)
    return 0;
  ...
  DBUG_PRINT("enter", ("query: '%-.4096s', no_lock_queue: %d", thd_query_unsafe(thd).str, no_lock_queue));
  if (thd_ndb->global_schema_lock_count)
  {
    ...
    DBUG_PRINT("exit", ("global_schema_lock_count: %d", thd_ndb->global_schema_lock_count));
    DBUG_RETURN(0);
  }
  ...
  */
  ...
  {
    sql_print_warning("NDB: Could not acquire global schema lock (%d)%s", ndb_error.code, ndb_error.message);
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER_DEFAULT(ER_GET_ERRMSG), ndb_error.code,
                        ndb_error.message, "NDB. Could not acquire global schema lock");
  }
...
static int ndbcluster_global_schema_unlock(THD *thd)
{
  ...
  */
  ...
  if (trans)
  {
    ...
    {
      sql_print_warning("NDB: Releasing global schema lock (%d)%s", ndb_error.code, ndb_error.message);
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER_DEFAULT(ER_GET_ERRMSG), ndb_error.code,
                          ndb_error.message, "ndb. Releasing global schema lock");
      DBUG_RETURN(-1);
    }
    if (opt_ndb_extra_logging > 19)
    {
...
static int ndbcluster_global_schema_func(THD *thd, bool lock, void *args)
{
  if (lock)
  {
...
int Ndb_global_schema_lock_guard::lock(bool no_lock_queue, bool report_cluster_disconnected)
{
  ...
  */
  ...
  return ndbcluster_global_schema_lock(m_thd, no_lock_queue, report_cluster_disconnected);
}
