...
void Mutex_cond_array::enter_cond(THD *thd, int n, PSI_stage_info *stage, PSI_stage_info *old_stage) const
{
  ...
  if (n > max_index)
  {
    for (int i = max_index + 1; i <= n; i++)
    {
      Mutex_cond *mutex_cond =
          static_cast<Mutex_cond *>(my_malloc(key_memory_Mutex_cond_array_Mutex_cond, sizeof(Mutex_cond), MYF(MY_WME)));
      if (mutex_cond == NULL)
        goto error;
