...
@verbatim
  ...
  */
  Query_result_insert(TABLE_LIST *table_list_par, TABLE *table_par, List<Item> *target_columns,
                      List<Item> *target_or_source_columns, List<Item> *update_fields, List<Item> *update_values,
        ...
        insert_into_view(table_list_par && table_list_par->is_view())
  {
    ...
    assert(target_columns == target_or_source_columns || target_columns == NULL);
  }
...
class Query_result_create : public Query_result_insert
{
 ...
 public:
  Query_result_create(TABLE_LIST *table_arg, HA_CREATE_INFO *create_info_par, Alter_info *alter_info_arg,
...
class Sql_cmd_insert_base : public Sql_cmd_dml
{
  ...
  */
  ...
  explicit Sql_cmd_insert_base(bool is_replace_arg, enum_duplicates duplicates_arg)
      : empty_field_list_on_rset(false), is_replace(is_replace_arg), duplicates(duplicates_arg)
  {
  }
 ...
 protected:
  bool mysql_prepare_insert(THD *thd, TABLE_LIST *table_list, List_item *values, bool select_insert);
  bool insert_precheck(THD *thd, TABLE_LIST *tables);
  bool mysql_prepare_insert_check_table(THD *thd, TABLE_LIST *table_list, List<Item> &fields, bool select_insert);
};

class Sql_cmd_insert : public Sql_cmd_insert_base
{
 public:
  explicit Sql_cmd_insert(bool is_replace_arg, enum_duplicates duplicates_arg)
      : Sql_cmd_insert_base(is_replace_arg, duplicates_arg)
  {
  }

  virtual enum_sql_command sql_command_code() const { return is_replace ? SQLCOM_REPLACE : SQLCOM_INSERT; }
...
{
 public:
  explicit Sql_cmd_insert_select(bool is_replace_arg, enum_duplicates duplicates_arg)
      : Sql_cmd_insert_base(is_replace_arg, duplicates_arg)
  {
  }
