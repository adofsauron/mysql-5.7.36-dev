...
st_row_event_key_info::st_row_event_key_info(const NdbDictionary::Table *_table, const uchar *_key_buff,
                                             Uint32 _key_buff_len, Uint64 _transaction_id)
    : tableObj(_table),
...
bool st_row_event_key_info::equal(const st_row_event_key_info *other) const
{
  ...
  return ((tableObj == other->tableObj) && (packed_key_len == other->packed_key_len) &&
...
st_trans_dependency::st_trans_dependency(st_transaction *_target_transaction, st_transaction *_dependent_transaction,
 ...
 * buffer.
 */
static int pack_key_to_buffer(const NdbDictionary::Table *table, const NdbRecord *key_rec, const uchar *record,
...
static Uint32 determine_packed_key_size(const NdbDictionary::Table *table, const NdbRecord *key_rec,
                                        const uchar *record)
{
...
{
  st_mem_root_allocator *a = (st_mem_root_allocator *)ctx;
  return alloc_root(a->mem_root, nelem * bytes);
}

void st_mem_root_allocator::mem_free(void *ctx, void *mem){
    /* Do nothing, will be globally freed when arena (mem_root)
     * released
     */
};

st_mem_root_allocator::st_mem_root_allocator(MEM_ROOT *_mem_root) : mem_root(_mem_root){};
...
DependencyTracker *DependencyTracker::newDependencyTracker(MEM_ROOT *mem_root)
{
  ...
  void *mem = alloc_root(mem_root, sizeof(DependencyTracker));
  if (mem)
  {
...
int DependencyTracker::track_operation(const NdbDictionary::Table *table, const NdbRecord *key_rec, const uchar *row,
                                       Uint64 transaction_id)
{
  ...
  uchar *packed_key_buff = (uchar *)alloc_root(mra.mem_root, required_buff_size);
  void *element_mem = alloc_root(mra.mem_root, sizeof(st_row_event_key_info));

  if (pack_key_to_buffer(table, key_rec, row, packed_key_buff, required_buff_size))
  {
    if (!error_text)
  ...
  st_row_event_key_info *key_info =
      new (element_mem) st_row_event_key_info(table, packed_key_buff, required_buff_size, transaction_id);
...
int DependencyTracker::mark_conflict(Uint64 trans_id)
{
  ...
  {
    DBUG_PRINT("info",
               ("Visiting transaction %llu, conflict : %u", dependent->getTransactionId(), dependent->getInConflict()));
    /*
...
st_transaction *DependencyTracker::get_or_create_transaction(Uint64 trans_id)
{
  ...
  if (!(transEntry = trans_hash.get(&transKey)))
  {
    /*
       ...
       and add to the hash
    */
    DBUG_PRINT("info", ("Creating new hash entry for transaction (%llu)", trans_id));

    transEntry = (st_transaction *)st_mem_root_allocator::alloc(&mra, sizeof(st_transaction));
...
int DependencyTracker::add_dependency(Uint64 trans_id, Uint64 dependent_trans_id)
{
  DBUG_ENTER("add_dependency");
  DBUG_PRINT("info", ("Recording dependency of %llu on %llu", dependent_trans_id, trans_id));
  ...
  if (!(dep = dependency_hash.get(&depKey)))
  {
    DBUG_PRINT("info", ("Creating new dependency hash entry for "
                        ...
                        dependentEntry->getTransactionId(), targetEntry->getTransactionId()));

    dep = (st_trans_dependency *)st_mem_root_allocator::alloc(&mra, sizeof(st_trans_dependency));

    new (dep) st_trans_dependency(targetEntry, dependentEntry, targetEntry->getDependencyListHead());
  ...
  */
  if (include_dependents_of_current)
  {
    ...
    while (dependency)
    {
      ...
      DBUG_PRINT("info", ("Adding dependency %llu->%llu", dependency->getDependentTransaction()->getTransactionId(),
                          dependency->getTargetTransaction()->getTransactionId()));

      Uint64 dependentTransactionId = dependency->getDependentTransaction()->getTransactionId();
...
void DependencyTracker::dump_dependents(Uint64 trans_id)
{
  ...
  if ((dependent = trans_hash.get(&key)))
  {
    ...
    {
      {
        fprintf(stderr, "%s%llu%s", sep, dependent->getTransactionId(), (dependent->getInConflict() ? "-C" : ""));
        sep = comma;
      }
