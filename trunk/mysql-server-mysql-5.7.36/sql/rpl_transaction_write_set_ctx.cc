...
bool Rpl_transaction_write_set_ctx::add_write_set(uint64 hash)
{
  ...
  if (!m_local_has_reached_write_set_limit)
  {
    ulong binlog_trx_dependency_history_size =
        mysql_bin_log.m_dependency_tracker.get_writeset()->get_opt_max_history_size();
    bool is_full_writeset_required = m_global_component_requires_write_sets && !m_local_allow_drop_write_set;
...
void Rpl_transaction_write_set_ctx::update_global_write_set_memory_size_limit(int64 limit)
{
  m_global_write_set_memory_size_limit = limit;
}
...
{
  Rpl_transaction_write_set_ctx::set_global_write_set_memory_size_limit(size_limit);
}

void update_write_set_memory_size_limit(long long size_limit)
{
  Rpl_transaction_write_set_ctx::update_global_write_set_memory_size_limit(size_limit);
}
...
Transaction_write_set *get_transaction_write_set(unsigned long m_thread_id)
{
  ...
  if (thd)
  {
    std::set<uint64> *write_set = thd->get_transaction()->get_transaction_write_set_ctx()->get_write_set();
    ...
    result_set =
        (Transaction_write_set *)my_malloc(key_memory_write_set_extraction, sizeof(Transaction_write_set), MYF(0));
    result_set->write_set_size = write_set_size;
    result_set->write_set = (unsigned long long *)my_malloc(key_memory_write_set_extraction,
                                                            write_set_size * sizeof(unsigned long long), MYF(0));
    int result_set_index = 0;
    for (std::set<uint64>::iterator it = write_set->begin(); it != write_set->end(); ++it)
    {
      uint64 temp = *it;
...
void Rpl_transaction_write_set_ctx::add_savepoint(char *name)
{
  ...
  DBUG_EXECUTE_IF("transaction_write_set_savepoint_level", assert(savepoint.size() == 0););
  ...
  */
  ...
  savepoint.insert(std::pair<std::string, size_t>(identifier, write_set.size()));
