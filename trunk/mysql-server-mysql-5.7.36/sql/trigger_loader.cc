...
static File_option trn_file_parameters[] = {
    {{C_STRING_WITH_LEN("trigger_table")}, offsetof(struct Trn_file_data, trigger_table), FILE_OPTIONS_ESTRING},
...
struct Trg_file_data
{
  ...
  {
    return definitions.push_back(t->get_definition_ptr(), m) || sql_modes.push_back(t->get_sql_mode_ptr(), m) ||
...
static File_option trg_file_parameters[] = {
    {{C_STRING_WITH_LEN("triggers")}, my_offsetof(struct Trg_file_data, definitions), FILE_OPTIONS_STRLIST},
    {{C_STRING_WITH_LEN("sql_modes")}, my_offsetof(struct Trg_file_data, sql_modes), FILE_OPTIONS_ULLLIST},
    {{C_STRING_WITH_LEN("definers")}, my_offsetof(struct Trg_file_data, definers_list), FILE_OPTIONS_STRLIST},
    {{C_STRING_WITH_LEN("client_cs_names")}, my_offsetof(struct Trg_file_data, client_cs_names), FILE_OPTIONS_STRLIST},
    {{C_STRING_WITH_LEN("connection_cl_names")},
     my_offsetof(struct Trg_file_data, connection_cl_names),
     FILE_OPTIONS_STRLIST},
    {{C_STRING_WITH_LEN("db_cl_names")}, my_offsetof(struct Trg_file_data, db_cl_names), FILE_OPTIONS_STRLIST},
    {{C_STRING_WITH_LEN("created")}, my_offsetof(struct Trg_file_data, created_timestamps), FILE_OPTIONS_ULLLIST},
    {{0, 0}, 0, FILE_OPTIONS_STRING}};
...
LEX_STRING Trigger_loader::build_trn_path(char *trn_file_name_buffer, int trn_file_name_buffer_size,
                                          const char *db_name, const char *trigger_name)
{
  ...
  trn_file_name.length = build_table_filename(trn_file_name_buffer, trn_file_name_buffer_size - 1, db_name,
                                              trigger_name, TRN_EXT, 0, &was_truncated);

  if (!was_truncated)
    return trn_file_name;

  my_error(ER_IDENT_CAUSES_TOO_LONG_PATH, MYF(0), sizeof(trn_file_name_buffer) - 1, trn_file_name_buffer);
...
static bool save_trg_file(const char *db_name, const char *table_name, const Trg_file_data *trg)
{
  char trg_file_name_buffer[FN_REFLEN];
  LEX_STRING trg_file_name;
  bool was_truncated = false;

  trg_file_name.length =
      build_table_filename(trg_file_name_buffer, FN_REFLEN - 1, db_name, table_name, TRG_EXT, 0, &was_truncated);

  if (was_truncated)
  {
    my_error(ER_IDENT_CAUSES_TOO_LONG_PATH, MYF(0), sizeof(trg_file_name_buffer) - 1, trg_file_name_buffer);
    return true;
  }

  trg_file_name.str = trg_file_name_buffer;
  return sql_create_definition_file(NULL, &trg_file_name, &trg_file_type, (uchar *)trg, trg_file_parameters);
}
...
static bool fill_trg_data(Trg_file_data *trg, MEM_ROOT *mem_root, List<Trigger> *triggers)
{
...
static Trigger *change_table_name_in_trn_files(List<Trigger> *triggers, const char *db_name, const char *new_db_name,
                                               const LEX_STRING *new_table_name, const Trigger *stopper)
{
  ...
  while ((t = it++))
  {
    ...
    LEX_STRING trn_file_name =
        Trigger_loader::build_trn_path(trn_file_name_buffer, FN_REFLEN, new_db_name, t->get_trigger_name().str);
    ...
    if (sql_create_definition_file(NULL, &trn_file_name, &trn_file_type, (uchar *)&trn, trn_file_parameters))
    {
      return t;
    }
...
class Handle_old_incorrect_sql_modes_hook : public Unknown_key_hook
{
 ...
 public:
  Handle_old_incorrect_sql_modes_hook(char *file_path) : path(file_path){};
                                                                 ...
                                                                 MEM_ROOT *mem_root, const char *end)
{
      ...
      !memcmp(unknown_key, STRING_WITH_LEN("sql_modes")))
  {
    ...
    push_warning_printf(current_thd, Sql_condition::SL_NOTE, ER_OLD_FILE_FORMAT, ER(ER_OLD_FILE_FORMAT), path,
                        "TRIGGER");
...
*/
bool Trigger_loader::trg_file_exists(const char *db_name, const char *table_name)
{
  char path[FN_REFLEN];
  build_table_filename(path, FN_REFLEN - 1, db_name, table_name, TRG_EXT, 0);
...
bool Trigger_loader::load_triggers(THD *thd, MEM_ROOT *mem_root, const char *db_name, const char *table_name,
                                   List<Trigger> *triggers)
{
  ...
  trg_file_path.length = build_table_filename(trg_file_path_buffer, FN_REFLEN - 1, db_name, table_name, TRG_EXT, 0);
  ...
  if (parser->parse((uchar *)&trg, mem_root, trg_file_parameters, TRG_NUM_REQUIRED_PARAMETERS, &sql_modes_hook))
    DBUG_RETURN(true);
  ...
  if (trg.client_cs_names.is_empty() || trg.connection_cl_names.is_empty() || trg.db_cl_names.is_empty())
  {
    if (!trg.client_cs_names.is_empty() || !trg.connection_cl_names.is_empty() || !trg.db_cl_names.is_empty())
    {
    ...
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TRG_NO_CREATION_CTX, ER(ER_TRG_NO_CREATION_CTX), db_name,
                        table_name);
    ...
    */
    ...
    lex_string_set(&default_client_cs_name, thd->variables.character_set_client->csname);

    lex_string_set(&default_connection_cl_name, thd->variables.collation_connection->name);

    lex_string_set(&default_db_cl_name, thd->variables.collation_database->name);
  }
  ...
  while (true)
  {
    ...
    Trigger *t = Trigger::create_from_dd(mem_root, db_name_str, table_name_str, *definition, *sql_mode, *definer,
                                         *client_cs_name, *connection_cl_name, *db_cl_name, created_timestamp);
...
bool Trigger_loader::store_trigger(const LEX_STRING &db_name, const LEX_STRING &table_name, MEM_ROOT *mem_root,
                                   Trigger *new_trigger, List<Trigger> *triggers)
{
  ...
  LEX_STRING trn_file_name =
      Trigger_loader::build_trn_path(trn_file_name_buffer, FN_REFLEN, db_name.str, new_trigger->get_trigger_name().str);
...
bool Trigger_loader::drop_trigger(const LEX_STRING &db_name, const LEX_STRING &table_name,
                                  const LEX_STRING &trigger_name, MEM_ROOT *mem_root, List<Trigger> *triggers,
                                  bool *trigger_found)
{
  ...
  *trigger_found = false;

  {
    ...
    while ((t = it++))
    {
      if (my_strcasecmp(table_alias_charset, t->get_trigger_name().str, trigger_name.str) == 0)
      {
        delete t;
  ...
  return triggers->is_empty() ? rm_trg_file(db_name.str, table_name.str)
...
bool Trigger_loader::load_trn_file(THD *thd, const LEX_STRING &trigger_name, const LEX_STRING &trn_path,
                                   LEX_STRING *tbl_name)
{
  ...
  {
    my_error(ER_WRONG_OBJECT, MYF(0), trigger_name.str, TRN_EXT + 1, "TRIGGERNAME");
  ...
  Handle_old_incorrect_trigger_table_hook trigger_table_hook(trn_path.str, &trn.trigger_table);

  if (parser->parse((uchar *)&trn, thd->mem_root, trn_file_parameters, 1, &trigger_table_hook))
    DBUG_RETURN(true);
...
bool Trigger_loader::drop_all_triggers(const char *db_name, const char *table_name, List<Trigger> *triggers)
{
  bool rc = false;
  ...
  while ((t = it++))
  {
    ...
    MYSQL_DROP_SP(SP_TYPE_TRIGGER, db_name.str, db_name.length, trigger_name.str, trigger_name.length);
#endif
  }
...
bool Trigger_loader::rename_subject_table(MEM_ROOT *mem_root, List<Trigger> *triggers, const char *db_name,
                                          LEX_STRING *table_name, const char *new_db_name, LEX_STRING *new_table_name,
                                          bool upgrading50to51)
{
  ...
  Trigger *err_trigger =
      change_table_name_in_trn_files(triggers, upgrading50to51 ? db_name : NULL, new_db_name, new_table_name, NULL);
  ...
  if (err_trigger)
  {
    ...
    */
    change_table_name_in_trn_files(triggers, upgrading50to51 ? new_db_name : NULL, db_name, table_name, err_trigger);
    return true;
  }
