...
static int prepare_for_repair(THD *thd, TABLE_LIST *table_list, HA_CHECK_OPT *check_opt)
{
  ...
  if (!(table = table_list->table))
  {
    ...
    */
    ...
    share = get_table_share(thd, table_list, key, key_length, 0, &error, hash_value);
  ...
  */
  ...
  {
    error = send_check_errmsg(thd, table_list, "repair", "Cannot repair temporary table from .frm file");
    goto end;
  }
  ...
  */
  ...
  {
    error = send_check_errmsg(thd, table_list, "repair", "Failed repairing incompatible .frm file");
    goto end;
  }
  ...
  */
  ...
  my_snprintf(tmp, sizeof(tmp), "%s-%lx_%x", from, current_pid, thd->thread_id());
  ...
  */
  ...
  {
    error = send_check_errmsg(thd, table_list, "repair", "Failed to open partially repaired table");
    goto end;
  }
...
{
  return (sql_errno == ER_NO_SUCH_TABLE || sql_errno == ER_FILE_NOT_FOUND || sql_errno == ER_LOCK_WAIT_TIMEOUT ||
          sql_errno == ER_LOCK_DEADLOCK || sql_errno == ER_CANT_LOCK_LOG_TABLE || sql_errno == ER_OPEN_AS_READONLY ||
...
static bool mysql_admin_table(THD *thd, TABLE_LIST *tables, HA_CHECK_OPT *check_opt, const char *operator_name,
                              thr_lock_type lock_type, bool open_for_modify, bool repair_table_use_frm,
                              ...
                              int(view_operator_func)(THD *, TABLE_LIST *))
{
  ...
  bool gtid_rollback_must_be_skipped =
      ((thd->variables.gtid_next.type == GTID_GROUP || thd->variables.gtid_next.type == ANONYMOUS_GROUP) &&
  ...
  item->maybe_null = 1;
  field_list.push_back(item = new Item_empty_string("Msg_text", SQL_ADMIN_MSG_TEXT_SIZE));
  item->maybe_null = 1;
  ...
  */
  ...
  for (table = tables; table; table = table->next_local)
  {
    ...
    */
    table->mdl_request.set_type((lock_type >= TL_WRITE_ALLOW_WRITE) ? MDL_SHARED_NO_READ_WRITE : MDL_SHARED_READ);
    /* open only one table from local list of command */
    {
      ...
      */
      ...
      /*
        ...
        allowed.
      */
      if (lex->alter_info.flags & Alter_info::ALTER_ADMIN_PARTITION || view_operator_func == NULL)
      ...
      if (!thd->locked_tables_mode && repair_table_use_frm)
      {
        ...
        */
        ...
        if (tmp_da.is_error())
        {
          ...
          thd->get_stmt_da()->set_error_status(tmp_da.mysql_errno(), tmp_da.message_text(), tmp_da.returned_sqlstate());
        }
      }
      else
      {
        ...
        */
        ...
        issue_deprecation_warning = (!open_error && !table->is_view() && table->table->s->db_type() &&
      ...
      if (table->table)
      {
        ...
        */
        ...
        if (alter_info->flags & Alter_info::ALTER_ADMIN_PARTITION)
        {
          ...
          if (set_part_state(alter_info, table->table->part_info, PART_ADMIN, true))
          {
    ...
    */
    if (!table->table)
    {
      ...
      if (thd->get_stmt_da()->cond_count() == 0)
        push_warning(thd, Sql_condition::SL_WARNING, ER_CHECK_NO_SUCH_TABLE, ER(ER_CHECK_NO_SUCH_TABLE));
      /* if it was a view will check md5 sum */
      if (table->is_view() && view_checksum(thd, table) == HA_ADMIN_WRONG_CHECKSUM)
        push_warning(thd, Sql_condition::SL_WARNING, ER_VIEW_CHECKSUM, ER(ER_VIEW_CHECKSUM));
      if (thd->get_stmt_da()->is_error() && table_not_corrupt_error(thd->get_stmt_da()->mysql_errno()))
        result_code = HA_ADMIN_FAILED;
      else
    ...
    */
    ...
    {
      if (wait_while_table_is_used(thd, table->table, HA_EXTRA_PREPARE_FOR_RENAME))
        goto err;
    ...
    if (operator_func == &handler::ha_repair && !(check_opt->sql_flags & TT_USEFRM))
    {
      if ((table->table->file->check_old_types() == HA_ADMIN_NEEDS_ALTER) ||
          (table->table->file->ha_check_for_upgrade(check_opt) == HA_ADMIN_NEEDS_ALTER))
      {
    ...
    */
    if (operator_func == &handler::ha_check && (check_opt->sql_flags & TT_FOR_UPGRADE) && table->table->triggers)
    {
  ...
  send_result:
    ...
    thd->clear_error();  // these errors shouldn't get client
    {
      ...
      while ((err = it++))
      {
        ...
        protocol->store(table_name, system_charset_info);
        protocol->store((char *)operator_name, system_charset_info);
        protocol->store(warning_level_names[err->severity()].str, warning_level_names[err->severity()].length,
                        system_charset_info);
  ...
  send_result_message:
    ...
    switch (result_code)
    {
      case HA_ADMIN_NOT_IMPLEMENTED:
      {
        ...
        size_t length = my_snprintf(buf, sizeof(buf), ER(ER_CHECK_NOT_IMPLEMENTED), operator_name);
      ...
      case HA_ADMIN_NOT_BASE_TABLE:
      {
        ...
        size_t length = my_snprintf(buf, sizeof(buf), ER(ER_BAD_TABLE_ERROR), tbl_name.c_ptr());
        protocol->store(STRING_WITH_LEN("note"), system_charset_info);
        protocol->store(buf, length, system_charset_info);
      }
      break;
      ...
      case HA_ADMIN_TRY_ALTER:
      {
        ...
        */
        ...
        result_code = (open_temporary_tables(thd, table) || mysql_recreate_table(thd, table, false));
        reenable_binlog(thd);
        /*
          mysql_recreate_table() can push OK or ERROR.
          Clear 'OK' status. If there is an error, keep it:
        ...
        */
        ...
        if (!result_code)  // recreation went ok
        {
          ...
          if (!open_temporary_tables(thd, table) &&
              (table->table = open_n_lock_single_table(thd, table, TL_READ_NO_INSERT, 0)))
          {
            /*
      ...
      case HA_ADMIN_NEEDS_ALTER:
      {
        ...
        if (table->table->file->ha_table_flags() & HA_CAN_REPAIR)
          length = my_snprintf(buf, sizeof(buf), ER(ER_TABLE_NEEDS_UPGRADE), table->table_name);
        else
          length = my_snprintf(buf, sizeof(buf), ER(ER_TABLE_NEEDS_REBUILD), table->table_name);
        protocol->store(buf, length, system_charset_info);
        fatal_error = 1;
        break;
      }
      ...
      case HA_ADMIN_NEEDS_UPG_PART:
      {
        ...
        protocol->store(STRING_WITH_LEN("error"), system_charset_info);
        length = my_snprintf(buf, sizeof(buf), ER(ER_TABLE_NEEDS_UPG_PART), table->db, table->table_name);
        protocol->store(buf, length, system_charset_info);
        break;
      }
    ...
    */
    if (issue_deprecation_warning)
    {
      ...
      protocol->store(STRING_WITH_LEN("warning"), system_charset_info);
      length =
          my_snprintf(buf, sizeof(buf), ER(ER_PARTITION_ENGINE_DEPRECATED_FOR_TABLE), table->db, table->table_name);
      protocol->store(buf, length, system_charset_info);
      // Row is ended below.
    }
    ...
    if (table->table)
    {
      ...
      {
        tdc_remove_table(thd, TDC_RT_REMOVE_UNUSED, table->db, table->table_name, FALSE);
        /*
      ...
      else
      {
        /*
          ...
          CACHE INDEX/LOAD INDEX for specified partitions
        */
        if (table->table->part_info && lex->alter_info.flags & Alter_info::ALTER_ADMIN_PARTITION)
        {
...
bool mysql_preload_keys(THD *thd, TABLE_LIST *tables)
{
  ...
  */
  DBUG_RETURN(
      mysql_admin_table(thd, tables, 0, "preload_keys", TL_READ_NO_INSERT, 0, 0, 0, 0, &handler::preload_keys, 0));
}
...
bool Sql_cmd_analyze_table::execute(THD *thd)
{
  ...
  res = mysql_admin_table(thd, first_table, &thd->lex->check_opt, "analyze", lock_type, 1, 0, 0, 0,
...
bool Sql_cmd_check_table::execute(THD *thd)
{
  ...
  res = mysql_admin_table(thd, first_table, &thd->lex->check_opt, "check", lock_type, 0, 0, HA_OPEN_FOR_REPAIR, 0,
...
bool Sql_cmd_optimize_table::execute(THD *thd)
{
  ...
  res = (specialflag & SPECIAL_NO_NEW_FUNC) ? mysql_recreate_table(thd, first_table, true)
                                            : mysql_admin_table(thd, first_table, &thd->lex->check_opt, "optimize",
                                                                TL_WRITE, 1, 0, 0, 0, &handler::ha_optimize, 0);
  /* ! we write after unlocking the table */
  if (!res && !thd->lex->no_write_to_binlog)
  {
    /*
...
bool Sql_cmd_repair_table::execute(THD *thd)
{
  ...
  if (check_table_access(thd, SELECT_ACL | INSERT_ACL, first_table, FALSE, UINT_MAX, FALSE))
    goto error; /* purecov: inspected */
  thd->enable_slow_log = opt_log_slow_admin_statements;
  res = mysql_admin_table(thd, first_table, &thd->lex->check_opt, "repair", TL_WRITE, 1,
                          MY_TEST(thd->lex->check_opt.sql_flags & TT_USEFRM), HA_OPEN_FOR_REPAIR, &prepare_for_repair,
                          &handler::ha_repair, 0);
