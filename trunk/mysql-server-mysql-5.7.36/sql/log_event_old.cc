...
int Old_rows_log_event::do_apply_event(Old_rows_log_event *ev, const Relay_log_info *rli)
{
  ...
  /*
    ...
    success.
   */
  if ((ev->m_table_id.id() == ~0U || ev->m_table_id.id() == (~0ULL >> 16)) && ev->m_cols.n_bits == 1 &&
      ev->m_cols.bitmap[0] == 0)
    ...
    */
    ...
    if (open_and_lock_tables(ev_thd, rli->tables_to_lock, 0))
    {
      if (ev_thd->is_error())
      {
        ...
        */
        rli->report(ERROR_LEVEL, ev_thd->get_stmt_da()->mysql_errno(), "Error '%s' on opening tables",
...
static int copy_extra_record_fields(TABLE *table, size_t master_reclength, my_ptrdiff_t master_fields)
{
  ...
  DBUG_PRINT("info", ("Copying to 0x%lx "
                      ...
                      (long)table->record[0], (ulong)master_fields, (ulong)master_reclength, table->s->fields,
                      table->s->reclength));
  /*
  ...
  */
  {
    ...
    for (; *field_ptr; ++field_ptr)
    {
       ...
       */
      if ((*field_ptr)->maybe_null() && (*field_ptr)->is_null_in_record(table->record[1]))
        (*field_ptr)->set_null();
      else
...
static int find_and_fetch_row(TABLE *table, uchar *key)
{
  ...
  DBUG_PRINT("enter", ("table: 0x%lx, key: 0x%lx  record: 0x%lx", (long)table, (long)key, (long)table->record[1]));
  ...
  if (table->s->keys > 0)
  {
    ...
    */
    ...
    if ((error = table->file->ha_index_read_map(table->record[1], key, HA_WHOLE_KEY, HA_READ_KEY_EXACT)))
    {
      table->file->print_error(error, MYF(0));
    ...
    */
    ...
    while (record_compare(table))
    {
      ...
      */
      if (table->s->null_bytes > 0)
      {
        table->record[1][table->s->null_bytes - 1] |= 256U - (1U << table->s->last_null_bit_pos);
      }
...
int Write_rows_log_event_old::do_prepare_row(THD *thd_arg, Relay_log_info const *rli, TABLE *table,
                                             uchar const *row_start, uchar const **row_end)
{
  ...
  int error;
  error = unpack_row_old(const_cast<Relay_log_info *>(rli), table, m_width, table->record[0], row_start, &m_cols,
                         row_end, &m_master_reclength, table->write_set, binary_log::PRE_GA_WRITE_ROWS_EVENT);
...
int Delete_rows_log_event_old::do_prepare_row(THD *thd_arg, Relay_log_info const *rli, TABLE *table,
                                              uchar const *row_start, uchar const **row_end)
{
  int error;
  assert(row_start && row_end);
  ...
  */
  ...
  error = unpack_row_old(const_cast<Relay_log_info *>(rli), table, m_width, table->record[0], row_start, &m_cols,
                         row_end, &m_master_reclength, table->read_set, binary_log::PRE_GA_DELETE_ROWS_EVENT);
  /*
...
int Update_rows_log_event_old::do_prepare_row(THD *thd_arg, Relay_log_info const *rli, TABLE *table,
                                              uchar const *row_start, uchar const **row_end)
{
  int error;
  assert(row_start && row_end);
  ...
  */
  ...
  error = unpack_row_old(const_cast<Relay_log_info *>(rli), table, m_width, table->record[0], row_start, &m_cols,
                         row_end, &m_master_reclength, table->read_set, binary_log::PRE_GA_UPDATE_ROWS_EVENT);
  row_start = *row_end;
  /* m_after_image is the after image for the update */
  error = unpack_row_old(const_cast<Relay_log_info *>(rli), table, m_width, m_after_image, row_start, &m_cols, row_end,
                         &m_master_reclength, table->write_set, binary_log::PRE_GA_UPDATE_ROWS_EVENT);
...
*/
Old_rows_log_event::Old_rows_log_event(THD *thd_arg, TABLE *tbl_arg, ulong tid, MY_BITMAP const *cols, bool using_trans)
...
Old_rows_log_event::Old_rows_log_event(const char *buf, uint event_len, Log_event_type event_type,
                                       const Format_description_event *description_event)
    : Binary_log_event(&buf, description_event->binlog_version, description_event->server_version),
...
#endif
{
  ...
  DBUG_PRINT("info", ("m_table_id: %llu  m_flags: %d  m_width: %lu  data_size: %lu", m_table_id.id(), m_flags, m_width,
                      (ulong)data_size));
...
size_t Old_rows_log_event::get_data_size()
{
  ...
  DBUG_EXECUTE_IF("old_row_based_repl_4_byte_map_id_master",
                  return 6 + no_bytes_in_map(&m_cols) + (end - buf) + (m_rows_cur - m_rows_buf););
...
int Old_rows_log_event::do_add_row_data(uchar *row_data, size_t length)
{
  ...
  */
  ...
  if (static_cast<size_t>(m_rows_end - m_rows_cur) <= length)
  {
    ...
    uchar *const new_buf =
        (uchar *)my_realloc(key_memory_log_event_old, m_rows_buf, (uint)new_alloc, MYF(MY_ALLOW_ZERO_PTR | MY_WME));
    if (unlikely(!new_buf))
...
int Old_rows_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  /*
    ...
    success.
   */
  if ((m_table_id.id() == ~0U || m_table_id.id() == (~0ULL >> 16)) && m_cols.n_bits == 1 && m_cols.bitmap[0] == 0)
  {
    /*
       This one is supposed to be set: just an extra check so that
       nothing strange has happened.
  ...
  */
  if (!thd->lock)
  {
    ...
    */
    ...
    if ((error = lock_tables(thd, rli->tables_to_lock, rli->tables_to_lock_count, 0)))
    {
      if (thd->is_slave_error || thd->is_fatal_error)
      {
        ...
        */
        ...
        rli->report(
            ERROR_LEVEL, actual_error, "Error '%s' in %s event: when locking tables",
            (actual_error ? thd->get_protocol_classic()->get_last_error() : "unexpected success or fatal error"),
            get_type_str());
        thd->is_fatal_error = 1;
      }
      else
      {
        rli->report(ERROR_LEVEL, error, "Error in %s event: when locking tables", get_type_str());
      }
      const_cast<Relay_log_info *>(rli)->slave_close_thread_tables(thd);
      DBUG_RETURN(error);
    }
  ...
  TABLE *table = m_table = const_cast<Relay_log_info *>(rli)->m_table_map.get_table(m_table_id);
    ...
    */
    ...
    while (error == 0 && m_curr_row < m_rows_end)
    {
      ...
      switch (error)
      {
        ...
        default:
          rli->report(ERROR_LEVEL, thd->get_protocol_classic()->get_last_errno(),
                      "Error in %s event: row application failed. %s", get_type_str(),
      ...
      */
      ...
      DBUG_PRINT("info", ("error: %d", error));
      DBUG_PRINT("info", ("curr_row: 0x%lu; curr_row_end: 0x%lu; rows_end: 0x%lu", (ulong)m_curr_row,
                          (ulong)m_curr_row_end, (ulong)m_rows_end));
  ...
  */
  if (table && (table->s->primary_key == MAX_KEY) && !is_using_trans_cache() && get_flags(STMT_END_F) == RLE_NO_FLAGS)
  {
    /*
...
bool Old_rows_log_event::write_data_body(IO_CACHE *file)
{
  ...
  */
  ...
  DBUG_DUMP("m_cols", (uchar *)m_cols.bitmap, no_bytes_in_map(&m_cols));
  res = res || my_b_safe_write(file, (uchar *)m_cols.bitmap, no_bytes_in_map(&m_cols));
...
#ifdef MYSQL_CLIENT
void Old_rows_log_event::print_helper(FILE *file, PRINT_EVENT_INFO *print_event_info, char const *const name)
{
...
int Old_rows_log_event::write_row(const Relay_log_info *const rli, const bool overwrite)
{
  ...
  if ((error = prepare_record(table, table->write_set, TRUE /* check if columns have def. values */)))
    DBUG_RETURN(error);
 ...
 */
#ifdef HAVE_REPLICATION
Write_rows_log_event_old::Write_rows_log_event_old(const char *buf, uint event_len,
                                                   const Format_description_event *description_event)
    : Old_rows_log_event(buf, event_len, binary_log::PRE_GA_WRITE_ROWS_EVENT, description_event)
{
}
#endif
...
#ifdef MYSQL_CLIENT
void Write_rows_log_event_old::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
 ...
 */
#ifdef HAVE_REPLICATION
Delete_rows_log_event_old::Delete_rows_log_event_old(const char *buf, uint event_len,
                                                     const Format_description_event *description_event)
    : Old_rows_log_event(buf, event_len, binary_log::PRE_GA_DELETE_ROWS_EVENT, description_event),
      m_after_image(NULL),
      m_memory(NULL)
{
}
...
#ifdef MYSQL_CLIENT
void Delete_rows_log_event_old::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
 ...
 */
#ifdef HAVE_REPLICATION
Update_rows_log_event_old::Update_rows_log_event_old(const char *buf, uint event_len,
                                                     const Format_description_event *description_event)
    : Old_rows_log_event(buf, event_len, binary_log::PRE_GA_UPDATE_ROWS_EVENT, description_event),
      m_after_image(NULL),
      m_memory(NULL)
{
}
...
#ifdef MYSQL_CLIENT
void Update_rows_log_event_old::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
