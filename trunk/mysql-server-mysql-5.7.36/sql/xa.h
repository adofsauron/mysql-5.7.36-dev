...
{
 public:
  Sql_cmd_xa_start(xid_t *xid_arg, enum xa_option_words xa_option) : m_xid(xid_arg), m_xa_opt(xa_option) {}
...
{
 public:
  Sql_cmd_xa_end(xid_t *xid_arg, enum xa_option_words xa_option) : m_xid(xid_arg), m_xa_opt(xa_option) {}
...
class Sql_cmd_xa_prepare : public Sql_cmd
{
 public:
  ...
  virtual enum_sql_command sql_command_code() const { return SQLCOM_XA_PREPARE; }
...
{
 public:
  explicit Sql_cmd_xa_recover(bool print_xid_as_hex) : m_print_xid_as_hex(print_xid_as_hex) {}

  virtual enum_sql_command sql_command_code() const { return SQLCOM_XA_RECOVER; }
...
{
 public:
  Sql_cmd_xa_commit(xid_t *xid_arg, enum xa_option_words xa_option) : m_xid(xid_arg), m_xa_opt(xa_option) {}
...
class Sql_cmd_xa_rollback : public Sql_cmd
{
 public:
  ...
  virtual enum_sql_command sql_command_code() const { return SQLCOM_XA_ROLLBACK; }
...
typedef struct xid_t
{
 ...
 public:
  xid_t() : formatID(-1), gtrid_length(0), bqual_length(0) { memset(data, 0, XIDDATASIZE); }

  long get_format_id() const { return formatID; }

  void set_format_id(long v) { formatID = v; }
  ...
  {
    if (gtrid_length == static_cast<long>(MYSQL_XID_GTRID_LEN) && bqual_length == 0 &&
  ...
  const uchar *key() const { return reinterpret_cast<const uchar *>(&gtrid_length); }

  uint key_length() const { return sizeof(gtrid_length) + sizeof(bqual_length) + gtrid_length + bqual_length; }
  ...
  */
  ...
  char *serialize(char *buf) const { return serialize_xid(buf, formatID, gtrid_length, bqual_length, data); }
 ...
 private:
  void set(const xid_t *xid) { memcpy(this, xid, sizeof(xid->formatID) + xid->key_length()); }
...
class XID_STATE
{
 ...
 public:
  XID_STATE() : xa_state(XA_NOTR), in_recovery(false), rm_error(0), m_is_binlogged(false) { m_xid.null(); }
