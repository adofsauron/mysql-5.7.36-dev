...
Ndb_index_stat_thread::Ndb_index_stat_thread() : Ndb_component("Index Stat"), client_waiting(false)
{
...
static time_t ndb_index_stat_time()
{
  ...
  {
    DBUG_PRINT("index_stat", ("time moved backwards %d seconds", int(ndb_index_stat_time_now - now)));
...
static bool ndb_index_stat_allow(int flag = -1)
{
  if (flag != -1)
  ...
  *r++ = 0;
  ...
  for (uint i = 0; i < imax; i++)
  {
    ...
    switch (v.unit)
    {
      case Ndb_index_stat_opt::Ubool:
      {
        if ((s > r && *s == 0 && val == 0) || strcmp(r, "off") == 0 || strcmp(r, "false") == 0)
          val = 0;
        else if ((s > r && *s == 0 && val == 1) || strcmp(r, "on") == 0 || strcmp(r, "true") == 0)
          val = 1;
        else
          DBUG_RETURN(-1);
        v.val = (uint)val;
...
int ndb_index_stat_option_check(MYSQL_THD, struct st_mysql_sys_var *var, void *save, struct st_mysql_value *value)
{
...
void ndb_index_stat_option_update(MYSQL_THD, struct st_mysql_sys_var *var, void *var_ptr, const void *save)
{
...
void Ndb_index_stat_glob::set_status()
{
  ...
  sprintf(p, "allow:%d,enable:%d,busy:%d,loop:%u", th_allow, th_enable, th_busy, th_loop);
  p += strlen(p);
  ...
  p += strlen(p);
  sprintf(p, ",event:(act:%u,skip:%u,miss:%u)", event_act, event_skip, event_miss);
  p += strlen(p);
  sprintf(p, ",cache:(refresh:%u,clean:%u,pinned:%u,drop:%u,evict:%u)", refresh_count, clean_count, pinned_count,
          drop_count, evict_count);
  p += strlen(p);
  sprintf(p, ")");
  p += strlen(p);
  ...
  sprintf(p,
          ...
          cache_query_bytes, cache_clean_bytes, cache_drop_bytes, cache_evict_bytes, cache_pct, cache_high_pct);
  p += strlen(p);
...
*/
static void ndb_index_stat_error(Ndb_index_stat *st, int from, const char *place, int line)
{
  ...
  DBUG_PRINT("index_stat", ("%s line %d: error %d line %d extra %d", place, line, error.code, error.line, error.extra));
}
...
static void ndb_index_stat_ref_count(Ndb_index_stat *st, bool flag)
{
  ...
  DBUG_PRINT("index_stat", ("st %s ref_count:%u->%u", st->id, old_count, st->ref_count));
}
...
static Ndb_index_stat *ndb_index_stat_alloc(const NDBINDEX *index, const NDBTAB *table, int &err_out)
{
  err_out = 0;
...
/* Subroutine, have lock */
static Ndb_index_stat *ndb_index_stat_find_share(NDB_SHARE *share, const NDBINDEX *index, Ndb_index_stat *&st_last)
{
  ...
  while (st != 0)
  {
    ...
    st->is->get_head(head);
    if (head.m_indexId == (uint)index->getObjectId() && head.m_indexVersion == (uint)index->getObjectVersion())
      break;
    st_last = st;
    st = st->share_next;
  }
...
/* Subroutine, have lock */
static void ndb_index_stat_add_share(NDB_SHARE *share, Ndb_index_stat *st, Ndb_index_stat *st_last)
{
  st->share = share;
  if (st_last == 0)
...
static Ndb_index_stat *ndb_index_stat_get_share(NDB_SHARE *share, const NDBINDEX *index, const NDBTAB *table,
                                                Ndb_index_stat_snap &snap, int &err_out, bool allow_add,
                                                bool force_update)
{
  Ndb_index_stat_glob &glob = ndb_index_stat_glob;
...
{
  DBUG_ENTER("ndb_index_stat_free");
  DBUG_PRINT("index_stat", ("(index_id:%d index_version:%d", index_id, index_version));
  Ndb_index_stat_glob &glob = ndb_index_stat_glob;
  native_mutex_lock(&ndb_index_stat_thread.stat_mutex);
...
static Ndb_index_stat *ndb_index_stat_find_entry(int index_id, int index_version, int table_id)
{
  DBUG_ENTER("ndb_index_stat_find_entry");
  native_mutex_lock(&ndbcluster_mutex);
  native_mutex_lock(&ndb_index_stat_thread.stat_mutex);
  DBUG_PRINT("index_stat", ("find index:%d version:%d table:%d", index_id, index_version, table_id));
...
static void ndb_index_stat_cache_move(Ndb_index_stat *st)
{
  ...
  DBUG_PRINT("index_stat", ("st %s cache move: query:%u clean:%u", st->id, new_query_bytes, old_query_bytes));
  st->is->move_cache();
...
static bool ndb_index_stat_cache_clean(Ndb_index_stat *st)
{
  ...
  DBUG_PRINT("index_stat", ("st %s cache clean: clean:%u ref_count:%u", st->id, old_clean_bytes, ref_count));
  if (ref_count != 0)
    return false;
  st->is->clean_cache();
  st->clean_bytes = 0;
...
static void ndb_index_stat_cache_evict(Ndb_index_stat *st)
{
  ...
  DBUG_PRINT("index_stat", ("evict table: %u index: %u version: %u"
                            ...
                            head.m_tableId, head.m_indexId, head.m_indexVersion, head.m_sampleVersion,
...
struct Ndb_index_stat_proc
{
  ...
  bool init_ndb(Ndb_cluster_connection *connection)
  {
    ...
    {
      sql_print_error(
          "ndb_index_stat_proc: Failed to set object name, "
          "error code %d",
    ...
    {
      sql_print_error("ndb_index_stat_proc: Failed to change database to %s", NDB_INDEX_STAT_DB);
      return false;
    }

    sql_print_information(
        "ndb_index_stat_proc: Created Ndb object, "
...
static void ndb_index_stat_proc_new(Ndb_index_stat_proc &pr, Ndb_index_stat *st)
{
...
static void ndb_index_stat_proc_update(Ndb_index_stat_proc &pr, Ndb_index_stat *st)
{
...
static void ndb_index_stat_proc_read(Ndb_index_stat_proc &pr, Ndb_index_stat *st)
{
  Ndb_index_stat_glob &glob = ndb_index_stat_glob;
  NdbIndexStat::Head head;
  if (st->is->read_stat(pr.ndb) == -1)
  {
    ...
    if (st->is->getNdbError().code == NdbIndexStat::NoIndexStats && !force_update)
    {
...
static void ndb_index_stat_proc_idle(Ndb_index_stat_proc &pr, Ndb_index_stat *st)
{
  Ndb_index_stat_glob &glob = ndb_index_stat_glob;
  const Ndb_index_stat_opt &opt = ndb_index_stat_opt;
...
static void ndb_index_stat_proc_check(Ndb_index_stat_proc &pr, Ndb_index_stat *st)
{
  pr.now = ndb_index_stat_time();
  st->check_time = pr.now;
  NdbIndexStat::Head head;
  ...
  {
    DBUG_PRINT("index_stat", ("st %s sample version old:%u new:%u", st->id, version_old, version_new));
    pr.lt = Ndb_index_stat::LT_Read;
    return;
  }
  pr.lt = Ndb_index_stat::LT_Idle;
...
static bool ndb_index_stat_evict(const Ndb_index_stat *st1, const Ndb_index_stat *st2)
{
  if (st1->access_time < st2->access_time)
    return true;
  if (st1->access_time == st2->access_time &&
      st1->query_bytes + st1->clean_bytes >= st2->query_bytes + st2->clean_bytes)
    return true;
  return false;
}
...
static void ndb_index_stat_proc_evict(Ndb_index_stat_proc &pr, int lt)
{
  ...
  while (st_loop != 0 && st_lru_cnt < batch)
  {
    ...
    st_loop = st_loop->list_next;
    const longlong st_read_time = (longlong)st->read_time;
    if (st_read_time + evict_delay <= pr_now && st->query_bytes + st->clean_bytes != 0 && !st->to_delete)
    {
...
static void ndb_index_stat_proc_delete(Ndb_index_stat_proc &pr)
{
  ...
  while (st_loop != 0 && cnt < batch)
  {
    ...
    */
    if (st->ref_count != 0)
    {
      DBUG_PRINT("index_stat", ("st %s proc %s: ref_count:%u", st->id, list.name, st->ref_count));
      continue;
    }
...
static void ndb_index_stat_proc_error(Ndb_index_stat_proc &pr, Ndb_index_stat *st)
{
  const Ndb_index_stat_opt &opt = ndb_index_stat_opt;
  ...
  DBUG_PRINT("index_stat", ("st %s error_wait:%lld error_count:%u"
                            " force_update:%d to_delete:%d",
                            st->id, error_wait, st->error_count, st->force_update, st->to_delete));
...
static void ndb_index_stat_proc_event(Ndb_index_stat_proc &pr, Ndb_index_stat *st)
{
  /*
...
static void ndb_index_stat_proc_event(Ndb_index_stat_proc &pr)
{
  ...
  while (1)
  {
    ...
    NdbIndexStat::Head head;
    is->get_head(head);
    DBUG_PRINT("index_stat", ("next_listener eventType: %d indexId: %u", head.m_eventType, head.m_indexId));

    Ndb_index_stat *st = ndb_index_stat_find_entry(head.m_indexId, head.m_indexVersion, head.m_tableId);
    /*
...
static void ndb_index_stat_proc_control(Ndb_index_stat_proc &pr)
{
  Ndb_index_stat_glob &glob = ndb_index_stat_glob;
  Ndb_index_stat_opt &opt = ndb_index_stat_opt;
...
#ifndef NDEBUG
static void ndb_index_stat_entry_verify(Ndb_index_stat_proc &pr, const Ndb_index_stat *st)
{
...
static int ndb_index_stat_check_or_create_systables(Ndb_index_stat_proc &pr)
{
      ...
      is->getNdbError().code == 4009)  // no connection
  {
    ...
    DBUG_PRINT("index_stat",
               ("create index stats tables failed: error %d line %d", is->getNdbError().code, is->getNdbError().line));
    DBUG_RETURN(-1);
  }

  sql_print_warning("create index stats tables failed: error %d line %d", is->getNdbError().code,
                    is->getNdbError().line);
  DBUG_RETURN(-1);
}

static int ndb_index_stat_check_or_create_sysevents(Ndb_index_stat_proc &pr)
{
  ...
  if (is->getNdbError().code == 746)
  {
    // race between mysqlds, maybe
    DBUG_PRINT("index_stat",
               ("create index stats events failed: error %d line %d", is->getNdbError().code, is->getNdbError().line));
    DBUG_RETURN(-1);
  }

  sql_print_warning("create index stats events failed: error %d line %d", is->getNdbError().code,
                    is->getNdbError().line);
  DBUG_RETURN(-1);
}

static int ndb_index_stat_start_listener(Ndb_index_stat_proc &pr)
{
  ...
  {
    sql_print_warning("create index stats listener failed: error %d line %d", is->getNdbError().code,
                      is->getNdbError().line);
    DBUG_RETURN(-1);
  }
  ...
  {
    sql_print_warning("execute index stats listener failed: error %d line %d", is->getNdbError().code,
                      is->getNdbError().line);
...
static int ndb_index_stat_stop_listener(Ndb_index_stat_proc &pr)
{
  ...
  {
    sql_print_warning("drop index stats listener failed: error %d line %d", is->getNdbError().code,
                      is->getNdbError().line);
    DBUG_RETURN(-1);
  }
...
void Ndb_index_stat_thread::do_run()
{
  ...
  */
  ...
  while (!is_stop_requested() && !g_ndb_status.cluster_node_id && (ndbcluster_hton->slot != ~(uint)0))
  {
  ...
  for (;;)
  {
    ...
    {
      int ret = native_cond_timedwait(&COND, &LOCK, &abstime);
    ...
    do
    {
      ...
      {
        DBUG_PRINT("index_stat", ("global enable: %d -> %d", enable_ok, enable_ok_new));

        if (enable_ok_new)
        {
          // at enable check or create stats tables and events
          if (ndb_index_stat_check_or_create_systables(pr) == -1 ||
              ndb_index_stat_check_or_create_sysevents(pr) == -1 || ndb_index_stat_start_listener(pr) == -1)
          {
...
static int ndb_index_stat_wait_query(Ndb_index_stat *st, const Ndb_index_stat_snap &snap)
{
  ...
  while (true)
  {
    ...
    */
    if (st->load_time != snap.load_time || st->sample_version != snap.sample_version)
    {
      assert(false);
      err = NdbIndexStat::NoIndexStats;
      break;
    ...
    set_timespec(&abstime, 1);
    ret = native_cond_timedwait(&ndb_index_stat_thread.stat_cond, &ndb_index_stat_thread.stat_mutex, &abstime);
  ...
  DBUG_PRINT("index_stat",
             ("st %s wait_query ok: sample_version %u -> %u", st->id, snap.sample_version, st->sample_version));
  DBUG_RETURN(0);
}

static int ndb_index_stat_wait_analyze(Ndb_index_stat *st, const Ndb_index_stat_snap &snap)
{
  ...
  DBUG_PRINT("index_stat",
             ("st %s wait_analyze ok: sample_version %u -> %u", st->id, snap.sample_version, st->sample_version));
  DBUG_RETURN(0);
}

int ha_ndbcluster::ndb_index_stat_query(uint inx, const key_range *min_key, const key_range *max_key,
                                        NdbIndexStat::Stat &stat, int from)
{
  ...
  Ndb_index_stat *st = ndb_index_stat_get_share(m_share, index, m_table, snap, err, true, false);
  if (st == 0)
    DBUG_RETURN(err);
...
int ha_ndbcluster::ndb_index_stat_get_rir(uint inx, key_range *min_key, key_range *max_key, ha_rows *rows_out)
{
...
int ha_ndbcluster::ndb_index_stat_analyze(Ndb *ndb, uint *inx_list, uint inx_count)
{
  ...
  for (uint i = 0; i < inx_count; i++)
  {
    ...
    r.st = ndb_index_stat_get_share(m_share, index, m_table, r.snap, r.err, true, true);
