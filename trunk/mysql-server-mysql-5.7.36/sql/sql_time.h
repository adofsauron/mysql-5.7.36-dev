...
inline void date_to_datetime(MYSQL_TIME *ltime) { ltime->time_type = MYSQL_TIMESTAMP_DATETIME; }
...
inline void make_truncated_value_warning(ErrConvString val, timestamp_type time_type)
{
  make_truncated_value_warning(current_thd, Sql_condition::SL_WARNING, val, time_type, NullS);
}
...
static inline bool str_to_time(const String *str, MYSQL_TIME *ltime, my_time_flags_t flags, MYSQL_TIME_STATUS *status)
{
  return str_to_time(str->charset(), str->ptr(), str->length(), ltime, flags, status);
}
...
bool str_to_datetime(const CHARSET_INFO *cs, const char *str, size_t length, MYSQL_TIME *l_time, my_time_flags_t flags,
                     MYSQL_TIME_STATUS *status);
static inline bool str_to_datetime(const String *str, MYSQL_TIME *ltime, my_time_flags_t flags,
                                   MYSQL_TIME_STATUS *status)
{
  return str_to_datetime(str->charset(), str->ptr(), str->length(), ltime, flags, status);
}
...
inline void my_datetime_trunc(MYSQL_TIME *ltime, uint decimals) { return my_time_trunc(ltime, decimals); }
...
inline double TIME_microseconds(const MYSQL_TIME *ltime) { return (double)ltime->second_part / 1000000; }
...
static inline bool check_fuzzy_date(const MYSQL_TIME *ltime, my_time_flags_t fuzzydate)
{
  return !(fuzzydate & TIME_FUZZY_DATE) && (!ltime->month || !ltime->day);
}

static inline bool non_zero_date(const MYSQL_TIME *ltime) { return ltime->year || ltime->month || ltime->day; }
...
longlong TIME_to_longlong_packed(const MYSQL_TIME *tm, enum enum_field_types type);
...
static inline timestamp_type field_type_to_timestamp_type(enum enum_field_types type)
{
  switch (type)
  {
