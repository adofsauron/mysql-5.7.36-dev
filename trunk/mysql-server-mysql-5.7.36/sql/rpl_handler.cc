...
Observer_info::Observer_info(void *ob, st_plugin_int *p) : observer(ob), plugin_int(p)
{
...
)
{
  ...
  my_atomic_fas32(&m_configured_lock_type,
                  opt_replication_optimize_for_static_plugin_config ? DELEGATE_SPIN_LOCK : DELEGATE_OS_LOCK);
...
void Delegate::update_lock_type()
{
  ...
  int32 opt_value = opt_replication_optimize_for_static_plugin_config ? DELEGATE_SPIN_LOCK : DELEGATE_OS_LOCK;
...
void Delegate::update_plugin_ref_count()
{
  ...
  {
    for (std::map<plugin_ref, size_t>::iterator ref = m_acquired_references.begin(); ref != m_acquired_references.end();
         ++ref)
    {
...
{
  return my_atomic_load32(&m_acquired_locks) < 0 ||  // If there are acquisitions using the read-write lock
         (my_atomic_load32(&m_configured_lock_type) == DELEGATE_OS_LOCK &&  // or the lock type has been set to use the
                                                                            // read-write lock
          my_atomic_load32(&m_acquired_locks) == 0);  // and there are no outstanding acquisitions using shared
                                                      // spin-lock, use the read-write lock
}

bool Delegate::use_spin_lock_type()
{
  return my_atomic_load32(&m_acquired_locks) > 0 ||  // If there are acquisitions using the shared spin-lock
         (my_atomic_load32(&m_configured_lock_type) == DELEGATE_SPIN_LOCK &&  // or the lock type has been set to use
                                                                              // the shared spin-lock
          my_atomic_load32(&m_acquired_locks) == 0);  // and there are no outstanding acquisitions using read-write
...
void Delegate::lock_it(enum_delegate_lock_mode mode)
{
  do
  {
    if (this->use_spin_lock_type())
    {
      ...
      if (my_atomic_load32(&m_configured_lock_type) != DELEGATE_SPIN_LOCK)  // Lock type changed in the meanwhile, lets
    ...
    if (this->use_rw_lock_type())
    {
      ...
      if (my_atomic_load32(&m_configured_lock_type) != DELEGATE_OS_LOCK)  // Lock type changed in the meanwhile, lets
                                                                          // revert the acquisition and try again
...
int get_user_var_int(const char *name, long long int *value, int *null_value)
{
  ...
  user_var_entry *entry = (user_var_entry *)my_hash_search(&thd->user_vars, (uchar *)name, strlen(name));
  if (!entry)
  {
...
int delegates_init()
{
  static my_aligned_storage<sizeof(Trans_delegate), MY_ALIGNOF(longlong)> trans_mem;
  static my_aligned_storage<sizeof(Binlog_storage_delegate), MY_ALIGNOF(longlong)> storage_mem;
  static my_aligned_storage<sizeof(Server_state_delegate), MY_ALIGNOF(longlong)> server_state_mem;
#ifdef HAVE_REPLICATION
  static my_aligned_storage<sizeof(Binlog_transmit_delegate), MY_ALIGNOF(longlong)> transmit_mem;
  static my_aligned_storage<sizeof(Binlog_relay_IO_delegate), MY_ALIGNOF(longlong)> relay_io_mem;
#endif
...
#define FOREACH_OBSERVER(r, f, thd, args)                                                                  \
  ...
  */                                                                                                       \
  ...
  for (; info; info = iter++)                                                                              \
  {                                                                                                        \
    bool replication_optimize_for_static_plugin_config = this->use_spin_lock_type();                       \
    plugin_ref plugin =                                                                                    \
        (replication_optimize_for_static_plugin_config ? info->plugin : my_plugin_lock(0, &info->plugin)); \
    if (!plugin)                                                                                           \
    {                                                                                                      \
    ...
    {                                                                                                      \
      r = 1;                                                                                               \
      sql_print_error("Run function '" #f "' in plugin '%s' failed", info->plugin_int->name.str);          \
      break;                                                                                               \
    }                                                                                                      \
  }                                                                                                        \
  unlock();                                                                                                \
...
int Trans_delegate::before_commit(THD *thd, bool all, IO_CACHE *trx_cache_log, IO_CACHE *stmt_cache_log,
                                  ulonglong cache_log_max_size)
{
  ...
  bool is_real_trans = (all || !thd->get_transaction()->is_active(Transaction_ctx::SESSION));
  if (is_real_trans)
...
bool has_cascade_foreign_key(TABLE *table, THD *thd)
{
  ...
  while ((f_key_info = foreign_key_iterator++))
  {
    ...
    */
    if (f_key_info->update_method->str[0] == 'C' || f_key_info->delete_method->str[0] == 'C' ||
        f_key_info->update_method->str[0] == 'S' || f_key_info->delete_method->str[0] == 'S')
    {
 ...
 */
void Trans_delegate::prepare_table_info(THD *thd, Trans_table_info *&table_info_list, uint &number_of_tables)
{
  ...
  if (table_info_holder.size() > 0)
  {
    ...
    table_info_list =
        (Trans_table_info *)my_malloc(PSI_NOT_INSTRUMENTED, number_of_tables * sizeof(Trans_table_info), MYF(0));

    std::vector<Trans_table_info>::iterator table_info_holder_it = table_info_holder.begin();
    for (int table = 0; table_info_holder_it != table_info_holder.end(); table_info_holder_it++, table++)
    {
      table_info_list[table].number_of_primary_keys = (*table_info_holder_it).number_of_primary_keys;
      ...
      table_info_list[table].has_cascade_foreign_key = (*table_info_holder_it).has_cascade_foreign_key;
    }
  }
...
void prepare_transaction_context(THD *thd, Trans_context_info &ctx_info)
{
  ...
  ctx_info.transaction_write_set_extraction = thd->variables.transaction_write_set_extraction;
...
int Binlog_storage_delegate::after_flush(THD *thd, const char *log_file, my_off_t log_pos)
{
  DBUG_ENTER("Binlog_storage_delegate::after_flush");
  DBUG_PRINT("enter", ("log_file: %s, log_pos: %llu", log_file, (ulonglong)log_pos));
...
int Binlog_storage_delegate::after_sync(THD *thd, const char *log_file, my_off_t log_pos)
{
...
#ifdef HAVE_REPLICATION
int Binlog_transmit_delegate::transmit_start(THD *thd, ushort flags, const char *log_file, my_off_t log_pos,
...
int Binlog_transmit_delegate::reserve_header(THD *thd, ushort flags, String *packet)
{
...
#define RESERVE_HEADER_SIZE 32
  ...
  for (; info; info = iter++)
  {
    bool replication_optimize_for_static_plugin_config = this->use_spin_lock_type();
    plugin_ref plugin =
        (replication_optimize_for_static_plugin_config ? info->plugin : my_plugin_lock(thd, &info->plugin));
    if (!plugin)
    {
      ret = 1;
      break;
    ...
    if (((Observer *)info->observer)->reserve_header &&
        ((Observer *)info->observer)->reserve_header(&param, header, RESERVE_HEADER_SIZE, &hlen))
    {
      ret = 1;
      if (!replication_optimize_for_static_plugin_config)
...
int Binlog_transmit_delegate::before_send_event(THD *thd, ushort flags, String *packet, const char *log_file,
                                                my_off_t log_pos)
{
  Binlog_transmit_param param;
  param.flags = flags;
  ...
  FOREACH_OBSERVER(ret, before_send_event, thd,
                   (&param, (uchar *)packet->ptr(), packet->length(), log_file + dirname_length(log_file), log_pos));
  return ret;
}

int Binlog_transmit_delegate::after_send_event(THD *thd, ushort flags, String *packet, const char *skipped_log_file,
                                               my_off_t skipped_log_pos)
{
  ...
  int ret = 0;
  FOREACH_OBSERVER(
      ret, after_send_event, thd,
      (&param, packet->ptr(), packet->length(), skipped_log_file + dirname_length(skipped_log_file), skipped_log_pos));
  return ret;
}
...
void Binlog_relay_IO_delegate::init_param(Binlog_relay_IO_param *param, Master_info *mi)
{
...
int Binlog_relay_IO_delegate::applier_stop(THD *thd, Master_info *mi, bool aborted)
{
  Binlog_relay_IO_param param;
  init_param(&param, mi);
  param.server_id = thd->server_id;
...
int Binlog_relay_IO_delegate::before_request_transmit(THD *thd, Master_info *mi, ushort flags)
{
  Binlog_relay_IO_param param;
  init_param(&param, mi);
  param.server_id = thd->server_id;
...
int Binlog_relay_IO_delegate::after_read_event(THD *thd, Master_info *mi, const char *packet, ulong len,
                                               const char **event_buf, ulong *event_len)
{
  ...
  int ret = 0;
  FOREACH_OBSERVER(ret, after_read_event, thd, (&param, packet, len, event_buf, event_len));
  return ret;
}

int Binlog_relay_IO_delegate::after_queue_event(THD *thd, Master_info *mi, const char *event_buf, ulong event_len,
                                                bool synced)
{
  Binlog_relay_IO_param param;
  init_param(&param, mi);
  ...
  int ret = 0;
  FOREACH_OBSERVER(ret, after_queue_event, thd, (&param, event_buf, event_len, flags));
  return ret;
}
