...
*/
Table_trigger_dispatcher::Table_trigger_dispatcher(const char *db_name, const char *table_name)
      ...
      m_has_unparseable_trigger(false)
{
  ...
  init_sql_alloc(key_memory_Table_trigger_dispatcher, &m_mem_root, 8192, 0);
...
bool Table_trigger_dispatcher::create_trigger(THD *thd, String *binlog_create_trigger_stmt)
{
  LEX *lex = thd->lex;
...

  {
    ...
    LEX_STRING trn_path = Trigger_loader::build_trn_path(trn_path_buffer, FN_REFLEN, thd->lex->spname->m_db.str,
  ...
  */
  ...
  if (lex->definer &&
      (strcmp(lex->definer->user.str, thd->security_context()->priv_user().str) ||
  ...
  if (lex->definer && !is_acl_user(lex->definer->host.str, lex->definer->user.str))
  {
  ...
  if (lex->sphead->setup_trigger_fields(thd, get_trigger_field_support(), NULL, true))
    return true;
  ...
  Trigger *t = Trigger::create_from_parser(thd, m_subject_table, binlog_create_trigger_stmt);
  ...
  Trigger_chain *tc = create_trigger_chain(t->get_event(), t->get_action_time());
  ...
  if (tc->add_trigger(get_mem_root(), t, lex->sphead->m_trg_chistics.ordering_clause,
  ...
  */
  ...
  return rebuild_trigger_list() ||
         Trigger_loader::store_trigger(m_db_name, m_subject_table_name, get_mem_root(), t, &m_triggers);
}
...
bool Table_trigger_dispatcher::drop_trigger(THD *thd, const LEX_STRING &trigger_name, bool *trigger_found)
{
  if (rebuild_trigger_list() || Trigger_loader::drop_trigger(m_db_name, m_subject_table_name, trigger_name,
                                                             get_mem_root(), &m_triggers, trigger_found))
    return true;
...
bool Table_trigger_dispatcher::prepare_record1_accessors()
{
  ...
  m_record1_field = (Field **)alloc_root(get_mem_root(), (m_subject_table->s->fields + 1) * sizeof(Field *));

  if (!m_record1_field)
    return true;

  for (fld = m_subject_table->field, old_fld = m_record1_field; *fld; fld++, old_fld++)
  {
    /*
      QQ: it is supposed that it is ok to use this function for field
      cloning...
    */
    *old_fld = (*fld)->new_field(get_mem_root(), m_subject_table, m_subject_table == (*fld)->table);

    if (!(*old_fld))
      return true;

    (*old_fld)->move_field_offset((my_ptrdiff_t)(m_subject_table->record[1] - m_subject_table->record[0]));
  }
...
bool Table_trigger_dispatcher::check_n_load(THD *thd, bool names_only)
{
  ...
  if (Trigger_loader::load_triggers(thd, get_mem_root(), m_db_name.str, m_subject_table_name.str, &m_triggers))
  {
    return true;
  }
...

  {
    ...
    {
      Trigger_chain *tc =
          t->has_parse_error() ? m_unparseable_triggers : create_trigger_chain(t->get_event(), t->get_action_time());
  ...
  if (!names_only)  // if we're doing complete trigger loading
  {
    ...
    */
    ...
    while ((t = it++))
    {
      ...
      sp->setup_trigger_fields(thd, get_trigger_field_support(), t->get_subject_table_grant(), false);
    }
  }
...
Trigger_chain *Table_trigger_dispatcher::create_trigger_chain(enum_trigger_event_type event,
...
bool Table_trigger_dispatcher::rename_subject_table(THD *thd, const char *old_db_name, const char *new_db_name,
                                                    const char *old_table_name_str, const char *new_table_name_str,
                                                    bool upgrading50to51)
{
  ...
  return Trigger_loader::rename_subject_table(get_mem_root(), &m_triggers, old_db_name, &old_table_name, new_db_name,
                                              &new_table_name, upgrading50to51);
}
...
bool Table_trigger_dispatcher::process_triggers(THD *thd, enum_trigger_event_type event,
                                                enum_trigger_action_time_type action_time, bool old_row_is_record1)
{
  ...
  /*
    ...
    algorithm.
  */
  assert(m_subject_table->pos_in_table_list->trg_event_map & static_cast<uint>(1 << static_cast<int>(event)));
...
bool Table_trigger_dispatcher::add_tables_and_routines_for_triggers(THD *thd, Query_tables_list *prelocking_ctx,
                                                                    TABLE_LIST *table_list)
{
  assert(static_cast<int>(table_list->lock_type) >= static_cast<int>(TL_WRITE_ALLOW_WRITE));
