...
Join_plan::Join_plan(const JOIN *join)
    : m_qep_tabs(join->qep_tab), m_access_count(join->primary_tables), m_table_accesses(NULL)
{
  /*
    This combination is assumed not to appear. If it does, code must
    be written to handle it.
  */
  assert(!m_qep_tabs[0].dynamic_range() || (m_qep_tabs[0].type() == JT_ALL) || (m_qep_tabs[0].quick() == NULL));
...
enum_join_type Table_access::get_join_type(const Table_access *predecessor) const
{
  ...
  {
    DBUG_PRINT("info",
               ("JT_INNER_JOIN between %s and %s", predecessor->get_qep_tab()->table()->alias, me->table()->alias));
    DBUG_RETURN(JT_INNER_JOIN);
  }
  else
  {
    DBUG_PRINT("info",
               ("JT_OUTER_JOIN between %s and %s", predecessor->get_qep_tab()->table()->alias, me->table()->alias));
...
{
  assert(m_access_type == AT_PRIMARY_KEY || m_access_type == AT_UNIQUE_KEY || m_access_type == AT_MULTI_PRIMARY_KEY ||
         m_access_type == AT_MULTI_UNIQUE_KEY || m_access_type == AT_ORDERED_INDEX_SCAN);  // Used as 'range scan'
...
Item_equal *Table_access::get_item_equal(const Item_field *field_item) const
{
  ...
  if (cond_equal != NULL)
  {
    return (field_item->item_equal != NULL) ? field_item->item_equal
...
void Table_access::dbug_print() const
{
  ...
  DBUG_PRINT("info", ("skip_sort_order:%d", get_qep_tab()->join()->skip_sort_order));
  ...
  DBUG_PRINT("info", ("group_optimized_away:%d", get_qep_tab()->join()->group_optimized_away));

  DBUG_PRINT("info", ("need_tmp:%d", get_qep_tab()->join()->need_tmp));
  DBUG_PRINT("info", ("select_distinct:%d", get_qep_tab()->join()->select_distinct));
  ...
  {
    DBUG_PRINT("info", ("quick->get_type():%d", get_qep_tab()->quick()->get_type()));
  }
}
...
void Table_access::compute_type_and_index() const
{
  ...
  if (join->group_list && !join->tmp_table_param.quick_group)
  {
    ...
    m_other_access_reason = "GROUP BY cannot be done using index on grouped columns.";
    DBUG_VOID_RETURN;
  }
  ...
  */
  switch (qep_tab->type())
  {
    case JT_EQ_REF:
      ...
      {
        DBUG_PRINT("info", ("Operation %d is a unique index lookup.", m_tab_no));
    ...
    case JT_REF:
    {
      ...
      */
      ...
      if (key_info[m_index_no].user_defined_key_parts == qep_tab->ref().key_parts &&
          key_info[m_index_no].flags & HA_NOSAME)
      {
        m_access_type =
            (m_index_no == static_cast<int32>(qep_tab->table()->s->primary_key)) ? AT_PRIMARY_KEY : AT_UNIQUE_KEY;
        DBUG_PRINT("info", ("Operation %d is an unique key referrence.", m_tab_no));
      }
      else
      {
        assert(qep_tab->ref().key_parts > 0);
        assert(qep_tab->ref().key_parts <= key_info[m_index_no].user_defined_key_parts);
    ...
    case JT_INDEX_MERGE:
      if (qep_tab->dynamic_range())
      {
        ...
        */
        DBUG_PRINT("info", ("Operation %d has 'dynamic range' -> not pushable", m_tab_no));
      ...
      else
      {
        if (qep_tab->quick() != NULL)
        {
          ...
          assert((quick->index == MAX_KEY) == ((quick->get_type() == QUICK_SELECT_I::QS_TYPE_INDEX_MERGE) ||
    ...
    default:
      /*
        ...
        not yet in place.
      */
      DBUG_PRINT("info", ("Operation %d has join_type %d. -> Not pushable.", m_tab_no, qep_tab->type()));
      m_access_type = AT_OTHER;
      m_index_no = -1;
...
Table_access::Table_access()
    : m_join_plan(NULL), m_tab_no(0), m_access_type(AT_VOID), m_other_access_reason(NULL), m_index_no(-1)
{
}
...
{
  return get_qep_tab()->op && get_qep_tab()->op->type() == QEP_operation::OT_CACHE;
}
...
const Table_access *Table_access::get_firstmatch_last_skipped() const
{
  ...
  if (qep_tab->do_firstmatch())
  {
    ...
    const uint firstmatch_last_skipped = qep_tab->firstmatch_return + 1;
...
{
  if (m_access_type == AT_PRIMARY_KEY || m_access_type == AT_UNIQUE_KEY)
  {
    return false;
  }
