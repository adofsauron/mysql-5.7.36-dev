...
void ndbcluster_build_key_map(const NdbDictionary::Table *table, const NDB_INDEX_DATA &index, const KEY *key_def,
                              uint ix_map[]);
...
class ndb_table_access_map : public table_bitmap
{
 public:
  ...
  explicit ndb_table_access_map(uint table_no) : table_bitmap(0) { set_bit(table_no); }
  ...
  bool contain(const ndb_table_access_map &table_map) const { return table_map.is_subset(*this); }
...
{
 public:
  explicit ndb_pushed_join(const ndb_pushed_builder_ctx &builder_ctx, const NdbQueryDef *query_def);
...
class ndb_pushed_builder_ctx
{
  friend ndb_pushed_join::ndb_pushed_join(const ndb_pushed_builder_ctx &builder_ctx, const NdbQueryDef *query_def);
 ...
 private:
  ...
  bool is_const_item_pushable(const Item *key_item, const KEY_PART_INFO *key_part);

  bool is_field_item_pushable(const AQP::Table_access *table, const Item *key_item, const KEY_PART_INFO *key_part,
  ...
  void collect_key_refs(const AQP::Table_access *table, const Item *key_refs[]) const;
