...
class Addon_fields
{
 public:
  Addon_fields(Addon_fields_array arr)
      : m_field_descriptors(arr), m_addon_buf(NULL), m_addon_buf_length(0), m_using_packed_addons(false)
  {
    assert(!arr.is_null());
  }
...
class Sort_param
{
  ...
  */
  void init_for_filesort(Filesort *file_sort, uint sortlen, TABLE *table, ulong max_length_for_sort_data,
  ...
  {
    assert(m_using_packed_addons == (addon_fields != NULL && addon_fields->using_packed_addons()));
  ...
  size_t compare_length() const { return sort_length; }
...
class Filesort_info
{
 ...
 public:
  ...
  Filesort_info() : sorted_result_in_fsbuf(false), sorted_result(NULL), sorted_result_end(NULL){};

  bool has_filesort_result_in_memory() const { return sorted_result || sorted_result_in_fsbuf; }

  bool has_filesort_result() const { return has_filesort_result_in_memory() || (io_cache && my_b_inited(io_cache)); }
  ...
  */
  ...
  bool using_packed_addons() const { return addon_fields != NULL && addon_fields->using_packed_addons(); }

  /**
    Accessors for filesort_buffer (@see Filesort_buffer for documentation).
  */
  size_t space_used_for_data() const { return filesort_buffer.space_used_for_data(); }
  ...
  void adjust_next_record_pointer(uint32 val) { filesort_buffer.adjust_next_record_pointer(val); }

  uchar *get_sorted_record(uint idx) { return filesort_buffer.get_sorted_record(idx); }
...
int merge_many_buff(Sort_param *param, Sort_buffer sort_buffer, Merge_chunk_array chunk_array, size_t *num_chunks,
                    IO_CACHE *t_file);
...
int merge_buffers(Sort_param *param, IO_CACHE *from_file, IO_CACHE *to_file, Sort_buffer sort_buffer,
                  Merge_chunk *lastbuff, Merge_chunk_array chunk_array, int flag);
