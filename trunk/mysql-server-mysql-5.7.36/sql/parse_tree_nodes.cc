...
bool PT_order::contextualize(Parse_context *pc)
{
  ...
  if (pc->select->linkage != GLOBAL_OPTIONS_TYPE && pc->select->olap != UNSPECIFIED_OLAP_TYPE &&
  ...
  if (lex->sql_command != SQLCOM_ALTER_TABLE && !unit->fake_select_lex)
  {
    ...
    */
    ...
    if (!unit->is_union() && (first_sl->order_list.elements || first_sl->select_limit))
    {
...
bool PT_table_factor_select_sym::contextualize(Parse_context *pc)
{
  ...
  const bool is_deeply_nested = embedding && !embedding->nested_join->join_list.elements;
...
bool PT_table_factor_parenthesis::contextualize(Parse_context *pc)
{
  ...
  */
  if (!(select_derived_union->value || opt_table_alias) && outer_select->embedding &&
  ...
  else if (!select_derived_union->value)
  {
    ...
    */
    ...
    value = pc->select->add_table_to_list(pc->thd, ti, opt_table_alias, 0, TL_READ, MDL_SHARED_READ);
    if (value == NULL)
      return true;
...
bool PT_option_value_no_option_type_internal::contextualize(Parse_context *pc)
{
  ...
  else
  {
    ...
    */
    ...
    sp_instr_set *i = new sp_instr_set(sp->instructions(), lex, spv->offset, opt_expr, expr_query,
...
bool PT_option_value_no_option_type_password::contextualize(Parse_context *pc)
{
  ...
  set_var_password *var = new set_var_password(user, const_cast<char *>(password));
  if (var == NULL)
    return true;
...
{
  const ulong table_opts = is_multitable() ? TL_OPTION_UPDATING | TL_OPTION_ALIAS : TL_OPTION_UPDATING;
  const thr_lock_type lock_type = (opt_delete_options & DELETE_LOW_PRIORITY) ? TL_WRITE_LOW_PRIORITY : TL_WRITE_DEFAULT;
  const enum_mdl_type mdl_type =
      (opt_delete_options & DELETE_LOW_PRIORITY) ? MDL_SHARED_WRITE_LOW_PRIO : MDL_SHARED_WRITE;
  return !pc->select->add_table_to_list(pc->thd, table, NULL, table_opts, lock_type, mdl_type, NULL, opt_use_partition);
}
...
bool PT_delete::contextualize(Parse_context *pc)
{
  ...
  if (opt_where_clause != NULL && opt_where_clause->itemize(pc, &opt_where_clause))
    return true;
...
bool PT_insert::contextualize(Parse_context *pc)
{
  ...
  Yacc_state *yyps = &pc->thd->m_parser_state->m_yacc;
  if (!pc->select->add_table_to_list(pc->thd, table_ident, NULL, TL_OPTION_UPDATING, yyps->m_lock_type,
                                     yyps->m_mdl_type, NULL, opt_use_partition))
  {
    return true;
  }
  ...
  if (opt_on_duplicate_column_list != NULL)
  {
    ...
    assert(opt_on_duplicate_value_list != NULL &&
           opt_on_duplicate_value_list->elements() == opt_on_duplicate_column_list->elements());
    ...
    if (opt_on_duplicate_column_list->contextualize(pc) || opt_on_duplicate_value_list->contextualize(pc))
      return true;
...
Sql_cmd *PT_insert::make_cmd(THD *thd)
{
  ...
  if (has_select())
    sql_cmd = new (thd->mem_root) Sql_cmd_insert_select(is_replace, thd->lex->duplicates);
  else
