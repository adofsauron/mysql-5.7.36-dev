...
class Audit_error_handler : public Internal_error_handler
{
  ...
  */
  Audit_error_handler(const Audit_error_handler &obj MY_ATTRIBUTE((unused)))
      : m_thd(NULL), m_warning_message(NULL), m_error_reported(false), m_active(false)
  {
  }
  ...
  */
  Audit_error_handler(THD *thd, const char *warning_message, bool active = true)
      : m_thd(thd), m_warning_message(warning_message), m_error_reported(false), m_active(active)
  {
    if (m_active)
    {
  ...
  */
  ...
  {
    sql_print_warning("%s. The trigger error was (%d) [%s]: %s", warn_msg, sql_errno,
...
class Auto_THD : public Internal_error_handler
{
  ...
  */
  virtual bool handle_condition(THD *thd MY_ATTRIBUTE((unused)), uint sql_errno MY_ATTRIBUTE((unused)),
                                const char *sqlstate MY_ATTRIBUTE((unused)),
                                Sql_condition::enum_severity_level *level MY_ATTRIBUTE((unused)), const char *msg)
  {
...
static int event_class_dispatch_error(THD *thd, mysql_event_class_t event_class, const char *event_name,
                                      const void *event);
...
*/
inline void thd_get_audit_query(THD *thd, MYSQL_LEX_CSTRING *query, const struct charset_info_st **charset)
{
  /*
...
class Ignore_event_error_handler : public Audit_error_handler
{
 public:
  /**
    ...
    @param warning_message[in] Warning message used when error has been
                               suppressed.
  */
  Ignore_event_error_handler(THD *thd, const char *event_name) : Audit_error_handler(thd, ""), m_event_name(event_name)
  {
  }
  ...
  */
  ...
  {
    sql_print_warning(
        "Event '%s' cannot be aborted. "
        "The trigger error was (%d) [%s]: %s",
        m_event_name, sql_errno, sqlstate ? sqlstate : "<NO_STATE>", msg ? msg : "<NO_MESSAGE>");
  }
                       ...
                       const char *msg, size_t msg_len)
{
  ...
  if (mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_GENERAL_CLASS, static_cast<unsigned long>(subclass)))
    return 0;
  ...
  thd_get_audit_query(thd, &event.general_query, (const charset_info_st **)&event.general_charset);

  event.general_time = thd->start_time.tv_sec;

  DBUG_EXECUTE_IF("audit_log_negative_general_error_code", event.general_error_code *= -1;);

  event.general_command.str = msg;
  event.general_command.length = msg_len;

  if (subclass == MYSQL_AUDIT_GENERAL_ERROR || subclass == MYSQL_AUDIT_GENERAL_STATUS ||
      subclass == MYSQL_AUDIT_GENERAL_RESULT)
  {
    Ignore_event_error_handler handler(thd, subclass_name);

    return handler.get_result(event_class_dispatch(thd, MYSQL_AUDIT_GENERAL_CLASS, &event));
  }

  return event_class_dispatch_error(thd, MYSQL_AUDIT_GENERAL_CLASS, subclass_name, &event);
}
...
int mysql_audit_notify(THD *thd, mysql_event_connection_subclass_t subclass, const char *subclass_name, int errcode)
{
  ...
  if (mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_CONNECTION_CLASS, static_cast<unsigned long>(subclass)))
    return 0;
  ...
  if (subclass == MYSQL_AUDIT_CONNECTION_DISCONNECT)
  {
    ...
    return handler.get_result(event_class_dispatch_error(thd, MYSQL_AUDIT_CONNECTION_CLASS, subclass_name, &event));
  }

  return event_class_dispatch_error(thd, MYSQL_AUDIT_CONNECTION_CLASS, subclass_name, &event);
}
                       ...
                       mysql_event_parse_rewrite_plugin_flag *flags, LEX_CSTRING *rewritten_query)
{
  ...
  return event_class_dispatch_error(thd, MYSQL_AUDIT_PARSE_CLASS, subclass_name, &event);
}
...
*/
inline static void set_table_access_subclass(mysql_event_table_access_subclass_t *out_subclass,
...
*/
int mysql_audit_notify(THD *thd, mysql_event_table_access_subclass_t subclass, const char *subclass_name,
                       TABLE_LIST *table)
{
  LEX_CSTRING str;
  ...
  if (!generate_table_access_event(table) ||
      mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_TABLE_ACCESS_CLASS, static_cast<unsigned long>(subclass)))
    return 0;
  ...
  return event_class_dispatch_error(thd, MYSQL_AUDIT_TABLE_ACCESS_CLASS, subclass_name, &event);
}
...
int mysql_audit_table_access_notify(THD *thd, TABLE_LIST *table)
{
  ...
  switch (thd->lex->sql_command)
  {
    ...
    case SQLCOM_INSERT_SELECT:
    {
      ...
      */
      set_table_access_subclass(&subclass, &subclass_name, AUDIT_EVENT(MYSQL_AUDIT_TABLE_ACCESS_INSERT));
      ...
      set_table_access_subclass(&subclass, &subclass_name, AUDIT_EVENT(MYSQL_AUDIT_TABLE_ACCESS_READ));
      break;
    }
    case SQLCOM_INSERT:
    case SQLCOM_REPLACE:
    ...
    case SQLCOM_TRUNCATE:
      set_table_access_subclass(&subclass, &subclass_name, AUDIT_EVENT(MYSQL_AUDIT_TABLE_ACCESS_DELETE));
      break;
    case SQLCOM_UPDATE:
    case SQLCOM_UPDATE_MULTI:
      /* Update state is taken from the table instance in the
         mysql_audit_notify function. */
      set_table_access_subclass(&subclass, &subclass_name, AUDIT_EVENT(MYSQL_AUDIT_TABLE_ACCESS_UPDATE));
      break;
    case SQLCOM_SELECT:
    case SQLCOM_HA_READ:
    case SQLCOM_ANALYZE:
  ...
  for (; table; table = table->next_global)
  {
    ...
    */
    if (subclass == MYSQL_AUDIT_TABLE_ACCESS_UPDATE && !table->referencing_view && !table->updating)
      set_table_access_subclass(&subclass, &subclass_name, AUDIT_EVENT(MYSQL_AUDIT_TABLE_ACCESS_READ));
                       ...
                       const char *name, const char *value, const unsigned int value_length)
{
  ...
  if (mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_GLOBAL_VARIABLE_CLASS, static_cast<unsigned long>(subclass)))
    return 0;
  ...
  return event_class_dispatch_error(thd, MYSQL_AUDIT_GLOBAL_VARIABLE_CLASS, subclass_name, &event);
}
                       ...
                       unsigned int argc)
{
  ...
  if (mysql_audit_acquire_plugins(thd.thd, MYSQL_AUDIT_SERVER_STARTUP_CLASS, static_cast<unsigned long>(subclass)))
    return 0;
  ...
  return event_class_dispatch_error(thd.thd, MYSQL_AUDIT_SERVER_STARTUP_CLASS, subclass_name, &event);
}
...
*/
int mysql_audit_notify(THD *thd, mysql_event_server_shutdown_subclass_t subclass, mysql_server_shutdown_reason_t reason,
                       int exit_code)
{
  mysql_event_server_shutdown event;

  if (mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_SERVER_SHUTDOWN_CLASS, static_cast<unsigned long>(subclass)))
    return 0;
...
class Ignore_command_start_error_handler : public Audit_error_handler
{
  ...
  */
  Ignore_command_start_error_handler(THD *thd, enum_server_command command, const char *command_text)
      : Audit_error_handler(thd, "", ignore_command(command)), m_command(command), m_command_text(command_text)
  {
  }
  ...
  */
  ...
  {
    sql_print_warning(
        "Command '%s' cannot be aborted. "
        "The trigger error was (%d) [%s]: %s",
        m_command_text, sql_errno, sqlstate ? sqlstate : "<NO_STATE>", msg ? msg : "<NO_MESSAGE>");
  }
  ...
  */
  static bool ignore_command(enum_server_command command)
  {
    ...
    if (command == COM_QUIT || command == COM_PING || command == COM_SLEEP || /* Deprecated commands from here. */
        command == COM_CONNECT || command == COM_TIME || command == COM_DELAYED_INSERT || command == COM_END)
    {
      return true;
    }
                       ...
                       enum_server_command command, const char *command_text)
{
  ...
  if (mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_COMMAND_CLASS, static_cast<unsigned long>(subclass)))
    return 0;

  event.event_subclass = subclass;
  event.status = thd->get_stmt_da()->is_error() ? thd->get_stmt_da()->mysql_errno() : 0;
  ...
  if (subclass == MYSQL_AUDIT_COMMAND_START)
  {
    ...
    return handler.get_result(event_class_dispatch_error(thd, MYSQL_AUDIT_COMMAND_CLASS, subclass_name, &event));
  }
...
int mysql_audit_notify(THD *thd, mysql_event_query_subclass_t subclass, const char *subclass_name)
{
  ...
  if (mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_QUERY_CLASS, static_cast<unsigned long>(subclass)))
    return 0;
  ...
  return event_class_dispatch_error(thd, MYSQL_AUDIT_QUERY_CLASS, subclass_name, &event);
}
                       ...
                       const char *database, const char *name, void *parameters)
{
  ...
  if (mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_STORED_PROGRAM_CLASS, static_cast<unsigned long>(subclass)))
    return 0;
  ...
  return event_class_dispatch_error(thd, MYSQL_AUDIT_STORED_PROGRAM_CLASS, subclass_name, &event);
}
...
static my_bool acquire_lookup_mask(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  if (!check_audit_mask(audit->class_mask[evt->event_class], evt->event_subclass))
    ...
    */
    ...
    for (i = MYSQL_AUDIT_GENERAL_CLASS; i < MYSQL_AUDIT_CLASS_MASK_SIZE; i++)
      add_audit_mask(&thd->audit_class_mask[i],
                     (evt.subscribed_mask[i] ^ evt.not_subscribed_mask[i]) & evt.subscribed_mask[i]);
...
void mysql_audit_release(THD *thd)
{
  ...
  plugin_unlock_list(NULL, thd->audit_class_plugins.begin(), thd->audit_class_plugins.size());
...
static int plugins_dispatch(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  return data->event_notify(thd, event_generic->event_class, event_generic->event);
}
...
static int event_class_dispatch(THD *thd, mysql_event_class_t event_class, const void *event)
{
  ...
  */
  if (unlikely(!thd))
  {
    return plugin_foreach(thd, plugins_dispatch_bool, MYSQL_AUDIT_PLUGIN, &event_generic) ? 1 : 0;
  }
  else
  {
    plugin_ref *plugins, *plugins_last;
...
bool is_audit_plugin_class_active(THD *thd MY_ATTRIBUTE((unused)), unsigned long event_class)
{
