...
class st_alter_tablespace : public Sql_alloc
{
 public:
  ...
  {
    return ts_cmd_type == CREATE_TABLESPACE || ts_cmd_type == ALTER_TABLESPACE || ts_cmd_type == DROP_TABLESPACE ||
           ts_cmd_type == CHANGE_FILE_TABLESPACE || ts_cmd_type == ALTER_ACCESS_MODE_TABLESPACE;
  }
...
struct handlerton
{
  ...
  */
  ...
  int (*find_files)(handlerton *hton, THD *thd, const char *db, const char *path, const char *wild, bool dir,
  ...
  */
  bool (*notify_exclusive_mdl)(THD *thd, const MDL_key *mdl_key, ha_notification_type notification_type,
                               bool *victimized);
...
class Alter_inplace_info
{
  ...
  */
  ...
  static const HA_ALTER_FLAGS ADD_COLUMN = ADD_VIRTUAL_COLUMN | ADD_STORED_BASE_COLUMN | ADD_STORED_GENERATED_COLUMN;

  // Drop column
  static const HA_ALTER_FLAGS DROP_VIRTUAL_COLUMN = 1ULL << 9;
  static const HA_ALTER_FLAGS DROP_STORED_COLUMN = 1ULL << 10;
  static const HA_ALTER_FLAGS DROP_COLUMN = DROP_VIRTUAL_COLUMN | DROP_STORED_COLUMN;
  ...
  */
  ...
  void add_renamed_key(KEY *old_key, KEY *new_key)
  {
    ...
    DBUG_PRINT("info", ("index renamed: '%s' to '%s'", old_key->name, new_key->name));
  }
...
class TABLEOP_HOOKS
{
 public:
  ...
  inline void prelock(TABLE **tables, uint count) { do_prelock(tables, count); }

  inline int postlock(TABLE **tables, uint count) { return do_postlock(tables, count); }

 private:
  /* Function primitive that is called prior to locking tables */
  virtual void do_prelock(TABLE **tables, uint count) { /* Default is to do nothing */ }
   ...
   */
  virtual int do_postlock(TABLE **tables, uint count) { return 0; /* Default is to do nothing */ }
};
...
class Cost_estimate
{
  ...
  */
  bool is_zero() const { return !(io_cost || cpu_cost || import_cost || mem_cost); }
  /**
  ...
  */
  ...
  bool operator>(const Cost_estimate &other) const { return total_cost() > other.total_cost() ? true : false; }

  bool operator<(const Cost_estimate &other) const { return other > *this ? true : false; }
...
class handler : public Sql_alloc
{
 ...
 public:
  ...
  virtual ha_rows multi_range_read_info(uint keyno, uint n_ranges, uint keys, uint *bufsz, uint *flags,
                                        Cost_estimate *cost);
  virtual int multi_range_read_init(RANGE_SEQ_IF *seq, void *seq_init_param, uint n_ranges, uint mode,
                                    HANDLER_BUFFER *buf);
  ...
  */
  ...
  /**
    ...
    @return The handler error code or zero for success
  */
  virtual int get_cascade_foreign_key_table_list(THD *thd, List<st_handler_tablename> *fk_table_list) { return 0; }
  ...
  */
  ...
  uint max_record_length() const { return std::min(HA_MAX_REC_LENGTH, max_supported_record_length()); }
  uint max_keys() const { return std::min<uint>(MAX_KEY, max_supported_keys()); }
  uint max_key_parts() const { return std::min(MAX_REF_PARTS, max_supported_key_parts()); }
  uint max_key_length() const { return std::min(MAX_KEY_LENGTH, max_supported_key_length()); }
  ...
  virtual uint max_supported_key_part_length(HA_CREATE_INFO *create_info MY_ATTRIBUTE((unused))) const { return 255; }
...
#define CHF_INDEX_FLAG 3
  ...
  */
  ...
  virtual my_bool register_query_cache_table(THD *thd, char *table_key, size_t key_length,
  ...
  */
  ...
  virtual int index_read_pushed(uchar *buf, const uchar *key, key_part_map keypart_map) { return HA_ERR_WRONG_COMMAND; }

  virtual int index_next_pushed(uchar *buf) { return HA_ERR_WRONG_COMMAND; }
  ...
  */
  void ha_notify_table_changed() { notify_table_changed(); }
  ...
  */
  ...
  virtual int delete_row(const uchar *buf MY_ATTRIBUTE((unused))) { return HA_ERR_WRONG_COMMAND; }
  /**
  ...
  */
  virtual int external_lock(THD *thd MY_ATTRIBUTE((unused)), int lock_type MY_ATTRIBUTE((unused))) { return 0; }
  ...
  */
  ...
  static bool my_prepare_gcolumn_template(THD *thd, const char *db_name, const char *table_name,
                                          my_gcolumn_template_callback_t myc, void *ib_table);
  static bool my_eval_gcolumn_expr_with_open(THD *thd, const char *db_name, const char *table_name,
  ...
  */
  static bool my_eval_gcolumn_expr(THD *thd, TABLE *table, const MY_BITMAP *const fields, uchar *record);
...
class DsMrr_impl
{
  ...
  */
  ...
  int dsmrr_init(handler *h, RANGE_SEQ_IF *seq_funcs, void *seq_init_param, uint n_ranges, uint mode,
                 HANDLER_BUFFER *buf);
  void dsmrr_close();
...
{
  return (db_type && db_type->create) ? (db_type->state == SHOW_OPTION_YES) : FALSE;
}
...
int ha_create_table(THD *thd, const char *path, const char *db, const char *table_name, HA_CREATE_INFO *create_info,
                    bool update_create_info, bool is_temp_table = false);

int ha_delete_table(THD *thd, handlerton *db_type, const char *path, const char *db, const char *alias,
...
int ha_discover(THD *thd, const char *dbname, const char *name, uchar **frmblob, size_t *frmlen);
...
void ha_binlog_log_query(THD *thd, handlerton *db_type, enum_binlog_command binlog_command, const char *query,
...
bool ha_notify_exclusive_mdl(THD *thd, const MDL_key *mdl_key, ha_notification_type notification_type,
                             bool *victimized);
