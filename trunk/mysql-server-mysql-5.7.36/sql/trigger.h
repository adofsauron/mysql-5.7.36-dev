...
class Trigger : public Sql_alloc
{
 public:
  ...
  static Trigger *create_from_dd(MEM_ROOT *mem_root, const LEX_CSTRING &db_name, const LEX_CSTRING &subject_table_name,
 ...
 public:
  ...
  const LEX_CSTRING &get_subject_table_name() const { return m_subject_table_name; }

  const LEX_STRING &get_trigger_name() const { return m_trigger_name; }

  const LEX_STRING &get_definition() const { return m_definition; }

  sql_mode_t get_sql_mode() const { return m_sql_mode; }

  const LEX_STRING &get_definer() const { return m_definer; }

  const LEX_STRING &get_on_table_name() const { return m_on_table_name; }

  const LEX_STRING &get_client_cs_name() const { return m_client_cs_name; }

  const LEX_STRING &get_connection_cl_name() const { return m_connection_cl_name; }

  const LEX_STRING &get_db_cl_name() const { return m_db_cl_name; }

  enum_trigger_event_type get_event() const { return m_event; }

  enum_trigger_action_time_type get_action_time() const { return m_action_time; }

  bool is_created_timestamp_null() const { return m_created_timestamp == 0; }
  ...
  void set_action_order(ulonglong action_order) { m_action_order = action_order; }
 ...
 public:
  ...
  LEX_STRING *get_definition_ptr() { return &m_definition; }

  sql_mode_t *get_sql_mode_ptr() { return &m_sql_mode; }

  LEX_STRING *get_definer_ptr() { return &m_definer; }

  LEX_STRING *get_client_cs_name_ptr() { return &m_client_cs_name; }

  LEX_STRING *get_connection_cl_name_ptr() { return &m_connection_cl_name; }

  LEX_STRING *get_db_cl_name_ptr() { return &m_db_cl_name; }

  longlong *get_created_timestamp_ptr() { return &m_created_timestamp; }

 private:
  Trigger(MEM_ROOT *mem_root, const LEX_CSTRING &db_name, const LEX_CSTRING &table_name, const LEX_STRING &definition,
          sql_mode_t sql_mode, const LEX_STRING &definer, const LEX_STRING &client_cs_name,
          const LEX_STRING &connection_cl_name, const LEX_STRING &db_cl_name, enum_trigger_event_type event_type,
          enum_trigger_action_time_type action_time, longlong created_timestamp);

 public:
  ~Trigger();

 private:
  void set_trigger_name(const LEX_STRING &trigger_name) { m_trigger_name = trigger_name; }
