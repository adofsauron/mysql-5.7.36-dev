...
bool check_mqh(THD *thd, uint check_command)
{
  ...
  if (uc->user_resources.questions)
  {
    ...
    {
      my_error(ER_USER_LIMIT_REACHED, MYF(0), uc->user, "max_questions", (long)uc->user_resources.questions);
      error = 1;
      goto end;
    }
  }
  if (check_command < (uint)SQLCOM_END)
  {
    ...
    if (uc->user_resources.updates && (sql_command_flags[check_command] & CF_CHANGES_DATA))
    {
      ...
      {
        my_error(ER_USER_LIMIT_REACHED, MYF(0), uc->user, "max_updates", (long)uc->user_resources.updates);
        error = 1;
        goto end;
      }
    }
...
extern "C" uchar *get_key_conn(user_conn *buff, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
  *length = buff->len;
...
{
#ifndef NO_EMBEDDED_ACCESS_CHECKS
  (void)my_hash_init(&hash_user_connections, system_charset_info, max_connections, 0, 0, (my_hash_get_key)get_key_conn,
...
bool thd_init_client_charset(THD *thd, uint cs_number)
{
  ...
  */
  if (!opt_character_set_client_handshake || !(cs = get_charset(cs_number, MYF(0))) ||
      !my_strcasecmp(&my_charset_latin1, global_system_variables.character_set_client->name, cs->name))
  {
    if (!is_supported_parser_charset(global_system_variables.character_set_client))
    {
  ...
  else
  {
    ...
    {
      /* Disallow non-supported parser character sets: UCS2, UTF16, UTF32 */
      my_error(ER_WRONG_VALUE_FOR_VAR, MYF(0), "character_set_client", cs->csname);
      return true;
    }
...
static int check_connection(THD *thd)
{
  ...
  if (!thd->m_main_security_ctx.host().length)  // If TCP/IP connection
  {
    ...
    */
    ...
    thd->m_main_security_ctx.set_host_or_ip_ptr(main_sctx_ip.str, main_sctx_ip.length);
    if (!(specialflag & SPECIAL_NO_RESOLVE))
    {
      ...
      rc = ip_to_hostname(&net->vio->remote, main_sctx_ip.str, &host, &connect_errors);
      ...
      if (main_sctx_host.length)
      {
        if (main_sctx_host.str != my_localhost)
          thd->m_main_security_ctx.set_host_ptr(main_sctx_host.str,
                                                min<size_t>(main_sctx_host.length, HOSTNAME_LENGTH));
        thd->m_main_security_ctx.set_host_or_ip_ptr(main_sctx_host.str, main_sctx_host.length);
      }

      if (rc == RC_BLOCKED_HOST)
      {
        /* HOST_CACHE stats updated by ip_to_hostname(). */
        my_error(ER_HOST_IS_BLOCKED, MYF(0), thd->m_main_security_ctx.host_or_ip().str);
        return 1;
      }
    }
    DBUG_PRINT("info", ("Host: %s  ip: %s",
                        (thd->m_main_security_ctx.host().length ? thd->m_main_security_ctx.host().str : "unknown host"),
    ...
    if (acl_check_host(thd->m_main_security_ctx.host().str, main_sctx_ip.str))
    {
      ...
      my_error(ER_HOST_NOT_PRIVILEGED, MYF(0), thd->m_main_security_ctx.host_or_ip().str);
      return 1;
    }
  }
  ...
  if (thd->get_protocol_classic()->get_packet()->alloc(thd->variables.net_buffer_length))
  {
    /*
      Important note:
  ...
  */
  ...
  if (net->vio->ssl_arg)
  {
    ...
    if (version == TLS1_VERSION || version == TLS1_1_VERSION)
    {
      ...
      sql_print_warning(ER(ER_DEPRECATED_TLS_VERSION_SESSION), SSL_get_version((SSL *)net->vio->ssl_arg),
                        sctx->priv_user().str, sctx->priv_host().str, sctx->host_or_ip().str, sctx->user().str);
    }
  }
...
static bool login_connection(THD *thd)
{
  ...
  DBUG_PRINT("info", ("login_connection called by thread %u", thd->thread_id()));
  ...
  if (net->error && net->vio != 0)
  {
    if (!thd->killed)
    {
      ...
      sql_print_information(
          ER(ER_NEW_ABORTING_CONNECTION), thd->thread_id(), (thd->db().str ? thd->db().str : "unconnected"),
          sctx_user.str ? sctx_user.str : "unauthenticated", sctx->host_or_ip().str,
          (thd->get_stmt_da()->is_error() ? thd->get_stmt_da()->message_text() : ER(ER_UNKNOWN_ERROR)));
    }
  }
}
...
static void prepare_new_connection_state(THD *thd)
{
  ...
  */
  ...
  if (opt_init_connect.length && !(sctx->check_access(SUPER_ACL)))
  {
    ...
    {
      sql_print_warning(
          "init_connect variable is ignored for user: %s "
          "host: %s due to expired password.",
    ...
    if (thd->is_error())
    {
      ...
      sql_print_warning(ER(ER_NEW_ABORTING_CONNECTION), thd->thread_id(), thd->db().str ? thd->db().str : "unconnected",
                        sctx_user.str ? sctx_user.str : "unauthenticated", sctx->host_or_ip().str,
...
bool thd_prepare_connection(THD *thd)
{
  ...
  MYSQL_CONNECTION_START(thd->thread_id(), (char *)&thd->security_context()->priv_user().str[0],
...
void close_connection(THD *thd, uint sql_errno, bool server_shutdown, bool generate_event)
{
  ...
  if (generate_event)
    mysql_audit_notify(thd, AUDIT_EVENT(MYSQL_AUDIT_CONNECTION_DISCONNECT), sql_errno);
