...
static void dbug_print_table(const char *info, TABLE *table)
{
  ...
  DBUG_PRINT("info", ("%s: %s.%s s->fields: %d  "
                      "reclength: %lu  rec_buff_length: %u  record[0]: 0x%lx  "
                      "record[1]: 0x%lx",
                      info, table->s->db.str, table->s->table_name.str, table->s->fields, table->s->reclength,
                      table->s->rec_buff_length, (long)table->record[0], (long)table->record[1]));
  ...
  for (unsigned int i = 0; i < table->s->fields; i++)
  {
    ...
    DBUG_PRINT("info",
               ("[%d] \"%s\"(0x%lx:%s%s%s%s%s%s) type: %d  pack_length: %d  "
                ...
                i, f->field_name, (long)f->flags, (f->flags & PRI_KEY_FLAG) ? "pri" : "attr",
                (f->flags & NOT_NULL_FLAG) ? "" : ",nullable", (f->flags & UNSIGNED_FLAG) ? ",unsigned" : ",signed",
                (f->flags & ZEROFILL_FLAG) ? ",zerofill" : "", (f->flags & BLOB_FLAG) ? ",blob" : "",
                (f->flags & BINARY_FLAG) ? ",binary" : "", f->real_type(), f->pack_length(), (long)f->ptr,
                (int)(f->ptr - table->record[0]), f->null_bit, (long)f->null_offset(0), (int)f->null_offset()));
    if (f->type() == MYSQL_TYPE_BIT)
    {
      Field_bit *g = (Field_bit *)f;
      DBUG_PRINT("MYSQL_TYPE_BIT", ("field_length: %d  bit_ptr: 0x%lx[+%d] "
                                    "bit_ofs: %d  bit_len: %u",
                                    g->field_length, (long)g->bit_ptr, (int)((uchar *)g->bit_ptr - table->record[0]),
                                    g->bit_ofs, g->bit_len));
    }
  }
}
...
static void run_query(THD *thd, char *buf, char *end, const int *no_print_error)
{
  /*
  ...
  */
  (void)mysqld.raw_run_query(buf, (end - buf), no_print_error);
}

static void ndb_binlog_close_shadow_table(NDB_SHARE *share)
{
...
static int ndb_binlog_open_shadow_table(THD *thd, NDB_SHARE *share)
{
  ...
  *root_ptr = &event_data->mem_root;
  ...
  TABLE_SHARE *shadow_table_share = (TABLE_SHARE *)alloc_root(&event_data->mem_root, sizeof(TABLE_SHARE));
  TABLE *shadow_table = (TABLE *)alloc_root(&event_data->mem_root, sizeof(TABLE));

  init_tmp_table_share(thd, shadow_table_share, share->db, 0, share->table_name, share->key_string());
  if ((error = open_table_def(thd, shadow_table_share, 0)) ||
      (error = open_table_from_share(thd, shadow_table_share, "", 0,
                                     (uint)(OPEN_FRM_FILE_ONLY | DELAYED_OPEN | READ_ALL), 0, shadow_table, false)))
  {
    DBUG_PRINT("error", ("failed to open shadow table, error: %d my_errno: %d", error, my_errno()));
  ...
  lex_string_copy(&event_data->mem_root, &shadow_table->s->db, share->db);
  lex_string_copy(&event_data->mem_root, &shadow_table->s->table_name, share->table_name);

  /* We can't use 'use_all_columns()' as the file object is not setup yet */
  shadow_table->column_bitmaps_set_no_signal(&shadow_table->s->all_set, &shadow_table->s->all_set);
...
static int get_ndb_blobs_value(TABLE *table, NdbValue *value_array, uchar *&buffer, uint &buffer_size,
      ...
      if (isNull == 0)
      {
        ...
        if (loop == 1)
        {
          ...
          if (ndb_blob->readData(buf, len) != 0)
            DBUG_RETURN(-1);
          DBUG_PRINT("info",
                     ("[%u] offset: %u  buf: 0x%lx  len=%u  [ptrdiff=%d]", i, offset, (long)buf, len, (int)ptrdiff));
          assert(len == len64);
...
static void ndbcluster_binlog_wait(THD *thd)
{
  if (ndb_binlog_running)
  {
    ...
    assert(thd);
    assert(thd_sql_command(thd) == SQLCOM_SHOW_BINLOG_EVENTS || thd_sql_command(thd) == SQLCOM_FLUSH ||
    ...
    */
     ...
     */
    const Uint64 session_last_committed_epoch = thd_ndb->m_last_commit_epoch_session;
    ...
    while (!thd->killed && count && ndb_binlog_running &&
           (ndb_latest_handled_binlog_epoch == 0 || ndb_latest_handled_binlog_epoch < session_last_committed_epoch))
    {
      count--;
    ...
    {
      sql_print_warning(
          "NDB: Thread id %u timed out (30s) waiting for epoch %u/%u "
          "to be handled.  Progress : %u/%u -> %u/%u.",
          thd->thread_id(), Uint32((session_last_committed_epoch >> 32) & 0xffffffff),
          Uint32(session_last_committed_epoch & 0xffffffff), Uint32((start_handled_epoch >> 32) & 0xffffffff),
          Uint32(start_handled_epoch & 0xffffffff), Uint32((ndb_latest_handled_binlog_epoch >> 32) & 0xffffffff),
...
static int ndbcluster_reset_logs(THD *thd)
{
  ...
  if (!((thd->lex->sql_command == SQLCOM_RESET) && (thd->lex->type & REFRESH_MASTER)))
    return 0;
  ...
  */
  ...
  if (mysqld.truncate_table(STRING_WITH_LEN("mysql"), STRING_WITH_LEN("ndb_binlog_index"), ignore_no_such_table))
  {
...
THD *ndb_create_thd(char *stackptr)
{
  ...
  CHARSET_INFO *charset_connection = get_charset_by_csname("utf8", MY_CS_PRIMARY, MYF(MY_WME));
...
static int ndbcluster_binlog_index_purge_file(THD *passed_thd, const char *file)
{
  int stack_base = 0;
  int error = 0;
  ...
  if ((my_thd = ndb_create_thd((char *)&stack_base) /* stack ptr */) == 0)
  {
     ...
     */
    sql_print_warning("NDB: Unable to purge " NDB_REP_DB "." NDB_REP_TABLE " File=%s (failed to setup thd)", file);
    DBUG_RETURN(0);
  }
  ...
  */
  ...
  const bool ignore_no_such_table = true;
  if (mysqld.delete_rows(STRING_WITH_LEN("mysql"), STRING_WITH_LEN("ndb_binlog_index"), ignore_no_such_table, "File='",
                         file, "'", NULL))
  {
...
bool Ndb_dist_priv_util::priv_tables_are_in_ndb(THD *thd)
{
...
static void ndbcluster_binlog_log_query(handlerton *hton, THD *thd, enum_binlog_command binlog_command,
                                        const char *query, uint query_length, const char *db, const char *table_name)
{
  DBUG_ENTER("ndbcluster_binlog_log_query");
  DBUG_PRINT("enter", ("db: %s  table_name: %s  query: %s", db, table_name, query));
  ...
  ndbcluster_log_schema_op(thd, query, query_length, db, table_name, table_id, table_version, type, NULL, NULL);
  DBUG_VOID_RETURN;
}
...
static void ndbcluster_reset_slave(THD *thd)
{
  ...
  */
  Ndb_local_connection mysqld(thd);
  const bool ignore_no_such_table = true;
  if (mysqld.delete_rows(STRING_WITH_LEN("mysql"), STRING_WITH_LEN("ndb_apply_status"), ignore_no_such_table, NULL))
  {
    // Failed to delete rows from table
    error = 1;
  }
...
static int ndbcluster_binlog_func(handlerton *hton, THD *thd, enum_binlog_func fn, void *arg)
{
...
*/
static size_t ndb_open_tables__create_key(char *key_buf, size_t key_buf_length, const char *db, size_t db_length,
                                          const char *table, size_t table_length)
{
  size_t key_length = my_snprintf(key_buf, key_buf_length, "./%*s/%*s", db_length, db, table_length, table) - 1;
  assert(key_length > 0);
...
*/
static bool ndb_open_tables__is_table_open(const char *db, size_t db_length, const char *table, size_t table_length)
{
  char key[FN_REFLEN + 1];
  size_t key_length = ndb_open_tables__create_key(key, sizeof(key), db, db_length, table, table_length);
  DBUG_ENTER("ndb_open_tables__is_table_open");
  DBUG_PRINT("enter", ("db: '%s', table: '%s', key: '%s'", db, table, key));

  native_mutex_lock(&ndbcluster_mutex);
  bool result = my_hash_search(&ndbcluster_open_tables, (const uchar *)key, key_length) != NULL;
...
{
  return ndb_open_tables__is_table_open(STRING_WITH_LEN("mysql"), STRING_WITH_LEN("ndb_schema"));
}

static bool ndbcluster_check_ndb_apply_status_share()
{
  return ndb_open_tables__is_table_open(STRING_WITH_LEN("mysql"), STRING_WITH_LEN("ndb_apply_status"));
}

static bool create_cluster_sys_table(THD *thd, const char *db, size_t db_length, const char *table, size_t table_length,
                                     const char *create_definitions, const char *create_options)
{
  ...
  */
  {
    ...
    build_table_filename(path, sizeof(path) - 1, db, table, reg_ext, 0);
    if (my_delete(path, MYF(0)) == 0)
    {
      ...
      */
      ...
      (void)mysqld.flush_table(db, db_length, table, table_length);
    }
  }

  const bool create_if_not_exists = true;
  const bool res = mysqld.create_sys_table(db, db_length, table, table_length, create_if_not_exists, create_definitions,
                                           create_options);
  return res;
}
...
static bool ndb_apply_table__create(THD *thd)
{
  ...
  const bool res = create_cluster_sys_table(thd, STRING_WITH_LEN("mysql"), STRING_WITH_LEN("ndb_apply_status"),
                                            // table_definition
...
static bool ndb_schema_table__create(THD *thd)
{
  ...
  /* NOTE! Updating this table schema must be reflected in ndb_restore */
  const bool res = create_cluster_sys_table(thd, STRING_WITH_LEN("mysql"), STRING_WITH_LEN("ndb_schema"),
                                            // table_definition
                                            "db VARBINARY(" NDB_MAX_DDL_NAME_BYTESIZE_STR
                                            ") NOT NULL,"
                                            "name VARBINARY(" NDB_MAX_DDL_NAME_BYTESIZE_STR
                                            ") NOT NULL,"
...
static void clean_away_stray_files(THD *thd)
{
  ...
  while ((db_name = db_names.next()))
  {
    ...
    {
      sql_print_information("NDB: Cleaning stray tables from database '%s'", db_name->str);
...
static int ndbcluster_find_all_databases(THD *thd)
{
  ...
  */
  ...
  while (1)
  {
    ...
    op->readTuples(NdbScanOperation::LM_Read, NdbScanOperation::SF_TupScan, 1);
      ...
      if (db_len > 0 && name_len == 0)
      {
        ...
        if (native_strncasecmp("CREATE", query, 6) == 0)
        {
          ...
          if (!database_exists)
          {
            ...
            run_query(thd, query, query + query_length, no_print_error);
          }
        }
        else if (native_strncasecmp("ALTER", query, 5) == 0)
        {
          ...
          if (!database_exists)
          {
            ...
            run_query(thd, name, name + name_len, no_print_error);
            run_query(thd, query, query + query_length, no_print_error);
          }
        }
  ...
  error:
    ...
    if (ndb_error.status == NdbError::TemporaryError && !thd->killed)
    {
      if (retries--)
      {
        sql_print_warning("NDB: ndbcluster_find_all_databases retry: %u - %s", ndb_error.code, ndb_error.message);
    ...
    if (!thd->killed)
    {
      sql_print_error("NDB: ndbcluster_find_all_databases fail: %u - %s", ndb_error.code, ndb_error.message);
    }
...
static int ndbcluster_find_all_files(THD *thd)
{
  ...
  do
  {
    ...
    for (uint i = 0; i < list.count; i++)
    {
      ...
      if (elmt.state != NDBOBJ::StateOnline && elmt.state != NDBOBJ::StateBackup && elmt.state != NDBOBJ::StateBuilding)
      {
        sql_print_information("NDB: skipping setup table %s.%s, in state %d", elmt.database, elmt.name, elmt.state);
        skipped++;
        continue;
      }
      ...
      {
        if (retries == 0)
          sql_print_error("NDB: failed to setup table %s.%s, error: %d, %s", elmt.database, elmt.name,
      ...
      char *end = key + build_table_filename(key, sizeof(key) - 1, elmt.database, "", "", 0);
      ...
      end += tablename_to_filename(elmt.name, end, (uint)(sizeof(key) - (end - key)));
      uchar *data = 0, *pack_data = 0;
      size_t length, pack_length;
      int discover = 0;
      if (readfrm(key, &data, &length) || packfrm(data, length, &pack_data, &pack_length))
      {
        discover = 1;
        sql_print_information("NDB: missing frm for %s.%s, discovering...", elmt.database, elmt.name);
      }
      ...
      else
      {
        ...
        ndbcluster_create_binlog_setup(thd, ndb, key, (uint)(end - key), elmt.database, elmt.name, 0);
      }
    }
...
bool ndb_binlog_setup(THD *thd)
{
  ...
  */
  ...
  if (!ndb_schema_share && ndbcluster_check_ndb_schema_share() == 0)
  {
  ...
  if (!ndb_apply_status_share && ndbcluster_check_ndb_apply_status_share() == 0)
  {
...
*/
static void ndb_report_waiting(const char *key, int the_time, const char *op, const char *obj, const MY_BITMAP *map)
{
  ...
  if (map == 0)
  {
    sql_print_information(
        ...
        key, the_time, op, obj, (uint)(ndb_latest_handled_binlog_epoch >> 32), (uint)(ndb_latest_handled_binlog_epoch),
        (uint)(ndb_latest_received_binlog_epoch >> 32), (uint)(ndb_latest_received_binlog_epoch),
  ...
  else
  {
    sql_print_information(
        ...
        key, the_time, op, obj, (uint)(ndb_latest_handled_binlog_epoch >> 32), (uint)(ndb_latest_handled_binlog_epoch),
        (uint)(ndb_latest_received_binlog_epoch >> 32), (uint)(ndb_latest_received_binlog_epoch),
        (uint)(ndb_latest_epoch >> 32), (uint)(ndb_latest_epoch), proc_info, map->bitmap[0], map->bitmap[1]);
  }
}
...
int ndbcluster_log_schema_op(THD *thd, const char *query, int query_length, const char *db, const char *table_name,
                             ...
                             const char *new_db, const char *new_table_name, bool log_query_on_participant)
{
  ...
  DBUG_PRINT("enter",
             ("query: %s  db: %s  table_name: %s  thd_ndb->options: %d", query, db, table_name, thd_ndb->options));
  ...
  if (strlen(db) > NDB_MAX_DDL_NAME_BYTESIZE)
  {
    ...
    assert(type == SOT_CREATE_DB || type == SOT_ALTER_DB || type == SOT_DROP_DB);
  ...
  switch (type)
  {
    case SOT_DROP_TABLE:
      ...
      */
      ...
      query = tmp_buf2;
      id_length = my_strmov_quoted_identifier(thd, (char *)quoted_table1, table_name, 0);
      quoted_table1[id_length] = '\0';
      id_length = my_strmov_quoted_identifier(thd, (char *)quoted_db1, db, 0);
      quoted_db1[id_length] = '\0';
      query_length = (uint)(strxmov(tmp_buf2, "drop table ", quoted_db1, ".", quoted_table1, NullS) - tmp_buf2);
      type_str = "drop table";
      break;
    ...
    case SOT_RENAME_TABLE:
      ...
      */
      ...
      id_length = my_strmov_quoted_identifier(thd, (char *)quoted_table1, table_name, 0);
      quoted_table1[id_length] = '\0';
      id_length = my_strmov_quoted_identifier(thd, (char *)quoted_db2, new_db, 0);
      quoted_db2[id_length] = '\0';
      id_length = my_strmov_quoted_identifier(thd, (char *)quoted_table2, new_table_name, 0);
      quoted_table2[id_length] = '\0';
      query_length = (uint)(strxmov(tmp_buf2, "rename table ", quoted_db1, ".", quoted_table1, " to ", quoted_db2, ".",
                                    quoted_table2, NullS) -
                            tmp_buf2);
  ...
  if (ndbtab == 0)
  {
    if (strcmp(NDB_REP_DB, db) != 0 || strcmp(NDB_SCHEMA_TABLE, table_name))
    {
      ndb_error = &dict->getNdbError();
    }
    goto end;
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      assert(r == 0);
      /* name */
      ndb_pack_varchar(col[SCHEMA_NAME_I], tmp_buf, log_tab, (int)strlen(log_tab));
      r |= op->equal(SCHEMA_NAME_I, tmp_buf);
      assert(r == 0);
      /* slock */
      assert(sz[SCHEMA_SLOCK_I] == no_bytes_in_map(&ndb_schema_object->slock_bitmap));
      ...
      else
      {
        ...
        */
        DBUG_PRINT("info", ("Replicated schema event with original server id %d", thd->server_id));
    ...
    if (trans->execute(NdbTransaction::Commit, NdbOperation::DefaultAbortOption, 1 /* force send */) == 0)
    {
  ...
  err:
    const NdbError *this_error = trans ? &trans->getNdbError() : &ndb->getNdbError();
  ...
  if (ndb_error)
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER(ER_GET_ERRMSG), ndb_error->code,
                        ndb_error->message, "Could not log query '%s' on other mysqld's");
  ...
  {
    sql_print_information("NDB: distributed %s.%s(%u/%u) type: %s(%u) query: \'%s\' to %x%x", db, table_name,
                          ndb_table_id, ndb_table_version, get_schema_type_name(log_type), log_type, query,
                          ndb_schema_object->slock_bitmap.bitmap[0], ndb_schema_object->slock_bitmap.bitmap[1]);
  }
  ...
  */
  if (ndb_error == 0 && !bitmap_is_clear_all(&ndb_schema_object->slock_bitmap))
  {
    ...
    while (1)
    {
      ...
      native_mutex_lock(&injector_mutex);
      int ret = native_cond_timedwait(&injector_cond, &injector_mutex, &abstime);
      native_mutex_unlock(&injector_mutex);
      native_mutex_lock(&ndb_schema_object->mutex);
      ...
      if (ret)
      {
        ...
        if (max_timeout == 0)
        {
          sql_print_error("NDB %s: distributing %s timed out. Ignoring...", type_str, ndb_schema_object->key);
          assert(false);
          break;
        }
        if (opt_ndb_extra_logging)
          ndb_report_waiting(type_str, max_timeout, "distributing", ndb_schema_object->key,
  ...
  {
    sql_print_error("NDB %s: distributing %s err: %u", type_str, ndb_schema_object->key, ndb_error->code);
  }
  else if (opt_ndb_extra_logging > 19)
  {
    sql_print_information("NDB %s: not waiting for distributing %s", type_str, ndb_schema_object->key);
  }
  ...
  if (opt_ndb_extra_logging > 19)
  {
    sql_print_information(
        ...
        " - complete!",
        db, table_name, ndb_table_id, ndb_table_version, get_schema_type_name(log_type), log_type, query);
  }
...
static int ndb_handle_schema_change(THD *thd, Ndb *is_ndb, NdbEventOperation *pOp, const Ndb_event_data *event_data)
{
  DBUG_ENTER("ndb_handle_schema_change");
  DBUG_PRINT("enter", ("pOp: %p", pOp));

  // Only called for TE_DROP and TE_CLUSTER_FAILURE event
  assert(pOp->getEventType() == NDBEVENT::TE_DROP || pOp->getEventType() == NDBEVENT::TE_CLUSTER_FAILURE);
  ...
  assert(share->state == NSS_DROPPED || share->op == pOp || share->new_op == pOp);
  if (share->new_op)
  {
    share->new_op = 0;
  }
  ...
  DBUG_PRINT("NDB_SHARE", ("%s binlog free  use_count: %u", share->key_string(), share->use_count));
 ...
 public:
  ...
  void init(Ndb_cluster_connection *cluster_connection)
  {
...

    {
      ...
      subscriber_bitmap = (MY_BITMAP *)my_malloc(PSI_INSTRUMENT_ME, no_nodes * sizeof(MY_BITMAP), MYF(MY_WME));
      for (unsigned i = 0; i < no_nodes; i++)
      {
        bitmap_init(&subscriber_bitmap[i], (Uint32 *)my_malloc(PSI_INSTRUMENT_ME, max_ndb_nodes / 8, MYF(MY_WME)),
                    max_ndb_nodes, FALSE);
  ...
  {
    assert(data_node_id < (sizeof(m_data_node_id_list) / sizeof(m_data_node_id_list[0])));
  ...
  void report_data_node_failure(unsigned data_node_id)
  {
    ...
    {
      sql_print_information(
          "NDB Schema dist: Data node: %d failed,"
          " subscriber bitmask %x%x",
          data_node_id, subscriber_bitmap[idx].bitmap[1], subscriber_bitmap[idx].bitmap[0]);
    }
  }
  ...
  void report_subscribe(unsigned data_node_id, unsigned subscriber_node_id)
  {
    ...
    DBUG_PRINT("info", ("Data node %u reported node %u subscribed ", data_node_id, subscriber_node_id));
    if (opt_ndb_extra_logging)
    {
      sql_print_information(
          "NDB Schema dist: Data node: %d reports "
          "subscribe from node %d, subscriber bitmask %x%x",
          data_node_id, subscriber_node_id, subscriber_bitmap[idx].bitmap[1], subscriber_bitmap[idx].bitmap[0]);
    }
  }
  ...
  void report_unsubscribe(unsigned data_node_id, unsigned subscriber_node_id)
  {
    ...
    DBUG_PRINT("info", ("Data node %u reported node %u unsubscribed ", data_node_id, subscriber_node_id));
    if (opt_ndb_extra_logging)
    {
      sql_print_information(
          "NDB Schema dist: Data node: %d reports "
...
class Ndb_schema_event_handler
{
  class Ndb_schema_op
  {
    ...
    void unpack_event(const Ndb_event_data *event_data)
    {
      ...
      {
        ptrdiff_t ptrdiff = 0;
        int ret = get_ndb_blobs_value(table, event_data->ndb_value[0], blobs_buffer, blobs_buffer_size, ptrdiff);
        if (ret != 0)
        {
    ...
    */
    static Ndb_schema_op *create(const Ndb_event_data *event_data, Uint32 any_value)
    {
      DBUG_ENTER("Ndb_schema_op::create");
      Ndb_schema_op *schema_op = (Ndb_schema_op *)sql_alloc(sizeof(Ndb_schema_op));
      bitmap_init(&schema_op->slock, schema_op->slock_buf, 8 * SCHEMA_SLOCK_SIZE, FALSE);
      schema_op->unpack_event(event_data);
      schema_op->any_value = any_value;
      DBUG_PRINT("exit",
                 ("%s.%s: query: '%s'  type: %d", schema_op->db, schema_op->name, schema_op->query, schema_op->type));
      DBUG_RETURN(schema_op);
    }
  };

  static void print_could_not_discover_error(THD *thd, const Ndb_schema_op *schema)
  {
    sql_print_error(
        "NDB Binlog: Could not discover table '%s.%s' from "
        "binlog schema event '%s' from node %d. "
        "my_errno: %d",
        schema->db, schema->name, schema->query, schema->node_id, my_errno());
    thd_print_warning_list(thd, "NDB Binlog");
  }

  static void write_schema_op_to_binlog(THD *thd, Ndb_schema_op *schema)
  {
    if (!ndb_binlog_running)
    {
  ...
  */
  int ack_schema_op(const char *db, const char *table_name, uint32 table_id, uint32 table_version)
  {
    ...
    while (1)
    {
        ...
        goto err;
      {
        ...
        assert(r == 0);
        /* name */
        ndb_pack_varchar(col[SCHEMA_NAME_I], tmp_buf, table_name, (int)strlen(table_name));
        r |= op->equal(SCHEMA_NAME_I, tmp_buf);
        assert(r == 0);
        /* slock */
      ...
      if (opt_ndb_extra_logging > 19)
      {
        ...
        sql_print_information("NDB: reply to %s.%s(%u/%u) from %x%x to %x%x", db, table_name, table_id, table_version,
                              copy[0], copy[1], slock.bitmap[0], slock.bitmap[1]);
      }
      else
      {
        bitmap_clear_bit(&slock, own_nodeid());
      ...
      if (trans->execute(NdbTransaction::Commit, NdbOperation::DefaultAbortOption, 1 /*force send*/) == 0)
      {
        DBUG_PRINT("info", ("node %d cleared lock on '%s.%s'", own_nodeid(), db, table_name));
        dict->forceGCPWait(1);
        break;
      }
    err:
      const NdbError *this_error = trans ? &trans->getNdbError() : &ndb->getNdbError();
      if (this_error->status == NdbError::TemporaryError && !thd_killed(m_thd))
      {
        if (retries--)
        {
          if (trans)
    ...
    {
      sql_print_warning(
          "NDB: Could not release slock on '%s.%s', "
          "Error code: %d Message: %s",
          db, table_name, ndb_error->code, ndb_error->message);
    }
    if (trans)
      ndb->closeTransaction(trans);
    ndb->setDatabaseName(save_db);
  ...
  void handle_after_epoch(Ndb_schema_op *schema)
  {
  ...
  void ack_after_epoch(Ndb_schema_op *schema)
  {
  ...
  void mysqld_close_cached_table(const char *db_name, const char *table_name) const
  {
    ...
    close_cached_tables(m_thd, &table_list, wait_for_refresh, timeout);
    DBUG_VOID_RETURN;
  }

  void mysqld_write_frm_from_ndb(const char *db_name, const char *table_name) const
  {
    ...
    if (!ndbtab)
    {
      ...
      */
      sql_print_error("NDB schema: Could not find table '%s.%s' in NDB", db_name, table_name);
      assert(false);
      DBUG_VOID_RETURN;
    }

    char key[FN_REFLEN];
    build_table_filename(key, sizeof(key) - 1, db_name, table_name, NullS, 0);

    uchar *data = 0, *pack_data = 0;
    size_t length, pack_length;

    if (readfrm(key, &data, &length) == 0 && packfrm(data, length, &pack_data, &pack_length) == 0 &&
        cmp_frm(ndbtab, pack_data, pack_length))
    {
      DBUG_PRINT("info", ("Detected frm change of table %s.%s", db_name, table_name));

      DBUG_DUMP("frm", (uchar *)ndbtab->getFrmData(), ndbtab->getFrmLength());
      my_free(data);
      data = NULL;

      int error;
      if ((error = unpackfrm(&data, &length, (const uchar *)ndbtab->getFrmData())) ||
          (error = writefrm(key, data, length)))
      {
        sql_print_error("NDB: Failed write frm for %s.%s, error %d", db_name, table_name, error);
      }
    }
    my_free(data);
    my_free(pack_data);
  ...
  bool check_if_local_tables_in_db(const char *dbname) const
  {
  ...
  void handle_clear_slock(Ndb_schema_op *schema)
  {
    ...
    if (!ndb_schema_object)
    {
      ...
      if (opt_ndb_extra_logging > 19)
        sql_print_information("NDB: Discarding event...no obj: %s (%u/%u)", key, schema->id, schema->version);
      DBUG_VOID_RETURN;
    }

    if (ndb_schema_object->table_id != schema->id || ndb_schema_object->table_version != schema->version)
    {
      ...
      if (opt_ndb_extra_logging > 19)
        sql_print_information(
            ...
            key, ndb_schema_object->table_id, ndb_schema_object->table_version, schema->id, schema->version);
      ndb_free_schema_object(&ndb_schema_object);
      DBUG_VOID_RETURN;
    }
    ...
    */
    ...
    if (opt_ndb_extra_logging > 19)
    {
      sql_print_information(
          ...
          " %x%x to %x%x",
          key, schema->id, schema->version, ndb_schema_object->slock[0], ndb_schema_object->slock[1],
          schema->slock_buf[0], schema->slock_buf[1]);
    }
    memcpy(ndb_schema_object->slock, schema->slock_buf, sizeof(ndb_schema_object->slock));
    DBUG_DUMP("ndb_schema_object->slock_bitmap.bitmap", (uchar *)ndb_schema_object->slock_bitmap.bitmap,
  ...
  void handle_offline_alter_table_commit(Ndb_schema_op *schema)
  {
    ...
    if (is_local_table(schema->db, schema->name) &&
        !Ndb_dist_priv_util::is_distributed_priv_table(schema->db, schema->name))
    {
      sql_print_error(
          "NDB Binlog: Skipping locally defined table '%s.%s' "
          "from binlog schema event '%s' from node %d.",
          schema->db, schema->name, schema->query, schema->node_id);
      DBUG_VOID_RETURN;
    }
  ...
  void handle_online_alter_table_prepare(Ndb_schema_op *schema)
  {
    assert(is_post_epoch());  // Always after epoch
  ...
  void handle_online_alter_table_commit(Ndb_schema_op *schema)
  {
    assert(is_post_epoch());  // Always after epoch
    ...
    if (share)
    {
      ...
      {
        sql_print_error("NDB Binlog: Failed to re-open shadow table %s.%s", schema->db, schema->name);
        native_mutex_unlock(&share->mutex);
      }
      else
      {
        ...
        */
        ...
        ndb_rep_event_name(&event_name, schema->db, schema->name, get_binlog_full(share));
  ...
  void handle_drop_table(Ndb_schema_op *schema)
  {
    ...
    if (tab.is_local_table())
    {
      ...
      sql_print_error(
          "NDB Binlog: Skipping drop of locally "
  ...
  */
  ...
  void handle_rename_table_prepare(Ndb_schema_op *schema)
  {
    ...
    {
      NDB_SHARE_KEY *old_prepared_key = m_schema_dist_data.get_prepared_rename_key();
      if (old_prepared_key)
  ...
  void handle_rename_table(Ndb_schema_op *schema)
  {
    ...
    from.rename_table(NDB_SHARE::key_get_db_name(prepared_key), NDB_SHARE::key_get_table_name(prepared_key));
  ...
  void handle_drop_db(Ndb_schema_op *schema)
  {
    ...
    const int no_print_error[1] = {0};
    run_query(m_thd, schema->query, schema->query + schema->query_length, no_print_error);

    DBUG_VOID_RETURN;
  }

  void handle_truncate_table(Ndb_schema_op *schema)
  {
    ...
    {
      sql_print_error(
          "NDB Binlog: Skipping locally defined table "
  ...
  void handle_create_table(Ndb_schema_op *schema)
  {
  ...
  void handle_create_db(Ndb_schema_op *schema)
  {
  ...
  void handle_alter_db(Ndb_schema_op *schema)
  {
  ...
  void handle_grant_op(Ndb_schema_op *schema)
  {
    ...
    run_query(m_thd, cmd, cmd + strlen(cmd), no_print_error);

    DBUG_VOID_RETURN;
  }

  int handle_schema_op(Ndb_schema_op *schema)
  {
    DBUG_ENTER("handle_schema_op");
    {
      ...
      if (opt_ndb_extra_logging > 19)
      {
        sql_print_information("NDB: got schema event on %s.%s(%u/%u) query: '%s' type: %s(%d) node: %u slock: %x%x",
                              schema->db, schema->name, schema->id, schema->version, schema->query,
                              get_schema_type_name(schema_type), schema_type, schema->node_id, schema->slock.bitmap[0],
      ...
      switch (schema_type)
      {
        ...
        case SOT_RENAME_TABLE_NEW:
          ...
          */
          sql_print_error(
              "NDB schema: Skipping old schema operation"
      ...
      {
        ack_schema_op(schema->db, schema->name, schema->id, schema->version);
      }
    }
    DBUG_RETURN(0);
  }

  void handle_schema_op_post_epoch(Ndb_schema_op *schema)
  {
    DBUG_ENTER("handle_schema_op_post_epoch");
    DBUG_PRINT("enter", ("%s.%s: query: '%s'  type: %d", schema->db, schema->name, schema->query, schema->type));

    {
      const SCHEMA_OP_TYPE schema_type = (SCHEMA_OP_TYPE)schema->type;
      if (opt_ndb_extra_logging > 9)
        sql_print_information("%s - %s.%s", get_schema_type_name(schema_type), schema->db ? schema->db : "(null)",
 ...
 public:
  ...
  Ndb_schema_event_handler(THD *thd, MEM_ROOT *mem_root, uint own_nodeid, Ndb_schema_dist_data &schema_dist_data)
      : m_thd(thd),
        m_mem_root(mem_root),
  ...
  void handle_event(Ndb *s_ndb, NdbEventOperation *pOp)
  {
    ...
    const Ndb_event_data *event_data = static_cast<const Ndb_event_data *>(pOp->getCustomData());
    ...
    switch (ev_type)
    {
      ...
      case NDBEVENT::TE_UPDATE:
      {
        ...
        Ndb_schema_op *schema_op = Ndb_schema_op::create(event_data, pOp->getAnyValue());
      ...
      case NDBEVENT::TE_CLUSTER_FAILURE:
        if (opt_ndb_extra_logging)
          sql_print_information(
              "NDB Schema dist: cluster failure "
              "at epoch %u/%u.",
              (uint)(pOp->getGCI() >> 32), (uint)(pOp->getGCI()));
        // fall through
      case NDBEVENT::TE_DROP:
        /* ndb_schema table DROPped */
        if (opt_ndb_extra_logging && ndb_binlog_tables_inited && ndb_binlog_running)
          sql_print_information(
      ...
      default:
      {
        sql_print_error("NDB Schema dist: unknown event %u, ignoring...", ev_type);
      }
    }
...
*/
static int ndb_binlog_index_table__open(THD *thd, TABLE **ndb_binlog_index)
{
  const char *save_proc_info = thd_proc_info(thd, "Opening " NDB_REP_DB "." NDB_REP_TABLE);
  ...
  const uint flags = MYSQL_LOCK_IGNORE_TIMEOUT; /* Wait for lock "infinitely" */
  if (open_and_lock_tables(thd, &tables, flags))
  {
    if (thd->killed)
      DBUG_PRINT("error", ("NDB Binlog: Opening ndb_binlog_index: killed"));
    else
      sql_print_error("NDB Binlog: Opening ndb_binlog_index: %d, '%s'", thd->get_stmt_da()->mysql_errno(),
...
*/
static int ndb_binlog_index_table__write_rows(THD *thd, ndb_binlog_index_row *row)
{
  int error = 0;
  ...
  */
  ...
  do
  {
    ...
    ndb_binlog_index->field[NBICOL_START_POS]->store(first->start_master_log_pos, true);
    ndb_binlog_index->field[NBICOL_START_FILE]->store(first->start_master_log_file,
    ...
    if (ndb_binlog_index->s->fields > NBICOL_ORIG_SERVERID)
    {
       ...
       * separately
       */
      ndb_binlog_index->field[NBICOL_NUM_INSERTS]->store(row->n_inserts, true);
      ndb_binlog_index->field[NBICOL_NUM_UPDATES]->store(row->n_updates, true);
      ndb_binlog_index->field[NBICOL_NUM_DELETES]->store(row->n_deletes, true);
      ndb_binlog_index->field[NBICOL_NUM_SCHEMAOPS]->store(row->n_schemaops, true);
      ndb_binlog_index->field[NBICOL_ORIG_SERVERID]->store(orig_server_id = row->orig_server_id, true);
      ndb_binlog_index->field[NBICOL_ORIG_EPOCH]->store(orig_epoch = row->orig_epoch, true);
      ...
      if (ndb_binlog_index->s->fields > NBICOL_NEXT_POS)
      {
        ...
        ndb_binlog_index->field[NBICOL_NEXT_POS]->store(first->next_master_log_pos, true);
        ndb_binlog_index->field[NBICOL_NEXT_FILE]->store(first->next_master_log_file,
    ...
    else
    {
      ...
      ndb_binlog_index->field[NBICOL_NUM_INSERTS]->store((ulonglong)first->n_inserts, true);
      ndb_binlog_index->field[NBICOL_NUM_UPDATES]->store((ulonglong)first->n_updates, true);
      ndb_binlog_index->field[NBICOL_NUM_DELETES]->store((ulonglong)first->n_deletes, true);
      ndb_binlog_index->field[NBICOL_NUM_SCHEMAOPS]->store((ulonglong)first->n_schemaops, true);
    }
    ...
    {
      sql_print_error(
          "NDB Binlog: Failed writing to ndb_binlog_index for epoch %u/%u "
          " orig_server_id %u orig_epoch %u/%u "
          "with error %d.",
          uint(epoch >> 32), uint(epoch), orig_server_id, uint(orig_epoch >> 32), uint(orig_epoch), error);
      ...
      do
      {
        char tmp[128];
        if (ndb_binlog_index->s->fields > NBICOL_ORIG_SERVERID)
          my_snprintf(tmp, sizeof(tmp), "%u/%u,%u,%u/%u", uint(epoch >> 32), uint(epoch), uint(cursor->orig_server_id),
        ...
        sql_print_error("NDB Binlog: Writing row (%s) to ndb_binlog_index - %s", tmp,
...
int ndbcluster_binlog_start()
{
  ...
  */
  ...
  {
    sql_print_error(
        "NDB: --ndb-log-transaction-id requires v2 Binlog row events "
...
**************************************************************/
void ndb_rep_event_name(String *event_name, const char *db, const char *tbl, my_bool full)
{
  if (full)
...
static void set_binlog_flags(NDB_SHARE *share, Ndb_binlog_type ndb_binlog_type)
{
...
*/
int ndbcluster_get_binlog_replication_info(THD *thd, Ndb *ndb, const char *db, const char *table_name, uint server_id,
                                           Uint32 *binlog_flags, const st_conflict_fn_def **conflict_fn,
                                           st_conflict_fn_arg *args, Uint32 *num_args)
{
  ...
  {
    if (strcmp(db, NDB_REP_DB) == 0 && strcmp(table_name, NDB_APPLY_TABLE) == 0)
    {
      /*
  ...
  int rc = rep_tab_reader.lookup(ndb, db, table_name, server_id);

  const char *msg = rep_tab_reader.get_warning_message();
  if (msg != NULL)
  {
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_NDB_REPLICATION_SCHEMA_ERROR,
                        ER(ER_NDB_REPLICATION_SCHEMA_ERROR), msg);
    sql_print_warning("NDB Binlog: %s", msg);
  }
  ...
  *binlog_flags = rep_tab_reader.get_binlog_flags();
  ...
  if (conflict_fn_spec != NULL)
  {
    ...
    if (parse_conflict_fn_spec(conflict_fn_spec, conflict_fn, args, num_args, msgbuf, sizeof(msgbuf)) != 0)
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_CONFLICT_FN_PARSE_ERROR, ER(ER_CONFLICT_FN_PARSE_ERROR),
                          msgbuf);
      ...
      */
      if (opt_ndb_extra_logging)
      {
        sql_print_warning("NDB Slave: Table %s.%s : Parse error on conflict fn : %s", db, table_name, msgbuf);
      }
...
int ndbcluster_apply_binlog_replication_info(THD *thd, NDB_SHARE *share, const NDBTAB *ndbtab,
                                             const st_conflict_fn_def *conflict_fn, const st_conflict_fn_arg *args,
                                             Uint32 num_args, bool do_set_binlog_flags, Uint32 binlog_flags)
{
  ...
  if (conflict_fn != NULL)
  {
    if (setup_conflict_fn(get_thd_ndb(thd)->ndb, &share->m_cfn_share, share->db, share->table_name,
                          ((share->flags & NSF_BLOB_FLAG) != 0), get_binlog_use_update(share), ndbtab, tmp_buf,
                          sizeof(tmp_buf), conflict_fn, args, num_args) == 0)
    {
      if (opt_ndb_extra_logging)
      {
    ...
    else
    {
      /*
        ...
        ignored
      */
      sql_print_warning("NDB Slave: Table %s.%s : %s", share->db, share->table_name, tmp_buf);
...
int ndbcluster_read_binlog_replication(THD *thd, Ndb *ndb, NDB_SHARE *share, const NDBTAB *ndbtab, uint server_id,
                                       bool do_set_binlog_flags)
{
  ...
  if ((ndbcluster_get_binlog_replication_info(thd, ndb, share->db, share->table_name, server_id, &binlog_flags,
                                              &conflict_fn, args, &num_args) != 0) ||
      (ndbcluster_apply_binlog_replication_info(thd, share, ndbtab, conflict_fn, args, num_args, do_set_binlog_flags,
                                                binlog_flags) != 0))
  {
    DBUG_RETURN(-1);
  }
...
/*
  ...
  create/discover.
*/
int ndbcluster_create_binlog_setup(THD *thd, Ndb *ndb, const char *key, uint key_len, const char *db,
                                   const char *table_name, TABLE *table)
{
  DBUG_ENTER("ndbcluster_create_binlog_setup");
  DBUG_PRINT("enter", ("key: %s  key_len: %d  %s.%s", key, key_len, db, table_name));
  ...
  while (share && !IS_TMP_PREFIX(table_name))
  {
    ...
    if (ndbtab == 0)
    {
      if (opt_ndb_extra_logging)
        sql_print_information(
            ...
            "%s, %d",
            key, dict->getNdbError().message, dict->getNdbError().code);
      break;  // error
    }
#ifdef HAVE_NDB_BINLOG
    /*
    ...
    */
    if ((share->flags & NSF_HIDDEN_PK) && (share->flags & NSF_BLOB_FLAG) && !(share->flags & NSF_NO_BINLOG))
    {
    ...
    {
      if (opt_ndb_extra_logging)
        sql_print_information("NDB Binlog: NOT logging %s", share->key_string());
      native_mutex_unlock(&share->mutex);
      DBUG_RETURN(0);
    }
    ...
    */
    ...
    else
    {
      delete ev;
      if (opt_ndb_extra_logging)
        sql_print_information("NDB Binlog: DISCOVER TABLE Event: %s", event_name.c_ptr());
    }
...
int ndbcluster_create_event(THD *thd, Ndb *ndb, const NDBTAB *ndbtab, const char *event_name, NDB_SHARE *share,
                            int push_warning)
{
  DBUG_ENTER("ndbcluster_create_event");
  DBUG_PRINT("enter", ("table: '%s', version: %d", ndbtab->getName(), ndbtab->getObjectVersion()));
  DBUG_PRINT("enter", ("event: '%s', share->key: '%s'", event_name, share->key_string()));
  ...
  if (share->flags & NSF_HIDDEN_PK)
  {
    ...
    my_event.setReport((NDBEVENT::EventReport)(NDBEVENT::ER_ALL | NDBEVENT::ER_DDL));
    DBUG_PRINT("info", ("subscription all"));
  }
  else
  {
    if (strcmp(share->db, NDB_REP_DB) == 0 && strcmp(share->table_name, NDB_SCHEMA_TABLE) == 0)
    {
      /**
       * ER_SUBSCRIBE is only needed on NDB_SCHEMA_TABLE
       */
      my_event.setReport((NDBEVENT::EventReport)(NDBEVENT::ER_ALL | NDBEVENT::ER_SUBSCRIBE | NDBEVENT::ER_DDL));
    ...
    else
    {
      ...
      {
        my_event.setReport((NDBEVENT::EventReport)(NDBEVENT::ER_UPDATED | NDBEVENT::ER_DDL));
  ...
  if (dict->createEvent(my_event))  // Add event to database
  {
    if (dict->getNdbError().classification != NdbError::SchemaObjectExists)
    {
      ...
      */
      if (push_warning > 1)
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER(ER_GET_ERRMSG), dict->getNdbError().code,
                            dict->getNdbError().message, "NDB");
      sql_print_error(
          "NDB Binlog: Unable to create event in database. "
          "Event: %s  Error Code: %d  Message: %s",
          event_name, dict->getNdbError().code, dict->getNdbError().message);
      DBUG_RETURN(-1);
    }
    ...
    */
    if (dict->getNdbError().code == NDB_INVALID_SCHEMA_OBJECT && dict->dropEvent(my_event.getName(), 1))
    {
      if (push_warning > 1)
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER(ER_GET_ERRMSG), dict->getNdbError().code,
                            dict->getNdbError().message, "NDB");
...
{
  return !(field->flags & BLOB_FLAG) && field->type() != MYSQL_TYPE_BIT && field->pack_length() != 0;
}
...
*/
int ndbcluster_create_event_ops(THD *thd, NDB_SHARE *share, const NDBTAB *ndbtab, const char *event_name)
{
  /*
  ...
  */
  ...
  DBUG_PRINT("enter",
             ("table: '%s' event: '%s', share->key: '%s'", ndbtab->getName(), event_name, share->key_string()));
  ...
  assert(!Ndb_dist_priv_util::is_distributed_priv_table(share->db, share->table_name));

  int do_ndb_schema_share = 0, do_ndb_apply_status_share = 0;
  if (!ndb_schema_share && strcmp(share->db, NDB_REP_DB) == 0 && strcmp(share->table_name, NDB_SCHEMA_TABLE) == 0)
...
#ifdef HAVE_NDB_BINLOG
      if (!binlog_filter->db_ok(share->db) || !ndb_binlog_running || is_exceptions_table(share->table_name))
#endif
  {
    share->flags |= NSF_NO_BINLOG;
    DBUG_RETURN(0);
  ...
  if (share->op)
  {
    ...
    DBUG_PRINT("NDB_SHARE", ("%s ToDo free  use_count: %u", share->key_string(), share->use_count));
  ...
  */
  ...
  while (1)
  {
    ...
    if (!op)
    {
      sql_print_error(
          ...
          " %s",
          event_name);
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER(ER_GET_ERRMSG), ndb->getNdbError().code,
                          ndb->getNdbError().message, "NDB");
      DBUG_RETURN(-1);
    }
    ...
    */
    if (my_multi_malloc(PSI_INSTRUMENT_ME, MYF(MY_WME), &event_data->ndb_value[0], val_length,
    ...
    for (uint j = 0; j < n_columns; j++)
    {
      ...
      if (j < n_fields)
      {
        ...
        if (is_ndb_compatible_type(f))
        {
          ...
          attr1.rec = op->getPreValue(col_name, (f->ptr - table->record[0]) + (char *)table->record[1]);
        }
        ...
        else
        {
          ...
          if (attr0.blob == NULL || attr1.blob == NULL)
          {
            ...
            push_warning_printf(thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER(ER_GET_ERRMSG),
                                op->getNdbError().code, op->getNdbError().message, "NDB");
    ...
    if (op->execute())
    {
      share->op = NULL;
      retries--;
      if (op->getNdbError().status != NdbError::TemporaryError && op->getNdbError().code != 1407)
        retries = 0;
      if (retries == 0)
      {
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER(ER_GET_ERRMSG), op->getNdbError().code,
                            op->getNdbError().message, "NDB");
        sql_print_error("NDB Binlog: ndbevent->execute failed for %s; %d %s", event_name, op->getNdbError().code,
  ...
  get_share(share);
  DBUG_PRINT("NDB_SHARE", ("%s binlog  use_count: %u", share->key_string(), share->use_count));
  if (do_ndb_apply_status_share)
  {
    /* ndb_share reference binlog extra */
    ndb_apply_status_share = get_share(share);
    DBUG_PRINT("NDB_SHARE", ("%s binlog extra  use_count: %u", share->key_string(), share->use_count));
    (void)native_cond_signal(&injector_cond);
  }
  ...
  DBUG_PRINT("info",
             ("%s share->op: 0x%lx  share->use_count: %u", share->key_string(), (long)share->op, share->use_count));

  if (opt_ndb_extra_logging)
    sql_print_information("NDB Binlog: logging %s (%s,%s)", share->key_string(),
                          get_binlog_full(share) ? "FULL" : "UPDATED",
                          get_binlog_use_update(share) ? "USE_UPDATE" : "USE_WRITE");
  DBUG_RETURN(0);
}

int ndbcluster_drop_event(THD *thd, Ndb *ndb, NDB_SHARE *share, const char *dbname, const char *tabname)
{
  ...
  for (uint i = 0; i < 2; i++)
  {
    ...
    if (dict->getNdbError().code != 4710 && dict->getNdbError().code != 1419)
    {
      ...
      sql_print_error(
          ...
          "Event: %s Error Code: %d Message: %s",
          event_name.c_ptr(), dict->getNdbError().code, dict->getNdbError().message);
      /* ToDo; handle error? */
      if (share && share->op && share->op->getState() == NdbEventOperation::EO_EXECUTING &&
...
int ndbcluster_handle_drop_table(THD *thd, Ndb *ndb, NDB_SHARE *share, const char *type_str, const char *dbname,
                                 const char *tabname)
{
...
#define SYNC_DROP_
  ...
  while (share->op)
  {
    ...
    if (ret)
    {
      ...
      if (max_timeout == 0)
      {
        sql_print_error("NDB %s: %s timed out. Ignoring...", type_str, share->key_string());
        assert(false);
        break;
      }
      if (opt_ndb_extra_logging)
        ndb_report_waiting(type_str, max_timeout, type_str, share->key_string(), 0);
    }
  }
  native_mutex_unlock(&share->mutex);
#else
...
static void ndb_unpack_record(TABLE *table, NdbValue *value, MY_BITMAP *defined, uchar *buf)
{
  ...
  */
  ...
  /*
    Set null flag(s)
  */
  for (; field; p_field++, value++, field = *p_field)
  {
    ...
    if ((*value).ptr)
    {
      if (!(field->flags & BLOB_FLAG))
      {
        ...
        else if (field->type() == MYSQL_TYPE_BIT)
        {
          ...
          if (field->pack_length() < 5)
          {
            ...
            field_bit->Field_bit::store((longlong)(*value).rec->u_32_value(), TRUE);
          }
          else
          {
            ...
            field_bit->Field_bit::store(
                (((longlong)*buf) & 0x00000000FFFFFFFFLL) | ((((longlong) * (buf + 1)) << 32) & 0xFFFFFFFF00000000LL),
                TRUE);
#else
            field_bit->Field_bit::store((longlong)(*value).rec->u_64_value(), TRUE);
#endif
          }
          /*
           ...
           */
          field_bit->Field_bit::move_field_offset(-row_offset);
          DBUG_PRINT("info", ("[%u] SET", (*value).rec->getColumn()->getColumnNo()));
      ...
      else
      {
        ...
        else
        {
          ...
          DBUG_PRINT("info", ("[%u] SET ptr: 0x%lx  len: %u", col_no, (long)ptr, len));
#endif
        }
      }
    }
...
static int handle_error(NdbEventOperation *pOp)
{
  ...
  sql_print_error("NDB Binlog: unhandled error %d for table %s", pOp->hasError(), share->key_string());
  pOp->clearError();
  DBUG_RETURN(0);
}
...
static void handle_non_data_event(THD *thd, NdbEventOperation *pOp, ndb_binlog_index_row &row)
{
  const Ndb_event_data *event_data = static_cast<const Ndb_event_data *>(pOp->getCustomData());
  NDB_SHARE *share = event_data->share;
  ...
  DBUG_PRINT("enter", ("pOp: %p, event_data: %p, share: %p", pOp, event_data, share));
  ...
  switch (type)
  {
    case NDBEVENT::TE_CLUSTER_FAILURE:
      if (opt_ndb_extra_logging)
        sql_print_information("NDB Binlog: cluster failure for %s at epoch %u/%u.", share->key_string(),
                              (uint)(pOp->getGCI() >> 32), (uint)(pOp->getGCI()));
      // fallthrough
    case NDBEVENT::TE_DROP:
...
*/
inline ndb_binlog_index_row *ndb_find_binlog_index_row(ndb_binlog_index_row **rows, uint orig_server_id, int flag)
{
...
static int handle_data_event(THD *thd, Ndb *ndb, NdbEventOperation *pOp, ndb_binlog_index_row **rows,
                             injector::transaction &trans, unsigned &trans_row_count, unsigned &trans_slave_row_count)
{
  Ndb_event_data *event_data = (Ndb_event_data *)pOp->getCustomData();
  TABLE *table = event_data->shadow_table;
  NDB_SHARE *share = event_data->share;
      ...
      if (likely(event_has_data))
      {
        ...
        if (opt_ndb_log_apply_status)
        {
          ...
          */
          ...
          {
            bool isFromImmediateMaster = (ndb_apply_status_server_id == ndb_apply_status_logging_server_id);
        ...
        {
          /* store */
          ndb_binlog_index_row *row = ndb_find_binlog_index_row(rows, ndb_apply_status_server_id, 1);
  ...
  */
  ...
  DBUG_EXECUTE_IF("ndb_injector_set_event_conflict_flags", { event_conflict_flags = 0xfafa; });
  ...
  if (erif_flags != 0)
  {
    ...
    else
    {
      ...
      {
        sql_print_error(
            "NDB: Binlog Injector discarding row event "
  ...
  uint n_fields = table->s->fields;
  DBUG_PRINT("info", ("Assuming %u columns for table %s", n_fields, table->s->table_name.str));
  MY_BITMAP b;
  my_bitmap_map bitbuf[(NDB_MAX_ATTRIBUTES_IN_TABLE + 8 * sizeof(my_bitmap_map) - 1) / (8 * sizeof(my_bitmap_map))];
  bitmap_init(&b, bitbuf, n_fields, FALSE);
  bitmap_set_all(&b);
  ...
  */
  ...
  ndb_binlog_index_row *row = ndb_find_binlog_index_row(rows, originating_server_id, 0);
  ...
  switch (pOp->getEventType())
  {
    case NDBEVENT::TE_INSERT:
      ...
      DBUG_PRINT("info", ("INSERT INTO %s.%s", table->s->db.str, table->s->table_name.str));
      {
        int ret;
        if (share->flags & NSF_BLOB_FLAG)
        {
          my_ptrdiff_t ptrdiff = 0;
          ret = get_ndb_blobs_value(table, event_data->ndb_value[0], blobs_buffer[0], blobs_buffer_size[0], ptrdiff);
          assert(ret == 0);
        }
        ndb_unpack_record(table, event_data->ndb_value[0], &b, table->record[0]);
        ret = trans.write_row(logged_server_id, injector::transaction::table(table, true), &b, n_fields,
    ...
    case NDBEVENT::TE_DELETE:
      ...
      DBUG_PRINT("info", ("DELETE FROM %s.%s", table->s->db.str, table->s->table_name.str));
      {
        /*
        ...
        */
        ...
        if (share->flags & NSF_BLOB_FLAG)
        {
          ...
          ret = get_ndb_blobs_value(table, event_data->ndb_value[n], blobs_buffer[n], blobs_buffer_size[n], ptrdiff);
          assert(ret == 0);
        }
        ndb_unpack_record(table, event_data->ndb_value[n], &b, table->record[n]);
        DBUG_EXECUTE("info", print_records(table, table->record[n]););
        ret = trans.delete_row(logged_server_id, injector::transaction::table(table, true), &b, n_fields,
    ...
    case NDBEVENT::TE_UPDATE:
      ...
      DBUG_PRINT("info", ("UPDATE %s.%s", table->s->db.str, table->s->table_name.str));
      {
        int ret;
        if (share->flags & NSF_BLOB_FLAG)
        {
        ...
        if (table->s->primary_key != MAX_KEY && !get_binlog_use_update(share))
        {
          /*
        ...
        else
        {
          ...
          */
          if (share->flags & NSF_BLOB_FLAG)
          {
            ...
            ret = get_ndb_blobs_value(table, event_data->ndb_value[1], blobs_buffer[1], blobs_buffer_size[1], ptrdiff);
            assert(ret == 0);
          }
          ...
          my_bitmap_map
              bitbuf[(NDB_MAX_ATTRIBUTES_IN_TABLE + 8 * sizeof(my_bitmap_map) - 1) / (8 * sizeof(my_bitmap_map))];
          ...
          ret = trans.update_row(logged_server_id, injector::transaction::table(table, true), &col_bitmap_before_update,
                                 &b, n_fields,
...
static void remove_event_operations(Ndb *ndb)
{
  ...
  while ((op = ndb->getEventOperation()))
  {
    ...
    DBUG_PRINT("info", ("removing event operation on %s", op->getEvent()->getName()));
...
void updateInjectorStats(Ndb *schemaNdb, Ndb *dataNdb)
{
   ...
   */
  g_event_data_count =
      schemaNdb->getClientStat(Ndb::DataEventsRecvdCount) + dataNdb->getClientStat(Ndb::DataEventsRecvdCount);
  g_event_nondata_count =
      schemaNdb->getClientStat(Ndb::NonDataEventsRecvdCount) + dataNdb->getClientStat(Ndb::NonDataEventsRecvdCount);
  g_event_bytes_count =
      schemaNdb->getClientStat(Ndb::EventBytesRecvdCount) + dataNdb->getClientStat(Ndb::EventBytesRecvdCount);
}
...
/**
   ...
   marker
*/
static bool injectApplyStatusWriteRow(injector::transaction &trans, ulonglong gci)
{
  ...
  DBUG_EXECUTE_IF("ndb_binlog_injector_cycle_gcis", {
    ...
    gciHi = (gciHi % 3);
    sql_print_warning("NDB Binlog injector cycling gcis (%llu -> %llu)", gci_to_store, (gciHi << 32) + gciLo);
  ...
  DBUG_EXECUTE_IF("ndb_binlog_injector_repeat_gcis", {
    ...
    gciHi = 0xffffff00;
    gciLo = 0;
    sql_print_warning("NDB Binlog injector repeating gcis (%llu -> %llu)", gci_to_store, (gciHi << 32) + gciLo);
    gci_to_store = (gciHi << 32) + gciLo;
  });
#endif
  ...
  */
  ...
  Ndb_event_data *event_data = (Ndb_event_data *)ndb_apply_status_share->op->getCustomData();
  assert(event_data);
  assert(event_data->shadow_table);
  ...
  */
  ...
  ret = trans.write_row(::server_id, injector::transaction::table(apply_status_table, true),
                        &apply_status_table->s->all_set, apply_status_table->s->fields, apply_status_table->record[0]);
...
restart_cluster_failure:
  ...
  if (!(s_ndb = new Ndb(g_ndb_cluster_connection, NDB_REP_DB)) ||
      s_ndb->setNdbObjectName("Ndb Binlog schema change monitoring") || s_ndb->init())
  {
  ...
  log_info("Created schema Ndb object, reference: 0x%x, name: '%s'", s_ndb->getReference(), s_ndb->getNdbObjectName());

  // empty database
  if (!(i_ndb = new Ndb(g_ndb_cluster_connection, "")) ||
      i_ndb->setNdbObjectName("Ndb Binlog data change monitoring") || i_ndb->init())
  {
  ...
  log_info("Created injector Ndb object, reference: 0x%x, name: '%s'", i_ndb->getReference(),
           i_ndb->getNdbObjectName());
  ...
  */
  ...
  while (do_incident && ndb_binlog_running)
  {
    ...
    */
    if (incident_id == 0)
    {
      ...
      uint no = 0;
      if ((sscanf(log_info.log_file_name + len - 6, "%u", &no) == 1) && no == 1)
      {
    ...
    */
    LEX_STRING const msg[2] = {{C_STRING_WITH_LEN("mysqld startup")}, {C_STRING_WITH_LEN("cluster disconnect")}};
    int ret = inj->record_incident(thd, binary_log::Incident_event::INCIDENT_LOST_EVENTS, msg[incident_id]);
    assert(ret == 0);
    NDB_IGNORE_VALUE(ret);
  ...
  incident_id = 1;
  {
    ...
    native_mutex_lock(&injector_mutex);
    while (!ndb_schema_share || (ndb_binlog_running && !ndb_apply_status_share) || !ndb_binlog_tables_inited)
    {
      ...
      if (thd->killed == THD::KILL_CONNECTION)
      {
        ...
        */
        sql_print_information(
            "NDB Binlog: Server shutdown detected while "
...

  {
    ...
    DBUG_PRINT("info", ("schema_res: %d  schema_gci: %u/%u", schema_res, (uint)(schema_gci >> 32), (uint)(schema_gci)));
    {
      ...
      if (schema_gci < ndb_latest_handled_binlog_epoch)
      {
        sql_print_error(
            ...
            (uint)(ndb_latest_handled_binlog_epoch >> 32), (uint)(ndb_latest_handled_binlog_epoch),
            (uint)(schema_gci >> 32), (uint)(schema_gci));
      ...
      if (opt_ndb_extra_logging)
      {
        sql_print_information("NDB Binlog: starting log at epoch %u/%u", (uint)(schema_gci >> 32), (uint)(schema_gci));
      }
    }
  ...
  */
  ...
  for (; !((is_stop_requested() || binlog_thread_state) &&
         ...
         binlog_thread_state != BCCC_restart;)
  {
    ...
    if (binlog_thread_state)
    {
      DBUG_PRINT("info", ("binlog_thread_state: %d, "
                          ...
                          binlog_thread_state, (uint)(ndb_latest_handled_binlog_epoch >> 32),
                          (uint)(ndb_latest_handled_binlog_epoch), (uint)(ndb_get_latest_trans_gci() >> 32),
    ...
    */
    ...
    while (gci > schema_gci && schema_res >= 0)
    {
      ...
      my_snprintf(buf, sizeof(buf), "%s %u/%u(%u/%u)", thd->proc_info, (uint)(schema_gci >> 32), (uint)(schema_gci),
                  (uint)(gci >> 32), (uint)(gci));
      thd->proc_info = buf;
      schema_res = s_ndb->pollEvents(10, &schema_gci);
    }

    if ((is_stop_requested() || binlog_thread_state) &&
        (ndb_latest_handled_binlog_epoch >= ndb_get_latest_trans_gci() || !ndb_binlog_running))
    ...
    Ndb_schema_event_handler schema_event_handler(thd, &mem_root, g_ndb_cluster_connection->node_id(),
                                                  schema_dist_data);
    ...
    *root_ptr = &mem_root;
    ...
    if (unlikely(schema_res > 0))
    {
      ...
      s_ndb->setReportThreshEventGCISlip(opt_ndb_report_thresh_binlog_epoch_slip);
      s_ndb->setReportThreshEventFreeMem(opt_ndb_report_thresh_binlog_mem_usage);
      ...
      while (pOp != NULL)
      {
        if (!pOp->hasError())
        {
          ...
          DBUG_PRINT("info", ("i_ndb first: %s", i_ndb->getEventOperation()
                                                     ...
                                                     : "<empty>"));
          if (i_ndb->getEventOperation() == NULL && s_ndb->getEventOperation() == NULL &&
              binlog_thread_state == BCCC_running)
          {
            ...
            if (ndb_latest_received_binlog_epoch < ndb_get_latest_trans_gci() && ndb_binlog_running)
            {
              sql_print_error(
                  ...
                  (uint)(ndb_get_latest_trans_gci() >> 32), (uint)(ndb_get_latest_trans_gci()),
                  (uint)(ndb_latest_received_binlog_epoch >> 32), (uint)(ndb_latest_received_binlog_epoch));
            }
          }
        }
          ...
          sql_print_error(
              ...
              "binlog schema event",
              (ulong)pOp->getNdbError().code, pOp->getNdbError().message);
        pOp = s_ndb->nextEvent();
      }
    ...
    if (!ndb_binlog_running)
    {
      ...
      */
      ...
      if (i_ndb->pollEvents(0, &tmp_gci))
      {
        ...
        {
          if ((unsigned)pOp->getEventType() >= (unsigned)NDBEVENT::TE_FIRST_NON_DATA_EVENT)
          {
    ...
    else if (res > 0 || (ndb_log_empty_epochs() && gci > ndb_latest_handled_binlog_epoch))
    {
      ...
      while (pOp != NULL)
      {
        rows = &_row;
        gci = pOp->getGCI();
        DBUG_PRINT("info", ("Handling gci: %u/%u", (uint)(gci >> 32), (uint)(gci)));
        ...
        // pass table map before epoch
        {
          ...
          while ((gci_op = i_ndb->getGCIEventOperations(&iter, &event_types)) != NULL)
          {
            ...
            DBUG_PRINT("info",
                       ("per gci_op: 0x%lx  share: 0x%lx  event_types: 0x%x", (long)gci_op, (long)share, event_types));
            ...
            {
              DBUG_PRINT("info", ("Skipped TE_STOP on table %s", gci_op->getEvent()->getTable()->getName()));
              continue;
            }
            // this should not happen
            if (share == NULL || event_data->shadow_table == NULL)
            {
              DBUG_PRINT("info", ("no share or table %s!", gci_op->getEvent()->getTable()->getName()));
              continue;
            }
            if (share == ndb_apply_status_share)
            {
...
#endif
            if ((event_types & (NdbDictionary::Event::TE_INSERT | NdbDictionary::Event::TE_UPDATE |
                                NdbDictionary::Event::TE_DELETE)) == 0)
            {
              DBUG_PRINT("info", ("skipping non data event table: %.*s", (int)name.length, name.str));
              continue;
            }
            if (!trans.good())
            {
              DBUG_PRINT("info", ("Found new data event, initializing transaction"));
              inj->new_trans(thd, &trans);
            }
            DBUG_PRINT("info", ("use_table: %.*s, cols %u", (int)name.length, name.str, table->s->fields));
        ...
        if (trans.good())
        {
          ...
          if (!injectApplyStatusWriteRow(trans, gci))
          {
        ...
        do
        {
          if (pOp->hasError() && handle_error(pOp) < 0)
            goto err;
...
#ifndef NDEBUG
          {
            Ndb_event_data *event_data = (Ndb_event_data *)pOp->getCustomData();
            NDB_SHARE *share = (event_data) ? event_data->share : NULL;
            DBUG_PRINT("info", ("EVENT TYPE: %d  GCI: %u/%u last applied: %u/%u  "
                                "share: 0x%lx (%s.%s)",
                                pOp->getEventType(), (uint)(gci >> 32), (uint)(gci),
                                (uint)(ndb_latest_applied_binlog_epoch >> 32), (uint)(ndb_latest_applied_binlog_epoch),
                                (long)share, share ? share->db : "'NULL'", share ? share->table_name : "'NULL'"));
            assert(share != 0);
          }
          ...
          if ((unsigned)pOp->getEventType() < (unsigned)NDBEVENT::TE_FIRST_NON_DATA_EVENT)
            handle_data_event(thd, i_ndb, pOp, &rows, trans, trans_row_count, trans_slave_row_count);
          else
          {
                ...
                binlog_thread_state == BCCC_running)
            {
              ...
              if (ndb_latest_received_binlog_epoch < ndb_get_latest_trans_gci() && ndb_binlog_running)
              {
                sql_print_error(
                    ...
                    (ulong)ndb_get_latest_trans_gci(), (ulong)ndb_latest_received_binlog_epoch);
              }
            }
          }
          ...
          if (!ndb_log_empty_epochs())
          {
            /*
              ...
              THEN
                ...
                is propagating)
            */
            if ((trans_row_count == 0) && (!(opt_ndb_log_apply_status && trans_slave_row_count)))
            {
...
err:
  ...
  */
  ...
  if (ndb_apply_status_share)
  {
    ...
    DBUG_PRINT("NDB_SHARE", ("%s binlog extra free  use_count: %u", ndb_apply_status_share->key_string(),
  ...
  if (ndb_schema_share)
  {
    ...
    DBUG_PRINT("NDB_SHARE",
               ("%s binlog extra free  use_count: %u", ndb_schema_share->key_string(), ndb_schema_share->use_count));
    free_share(&ndb_schema_share);
    ndb_schema_share = 0;
    native_mutex_unlock(&ndb_schema_share_mutex);
    /* end protect ndb_schema_share */
  ...
  if (opt_ndb_extra_logging > 15)
  {
    ...
    {
      NDB_SHARE *share = (NDB_SHARE *)my_hash_element(&ndbcluster_open_tables, i);
      sql_print_information("  %s.%s state: %u use_count: %u", share->db, share->table_name, (uint)share->state,
                            share->use_count);
    }
  }
...
size_t ndbcluster_show_status_binlog(char *buf, size_t buf_size)
{
  ...
  if (injector_ndb)
  {
    ...
    const size_t buf_len = my_snprintf(buf, buf_size,
                                       "latest_epoch=%llu, "
