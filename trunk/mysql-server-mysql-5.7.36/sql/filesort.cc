...
static ha_rows find_all_keys(Sort_param *param, QEP_TAB *qep_tab, Filesort_info *fs_info, IO_CACHE *buffer_file,
                             IO_CACHE *chunk_file, Bounded_queue<uchar *, uchar *, Sort_param, Mem_compare> *pq,
                             ha_rows *found_rows);
static int write_keys(Sort_param *param, Filesort_info *fs_info, uint count, IO_CACHE *buffer_file, IO_CACHE *tempfile);
static void register_used_fields(Sort_param *param);
static int merge_index(Sort_param *param, Sort_buffer sort_buffer, Merge_chunk_array chunk_array, IO_CACHE *tempfile,
                       IO_CACHE *outfile);
static bool save_index(Sort_param *param, uint count, Filesort_info *table_sort);
static uint suffix_length(ulong string_length);

static bool check_if_pq_applicable(Opt_trace_context *trace, Sort_param *param, Filesort_info *info, TABLE *table,
                                   ha_rows records, ulong memory_available, bool keep_addon_fields);

void Sort_param::init_for_filesort(Filesort *file_sort, uint sortlen, TABLE *table, ulong max_length_for_sort_data,
                                   ha_rows maxrows, bool sort_positions)
{
  assert(max_rows == 0);  // function should not be called twice
  sort_length = sortlen;
  ref_length = table->file->ref_length;
  if (!(table->file->ha_table_flags() & HA_FAST_KEY_READ) && !table->fulltext_searched && !sort_positions)
  {
    /*
      Get the descriptors of all fields whose values are appended
      to sorted fields and get its total length in addon_length.
    */
    addon_fields = file_sort->get_addon_fields(max_length_for_sort_data, table->field, sort_length, &addon_length,
                                               &m_packable_length);
  }
...
static void trace_filesort_information(Opt_trace_context *trace, const st_sort_field *sortorder, uint s_length)
{
  ...
  for (; s_length--; sortorder++)
  {
    ...
    if (sortorder->field)
    {
      ...
      oto.add_alnum("field", sortorder->field->field_name ? sortorder->field->field_name : "tmp_table_column");
    }
    else
...
bool filesort(THD *thd, Filesort *filesort, bool sort_positions, ha_rows *examined_rows, ha_rows *found_rows,
              ha_rows *returned_rows)
{
  ...
  */
  ...
  Item_subselect *const subselect = tab && tab->join() ? tab->join()->select_lex->master_unit()->item : NULL;

  MYSQL_FILESORT_START(const_cast<char *>(table->s->db.str), const_cast<char *>(table->s->table_name.str));
  ...
  */
  ...
  param.init_for_filesort(filesort,
                          sortlength(thd, filesort->sortorder, s_length, &multi_byte_charset, &param.use_hash), table,
  ...
  if (multi_byte_charset &&
      !(param.tmp_buffer = (char *)my_malloc(key_memory_Sort_param_tmp_buffer, param.sort_length, MYF(MY_WME))))
    goto err;

  if (check_if_pq_applicable(trace, &param, &table_sort, table, num_rows, memory_available, subselect != NULL))
  {
    ...
    if (pq.init(param.max_rows, &param, table_sort.get_sort_keys()))
    {
      /*
  ...
  else
  {
    ...
    */
    const ulong min_sort_memory =
        max<ulong>(MIN_SORT_MEMORY, ALIGN_SIZE(MERGEBUFF2 * (param.rec_length + sizeof(uchar *))));
    /*
  ...
  if (open_cached_file(&chunk_file, mysql_tmpdir, TEMP_PREFIX, DISK_BUFFER_SIZE, MYF(MY_WME)))
    goto err;

  param.sort_form = table;
  param.local_sortorder = Bounds_checked_array<st_sort_field>(filesort->sortorder, s_length);
  // New scope, because subquery execution must be traced within an array.
  {
    Opt_trace_array ota(trace, "filesort_execution");
    num_rows = find_all_keys(&param, tab, &table_sort, &chunk_file, &tempfile, param.using_pq ? &pq : NULL, found_rows);
    if (num_rows == HA_POS_ERROR)
      goto err;
  }

  num_chunks = static_cast<size_t>(my_b_tell(&chunk_file)) / sizeof(Merge_chunk);
  ...
  Opt_trace_object(trace, "filesort_summary")
      ...
      .add_alnum("sort_mode", param.using_packed_addons()  ? "<sort_key, packed_additional_fields>"
                              : param.using_addon_fields() ? "<sort_key, additional_fields>"
                                                           : "<sort_key, rowid>");
  ...
  else
  {
    ...
    if (table_sort.using_addon_fields() && !(table_sort.addon_fields->allocate_addon_buf(param.addon_length)))
    ...
    */
    ...
    if (merge_many_buff(&param, table_sort.get_raw_buf(), table_sort.merge_chunks, &num_chunks, &tempfile))
      goto err;
    if (flush_io_cache(&tempfile) || reinit_io_cache(&tempfile, READ_CACHE, 0L, 0, 0))
      goto err;
    if (merge_index(&param, table_sort.get_raw_buf(), Merge_chunk_array(table_sort.merge_chunks.begin(), num_chunks),
                    &tempfile, outfile))
      goto err;
  }
...
err:
  ...
  if (error)
  {
    ...
    */
    const char *cause =
        kill_errno ? ((kill_errno == THD::KILL_CONNECTION && !abort_loop) ? ER(THD::KILL_QUERY) : ER(kill_errno))
    ...
    if (thd->is_fatal_error)
      sql_print_information(
          ...
          msg, thd->security_context()->host_or_ip().str, thd->security_context()->priv_user().str, thd->thread_id(),
          cause, thd->query().str);
  }
  else
  ...
  *returned_rows = num_rows;
  ...
  DBUG_PRINT("exit", ("num_rows: %ld examined_rows: %ld found_rows: %ld", (long)num_rows, (long)*examined_rows,
                      (long)*found_rows));
...
uint Filesort::make_sortorder()
{
  ...
  for (ord = order; ord; ord = ord->next, pos++)
  {
    ...
    if (real_item->type() == Item::FIELD_ITEM)
    {
      ...
      */
      assert(item->type() == Item::FIELD_ITEM ||
             (item->type() == Item::REF_ITEM && (down_cast<Item_ref *>(item)->ref_type() == Item_ref::VIEW_REF ||
    ...
    else if (real_item->type() == Item::SUM_FUNC_ITEM && !real_item->const_item())
    {
      // Aggregate, or Item_aggregate_ref
      assert(item->type() == Item::SUM_FUNC_ITEM ||
             (item->type() == Item::REF_ITEM && static_cast<Item_ref *>(item)->ref_type() == Item_ref::AGGREGATE_REF));
...
void Filesort_info::read_chunk_descriptors(IO_CACHE *chunk_file, uint count)
{
  ...
  if (reinit_io_cache(chunk_file, READ_CACHE, 0L, 0, 0) || my_b_read(chunk_file, static_cast<uchar *>(rawmem), length))
  {
...
class Filesort_error_handler : public Internal_error_handler
{
  ...
  */
  ...
  {
    if (*level == Sql_condition::SL_WARNING && sql_errno == ER_NOT_SUPPORTED_YET)
    {
...
static ha_rows find_all_keys(Sort_param *param, QEP_TAB *qep_tab, Filesort_info *fs_info, IO_CACHE *chunk_file,
                             IO_CACHE *tempfile, Bounded_queue<uchar *, uchar *, Sort_param, Mem_compare> *pq,
                             ha_rows *found_rows)
{
  int error, flag;
  ...
  */
  ...
  DBUG_PRINT("info", ("using: %s", (qep_tab->condition() ? qep_tab->quick() ? "ranges" : "where" : "every row")));
  ...
  *found_rows = 0;
  ...
  /*
    ...
    it's not used
  */
  assert(sort_form->tmp_set.n_bits == 0 || bitmap_is_clear_all(&sort_form->tmp_set));
  ...
  if (qep_tab->condition())
  {
    ...
    qep_tab->condition()->walk(&Item::mark_field_in_map, walk_subquery, (uchar *)&mf);
  }
  // Include fields used by pushed conditions in the read_set.
  if (qep_tab->table()->file->pushed_idx_cond)
  {
    Mark_field mf(sort_form, MARK_COLUMNS_TEMP);
    qep_tab->table()->file->pushed_idx_cond->walk(&Item::mark_field_in_map, walk_subquery, (uchar *)&mf);
  }
  ...
  if (indexpos && idx && write_keys(param, fs_info, idx, chunk_file, tempfile))
  {
...
static int write_keys(Sort_param *param, Filesort_info *fs_info, uint count, IO_CACHE *chunk_file, IO_CACHE *tempfile)
{
  ...
  for (uint ix = 0; ix < count; ++ix)
  {
    ...
    {
      rec_length = param->sort_length + Addon_fields::read_addon_length(record + param->sort_length);
    }
    else
...
#endif
void copy_native_longlong(uchar *to, size_t to_length, longlong val, bool is_unsigned)
{
  copy_integer<Is_big_endian>(to, to_length, static_cast<uchar *>(static_cast<void *>(&val)), sizeof(longlong),
                              is_unsigned);
}
...
*/
static void MY_ATTRIBUTE((noinline)) make_json_sort_key(Item *item, uchar *to, size_t length, ulonglong *hash)
{
...
uint Sort_param::make_sortkey(uchar *to, const uchar *ref_pos)
{
  ...
  for (sort_field = local_sortorder.begin(); sort_field != local_sortorder.end(); sort_field++)
  {
      ...
      switch (sort_field->result_type)
      {
        case STRING_RESULT:
        {
          ...
          if (sort_field->need_strxnfrm)
          {
            ...
            tmp_length = cs->coll->strnxfrm(cs, to, sort_field->length, item->max_char_length(), (uchar *)from, length,
        ...
        case INT_RESULT:
        {
          longlong value = item->field_type() == MYSQL_TYPE_TIME ? item->val_time_temporal_result()
                           : item->is_temporal_with_date()       ? item->val_date_temporal_result()
                                                                 : item->val_int_result();
          if (maybe_null)
          {
          ...
          copy_native_longlong(to, sort_field->length, value, item->unsigned_flag);
          break;
        }
        case DECIMAL_RESULT:
        {
          ...
          if (sort_field->length < DECIMAL_MAX_FIELD_SIZE)
          {
            ...
            my_decimal2binary(E_DEC_FATAL_ERROR, dec_val, buf, item->max_length - (item->decimals ? 1 : 0),
                              item->decimals);
            memcpy(to, buf, sort_field->length);
          }
          else
          {
            my_decimal2binary(E_DEC_FATAL_ERROR, dec_val, to, item->max_length - (item->decimals ? 1 : 0),
                              item->decimals);
          }
          break;
        }
...
static void register_used_fields(Sort_param *param)
{
  ...
  for (sort_field = param->local_sortorder.begin(); sort_field != param->local_sortorder.end(); sort_field++)
  {
    ...
    {  // Item
      sort_field->item->walk(&Item::mark_field_in_map, walk_subquery, (uchar *)&mf);
    }
  }
...
static bool save_index(Sort_param *param, uint count, Filesort_info *table_sort)
{
  ...
  if (!(to = table_sort->sorted_result =
            static_cast<uchar *>(my_malloc(key_memory_Filesort_info_record_pointers, buf_size, MYF(MY_WME)))))
    DBUG_RETURN(1); /* purecov: inspected */
  table_sort->sorted_result_end = table_sort->sorted_result + buf_size;
...
bool check_if_pq_applicable(Opt_trace_context *trace, Sort_param *param, Filesort_info *filesort_info, TABLE *table,
                            ha_rows num_rows, ulong memory_available, bool keep_addon_fields)
{
  ...
  */
  ...
  Opt_trace_object trace_filesort(trace, "filesort_priority_queue_optimization");
  if (param->max_rows == HA_POS_ERROR)
  {
    trace_filesort.add("usable", false).add_alnum("cause", "not applicable (no LIMIT)");
    DBUG_RETURN(false);
  }
  ...
  ulong num_available_keys = memory_available / (param->rec_length + sizeof(char *));
  ...
  if (num_rows < num_available_keys)
  {
    ...
    {
      // PQ will be slower.
      trace_filesort.add("chosen", false).add_alnum("cause", "quicksort_is_cheaper");
      DBUG_RETURN(false);
    }
  }
  ...
  {
    const ulong row_length = param->sort_length + param->ref_length + sizeof(char *);
    ...
    else
    {
      ...
      const double sort_merge_cost =
          get_merge_many_buffs_cost_fast(num_rows, num_available_keys, row_length, cost_model);
...
*/
int merge_many_buff(Sort_param *param, Sort_buffer sort_buffer, Merge_chunk_array chunk_array, size_t *p_num_chunks,
                    IO_CACHE *t_file)
{
  uint i;
  ...
  *p_num_chunks = num_chunks;
  ...
  while (num_chunks > MERGEBUFF2)
  {
    ...
    if (merge_buffers(param, from_file, to_file, sort_buffer, last_chunk++,
...
uint read_to_buffer(IO_CACHE *fromfile, Merge_chunk *merge_chunk, Sort_param *param)
{
  ...
  if ((count = min(merge_chunk->max_keys(), merge_chunk->rowcount())))
  {
    ...
    if (param->using_packed_addons())
    {
      ...
      bytes_to_read =
          min(merge_chunk->buffer_size(), static_cast<size_t>(fromfile->end_of_file - merge_chunk->file_position()));
    }
    else
      bytes_to_read = rec_length * static_cast<size_t>(count);

    DBUG_PRINT("info", ("read_to_buffer %p at file_pos %llu bytes %llu", merge_chunk,
                        static_cast<ulonglong>(merge_chunk->file_position()), static_cast<ulonglong>(bytes_to_read)));
    if (mysql_file_pread(fromfile->file, merge_chunk->buffer_start(), bytes_to_read, merge_chunk->file_position(),
                         MYF_RW))
    ...
    if (param->using_packed_addons())
    {
      ...
      for (; ix < count; ++ix)
      {
        if (record + param->sort_length + Addon_fields::size_of_length_field >= merge_chunk->buffer_end())
...
int merge_buffers(Sort_param *param, IO_CACHE *from_file, IO_CACHE *to_file, Sort_buffer sort_buffer,
                  Merge_chunk *last_chunk, Merge_chunk_array chunk_array, int flag)
{
  int error;
  ...
  Merge_chunk_less mcl = doing_unique ? Merge_chunk_less(cmp, first_cmp_arg) : Merge_chunk_less(sort_length);
  Priority_queue<Merge_chunk *, std::vector<Merge_chunk *, Malloc_allocator<Merge_chunk *>>, Merge_chunk_less> queue(
      mcl, Malloc_allocator<Merge_chunk *>(key_memory_Filesort_info_merge));

  if (queue.reserve(chunk_array.size()))
    DBUG_RETURN(1);

  for (merge_chunk = chunk_array.begin(); merge_chunk != chunk_array.end(); merge_chunk++)
  {
    merge_chunk->set_buffer(strpos, strpos + (sort_buffer.size() / (chunk_array.size())));
    merge_chunk->set_max_keys(maxcount);
    strpos += (uint)(error = (int)read_to_buffer(from_file, merge_chunk, param));
  ...
  while (queue.size() > 1)
  {
    ...
    for (;;)
    {
      ...
      {
        param->get_rec_and_res_len(merge_chunk->current_key(), &rec_length, &res_length);
        const uint bytes_to_write = (flag == 0) ? rec_length : res_length;

        DBUG_PRINT("info", ("write record at %llu len %u", my_b_tell(to_file), bytes_to_write));
        if (my_b_write(to_file, merge_chunk->current_key() + offset, bytes_to_write))
        {
          DBUG_RETURN(1); /* purecov: inspected */
        }
        if (!--max_rows)
  ...
  merge_chunk = queue.top();
  merge_chunk->set_buffer(sort_buffer.array(), sort_buffer.array() + sort_buffer.size());
  ...
  */
  ...
  } while ((error = (int)read_to_buffer(from_file, merge_chunk, param)) != -1 && error != 0);
  ...
  *use_hash = false;
  ...
  for (; s_length--; sortorder++)
  {
    ...
    if (sortorder->field)
    {
      ...
      if (sortorder->field->result_type() == STRING_RESULT && !sortorder->field->is_temporal())
      {
...
Addon_fields *Filesort::get_addon_fields(ulong max_length_for_sort_data, Field **ptabfield, uint sortlength,
                                         uint *plength, uint *ppackable_length)
{
  ...
  */
  const bool filter_covering =
      index != MAX_KEY && table->covering_keys.is_set(index) && table->index_contains_some_virtual_gcol(index);
  ...
  *plength = *ppackable_length = 0;
  ...
  for (pfield = ptabfield; (field = *pfield); pfield++)
  {
    ...
    if (field->maybe_null() || field_type == MYSQL_TYPE_STRING || field_type == MYSQL_TYPE_VARCHAR ||
  ...
  *ppackable_length = packable_length;
  ...
  if (addon_fields == NULL)
  {
    ...
    Addon_fields_array addon_array(static_cast<Sort_addon_field *>(rawmem2), num_fields);
  ...
  *plength = total_length;
  ...
  for (pfield = ptabfield; (field = *pfield); pfield++)
  {
    ...
    DBUG_PRINT("info", ("addon_field %s max_length %u", addonf->field->field_name, addonf->max_length));
