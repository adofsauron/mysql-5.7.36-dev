...
static MYSQL_THDVAR_UINT(max_rows, /* name */
                         PLUGIN_VAR_RQCMDARG, "Specify max number of rows to fetch per roundtrip to cluster",
                         NULL, /* check func. */
                         NULL, /* update func. */
                         10,   /* default */
                         1,    /* min */
...
static MYSQL_THDVAR_UINT(max_bytes, /* name */
                         PLUGIN_VAR_RQCMDARG, "Specify approx. max number of bytes to fetch per roundtrip to cluster",
                         NULL,  /* check func. */
                         NULL,  /* update func. */
                         0,     /* default */
                         0,     /* min */
...
static MYSQL_THDVAR_BOOL(show_hidden,                                                             /* name */
                         PLUGIN_VAR_RQCMDARG, "Control if tables should be visible or not", NULL, /* check func. */
                         NULL,                                                                    /* update func. */
                         FALSE                                                                    /* default */
);
...
static MYSQL_SYSVAR_STR(database,           /* name */
                        ...
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY, "Name of the database used by ndbinfo",
                        NULL, /* check func. */
                        NULL, /* update func. */
                        NULL  /* default */
);
...
static MYSQL_SYSVAR_STR(table_prefix,             /* name */
                        ...
                        "Prefix to use for all virtual tables loaded from NDB", NULL, /* check func. */
                        NULL,                                                         /* update func. */
                        NULL                                                          /* default */
);
...
static MYSQL_SYSVAR_UINT(version,             /* name */
                         ...
                         PLUGIN_VAR_NOCMDOPT | PLUGIN_VAR_READONLY, "Compile version for ndbinfo",
                         NULL, /* check func. */
                         NULL, /* update func. */
                         0,    /* default */
                         0,    /* min */
...
static void offline_update(THD *thd, struct st_mysql_sys_var *var, void *var_ptr, const void *save)
{
  DBUG_ENTER("offline_update");

  const my_bool new_offline = (*(static_cast<const my_bool *>(save)) != 0);
...
static struct error_message
{
  int error;
  const char *message;
} error_messages[] = {{ERR_INCOMPAT_TABLE_DEF, "Incompatible table definitions"},
...
static int err2mysql(int error)
{
  ...
  push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_GET_ERRNO, ER(ER_GET_ERRNO), error);
...
{
  sql.appfmt("'CREATE TABLE `%s`.`%s%s` (", opt_ndbinfo_dbname, opt_ndbinfo_table_prefix, ndb_tab->getName());
...
static void warn_incompatible(const NdbInfo::Table *ndb_tab, bool fatal, const char *format, ...)
{
  BaseString msg;
  ...
  const Sql_condition::enum_severity_level level = (fatal ? Sql_condition::SL_WARNING : Sql_condition::SL_NOTE);
  push_warning(current_thd, level, ERR_INCOMPAT_TABLE_DEF, msg.c_str());

  DBUG_VOID_RETURN;
}

int ha_ndbinfo::create(const char *name, TABLE *form, HA_CREATE_INFO *create_info)
{
  ...
  */
  ...
  for (uint i = 0; i < table->s->fields; i++)
  {
    ...
    if (const_cast<Field *>(field)->real_maybe_null() == false)
    {
      ...
      warn_incompatible(ndb_tab, true, "column '%s' is NOT NULL", field->field_name);
    ...
    if (!compatible)
    {
      ...
      warn_incompatible(ndb_tab, true, "column '%s' is not compatible", field->field_name);
      delete m_impl.m_table;
      m_impl.m_table = 0;
      DBUG_RETURN(ERR_INCOMPAT_TABLE_DEF);
    }
...
int ha_ndbinfo::rnd_init(bool scan)
{
  ...
  if (m_impl.m_first_use)
  {
    ...
    */
    ...
    for (uint i = 0; i < table->s->fields; i++)
    {
      ...
      {
        // The column didn't exist
        warn_incompatible(ndb_tab, true, "column '%s' does not exist", field->field_name);
        continue;
      }
      fields_found_in_ndb++;
    }
  ...
  if ((err = g_ndbinfo->createScanOperation(m_impl.m_table, &scan_op, THDVAR(thd, max_rows), THDVAR(thd, max_bytes))) !=
      0)
    DBUG_RETURN(err2mysql(err));
    ...
    if (record && !record->isNULL())
    {
      ...
      switch (field->type())
      {
        case (MYSQL_TYPE_VARCHAR):
        {
          ...
          (void)vfield->store(record->c_str(), MIN(record->length(), field->field_length) - 1, field->charset());
...
static int ndbinfo_find_files(handlerton *hton, THD *thd, const char *db, const char *path, const char *wild, bool dir,
...
static int ndbinfo_init(void *plugin)
{
  ...
  build_table_filename(prefix, sizeof(prefix) - 1, opt_ndbinfo_dbname, opt_ndbinfo_table_prefix, "", 0);
  DBUG_PRINT("info", ("prefix: '%s'", prefix));
  assert(g_ndb_cluster_connection);
  g_ndbinfo = new NdbInfo(g_ndb_cluster_connection, prefix, opt_ndbinfo_dbname, opt_ndbinfo_table_prefix);
  if (!g_ndbinfo)
  {
...
static int ndbinfo_deinit(void *plugin)
{
...
struct st_mysql_sys_var *ndbinfo_system_variables[] = {MYSQL_SYSVAR(max_rows),
...
struct st_mysql_storage_engine ndbinfo_storage_engine = {MYSQL_HANDLERTON_INTERFACE_VERSION};

struct st_mysql_plugin ndbinfo_plugin = {MYSQL_STORAGE_ENGINE_PLUGIN,
