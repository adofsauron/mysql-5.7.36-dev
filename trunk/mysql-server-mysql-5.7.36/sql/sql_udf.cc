...
extern "C" uchar *get_hash_key(const uchar *buff, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
...
void udf_init()
{
  ...
  THD *new_thd = new THD;
  if (!new_thd || my_hash_init(&udf_hash, system_charset_info, 32, 0, 0, get_hash_key, NULL, 0, key_memory_udf_mem))
  {
  ...
  while (!(error = read_record_info.read_record(&read_record_info)))
  {
    ...
    */
    ...
    if (check_valid_path(dl_name, strlen(dl_name)) ||
        check_string_char_length(name_cstr, "", NAME_CHAR_LEN, system_charset_info, 1))
    {
      sql_print_error("Invalid row in mysql.func table for function '%.64s'", name.str);
      continue;
    }
...
udf_func *find_udf(const char *name, size_t length, bool mark_used)
{
  ...
  if ((udf = (udf_func *)my_hash_search(&udf_hash, (uchar *)name, length ? length : strlen(name))))
  {
    if (!udf->dlhandle)
...
int mysql_create_function(THD *thd, udf_func *udf)
{
  ...
  */
  ...
  if (check_string_char_length(udf_name_cstr, "", NAME_CHAR_LEN, system_charset_info, 1))
  {
  ...
  */
  ...
  if (!(dl = find_udf_dl(udf->dl)))
  {
    ...
    if (!(dl = dlopen(dlpath, RTLD_NOW)))
    {
      ...
      DBUG_PRINT("error", ("dlopen of %s failed, error: %d (%s)", udf->dl, error_number, errmsg));
  ...
  if (error)
  {
    ...
    my_error(ER_ERROR_ON_WRITE, MYF(0), "mysql.func", error, my_strerror(errbuf, sizeof(errbuf), error));
    del_udf(u_d);
    goto err;
  }
  mysql_rwlock_unlock(&THR_LOCK_udf);
...
int mysql_drop_function(THD *thd, const LEX_STRING *udf_name)
{
  ...
  */
  ...
  if (!table->file->ha_index_read_idx_map(table->record[0], 0, table->field[0]->ptr, HA_WHOLE_KEY, HA_READ_KEY_EXACT))
  {
    int delete_err;
