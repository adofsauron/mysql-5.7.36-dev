...
bool mysql_create_or_drop_trigger(THD *thd, TABLE_LIST *tables, bool create)
{
  ...
  */
  ...
  /*
    ...
    applies to them too.
  */
  if (!trust_function_creators && mysql_bin_log.is_open() && !(thd->security_context()->check_access(SUPER_ACL)))
  {
  ...
  if (!create)
  {
    ...
    */
    ...
    if (add_table_for_trigger(thd, thd->lex->spname->m_db, thd->lex->spname->m_name, if_exists, &tables))
      goto end;
  ...
  */
  ...
  if (thd->locked_tables_mode)
  {
    ...
    if (!(tables->table = find_table_for_mdl_upgrade(thd, tables->db, tables->table_name, FALSE)))
      goto end;
  }
  else
  {
  ...
  else
  {
    ...
    result = table->triggers->drop_trigger(thd, thd->lex->spname->m_name, &trigger_found);
  ...
  */
  ...
  if (!result)
  {
    ...
    MYSQL_DROP_SP(SP_TYPE_TRIGGER, thd->lex->spname->m_db.str, thd->lex->spname->m_db.length,
                           ...
                           bool continue_if_not_exist, TABLE_LIST **table)
{
  ...
  LEX_STRING trn_path = Trigger_loader::build_trn_path(trn_path_buff, FN_REFLEN, db_name.str, trigger_name.str);

  if (Trigger_loader::check_trn_exists(trn_path))
  {
    if (continue_if_not_exist)
    {
      push_warning(thd, Sql_condition::SL_NOTE, ER_TRG_DOES_NOT_EXIST, ER(ER_TRG_DOES_NOT_EXIST));
                               ...
                               const char *new_db_name, const char *new_table_name)
{
  ...
  */
  assert(thd->mdl_context.owns_equal_or_stronger_lock(MDL_key::TABLE, db_name, table_name, MDL_EXCLUSIVE));
  ...
  return d.check_n_load(thd, true) || d.check_for_broken_triggers() ||
...
bool drop_all_triggers(THD *thd, const char *db_name, const char *table_name)
{
  ...
  */
  ...
  return d.check_n_load(thd, true) || Trigger_loader::drop_all_triggers(db_name, table_name, &d.get_trigger_list());
}
