...
void handle_gis_exception(const char *funcname)
{
  ...
  {
    my_error(ER_BOOST_GEOMETRY_SELF_INTERSECTION_POINT_EXCEPTION, MYF(0), funcname);
  }
...
void BG_geometry_collection::merge_components(my_bool *pnull_value)
{
  if (is_comp_no_overlapped())
    return;

  POS pos = {{NULL, NULL}, {NULL, NULL}};
  Item_func_spatial_operation ifso(pos, NULL, NULL, Item_func_spatial_operation::op_union);
  bool do_again = true;
...
template void BG_geometry_collection::merge_components<boost::geometry::cs::cartesian>(char *);

template <typename Coordsys>
inline bool linestring_overlaps_polygon_outerring(const Gis_line_string &ls, const Gis_polygon &plgn)
{
  Gis_polygon_ring &oring = plgn.outer();
  Gis_line_string ls2(oring.get_ptr(), oring.get_nbytes(), oring.get_flags(), oring.get_srid());
  return boost::geometry::overlaps(ls, ls2);
}

template <typename Coordsys>
bool linear_areal_intersect_infinite(Geometry *g1, Geometry *g2, my_bool *pnull_value)
{
  bool res = false;
  ...
  */
  if (Item_func_spatial_rel::bg_geo_relation_check<Coordsys>(g1, g2, Item_func::SP_CROSSES_FUNC, pnull_value) &&
      !*pnull_value)
  {
    Geometry::wkbType g2_type = g2->get_type();
    if (g2_type == Geometry::wkb_polygon)
    {
      Gis_polygon plgn(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
      res = bg::is_valid(plgn);
    }
    else if (g2_type == Geometry::wkb_multipolygon)
    {
      Gis_multi_polygon mplgn(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  {
    Gis_line_string ls(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
  ...
  {
    Gis_multi_line_string mls(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    if (g2->get_type() == Geometry::wkb_polygon)
    {
      Gis_polygon plgn(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
 ...
 */
template <typename Coordsys>
bool BG_geometry_collection::merge_one_run(Item_func_spatial_operation *ifso, my_bool *pnull_value)
{
  Geometry *gres = NULL;
  bool has_new = false;
  ...
  for (Geometry_list::iterator i = m_geos.begin(); i != m_geos.end(); ++i)
  {
    ...
    for (Rtree_result::iterator j = rtree_result.begin(); j != rtree_result.end(); ++j)
    {
      ...
      if (Item_func_spatial_rel::bg_geo_relation_check<Coordsys>(geom2, *i, Item_func::SP_EQUALS_FUNC, &null_value) &&
          !null_value)
      {
        *i = NULL;
        break;
      ...
      if (Item_func_spatial_rel::bg_geo_relation_check<Coordsys>(*i, geom2, Item_func::SP_WITHIN_FUNC, &null_value) &&
          !null_value)
      {
        *i = NULL;
        break;
      ...
      if (Item_func_spatial_rel::bg_geo_relation_check<Coordsys>(geom2, *i, Item_func::SP_WITHIN_FUNC, &null_value) &&
          !null_value)
      {
      ...
      */
      if ((!is_linear_areal &&
           Item_func_spatial_rel::bg_geo_relation_check<Coordsys>(*i, geom2, Item_func::SP_INTERSECTS_FUNC,
                                                                  &null_value) &&
           !null_value) ||
          (is_linear_areal && linear_areal_intersect_infinite<Coordsys>(geom_d1, geom_d2, &null_value)))
      {
        String wkbres;
        ...
        gres = ifso->bg_geo_set_op<Coordsys>(*i, geom2, &wkbres);
  ...
  Geometry_list::iterator jj = std::remove_if(m_geos.begin(), m_geos.end(), pred);
  m_geos.resize(jj - m_geos.begin());

  for (Pointer_vector<Geometry>::iterator i = added.begin(); i != added.end(); ++i)
  {
    /*
...
bool post_fix_result(BG_result_buf_mgr *resbuf_mgr, BG_geotype &geout, String *res)
{
  ...
  if (geout.get_type() == Geometry::wkb_multilinestring || geout.get_type() == Geometry::wkb_multipolygon)
  ...
  if (res)
  {
    ...
    */
    ...
    write_geometry_header(const_cast<char *>(resptr), geout.get_srid(), geout.get_geotype());
...
bool is_empty_geocollection(const Geometry *g)
{
  ...
  wkb_scanner(g->get_cptr(), &len, Geometry::wkb_geometrycollection, false, &checker);
...
bool is_empty_geocollection(const String &wkbres)
{
  ...
  wkb_scanner(wkbres.ptr() + GEOM_HEADER_SIZE, &len, Geometry::wkb_geometrycollection, false, &checker);
  return checker.is_empty;
}
