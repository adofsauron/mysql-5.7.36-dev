...
void Event_parse_data::check_if_in_the_past(THD *thd, my_time_t ltime_utc)
{
  ...
  */
  ...
  if (on_completion == Event_parse_data::ON_COMPLETION_DROP)
  {
    switch (thd->lex->sql_command)
    {
      case SQLCOM_CREATE_EVENT:
        push_warning(thd, Sql_condition::SL_NOTE, ER_EVENT_CANNOT_CREATE_IN_THE_PAST,
  ...
  else if (status == Event_parse_data::ENABLED)
  {
    ...
    status_changed = true;
    push_warning(thd, Sql_condition::SL_NOTE, ER_EVENT_EXEC_TIME_IN_THE_PAST, ER(ER_EVENT_EXEC_TIME_IN_THE_PAST));
  }
}
...
bool Event_parse_data::check_parse_data(THD *thd)
{
  ...
  DBUG_PRINT("info", ("execute_at: 0x%lx  expr=0x%lx  starts=0x%lx  ends=0x%lx", (long)item_execute_at,
                      (long)item_expression, (long)item_starts, (long)item_ends));
...
void Event_parse_data::init_definer(THD *thd)
{
  ...
  DBUG_PRINT("info", ("init definer_user thd->mem_root: 0x%lx  "
                      "definer_user: 0x%lx",
                      (long)thd->mem_root, (long)definer_user));
