 ...
 public:
  /**
    ...
    @param prev_event_type The type of the event processed just before the
                           current one
   */
  Observe_transmission_guard(bool &flag, binary_log::Log_event_type event_type, const char *event_ptr, uint32 event_len,
      ...
      switch (event_type)
      {
        ...
        case binary_log::QUERY_EVENT:
        {
          bool first_event_after_gtid =
              prev_event_type == binary_log::ANONYMOUS_GTID_LOG_EVENT || prev_event_type == binary_log::GTID_LOG_EVENT;

          Format_description_log_event fd_ev(BINLOG_VERSION);
          fd_ev.common_footer->checksum_alg = checksum_alg;
          Query_log_event ev(event_ptr, event_len, &fd_ev, binary_log::QUERY_EVENT);
          if (first_event_after_gtid)
            m_to_set = (strcmp("BEGIN", ev.query) != 0);
          else
            m_to_set = (strncmp("XA COMMIT", ev.query, 9) == 0) || (strncmp("XA ABORT", ev.query, 8) == 0) ||
...
Binlog_sender::Binlog_sender(THD *thd, const char *start_file, my_off_t start_pos, Gtid_set *exclude_gtids, uint32 flag)
    : m_thd(thd),
...
void Binlog_sender::init()
{
  ...
  m_new_shrink_size = PACKET_MIN_SIZE;
  DBUG_PRINT("info", ("Initial packet->alloced_length: %zu", m_packet.alloced_length()));
  ...
  sql_print_information(
      "Start binlog_dump to master_thread_id(%u) "
      "slave_server(%u), pos(%s, %llu)",
      thd->thread_id(), thd->server_id, m_start_file, m_start_pos);

  if (RUN_HOOK(binlog_transmit, transmit_start, (thd, m_flag, m_start_file, m_start_pos, &m_observe_transmission)))
  {
  ...
  /*
    ...
    killed.
  */
  m_wait_new_events = !((thd->server_id == 0) || ((m_flag & BINLOG_DUMP_NON_BLOCK) != 0));
...
void Binlog_sender::run()
{
  ...
  while (!has_error() && !m_thd->killed)
  {
    ...
    */
    ...
    if (!mysql_bin_log.is_open())
    {
      if (mysql_bin_log.open_index_file(mysql_bin_log.get_index_fname(), log_file, FALSE))
      {
        set_fatal_error(
    ...
    if (unlikely(error))
    {
      ...
      if (is_index_file_reopened_on_binlog_disable)
        mysql_bin_log.close(LOG_CLOSE_INDEX, true /*need_lock_log=true*/, true /*need_lock_index=true*/);
  ...
  if (file > 0)
  {
    if (is_fatal_error())
    {
      ...
      my_snprintf(error_text, sizeof(error_text),
                  ...
                  m_errmsg, m_start_file, m_start_pos, m_last_file, m_last_pos, log_file, my_b_tell(&log_cache));
...
my_off_t Binlog_sender::send_binlog(IO_CACHE *log_cache, my_off_t start_pos)
{
  ...
  */
  ...
  while (!m_thd->killed)
  {
    ...
    m_thd->killed = DBUG_EVALUATE_IF("simulate_kill_dump", THD::KILL_CONNECTION, m_thd->killed);
...
inline my_off_t Binlog_sender::get_binlog_end_pos(IO_CACHE *log_cache)
{
  ...
  do
  {
    ...
    DBUG_PRINT("info", ("Reading file %s, seek pos %llu, end_pos is %llu", m_linfo.log_file_name, log_pos, end_pos));
...
int Binlog_sender::send_events(IO_CACHE *log_cache, my_off_t end_pos)
{
  ...
  while (likely(log_pos < end_pos))
  {
    ...
    if (unlikely(read_event(log_cache, m_event_checksum_alg, &event_ptr, &event_len)))
      DBUG_RETURN(1);
    ...
    Observe_transmission_guard obs_guard(m_observe_transmission, event_type,
                                         const_cast<const char *>(reinterpret_cast<char *>(event_ptr)), event_len,
    ...
    */
    if (m_exclude_gtid && (in_exclude_group = skip_event(event_ptr, event_len, in_exclude_group)))
    {
      /*
       ...
       */
      DBUG_EXECUTE_IF("inject_2sec_sleep_when_skipping_an_event", { my_sleep(2000000); });
      time_t now = time(0);
      assert(now >= m_last_event_sent_ts);
      bool time_for_hb_event =
          ((ulonglong)(now - m_last_event_sent_ts) >= (ulonglong)(m_heartbeat_period / 1000000000UL));
      ...
      DBUG_PRINT("info", ("Event of type %s is skipped", Log_event::get_type_str(event_type)));
    }
    else
    {
      /*
...
bool Binlog_sender::check_event_type(Log_event_type type, const char *log_file, my_off_t log_pos)
{
  if (type == binary_log::ANONYMOUS_GTID_LOG_EVENT)
  {
    ...
    */
    if (m_using_gtid_protocol)
    {
      DBUG_EXECUTE_IF("skip_sender_anon_autoposition_error", { return false; };);
      char buf[MYSQL_ERRMSG_SIZE];
      sprintf(buf, ER(ER_CANT_REPLICATE_ANONYMOUS_WITH_AUTO_POSITION), log_file, log_pos);
      set_fatal_error(buf);
      return true;
    }
    /*
    ...
    */
    ...
    {
      char buf[MYSQL_ERRMSG_SIZE];
      sprintf(buf, ER(ER_CANT_REPLICATE_ANONYMOUS_WITH_GTID_MODE_ON), log_file, log_pos);
      set_fatal_error(buf);
      return true;
    }
  }
  else if (type == binary_log::GTID_LOG_EVENT)
  {
    ...
    */
    ...
    {
      char buf[MYSQL_ERRMSG_SIZE];
      sprintf(buf, ER(ER_CANT_REPLICATE_GTID_WITH_GTID_MODE_OFF), log_file, log_pos);
      set_fatal_error(buf);
      return true;
    }
  }
...
inline bool Binlog_sender::skip_event(const uchar *event_ptr, uint32 event_len, bool in_exclude_group)
{
  ...
  switch (event_type)
  {
    case binary_log::GTID_LOG_EVENT:
    {
      ...
      Gtid_log_event gtid_ev((const char *)event_ptr, event_checksum_on() ? event_len - BINLOG_CHECKSUM_LEN : event_len,
                             &fd_ev);
      Gtid gtid;
...
int Binlog_sender::wait_new_events(my_off_t log_pos)
{
  ...
  mysql_bin_log.lock_binlog_end_pos();
  m_thd->ENTER_COND(mysql_bin_log.get_log_cond(), mysql_bin_log.get_binlog_end_pos_lock(),
                    &stage_master_has_sent_all_binlog_to_slave, &old_stage);

  if (mysql_bin_log.get_binlog_end_pos() <= log_pos && mysql_bin_log.is_active(m_linfo.log_file_name))
  {
...
void Binlog_sender::init_heartbeat_period()
{
  ...
  user_var_entry *entry = (user_var_entry *)my_hash_search(&m_thd->user_vars, (uchar *)name.str, name.length);
...
int Binlog_sender::check_start_file()
{
  ...
  else if (m_using_gtid_protocol)
  {
    ...
    */
    ...
    Gtid_set gtid_executed_and_owned(gtid_state->get_executed_gtids()->get_sid_map());

    // gtids = executed_gtids & owned_gtids
    if (gtid_executed_and_owned.add_gtid_set(gtid_state->get_executed_gtids()) != RETURN_STATUS_OK)
    {
      assert(0);
    }
    gtid_state->get_owned_gtids()->get_gtids(gtid_executed_and_owned);

    if (!m_exclude_gtid->is_subset_for_sid(&gtid_executed_and_owned, gtid_state->get_server_sidno(), subset_sidno))
    {
    ...
    */
    ...
    global_sid_lock->unlock();
    Gtid first_gtid = {0, 0};
    if (mysql_bin_log.find_first_log_not_in_gtid_set(index_entry_name, m_exclude_gtid, &first_gtid, errmsg))
    {
      set_fatal_error(errmsg.c_str());
      return 1;
    }
    ...
    */
    m_gtid_clear_fd_created_flag =
        (first_gtid.sidno >= 1 && first_gtid.gno >= 1 && m_exclude_gtid->contains_gtid(first_gtid));
  }
...
void Binlog_sender::init_checksum_alg()
{
  ...
  entry = (user_var_entry *)my_hash_search(&m_thd->user_vars, (uchar *)name.str, name.length);
  if (entry)
  {
...
int Binlog_sender::fake_rotate_event(const char *next_log_file, my_off_t log_pos)
{
...
inline int Binlog_sender::reset_transmit_packet(ushort flags, size_t event_len)
{
  ...
  DBUG_PRINT("info", ("event_len: %zu, m_packet->alloced_length: %zu", event_len, m_packet.alloced_length()));
  ...
  if (m_observe_transmission && RUN_HOOK(binlog_transmit, reserve_header, (m_thd, flags, &m_packet)))
  {
...
int Binlog_sender::send_format_description_event(IO_CACHE *log_cache, my_off_t start_pos)
{
  DBUG_ENTER("Binlog_sender::send_format_description_event");
  uchar *event_ptr;
  uint32 event_len;

  if (read_event(log_cache, binary_log::BINLOG_CHECKSUM_ALG_OFF, &event_ptr, &event_len))
    DBUG_RETURN(1);

  DBUG_PRINT("info", ("Looked for a Format_description_log_event, found event type %s",
  ...
  m_event_checksum_alg = Log_event_footer::get_checksum_alg((const char *)event_ptr, event_len);
  ...
  if (m_slave_checksum_alg == binary_log::BINLOG_CHECKSUM_ALG_UNDEF && event_checksum_on())
  {
    set_fatal_error(
...
int Binlog_sender::has_previous_gtid_log_event(IO_CACHE *log_cache, bool *found)
{
...
inline int Binlog_sender::read_event(IO_CACHE *log_cache, enum_binlog_checksum_alg checksum_alg, uchar **event_ptr,
                                     uint32 *event_len)
{
  ...
  */
  if ((error = Log_event::read_log_event(log_cache, &m_packet, NULL, checksum_alg, NULL, NULL, header)))
    goto read_error;
  ...
  *event_ptr = (uchar *)m_packet.ptr() + event_offset;
  ...
  DBUG_PRINT("info", ("Read event %s", Log_event::get_type_str(Log_event_type((*event_ptr)[EVENT_TYPE_OFFSET]))));
#ifndef NDEBUG
...
int Binlog_sender::send_heartbeat_event(my_off_t log_pos)
{
  ...
  size_t ident_len = strlen(p);
  size_t event_len = ident_len + LOG_EVENT_HEADER_LEN + (event_checksum_on() ? BINLOG_CHECKSUM_LEN : 0);
...
{
  if (DBUG_EVALUATE_IF("simulate_flush_error", 1, m_thd->get_protocol_classic()->flush_net()))
  {
...
inline int Binlog_sender::send_packet()
{
  ...
  DBUG_PRINT("info", ("Sending event of type %s",
                      Log_event::get_type_str((Log_event_type)m_packet.ptr()[1 + EVENT_TYPE_OFFSET])));
  // We should always use the same buffer to guarantee that the reallocation
  // logic is not broken.
  if (DBUG_EVALUATE_IF(
          "simulate_send_error", true,
          my_net_write(m_thd->get_protocol_classic()->get_net(), (uchar *)m_packet.ptr(), m_packet.length())))
  {
...
inline int Binlog_sender::before_send_hook(const char *log_file, my_off_t log_pos)
{
  if (m_observe_transmission &&
      RUN_HOOK(binlog_transmit, before_send_event, (m_thd, m_flag, &m_packet, log_file, log_pos)))
  {
...
{
  if (max_binlog_dump_events != 0 && (++m_event_count > max_binlog_dump_events))
  {
...
inline size_t Binlog_sender::calc_grow_buffer_size(size_t current_size, size_t min_size)
{
  ...
  size_t new_size = static_cast<size_t>(
      std::min(static_cast<double>(PACKET_MAX_SIZE), static_cast<double>(current_size * PACKET_GROW_FACTOR)));
...
{
  size_t new_size = static_cast<size_t>(
      std::max(static_cast<double>(PACKET_MIN_SIZE), static_cast<double>(current_size * PACKET_SHRINK_FACTOR)));
