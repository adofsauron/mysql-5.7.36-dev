...
bool key_cmp_if_same(TABLE *table, const uchar *key, uint idx, uint key_length)
{
  ...
  for (key_part = table->key_info[idx].key_part; key < key_end; key_part++, key += store_length)
  {
    ...
    if (!(key_part->key_type & (FIELDFLAG_NUMBER + FIELDFLAG_BINARY + FIELDFLAG_PACK)))
    {
      ...
      if (cs->coll->strnncollsp(cs, key, length, pos, char_length, 0))
        return 1;
      continue;
    }
    ...
    */
    ...
    if (cs->mbmaxlen > 1 && prefix_key)
    {
      ...
      */
      ...
      if ((charpos = my_charpos(cs, tmp.ptr(), tmp.ptr() + tmp.length(), char_length)) < tmp.length())
        tmp.length(charpos);
    }
...
int key_cmp2(KEY_PART_INFO *key_part, const uchar *key1, uint key1_length, const uchar *key2, uint key2_length)
{
  ...
  for (const uchar *end = key1 + key1_length; key1 < end; key1 += store_length, key2 += store_length, key_part++)
  {
    store_length = key_part->store_length;
    /* This key part allows null values; NULL is lower than everything */
    if (key_part->null_bit)
...
int key_rec_cmp(KEY **key, uchar *first_rec, uchar *second_rec)
{
  ...
  assert(bitmap_is_set(key_info->table->read_set, key_info->key_part->field->field_index));
  ...
  do
  {
    ...
    do
    {
      ...
      */
      if ((result = field->cmp_max(field->ptr + first_diff, field->ptr + sec_diff, key_part->length)))
        DBUG_RETURN(result);
    next_loop:
      key_part++;
      key_part_num++;
