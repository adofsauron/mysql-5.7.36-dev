...
static bool pack_header(uchar *forminfo, enum legacy_db_type table_type, List<Create_field> &create_fields,
                        uint info_length, uint screens, uint table_options, ulong data_offset, handler *file);
...
static bool make_empty_rec(THD *thd, int file, uint table_options, List<Create_field> &create_fields, uint reclength,
                           ulong data_offset, handler *handler);
/**
...
bool mysql_create_frm(THD *thd, const char *file_name, const char *db, const char *table, HA_CREATE_INFO *create_info,
                      List<Create_field> &create_fields, uint keys, KEY *key_info, handler *db_file)
{
  ...
  error = pack_header(forminfo, ha_legacy_type(create_info->db_type), create_fields, info_length, screens,
                      create_info->table_options, data_offset, db_file);
  ...
  if (error)
  {
    ...
    if (!(screen_buff = pack_screens(create_fields, &info_length, &screens, 1)))
      DBUG_RETURN(1);
    if (pack_header(forminfo, ha_legacy_type(create_info->db_type), create_fields, info_length, screens,
                    create_info->table_options, data_offset, db_file))
    {
      my_free(screen_buff);
      DBUG_RETURN(1);
  ...
  */
  if (create_info->comment.length > TABLE_COMMENT_MAXLEN)
  {
    ...
    while ((field = it++))
    {
      if (field->field && field->field->table && (real_table_name = field->field->table->s->table_name.str))
        break;
    }
    if (validate_comment_length(thd, create_info->comment.str, &create_info->comment.length, TABLE_COMMENT_MAXLEN,
                                ER_TOO_LONG_TABLE_COMMENT, real_table_name))
    {
      my_free(screen_buff);
      DBUG_RETURN(true);
    }
  ...
  */
  if (create_info->tablespace)
  {
    ...
    */
    if (tablespace_length > 0 && !(create_info->options & HA_LEX_CREATE_TMP_TABLE))
      assert(thd->mdl_context.owns_equal_or_stronger_lock(MDL_key::TABLESPACE, "", create_info->tablespace,
  ...
  if ((file = create_frm(thd, file_name, db, table, reclength, fileinfo, create_info, keys, key_info)) < 0)
  {
    my_free(screen_buff);
    DBUG_RETURN(1);
  }
  ...
  keybuff = (uchar *)my_malloc(key_memory_frm, key_buff_length, MYF(0));
  ...
  */
  ...
  if (mysql_file_pwrite(file, fileinfo, 64, 0L, MYF_RW) ||
      mysql_file_pwrite(file, keybuff, key_info_length, (ulong)uint2korr(fileinfo + 6), MYF_RW))
    goto err;
  mysql_file_seek(file, (ulong)uint2korr(fileinfo + 6) + (ulong)key_buff_length, MY_SEEK_SET, MYF(0));
  if (make_empty_rec(thd, file, create_info->table_options, create_fields, reclength, data_offset, db_file))
    goto err;
  ...
  if (mysql_file_write(file, (const uchar *)buff, 2, MYF(MY_NABP)) ||
      mysql_file_write(file, (const uchar *)create_info->connect_string.str, create_info->connect_string.length,
                       MYF(MY_NABP)))
    goto err;

  int2store(buff, static_cast<uint16>(str_db_type.length));
  if (mysql_file_write(file, (const uchar *)buff, 2, MYF(MY_NABP)) ||
      mysql_file_write(file, (const uchar *)str_db_type.str, str_db_type.length, MYF(MY_NABP)))
    goto err;
  ...
  if (part_info)
  {
    ...
    if (mysql_file_write(file, (const uchar *)buff, 4, MYF_RW) ||
        mysql_file_write(file, (const uchar *)part_info->part_info_string, part_info->part_info_len + 1, MYF_RW) ||
  ...
  for (i = 0; i < keys; i++)
  {
    if (key_info[i].parser_name)
    {
      if (mysql_file_write(file, (const uchar *)key_info[i].parser_name->str, key_info[i].parser_name->length + 1,
                           MYF(MY_NABP)))
        goto err;
    }
  }
  if (forminfo[46] == (uchar)255)
  {
    ...
    if (mysql_file_write(file, comment_length_buff, 2, MYF(MY_NABP)) ||
        mysql_file_write(file, (uchar *)create_info->comment.str, create_info->comment.length, MYF(MY_NABP)))
      goto err;
  }
  ...
  /* "Format section" with additional table and column properties */
  {
    ...
    if (!(format_section_buff = (uchar *)my_malloc(key_memory_frm, format_section_length, MYF(MY_WME))))
      goto err;
    ptr = format_section_buff;

    /* header */
    const uint format_section_flags = create_info->storage_media;  // 3 bits
    ...
    *ptr = 0; /* tablespace string terminating zero */
    ...
    if (mysql_file_write(file, format_section_buff, format_section_length, MYF_RW))
    {
      my_free(format_section_buff);
      goto err;
    }
    DBUG_PRINT("info", ("wrote format section, length: %u", format_section_length));
    my_free(format_section_buff);
  }
  ...
  /* Write out the COMPRESS table attribute */
  {
    ...
    if (mysql_file_write(file, length_buff, 2, MYF(MY_NABP)) ||
        mysql_file_write(file, (uchar *)create_info->compress.str, create_info->compress.length, MYF(MY_NABP)))
      goto err;
  }
  ...
  /* Write out the ENCRYPT table attribute */
  {
    ...
    int2store(length_buff, static_cast<uint16>(create_info->encrypt_type.length));

    if (mysql_file_write(file, length_buff, 2, MYF(MY_NABP)) ||
        mysql_file_write(file, (uchar *)create_info->encrypt_type.str, create_info->encrypt_type.length, MYF(MY_NABP)))
      goto err;
  }

  mysql_file_seek(file, filepos, MY_SEEK_SET, MYF(0));
  if (mysql_file_write(file, forminfo, 288, MYF_RW) || mysql_file_write(file, screen_buff, info_length, MYF_RW) ||
  ...
  if (opt_sync_frm && !(create_info->options & HA_LEX_CREATE_TMP_TABLE) &&
      (mysql_file_sync(file, MYF(MY_WME)) || my_sync_dir_by_file(file_name, MYF(MY_WME))))
    goto err2;
...
int rea_create_table(THD *thd, const char *path, const char *db, const char *table_name, HA_CREATE_INFO *create_info,
                     List<Create_field> &create_fields, uint keys, KEY *key_info, handler *file, bool no_ha_table)
{
  ...
  if (mysql_create_frm(thd, frm_name, db, table_name, create_info, create_fields, keys, key_info, file))
  ...
  if (!no_ha_table && ha_create_table(thd, path, db, table_name, create_info, 0))
    goto err_handler;
  DBUG_RETURN(0);
...
static uint pack_keys(uchar *keybuff, uint key_count, KEY *keyinfo, ulong data_offset)
{
  ...
  for (key = keyinfo, end = keyinfo + key_count; key != end; key++)
  {
    ...
    DBUG_PRINT("loop",
               ("flags: %lu  key_parts: %d at 0x%lx", key->flags, key->user_defined_key_parts, (long)key->key_part));
  ...
  *(pos++) = 0;
  for (key = keyinfo, end = keyinfo + key_count; key != end; key++)
  {
    if (key->flags & HA_USES_COMMENT)
    {
      ...
      uchar *tmp = (uchar *)my_stpnmov((char *)pos + 2, key->comment.str, key->comment.length);
      pos = tmp;
    }
  }
                        ...
                        uint info_length, uint screens, uint table_options, ulong data_offset, handler *file)
{
  ...
  while ((field = it++))
  {
    ...
    if (validate_comment_length(thd, field->comment.str, &field->comment.length, COLUMN_COMMENT_MAXLEN,
                                ER_TOO_LONG_FIELD_COMMENT, (char *)field->field_name))
      DBUG_RETURN(true);
    if (field->gcol_info)
    {
      ...
      field->gcol_info->expr_item->print(&s, enum_query_type(QT_NO_DB | QT_NO_TABLE));
      ...
      */
      ...
      uint tmp_len = system_charset_info->cset->charpos(
          system_charset_info, field->gcol_info->expr_str.str,
          field->gcol_info->expr_str.str + field->gcol_info->expr_str.length, GENERATED_COLUMN_EXPRESSION_MAXLEN);

      if (tmp_len < field->gcol_info->expr_str.length)
      {
        my_error(ER_WRONG_STRING_LENGTH, MYF(0), field->gcol_info->expr_str.str, "GENERATED COLUMN EXPRESSION",
    ...
    */
    if (field->sql_type == MYSQL_TYPE_TIMESTAMP && MTYP_TYPENR(field->unireg_check) != Field::NONE && !time_stamp_pos)
    ...
    if (field->interval)
    {
      ...
      if (field->charset->mbminlen > 1)
      {
        ...
        *field->interval = *field->save_interval;
        field->interval->type_names = (const char **)sql_alloc(sizeof(char *) * (field->interval->count + 1));
        field->interval->type_names[field->interval->count] = 0;
        field->interval->type_lengths = (uint *)sql_alloc(sizeof(uint) * field->interval->count);
        ...
        for (uint pos = 0; pos < field->interval->count; pos++)
        {
          ...
          field->interval->type_names[pos] = dst = (char *)sql_alloc(hex_length + 1);
  ...
  if (info_length + (ulong)create_fields.elements * FCOMP + 288 + n_length + int_length + com_length +
              gcol_info_length >
          65535L ||
      int_count > 255)
  {
  ...
  length = (info_length + create_fields.elements * FCOMP + 288 + n_length + int_length + com_length + gcol_info_length);
  ...
  if (comment_length)
  {
    ...
    while ((field = it++))
    {
      if (field->comment.length)
        if (mysql_file_write(file, (uchar *)field->comment.str, field->comment.length, MYF_RW))
          DBUG_RETURN(1);
    }
  }
  if (gcol_info_length)
      ...
      */
      if (field->gcol_info && field->gcol_info->expr_str.length)
      {
        ...
        if (my_write(file, buff, FRM_GCOL_HEADER_SIZE, MYF_RW))
          DBUG_RETURN(1);
        if (my_write(file, (uchar *)field->gcol_info->expr_str.str, field->gcol_info->expr_str.length, MYF_RW))
          DBUG_RETURN(1);
      }
    }
  }
...
bool make_default_value(THD *thd, TABLE *table, Create_field *field, uchar *rec_pos, uchar *null_pos, uint *null_count)
{
  Field *regfield =
      make_field(table->s, rec_pos + field->offset, field->length, null_pos + *null_count / 8, *null_count & 7,
                 field->pack_flag, field->sql_type, field->charset, field->geom_type, field->unireg_check,
                 field->save_interval ? field->save_interval : field->interval, field->field_name);
  if (!regfield)
  ...
  if (field->def)
  {
    ...
    */
    ...
    if (res != TYPE_OK && res != TYPE_NOTE_TIME_TRUNCATED && res != TYPE_NOTE_TRUNCATED)
    {
      /*
  ...
  else if (regfield->real_type() == MYSQL_TYPE_ENUM && (field->flags & NOT_NULL_FLAG))
  {
...
static bool make_empty_rec(THD *thd, File file, uint table_options, List<Create_field> &create_fields, uint reclength,
                           ulong data_offset, handler *handler)
{
  int error = 0;
  uint null_count;
  ...
  if (!(buff = (uchar *)my_malloc(key_memory_frm, (size_t)reclength, MYF(MY_WME | MY_ZEROFILL))))
  {
    DBUG_RETURN(1);
  }
  ...
  while ((field = it++))
  {
    if (field->stored_in_db)
    {
      ...
      */
      if (make_default_value(thd, &table, field, buff + data_offset, null_pos, &null_count))
      {
        error = 1;
        goto err;
      }
