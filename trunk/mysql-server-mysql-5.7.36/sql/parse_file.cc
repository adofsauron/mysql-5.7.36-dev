...
static my_bool write_parameter(IO_CACHE *file, uchar *base, File_option *parameter)
{
  ...
  switch (parameter->type)
  {
    ...
    case FILE_OPTIONS_TIMESTAMP:
    {
      ...
      if (my_b_append(file, (const uchar *)val_s->str, PARSE_FILE_TIMESTAMPLENGTH))
        DBUG_RETURN(TRUE);
      break;
    }
    case FILE_OPTIONS_STRLIST:
    {
      ...
      while ((str = it++))
      {
        ...
        if ((!first && my_b_append(file, (const uchar *)STRING_WITH_LEN(" "))) ||
            my_b_append(file, (const uchar *)STRING_WITH_LEN("\'")) || write_escaped_string(file, str) ||
                                   ...
                                   uchar *base, File_option *parameters)
{
  ...
  DBUG_PRINT("enter", ("Dir: %s, file: %s, base 0x%lx", dir ? dir->str : "(null)", file_name->str, (ulong)base));
  ...
  {
    if (my_b_append(&file, (const uchar *)param->name.str, param->name.length) ||
        my_b_append(&file, (const uchar *)STRING_WITH_LEN("=")) || write_parameter(&file, base, param) ||
        my_b_append(&file, (const uchar *)STRING_WITH_LEN("\n")))
      goto err_w_cache;
  }
...
File_parser *sql_parse_prepare(const LEX_STRING *file_name, MEM_ROOT *mem_root, bool bad_format_errors)
{
  ...
  if (!(buff = (char *)alloc_root(mem_root, static_cast<size_t>(stat_info.st_size) + 1)))
  {
    DBUG_RETURN(0);
  }
  ...
  if ((len = mysql_file_read(file, (uchar *)buff, static_cast<size_t>(stat_info.st_size), MYF(MY_WME))) ==
      MY_FILE_ERROR)
  {
