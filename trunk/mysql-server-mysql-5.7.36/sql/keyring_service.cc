...
static my_bool key_fetch(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  if (plugin)
  {
    ...
    key_data->result = keyring->mysql_key_fetch(key_data->key_id, key_data->key_type_to_fetch, key_data->user_id,
...
static my_bool key_store(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  {
    st_mysql_keyring *keyring = (st_mysql_keyring *)plugin_decl(plugin)->info;
    key_data->result = keyring->mysql_key_store(key_data->key_id, key_data->key_type_to_store, key_data->user_id,
...
static my_bool key_generate(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  {
    st_mysql_keyring *keyring = (st_mysql_keyring *)plugin_decl(plugin)->info;
    key_data->result = keyring->mysql_key_generate(key_data->key_id, key_data->key_type_to_store, key_data->user_id,
