...
static const LEX_STRING trg_action_time_type_names[] = {{C_STRING_WITH_LEN("BEFORE")}, {C_STRING_WITH_LEN("AFTER")}};

static const LEX_STRING trg_event_type_names[] = {
    {C_STRING_WITH_LEN("INSERT")}, {C_STRING_WITH_LEN("UPDATE")}, {C_STRING_WITH_LEN("DELETE")}};
...
static void get_cs_converted_string_value(THD *thd, String *input_str, String *output_str, const CHARSET_INFO *cs,
                                          bool use_hex);
...
static my_bool show_plugins(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  if (plugin_dl)
  {
    ...
    table->field[6]->store(version_buf, make_version_string(version_buf, sizeof(version_buf), plugin_dl->version), cs);
...
int fill_plugins(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  if (plugin_foreach_with_mask(thd, show_plugins, MYSQL_ANY_PLUGIN, ~PLUGIN_IS_FREED, tables->table))
    DBUG_RETURN(1);
...
static uchar *db_dirs_hash_get_key(const uchar *data, size_t *len_ret, my_bool MY_ATTRIBUTE((unused)))
{
...
bool push_ignored_db_dir(char *path)
{
  ...
  if (!my_multi_malloc(key_memory_ignored_db, 0, &new_elt, sizeof(LEX_STRING), &new_elt_buffer, path_len + 1, NullS))
    return true;
...
bool ignore_db_dirs_process_additions()
{
  ...
  if (my_hash_init(&ignore_db_dirs_hash, lower_case_table_names ? character_set_filesystem : &my_charset_bin, 0, 0, 0,
                   db_dirs_hash_get_key, my_free, HASH_UNIQUE, key_memory_ignored_db))
    return true;

  /* len starts from 1 because of the terminating zero. */
  len = 1;
  LEX_STRING **iter;
  for (iter = ignore_db_dirs_array->begin(); iter != ignore_db_dirs_array->end(); ++iter)
  {
...
find_files_result find_files(THD *thd, List<LEX_STRING> *files, const char *db, const char *path, const char *wild,
                             bool dir, MEM_ROOT *tmp_mem_root)
{
  uint i;
  MY_DIR *dirp;
  ...
  if (!(dirp = my_dir(path, MYF(dir ? MY_WANT_STAT : 0))))
  {
    ...
    else
    {
      ...
      my_error(ER_CANT_READ_DIR, MYF(0), path, my_errno(), my_strerror(errbuf, sizeof(errbuf), my_errno()));
    }
  ...
  for (i = 0; i < dirp->number_off_files; i++)
  {
    ...
    if (wild)
    {
      ...
      {
        if (my_wildcmp(files_charset_info, uname, uname + file_name_len, wild, wild + wild_length, wild_prefix,
                       wild_one, wild_many))
          continue;
      }
...
#endif
    if (!(file_name = tmp_mem_root ? make_lex_string_root(tmp_mem_root, file_name, uname, file_name_len, TRUE)
  ...
  */
  const char *get_view_access_denied_message()
  {
    if (!m_view_access_denied_message_ptr)
    {
      ...
      my_snprintf(m_view_access_denied_message, MYSQL_ERRMSG_SIZE, ER(ER_TABLEACCESS_DENIED_ERROR), "SHOW VIEW",
                  m_sctx->priv_user().str, m_sctx->host_or_ip().str, m_top_view->get_table_name());
    }
    ...
    */
    ...
    switch (sql_errno)
    {
      ...
      case ER_SP_DOES_NOT_EXIST:
        ...
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_VIEW_INVALID, ER(ER_VIEW_INVALID),
  ...
  */
  if (!table_list->is_view() && table_list->table->s->db_type() &&
    ...
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_DEPRECATED_SYNTAX,
                        ER_THD(thd, ER_PARTITION_ENGINE_DEPRECATED_FOR_TABLE), table_list->db, table_list->table_name);

  if ((table_list->is_view() ? view_store_create_info(thd, table_list, &buffer)
                             : store_create_info(thd, table_list, &buffer, NULL, FALSE /* show_database */)))
    goto exit;
  ...
  if (table_list->is_view())
  {
    protocol->store(buffer.ptr(), buffer.length(), table_list->view_creation_ctx->get_client_cs());

    protocol->store(table_list->view_creation_ctx->get_client_cs()->csname, system_charset_info);

    protocol->store(table_list->view_creation_ctx->get_connection_cl()->name, system_charset_info);
  }
  else
...
bool mysqld_show_create_db(THD *thd, char *dbname, HA_CREATE_INFO *create_info)
{
  ...
  if (sctx->check_access(DB_ACLS))
    ...
    db_access = (acl_get(sctx->host().str, sctx->ip().str, sctx->priv_user().str, dbname, 0) | sctx->master_access());
  if (!(db_access & DB_ACLS) && check_grant_db(thd, dbname))
  {
    my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), sctx->priv_user().str, sctx->host_or_ip().str, dbname);
    query_logger.general_log_print(thd, COM_INIT_DB, ER(ER_DBACCESS_DENIED_ERROR), sctx->priv_user().str,
...
void mysqld_list_fields(THD *thd, TABLE_LIST *table_list, const char *wild)
{
  ...
  if (open_tables_for_query(thd, table_list, MYSQL_OPEN_FORCE_SHARED_HIGH_PRIO_MDL))
    DBUG_VOID_RETURN;
  ...
  for (ptr = table->field; (field = *ptr); ptr++)
  {
    ...
    {
      if (table_list->is_view())
        field_list.push_back(new Item_ident_for_show(field, table_list->view_db.str, table_list->view_name.str));
      else
...
void append_identifier(THD *thd, String *packet, const char *name, size_t length, const CHARSET_INFO *from_cs,
...
static bool print_default_clause(THD *thd, Field *field, String *def_value, bool quoted)
{
  ...
  const bool has_default = (field_type != FIELD_TYPE_BLOB && !(field->flags & NO_DEFAULT_VALUE_FLAG) &&
                      ...
                      bool show_database)
{
  ...
  if (create_info_arg && (create_info_arg->options & HA_LEX_CREATE_IF_NOT_EXISTS))
  ...
  if (show_database)
  {
    const LEX_STRING *const db = table_list->schema_table ? &INFORMATION_SCHEMA_NAME : &table->s->db;
  ...
  for (ptr = table->field; (field = *ptr); ptr++)
  {
    ...
    */
    if (thd->variables.show_old_temporals &&
        (field_type == MYSQL_TYPE_TIME || field_type == MYSQL_TYPE_DATETIME || field_type == MYSQL_TYPE_TIMESTAMP))
    ...
    if (field->gcol_info)
    {
      ...
      packet->append(field->gcol_info->expr_str.str, field->gcol_info->expr_str.length, system_charset_info);
      packet->append(STRING_WITH_LEN(")"));
  ...
  */
  ...
  if (!(thd->variables.sql_mode & MODE_NO_TABLE_OPTIONS) && !foreign_db_mode)
  {
    ...
    if (share->tablespace || share->default_storage_media != HA_SM_DEFAULT)
    {
      packet->append(STRING_WITH_LEN(" /*!50100"));
      if (share->tablespace)
      {
        packet->append(STRING_WITH_LEN(" TABLESPACE "));
        append_identifier(thd, packet, share->tablespace, strlen(share->tablespace));
      }
    ...
    */
    if (!create_info_arg || (create_info_arg->used_fields & HA_CREATE_USED_ENGINE))
    {
      ...
      */
      if (table->part_info)
      {
        packet->append(ha_resolve_storage_engine_name(table->part_info->default_engine_type));
      }
      else
      {
    ...
    */
    if (thd->variables.show_create_table_verbosity)
    {
      ...
      {
        row_type = ((share->db_options_in_use & HA_OPTION_COMPRESS_RECORD) ? ROW_TYPE_COMPRESSED
                    : (share->db_options_in_use & HA_OPTION_PACK_RECORD)   ? ROW_TYPE_DYNAMIC
                                                                           : ROW_TYPE_FIXED);
      }
  ...
  {
    if (table->part_info &&
        !(table->s->db_type()->partition_flags && (table->s->db_type()->partition_flags() & HA_USE_AUTO_PARTITION) &&
          table->part_info->is_auto_partitioned))
    {
      ...
      */
      ...
      if ((part_syntax = generate_partition_syntax(table->part_info, &part_syntax_len, FALSE, show_table_options, NULL,
                                                   NULL, comment_start.c_ptr())))
      {
        packet->append(comment_start);
        if (packet->append(part_syntax, part_syntax_len) || packet->append(STRING_WITH_LEN(" */")))
          error = 1;
        my_free(part_syntax);
      }
    }
...
static void store_key_options(THD *thd, String *packet, TABLE *table, KEY *key_info)
{
  ...
  if (!(thd->variables.sql_mode & MODE_NO_KEY_OPTIONS) && !limited_mysql_mode && !foreign_db_mode)
  {
    ...
    assert(MY_TEST(key_info->flags & HA_USES_COMMENT) == (key_info->comment.length > 0));
...
void append_definer(THD *thd, String *buffer, const LEX_CSTRING &definer_user, const LEX_CSTRING &definer_host)
{
...
int view_store_create_info(THD *thd, TABLE_LIST *table, String *buff)
{
  ...
  else
  {
    ...
    */
    ...
    {
      if (strcmp(table->view_db.str, tbl->is_view() ? tbl->view_db.str : tbl->db) != 0)
      {
  ...
  */
  table->view_query()->unit->print(buff,
                                   enum_query_type(QT_TO_ARGUMENT_CHARSET | (compact_view_format ? QT_NO_DB : 0)));
...
class thread_info : public Sql_alloc
{
 public:
  thread_info()
      : thread_id(0), start_time(0), command(0), user(NULL), host(NULL), db(NULL), proc_info(NULL), state_info(NULL)
  {
  }
...
class List_process_list : public Do_THD_Impl
{
 ...
 public:
  List_process_list(const char *user_value, Thread_info_array *thread_infos, THD *thd_value, size_t max_query_length)
      : m_user(user_value), m_thread_infos(thread_infos), m_client_thd(thd_value), m_max_query_length(max_query_length)
  {
  }
  ...
  virtual void operator()(THD *inspect_thd)
  {
    ...
    if ((!inspect_thd->get_protocol()->connection_alive() && !inspect_thd->system_thread) ||
        (m_user && (inspect_thd->system_thread || !inspect_sctx_user.str || strcmp(inspect_sctx_user.str, m_user))))
      return;
    ...
    /* HOST */
    if (inspect_thd->peer_port && (inspect_sctx_host.length || inspect_sctx->ip().length) &&
        m_client_thd->security_context()->host_or_ip().str[0])
    {
      if ((thd_info->host = (char *)m_client_thd->alloc(LIST_PROCESS_HOST_LEN + 1)))
        my_snprintf((char *)thd_info->host, LIST_PROCESS_HOST_LEN, "%s:%u", inspect_sctx_host_or_ip.str,
                    inspect_thd->peer_port);
    }
    else
      thd_info->host = m_client_thd->mem_strdup(inspect_sctx_host_or_ip.str[0] ? inspect_sctx_host_or_ip.str
                                                : inspect_sctx_host.length     ? inspect_sctx_host.str
                                                                               : "");

    DBUG_EXECUTE_IF("processlist_acquiring_dump_threads_LOCK_thd_data", {
      if (inspect_thd->get_command() == COM_BINLOG_DUMP || inspect_thd->get_command() == COM_BINLOG_DUMP_GTID)
    ...
    mysql_mutex_lock(&inspect_thd->LOCK_thd_query);
    {
      ...
      */
      ...
      if (query_str)
      {
        ...
        thd_info->query_string = CSET_STRING(q, q ? width : 0, inspect_thd->charset());
      }
    }
...
class Fill_process_list : public Do_THD_Impl
{
 ...
 public:
  ...
  virtual void operator()(THD *inspect_thd)
  {
    ...
    LEX_CSTRING inspect_sctx_host = inspect_sctx->host();
    LEX_CSTRING inspect_sctx_host_or_ip = inspect_sctx->host_or_ip();
    const char *client_priv_user = m_client_thd->security_context()->priv_user().str;
    const char *user = m_client_thd->security_context()->check_access(PROCESS_ACL) ? NullS : client_priv_user;

    if ((!inspect_thd->get_protocol()->connection_alive() && !inspect_thd->system_thread) ||
        (user && (inspect_thd->system_thread || !inspect_sctx_user.str || strcmp(inspect_sctx_user.str, user))))
      return;
        ...
        m_client_thd->security_context()->host_or_ip().str[0])
    {
      ...
      my_snprintf(host, LIST_PROCESS_HOST_LEN, "%s:%u", inspect_sctx_host_or_ip.str, inspect_thd->peer_port);
      table->field[2]->store(host, strlen(host), system_charset_info);
    }
    else
      table->field[2]->store(inspect_sctx_host_or_ip.str, inspect_sctx_host_or_ip.length, system_charset_info);
    ...
    if (inspect_thd->killed == THD::KILL_CONNECTION)
    {
      ...
      table->field[4]->store(command_name[inspect_thd->get_command()].str,
                             command_name[inspect_thd->get_command()].length, system_charset_info);
...
{
 public:
  bool operator()(const st_mysql_show_var &var1, const st_mysql_show_var &var2)
  {
...
static void shrink_var_array(Status_var_array *array)
{
  ...
  array->erase(std::remove_if(array->begin(), array->end(), is_show_undef), array->end());
  if (array->empty())
...
int add_status_vars(const SHOW_VAR *list)
{
  ...
  {
    my_error(ER_OUTOFMEMORY, MYF(ME_FATALERROR), static_cast<int>(sizeof(Status_var_array::value_type)));
    return 1;
  }
...
bool get_status_var(THD *thd, SHOW_VAR *list, const char *name, char *const value, enum_var_type var_type,
                    size_t *length)
{
  for (; list->name; list++)
  {
...
{
  return get_one_variable_ext(thd, thd, variable, value_type, show_type, status_var, charset, buff, length);
}
...
const char *get_one_variable_ext(THD *running_thd, THD *target_thd, const SHOW_VAR *variable, enum_var_type value_type,
                                 SHOW_TYPE show_type, system_status_var *status_var, const CHARSET_INFO **charset,
...
static bool show_status_array(THD *thd, const char *wild, SHOW_VAR *variables, enum enum_var_type value_type,
                              struct system_status_var *status_var, const char *prefix, TABLE_LIST *tl,
                              bool ucase_names, Item *cond)
{
  ...
  for (; variables->name; variables++)
  {
    ...
    */
    ...
    {
      show_status_array(thd, wild, (SHOW_VAR *)var->value, value_type, status_var, name_buffer, tl, ucase_names,
                        partial_cond);
    }
    else
    {
          ...
          (!partial_cond || partial_cond->val_int()))
      {
        ...
        pos = get_one_variable(thd, var, value_type, show_type, status_var, &charset, buff, &length);
        ...
        if (variables->type != SHOW_FUNC && value_type == OPT_GLOBAL &&
            mysql_audit_notify(thd, AUDIT_EVENT(MYSQL_AUDIT_GLOBAL_VARIABLE_GET), var->name, pos, length))
        {
          res = TRUE;
          goto end;
        }
...
bool convert_heap_table_to_ondisk(THD *thd, TABLE *table, int error)
{
  ...
  return (create_ondisk_from_heap(thd, table, param->start_recinfo, &param->recinfo, error, FALSE, NULL));
}

static int make_table_list(THD *thd, SELECT_LEX *sel, const LEX_CSTRING &db_name, const LEX_CSTRING &table_name)
{
...
bool get_lookup_value(THD *thd, Item_func *item_func, TABLE_LIST *table, LOOKUP_FIELD_VALUES *lookup_field_vals)
{
  ...
  if (item_func->functype() == Item_func::EQ_FUNC || item_func->functype() == Item_func::EQUAL_FUNC)
  {
    ...
    if (item_func->arguments()[0]->type() == Item::FIELD_ITEM && item_func->arguments()[1]->const_item())
    {
      idx_field = 0;
      idx_val = 1;
    }
    else if (item_func->arguments()[1]->type() == Item::FIELD_ITEM && item_func->arguments()[0]->const_item())
    {
      idx_field = 1;
      idx_val = 0;
    }
    ...
    if (!cs->coll->strnncollsp(cs, (uchar *)field_name1, strlen(field_name1), (uchar *)item_field->field_name,
                               strlen(item_field->field_name), 0))
    {
      thd->make_lex_string(&lookup_field_vals->db_value, tmp_str->ptr(), tmp_str->length(), FALSE);
    }
    /* Lookup value is table name */
    else if (!cs->coll->strnncollsp(cs, (uchar *)field_name2, strlen(field_name2), (uchar *)item_field->field_name,
                                    strlen(item_field->field_name), 0))
    {
      thd->make_lex_string(&lookup_field_vals->table_value, tmp_str->ptr(), tmp_str->length(), FALSE);
    }
  }
  return 0;
}
...
bool calc_lookup_values_from_cond(THD *thd, Item *cond, TABLE_LIST *table, LOOKUP_FIELD_VALUES *lookup_field_vals)
{
  if (!cond)
    return 0;
...
bool get_lookup_field_values(THD *thd, Item *cond, TABLE_LIST *tables, LOOKUP_FIELD_VALUES *lookup_field_values)
{
  LEX *lex = thd->lex;
  ...
  switch (lex->sql_command)
  {
    case SQLCOM_SHOW_DATABASES:
      if (wild)
      {
        thd->make_lex_string(&lookup_field_values->db_value, wild, strlen(wild), 0);
    ...
    case SQLCOM_SHOW_EVENTS:
      thd->make_lex_string(&lookup_field_values->db_value, lex->select_lex->db, strlen(lex->select_lex->db), 0);
      if (wild)
      {
        thd->make_lex_string(&lookup_field_values->table_value, wild, strlen(wild), 0);
  ...
  if (lower_case_table_names && !rc)
  {
    ...
    */
    ...
    if (lookup_field_values->table_value.str && lookup_field_values->table_value.str[0])
...
int make_db_list(THD *thd, List<LEX_STRING> *files, LOOKUP_FIELD_VALUES *lookup_field_vals, bool *with_i_schema,
                 MEM_ROOT *tmp_mem_root)
{
  LEX_STRING *i_s_name_copy = 0;
  i_s_name_copy =
      thd->make_lex_string(i_s_name_copy, INFORMATION_SCHEMA_NAME.str, INFORMATION_SCHEMA_NAME.length, TRUE);
  *with_i_schema = 0;
  if (lookup_field_vals->wild_db_value)
  {
    ...
    */
    ...
    return (find_files(thd, files, NullS, mysql_data_home, lookup_field_vals->db_value.str, 1, tmp_mem_root) !=
            FIND_FILES_OK);
  }
  ...
  */
  if (lookup_field_vals->db_value.str)
  {
    ...
    if (is_infoschema_db(lookup_field_vals->db_value.str, lookup_field_vals->db_value.length))
    {
      *with_i_schema = 1;
      if (files->push_back(i_s_name_copy))
        return 1;
  ...
  */
  if (files->push_back(i_s_name_copy))
    return 1;
  *with_i_schema = 1;
  return (find_files(thd, files, NullS, mysql_data_home, NullS, 1, tmp_mem_root) != FIND_FILES_OK);
}
...
static my_bool add_schema_table(THD *thd, plugin_ref plugin, void *p_data)
{
  ...
  if ((file_name = thd->make_lex_string(file_name, schema_table->table_name, strlen(schema_table->table_name), TRUE)) &&
...
int schema_tables_add(THD *thd, List<LEX_STRING> *files, const char *wild)
{
  ...
  for (; tmp_schema_table->table_name; tmp_schema_table++)
  {
    ...
    if ((file_name = thd->make_lex_string(file_name, tmp_schema_table->table_name, strlen(tmp_schema_table->table_name),
                                          TRUE)) &&
        !files->push_back(file_name))
      continue;
    DBUG_RETURN(1);
...
static int make_table_name_list(THD *thd, List<LEX_STRING> *table_names, LEX *lex,
                                LOOKUP_FIELD_VALUES *lookup_field_vals, bool with_i_schema, LEX_STRING *db_name,
                                MEM_ROOT *tmp_mem_root)
{
  ...
  if (!lookup_field_vals->wild_table_value && lookup_field_vals->table_value.str)
  {
    ...
    if (with_i_schema)
    {
      ...
      ST_SCHEMA_TABLE *schema_table = find_schema_table(thd, lookup_field_vals->table_value.str);
      if (schema_table && !schema_table->hidden)
      {
        if (!(name = thd->make_lex_string(name, schema_table->table_name, strlen(schema_table->table_name), TRUE)) ||
    ...
    else
    {
      ...
      */
      (void)ha_find_files(thd, db_name->str, path, lookup_field_vals->table_value.str, 0, table_names);
    }
    return 0;
  }
  ...
  */
  ...
  find_files_result res =
      find_files(thd, table_names, db_name->str, path, lookup_field_vals->table_value.str, 0, tmp_mem_root);
                                      ...
                                      bool can_deadlock)
{
  ...
  */
  if (!thd->make_lex_string(&db_name_lex_cstr, orig_db_name->str, orig_db_name->length, FALSE) ||
      !thd->make_lex_string(&table_name_lex_cstr, orig_table_name->str, orig_table_name->length, FALSE))
    goto end;
  ...
  */
  if (make_table_list(thd, lex->select_lex, db_name_lex_cstr, table_name_lex_cstr))
    goto end;
  ...
  */
  if (!is_show_fields_or_keys && result && thd->is_error() && thd->get_stmt_da()->mysql_errno() == ER_NO_SUCH_TABLE)
  {
    /*
...
static int fill_schema_table_names(THD *thd, TABLE *table, LEX_STRING *db_name, LEX_STRING *table_name,
                                   bool with_i_schema, bool need_table_type)
{
  ...
  if (need_table_type)
  {
    ...
    else
    {
      ...
      char path[FN_REFLEN + 1];
      (void)build_table_filename(path, sizeof(path) - 1, db_name->str, table_name->str, reg_ext, 0);
      switch (dd_frm_type(thd, path, &not_used))
      {
        case FRMTYPE_ERROR:
          ...
          break;
        case FRMTYPE_TABLE:
          table->field[3]->store(STRING_WITH_LEN("BASE TABLE"), system_charset_info);
          break;
        case FRMTYPE_VIEW:
          table->field[3]->store(STRING_WITH_LEN("VIEW"), system_charset_info);
          break;
        default:
          assert(0);
      }
      if (thd->is_error() && thd->get_stmt_da()->mysql_errno() == ER_NO_SUCH_TABLE)
      {
        thd->clear_error();
        return 0;
      }
...
uint get_table_open_method(TABLE_LIST *tables, ST_SCHEMA_TABLE *schema_table, enum enum_schema_tables schema_table_idx)
{
  ...
  */
  if (schema_table->i_s_requested_object & OPTIMIZE_I_S_TABLE)
  {
    ...
    {
      star_table_open_method = min(star_table_open_method, schema_table->fields_info[field_indx].open_method);
...
{
  bool error;
  MDL_REQUEST_INIT(&table->mdl_request, MDL_key::TABLE, table->db, table->table_name, MDL_SHARED_HIGH_PRIO,
                   MDL_TRANSACTION);
...
static int fill_schema_table_from_frm(THD *thd, TABLE_LIST *tables, ST_SCHEMA_TABLE *schema_table, LEX_STRING *db_name,
                                      LEX_STRING *table_name, enum enum_schema_tables schema_table_idx,
                                      Open_tables_backup *open_tables_state_backup, bool can_deadlock)
{
  ...
  */
  ...
  if (!table_list.mdl_request.ticket)
  {
    ...
    */
    ...
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_I_S_SKIPPED_TABLE, ER(ER_WARN_I_S_SKIPPED_TABLE),
  ...
  if (schema_table->i_s_requested_object & OPEN_TRIGGER_ONLY)
  {
    ...
    if (!d.check_n_load(thd, true))
    {
      ...
      init_sql_alloc(key_memory_table_triggers_list, &tbl.mem_root, TABLE_ALLOC_BLOCK_SIZE, 0);
  ...
  */
  {
    ...
    share = get_table_share(thd, &table_list, key, key_length, OPEN_VIEW, &not_used, hash_value);
...

  {
    ...
    if (!open_table_from_share(thd, share, table_name->str, 0, (EXTRA_RECORD | OPEN_FRM_FILE_ONLY), thd->open_options,
                               &tbl, false))
    {
      tbl.s = share;
      table_list.table = &tbl;
...
class Trigger_error_handler : public Internal_error_handler
{
 public:
  virtual bool handle_condition(THD *thd, uint sql_errno, const char *sqlstate,
                                Sql_condition::enum_severity_level *level, const char *msg)
  {
    if (sql_errno == ER_PARSE_ERROR || sql_errno == ER_TRG_NO_DEFINER || sql_errno == ER_TRG_NO_CREATION_CTX)
      return true;
...
int get_all_tables(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  */
  ...
  tables->table_open_method = table_open_method = get_table_open_method(tables, schema_table, schema_table_idx);
  ...
  */
  if (lsel && lsel->table_list.first)
  {
    ...
    error = fill_schema_table_by_open(thd, &tmp_mem_root, TRUE, table, schema_table, &db_name, &table_name,
  ...
  DBUG_PRINT("INDEX VALUES", ("db_name='%s', table_name='%s'", STR_OR_NIL(lookup_field_vals.db_value.str),
  ...
  if (lookup_field_vals.table_value.length && !lookup_field_vals.wild_table_value)
  ...
  while ((db_name = it++))
  {
    ...
    if (!(check_access(thd, SELECT_ACL, db_name->str, &thd->col_access, NULL, 0, 1) ||
        ...
        acl_get(sctx->host().str, sctx->ip().str, sctx->priv_user().str, db_name->str, 0))
#endif
    {
      List<LEX_STRING> table_names;
      int res = make_table_name_list(thd, &table_names, lex, &lookup_field_vals, with_i_schema, db_name, &tmp_mem_root);
        ...
        if (!partial_cond || partial_cond->val_int())
        {
          ...
          */
          if (!table_open_method && schema_table_idx == SCH_TABLES &&
              (!lookup_field_vals.table_value.length || lookup_field_vals.wild_table_value))
          {
          ...
          {
            if (!(table_open_method & ~OPEN_FRM_ONLY) && !with_i_schema)
            {
              /*
              ...
              */
              ...
              int res = fill_schema_table_from_frm(thd, tables, schema_table, db_name, table_name, schema_table_idx,
                                                   &open_tables_state_backup, can_deadlock);
...
bool store_schema_shemata(THD *thd, TABLE *table, LEX_STRING *db_name, const CHARSET_INFO *cs)
{
  restore_record(table, s->default_values);
  table->field[0]->store(STRING_WITH_LEN("def"), system_charset_info);
...
int fill_schema_schemata(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  */
  ...
  if (make_db_list(thd, &db_names, &lookup_field_vals, &with_i_schema, &dummy_member.tmp_mem_root))
    DBUG_RETURN(1);

  /*
    If we have lookup db value we should check that the database exists
  */
  if (lookup_field_vals.db_value.str && !lookup_field_vals.wild_db_value && !with_i_schema)
  {
    ...
    if (!lookup_field_vals.db_value.str[0])
      DBUG_RETURN(0);
    path_len = build_table_filename(path, sizeof(path) - 1, lookup_field_vals.db_value.str, "", "", 0);
    path[path_len - 1] = 0;
  ...
  while ((db_name = it++))
  {
    ...
    {
      if (store_schema_shemata(thd, table, db_name, system_charset_info))
        DBUG_RETURN(1);
      with_i_schema = 0;
      continue;
    }
...
#endif
    {
      ...
      if (store_schema_shemata(thd, table, db_name, create.default_table_charset))
        DBUG_RETURN(1);
    }
  }
  DBUG_RETURN(0);
...
static int get_schema_tables_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                    LEX_STRING *table_name)
{
  ...
  else
  {
    ...
    if (is_partitioned)
    {
      ...
      */
      if (share->db_type() && is_ha_partition_handlerton(share->db_type()))
      {
        ...
        */
        if (thd->bootstrap)
          sql_print_warning(ER_THD(thd, ER_PARTITION_ENGINE_DEPRECATED_FOR_TABLE), share->db.str,
                            share->table_name.str);
        else
          push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_DEPRECATED_SYNTAX,
                              ER_THD(thd, ER_PARTITION_ENGINE_DEPRECATED_FOR_TABLE), share->db.str,
                              share->table_name.str);
      }
    }
    ...
    if (file)
    {
      ...
      if ((info_error = file->info(HA_STATUS_VARIABLE | HA_STATUS_TIME | HA_STATUS_VARIABLE_EXTRA | HA_STATUS_AUTO)) !=
          0)
        goto err;
...
err:
  if (res || info_error)
  {
    ...
    */
    ...
    if (thd->is_error())
    {
      push_warning(thd, Sql_condition::SL_WARNING, thd->get_stmt_da()->mysql_errno(),
...
void store_column_type(THD *thd, TABLE *table, Field *field, CHARSET_INFO *cs, uint offset)
{
  bool is_blob;
  ...
  */
  ...
  if (!tmp_buff)
    ...
    */
    tmp_buff = strchr(column_type.ptr(), ' ');
  table->field[offset]->store(column_type.ptr(), (tmp_buff ? tmp_buff - column_type.ptr() : column_type.length()), cs);

  is_blob = (field->type() == MYSQL_TYPE_BLOB);
  if (field->has_charset() || is_blob || field->real_type() == MYSQL_TYPE_VARCHAR ||  // For varbinary type
      field->real_type() == MYSQL_TYPE_STRING)                                        // For binary type
  {
    uint32 octet_max_length = field->max_display_length();
    if (is_blob && octet_max_length != 4294967295U)
      octet_max_length /= field->charset()->mbmaxlen;
    longlong char_max_len = is_blob ? (longlong)octet_max_length / field->charset()->mbminlen
  ...
  */
  ...
  switch (field->type())
  {
    ...
    case MYSQL_TYPE_LONGLONG:
      field_length = field->max_display_length() - ((field->flags & UNSIGNED_FLAG) ? 0 : 1);
      break;
    case MYSQL_TYPE_BIT:
...
static int get_schema_column_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                    LEX_STRING *table_name)
{
  LEX *lex = thd->lex;
  const char *wild = lex->wild ? lex->wild->ptr() : NullS;
  ...
  for (; (field = *ptr); ptr++)
  {
    ...
    col_access = get_column_grant(thd, &tables->grant, db_name->str, table_name->str, field->field_name) & COL_ACLS;
    ...
    table->field[IS_COLUMNS_PRIVILEGES]->store(tmp + 1, end == tmp ? 0 : (uint)(end - tmp - 1), cs);

#endif
    table->field[IS_COLUMNS_TABLE_CATALOG]->store(STRING_WITH_LEN("def"), cs);
    table->field[IS_COLUMNS_TABLE_SCHEMA]->store(db_name->str, db_name->length, cs);
    table->field[IS_COLUMNS_TABLE_NAME]->store(table_name->str, table_name->length, cs);
    table->field[IS_COLUMNS_COLUMN_NAME]->store(field->field_name, strlen(field->field_name), cs);
    ...
    {
      table->field[IS_COLUMNS_COLUMN_DEFAULT]->store(type.ptr(), type.length(), cs);
      table->field[IS_COLUMNS_COLUMN_DEFAULT]->set_notnull();
    }
    pos = (uchar *)((field->flags & NOT_NULL_FLAG) ? "NO" : "YES");
    table->field[IS_COLUMNS_IS_NULLABLE]->store((const char *)pos, strlen((const char *)pos), cs);
    store_column_type(thd, table, field, cs, IS_COLUMNS_DATA_TYPE);
    pos = (uchar *)((field->flags & PRI_KEY_FLAG)        ? "PRI"
                    : (field->flags & UNIQUE_KEY_FLAG)   ? "UNI"
                    : (field->flags & MULTIPLE_KEY_FLAG) ? "MUL"
                                                         : "");
    table->field[IS_COLUMNS_COLUMN_KEY]->store((const char *)pos, strlen((const char *)pos), cs);

    if (field->unireg_check == Field::NEXT_NUMBER)
      table->field[IS_COLUMNS_EXTRA]->store(STRING_WITH_LEN("auto_increment"), cs);
    ...
    if (field->gcol_info)
    {
      ...
      table->field[IS_COLUMNS_GENERATION_EXPRESSION]->store(field->gcol_info->expr_str.str,
                                                            field->gcol_info->expr_str.length, cs);
    }
    else
...
int fill_schema_charsets(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  for (cs = all_charsets; cs < all_charsets + array_elements(all_charsets); cs++)
  {
    ...
    if (tmp_cs && (tmp_cs->state & MY_CS_PRIMARY) && (tmp_cs->state & MY_CS_AVAILABLE) &&
        !(tmp_cs->state & MY_CS_HIDDEN) && !(wild && wild[0] && wild_case_compare(scs, tmp_cs->csname, wild)))
    {
      const char *comment;
      restore_record(table, s->default_values);
...
static my_bool iter_schema_engines(THD *thd, plugin_ref plugin, void *ptable)
{
  ...
  if (!(hton->flags & HTON_HIDDEN))
  {
    ...
    {
      LEX_STRING yesno[2] = {{C_STRING_WITH_LEN("NO")}, {C_STRING_WITH_LEN("YES")}};
      LEX_STRING *tmp;
...
int fill_schema_engines(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  if (plugin_foreach_with_mask(thd, iter_schema_engines, MYSQL_STORAGE_ENGINE_PLUGIN, ~PLUGIN_IS_FREED, tables->table))
    DBUG_RETURN(1);
  DBUG_RETURN(0);
}
...
bool store_schema_params(THD *thd, TABLE *table, TABLE *proc_table, const char *wild, bool full_access,
                         const char *sp_user)
{
  TABLE_SHARE share;
  TABLE tbl;
  CHARSET_INFO *cs = system_charset_info;
  char params_buff[MAX_FIELD_WIDTH], returns_buff[MAX_FIELD_WIDTH], sp_db_buff[NAME_LEN], sp_name_buff[NAME_LEN],
      path[FN_REFLEN], definer_buff[USERNAME_LENGTH + HOSTNAME_LENGTH + 1];
  ...
  if (!full_access && check_some_routine_access(thd, sp_db.ptr(), sp_name.ptr(), routine_type == SP_TYPE_PROCEDURE))
    DBUG_RETURN(0);
  ...
  if (routine_type == SP_TYPE_FUNCTION)
    get_field(thd->mem_root, proc_table->field[MYSQL_PROC_FIELD_RETURNS], &returns);

  sp = sp_load_for_information_schema(thd, proc_table, &sp_db, &sp_name,
                                      (sql_mode_t)proc_table->field[MYSQL_PROC_FIELD_SQL_MODE]->val_int(), routine_type,
                                      returns.c_ptr_safe(), params.c_ptr_safe(), &free_sp_head);
  ...
  if (sp)
  {
    ...
    if (routine_type == SP_TYPE_FUNCTION)
    {
      ...
      table->field[IS_PARAMETERS_SPECIFIC_SCHEMA]->store(sp_db.ptr(), sp_db.length(), cs);
      table->field[IS_PARAMETERS_SPECIFIC_NAME]->store(sp_name.ptr(), sp_name.length(), cs);
      table->field[IS_PARAMETERS_ORDINAL_POSITION]->store((longlong)0, TRUE);
      get_field(thd->mem_root, proc_table->field[MYSQL_PROC_MYSQL_TYPE], &tmp_string);
      table->field[IS_PARAMETERS_ROUTINE_TYPE]->store(tmp_string.ptr(), tmp_string.length(), cs);
      field_def = &sp->m_return_field_def;
      field =
          make_field(&share, (uchar *)0, field_def->length, (uchar *)"", 0, field_def->pack_flag, field_def->sql_type,
                     field_def->charset, field_def->geom_type, Field::NONE, field_def->interval, "");
    ...
    for (uint i = 0; i < sp_root_parsing_ctx->context_var_count(); i++)
    {
      ...
      table->field[IS_PARAMETERS_SPECIFIC_SCHEMA]->store(sp_db.ptr(), sp_db.length(), cs);
      table->field[IS_PARAMETERS_SPECIFIC_NAME]->store(sp_name.ptr(), sp_name.length(), cs);
      table->field[IS_PARAMETERS_ORDINAL_POSITION]->store((longlong)i + 1, TRUE);
      table->field[IS_PARAMETERS_PARAMETER_MODE]->store(tmp_buff, strlen(tmp_buff), cs);
      table->field[IS_PARAMETERS_PARAMETER_MODE]->set_notnull();
      table->field[IS_PARAMETERS_PARAMETER_NAME]->store(spvar->name.str, spvar->name.length, cs);
      ...
      field =
          make_field(&share, (uchar *)0, field_def->length, (uchar *)"", 0, field_def->pack_flag, field_def->sql_type,
                     field_def->charset, field_def->geom_type, Field::NONE, field_def->interval, spvar->name.str);
                       ...
                       const char *sp_user)
{
  ...
  LEX *lex = thd->lex;
  CHARSET_INFO *cs = system_charset_info;
  char sp_db_buff[NAME_LEN + 1], sp_name_buff[NAME_LEN + 1], definer_buff[USERNAME_LENGTH + HOSTNAME_LENGTH + 2],
  ...
  enum_sp_type sp_type = (enum_sp_type)proc_table->field[MYSQL_PROC_MYSQL_TYPE]->val_int();

  if (!full_access)
    full_access = !strcmp(sp_user, definer.c_ptr_safe());
  if (!full_access &&
      check_some_routine_access(thd, sp_db.c_ptr_safe(), sp_name.c_ptr_safe(), sp_type == SP_TYPE_PROCEDURE))
    return 0;

  if ((lex->sql_command == SQLCOM_SHOW_STATUS_PROC && sp_type == SP_TYPE_PROCEDURE) ||
      (lex->sql_command == SQLCOM_SHOW_STATUS_FUNC && sp_type == SP_TYPE_FUNCTION) ||
      (sql_command_flags[lex->sql_command] & CF_STATUS_COMMAND) == 0)
  {
    ...
    if (!wild || !wild[0] || !wild_case_compare(system_charset_info, sp_name.c_ptr_safe(), wild))
    {
      ...
      table->field[IS_ROUTINES_ROUTINE_NAME]->store(sp_name.ptr(), sp_name.length(), cs);

      copy_field_as_string(table->field[IS_ROUTINES_SPECIFIC_NAME], proc_table->field[MYSQL_PROC_FIELD_SPECIFIC_NAME]);
      table->field[IS_ROUTINES_ROUTINE_CATALOG]->store(STRING_WITH_LEN("def"), cs);
      table->field[IS_ROUTINES_ROUTINE_SCHEMA]->store(sp_db.ptr(), sp_db.length(), cs);
      copy_field_as_string(table->field[IS_ROUTINES_ROUTINE_TYPE], proc_table->field[MYSQL_PROC_MYSQL_TYPE]);
      ...
      if (sp_type == SP_TYPE_FUNCTION)
      {
        ...
        sp = sp_load_for_information_schema(thd, proc_table, &sp_db, &sp_name,
                                            ...
                                            SP_TYPE_FUNCTION, returns.c_ptr_safe(), "", &free_sp_head);
        ...
        if (sp)
        {
          ...
          (void)build_table_filename(path, sizeof(path), "", "", "", 0);
          init_tmp_table_share(thd, &share, "", 0, "", path);
          field = make_field(&share, (uchar *)0, field_def->length, (uchar *)"", 0, field_def->pack_flag,
                             field_def->sql_type, field_def->charset, field_def->geom_type, Field::NONE,
                             field_def->interval, "");
      ...
      table->field[IS_ROUTINES_PARAMETER_STYLE]->store(STRING_WITH_LEN("SQL"), cs);
      ...
      copy_field_as_string(table->field[IS_ROUTINES_SECURITY_TYPE], proc_table->field[MYSQL_PROC_FIELD_SECURITY_TYPE]);
      ...
      copy_field_as_string(table->field[IS_ROUTINES_SQL_MODE], proc_table->field[MYSQL_PROC_FIELD_SQL_MODE]);
      copy_field_as_string(table->field[IS_ROUTINES_ROUTINE_COMMENT], proc_table->field[MYSQL_PROC_FIELD_COMMENT]);

      table->field[IS_ROUTINES_DEFINER]->store(definer.ptr(), definer.length(), cs);
...
int fill_schema_proc(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  strxmov(definer, thd->security_context()->priv_user().str, "@", thd->security_context()->priv_host().str, NullS);
  ...
  if (schema_table_idx == SCH_PROCEDURES ? store_schema_proc(thd, table, proc_table, wild, full_access, definer)
...
static int get_schema_stat_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                  LEX_STRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
    ...
    for (uint i = 0; i < show_table->s->keys; i++, key_info++)
    {
      ...
      for (uint j = 0; j < key_info->user_defined_key_parts; j++, key_part++)
      {
        ...
        if (show_table->file)
        {
          ...
          {
            double records = (show_table->file->stats.records / key->records_per_key(j));
...
static int get_schema_views_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                   LEX_STRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
  char definer[USER_HOST_BUFF_SIZE];
  ...
  if (tables->is_view())
  {
    ...
    {
      if (!my_strcasecmp(system_charset_info, tables->definer.user.str, sctx->priv_user().str) &&
          !my_strcasecmp(system_charset_info, tables->definer.host.str, sctx->priv_host().str))
...
bool store_constraints(THD *thd, TABLE *table, LEX_STRING *db_name, LEX_STRING *table_name, const char *key_name,
...
static int get_schema_constraints_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                         LEX_STRING *table_name)
{
  ...
  else if (!tables->is_view())
  {
    ...
    for (uint i = 0; i < show_table->s->keys; i++, key_info++)
    {
      ...
      if (i == primary_key && !strcmp(key_info->name, primary_key_name))
      {
        if (store_constraints(thd, table, db_name, table_name, key_info->name, strlen(key_info->name),
...
static bool store_trigger(THD *thd, TABLE *table, Trigger *trigger)
{
  ...
  restore_record(table, s->default_values);
  table->field[0]->store(STRING_WITH_LEN("def"), cs);
  table->field[1]->store(trigger->get_db_name().str, trigger->get_db_name().length, cs);
  table->field[2]->store(trigger->get_trigger_name().str, trigger->get_trigger_name().length, cs);
  ...
  table->field[5]->store(trigger->get_db_name().str, trigger->get_db_name().length, cs);
  table->field[6]->store(trigger->get_subject_table_name().str, trigger->get_subject_table_name().length, cs);
  ...
  table->field[18]->store(trigger->get_definer().str, trigger->get_definer().length, cs);
  table->field[19]->store(trigger->get_client_cs_name().str, trigger->get_client_cs_name().length, cs);
  table->field[20]->store(trigger->get_connection_cl_name().str, trigger->get_connection_cl_name().length, cs);
  table->field[21]->store(trigger->get_db_cl_name().str, trigger->get_db_cl_name().length, cs);

  return schema_table_store_record(thd, table);
}

static int get_schema_triggers_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                      LEX_STRING *table_name)
{
...
void store_key_column_usage(TABLE *table, LEX_STRING *db_name, LEX_STRING *table_name, const char *key_name,
                            size_t key_len, const char *con_type, size_t con_len, longlong idx)
{
  CHARSET_INFO *cs = system_charset_info;
  table->field[0]->store(STRING_WITH_LEN("def"), cs);
  table->field[1]->store(db_name->str, db_name->length, cs);
...
static int get_schema_key_column_usage_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                              LEX_STRING *table_name)
{
    ...
    for (uint i = 0; i < show_table->s->keys; i++, key_info++)
    {
      ...
      for (uint j = 0; j < key_info->user_defined_key_parts; j++, key_part++)
      {
        if (key_part->field)
        {
          f_idx++;
          restore_record(table, s->default_values);
          store_key_column_usage(table, db_name, table_name, key_info->name, strlen(key_info->name),
...
static void collect_partition_expr(THD *thd, List<char> &field_list, String *str)
{
...
int get_cs_converted_part_value_from_string(THD *thd, Item *item, String *input_str, String *output_str,
                                           ...
                                           partition_element *part_elem, handler *file, uint part_id)
{
  ...
  if (!part_handler)
  {
    ...
    file->info(HA_STATUS_CONST | HA_STATUS_TIME | HA_STATUS_VARIABLE | HA_STATUS_NO_LOCK);
  ...
  if (part_elem)
  {
    ...
    if (part_elem->tablespace_name)
      table->field[24]->store(part_elem->tablespace_name, strlen(part_elem->tablespace_name), cs);
    else
    {
                                            ...
                                            String &tmp_str)
{
  ...
  for (i = 0; i < num_elements; i++)
  {
    ...
    else
    {
      ...
      if (!(item = part_info->get_column_item(item, part_info->part_field_array[i])))
      {
        DBUG_RETURN(1);
      }
      String *res = item->val_str(&str);
      if (get_cs_converted_part_value_from_string(thd, item, res, &val_conv, part_info->part_field_array[i]->charset(),
                                                  FALSE))
      {
        DBUG_RETURN(1);
      }
...
static int get_schema_partitions_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                        LEX_STRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
  char buff[61];
  ...
  if (part_info)
  {
    ...
    switch (part_info->part_type)
    {
      ...
      case LIST_PARTITION:
        ...
        if (part_info->part_type == RANGE_PARTITION)
          ...
          tmp_res.append(partition_keywords[PKW_LIST].str, partition_keywords[PKW_LIST].length);
        if (part_info->column_list)
          tmp_res.append(partition_keywords[PKW_COLUMNS].str, partition_keywords[PKW_COLUMNS].length);
        table->field[7]->store(tmp_res.ptr(), tmp_res.length(), cs);
        break;
      case HASH_PARTITION:
        tmp_res.length(0);
        if (part_info->linear_hash_ind)
          tmp_res.append(partition_keywords[PKW_LINEAR].str, partition_keywords[PKW_LINEAR].length);
        if (part_info->list_of_part_fields)
          tmp_res.append(partition_keywords[PKW_KEY].str, partition_keywords[PKW_KEY].length);
        else
          tmp_res.append(partition_keywords[PKW_HASH].str, partition_keywords[PKW_HASH].length);
        table->field[7]->store(tmp_res.ptr(), tmp_res.length(), cs);
        break;
      default:
        assert(0);
    ...
    {
      table->field[9]->store(part_info->part_func_string, part_info->part_func_len, cs);
    }
    ...
    if (part_info->is_sub_partitioned())
    {
      ...
      {
        table->field[10]->store(part_info->subpart_func_string, part_info->subpart_func_len, cs);
      }
      ...
      if (part_info->part_type == RANGE_PARTITION)
      {
        ...
        else
        {
          if (part_elem->range_value != LLONG_MAX)
            ...
            table->field[11]->store(partition_keywords[PKW_MAXVALUE].str, partition_keywords[PKW_MAXVALUE].length, cs);
        }
        table->field[11]->set_notnull();
      }
...
int copy_event_to_schema_table(THD *thd, TABLE *sch_table, TABLE *event_table)
{
  ...
  */
  ...
  sch_table->field[ISE_CLIENT_CS]->store(et.creation_ctx->get_client_cs()->csname,
                                         strlen(et.creation_ctx->get_client_cs()->csname), scs);

  sch_table->field[ISE_CONNECTION_CL]->set_notnull();
  sch_table->field[ISE_CONNECTION_CL]->store(et.creation_ctx->get_connection_cl()->name,
                                             strlen(et.creation_ctx->get_connection_cl()->name), scs);

  sch_table->field[ISE_DB_CL]->set_notnull();
  sch_table->field[ISE_DB_CL]->store(et.creation_ctx->get_db_cl()->name, strlen(et.creation_ctx->get_db_cl()->name),
                                     scs);
...
void push_select_warning(THD *thd, enum enum_var_type option_type, bool status)
{
  ...
  push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_DEPRECATED_SYNTAX, ER_THD(thd, ER_WARN_DEPRECATED_SYNTAX),
                      old_name, new_name);
}
...
int fill_variables(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  */
  ...
  res = show_status_array(thd, wild, sys_var_array.begin(), option_type, NULL, "", tables, upper_case_names, cond);
...
int fill_status(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  */
  ...
  res = show_status_array(thd, wild, &all_status_vars[0], option_type, status_var_ptr, "", tables, upper_case_names,
                          cond);
...
static int get_referential_constraints_record(THD *thd, TABLE_LIST *tables, TABLE *table, bool res, LEX_STRING *db_name,
                                              LEX_STRING *table_name)
{
  CHARSET_INFO *cs = system_charset_info;
  ...
  if (!tables->is_view())
  {
    ...
    while ((f_key_info = it++))
    {
      ...
      table->field[7]->store(f_key_info->update_method->str, f_key_info->update_method->length, cs);
      table->field[8]->store(f_key_info->delete_method->str, f_key_info->delete_method->length, cs);
      if (schema_table_store_record(thd, table))
        DBUG_RETURN(1);
    }
  }
...
TABLE *create_schema_table(THD *thd, TABLE_LIST *table_list)
{
  ...
  for (; fields_info->field_name; fields_info++)
  {
    switch (fields_info->field_type)
    {
      ...
      case MYSQL_TYPE_LONGLONG:
      case MYSQL_TYPE_INT24:
        if (!(item = new Item_return_int(fields_info->field_name, fields_info->field_length, fields_info->field_type,
                                         fields_info->value)))
        {
          DBUG_RETURN(0);
        }
      ...
      case MYSQL_TYPE_DATETIME:
      {
        const Name_string field_name(fields_info->field_name, strlen(fields_info->field_name));
        if (!(item = new Item_temporal(fields_info->field_type, field_name, 0, 0)))
          DBUG_RETURN(0);

        if (fields_info->field_type == MYSQL_TYPE_TIMESTAMP || fields_info->field_type == MYSQL_TYPE_DATETIME)
      ...
      {
        const Name_string field_name(fields_info->field_name, strlen(fields_info->field_name));
        if ((item = new Item_float(field_name, 0.0, NOT_FIXED_DEC, fields_info->field_length)) == NULL)
          DBUG_RETURN(NULL);
        break;
      }
      case MYSQL_TYPE_DECIMAL:
      ...
      case MYSQL_TYPE_BLOB:
        if (!(item = new Item_blob(fields_info->field_name, fields_info->field_length)))
        {
          DBUG_RETURN(0);
        }
        break;
  ...
  if (!(table =
            create_tmp_table(thd, tmp_table_param, field_list, (ORDER *)0, 0, 0,
                             select_lex->active_options() | TMP_TABLE_ALL_COLUMNS, HA_POS_ERROR, table_list->alias)))
    DBUG_RETURN(0);
...
{
  int fields_arr[] = {IS_COLUMNS_COLUMN_NAME,    IS_COLUMNS_COLUMN_TYPE,
                      IS_COLUMNS_COLLATION_NAME, IS_COLUMNS_IS_NULLABLE,
                      IS_COLUMNS_COLUMN_KEY,     IS_COLUMNS_COLUMN_DEFAULT,
                      IS_COLUMNS_EXTRA,          IS_COLUMNS_PRIVILEGES,
                      IS_COLUMNS_COLUMN_COMMENT, -1};
  ...
  for (; *field_num >= 0; field_num++)
  {
    ...
    if (!thd->lex->verbose && (*field_num == IS_COLUMNS_COLLATION_NAME || *field_num == IS_COLUMNS_PRIVILEGES ||
...
{
  int fields_arr[] = {IS_ROUTINES_ROUTINE_SCHEMA,       IS_ROUTINES_ROUTINE_NAME,
                      IS_ROUTINES_ROUTINE_TYPE,         IS_ROUTINES_DEFINER,
                      IS_ROUTINES_LAST_ALTERED,         IS_ROUTINES_CREATED,
                      IS_ROUTINES_SECURITY_TYPE,        IS_ROUTINES_ROUTINE_COMMENT,
                      IS_ROUTINES_CHARACTER_SET_CLIENT, IS_ROUTINES_COLLATION_CONNECTION,
                      IS_ROUTINES_DATABASE_COLLATION,   -1};
  int *field_num = fields_arr;
  ST_FIELD_INFO *field_info;
  Name_resolution_context *context = &thd->lex->select_lex->context;
...
int make_schema_select(THD *thd, SELECT_LEX *sel, enum enum_schema_tables schema_table_idx)
{
  ...
  */
  thd->make_lex_string(&db, INFORMATION_SCHEMA_NAME.str, INFORMATION_SCHEMA_NAME.length, 0);
  thd->make_lex_string(&table, schema_table->table_name, strlen(schema_table->table_name), 0);

  if (schema_table->old_format(thd, schema_table) || /* Handle old syntax */
      !sel->add_table_to_list(thd, new Table_ident(thd, to_lex_cstring(db), to_lex_cstring(table), 0), 0, 0, TL_READ,
                              MDL_SHARED_READ))
  {
    DBUG_RETURN(1);
  }
...
static bool do_fill_table(THD *thd, TABLE_LIST *table_list, QEP_TAB *qep_tab)
{
  ...
  */
  ...
  bool res = table_list->schema_table->fill_table(thd, table_list, qep_tab->condition());
  ...
  if (tmp_da.is_error())
  {
    da->set_error_status(tmp_da.mysql_errno(), tmp_da.message_text(), tmp_da.returned_sqlstate());
    da->push_warning(thd, tmp_da.mysql_errno(), tmp_da.returned_sqlstate(), Sql_condition::SL_ERROR,
                     tmp_da.message_text());
  }
...
bool get_schema_tables_result(JOIN *join, enum enum_schema_table_state executed_place)
{
  ...
  for (uint i = 0; i < join->tables; i++)
  {
    ...
    {
      bool is_subselect = join->select_lex->master_unit() && join->select_lex->master_unit()->item;
      ...
      if (thd->lex->describe && (table_list->schema_table->fill_table != get_all_tables))
        continue;
      ...
      */
      ...
      if (is_infoschema_db(table_list->db, table_list->db_length))
      {
        ...
        if (my_strcasecmp(system_charset_info, table_list->schema_table_name, INNODB_LOCKS.str) == 0)
        {
          /* Deprecated in 5.7 */
          push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT,
                              ER_THD(thd, ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT), "INFORMATION_SCHEMA.INNODB_LOCKS");
        }
        else if (my_strcasecmp(system_charset_info, table_list->schema_table_name, INNODB_LOCK_WAITS.str) == 0)
        {
          /* Deprecated in 5.7 */
          push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT,
...
ST_FIELD_INFO schema_fields_info[] = {
    ...
    {"SCHEMA_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Database", SKIP_OPEN_TABLE},
    {"DEFAULT_CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {"DEFAULT_COLLATION_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
...
ST_FIELD_INFO tables_fields_info[] = {
    ...
    {"TABLE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Name", SKIP_OPEN_TABLE},
    {"TABLE_TYPE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
    {"ENGINE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, "Engine", OPEN_FRM_ONLY},
    {"VERSION", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Version",
     OPEN_FRM_ONLY},
    {"ROW_FORMAT", 10, MYSQL_TYPE_STRING, 0, 1, "Row_format", OPEN_FULL_TABLE},
    {"TABLE_ROWS", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Rows",
     OPEN_FULL_TABLE},
    {"AVG_ROW_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED),
     "Avg_row_length", OPEN_FULL_TABLE},
    {"DATA_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED),
     "Data_length", OPEN_FULL_TABLE},
    {"MAX_DATA_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED),
     "Max_data_length", OPEN_FULL_TABLE},
    {"INDEX_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED),
     "Index_length", OPEN_FULL_TABLE},
    {"DATA_FREE", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED),
     "Data_free", OPEN_FULL_TABLE},
    {"AUTO_INCREMENT", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED),
     "Auto_increment", OPEN_FULL_TABLE},
    {"CREATE_TIME", 0, MYSQL_TYPE_DATETIME, 0, 1, "Create_time", OPEN_FULL_TABLE},
    {"UPDATE_TIME", 0, MYSQL_TYPE_DATETIME, 0, 1, "Update_time", OPEN_FULL_TABLE},
    {"CHECK_TIME", 0, MYSQL_TYPE_DATETIME, 0, 1, "Check_time", OPEN_FULL_TABLE},
    {"TABLE_COLLATION", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 1, "Collation", OPEN_FRM_ONLY},
    {"CHECKSUM", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Checksum",
     OPEN_FULL_TABLE},
    {"CREATE_OPTIONS", 255, MYSQL_TYPE_STRING, 0, 1, "Create_options", OPEN_FRM_ONLY},
...
ST_FIELD_INFO columns_fields_info[] = {
    ...
    {"COLUMN_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Field", OPEN_FRM_ONLY},
    {"ORDINAL_POSITION", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0, OPEN_FRM_ONLY},
    {"COLUMN_DEFAULT", MAX_FIELD_VARCHARLENGTH, MYSQL_TYPE_STRING, 0, 1, "Default", OPEN_FRM_ONLY},
    {"IS_NULLABLE", 3, MYSQL_TYPE_STRING, 0, 0, "Null", OPEN_FRM_ONLY},
    {"DATA_TYPE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
    {"CHARACTER_MAXIMUM_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0,
     (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FRM_ONLY},
    {"CHARACTER_OCTET_LENGTH", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0,
     (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FRM_ONLY},
    {"NUMERIC_PRECISION", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0,
     OPEN_FRM_ONLY},
    {"NUMERIC_SCALE", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0,
     OPEN_FRM_ONLY},
    {"DATETIME_PRECISION", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED),
     0, OPEN_FULL_TABLE},
    {"CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FRM_ONLY},
    {"COLLATION_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 1, "Collation", OPEN_FRM_ONLY},
    ...
    {"GENERATION_EXPRESSION", GENERATED_COLUMN_EXPRESSION_MAXLEN, MYSQL_TYPE_STRING, 0, 0, "Generation expression",
     OPEN_FRM_ONLY},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};

ST_FIELD_INFO charsets_fields_info[] = {
    {"CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "Charset", SKIP_OPEN_TABLE},
    {"DEFAULT_COLLATE_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "Default collation", SKIP_OPEN_TABLE},
    {"DESCRIPTION", 60, MYSQL_TYPE_STRING, 0, 0, "Description", SKIP_OPEN_TABLE},
    {"MAXLEN", 3, MYSQL_TYPE_LONGLONG, 0, 0, "Maxlen", SKIP_OPEN_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};

ST_FIELD_INFO collation_fields_info[] = {
    {"COLLATION_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "Collation", SKIP_OPEN_TABLE},
    {"CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "Charset", SKIP_OPEN_TABLE},
    {"ID", MY_INT32_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 0, "Id", SKIP_OPEN_TABLE},
    {"IS_DEFAULT", 3, MYSQL_TYPE_STRING, 0, 0, "Default", SKIP_OPEN_TABLE},
    {"IS_COMPILED", 3, MYSQL_TYPE_STRING, 0, 0, "Compiled", SKIP_OPEN_TABLE},
    {"SORTLEN", 3, MYSQL_TYPE_LONGLONG, 0, 0, "Sortlen", SKIP_OPEN_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};

ST_FIELD_INFO engines_fields_info[] = {{"ENGINE", 64, MYSQL_TYPE_STRING, 0, 0, "Engine", SKIP_OPEN_TABLE},
...
ST_FIELD_INFO events_fields_info[] = {
    ...
    {"EVENT_SCHEMA", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Db", SKIP_OPEN_TABLE},
    {"EVENT_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Name", SKIP_OPEN_TABLE},
    ...
    {"INTERVAL_VALUE", 256, MYSQL_TYPE_STRING, 0, 1, "Interval value", SKIP_OPEN_TABLE},
    {"INTERVAL_FIELD", 18, MYSQL_TYPE_STRING, 0, 1, "Interval field", SKIP_OPEN_TABLE},
    ...
    {"CHARACTER_SET_CLIENT", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "character_set_client", SKIP_OPEN_TABLE},
    {"COLLATION_CONNECTION", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "collation_connection", SKIP_OPEN_TABLE},
    {"DATABASE_COLLATION", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "Database Collation", SKIP_OPEN_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};

ST_FIELD_INFO coll_charset_app_fields_info[] = {
    {"COLLATION_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {"CHARACTER_SET_NAME", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};
...
ST_FIELD_INFO proc_fields_info[] = {
    ...
    {"ROUTINE_SCHEMA", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Db", SKIP_OPEN_TABLE},
    {"ROUTINE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Name", SKIP_OPEN_TABLE},
...
ST_FIELD_INFO stat_fields_info[] = {
    ...
    {"INDEX_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Key_name", OPEN_FRM_ONLY},
    {"SEQ_IN_INDEX", 2, MYSQL_TYPE_LONGLONG, 0, 0, "Seq_in_index", OPEN_FRM_ONLY},
    {"COLUMN_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Column_name", OPEN_FRM_ONLY},
    {"COLLATION", 1, MYSQL_TYPE_STRING, 0, 1, "Collation", OPEN_FRM_ONLY},
    {"CARDINALITY", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, 1, "Cardinality", OPEN_FULL_TABLE},
...
ST_FIELD_INFO view_fields_info[] = {
    ...
    {"CHARACTER_SET_CLIENT", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
    {"COLLATION_CONNECTION", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};
...
ST_FIELD_INFO table_constraints_fields_info[] = {
    ...
    {"CONSTRAINT_TYPE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};
...
ST_FIELD_INFO key_column_usage_fields_info[] = {
    ...
    {"POSITION_IN_UNIQUE_CONSTRAINT", 10, MYSQL_TYPE_LONGLONG, 0, 1, 0, OPEN_FULL_TABLE},
    {"REFERENCED_TABLE_SCHEMA", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {"REFERENCED_TABLE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {"REFERENCED_COLUMN_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};

ST_FIELD_INFO table_names_fields_info[] = {
    {"TABLE_CATALOG", FN_REFLEN, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {"TABLE_SCHEMA", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {"TABLE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Tables_in_", SKIP_OPEN_TABLE},
    {"TABLE_TYPE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Table_type", OPEN_FRM_ONLY},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};

ST_FIELD_INFO open_tables_fields_info[] = {
    {"Database", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Database", SKIP_OPEN_TABLE},
...
ST_FIELD_INFO triggers_fields_info[] = {
    ...
    {"TRIGGER_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Trigger", OPEN_FRM_ONLY},
    {"EVENT_MANIPULATION", 6, MYSQL_TYPE_STRING, 0, 0, "Event", OPEN_FRM_ONLY},
    {"EVENT_OBJECT_CATALOG", FN_REFLEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
    {"EVENT_OBJECT_SCHEMA", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
    {"EVENT_OBJECT_TABLE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Table", OPEN_FRM_ONLY},
    {"ACTION_ORDER", 4, MYSQL_TYPE_LONGLONG, 0, 0, 0, OPEN_FRM_ONLY},
    {"ACTION_CONDITION", 65535, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FRM_ONLY},
    {"ACTION_STATEMENT", 65535, MYSQL_TYPE_STRING, 0, 0, "Statement", OPEN_FRM_ONLY},
    {"ACTION_ORIENTATION", 9, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FRM_ONLY},
    {"ACTION_TIMING", 6, MYSQL_TYPE_STRING, 0, 0, "Timing", OPEN_FRM_ONLY},
    {"ACTION_REFERENCE_OLD_TABLE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FRM_ONLY},
    {"ACTION_REFERENCE_NEW_TABLE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FRM_ONLY},
    ...
    */
    ...
    {"CHARACTER_SET_CLIENT", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "character_set_client", OPEN_FRM_ONLY},
    {"COLLATION_CONNECTION", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "collation_connection", OPEN_FRM_ONLY},
    {"DATABASE_COLLATION", MY_CS_NAME_SIZE, MYSQL_TYPE_STRING, 0, 0, "Database Collation", OPEN_FRM_ONLY},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};
...
ST_FIELD_INFO partitions_fields_info[] = {
    ...
    {"SUBPARTITION_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {"PARTITION_ORDINAL_POSITION", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0,
     OPEN_FULL_TABLE},
    {"SUBPARTITION_ORDINAL_POSITION", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0,
     OPEN_FULL_TABLE},
    {"PARTITION_METHOD", 18, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {"SUBPARTITION_METHOD", 12, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {"PARTITION_EXPRESSION", 65535, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {"SUBPARTITION_EXPRESSION", 65535, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {"PARTITION_DESCRIPTION", 65535, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {"TABLE_ROWS", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0, OPEN_FULL_TABLE},
    {"AVG_ROW_LENGTH", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0, OPEN_FULL_TABLE},
    {"DATA_LENGTH", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0, OPEN_FULL_TABLE},
    {"MAX_DATA_LENGTH", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
    {"INDEX_LENGTH", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0, OPEN_FULL_TABLE},
    {"DATA_FREE", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, 0, OPEN_FULL_TABLE},
    ...
    {"CHECKSUM", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, OPEN_FULL_TABLE},
    {"PARTITION_COMMENT", 80, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"NODEGROUP", 12, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"TABLESPACE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, OPEN_FULL_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};

ST_FIELD_INFO variables_fields_info[] = {
    {"VARIABLE_NAME", 64, MYSQL_TYPE_STRING, 0, 0, "Variable_name", SKIP_OPEN_TABLE},
...
ST_FIELD_INFO plugin_fields_info[] = {
    {"PLUGIN_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, "Name", SKIP_OPEN_TABLE},
    {"PLUGIN_VERSION", 20, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {"PLUGIN_STATUS", 10, MYSQL_TYPE_STRING, 0, 0, "Status", SKIP_OPEN_TABLE},
    {"PLUGIN_TYPE", 80, MYSQL_TYPE_STRING, 0, 0, "Type", SKIP_OPEN_TABLE},
    {"PLUGIN_TYPE_VERSION", 20, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {"PLUGIN_LIBRARY", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, "Library", SKIP_OPEN_TABLE},
...
ST_FIELD_INFO files_fields_info[] = {
    ...
    {"TABLESPACE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
    ...
    {"LOGFILE_GROUP_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 1, 0, SKIP_OPEN_TABLE},
    ...
    {"INITIAL_SIZE", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, SKIP_OPEN_TABLE},
    {"MAXIMUM_SIZE", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, SKIP_OPEN_TABLE},
    {"AUTOEXTEND_SIZE", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), 0, SKIP_OPEN_TABLE},
    ...
    {"VERSION", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Version", SKIP_OPEN_TABLE},
    {"ROW_FORMAT", 10, MYSQL_TYPE_STRING, 0, 1, "Row_format", SKIP_OPEN_TABLE},
    {"TABLE_ROWS", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Rows", SKIP_OPEN_TABLE},
    {"AVG_ROW_LENGTH", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Avg_row_length",
     SKIP_OPEN_TABLE},
    {"DATA_LENGTH", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Data_length", SKIP_OPEN_TABLE},
    {"MAX_DATA_LENGTH", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Max_data_length",
     SKIP_OPEN_TABLE},
    {"INDEX_LENGTH", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Index_length",
     SKIP_OPEN_TABLE},
    {"DATA_FREE", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Data_free", SKIP_OPEN_TABLE},
    ...
    {"CHECKSUM", 21, MYSQL_TYPE_LONGLONG, 0, (MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED), "Checksum", SKIP_OPEN_TABLE},
...
ST_FIELD_INFO referential_constraints_fields_info[] = {
    ...
    {"CONSTRAINT_SCHEMA", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"CONSTRAINT_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"UNIQUE_CONSTRAINT_CATALOG", FN_REFLEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"UNIQUE_CONSTRAINT_SCHEMA", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"UNIQUE_CONSTRAINT_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, MY_I_S_MAYBE_NULL, 0, OPEN_FULL_TABLE},
    {"MATCH_OPTION", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"UPDATE_RULE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"DELETE_RULE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"TABLE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"REFERENCED_TABLE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};

ST_FIELD_INFO parameters_fields_info[] = {
    {"SPECIFIC_CATALOG", FN_REFLEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
    {"SPECIFIC_SCHEMA", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, OPEN_FULL_TABLE},
...
ST_FIELD_INFO tablespaces_fields_info[] = {
    {"TABLESPACE_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {"ENGINE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE},
    {"TABLESPACE_TYPE", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, MY_I_S_MAYBE_NULL, 0, SKIP_OPEN_TABLE},
    {"LOGFILE_GROUP_NAME", NAME_CHAR_LEN, MYSQL_TYPE_STRING, 0, MY_I_S_MAYBE_NULL, 0, SKIP_OPEN_TABLE},
    {"EXTENT_SIZE", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED, 0, SKIP_OPEN_TABLE},
    {"AUTOEXTEND_SIZE", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED, 0, SKIP_OPEN_TABLE},
    {"MAXIMUM_SIZE", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED, 0, SKIP_OPEN_TABLE},
    {"NODEGROUP_ID", 21, MYSQL_TYPE_LONGLONG, 0, MY_I_S_MAYBE_NULL | MY_I_S_UNSIGNED, 0, SKIP_OPEN_TABLE},
    {"TABLESPACE_COMMENT", 2048, MYSQL_TYPE_STRING, 0, MY_I_S_MAYBE_NULL, 0, SKIP_OPEN_TABLE},
    {0, 0, MYSQL_TYPE_STRING, 0, 0, 0, SKIP_OPEN_TABLE}};
...
ST_SCHEMA_TABLE schema_tables[] = {
    {"CHARACTER_SETS", charsets_fields_info, create_schema_table, fill_schema_charsets, make_character_sets_old_format,
     0, -1, -1, 0, 0},
    {"COLLATIONS", collation_fields_info, create_schema_table, fill_schema_collation, make_old_format, 0, -1, -1, 0, 0},
    {"COLLATION_CHARACTER_SET_APPLICABILITY", coll_charset_app_fields_info, create_schema_table,
     fill_schema_coll_charset_app, 0, 0, -1, -1, 0, 0},
    {"COLUMNS", columns_fields_info, create_schema_table, get_all_tables, make_columns_old_format,
     get_schema_column_record, 1, 2, 0, OPTIMIZE_I_S_TABLE | OPEN_VIEW_FULL},
    {"COLUMN_PRIVILEGES", column_privileges_fields_info, create_schema_table, fill_schema_column_privileges, 0, 0, -1,
     -1, 0, 0},
    {"ENGINES", engines_fields_info, create_schema_table, fill_schema_engines, make_old_format, 0, -1, -1, 0, 0},
#ifndef EMBEDDED_LIBRARY
    {"EVENTS", events_fields_info, create_schema_table, Events::fill_schema_events, make_old_format, 0, -1, -1, 0, 0},
#else  // for alignment with enum_schema_tables
    {"EVENTS", events_fields_info, create_schema_table, 0, make_old_format, 0, -1, -1, 0, 0},
#endif
    {"FILES", files_fields_info, create_schema_table, hton_fill_schema_table, 0, 0, -1, -1, 0, 0},
    {"GLOBAL_STATUS", variables_fields_info, create_schema_table, fill_status, make_old_format, 0, 0, -1, 0, 0},
    {"GLOBAL_VARIABLES", variables_fields_info, create_schema_table, fill_variables, make_old_format, 0, 0, -1, 0, 0},
    {"KEY_COLUMN_USAGE", key_column_usage_fields_info, create_schema_table, get_all_tables, 0,
     get_schema_key_column_usage_record, 4, 5, 0, OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    {"OPEN_TABLES", open_tables_fields_info, create_schema_table, fill_open_tables, make_old_format, 0, -1, -1, 1, 0},
#ifdef OPTIMIZER_TRACE
    {"OPTIMIZER_TRACE", optimizer_trace_info, create_schema_table, fill_optimizer_trace_info, NULL, NULL, -1, -1, false,
     0},
#else  // for alignment with enum_schema_tables
    {"OPTIMIZER_TRACE", optimizer_trace_info, create_schema_table, NULL, NULL, NULL, -1, -1, false, 0},
#endif
    {"PARAMETERS", parameters_fields_info, create_schema_table, fill_schema_proc, 0, 0, -1, -1, 0, 0},
    {"PARTITIONS", partitions_fields_info, create_schema_table, get_all_tables, 0, get_schema_partitions_record, 1, 2,
     0, OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    {"PLUGINS", plugin_fields_info, create_schema_table, fill_plugins, make_old_format, 0, -1, -1, 0, 0},
    {"PROCESSLIST", processlist_fields_info, create_schema_table, fill_schema_processlist, make_old_format, 0, -1, -1,
     0, 0},
    {"PROFILING", query_profile_statistics_info, create_schema_table, fill_query_profile_statistics_info,
     make_profile_table_for_show, NULL, -1, -1, false, 0},
    {"REFERENTIAL_CONSTRAINTS", referential_constraints_fields_info, create_schema_table, get_all_tables, 0,
     get_referential_constraints_record, 1, 9, 0, OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    {"ROUTINES", proc_fields_info, create_schema_table, fill_schema_proc, make_proc_old_format, 0, -1, -1, 0, 0},
    {"SCHEMATA", schema_fields_info, create_schema_table, fill_schema_schemata, make_schemata_old_format, 0, 1, -1, 0,
     0},
    {"SCHEMA_PRIVILEGES", schema_privileges_fields_info, create_schema_table, fill_schema_schema_privileges, 0, 0, -1,
     -1, 0, 0},
    {"SESSION_STATUS", variables_fields_info, create_schema_table, fill_status, make_old_format, 0, 0, -1, 0, 0},
    {"SESSION_VARIABLES", variables_fields_info, create_schema_table, fill_variables, make_old_format, 0, 0, -1, 0, 0},
    {"STATISTICS", stat_fields_info, create_schema_table, get_all_tables, make_old_format, get_schema_stat_record, 1, 2,
     0, OPEN_TABLE_ONLY | OPTIMIZE_I_S_TABLE},
    {"STATUS", variables_fields_info, create_schema_table, fill_status, make_old_format, 0, 0, -1, 1, 0},
    {"TABLES", tables_fields_info, create_schema_table, get_all_tables, make_old_format, get_schema_tables_record, 1, 2,
     0, OPTIMIZE_I_S_TABLE},
    {"TABLESPACES", tablespaces_fields_info, create_schema_table, hton_fill_schema_table, 0, 0, -1, -1, 0, 0},
    {"TABLE_CONSTRAINTS", table_constraints_fields_info, create_schema_table, get_all_tables, 0,
     get_schema_constraints_record, 3, 4, 0, OPTIMIZE_I_S_TABLE | OPEN_TABLE_ONLY},
    ...
    {"TRIGGERS", triggers_fields_info, create_schema_table, get_all_tables, make_old_format, get_schema_triggers_record,
     5, 6, 0, OPEN_TRIGGER_ONLY | OPTIMIZE_I_S_TABLE},
    {"USER_PRIVILEGES", user_privileges_fields_info, create_schema_table, fill_schema_user_privileges, 0, 0, -1, -1, 0,
     0},
    {"VARIABLES", variables_fields_info, create_schema_table, fill_variables, make_old_format, 0, 0, -1, 1, 0},
...
int finalize_schema_table(st_plugin_int *plugin)
{
  ...
  if (schema_table)
  {
    if (plugin->plugin->deinit)
    {
      ...
      {
        DBUG_PRINT("warning", ("Plugin '%s' deinit function returned error.", plugin->name.str));
      }
    }
    my_free(schema_table);
  }
...
static bool show_create_trigger_impl(THD *thd, Trigger *trigger)
{
...

  {
    ...
    */
    ...
    Item_empty_string *stmt_fld =
        new Item_empty_string("SQL Original Statement", max<size_t>(trigger->get_definition().length, 1024));
  ...
  fields.push_back(new Item_empty_string("character_set_client", MY_CS_NAME_SIZE));

  fields.push_back(new Item_empty_string("collation_connection", MY_CS_NAME_SIZE));

  fields.push_back(new Item_empty_string("Database Collation", MY_CS_NAME_SIZE));

  fields.push_back(new Item_temporal(MYSQL_TYPE_TIMESTAMP, Name_string("Created", sizeof("created") - 1), 0, 0));
...
static TABLE_LIST *get_trigger_table(THD *thd, const sp_name *trg_name)
{
  ...
  LEX_STRING trn_path =
      Trigger_loader::build_trn_path(trn_path_buff, FN_REFLEN, trg_name->m_db.str, trg_name->m_name.str);
  ...
  table->init_one_table(db.str, db.length, tbl_name.str, tbl_name.length, tbl_name.str, TL_IGNORE);
...
bool show_create_trigger(THD *thd, const sp_name *trg_name)
{
  ...
  */
  ...
  {
    my_error(ER_TRG_CANT_OPEN_TABLE, MYF(0), trg_name->m_db.str, lst->table_name);
  ...
  if (!trigger)
  {
    my_error(ER_TRG_CORRUPTED_FILE, MYF(0), trg_name->m_db.str, lst->table_name);
                                          ...
                                          bool use_hex)
{
  ...
  if (!use_hex)
  {
    ...
    try_val.copy(input_str->ptr(), input_str->length(), cs, thd->variables.character_set_client, &try_conv_error);
    if (!try_conv_error)
    {
      String val;
      uint conv_error = 0;

      val.copy(input_str->ptr(), input_str->length(), cs, system_charset_info, &conv_error);
      if (!conv_error)
      {
