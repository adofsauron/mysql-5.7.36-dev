...
{
  const char *name = (target->orig_name.is_set() ? target->orig_name.ptr() : target->item_name.ptr());
  size_t name_len;
  uint attempt;
...
static void make_valid_column_names(LEX *lex)
{
  ...
  for (SELECT_LEX *sl = lex->select_lex; sl; sl = sl->next_select())
  {
    ...
    {
      if (!item->item_name.is_autogenerated() || !check_column_name(item->item_name.ptr()))
        continue;
...
static bool fill_defined_view_parts(THD *thd, TABLE_LIST *view)
{
  ...
  */
  ...
  if (tdc_open_view(thd, &decoy, decoy.alias, key, key_length, OPEN_VIEW_NO_PARSE))
    return TRUE;
  ...
  if (lex->create_view_suid == VIEW_SUID_DEFAULT)
    lex->create_view_suid = decoy.view_suid ? VIEW_SUID_DEFINER : VIEW_SUID_INVOKER;
...
bool create_view_precheck(THD *thd, TABLE_LIST *tables, TABLE_LIST *view, enum_view_create_mode mode)
{
  ...
  */
  if ((check_access(thd, CREATE_VIEW_ACL, view->db, &view->grant.privilege, &view->grant.m_internal, 0, 0) ||
       check_grant(thd, CREATE_VIEW_ACL, view, FALSE, 1, FALSE)) ||
      (mode != VIEW_CREATE_NEW &&
       (check_access(thd, DROP_ACL, view->db, &view->grant.privilege, &view->grant.m_internal, 0, 0) ||
  ...
  if (lex->select_lex != lex->all_selects_list)
  {
    ...
    for (TABLE_LIST *tbl = tables; tbl; tbl = tbl->next_global)
    {
      ...
      {
        if (check_access(thd, SELECT_ACL, tbl->db, &tbl->grant.privilege, &tbl->grant.m_internal, 0, 0) ||
...
bool mysql_create_view(THD *thd, TABLE_LIST *views, enum_view_create_mode mode)
{
  ...
  assert(!lex->proc_analyse && !lex->result && !lex->param_list.elements);
  ...
  */
  if (lex->definer &&
      (strcmp(lex->definer->user.str, thd->security_context()->priv_user().str) != 0 ||
       my_strcasecmp(system_charset_info, lex->definer->host.str, thd->security_context()->priv_host().str) != 0))
  {
  ...
  */
  for (tbl = lex->query_tables; tbl; tbl = tbl->next_global)
  {
    ...
    if (tbl->is_view() && strcmp(tbl->view_db.str, view->db) == 0 && strcmp(tbl->view_name.str, view->table_name) == 0)
    {
    ...
    */
    if (tbl->table)
    {
      ...
      if (tbl->table->s->tmp_table != NO_TMP_TABLE && !tbl->is_view() && !tbl->schema_table)
      {
  ...
  if (lex->view_list.elements)
  {
    ...
    {
      item->item_name.copy(name->str, (uint)name->length, system_charset_info, false);
    }
  }
  ...
  */
  if (view->is_derived())
    ...
    fill_effective_table_privileges(thd, &view->grant, view->db, view->table_name);
   ...
   */
  {
    ...
    for (sl = select_lex; sl; sl = sl->next_select())
    {
      ...
      while ((item = it++))
      {
        ...
        uint priv =
            (get_column_grant(thd, &view->grant, view->db, view->table_name, item->item_name.ptr()) & VIEW_ANY_ACL);
    ...
    {
      my_error(ER_COLUMNACCESS_DENIED_ERROR, MYF(0), "create view", thd->security_context()->priv_user().str,
               thd->security_context()->priv_host().str, report_item->item_name.ptr(), view->table_name);
      res = TRUE;
      goto err;
    }
  }
  ...
  if (!res)
  {
    ...
    if (mysql_bin_log.is_open())
    {
      ...
      const LEX_STRING command[3] = {
          {C_STRING_WITH_LEN("CREATE ")}, {C_STRING_WITH_LEN("ALTER ")}, {C_STRING_WITH_LEN("CREATE OR REPLACE ")}};

      buff.append(command[thd->lex->create_view_mode].str, command[thd->lex->create_view_mode].length);
      view_store_options(thd, views, &buff);
      buff.append(STRING_WITH_LEN("VIEW "));
      /* Test if user supplied a db (ie: we did not use thd->db) */
      if (views->db && views->db[0] && (thd->db().str == NULL || strcmp(views->db, thd->db().str)))
      {
      ...
      if (thd->binlog_query(THD::STMT_QUERY_TYPE, buff.ptr(), buff.length(), FALSE, FALSE, FALSE, errcode))
        res = TRUE;
    }
  }
...
static File_option view_parameters[] = {
    {{C_STRING_WITH_LEN("query")}, my_offsetof(TABLE_LIST, select_stmt), FILE_OPTIONS_ESTRING},
    {{C_STRING_WITH_LEN("md5")}, my_offsetof(TABLE_LIST, md5), FILE_OPTIONS_STRING},
    {{C_STRING_WITH_LEN("updatable")}, my_offsetof(TABLE_LIST, updatable_view), FILE_OPTIONS_ULONGLONG},
    {{C_STRING_WITH_LEN("algorithm")}, my_offsetof(TABLE_LIST, algorithm), FILE_OPTIONS_ULONGLONG},
    {{C_STRING_WITH_LEN("definer_user")}, my_offsetof(TABLE_LIST, definer.user), FILE_OPTIONS_STRING},
    {{C_STRING_WITH_LEN("definer_host")}, my_offsetof(TABLE_LIST, definer.host), FILE_OPTIONS_STRING},
    {{C_STRING_WITH_LEN("suid")}, my_offsetof(TABLE_LIST, view_suid), FILE_OPTIONS_ULONGLONG},
    {{C_STRING_WITH_LEN("with_check_option")}, my_offsetof(TABLE_LIST, with_check), FILE_OPTIONS_ULONGLONG},
    {{C_STRING_WITH_LEN("timestamp")}, my_offsetof(TABLE_LIST, timestamp), FILE_OPTIONS_TIMESTAMP},
    {{C_STRING_WITH_LEN("create-version")}, my_offsetof(TABLE_LIST, file_version), FILE_OPTIONS_ULONGLONG},
    {{C_STRING_WITH_LEN("source")}, my_offsetof(TABLE_LIST, source), FILE_OPTIONS_ESTRING},
...
static int mysql_register_view(THD *thd, TABLE_LIST *view, enum_view_create_mode mode)
{
  ...
  */
  const bool can_be_merged = lex->unit->is_mergeable() && lex->create_view_algorithm != VIEW_ALGORITHM_TEMPTABLE;

  if (can_be_merged)
  {
    for (ORDER *order = lex->select_lex->order_list.first; order; order = order->next)
  ...
  if (!thd->make_lex_string(&view->select_stmt, view_query.ptr(), view_query.length(), false))
  {
  ...
  {
    push_warning(thd, Sql_condition::SL_WARNING, ER_WARN_VIEW_MERGE, ER(ER_WARN_VIEW_MERGE));
  ...
  if ((view->updatable_view = can_be_merged))
  {
    ...
    for (TABLE_LIST *tbl = lex->select_lex->table_list.first; tbl; tbl = tbl->next_local)
    {
      updatable |= !((tbl->is_view() && !tbl->updatable_view) || tbl->schema_table);
  ...
  /* print file name */
  dir.length = build_table_filename(dir_buff, sizeof(dir_buff) - 1, view->db, "", "", 0);
  dir.str = dir_buff;

  path.length =
      build_table_filename(path_buff, sizeof(path_buff) - 1, view->db, view->table_name, reg_ext, 0, &was_truncated);
  // Check if we hit FN_REFLEN bytes in path length
  if (was_truncated)
  {
    my_error(ER_IDENT_CAUSES_TOO_LONG_PATH, MYF(0), sizeof(path_buff) - 1, path_buff);
    error = -1;
    goto err;
  }
  path.str = path_buff;
  ...
  */
  ...
  lex_string_set(&view->view_client_cs_name, view->view_creation_ctx->get_client_cs()->csname);

  lex_string_set(&view->view_connection_cl_name, view->view_creation_ctx->get_connection_cl()->name);

  if (!thd->make_lex_string(&view->view_body_utf8, is_query.ptr(), is_query.length(), false))
  {
    my_error(ER_OUT_OF_RESOURCES, MYF(0));
    error = -1;
    goto err;
  ...
  /*
    ...
    UNION
  */
  if (view->updatable_view && !lex->select_lex->master_unit()->is_union() &&
...
bool open_and_read_view(THD *thd, TABLE_SHARE *share, TABLE_LIST *view_ref)
{
  ...
  if (view_ref->is_view())
  {
    ...
    */
    DBUG_PRINT("info", ("VIEW %s.%s is already processed on previous PS/SP execution", view_ref->view_db.str,
                        view_ref->view_name.str));
    DBUG_RETURN(false);
  }

  if (view_ref->index_hints && view_ref->index_hints->elements)
  {
    my_error(ER_KEY_DOES_NOT_EXITS, MYF(0), view_ref->index_hints->head()->key_name.str, view_ref->table_name);
    DBUG_RETURN(true);
  }

  // Check that view is not referenced recursively
  for (TABLE_LIST *precedent = view_ref->referencing_view; precedent; precedent = precedent->referencing_view)
  {
    if (precedent->view_name.length == view_ref->table_name_length &&
        precedent->view_db.length == view_ref->db_length &&
        my_strcasecmp(system_charset_info, precedent->view_name.str, view_ref->table_name) == 0 &&
        my_strcasecmp(system_charset_info, precedent->view_db.str, view_ref->db) == 0)
    {
  ...
  if (share->view_def->parse((uchar *)view_ref, thd->mem_root, view_parameters, required_view_parameters,
  ...
  {
    assert(!view_ref->definer.host.str && !view_ref->definer.user.length && !view_ref->definer.host.length);
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_VIEW_FRM_NO_USER, ER(ER_VIEW_FRM_NO_USER), view_ref->db,
                        view_ref->table_name);
...
{
  for (SELECT_LEX *select = view_lex->all_selects_list; select != NULL; select = select->next_select_in_list())
...
bool parse_view_definition(THD *thd, TABLE_LIST *view_ref)
{
  ...
  */
  ...
  if ((result = parser_state.init(thd, view_ref->select_stmt.str, view_ref->select_stmt.length)))
  ...
  */
  bool dbchanged;
  if ((result = mysql_opt_change_db(thd, view_ref->view_db, &old_db, 1, &dbchanged)))
    DBUG_RETURN(true); /* purecov: inspected */
  ...
  */
  thd->variables.sql_mode &= ~(MODE_PIPES_AS_CONCAT | MODE_ANSI_QUOTES | MODE_IGNORE_SPACE | MODE_NO_BACKSLASH_ESCAPES);
  ...
  */
  if (!view_ref->prelocking_placeholder)
  {
    ...
    */
    ...
    else if ((old_lex->sql_command == SQLCOM_SHOW_CREATE) && !view_ref->belong_to_view)
    {
      if ((result = check_table_access(thd, SHOW_VIEW_ACL, view_ref, false, UINT_MAX, false)))
        DBUG_RETURN(true);
    }
  }
  ...
  */
  ...
  for (TABLE_LIST *tbl = view_tables; tbl; tbl = (view_tables_tail = tbl)->next_global)
  {
  ...
  */
  ...
  const bool view_is_mergeable = view_ref->algorithm != VIEW_ALGORITHM_TEMPTABLE && view_lex->unit->is_mergeable();
    ...
    for (TABLE_LIST *tbl = view_main_select_tables; tbl; tbl = tbl->next_local)
    {
      ...
      if (old_lex->sql_command == SQLCOM_LOCK_TABLES)
      {
        ...
        */
        ...
        {
          mdl_lock_type = (tbl->lock_type == TL_READ) ? MDL_SHARED_READ : MDL_SHARED_READ_ONLY;
        }
      }
      else
      {
  ...
  */
  ...
  if (view_tables)
  {
    ...
    for (TABLE_LIST *tbl = view_tables; tbl != view_tables_tail->next_global; tbl = tbl->next_global)
      tbl->security_ctx = security_ctx;
  }

  // Assign security context to name resolution contexts of view
  for (SELECT_LEX *sl = view_lex->all_selects_list; sl; sl = sl->next_select_in_list())
  ...
  */
  ...
  view_lex->all_selects_list->include_chain_in_global(&old_lex->all_selects_list);
  ...
  if (!result && old_lex->sql_command == SQLCOM_LOCK_TABLES && view_tables)
  {
    ...
    */
    ...
    for (TABLE_LIST *tbl = view_tables; tbl != view_tables_tail->next_global; tbl = tbl->next_global)
    {
      ...
      if (check_lock_view_underlying_table_access(thd, tbl, &fake_lock_tables_acl))
      {
        result = true;
        break;
      }
...
bool mysql_drop_view(THD *thd, TABLE_LIST *views, enum_drop_mode drop_mode)
{
  ...
  */
  ...
  for (view = views; view; view = view->next_local)
  {
    ...
    build_table_filename(path, sizeof(path) - 1, view->db, view->table_name, reg_ext, 0);

    if (access(path, F_OK) || FRMTYPE_VIEW != (type = dd_frm_type(thd, path, &not_used)))
    {
  ...
  if (wrong_object_name)
  {
    my_error(ER_WRONG_OBJECT, MYF(0), wrong_object_db, wrong_object_name, "VIEW");
  }
  if (non_existant_views.length())
  {
  ...
  if (some_views_deleted || !something_wrong)
  {
    ...
    if (ret == 1)
    {
      ...
      */
      if (write_bin_log(thd, !something_wrong, thd->query().str, thd->query().length))
        something_wrong = 1;
    }
    else if (ret == -1)
      something_wrong = 1;
...
bool check_key_in_view(THD *thd, TABLE_LIST *view, const TABLE_LIST *table_ref)
{
  ...
  */
  if ((!view->is_view() && !view->belong_to_view) || thd->lex->sql_command == SQLCOM_INSERT ||
      ...
      if (fld == end_of_trans)  // If field didn't exists
      {
        ...
        */
        if (thd->variables.updatable_views_with_limit)
        {
          ...
          push_warning(thd, Sql_condition::SL_NOTE, ER_WARN_VIEW_WITHOUT_KEY, ER(ER_WARN_VIEW_WITHOUT_KEY));
          DBUG_RETURN(FALSE);
        }
        /* prohibit update */
        DBUG_RETURN(TRUE);
...
bool mysql_rename_view(THD *thd, const char *new_db, const char *new_name, TABLE_LIST *view)
{
  ...
  pathstr.length = build_table_filename(path_buff, sizeof(path_buff) - 1, view->db, view->table_name, reg_ext, 0);
  ...
  if ((parser = sql_parse_prepare(&pathstr, thd->mem_root, 1)) && is_equal(&view_type, parser->type()))
  {
    ...
    */
    ...
    dir.str = dir_buff;
    dir.length = build_table_filename(dir_buff, sizeof(dir_buff) - 1, new_db, "", "", 0);

    pathstr.str = path_buff;
    pathstr.length =
        build_table_filename(path_buff, sizeof(path_buff) - 1, new_db, new_name, reg_ext, 0, &was_truncated);
