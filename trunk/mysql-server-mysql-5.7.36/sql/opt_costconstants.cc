...
cost_constant_error Server_cost_constants::set(const LEX_CSTRING &name, double value)
{
  ...
  */
  ...
  // ROW_EVALUATE_COST
  if (my_strcasecmp(&my_charset_utf8_general_ci, "ROW_EVALUATE_COST", name.str) == 0)
  {
    m_row_evaluate_cost = value;
    return COST_CONSTANT_OK;
  }
  // KEY_COMPARE_COST
  if (my_strcasecmp(&my_charset_utf8_general_ci, "KEY_COMPARE_COST", name.str) == 0)
  {
    m_key_compare_cost = value;
    return COST_CONSTANT_OK;
  }
  // MEMORY_TEMPTABLE_CREATE_COST
  if (my_strcasecmp(&my_charset_utf8_general_ci, "MEMORY_TEMPTABLE_CREATE_COST", name.str) == 0)
  {
    m_memory_temptable_create_cost = value;
    return COST_CONSTANT_OK;
  }
  // MEMORY_TEMPTABLE_ROW_COST
  if (my_strcasecmp(&my_charset_utf8_general_ci, "MEMORY_TEMPTABLE_ROW_COST", name.str) == 0)
  {
    m_memory_temptable_row_cost = value;
    return COST_CONSTANT_OK;
  }
  // DISK_TEMPTABLE_CREATE_COST
  if (my_strcasecmp(&my_charset_utf8_general_ci, "DISK_TEMPTABLE_CREATE_COST", name.str) == 0)
  {
    m_disk_temptable_create_cost = value;
    return COST_CONSTANT_OK;
  }
  // DISK_TEMPTABLE_ROW_COST
  if (my_strcasecmp(&my_charset_utf8_general_ci, "DISK_TEMPTABLE_ROW_COST", name.str) == 0)
  {
...
cost_constant_error SE_cost_constants::set(const LEX_CSTRING &name, const double value, bool default_value)
{
  assert(name.str != NULL);
  assert(name.length > 0);
  ...
  */
  ...
  if (my_strcasecmp(&my_charset_utf8_general_ci, "MEMORY_BLOCK_READ_COST", name.str) == 0)
  {
    update_cost_value(&m_memory_block_read_cost, &m_memory_block_read_cost_default, value, default_value);
    return COST_CONSTANT_OK;
  }
  // IO_BLOCK_READ_COST
  if (my_strcasecmp(&my_charset_utf8_general_ci, "IO_BLOCK_READ_COST", name.str) == 0)
  {
    update_cost_value(&m_io_block_read_cost, &m_io_block_read_cost_default, value, default_value);
    return COST_CONSTANT_OK;
  }
...
cost_constant_error SE_cost_constants::update(const LEX_CSTRING &name, const double value)
{
  return set(name, value, false);
}

cost_constant_error SE_cost_constants::update_default(const LEX_CSTRING &name, const double value)
{
  return set(name, value, true);
}

void SE_cost_constants::update_cost_value(double *cost_constant, bool *cost_constant_is_default, double new_value,
...
Cost_model_constants::Cost_model_constants() : m_ref_counter(0)
{
  /**
...
const SE_cost_constants *Cost_model_constants::get_se_cost_constants(const TABLE *table) const
{
  ...
  const SE_cost_constants *se_cc = m_engines[table->file->ht->slot].get_cost_constants(DEFAULT_STORAGE_CLASS);
  assert(se_cc != NULL);

  return se_cc;
}

cost_constant_error Cost_model_constants::update_server_cost_constant(const LEX_CSTRING &name, double value)
{
  return m_server_constants.set(name, value);
}

cost_constant_error Cost_model_constants::update_engine_cost_constant(THD *thd, const LEX_CSTRING &se_name,
                                                                      uint storage_category, const LEX_CSTRING &name,
                                                                      double value)
{
  ...
  else
  {
    ...
    SE_cost_constants *se_cc = m_engines[ht_slot_id].get_cost_constants(storage_category);
    assert(se_cc != NULL);
...
uint Cost_model_constants::find_handler_slot_from_name(THD *thd, const LEX_CSTRING &name) const
{
...
cost_constant_error Cost_model_constants::update_engine_default_cost(const LEX_CSTRING &name, uint storage_category,
                                                                     double value)
{
