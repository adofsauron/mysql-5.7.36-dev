...
class READ_INFO
{
 ...
 public:
  ...
  READ_INFO(File file, uint tot_length, const CHARSET_INFO *cs, const String &field_term, const String &line_start,
            const String &line_term, const String &enclosed, int escape, bool get_it_from_net, bool is_fifo);
  ~READ_INFO();
  int read_field();
...
static int read_sep_field(THD *thd, COPY_INFO &info, TABLE_LIST *table_list, List<Item> &fields_vars,
                          List<Item> &set_fields, List<Item> &set_values, READ_INFO &read_info, const String &enclosed,
                          ulong skip_lines);
...
static bool write_execute_load_query_log_event(THD *thd, sql_exchange *ex, const char *db_arg, /* table's database */
...
int mysql_load(THD *thd, sql_exchange *ex, TABLE_LIST *table_list, List<Item> &fields_vars, List<Item> &set_fields,
               List<Item> &set_values, enum enum_duplicates handle_duplicates, bool read_file_from_client)
{
  char name[FN_REFLEN];
  File file;
  int error = 0;
  ...
  */
  ...
  if (!escaped->is_ascii() || !enclosed->is_ascii() || !field_term->is_ascii() || !ex->line.line_term->is_ascii() ||
      !ex->line.line_start->is_ascii())
  {
  ...
  TABLE_LIST *const insert_table_ref = table_list->is_updatable() &&                 // View must be updatable
  ...
  if (insert_table_ref == NULL || check_key_in_view(thd, table_list, insert_table_ref))
  {
  ...
  if (table_list->is_merged())
  {
    ...
    if (handle_duplicates == DUP_REPLACE && table_list->prepare_replace_filter(thd))
      DBUG_RETURN(true);
  }
  ...
  */
  ...
  if (!fields_vars.elements)
  {
    ...
    /*
      ...
      in this case.
    */
    if (setup_fields(thd, Ref_ptr_array(), set_fields, INSERT_ACL, NULL, false, true) ||
        setup_fields(thd, Ref_ptr_array(), set_values, SELECT_ACL, NULL, false, false))
      DBUG_RETURN(TRUE);
  }
  else
  {  // Part field list
    ...
    */
    if (setup_fields(thd, Ref_ptr_array(), fields_vars, INSERT_ACL, NULL, false, false) ||
        setup_fields(thd, Ref_ptr_array(), set_fields, INSERT_ACL, NULL, false, true))
      DBUG_RETURN(TRUE);
    ...
    */
    ...
    while ((item = it++))
    {
      if ((item->type() == Item::FIELD_ITEM || item->type() == Item::REF_ITEM) && item->field_for_view_update() == NULL)
      {
        my_error(ER_NONUPDATEABLE_COLUMN, MYF(0), item->item_name.ptr());
        DBUG_RETURN(true);
      }
    ...
    if (setup_fields(thd, Ref_ptr_array(), set_values, SELECT_ACL, NULL, false, false))
      DBUG_RETURN(TRUE);
  }

  const int escape_char =
      (escaped->length() && (ex->escaped_given() || !(thd->variables.sql_mode & MODE_NO_BACKSLASH_ESCAPES)))
          ? (*escaped)[0]
          : INT_MAX;
  ...
  */
  ...
  COPY_INFO info(COPY_INFO::INSERT_OPERATION, &fields_vars, &set_fields, manage_defaults, handle_duplicates,
                 escape_char);
  ...
  {
    (void)net_request_file(thd->get_protocol_classic()->get_net(), ex->file_name);
    file = -1;
  }
  else
#endif
  {
    ...
    if ((thd->system_thread & (SYSTEM_THREAD_SLAVE_SQL | SYSTEM_THREAD_SLAVE_WORKER)) != 0)
    {
#if defined(HAVE_REPLICATION) && !defined(MYSQL_CLIENT)
      Relay_log_info *rli = thd->rli_slave->get_c_rli();

      if (strncmp(rli->slave_patternload_file, name, rli->slave_patternload_file_size))
      {
        /*
  ...
  READ_INFO read_info(file, tot_length, ex->cs ? ex->cs : thd->variables.collation_database, *field_term,
                      *ex->line.line_start, *ex->line.line_term, *enclosed, info.escape_char, read_file_from_client,
                      is_fifo);
  if (read_info.error)
  {
    if (file >= 0)
  ...
  if (!(error = MY_TEST(read_info.error)))
  {
    ...
    if (ex->filetype == FILETYPE_XML) /* load xml */
      error = read_xml_field(thd, info, insert_table_ref, fields_vars, set_fields, set_values, read_info, skip_lines);
    else if (!field_term->length() && !enclosed->length())
      error =
          read_fixed_length(thd, info, insert_table_ref, fields_vars, set_fields, set_values, read_info, skip_lines);
    else
      error = read_sep_field(thd, info, insert_table_ref, fields_vars, set_fields, set_values, read_info, *enclosed,
                             skip_lines);
  ...
  */
        ...
        */
        ...
        if (lf_info.wrote_create_file)
        {
          ...
          if (thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT))
            (void)write_execute_load_query_log_event(thd, ex, table_list->db, table_list->table_name, is_concurrent,
  ...
  my_snprintf(name, sizeof(name), ER(ER_LOAD_INFO), (long)info.stats.records, (long)info.stats.deleted,
              (long)(info.stats.records - info.stats.copied), (long)thd->get_stmt_da()->current_statement_cond_count());
  ...
  if (mysql_bin_log.is_open())
  {
    ...
    else
    {
      ...
      */
      ...
      {
        int errcode = query_error_code(thd, killed_status == THD::NOT_KILLED);
        error = write_execute_load_query_log_event(thd, ex, table_list->db, table_list->table_name, is_concurrent,
                                                   handle_duplicates, transactional_table, errcode);
      }
...
err:
  assert(table->file->has_transactions() || !(info.stats.copied || info.stats.deleted) ||
...
static bool write_execute_load_query_log_event(THD *thd, sql_exchange *ex, const char *db_arg, /* table's database */
                                               const char *table_name_arg, bool is_concurrent,
                                               enum enum_duplicates duplicates, bool transactional_table, int errcode)
{
  ...
  Load_log_event lle(thd, ex, tdb, tbl, fv, is_concurrent, duplicates, thd->lex->is_ignore(), transactional_table);
  ...
  */
  if (!thd->lex->load_field_list.is_empty())
  {
    ...
    while ((item = li++))
    {
      ...
      if (item->type() == Item::FIELD_ITEM || item->type() == Item::REF_ITEM)
        append_identifier(thd, &pfields, item->item_name.ptr(), strlen(item->item_name.ptr()));
      else
  ...
  Execute_load_query_log_event e(
      ...
      (duplicates == DUP_REPLACE) ? binary_log::LOAD_DUP_REPLACE
                                  : (thd->lex->is_ignore() ? binary_log::LOAD_DUP_IGNORE : binary_log::LOAD_DUP_ERROR),
  ...
  while (!read_info.read_fixed_length())
  {
    ...
    */
    ...
    while ((item = it++))
    {
      ...
      */
      ...
      if (pos == read_info.row_end)
      {
        ...
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_TOO_FEW_RECORDS, ER(ER_WARN_TOO_FEW_RECORDS),
    ...
    if (pos != read_info.row_end)
    {
      ...
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_TOO_MANY_RECORDS, ER(ER_WARN_TOO_MANY_RECORDS),
                          thd->get_stmt_da()->current_row_for_condition());
    }

    if (thd->killed || fill_record_n_invoke_before_triggers(thd, &info, set_fields, set_values, table, TRG_EVENT_INSERT,
                                                            table->s->fields))
      DBUG_RETURN(1);
        ...
        if (item->type() == Item::FIELD_ITEM)
        {
          ...
          if (!field->maybe_null())
          {
            if (field->type() == FIELD_TYPE_TIMESTAMP)
              ...
              field->set_warning(Sql_condition::SL_WARNING, ER_WARN_NULL_TO_NOTNULL, 1);
          }
        }
        else
        {
...
READ_INFO::READ_INFO(File file_par, uint tot_length, const CHARSET_INFO *cs, const String &field_term,
                     const String &line_start, const String &line_term, const String &enclosed_par, int escape,
      ...
      read_charset(cs)
{
  ...
  */
  field_term_ptr = static_cast<const uchar *>(static_cast<const void *>(field_term.ptr()));
  field_term_length = field_term.length();
  line_term_ptr = static_cast<const uchar *>(static_cast<const void *>(line_term.ptr()));
  ...
  if (!(buffer = (uchar *)my_malloc(key_memory_READ_INFO, buff_length + 1, MYF(MY_WME))))
...
int READ_INFO::read_field()
{
  ...
  for (;;)
  {
    ...
    while (to < end_of_buff)
    {
      ...
      {
        *to = '\0';
        my_error(ER_INVALID_CHARACTER_STRING, MYF(0), read_charset->csname, buffer);
        error = true;
        return 1;
      }
    ...
    */
    if (!(new_buffer =
              (uchar *)my_realloc(key_memory_READ_INFO, (char *)buffer, buff_length + 1 + IO_SIZE, MYF(MY_WME))))
      return (error = true);
  ...
  for (chr = my_tospace(GET); chr != my_b_EOF;)
  {
    switch (chr)
    {
      case '<': /* read tag */
        ...
        if ((tag.length() == line_term_length - 2) && (memcmp(tag.ptr(), line_term_ptr + 1, tag.length()) == 0))
        {
          DBUG_PRINT("read_xml", ("start-of-row: %i %s %s", level, tag.c_ptr_safe(), line_term_ptr));
        }
      ...
      case '=': /* attribute name end - read the value */
        ...
        {
          DBUG_PRINT("read_xml", ("lev:%i att:%s val:%s\n", level + 1, attribute.c_ptr_safe(), value.c_ptr_safe()));
