 ...
 public:
  ...
  uint32 save_in_field(Ndb_item *field_item)
  {
    ...
    if (item && field)
    {
      ...
      my_bitmap_map *old_map = dbug_tmp_use_all_columns(field->table, field->table->write_set);
...
class Ndb_expect_stack : public Sql_alloc
{
 ...
 public:
  Ndb_expect_stack() : collation(NULL), length(0), max_length(0), next(NULL)
  {
    ...
    bitmap_init(&expect_field_type_mask, m_expect_field_type_buf, MAX_EXPECT_FIELD_TYPES, FALSE);
    bitmap_init(&expect_field_result_mask, m_expect_field_result_buf, MAX_EXPECT_FIELD_RESULTS, FALSE);
  }
  ~Ndb_expect_stack()
  {
    if (next)
  ...
  void dont_expect(Item::Type type) { bitmap_clear_bit(&expect_mask, (uint)type); }
  ...
  void expect_field_type(enum_field_types type) { bitmap_set_bit(&expect_field_type_mask, (uint)type); }
  ...
  void expect_no_field_type() { bitmap_clear_all(&expect_field_type_mask); }
  bool expecting_no_field_type() { return bitmap_is_clear_all(&expect_field_type_mask); }
  ...
  void expect_field_result(Item_result result) { bitmap_set_bit(&expect_field_result_mask, (uint)result); }
  ...
  bool expecting_no_field_result() { return bitmap_is_clear_all(&expect_field_result_mask); }
  ...
  bool expecting_length(Uint32 len) { return max_length == 0 || len <= max_length; }
 ...
 private:
  ...
  my_bitmap_map m_expect_field_type_buf[bitmap_buffer_size(MAX_EXPECT_FIELD_TYPES)];
  my_bitmap_map m_expect_field_result_buf[bitmap_buffer_size(MAX_EXPECT_FIELD_RESULTS)];
...
class Ndb_cond_traverse_context : public Sql_alloc
{
 public:
  Ndb_cond_traverse_context(TABLE *tab, const NdbDictionary::Table *ndb_tab, Ndb_cond_stack *stack)
      : table(tab), ndb_table(ndb_tab), supported(TRUE), cond_stack(stack), cond_ptr(NULL), skip(0), rewrite_stack(NULL)
  {
    if (stack)
  ...
  inline bool expecting(Item::Type type) { return expect_stack.expecting(type); }
  ...
  inline void expect_field_type(enum_field_types type) { expect_stack.expect_field_type(type); }
  inline void expect_all_field_types() { expect_stack.expect_all_field_types(); }
  inline bool expecting_field_type(enum_field_types type) { return expect_stack.expecting_field_type(type); }
  inline void expect_no_field_type() { expect_stack.expect_no_field_type(); }
  inline bool expecting_no_field_type() { return expect_stack.expecting_no_field_type(); }
  inline void expect_only_field_type(enum_field_types result) { expect_stack.expect_only_field_type(result); }

  inline void expect_field_result(Item_result result) { expect_stack.expect_field_result(result); }
  inline bool expecting_field_result(Item_result result) { return expect_stack.expecting_field_result(result); }
  inline void expect_no_field_result() { expect_stack.expect_no_field_result(); }
  inline bool expecting_no_field_result() { return expect_stack.expecting_no_field_result(); }
  inline void expect_only_field_result(Item_result result) { expect_stack.expect_only_field_result(result); }
  inline void expect_collation(const CHARSET_INFO *col) { expect_stack.expect_collation(col); }
  inline bool expecting_collation(const CHARSET_INFO *col) { return expect_stack.expecting_collation(col); }
  inline void expect_length(Uint32 length) { expect_stack.expect_length(length); }
  inline void expect_max_length(Uint32 max) { expect_stack.expect_max_length(max); }
  inline bool expecting_length(Uint32 length) { return expect_stack.expecting_length(length); }
  inline bool expecting_max_length(Uint32 max) { return expect_stack.expecting_max_length(max); }
...
{
 public:
  ha_ndbcluster_cond() : m_cond_stack(NULL) {}
  ~ha_ndbcluster_cond()
  {
    if (m_cond_stack)
      delete m_cond_stack;
  }
  const Item *cond_push(const Item *cond, TABLE *table, const NdbDictionary::Table *ndb_table);
  void cond_pop();
  void cond_clear();
  int generate_scan_filter(NdbInterpretedCode *code, NdbScanOperation::ScanOptions *options) const;
  int generate_scan_filter_from_cond(NdbScanFilter &filter) const;
  int generate_scan_filter_from_key(NdbInterpretedCode *code, NdbScanOperation::ScanOptions *options,
                                    const KEY *key_info, const key_range *start_key, const key_range *end_key) const;

 private:
  bool serialize_cond(const Item *cond, Ndb_cond_stack *ndb_cond, TABLE *table,
                      const NdbDictionary::Table *ndb_table) const;
  int build_scan_filter_predicate(Ndb_cond *&cond, NdbScanFilter *filter, bool negated = false) const;
  int build_scan_filter_group(Ndb_cond *&cond, NdbScanFilter *filter) const;
