...
class Old_rows_log_event : public Binary_log_event, public Log_event
{
  ...
  */
...
#endif
  Old_rows_log_event(const char *row_data, uint event_len, Log_event_type event_type,
  ...
  int unpack_current_row(const Relay_log_info *const rli)
  {
    ...
    return ::unpack_row(rli, m_table, m_width, m_curr_row, &m_cols, &m_curr_row_end, &m_master_reclength, m_rows_end);
  }
#endif
  ...
  */
   ...
   */
  virtual int do_prepare_row(THD *, Relay_log_info const *, TABLE *, uchar const *row_start, uchar const **row_end) = 0;
...
class Write_rows_log_event_old : public Old_rows_log_event
{
 ...
 public:
  ...
  Write_rows_log_event_old(const char *buf, uint event_len, const Format_description_event *description_event);
#endif
#if !defined(MYSQL_CLIENT)
 ...
 private:
  ...
  virtual int do_apply_event(const Relay_log_info *rli) { return Old_rows_log_event::do_apply_event(this, rli); }

  // primitives for old version of do_apply_event()
  virtual int do_before_row_operations(TABLE *table);
  virtual int do_after_row_operations(TABLE *table, int error);
  virtual int do_prepare_row(THD *, Relay_log_info const *, TABLE *, uchar const *row_start, uchar const **row_end);
...
#ifdef HAVE_REPLICATION
  Update_rows_log_event_old(const char *buf, uint event_len, const Format_description_event *description_event);
#endif
...
#endif
#ifdef HAVE_REPLICATION
  Delete_rows_log_event_old(const char *buf, uint event_len, const Format_description_event *description_event);
#endif
#if !defined(MYSQL_CLIENT)
  static bool binlog_row_logging_function(THD *thd, TABLE *table, bool is_transactional, const uchar *before_record,
