...
Error_log_throttle slave_ignored_err_throttle(window_size, sql_print_information,
...
*/
static void inline slave_rows_error_report(enum loglevel level, int ha_error, Relay_log_info const *rli, THD *thd,
                                           TABLE *table, const char *type, const char *log_name, ulong pos)
{
  ...
  {
    len = my_snprintf(slider, buff_end - slider, " %s, Error_code: %d;", err->message_text(), err->mysql_errno());
  }
  if (is_group_replication_applier_channel)
  {
    if (ha_error != 0)
    {
      rli->report(level, thd->is_error() ? thd->get_stmt_da()->mysql_errno() : ER_UNKNOWN_ERROR,
...
int convert_handler_error(int error, THD *thd, TABLE *table)
{
  ...
  if (actual_error == 0)
  {
    ...
    actual_error = (thd->is_error() ? thd->get_stmt_da()->mysql_errno() : ER_UNKNOWN_ERROR);
...
static bool write_str_at_most_255_bytes(IO_CACHE *file, const char *str, uint length)
{
  uchar tmp[1];
  tmp[0] = (uchar)length;
  return (my_b_safe_write(file, tmp, sizeof(tmp)) || my_b_safe_write(file, (uchar *)str, length));
}
...
static void print_set_option(IO_CACHE *file, uint32 bits_changed, uint32 option, uint32 flags, const char *name,
                             bool *need_comma)
{
...
Log_event::Log_event(THD *thd_arg, uint16 flags_arg, enum_event_cache_type cache_type_arg,
                     enum_event_logging_type logging_type_arg, Log_event_header *header, Log_event_footer *footer)
...
Log_event::Log_event(Log_event_header *header, Log_event_footer *footer, enum_event_cache_type cache_type_arg,
...
Log_event::enum_skip_reason Log_event::do_shall_skip(Relay_log_info *rli)
{
  ...
  */
  DBUG_PRINT("info", ("ev->server_id=%lu, ::server_id=%lu,"
                      ...
                      (ulong)server_id, (ulong)::server_id, rli->replicate_same_server_id, rli->slave_skip_counter));
...
void Log_event::init_show_field_list(List<Item> *field_list)
{
  ...
  field_list->push_back(new Item_return_int("End_log_pos", MY_INT32_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG));
...
my_bool Log_event::need_checksum()
{
  ...
  */
  ...
  assert(get_type_code() != binary_log::FORMAT_DESCRIPTION_EVENT || ret || common_header->data_written == 0);
  ...
  assert(!ret || ((common_footer->checksum_alg == static_cast<enum_binlog_checksum_alg>(binlog_checksum_options) ||
                   /*
  ...
  assert(((get_type_code() != binary_log::ROTATE_EVENT && get_type_code() != binary_log::STOP_EVENT) ||
          get_type_code() != binary_log::FORMAT_DESCRIPTION_EVENT) ||
         event_cache_type == Log_event::EVENT_NO_CACHE);
...
bool Log_event::wrapper_my_b_safe_write(IO_CACHE *file, const uchar *buf, size_t size)
{
  ...
  DBUG_EXECUTE_IF("simulate_temp_file_write_error", { DBUG_SET("-d,simulate_file_write_error"); });
  return ret;
}
...
uint32 Log_event::write_header_to_memory(uchar *buf)
{
...
#ifndef NDEBUG
  if (DBUG_EVALUATE_IF("inc_event_time_by_1_hour", 1, 0) && DBUG_EVALUATE_IF("dec_event_time_by_1_hour", 1, 0))
  {
    /**
  ...
  */
  ...
  int4store(buf + EVENT_LEN_OFFSET, static_cast<uint32>(common_header->data_written));
...
bool Log_event::write_header(IO_CACHE *file, size_t event_data_length)
{
  ...
  */
  ...
  /*
    ...
    too.
  */
  if (need_checksum() && (common_header->flags & LOG_EVENT_BINLOG_IN_USE_F) != 0)
  {
...
int Log_event::read_log_event(IO_CACHE *file, String *packet, mysql_mutex_t *log_lock,
                              enum_binlog_checksum_alg checksum_alg_arg, const char *log_file_name_arg,
                              bool *is_binlog_active, char *event_header)
{
  ...
  if (data_len < LOG_EVENT_MINIMAL_HEADER_LEN ||
      data_len > max(current_thd->variables.max_allowed_packet, opt_binlog_rows_event_max_size + MAX_LOG_EVENT_HEADER))
  {
  ...
  */
  if (buf == local_buf)
  {
    ...
    if (new_alloc_len > packet->alloced_length() && packet->mem_realloc(new_alloc_len))
    {
  ...
  if (packet->alloced_length() - packet->length() < data_len || packet->append(buf, LOG_EVENT_MINIMAL_HEADER_LEN))
  {
  ...
  if (data_len)
  {
    ...
    */
    char *event_data_buffer = const_cast<char *>(packet->ptr() + packet->length());
    result = my_b_read(file, reinterpret_cast<uchar *>(event_data_buffer), data_len);
    if (result)
    {
      /*
      ...
      */
      ...
      result = (my_errno() == ENOMEM ? LOG_READ_MEM : (file->error >= 0 ? LOG_READ_TRUNC : LOG_READ_IO));
      goto end;
    }
    else
    {
      ...
      */
      DBUG_EXECUTE_IF(
          "corrupt_read_log_event", uchar *debug_event_buf_c = (uchar *)packet->ptr() + ev_offset;
          if (debug_event_buf_c[EVENT_TYPE_OFFSET] != binary_log::FORMAT_DESCRIPTION_EVENT &&
            ...
            int debug_cor_pos = rand() % (data_len + LOG_EVENT_MINIMAL_HEADER_LEN - BINLOG_CHECKSUM_LEN);
...
#endif
{
  ...
  */
  uint header_size = min<uint>(description_event->common_header_len, LOG_EVENT_MINIMAL_HEADER_LEN);
  ...
  ulong const max_size = max<ulong>(log_max_allowed_packet, opt_binlog_rows_event_max_size + MAX_LOG_EVENT_HEADER);
  ...
  if (!(buf = (char *)my_malloc(key_memory_log_event, data_len + 1, MYF(MY_WME))))
  {
    error = "Out of memory";
    goto err;
  }
                                     ...
                                     const Format_description_log_event *description_event, my_bool crc_check)
{
  ...
  if (event_len < EVENT_LEN_OFFSET || event_len != uint4korr(buf + EVENT_LEN_OFFSET))
  {
    DBUG_PRINT("error", ("event_len=%u EVENT_LEN_OFFSET=%d "
                         ...
                         event_len, EVENT_LEN_OFFSET, buf[EVENT_TYPE_OFFSET], binary_log::ENUM_END_EVENT,
  ...
  if (event_type == binary_log::START_EVENT_V3)
    (const_cast<Format_description_log_event *>(description_event))->common_footer->checksum_alg =
        binary_log::BINLOG_CHECKSUM_ALG_OFF;
  ...
  */
  alg = (event_type != binary_log::FORMAT_DESCRIPTION_EVENT) ? description_event->common_footer->checksum_alg
                                                             : Log_event_footer::get_checksum_alg(buf, event_len);
  // Emulate the corruption during reading an event
  DBUG_EXECUTE_IF(
      "corrupt_read_log_event_char", if (event_type != binary_log::FORMAT_DESCRIPTION_EVENT) {
  ...
  binary_log_debug::debug_checksum_test = DBUG_EVALUATE_IF("simulate_checksum_test_failure", true, false);
#endif
  if (crc_check && Log_event_footer::event_checksum_test((uchar *)buf, event_len, alg) &&
  ...
  if (event_type > description_event->number_of_event_types && event_type != binary_log::FORMAT_DESCRIPTION_EVENT &&
      /*
  ...
  else
  {
    ...
    */
    ...
    if (alg != binary_log::BINLOG_CHECKSUM_ALG_UNDEF &&
        (event_type == binary_log::FORMAT_DESCRIPTION_EVENT || alg != binary_log::BINLOG_CHECKSUM_ALG_OFF))
    ...
    switch (event_type)
    {
      ...
      case binary_log::NEW_LOAD_EVENT:
#ifndef NDEBUG
        binary_log_debug::debug_simulate_invalid_address = DBUG_EVALUATE_IF("simulate_invalid_address", true, false);
#endif
  ...
  DBUG_PRINT("read_event", ("%s(type_code: %d; event_len: %d)", ev ? ev->get_type_str() : "<unknown>",
...
void Log_event::print_header(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info, bool is_more MY_ATTRIBUTE((unused)))
{
  char llbuff[22];
  ...
  my_b_printf(file, " server id %lu  end_log_pos %s ", (ulong)server_id, llstr(common_header->log_pos, llbuff));
      ...
      common_footer->checksum_alg != binary_log::BINLOG_CHECKSUM_ALG_UNDEF)
  {
    ...
    size_t const bytes_written = my_snprintf(checksum_buf, sizeof(checksum_buf), "0x%08lx ", (ulong)crc);
    my_b_printf(file, "%s ", get_type(&binlog_checksum_typelib, common_footer->checksum_alg));
  ...
  if (print_event_info->hexdump_from)
  {
    ...
    my_off_t size = uint4korr(ptr + EVENT_LEN_OFFSET) - LOG_EVENT_MINIMAL_HEADER_LEN;
    my_off_t i;
    ...
    if (print_event_info->common_header_len == LOG_EVENT_MINIMAL_HEADER_LEN)
    {
      ...
      size_t const bytes_written =
          my_snprintf(emit_buf, sizeof(emit_buf),
                      ...
                      (unsigned long)hexdump_from, ptr[0], ptr[1], ptr[2], ptr[3], ptr[4], ptr[5], ptr[6], ptr[7],
                      ptr[8], ptr[9], ptr[10], ptr[11], ptr[12], ptr[13], ptr[14], ptr[15], ptr[16], ptr[17], ptr[18]);
    ...
    for (i = 0, c = char_string, h = hex_string; i < size; i++, ptr++)
    {
      ...
      *c++ = my_isalnum(&my_charset_bin, *ptr) ? *ptr : '.';
      ...
      if (i % 16 == 15)
      {
        ...
        char emit_buf[256];
        size_t const bytes_written =
            my_snprintf(emit_buf, sizeof(emit_buf), "# %8.8lx %-48.48s |%16s|\n",
...
*/
static size_t log_event_print_value(IO_CACHE *file, const uchar *ptr, uint type, uint meta, char *typestr,
                                    size_t typestr_length)
{
  ...
  switch (type)
  {
    ...
    case MYSQL_TYPE_NEWDECIMAL:
    {
      ...
      my_decimal dec;
      binary2my_decimal(E_DEC_FATAL_ERROR, (uchar *)ptr, &dec, precision, decimals);
    ...
    case MYSQL_TYPE_DATETIME:
    {
      ...
      t = i64 % 1000000;
      my_b_printf(file, "%04d-%02d-%02d %02d:%02d:%02d", static_cast<int>(d / 10000), static_cast<int>(d % 10000) / 100,
                  static_cast<int>(d % 100), static_cast<int>(t / 10000), static_cast<int>(t % 10000) / 100,
    ...
    case MYSQL_TYPE_TIME:
    {
      ...
      my_b_printf(file, "'%02d:%02d:%02d'", i32 / 10000, (i32 % 10000) / 100, i32 % 100);
      return 3;
    }
                                             ...
                                             MY_BITMAP *cols_bitmap, const uchar *value, const uchar *prefix)
{
  ...
  for (size_t i = 0; i < td->size(); i++)
  {
    ...
    size_t size = log_event_print_value(file, is_null ? NULL : value, td->type(i), td->field_metadata(i), typestr,
                                        sizeof(typestr));
    if (!size)
      return 0;
...
void Rows_log_event::print_verbose(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info)
{
  ...
  if (m_extra_row_data)
  {
    ...
    my_b_printf(file, "### Extra row data format: %u, len: %u :", m_extra_row_data[EXTRA_ROW_INFO_FORMAT_OFFSET],
                extra_payload_len);
  ...
  {
    char llbuff[22];
    my_b_printf(file, "### Row event for unknown table #%s", llstr(m_table_id, llbuff));
    return;
  }
  ...
  for (const uchar *value = m_rows_buf; value < m_rows_end;)
  {
    size_t length;
#ifdef MYSQL_SERVER
    quoted_db_len = my_strmov_quoted_identifier(this->thd, (char *)quoted_db, map->get_db_name(), 0);
    quoted_table_len = my_strmov_quoted_identifier(this->thd, (char *)quoted_table, map->get_table_name(), 0);
#else
    quoted_db_len = my_strmov_quoted_identifier((char *)quoted_db, map->get_db_name());
    quoted_table_len = my_strmov_quoted_identifier((char *)quoted_table, map->get_table_name());
#endif
...
void Log_event::print_base64(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info, bool more)
{
  ...
  char *const tmp_str = (char *)my_malloc(key_memory_log_event, tmp_str_sz, MYF(MY_WME));
  if (!tmp_str)
  {
    fprintf(stderr,
            "\nError: Out of memory. "
...
void Log_event::print_timestamp(IO_CACHE *file, time_t *ts)
{
  ...
  */
  ...
  my_b_printf(file, "%02d%02d%02d %2d:%02d:%02d", res->tm_year % 100, res->tm_mon + 1, res->tm_mday, res->tm_hour,
...
bool Log_event::contains_partition_info(bool end_group_sets_max_dbs)
{
  ...
  switch (get_type_code())
  {
    ...
    case binary_log::QUERY_EVENT:
    {
      ...
      if ((ends_group() && end_group_sets_max_dbs) || (qev->is_query_prefix_match(STRING_WITH_LEN("XA COMMIT")) ||
...
bool schedule_next_event(Log_event *ev, Relay_log_info *rli)
{
  ...
  switch (error)
  {
    case ER_MTS_CANT_PARALLEL:
      ...
      my_error(ER_MTS_CANT_PARALLEL, MYF(0), ev->get_type_str(), rli->get_event_relay_log_name(), llbuff,
...
Slave_worker *Log_event::get_slave_worker(Relay_log_info *rli)
{
  ...
  if ((is_s_event = starts_group()) || is_gtid_event(this) ||
      ...
      (!rli->curr_group_seen_begin && !rli->curr_group_seen_gtid &&
       /*
         ...
         mapped-to-a-worker.
       */
       (gaq->empty() || gaq->get_job_group(rli->gaq->assigned_group_index)->worker_id != MTS_WORKER_UNDEF)))
  {
    if (!rli->curr_group_seen_gtid && !rli->curr_group_seen_begin)
    {
      ...
      DBUG_PRINT("info", ("gaq_idx= %ld  gaq->size=%ld", gaq->assigned_group_index, gaq->size));
      assert(gaq->assigned_group_index != MTS_WORKER_UNDEF);
      assert(gaq->assigned_group_index < gaq->size);
      assert(gaq->get_job_group(rli->gaq->assigned_group_index)->group_relay_log_name == NULL);
      assert(rli->last_assigned_worker == NULL || !is_mts_db_partitioned(rli));

      if (is_s_event || is_gtid_event(this))
      {
        Slave_job_item job_item = {this, rli->get_event_relay_log_number(), rli->get_event_start_pos()};
    ...
    else
    {
      /*
       ...
       return true.
      */
      Slave_job_item job_item = {this, rli->get_event_relay_log_number(), rli->get_event_relay_log_pos()};
  ...
  else if (contains_partition_info(rli->mts_end_group_sets_max_dbs))
  {
    ...
    */
#ifndef NDEBUG
    {
      bool empty_group_with_gtids = rli->curr_group_seen_begin && rli->curr_group_seen_gtid && ends_group();
      ...
      bool begin_load_query_event =
          ...
          (rli->curr_group_da.back().data->get_type_code() == binary_log::BEGIN_LOAD_QUERY_EVENT);

      bool delete_file_event = ((rli->curr_group_da.size() == 4 && rli->curr_group_seen_gtid) ||
                                (rli->curr_group_da.size() == 3 && !rli->curr_group_seen_gtid)) &&
                               (rli->curr_group_da.back().data->get_type_code() == binary_log::DELETE_FILE_EVENT);

      assert((!ends_group() ||
              (get_type_code() == binary_log::QUERY_EVENT &&
               static_cast<Query_log_event *>(this)->is_query_prefix_match(STRING_WITH_LEN("XA ROLLBACK")))) ||
             empty_group_with_gtids ||
             (rli->mts_end_group_sets_max_dbs && (begin_load_query_event || delete_file_event)));
    }
#endif
    ...
    for (i = 0; i < ((mts_dbs.num != OVER_MAX_DBS_IN_EVENT_MTS) ? mts_dbs.num : 1); i++)
    {
      /*
      ...
      */
      ...
      if (!(ret_worker = map_db_to_worker(mts_dbs.num == OVER_MAX_DBS_IN_EVENT_MTS ? all_db : mts_dbs.name[i], rli,
      ...
      {
        llstr(rli->get_event_relay_log_pos(), llbuff);
        my_error(ER_MTS_CANT_PARALLEL, MYF(0), get_type_str(), rli->get_event_relay_log_name(), llbuff,
      ...
      assert(
          !strcmp(mts_assigned_partitions[i]->db, mts_dbs.num != OVER_MAX_DBS_IN_EVENT_MTS ? mts_dbs.name[i] : all_db));
      assert(ret_worker == mts_assigned_partitions[i]->worker);
      assert(mts_assigned_partitions[i]->usage >= 0);
    }

    if ((ptr_group = gaq->get_job_group(rli->gaq->assigned_group_index))->worker_id == MTS_WORKER_UNDEF)
    {
      ptr_group->worker_id = ret_worker->id;
  ...
  else
  {
    ...
    {
      if (!(get_type_code() == binary_log::INTVAR_EVENT || get_type_code() == binary_log::RAND_EVENT ||
            get_type_code() == binary_log::USER_VAR_EVENT || get_type_code() == binary_log::BEGIN_LOAD_QUERY_EVENT ||
            get_type_code() == binary_log::APPEND_BLOCK_EVENT || get_type_code() == binary_log::DELETE_FILE_EVENT ||
            is_ignorable_event()))
      {
        assert(!ret_worker);
  ...
  */
  if (!ret_worker->master_log_change_notified)
  {
    if (!ptr_group)
      ptr_group = gaq->get_job_group(rli->gaq->assigned_group_index);
    ptr_group->group_master_log_name = my_strdup(key_memory_log_event, rli->get_group_master_log_name(), MYF(MY_WME));
  ...
  if (ends_group() ||
      (!rli->curr_group_seen_begin && (get_type_code() == binary_log::QUERY_EVENT ||
                                       /*
                                       ...
                                       !rli->curr_group_seen_gtid)))
  {
    ...
    */
    if (!ret_worker->relay_log_change_notified)
    {
      ...
      */
      ...
      ptr_group->group_relay_log_name =
          (char *)my_malloc(key_memory_log_event, strlen(rli->get_group_relay_log_name()) + 1, MYF(MY_WME));
    ...
    if (!ret_worker->checkpoint_notified)
    {
      if (!ptr_group)
        ptr_group = gaq->get_job_group(rli->gaq->assigned_group_index);
      ptr_group->checkpoint_log_name = my_strdup(key_memory_log_event, rli->get_group_master_log_name(), MYF(MY_WME));
...
int Log_event::apply_event(Relay_log_info *rli)
{
  ...
  if (rli->is_mts_recovery())
  {
    bool skip = bitmap_is_set(&rli->recovery_groups, rli->mts_recovery_index) &&
                (get_mts_execution_mode(::server_id, rli->mts_group_status == Relay_log_info::MTS_IN_GROUP,
  ...
  if (!(parallel = rli->is_parallel_exec()) ||
      ((actual_exec_mode = get_mts_execution_mode(::server_id, rli->mts_group_status == Relay_log_info::MTS_IN_GROUP,
                                                  rli->current_mts_submode->get_type() == MTS_PARALLEL_TYPE_DB_NAME)) !=
       EVENT_EXEC_PARALLEL))
  {
    if (parallel)
      ...
      */
      ...
      if (actual_exec_mode != EVENT_EXEC_ASYNC)
      {
        ...
        */
        ...
        if (get_type_code() == binary_log::INCIDENT_EVENT && rli->curr_group_da.size() > 0 &&
            rli->current_mts_submode->get_type() == MTS_PARALLEL_TYPE_LOGICAL_CLOCK)
        {
          ...
          */
          ((Mts_submode_logical_clock *)rli->current_mts_submode)->withdraw_delegated_job();
        }
        /*
        ...
        */
        assert(rli->mts_group_status == Relay_log_info::MTS_NOT_IN_GROUP || !is_mts_db_partitioned(rli));
  ...
  assert(!(rli->curr_group_seen_begin && ends_group()) ||
         ...
         */
         (rli->curr_group_seen_begin && rli->curr_group_seen_gtid && ends_group()) || is_mts_db_partitioned(rli) ||
  ...
  worker = (Relay_log_info *)(rli->last_assigned_worker = get_slave_worker(rli));

#ifndef NDEBUG
  if (rli->last_assigned_worker)
    DBUG_PRINT("mts", ("Assigning job to worker %lu", rli->last_assigned_worker->id));
#endif
...
bool Query_log_event::write(IO_CACHE *file)
{
  ...
  */
  ...
  {
    write_str_with_code_and_len(&start, catalog, catalog_len, Q_CATALOG_NZ_CODE);
    /*
  ...
  if (time_zone_len)
  {
    ...
    write_str_with_code_and_len(&start, time_zone_str, time_zone_len, Q_TIME_ZONE_CODE);
  }
  ...
  if (thd && thd->get_binlog_accessed_db_names() != NULL)
  {
    ...
    */
    dbs = (thd->get_binlog_accessed_db_names()->elements <= MAX_DBS_IN_EVENT_MTS)
  ...
  */
  ...
  return (write_header(file, event_length) ||
          ...
          wrapper_my_b_safe_write(file, start_of_status, (uint)(start - start_of_status)) ||
          wrapper_my_b_safe_write(file, db ? (uchar *)db : (uchar *)"", db_len + 1) ||
          wrapper_my_b_safe_write(file, (uchar *)query, q_len) || write_footer(file))
             ? 1
             : 0;
}
...
Query_log_event::Query_log_event(THD *thd_arg, const char *query_arg, size_t query_length, bool using_trans,
                                 bool immediate, bool suppress_use, int errcode, bool ignore_cmd_internals)

    : binary_log::Query_event(query_arg, thd_arg->catalog().str, thd_arg->db().str, query_length, thd_arg->thread_id(),
                              thd_arg->variables.sql_mode, thd_arg->variables.auto_increment_increment,
                              thd_arg->variables.auto_increment_offset, thd_arg->variables.lc_time_names->number,
      ...
      Log_event(thd_arg,
                ...
                using_trans ? Log_event::EVENT_TRANSACTIONAL_CACHE : Log_event::EVENT_STMT_CACHE,
                Log_event::EVENT_NORMAL_LOGGING, header(), footer()),
      data_buf(0)
{
  DBUG_EXECUTE_IF("debug_lock_before_query_log_event", DBUG_SYNC_POINT("debug_lock.before_query_log_event", 10););
  ...
  */
  ...
  if (!immediate)
  {
    switch (lex->sql_command)
    {
      case SQLCOM_DROP_TABLE:
        cmd_can_generate_row_events = lex->drop_temporary && thd->in_multi_stmt_transaction_mode();
        break;
      case SQLCOM_CREATE_TABLE:
        cmd_must_go_to_trx_cache = lex->select_lex->item_list.elements && thd->is_current_stmt_binlog_format_row();
        cmd_can_generate_row_events =
            ((lex->create_info.options & HA_LEX_CREATE_TMP_TABLE) && thd->in_multi_stmt_transaction_mode()) ||
      ...
      case SQLCOM_XA_PREPARE:
        ...
        break;
      default:
        cmd_can_generate_row_events = sqlcom_can_generate_row_events(thd->lex->sql_command);
        break;
    }
  }
  ...
  DBUG_PRINT("info", ("Query_log_event has flags2: %lu  sql_mode: %llu", (ulong)flags2, (ulonglong)sql_mode));
}
#endif /* MYSQL_CLIENT */
...
*/
Query_log_event::Query_log_event(const char *buf, uint event_len, const Format_description_event *description_event,
                                 Log_event_type event_type)
    : binary_log::Query_event(buf, event_len, description_event, event_type), Log_event(header(), footer())
{
  DBUG_ENTER("Query_log_event::Query_log_event(char*,...)");
  slave_proxy_id = thread_id;
  exec_time = query_exec_time;

  ulong buf_len = catalog_len + 1 + time_zone_len + 1 + user_len + 1 + host_len + 1 + data_len + 1;
#if !defined(MYSQL_CLIENT)
  buf_len += sizeof(size_t) /*for db_len */ + db_len + 1 + QUERY_CACHE_FLAGS_SIZE;
#endif

  if (!(data_buf = (Log_event_header::Byte *)my_malloc(key_memory_log_event, buf_len, MYF(MY_WME))))
    DBUG_VOID_RETURN;
  /*
...
/**
  ...
  @todo
    print the catalog ??
*/
void Query_log_event::print_query_header(IO_CACHE *file, PRINT_EVENT_INFO *print_event_info)
{
  ...
  end = int10_to_str((long)common_header->when.tv_sec, my_stpcpy(buff, "SET TIMESTAMP="), 10);
  ...
  *end++ = '\n';
  ...
  if ((!print_event_info->thread_id_printed ||
       ((common_header->flags & LOG_EVENT_THREAD_SPECIFIC_F) && thread_id != print_event_info->thread_id)))
  {
  ...
  */
      ...
      (unlikely(!print_event_info->charset_inited || memcmp(print_event_info->charset, charset, 6))))
  {
    ...
    my_b_printf(file,
                ...
                "%s\n",
                uint2korr(charset_p), uint2korr(charset + 2), uint2korr(charset + 4), print_event_info->delimiter);
  ...
  if (explicit_defaults_ts != TERNARY_UNSET)
    my_b_printf(file, "SET @@session.explicit_defaults_for_timestamp=%d%s\n",
                explicit_defaults_ts == TERNARY_OFF ? 0 : 1, print_event_info->delimiter);
}
...
void Query_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
   ...
   */
  DBUG_EXECUTE_IF("simulate_file_write_error", { head->write_pos = head->write_end - 500; });
...
*/
void Query_log_event::attach_temp_tables_worker(THD *thd_arg, const Relay_log_info *rli)
{
...
*/
void Query_log_event::detach_temp_tables_worker(THD *thd_arg, const Relay_log_info *rli)
{
...
static bool is_silent_error(THD *thd)
{
  ...
  while ((err = it++))
  {
    DBUG_PRINT("info", ("has condition %d %s", err->mysql_errno(), err->message_text()));
...
int Query_log_event::do_apply_event(Relay_log_info const *rli, const char *query_arg, size_t q_len_arg)
{
  ...
  DBUG_PRINT("info", ("query=%s, q_len_arg=%lu", query, static_cast<unsigned long>(q_len_arg)));
  ...
  */
  ...
  size_t valid_len;
  bool len_error;
  bool is_invalid_db_name = validate_string(system_charset_info, db, db_len, &valid_len, &len_error);

  DBUG_PRINT("debug", ("is_invalid_db_name= %s, valid_len=%zu, len_error=%s", is_invalid_db_name ? "true" : "false",
                       valid_len, len_error ? "true" : "false"));
  ...
  if (explicit_defaults_ts != TERNARY_UNSET)
    thd->variables.explicit_defaults_for_timestamp = explicit_defaults_ts == TERNARY_OFF ? 0 : 1;
  ...
  */
...

  {
    ...
    if (ignored_error_code((expected_error = error_code)) || !unexpected_error_code(expected_error))
    {
      if (flags2_inited)
        /*
      ...
      */
      ...
      if (charset_inited)
      {
        if (rli->cached_charset_compare(charset))
        {
          ...
          if (!(thd->variables.character_set_client = get_charset(uint2korr(charset_p), MYF(MY_WME))) ||
      ...
      */
      if (expected_error && (ignored_error_code(expected_error) || concurrency_error_code(expected_error)))
      {
      ...
      if (!parser_state.init(thd, thd->query().str, thd->query().length))
      {
        ...
        */
        if (is_sbr_logging_format() && thd->variables.tx_isolation > ISO_READ_COMMITTED &&
            thd->tx_isolation == ISO_READ_COMMITTED)
        {
          ...
          message.append("'");
          rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, ER_THD(thd, ER_SLAVE_FATAL_ERROR), message.c_ptr());
          thd->is_slave_error = true;
          goto end;
        }
    ...
    else
    {
      ...
      */
      ...
      {
        rli->report(ERROR_LEVEL, ER_ERROR_ON_MASTER, ER(ER_ERROR_ON_MASTER), expected_error, thd->query().str);
    ...
    if (!thd->is_error() || thd->get_stmt_da()->mysql_errno() != ER_SLAVE_IGNORED_TABLE)
    {
      /*
      ...
      */
      if (opt_general_log_raw || thd->rewritten_query().length() == 0)
        ...
        query_logger.general_log_write(thd, COM_QUERY, thd->rewritten_query().ptr(), thd->rewritten_query().length());
    }

  compare_errors:
    /* Parser errors shall be ignored when (GTID) skipping statements */
    if (thd->is_error() && thd->get_stmt_da()->mysql_errno() == ER_PARSE_ERROR &&
    ...
    */
    if (thd->lex->sql_command == SQLCOM_DROP_TABLE && thd->lex->drop_temporary && thd->is_error() &&
        thd->get_stmt_da()->mysql_errno() == ER_BAD_TABLE_ERROR && !expected_error)
      thd->get_stmt_da()->reset_diagnostics_area();
    /*
    ...
    */
    if ((expected_error && rpl_filter->db_ok(thd->db().str) && expected_error != actual_error &&
         !concurrency_error_code(expected_error)) &&
        !ignored_error_code(actual_error) && !ignored_error_code(expected_error))
    {
      if (!ignored_error_code(ER_INCONSISTENT_ERROR))
      {
        rli->report(ERROR_LEVEL, ER_INCONSISTENT_ERROR, ER(ER_INCONSISTENT_ERROR), ER_THD(thd, expected_error),
                    expected_error, (actual_error ? thd->get_stmt_da()->message_text() : "no error"), actual_error,
      ...
      else
      {
        rli->report(INFORMATION_LEVEL, actual_error,
                    ...
                    ER(expected_error), expected_error, thd->get_stmt_da()->message_text());
        clear_all_errors(thd, const_cast<Relay_log_info *>(rli));
      }
    }
    /*
    ...
    */
    else if ((expected_error == actual_error && !concurrency_error_code(expected_error)) ||
        ...
        if (actual_error == ER_SLAVE_IGNORED_TABLE)
        {
          if (!slave_ignored_err_throttle.log())
            rli->report(INFORMATION_LEVEL, actual_error,
                        ...
                        get_type_str(), thd->get_stmt_da()->message_text(), (window_size / 1000000));
        }
        else
          rli->report(INFORMATION_LEVEL, actual_error, "Could not execute %s event. Detailed error: %s;",
    ...
    */
    else if (thd->is_slave_error || thd->is_fatal_error)
    {
      ...
      {
        rli->report(ERROR_LEVEL, actual_error, "Error '%s' on query. Default database: '%s'. Query: '%s'",
                    (actual_error ? thd->get_stmt_da()->message_text() : "unexpected success or fatal error"),
...

  {
    ...
    */
    ...
    DBUG_EXECUTE_IF(
        "stop_slave_middle_group", if (strcmp("COMMIT", query) != 0 && strcmp("BEGIN", query) != 0) {
          if (thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::SESSION))
...
*/
size_t Query_log_event::get_query(const char *buf, size_t length, const Format_description_log_event *fd_event,
                                  char **query)
{
  ...
  /* Error if the event content is too small */
  if (length < (common_header_len + query_header_len + db_len + 1 + status_vars_len + checksum_size))
    goto err;
...
Start_log_event_v3::Start_log_event_v3()
    : binary_log::Start_event_v3(),
      Log_event(header(), footer(), Log_event::EVENT_INVALID_CACHE, Log_event::EVENT_INVALID_LOGGING)
{
  is_valid_param = true;
}
#endif
...
Start_log_event_v3::Start_log_event_v3(const char *buf, uint event_len,
    ...
    : binary_log::Start_event_v3(buf, event_len, description_event), Log_event(header(), footer())
{
  is_valid_param = server_version[0] != 0;
  if (event_len < (uint)description_event->common_header_len + ST_COMMON_HEADER_LEN_OFFSET)
  {
    server_version[0] = 0;
    return;
  }
...
bool Start_log_event_v3::write(IO_CACHE *file)
{
  ...
  return (write_header(file, sizeof(buff)) || wrapper_my_b_safe_write(file, (uchar *)buff, sizeof(buff)) ||
          write_footer(file));
}
#endif
...
int Start_log_event_v3::do_apply_event(Relay_log_info const *rli)
{
  ...
  switch (binlog_version)
  {
    ...
    case 4:
      ...
      */
    case 1:
      if (strncmp(rli->get_rli_description_event()->server_version, "3.23.57", 7) >= 0 && created)
      {
        /*
...
Format_description_log_event::Format_description_log_event(uint8_t binlog_ver, const char *server_ver)
    : binary_log::Start_event_v3(binary_log::FORMAT_DESCRIPTION_EVENT),
      Format_description_event(binlog_ver, (binlog_ver <= 3 || server_ver != 0) ? server_ver : ::server_version)
{
...
bool Format_description_log_event::write(IO_CACHE *file)
{
  ...
  */
  ...
  memcpy((char *)buff + ST_COMMON_HEADER_LEN_OFFSET + 1, &post_header_len.front(), number_of_events);
  /*
  ...
  */
  ...
  buff[Binary_log_event::FORMAT_DESCRIPTION_HEADER_LEN] =
      need_checksum() ? (uint8)common_footer->checksum_alg : (uint8)binary_log::BINLOG_CHECKSUM_ALG_OFF;
  /*
  ...
  */
  ...
  if ((no_checksum = (common_footer->checksum_alg == binary_log::BINLOG_CHECKSUM_ALG_OFF)))
  {
    // Forcing (V) room to fill anyway
    common_footer->checksum_alg = binary_log::BINLOG_CHECKSUM_ALG_CRC32;
  }
  ret = (write_header(file, rec_size) || wrapper_my_b_safe_write(file, buff, rec_size) || write_footer(file));
  if (no_checksum)
    common_footer->checksum_alg = binary_log::BINLOG_CHECKSUM_ALG_OFF;
  return ret;
}
...
uint Load_log_event::get_query_buffer_length()
{
  return
      ...
      5 + 2 * db_len + 3 + 18 + fname_len * 4 + 2 +  // "LOAD DATA INFILE 'file''"
      11 +                                           // "CONCURRENT "
      7 +                                            // LOCAL
      9 +                                            // " REPLACE or IGNORE "
...
void Load_log_event::print_query(bool need_db, const char *cs, char *buf, char **end, char **fn_start, char **fn_end)
{
...
#ifdef MYSQL_SERVER
    quoted_id_len = my_strmov_quoted_identifier(this->thd, (char *)quoted_id, db, 0);
#else
  ...
  pos = pretty_print_str(pos, sql_ex.data_info.field_term, sql_ex.data_info.field_term_len);
  if (sql_ex.data_info.opt_flags & OPT_ENCLOSED_FLAG)
    pos = my_stpcpy(pos, " OPTIONALLY ");
  pos = my_stpcpy(pos, " ENCLOSED BY ");
  pos = pretty_print_str(pos, sql_ex.data_info.enclosed, sql_ex.data_info.enclosed_len);

  pos = my_stpcpy(pos, " ESCAPED BY ");
  pos = pretty_print_str(pos, sql_ex.data_info.escaped, sql_ex.data_info.escaped_len);

  pos = my_stpcpy(pos, " LINES TERMINATED BY ");
  pos = pretty_print_str(pos, sql_ex.data_info.line_term, sql_ex.data_info.line_term_len);
  if (sql_ex.data_info.line_start_len)
  {
    pos = my_stpcpy(pos, " STARTING BY ");
    pos = pretty_print_str(pos, sql_ex.data_info.line_start, sql_ex.data_info.line_start_len);
  }
  ...
  if (num_fields)
  {
    ...
    for (i = 0; i < num_fields; i++)
    {
      ...
      quoted_id_len = my_strmov_quoted_identifier(this->thd, quoted_id, field, 0);
...
int Load_log_event::pack_info(Protocol *protocol)
{
  ...
  if (!(buf = (char *)my_malloc(key_memory_log_event, get_query_buffer_length(), MYF(MY_WME))))
    return 1;
...
bool Load_log_event::write_data_body(IO_CACHE *file)
{
  ...
  return (my_b_safe_write(file, (uchar *)table_name, table_name_len + 1) ||
          my_b_safe_write(file, (uchar *)db, db_len + 1) || my_b_safe_write(file, (uchar *)fname, fname_len));
}
...
Load_log_event::Load_log_event(THD *thd_arg, sql_exchange *ex, const char *db_arg, const char *table_name_arg,
    ...
    : binary_log::Load_event(),
      Log_event(thd_arg, thd_arg->thread_specific_used ? LOG_EVENT_THREAD_SPECIFIC_F : 0,
                using_trans ? Log_event::EVENT_TRANSACTIONAL_CACHE : Log_event::EVENT_STMT_CACHE,
                Log_event::EVENT_NORMAL_LOGGING, header(), footer())
{
...
/**
  @note
    ...
    constructed event.
*/
Load_log_event::Load_log_event(const char *buf, uint event_len, const Format_description_event *description_event)
    : binary_log::Load_event(buf, event_len, description_event), Log_event(header(), footer())
{
...
void Load_log_event::print(FILE *file_arg, PRINT_EVENT_INFO *print_event_info, bool commented)
{
  ...
  pretty_print_str(head, sql_ex.data_info.field_term, sql_ex.data_info.field_term_len);

  if (sql_ex.data_info.opt_flags & OPT_ENCLOSED_FLAG)
    my_b_printf(head, " OPTIONALLY ");
  my_b_printf(head, " ENCLOSED BY ");
  pretty_print_str(head, sql_ex.data_info.enclosed, sql_ex.data_info.enclosed_len);

  my_b_printf(head, " ESCAPED BY ");
  pretty_print_str(head, sql_ex.data_info.escaped, sql_ex.data_info.escaped_len);

  my_b_printf(head, " LINES TERMINATED BY ");
  pretty_print_str(head, sql_ex.data_info.line_term, sql_ex.data_info.line_term_len);

  if (sql_ex.data_info.line_start)
  {
    my_b_printf(head, " STARTING BY ");
    pretty_print_str(head, sql_ex.data_info.line_start, sql_ex.data_info.line_start_len);
  }
  if ((long)skip_lines > 0)
...
int Load_log_event::do_apply_event(NET *net, Relay_log_info const *rli, bool use_rli_only_for_errors)
{
 ...
 */
  if (rpl_filter->db_ok(thd->db().str))
  {
    ...
    tables.init_one_table(thd->strmake(thd->db().str, thd->db().length), thd->db().length, table_buf, strlen(table_buf),
                          table_buf, TL_WRITE);
    tables.updating = 1;
    ...
    else
    {
      ...
      */
      ...
      String field_term(sql_ex.data_info.field_term, sql_ex.data_info.field_term_len, log_cs);
      String enclosed(sql_ex.data_info.enclosed, sql_ex.data_info.enclosed_len, log_cs);
      String line_term(sql_ex.data_info.line_term, sql_ex.data_info.line_term_len, log_cs);
      String line_start(sql_ex.data_info.line_start, sql_ex.data_info.line_start_len, log_cs);
      String escaped(sql_ex.data_info.escaped, sql_ex.data_info.escaped_len, log_cs);
      ...
      */
      ...
      if (open_temporary_tables(thd, &tables) ||
          mysql_load(thd, &ex, &tables, field_list, tmp_list, tmp_list, handle_dup, net != 0))
        thd->is_slave_error = 1;
      if (thd->cuted_fields)
      {
        ...
        sql_print_warning(
            ...
            const_cast<Relay_log_info *>(rli)->get_rpl_log_name(), (ulong)thd->cuted_fields,
...
Error '%s' running LOAD DATA INFILE on table '%s'. Default database: '%s'",
  ...
  if (thd->is_fatal_error)
  {
    ...
    rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, ER(ER_SLAVE_FATAL_ERROR), buf);
    return 1;
  }
...
#ifndef MYSQL_CLIENT
Rotate_log_event::Rotate_log_event(const char *new_log_ident_arg, size_t ident_len_arg, ulonglong pos_arg,
                                   uint flags_arg)
      ...
      Log_event(header(), footer(), Log_event::EVENT_NO_CACHE, Log_event::EVENT_IMMEDIATE_LOGGING)
{
  ...
  char buff[22];
  DBUG_PRINT("enter",
             ("new_log_ident: %s  pos: %s  flags: %lu", new_log_ident_arg, llstr(pos_arg, buff), (ulong)flags));
#endif
  if (flags & DUP_NAME)
    new_log_ident = my_strndup(key_memory_log_event, new_log_ident_arg, ident_len, MYF(MY_WME));
  if (new_log_ident != 0)
    is_valid_param = true;
  if (flags & RELAY_LOG)
    set_relay_log_event();
...
Rotate_log_event::Rotate_log_event(const char *buf, uint event_len, const Format_description_event *description_event)
    : binary_log::Rotate_event(buf, event_len, description_event), Log_event(header(), footer())
{
...
bool Rotate_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, Binary_log_event::ROTATE_HEADER_LEN + ident_len) ||
          ...
          wrapper_my_b_safe_write(file, (uchar *)new_log_ident, (uint)ident_len) || write_footer(file));
}
#endif
...
int Rotate_log_event::do_update_pos(Relay_log_info *rli)
{
  ...
  DBUG_PRINT("info", ("server_id=%lu; ::server_id=%lu", (ulong)this->server_id, (ulong)::server_id));
  ...
  */
  bool in_group =
      rli->is_parallel_exec() ? (rli->mts_group_status == Relay_log_info::MTS_IN_GROUP) : rli->is_in_group();

  if ((server_id != ::server_id || rli->replicate_same_server_id) && !is_relay_log_event() && !in_group)
  {
    if (!is_mts_db_partitioned(rli) && server_id != ::server_id)
    {
      // force the coordinator to start a new binlog segment.
      static_cast<Mts_submode_logical_clock *>(rli->current_mts_submode)->start_new_group();
    }
    if (rli->is_parallel_exec())
    {
      ...
      */
      if ((error = mts_checkpoint_routine(rli, 0, false, true /*need_data_lock=true*/)))
        goto err;
    }
    ...
    memcpy((void *)rli->get_group_master_log_name(), new_log_ident, ident_len + 1);
    rli->notify_group_master_log_name_update();
    if ((error = rli->inc_group_relay_log_pos(pos, false /*need_data_lock=false*/)))
    {
    ...
    if (rli->is_parallel_exec())
    {
      ...
      rli->reset_notified_checkpoint(0, real_event ? common_header->when.tv_sec + (time_t)exec_time : 0,
...
*/
Intvar_log_event::Intvar_log_event(const char *buf, const Format_description_event *description_event)
    : binary_log::Intvar_event(buf, description_event), Log_event(header(), footer())
{
  is_valid_param = true;
}
...
bool Intvar_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, sizeof(buf)) || wrapper_my_b_safe_write(file, buf, sizeof(buf)) || write_footer(file));
}
#endif
...
Rand_log_event::Rand_log_event(const char *buf, const Format_description_event *description_event)
    : binary_log::Rand_event(buf, description_event), Log_event(header(), footer())
{
  is_valid_param = true;
}
...
Xid_log_event::Xid_log_event(const char *buf, const Format_description_event *description_event)
    : binary_log::Xid_event(buf, description_event), Xid_apply_log_event(buf, description_event, header(), footer())
{
  is_valid_param = true;
}
...
bool Xid_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, sizeof(xid)) || wrapper_my_b_safe_write(file, (uchar *)&xid, sizeof(xid)) ||
          write_footer(file));
}
#endif
...
{
  DBUG_EXECUTE_IF("dbug.reached_commit", { DBUG_SET("+d,dbug.enabled_commit"); });
  bool error = trans_commit(thd_arg); /* Automatically rolls back on error. */
  DBUG_EXECUTE_IF("crash_after_apply", sql_print_information("Crashing crash_after_apply."); DBUG_SUICIDE(););
...
int Xid_apply_log_event::do_apply_event_worker(Slave_worker *w)
{
  ...
  query_logger.general_log_print(thd, COM_QUERY, "COMMIT /* implicit, from Xid_log_event */");

  DBUG_PRINT("mts", ("do_apply group master %s %llu  group relay %s %llu event %s %llu.",
                     w->get_group_master_log_name(), w->get_group_master_log_pos(), w->get_group_relay_log_name(),
                     w->get_group_relay_log_pos(), w->get_event_relay_log_name(), w->get_event_relay_log_pos()));

  DBUG_EXECUTE_IF("crash_before_update_pos", sql_print_information("Crashing crash_before_update_pos.");
                  DBUG_SUICIDE(););

  ulong gaq_idx = mts_group_idx;
  Slave_job_group *ptr_group = coordinator_gaq->get_job_group(gaq_idx);

  if (!thd->get_transaction()->xid_state()->check_in_xa(false) && w->is_transactional())
  {
    /*
...
int Xid_apply_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  */
   ...
   */
  strmake(saved_group_master_log_name, rli_ptr->get_group_master_log_name(), FN_REFLEN - 1);
  saved_group_master_log_pos = rli_ptr->get_group_master_log_pos();
  strmake(saved_group_relay_log_name, rli_ptr->get_group_relay_log_name(), FN_REFLEN - 1);
  ...
  DBUG_PRINT("info", ("do_apply group master %s %llu  group relay %s %llu event %s %llu\n",
                      rli_ptr->get_group_master_log_name(), rli_ptr->get_group_master_log_pos(),
                      rli_ptr->get_group_relay_log_name(), rli_ptr->get_group_relay_log_pos(),
                      rli_ptr->get_event_relay_log_name(), rli_ptr->get_event_relay_log_pos()));
  ...
  */
   ...
   */
  if (!thd->get_transaction()->xid_state()->check_in_xa(false) && rli_ptr->is_transactional())
  {
  ...
  /**
    ...
    error.
   */
  DBUG_EXECUTE_IF("simulate_commit_failure", { thd->get_transaction()->xid_state()->set_state(XID_STATE::XA_IDLE); });
   ...
   */
  strmake(new_group_master_log_name, rli_ptr->get_group_master_log_name(), FN_REFLEN - 1);
  new_group_master_log_pos = rli_ptr->get_group_master_log_pos();
  strmake(new_group_relay_log_name, rli_ptr->get_group_relay_log_name(), FN_REFLEN - 1);
  ...
  if (error)
  {
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(),
                "Error in Xid_log_event: Commit could not be completed, '%s'", thd->get_stmt_da()->message_text());
  }
  else
  {
...
Log_event::enum_skip_reason Xid_apply_log_event::do_shall_skip(Relay_log_info *rli)
{
...
int XA_prepare_log_event::pack_info(Protocol *protocol)
{
  ...
  serialize_xid(buf, my_xid.formatID, my_xid.gtrid_length, my_xid.bqual_length, my_xid.data);
...
bool XA_prepare_log_event::write(IO_CACHE *file)
{
  ...
  return write_header(file, sizeof(one_byte) + xid_bufs_size + static_cast<XID *>(xid)->get_gtrid_length() +
                                static_cast<XID *>(xid)->get_bqual_length()) ||
         wrapper_my_b_safe_write(file, &one_byte, sizeof(one_byte)) ||
         wrapper_my_b_safe_write(file, buf_f, sizeof(buf_f)) || wrapper_my_b_safe_write(file, buf_g, sizeof(buf_g)) ||
         wrapper_my_b_safe_write(file, buf_b, sizeof(buf_b)) ||
         wrapper_my_b_safe_write(
             file, (uchar *)static_cast<XID *>(xid)->get_data(),
             static_cast<XID *>(xid)->get_gtrid_length() + static_cast<XID *>(xid)->get_bqual_length()) ||
         write_footer(file);
}
#endif
...
bool XA_prepare_log_event::do_commit(THD *thd)
{
  ...
  if (state == GTID_STATEMENT_CANCEL)
  {
    ...
    assert(error != 0);
    thd->rli_slave->report(ERROR_LEVEL, error, "Error executing XA PREPARE event: '%s'",
  ...
  xid.set(my_xid.formatID, my_xid.data, my_xid.gtrid_length, my_xid.data + my_xid.gtrid_length, my_xid.bqual_length);
  if (!one_phase)
  {
    /*
...
int User_var_log_event::pack_info(Protocol *protocol)
{
  ...
  if (is_null)
  {
    if (!(buf = (char *)my_malloc(key_memory_log_event, val_offset + 5, MYF(MY_WME))))
      return 1;
  ...
  else
  {
    switch (type)
    {
      case REAL_TYPE:
        ...
        if (!(buf = (char *)my_malloc(key_memory_log_event, val_offset + MY_GCVT_MAX_FIELD_WIDTH + 1, MYF(MY_WME))))
          return 1;
        event_len += my_gcvt(real_val, MY_GCVT_ARG_DOUBLE, MY_GCVT_MAX_FIELD_WIDTH, buf + val_offset, NULL);
        break;
      case INT_TYPE:
        if (!(buf = (char *)my_malloc(key_memory_log_event, val_offset + 22, MYF(MY_WME))))
          return 1;
        event_len =
            longlong10_to_str(uint8korr(val), buf + val_offset, ((flags & User_var_log_event::UNSIGNED_F) ? 10 : -10)) -
            buf;
        break;
      case DECIMAL_TYPE:
      {
        if (!(buf = (char *)my_malloc(key_memory_log_event, val_offset + DECIMAL_MAX_STR_LENGTH + 1, MYF(MY_WME))))
          return 1;
        String str(buf + val_offset, DECIMAL_MAX_STR_LENGTH + 1, &my_charset_bin);
        my_decimal dec;
        binary2my_decimal(E_DEC_FATAL_ERROR, (uchar *)(val + 2), &dec, val[0], val[1]);
      ...
      case STRING_TYPE:
        ...
        buf = (char *)my_malloc(key_memory_log_event, event_len + val_len * 2 + 1 + 2 * MY_CS_NAME_SIZE + 15,
                                MYF(MY_WME));
        CHARSET_INFO *cs;
        if (!buf)
          return 1;
...
User_var_log_event::User_var_log_event(const char *buf, uint event_len,
                                       const Format_description_event *description_event)
...
bool User_var_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, event_length) || wrapper_my_b_safe_write(file, (uchar *)buf, sizeof(buf)) ||
          wrapper_my_b_safe_write(file, (uchar *)name, name_len) ||
          wrapper_my_b_safe_write(file, (uchar *)buf1, buf1_length) || wrapper_my_b_safe_write(file, pos, val_len) ||
          wrapper_my_b_safe_write(file, &flags, unsigned_len) || write_footer(file));
}
#endif
...
void User_var_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  ...
  name_id[name_len] = '\0';
  my_b_printf(head, "SET @");
  quoted_len = my_strmov_quoted_identifier((char *)quoted_id, (const char *)name_id);
  quoted_id[quoted_len] = '\0';
    ...
    switch (type)
    {
      ...
      case STRING_TYPE:
      {
        ...
        */
        ...
        hex_str = (char *)my_malloc(key_memory_log_event, 2 * val_len + 1 + 2, MYF(MY_WME));  // 2 hex digits / byte
        if (!hex_str)
          return;
...
int User_var_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  Item_func_set_user_var *e = new Item_func_set_user_var(Name_string(name, name_len, false), it, false);
  /*
...
bool Create_file_log_event::write_data_body(IO_CACHE *file)
{
  ...
  if ((res = Load_log_event::write_data_body(file)) || fake_base)
    return res;
  return (my_b_safe_write(file, (uchar *)"", 1) || my_b_safe_write(file, block, block_len));
}
...
#ifdef MYSQL_CLIENT
void Create_file_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info, bool enable_local)
{
  if (print_event_info->short_form)
  {
  ...
  if (enable_local)
  {
    Load_log_event::print(file, print_event_info, !check_fname_outside_temp_buf());
    /**
      reduce the size of io cache so that the write function is called
      for every call to my_b_printf().
     */
    DBUG_EXECUTE_IF("simulate_create_event_write_error", {
      (&print_event_info->head_cache)->write_pos = (&print_event_info->head_cache)->write_end;
      DBUG_SET("+d,simulate_file_write_error");
    });
    /*
...
int Create_file_log_event::do_apply_event(Relay_log_info const *rli)
{
   ...
   */
  DBUG_EXECUTE_IF("simulate_file_create_error_create_log_event", { strcat(fname_buf, "/"); });
  ...
  {
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(),
                "Error in Create_file event: could not open file '%s', '%s'", fname_buf,
                thd->get_stmt_da()->message_text());
    goto err;
  }
  ...
  if (write_base(&file))
  {
    ...
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(),
                "Error in Create_file event: could not write to file '%s', '%s'", fname_buf,
                thd->get_stmt_da()->message_text());
    goto err;
  }
  end_io_cache(&file);
  ...
  DBUG_EXECUTE_IF("simulate_file_create_error_create_log_event_2", { strcat(fname_buf, "/"); });
   ...
   */
  DBUG_EXECUTE_IF("simulate_file_write_error_create_log_event", { mysql_file_close(fd, MYF(0)); });
  if (mysql_file_write(fd, block, block_len, MYF(MY_WME + MY_NABP)))
  {
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(),
                "Error in Create_file event: write to '%s' failed, '%s'", fname_buf,
                thd->get_stmt_da()->message_text());
    goto err;
  }
...
Append_block_log_event::Append_block_log_event(THD *thd_arg, const char *db_arg, uchar *block_arg, uint block_len_arg,
    ...
    : binary_log::Append_block_event(db_arg, block_arg, block_len_arg, thd_arg->file_id),
      Log_event(thd_arg, 0, using_trans ? Log_event::EVENT_TRANSACTIONAL_CACHE : Log_event::EVENT_STMT_CACHE,
                Log_event::EVENT_NORMAL_LOGGING, header(), footer())
{
  if (block != 0)
    is_valid_param = true;
...
Append_block_log_event::Append_block_log_event(const char *buf, uint len,
                                               const Format_description_event *description_event)
    : binary_log::Append_block_event(buf, len, description_event), Log_event(header(), footer())
{
...
bool Append_block_log_event::write(IO_CACHE *file)
{
  ...
  return (write_header(file, Binary_log_event::APPEND_BLOCK_HEADER_LEN + block_len) ||
          ...
          wrapper_my_b_safe_write(file, block, block_len) || write_footer(file));
}
#endif
...
#ifdef MYSQL_CLIENT
void Append_block_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return;
  print_header(&print_event_info->head_cache, print_event_info, FALSE);
...
int Append_block_log_event::pack_info(Protocol *protocol)
{
  char buf[256];
  size_t length;
  length = my_snprintf(buf, sizeof(buf), ";file_id=%u;block_len=%u", file_id, block_len);
...
int Append_block_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  if (get_create_or_append())
  {
    ...
    */
    ...
    DBUG_EXECUTE_IF("simulate_file_create_error_Append_block_event", { strcat(fname, "/"); });
    if ((fd = mysql_file_create(key_file_log_event_data, fname, CREATE_MODE, O_WRONLY | O_BINARY | O_EXCL | O_NOFOLLOW,
                                MYF(MY_WME))) < 0)
    {
      rli->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(), "Error in %s event: could not create file '%s', '%s'",
      ...
      goto err;
                                 ...
                                 MYF(MY_WME))) < 0)
  {
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(), "Error in %s event: could not open file '%s', '%s'",
                get_type_str(), fname, thd->get_stmt_da()->message_text());
    goto err;
  }
  DBUG_EXECUTE_IF("remove_slave_load_file_before_write", { my_delete_allow_opened(fname, MYF(0)); });

  DBUG_EXECUTE_IF("simulate_file_write_error_Append_block_event", { mysql_file_close(fd, MYF(0)); });
  if (mysql_file_write(fd, block, block_len, MYF(MY_WME + MY_NABP)))
  {
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(), "Error in %s event: write to '%s' failed, '%s'",
                get_type_str(), fname, thd->get_stmt_da()->message_text());
    goto err;
  }
  error = 0;
...
Delete_file_log_event::Delete_file_log_event(const char *buf, uint len,
                                             const Format_description_event *description_event)
    : binary_log::Delete_file_event(buf, len, description_event), Log_event(header(), footer())
{
  if (file_id != 0)
    is_valid_param = true;
}
...
#ifdef MYSQL_CLIENT
void Delete_file_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return;
  print_header(&print_event_info->head_cache, print_event_info, FALSE);
...
Execute_load_log_event::Execute_load_log_event(const char *buf, uint len,
                                               const Format_description_event *description_event)
    : binary_log::Execute_load_event(buf, len, description_event), Log_event(header(), footer())
{
  if (file_id != 0)
    is_valid_param = true;
}
...
#ifdef MYSQL_CLIENT
void Execute_load_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return;
  print_header(&print_event_info->head_cache, print_event_info, FALSE);
...
int Execute_load_log_event::do_apply_event(Relay_log_info const *rli)
{
   ...
   */
  DBUG_EXECUTE_IF("simulate_file_open_error_exec_event", { strcat(fname, "/"); });
  ...
  {
    rli->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(), "Error in Exec_load event: could not open file, '%s'",
                thd->get_stmt_da()->message_text());
    goto err;
  }
  if (!(lev = (Load_log_event *)Log_event::read_log_event(&file, (mysql_mutex_t *)0, rli->get_rli_description_event(),
  ...
  */
  if (lev->do_apply_event(0, rli, 1))
  {
    ...
    */
    char *tmp = my_strdup(key_memory_log_event, rli->last_error().message, MYF(MY_WME));
    if (tmp)
    {
      rli->report(ERROR_LEVEL, rli->last_error().number, "%s. Failed executing load from '%s'", tmp, fname);
      my_free(tmp);
    }
    goto err;
  }
...
err:
  DBUG_EXECUTE_IF("simulate_file_open_error_exec_event", { my_stpcpy(ext, ".info"); });
  mysql_file_delete(key_file_log_event_info, fname, MYF(MY_WME));
  my_stpcpy(ext, ".data");
  mysql_file_delete(key_file_log_event_data, fname, MYF(MY_WME));
  delete lev;
...
Execute_load_query_log_event::Execute_load_query_log_event(THD *thd_arg, const char *query_arg, ulong query_length_arg,
                                                           ...
                                                           int errcode)
    : binary_log::Query_event(query_arg, thd_arg->catalog().str, thd_arg->db().str, query_length_arg,
                              thd_arg->thread_id(), thd_arg->variables.sql_mode,
                              thd_arg->variables.auto_increment_increment, thd_arg->variables.auto_increment_offset,
                              thd_arg->variables.lc_time_names->number, (ulonglong)thd_arg->table_map_for_update,
                              errcode, thd_arg->db().str ? strlen(thd_arg->db().str) : 0,
                              thd_arg->catalog().str ? strlen(thd_arg->catalog().str) : 0),
      Query_log_event(thd_arg, query_arg, query_length_arg, using_trans, immediate, suppress_use, errcode),
      binary_log::Execute_load_query_event(thd_arg->file_id, fn_pos_start_arg, fn_pos_end_arg, dup_handling_arg)
{
...
Execute_load_query_log_event::Execute_load_query_log_event(const char *buf, uint event_len,
                                                           const Format_description_event *desc_event)
    : binary_log::Query_event(buf, event_len, desc_event, binary_log::EXECUTE_LOAD_QUERY_EVENT),
      Query_log_event(buf, event_len, desc_event, binary_log::EXECUTE_LOAD_QUERY_EVENT),
...
#ifdef MYSQL_CLIENT
void Execute_load_query_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
...
*/
void Execute_load_query_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info, const char *local_fname)
{
  IO_CACHE *const head = &print_event_info->head_cache;
...
{
  char *buf, *pos;
  if (!(buf = (char *)my_malloc(key_memory_log_event, 9 + (db_len * 2) + 2 + q_len + 10 + 21, MYF(MY_WME))))
    return 1;
  pos = buf;
  if (db && db_len)
  {
...
int Execute_load_query_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  buf = (char *)my_malloc(key_memory_log_event,
                          q_len + 1 - (fn_pos_end - fn_pos_start) + (FN_REFLEN + TEMP_FILE_MAX_LEN) + 10 + 8 + 5,
                          MYF(MY_WME));
  ...
  if (buf == NULL)
  {
    rli->report(ERROR_LEVEL, ER_SLAVE_FATAL_ERROR, ER(ER_SLAVE_FATAL_ERROR), "Not enough memory");
    return 1;
  }
...
bool sql_ex_info::write_data(IO_CACHE *file)
{
  ...
  {
    return (write_str_at_most_255_bytes(file, data_info.field_term, (uint)data_info.field_term_len) ||
            write_str_at_most_255_bytes(file, data_info.enclosed, (uint)data_info.enclosed_len) ||
            write_str_at_most_255_bytes(file, data_info.line_term, (uint)data_info.line_term_len) ||
            write_str_at_most_255_bytes(file, data_info.line_start, (uint)data_info.line_start_len) ||
            write_str_at_most_255_bytes(file, data_info.escaped, (uint)data_info.escaped_len) ||
...
*/
const char *sql_ex_info::init(const char *buf, const char *buf_end, bool use_new_format)
{
...
#ifndef MYSQL_CLIENT
Rows_log_event::Rows_log_event(THD *thd_arg, TABLE *tbl_arg, const Table_id &tid, MY_BITMAP const *cols,
                               bool using_trans, Log_event_type event_type, const uchar *extra_row_info)
...
#endif
{
  ...
  if (extra_row_info)
  {
    ...
    m_extra_row_data = (uchar *)my_malloc(key_memory_log_event, extra_data_len, MYF(MY_WME));
  ...
  if (likely(!bitmap_init(&m_cols, m_width <= sizeof(m_bitbuf) * 8 ? m_bitbuf : NULL, m_width, false)))
  {
...
Rows_log_event::Rows_log_event(const char *buf, uint event_len, const Format_description_event *description_event)
...
#endif
{
  ...
  */
  ...
  if ((m_type == binary_log::UPDATE_ROWS_EVENT) || (m_type == binary_log::UPDATE_ROWS_EVENT_V1))
  {
    /* if bitmap_init fails, is_valid will be set to false*/
    if (likely(!bitmap_init(&m_cols_ai, m_width <= sizeof(m_bitbuf_ai) * 8 ? m_bitbuf_ai : NULL, m_width, false)))
    {
...
size_t Rows_log_event::get_data_size()
{
  ...
  DBUG_EXECUTE_IF("old_row_based_repl_4_byte_map_id_master",
                  return 6 + no_bytes_in_map(&m_cols) + (end - buf) +
                         (general_type_code == binary_log::UPDATE_ROWS_EVENT ? no_bytes_in_map(&m_cols_ai) : 0) +
  ...
  if (is_v2_event)
  {
    data_size = Binary_log_event::ROWS_HEADER_LEN_V2 +
                (m_extra_row_data ? ROWS_V_TAG_LEN + m_extra_row_data[EXTRA_ROW_INFO_LEN_OFFSET] : 0);
  }
  else
  {
...
int Rows_log_event::do_add_row_data(uchar *row_data, size_t length)
{
  ...
  */
  ...
  DBUG_PRINT("enter", ("row_data: 0x%lx  length: %lu", (ulong)row_data, (ulong)length));
...
static my_bool is_any_column_signaled_for_table(TABLE *table, MY_BITMAP *cols)
{
  ...
  for (Field **ptr = table->field; *ptr && ((*ptr)->field_index < cols->n_bits); ptr++)
  {
...
static uint search_key_in_table(TABLE *table, MY_BITMAP *bi_cols, uint key_type)
{
  ...
  if (key_type & UNIQUE_KEY_FLAG)
  {
    ...
    for (key = 0, keyinfo = table->key_info; (key < table->s->keys) && (res == MAX_KEY); key++, keyinfo++)
    {
      /*
        ...
        - Skip primary keys
      */
      if (!((keyinfo->flags & (HA_NOSAME | HA_NULL_PART_KEY)) == HA_NOSAME) || (key == table->s->primary_key))
        continue;
  ...
  if (key_type & MULTIPLE_KEY_FLAG && table->s->keys)
  {
    ...
    for (key = 0, keyinfo = table->key_info; (key < table->s->keys) && (res == MAX_KEY); key++, keyinfo++)
    {
      /*
        ...
        - Skip primary keys
      */
      if (!(table->s->keys_in_use.is_set(key)) || ((keyinfo->flags & (HA_NOSAME | HA_NULL_PART_KEY)) == HA_NOSAME) ||
          !(table->file->index_flags(key, 0, true) & HA_READ_NEXT) || (key == table->s->primary_key))
        continue;
  ...
  */
  ...
  const char *s =
      ((m_rows_lookup_algorithm == Rows_log_event::ROW_LOOKUP_TABLE_SCAN)
           ? "TABLE_SCAN"
           : ((m_rows_lookup_algorithm == Rows_log_event::ROW_LOOKUP_HASH_SCAN) ? "HASH_SCAN" : "INDEX_SCAN"));
...
int Rows_log_event::row_operations_scan_and_key_setup()
{
  ...
  switch (m_rows_lookup_algorithm)
  {
    ...
    case ROW_LOOKUP_INDEX_SCAN:
    {
      ...
      m_key = (uchar *)my_malloc(key_memory_log_event, m_key_info->key_length, MYF(MY_WME));
      if (!m_key)
        error = HA_ERR_OUT_OF_MEM;
      goto err;
    }
...
static bool record_compare(TABLE *table, MY_BITMAP *cols)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
      ...
      */
      if (table->s->last_null_bit_pos > 0)
      {
        ...
        table->record[i][table->s->null_bytes - 1] |= 256U - (1U << table->s->last_null_bit_pos);
      }
    }
  }
  ...
  /**
    ...
    */
  if ((table->s->blob_fields + table->s->varchar_fields + table->s->null_fields) == 0 && bitmap_is_set_all(cols))
  {
  ...
  {
    for (Field **ptr = table->field; *ptr && ((*ptr)->field_index < cols->n_bits) && !result; ptr++)
    {
      Field *field = *ptr;
...
void Rows_log_event::do_post_row_operations(Relay_log_info const *rli, int error)
{
  ...
  */
  ...
  DBUG_PRINT("info", ("curr_row: 0x%lu; curr_row_end: 0x%lu; rows_end: 0x%lu", (ulong)m_curr_row, (ulong)m_curr_row_end,
                      (ulong)m_rows_end));
  ...
  {
    thd->get_transaction()->set_unsafe_rollback_flags(Transaction_ctx::SESSION, TRUE);
    thd->get_transaction()->set_unsafe_rollback_flags(Transaction_ctx::STMT, TRUE);
  }
}
...
int Rows_log_event::handle_idempotent_and_ignored_errors(Relay_log_info const *rli, int *err)
{
  ...
  if (error)
  {
    ...
    bool idempotent_error = (idempotent_error_code(error) && (rbr_exec_mode == RBR_EXEC_MODE_IDEMPOTENT));
    ...
    if (idempotent_error || ignored_error)
    {
      ...
      slave_rows_error_report(ll, error, rli, thd, m_table, get_type_str(),
                              const_cast<Relay_log_info *>(rli)->get_rpl_log_name(), (ulong)common_header->log_pos);
    ...
    */
    ...
    if (!first_read)
    {
      /*
        ...
        will be returned.
       */
      error = table->file->ha_index_next_same(table->record[0], m_key, m_key_info->key_length);
    ...
    if (first_read)
      if ((error = table->file->ha_index_read_map(table->record[0], m_key, HA_WHOLE_KEY, HA_READ_KEY_EXACT)))
      {
...
int Rows_log_event::add_key_to_distinct_keyset()
{
  ...
  std::pair<std::set<uchar *, Key_compare>::iterator, bool> ret = m_distinct_keys.insert(m_distinct_key_spare_buf);
  if (ret.second)
  {
...
INDEX_SCAN:
  ...
  */
  ...
  if ((get_general_type_code() == binary_log::UPDATE_ROWS_EVENT) && (saved_m_curr_row == m_curr_row))
  {
...
int Rows_log_event::do_scan_and_update(Relay_log_info const *rli)
{
   ...
   */
  while (((idempotent_errors < m_hash.size()) && !m_hash.is_empty()) && (!error || (error == HA_ERR_RECORD_DELETED)));
...
close_table:
  ...
  assert((m_hash.is_empty() && !error) || (!m_hash.is_empty() && ((error) || (idempotent_errors >= m_hash.size()))));
...
int Rows_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  */
  if (!thd->lock)
  {
    ...
    */
    ...
    if (state == GTID_STATEMENT_CANCEL)
    {
      uint error = thd->get_stmt_da()->mysql_errno();
      assert(error != 0);
      rli->report(ERROR_LEVEL, error, "Error executing row event: '%s'", thd->get_stmt_da()->message_text());
      thd->is_slave_error = 1;
      DBUG_RETURN(-1);
    }
    else if (state == GTID_STATEMENT_SKIP)
    ...
    */
    ...
    if (open_and_lock_tables(thd, rli->tables_to_lock, 0))
    {
      if (thd->is_error())
      {
        ...
        {
          if (log_warnings > 1)
            rli->report(WARNING_LEVEL, actual_error, "Error executing row event: '%s'",
                        thd->get_stmt_da()->message_text());
          thd->get_stmt_da()->reset_condition_info(thd);
          clear_all_errors(thd, const_cast<Relay_log_info *>(rli));
          error = 0;
        ...
        {
          rli->report(ERROR_LEVEL, actual_error, "Error executing row event: '%s'", thd->get_stmt_da()->message_text());
          thd->is_slave_error = 1;
        }
      }
      DBUG_RETURN(1);
    ...
    */
    ...
    {
      DBUG_PRINT("debug", ("Checking compability of tables to lock - tables_to_lock: %p", rli->tables_to_lock));
  ...
  DBUG_PRINT("debug", ("m_table: 0x%lx, m_table_id: %llu", (ulong)m_table, m_table_id.id()));
  ...
  */
  ...
  if (table)
  {
    ...
    */
    ...
    if ((m_rows_lookup_algorithm != ROW_LOOKUP_NOT_NEEDED) && !is_any_column_signaled_for_table(table, &m_cols))
    {
      error = HA_ERR_END_OF_FILE;
      goto AFTER_MAIN_EXEC_ROW_LOOP;
    }
  ...
  AFTER_MAIN_EXEC_ROW_LOOP:
    ...
    */
    ...
    { /*
          ...
          to shutdown trying to finish incomplete events group.
      */
      DBUG_EXECUTE_IF("stop_slave_middle_group", if (thd->get_transaction()->cannot_safely_rollback(
                                                         Transaction_ctx::SESSION)) const_cast<Relay_log_info *>(rli)
                                                     ->abort_slave = 1;);
    }

    if ((error = do_after_row_operations(rli, error)) && ignored_error_code(convert_handler_error(error, thd, table)))
    {
      slave_rows_error_report(INFORMATION_LEVEL, error, rli, thd, table, get_type_str(),
                              const_cast<Relay_log_info *>(rli)->get_rpl_log_name(), (ulong)common_header->log_pos);
      thd->get_stmt_da()->reset_condition_info(thd);
      clear_all_errors(thd, const_cast<Relay_log_info *>(rli));
      error = 0;
  ...
  if (error)
  {
    slave_rows_error_report(ERROR_LEVEL, error, rli, thd, table, get_type_str(),
                            const_cast<Relay_log_info *>(rli)->get_rpl_log_name(), (ulong)common_header->log_pos);
    /*
  ...
  if (get_flags(STMT_END_F))
  {
    ...
    {
      if (table)
        slave_rows_error_report(ERROR_LEVEL, thd->is_error() ? 0 : error, rli, thd, table, get_type_str(),
                                const_cast<Relay_log_info *>(rli)->get_rpl_log_name(), (ulong)common_header->log_pos);
      else
      {
        rli->report(ERROR_LEVEL, thd->is_error() ? thd->get_stmt_da()->mysql_errno() : error,
                    "Error in cleaning up after an event of type:%s; %s; the group"
                    " log file/position: %s %lu",
                    get_type_str(), thd->is_error() ? thd->get_stmt_da()->message_text() : "unexpected error",
                    const_cast<Relay_log_info *>(rli)->get_rpl_log_name(), (ulong)common_header->log_pos);
      }
    }
...
bool Rows_log_event::write_data_body(IO_CACHE *file)
{
  ...
  */
  ...
  DBUG_DUMP("m_cols", (uchar *)m_cols.bitmap, no_bytes_in_map(&m_cols));
  res = res || wrapper_my_b_safe_write(file, (uchar *)m_cols.bitmap, no_bytes_in_map(&m_cols));
  /*
  ...
  {
    DBUG_DUMP("m_cols_ai", (uchar *)m_cols_ai.bitmap, no_bytes_in_map(&m_cols_ai));
    res = res || wrapper_my_b_safe_write(file, (uchar *)m_cols_ai.bitmap, no_bytes_in_map(&m_cols_ai));
  }
...
int Rows_log_event::pack_info(Protocol *protocol)
{
  ...
  size_t bytes = my_snprintf(buf, sizeof(buf), "table_id: %llu%s", m_table_id.id(), flagstr);
...
#ifdef MYSQL_CLIENT
void Rows_log_event::print_helper(FILE *file, PRINT_EVENT_INFO *print_event_info, char const *const name)
{
  IO_CACHE *const head = &print_event_info->head_cache;
  ...
  if (!print_event_info->short_form)
  {
    ...
    my_b_printf(head, "\t%s: table id %llu%s\n", name, m_table_id.id(), last_stmt_event ? " flags: STMT_END_F" : "");
 ...
 */
#if !defined(MYSQL_CLIENT)
Table_map_log_event::Table_map_log_event(THD *thd_arg, TABLE *tbl, const Table_id &tid, bool using_trans)
    : binary_log::Table_map_event(tid, tbl->s->fields, (tbl->s->db.str), ((tbl->s->db.str) ? tbl->s->db.length : 0),
                ...
                Log_event::EVENT_NORMAL_LOGGING, header(), footer())
{
  ...
  */
  assert((tbl->s->db.str == 0) || (tbl->s->db.str[tbl->s->db.length] == 0));
  ...
  */
  m_null_bits = (uchar *)my_malloc(key_memory_log_event, num_null_bytes, MYF(MY_WME));

  m_field_metadata = (uchar *)my_malloc(key_memory_log_event, (m_colcnt * 2), MYF(MY_WME));
  ...
  */
  uchar dbs = thd_arg->get_binlog_accessed_db_names() ? thd_arg->get_binlog_accessed_db_names()->elements : 0;
  if (dbs == 1)
  {
...
static enum_tbl_map_status check_table_map(Relay_log_info const *rli, RPL_TABLE_LIST *table_list)
{
  ...
  if (rli->info_thd->slave_thread /* filtering is for slave only */ &&
      (!rpl_filter->db_ok(table_list->db) || (rpl_filter->is_on() && !rpl_filter->tables_ok("", table_list))))
    if (rli->info_thd->get_transaction()->xid_state()->has_state(XID_STATE::XA_ACTIVE))
  ...
  else
  {
    ...
    for (uint i = 0; ptr && (i < rli->tables_to_lock_count); ptr = static_cast<RPL_TABLE_LIST *>(ptr->next_local), i++)
    {
      if (ptr->table_id == table_list->table_id)
      {
        if (strcmp(ptr->db, table_list->db) || strcmp(ptr->alias, table_list->table_name) ||
...
int Table_map_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  if (!(memory = my_multi_malloc(key_memory_log_event, MYF(MY_WME), &table_list, sizeof(RPL_TABLE_LIST), &db_mem,
  ...
  table_list->table_id = DBUG_EVALUATE_IF("inject_tblmap_same_id_maps_diff_table", 0, m_table_id.id());
  table_list->updating = 1;
  table_list->required_type = FRMTYPE_TABLE;
  DBUG_PRINT("debug", ("table: %s is mapped to %llu", table_list->table_name, table_list->table_id.id()));
  ...
  else  // FILTERED_OUT, SAME_ID_MAPPING_*
  {
    ...
    {
      if (thd->slave_thread)
        rli->report(ERROR_LEVEL, ER_XA_REPLICATION_FILTERS, "%s", ER_THD(thd, ER_XA_REPLICATION_FILTERS));
      else
        /*
          For the cases in which a 'BINLOG' statement is set to
          execute in a user session
         */
        my_printf_error(ER_XA_REPLICATION_FILTERS, "%s", MYF(0), ER_THD(thd, ER_XA_REPLICATION_FILTERS));
    }
    /*
    ...
    else if (tblmap_status == SAME_ID_MAPPING_DIFFERENT_TABLE)
    {
      ...
      if (thd->slave_thread)
        ...
        /*
          ...
          execute in a user session
         */
        my_printf_error(ER_SLAVE_FATAL_ERROR, ER(ER_SLAVE_FATAL_ERROR), MYF(0), buf);
    }
...
bool Table_map_log_event::write_data_body(IO_CACHE *file)
{
  ...
  */
  ...
  return (wrapper_my_b_safe_write(file, dbuf, sizeof(dbuf)) ||
              wrapper_my_b_safe_write(file, (const uchar *)m_dbnam.c_str(), m_dblen + 1) ||
              wrapper_my_b_safe_write(file, tbuf, sizeof(tbuf)) ||
              wrapper_my_b_safe_write(file, (const uchar *)m_tblnam.c_str(), m_tbllen + 1) ||
...
int Table_map_log_event::pack_info(Protocol *protocol)
{
  char buf[256];
  size_t bytes =
      my_snprintf(buf, sizeof(buf), "table_id: %llu (%s.%s)", m_table_id.id(), m_dbnam.c_str(), m_tblnam.c_str());
  protocol->store(buf, bytes, &my_charset_bin);
  return 0;
}
#endif
...
void Table_map_log_event::print(FILE *, PRINT_EVENT_INFO *print_event_info)
{
  if (!print_event_info->short_form)
  {
    ...
    my_b_printf(&print_event_info->head_cache, "\tTable_map: `%s`.`%s` mapped to number %llu\n", m_dbnam.c_str(),
                m_tblnam.c_str(), m_table_id.id());
 ...
 */
#if !defined(MYSQL_CLIENT)
Write_rows_log_event::Write_rows_log_event(THD *thd_arg, TABLE *tbl_arg, const Table_id &tid_arg, bool is_transactional,
...
int Write_rows_log_event::do_before_row_operations(const Slave_reporting_capability *const)
{
  ...
  */
  if ((rbr_exec_mode == RBR_EXEC_MODE_IDEMPOTENT) || (m_table->s->db_type()->db_type == DB_TYPE_NDBCLUSTER))
  {
    /*
...
int Write_rows_log_event::write_row(const Relay_log_info *const rli, const bool overwrite)
{
  ...
  prepare_record(table, &m_cols, table->file->ht->db_type != DB_TYPE_NDBCLUSTER);
    ...
    */
    ...
    else
    {
      ...
      if ((uint)keynum < MAX_KEY)
      {
        ...
        error = table->file->ha_index_read_idx_map(table->record[1], keynum, (const uchar *)key, HA_WHOLE_KEY,
                                                   HA_READ_KEY_EXACT);
      }
      else
        /*
...
#ifndef MYSQL_CLIENT
Delete_rows_log_event::Delete_rows_log_event(THD *thd_arg, TABLE *tbl_arg, const Table_id &tid, bool is_transactional,
                                             const uchar *extra_row_info)
    : binary_log::Rows_event(m_type),
      Rows_log_event(thd_arg, tbl_arg, tid, tbl_arg->read_set, is_transactional,
                     log_bin_use_v1_row_events ? binary_log::DELETE_ROWS_EVENT_V1 : binary_log::DELETE_ROWS_EVENT,
                     extra_row_info),
...
#ifdef MYSQL_CLIENT
void Delete_rows_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
 ...
 */
#if !defined(MYSQL_CLIENT)
Update_rows_log_event::Update_rows_log_event(THD *thd_arg, TABLE *tbl_arg, const Table_id &tid, bool is_transactional,
                                             const uchar *extra_row_info)
    : binary_log::Rows_event(m_type),
      Rows_log_event(thd_arg, tbl_arg, tid, tbl_arg->read_set, is_transactional,
                     log_bin_use_v1_row_events ? binary_log::UPDATE_ROWS_EVENT_V1 : binary_log::UPDATE_ROWS_EVENT,
                     extra_row_info)
{
  common_header->type_code = m_type;
...
#ifdef MYSQL_CLIENT
void Update_rows_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
...
Incident_log_event::Incident_log_event(const char *buf, uint event_len,
                                       const Format_description_event *description_event)
    : binary_log::Incident_event(buf, event_len, description_event), Log_event(header(), footer())
{
...
int Incident_log_event::pack_info(Protocol *protocol)
{
  ...
  if (message_length > 0)
    ...
    bytes = my_snprintf(buf, sizeof(buf), "#%d (%s): %s", incident, description(), message);
  protocol->store(buf, bytes, &my_charset_bin);
  return 0;
}
#endif
...
int Incident_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  */
  ...
  if (state == GTID_STATEMENT_CANCEL)
  {
    uint error = thd->get_stmt_da()->mysql_errno();
    assert(error != 0);
    rli->report(ERROR_LEVEL, error, "Error executing incident event: '%s'", thd->get_stmt_da()->message_text());
    thd->is_slave_error = 1;
    DBUG_RETURN(-1);
  }
  else if (state == GTID_STATEMENT_SKIP)
  ...
  rli->report(ERROR_LEVEL, ER_SLAVE_INCIDENT, ER(ER_SLAVE_INCIDENT), description(),
...
Ignorable_log_event::Ignorable_log_event(const char *buf, const Format_description_event *descr_event)
    : binary_log::Ignorable_event(buf, descr_event), Log_event(header(), footer())
{
...
void Ignorable_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (print_event_info->short_form)
    return;
  ...
  my_b_printf(&print_event_info->head_cache, "# Unrecognized ignorable event\n");
}
#endif

Rows_query_log_event::Rows_query_log_event(const char *buf, uint event_len, const Format_description_event *descr_event)
...
#ifdef MYSQL_CLIENT
void Rows_query_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  if (!print_event_info->short_form && print_event_info->verbose > 1)
  {
    ...
    if (!(rows_query_copy = my_strdup(key_memory_log_event, m_rows_query, MYF(MY_WME))))
      return;
...
bool Rows_query_log_event::write_data_body(IO_CACHE *file)
{
  ...
  */
  DBUG_RETURN(write_str_at_most_255_bytes(file, m_rows_query, strlen(m_rows_query)));
}
...
Gtid_log_event::Gtid_log_event(const char *buffer, uint event_len, const Format_description_event *description_event)
    : binary_log::Gtid_event(buffer, event_len, description_event), Log_event(header(), footer())
{
  DBUG_ENTER(
  ...
  uint8 const post_header_len = buffer[EVENT_TYPE_OFFSET] == binary_log::ANONYMOUS_GTID_LOG_EVENT
  ...
  is_valid_param = true;
  spec.type = get_type_code() == binary_log::ANONYMOUS_GTID_LOG_EVENT ? ANONYMOUS_GROUP : GTID_GROUP;
...
#ifndef MYSQL_CLIENT
Gtid_log_event::Gtid_log_event(THD *thd_arg, bool using_trans, int64 last_committed_arg, int64 sequence_number_arg,
                               bool may_have_sbr_stmts_arg)
    : binary_log::Gtid_event(last_committed_arg, sequence_number_arg, may_have_sbr_stmts_arg),
      Log_event(thd_arg, thd_arg->variables.gtid_next.type == ANONYMOUS_GROUP ? LOG_EVENT_IGNORABLE_F : 0,
                using_trans ? Log_event::EVENT_TRANSACTIONAL_CACHE : Log_event::EVENT_STMT_CACHE,
                Log_event::EVENT_NORMAL_LOGGING, header(), footer())
{
  ...
  Log_event_type event_type =
      (spec.type == ANONYMOUS_GROUP ? binary_log::ANONYMOUS_GTID_LOG_EVENT : binary_log::GTID_LOG_EVENT);
...
Gtid_log_event::Gtid_log_event(uint32 server_id_arg, bool using_trans, int64 last_committed_arg,
                               int64 sequence_number_arg, bool may_have_sbr_stmts_arg,
                               const Gtid_specification spec_arg)
    : binary_log::Gtid_event(last_committed_arg, sequence_number_arg, may_have_sbr_stmts_arg),
      Log_event(header(), footer(), using_trans ? Log_event::EVENT_TRANSACTIONAL_CACHE : Log_event::EVENT_STMT_CACHE,
...
void Gtid_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  ...
  if (!print_event_info->short_form)
  {
    ...
    my_b_printf(head,
                ...
                "rbr_only=%s\n",
                get_type_code() == binary_log::GTID_LOG_EVENT ? "GTID" : "Anonymous_GTID", last_committed,
                sequence_number, may_have_sbr_stmts ? "no" : "yes");
  }
...
uint32 Gtid_log_event::write_data_header_to_memory(uchar *buffer)
{
  ...
  uchar gtid_flags = 0;
  gtid_flags |= may_have_sbr_stmts ? binary_log::Gtid_event::FLAG_MAY_HAVE_SBR : 0;
  *ptr_buffer = gtid_flags;
  ...
  sid.to_string(buf);
  DBUG_PRINT("info", ("sid=%s sidno=%d gno=%lld", buf, spec.gtid.sidno, spec.gtid.gno));
#endif
  ...
  *ptr_buffer = LOGICAL_TIMESTAMP_TYPECODE;
  ...
  assert((sequence_number == 0 && last_committed == 0) || (sequence_number > last_committed));
  DBUG_EXECUTE_IF("set_commit_parent_100",
                  { last_committed = max<int64>(sequence_number > 1 ? 1 : 0, sequence_number - 100); });
  DBUG_EXECUTE_IF("set_commit_parent_150",
                  { last_committed = max<int64>(sequence_number > 1 ? 1 : 0, sequence_number - 150); });
...
bool Gtid_log_event::write_data_header(IO_CACHE *file)
{
  ...
  DBUG_RETURN(wrapper_my_b_safe_write(file, (uchar *)buffer, POST_HEADER_LENGTH));
}
...
int Gtid_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  */
  ...
  if (!thd->owned_gtid.is_empty() || (thd->owned_gtid.is_empty() && gtid_next->type == GTID_GROUP))
  {
    /*
...
int Gtid_log_event::do_update_pos(Relay_log_info *rli)
{
  ...
  */
  rli->inc_event_relay_log_pos();
  DBUG_EXECUTE_IF("crash_after_update_pos_gtid", sql_print_information("Crashing crash_after_update_pos_gtid.");
                  DBUG_SUICIDE(););
  return 0;
}
...
Previous_gtids_log_event::Previous_gtids_log_event(const char *buf, uint event_len,
                                                   const Format_description_event *description_event)
    : binary_log::Previous_gtids_event(buf, event_len, description_event), Log_event(header(), footer())
{
      ...
      Log_event(header(), footer(), Log_event::EVENT_NO_CACHE, Log_event::EVENT_IMMEDIATE_LOGGING)
{
  ...
  uchar *buffer = (uchar *)my_malloc(key_memory_log_event, buf_size, MYF(MY_WME));
  if (buffer != NULL)
  {
    set->encode(buffer);
...
#ifdef MYSQL_CLIENT
void Previous_gtids_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
  IO_CACHE *const head = &print_event_info->head_cache;
...
int Previous_gtids_log_event::add_to_set(Gtid_set *target) const
{
  ...
  PROPAGATE_REPORTED_ERROR_INT(target->add_gtid_encoding(buf, buf_size + add_size, &end_pos));
  assert(end_pos <= buf_size);
  DBUG_RETURN(0);
}

char *Previous_gtids_log_event::get_str(size_t *length_p, const Gtid_set::String_format *string_format) const
{
...
**************************************************************************/
#ifndef MYSQL_CLIENT
Transaction_context_log_event::Transaction_context_log_event(const char *server_uuid_arg, bool using_trans,
                                                             my_thread_id thread_id_arg, bool is_gtid_specified_arg)
    : binary_log::Transaction_context_event(thread_id_arg, is_gtid_specified_arg),
      Log_event(header(), footer(), using_trans ? Log_event::EVENT_TRANSACTIONAL_CACHE : Log_event::EVENT_STMT_CACHE,
                Log_event::EVENT_NORMAL_LOGGING)
{
...
Transaction_context_log_event::Transaction_context_log_event(const char *buffer, uint event_len,
                                                             const Format_description_event *descr_event)
    : binary_log::Transaction_context_event(buffer, event_len, descr_event), Log_event(header(), footer())
{
  DBUG_ENTER(
      "Transaction_context_log_event::Transaction_context_log_event (const char *, uint, const "
...
size_t Transaction_context_log_event::to_string(char *buf, ulong len) const
{
  ...
  DBUG_RETURN(my_snprintf(buf, len, "server_uuid=%s\tthread_id=%lu", server_uuid, thread_id));
}
...
#ifdef MYSQL_CLIENT
void Transaction_context_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
...
bool Transaction_context_log_event::write_data_header(IO_CACHE *file)
{
  ...
  DBUG_RETURN(wrapper_my_b_safe_write(file, (const uchar *)buf, Binary_log_event::TRANSACTION_CONTEXT_HEADER_LEN));
}
...
bool Transaction_context_log_event::write_data_body(IO_CACHE *file)
{
  ...
  if (wrapper_my_b_safe_write(file, (const uchar *)server_uuid, strlen(server_uuid)) || write_snapshot_version(file) ||
      write_data_set(file, &write_set) || write_data_set(file, &read_set))
    DBUG_RETURN(true);
...
bool Transaction_context_log_event::write_data_set(IO_CACHE *file, std::list<const char *> *set)
{
  DBUG_ENTER("Transaction_context_log_event::write_data_set");
  for (std::list<const char *>::iterator it = set->begin(); it != set->end(); ++it)
  {
    ...
    int2store(buf, len);
    if (wrapper_my_b_safe_write(file, (const uchar *)buf, ENCODED_READ_WRITE_SET_ITEM_LEN) ||
...
bool Transaction_context_log_event::read_snapshot_version()
{
  ...
  DBUG_RETURN(snapshot_version->add_gtid_encoding(encoded_snapshot_version, encoded_snapshot_version_length) !=
              RETURN_STATUS_OK);
}
...
View_change_log_event::View_change_log_event(const char *buffer, uint event_len,
                                             const Format_description_event *descr_event)
    : binary_log::View_change_event(buffer, event_len, descr_event), Log_event(header(), footer())
{
  DBUG_ENTER(
...
size_t View_change_log_event::get_size_data_map(std::map<std::string, std::string> *map)
{
...
#ifdef MYSQL_CLIENT
void View_change_log_event::print(FILE *file, PRINT_EVENT_INFO *print_event_info)
{
...
int View_change_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  if (state == GTID_STATEMENT_CANCEL ||
      (state == GTID_STATEMENT_EXECUTE && gtid_pre_statement_post_implicit_commit_checks(thd)))
  {
    uint error = thd->get_stmt_da()->mysql_errno();
    assert(error != 0);
    rli->report(ERROR_LEVEL, error, "Error executing View Change event: '%s'", thd->get_stmt_da()->message_text());
    thd->is_slave_error = 1;
    return -1;
  }
...
bool View_change_log_event::write_data_header(IO_CACHE *file)
{
  ...
  DBUG_RETURN(wrapper_my_b_safe_write(file, (const uchar *)buf, Binary_log_event::VIEW_CHANGE_HEADER_LEN));
}
...
bool View_change_log_event::write_data_map(IO_CACHE *file, std::map<std::string, std::string> *map)
{
  ...
  for (iter = map->begin(); iter != map->end(); iter++)
  {
    ...
    if (wrapper_my_b_safe_write(file, buf_key_len, ENCODED_CERT_INFO_KEY_SIZE_LEN) ||
        wrapper_my_b_safe_write(file, (const uchar *)key, key_len) ||
        wrapper_my_b_safe_write(file, buf_value_len, ENCODED_CERT_INFO_VALUE_LEN) ||
...
*/
void View_change_log_event::set_certification_info(std::map<std::string, std::string> *info, size_t *event_size)
{
  ...
  *event_size += (ENCODED_CERT_INFO_KEY_SIZE_LEN + ENCODED_CERT_INFO_VALUE_LEN) * certification_info.size();
...
Heartbeat_log_event::Heartbeat_log_event(const char *buf, uint event_len,
                                         const Format_description_event *description_event)
    : binary_log::Heartbeat_event(buf, event_len, description_event), Log_event(header(), footer())
{
