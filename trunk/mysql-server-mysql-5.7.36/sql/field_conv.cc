 ...
 */
inline static bool is_subtype_of(Field::geometry_type sub, Field::geometry_type super)
{
...
type_conversion_status set_field_to_null_with_conversions(Field *field, bool no_conversions)
{
  ...
  /*
    ...
    no special value.
  */
  if (field->type() == MYSQL_TYPE_TIMESTAMP && !field->table->in_use->variables.explicit_defaults_for_timestamp)
  {
    /*
...
static void do_copy_not_null(Copy_field *copy)
{
  if (*copy->null_row || (copy->from_null_ptr && (*copy->from_null_ptr & copy->from_bit)))
  {
    if (copy->to_field()->reset() == TYPE_ERR_NULL_CONSTRAINT_VIOLATION)
      ...
      copy->to_field()->set_warning(Sql_condition::SL_WARNING, WARN_DATA_TRUNCATED, 1);
  }
  else
    copy->invoke_do_copy2(copy);
}
...
static void do_copy_blob(Copy_field *copy)
{
  ...
  if (to_length < from_length)
  {
    ...
    {
      copy->to_field()->set_warning(Sql_condition::SL_WARNING, ER_DATA_TOO_LONG, 1);
    }
    else
    {
      copy->to_field()->set_warning(Sql_condition::SL_WARNING, WARN_DATA_TRUNCATED, 1);
...
static void do_field_varbinary_pre50(Copy_field *copy)
{
  ...
  size_t length = my_lengthsp_8bit(&my_charset_bin, copy->tmp.c_ptr_quick(), copy->from_field()->field_length);
...
static void do_cut_string(Copy_field *copy)
{
  ...
  if (cs->cset->scan(cs, (char *)copy->from_ptr + copy->to_length(), (char *)copy->from_ptr + copy->from_length(),
...
{  // Shorter string field
  ...
  size_t copy_length = cs->cset->well_formed_len(cs, (char *)copy->from_ptr, (char *)from_end,
  ...
  if (well_formed_error || cs->cset->scan(cs, (char *)copy->from_ptr + copy_length, (char *)from_end, MY_SEQ_SPACES) <
  ...
  if (copy_length < copy->to_length())
    cs->cset->fill(cs, (char *)copy->to_ptr + copy_length, copy->to_length() - copy_length, ' ');
}
...
static void do_expand_binary(Copy_field *copy)
{
  const CHARSET_INFO *cs = copy->from_field()->charset();
  memcpy(copy->to_ptr, copy->from_ptr, copy->from_length());
  cs->cset->fill(cs, (char *)copy->to_ptr + copy->from_length(), copy->to_length() - copy->from_length(), '\0');
}
...
static void do_expand_string(Copy_field *copy)
{
  const CHARSET_INFO *cs = copy->from_field()->charset();
  memcpy(copy->to_ptr, copy->from_ptr, copy->from_length());
  cs->cset->fill(cs, (char *)copy->to_ptr + copy->from_length(), copy->to_length() - copy->from_length(), ' ');
}
...
static size_t get_varstring_copy_length(Field_varstring *to, const Field_varstring *from)
{
  ...
  if (is_multibyte_charset)
  {
    ...
    bytes_to_copy =
        cs->cset->well_formed_len(cs, from_beg, from_beg + from_byte_length, to_char_length, &well_formed_error);
...
void Copy_field::set(Field *to, Field *from, bool save)
{
  ...
  {
    if (to->real_type() == MYSQL_TYPE_JSON && from->real_type() == MYSQL_TYPE_JSON)
...
Copy_field::Copy_func *Copy_field::get_copy_func(Field *to, Field *from)
{
  ...
  {
    if (from->type() != MYSQL_TYPE_GEOMETRY || to->maybe_null() != from->maybe_null())
      return do_conv_blob;
    ...
    if (to_geom->get_geometry_type() == from_geom->get_geometry_type() ||
        is_subtype_of(from_geom->get_geometry_type(), to_geom->get_geometry_type()))
      return do_field_eq;
  ...
  {
    if (to->real_type() == MYSQL_TYPE_BIT || from->real_type() == MYSQL_TYPE_BIT)
      return do_field_int;
    ...
    if (from->result_type() == STRING_RESULT)
    {
      ...
      /*
        ...
        repairs data.
      */
      if (from->type() == MYSQL_TYPE_VAR_STRING && !from->has_charset() && to->type() == MYSQL_TYPE_VARCHAR &&
          !to->has_charset())
      ...
      */
      if (to->real_type() != from->real_type() || to->decimals() != from->decimals() /* e.g. TIME vs TIME(6) */ ||
          !compatible_db_low_byte_first ||
          (((to->table->in_use->variables.sql_mode & (MODE_NO_ZERO_IN_DATE | MODE_NO_ZERO_DATE | MODE_INVALID_DATES)) &&
            to->type() == MYSQL_TYPE_DATE) ||
           to->type() == MYSQL_TYPE_DATETIME))
      {
        if (from->real_type() == MYSQL_TYPE_ENUM || from->real_type() == MYSQL_TYPE_SET)
          if (to->result_type() != STRING_RESULT)
            return do_field_int;  // Convert SET to number
        return do_field_string;
      }
      if (to->real_type() == MYSQL_TYPE_ENUM || to->real_type() == MYSQL_TYPE_SET)
      {
        if (!to->eq_def(from))
        {
          if (from->real_type() == MYSQL_TYPE_ENUM && to->real_type() == MYSQL_TYPE_ENUM)
    ...
    else if (to->real_type() != from->real_type() || m_to_length != m_from_length || !compatible_db_low_byte_first)
    {
      if (to->real_type() == MYSQL_TYPE_DECIMAL || to->result_type() == STRING_RESULT)
        return do_field_string;
      if (to->result_type() == INT_RESULT)
        return do_field_int;
      return do_field_real;
...
type_conversion_status field_conv(Field *to, Field *from)
{
  ...
  if (to->real_type() == from->real_type() && !((is_blob_type(to)) && to->table->copy_blobs) &&
      to->charset() == from->charset())
  {
    if (to->real_type() == MYSQL_TYPE_VARCHAR && from->real_type() == MYSQL_TYPE_VARCHAR)
    {
    ...
    if (to->pack_length() == from->pack_length() && !(to->flags & UNSIGNED_FLAG && !(from->flags & UNSIGNED_FLAG)) &&
        to->real_type() != MYSQL_TYPE_ENUM && to->real_type() != MYSQL_TYPE_SET && to->real_type() != MYSQL_TYPE_BIT &&
        ...
        (!(to->table->in_use->variables.sql_mode & (MODE_NO_ZERO_IN_DATE | MODE_NO_ZERO_DATE | MODE_INVALID_DATES)) ||
         (to->type() != MYSQL_TYPE_DATE && to->type() != MYSQL_TYPE_DATETIME &&
          (!to->table->in_use->variables.explicit_defaults_for_timestamp || to->type() != MYSQL_TYPE_TIMESTAMP))) &&
  ...
  if (from->real_type() == MYSQL_TYPE_ENUM && to->real_type() == MYSQL_TYPE_ENUM && from->val_int() == 0)
  {
  ...
  else if (from->is_temporal() && to->result_type() == INT_RESULT)
  {
    ...
    return to->store(ltime.neg ? -nr : nr, 0);
           ...
           (to->result_type() == REAL_RESULT || to->result_type() == DECIMAL_RESULT || to->result_type() == INT_RESULT))
  {
    my_decimal tmp;
    /*
  ...
  else if (from->is_temporal() && to->is_temporal())
  {
    return copy_time_to_time(from, to);
           ...
           (to_type == MYSQL_TYPE_TINY || to_type == MYSQL_TYPE_SHORT || to_type == MYSQL_TYPE_INT24 ||
            to_type == MYSQL_TYPE_LONG || to_type == MYSQL_TYPE_LONGLONG))
  {
  ...
  else if (from_type == MYSQL_TYPE_JSON && (to_type == MYSQL_TYPE_FLOAT || to_type == MYSQL_TYPE_DOUBLE))
  {
  ...
  else if (from_type == MYSQL_TYPE_JSON && to->is_temporal())
  {
    ...
    return to->store_time(&ltime);
            ...
            (to->result_type() == STRING_RESULT ||
             (from->real_type() != MYSQL_TYPE_ENUM && from->real_type() != MYSQL_TYPE_SET))) ||
