...
String *Item_func_inet_ntoa::val_str(String *str)
{
  ...
  */
  ...
  if (n > (ulonglong)4294967295LL)
  {
    ...
    args[0]->print(&err, QT_NO_DATA_EXPANSION);
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WRONG_VALUE_FOR_TYPE,
                        ER_THD(current_thd, ER_WRONG_VALUE_FOR_TYPE), "integer", err.c_ptr_safe(), func_name());
    return NULL;
  }
  null_value = false;
...
static bool str_to_ipv4(const char *str, int str_length, in_addr *ipv4_address)
{
  ...
  DBUG_PRINT("info", ("str_to_ipv4(%.*s): valid IPv4 address: %d.%d.%d.%d", str_length, str, ipv4_bytes[0],
                      ipv4_bytes[1], ipv4_bytes[2], ipv4_bytes[3]));
  return true;
}
...
static bool str_to_ipv6(const char *str, int str_length, in6_addr *ipv6_address)
{
  if (str_length < 2)
  {
    DBUG_PRINT("error", ("str_to_ipv6(%.*s): invalid IPv6 address: too short.", str_length, str));
    return false;
  }

  if (str_length > 8 * 4 + 7)
  {
    DBUG_PRINT("error", ("str_to_ipv6(%.*s): invalid IPv6 address: too long.", str_length, str));
    return false;
  }
  ...
  while (((p - str) < str_length) && *p)
  {
    ...
    else if (c == '.')
    {
      ...
      if (!str_to_ipv4(group_start_ptr, str + str_length - group_start_ptr, (in_addr *)dst))
      {
        DBUG_PRINT("error", ("str_to_ipv6(%.*s): invalid IPv6 address: "
                             "invalid IPv4-part.",
                             str_length, str));
...
static void ipv4_to_str(const in_addr *ipv4, char *str)
{
  ...
  sprintf(str, "%d.%d.%d.%d", ipv4_bytes[0], ipv4_bytes[1], ipv4_bytes[2], ipv4_bytes[3]);
}
///////////////////////////////////////////////////////////////////////////
