...
static store_key *get_store_key(THD *thd, Key_use *keyuse, table_map used_tables, KEY_PART_INFO *key_part,
...
bool handle_query(THD *thd, LEX *lex, Query_result *result, ulonglong added_options, ulonglong removed_options)
{
  ...
  assert(!unit->is_prepared() && !unit->is_optimized() && !unit->is_executed());
  ...
  {
    if (unit->prepare(thd, result, SELECT_NO_UNLOCK | added_options, removed_options))
      goto err;
  }
    ...
    switch (pos->sj_strategy)
    {
      ...
      case SJ_OPT_DUPS_WEEDOUT:
      {
        ...
        */
        ...
        for (uint sj_tableno = tableno; sj_tableno < tableno + pos->n_sj_tables; sj_tableno++)
        {
          if (join->best_ref[sj_tableno]->use_join_cache() && sj_tableno <= no_jbuf_after)
          {
        ...
        if (last_sj_tab->first_inner() != NO_PLAN_IDX && first_sj_tab->first_inner() != last_sj_tab->first_inner())
        {
          /*
          ...
          */
          while (tab2->first_upper() != NO_PLAN_IDX && tab2->first_upper() != first_sj_tab->first_inner())
        ...
        */
        for (QEP_TAB *tab_in_range = qep_array + first_table; tab_in_range <= last_sj_tab; tab_in_range++)
        {
        ...
        if (jt_rowid_offset) /* Temptable has at least one rowid */
        {
          ...
          sjtbl->tmp_table = create_duplicate_weedout_tmp_table(thd, sjtbl->rowid_len + sjtbl->null_bytes, sjtbl);
      ...
      case SJ_OPT_FIRST_MATCH:
      {
        ...
        */
        ...
        assert(tab_in_sj_nest);  // First table must be inner
        for (QEP_TAB *tab_in_range = tab; tab_in_range <= last_sj_tab; tab_in_range++)
        {
          ...
          else
          {
            /*
              ...
              inner tables.
            */
            if (tab_in_range == last_sj_tab || !join->best_ref[tab_in_range->idx() + 1]->emb_sj_nest)
            {
...
bool JOIN::prepare_result()
{
  ...
  if ((select_lex->active_options() & OPTION_SCHEMA_TABLE) && get_schema_tables_result(this, PROCESSED_BY_JOIN_EXEC))
    goto err;
...
void calc_used_field_length(THD *thd, TABLE *table, bool keep_current_rowid, uint *p_used_fields,
                            uint *p_used_fieldlength, uint *p_used_blobs, bool *p_used_null_fields,
                            bool *p_used_uneven_bit_fields)
{
  ...
  if (blobs)
  {
    uint blob_length = (uint)(table->file->stats.mean_rec_length - (table->s->reclength - rec_length));
...
bool JOIN::init_ref_access()
{
  ...
  for (uint tableno = const_tables; tableno < tables; tableno++)
  {
    ...
    if (tab->type() == JT_REF)  // Here JT_REF means all kinds of ref access
    {
      ...
      if (create_ref_for_key(this, tab, tab->position()->key, tab->prefix_tables()))
        DBUG_RETURN(true);
    }
  }
      ...
      case SJ_OPT_FIRST_MATCH:
        ...
        */
        ...
        plan_idx last_sj_inner = (pos->sj_strategy == SJ_OPT_DUPS_WEEDOUT)
                                     ...
                                     NO_PLAN_IDX
                                     : last_sj_tab;
        for (plan_idx tab_in_range = tableno; tab_in_range <= last_sj_tab; tab_in_range++)
        {
...
void calc_length_and_keyparts(Key_use *keyuse, JOIN_TAB *tab, const uint key, table_map used_tables,
                              Key_use **chosen_keyuses, uint *length_out, uint *keyparts_out, table_map *dep_map,
                              bool *maybe_null)
{
    ...
    */
        ...
        !(found_part_ref_or_null & keyuse->optimize))
    {
      ...
      if (dep_map)
      {
        ...
        *maybe_null |= keyinfo->key_part[keyuse->keypart].null_bit && (keyuse->optimize & KEY_OPTIMIZE_REF_OR_NULL);
      }
    }
    keyuse++;
...
bool create_ref_for_key(JOIN *join, JOIN_TAB *j, Key_use *org_keyuse, table_map used_tables)
{
  ...
  if (ftkey)
  {
    ...
    calc_length_and_keyparts(keyuse, j, key, used_tables, chosen_keyuses, &length, &keyparts, NULL, NULL);
  /* set up fieldref */
  j->ref().key_parts = keyparts;
  j->ref().key_length = length;
  j->ref().key = (int)key;
  if (!(j->ref().key_buff = (uchar *)thd->mem_calloc(ALIGN_SIZE(length) * 2)) ||
      !(j->ref().key_copy = (store_key **)thd->alloc((sizeof(store_key *) * (keyparts)))) ||
  ...
  else
  {
    ...
    for (uint part_no = 0; part_no < keyparts; part_no++)
    {
      ...
      store_key *s_key =
          get_store_key(thd, keyuse, join->const_table_map, &keyinfo->key_part[part_no], key_buff, maybe_null);
      ...
      else
      {
        ...
        */
        ...
        keyuse->val->walk(&Item::repoint_const_outer_ref, Item::WALK_PREFIX, pointer_cast<uchar *>(&dummy_value));
        /*
...
void QEP_TAB::push_index_cond(const JOIN_TAB *join_tab, uint keyno, Opt_trace_object *trace_obj)
{
  ...
  if (internal_tmp_disk_storage_engine == TMP_TABLE_INNODB && tbl->s->db_type() == innodb_hton &&
      tbl->s->tmp_table != NO_TMP_TABLE && tbl->s->tmp_table != TRANSACTIONAL_TMP_TABLE)
    DBUG_VOID_RETURN;
  ...
  */
  const bool other_tbls_ok =
      !((type() == JT_ALL || type() == JT_INDEX_SCAN || type() == JT_RANGE || type() == JT_INDEX_MERGE) &&
  ...
  */
  if (condition() && tbl->file->index_flags(keyno, 0, 1) & HA_DO_INDEX_COND_PUSHDOWN &&
      hint_key_state(join_->thd, tbl, keyno, ICP_HINT_ENUM, OPTIMIZER_SWITCH_INDEX_CONDITION_PUSHDOWN) &&
      join_->thd->lex->sql_command != SQLCOM_UPDATE_MULTI && join_->thd->lex->sql_command != SQLCOM_DELETE_MULTI &&
      !has_guarded_conds() && type() != JT_CONST && type() != JT_SYSTEM &&
      !(keyno == tbl->s->primary_key && tbl->file->primary_key_is_clustered()))
  {
      ...
      */
      if (join_tab->use_join_cache() &&
          /*
            ...
            cache->is_key_access().
          */
          other_tbls_ok && (idx_cond->used_tables() & ~(table_ref->map() | join_->const_table_map)))
      {
      ...
      else
      {
        ...
        DBUG_EXECUTE("where", print_where(tbl->file->pushed_idx_cond, "icp cond", QT_ORDINARY););
      }
      /*
...
bool JOIN::setup_semijoin_materialized_table(JOIN_TAB *tab, uint tableno, const POSITION *inner_pos, POSITION *sjm_pos)
{
  ...
  assert(inner_pos->sj_strategy == SJ_OPT_MATERIALIZE_LOOKUP || inner_pos->sj_strategy == SJ_OPT_MATERIALIZE_SCAN);
  ...
  */
  ...
  count_field_types(select_lex, &sjm_exec->table_param, emb_sj_nest->nested_join->sj_inner_exprs, false, true);
  sjm_exec->table_param.bit_fields_as_long = true;

  char buffer[NAME_LEN];
  const size_t len = my_snprintf(buffer, sizeof(buffer) - 1, "<subquery%u>", emb_sj_nest->nested_join->query_block_id);
  ...
  TABLE *table;
  if (!(table =
            create_tmp_table(thd, &sjm_exec->table_param, emb_sj_nest->nested_join->sj_inner_exprs, NULL,
                             true /* distinct */, true /* save_sum_fields */,
                             thd->variables.option_bits | TMP_TABLE_ALL_COLUMNS, HA_POS_ERROR /* rows_limit */, name)))
    DBUG_RETURN(true); /* purecov: inspected */
  ...
  */
  assert((inner_pos->sj_strategy == SJ_OPT_MATERIALIZE_LOOKUP && !table->hash_field) ||
  ...
  if (!(sjm_opt->mat_fields = (Item_field **)alloc_root(thd->mem_root, field_count * sizeof(Item_field **))))
    DBUG_RETURN(true);

  for (uint fieldno = 0; fieldno < field_count; fieldno++)
  {
    if (!(sjm_opt->mat_fields[fieldno] = new Item_field(table->visible_field_ptr()[fieldno])))
      DBUG_RETURN(true);
  }
  ...
  */
  Key_use_array *keyuse =
      create_keyuse_for_table(thd, table, field_count, sjm_opt->mat_fields, emb_sj_nest->nested_join->sj_outer_exprs);
  if (!keyuse)
    DBUG_RETURN(true);
  ...
  if (!sjm_exec->is_scan)
  {
    ...
    sjm_pos->read_cost = emb_sj_nest->nested_join->sjm.lookup_cost.total_cost() * fanout;
...
bool make_join_readinfo(JOIN *join, uint no_jbuf_after)
{
  ...
  const bool prep_for_pos = join->need_tmp || join->select_distinct || join->group_list || join->order;
    ...
    */
    ...
    switch (qep_tab->type())
    {
      ...
      case JT_ALL:
        ...
        /* Fall through */
      case JT_INDEX_SCAN:
        if (tab->position()->filter_effect != COND_FILTER_STALE_NO_CONST && !tab->sj_mat_exec())
        {
          /*
          ...
          */
          ...
          tab->position()->rows_fetched = static_cast<double>(table->file->stats.records);
          if (tab->position()->filter_effect != COND_FILTER_STALE)
          {
            ...
            if (tab->position()->rows_fetched == 0)  // avoid division by zero
              ...
              tab->position()->filter_effect *= static_cast<float>(rows_w_const_cond / tab->position()->rows_fetched);
          }
        }
      ...
      case JT_INDEX_MERGE:
        ...
        if (!table->no_keyread && qep_tab->type() == JT_RANGE)
        {
          ...
          if (!table->key_read)
            qep_tab->push_index_cond(tab, qep_tab->quick()->index, &trace_refine_table);
        }
      ...
      */
      tab->position()->filter_effect =
          join->thd->lex->describe
              ? calculate_condition_filter(tab, (tab->ref().key != -1) ? tab->position()->key : NULL,
                                           tab->prefix_tables() & ~tab->table_ref->map(), tab->position()->rows_fetched,
                                           false)
              : COND_FILTER_ALLPASS;
    }
...
bool error_if_full_join(JOIN *join)
{
  ...
  for (uint i = 0; i < join->primary_tables; i++)
  {
    ...
    */
    ...
    if (!thd->lex->is_explain() && tab->table()->pos_in_table_list->updating && tab->type() == JT_ALL)
    {
      my_error(ER_UPDATE_WITHOUT_KEY_IN_SAFE_MODE, MYF(0), thd->get_stmt_da()->get_first_condition_message());
      return true;
    }
  }
  return false;
...
uint QEP_TAB::sjm_query_block_id() const
{
  ...
  for (uint i = 0; i < join()->primary_tables; ++i)
  {
    ...
    if (sjm && (uint)idx() >= sjm->inner_table_index && (uint)idx() < sjm->inner_table_index + sjm->table_count)
...
void JOIN::join_free()
{
  ...
  */
  if (can_unlock && lock && thd->lock && !thd->locked_tables_mode &&
      !(select_lex->active_options() & SELECT_NO_UNLOCK) && !select_lex->subquery_in_having &&
  ...
  return r->const_item() &&
         ...
         (Arg_comparator::can_compare_as_dates(l, r, 0) ||
          ...
          (r->result_type() == l->result_type() &&
           ...
           (l->result_type() != STRING_RESULT || l->collation.collation == r->collation.collation)));
}
...
void count_field_types(SELECT_LEX *select_lex, Temp_table_param *param, List<Item> &fields, bool reset_with_sum_func,
...
bool JOIN::alloc_func_list()
{
  ...
  */
  ...
  sum_funcs =
      (Item_sum **)thd->mem_calloc(sizeof(Item_sum **) * (func_count + 1) + sizeof(Item_sum ***) * (group_parts + 1));
      ...
      else
      {
        ...
        for (group_tmp = start_group, i = pos; group_tmp; group_tmp = group_tmp->next, i++)
        {
          if (*group_tmp->item == item)
          {
            ...
            */
            Item_null_result *null_item = new (thd->mem_root) Item_null_result(item->field_type(), item->result_type());
            if (!null_item)
              return 1;
...
bool SELECT_LEX::change_query_result(Query_result_interceptor *new_result, Query_result_interceptor *old_result)
{
  DBUG_ENTER("SELECT_LEX::change_query_result");
  if (old_result == NULL || query_result() == old_result)
  {
    set_query_result(new_result);
    if (query_result()->prepare(fields_list, master_unit()) || query_result()->prepare2())
      DBUG_RETURN(true); /* purecov: inspected */
    DBUG_RETURN(false);
  }
  else
...
bool JOIN::add_having_as_tmp_table_cond(uint curr_tmp_table)
{
  ...
  else
  {
    ...
    */
    assert(having_cond->has_subquery() || !(having_cond->used_tables() & ~(1 | PSEUDO_TABLE_BITS)));
    used_tables = 1;
  }
  ...
  */
  Item *sort_table_cond = make_cond_for_table(having_cond, used_tables, (table_map)0, false);
  if (sort_table_cond)
  {
    ...
    {
      curr_table->set_condition(new Item_cond_and(curr_table->condition(), sort_table_cond));
      if (curr_table->condition()->fix_fields(thd, 0))
        DBUG_RETURN(true);
    }
    curr_table->condition()->top_level_item();
    DBUG_EXECUTE("where", print_where(curr_table->condition(), "select and having", QT_ORDINARY););

    having_cond = make_cond_for_table(having_cond, ~(table_map)0, ~used_tables, false);
    DBUG_EXECUTE("where", print_where(having_cond, "having after sort", QT_ORDINARY););
...
bool JOIN::make_tmp_tables_info()
{
  ...
  */
  if (qep_tab && qep_tab[0].quick() && qep_tab[0].quick()->is_loose_index_scan())
    tmp_table_param.precomputed_group_by = !qep_tab[0].quick()->is_agg_loose_index_scan();
  ...
  if (need_tmp)
  {
    ...
    */
    ...
    if (create_intermediate_table(&qep_tab[curr_tmp_table], &all_fields, tmp_group, group_list && simple_group))
      DBUG_RETURN(true);
    ...
    */
    assert(!(ordered_index_usage == ordered_index_void && !plan_is_const() &&
             qep_tab[const_tables].position()->sj_strategy != SJ_OPT_LOOSE_SCAN && qep_tab[const_tables].use_order()));

    /* Change sum_fields reference to calculated fields in tmp_table */
    assert(items1.is_null());
    items1 = select_lex->ref_ptr_array_slice(2);
    if (sort_and_group || qep_tab[curr_tmp_table].table()->group || tmp_table_param.precomputed_group_by)
    {
    ...
    /*
      ...
      to the client.
    */
    if (having_cond && (sort_and_group || (exec_tmp_table->distinct && !group_list)))
    {
      /*
    ...
    */
    ...
    { /* Must copy to another table */
      ...
      count_field_types(select_lex, &tmp_table_param, tmp_all_fields1, select_distinct && !group_list, false);
      ...
      */
      ...
      if (create_intermediate_table(&qep_tab[curr_tmp_table], curr_all_fields, dummy, true))
        DBUG_RETURN(true);
      ...
      if (group_list || tmp_table_param.sum_func_count)
      {
        if (make_sum_func_list(*curr_all_fields, *curr_fields_list, true, true))
          DBUG_RETURN(true);
        const bool need_distinct = !(qep_tab[0].quick() && qep_tab[0].quick()->is_agg_loose_index_scan());
    ...
    calc_group_buffer(this, group_list);
    count_field_types(select_lex, &tmp_table_param, *curr_all_fields, false, false);
  }
  ...
  if (grouped || implicit_grouping || tmp_table_param.sum_func_count)
  {
    ...
    if (make_sum_func_list(*curr_all_fields, *curr_fields_list, true, true))
      DBUG_RETURN(true);
    const bool need_distinct = !(qep_tab && qep_tab[0].quick() && qep_tab[0].quick()->is_agg_loose_index_scan());
    if (prepare_sum_aggregators(sum_funcs, need_distinct))
      DBUG_RETURN(true);
  ...
  if (qep_tab && (group_list || order))
  {
    ...
    else if (!need_tmp)
    {
      ...
      */
      for (uint i = const_tables + 1; i < primary_tables; i++)
      {
        QEP_TAB *const tab = qep_tab + i;
        if (tab->condition() ||                                                     // 1
            (best_ref[tab->idx()]->keyuse() && tab->first_inner() == NO_PLAN_IDX))  // 2
        {
    ...
    */
    ...
    if (qep_tab && ordered_index_usage != (group_list ? ordered_index_group_by : ordered_index_order_by) &&
        qep_tab[curr_tmp_table].type() != JT_CONST && qep_tab[curr_tmp_table].type() != JT_EQ_REF)  // Don't sort 1 row
    {
    ...
    if (!plan_is_const() && !qep_tab[const_tables].table()->sort.io_cache)
    {
      /*
  ...
  if (qep_tab)
  {
    qep_tab[primary_tables + tmp_tables - 1].next_select = get_end_select_func();
  }
...
bool JOIN::add_sorting_to_table(uint idx, ORDER_with_src *sort_order)
{
  ...
  tab->filesort = new (thd->mem_root) Filesort(tab, *sort_order, HA_POS_ERROR);
  if (!tab->filesort)
    return true;
  {
    if (tab->ref().key >= 0)
      ...
      else
      {
        ...
        */
        ...
        QUICK_SELECT_I *q = tab->type() == JT_FT
                                ...
                                : get_quick_select_for_ref(thd, table, &tab->ref(), jtab->found_records);
        if (!q)
...
bool test_if_cheaper_ordering(const JOIN_TAB *tab, ORDER *order, TABLE *table, key_map usable_keys, int ref_key,
                              ha_rows select_limit, int *new_key, int *new_key_direction, ha_rows *new_select_limit,
                              uint *new_used_key_parts, uint *saved_best_key_parts)
{
  ...
  if (ref_key >= 0 && cur_access_method == JT_REF)
  {
    ...
    else
    {
      ...
      if (ref_keyinfo->has_records_per_key(tab->ref().key_parts - 1))
        refkey_rows_estimate = ref_keyinfo->records_per_key(tab->ref().key_parts - 1);
      else
  ...
  for (nr = 0; nr < table->s->keys; nr++)
  {
    ...
    if (keys.is_set(nr) && (direction = test_if_order_by_key(order, table, nr, &used_key_parts)))
    {
      ...
      */
      ...
      bool is_covering =
          table->covering_keys.is_set(nr) || (nr == table->s->primary_key && table->file->primary_key_is_clustered());
      ...
      */
      if (is_covering || select_limit != HA_POS_ERROR || (ref_key < 0 && (group || table->force_index)))
      {
        ...
        if (group)
        {
          ...
          */
          rec_per_key = used_key_parts && used_key_parts <= actual_key_parts(keyinfo)
        ...
        */
        ...
        const double index_scan_time =
            ...
            min<double>(table->cost_model()->page_read_cost(rec_per_key), table_scan_time.total_cost());
        ...
        */
            ...
            index_scan_time < read_time)
        {
          ...
          const ha_rows refkey_select_limit = (ref_key >= 0 && table->covering_keys.is_set(ref_key))
                                                  ? static_cast<ha_rows>(refkey_rows_estimate)
                                                  : HA_POS_ERROR;

          if ((is_best_covering && !is_covering) || (is_covering && refkey_select_limit < select_limit))
            continue;
          if (table->quick_keys.is_set(nr))
            quick_records = table->quick_rows[nr];
          if (best_key < 0 ||
              (select_limit <= min(quick_records, best_records) ? keyinfo->user_defined_key_parts < best_key_parts
...
uint get_index_for_order(ORDER *order, QEP_TAB *tab, ha_rows limit, bool *need_sort, bool *reverse)
{
  ...
  if (tab->quick())
  {
    ...
    uint used_key_parts;
    switch (test_if_order_by_key(order, table, tab->quick()->index, &used_key_parts))
    {
      ...
      case -1:  // desired order, but opposite direction
      {
        ...
        if ((reverse_quick = tab->quick()->make_reverse(used_key_parts)))
        {
          delete tab->quick();
...
{
  return key_info->table->in_use->optimizer_switch_flag(OPTIMIZER_SWITCH_USE_INDEX_EXTENSIONS)
...
{
  return key_info->table->in_use->optimizer_switch_flag(OPTIMIZER_SWITCH_USE_INDEX_EXTENSIONS) ? key_info->actual_flags
                                                                                               : key_info->flags;
}

join_type calc_join_type(int quick_type)
{
  if ((quick_type == QUICK_SELECT_I::QS_TYPE_INDEX_MERGE) || (quick_type == QUICK_SELECT_I::QS_TYPE_ROR_INTERSECT) ||
