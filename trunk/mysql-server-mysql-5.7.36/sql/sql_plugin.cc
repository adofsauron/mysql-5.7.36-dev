...
*/
const LEX_STRING plugin_type_names[MYSQL_MAX_PLUGIN_TYPE_NUM] = {{C_STRING_WITH_LEN("UDF")},
...
plugin_type_init plugin_type_initialize[MYSQL_MAX_PLUGIN_TYPE_NUM] = {
    0, ha_initialize_handlerton, 0, 0, initialize_schema_table, initialize_audit_plugin, 0, 0, 0};

plugin_type_init plugin_type_deinitialize[MYSQL_MAX_PLUGIN_TYPE_NUM] = {
    0, ha_finalize_handlerton, 0, 0, finalize_schema_table, finalize_audit_plugin, 0, 0, 0};
...
*/
static int min_plugin_info_interface_version[MYSQL_MAX_PLUGIN_TYPE_NUM] = {0x0000,
...
static int cur_plugin_info_interface_version[MYSQL_MAX_PLUGIN_TYPE_NUM] = {0x0000, /* UDF: not implemented */
                                                                           MYSQL_HANDLERTON_INTERFACE_VERSION,
                                                                           MYSQL_FTPARSER_INTERFACE_VERSION,
                                                                           MYSQL_DAEMON_INTERFACE_VERSION,
                                                                           MYSQL_INFORMATION_SCHEMA_INTERFACE_VERSION,
...
class sys_var_pluginvar : public sys_var
{
  ...
  */
  ...
  sys_var_pluginvar(sys_var_chain *chain, const char *name_arg, st_mysql_sys_var *plugin_var_arg)
      : sys_var(chain, name_arg, plugin_var_arg->comment,
                (plugin_var_arg->flags & PLUGIN_VAR_THDLOCAL ? SESSION : GLOBAL) |
                    (plugin_var_arg->flags & PLUGIN_VAR_READONLY ? READONLY : 0) |
                    (plugin_var_arg->flags & PLUGIN_VAR_INVISIBLE ? INVISIBLE : 0),
                0, -1, NO_ARG, pluginvar_show_type(plugin_var_arg), 0, 0, VARIABLE_NOT_IN_BINLOG,
                (plugin_var_arg->flags & PLUGIN_VAR_NODEFAULT) ? on_check_pluginvar : NULL, NULL, NULL, PARSE_NORMAL),
  ...
  uchar *do_value_ptr(THD *thd, enum_var_type type, LEX_STRING *base) { return do_value_ptr(thd, thd, type, base); }
...
static bool plugin_load_list(MEM_ROOT *tmp_root, int *argc, char **argv, const char *list);
static my_bool check_if_option_is_deprecated(int optid, const struct my_option *opt, char *argument);
...
static bool plugin_var_memalloc_session_update(THD *thd, st_mysql_sys_var *var, char **dest, const char *value);
static bool plugin_var_memalloc_global_update(THD *thd, st_mysql_sys_var *var, char **dest, const char *value);
static void plugin_var_memalloc_free(struct system_variables *vars);
static void restore_pluginvar_names(sys_var *first);
static void plugin_opt_set_limits(struct my_option *, const st_mysql_sys_var *);
...
{
  size_t prefix = my_strcspn(files_charset_info, path, path + len, FN_DIRSEP, strlen(FN_DIRSEP));
  return prefix < len;
}
...
static const char *item_val_str(st_mysql_value *value, char *buffer, int *length)
{
...
static st_plugin_dl *plugin_dl_find(const LEX_STRING *dl)
{
  ...
  for (st_plugin_dl **it = plugin_dl_array->begin(); it != plugin_dl_array->end(); ++it)
  {
    st_plugin_dl *tmp = *it;
    if (tmp->ref_count && !my_strnncoll(files_charset_info, pointer_cast<uchar *>(dl->str), dl->length,
...
static st_plugin_dl *plugin_dl_insert_or_reuse(st_plugin_dl *plugin_dl)
{
  ...
  tmp = plugin_dl_array->back() =
      static_cast<st_plugin_dl *>(memdup_root(&plugin_mem_root, plugin_dl, sizeof(st_plugin_dl)));
  DBUG_RETURN(tmp);
}
#endif /* HAVE_DLOPEN */
...
static st_plugin_dl *plugin_dl_add(const LEX_STRING *dl, int report)
{
  ...
  */
  ...
  if (check_valid_path(dl->str, dl->length) ||
      check_string_char_length(dl_cstr, "", NAME_CHAR_LEN, system_charset_info, 1) ||
  ...
  /* Compile dll path */
  dlpathlen = strxnmov(dlpath, sizeof(dlpath) - 1, opt_plugin_dir, "/", dl->str, NullS) - dlpath;
      ...
      (plugin_dl.version >> 8) > (MYSQL_PLUGIN_INTERFACE_VERSION >> 8))
  {
    free_plugin_mem(&plugin_dl);
    mysql_mutex_unlock(&LOCK_plugin);
    report_error(report, ER_CANT_OPEN_LIBRARY, dlpath, 0, "plugin interface version mismatch");
    DBUG_RETURN(NULL);
  }
  ...
  for (i = 0; i < array_elements(list_of_services); i++)
  {
    if ((sym = dlsym(plugin_dl.handle, list_of_services[i].name)))
    {
      ...
      if ((*(void **)sym) != list_of_services[i].service && /* already replaced */
          (ver > list_of_services[i].version || (ver >> 8) < (list_of_services[i].version >> 8)))
      {
  ...
  if (plugin_dl.version != MYSQL_PLUGIN_INTERFACE_VERSION)
  {
     ...
     */
    for (i = 0; ((st_mysql_plugin *)(ptr + i * sizeof_st_plugin))->info; i++) /* no op */;

    cur = (st_mysql_plugin *)my_malloc(key_memory_mysql_plugin, (i + 1) * sizeof(st_mysql_plugin),
  ...
  plugin_dl.dl.length = copy_and_convert(plugin_dl.dl.str, plugin_dl.dl.length, files_charset_info, dl->str, dl->length,
  ...
  if (!(tmp = plugin_dl_insert_or_reuse(&plugin_dl)))
  {
    mysql_mutex_unlock(&LOCK_plugin);
    free_plugin_mem(&plugin_dl);
    report_error(report, ER_OUTOFMEMORY, static_cast<int>(sizeof(st_plugin_dl)));
    DBUG_RETURN(NULL);
  }
  DBUG_RETURN(tmp);
#else
...
static st_plugin_int *plugin_find_internal(const LEX_CSTRING &name, int type)
{
  ...
  if (type == MYSQL_ANY_PLUGIN)
  {
    ...
    {
      st_plugin_int *plugin =
          (st_plugin_int *)my_hash_search(&plugin_hash[i], reinterpret_cast<const uchar *>(name.str), name.length);
      if (plugin)
        DBUG_RETURN(plugin);
    }
  }
    ...
    DBUG_RETURN(
        (st_plugin_int *)my_hash_search(&plugin_hash[type], reinterpret_cast<const uchar *>(name.str), name.length));
  DBUG_RETURN(NULL);
}
...
static plugin_ref intern_plugin_lock(LEX *lex, plugin_ref rc)
{
  ...
  if (pi->state & (PLUGIN_IS_READY | PLUGIN_IS_UNINITIALIZED))
  {
    ...
    */
    if (!(plugin = (plugin_ref)my_malloc(key_memory_plugin_ref, sizeof(pi), MYF(MY_WME))))
      DBUG_RETURN(NULL);

    *plugin = pi;
#endif
    pi->ref_count++;
    DBUG_PRINT("info", ("thd: %p, plugin: \"%s\", ref_count: %d", current_thd, pi->name.str, pi->ref_count));
    if (lex)
...
static st_plugin_int *plugin_insert_or_reuse(st_plugin_int *plugin)
{
  ...
  st_plugin_int *tmp;
  for (st_plugin_int **it = plugin_array->begin(); it != plugin_array->end(); ++it)
  {
    tmp = *it;
  ...
  tmp = plugin_array->back() =
      static_cast<st_plugin_int *>(memdup_root(&plugin_mem_root, plugin, sizeof(st_plugin_int)));
  DBUG_RETURN(tmp);
}
...
*/
static bool plugin_add(MEM_ROOT *tmp_root, const LEX_STRING *name, const LEX_STRING *dl, int *argc, char **argv,
                       int report)
{
  st_plugin_int tmp;
  ...
  for (plugin = tmp.plugin_dl->plugins; plugin->info; plugin++)
  {
    ...
    if (plugin->type >= 0 && plugin->type < MYSQL_MAX_PLUGIN_TYPE_NUM &&
        !my_strnncoll(system_charset_info, pointer_cast<const uchar *>(name->str), name->length,
                      pointer_cast<const uchar *>(plugin->name), name_len))
    {
          ...
          ((*(int *)plugin->info) >> 8) > (cur_plugin_info_interface_version[plugin->type] >> 8))
      {
        ...
        strxnmov(buf, sizeof(buf) - 1, "API version for ", plugin_type_names[plugin->type].str,
      ...
      if ((tmp_plugin_ptr = plugin_insert_or_reuse(&tmp)))
      {
        ...
        {
          init_alloc_root(key_memory_plugin_int_mem_root, &tmp_plugin_ptr->mem_root, 4096, 4096);
          DBUG_RETURN(false);
        }
...
static void plugin_deinitialize(st_plugin_int *plugin, bool ref_check)
{
  ...
  */
  ...
  if (plugin_type_deinitialize[plugin->plugin->type])
  {
    ...
    {
      sql_print_error("Plugin '%s' of type %s failed deinitialization", plugin->name.str,
...
static void intern_plugin_unlock(LEX *lex, plugin_ref plugin)
{
  ...
  DBUG_PRINT("info", ("unlocking plugin, name= %s, ref_count= %d", pi->name.str, pi->ref_count));
  if (lex)
  {
    /*
...
uchar *get_plugin_hash_key(const uchar *buff, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
...
uchar *get_bookmark_hash_key(const uchar *buff, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
...
static bool plugin_init_initialize_and_reap()
{
  ...
  *(reap++) = NULL;
  ...
  {
    mysql_mutex_unlock(&LOCK_plugin);
    if (plugin_ptr->load_option == PLUGIN_FORCE || plugin_ptr->load_option == PLUGIN_FORCE_PLUS_PERMANENT)
...
static bool plugin_init_internals()
{
  ...
  if (my_hash_init(&bookmark_hash, &my_charset_bin, 16, 0, 0, get_bookmark_hash_key, NULL, HASH_UNIQUE,
                   key_memory_plugin_bookmark))
    goto err;
  if (my_hash_init(&malloced_string_type_sysvars_bookmark_hash, &my_charset_bin, 16, 0, 0, get_bookmark_hash_key, NULL,
                   HASH_UNIQUE, key_memory_plugin_bookmark))
    goto err;

  mysql_mutex_init(key_LOCK_plugin, &LOCK_plugin, MY_MUTEX_INIT_FAST);
  mysql_mutex_init(key_LOCK_plugin_delete, &LOCK_plugin_delete, MY_MUTEX_INIT_FAST);

  plugin_dl_array = new (std::nothrow) Prealloced_array<st_plugin_dl *, 16>(key_memory_mysql_plugin_dl);
  plugin_array = new (std::nothrow) Prealloced_array<st_plugin_int *, 16>(key_memory_mysql_plugin);
  if (plugin_dl_array == NULL || plugin_array == NULL)
    goto err;

  for (uint i = 0; i < MYSQL_MAX_PLUGIN_TYPE_NUM; i++)
  {
    if (my_hash_init(&plugin_hash[i], system_charset_info, 16, 0, 0, get_plugin_hash_key, NULL, HASH_UNIQUE,
...
bool plugin_register_early_plugins(int *argc, char **argv, int flags)
{
  ...
  i_string *item;
  while (NULL != (item = iter++)) plugin_load_list(&tmp_root, argc, argv, item->ptr);
  ...
  for (struct st_mysql_plugin **builtins = mysql_mandatory_plugins; *builtins || mandatory; builtins++)
  {
    ...
    for (struct st_mysql_plugin *plugin = *builtins; plugin->info; plugin++)
    {
      ...
      */
      ...
      if (!is_myisam && (!is_innodb || opt_help) && my_strcasecmp(&my_charset_latin1, plugin->name, "CSV"))
        continue;

      if (plugin_ptr->state != PLUGIN_IS_UNINITIALIZED || plugin_initialize(plugin_ptr))
        goto err_unlock;
      ...
      */
      if (is_myisam)
      {
        ...
        global_system_variables.table_plugin = my_intern_plugin_lock(NULL, plugin_int_to_ref(plugin_ptr));
        global_system_variables.temp_table_plugin = my_intern_plugin_lock(NULL, plugin_int_to_ref(plugin_ptr));
...
static bool register_builtin(st_mysql_plugin *plugin, st_plugin_int *tmp, st_plugin_int **ptr)
{
  ...
  *ptr = plugin_array->back() = static_cast<st_plugin_int *>(memdup_root(&plugin_mem_root, tmp, sizeof(st_plugin_int)));
  ...
  while (!(error = read_record_info.read_record(&read_record_info)))
  {
    ...
    */
    ...
    if (plugin_add(tmp_root, &name, &dl, argc, argv, REPORT_TO_LOG))
      sql_print_warning("Couldn't load plugin named '%s' with soname '%s'.", str_name.c_ptr(), str_dl.c_ptr());
    else
      mysql_mutex_unlock(&LOCK_plugin);
    mysql_rwlock_unlock(&LOCK_system_variables_hash);
    free_root(tmp_root, MYF(MY_MARK_BLOCKS_FREE));
...
static bool plugin_load_list(MEM_ROOT *tmp_root, int *argc, char **argv, const char *list)
{
  ...
  DBUG_RETURN(FALSE);
error:
  sql_print_error("Couldn't load plugin named '%s' with soname '%s'.", name.str, dl.str);
  DBUG_RETURN(TRUE);
}
...
void memcached_shutdown(void)
{
  if (initialized)
  {
    for (st_plugin_int **it = plugin_array->begin(); it != plugin_array->end(); ++it)
    {
      ...
      if (plugin->state == PLUGIN_IS_READY && strcmp(plugin->name.str, "daemon_memcached") == 0)
      {
        plugin_deinitialize(plugin, true);
  ...
  if (initialized)
  {
    ...
    */
    while (reap_needed && (count = plugin_array->size()))
    {
      ...
      for (i = 0; i < count; i++)
      {
        ...
        if (plugin->state == PLUGIN_IS_READY && strcmp(plugin->name.str, "binlog") == 0 && skip_binlog)
        {
          skip_binlog = false;
        }
    ...
    */
    for (i = 0; i < count; i++)
      ...
      {
        sql_print_warning("Plugin '%s' will be forced to shutdown", plugins[i]->name.str);
        /*
...
static bool mysql_install_plugin(THD *thd, const LEX_STRING *name, const LEX_STRING *dl)
{
  TABLE_LIST tables;
  TABLE *table;
  bool error = true;
  ...
  if (!(table = open_ltable(thd, &tables, TL_WRITE, MYSQL_LOCK_IGNORE_TIMEOUT)))
    ...
    /*
      ...
      */
#ifndef EMBEDDED_LIBRARY
  mysql_audit_acquire_plugins(thd, MYSQL_AUDIT_GENERAL_CLASS, MYSQL_AUDIT_GENERAL_ALL);
#endif
  ...
  {
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_CANT_INITIALIZE_UDF, ER(ER_CANT_INITIALIZE_UDF), name->str,
                        "Plugin is disabled");
  }
  else
  {
...
static bool mysql_uninstall_plugin(THD *thd, const LEX_STRING *name)
{
  ...
  */
  if (!strcmp(name->str, "rpl_semi_sync_master") &&
      get_status_var(thd, plugin->plugin->status_vars, "Rpl_semi_sync_master_clients", buff, OPT_DEFAULT,
                     &buff_length) &&
      strcmp(buff, "0"))
  {
    mysql_mutex_unlock(&LOCK_plugin);
  ...
  */
  if (!strcmp(name->str, "rpl_semi_sync_slave") &&
      get_status_var(thd, plugin->plugin->status_vars, "Rpl_semi_sync_slave_status", buff, OPT_DEFAULT, &buff_length) &&
      !strcmp(buff, "ON"))
  {
    mysql_mutex_unlock(&LOCK_plugin);
    my_error(ER_PLUGIN_CANNOT_BE_UNINSTALLED, MYF(0), name->str,
  ...
  plugin->state = PLUGIN_IS_DELETED;
  if (plugin->ref_count)
    push_warning(thd, Sql_condition::SL_WARNING, WARN_PLUGIN_BUSY, ER(WARN_PLUGIN_BUSY));
  else
    reap_needed = true;
  reap_plugins();
  mysql_mutex_unlock(&LOCK_plugin);
...
bool plugin_foreach_with_mask(THD *thd, plugin_foreach_func **funcs, int type, uint state_mask, void *arg)
{
  ...
  mysql_mutex_lock(&LOCK_plugin);
  total = type == MYSQL_ANY_PLUGIN ? plugin_array->size() : plugin_hash[type].records;
  /*
...
static int check_func_bool(THD *thd, st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
...
static int check_func_int(THD *thd, st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  ...
  return throw_bounds_warning(thd, var->name, fixed1 || fixed2, value->is_unsigned(value), orig);
}

static int check_func_long(THD *thd, st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  my_bool fixed1, fixed2;
  long long orig, val;
  struct my_option options;
...
static int check_func_longlong(THD *thd, st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  my_bool fixed1, fixed2;
  long long orig, val;
  struct my_option options;
...
static int check_func_str(THD *thd, st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  char buff[STRING_BUFFER_USUAL_SIZE];
  const char *str;
  int length;
...
static int check_func_enum(THD *thd, st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  char buff[STRING_BUFFER_USUAL_SIZE];
  const char *str;
  TYPELIB *typelib;
...
static int check_func_set(THD *thd, st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
...
static int check_func_double(THD *thd, st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  double v;
  my_bool fixed;
...
static void update_func_bool(THD *thd, st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(my_bool *)tgt = *(my_bool *)save ? TRUE : FALSE;
}

static void update_func_int(THD *thd, st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(int *)tgt = *(int *)save;
}

static void update_func_long(THD *thd, st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(long *)tgt = *(long *)save;
}

static void update_func_longlong(THD *thd, st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(longlong *)tgt = *(ulonglong *)save;
}

static void update_func_str(THD *thd, st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(char **)tgt = *(char **)save;
}

static void update_func_double(THD *thd, st_mysql_sys_var *var, void *tgt, const void *save)
{
...
sys_var *find_sys_var_ex(THD *thd, const char *str, size_t length, bool throw_error, bool locked)
{
  sys_var *var;
...
static st_bookmark *register_var(const char *plugin, const char *name, int flags)
{
  ...
  {
    result = (st_bookmark *)alloc_root(&plugin_mem_root, sizeof(st_bookmark) + length - 1);
    varname[0] = flags & PLUGIN_VAR_TYPEMASK;
    ...
    if (new_size > global_variables_dynamic_size)
    {
      ...
      /*
        ...
        string.
      */
      memset(global_system_variables.dynamic_variables_ptr + global_variables_dynamic_size, 0,
             new_size - global_variables_dynamic_size);
      memset(max_system_variables.dynamic_variables_ptr + global_variables_dynamic_size, 0,
             new_size - global_variables_dynamic_size);
    ...
    */
    if (((flags & PLUGIN_VAR_TYPEMASK) == PLUGIN_VAR_STR) && (flags & PLUGIN_VAR_MEMALLOC) &&
        (my_hash_insert(&malloced_string_type_sysvars_bookmark_hash, (uchar *)result)))
    {
      fprintf(stderr,
...
void alloc_and_copy_thd_dynamic_variables(THD *thd, bool global_lock)
{
  ...
  */
  for (idx = 0; idx < malloced_string_type_sysvars_bookmark_hash.records; idx++)
  {
    ...
    int varoff;
    char **thdvar, **sysvar;
    st_bookmark *v = (st_bookmark *)my_hash_element(&malloced_string_type_sysvars_bookmark_hash, idx);

    if (v->version <= thd->variables.dynamic_variables_version ||
        !(var = intern_find_sys_var(v->key + 1, v->name_len)) || !(pi = var->cast_pluginvar()) ||
...
void plugin_thdvar_init(THD *thd, bool enable_plugins)
{
  ...
  {
    mysql_mutex_lock(&LOCK_plugin);
    thd->variables.table_plugin = my_intern_plugin_lock(NULL, global_system_variables.table_plugin);
    intern_plugin_unlock(NULL, old_table_plugin);
    thd->variables.temp_table_plugin = my_intern_plugin_lock(NULL, global_system_variables.temp_table_plugin);
...
void plugin_thdvar_cleanup(THD *thd, bool enable_plugins)
{
  ...
  if (enable_plugins)
  {
    ...
    if ((idx = thd->lex->plugins.size()))
    {
      ...
      while (list >= thd->lex->plugins.begin()) intern_plugin_unlock(thd->lex, *list--);
    }
...
static void plugin_vars_free_values(sys_var *vars)
{
  ...
  for (sys_var *var = vars; var; var = var->next)
  {
        ...
        (piv->plugin_var->flags & PLUGIN_VAR_MEMALLOC))
    {
      ...
      DBUG_PRINT("plugin", ("freeing value for: '%s'  addr: 0x%lx", var->name.str, (long)valptr));
      my_free(*valptr);
      *valptr = NULL;
    }
  }
...

{
  ...
  if (value)
  {
    ...
    LIST *element;
    if (!(element = (LIST *)my_malloc(key_memory_THD_variables, sizeof(LIST) + length, MYF(MY_WME))))
      DBUG_RETURN(true);
    memcpy(element + 1, value, length);
    value = (const char *)(element + 1);
    vars->dynamic_variables_allocs = list_add(vars->dynamic_variables_allocs, element);
  }
  ...
  if (old_element)
  {
    vars->dynamic_variables_allocs = list_delete(vars->dynamic_variables_allocs, old_element);
    my_free(old_element);
  }
  DBUG_RETURN(false);
}
...
static bool plugin_var_memalloc_global_update(THD *thd, st_mysql_sys_var *var, char **dest, const char *value)
{
  ...
  if (value && !(value = my_strdup(key_memory_global_system_variables, value, MYF(MY_WME))))
    DBUG_RETURN(true);
...
uchar *sys_var_pluginvar::do_value_ptr(THD *running_thd, THD *target_thd, enum_var_type type, LEX_STRING *base)
{
  uchar *result;
  ...
  if ((plugin_var->flags & PLUGIN_VAR_TYPEMASK) == PLUGIN_VAR_ENUM)
    ...
    result = (uchar *)set_to_string(running_thd, 0, *(ulonglong *)result, plugin_var_typelib()->type_names);
  return result;
}
...
bool sys_var_pluginvar::session_update(THD *thd, set_var *var)
{
  ...
  if ((plugin_var->flags & PLUGIN_VAR_TYPEMASK) == PLUGIN_VAR_STR && plugin_var->flags & PLUGIN_VAR_MEMALLOC)
    rc = plugin_var_memalloc_session_update(thd, plugin_var, (char **)tgt, *(const char **)src);
  else
...
bool sys_var_pluginvar::global_update(THD *thd, set_var *var)
{
  ...
  if ((plugin_var->flags & PLUGIN_VAR_TYPEMASK) == PLUGIN_VAR_STR && plugin_var->flags & PLUGIN_VAR_MEMALLOC)
    rc = plugin_var_memalloc_global_update(thd, plugin_var, (char **)tgt, *(const char **)src);
  else
    plugin_var->update(thd, plugin_var, tgt, src);
...
static void plugin_opt_set_limits(struct my_option *options, const st_mysql_sys_var *opt)
{
  options->sub_size = 0;
...
extern "C" my_bool get_one_plugin_option(int optid, const struct my_option *, char *);

my_bool get_one_plugin_option(int optid MY_ATTRIBUTE((unused)), const struct my_option *opt, char *argument)
{
  return 0;
}
...
static int construct_options(MEM_ROOT *mem_root, st_plugin_int *tmp, my_option *options)
{
  ...
  if (tmp->load_option != PLUGIN_FORCE && tmp->load_option != PLUGIN_FORCE_PLUS_PERMANENT)
  {
...
static my_bool check_if_option_is_deprecated(int optid, const struct my_option *opt,
...
static int test_plugin_options(MEM_ROOT *tmp_root, st_plugin_int *tmp, int *argc, char **argv)
{
  struct sys_var_chain chain = {NULL, NULL};
  bool disable_plugin;
  enum_plugin_load_option plugin_load_option = tmp->load_option;

  MEM_ROOT *mem_root = alloc_root_inited(&tmp->mem_root) ? &tmp->mem_root : &plugin_mem_root;
  st_mysql_sys_var **opt;
  my_option *opts = NULL;
