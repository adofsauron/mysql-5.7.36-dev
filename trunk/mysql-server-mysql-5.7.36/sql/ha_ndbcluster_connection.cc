...
int ndbcluster_connect(int (*connect_callback)(void),
                       ...
                       uint connection_pool_size, bool optimized_node_select, const char *connect_string,
                       uint force_nodeid, uint recv_thread_activation_threshold)
{
  ...
  DBUG_PRINT("enter", ("connect_string: %s, force_nodeid: %d", connect_string, force_nodeid));

  global_flag_skip_waiting_for_clean_cache = 1;

  g_ndb_cluster_connection = new Ndb_cluster_connection(connect_string, force_nodeid);
  ...
  {
    char buf[128];
    my_snprintf(buf, sizeof(buf), "%s --server-id=%lu", mysqld_name, server_id);
    g_ndb_cluster_connection->set_name(buf);
  }
  g_ndb_cluster_connection->set_optimized_node_selection(optimized_node_select);
  g_ndb_cluster_connection->set_recv_thread_activation_threshold(recv_thread_activation_threshold);
  ...
  if (g_ndb->init() != 0)
  {
    DBUG_PRINT("error", ("%d  message: %s", g_ndb->getNdbError().code, g_ndb->getNdbError().message));
    DBUG_RETURN(-1);
  }
...

  {
    ...
    g_pool = (Ndb_cluster_connection **)my_malloc(PSI_INSTRUMENT_ME, g_pool_alloc * sizeof(Ndb_cluster_connection *),
                                                  MYF(MY_WME | MY_ZEROFILL));
    native_mutex_init(&g_pool_mutex, MY_MUTEX_INIT_FAST);
    g_pool[0] = g_ndb_cluster_connection;
    for (uint i = 1; i < g_pool_alloc; i++)
    {
      if ((g_pool[i] = new Ndb_cluster_connection(connect_string, g_ndb_cluster_connection)) == 0)
      {
        sql_print_error("NDB[%u]: failed to allocate cluster connect object", i);
        DBUG_PRINT("error", ("Ndb_cluster_connection[%u](%s)", i, connect_string));
        DBUG_RETURN(-1);
      }
      {
        char buf[128];
        my_snprintf(buf, sizeof(buf), "%s --server-id=%lu (connection %u)", mysqld_name, server_id, i + 1);
  ...
  if (res == 0)
  {
    ...
    for (uint i = 0; i < g_pool_alloc; i++)
    {
      ...
      DBUG_PRINT("info", ("NDBCLUSTER storage engine (%u) at %s on port %d", i, g_pool[i]->get_connected_host(),
      ...
      sql_print_information("NDB[%u]: NodeID: %d, %s", i, node_id, msg);
    }
  }
  else if (res == 1)
  {
  ...
  else
  {
    ...
    sql_print_error("NDB: error (%u) %s", g_ndb_cluster_connection->get_latest_error(),
...
int ndb_set_recv_thread_cpu(Uint16 *cpuid_array, Uint32 cpuid_array_size)
{
  ...
  for (Uint32 i = 0; i < g_pool_alloc; i++)
  {
    g_pool[i]->set_recv_thread_cpu(&cpuid_array[i], (Uint32)1, 0);
  }
  return 0;
}
...
static ST_FIELD_INFO ndb_transid_mysql_connection_map_fields_info[] = {
    {"mysql_connection_id", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, "", SKIP_OPEN_TABLE},

    {"node_id", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONG, 0, MY_I_S_UNSIGNED, "", SKIP_OPEN_TABLE},
    {"ndb_transid", MY_INT64_NUM_DECIMAL_DIGITS, MYSQL_TYPE_LONGLONG, 0, MY_I_S_UNSIGNED, "", SKIP_OPEN_TABLE},

    {0, 0, MYSQL_TYPE_NULL, 0, 0, "", SKIP_OPEN_TABLE}};

#include <mysql/innodb_priv.h>

static int ndb_transid_mysql_connection_map_fill_table(THD *thd, TABLE_LIST *tables, Item *)
{
...
static int ndb_transid_mysql_connection_map_deinit(void *p)
{
