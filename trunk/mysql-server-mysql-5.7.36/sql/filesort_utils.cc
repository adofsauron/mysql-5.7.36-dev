 ...
 */
double get_merge_cost(ha_rows num_elements, ha_rows num_buffers, uint elem_size, const Cost_model_table *cost_model)
{
  ...
  const double cpu_cost = cost_model->key_compare_cost(num_elements * log((double)num_buffers) / M_LN2);
                                      ...
                                      const Cost_model_table *cost_model)
{
  ...
  total_cost = num_buffers * cost_model->key_compare_cost(num_keys_per_buffer * log(1.0 + num_keys_per_buffer)) +
  ...
  while (num_buffers >= MERGEBUFF2)
  {
    ...
    total_cost += num_merge_calls * get_merge_cost(num_keys_per_buffer * MERGEBUFF, MERGEBUFF, elem_size, cost_model);
    ...
    total_cost += get_merge_cost(last_n_elems, 1 + num_remaining_buffs, elem_size, cost_model);
  ...
  total_cost += get_merge_cost(last_n_elems, 1 + num_buffers, elem_size, cost_model);
  return total_cost;
}
...
uchar *Filesort_buffer::alloc_sort_buffer(uint num_records, uint record_length)
{
  ...
  if (m_rawmem != NULL)
  {
    if (num_records != m_num_records || record_length != m_record_length)
      free_sort_buffer();
  }

  m_size_in_bytes = ALIGN_SIZE(num_records * (record_length + sizeof(uchar *)));
  if (m_rawmem == NULL)
    m_rawmem = (uchar *)my_malloc(key_memory_Filesort_buffer_sort_keys, m_size_in_bytes, MYF(0));
  if (m_rawmem == NULL)
  {
    m_size_in_bytes = 0;
    return NULL;
...
void Filesort_buffer::sort_buffer(const Sort_param *param, uint count)
{
  ...
  if (radixsort_is_appliccable(count, param->sort_length) && try_reserve(&buffer, count))
  {
  ...
  */
  if (count <= 100)
  {
    ...
    {
      std::sort(m_sort_keys, m_sort_keys + count, Mem_compare(param->sort_length));
      return;
    }
    std::sort(m_sort_keys, m_sort_keys + count, Mem_compare_longkey(param->sort_length));
    return;
  }
  // Heuristics here: avoid function overhead call for short keys.
  if (param->sort_length < 10)
  {
    std::stable_sort(m_sort_keys, m_sort_keys + count, Mem_compare(param->sort_length));
    return;
  }
  std::stable_sort(m_sort_keys, m_sort_keys + count, Mem_compare_longkey(param->sort_length));
}
