...
static const char *mysql_ha_hash_get_key(TABLE_LIST *tables, size_t *key_len_p, my_bool first MY_ATTRIBUTE((unused)))
{
...
bool Sql_cmd_handler_open::execute(THD *thd)
{
  ...
  if (!my_hash_inited(&thd->handler_tables_hash))
  {
    ...
    */
    if (my_hash_init(&thd->handler_tables_hash, &my_charset_latin1, HANDLER_TABLES_HASH_SIZE, 0, 0,
                     (my_hash_get_key)mysql_ha_hash_get_key, (my_hash_free_key)mysql_ha_hash_free, 0,
  ...
  else
  {
    ...
    */
    if (my_hash_search(&thd->handler_tables_hash, (uchar *)tables->alias, strlen(tables->alias) + 1))
    {
  ...
  if (!(my_multi_malloc(key_memory_THD_handler_tables_hash, MYF(MY_WME), &hash_tables, sizeof(*hash_tables), &db, dblen,
                        &name, namelen, &alias, aliaslen, NullS)))
  {
    DBUG_PRINT("exit", ("ERROR"));
    DBUG_RETURN(TRUE);
  }
  ...
  *hash_tables = *tables;
  ...
  memcpy(const_cast<char *>(hash_tables->table_name), tables->table_name, namelen);
  ...
  */
  MDL_REQUEST_INIT(&hash_tables->mdl_request, MDL_key::TABLE, db, name, MDL_SHARED, MDL_TRANSACTION);
  ...
  if (open_temporary_tables(thd, hash_tables) ||
      check_table_access(thd, SELECT_ACL, hash_tables, FALSE, UINT_MAX, FALSE) || mysql_ha_open_table(thd, hash_tables))
...
static bool mysql_ha_open_table(THD *thd, TABLE_LIST *hash_tables)
{
  ...
  */
  ...
  if (!error && !(hash_tables->table->file->ha_table_flags() & HA_CAN_SQL_HANDLER))
  {
  ...
  {
    thd->mdl_context.set_lock_duration(hash_tables->mdl_request.ticket, MDL_EXPLICIT);
  ...
  */
  assert(hash_tables->table->next == NULL || hash_tables->table->s->tmp_table);
  /*
...
bool Sql_cmd_handler_close::execute(THD *thd)
{
  ...
  if ((hash_tables =
           (TABLE_LIST *)my_hash_search(&thd->handler_tables_hash, (uchar *)tables->alias, strlen(tables->alias) + 1)))
  {
...
bool Sql_cmd_handler_read::execute(THD *thd)
{
  ...
  */
  ...
  list.push_front(new Item_asterisk(&select_lex->context, NULL, NULL));
...
retry:
           ...
           (TABLE_LIST *)my_hash_search(&thd->handler_tables_hash, (uchar *)tables->alias, strlen(tables->alias) + 1)))
  {
    ...
    DBUG_PRINT("info-in-hash", ("'%s'.'%s' as '%s' table: 0x%lx", hash_tables->db, hash_tables->table_name,
  ...
  */
  ...
  if (hash_tables->table->s->tmp_table == NO_TMP_TABLE)
  {
    ...
    MDL_REQUEST_INIT_BY_KEY(&read_request, &hash_tables->mdl_request.key, MDL_SHARED_READ, MDL_TRANSACTION);

    thd->push_internal_handler(&sql_handler_lock_error);

    error = thd->mdl_context.acquire_lock(&read_request, thd->variables.lock_wait_timeout);
  ...
  */
  ...
  if (m_key_name)
  {
    keyno = find_type((char *)m_key_name, &table->s->keynames, FIND_TYPE_NO_PREFIX) - 1;
    if (keyno < 0)
    {
  ...
  if (insert_fields(thd, &select_lex->context, tables->db, tables->alias, &it, 0))
    goto err;

  thd->send_result_metadata(&list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF);
  ...
  for (num_rows = 0; num_rows < select_limit_cnt;)
  {
    switch (mode)
    {
      ...
      case RFIRST:
        if (m_key_name)
        {
          if (!(error = table->file->ha_index_or_rnd_end()) && !(error = table->file->ha_index_init(keyno, 1)))
            error = table->file->ha_index_first(table->record[0]);
        }
        else
        {
          if (!(error = table->file->ha_index_or_rnd_end()) && !(error = table->file->ha_rnd_init(1)))
            error = table->file->ha_rnd_next(table->record[0]);
        }
        mode = RNEXT;
        break;
      ...
      case RKEY:
      {
        ...
        for (keypart_map = key_len = 0; (item = it_ke++); key_part++)
        {
          ...
          type_conversion_status conv_status = item->save_in_field(key_part->field, true);
        ...
        if (!(error = table->file->ha_index_init(keyno, 1)))
          error = table->file->ha_index_read_map(table->record[0], key, keypart_map, m_rkey_mode);
  ...
  for (uint i = 0; i < thd->handler_tables_hash.records; i++)
  {
    ...
    for (tables = first; tables; tables = tables->next_local)
    {
      ...
      if ((!*tables->get_db_name() ||
           !my_strcasecmp(&my_charset_latin1, hash_tables->get_db_name(), tables->get_db_name())) &&
          !my_strcasecmp(&my_charset_latin1, hash_tables->get_table_name(), tables->get_table_name()))
        break;
    }
    if (tables)
    {
