...
class injector
{
  ...
  */
  ...
  class transaction
  {
    ...
    */
    class table
    {
     public:
      class save_sets
      {
       public:
        save_sets(table const &tbl, MY_BITMAP const *new_rs, MY_BITMAP const *new_ws)
            : m_table(tbl.get_table()), save_read_set(m_table->read_set), save_write_set(m_table->write_set)
        {
          m_table->column_bitmaps_set_no_signal(const_cast<MY_BITMAP *>(new_rs), const_cast<MY_BITMAP *>(new_ws));
        }

        ~save_sets() { m_table->column_bitmaps_set_no_signal(save_read_set, save_write_set); }
      ...
      table(TABLE *table, bool is_transactional) : m_table(table), m_is_transactional(is_transactional) {}
    ...
    */
    int update_row(server_id_type sid, table tbl, MY_BITMAP const *before_cols, MY_BITMAP const *after_cols,
                   size_t colcnt, record_type before, record_type after, const uchar *extra_row_info);
    int update_row(server_id_type sid, table tbl, MY_BITMAP const *cols, size_t colcnt, record_type before,
                   record_type after);
  ...
  int record_incident(THD *, binary_log::Incident_event::enum_incident incident, LEX_STRING const message);
