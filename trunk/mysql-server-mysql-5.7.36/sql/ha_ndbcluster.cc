...
static MYSQL_THDVAR_UINT(autoincrement_prefetch_sz, /* name */
                         PLUGIN_VAR_RQCMDARG, "Specify number of autoincrement values that are prefetched.",
                         NULL,  /* check func. */
                         NULL,  /* update func. */
                         1,     /* default */
                         1,     /* min */
...
static MYSQL_THDVAR_BOOL(force_send, /* name */
                         PLUGIN_VAR_OPCMDARG,
...
static MYSQL_THDVAR_BOOL(use_exact_count, /* name */
                         PLUGIN_VAR_OPCMDARG,
...
static MYSQL_THDVAR_BOOL(use_transactions, /* name */
                         PLUGIN_VAR_OPCMDARG,
...
static MYSQL_THDVAR_BOOL(use_copying_alter_table, /* name */
                         PLUGIN_VAR_OPCMDARG,
...
static MYSQL_THDVAR_UINT(optimized_node_selection, /* name */
                         PLUGIN_VAR_OPCMDARG, "Select nodes for transactions in a more optimal way.",
                         NULL, /* check func. */
                         NULL, /* update func. */
                         3,    /* default */
                         0,    /* min */
...
static MYSQL_THDVAR_ULONG(batch_size,                                        /* name */
                          PLUGIN_VAR_RQCMDARG, "Batch size in bytes.", NULL, /* check func. */
                          NULL,                                              /* update func. */
                          32768,                                             /* default */
                          0,                                                 /* min */
...
static MYSQL_THDVAR_ULONG(optimization_delay, /* name */
                          PLUGIN_VAR_RQCMDARG,
...
static MYSQL_THDVAR_BOOL(index_stat_enable,                                                            /* name */
                         PLUGIN_VAR_OPCMDARG, "Use ndb index statistics in query optimization.", NULL, /* check func. */
                         NULL,                         /* update func. */
...
static MYSQL_THDVAR_UINT(deferred_constraints, /* name */
                         PLUGIN_VAR_RQCMDARG, "Specified that constraints should be checked deferred (when supported)",
                         NULL, /* check func */
                         NULL, /* update func */
                         0,    /* default */
                         0,    /* min */
...
static MYSQL_THDVAR_BOOL(show_foreign_key_mock_tables, /* name */
                         PLUGIN_VAR_OPCMDARG,
...
static MYSQL_THDVAR_BOOL(join_pushdown,                                                         /* name */
                         PLUGIN_VAR_OPCMDARG, "Enable pushing down of join to datanodes", NULL, /* check func. */
                         NULL,                                                                  /* update func. */
...
static bool ndbcluster_show_status(handlerton *hton, THD *, stat_print_fn *, enum ha_stat_type);

static int ndbcluster_get_tablespace(THD *thd, LEX_CSTRING db_name, LEX_CSTRING table_name,
                                     LEX_CSTRING *tablespace_name);
static int ndbcluster_alter_tablespace(handlerton *hton, THD *thd, st_alter_tablespace *info);
static int ndbcluster_fill_files_table(handlerton *hton, THD *thd, TABLE_LIST *tables, Item *cond);
 ...
 */
static int ndbcluster_fill_is_table(handlerton *hton, THD *thd, TABLE_LIST *tables, Item *cond,
...
static handler *ndbcluster_create_handler(handlerton *hton, TABLE_SHARE *table, MEM_ROOT *mem_root)
{
...
{
  return (HA_CAN_UPDATE_PARTITION_KEY | HA_CAN_PARTITION_UNIQUE | HA_USE_AUTO_PARTITION);
}
...
static uchar *ndbcluster_get_key(NDB_SHARE *share, size_t *length, my_bool);

static void modify_shared_stats(NDB_SHARE *share, Ndb_local_table_statistics *local_stat);

static int ndb_get_table_statistics(THD *thd, ha_ndbcluster *, bool, Ndb *, const NdbRecord *, struct Ndb_statistics *,
                                    uint part_id = ~(uint)0);
...
static int check_slave_state(THD *thd)
{
  ...
  if (unlikely(runId != g_ndb_slave_state.sql_run_id))
  {
     ...
     */
    {
      ...
      */
      DBUG_PRINT("info", ("Loading applied epoch information from %s", NDB_APPLY_TABLE));
      NdbError ndb_error;
      ...
      do
      {
        ...
        do
        {
          ...
          while (0 == (rc = sop->nextResult(true)))
          {
            ...
            if ((serverid == ::server_id) || (ndb_mi_get_ignore_server_id(serverid)))
            {
      ...
      */
      ...
      sql_print_information("NDB Slave : MaxReplicatedEpoch set to %llu (%u/%u) at Slave start",
                            g_ndb_slave_state.max_rep_epoch, (Uint32)(g_ndb_slave_state.max_rep_epoch >> 32),
...
static int update_status_variables(Thd_ndb *thd_ndb, st_ndb_status *ns, Ndb_cluster_connection *c)
{
  ...
  if (ns->cluster_node_id != (int)c->node_id())
  {
    ...
    if (&g_ndb_status == ns && g_ndb_cluster_connection == c)
      sql_print_information("NDB: NodeID is %lu, management server '%s:%lu'", ns->cluster_node_id, ns->connected_host,
                            ns->connected_port);
  }
...
#define NDBAPI_COUNTERS(NAME_SUFFIX, ARRAY_LOCATION)                                                                   \
  {"api_wait_exec_complete_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::WaitExecCompleteCount], SHOW_LONGLONG,      \
   SHOW_SCOPE_GLOBAL},                                                                                                 \
      {"api_wait_scan_result_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::WaitScanResultCount], SHOW_LONGLONG,      \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_wait_meta_request_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::WaitMetaRequestCount], SHOW_LONGLONG,    \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_wait_nanos_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::WaitNanosCount], SHOW_LONGLONG,                 \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_bytes_sent_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::BytesSentCount], SHOW_LONGLONG,                 \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_bytes_received_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::BytesRecvdCount], SHOW_LONGLONG,            \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_trans_start_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::TransStartCount], SHOW_LONGLONG,               \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_trans_commit_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::TransCommitCount], SHOW_LONGLONG,             \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_trans_abort_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::TransAbortCount], SHOW_LONGLONG,               \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_trans_close_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::TransCloseCount], SHOW_LONGLONG,               \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_pk_op_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::PkOpCount], SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},       \
      {"api_uk_op_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::UkOpCount], SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},       \
      {"api_table_scan_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::TableScanCount], SHOW_LONGLONG,                 \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_range_scan_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::RangeScanCount], SHOW_LONGLONG,                 \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_pruned_scan_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::PrunedScanCount], SHOW_LONGLONG,               \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_scan_batch_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::ScanBatchCount], SHOW_LONGLONG,                 \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_read_row_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::ReadRowCount], SHOW_LONGLONG, SHOW_SCOPE_GLOBAL}, \
      {"api_trans_local_read_row_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::TransLocalReadRowCount],              \
       SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},                                                                              \
      {"api_adaptive_send_forced_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::ForcedSendsCount], SHOW_LONGLONG,     \
       SHOW_SCOPE_GLOBAL},                                                                                             \
      {"api_adaptive_send_unforced_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::UnforcedSendsCount], SHOW_LONGLONG, \
       SHOW_SCOPE_GLOBAL},                                                                                             \
  {                                                                                                                    \
    "api_adaptive_send_deferred_count" NAME_SUFFIX, (char *)ARRAY_LOCATION[Ndb::DeferredSendsCount], SHOW_LONGLONG,    \
        SHOW_SCOPE_GLOBAL                                                                                              \
  }
...
SHOW_VAR ndb_status_variables_dynamic[] = {
    ...
    {"number_of_ready_data_nodes", (char *)&g_ndb_status.number_of_ready_data_nodes, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    ...
    {"pushed_queries_defined", (char *)&g_ndb_status.pushed_queries_defined, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"pushed_queries_dropped", (char *)&g_ndb_status.pushed_queries_dropped, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"pushed_queries_executed", (char *)&g_ndb_status.pushed_queries_executed, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"pushed_reads", (char *)&g_ndb_status.pushed_reads, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"last_commit_epoch_server", (char *)&g_ndb_status.last_commit_epoch_server, SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"last_commit_epoch_session", (char *)&g_ndb_status.last_commit_epoch_session, SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
...
SHOW_VAR ndb_status_server_client_stat_variables[] = {
    ...
    {"api_event_data_count", (char *)&g_server_api_client_stats[Ndb::DataEventsRecvdCount], SHOW_LONGLONG,
     SHOW_SCOPE_GLOBAL},
    {"api_event_nondata_count", (char *)&g_server_api_client_stats[Ndb::NonDataEventsRecvdCount], SHOW_LONGLONG,
     SHOW_SCOPE_GLOBAL},
    {"api_event_bytes_count", (char *)&g_server_api_client_stats[Ndb::EventBytesRecvdCount], SHOW_LONGLONG,
     SHOW_SCOPE_GLOBAL},
    {NullS, NullS, SHOW_LONG, SHOW_SCOPE_GLOBAL}};
...

  {
    ...
    */
    if (ndberr->status == NdbError::TemporaryError)
      push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_GET_TEMPORARY_ERRMSG, ER(ER_GET_TEMPORARY_ERRMSG),
                          ndberr->code, ndberr->message, "NDB");
    else
      push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_GET_ERRMSG, ER(ER_GET_ERRMSG), ndberr->code,
                          ndberr->message, "NDB");
  }
  return error;
}
...
static int handle_conflict_op_error(NdbTransaction *trans, const NdbError &err, const NdbOperation *op);

static int handle_row_conflict(NDB_CONFLICT_FN_SHARE *cfn_share, const char *tab_name, bool table_has_blobs,
                               const char *handling_type, const NdbRecord *key_rec, const NdbRecord *data_rec,
                               const uchar *old_row, const uchar *new_row, enum_conflicting_op_type op_type,
                               enum_conflict_cause conflict_cause, const NdbError &conflict_error,
                               NdbTransaction *conflict_trans, const MY_BITMAP *write_set, Uint64 transaction_id);
#endif

static const Uint32 error_op_after_refresh_op = 920;

static inline int check_completed_operations_pre_commit(Thd_ndb *thd_ndb, NdbTransaction *trans,
                                                        const NdbOperation *first, const NdbOperation *last,
                                                        uint *ignore_count)
{
  uint ignores = 0;
  ...
  */
  ...
  while (true)
  {
    ...
    if (!op_has_conflict_detection)
    {
      ...
      if (err.classification != NdbError::NoError && err.classification != NdbError::ConstraintViolation &&
  ...
  */
  if (trans->getLastDefinedOperation() != lastUserOp)
  {
    ...
    if (trans->execute(NdbTransaction::NoCommit, NdbOperation::AO_IgnoreError, thd_ndb->m_force_send))
    {
    ...
    if (nonMaskedError.code != 0)
    {
      ...
      else
      {
        ...
        push_warning_printf(current_thd, Sql_condition::SL_ERROR, ER_EXCEPTIONS_WRITE_ERROR,
...
static inline int check_completed_operations(Thd_ndb *thd_ndb, NdbTransaction *trans, const NdbOperation *first,
  ...
  */
  while (true)
  {
        ...
        err.classification != NdbError::NoDataFound)
    {
      ...
      assert((err.code != (int)error_conflict_fn_violation) && (err.code != (int)error_op_after_refresh_op));
#endif
      DBUG_RETURN(err.code);
    }
    if (err.classification != NdbError::NoError)
...
static inline int execute_no_commit(Thd_ndb *thd_ndb, NdbTransaction *trans, bool ignore_no_key, uint *ignore_count = 0)
{
  ...
  do
  {
    ...
    rc = check_completed_operations_pre_commit(thd_ndb, trans, first, last, ignore_count);
  } while (0);
...
static inline int execute_commit(Thd_ndb *thd_ndb, NdbTransaction *trans, int force_send, int ignore_error,
                                 uint *ignore_count = 0)
{
  ...
  do
  {
    ...
    rc = check_completed_operations(thd_ndb, trans, first, last, ignore_count);
  } while (0);
...
static inline int execute_no_commit_ie(Thd_ndb *thd_ndb, NdbTransaction *trans)
{
  DBUG_ENTER("execute_no_commit_ie");
  ha_ndbcluster::release_completed_operations(trans);
  int res = trans->execute(NdbTransaction::NoCommit, NdbOperation::AO_IgnoreError, thd_ndb->m_force_send);
  thd_ndb->m_unsent_bytes = 0;
  thd_ndb->m_execute_count++;
  DBUG_PRINT("info", ("execute_count: %u", thd_ndb->m_execute_count));
  DBUG_RETURN(res);
...
static uchar *thd_ndb_share_get_key(THD_NDB_SHARE *thd_ndb_share, size_t *length,
      ...
      m_last_commit_epoch_session(0)
{
  ...
  m_error = FALSE;
  options = 0;
  (void)my_hash_init(&open_tables, table_alias_charset, 5, 0, 0, (my_hash_get_key)thd_ndb_share_get_key, 0, 0,
                     PSI_INSTRUMENT_ME);
  m_unsent_bytes = 0;
  m_execute_count = 0;
  m_scan_count = 0;
  ...
  init_alloc_root(PSI_INSTRUMENT_ME, &m_batch_mem_root, BATCH_FLUSH_SIZE / 4, 0);
}
...
Thd_ndb::~Thd_ndb()
{
  if (opt_ndb_extra_logging > 1)
  {
    ...
    */
    for (int i = 0; i < MAX_NDB_NODES; i++)
    {
      if (m_transaction_hint_count[i] > 0 || m_transaction_no_hint_count[i] > 0)
      {
        sql_print_information(
            "tid %u: node[%u] "
            "transaction_hint=%u, transaction_no_hint=%u",
            m_thd->thread_id(), i, m_transaction_hint_count[i], m_transaction_no_hint_count[i]);
      }
    }
  }
  if (ndb)
  ...
  for (uint i = 0; i < table_share->keys; i++)
  {
    ...
    switch (get_index_type(i))
    {
      ...
      case ORDERED_INDEX:
        ...
        {
          THD *thd = current_thd;
          const bool index_stat_enable = THDVAR(NULL, index_stat_enable) && THDVAR(thd, index_stat_enable);
          if (index_stat_enable)
          {
            ...
            {
              push_warning_printf(thd, Sql_condition::SL_WARNING, ER_CANT_GET_STAT, /* pun? */
...
int ha_ndbcluster::records(ha_rows *num_rows)
{
  ...
  DBUG_PRINT("info",
             ("id=%d, no_uncommitted_rows_count=%d", m_table->getTableId(), m_table_info->no_uncommitted_rows_count));
...
void ha_ndbcluster::no_uncommitted_rows_update(int c)
{
  ...
  DBUG_PRINT("info",
             ("id=%d, no_uncommitted_rows_count=%d", m_table->getTableId(), local_info->no_uncommitted_rows_count));
  DBUG_VOID_RETURN;
}
...
int ha_ndbcluster::ndb_err(NdbTransaction *trans)
{
  ...
  if (res == HA_ERR_FOUND_DUPP_KEY)
  {
    ...
    {
      if (m_index[i].type == UNIQUE_INDEX || m_index[i].type == UNIQUE_ORDERED_INDEX)
      {
        const NDBINDEX *unique_index = (const NDBINDEX *)m_index[i].unique_index;
...
/*
  ...
  bytes are unused.
*/
static uint32 field_used_length(const Field *field)
{
...
int g_get_ndb_blobs_value(NdbBlob *ndb_blob, void *arg)
{
  ...
  if (isNull == 0)
  {
    ...
    DBUG_PRINT("info", ("Blob number %d needs size %llu, total buffer reqt. now %llu", ha->m_blob_counter, len64,
  ...
  */
  ...
  if (ha->m_blobs_row_total_size > ha->m_blobs_buffer_size)
  {
    ...
    DBUG_PRINT("info", ("allocate blobs buffer size %u", (uint32)(ha->m_blobs_row_total_size)));
    ...
    ha->m_blobs_buffer = (uchar *)my_malloc(PSI_INSTRUMENT_ME, (size_t)ha->m_blobs_row_total_size, MYF(MY_WME));
  ...
  */
  ...
  for (uint i = 0; i < ha->table->s->fields; i++)
  {
    ...
    if (isNull == 0)
    {
      ...
      DBUG_PRINT("info", ("[%u] offset: %u  buf: 0x%lx  len=%u", i, offset, (long)buf, len));
      assert(len == len64);
      if (ha->m_blob_destination_record)
      {
        my_ptrdiff_t ptrdiff = ha->m_blob_destination_record - ha->table->record[0];
        field_blob->move_field_offset(ptrdiff);

        if (len > field_blob->max_data_length())
        {
          len = calc_ndb_blob_len(field_blob->charset(), buf, field_blob->max_data_length());

          // push a warning
          push_warning_printf(current_thd, Sql_condition::SL_WARNING, WARN_DATA_TRUNCATED,
...
/*
  ...
  unpack_record().
*/
int ha_ndbcluster::get_blob_values(const NdbOperation *ndb_op, uchar *dst_record, const MY_BITMAP *bitmap)
{
  uint i;
  ...
  ndb_op->getNdbTransaction()->setMaxPendingBlobReadBytes(THDVAR(current_thd, blob_read_batch_bytes));
  ...
  for (i = 0; i < table_share->fields; i++)
  {
    ...
    {
      if ((ndb_blob = ndb_op->getBlobHandle(i)) == NULL || ndb_blob->setActiveHook(g_get_ndb_blobs_value, this) != 0)
        DBUG_RETURN(1);
...
int ha_ndbcluster::set_blob_values(const NdbOperation *ndb_op, my_ptrdiff_t row_offset, const MY_BITMAP *bitmap,
                                   uint *set_count, bool batch)
{
  ...
  *set_count = 0;
  ...
  ndb_op->getNdbTransaction()->setMaxPendingBlobWriteBytes(THDVAR(current_thd, blob_write_batch_bytes));
  ...
  do
  {
    ...
    else
    {
      ...
      DBUG_PRINT("value", ("set blob ptr: 0x%lx  len: %u", (long)blob_ptr, blob_len));
...
{
  bool ret =
      (mysql_type != MYSQL_TYPE_BLOB && mysql_type != MYSQL_TYPE_TINY_BLOB && mysql_type != MYSQL_TYPE_MEDIUM_BLOB &&
       mysql_type != MYSQL_TYPE_LONG_BLOB && mysql_type != MYSQL_TYPE_JSON && mysql_type != MYSQL_TYPE_GEOMETRY);
  ...
  */
  ...
  if (ndbtab->hasDefaultValues())
  {
    ...
    for (uint f = 0; f < table_share->fields; f++)
    {
      ...
      if ((!(field->flags & (PRI_KEY_FLAG | NO_DEFAULT_VALUE_FLAG))) && type_supports_default_value(field->real_type()))
      {
        /* We expect Ndb to have a native default for this
         * column
         */
        my_ptrdiff_t src_offset = table_share->default_values - field->table->record[0];
        ...
        {
          sql_print_error(
              "NDB Internal error: Default values differ "
      ...
      else
      {
        ...
        if (unlikely(ndbCol->getDefaultValue() != NULL))
        {
          /* Didn't expect that */
          sql_print_error(
              "NDB Internal error: Column %u has native "
      ...
      if (unlikely(!defaults_aligned))
      {
        ...
        sql_print_error(
            ...
            field->field_name, field->type(), field->real_type(), field->flags, field->is_null());
        // Dump ndbCol
        sql_print_error(
            "ndbCol[name: '%s', type: %u, column_no: %d, "
            "nullable: %d]",
            ndbCol->getName(), ndbCol->getType(), ndbCol->getColumnNo(), ndbCol->getNullable());
        break;
      }
    }
...
int ha_ndbcluster::get_metadata(THD *thd, const char *path)
{
  ...
  */
  ...
  if (get_ndb_share_state(m_share) != NSS_ALTERED && cmp_frm(tab, pack_data, pack_length))
  {
    DBUG_PRINT("error", ("metadata, pack_length: %lu  getFrmLength: %d  memcmp: %d", (ulong)pack_length,
                         tab->getFrmLength(), memcmp(pack_data, tab->getFrmData(), pack_length)));
  ...
  */
  if (table_share->mysql_version < 50120 && !table_share->tablespace /* safety */)
  {
    Uint32 id;
    if (tab->getTablespace(&id))
    {
      ...
      if (ndberr.classification == NdbError::NoError)
      {
        ...
        if (tablespace_len != 0)
        {
          ...
          table_share->tablespace = strmake_root(&table_share->mem_root, tablespace, tablespace_len);
        }
      }
    }
  }
...
static int fix_unique_index_attr_order(NDB_INDEX_DATA &data, const NDBINDEX *index, KEY *key_info)
{
  ...
  {
    sql_print_error("fix_unique_index_attr_order: my_malloc(%u) failure", (unsigned int)sz);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
...
static void ndb_protect_char(const char *from, char *to, uint to_length, char protect)
{
  uint fpos = 0, tpos = 0;
 ...
 */
int ha_ndbcluster::add_index_handle(THD *thd, NDBDICT *dict, KEY *key_info, const char *key_name, uint index_no)
{
  ...
  if (idx_type != PRIMARY_KEY_INDEX && idx_type != UNIQUE_INDEX)
  {
    ...
    do
    {
      ...
      if (!index)
        ERR_RETURN(dict->getNdbError());
      DBUG_PRINT("info",
                 ("index: 0x%lx  id: %d  version: %d.%d  status: %d", (long)index, index->getObjectId(),
                  index->getObjectVersion() & 0xFFFFFF, index->getObjectVersion() >> 24, index->getObjectStatus()));
...
static void ndb_set_record_specification(uint field_no, NdbDictionary::RecordSpecification *spec, const TABLE *table,
                                         const NdbDictionary::Table *ndb_table)
{
  ...
  if (table->field[field_no]->real_maybe_null())
  {
    ...
    spec->nullbit_bit_in_byte = null_bit_mask_to_bit_number(table->field[field_no]->null_bit);
  }
  ...
  spec->column_flags = 0;
  if (table->field[field_no]->type() == MYSQL_TYPE_STRING && table->field[field_no]->pack_length() == 0)
  {
    /*
      This is CHAR(0), which we represent as
      a nullable BIT(1) column where we ignore the data bit
    */
    spec->column_flags |= NdbDictionary::RecordSpecification::BitColMapsNullBitOnly;
  }
}
...
int ha_ndbcluster::add_index_ndb_record(NDBDICT *dict, KEY *key_info, uint index_no)
{
  ...
  if (m_index[index_no].index)
  {
    ...
    */
    rec = dict->createRecord(m_index[index_no].index, m_table, spec, key_info->user_defined_key_parts, sizeof(spec[0]),
                             (NdbDictionary::RecMysqldShrinkVarchar | NdbDictionary::RecMysqldBitfield));
    if (!rec)
      ERR_RETURN(dict->getNdbError());
  ...
  {
    rec =
        dict->createRecord(m_index[index_no].unique_index, m_table, spec, key_info->user_defined_key_parts,
                           sizeof(spec[0]), (NdbDictionary::RecMysqldShrinkVarchar | NdbDictionary::RecMysqldBitfield));
    if (!rec)
      ERR_RETURN(dict->getNdbError());
  ...
  else if (index_no == table_share->primary_key)
  {
    ...
    rec = dict->createRecord(m_table, spec, key_info->user_defined_key_parts, sizeof(spec[0]),
                             (NdbDictionary::RecMysqldShrinkVarchar | NdbDictionary::RecMysqldBitfield));
    if (!rec)
      ERR_RETURN(dict->getNdbError());
    m_index[index_no].ndb_unique_record_key = rec;
  ...
  if (m_index[index_no].unique_index)
  {
    rec = dict->createRecord(m_index[index_no].unique_index, m_table, spec, key_info->user_defined_key_parts,
...
*/
int ha_ndbcluster::open_indexes(THD *thd, Ndb *ndb, TABLE *tab, bool ignore_error)
{
  uint i;
  int error = 0;
  NDBDICT *dict = ndb->getDictionary();
...
void ha_ndbcluster::renumber_indexes(Ndb *ndb, TABLE *tab)
{
  ...
  for (i = 0; i < tab->s->keys; i++, key_info++, key_name++)
  {
    ...
    {
      DBUG_PRINT("info", ("Shifting index %s(%i) out of the list", index_name, i));
      NDB_INDEX_DATA tmp;
      uint j = i + 1;
...
{
  return get_index_type_from_key(inx, table_share->key_info, inx == table_share->primary_key);
}

NDB_INDEX_TYPE ha_ndbcluster::get_index_type_from_key(uint inx, KEY *key_info, bool primary) const
{
  bool is_hash_index = (key_info[inx].algorithm == HA_KEY_ALG_HASH);
  if (primary)
    return is_hash_index ? PRIMARY_KEY_INDEX : PRIMARY_KEY_ORDERED_INDEX;

  return ((key_info[inx].flags & HA_NOSAME) ? (is_hash_index ? UNIQUE_INDEX : UNIQUE_ORDERED_INDEX) : ORDERED_INDEX);
}
...
*/
static inline NdbOperation::LockMode get_ndb_lock_mode(enum thr_lock_type type)
{
...
bool ha_ndbcluster::primary_key_is_clustered() const
{
  ...
  */
  const ndb_index_type idx_type = get_index_type_from_table(table->s->primary_key);
  return (idx_type == PRIMARY_KEY_ORDERED_INDEX || idx_type == UNIQUE_ORDERED_INDEX || idx_type == ORDERED_INDEX);
}
...
int ha_ndbcluster::pk_read(const uchar *key, uint key_len, uchar *buf, uint32 *part_id)
{
  NdbConnection *trans = m_thd_ndb->trans;
  int res;
  DBUG_ENTER("pk_read");
  DBUG_PRINT("enter", ("key_len: %u read_set=%x", key_len, table->read_set->bitmap[0]));
  ...
  if (check_if_pushable(NdbQueryOperationDef::PrimaryKeyAccess, table->s->primary_key))
  {
    ...
    const int error =
        pk_unique_index_read_key_pushed(table->s->primary_key, key, (m_user_defined_partitioning ? part_id : NULL));
    if (unlikely(error))
      DBUG_RETURN(error);

    assert(m_active_query != NULL);
    if ((res = execute_no_commit_ie(m_thd_ndb, trans)) != 0 || m_active_query->getNdbError().code)
    {
  ...
  else
  {
    ...
    if ((res = execute_no_commit_ie(m_thd_ndb, trans)) != 0 || op->getNdbError().code)
    {
      table->status = STATUS_NOT_FOUND;
      DBUG_RETURN(ndb_err(trans));
    }
...
int ha_ndbcluster::ndb_pk_update_row(THD *thd, const uchar *old_data, uchar *new_data)
{
  ...
  */
  if (table->found_next_number_field && bitmap_is_set(table->write_set, table->found_next_number_field->field_index) &&
  ...
  */
  ...
  if (error)
  {
    ...
    if (trans->commitStatus() == NdbConnection::Started)
    {
      ...
      if (undo_res)
        push_warning(table->in_use, Sql_condition::SL_WARNING, undo_res,
...
*/
bool ha_ndbcluster::check_all_operations_for_error(NdbTransaction *trans, const NdbOperation *first,
...
static int check_null_in_record(const KEY *key_info, const uchar *record)
{
  ...
  {
    if (curr_part->null_bit && (record[curr_part->null_offset] & curr_part->null_bit))
      return 1;
    curr_part++;
  }
  return 0;
...
int ha_ndbcluster::peek_indexed_rows(const uchar *record, NDB_WRITE_OP write_op)
{
  ...
  if (write_op != NDB_UPDATE && table->s->primary_key != MAX_KEY)
  {
     ...
     */
    const NdbRecord *key_rec = m_index[table->s->primary_key].ndb_unique_record_row;
    ...
    if (!(op = trans->readTuple(key_rec, (const char *)record, m_ndb_record, dummy_row, lm, empty_mask, poptions,
  ...
  for (i = 0, key_info = table->key_info; i < table->s->keys; i++, key_info++)
  {
        ...
        bitmap_is_overlapping(table->write_set, m_key_fields[i]))
    {
      ...
      */
      ...
      if (!(iop = trans->readTuple(key_rec, (const char *)record, m_ndb_record, dummy_row, lm, empty_mask)))
        ERR_RETURN(trans->getNdbError());
...
int ha_ndbcluster::unique_index_read(const uchar *key, uint key_len, uchar *buf)
{
  NdbTransaction *trans = m_thd_ndb->trans;
  ...
  if (check_if_pushable(NdbQueryOperationDef::UniqueIndexAccess, active_index))
  {
    ...
    assert(m_active_query != NULL);
    if (execute_no_commit_ie(m_thd_ndb, trans) != 0 || m_active_query->getNdbError().code)
    {
...
int ha_ndbcluster::scan_handle_lock_tuple(NdbScanOperation *scanOp, NdbTransaction *trans)
{
  ...
  if (m_lock_tuple)
  {
    ...
    */
    ...
    if (!(scanOp->lockCurrentTuple(trans, m_ndb_record, dummy_row, empty_mask)))
    {
      m_lock_tuple = false;
      ERR_RETURN(trans->getNdbError());
    }
     ...
     */
    if ((m_lock.type >= TL_WRITE_ALLOW_WRITE) && ndb_log_exclusive_reads(current_thd))
    {
...
inline int ha_ndbcluster::fetch_next(NdbScanOperation *cursor)
{
  ...
  bool contact_ndb = m_lock.type < TL_WRITE_ALLOW_WRITE && m_lock.type != TL_READ_WITH_SHARED_LOCKS;
  do
  {
    ...
    */
    ...
    if ((local_check = cursor->nextResult(&_m_next_row, contact_ndb, m_thd_ndb->m_force_send)) == 0)
    {
      /*
    ...
    else if (local_check == 1 || local_check == 2)
    {
      ...
      /*
        ...
        be sent to NDB
      */
      DBUG_PRINT("info", ("thd_ndb->m_unsent_bytes: %ld", (long)m_thd_ndb->m_unsent_bytes));
...
int ha_ndbcluster::index_read_pushed(uchar *buf, const uchar *key, key_part_map keypart_map)
{
...
int ha_ndbcluster::log_exclusive_read(const NdbRecord *key_rec, const uchar *key, uchar *buf, Uint32 *ppartition_id)
{
  DBUG_ENTER("log_exclusive_read");
  NdbOperation::OperationOptions opts;
  opts.optionsPresent = NdbOperation::OperationOptions::OO_ABORTOPTION | NdbOperation::OperationOptions::OO_ANYVALUE;
  ...
  */
  ...
  const NdbOperation *markingOp = m_thd_ndb->trans->updateTuple(key_rec, (const char *)key, m_ndb_record, (char *)buf,
                                                                empty_mask, &opts, opts.size());
  if (!markingOp)
  {
    char msg[FN_REFLEN];
    my_snprintf(msg, sizeof(msg), "Error logging exclusive reads, failed creating markingOp, %u, %s\n",
                m_thd_ndb->trans->getNdbError().code, m_thd_ndb->trans->getNdbError().message);
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_EXCEPTIONS_WRITE_ERROR,
                        ER(ER_EXCEPTIONS_WRITE_ERROR), msg);
    /*
...
int ha_ndbcluster::scan_log_exclusive_read(NdbScanOperation *cursor, NdbTransaction *trans)
{
  ...
  */
  ...
  const NdbOperation *markingOp = cursor->updateCurrentTuple(trans, m_ndb_record, dummy_row, empty_mask, &opts,
                                                             sizeof(NdbOperation::OperationOptions));
  if (markingOp == NULL)
  {
    char msg[FN_REFLEN];
...
const NdbOperation *ha_ndbcluster::pk_unique_index_read_key(uint idx, const uchar *key, uchar *buf,
                                                            NdbOperation::LockMode lm, Uint32 *ppartition_id)
{
  ...
  ndb_index_type idx_type = (idx != MAX_KEY) ? get_index_type(idx) : UNDEFINED_INDEX;
  ...
  op =
      m_thd_ndb->trans->readTuple(key_rec, (const char *)key, m_ndb_record, (char *)buf, lm,
                                  (uchar *)(table->read_set->bitmap), poptions, sizeof(NdbOperation::OperationOptions));

  if (uses_blob_value(table->read_set) && get_blob_values(op, buf, table->read_set) != 0)
    DBUG_RETURN(NULL);
  ...
  if ((lm == NdbOperation::LM_Exclusive) &&
      ...
      */
      idx_type != UNDEFINED_INDEX && ndb_log_exclusive_reads(current_thd))
  {
...
int ha_ndbcluster::pk_unique_index_read_key_pushed(uint idx, const uchar *key, Uint32 *ppartition_id)
{
                          ...
                          const key_range *end_key, int from)
{
...
#ifndef NDEBUG
  DBUG_PRINT("info", ("key parts: %u length: %u", key_info->user_defined_key_parts, key_info->key_length));
  {
    for (uint j = 0; j <= 1; j++)
    {
      ...
      if (kr)
      {
        DBUG_PRINT("info", ("key range %u: length: %u map: %lx flag: %d", j, kr->length, kr->keypart_map, kr->flag));
  ...
  if (start_key)
  {
    ...
    bound.low_inclusive = start_key->flag != HA_READ_AFTER_KEY && start_key->flag != HA_READ_BEFORE_KEY;
  }
  else
  {
    bound.low_key = NULL;
  ...
  if (start_key && (start_key->flag == HA_READ_KEY_EXACT || start_key->flag == HA_READ_PREFIX_LAST) && from != 1)
  {
  ...
  else if (end_key)
  {
    ...
    */
    ...
    if (end_key->flag == HA_READ_KEY_EXACT || end_key->flag == HA_READ_PREFIX_LAST)
    {
  ...
  DBUG_PRINT("info", ("start_flag=%d end_flag=%d"
                      ...
                      start_key ? start_key->flag : 0, end_key ? end_key->flag : 0, bound.low_key_count,
                      bound.low_key_count ? bound.low_inclusive : 0, bound.high_key_count,
...
int ha_ndbcluster::ordered_index_scan(const key_range *start_key, const key_range *end_key, bool sorted,
                                      bool descending, uchar *buf, part_id_range *part_spec)
{
  NdbTransaction *trans;
  ...
  DBUG_PRINT("enter", ("index: %u, sorted: %d, descending: %d read_set=0x%x", active_index, sorted, descending,
                       table->read_set->bitmap[0]));
  ...
  if (start_key != NULL || end_key != NULL)
  {
    /*
       ...
       if descending
     */
    compute_index_bounds(bound, table->key_info + active_index, (descending ? end_key : start_key),
  ...
  else
  {
    ...
    /* Partition pruning */
    if (m_use_partition_pruning && m_user_defined_partitioning && part_spec != NULL &&
    ...
    if (!(op = trans->scanIndex(key_rec, row_rec, lm, (uchar *)(table->read_set->bitmap), pbound, &options,
    ...
    if (uses_blob_value(table->read_set) && get_blob_values(op, NULL, table->read_set) != 0)
...
static int guess_scan_flags(NdbOperation::LockMode lm, const NDBTAB *tab, const MY_BITMAP *readset)
{
  int flags = 0;
...
int ha_ndbcluster::full_table_scan(const KEY *key_info, const key_range *start_key, const key_range *end_key,
                                   uchar *buf)
{
  int error;
  NdbTransaction *trans = m_thd_ndb->trans;
  ...
  if (m_use_partition_pruning && m_user_defined_partitioning)
  {
    ...
    DBUG_PRINT("info", ("part_spec.start_part: %u  part_spec.end_part: %u", part_spec.start_part, part_spec.end_part));
    /*
    ...
    */
    ...
    if (part_spec.start_part == part_spec.end_part)
    {
      ...
      if (!trans)
        if (unlikely(!(trans = get_transaction_part_id(part_spec.start_part, error))))
          DBUG_RETURN(error);
    }
  }
  if (!trans)
  ...
  const NdbOperation::LockMode lm = get_ndb_lock_mode(m_lock.type);
  NdbScanOperation::ScanOptions options;
  options.optionsPresent = (NdbScanOperation::ScanOptions::SO_SCANFLAGS | NdbScanOperation::ScanOptions::SO_PARALLEL);
  ...
  else
  {
    ...
    else
    {
      ...
      if (m_cond->generate_scan_filter_from_key(&code, &options, key_info, start_key, end_key))
        ERR_RETURN(code.getNdbError());
    }

    if (!(op = trans->scanTable(m_ndb_record, lm, (uchar *)(table->read_set->bitmap), &options,
                                sizeof(NdbScanOperation::ScanOptions))))
      ERR_RETURN(trans->getNdbError());
...
class Ndb_tuple_id_range_guard
{
 ...
 public:
  Ndb_tuple_id_range_guard(NDB_SHARE *share) : m_share(share), range(share->tuple_id_range)
  {
...
void ha_ndbcluster::get_read_set(bool use_cursor, uint idx)
{
  const bool is_delete =
      table->in_use->lex->sql_command == SQLCOM_DELETE || table->in_use->lex->sql_command == SQLCOM_DELETE_MULTI;

  const bool is_update =
      table->in_use->lex->sql_command == SQLCOM_UPDATE || table->in_use->lex->sql_command == SQLCOM_UPDATE_MULTI;

  assert(use_cursor || idx == table_share->primary_key || table->key_info[idx].flags & HA_NOSAME);
  ...
  if (is_update && m_has_unique_index)
  {
    for (uint i = 0; i < table_share->keys; i++)
    {
      if ((table->key_info[i].flags & HA_NOSAME) && bitmap_is_overlapping(table->write_set, m_key_fields[i]))
      {
...
void ha_ndbcluster::get_hidden_fields_keyop(NdbOperation::OperationOptions *options, NdbOperation::GetValueSpec gets[2])
{
...
void ha_ndbcluster::get_hidden_fields_scan(NdbScanOperation::ScanOptions *options, NdbOperation::GetValueSpec gets[2])
{
  Uint32 num_gets = setup_get_hidden_fields(gets);
...
inline void ha_ndbcluster::eventSetAnyValue(THD *thd, NdbOperation::OperationOptions *options) const
{
  options->anyValue = 0;
...
/**
   ...
   is set to true.
*/
int ha_ndbcluster::prepare_conflict_detection(enum_conflicting_op_type op_type, const NdbRecord *key_rec,
                                              ...
                                              bool &conflict_handled, bool &avoid_ndbapi_write)
{
  ...
  */
  ...
  if (thd->binlog_row_event_extra_data)
  {
    ...
    if (extra_row_info.getFlags() & Ndb_binlog_extra_row_info::NDB_ERIF_TRANSID)
      transaction_id = extra_row_info.getTransactionId();

    if (extra_row_info.getFlags() & Ndb_binlog_extra_row_info::NDB_ERIF_CFT_FLAGS)
    {
      DBUG_PRINT("info", ("Slave : have conflict flags : %x\n", extra_row_info.getConflictFlags()));
      ...
      assert(!(op_is_marked_as_read && (op_is_marked_as_reflected || op_is_marked_as_refresh)));
    }
  }

  const st_conflict_fn_def *conflict_fn = (m_share->m_cfn_share ? m_share->m_cfn_share->m_conflict_fn : NULL);
...

  {
    ...
    int res =
        g_ndb_slave_state.atPrepareConflictDetection(m_table, key_rec, row_data, transaction_id, handle_conflict_now);
    if (res)
      DBUG_RETURN(res);
    ...
    if (handle_conflict_now)
    {
       ...
       */
      enum_conflicting_op_type conflicting_op = (op_type == UPDATE_ROW && op_is_marked_as_read) ? READ_ROW : op_type;
      /*
         Directly handle the conflict here - e.g refresh/ write to
         exceptions table etc.
      */
      res = handle_row_conflict(m_share->m_cfn_share, m_share->table_name, m_share->flags & NSF_BLOB_FLAG,
                                "Transaction", key_rec, data_rec, old_data, new_data, conflicting_op, TRANS_IN_CONFLICT,
  ...
  {
    sql_print_warning(
        "NDB Slave : Transactional conflict detection defined on table %s, but "
  ...
  */
  ...
  {
    res = conflict_fn->prep_func(m_share->m_cfn_share, op_type, m_ndb_record, old_data, new_data,
  ...
  */
  ...
  if (table->write_set)
  {
    ...
    ex_data.bitmap_buf = (my_bitmap_map *)get_buffer(m_thd_ndb, table->s->column_bitmap_size);
    ...
    bitmap_init(ex_data.write_set, ex_data.bitmap_buf, table->write_set->n_bits, false);
...
static int handle_conflict_op_error(NdbTransaction *trans, const NdbError &err, const NdbOperation *op)
{
  ...
  if ((err.code == (int)error_conflict_fn_violation) || (err.code == (int)error_op_after_refresh_op) ||
      (err.classification == NdbError::ConstraintViolation) || (err.classification == NdbError::NoDataFound))
  {
    DBUG_PRINT("info", ("err.code = %s, err.classification = %s",
                        ((err.code == (int)error_conflict_fn_violation)
                             ? "error_conflict_fn_violation"
                             : ((err.code == (int)error_op_after_refresh_op) ? "error_op_after_refresh_op" : "?")),
                        ((err.classification == NdbError::ConstraintViolation)
                             ? "ConstraintViolation"
                             : ((err.classification == NdbError::NoDataFound) ? "NoDataFound" : "?"))));
...
#ifndef NDEBUG
    const uchar *row = (ex_data.op_type == DELETE_ROW) ? ex_data.old_row : ex_data.new_row;
#endif
    ...
    if (ex_data.reflected_operation)
    {
       ...
       *   duplicate.
       */
      assert((err.code == (int)error_conflict_fn_violation) || (err.classification == NdbError::ConstraintViolation) ||
             (err.classification == NdbError::NoDataFound));
...

    {
       ...
       * SECONDARY.
       */
      bool secondary = cfn_share && cfn_share->m_conflict_fn && (cfn_share->m_conflict_fn->flags & CF_USE_ROLE_VAR) &&
    ...
    bool table_has_trans_conflict_detection =
        cfn_share && cfn_share->m_conflict_fn && (cfn_share->m_conflict_fn->flags & CF_TRANSACTIONAL);
    ...
    if (table_has_trans_conflict_detection)
    {
       ...
       * in the normal way
       */
      if (!((causing_op_type == DELETE_ROW) && (conflict_cause == ROW_DOES_NOT_EXIST)))
      {
    ...
    if (cfn_share)
    {
      ...
      int res = handle_row_conflict(cfn_share, share->table_name, false, /* table_has_blobs */
                                    "Row", key_rec, data_rec, old_row, new_row, causing_op_type, conflict_cause, err,
                                    trans, write_set,
                                    /*
...
int ha_ndbcluster::write_row(uchar *record)
{
  ...
  if (m_share == ndb_apply_status_share && table->in_use->slave_thread)
  {
    ...
    uint64 row_epoch;
    memcpy(&row_server_id, table->field[0]->ptr + (record - table->record[0]), sizeof(row_server_id));
    memcpy(&row_epoch, table->field[1]->ptr + (record - table->record[0]), sizeof(row_epoch));
    int rc = g_ndb_slave_state.atApplyStatusWrite(master_server_id, row_server_id, row_epoch,
...
*/
int ha_ndbcluster::ndb_write_row(uchar *record, bool primary_key_update, bool batched_update)
{
  ...
  if (has_auto_increment && table_share->primary_key != MAX_KEY)
  {
    ...
    if ((error = update_auto_increment()))
      DBUG_RETURN(error);
    m_skip_auto_increment = (insert_id_for_cur_row == 0 || thd->auto_inc_intervals_forced.nb_elements());
  }
  ...
  if (table_share->primary_key == MAX_KEY)
  {
    ...
    for (;;)
    {
      ...
      {
        if (--retries && !thd->killed && ndb->getNdbError().status == NdbError::TemporaryError)
        {
  ...
  const bool need_flush = thd_ndb->add_row_check_if_batch_full(m_bytes_per_write);
  ...
  {
    options.optionsPresent |= NdbOperation::OperationOptions::OO_DEFERRED_CONSTAINTS;
  }
  ...
  if (thd->slave_thread)
  {
    ...
    if (unlikely((error = prepare_conflict_detection(WRITE_ROW, key_rec, m_ndb_record, NULL, /* old_data */
                                                     record,                                 /* new_data */
  ...
  if (m_use_write && !avoidNdbApiWriteOp)
  {
    ...
    op = trans->writeTuple(key_rec, (const char *)key_row, m_ndb_record, (char *)record, mask, poptions,
                           sizeof(NdbOperation::OperationOptions));
  }
  else
  {
    ...
    if (m_table->hasDefaultValues())
    {
      ...
      */
      ...
      bitmap_init(user_cols_written_bitmap, tmpBitmapSpace, table->write_set->n_bits, false);
      ...
      for (uint i = 0; i < table->s->fields; i++)
      {
        ...
        DBUG_PRINT("info", ("Field#%u, (%u), Type : %u "
                            ...
                            i, field->field_index, field->real_type(), field->flags & NO_DEFAULT_VALUE_FLAG,
    ...
    op = trans->insertTuple(key_rec, (const char *)key_row, m_ndb_record, (char *)record,
  ...
  if (table_share->blob_fields > 0)
  {
    ...
    int res = set_blob_values(op, record - table->record[0], user_cols_written_bitmap, &blob_count, do_batch);
    dbug_tmp_restore_column_map(table->read_set, old_map);
    if (res != 0)
      DBUG_RETURN(res);
  }
  ...
  */
  ...
  if (((m_rows_to_insert == 1 || uses_blobs) && !do_batch) || primary_key_update || need_flush)
  {
...
int ha_ndbcluster::primary_key_cmp(const uchar *old_row, const uchar *new_row)
{
  ...
  for (; key_part != end; key_part++)
  {
    ...
    {
      if (key_part->field->cmp_binary((old_row + key_part->offset), (new_row + key_part->offset),
...
static Ndb_exceptions_data StaticRefreshExceptionsData = {NULL, NULL, NULL,        NULL,  NULL,
                                                          NULL, NULL, REFRESH_ROW, false, 0};

static int handle_row_conflict(NDB_CONFLICT_FN_SHARE *cfn_share, const char *table_name, bool table_has_blobs,
                               const char *handling_type, const NdbRecord *key_rec, const NdbRecord *data_rec,
                               const uchar *old_row, const uchar *new_row, enum_conflicting_op_type op_type,
                               enum_conflict_cause conflict_cause, const NdbError &conflict_error,
                               NdbTransaction *conflict_trans, const MY_BITMAP *write_set, Uint64 transaction_id)
{
  ...
  */
  bool refresh_row = (conflict_cause == TRANS_IN_CONFLICT) || (cfn_share && (cfn_share->m_flags & CFF_REFRESH_ROWS));
  ...
  if (refresh_row)
  {
    ...
    do
    {
       ...
       * it again
       */
      if ((conflict_cause == ROW_IN_CONFLICT) && (conflict_error.code == (int)error_op_after_refresh_op))
      {
      ...
      */
      ...
      options.optionsPresent =
          NdbOperation::OperationOptions::OO_CUSTOMDATA | NdbOperation::OperationOptions::OO_ANYVALUE;
      ...
      //      Keyless table?
      //      Unique index
      const NdbOperation *refresh_op =
          conflict_trans->refreshTuple(key_rec, (const char *)row, &options, sizeof(options));
      if (!refresh_op)
      {
  ...
  DBUG_PRINT("info", ("Table %s does%s have an exceptions table", table_name,
                      (cfn_share && cfn_share->m_ex_tab_writer.hasTable()) ? "" : " not"));
  if (cfn_share && cfn_share->m_ex_tab_writer.hasTable())
  {
    NdbError err;
    if (cfn_share->m_ex_tab_writer.writeRow(conflict_trans, key_rec, data_rec, ::server_id,
                                            ndb_mi_get_master_server_id(),
                                            g_ndb_slave_state.current_master_server_epoch, old_row, new_row, op_type,
                                            conflict_cause, transaction_id, write_set, err) != 0)
    {
      if (err.code != 0)
      {
        if (err.status == NdbError::TemporaryError)
...
int ha_ndbcluster::bulk_update_row(const uchar *old_data, uchar *new_data, uint *dup_key_found)
{
  ...
  *dup_key_found = 0;
  ...
  if (m_thd_ndb->m_handler && m_read_before_write_removal_possible)
  {
    ...
    */
    ...
    if (execute_commit(m_thd_ndb, trans, m_thd_ndb->m_force_send, ignore_error, &ignore_count) != 0)
    {
  ...
  uint ignore_count = 0;
  if (execute_no_commit(m_thd_ndb, trans, m_ignore_no_key || m_read_before_write_removal_used, &ignore_count) != 0)
  {
    no_uncommitted_rows_execute_failure();
    DBUG_RETURN(ndb_err(trans));
  }
...
int ha_ndbcluster::ndb_update_row(const uchar *old_data, uchar *new_data, int is_bulk_update)
{
  THD *thd = table->in_use;
  Thd_ndb *thd_ndb = m_thd_ndb;
  ...
  bool batch_allowed = !m_update_cannot_batch && (is_bulk_update || thd_allow_batch(thd));
   ...
   */
  if (m_ignore_dup_key && (thd->lex->sql_command == SQLCOM_UPDATE || thd->lex->sql_command == SQLCOM_UPDATE_MULTI))
  {
  ...
  if (m_use_partition_pruning)
  {
    ...
    if ((error = get_parts_for_update(old_data, new_data, table->record[0], m_part_info, &old_part_id, &new_part_id,
                                      &func_value)))
    {
      m_part_info->err_value = func_value;
      DBUG_RETURN(error);
  ...
  */
  ...
  if (cursor)
  {
    ...
    */
    DBUG_PRINT("info", ("Calling updateTuple on cursor, write_set=0x%x", table->write_set->bitmap[0]));

    if (options.optionsPresent != 0)
      poptions = &options;

    if (!(op = cursor->updateCurrentTuple(trans, m_ndb_record, (const char *)new_data, mask, poptions,
                                          sizeof(NdbOperation::OperationOptions))))
      ERR_RETURN(trans->getNdbError());
  ...
  else
  {
    const NdbRecord *key_rec;
    const uchar *key_row;
    setup_key_ref_for_ndb_record(&key_rec, &key_row, new_data, m_read_before_write_removal_used);
    ...
    if (thd->slave_thread)
    {
      ...
      if (unlikely((error = prepare_conflict_detection(UPDATE_ROW, key_rec, m_ndb_record, old_data, new_data,
    ...
    {
      if (!(op = trans->updateTuple(key_rec, (const char *)key_row, m_ndb_record, (const char *)new_data, mask,
                                    poptions, sizeof(NdbOperation::OperationOptions))))
        ERR_RETURN(trans->getNdbError());
    }
    else
    {
      ...
      if (!(op = trans->writeTuple(key_rec, (const char *)key_row, m_ndb_record, (const char *)new_data, mask, poptions,
                                   sizeof(NdbOperation::OperationOptions))))
        ERR_RETURN(trans->getNdbError());
    }
  }
  ...
  if (uses_blob_value(table->write_set))
  {
    ...
    int res = set_blob_values(op, row_offset, table->write_set, &blob_count, (batch_allowed && !need_flush));
    if (res != 0)
      DBUG_RETURN(res);
  }
  uint ignore_count = 0;
  ...
  */
  if (m_update_cannot_batch || !(cursor || (batch_allowed && have_pk)) || need_flush)
  {
    if (execute_no_commit(m_thd_ndb, trans, m_ignore_no_key || m_read_before_write_removal_used, &ignore_count) != 0)
    {
      no_uncommitted_rows_execute_failure();
...
int ha_ndbcluster::ndb_delete_row(const uchar *record, bool primary_key_update)
{
  ...
  int error;
  bool allow_batch = !m_delete_cannot_batch && (m_is_bulk_delete || thd_allow_batch(thd));
  ...
  if (m_use_partition_pruning)
  {
    ...
    if ((error = get_part_for_delete(record, table->record[0], m_part_info, &part_id)))
    {
      DBUG_RETURN(error);
    }
  skip_partition_pruning:
  ...
  */
  ...
  else
  {
    ...
    setup_key_ref_for_ndb_record(&key_rec, &key_row, record, m_read_before_write_removal_used);
    ...
    if (thd->slave_thread)
    {
      ...
      /* Conflict resolution in slave thread. */
      if (unlikely((error = prepare_conflict_detection(DELETE_ROW, key_rec, m_ndb_record, key_row, /* old_data */
                                                       NULL,                                       /* new_data */
                                                       table->write_set, trans, &code, &options, conflict_handled,
    ...
    */
    ...
    if (allow_batch && table_share->primary_key != MAX_KEY && !primary_key_update && !need_flush)
    {
      DBUG_RETURN(0);
    }
  }
    ...
    if (bitmap_is_set(table->read_set, i))
    {
      ...
      else if (field->flags & BLOB_FLAG)
      {
        ...
        if (!isNull)
        {
          ...
          {
            len64 = calc_ndb_blob_len(ndb_blob->getColumn()->getCharset(), blob_ptr, field_blob->max_data_length());

            // push a warning
            push_warning_printf(table->in_use, Sql_condition::SL_WARNING, WARN_DATA_TRUNCATED,
                                "Truncated value from TEXT field \'%s\'", field_blob->field_name);
          }
          field->set_notnull();
        }
...
*/
static int check_null_in_key(const KEY *key_info, const uchar *key, uint key_len)
{
  KEY_PART_INFO *curr_part, *end_part;
...
int ha_ndbcluster::index_read(uchar *buf, const uchar *key, uint key_len, enum ha_rkey_function find_flag)
{
  ...
  const int error = read_range_first_to_buf(&start_key, end_key_p, descending, m_sorted, buf);
...
int ha_ndbcluster::read_range_first_to_buf(const key_range *start_key, const key_range *end_key, bool desc, bool sorted,
                                           uchar *buf)
{
  part_id_range part_spec;
  ndb_index_type type = get_index_type(active_index);
  ...
  switch (type)
  {
    ...
    case PRIMARY_KEY_INDEX:
      ...
      {
        if (!m_thd_ndb->trans)
          if (unlikely(!start_transaction_key(active_index, start_key->key, error)))
            DBUG_RETURN(error);
        error =
            pk_read(start_key->key, start_key->length, buf, (m_use_partition_pruning) ? &(part_spec.start_part) : NULL);
  ...
  // Start the ordered index scan and fetch the first row
  DBUG_RETURN(ordered_index_scan(start_key, end_key, sorted, desc, buf, (m_use_partition_pruning) ? &part_spec : NULL));
}
...
int ha_ndbcluster::rnd_pos(uchar *buf, uchar *pos)
{
  ...
  // Perform a pk_read using primary key "index"
  {
    ...
    int res = pk_read(pos, key_length, buf, (m_user_defined_partitioning) ? &(part_spec.start_part) : NULL);
...
int ha_ndbcluster::info(uint flag)
{
  ...
  if (flag & HA_STATUS_CONST)
  {
    ...
    */
    stats.mrr_length_per_rec =
        multi_range_fixed_size(1) + multi_range_max_entry(PRIMARY_KEY_INDEX, table_share->reclength);
  }
  while (flag & HA_STATUS_VARIABLE)
  {
    ...
    */
    ...
    if (thd->lex->sql_command != SQLCOM_SHOW_TABLE_STATUS && thd->lex->sql_command != SQLCOM_SHOW_KEYS)
    {
      /*
  ...
  if (flag & HA_STATUS_AUTO)
  {
    ...
    if (m_table && table->found_next_number_field)
    {
      ...
      if (ndb->readAutoIncrementValue(m_table, g.range, auto_increment_value64) == -1)
      {
        const NdbError err = ndb->getNdbError();
        sql_print_error("Error %lu in readAutoIncrementValue(): %s", (ulong)err.code, err.message);
...
void ha_ndbcluster::get_dynamic_partition_info(ha_statistics *stat_info, ha_checksum *check_sum, uint part_id)
{
...
err:
  ...
  DBUG_PRINT("warning", ("ha_ndbcluster::get_dynamic_partition_info failed with error code %u", error));
}
...
bool ha_ndbcluster::start_read_removal()
{
  ...
  if (m_has_unique_index)
  {
    for (uint i = 0; i < table_share->keys; i++)
    {
      ...
      if ((key->flags & HA_NOSAME) && bitmap_is_overlapping(table->write_set, m_key_fields[i]))
      {
...
int ha_ndbcluster::flush_bulk_insert(bool allow_batch)
{
  ...
  DBUG_PRINT("info", ("Sending inserts to NDB, rows_inserted: %d", (int)m_rows_inserted));
  assert(trans);

  if (!(m_thd_ndb->trans_options & TNTO_TRANSACTIONS_OFF))
  {
    if (!allow_batch && execute_no_commit(m_thd_ndb, trans, m_ignore_no_key) != 0)
    {
      no_uncommitted_rows_execute_failure();
      DBUG_RETURN(ndb_err(trans));
    }
  ...
  else
  {
    ...
    */
    ...
    if (execute_commit(m_thd_ndb, trans, m_thd_ndb->m_force_send, m_ignore_no_key) != 0)
    {
      no_uncommitted_rows_execute_failure();
      DBUG_RETURN(ndb_err(trans));
    }
...
void ha_ndbcluster::start_bulk_insert(ha_rows rows)
{
  ...
  if (rows == (ha_rows)0)
  {
    ...
    m_rows_to_insert =
        (m_autoincrement_prefetch > DEFAULT_AUTO_PREFETCH) ? m_autoincrement_prefetch : DEFAULT_AUTO_PREFETCH;
...
static int ndbcluster_update_apply_status(THD *thd, int do_update)
{
  ...
  const Uint64 future_event_relay_log_pos = ndb_mi_get_future_event_relay_log_pos();
  const Uint64 group_relay_log_pos = ndb_mi_get_group_relay_log_pos();

  // log_name
  char tmp_buf[FN_REFLEN];
  ndb_pack_varchar(ndbtab->getColumn(2u), tmp_buf, group_master_log_name, (int)strlen(group_master_log_name));
  ...
  assert(r == 0);
  // end_pos
  r |= op->setValue(4u, group_master_log_pos + (future_event_relay_log_pos - group_relay_log_pos));
  assert(r == 0);
  return 0;
}
#endif /* HAVE_NDB_BINLOG */
...
void Thd_ndb::transaction_checks()
{
  ...
  else
  {
    ...
    THDVAR(thd, optimized_node_selection) = THDVAR(NULL, optimized_node_selection) & 1; /* using global value */
  }
}

int ha_ndbcluster::start_statement(THD *thd, Thd_ndb *thd_ndb, uint table_count)
{
  NdbTransaction *trans = thd_ndb->trans;
  int error;
  ...
  if (!trans && table_count == 0)
  {
    ...
    if (!(thd_options(thd) & OPTION_BIN_LOG) || thd->variables.binlog_format == BINLOG_FORMAT_STMT)
    {
...
int ha_ndbcluster::add_handler_to_open_tables(THD *thd, Thd_ndb *thd_ndb, ha_ndbcluster *handler)
{
  ...
  THD_NDB_SHARE *thd_ndb_share =
      (THD_NDB_SHARE *)my_hash_first(&thd_ndb->open_tables, (const uchar *)&key, sizeof(key), &state);
  while (thd_ndb_share && thd_ndb_share->key != key)
  {
    thd_ndb_share = (THD_NDB_SHARE *)my_hash_next(&thd_ndb->open_tables, (const uchar *)&key, sizeof(key), &state);
  }
  if (thd_ndb_share == 0)
  {
    thd_ndb_share = (THD_NDB_SHARE *)thd->get_transaction()->allocate_memory(sizeof(THD_NDB_SHARE));
    if (!thd_ndb_share)
    {
...
int ha_ndbcluster::external_lock(THD *thd, int lock_type)
{
  ...
  else
  {
    ...
    if (m_rows_changed && global_system_variables.query_cache_type)
    {
      ...
      if (thd_ndb->trans && thd_options(thd) & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))
      {
        DBUG_PRINT("info", ("Add share to list of changed tables, %p", m_share));
        /* NOTE push_back allocates memory using transactions mem_root! */
        thd_ndb->changed_tables.push_back(get_share(m_share), thd->get_transaction()->transaction_memroot());
      }
    ...
    if (!--thd_ndb->lock_count)
    {
      ...
      if ((!(thd_options(thd) & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))) && thd_ndb->trans)
      {
        if (thd_ndb->trans)
        {
          /*
...
NdbTransaction *ha_ndbcluster::start_transaction_row(const NdbRecord *ndb_record, const uchar *record, int &error)
{
  NdbTransaction *trans;
  ...
  char *buf = (char *)&tmp[0];
  trans = ndb->startTransaction(ndb_record, (const char *)record, buf, sizeof(tmp));
...
NdbTransaction *ha_ndbcluster::start_transaction_key(uint inx_no, const uchar *key_data, int &error)
{
  NdbTransaction *trans;
  ...
  Uint64 tmp[(MAX_KEY_SIZE_IN_WORDS * MAX_XFRM_MULTIPLY) >> 1];
  char *buf = (char *)&tmp[0];
  trans = ndb->startTransaction(key_rec, (const char *)key_data, buf, sizeof(tmp));
...
int ndbcluster_commit(handlerton *hton, THD *thd, bool all)
{
  ...
  {
    if (thd_ndb->m_handler && thd_ndb->m_handler->m_read_before_write_removal_possible)
    {
      /*
    ...
    else
    {
      ...
      res = execute_commit(thd_ndb, trans, THDVAR(thd, force_send), ignore_error);
    }
  }
  ...
  if (res != 0)
  {
    ...
    {
      if (st_ndb_slave_state::MAX_RETRY_TRANS_COUNT > g_ndb_slave_state.retry_trans_count++)
      {
        /*
           Warning is necessary to cause retry from slave.cc
           exec_relay_log_event()
        */
        push_warning(thd, Sql_condition::SL_WARNING, ER_SLAVE_SILENT_RETRY_TRANSACTION,
  ...
  else
  {
    ...
    {
      THD_NDB_SHARE *thd_share = (THD_NDB_SHARE *)my_hash_element(&thd_ndb->open_tables, i);
  ...
  while ((share = it++))
  {
    DBUG_PRINT("info", ("Remove share to list of changed tables, %p", share));
    native_mutex_lock(&share->mutex);
    DBUG_PRINT("info", ("Invalidate commit_count for %s, share->commit_count: %lu", share->table_name,
...
static int ndbcluster_rollback(handlerton *hton, THD *thd, bool all)
{
  ...
  DBUG_PRINT("enter", ("all: %d  thd_ndb->save_point_count: %d", all, thd_ndb->save_point_count));
  assert(ndb);
  thd_ndb->start_stmt_count = 0;
  if (trans == NULL)
  {
  ...
  if (!all && (thd_options(thd) & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN)) && (thd_ndb->save_point_count > 0))
  {
    /*
      Ignore end-of-statement until real rollback or commit is called
...
static const struct NDB_Modifier ndb_table_modifiers[] = {{NDB_Modifier::M_BOOL, STRING_WITH_LEN("NOLOGGING"), 0, {0}},
                                                          {NDB_Modifier::M_BOOL, 0, 0, 0, {0}}};

static const struct NDB_Modifier ndb_column_modifiers[] = {
    {NDB_Modifier::M_BOOL, STRING_WITH_LEN("MAX_BLOB_PART_SIZE"), 0, {0}}, {NDB_Modifier::M_BOOL, 0, 0, 0, {0}}};
...
int NDB_Modifiers::parse_modifier(THD *thd, const char *prefix, struct NDB_Modifier *m, const char *str)
{
  if (m->m_found)
  {
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION, "%s : modifier %s specified twice",
                        prefix, m->m_name);
  }
...

  {
    ...
    if (end)
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
                          "%s : invalid value '%.*s' for %s", prefix, (int)(end - str), str, m->m_name);
    }
    else
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION, "%s : invalid value '%s' for %s",
                          prefix, str, m->m_name);
    }
  }
  return -1;
...
int NDB_Modifiers::parse(THD *thd, const char *prefix, const char *_source, size_t _source_len)
{
  ...
  if (source == 0)
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
                          "%s : unable to parse due to out of memory", prefix);
      return -1;
    }
  ...
  while (pos && pos[0] != 0 && pos[0] != ' ')
  {
    ...
    unknown:
      if (end)
      {
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION, "%s : unknown modifier: %.*s",
...
static int create_ndb_column(THD *thd, NDBCOL &col, Field *field, HA_CREATE_INFO *create_info,
                             column_format_type default_format = COLUMN_FORMAT_TYPE_DEFAULT)
{
  ...
  column_modifiers.parse(thd, "NDB_COLUMN=", field->comment.str, field->comment.length);
...

  {
     ...
     * default.
     */
    bool nativeDefaults = !(thd && (!ndb_native_default_support(get_thd_ndb(thd)->ndb->getMinDbNodeVersion())));

    if (likely(nativeDefaults))
    {
      if ((!(field->flags & PRI_KEY_FLAG)) && type_supports_default_value(mysql_type))
      {
        if (!(field->flags & NO_DEFAULT_VALUE_FLAG))
        {
          my_ptrdiff_t src_offset = field->table->default_values_offset();
          if ((!field->is_real_null(src_offset)) || ((field->flags & NOT_NULL_FLAG)))
          {
  ...
  if (field->flags & AUTO_INCREMENT_FLAG)
  {
    ...
    ulonglong value = create_info->auto_increment_value ? create_info->auto_increment_value : (ulonglong)1;
  ...
  DBUG_PRINT("info", ("storage: %u  format: %u  ", field->field_storage_type(), field->column_format()));
  ...
  DBUG_PRINT("info", ("Column %s is declared %s", field->field_name, (dynamic) ? "dynamic" : "static"));
  ...
  DBUG_PRINT("info", ("Format %s, Storage %s", (dynamic) ? "dynamic" : "fixed",
      ...
      if (field->flags & AUTO_INCREMENT_FLAG)
      {
        ...
        for (;;)
        {
          ...
          if (ndb->readAutoIncrementValue(ndbtab, g.range, auto_value))
          {
            ...
            const NdbError err = ndb->getNdbError();
            sql_print_error("Error %lu in ::update_create_info(): %s", (ulong)err.code, err.message);
            DBUG_VOID_RETURN;
          }
          break;
        }
...
*/
static bool adjusted_frag_count(Ndb *ndb, uint requested_frags, uint &reported_frags)
{
   ...
   */
  while (reported_frags < requested_frags && (replicas + usable_nodes * no_threads * no_replicas) <= max_replicas)
  {
...
extern bool ndb_fk_util_truncate_allowed(THD *thd, NdbDictionary::Dictionary *dict, const char *db,
                                         const NdbDictionary::Table *tab, bool &allow);

/*
  Forward declaration of the utility functions used
  when creating partitioned tables
*/
static int create_table_set_up_partition_info(HA_CREATE_INFO *create_info, partition_info *part_info,
                                              NdbDictionary::Table &);
static int create_table_set_range_data(const partition_info *part_info, NdbDictionary::Table &);
static int create_table_set_list_data(const partition_info *part_info, NdbDictionary::Table &);

/**
  Create a table in NDB Cluster
*/

int ha_ndbcluster::create(const char *name, TABLE *form, HA_CREATE_INFO *create_info)
{
  THD *thd = current_thd;
  NDBTAB tab;
  NDBCOL col;
  ...
  */
  if (strlen(m_dbname) > NDB_MAX_DDL_NAME_BYTESIZE || strlen(m_tabname) > NDB_MAX_DDL_NAME_BYTESIZE)
  {
    char *invalid_identifier = (strlen(m_dbname) > NDB_MAX_DDL_NAME_BYTESIZE) ? m_dbname : m_tabname;
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TOO_LONG_IDENT,
  ...
  if (create_from_engine)
  {
    ...
    */
    ...
    ndbcluster_create_binlog_setup(thd, ndb, name, (uint)strlen(name), m_dbname, m_tabname, form);
    if (my_errno() == HA_ERR_TABLE_EXIST)
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TABLE_EXISTS_ERROR,
  ...
  if (!ndb_schema_dist_is_ready())
  {
    ...
    */
    if (!(strcmp(m_dbname, NDB_REP_DB) == 0 && strcmp(m_tabname, NDB_SCHEMA_TABLE) == 0))
    {
  ...
  {
    if ((strcmp(m_dbname, NDB_REP_DB) == 0 && strcmp(m_tabname, NDB_APPLY_TABLE) == 0))
    {
      ndb_sys_table = TRUE;
    }
  }
  ...
  if (is_truncate)
  {
     ...
     */
    bool allow;
    if (!ndb_fk_util_truncate_allowed(thd, dict, m_dbname, ndbtab_g.get_table(), allow))
    {
      DBUG_RETURN(HA_ERR_NO_CONNECTION);
    }
    if (!allow)
  ...
  table_modifiers.parse(thd, "NDB_TABLE=", create_info->comment.str, create_info->comment.length);
  ...
  int rep_read_rc = ndbcluster_get_binlog_replication_info(thd, ndb, m_dbname, m_tabname, ::server_id, &binlog_flags,
                                                           &conflict_fn, args, &num_args);
  if (rep_read_rc != 0)
  {
  ...
  DBUG_PRINT("info", ("setFrm data: 0x%lx  len: %lu", (long)pack_data, (ulong)pack_length));
  ...
  */
  ...
  for (i = 0; i < form->s->fields; i++)
  {
    ...
    if (!use_disk && col.getStorageType() == NDBCOL::StorageTypeDisk)
      use_disk = TRUE;
  ...
  DBUG_PRINT("info", ("Table %s is %s stored with tablespace %s", m_tabname, (use_disk) ? "disk" : "memory",
  ...
  for (i = 0, key_info = form->key_info; i < form->s->keys; i++, key_info++)
  {
    ...
    for (; key_part != end; key_part++)
    {
      ...
      {
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
                            ER(ER_ILLEGAL_HA_CREATE_OPTION), ndbcluster_hton_name,
                            "Index on field "
                            "declared with "
                            "STORAGE DISK is not supported");
      ...
      tab.getColumn(key_part->fieldnr - 1)->setStorageType(NdbDictionary::Column::StorageTypeMemory);
    }
  }
  ...
  set_my_errno(create_table_set_up_partition_info(create_info, form->part_info, tab));
  if (my_errno())
    goto abort;

  if (tab.getFragmentType() == NDBTAB::HashMapPartition && tab.getDefaultNoPartitionsFlag() &&
      (create_info->max_rows != 0 || create_info->min_rows != 0))
  {
    ulonglong rows = create_info->max_rows >= create_info->min_rows ? create_info->max_rows : create_info->min_rows;
  ...
  if (!fk_list_for_truncate.is_empty() && my_errno() == 0)
  {
    /*
     ...
     for truncate table.
     */
    set_my_errno(recreate_fk_for_truncate(thd, ndb, tab.getName(), fk_list_for_truncate));
  }
  ...
  abort:
     ...
     */
    DBUG_PRINT("info", ("Aborting schema transaction due to error %i", my_errno()));
    if (dict->endSchemaTrans(NdbDictionary::Dictionary::SchemaTransAbort) == -1)
      DBUG_PRINT("info", ("Failed to abort schema transaction, %i", dict->getNdbError().code));
    m_table = 0;
  ...
  if (my_errno())
  {
    ...
    */
    while (!thd->killed)
    {
      ...
      {
      cleanup_failed:
        DBUG_PRINT("info", ("Could not cleanup failed create %i", dict->getNdbError().code));
  ...
  else  // if (!my_errno)
  {
    ...
    */
    {
      ...
      if ((share = (NDB_SHARE *)my_hash_search(&ndbcluster_open_tables, (const uchar *)name, length)))
    ...
    */
    ...
    {
      DBUG_PRINT("NDB_SHARE", ("%s binlog create  use_count: %u", share->key_string(), share->use_count));
    }
    ...
    while (!IS_TMP_PREFIX(m_tabname))
    {
      ...
      if (share)
      {
        ...
        ndbcluster_apply_binlog_replication_info(thd, share, m_table, conflict_fn, args, num_args,
                                                 TRUE, /* Do set binlog flags */
                                                 binlog_flags);
      }
#endif
      String event_name(INJECTOR_EVENT_LEN);
      ndb_rep_event_name(&event_name, m_dbname, m_tabname, get_binlog_full(share));
      int do_event_op = ndb_binlog_running;

      if (!ndb_schema_dist_is_ready() && strcmp(share->db, NDB_REP_DB) == 0 &&
      ...
      */
                                   ...
                                   do_event_op ? 2 : 1 /* push warning */))
      {
        if (opt_ndb_extra_logging)
          sql_print_information("NDB Binlog: CREATE TABLE Event: %s", event_name.c_ptr());

        if (ndbcluster_create_event_ops(thd, share, m_table, event_name.c_ptr()))
        {
          sql_print_error(
      ...
      */
      ...
      ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, share->db, share->table_name,
                               m_table->getObjectId(), m_table->getObjectVersion(),
                               (is_truncate) ? SOT_TRUNCATE_TABLE : SOT_CREATE_TABLE, NULL, NULL);
      break;
    }
  }
...
int ha_ndbcluster::create_index(THD *thd, const char *name, KEY *key_info, NDB_INDEX_TYPE idx_type, uint idx_no) const
{
  int error = 0;
  ...
  if (idx_type == UNIQUE_ORDERED_INDEX || idx_type == UNIQUE_INDEX)
  {
    ...
    DBUG_PRINT("info", ("Created unique index name \'%s\' for index %d", unique_name, idx_no));
  }
  ...
  switch (idx_type)
  {
    ...
    case UNIQUE_INDEX:
      if (check_index_fields_not_null(key_info))
      {
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_NULL_COLUMN_IN_INDEX,
                            "Ndb does not support unique index on NULL valued attributes, index access with NULL value "
                            "will become full table scan");
      }
      error = create_unique_index(thd, unique_name, key_info);
      break;
    case ORDERED_INDEX:
...
int ha_ndbcluster::create_ordered_index(THD *thd, const char *name, KEY *key_info) const
{
  DBUG_ENTER("ha_ndbcluster::create_ordered_index");
  DBUG_RETURN(create_ndb_index(thd, name, key_info, FALSE));
}

int ha_ndbcluster::create_unique_index(THD *thd, const char *name, KEY *key_info) const
{
...
int ha_ndbcluster::create_ndb_index(THD *thd, const char *name, KEY *key_info, bool unique) const
{
  char index_name[FN_LEN + 1];
  Ndb *ndb = get_ndb(thd);
  ...
  for (; key_part != end; key_part++)
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION, ER(ER_ILLEGAL_HA_CREATE_OPTION),
...
void ha_ndbcluster::prepare_for_alter()
{
  ...
  DBUG_PRINT("NDB_SHARE", ("%s binlog schema  use_count: %u", m_share->key_string(), m_share->use_count));
...
int ha_ndbcluster::add_index_impl(THD *thd, TABLE *table_arg, KEY *key_info, uint num_of_keys)
{
  int error = 0;
  uint idx;
...
*/
int ha_ndbcluster::prepare_drop_index(TABLE *table_arg, uint *key_num, uint num_of_keys)
{
  ...
  for (idx = 0; idx < num_of_keys; idx++)
  {
    ...
    if (m_index[i].type == PRIMARY_KEY_ORDERED_INDEX || m_index[i].type == UNIQUE_ORDERED_INDEX ||
...
extern void ndb_fk_util_resolve_mock_tables(THD *thd, NdbDictionary::Dictionary *dict, const char *new_parent_db,
                                            const char *new_parent_name);

int ha_ndbcluster::rename_table_impl(THD *thd, Ndb *ndb, const NdbDictionary::Table *orig_tab, const char *from,
                                     const char *to, const char *old_dbname, const char *old_tabname,
                                     const char *new_dbname, const char *new_tabname, bool real_rename,
                                     const char *real_rename_db, const char *real_rename_name,
                                     bool real_rename_log_on_participant, bool drop_events, bool create_events,
                                     bool commit_alter)
{
  ...
  DBUG_PRINT("info", ("real_rename_log_on_participant: %d", real_rename_log_on_participant));
  // Verify default values of real_rename related parameters
  assert(real_rename ||
         (real_rename_db == NULL && real_rename_name == NULL && real_rename_log_on_participant == false));
  ...
  if (real_rename)
  {
    /*
      ...
      at once.
   */
    ndbcluster_log_schema_op(thd, to, (int)strlen(to), real_rename_db, real_rename_name, ndb_table_id,
                             ndb_table_version, SOT_RENAME_TABLE_PREPARE, new_dbname /* unused */,
                             new_tabname /* unused */);
  }
  ...
  ndb_fk_util_resolve_mock_tables(thd, ndb->getDictionary(), new_dbname, new_tabname);
  ...
  if (drop_events)
  {
    ndbcluster_drop_event(thd, ndb, share, old_dbname, old_tabname);
  }
...
#ifdef HAVE_NDB_BINLOG
    ndbcluster_read_binlog_replication(thd, ndb, share, ndbtab, ::server_id, TRUE);
#endif
    /* always create an event for the table */
    String event_name(INJECTOR_EVENT_LEN);
    ndb_rep_event_name(&event_name, new_dbname, new_tabname, get_binlog_full(share));

    if (!Ndb_dist_priv_util::is_distributed_priv_table(new_dbname, new_tabname) &&
        !ndbcluster_create_event(thd, ndb, ndbtab, event_name.c_ptr(), share,
                                 ndb_binlog_running ? 2 : 1 /* push warning */))
    {
      if (opt_ndb_extra_logging)
        sql_print_information("NDB Binlog: RENAME Event: %s", event_name.c_ptr());
      if (share->op == 0 && ndbcluster_create_event_ops(thd, share, ndbtab, event_name.c_ptr()))
      {
        sql_print_error(
  ...
  if (real_rename)
  {
    ...
    */
    ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, real_rename_db, real_rename_name, ndb_table_id,
                             ndb_table_version, SOT_RENAME_TABLE, new_dbname, new_tabname,
  ...
  if (commit_alter)
  {
    ...
    ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, new_dbname, new_tabname, ndb_table_id,
                             ndb_table_version, SOT_ALTER_TABLE_COMMIT, NULL, NULL);
  }
  ...
  for (unsigned i = 0; i < index_list.count; i++)
  {
    ...
    if (my_strcasecmp(system_charset_info, index_el.database, NDB_SYSTEM_DATABASE))
    {
      // Get old index
      ndb->setDatabaseName(old_dbname);
      const NDBINDEX *index = dict->getIndexGlobal(index_el.name, new_tab);
      DBUG_PRINT("info", ("Creating index %s/%s", index_el.database, index->getName()));
      // Create the same "old" index on new tab
      dict->createIndex(*index, new_tab);
      DBUG_PRINT("info", ("Dropping index %s/%s", index_el.database, index->getName()));
      // Drop old index
      ndb->setDatabaseName(old_dbname);
...
int ha_ndbcluster::rename_table(const char *from, const char *to)
{
  ...
  */
  ...
  if (strlen(new_dbname) > NDB_MAX_DDL_NAME_BYTESIZE || strlen(new_tabname) > NDB_MAX_DDL_NAME_BYTESIZE)
  {
    char *invalid_identifier = (strlen(new_dbname) > NDB_MAX_DDL_NAME_BYTESIZE) ? new_dbname : new_tabname;
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TOO_LONG_IDENT,
  ...
  switch (thd_sql_command(thd))
  {
    ...
    case SQLCOM_ALTER_TABLE:
      ...
      if (!new_is_temp && !old_is_temp)
      {
        ...
        DBUG_RETURN(rename_table_impl(thd, ndb, orig_tab, from, to, old_dbname, m_tabname, new_dbname, new_tabname,
                                      true,        // real_rename
      ...
      assert(thd->lex->alter_info.requested_algorithm != Alter_info::ALTER_TABLE_ALGORITHM_INPLACE);
        ...
        */
             ...
             my_strcasecmp(system_charset_info, orig_name, new_tabname)))
        {
          ...
          */
          ...
          DBUG_RETURN(rename_table_impl(thd, ndb, orig_tab, from, to, old_dbname, m_tabname, new_dbname, new_tabname,
                                        true,  // real_rename
                                        real_rename_db, real_rename_name, real_rename_log_on_participant,
                                        false,   // drop_events
                                        true,    // create events
                                        true));  // commit_alter
        }
    ...
    default:
      sql_print_error("Unexpected rename case detected, sql_command: %d", thd_sql_command(thd));
      abort();
      break;
  }
...
static void delete_table_drop_share(NDB_SHARE *share, const char *path)
{
  ...
  do_drop:
    if (share->state != NSS_DROPPED)
    {
      ...
      */
      ...
      DBUG_PRINT("NDB_SHARE", ("%s create free  use_count: %u", share->key_string(), share->use_count));
      free_share(&share, TRUE);
    }
    /* ndb_share reference temporary free */
    DBUG_PRINT("NDB_SHARE", ("%s temporary free  use_count: %u", share->key_string(), share->use_count));
    free_share(&share, TRUE);
    native_mutex_unlock(&ndbcluster_mutex);
  }
  else if (path)
...
extern bool ndb_fk_util_build_list(THD *, NdbDictionary::Dictionary *, const NdbDictionary::Table *, List<char> &);
extern void ndb_fk_util_drop_list(THD *, Ndb *ndb, NdbDictionary::Dictionary *, List<char> &);
extern bool ndb_fk_util_drop_table(THD *, Ndb *ndb, NdbDictionary::Dictionary *, const NdbDictionary::Table *);
extern bool ndb_fk_util_is_mock_name(const char *table_name);

bool ha_ndbcluster::drop_table_and_related(THD *thd, Ndb *ndb, NdbDictionary::Dictionary *dict,
                                           const NdbDictionary::Table *table, int drop_flags, bool skip_related)
{
  ...
  DBUG_PRINT("enter", ("cascade_constraints: %d dropdb: %d skip_related: %d",
                       ...
                       MY_TEST(drop_flags & NDBDICT::DropTableCascadeConstraintsDropDB), skip_related));
  ...
  */
  ...
  if (dict->dropTableGlobal(*table, drop_flags) != 0)
  {
    ...
    if (ndb_err.code == 21080 && thd_test_options(thd, OPTION_NO_FOREIGN_KEY_CHECKS))
    {
      /*
...
int ha_ndbcluster::drop_table_impl(THD *thd, ha_ndbcluster *h, Ndb *ndb, const char *path, const char *db,
                                   const char *table_name)
{
  ...
  if (share)
  {
    DBUG_PRINT("NDB_SHARE", ("%s temporary  use_count: %u", share->key_string(), share->use_count));
  }

  bool skip_related = false;
  int drop_flags = 0;
  /* Copying alter can leave #sql table which is parent of old FKs */
  if (thd->lex->sql_command == SQLCOM_ALTER_TABLE && strncmp(table_name, "#sql", 4) == 0)
  {
  ...
  retry_temporary_error1:
    if (drop_table_and_related(thd, ndb, dict, h->m_table, drop_flags, skip_related))
    {
  ...
  else
  {
    ...
    while (1)
    {
      ...
      retry_temporary_error2:
        if (drop_table_and_related(thd, ndb, dict, ndbtab_g.get_table(), drop_flags, skip_related))
        {
  ...
  */
...

  {
    if (table_dropped)
    {
      ndbcluster_handle_drop_table(thd, ndb, share, "delete table", db, table_name);
    }
    else
    {
      /**
       ...
       */
      ndbcluster_handle_drop_table(thd, ndb, share, "delete table", 0, 0);
    }
  }

  if (!IS_TMP_PREFIX(table_name) && share && thd->lex->sql_command != SQLCOM_TRUNCATE)
  {
    ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, share->db, share->table_name, ndb_table_id,
                             ndb_table_version, SOT_DROP_TABLE, NULL, NULL);
  }
...
int ha_ndbcluster::delete_table(const char *name)
{
  ...
  int error;
  Ndb *ndb = thd_ndb->ndb;
  if (!(error = drop_table_impl(thd, this, ndb, name, m_dbname, m_tabname)) || error == HA_ERR_NO_SUCH_TABLE)
  {
...
void ha_ndbcluster::get_auto_increment(ulonglong offset, ulonglong increment, ulonglong nb_desired_values,
                                       ulonglong *first_value, ulonglong *nb_reserved_values)
{
  ...
  {
    Ndb_tuple_id_range_guard g(m_share);
    if ((m_skip_auto_increment && ndb->readAutoIncrementValue(m_table, g.range, auto_value)) ||
        ndb->getAutoIncrementValue(m_table, g.range, auto_value, Uint32(m_autoincrement_prefetch), increment, offset))
    {
      if (--retries && !thd->killed && ndb->getNdbError().status == NdbError::TemporaryError)
      {
        do_retry_sleep(retry_sleep);
      ...
      const NdbError err = ndb->getNdbError();
      sql_print_error("Error %lu in ::get_auto_increment(): %s", (ulong)err.code, err.message);
...
ha_ndbcluster::~ha_ndbcluster()
{
  ...
  if (m_share)
  {
    ...
    DBUG_PRINT("NDB_SHARE", ("%s handler free  use_count: %u", m_share->key_string(), m_share->use_count));
    free_share(&m_share);
  }
...
int ha_ndbcluster::open(const char *name, int mode, uint test_if_locked)
{
...

  {
    ...
    m_key_fields = (MY_BITMAP **)my_malloc(PSI_INSTRUMENT_ME, ptr_size + map_size, MYF(MY_WME + MY_ZEROFILL));
    if (!m_key_fields)
    {
  ...
  if ((m_share = get_share(name, table, FALSE)) == 0)
  {
    ...
    {
      sql_print_information("Calling ndbcluster_create_binlog_setup(%s) in ::open", name);
    }
    Ndb *ndb = check_ndb_in_thd(thd);
    ndbcluster_create_binlog_setup(thd, ndb, name, (uint)strlen(name), m_dbname, m_tabname, table);
    if ((m_share = get_share(name, table, FALSE)) == 0)
    {
      local_close(thd, FALSE);
      DBUG_RETURN(1);
  ...
  DBUG_PRINT("NDB_SHARE", ("%s handler  use_count: %u", m_share->key_string(), m_share->use_count));
  ...
  for (i = 0; i < MAX_KEY; i++)
  {
    ...
    if (m_index[i].status == ACTIVE)
    {
      ...
      if (index)
      {
        ...
        {
          DBUG_PRINT("info", ("Optimze index %s returned %d", index->getName(), error));
          ERR_RETURN(ndb->getNdbError());
        }
        ...
        {
          DBUG_PRINT("info", ("Optimize index %s did not complete", index->getName()));
          ERR_RETURN(ndb->getNdbError());
        }
      }
      if (unique_index)
      {
        ...
        {
          DBUG_PRINT("info", ("Optimze unique index %s returned %d", unique_index->getName(), error));
          ERR_RETURN(ndb->getNdbError());
        }
        while ((result = ih.next()) == 1)
        {
...
int ha_ndbcluster::analyze_index(THD *thd)
{
  ...
  for (inx = 0; inx < table_share->keys; inx++)
  {
    ...
    if ((idx_type == PRIMARY_KEY_ORDERED_INDEX || idx_type == UNIQUE_ORDERED_INDEX || idx_type == ORDERED_INDEX))
    {
...
void ha_ndbcluster::set_part_info(partition_info *part_info, bool early)
{
  ...
  {
    m_use_partition_pruning = FALSE;
    if (!(m_part_info->part_type == HASH_PARTITION && m_part_info->list_of_part_fields &&
...
void ha_ndbcluster::local_close(THD *thd, bool release_metadata_flag)
{
  ...
  if (m_key_fields)
  {
    ...
    for (inx_bitmap = m_key_fields; (inx_bitmap != NULL) && ((*inx_bitmap) != NULL); inx_bitmap++)
...
*/
static int ndbcluster_discover(handlerton *hton, THD *thd, const char *db, const char *name, uchar **frmblob,
                               size_t *frmlen)
{
  int error = 0;
  NdbError ndb_error;
  ...
  {
    sql_print_information(
        "NDB: Could not find database directory '%s' "
        "while trying to discover table '%s'",
        db, name);
...
#ifdef HAVE_NDB_BINLOG
  if (ndbcluster_check_if_local_table(db, name) && !Ndb_dist_priv_util::is_distributed_priv_table(db, name))
  {
    DBUG_PRINT("info", ("ndbcluster_discover: Skipping locally defined table '%s.%s'", db, name));
    sql_print_error("ndbcluster_discover: Skipping locally defined table '%s.%s'", db, name);
    error = 1;
    goto err;
  }
#endif
...
*/
static int ndbcluster_table_exists_in_engine(handlerton *hton, THD *thd, const char *db, const char *name)
{
  Ndb *ndb;
...
extern "C" uchar *tables_get_key(const char *entry, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
  *length = strlen(entry);
  return (uchar *)entry;
}
...
int ndbcluster_drop_database_impl(THD *thd, const char *path)
{
  ...
  for (i = 0; i < list.count; i++)
  {
    ...
    // is deleted.
    if (my_strcasecmp(system_charset_info, elmt.database, dbname) || IS_NDB_BLOB_PREFIX(elmt.name) ||
  ...
  char *tmp = full_path + build_table_filename(full_path, sizeof(full_path) - 1, dbname, "", "", 0);
...
static void ndbcluster_drop_database_leftovers(const char *path)
{
  ...
  for (uint i = 0; i < dirp->number_off_files; i++)
  {
    ...
    DBUG_PRINT("info", ("Found leftover .ndb file '%s'! Try to delete it.", file_path));
    if (my_delete_with_symlink(file_path, MYF(0)))
    {
      ...
      DBUG_PRINT("error", ("Delete of of '%s' failed, my_errno: %d", file_path, my_errno()));
    }
  }
...
static void ndbcluster_drop_database(handlerton *hton, char *path)
{
  ...
  */
  ...
  ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, db, "", table_id, table_version, SOT_DROP_DB,
                           NULL, NULL);
  DBUG_VOID_RETURN;
}

int ndb_create_table_from_engine(THD *thd, const char *db, const char *table_name)
{
...
static int ndbcluster_find_files(handlerton *hton, THD *thd, const char *db, const char *path, const char *wild,
                                 bool dir, List<LEX_STRING> *files)
{
  ...
  {  // extra bracket to avoid gcc 2.95.3 warning
    ...
    if (my_hash_init(&ndb_tables, table_alias_charset, list.count, 0, 0, (my_hash_get_key)tables_get_key, 0, 0,
                     PSI_INSTRUMENT_ME))
    {
    ...
    if (my_hash_init(&ok_tables, system_charset_info, 32, 0, 0, (my_hash_get_key)tables_get_key, 0, 0,
                     PSI_INSTRUMENT_ME))
    {
    ...
    while ((file_name = it++))
    {
      ...
      if (my_hash_search(&ndb_tables, (const uchar *)file_name->str, file_name->length))
      {
        build_table_filename(name, sizeof(name) - 1, db, file_name->str, reg_ext, 0);
        if (my_access(name, F_OK))
        {
           ...
           */
          DBUG_PRINT("info", ("Table %s listed and need discovery", file_name->str));
          if (ndb_create_table_from_engine(thd, db, file_name->str))
          {
            push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TABLE_EXISTS_ERROR, "Discover of table %s.%s failed",
                                db, file_name->str);
            continue;
          }
        }
      ...
      build_table_filename(name, sizeof(name) - 1, db, file_name->str, ha_ndb_ext, 0);
      ...
      if (my_access(name, F_OK))
      {
        ...
        if (file_on_disk)
        {
          ...
          uchar *record = my_hash_search(&ndb_tables, (const uchar *)file_name->str, file_name->length);
          assert(record);
          my_hash_delete(&ndb_tables, record);
          push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TABLE_EXISTS_ERROR,
                              "Local table %s.%s shadows ndb table", db, file_name->str);
        }
        continue;
      }
      ...
      if (ndbcluster_table_exists_in_engine(hton, thd, db, file_name->str) == HA_ERR_NO_SUCH_TABLE)
      {
    ...
    if (!thd_ndb->skip_binlog_setup_in_find_files())
    {
      ...
      char *end, *end1 = name + build_table_filename(name, sizeof(name) - 1, db, "", "", 0);
      for (i = 0; i < ok_tables.records; i++)
      {
        file_name_str = (char *)my_hash_element(&ok_tables, i);
        end = end1 + tablename_to_filename(file_name_str, end1, (uint)(sizeof(name) - (end1 - name)));
        ndbcluster_create_binlog_setup(thd, ndb, name, (uint)(end - name), db, file_name_str, 0);
      }
    }
    ...
    for (i = 0; i < ndb_tables.records; i++)
    {
      ...
      if (!my_hash_search(&ok_tables, (const uchar *)file_name_str, strlen(file_name_str)))
      {
        /* Table in Cluster did not have frm or .ndb */
        build_table_filename(name, sizeof(name) - 1, db, file_name_str, reg_ext, 0);
        if (my_access(name, F_OK))
        {
    ...
    else
    {
      ...
      */
      ...
      {
        DBUG_PRINT("info", ("Deleting local files for table '%s.%s'", db, file_name_str));
    ...
    while ((file_name_str = it2++))
    {
      ...
      if (ndb_create_table_from_engine(thd, db, file_name_str) == 0)
      {
        ...
        tmp_file_name = thd->make_lex_string(tmp_file_name, file_name_str, (uint)strlen(file_name_str), TRUE);
...
*/
static bool is_supported_system_table(const char *db, const char *table_name, bool is_sql_layer_system_table)
{
...
static int ndb_wait_setup_func_impl(ulong max_wait)
{
  ...
  while (max_wait && (!ndb_setup_complete || !ndb_index_stat_thread.is_setup_complete()))
  {
    int rc = native_cond_timedwait(&COND_ndb_setup_complete, &ndbcluster_mutex, &abstime);
    if (rc)
    {
      if (rc == ETIMEDOUT)
      {
      ...
      {
        DBUG_PRINT("info", ("Bad native_cond_timedwait rc : %u", rc));
        assert(false);
        break;
      }
    }
...
static int ndbcluster_make_pushed_join(handlerton *, THD *, const AQP::Join_plan *);

/* Version in composite numerical format */
static Uint32 ndb_version = NDB_VERSION_D;
static MYSQL_SYSVAR_UINT(version,     /* name */
                         ndb_version, /* var */
                         PLUGIN_VAR_NOCMDOPT | PLUGIN_VAR_READONLY, "Compile version for ndbcluster",
                         NULL, /* check func. */
                         NULL, /* update func. */
                         0,    /* default */
                         0,    /* min */
...
static MYSQL_SYSVAR_STR(version_string,     /* name */
                        ...
                        PLUGIN_VAR_NOCMDOPT | PLUGIN_VAR_READONLY, "Compile version string for ndbcluster",
                        NULL, /* check func. */
                        NULL, /* update func. */
                        NULL  /* default */
);
...
static int ndbcluster_init(void *p)
{
  ...
  assert(DependencyTracker::InvalidTransactionId == Ndb_binlog_extra_row_info::InvalidTransactionId);
...
#endif
  if (ndb_util_thread.init() || DBUG_EVALUATE_IF("ndbcluster_init_fail1", true, false))
  {
...

  {
    ...
    h->flags = HTON_CAN_RECREATE | HTON_TEMPORARY_NOT_SUPPORTED | HTON_NO_BINLOG_ROW_OPT;
  ...
  if (ndbcluster_connect(connect_callback, opt_ndb_wait_connected, opt_ndb_cluster_connection_pool,
  ...
  (void)my_hash_init(&ndbcluster_open_tables, table_alias_charset, 32, 0, 0, (my_hash_get_key)ndbcluster_get_key, 0, 0,
                     PSI_INSTRUMENT_ME);
  (void)my_hash_init(&ndbcluster_dropped_tables, table_alias_charset, 32, 0, 0, (my_hash_get_key)ndbcluster_get_key, 0,
                     0, PSI_INSTRUMENT_ME);
  ...
  if (ndb_index_stat_thread.start() || DBUG_EVALUATE_IF("ndbcluster_init_fail2", true, false))
  {
...
#ifndef NDEBUG
static const char *get_share_state_string(NDB_SHARE_STATE s)
{
  switch (s)
  {
    case NSS_INITIAL:
...
static int ndbcluster_end(handlerton *hton, ha_panic_function type)
{
...
#ifndef NDEBUG
      fprintf(stderr, "NDB: table share %s with use_count %d state: %s(%u) not freed\n", share->key_string(),
              share->use_count, get_share_state_string(share->state), (uint)share->state);
#endif
  ...
  {
    native_mutex_lock(&ndbcluster_mutex);
    uint save = ndbcluster_dropped_tables.records;
    (void)save;
    while (ndbcluster_dropped_tables.records)
    {
      NDB_SHARE *share = (NDB_SHARE *)my_hash_element(&ndbcluster_dropped_tables, 0);
#ifndef NDEBUG
      fprintf(stderr, "NDB: table share %s with use_count %d state: %s(%u) not freed\n", share->key_string(),
              share->use_count, get_share_state_string(share->state), (uint)share->state);
      /**
       ...
       */
      if (Ndb_dist_priv_util::is_distributed_priv_table(share->db, share->table_name))
      {
        save--;
      }
#endif
...
void ha_ndbcluster::print_error(int error, myf errflag)
{
  ...
  {
    if (error == HA_ERR_FOUND_DUPP_KEY && (table == NULL || table->file == NULL))
    {
      /*
...
ha_rows ha_ndbcluster::records_in_range(uint inx, key_range *min_key, key_range *max_key)
{
  ...
  if ((idx_type != ORDERED_INDEX) &&
      ((min_key && min_key->length == key_length) && (max_key && max_key->length == key_length) &&
       (min_key->key == max_key->key || memcmp(min_key->key, max_key->key, key_length) == 0)))
    DBUG_RETURN(1);
  ...
  if ((idx_type == PRIMARY_KEY_ORDERED_INDEX || idx_type == UNIQUE_ORDERED_INDEX || idx_type == ORDERED_INDEX))
  {
    ...
    if (g_ndb_records_in_range_tree_dive)
    {
      ...
      do
      {
        if ((trans = active_trans) == NULL || trans->commitStatus() != NdbTransaction::Started)
        {
        ...
        NdbIndexStat is;
        if (is.records_in_range(index, trans, d.ndb_record_key, m_ndb_record, &ib, 0, &rows, 0) == -1)
  ...
  */
  do
  {
    ...
    if (!min_key_length)
    {
      rows = (!max_key_length) ? table_rows        // No range was specified
    ...
    else
    {
      ...
      for (; key_part != end; key_part++)
      {
        ...
        if (eq_bound_offs + part_length > bounds_len ||
            memcmp(&min_key->key[eq_bound_offs], &max_key->key[eq_bound_offs], part_length))
        {
          break;
        }
...
{
  THD *thd = current_thd;
  ulonglong f = HA_REC_NOT_IN_SEQ | HA_NULL_IN_KEY | HA_AUTO_PART_KEY | HA_NO_PREFIX_CHAR_KEYS | HA_CAN_GEOMETRY |
                HA_CAN_BIT_FIELD | HA_PRIMARY_KEY_REQUIRED_FOR_POSITION | HA_PARTIAL_COLUMN_READ |
                HA_HAS_OWN_BINLOGGING | HA_BINLOG_ROW_CAPABLE | HA_HAS_RECORDS | HA_READ_BEFORE_WRITE_REMOVAL | 0;
...
uint ndb_get_commitcount(THD *thd, char *norm_name, Uint64 *commit_count)
{
  ...
  native_mutex_lock(&ndbcluster_mutex);
  if (!(share = (NDB_SHARE *)my_hash_search(&ndbcluster_open_tables, (const uchar *)norm_name, strlen(norm_name))))
  {
    native_mutex_unlock(&ndbcluster_mutex);
    DBUG_PRINT("info", ("Table %s not found in ndbcluster_open_tables", norm_name));
    DBUG_RETURN(1);
  }
  ...
  struct Ndb_statistics stat;
  {
    ...
    if (ndbtab_g.get_table() == 0 ||
        ndb_get_table_statistics(thd, NULL, FALSE, ndb, ndbtab_g.get_table()->getDefaultRecord(), &stat))
    {
      /* ndb_share reference temporary free */
      DBUG_PRINT("NDB_SHARE", ("%s temporary free  use_count: %u", share->key_string(), share->use_count));
      free_share(&share);
...
static my_bool ndbcluster_cache_retrieval_allowed(THD *thd, char *full_name, uint full_name_len, ulonglong *engine_data)
{
  ...
  if (thd_options(thd) & (OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))
  {
    ...
    if (!thd_ndb->changed_tables.is_empty())
    {
      ...
      while ((share = it++))
      {
        if (strcmp(share->table_name, tabname) == 0 && strcmp(share->db, dbname) == 0)
        {
...
my_bool ha_ndbcluster::register_query_cache_table(THD *thd, char *full_name, size_t full_name_len,
...
#define dbug_print_open_tables() DBUG_EXECUTE("info", print_ndbcluster_open_tables(););
...
int handle_trailing_share(THD *thd, NDB_SHARE *share)
{
  ...
  if (opt_ndb_extra_logging > 9)
    sql_print_information("handle_trailing_share: %s use_count: %u", share->key_string(), share->use_count);
  DBUG_PRINT("NDB_SHARE", ("%s temporary  use_count: %u", share->key_string(), share->use_count));
  native_mutex_unlock(&ndbcluster_mutex);
  ...
  */
  ...
  DBUG_PRINT("info", ("NDB_SHARE: %s already exists use_count=%d, op=0x%lx.", share->key_string(), share->use_count,
                      (long)share->op));
  /*
  ...
  */
...

  {
    ...
    */
    ...
    my_snprintf(leak_name_buf, sizeof(leak_name_buf), "#leak%lu", trailing_share_id++);
...
int ndbcluster_rename_share(THD *thd, NDB_SHARE *share, NDB_SHARE_KEY *new_key)
{
  ...
  {
    NDB_SHARE *tmp = (NDB_SHARE *)my_hash_search(&ndbcluster_open_tables, NDB_SHARE::key_get_key(new_key),
  ...
  if (my_hash_insert(&ndbcluster_open_tables, (uchar *)share))
  {
    ...
    if (my_hash_insert(&ndbcluster_open_tables, (uchar *)share))
    {
      sql_print_error("ndbcluster_rename_share: failed to recover %s", share->key_string());
      DBUG_PRINT("error", ("Failed to reinsert share with old name %s", share->key_string()));
    }
    native_mutex_unlock(&ndbcluster_mutex);
    DBUG_RETURN(-1);
  }
  ...
  if (event_data && event_data->shadow_table)
  {
    if (!IS_TMP_PREFIX(share->table_name))
    {
      ...
      // released.
      lex_string_copy(&event_data->mem_root, &event_data->shadow_table->s->db, share->db);
      lex_string_copy(&event_data->mem_root, &event_data->shadow_table->s->table_name, share->table_name);
    }
    else
    {
...
NDB_SHARE *ndbcluster_get_share(NDB_SHARE *share)
{
  ...
  if (opt_ndb_extra_logging > 9)
    sql_print_information("ndbcluster_get_share: %s use_count: %u", share->key_string(), share->use_count);
  native_mutex_unlock(&ndbcluster_mutex);
  return share;
}

NDB_SHARE *NDB_SHARE::create(const char *key, TABLE *table)
{
  NDB_SHARE *share;
  if (!(share = (NDB_SHARE *)my_malloc(PSI_INSTRUMENT_ME, sizeof(*share), MYF(MY_WME | MY_ZEROFILL))))
    return NULL;
...
static inline NDB_SHARE *ndbcluster_get_share(const char *key, TABLE *table, bool create_if_not_exists)
{
  NDB_SHARE *share;
  DBUG_ENTER("ndbcluster_get_share");
  DBUG_PRINT("enter", ("key: '%s'", key));

  if (!(share = (NDB_SHARE *)my_hash_search(&ndbcluster_open_tables, (const uchar *)key, strlen(key))))
  {
...
NDB_SHARE *ndbcluster_get_share(const char *key, TABLE *table, bool create_if_not_exists, bool have_lock)
{
  NDB_SHARE *share;
  DBUG_ENTER("ndbcluster_get_share");
  DBUG_PRINT("enter", ("key: '%s', create_if_not_exists: %d, have_lock: %d", key, create_if_not_exists, have_lock));
...
void ndbcluster_real_free_share(NDB_SHARE **share)
{
  ...
  if (opt_ndb_extra_logging > 9)
    sql_print_information("ndbcluster_real_free_share: %s use_count: %u", (*share)->key_string(), (*share)->use_count);
...
void ndbcluster_free_share(NDB_SHARE **share, bool have_lock)
{
  ...
  {
    if (opt_ndb_extra_logging > 9)
      sql_print_information("ndbcluster_free_share: %s use_count: %u", (*share)->key_string(), (*share)->use_count);
    ndbcluster_real_free_share(share);
  }
  else
  {
...
void ndbcluster_mark_share_dropped(NDB_SHARE *share)
{
  ...
  if (opt_ndb_extra_logging > 9)
  {
    sql_print_information("ndbcluster_mark_share_dropped: %s use_count: %u", share->key_string(), share->use_count);
  }
}
...
int ha_ndbcluster::update_stats(THD *thd, bool do_read_stat, uint part_id)
{
  ...
  do
  {
    ...
    if (int err = ndb_get_table_statistics(thd, this, TRUE, ndb, m_ndb_record, &stat, part_id))
    {
      DBUG_RETURN(err);
    }
  ...
  DBUG_PRINT("exit", ("stats.records: %d  "
                      ...
                      (int)stats.records, (int)stat.row_count, (int)no_uncommitted_rows_count,
                      (uint)stat.fragment_extent_space, (uint)stat.fragment_extent_free_space));
  DBUG_RETURN(0);
}
...
static void modify_shared_stats(NDB_SHARE *share, Ndb_local_table_statistics *local_stat)
{
  if (local_stat->no_uncommitted_rows_count)
  {
    ...
    {
      DBUG_PRINT("info", ("Update row_count for %s, row_count: %lu, with:%d", share->table_name,
                          (ulong)share->stat.row_count, local_stat->no_uncommitted_rows_count));
      share->stat.row_count = ((Int64)share->stat.row_count + local_stat->no_uncommitted_rows_count > 0)
 ...
 */
static int ndb_get_table_statistics(THD *thd, ha_ndbcluster *file, bool report_error, Ndb *ndb, const NdbRecord *record,
                                    struct Ndb_statistics *ndbstat, uint part_id)
{
  ...
  */
  ...
  if ((code.interpret_exit_last_row() != 0) || (code.finalise() != 0))
  {
    ...
    DBUG_PRINT("exit", ("failed, reterr: %u, NdbError %u(%s)", reterr, error.code, error.message));
    DBUG_RETURN(reterr);
  }
  ...
  do
  {
    ...
    NdbScanOperation::ScanOptions options;
    options.optionsPresent = NdbScanOperation::ScanOptions::SO_BATCH | NdbScanOperation::ScanOptions::SO_GETVALUE |
    ...
    if ((pOp = pTrans->scanTable(record, NdbOperation::LM_CommittedRead, empty_mask, &options,
    ...
    thd_ndb->m_execute_count++;
    DBUG_PRINT("info", ("execute_count: %u", thd_ndb->m_execute_count));
    if (pTrans->execute(NdbTransaction::NoCommit, NdbOperation::AbortOnError, TRUE) == -1)
    {
      error = pTrans->getNdbError();
      goto retry;
    }
    ...
    while ((check = pOp->nextResult(&dummyRowPtr, TRUE, TRUE)) == 0)
    {
      DBUG_PRINT("info", ("nextResult rows: %llu, commits: %llu"
                          ...
                          rows, commits, fixed_mem, var_mem, fragid, ext_space, free_ext_space));
    ...
    DBUG_PRINT("exit", ("records: %llu commits: %llu row_size: %llu "
                        ...
                        sum_rows, sum_commits, sum_row_size, sum_mem, sum_ext_space, sum_free_ext_space, count));
  ...
  retry:
    ...
    if (error.status == NdbError::TemporaryError && retries-- && !thd->killed)
    {
      do_retry_sleep(retry_sleep);
      continue;
    }
...
int ha_ndbcluster::write_ndb_file(const char *name) const
{
  ...
  (void)strxnmov(path, FN_REFLEN - 1, mysql_data_home, "/", name, ha_ndb_ext, NullS);
#else
...
static ulong multi_range_entry_size(my_bool use_keyop, ulong reclength)
{
  /* Space for type byte. */
  ulong len = 1;
  if (use_keyop)
...
static void multi_range_put_custom(HANDLER_BUFFER *buffer, int range_no, char *custom)
{
  assert(range_no < MRR_MAX_RANGES);
...
/*
  ...
  instead.
*/
static my_bool read_multi_needs_scan(NDB_INDEX_TYPE cur_index_type, const KEY *key_info, const KEY_MULTI_RANGE *r,
                                     bool is_pushed)
{
  ...
  if (cur_index_type == PRIMARY_KEY_INDEX)
    return FALSE;
  if (cur_index_type == UNIQUE_INDEX)
  {  // a 'UNIQUE ... USING HASH' index
  ...
  assert(cur_index_type == PRIMARY_KEY_ORDERED_INDEX || cur_index_type == UNIQUE_ORDERED_INDEX);
  if (r->start_key.length != key_info->key_length || r->start_key.flag != HA_READ_KEY_EXACT)
    return TRUE;  // Not exact match, need scan
  if (cur_index_type == UNIQUE_ORDERED_INDEX && check_null_in_key(key_info, r->start_key.key, r->start_key.length))
...
ha_rows ha_ndbcluster::multi_range_read_info_const(uint keyno, RANGE_SEQ_IF *seq, void *seq_init_param, uint n_ranges,
                                                   uint *bufsz, uint *flags, Cost_estimate *cost)
{
  ...
  rows = handler::multi_range_read_info_const(keyno, seq, seq_init_param, n_ranges, &def_bufsz, &def_flags, cost);
  ...
  */
  if ((*flags & HA_MRR_USE_DEFAULT_IMPL) || choose_mrr_impl(keyno, n_ranges, rows, bufsz, flags, cost))
  {
...
ha_rows ha_ndbcluster::multi_range_read_info(uint keyno, uint n_ranges, uint n_rows, uint *bufsz, uint *flags,
                                             Cost_estimate *cost)
{
  ha_rows res;
  uint def_flags = *flags;
  ...
  /* Get cost/flags/mem_usage of default MRR implementation */
  res = handler::multi_range_read_info(keyno, n_ranges, n_rows, &def_bufsz, &def_flags, cost);
  ...
  if ((*flags & HA_MRR_USE_DEFAULT_IMPL) || choose_mrr_impl(keyno, n_ranges, n_rows, bufsz, flags, cost))
  {
...
bool ha_ndbcluster::choose_mrr_impl(uint keyno, uint n_ranges, ha_rows n_rows, uint *bufsz, uint *flags,
                                    Cost_estimate *cost)
{
  THD *thd = current_thd;
  ...
  if (!thd->optimizer_switch_flag(OPTIMIZER_SWITCH_MRR) || uses_blob_value(table->read_set) ||
      (key_type == UNIQUE_INDEX && has_null_in_unique_index(keyno) && !(*flags & HA_MRR_NO_NULL_ENDPOINTS)))
  {
   ...
   */
  {
    ...
    DBUG_PRINT("info", ("MRR bufsize suggested=%u want=%u limit=%d", save_bufsize, (uint)(n_rows + 1) * entry_size,
    ...
    else
    {
      ...
      *bufsz = min(save_bufsize, (uint)(n_rows * entry_size + multi_range_fixed_size(max_ranges)));
    }
...
int ha_ndbcluster::multi_range_read_init(RANGE_SEQ_IF *seq_funcs, void *seq_init_param, uint n_ranges, uint mode,
                                         HANDLER_BUFFER *buffer)
{
  ...
  */
  ...
  if (mode & HA_MRR_USE_DEFAULT_IMPL ||
      bufsize <
          multi_range_fixed_size(1) + multi_range_max_entry(get_index_type(active_index), table_share->reclength) ||
      (m_pushed_join_operation == PUSHED_ROOT && !m_disable_pushed_join &&
       !m_pushed_join_member->get_query_def().isScanQuery()) ||
      m_delete_cannot_batch || m_update_cannot_batch)
  {
    m_disable_multi_read = TRUE;
    DBUG_RETURN(handler::multi_range_read_init(seq_funcs, seq_init_param, n_ranges, mode, buffer));
  }
...
int ha_ndbcluster::multi_range_start_retrievals(uint starting_range)
{
  ...
  NdbTransaction *trans = m_thd_ndb->trans;
  int error;
  const bool is_pushed = check_if_pushable(NdbQueryOperationDef::OrderedIndexScan, active_index);
  ...
  */
  uint min_entry_size =
      multi_range_entry_size(!read_multi_needs_scan(cur_index_type, key_info, &mrr_cur_range, is_pushed), reclength);
  const ulong bufsize = multi_range_buffer_size(multi_range_buffer);
  int max_range = multi_range_max_ranges(ranges_in_seq, bufsize - min_entry_size);
  assert(max_range > 0);
  ...
  for (; !m_range_res; range_no++, m_range_res = mrr_funcs.next(mrr_iter, &mrr_cur_range))
  {
    if (range_no >= max_range)
      break;
    my_bool need_scan = read_multi_needs_scan(cur_index_type, key_info, &mrr_cur_range, is_pushed);
    ...
    if (m_use_partition_pruning)
    {
      get_partition_set(table, table->record[0], active_index, &mrr_cur_range.start_key, &part_spec);
      DBUG_PRINT("info",
                 ("part_spec.start_part: %u  part_spec.end_part: %u", part_spec.start_part, part_spec.end_part));
      /*
        If partition pruning has found no partition in set
      ...
      */
      ...
      if (!trans && (part_spec.start_part == part_spec.end_part))
        if (unlikely(!(trans = start_transaction_part_id(part_spec.start_part, error))))
          DBUG_RETURN(error);
    }
    ...
    if (need_scan)
    {
      ...
      else if (!m_multi_cursor)
      {
        ...
        options.optionsPresent =
            NdbScanOperation::ScanOptions::SO_SCANFLAGS | NdbScanOperation::ScanOptions::SO_PARALLEL;

        options.scan_flags = NdbScanOperation::SF_ReadRangeNo | NdbScanOperation::SF_MultiRange;
        ...
        NdbIndexScanOperation *scanOp =
            trans->scanIndex(m_index[active_index].ndb_record_key, m_ndb_record, lm, (uchar *)(table->read_set->bitmap),
       ...
       */
      if (m_use_partition_pruning && m_user_defined_partitioning && (part_spec.start_part == part_spec.end_part))
      {
        DBUG_PRINT("info", ("Range on user-def-partitioned table can be pruned to part %u", part_spec.start_part));
      ...
      NdbIndexScanOperation::IndexBound bound;
      compute_index_bounds(bound, key_info, &mrr_cur_range.start_key, &mrr_cur_range.end_key, 0);
      ...
      {
        if (m_multi_cursor->setBound(m_index[active_index].ndb_record_key, bound,
    ...
    else
    {
      ...
      if (!trans)
      {
        ...
        if (unlikely(!(trans = start_transaction_key(active_index, mrr_cur_range.start_key.key, error))))
          DBUG_RETURN(error);
      }
      ...
      if (m_read_before_write_removal_used)
      {
         ...
         */
        key_restore(multi_range_row(row_buf), (uchar *)mrr_cur_range.start_key.key, key_info, key_info->key_length);
      ...
      else
      {
        ...
        if (m_user_defined_partitioning &&
            (cur_index_type == PRIMARY_KEY_ORDERED_INDEX || cur_index_type == PRIMARY_KEY_INDEX))
        {
            ...
            !m_pushed_join_member->get_query_def().isScanQuery())
        {
          ...
          assert(lm == NdbOperation::LM_CommittedRead);
          const int error = pk_unique_index_read_key_pushed(active_index, mrr_cur_range.start_key.key, ppartitionId);
          if (unlikely(error))
            DBUG_RETURN(error);
        }
        else
        {
          ...
          if (!(op = pk_unique_index_read_key(active_index, mrr_cur_range.start_key.key, multi_range_row(row_buf), lm,
                                              ppartitionId)))
            ERR_RETURN(trans->getNdbError());
        }
      }
  ...
  if (m_active_query != NULL && m_pushed_join_member->get_query_def().isScanQuery())
  {
    m_thd_ndb->m_scan_count++;
    if (mrr_is_output_sorted)
    {
  ...
  if (m_multi_cursor)
  {
    DBUG_PRINT("info", ("Is MRR scan pruned to 1 partition? :%u", m_multi_cursor->getPruned()));
    m_thd_ndb->m_scan_count++;
  ...
  if (!m_range_res)
  {
    DBUG_PRINT("info", ("Split MRR read, %d-%d of %d bufsize=%lu used=%lu range_no=%d", starting_range,
                        mrr_range_no - 1, ranges_in_seq, (ulong)(end_of_buffer - multi_range_buffer->buffer),
    ...
    while (first_running_range < first_unstarted_range)
    {
      ...
      switch (multi_range_entry_type(row_buf))
      {
        ...
        case enum_unique_range:
          ...
          */
          first_running_range++;
          m_multi_range_result_ptr = multi_range_next_entry(m_multi_range_result_ptr, table_share->reclength);
          ...
          */
          ...
          /* Return the record. */
          *range_info = multi_range_get_custom(multi_range_buffer, expected_range_no);
          memcpy(table->record[0], multi_range_row(row_buf), table_share->reclength);
          DBUG_RETURN(0);
        ...
        case enum_ordered_range:
          ...
          else
          {
            ...
            */
            ...
            {
              *range_info = multi_range_get_custom(multi_range_buffer, current_range_no);
...
static int ndbcluster_make_pushed_join(handlerton *hton, THD *thd, const AQP::Join_plan *plan)
{
      ...
      ndb_join_pushdown(g_ndb_cluster_connection->get_min_db_version()))
  {
    ...
    for (uint i = 0; i < plan->get_access_count() - 1; i++)
    {
      ...
      if (pushed_join != NULL)
      {
        ha_ndbcluster *const handler = static_cast<ha_ndbcluster *>(join_root->get_table()->file);
...
int ha_ndbcluster::assign_pushed_join(const ndb_pushed_join *pushed_join)
{
  ...
  DBUG_PRINT("info", ("Assigned pushed join with %d child operations", pushed_join->get_operation_count() - 1));
                                      ...
                                      uint idx) const
{
  ...
  return m_pushed_join_operation == PUSHED_ROOT && m_pushed_join_member != NULL &&
         m_pushed_join_member->match_definition(type, (idx < MAX_KEY) ? &m_index[idx] : NULL);
}
...
int ha_ndbcluster::create_pushed_join(const NdbQueryParamValue *keyFieldParams, uint paramCnt)
{
  ...
  NdbQuery *const query = m_pushed_join_member->make_query_instance(m_thd_ndb->trans, keyFieldParams, paramCnt);
  ...
  for (uint i = 0; i < m_pushed_join_member->get_operation_count(); i++)
  {
    ...
    int res = op->setResultRowRef(resultRec, handler->_m_next_row, (uchar *)(tab->read_set->bitmap));
    if (unlikely(res))
      ERR_RETURN(query->getNdbError());
...
*/
Ndb_util_thread::Ndb_util_thread() : Ndb_component("Util")
{
...
void Ndb_util_thread::do_run()
{
  ...
  charset_connection = get_charset_by_csname("utf8", MY_CS_PRIMARY, MYF(MY_WME));
  ...
  */
  ...
  for (;;)
  {
    native_mutex_lock(&LOCK);
    if (!is_stop_requested())
      native_cond_timedwait(&COND, &LOCK, &abstime);
    if (is_stop_requested()) /* Stopping thread */
      goto ndb_util_thread_end;
    native_mutex_unlock(&LOCK);
#ifdef NDB_EXTRA_DEBUG_UTIL_THREAD
    DBUG_PRINT("ndb_util_thread", ("Started, cache_check_time: %lu", opt_ndb_cache_check_time));
#endif
    ...
    */
    ...
    for (i = 0, open_count = 0; i < record_count; i++)
    {
      ...
      if ((share->use_count - (int)(share->op != 0) - (int)(share->op != 0)) <= 0)
    ...
    {
      share = share_list[i];
      if ((share->use_count - (int)(share->op != 0) - (int)(share->op != 0)) <= 1)
      {
        /*
      ...
      DBUG_PRINT("ndb_util_thread", ("Fetching commit count for: %s", share->key_string()));
      ...
      native_mutex_unlock(&share->mutex);
      {
        ...
        if (ndbtab_g.get_table() &&
            ndb_get_table_statistics(thd, NULL, FALSE, ndb, ndbtab_g.get_table()->getDefaultRecord(), &stat) == 0)
        {
          DBUG_PRINT("info", ("Table: %s, commit_count: %llu,  rows: %llu", share->key_string(), stat.commit_count,
                              stat.row_count));
        }
        else
        {
          DBUG_PRINT("ndb_util_thread", ("Error: Could not get commit count for table %s", share->key_string()));
          stat.commit_count = 0;
        }
      }
    loop_next:
...
*/
bool ndbcluster_show_status(handlerton *hton, THD *thd, stat_print_fn *stat_print, enum ha_stat_type stat_type)
{
  char name[16];
  char buf[IO_SIZE];
  uint buflen;
  ...
  buflen = (uint)my_snprintf(buf, sizeof(buf),
                             ...
                             "connect_count=%ld",
                             ns.cluster_node_id, ns.connected_host, ns.connected_port, ns.number_of_data_nodes,
                             ns.number_of_ready_data_nodes, ns.connect_count);
  if (stat_print(thd, ndbcluster_hton_name, ndbcluster_hton_name_length, STRING_WITH_LEN("connection"), buf, buflen))
    DBUG_RETURN(TRUE);

  for (int i = 0; i < MAX_NDB_NODES; i++)
  {
    if (ns.transaction_hint_count[i] > 0 || ns.transaction_no_hint_count[i] > 0)
    {
      uint namelen = (uint)my_snprintf(name, sizeof(name), "node[%d]", i);
      buflen = (uint)my_snprintf(buf, sizeof(buf), "transaction_hint=%ld, transaction_no_hint=%ld",
                                 ns.transaction_hint_count[i], ns.transaction_no_hint_count[i]);
      if (stat_print(thd, ndbcluster_hton_name, ndbcluster_hton_name_length, name, namelen, buf, buflen))
        DBUG_RETURN(TRUE);
    }
  }
  ...
  if (ndb)
  {
    ...
    {
      buflen = (uint)my_snprintf(buf, sizeof(buf), "created=%u, free=%u, sizeof=%u", tmp.m_created, tmp.m_free,
                                 tmp.m_sizeof);
      if (stat_print(thd, ndbcluster_hton_name, ndbcluster_hton_name_length, tmp.m_name, (uint)strlen(tmp.m_name), buf,
                     buflen))
        DBUG_RETURN(TRUE);
    }
  }
  ...
  if (buflen)
  {
    if (stat_print(thd, ndbcluster_hton_name, ndbcluster_hton_name_length, STRING_WITH_LEN("binlog"), buf, buflen))
      DBUG_RETURN(TRUE);
  }
...
int ha_ndbcluster::get_default_num_partitions(HA_CREATE_INFO *create_info)
{
  ...
  uint no_fragments = get_no_fragments(max_rows >= min_rows ? max_rows : min_rows);
  uint reported_frags;
...
uint32 ha_ndbcluster::calculate_key_hash_value(Field **field_array)
{
  ...
  key_data[i].ptr = 0;
  if ((ret_val = Ndb::computeHash(&hash_value, m_table, key_data_ptr, buf, sizeof(tmp))))
  {
...
static TYPELIB distribution_typelib = {array_elements(distribution_names) - 1, "", distribution_names, NULL};
static MYSQL_SYSVAR_ENUM(distribution,                                                            /* name */
                         opt_ndb_distribution,                                                    /* var */
                         PLUGIN_VAR_RQCMDARG, "Default distribution for new tables in ndb", NULL, /* check func. */
                         NULL,                                                                    /* update func. */
...
static int create_table_set_range_data(const partition_info *part_info, NdbDictionary::Table &ndbtab)
{
...
static int create_table_set_list_data(const partition_info *part_info, NdbDictionary::Table &ndbtab)
{
  const uint num_list_values = part_info->num_list_values;
  int32 *list_data = (int32 *)my_malloc(PSI_INSTRUMENT_ME, num_list_values * 2 * sizeof(int32), MYF(0));
                                              ...
                                              NdbDictionary::Table &ndbtab)
{
  ...
  if (part_info->part_type == HASH_PARTITION && part_info->list_of_part_fields == TRUE)
  {
  ...
  else
  {
    ...
    {
      push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
                          ER(ER_ILLEGAL_HA_CREATE_OPTION), ndbcluster_hton_name,
...
enum_alter_inplace_result ha_ndbcluster::check_if_supported_inplace_alter(TABLE *altered_table,
                                                                          Alter_inplace_info *ha_alter_info)
{
  ...
  const Alter_inplace_info::HA_ALTER_FLAGS supported =
      Alter_inplace_info::ADD_INDEX | Alter_inplace_info::DROP_INDEX | Alter_inplace_info::ADD_UNIQUE_INDEX |
      Alter_inplace_info::DROP_UNIQUE_INDEX | Alter_inplace_info::ADD_COLUMN |
      Alter_inplace_info::ALTER_COLUMN_DEFAULT | Alter_inplace_info::ALTER_COLUMN_STORAGE_TYPE |
      Alter_inplace_info::ALTER_COLUMN_COLUMN_FORMAT | Alter_inplace_info::ADD_PARTITION |
      Alter_inplace_info::ALTER_TABLE_REORG | Alter_inplace_info::CHANGE_CREATE_OPTION |
  ...
  Alter_inplace_info::HA_ALTER_FLAGS add_column = Alter_inplace_info::ADD_COLUMN;
  ...
  DBUG_PRINT("info", ("alter_flags & not_supported 0x%llx", alter_flags & not_supported));

  bool auto_increment_value_changed = false;
  bool max_rows_changed = false;
  if (alter_flags & Alter_inplace_info::CHANGE_CREATE_OPTION)
  {
    if (create_info->auto_increment_value != table->file->stats.auto_increment_value)
  ...
  if (alter_flags & Alter_inplace_info::ALTER_COLUMN_DEFAULT && !(alter_flags & Alter_inplace_info::ADD_COLUMN))
  {
  ...
  {
    DBUG_PRINT("info", ("Detected unsupported change: 0x%llx", alter_flags & not_supported));
    DBUG_RETURN(HA_ALTER_INPLACE_NOT_SUPPORTED);
  }

  if (alter_flags & Alter_inplace_info::ADD_COLUMN || alter_flags & Alter_inplace_info::ADD_PARTITION ||
      alter_flags & Alter_inplace_info::ALTER_TABLE_REORG || max_rows_changed)
  {
    ...
    if (alter_flags & Alter_inplace_info::ADD_COLUMN)
    {
      ...
      */
      if (table_share->primary_key == MAX_KEY || part_info->part_type != HASH_PARTITION ||
      ...
      for (uint i = table->s->fields; i < altered_table->s->fields; i++)
      {
        ...
        DBUG_PRINT("info", ("storage_type %i, column_format %i", (uint)field->field_storage_type(),
        ...
        set_my_errno(create_ndb_column(thd, col, field, create_info, COLUMN_FORMAT_TYPE_DYNAMIC));
        if (my_errno())
        {
  ...
  */
  ...
  {
    if (((altered_table->s->keys - table->s->keys) != 1) || (alter_flags & dropping))
    {
  ...
  */
  ...
  {
    if (((table->s->keys - altered_table->s->keys) != 1) || (alter_flags & adding))
    {
  ...
  for (uint i = 0; i < table->s->fields; i++)
  {
    ...
    if ((field->flags & FIELD_IN_ADD_INDEX) && (col->getStorageType() == NdbDictionary::Column::StorageTypeDisk))
    {
  ...
  else
  {
    ...
    if (create_info->used_fields & HA_CREATE_USED_AUTO && get_row_type() != create_info->row_type)
    {
...
bool ha_ndbcluster::prepare_inplace_alter_table(TABLE *altered_table, Alter_inplace_info *ha_alter_info)
{
  int error = 0;
  uint i;
  THD *thd = current_thd;
  ...
  const Alter_inplace_info::HA_ALTER_FLAGS alter_flags = ha_alter_info->handler_flags;
  ...
  DBUG_PRINT("info", ("altered_table: '%s, alter_flags: 0x%llx", altered_table->s->table_name.str, alter_flags));
  ...
  if (alter_flags & adding)
  {
    ...
    key = key_info;
    for (idx_p = ha_alter_info->index_add_buffer, idx_end_p = idx_p + ha_alter_info->index_add_count; idx_p < idx_end_p;
         idx_p++, key++)
    {
      /* Copy the KEY struct. */
  ...
  if (alter_flags & dropping)
  {
    ...
    for (idx_p = ha_alter_info->index_drop_buffer, idx_end_p = idx_p + ha_alter_info->index_drop_count;
         idx_p < idx_end_p; idx_p++, keyno_p++)
    {
    ...
    */
    if ((error = prepare_drop_index(table, key_numbers, ha_alter_info->index_drop_count)))
    {
      table->file->print_error(error, MYF(0));
      goto abort;
    }
  ...
  if (alter_flags & Alter_inplace_info::ADD_COLUMN)
  {
    ...
    for (i = table->s->fields; i < altered_table->s->fields; i++)
    {
      ...
      */
      if (field->column_format() == COLUMN_FORMAT_TYPE_DEFAULT && create_info->row_type == ROW_TYPE_DEFAULT &&
          col.getDynamic())
      {
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_ILLEGAL_HA_CREATE_OPTION,
  ...
  if (alter_flags & Alter_inplace_info::ALTER_TABLE_REORG || alter_flags & Alter_inplace_info::ADD_PARTITION ||
      max_rows_changed)
  {
    if (alter_flags & Alter_inplace_info::ALTER_TABLE_REORG)
    {
  ...
  if (alter_flags & Alter_inplace_info::ADD_FOREIGN_KEY)
  {
    ...
    if (res != 0)
    {
      ...
      */
      ...
      {
        DBUG_PRINT("info", ("change error %d to %d", HA_ERR_CANNOT_ADD_FOREIGN, ER_CANNOT_ADD_FOREIGN));
...
int ha_ndbcluster::alter_frm(const char *file, NDB_ALTER_DATA *alter_data)
{
  ...
  if (readfrm(file, &data, &length) || packfrm(data, length, &pack_data, &pack_length))
  {
    ...
    error = 1;
    my_error(ER_FILE_NOT_FOUND, MYF(0), file, my_errno(), my_strerror(errbuf, sizeof(errbuf), my_errno()));
  }
  else
  {
    DBUG_PRINT("info", ("Table %s has changed, altering frm in ndb", m_tabname));
  ...
  DBUG_PRINT("NDB_SHARE", ("%s binlog schema(?) free  use_count: %u", m_share->key_string(), m_share->use_count));

  DBUG_RETURN(error);
}

bool ha_ndbcluster::inplace_alter_table(TABLE *altered_table, Alter_inplace_info *ha_alter_info)
{
  ...
  if (!error)
  {
    ...
    {
      error = ndb_to_mysql_error(&dict->getNdbError());
      DBUG_PRINT("info", ("Failed to commit schema transaction, error %u", error));
      table->file->print_error(error, MYF(0));
      goto err;
    }
    if (auto_increment_value_changed)
...
bool ha_ndbcluster::commit_inplace_alter_table(TABLE *altered_table, Alter_inplace_info *ha_alter_info, bool commit)
{
  ...
  ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, db, name, table_id, table_version,
                           SOT_ONLINE_ALTER_TABLE_PREPARE, NULL, NULL);
  delete alter_data;
  ha_alter_info->handler_ctx = 0;
...
bool ha_ndbcluster::abort_inplace_alter_table(TABLE *altered_table, Alter_inplace_info *ha_alter_info)
{
...
void ha_ndbcluster::notify_table_changed()
{
  ...
  */
  ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, db, name, table_id, table_version,
                           SOT_ONLINE_ALTER_TABLE_COMMIT, NULL, NULL);

  DBUG_VOID_RETURN;
}

static bool set_up_tablespace(st_alter_tablespace *alter_info, NdbDictionary::Tablespace *ndb_ts)
{
...
static bool set_up_datafile(st_alter_tablespace *alter_info, NdbDictionary::Datafile *ndb_df)
{
...
static bool set_up_logfile_group(st_alter_tablespace *alter_info, NdbDictionary::LogfileGroup *ndb_lg)
{
...
static bool set_up_undofile(st_alter_tablespace *alter_info, NdbDictionary::Undofile *ndb_uf)
{
                                     ...
                                     LEX_CSTRING *tablespace_name)
{
  ...
  DBUG_PRINT("enter", ("db_name: %s, table_name: %s", db_name.str, table_name.str));
...
static int ndbcluster_alter_tablespace(handlerton *hton, THD *thd, st_alter_tablespace *alter_info)
{
  int is_tablespace = 0;
  NdbError err;
  NDBDICT *dict;
  ...
  switch (alter_info->ts_cmd_type)
  {
    case (CREATE_TABLESPACE):
    {
      ...
      {
        push_warning_printf(current_thd, Sql_condition::SL_WARNING, dict->getWarningFlags(),
      ...
      if (dict->createDatafile(ndb_df))
      {
        ...
        if (dict->getNdbError().code == 0 && tmp.getObjectId() == objid.getObjectId() &&
      ...
      if (dict->getWarningFlags() & NdbDictionary::Dictionary::WarnDatafileRoundUp)
      {
        ...
        if (dict->getWarningFlags() & NdbDictionary::Dictionary::WarnDatafileRoundDown)
        {
          push_warning_printf(current_thd, Sql_condition::SL_WARNING, dict->getWarningFlags(),
    ...
    case (ALTER_TABLESPACE):
    {
      ...
      else if (alter_info->ts_alter_tablespace_type == ALTER_TABLESPACE_DROP_FILE)
      {
        ...
        if (ts.getObjectId() == objid.getObjectId() && strcmp(df.getPath(), alter_info->data_file_name) == 0)
        {
      ...
      {
        DBUG_PRINT("error", ("Unsupported alter tablespace: %d", alter_info->ts_alter_tablespace_type));
    ...
    case (CREATE_LOGFILE_GROUP):
    {
      ...
      table_id = objid.getObjectId();
      table_version = objid.getObjectVersion();
      if (dict->getWarningFlags() & NdbDictionary::Dictionary::WarnUndobufferRoundUp)
      {
        push_warning_printf(current_thd, Sql_condition::SL_WARNING, dict->getWarningFlags(),
      ...
      if (dict->getWarningFlags() & NdbDictionary::Dictionary::WarnUndofileRoundDown)
      {
        push_warning_printf(current_thd, Sql_condition::SL_WARNING, dict->getWarningFlags(),
    ...
    case (DROP_LOGFILE_GROUP):
    {
      error = ER_DROP_FILEGROUP_FAILED;
      errmsg = "LOGFILE GROUP";
      NdbDictionary::LogfileGroup lg = dict->getLogfileGroup(alter_info->logfile_group_name);
  ...
  if (is_tablespace)
    ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, "", alter_info->tablespace_name, table_id,
                             table_version, SOT_TABLESPACE, NULL, NULL);
  else
    ndbcluster_log_schema_op(thd, thd->query().str, thd->query().length, "", alter_info->logfile_group_name, table_id,
                             table_version, SOT_LOGFILE_GROUP, NULL, NULL);
  DBUG_RETURN(FALSE);
...
static int ndbcluster_fill_files_table(handlerton *hton, THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  for (i = 0; i < dflist.count; i++)
  {
    ...
    while ((id = g_ndb_cluster_connection->get_next_alive_node(iter)))
    {
      ...
      table->field[IS_FILES_FILE_NAME]->store(elt.name, (uint)strlen(elt.name), system_charset_info);
      table->field[IS_FILES_FILE_TYPE]->set_notnull();
      table->field[IS_FILES_FILE_TYPE]->store("DATAFILE", 8, system_charset_info);
      table->field[IS_FILES_TABLESPACE_NAME]->set_notnull();
      table->field[IS_FILES_TABLESPACE_NAME]->store(df.getTablespace(), (uint)strlen(df.getTablespace()),
                                                    system_charset_info);
      table->field[IS_FILES_LOGFILE_GROUP_NAME]->set_notnull();
      table->field[IS_FILES_LOGFILE_GROUP_NAME]->store(ts.getDefaultLogfileGroup(),
                                                       (uint)strlen(ts.getDefaultLogfileGroup()), system_charset_info);
      table->field[IS_FILES_ENGINE]->set_notnull();
      table->field[IS_FILES_ENGINE]->store(ndbcluster_hton_name, ndbcluster_hton_name_length, system_charset_info);

      table->field[IS_FILES_FREE_EXTENTS]->set_notnull();
      table->field[IS_FILES_FREE_EXTENTS]->store(df.getFree() / ts.getExtentSize(), true);
      table->field[IS_FILES_TOTAL_EXTENTS]->set_notnull();
      table->field[IS_FILES_TOTAL_EXTENTS]->store(df.getSize() / ts.getExtentSize(), true);
  ...
  for (i = 0; i < tslist.count; i++)
  {
    ...
    init_fill_schema_files_row(table);
    table->field[IS_FILES_FILE_TYPE]->set_notnull();
    table->field[IS_FILES_FILE_TYPE]->store("TABLESPACE", 10, system_charset_info);

    table->field[IS_FILES_TABLESPACE_NAME]->set_notnull();
    table->field[IS_FILES_TABLESPACE_NAME]->store(elt.name, (uint)strlen(elt.name), system_charset_info);
    table->field[IS_FILES_LOGFILE_GROUP_NAME]->set_notnull();
    table->field[IS_FILES_LOGFILE_GROUP_NAME]->store(ts.getDefaultLogfileGroup(),
                                                     (uint)strlen(ts.getDefaultLogfileGroup()), system_charset_info);
  ...
  for (i = 0; i < uflist.count; i++)
  {
    ...
    while ((id = g_ndb_cluster_connection->get_next_alive_node(iter)))
    {
      ...
      NdbDictionary::LogfileGroup lfg = dict->getLogfileGroup(uf.getLogfileGroup());
      ndberr = dict->getNdbError();
      if (ndberr.classification != NdbError::NoError)
      {
        if (ndberr.classification == NdbError::SchemaError)
      ...
      table->field[IS_FILES_FILE_NAME]->store(elt.name, (uint)strlen(elt.name), system_charset_info);
      table->field[IS_FILES_FILE_TYPE]->set_notnull();
      table->field[IS_FILES_FILE_TYPE]->store("UNDO LOG", 8, system_charset_info);
      NdbDictionary::ObjectId objid;
      uf.getLogfileGroupId(&objid);
      table->field[IS_FILES_LOGFILE_GROUP_NAME]->set_notnull();
      table->field[IS_FILES_LOGFILE_GROUP_NAME]->store(uf.getLogfileGroup(), (uint)strlen(uf.getLogfileGroup()),
                                                       system_charset_info);
      ...
      char extra[100];
      int len = (int)my_snprintf(extra, sizeof(extra), "CLUSTER_NODE=%u;UNDO_BUFFER_SIZE=%lu", id,
  ...
  for (i = 0; i < lfglist.count; i++)
  {
    ...
    table->field[IS_FILES_LOGFILE_GROUP_NAME]->set_notnull();
    table->field[IS_FILES_LOGFILE_GROUP_NAME]->store(elt.name, (uint)strlen(elt.name), system_charset_info);
    table->field[IS_FILES_LOGFILE_GROUP_NUMBER]->set_notnull();
    ...
    char extra[100];
    int len = (int)my_snprintf(extra, sizeof(extra), "UNDO_BUFFER_SIZE=%lu", (ulong)lfg.getUndoBufferSize());
    table->field[IS_FILES_EXTRA]->set_notnull();
    table->field[IS_FILES_EXTRA]->store(extra, len, system_charset_info);
    schema_table_store_record(thd, table);
  }
...
static int show_ndb_vars(THD *thd, SHOW_VAR *var, char *buff)
{
  ...
  SHOW_VAR *st_var;
  {
    char *mem = (char *)sql_alloc(sizeof(struct st_ndb_status) + sizeof(ndb_status_variables_dynamic));
...
static void cache_check_time_update(MYSQL_THD thd, struct st_mysql_sys_var *var, void *var_ptr, const void *save)
{
  push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT,
                      ER_THD(thd, ER_WARN_DEPRECATED_SYNTAX_NO_REPLACEMENT), "@@ndb_cache_check_time");

  opt_ndb_cache_check_time = *static_cast<const ulong *>(save);
}

static MYSQL_SYSVAR_ULONG(cache_check_time,         /* name */
...
static MYSQL_SYSVAR_ULONG(extra_logging,                                                       /* name */
                          ...
                          PLUGIN_VAR_OPCMDARG, "Turn on more logging in the error log.", NULL, /* check func. */
                          NULL,                                                                /* update func. */
                          1,                                                                   /* default */
                          0,                                                                   /* min */
                          0,                                                                   /* max */
...
static MYSQL_SYSVAR_UINT(cluster_connection_pool,         /* name */
                         opt_ndb_cluster_connection_pool, /* var */
                         PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
                         "Pool of cluster connections to be used by mysql server.", NULL, /* check func. */
                         NULL,                                                            /* update func. */
                         1,                                                               /* default */
                         1,                                                               /* min */
...
static int ndb_recv_thread_activation_threshold_check(MYSQL_THD thd, struct st_mysql_sys_var *var, void *save,
                                                      struct st_mysql_value *value)
{
  ...
  if (val != 0 || new_val < MIN_ACTIVATION_THRESHOLD || new_val > MAX_ACTIVATION_THRESHOLD)
  {
    return 1;
  }
...
static void ndb_recv_thread_activation_threshold_update(MYSQL_THD, struct st_mysql_sys_var *var, void *var_ptr,
                                                        const void *save)
{
  ndb_set_recv_thread_activation_threshold(opt_ndb_recv_thread_activation_threshold);
}

static MYSQL_SYSVAR_UINT(recv_thread_activation_threshold,         /* name */
...
static int ndb_recv_thread_cpu_mask_check(MYSQL_THD thd, struct st_mysql_sys_var *var, void *save,
                                          struct st_mysql_value *value)
{
...
static int ndb_recv_thread_cpu_mask_check_str(const char *str)
{
  ...
  for (i = bitmask.find(0); i != SparseBitmask::NotFound; i = bitmask.find(i + 1))
  {
...
static void ndb_recv_thread_cpu_mask_update()
{
  ndb_set_recv_thread_cpu(recv_thread_cpuid_array, recv_thread_num_cpus);
}

static void ndb_recv_thread_cpu_mask_update_func(MYSQL_THD, struct st_mysql_sys_var *var, void *var_ptr,
                                                 const void *save)
{
  ndb_recv_thread_cpu_mask_update();
}

static MYSQL_SYSVAR_STR(recv_thread_cpu_mask,         /* name */
...
extern int ndb_index_stat_option_check(MYSQL_THD, struct st_mysql_sys_var *var, void *save,
                                       struct st_mysql_value *value);
extern void ndb_index_stat_option_update(MYSQL_THD, struct st_mysql_sys_var *var, void *var_ptr, const void *save);

extern char ndb_index_stat_option_buf[];

static MYSQL_SYSVAR_STR(index_stat_option,         /* name */
                        opt_ndb_index_stat_option, /* var */
                        PLUGIN_VAR_RQCMDARG, "Comma-separated tunable options for ndb index statistics",
...
static MYSQL_SYSVAR_ULONG(report_thresh_binlog_epoch_slip,         /* name */
...
static MYSQL_SYSVAR_ULONG(report_thresh_binlog_mem_usage,         /* name */
...
static MYSQL_SYSVAR_BOOL(log_update_as_write,         /* name */
...
static MYSQL_SYSVAR_BOOL(log_update_minimal,         /* name */
...
static MYSQL_SYSVAR_BOOL(log_updated_only,         /* name */
...
static MYSQL_SYSVAR_BOOL(log_orig,         /* name */
...
static MYSQL_SYSVAR_BOOL(log_bin,         /* name */
                         opt_ndb_log_bin, /* var */
                         PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_BOOL(log_binlog_index,         /* name */
...
static MYSQL_SYSVAR_BOOL(log_apply_status,         /* name */
                         ...
                         PLUGIN_VAR_OPCMDARG, "Log ndb_apply_status updates from Master in the Binlog",
                         NULL, /* check func. */
                         NULL, /* update func. */
                         0     /* default */
);
...
static MYSQL_SYSVAR_BOOL(log_transaction_id,         /* name */
                         ...
                         PLUGIN_VAR_OPCMDARG, "Log Ndb transaction identities per row in the Binlog",
                         NULL, /* check func. */
                         NULL, /* update func. */
                         0     /* default */
);
...
static MYSQL_SYSVAR_STR(connectstring,         /* name */
                        ...
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY, "Connect string for ndbcluster.",
                        NULL, /* check func. */
                        NULL, /* update func. */
                        NULL  /* default */
);
...
static MYSQL_SYSVAR_STR(mgmd_host,             /* name */
                        opt_ndb_connectstring, /* var */
                        PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY, "Same as --ndb-connectstring",
                        NULL, /* check func. */
                        NULL, /* update func. */
                        NULL  /* default */
);

static MYSQL_SYSVAR_UINT(nodeid,         /* name */
                         opt_ndb_nodeid, /* var */
                         PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
static const char *slave_conflict_role_names[] = {"NONE", "SECONDARY", "PRIMARY", "PASS", NullS};

static TYPELIB slave_conflict_role_typelib = {array_elements(slave_conflict_role_names) - 1, "",
 ...
 */
static int slave_conflict_role_check_func(THD *thd, struct st_mysql_sys_var *var, void *save, st_mysql_value *value)
{
  ...
  do
  {
    ...
    if (!st_ndb_slave_state::checkSlaveConflictRoleChange((enum_slave_conflict_role)opt_ndb_slave_conflict_role,
                                                          (enum_slave_conflict_role)result, &failure_cause_str))
    {
      char msgbuf[256];
      my_snprintf(msgbuf, sizeof(msgbuf), "Role change from %s to %s failed : %s",
      ...
      thd->raise_error_printf(ER_ERROR_WHEN_EXECUTING_COMMAND, "SET GLOBAL ndb_slave_conflict_role", msgbuf);
 ...
 */
static void slave_conflict_role_update_func(THD *thd, struct st_mysql_sys_var *var, void *tgt, const void *save)
{
  *(long *)tgt = *(long *)save;
};

static MYSQL_SYSVAR_ENUM(slave_conflict_role,         /* Name */
                         opt_ndb_slave_conflict_role, /* Var */
                         PLUGIN_VAR_RQCMDARG, "Role for Slave to play in asymmetric conflict algorithms.",
...
static void dbg_check_shares_update(THD *, st_mysql_sys_var *, void *, const void *)
{
  sql_print_information("dbug_check_shares open:");
  for (uint i = 0; i < ndbcluster_open_tables.records; i++)
  {
    NDB_SHARE *share = (NDB_SHARE *)my_hash_element(&ndbcluster_open_tables, i);
    sql_print_information("  %s.%s: state: %s(%u) use_count: %u", share->db, share->table_name,
                          get_share_state_string(share->state), (unsigned)share->state, share->use_count);
...
static MYSQL_THDVAR_UINT(dbg_check_shares, /* name */
                         PLUGIN_VAR_RQCMDARG, "Debug, only...check that no shares are lingering...",
                         NULL,                    /* check func */
...
static struct st_mysql_sys_var *system_variables[] = {MYSQL_SYSVAR(cache_check_time),
...
struct st_mysql_storage_engine ndbcluster_storage_engine = {MYSQL_HANDLERTON_INTERFACE_VERSION};
...
mysql_declare_plugin(ndbcluster){
    ...
    i_s_ndb_transid_mysql_connection_map_plugin mysql_declare_plugin_end;
