...
bool net_send_error_packet(NET *, uint, const char *, const char *, bool, ulong, const CHARSET_INFO *);
...
bool Protocol_classic::net_store_data(const uchar *from, size_t length)
{
  ...
  */
  if (packet_length + 9 + length > packet->alloced_length() && packet->mem_realloc(packet_length + 9 + length))
    return 1;
...
#ifndef EMBEDDED_LIBRARY
bool Protocol_classic::net_store_data(const uchar *from, size_t length, const CHARSET_INFO *from_cs,
...
bool net_send_error(THD *thd, uint sql_errno, const char *err)
{
  ...
  */
  ...
  error = net_send_error_packet(thd, sql_errno, err, mysql_errno_to_sqlstate(sql_errno));
...
bool net_send_error(NET *net, uint sql_errno, const char *err)
{
  ...
  bool error = net_send_error_packet(net, sql_errno, err, mysql_errno_to_sqlstate(sql_errno), false, 0,
...
bool net_send_ok(THD *thd, uint server_status, uint statement_warn_count, ulonglong affected_rows, ulonglong id,
                 const char *message, bool eof_identifier)
{
  ...
  */
  ...
  if (protocol->has_client_capability(CLIENT_SESSION_TRACK) && thd->session_tracker.enabled_any() &&
...
bool net_send_error_packet(THD *thd, uint sql_errno, const char *err, const char *sqlstate)
{
  return net_send_error_packet(thd->get_protocol_classic()->get_net(), sql_errno, err, sqlstate, thd->bootstrap,
                               thd->get_protocol()->get_client_capabilities(), thd->variables.character_set_results);
}
...
bool net_send_error_packet(NET *net, uint sql_errno, const char *err, const char *sqlstate, bool bootstrap,
                           ulong client_capabilities, const CHARSET_INFO *character_set_results)
{
  uint length;
  /*
  ...
  */
  ...
  convert_error_message(converted_err, sizeof(converted_err), character_set_results, err, strlen(err),
                        system_charset_info, &error);
  /* Converted error message is always null-terminated. */
  length = (uint)(strmake(pos, converted_err, MYSQL_ERRMSG_SIZE - 1) - buff);

  DBUG_RETURN(net_write_command(net, (uchar)255, (uchar *)"", 0, (uchar *)buff, length));
}
...
bool Protocol_classic::send_ok(uint server_status, uint statement_warn_count, ulonglong affected_rows,
                               ulonglong last_insert_id, const char *message)
{
  DBUG_ENTER("Protocol_classic::send_ok");
  const bool retval =
      net_send_ok(m_thd, server_status, statement_warn_count, affected_rows, last_insert_id, message, false);
  DBUG_RETURN(retval);
}
...
bool Protocol_classic::send_eof(uint server_status, uint statement_warn_count)
{
  ...
  */
  ...
  if (has_client_capability(CLIENT_DEPRECATE_EOF) &&
      (m_thd->get_command() != COM_BINLOG_DUMP && m_thd->get_command() != COM_BINLOG_DUMP_GTID))
    retval = net_send_ok(m_thd, server_status, statement_warn_count, 0, 0, NULL, true);
  else
#endif
...
bool Protocol_classic::send_error(uint sql_errno, const char *err_msg, const char *sql_state)
{
...
{
  int ret;
  if ((packet_length = my_net_read(&m_thd->net)) && packet_length != packet_error)
  {
...
bool Protocol_classic::parse_packet(union COM_DATA *data, enum_server_command cmd)
{
  switch (cmd)
  {
    ...
    case COM_SHUTDOWN:
    {
      data->com_shutdown.level = packet_length == 0 ? SHUTDOWN_DEFAULT : (enum mysql_enum_shutdown_level)raw_packet[0];
      break;
    }
    ...
    case COM_FIELD_LIST:
    {
      ...
      */
      ...
      uint len = data->com_field_list.table_name_length = strend((char *)raw_packet) - (char *)raw_packet;
...
bool Protocol_classic::create_command(COM_DATA *com_data, enum_server_command cmd, uchar *pkt, size_t length)
{
  raw_packet = pkt;
...
{
  net_send_eof(m_thd, m_thd->server_status, 0 /* no warnings, we're inside SP */);
}
...
bool Protocol_classic::send_field_metadata(Send_field *field, const CHARSET_INFO *item_charset)
{
  ...
  {
    if (store(STRING_WITH_LEN("def"), cs) || store(field->db_name, strlen(field->db_name), cs) ||
        ...
        store(field->org_col_name, strlen(field->org_col_name), cs) || packet->mem_realloc(packet->length() + 12))
    {
      send_metadata = false;
      return true;
    }
    ...
    *pos++ = 12;  // Length of packed fields
    ...
    else
    {
      ...
      */
      max_length = (field->type >= MYSQL_TYPE_TINY_BLOB && field->type <= MYSQL_TYPE_BLOB)
  ...
  else
  {
    if (store(field->table_name, strlen(field->table_name), cs) ||
        store(field->col_name, strlen(field->col_name), cs) || packet->mem_realloc(packet->length() + 10))
    {
      send_metadata = false;
      return true;
    }
...
bool Protocol_classic::end_row()
{
  ...
  if (m_thd->get_protocol()->connection_alive())
    DBUG_RETURN(my_net_write(&m_thd->net, (uchar *)packet->ptr(), packet->length()));
  DBUG_RETURN(0);
}
#endif /* EMBEDDED_LIBRARY */
...
bool Protocol_classic::store_string_aux(const char *from, size_t length, const CHARSET_INFO *fromcs,
...
bool Protocol_text::store(const char *from, size_t length, const CHARSET_INFO *fromcs, const CHARSET_INFO *tocs)
{
  ...
  assert(send_metadata || field_types == 0 || field_types[field_pos] == MYSQL_TYPE_DECIMAL ||
         field_types[field_pos] == MYSQL_TYPE_BIT || field_types[field_pos] == MYSQL_TYPE_NEWDECIMAL ||
         field_types[field_pos] == MYSQL_TYPE_NEWDATE || field_types[field_pos] == MYSQL_TYPE_JSON ||
...
#endif
  char buff[20];
  return net_store_data((uchar *)buff, (size_t)(int10_to_str((int)from, buff, -10) - buff));
}
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(send_metadata || field_types == 0 || field_types[field_pos] == MYSQL_TYPE_YEAR ||
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(send_metadata || field_types == 0 || field_types[field_pos] == MYSQL_TYPE_INT24 ||
         field_types[field_pos] == MYSQL_TYPE_LONG);
  field_pos++;
#endif
  char buff[20];
  return net_store_data((uchar *)buff, (size_t)(int10_to_str((long int)from, buff, (from < 0) ? -10 : 10) - buff));
}
...
#endif
  char buff[22];
  return net_store_data((uchar *)buff, (size_t)(longlong10_to_str(from, buff, unsigned_flag ? 10 : -10) - buff));
}
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(send_metadata || field_types == 0 || field_types[field_pos] == MYSQL_TYPE_NEWDECIMAL);
  field_pos++;
#endif
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(send_metadata || field_types == 0 || field_types[field_pos] == MYSQL_TYPE_FLOAT);
  field_pos++;
#endif
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(send_metadata || field_types == 0 || field_types[field_pos] == MYSQL_TYPE_DOUBLE);
  field_pos++;
#endif
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(send_metadata || field_types == 0 || is_temporal_type_with_date_and_time(field_types[field_pos]));
  field_pos++;
#endif
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(send_metadata || field_types == 0 || field_types[field_pos] == MYSQL_TYPE_DATE);
  field_pos++;
#endif
  char buff[MAX_DATE_STRING_REP_LENGTH];
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(send_metadata || field_types == 0 || field_types[field_pos] == MYSQL_TYPE_TIME);
  field_pos++;
#endif
  char buff[MAX_DATE_STRING_REP_LENGTH];
...
bool Protocol_text::send_out_parameters(List<Item_param> *sp_params)
{
  ...
  List_iterator_fast<LEX_STRING> user_var_name_it(m_thd->lex->prepared_stmt_params);
  ...
  while (true)
  {
    ...
    Item_func_set_user_var *suv = new Item_func_set_user_var(*user_var_name, item_param, false);
    /*
...
/****************************************************************************
  ...
  Data format:
    ...
    [..]..[[length]data]              data
****************************************************************************/
bool Protocol_binary::start_result_metadata(uint num_cols, uint flags, const CHARSET_INFO *result_cs)
{
...
bool Protocol_binary::store(const char *from, size_t length, const CHARSET_INFO *fromcs, const CHARSET_INFO *tocs)
{
  if (send_metadata)
    return Protocol_text::store(from, length, fromcs, tocs);
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_DECIMAL || field_types[field_pos] == MYSQL_TYPE_BIT ||
         field_types[field_pos] == MYSQL_TYPE_NEWDECIMAL || field_types[field_pos] == MYSQL_TYPE_NEWDATE ||
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_TINY ||
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_YEAR || field_types[field_pos] == MYSQL_TYPE_SHORT ||
         field_types[field_pos] == MYSQL_TYPE_VAR_STRING);
#endif
  field_pos++;
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_INT24 || field_types[field_pos] == MYSQL_TYPE_LONG ||
         field_types[field_pos] == MYSQL_TYPE_VAR_STRING);
#endif
  field_pos++;
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_LONGLONG ||
         field_types[field_pos] == MYSQL_TYPE_VAR_STRING);
#endif
  field_pos++;
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_NEWDECIMAL ||
         field_types[field_pos] == MYSQL_TYPE_VAR_STRING);
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_FLOAT ||
         field_types[field_pos] == MYSQL_TYPE_VAR_STRING);
#endif
  field_pos++;
  char *to = packet->prep_append(4, PACKET_BUFFER_EXTRA_ALLOC);
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_DOUBLE ||
         field_types[field_pos] == MYSQL_TYPE_VAR_STRING);
#endif
  field_pos++;
  char *to = packet->prep_append(8, PACKET_BUFFER_EXTRA_ALLOC);
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_DATE ||
...
#ifndef NDEBUG
  // field_types check is needed because of the embedded protocol
  assert(field_types == 0 || field_types[field_pos] == MYSQL_TYPE_TIME ||
         field_types[field_pos] == MYSQL_TYPE_VAR_STRING);
#endif
  field_pos++;
  pos = buff + 1;
...
bool Protocol_binary::send_out_parameters(List<Item_param> *sp_params)
{
  ...
  */
  ...
  /* Send meta-data. */
  if (m_thd->send_result_metadata(&out_param_lst, SEND_NUM_ROWS | SEND_EOF))
    return TRUE;
  ...
  */
  ...
  if (has_client_capability(CLIENT_DEPRECATE_EOF))
    ret = net_send_ok(m_thd, (m_thd->server_status | SERVER_PS_OUT_PARAMS | SERVER_MORE_RESULTS_EXISTS),
                      m_thd->get_stmt_da()->current_statement_cond_count(), 0, 0, NULL, true);
  else
