...
#define OPEN_VIEW_NO_PARSE               \
  16384 /* Open frm only if it's a view, \
...
#define MYSQL_OPEN_REOPEN                                                                              \
  (MYSQL_OPEN_IGNORE_FLUSH | MYSQL_OPEN_IGNORE_GLOBAL_READ_LOCK | MYSQL_LOCK_IGNORE_GLOBAL_READ_ONLY | \
   MYSQL_LOCK_IGNORE_TIMEOUT | MYSQL_OPEN_IGNORE_KILLED | MYSQL_OPEN_GET_NEW_TABLE | MYSQL_OPEN_HAS_MDL_LOCK)
...
bool fill_record_n_invoke_before_triggers(THD *thd, Field **field, List<Item> &values, TABLE *table,
...
bool setup_fields(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &item, ulong privilege,
...
bool fill_record(THD *thd, TABLE *table, Field **field, List<Item> &values, MY_BITMAP *bitmap,
                 MY_BITMAP *insert_into_fields_bitmap);
...
bool lock_table_names(THD *thd, TABLE_LIST *table_list, TABLE_LIST *table_list_end, ulong lock_wait_timeout,
                      uint flags);
...
TABLE *open_performance_schema_table(THD *thd, TABLE_LIST *one_table, Open_tables_state *backup);
...
{
  return find_table_in_list(table, &TABLE_LIST::next_local, db_name, table_name);
}
...
class Open_table_context
{
  ...
  */
  ...
  bool has_protection_against_grl() const { return m_has_protection_against_grl; }
...
class View_error_handler : public Internal_error_handler
{
 ...
 public:
  ...
  virtual bool handle_condition(THD *thd, uint sql_errno, const char *, Sql_condition::enum_severity_level *level,
                                const char *message);
};
...
class Strict_error_handler : public Internal_error_handler
{
 public:
  ...
  Strict_error_handler() : m_set_select_behavior(DISABLE_SET_SELECT_STRICT_ERROR_HANDLER) {}

  Strict_error_handler(enum_set_select_behavior param) : m_set_select_behavior(param) {}
...
{
 public:
  Partition_in_shared_ts_error_handler() : m_is_already_reported(false) {}
