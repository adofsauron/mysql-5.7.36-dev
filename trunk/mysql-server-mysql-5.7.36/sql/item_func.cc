...
bool Item_func::itemize(Parse_context *pc, Item **res)
{
  ...
  for (size_t i = 0; i < arg_count; i++)
  {
    ...
    {
      my_error(functype() == FUNC_SP ? ER_WRONG_PARAMETERS_TO_STORED_FCT : ER_WRONG_PARAMETERS_TO_NATIVE_FCT, MYF(0),
               func_name());
      return true;
    }
  }
...
void Item_func::fix_after_pullout(st_select_lex *parent_select, st_select_lex *removed_select)
{
  if (const_item())
  {
    /*
...
Item *Item_func::compile(Item_analyzer analyzer, uchar **arg_p, Item_transformer transformer, uchar *arg_t)
{
...
void Item_func::split_sum_func(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &fields)
{
  Item **arg, **arg_end;
  for (arg = args, arg_end = args + arg_count; arg != arg_end; arg++)
...
void Item_func_sp::fix_after_pullout(SELECT_LEX *parent_select, SELECT_LEX *removed_select)
{
  ...
  switch (result_type())
  {
    case INT_RESULT:
      if (max_char_length() > MY_INT32_NUM_DECIMAL_DIGITS)
        field = new Field_longlong(max_char_length(), maybe_null, item_name.ptr(), unsigned_flag);
      else
        field = new Field_long(max_char_length(), maybe_null, item_name.ptr(), unsigned_flag);
      break;
    case REAL_RESULT:
...
type_conversion_status Item_func::save_possibly_as_json(Field *field, bool no_conversions)
{
  ...
  if (field_type() != MYSQL_TYPE_DATE)
  {
    for (uint i = 0; i < nitems; i++)
      set_if_bigger(decimals,
                    field_type() == MYSQL_TYPE_TIME ? item[i]->time_precision() : item[i]->datetime_precision());
  }
...
void Item_func::count_decimal_length(Item **item, uint nitems)
{
  ...
  fix_char_length(my_decimal_precision_to_length_no_truncation(precision, decimals, unsigned_flag));
}
...
/**
  ...
  @retval            False on success, true on error.
*/
bool Item_func::count_string_result_length(enum_field_types field_type, Item **items, uint nitems)
{
...
void Item_func::signal_divide_by_null()
{
  ...
  if (thd->variables.sql_mode & MODE_ERROR_FOR_DIVISION_BY_ZERO)
    push_warning(thd, Sql_condition::SL_WARNING, ER_DIVISION_BY_ZERO, ER(ER_DIVISION_BY_ZERO));
  null_value = 1;
}
...
{
  THD *thd = current_thd;
  push_warning(thd, Sql_condition::SL_WARNING, ER_INVALID_ARGUMENT_FOR_LOGARITHM,
...
const Item_field *Item_func::contributes_to_filter(table_map read_tables, table_map filter_for_table,
...
Item_field *get_gc_for_expr(Item_func **func, Field *fld, Item_result type)
{
  ...
  */
  if (!strcmp(expr->func_name(), "json_unquote") && strcmp((*func)->func_name(), "json_unquote"))
  {
    ...
    case GT_FUNC:
    {
      ...
      if (func)
      {
        ...
        while ((fld = li++))
        {
          ...
          if (!tkm.is_clear_all() && (field = get_gc_for_expr(func, fld, (*val)->result_type())))
          {
            // Matching expression is found, substutite arg with the new
            // field
            fld->table->in_use->change_item_tree(pointer_cast<Item **>(func), field);
            // Adjust comparator
    ...
    case IN_FUNC:
    {
      ...
      if (can_do_subst)
      {
        ...
        while ((fld = li++))
        {
          ...
          if (!tkm.is_clear_all() && (field = get_gc_for_expr(pointer_cast<Item_func **>(args), fld, type)))
          {
            // Matching expression is found, substutite arg[0] with the new
            // field
            fld->table->in_use->change_item_tree(pointer_cast<Item **>(args), field);
            // Adjust comparators
            if (functype() == IN_FUNC)
...
void reject_geometry_args(uint arg_count, Item **args, Item_result_field *me)
{
  ...
  */
  for (uint i = 0; i < arg_count; i++)
  {
    if (args[i]->result_type() != ROW_RESULT && args[i]->field_type() == MYSQL_TYPE_GEOMETRY)
    {
...
void unsupported_json_comparison(size_t arg_count, Item **args, const char *msg)
{
  for (size_t i = 0; i < arg_count; ++i)
  {
    if (args[i]->result_type() == STRING_RESULT && args[i]->field_type() == MYSQL_TYPE_JSON)
    {
      push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_NOT_SUPPORTED_YET,
...
{
  fix_char_length(std::min<uint32>(args[0]->max_char_length(), MY_INT64_NUM_DECIMAL_DIGITS));
  reject_geometry_args(arg_count, args, this);
}
...
longlong Item_func_unsigned::val_int()
{
  ...
  else if (args[0]->cast_to_int_type() != STRING_RESULT || args[0]->is_temporal())
  {
    value = args[0]->val_int();
    null_value = args[0]->null_value;
    return value;
...
err:
  push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WARN_DATA_OUT_OF_RANGE, ER(ER_WARN_DATA_OUT_OF_RANGE),
                      item_name.ptr(), 1L);
  return dec;
}
...
my_decimal *Item_func_plus::decimal_op(my_decimal *decimal_value)
{
  ...
  if (!(null_value = (args[1]->null_value || check_decimal_overflow(my_decimal_add(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW,
...
my_decimal *Item_func_minus::decimal_op(my_decimal *decimal_value)
{
  ...
  if ((null_value = args[0]->null_value))
    return 0;
  val2 = args[1]->val_decimal(&value2);
  if (!(null_value = (args[1]->null_value || (check_decimal_overflow(my_decimal_sub(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW,
                                                                                    decimal_value, val1, val2)) > 3))))
    return decimal_value;
  return 0;
}
...
my_decimal *Item_func_mul::decimal_op(my_decimal *decimal_value)
{
  ...
  if ((null_value = args[0]->null_value))
    return 0;
  val2 = args[1]->val_decimal(&value2);
  if (!(null_value = (args[1]->null_value || (check_decimal_overflow(my_decimal_mul(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW,
                                                                                    decimal_value, val1, val2)) > 3))))
    return decimal_value;
  return 0;
}
...
{
  uint precision = min<uint>(args[0]->decimal_precision() + args[1]->decimals + prec_increment, DECIMAL_MAX_PRECISION);
...
longlong Item_func_int_div::val_int()
{
  ...
  */
  if (args[0]->result_type() != INT_RESULT || args[1]->result_type() != INT_RESULT)
  {
    ...
    int err;
    if ((err = my_decimal_div(E_DEC_FATAL_ERROR & ~E_DEC_DIV_ZERO, &tmp, &val0, &val1, 0)) > 3)
    {
      if (err == E_DEC_DIV_ZERO)
        signal_divide_by_null();
      return 0;
...
void Item_func_int_div::fix_length_and_dec()
{
  ...
  uint32 char_length =
      args[0]->max_char_length() - (argtype == DECIMAL_RESULT || argtype == INT_RESULT ? args[0]->decimals : 0);
  fix_char_length(char_length > MY_INT64_NUM_DECIMAL_DIGITS ? MY_INT64_NUM_DECIMAL_DIGITS : char_length);
  maybe_null = 1;
  unsigned_flag = args[0]->unsigned_flag | args[1]->unsigned_flag;
  reject_geometry_args(arg_count, args, this);
}
...
longlong Item_func_mod::int_op()
{
  ...
  */
  ...
  uval1 = (ulonglong)(val1_negative ? -val1 : val1);
  res = uval0 % uval1;
  return check_integer_overflow(val0_negative ? -(longlong)res : res, !val0_negative);
}
...
void Item_func_mod::result_precision()
{
  ...
  if (!args[0]->unsigned_flag && args[1]->unsigned_flag && args[0]->max_length <= args[1]->max_length &&
  ...
  */
  if (hybrid_type == INT_RESULT && args[0]->const_item())
  {
    ...
    if ((ulonglong)val >= (ulonglong)LLONG_MIN &&
        ((ulonglong)val != (ulonglong)LLONG_MIN || args[0]->type() != INT_ITEM))
    {
      /*
...
*/
bool Item_func_latlongfromgeohash::check_geohash_argument_valid_type(Item *item)
{
...
*/
bool Item_func_latlongfromgeohash::decode_geohash(String *geohash, double upper_latitude, double lower_latitude,
                                                  double upper_longitude, double lower_longitude,
                                                  double *result_latitude, double *result_longitude)
{
  ...
  for (uint i = 0; i < input_length && latitude_accuracy > 0.0 && longitude_accuracy > 0.0; i++)
  {
    ...
    */
    ...
    else if (input_character >= 'b' && input_character <= 'z' && input_character != 'i' && input_character != 'l' &&
             input_character != 'o')
    {
    ...
    */
    for (int bit_number = 4; bit_number >= 0; bit_number -= 1)
    {
      ...
      assert(latitude_value >= lower_latitude && latitude_value <= upper_latitude &&
             longitude_value >= lower_longitude && longitude_value <= upper_longitude);
    }
  }

  *result_latitude = round_latlongitude(latitude_value, latitude_accuracy * 2.0, latitude_value - latitude_accuracy,
                                        latitude_value + latitude_accuracy);
  *result_longitude = round_latlongitude(longitude_value, longitude_accuracy * 2.0,
                                         longitude_value - longitude_accuracy, longitude_value + longitude_accuracy);
...
*/
double Item_func_latlongfromgeohash::round_latlongitude(double latlongitude, double error_range, double lower_limit,
                                                        double upper_limit)
{
  ...
  else
  {
    ...
    do
    {
      ...
      number_of_decimals++;
    } while ((lower_limit > return_value || return_value > upper_limit) && number_of_decimals <= DBL_DIG);
...
double Item_func_latlongfromgeohash::val_real()
{
  ...
  {
    my_error(ER_WRONG_VALUE_FOR_TYPE, MYF(0), "geohash", input_value->c_ptr_safe(), func_name());
    return error_real();
  }

  double latitude = 0.0;
  double longitude = 0.0;
  if (decode_geohash(input_value, upper_latitude, lower_latitude, upper_longitude, lower_longitude, &latitude,
                     &longitude))
  {
    my_error(ER_WRONG_VALUE_FOR_TYPE, MYF(0), "geohash", input_value->c_ptr_safe(), func_name());
    return error_real();
...
{
  ulonglong tmp_max_length = (ulonglong)args[0]->max_length - (args[0]->decimals ? args[0]->decimals + 1 : 0) + 2;
  max_length = tmp_max_length > (ulonglong)4294967295U ? (uint32)4294967295U : (uint32)tmp_max_length;
  uint tmp = float_length(decimals);
  set_if_smaller(max_length, tmp);
  decimals = 0;
}
...
double my_double_round(double value, longlong dec, bool dec_unsigned, bool truncate)
{
  ...
  */
  ...
  if (dec_negative && my_isinf(tmp))
    tmp2 = 0.0;
  else if (!dec_negative && (my_isinf(value_mul_tmp) || my_isnan(value_mul_tmp)))
    tmp2 = value;
  else if (truncate)
  {
    if (value >= 0.0)
...
double Item_func_round::real_op()
{
  ...
  if (!(null_value = args[0]->null_value || args[1]->null_value))
    return my_double_round(value, decimal_places, args[1]->unsigned_flag, truncate);
...
my_decimal *Item_func_round::decimal_op(my_decimal *decimal_value)
{
  ...
  if (!(null_value = (args[0]->null_value || args[1]->null_value ||
                      my_decimal_round(E_DEC_FATAL_ERROR, value, (int)dec, truncate, decimal_value) > 1)))
    return decimal_value;
  return 0;
}
...
void Item_func_min_max::fix_length_and_dec()
{
  ...
  for (uint i = 0; i < arg_count; i++)
  {
    ...
    if (args[i]->maybe_null)
      maybe_null = 1;
    cmp_type = item_cmp_type(cmp_type, args[i]->temporal_with_date_as_number_result_type());
  ...
  if (string_arg_count == arg_count)
  {
    ...
    agg_arg_charsets_for_string_result_with_comparison(collation, args, arg_count);
    if (datetime_found)
    {
  ...
  else if ((cmp_type == DECIMAL_RESULT) || (cmp_type == INT_RESULT))
  {
    ...
    fix_char_length(my_decimal_precision_to_length_no_truncation(max_int_part + decimals, decimals, unsigned_flag));
  }
  ...
  if (compare_as_dates)
  {
    if (is_temporal())
    {
      ...
      */
      ...
      return (null_value = my_TIME_to_str(&ltime, str, decimals)) ? (String *)0 : str;
    }
    else
    {
      /*
...
my_decimal *Item_func_min_max::val_decimal(my_decimal *dec)
{
  ...
  if (compare_as_dates)
  {
    ...
    return my_decimal_from_datetime_packed(dec, datetime_item->field_type(), value);
  }
  for (uint i = 0; i < arg_count; i++)
  {
    if (i == 0)
...
longlong Item_func_locate::val_int()
{
  ...
  if (!cmp_collation.collation->coll->instr(cmp_collation.collation, a->ptr() + start, (uint)(a->length() - start),
      ...
      if ((symbol_len = cs->cset->mb_wc(cs, &wc, (uchar *)str_end, (uchar *)real_end)) > 0)
      {
        ...
        if (is_separator || is_last_item)
        {
          ...
          if (is_last_item && !is_separator)
            str_end = substr_end;
          if (!my_strnncoll(cs, (const uchar *)str_begin, (uint)(str_end - str_begin), find_str, find_str_len))
...
longlong Item_master_pos_wait::val_int()
{
  ...
  if (timeout < 0)
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, ER(ER_WRONG_ARGUMENTS),
                          "MASTER_POS_WAIT.");
      null_value = 1;
    }
    return 0;
  ...
  if (mi == NULL || (event_count = mi->rli->wait_for_pos(thd, log_name, pos, timeout)) == -2)
  {
    null_value = 1;
    event_count = 0;
  }
...
longlong Item_wait_for_executed_gtid_set::val_int()
{
  ...
  if (wait_for_gtid_set.add_gtid_text(gtid_text->c_ptr_safe()) != RETURN_STATUS_OK)
  {
    global_sid_lock->unlock();
  ...
  if (thd->owned_gtid.sidno > 0 && wait_for_gtid_set.contains_gtid(thd->owned_gtid))
  {
    char buf[Gtid::MAX_TEXT_LENGTH + 1];
    thd->owned_gtid.to_string(global_sid_map, buf);
    global_sid_lock->unlock();
    my_error(ER_CANT_WAIT_FOR_EXECUTED_GTID_SET_WHILE_OWNING_A_GTID, MYF(0), buf);
    DBUG_RETURN(0);
  }
...
longlong Item_func_gtid_subset::val_int()
{
  ...
  if ((string1 = args[0]->val_str(&buf1)) != NULL && (charp1 = string1->c_ptr_safe()) != NULL &&
      (string2 = args[1]->val_str(&buf2)) != NULL && (charp2 = string2->c_ptr_safe()) != NULL)
  {
...
uchar *ull_get_key(const uchar *ptr, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
...
static bool check_and_convert_ull_name(char *buff, String *org_name)
{
  ...
  bytes_copied = well_formed_copy_nchars(system_charset_info, buff, NAME_LEN, org_name->charset(), org_name->ptr(),
                                         org_name->length(), NAME_CHAR_LEN, &well_formed_error_pos,
                                         &cannot_convert_error_pos, &from_end_pos);

  if (well_formed_error_pos || cannot_convert_error_pos || from_end_pos < org_name->ptr() + org_name->length())
  {
...
longlong Item_func_get_lock::val_int()
{
  ...
  */
  ...
  if (!my_hash_inited(&thd->ull_hash) && my_hash_init(&thd->ull_hash, &my_charset_bin, 16 /* small hash */, 0, 0,
                                                      ull_get_key, NULL, 0, key_memory_User_level_lock))
  {
    DBUG_RETURN(0);
  }

  MDL_request ull_request;
  MDL_REQUEST_INIT(&ull_request, MDL_key::USER_LEVEL_LOCK, "", name, MDL_EXCLUSIVE, MDL_EXPLICIT);
  MDL_key *ull_key = &ull_request.key;

  if ((ull = reinterpret_cast<User_level_lock *>(my_hash_search(&thd->ull_hash, ull_key->ptr(), ull_key->length()))))
  {
    /* Recursive lock. */
    ull->refs++;
    null_value = FALSE;
  ...
  bool error = thd->mdl_context.acquire_lock(&ull_request, static_cast<ulong>(timeout));
  ...
  ull = reinterpret_cast<User_level_lock *>(my_malloc(key_memory_User_level_lock, sizeof(User_level_lock), MYF(0)));
...
longlong Item_func_benchmark::val_int()
{
  ...
  if (args[0]->null_value || (!args[0]->unsigned_flag && (((longlong)loop_count) < 0)))
  {
    if (!args[0]->null_value)
    {
      ...
      push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WRONG_VALUE_FOR_TYPE, ER(ER_WRONG_VALUE_FOR_TYPE),
  ...
  */
  ...
  if (args[0]->null_value || timeout < 0)
  {
    if (!thd->lex->is_ignore() && thd->is_strict_mode())
    {
      ...
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WRONG_ARGUMENTS, ER(ER_WRONG_ARGUMENTS), "sleep.");
  }
  /*
...
static user_var_entry *get_variable(THD *thd, const Name_string &name, const CHARSET_INFO *cs)
{
  ...
  if (!(entry = (user_var_entry *)my_hash_search(hash, (uchar *)name.ptr(), name.length())) && cs != NULL)
  {
  ...
  else
  {
    const CHARSET_INFO *cs =
        create_if_not_exists
            ? (args[0]->collation.derivation == DERIVATION_NUMERIC ? default_charset() : args[0]->collation.collation)
            : NULL;
  ...
  else
  {
    ...
    if (m_length != length)
    {
      if (m_ptr == internal_buffer_ptr())
        m_ptr = 0;
      if (!(m_ptr = (char *)my_realloc(key_memory_user_var_entry_value, m_ptr, length,
...
bool user_var_entry::store(const void *ptr, size_t length, Item_result type, const CHARSET_INFO *cs, Derivation dv,
                           bool unsigned_arg)
{
  assert_locked();
...
bool Item_func_set_user_var::update_hash(const void *ptr, uint length, Item_result res_type, const CHARSET_INFO *cs,
...
String *user_var_entry::val_str(my_bool *null_value, String *str, uint decimals) const
{
  if ((*null_value = (m_ptr == 0)))
    return (String *)0;
...
my_decimal *user_var_entry::val_decimal(my_bool *null_value, my_decimal *val) const
{
  ...
  switch (m_type)
  {
    ...
    case DECIMAL_RESULT:
      ...
      break;
    case STRING_RESULT:
      str2my_decimal(E_DEC_FATAL_ERROR, m_ptr, m_length, collation.collation, val);
      break;
    case ROW_RESULT:
      assert(1);  // Impossible
      break;
...
bool Item_func_set_user_var::update()
{
  ...
  switch (cached_result_type)
  {
    case REAL_RESULT:
    {
      res = update_hash(&save_result.vreal, sizeof(save_result.vreal), REAL_RESULT, default_charset(),
                        DERIVATION_IMPLICIT, 0);
      break;
    }
    case INT_RESULT:
    {
      res = update_hash(&save_result.vint, sizeof(save_result.vint), INT_RESULT, default_charset(), DERIVATION_IMPLICIT,
                        unsigned_flag);
      break;
    }
    case STRING_RESULT:
    {
      if (!save_result.vstr)  // Null value
        res = update_hash(NULL, 0, STRING_RESULT, &my_charset_bin, DERIVATION_IMPLICIT, 0);
      else
        res = update_hash(save_result.vstr->ptr(), save_result.vstr->length(), STRING_RESULT,
                          save_result.vstr->charset(), DERIVATION_IMPLICIT, 0);
      break;
    }
    case DECIMAL_RESULT:
    {
      if (!save_result.vdec)  // Null value
        res = update_hash(NULL, 0, DECIMAL_RESULT, &my_charset_bin, DERIVATION_IMPLICIT, false);
      else
        res = update_hash(save_result.vdec, sizeof(my_decimal), DECIMAL_RESULT, default_charset(), DERIVATION_IMPLICIT,
                          0);
      break;
    }
    case ROW_RESULT:
  ...
  */
  ...
  if (!var_entry)
  {
    ...
    */
    ...
    thd->lex = &lex_tmp;
    lex_start(thd);
    tmp_var_list.push_back(new set_var_user(new Item_func_set_user_var(name, new Item_null(), false)));
    /* Create the variable */
  ...
  */
  ...
  else
  {
    ...
    if (user_var_event->type == DECIMAL_RESULT)
    {
      ...
      const my_decimal *dec = static_cast<const my_decimal *>(static_cast<const void *>(var_entry->ptr()));
      dec->sanity_check();
...
bool Item_user_var_as_out_param::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  const CHARSET_INFO *cs = thd->lex->exchange->cs ? thd->lex->exchange->cs : thd->variables.collation_database;
...
void Item_user_var_as_out_param::set_value(const char *str, size_t length, const CHARSET_INFO *cs)
{
  entry->lock();
  entry->store((void *)str, length, STRING_RESULT, cs, DERIVATION_IMPLICIT, 0 /* unsigned_arg */);
  entry->unlock();
}
...
void Item_func_get_system_var::fix_length_and_dec()
{
  ...
  switch (var->show_type())
  {
    ...
    case SHOW_CHAR_PTR:
      ...
      cptr = var->show_type() == SHOW_CHAR ? (char *)var->value_ptr(thd, var_type, &component)
                                           : *(char **)var->value_ptr(thd, var_type, &component);
      if (cptr)
        max_length = system_charset_info->cset->numchars(system_charset_info, cptr, cptr + strlen(cptr));
    ...
    case SHOW_LEX_STRING:
    {
      ...
      max_length = system_charset_info->cset->numchars(system_charset_info, ls->str, ls->str + ls->length);
      mysql_mutex_unlock(&LOCK_global_system_variables);
      collation.set(system_charset_info, DERIVATION_SYSCONST);
      max_length *= system_charset_info->mbmaxlen;
      decimals = NOT_FIXED_DEC;
...
longlong Item_func_get_system_var::val_int()
{
  ...
  if (cache_present && thd->query_id == used_query_id)
  {
    ...
    else if (cache_present & GET_SYS_VAR_CACHE_STRING)
    {
      null_value = cached_null_value;
      if (!null_value)
        cached_llval = longlong_from_string_with_check(cached_strval.charset(), cached_strval.c_ptr(),
  ...
  switch (var->show_type())
  {
    ...
    case SHOW_LEX_STRING:
    {
      ...
      if (str_val && str_val->length())
        cached_llval = longlong_from_string_with_check(system_charset_info, str_val->c_ptr(),
...
String *Item_func_get_system_var::val_str(String *str)
{
  ...
  switch (var->show_type())
  {
    ...
    {
      mysql_mutex_lock(&LOCK_global_system_variables);
      char *cptr = var->show_type() == SHOW_CHAR ? (char *)var->value_ptr(thd, var_type, &component)
                                                 : *(char **)var->value_ptr(thd, var_type, &component);
      if (cptr)
      {
...
double Item_func_get_system_var::val_real()
{
  ...
  if (cache_present && thd->query_id == used_query_id)
  {
    ...
    else if (cache_present & GET_SYS_VAR_CACHE_STRING)
    {
      null_value = cached_null_value;
      if (!null_value)
        cached_dval = double_from_string_with_check(cached_strval.charset(), cached_strval.c_ptr(),
                                                    cached_strval.c_ptr() + cached_strval.length());
      else
        cached_dval = 0;
      cache_present |= GET_SYS_VAR_CACHE_DOUBLE;
  ...
  switch (var->show_type())
  {
    ...
    case SHOW_CHAR_PTR:
    {
      ...
      if (cptr && *cptr)
        cached_dval = double_from_string_with_check(system_charset_info, cptr, cptr + strlen(cptr));
      else
      {
        null_value = TRUE;
        cached_dval = 0;
...
float Item_func_match::get_filtering_effect(table_map filter_for_table, table_map read_tables,
...
bool Item_func_match::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  ...
  if (Item_func::fix_fields(thd, ref) || fix_func_arg(thd, &against) || !against->const_during_execution())
  {
  ...
  for (uint i = 0; i < arg_count; i++)
  {
    ...
    allows_multi_table_search &= allows_search_on_non_indexed_columns(((Item_field *)item)->field->table);
  }
  ...
  */
  TABLE *const table = table_ref ? table_ref->table : ((Item_field *)item)->field->table;
  ...
  return agg_item_collations_for_comparison(cmp_collation, func_name(), args, arg_count, 0);
}
  ...
  */
  ...
  for (keynr = 0; keynr < table->s->keys; keynr++)
  {
    if ((table->key_info[keynr].flags & HA_FULLTEXT) &&
        (flags & FT_BOOL ? table->keys_in_use_for_query.is_set(keynr) : table->s->keys_in_use.is_set(keynr)))
  ...
  for (keynr = 0; keynr <= mkeys; keynr++)
  {
    ...
    if (max_cnt < arg_count || max_cnt < table->key_info[ft_to_key[keynr]].user_defined_key_parts)
      continue;
...
bool Item_func_match::eq(const Item *item, bool binary_cmp) const
{
  ...
  if (key == ifm->key && table_ref == ifm->table_ref && key_item()->eq(ifm->key_item(), binary_cmp))
    return 1;
...
void Item_func_match::set_hints(JOIN *join, uint ft_flag, ha_rows ft_limit, bool no_cond)
{
  assert(!master);
...
Item *get_system_var(Parse_context *pc, enum_var_type var_type, LEX_STRING name, LEX_STRING component, bool unsafe)
{
  THD *thd = pc->thd;
  sys_var *var;
  ...
  Item_func_get_system_var *item = new Item_func_get_system_var(var, var_type, component_name, NULL, 0);
  if (item == NULL)
    return NULL;  // OOM
  ...
  if (var_type == OPT_GLOBAL && var->check_scope(OPT_GLOBAL))
  {
    ...
    Item_func_get_system_var *si = new Item_func_get_system_var(var, var_type, component_name, NULL, 0);
    ...
    if (mysql_audit_notify(thd, AUDIT_EVENT(MYSQL_AUDIT_GLOBAL_VARIABLE_GET), var->name.str,
                           outStr ? outStr->ptr() : NULL, outStr ? outStr->length() : 0))
    {
      return 0;
    }
  }
...
Item_func_sp::Item_func_sp(const POS &pos, const LEX_STRING &db_name, const LEX_STRING &fn_name, bool use_explicit_name,
    ...
    : Item_func(pos, opt_list), m_sp(NULL), dummy_table(NULL), sp_result_field(NULL)
{
  ...
  THD *thd = current_thd;
  m_name = new (thd->mem_root) sp_name(to_lex_cstring(db_name), fn_name, use_explicit_name);
}
...
bool Item_func_sp::init_result_field(THD *thd)
{
  ...
  if (!(m_sp = sp_find_routine(thd, SP_TYPE_FUNCTION, m_name, &thd->sp_func_cache, TRUE)))
  {
  ...
  if (!(sp_result_field = m_sp->create_result_field(max_length, item_name.ptr(), dummy_table)))
  {
    DBUG_RETURN(TRUE);
  }
...
type_conversion_status Item_func_sp::save_in_field_inner(Field *field, bool no_conversions)
{
...
bool Item_func_sp::execute_impl(THD *thd)
{
  ...
  enum enum_sp_data_access access =
      (m_sp->m_chistics->daccess == SP_DEFAULT_ACCESS) ? SP_DEFAULT_ACCESS_MAPPING : m_sp->m_chistics->daccess;
...
bool Item_func_sp::sp_check_access(THD *thd)
{
  ...
  assert(m_sp);
#ifndef NO_EMBEDDED_ACCESS_CHECKS
  if (check_routine_access(thd, EXECUTE_ACL, m_sp->m_db.str, m_sp->m_name.str, 0, FALSE))
    DBUG_RETURN(TRUE);
#endif
...
bool Item_func_sp::fix_fields(THD *thd, Item **ref)
{
  ...
  if (!(thd->lex->context_analysis_only & CONTEXT_ANALYSIS_ONLY_VIEW) || (thd->lex->sql_command == SQLCOM_CREATE_VIEW))
  {
    ...
    res = check_routine_access(thd, EXECUTE_ACL, m_name->m_db.str, m_name->m_name.str, 0, FALSE);
    thd->set_security_context(save_security_ctx);
