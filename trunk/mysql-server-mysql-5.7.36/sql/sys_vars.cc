...
static bool update_buffer_size(THD *thd, KEY_CACHE *key_cache, ptrdiff_t offset, ulonglong new_value)
{
  bool error = false;
...
static bool update_keycache_param(THD *thd, KEY_CACHE *key_cache, ptrdiff_t offset, ulonglong new_value)
{
  bool error = false;
  assert(offset != offsetof(KEY_CACHE, param_buff_size));
...
#define PFS_TRAILING_PROPERTIES \
  NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(NULL), ON_UPDATE(NULL), NULL, sys_var::PARSE_EARLY

static Sys_var_mybool Sys_pfs_enabled("performance_schema", "Enable the performance schema.",
                                      READ_ONLY GLOBAL_VAR(pfs_param.m_enabled), CMD_LINE(OPT_ARG), DEFAULT(TRUE),
                                      PFS_TRAILING_PROPERTIES);

static Sys_var_charptr Sys_pfs_instrument("performance_schema_instrument",
...
static Sys_var_mybool Sys_pfs_consumer_events_stages_current(
    ...
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_stages_current_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_events_stages_history(
    "performance_schema_consumer_events_stages_history",
    "Default startup value for the events_stages_history consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_stages_history_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_events_stages_history_long(
    "performance_schema_consumer_events_stages_history_long",
    "Default startup value for the events_stages_history_long consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_stages_history_long_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_events_statements_current(
    "performance_schema_consumer_events_statements_current",
    "Default startup value for the events_statements_current consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_statements_current_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(TRUE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_events_statements_history(
    "performance_schema_consumer_events_statements_history",
    "Default startup value for the events_statements_history consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_statements_history_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(TRUE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_events_statements_history_long(
    "performance_schema_consumer_events_statements_history_long",
    "Default startup value for the events_statements_history_long consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_statements_history_long_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);
...
static Sys_var_mybool Sys_pfs_consumer_events_transactions_current(
    ...
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_transactions_current_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);
...
static Sys_var_mybool Sys_pfs_consumer_events_transactions_history(
    ...
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_transactions_history_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);
...
static Sys_var_mybool Sys_pfs_consumer_events_transactions_history_long(
    ...
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_transactions_history_long_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_events_waits_current(
    "performance_schema_consumer_events_waits_current", "Default startup value for the events_waits_current consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_waits_current_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_events_waits_history(
    "performance_schema_consumer_events_waits_history", "Default startup value for the events_waits_history consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_waits_history_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_events_waits_history_long(
    "performance_schema_consumer_events_waits_history_long",
    "Default startup value for the events_waits_history_long consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_events_waits_history_long_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(FALSE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_global_instrumentation(
    "performance_schema_consumer_global_instrumentation",
    "Default startup value for the global_instrumentation consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_global_instrumentation_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(TRUE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_thread_instrumentation(
    "performance_schema_consumer_thread_instrumentation",
    "Default startup value for the thread_instrumentation consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_thread_instrumentation_enabled), CMD_LINE(OPT_ARG),
    DEFAULT(TRUE), PFS_TRAILING_PROPERTIES);

static Sys_var_mybool Sys_pfs_consumer_statement_digest(
    "performance_schema_consumer_statements_digest", "Default startup value for the statements_digest consumer.",
    READ_ONLY NOT_VISIBLE GLOBAL_VAR(pfs_param.m_consumer_statement_digest_enabled), CMD_LINE(OPT_ARG), DEFAULT(TRUE),
    PFS_TRAILING_PROPERTIES);
...
static Sys_var_long Sys_pfs_events_waits_history_long_size(
    ...
    READ_ONLY GLOBAL_VAR(pfs_param.m_events_waits_history_long_sizing), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(-1, 1024 * 1024), DEFAULT(PFS_AUTOSIZE_VALUE), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);

static Sys_var_long Sys_pfs_events_waits_history_size("performance_schema_events_waits_history_size",
...
static Sys_var_long Sys_pfs_max_cond_instances("performance_schema_max_cond_instances",
                                               ...
                                               VALID_RANGE(-1, 1024 * 1024), DEFAULT(PFS_AUTOSCALE_VALUE),
                                               BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_long Sys_pfs_max_prepared_stmt_instances("performance_schema_max_prepared_statements_instances",
...
static Sys_var_ulong Sys_pfs_max_socket_classes("performance_schema_max_socket_classes",
                                                ...
                                                DEFAULT(PFS_MAX_SOCKET_CLASS), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_ulong Sys_pfs_max_mutex_classes("performance_schema_max_mutex_classes",
                                               ...
                                               CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 256),
                                               DEFAULT(PFS_MAX_MUTEX_CLASS), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_ulong Sys_pfs_max_rwlock_classes("performance_schema_max_rwlock_classes",
                                                ...
                                                CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 256),
                                                DEFAULT(PFS_MAX_RWLOCK_CLASS), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_ulong Sys_pfs_max_thread_classes("performance_schema_max_thread_classes",
                                                ...
                                                CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 256),
                                                DEFAULT(PFS_MAX_THREAD_CLASS), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_ulong Sys_pfs_max_stage_classes("performance_schema_max_stage_classes",
                                               ...
                                               CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 256),
                                               DEFAULT(PFS_MAX_STAGE_CLASS), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);

static Sys_var_long Sys_pfs_events_stages_history_long_size(
    "performance_schema_events_stages_history_long_size",
    "Number of rows in EVENTS_STAGES_HISTORY_LONG."
    " Use 0 to disable, -1 for automated sizing.",
    READ_ONLY GLOBAL_VAR(pfs_param.m_events_stages_history_long_sizing), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(-1, 1024 * 1024), DEFAULT(PFS_AUTOSIZE_VALUE), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);

static Sys_var_long Sys_pfs_events_stages_history_size("performance_schema_events_stages_history_size",
...
static Sys_var_ulong Sys_pfs_max_statement_classes(
    ...
    READ_ONLY GLOBAL_VAR(pfs_param.m_statement_class_sizing), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 256),
    DEFAULT((ulong)SQLCOM_END + (ulong)COM_END + 5 + SP_PSI_STATEMENT_INFO_COUNT), BLOCK_SIZE(1),
    PFS_TRAILING_PROPERTIES);
...
static Sys_var_long Sys_pfs_events_statements_history_long_size(
    ...
    " Use 0 to disable, -1 for automated sizing.",
    READ_ONLY GLOBAL_VAR(pfs_param.m_events_statements_history_long_sizing), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(-1, 1024 * 1024), DEFAULT(PFS_AUTOSIZE_VALUE), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_long Sys_pfs_events_statements_history_size(
    ...
    " Use 0 to disable, -1 for automated sizing.",
    READ_ONLY GLOBAL_VAR(pfs_param.m_events_statements_history_sizing), CMD_LINE(REQUIRED_ARG), VALID_RANGE(-1, 1024),
...
static Sys_var_ulong Sys_pfs_max_memory_classes("performance_schema_max_memory_classes",
                                                ...
                                                DEFAULT(PFS_MAX_MEMORY_CLASS), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_long Sys_pfs_events_transactions_history_long_size(
    ...
    " Use 0 to disable, -1 for automated sizing.",
    READ_ONLY GLOBAL_VAR(pfs_param.m_events_transactions_history_long_sizing), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(-1, 1024 * 1024), DEFAULT(PFS_AUTOSIZE_VALUE), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_long Sys_pfs_events_transactions_history_size(
    ...
    " Use 0 to disable, -1 for automated sizing.",
    READ_ONLY GLOBAL_VAR(pfs_param.m_events_transactions_history_sizing), CMD_LINE(REQUIRED_ARG), VALID_RANGE(-1, 1024),
    DEFAULT(PFS_AUTOSIZE_VALUE), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);
...
static Sys_var_long Sys_pfs_max_digest_length(
    ...
    READ_ONLY GLOBAL_VAR(pfs_param.m_max_digest_length), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 1024 * 1024),
    DEFAULT(1024), BLOCK_SIZE(1), PFS_TRAILING_PROPERTIES);

static Sys_var_long Sys_pfs_connect_attrs_size("performance_schema_session_connect_attrs_size",
...
static Sys_var_ulong Sys_back_log("back_log",
                                  ...
                                  READ_ONLY GLOBAL_VAR(back_log), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 65535),
                                  DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_charptr Sys_basedir("basedir",
                                   "Path to installation directory. All paths are "
                                   "usually resolved relative to this",
                                   READ_ONLY GLOBAL_VAR(mysql_home_ptr), CMD_LINE(REQUIRED_ARG, 'b'), IN_FS_CHARSET,
                                   DEFAULT(0));

static Sys_var_charptr Sys_default_authentication_plugin("default_authentication_plugin",
                                                         "The default authentication plugin "
                                                         "used by the server to hash the password.",
                                                         READ_ONLY GLOBAL_VAR(default_auth_plugin),
...
static Sys_var_uint Sys_default_password_lifetime("default_password_lifetime",
...
#ifndef EMBEDDED_LIBRARY
static Sys_var_charptr Sys_my_bind_addr("bind_address", "IP address to bind to.",
                                        READ_ONLY GLOBAL_VAR(my_bind_addr_str), CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET,
...
static Sys_var_ulong Sys_binlog_stmt_cache_size("binlog_stmt_cache_size",
...
static Sys_var_int32 Sys_binlog_max_flush_queue_time(
    ...
    GLOBAL_VAR(opt_binlog_max_flush_queue_time), CMD_LINE(REQUIRED_ARG, OPT_BINLOG_MAX_FLUSH_QUEUE_TIME),
    VALID_RANGE(0, 100000), DEFAULT(0), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0),
    DEPRECATED_VAR(""));

static Sys_var_long Sys_binlog_group_commit_sync_delay("binlog_group_commit_sync_delay",
                                                       ...
                                                       CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 1000000 /* max 1 sec */),
...
static Sys_var_ulong Sys_binlog_group_commit_sync_no_delay_count(
    ...
    GLOBAL_VAR(opt_binlog_group_commit_sync_no_delay_count), CMD_LINE(REQUIRED_ARG),
    VALID_RANGE(0, 100000 /* max connections */), DEFAULT(0), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG);
...
{
  bool is_prepared_trx = thd->get_transaction()->xid_state()->has_state(XID_STATE::XA_PREPARED);
...
static bool binlog_format_check(sys_var *self, THD *thd, set_var *var)
{
  ...
  */
  if (thd->temporary_tables && var->type == OPT_SESSION && var->save_result.ulonglong_value == BINLOG_FORMAT_STMT &&
...
static bool prevent_global_rbr_exec_mode_idempotent(sys_var *self, THD *thd, set_var *var)
{
  if (var->type == OPT_GLOBAL)
  {
...
static Sys_var_enum Sys_binlog_format("binlog_format",
                                      ...
                                      binlog_format_names, DEFAULT(BINLOG_FORMAT_ROW), NO_MUTEX_GUARD, NOT_IN_BINLOG,
                                      ON_CHECK(binlog_format_check), ON_UPDATE(fix_binlog_format_after_update));

static const char *rbr_exec_mode_names[] = {"STRICT", "IDEMPOTENT", 0};
static Sys_var_enum rbr_exec_mode("rbr_exec_mode",
...
static Sys_var_enum Sys_binlog_row_image("binlog_row_image",
                                         ...
                                         DEFAULT(BINLOG_ROW_IMAGE_FULL), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(NULL),
                                         ON_UPDATE(NULL));

static bool on_session_track_gtids_update(sys_var *self, THD *thd, enum_var_type type)
{
  thd->session_tracker.get_tracker(SESSION_GTIDS_TRACKER)->update(thd);
  return false;
}

static const char *session_track_gtids_names[] = {"OFF", "OWN_GTID", "ALL_GTIDS", NullS};
static Sys_var_enum Sys_session_track_gtids("session_track_gtids",
...
static Sys_var_mybool Sys_binlog_direct("binlog_direct_non_transactional_updates",
                                        ...
                                        SESSION_VAR(binlog_direct_non_trans_update), CMD_LINE(OPT_ARG), DEFAULT(FALSE),
...
static Sys_var_mybool Sys_explicit_defaults_for_timestamp(
    ...
    SESSION_VAR(explicit_defaults_for_timestamp), CMD_LINE(OPT_ARG), DEFAULT(FALSE), NO_MUTEX_GUARD, NOT_IN_BINLOG,
...
static bool repository_check(sys_var *self, THD *thd, set_var *var, SLAVE_THD_TYPE thread_mask)
{
  ...
  if (rpl_info_option == (thread_mask == SLAVE_THD_IO ? opt_mi_repository_id : opt_rli_repository_id))
    return FALSE;
  ...
  if (mi != NULL)
  {
    ...
    if (!running)
    {
      ...
      {
        case SLAVE_THD_IO:
          if (Rpl_info_factory::change_mi_repository(mi, static_cast<uint>(var->save_result.ulonglong_value), &msg))
          {
            ret = TRUE;
            my_error(ER_CHANGE_RPL_INFO_REPOSITORY_FAILURE, MYF(0), msg);
          }
        ...
        case SLAVE_THD_SQL:
          ...
          if (!mi->rli->is_mts_recovery())
          {
            if (Rpl_info_factory::reset_workers(mi->rli) ||
                Rpl_info_factory::change_rli_repository(mi->rli, static_cast<uint>(var->save_result.ulonglong_value),
                                                        &msg))
            {
              ret = TRUE;
              my_error(ER_CHANGE_RPL_INFO_REPOSITORY_FAILURE, MYF(0), msg);
...
static Sys_var_enum Sys_mi_repository("master_info_repository",
                                      ...
                                      DEFAULT(INFO_REPOSITORY_FILE), NO_MUTEX_GUARD, NOT_IN_BINLOG,
                                      ON_CHECK(master_info_repository_check), ON_UPDATE(0));

ulong opt_rli_repository_id = INFO_REPOSITORY_FILE;
static Sys_var_enum Sys_rli_repository("relay_log_info_repository",
                                       ...
                                       DEFAULT(INFO_REPOSITORY_FILE), NO_MUTEX_GUARD, NOT_IN_BINLOG,
                                       ON_CHECK(relay_log_info_repository_check), ON_UPDATE(0));

static Sys_var_mybool Sys_binlog_rows_query("binlog_rows_query_log_events",
                                            "Allow writing of Rows_query_log events into binary log.",
                                            SESSION_VAR(binlog_rows_query_log_events), CMD_LINE(OPT_ARG),
                                            DEFAULT(FALSE));

static Sys_var_mybool Sys_binlog_order_commits("binlog_order_commits",
...
static Sys_var_charptr Sys_character_sets_dir("character_sets_dir", "Directory where character sets are",
  ...
  if (!opt_bootstrap && !opt_noacl)
  {
    ...
    else
    {
      ...
      lex_string_set(&se_name,
                     pointer_cast<const char *>(down_cast<Sys_var_plugin *>(self)->global_value_ptr(thd, NULL)));
    }
    ...
    if ((plugin = ha_resolve_by_name(NULL, &se_name, FALSE)))
    {
      ...
      if (ha_is_storage_engine_disabled(hton))
        sql_print_warning("%s is set to a disabled storage engine %s.", self->name.str, se_name.str);
...
static bool check_charset(sys_var *self, THD *thd, set_var *var)
{
  ...
  if (var->value->result_type() == STRING_RESULT)
  {
    ...
    else
    {
      ...
      if (!(var->save_result.ptr = get_charset_by_csname(err.ptr(), MY_CS_PRIMARY, MYF(0))) &&
...
static Sys_var_enum Sys_completion_type("completion_type",
...
static Sys_var_struct Sys_collation_database("collation_database",
                                             ...
                                             "character set",
                                             SESSION_VAR(collation_database), NO_CMD_LINE, offsetof(CHARSET_INFO, name),
...
static Sys_var_struct Sys_collation_server("collation_server", "The server default collation",
                                           SESSION_VAR(collation_server), NO_CMD_LINE, offsetof(CHARSET_INFO, name),
                                           DEFAULT(&default_charset_info), NO_MUTEX_GUARD, IN_BINLOG,
                                           ON_CHECK(check_collation_not_null));

static const char *concurrent_insert_names[] = {"NEVER", "AUTO", "ALWAYS", 0};
static Sys_var_enum Sys_concurrent_insert("concurrent_insert",
                                          "Use concurrent insert with MyISAM. Possible "
                                          "values are NEVER, AUTO, ALWAYS",
                                          GLOBAL_VAR(myisam_concurrent_insert), CMD_LINE(OPT_ARG),
                                          concurrent_insert_names, DEFAULT(1));

static Sys_var_ulong Sys_connect_timeout("connect_timeout",
                                         "The number of seconds the mysqld server is waiting for a connect "
                                         "packet before responding with 'Bad handshake'",
                                         GLOBAL_VAR(connect_timeout), CMD_LINE(REQUIRED_ARG),
                                         VALID_RANGE(2, LONG_TIMEOUT), DEFAULT(CONNECT_TIMEOUT), BLOCK_SIZE(1));

static Sys_var_charptr Sys_datadir("datadir", "Path to the database root directory",
                                   READ_ONLY GLOBAL_VAR(mysql_real_data_home_ptr), CMD_LINE(REQUIRED_ARG, 'h'),
                                   IN_FS_CHARSET, DEFAULT(mysql_real_data_home));

#ifndef NDEBUG
static Sys_var_dbug Sys_dbug("debug", "Debug log", sys_var::SESSION, CMD_LINE(OPT_ARG, '#'), DEFAULT(""),
...
static Sys_var_enum Sys_delay_key_write("delay_key_write", "Type of DELAY_KEY_WRITE",
...
static bool event_scheduler_update(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  */
  bool ret = opt_event_scheduler_value == Events::EVENTS_ON ? Events::start(&err_no) : Events::stop();
  mysql_mutex_lock(&LOCK_global_system_variables);
  if (ret)
  {
...
static Sys_var_ulong Sys_expire_logs_days("expire_logs_days",
                                          "If non-zero, binary logs will be purged after expire_logs_days "
                                          "days; possible purges happen at startup and at binary log rotation",
                                          GLOBAL_VAR(expire_logs_days), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 99),
                                          DEFAULT(0), BLOCK_SIZE(1));
...
static Sys_var_charptr Sys_ft_stopword_file("ft_stopword_file", "Use stopwords from this file instead of built-in list",
...
static Sys_var_lexstring Sys_init_connect("init_connect",
...
static Sys_var_charptr Sys_init_file("init_file", "Read SQL commands from this file at startup",
                                     READ_ONLY GLOBAL_VAR(opt_init_file), CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET,
                                     DEFAULT(0));

static PolyLock_rwlock PLock_sys_init_slave(&LOCK_sys_init_slave);
static Sys_var_lexstring Sys_init_slave("init_slave",
                                        ...
                                        DEFAULT(""), &PLock_sys_init_slave, NOT_IN_BINLOG, ON_CHECK(check_init_string));
...
static Sys_var_ulong Sys_join_buffer_size("join_buffer_size", "The size of the buffer that is used for full joins",
...
static Sys_var_keycache Sys_key_cache_age_threshold("key_cache_age_threshold",
                                                    ...
                                                    VALID_RANGE(100, ULONG_MAX), DEFAULT(300), BLOCK_SIZE(100),
                                                    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
                                                    ON_UPDATE(update_keycache_param));

static Sys_var_mybool Sys_large_files_support("large_files_support",
                                              "Whether mysqld was compiled with options for large file support",
                                              READ_ONLY GLOBAL_VAR(opt_large_files), NO_CMD_LINE,
...
static Sys_var_mybool Sys_large_pages("large_pages", "Enable support for large pages",
                                      READ_ONLY GLOBAL_VAR(opt_large_pages), IF_WIN(NO_CMD_LINE, CMD_LINE(OPT_ARG)),
                                      DEFAULT(FALSE));

static Sys_var_charptr Sys_language("lc_messages_dir", "Directory where error messages are",
                                    READ_ONLY GLOBAL_VAR(lc_messages_dir_ptr),
                                    CMD_LINE(REQUIRED_ARG, OPT_LC_MESSAGES_DIRECTORY), IN_FS_CHARSET, DEFAULT(0));
...
static bool transaction_write_set_check(sys_var *self, THD *thd, set_var *var)
{
  ...
  if (var->type == OPT_GLOBAL && global_system_variables.binlog_format != BINLOG_FORMAT_ROW)
  {
    my_error(ER_PREVENTS_VARIABLE_WITHOUT_RBR, MYF(0), var->var->name.str);
    return true;
  }

  if (var->type == OPT_SESSION && thd->variables.binlog_format != BINLOG_FORMAT_ROW)
  {
    my_error(ER_PREVENTS_VARIABLE_WITHOUT_RBR, MYF(0), var->var->name.str);
    return true;
  }
  ...
  */
  if (mysql_bin_log.m_dependency_tracker.m_opt_tracking_mode != DEPENDENCY_TRACKING_COMMIT_ORDER)
  {
    my_error(ER_WRONG_USAGE, MYF(0), "transaction_write_set_extraction (changed)",
...
static Sys_var_enum Sys_extract_write_set("transaction_write_set_extraction",
                                          ...
                                          transaction_write_set_hashing_algorithms, DEFAULT(HASH_ALGORITHM_OFF),
                                          NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(transaction_write_set_check),
                                          ON_UPDATE(NULL));

static Sys_var_ulong Sys_rpl_stop_slave_timeout("rpl_stop_slave_timeout",
...
static Sys_var_enum Sys_binlog_error_action("binlog_error_action",
...
static Sys_var_mybool Sys_check_proxy_users("check_proxy_users",
...
static Sys_var_mybool Sys_mysql_native_password_proxy_users(
    ...
    GLOBAL_VAR(mysql_native_password_proxy_users), CMD_LINE(OPT_ARG), DEFAULT(FALSE));
...
static Sys_var_mybool Sys_use_v1_row_events("log_bin_use_v1_row_events",
                                            ...
                                            GLOBAL_VAR(log_bin_use_v1_row_events), CMD_LINE(OPT_ARG), DEFAULT(FALSE));

static Sys_var_charptr Sys_log_error("log_error", "Error log file", READ_ONLY GLOBAL_VAR(log_error_dest),
                                     CMD_LINE(OPT_ARG, OPT_LOG_ERROR), IN_FS_CHARSET, DEFAULT(disabled_my_option),
...
static Sys_var_mybool Sys_log_queries_not_using_indexes(
    ...
    GLOBAL_VAR(opt_log_queries_not_using_indexes), CMD_LINE(OPT_ARG), DEFAULT(FALSE));

static Sys_var_mybool Sys_log_slow_admin_statements(
    "log_slow_admin_statements",
    "Log slow OPTIMIZE, ANALYZE, ALTER and other administrative statements to "
    "the slow log if it is open.",
    GLOBAL_VAR(opt_log_slow_admin_statements), CMD_LINE(OPT_ARG), DEFAULT(FALSE));

static Sys_var_mybool Sys_log_slow_slave_statements(
    "log_slow_slave_statements", "Log slow statements executed by slave thread to the slow log if it is open.",
    GLOBAL_VAR(opt_log_slow_slave_statements), CMD_LINE(OPT_ARG), DEFAULT(FALSE));

static bool update_log_throttle_queries_not_using_indexes(sys_var *self, THD *thd, enum_var_type type)
{
...
static Sys_var_ulong Sys_log_throttle_queries_not_using_indexes(
    ...
    GLOBAL_VAR(opt_log_throttle_queries_not_using_indexes), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, ULONG_MAX),
...
static Sys_var_ulong Sys_log_warnings("log_warnings", "Log some not critical warnings to the log file",
                                      GLOBAL_VAR(log_warnings), CMD_LINE(OPT_ARG, 'W'), VALID_RANGE(0, 2), DEFAULT(2),
                                      BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
                                      ON_UPDATE(update_log_warnings), DEPRECATED_VAR("log_error_verbosity"));

static bool update_log_error_verbosity(sys_var *self, THD *thd, enum_var_type type)
{
...
static Sys_var_ulong Sys_log_error_verbosity("log_error_verbosity",
                                             ...
                                             DEFAULT(3), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
                                             ON_UPDATE(update_log_error_verbosity));

static Sys_var_enum Sys_log_timestamps("log_timestamps",
                                       ...
                                       GLOBAL_VAR(opt_log_timestamps), CMD_LINE(REQUIRED_ARG), timestamp_type_names,
                                       DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG);

static Sys_var_mybool Sys_log_statements_unsafe_for_binlog(
    "log_statements_unsafe_for_binlog", "Log statements considered unsafe when using statement based binary logging.",
...
static Sys_var_mybool Sys_log_syslog_enable("log_syslog",
...
// preserve current defaults for both platforms:
                                            ...
                                            DEFAULT(FALSE),
                                             ...
                                             DEFAULT(TRUE),
#endif
                                            NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(fix_syslog));

static Sys_var_charptr Sys_log_syslog_tag("log_syslog_tag",
...
static bool check_syslog_facility(sys_var *self, THD *THD, set_var *var)
{
  ...
  if (var->value && log_syslog_find_facility(var->save_result.string_value.str, &rsf))
    return true;
  return false;
}
...
static Sys_var_charptr Sys_log_syslog_facility("log_syslog_facility",
                                               "When logging issues using the host operating system's syslog, "
...
static Sys_var_mybool Sys_log_syslog_log_pid("log_syslog_include_pid",
                                             "When logging issues using the host operating system's syslog, "
...
static bool fix_low_prio_updates(sys_var *self, THD *thd, enum_var_type type)
{
  if (type == OPT_SESSION)
  {
    ...
    thd->insert_lock_default =
        (thd->variables.low_priority_updates ? TL_WRITE_LOW_PRIORITY : TL_WRITE_CONCURRENT_INSERT);
  }
  else
...
static bool check_max_allowed_packet(sys_var *self, THD *thd, set_var *var)
{
  ...
  {
    push_warning_printf(thd, Sql_condition::SL_WARNING, WARN_OPTION_BELOW_LIMIT, ER(WARN_OPTION_BELOW_LIMIT),
...
static Sys_var_ulong Sys_slave_max_allowed_packet(
    "slave_max_allowed_packet", "The maximum packet length to sent successfully from the master to slave.",
    GLOBAL_VAR(slave_max_allowed_packet), CMD_LINE(REQUIRED_ARG), VALID_RANGE(1024, MAX_MAX_ALLOWED_PACKET),
    DEFAULT(MAX_MAX_ALLOWED_PACKET), BLOCK_SIZE(1024));

static Sys_var_ulonglong Sys_max_binlog_cache_size(
    "max_binlog_cache_size", "Sets the total size of the transactional cache", GLOBAL_VAR(max_binlog_cache_size),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(IO_SIZE, ULLONG_MAX), DEFAULT((ULLONG_MAX / IO_SIZE) * IO_SIZE),
    BLOCK_SIZE(IO_SIZE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(fix_binlog_cache_size));

static Sys_var_ulonglong Sys_max_binlog_stmt_cache_size(
    "max_binlog_stmt_cache_size", "Sets the total size of the statement cache", GLOBAL_VAR(max_binlog_stmt_cache_size),
    CMD_LINE(REQUIRED_ARG), VALID_RANGE(IO_SIZE, ULLONG_MAX), DEFAULT((ULLONG_MAX / IO_SIZE) * IO_SIZE),
    BLOCK_SIZE(IO_SIZE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(fix_binlog_stmt_cache_size));
...
static Sys_var_ulong Sys_max_binlog_size("max_binlog_size",
...
static Sys_var_ulong Sys_max_connections("max_connections", "The number of simultaneous clients allowed",
                                         GLOBAL_VAR(max_connections), CMD_LINE(REQUIRED_ARG), VALID_RANGE(1, 100000),
                                         DEFAULT(MAX_CONNECTIONS_DEFAULT), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
                                         ON_CHECK(0), ON_UPDATE(0), NULL,
                                         /* max_connections is used as a sizing hint by the performance schema. */
                                         sys_var::PARSE_EARLY);

static Sys_var_ulong Sys_max_connect_errors("max_connect_errors",
                                            "If there is more than this number of interrupted connections from "
                                            "a host this host will be blocked from further connections",
                                            GLOBAL_VAR(max_connect_errors), CMD_LINE(REQUIRED_ARG),
                                            VALID_RANGE(1, ULONG_MAX), DEFAULT(100), BLOCK_SIZE(1));

static Sys_var_long Sys_max_digest_length("max_digest_length", "Maximum length considered for digest text.",
...
static Sys_var_ulong Sys_max_insert_delayed_threads(
    ...
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_max_delayed_threads), ON_UPDATE(0), DEPRECATED_VAR(""));
...
static Sys_var_ulong Sys_metadata_locks_cache_size("metadata_locks_cache_size", "Has no effect, deprecated",
...
static Sys_var_ulong Sys_metadata_locks_hash_instances("metadata_locks_hash_instances", "Has no effect, deprecated",
...
static Sys_var_uint Sys_pseudo_thread_id("pseudo_thread_id", "This variable is for internal server use",
...
static Sys_var_ulong Sys_max_points_in_geometry("max_points_in_geometry", "Maximum number of points in a geometry",
...
static Sys_var_ulong Sys_max_prepared_stmt_count(
    ...
    GLOBAL_VAR(max_prepared_stmt_count), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 1024 * 1024), DEFAULT(16382),
    BLOCK_SIZE(1), &PLock_prepared_stmt_count, NOT_IN_BINLOG, ON_CHECK(NULL), ON_UPDATE(NULL), NULL,
...
static bool fix_max_relay_log_size(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  for (mi_map::iterator it = channel_map.begin(); it != channel_map.end(); it++)
  {
    ...
    if (mi != NULL)
      mi->rli->relay_log.set_max_size(max_relay_log_size ? max_relay_log_size : max_binlog_size);
  }
  channel_map.unlock();
#endif
  return false;
...
static Sys_var_ulong Sys_max_tmp_tables("max_tmp_tables",
                                        "Maximum number of temporary tables a client can keep open at a time",
                                        SESSION_VAR(max_tmp_tables), CMD_LINE(REQUIRED_ARG), VALID_RANGE(1, ULONG_MAX),
                                        DEFAULT(32), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
...
static PolyLock_rwlock PLock_named_pipe_full_access_group(&LOCK_named_pipe_full_access_group);
static bool check_named_pipe_full_access_group(sys_var *self, THD *thd, set_var *var)
{
  if (!var->value)
    return false;  // DEFAULT is ok

  if (!is_valid_named_pipe_full_access_group(var->save_result.string_value.str))
  {
...
static Sys_var_charptr Sys_named_pipe_full_access_group(
    "named_pipe_full_access_group", "Name of Windows group granted full access to the named pipe",
    GLOBAL_VAR(named_pipe_full_access_group), CMD_LINE(REQUIRED_ARG, OPT_NAMED_PIPE_FULL_ACCESS_GROUP), IN_FS_CHARSET,
    DEFAULT(DEFAULT_NAMED_PIPE_FULL_ACCESS_GROUP), &PLock_named_pipe_full_access_group, NOT_IN_BINLOG,
    ON_CHECK(check_named_pipe_full_access_group), ON_UPDATE(fix_named_pipe_full_access_group));
...
static bool fix_net_read_timeout(sys_var *self, THD *thd, enum_var_type type)
{
  if (type != OPT_GLOBAL)
  {
    ...
    my_net_set_read_timeout(thd->get_protocol_classic()->get_net(), thd->variables.net_read_timeout);
  }
  return false;
}
...
static bool fix_net_write_timeout(sys_var *self, THD *thd, enum_var_type type)
{
  if (type != OPT_GLOBAL)
  {
    ...
    my_net_set_write_timeout(thd->get_protocol_classic()->get_net(), thd->variables.net_write_timeout);
  }
  return false;
}
...
static bool fix_net_retry_count(sys_var *self, THD *thd, enum_var_type type)
{
  if (type != OPT_GLOBAL)
  {
    ...
    thd->get_protocol_classic()->get_net()->retry_count = thd->variables.net_retry_count;
  }
  return false;
}
...
static Sys_var_mybool Sys_new_mode("new", "Use very new possible \"unsafe\" functions", SESSION_VAR(new_mode),
                                   CMD_LINE(OPT_ARG, 'n'), DEFAULT(FALSE));

static Sys_var_mybool Sys_old_mode("old", "Use compatible behavior", READ_ONLY GLOBAL_VAR(old_mode), CMD_LINE(OPT_ARG),
                                   DEFAULT(FALSE));

#ifndef EMBEDDED_LIBRARY
static Sys_var_mybool Sys_show_compatibility_56("show_compatibility_56",
...
static Sys_var_mybool Sys_old_alter_table("old_alter_table", "Use old, non-optimized alter table",
                                          SESSION_VAR(old_alter_table), CMD_LINE(OPT_ARG), DEFAULT(FALSE));

static bool old_passwords_check(sys_var *self MY_ATTRIBUTE((unused)), THD *thd MY_ATTRIBUTE((unused)), set_var *var)
{
...
static Sys_var_uint Sys_old_passwords("old_passwords",
                                      "Determine which hash algorithm to use when generating passwords using "
                                      "the PASSWORD() function",
                                      SESSION_VAR(old_passwords), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 2), DEFAULT(0),
                                      BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(old_passwords_check));
...
static Sys_var_ulong Sys_range_optimizer_max_mem_size("range_optimizer_max_mem_size",
                                                      ...
                                                      SESSION_VAR(range_optimizer_max_mem_size), CMD_LINE(REQUIRED_ARG),
                                                      VALID_RANGE(0, ULONG_MAX), DEFAULT(8388608), BLOCK_SIZE(1));

static bool limit_parser_max_mem_size(sys_var *self, THD *thd, set_var *var)
{
  if (var->type == OPT_GLOBAL)
    return false;
  ...
  if (val > global_system_variables.parser_max_mem_size)
  {
    if (thd->security_context()->check_access(SUPER_ACL))
      return false;
    var->save_result.ulonglong_value = global_system_variables.parser_max_mem_size;
...
*/
static Sys_var_ulonglong Sys_parser_max_mem_size("parser_max_mem_size",
...
export void update_parser_max_mem_size()
{
  ...
  const ulonglong new_val = std::min(max_max, global_system_variables.parser_max_mem_size);
  Sys_parser_max_mem_size.update_default(new_val);
  global_system_variables.parser_max_mem_size = new_val;
}

static const char *optimizer_switch_names[] = {"index_merge",
                                               "index_merge_union",
                                               ...
                                               "default",
                                               NullS};
static Sys_var_flagset Sys_optimizer_switch("optimizer_switch",
...
static Sys_var_mybool Sys_var_end_markers_in_json(
    ...
    SESSION_VAR(end_markers_in_json), CMD_LINE(OPT_ARG), DEFAULT(FALSE));
...
static Sys_var_flagset Sys_optimizer_trace_features(
    ...
    SESSION_VAR(optimizer_trace_features), CMD_LINE(REQUIRED_ARG), Opt_trace_context::feature_names,
    DEFAULT(Opt_trace_context::default_features));

/** Delete all old optimizer traces */
static bool optimizer_trace_update(sys_var *self, THD *thd, enum_var_type type)
{
  thd->opt_trace.reset();
  return false;
}

static Sys_var_long Sys_optimizer_trace_offset("optimizer_trace_offset",
...
static Sys_var_long Sys_optimizer_trace_limit("optimizer_trace_limit", "Maximum number of shown optimizer traces",
...
static Sys_var_charptr Sys_pid_file("pid_file", "Pid file used by safe_mysqld", READ_ONLY GLOBAL_VAR(pidfile_name_ptr),
                                    CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET, DEFAULT(0));

static Sys_var_charptr Sys_plugin_dir("plugin_dir", "Directory for plugins", READ_ONLY GLOBAL_VAR(opt_plugin_dir_ptr),
                                      CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET, DEFAULT(0));
...
static Sys_var_uint Sys_port("port",
                             ...
                             READ_ONLY GLOBAL_VAR(mysqld_port), CMD_LINE(REQUIRED_ARG, 'P'), VALID_RANGE(0, 65535),
                             DEFAULT(0), BLOCK_SIZE(1));
...
static Sys_var_uint Sys_protocol_version("protocol_version",
                                         ...
                                         READ_ONLY GLOBAL_VAR(protocol_version), NO_CMD_LINE, VALID_RANGE(0, ~0),
                                         DEFAULT(PROTOCOL_VERSION), BLOCK_SIZE(1));

static Sys_var_proxy_user Sys_proxy_user("proxy_user", "The proxy user account name used when logging in",
                                         IN_SYSTEM_CHARSET);

static Sys_var_external_user Sys_external_user("external_user", "The external user account used when logging in",
                                               IN_SYSTEM_CHARSET);
...
*/
static Sys_var_mybool Sys_readonly("read_only",
...
*/
static Sys_var_mybool Sys_super_readonly("super_read_only",
                                         "Make all non-temporary tables read-only, with the exception for "
                                         ...
                                         GLOBAL_VAR(super_read_only), CMD_LINE(OPT_ARG), DEFAULT(FALSE), NO_MUTEX_GUARD,
                                         NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(fix_super_read_only));
...
static Sys_var_ulong Sys_multi_range_count("multi_range_count",
                                           "Number of key ranges to request at once. "
                                           "This variable has no effect, and is deprecated. "
                                           "It will be removed in a future release.",
                                           SESSION_VAR(multi_range_count), CMD_LINE(REQUIRED_ARG),
                                           VALID_RANGE(1, ULONG_MAX), DEFAULT(256), BLOCK_SIZE(1), NO_MUTEX_GUARD,
                                           NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0), DEPRECATED_VAR(""));
...
static Sys_var_mybool Sys_shared_memory("shared_memory", "Enable the shared memory",
                                        READ_ONLY GLOBAL_VAR(opt_enable_shared_memory), CMD_LINE(OPT_ARG),
                                        DEFAULT(FALSE));

static Sys_var_charptr Sys_shared_memory_base_name("shared_memory_base_name", "Base name of shared memory",
                                                   READ_ONLY GLOBAL_VAR(shared_memory_base_name),
                                                   CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET, DEFAULT(0));
#endif
...
static Sys_var_charptr Sys_socket("socket", "Socket file to use for connection", READ_ONLY GLOBAL_VAR(mysqld_unix_port),
                                  CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET, DEFAULT(0));

static Sys_var_ulong Sys_thread_stack("thread_stack", "The stack size for each thread",
                                      READ_ONLY GLOBAL_VAR(my_thread_stack_size), CMD_LINE(REQUIRED_ARG),
                                      VALID_RANGE(128 * 1024, ULONG_MAX), DEFAULT(DEFAULT_THREAD_STACK),
                                      BLOCK_SIZE(1024));

static Sys_var_charptr Sys_tmpdir("tmpdir",
                                  ...
                                  READ_ONLY GLOBAL_VAR(opt_mysql_tmpdir), CMD_LINE(REQUIRED_ARG, 't'), IN_FS_CHARSET,
                                  DEFAULT(0));

static bool fix_trans_mem_root(sys_var *self, THD *thd, enum_var_type type)
{
  if (type != OPT_GLOBAL)
    thd->get_transaction()->init_mem_root_defaults(thd->variables.trans_alloc_block_size,
...
#ifndef EMBEDDED_LIBRARY
static const char *thread_handling_names[] = {"one-thread-per-connection", "no-threads", "loaded-dynamically", 0};
static Sys_var_enum Sys_thread_handling("thread_handling",
...
static bool fix_query_cache_size(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  */
  if (query_cache_size != new_cache_size)
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WARN_QC_RESIZE, ER(ER_WARN_QC_RESIZE),
                        query_cache_size, new_cache_size);

  query_cache_size = new_cache_size;
  return false;
}
static Sys_var_ulong Sys_query_cache_size("query_cache_size",
                                          "The memory allocated to store results from old queries. "
                                          "This variable is deprecated and will be removed in a future release.",
                                          GLOBAL_VAR(query_cache_size), CMD_LINE(REQUIRED_ARG),
                                          VALID_RANGE(0, ULONG_MAX), DEFAULT(1024U * 1024U), BLOCK_SIZE(1024),
                                          NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(fix_query_cache_size),
                                          DEPRECATED_VAR(""));

static Sys_var_ulong Sys_query_cache_limit("query_cache_limit",
...
static Sys_var_ulong Sys_query_cache_min_res_unit(
    ...
    "This variable is deprecated and will be removed in a future release.",
    GLOBAL_VAR(query_cache_min_res_unit), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, ULONG_MAX),
    DEFAULT(QUERY_CACHE_MIN_RESULT_DATA_SIZE), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
...
static Sys_var_enum Sys_query_cache_type("query_cache_type",
                                         ...
                                         "This variable is deprecated and will be removed in a future release.",
                                         SESSION_VAR(query_cache_type), CMD_LINE(REQUIRED_ARG), query_cache_type_names,
                                         DEFAULT(0), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_query_cache_type),
...
static Sys_var_mybool Sys_query_cache_wlock_invalidate(
    ...
    "This variable is deprecated and will be removed in a future release.",
    SESSION_VAR(query_cache_wlock_invalidate), CMD_LINE(OPT_ARG), DEFAULT(FALSE), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(NULL), ON_UPDATE(NULL), DEPRECATED_VAR(""));

static bool on_check_opt_secure_auth(sys_var *self, THD *thd, set_var *var)
{
  push_deprecated_warn_no_replacement(thd, "--secure-auth");
  return (!var->save_result.ulonglong_value);
}

static Sys_var_mybool Sys_secure_auth("secure_auth",
                                      "Disallow authentication for accounts that have old (pre-4.1) "
                                      "passwords. Deprecated. Always TRUE.",
                                      GLOBAL_VAR(opt_secure_auth), CMD_LINE(OPT_ARG, OPT_SECURE_AUTH), DEFAULT(TRUE),
                                      NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(on_check_opt_secure_auth));

static Sys_var_charptr Sys_secure_file_priv("secure_file_priv",
...
#else
                                             CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET,
                                             DEFAULT(DEFAULT_SECURE_FILE_PRIV_EMBEDDED_DIR));
#endif
...
static Sys_var_ulong Sys_server_id("server_id",
                                   "Uniquely identifies the server instance in the community of "
                                   "replication partners",
                                   GLOBAL_VAR(server_id), CMD_LINE(REQUIRED_ARG, OPT_SERVER_ID),
                                   VALID_RANGE(0, UINT_MAX32), DEFAULT(0), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
                                   ON_CHECK(0), ON_UPDATE(fix_server_id));

static Sys_var_charptr Sys_server_uuid("server_uuid", "Uniquely identifies the server instance in the universe",
                                       READ_ONLY GLOBAL_VAR(server_uuid_ptr), NO_CMD_LINE, IN_FS_CHARSET,
                                       DEFAULT(server_uuid));

static Sys_var_uint Sys_server_id_bits("server_id_bits", "Set number of significant bits in server-id",
                                       GLOBAL_VAR(opt_server_id_bits), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, 32),
                                       DEFAULT(32), BLOCK_SIZE(1));

static Sys_var_mybool Sys_slave_compressed_protocol("slave_compressed_protocol",
...
static Sys_var_enum Slave_exec_mode("slave_exec_mode",
...
const char *slave_type_conversions_name[] = {"ALL_LOSSY", "ALL_NON_LOSSY", "ALL_UNSIGNED", "ALL_SIGNED", 0};
...
static Sys_var_set Slave_rows_search_algorithms("slave_rows_search_algorithms",
                                                ...
                                                CMD_LINE(REQUIRED_ARG), slave_rows_search_algorithms_names,
...
static Sys_var_enum Mts_parallel_type("slave_parallel_type",
                                      "Specifies if the slave will use database partitioning "
                                      "or information from master to parallelize transactions."
                                      "(Default: DATABASE).",
                                      GLOBAL_VAR(mts_parallel_option), CMD_LINE(REQUIRED_ARG), mts_parallel_type_names,
                                      DEFAULT(MTS_PARALLEL_TYPE_DB_NAME), NO_MUTEX_GUARD, NOT_IN_BINLOG,
...
static bool check_binlog_transaction_dependency_tracking(sys_var *self, THD *thd, set_var *var)
{
  ...
  {
    my_error(ER_WRONG_USAGE, MYF(0), "binlog_transaction_dependency_tracking (!= COMMIT_ORDER)",
...
{
  my_atomic_store64(
      &mysql_bin_log.m_dependency_tracker.get_writeset()->m_opt_max_history_size,
      static_cast<int64>(mysql_bin_log.m_dependency_tracker.get_writeset()->m_opt_max_history_size_base_var));
  return false;
}

static PolyLock_mutex PLock_slave_trans_dep_tracker(&LOCK_slave_trans_dep_tracker);
static const char *opt_binlog_transaction_dependency_tracking_names[] = {"COMMIT_ORDER", "WRITESET", "WRITESET_SESSION",
                                                                         NullS};
static Sys_var_enum Binlog_transaction_dependency_tracking(
    ...
    GLOBAL_VAR(mysql_bin_log.m_dependency_tracker.m_opt_tracking_mode_value), CMD_LINE(REQUIRED_ARG),
    opt_binlog_transaction_dependency_tracking_names, DEFAULT(DEPENDENCY_TRACKING_COMMIT_ORDER),
    &PLock_slave_trans_dep_tracker, NOT_IN_BINLOG, ON_CHECK(check_binlog_transaction_dependency_tracking),
    ON_UPDATE(update_binlog_transaction_dependency_tracking));
static Sys_var_ulong Binlog_transaction_dependency_history_size(
    "binlog_transaction_dependency_history_size", "Maximum number of rows to keep in the writeset history.",
    GLOBAL_VAR(mysql_bin_log.m_dependency_tracker.get_writeset()->m_opt_max_history_size_base_var),
    CMD_LINE(REQUIRED_ARG, 0), VALID_RANGE(1, 1000000), DEFAULT(25000), BLOCK_SIZE(1), &PLock_slave_trans_dep_tracker,
    NOT_IN_BINLOG, ON_CHECK(NULL), ON_UPDATE(update_binlog_transaction_dependency_history_size));
...
static Sys_var_mybool Sys_slave_preserve_commit_order(
    ...
    "Disabled by default.",
    GLOBAL_VAR(opt_slave_preserve_commit_order), CMD_LINE(OPT_ARG), DEFAULT(FALSE), NO_MUTEX_GUARD, NOT_IN_BINLOG,
    ON_CHECK(check_slave_stopped), ON_UPDATE(NULL));
#endif
...
bool Sys_var_enum_binlog_checksum::global_update(THD *thd, set_var *var)
{
  ...
  */
  ...
  {
    bool alg_changed = (binlog_checksum_options != (uint)var->save_result.ulonglong_value);
    if (alg_changed)
  ...
  {
    binlog_checksum_options = static_cast<ulong>(var->save_result.ulonglong_value);
  }
  assert(binlog_checksum_options == var->save_result.ulonglong_value);
  assert(mysql_bin_log.checksum_alg_reset == binary_log::BINLOG_CHECKSUM_ALG_UNDEF);
...
static Sys_var_enum_binlog_checksum Binlog_checksum_enum(
    ...
    "default is CRC32.",
    GLOBAL_VAR(binlog_checksum_options), CMD_LINE(REQUIRED_ARG), binlog_checksum_type_names,
...
{
  const sql_mode_t strict_modes = (MODE_STRICT_TRANS_TABLES | MODE_STRICT_ALL_TABLES);

  const sql_mode_t new_strict_submodes = (MODE_NO_ZERO_IN_DATE | MODE_NO_ZERO_DATE | MODE_ERROR_FOR_DIVISION_BY_ZERO);
  ...
  if (((strict_modes_set | new_strict_submodes_set) != 0) &&
      ((new_strict_submodes_set != new_strict_submodes) || (strict_modes_set == 0)))
  {
    if (thd)
      push_warning(thd, Sql_condition::SL_WARNING, ER_SQL_MODE_MERGED, ER_THD(thd, ER_SQL_MODE_MERGED));
    else
      sql_print_warning(
...
export sql_mode_t expand_sql_mode(sql_mode_t sql_mode, THD *thd)
{
  if (sql_mode & MODE_ANSI)
  {
    ...
    */
    sql_mode |=
        (MODE_REAL_AS_FLOAT | MODE_PIPES_AS_CONCAT | MODE_ANSI_QUOTES | MODE_IGNORE_SPACE | MODE_ONLY_FULL_GROUP_BY);
  }
...
{
  var->save_result.ulonglong_value = expand_sql_mode(var->save_result.ulonglong_value, thd);

  /* Warning displayed only if the non default sql_mode is specified. */
  if (var->value)
  {
    /* Check if the NO_AUTO_CREATE_USER flag has been swapped. */
    if ((thd->variables.sql_mode ^ var->save_result.ulonglong_value) & MODE_NO_AUTO_CREATE_USER)
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_DEPRECATED_SQLMODE,
                          ER_THD(thd, ER_WARN_DEPRECATED_SQLMODE), "NO_AUTO_CREATE_USER");
    }
    static const sql_mode_t deprecated_mask = MODE_DB2 | MODE_MAXDB | MODE_MSSQL | MODE_MYSQL323 | MODE_MYSQL40 |
                                              MODE_ORACLE | MODE_POSTGRESQL | MODE_NO_FIELD_OPTIONS |
                                              MODE_NO_KEY_OPTIONS | MODE_NO_TABLE_OPTIONS;
    sql_mode_t deprecated_modes = var->save_result.ulonglong_value & deprecated_mask;
    if (deprecated_modes != 0)
    {
      ...
      if (sql_mode_string_representation(thd, deprecated_modes, &buf))
        return true;  // OOM
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_DEPRECATED_SQLMODE,
                          ER_THD(thd, ER_WARN_DEPRECATED_SQLMODE), buf.str);
    }
  }
...
*/
static Sys_var_set Sys_sql_mode("sql_mode",
                                "Syntax: sql-mode=mode[,mode[,mode...]]. See the manual for the "
                                "complete list of valid sql modes",
                                SESSION_VAR(sql_mode), CMD_LINE(REQUIRED_ARG), sql_mode_names,
                                DEFAULT(MODE_NO_ENGINE_SUBSTITUTION | MODE_ONLY_FULL_GROUP_BY |
                                        MODE_STRICT_TRANS_TABLES | MODE_NO_ZERO_IN_DATE | MODE_NO_ZERO_DATE |
...
static Sys_var_ulong Sys_max_execution_time(
    "max_execution_time", "Kill SELECT statement that takes over the specified number of milliseconds",
    SESSION_VAR(max_execution_time), CMD_LINE(REQUIRED_ARG), VALID_RANGE(0, ULONG_MAX), DEFAULT(0), BLOCK_SIZE(1));
...
static Sys_var_charptr Sys_ssl_ca("ssl_ca", "CA file in PEM format (check OpenSSL docs, implies --ssl)",
                                  READ_ONLY GLOBAL_VAR(opt_ssl_ca), SSL_OPT(OPT_SSL_CA), IN_FS_CHARSET, DEFAULT(0));

static Sys_var_charptr Sys_ssl_capath("ssl_capath", "CA directory (check OpenSSL docs, implies --ssl)",
                                      READ_ONLY GLOBAL_VAR(opt_ssl_capath), SSL_OPT(OPT_SSL_CAPATH), IN_FS_CHARSET,
                                      DEFAULT(0));

static Sys_var_charptr Sys_tls_version("tls_version",
                                       "TLS version, permitted values are TLSv1, TLSv1.1, TLSv1.2(Only for openssl)",
                                       READ_ONLY GLOBAL_VAR(opt_tls_version), SSL_OPT(OPT_TLS_VERSION), IN_FS_CHARSET,
                                       "TLSv1,TLSv1.1,TLSv1.2");

static Sys_var_charptr Sys_ssl_cert("ssl_cert", "X509 cert in PEM format (implies --ssl)",
                                    READ_ONLY GLOBAL_VAR(opt_ssl_cert), SSL_OPT(OPT_SSL_CERT), IN_FS_CHARSET,
                                    DEFAULT(0));

static Sys_var_charptr Sys_ssl_cipher("ssl_cipher", "SSL cipher to use (implies --ssl)",
                                      READ_ONLY GLOBAL_VAR(opt_ssl_cipher), SSL_OPT(OPT_SSL_CIPHER), IN_FS_CHARSET,
                                      DEFAULT(0));

static Sys_var_charptr Sys_ssl_key("ssl_key", "X509 key in PEM format (implies --ssl)",
                                   READ_ONLY GLOBAL_VAR(opt_ssl_key), SSL_OPT(OPT_SSL_KEY), IN_FS_CHARSET, DEFAULT(0));

static Sys_var_charptr Sys_ssl_crl("ssl_crl", "CRL file in PEM format (check OpenSSL docs, implies --ssl)",
                                   READ_ONLY GLOBAL_VAR(opt_ssl_crl), SSL_OPT(OPT_SSL_CRL), IN_FS_CHARSET, DEFAULT(0));

static Sys_var_charptr Sys_ssl_crlpath("ssl_crlpath", "CRL directory (check OpenSSL docs, implies --ssl)",
                                       READ_ONLY GLOBAL_VAR(opt_ssl_crlpath), SSL_OPT(OPT_SSL_CRLPATH), IN_FS_CHARSET,
                                       DEFAULT(0));

#if defined(HAVE_OPENSSL)
static Sys_var_mybool Sys_auto_generate_certs("auto_generate_certs",
                                              ...
                                              READ_ONLY GLOBAL_VAR(opt_auto_generate_certs), CMD_LINE(OPT_ARG),
...
static Sys_var_mybool Sys_sync_frm("sync_frm", "Sync .frm files to disk on creation", GLOBAL_VAR(opt_sync_frm),
                                   CMD_LINE(OPT_ARG), DEFAULT(TRUE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
                                   ON_UPDATE(0), DEPRECATED_VAR(""));

static char *system_time_zone_ptr;
static Sys_var_charptr Sys_system_time_zone("system_time_zone", "The server system time zone",
                                            READ_ONLY GLOBAL_VAR(system_time_zone_ptr), NO_CMD_LINE, IN_FS_CHARSET,
...
static Sys_var_ulong Sys_table_def_size("table_definition_cache", "The number of cached table definitions",
                                        GLOBAL_VAR(table_def_size), CMD_LINE(REQUIRED_ARG, OPT_TABLE_DEFINITION_CACHE),
                                        VALID_RANGE(TABLE_DEF_CACHE_MIN, 512 * 1024), DEFAULT(TABLE_DEF_CACHE_DEFAULT),
                                        BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(NULL), ON_UPDATE(NULL),
                                        NULL,
...
static Sys_var_ulong Sys_table_cache_size("table_open_cache",
                                          "The number of cached open tables "
                                          "(total for all table cache instances)",
                                          GLOBAL_VAR(table_cache_size), CMD_LINE(REQUIRED_ARG),
                                          VALID_RANGE(1, 512 * 1024), DEFAULT(TABLE_OPEN_CACHE_DEFAULT), BLOCK_SIZE(1),
                                          NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(NULL),
                                          ON_UPDATE(fix_table_cache_size), NULL,
                                          /* table_open_cache is used as a sizing hint by the performance schema. */
                                          sys_var::PARSE_EARLY);

static Sys_var_ulong Sys_table_cache_instances("table_open_cache_instances", "The number of table cache instances",
...
#ifndef EMBEDDED_LIBRARY
static Sys_var_ulong Sys_thread_cache_size("thread_cache_size", "How many threads we should keep in a cache for reuse",
...
/**
  ...
  @retval   TRUE    Error.
*/
static bool update_transaction_isolation(sys_var *self, THD *thd, enum_var_type type)
{
  SV *sv = type == OPT_GLOBAL ? &global_system_variables : &thd->variables;
...
/**
  ...
  @retval   TRUE    Error.
*/
static bool update_tx_isolation(sys_var *self, THD *thd, enum_var_type type)
{
  SV *sv = type == OPT_GLOBAL ? &global_system_variables : &thd->variables;
...
static Sys_var_tx_isolation Sys_tx_isolation("tx_isolation",
...
// NO_CMD_LINE
static Sys_var_tx_isolation Sys_transaction_isolation("transaction_isolation", "Default transaction isolation level",
  ...
  if (var->type == OPT_DEFAULT || !(thd->in_active_multi_stmt_transaction() || thd->in_sub_stmt))
  {
    ...
    if (thd->variables.session_track_transaction_info > TX_TRACK_NONE)
    {
      ...
      if (var->type == OPT_DEFAULT)
        tst->set_read_flags(thd, thd->tx_read_only ? TX_READ_ONLY : TX_READ_WRITE);
      else
...
/**
  ...
  @retval   FALSE   Success.
*/
static bool update_tx_read_only(sys_var *self, THD *thd, enum_var_type type)
{
  SV *sv = type == OPT_GLOBAL ? &global_system_variables : &thd->variables;
...
/**
  ...
  @retval   FALSE   Success.
*/
static bool update_transaction_read_only(sys_var *self, THD *thd, enum_var_type type)
{
  SV *sv = type == OPT_GLOBAL ? &global_system_variables : &thd->variables;
...
static Sys_var_tx_read_only Sys_tx_read_only("tx_read_only",
...
static Sys_var_tx_read_only Sys_transaction_read_only("transaction_read_only",
                                                      "Set default transaction access mode to read only.",
...
static Sys_var_version Sys_version("version", "Server version", READ_ONLY GLOBAL_VAR(server_version_ptr), NO_CMD_LINE,
                                   IN_SYSTEM_CHARSET, DEFAULT(server_version));

static char *server_version_comment_ptr;
static Sys_var_charptr Sys_version_comment("version_comment", "version_comment",
                                           READ_ONLY GLOBAL_VAR(server_version_comment_ptr), NO_CMD_LINE,
                                           IN_SYSTEM_CHARSET, DEFAULT(MYSQL_COMPILATION_COMMENT));

static char *server_version_compile_machine_ptr;
static Sys_var_charptr Sys_version_compile_machine("version_compile_machine", "version_compile_machine",
                                                   READ_ONLY GLOBAL_VAR(server_version_compile_machine_ptr),
                                                   NO_CMD_LINE, IN_SYSTEM_CHARSET, DEFAULT(MACHINE_TYPE));

static char *server_version_compile_os_ptr;
static Sys_var_charptr Sys_version_compile_os("version_compile_os", "version_compile_os",
...
static Sys_var_enum Sys_internal_tmp_disk_storage_engine("internal_tmp_disk_storage_engine",
                                                         "The default storage engine for on-disk internal tmp table",
                                                         GLOBAL_VAR(internal_tmp_disk_storage_engine),
                                                         CMD_LINE(OPT_ARG), internal_tmp_disk_storage_engine_names,
                                                         DEFAULT(TMP_TABLE_INNODB));
...
*/
static Sys_var_debug_sync Sys_debug_sync("debug_sync", "Debug Sync Facility", sys_var::ONLY_SESSION, NO_CMD_LINE,
...
static Sys_var_charptr Sys_date_format("date_format", "The DATE format (ignored)",
                                       ...
                                       IN_SYSTEM_CHARSET, DEFAULT(known_date_time_formats[ISO_FORMAT].date_format),
                                       NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0), DEPRECATED_VAR(""));
...
static Sys_var_charptr Sys_time_format("time_format", "The TIME format (ignored)",
                                       ...
                                       IN_SYSTEM_CHARSET, DEFAULT(known_date_time_formats[ISO_FORMAT].time_format),
                                       NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0), DEPRECATED_VAR(""));
...
{
  if (!(var->type == OPT_GLOBAL) && (thd->variables.option_bits & OPTION_NOT_AUTOCOMMIT) &&
...
static bool fix_autocommit(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  if (thd->variables.option_bits & OPTION_AUTOCOMMIT && thd->variables.option_bits & OPTION_NOT_AUTOCOMMIT)
...
static Sys_var_bit Sys_autocommit("autocommit", "autocommit", SESSION_VAR(option_bits), NO_CMD_LINE, OPTION_AUTOCOMMIT,
                                  DEFAULT(TRUE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), PRE_UPDATE(pre_autocommit),
...
static Sys_var_bit Sys_big_selects("sql_big_selects", "sql_big_selects", SESSION_VAR(option_bits), NO_CMD_LINE,
                                   OPTION_BIG_SELECTS, DEFAULT(FALSE));

static Sys_var_bit Sys_log_off("sql_log_off", "sql_log_off", SESSION_VAR(option_bits), NO_CMD_LINE, OPTION_LOG_OFF,
...
static Sys_var_sql_log_bin Sys_log_binlog("sql_log_bin", "Controls whether logging to the binary log is done",
                                          SESSION_VAR(sql_log_bin), NO_CMD_LINE, DEFAULT(TRUE), NO_MUTEX_GUARD,
                                          NOT_IN_BINLOG, ON_CHECK(check_sql_log_bin),
                                          ON_UPDATE(fix_sql_log_bin_after_update));

static Sys_var_bit Sys_transaction_allow_batching("transaction_allow_batching", "transaction_allow_batching",
                                                  SESSION_ONLY(option_bits), NO_CMD_LINE, OPTION_ALLOW_BATCH,
                                                  DEFAULT(FALSE));

static Sys_var_bit Sys_sql_warnings("sql_warnings", "sql_warnings", SESSION_VAR(option_bits), NO_CMD_LINE,
                                    OPTION_WARNINGS, DEFAULT(FALSE));

static Sys_var_bit Sys_sql_notes("sql_notes", "sql_notes", SESSION_VAR(option_bits), NO_CMD_LINE, OPTION_SQL_NOTES,
                                 DEFAULT(TRUE));

static Sys_var_bit Sys_auto_is_null("sql_auto_is_null", "sql_auto_is_null", SESSION_VAR(option_bits), NO_CMD_LINE,
                                    OPTION_AUTO_IS_NULL, DEFAULT(FALSE), NO_MUTEX_GUARD, IN_BINLOG);

static Sys_var_bit Sys_safe_updates("sql_safe_updates", "sql_safe_updates", SESSION_VAR(option_bits), NO_CMD_LINE,
                                    OPTION_SAFE_UPDATES, DEFAULT(FALSE));

static Sys_var_bit Sys_buffer_results("sql_buffer_result", "sql_buffer_result", SESSION_VAR(option_bits), NO_CMD_LINE,
                                      OPTION_BUFFER_RESULT, DEFAULT(FALSE));

static Sys_var_bit Sys_quote_show_create("sql_quote_show_create", "sql_quote_show_create", SESSION_VAR(option_bits),
                                         NO_CMD_LINE, OPTION_QUOTE_SHOW_CREATE, DEFAULT(TRUE));

static Sys_var_bit Sys_foreign_key_checks("foreign_key_checks", "foreign_key_checks", SESSION_VAR(option_bits),
                                          NO_CMD_LINE, REVERSE(OPTION_NO_FOREIGN_KEY_CHECKS), DEFAULT(TRUE),
                                          NO_MUTEX_GUARD, IN_BINLOG);

static Sys_var_bit Sys_unique_checks("unique_checks", "unique_checks", SESSION_VAR(option_bits), NO_CMD_LINE,
                                     REVERSE(OPTION_RELAXED_UNIQUE_CHECKS), DEFAULT(TRUE), NO_MUTEX_GUARD, IN_BINLOG);

#ifdef ENABLED_PROFILING
static Sys_var_bit Sys_profiling("profiling", "profiling", SESSION_VAR(option_bits), NO_CMD_LINE, OPTION_PROFILING,
                                 DEFAULT(FALSE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), PRE_UPDATE(0),
                                 ON_UPDATE(0), DEPRECATED_VAR(""));

static Sys_var_ulong Sys_profiling_history_size("profiling_history_size", "Limit of query profiling memory",
...
static bool update_timestamp(THD *thd, set_var *var)
{
  if (var->value)
  {
    ...
    tmp.tv_usec = static_cast<long>(rint((var->save_result.double_value - fl) * 1000000));
    thd->set_time(&tmp);
  }
...
{
  return (double)thd->start_time.tv_sec + (double)thd->start_time.tv_usec / 1000000;
}
...
static Sys_var_session_special_double Sys_timestamp("timestamp", "Set the time for this client", sys_var::ONLY_SESSION,
                                                    NO_CMD_LINE, VALID_RANGE(0, 0), BLOCK_SIZE(1), NO_MUTEX_GUARD,
                                                    IN_BINLOG, ON_CHECK(check_timestamp), ON_UPDATE(update_timestamp),
...
static Sys_var_ulong Sys_group_concat_max_len("group_concat_max_len",
...
static Sys_var_charptr Sys_hostname("hostname", "Server host name", READ_ONLY GLOBAL_VAR(glob_hostname_ptr),
                                    NO_CMD_LINE, IN_FS_CHARSET, DEFAULT(glob_hostname));
...
static Sys_var_charptr Sys_license("license", "The type of license the server has", READ_ONLY GLOBAL_VAR(license),
                                   NO_CMD_LINE, IN_SYSTEM_CHARSET, DEFAULT(STRINGIFY_ARG(LICENSE)));
...
static bool check_log_path(sys_var *self, THD *thd, set_var *var)
{
  ...
  if (!is_valid_log_name(var->save_result.string_value.str, var->save_result.string_value.length))
  {
    my_error(ER_WRONG_VALUE_FOR_VAR, MYF(0), self->name.str, var->save_result.string_value.str);
    return true;
  }
...
static bool fix_general_log_file(sys_var *self, THD *thd, enum_var_type type)
{
  if (!opt_general_logname)  // SET ... = DEFAULT
  {
    char buff[FN_REFLEN];
    opt_general_logname =
        my_strdup(key_memory_LOG_name, make_query_log_name(buff, QUERY_LOG_GENERAL), MYF(MY_FAE + MY_WME));
...
static Sys_var_charptr Sys_general_log_path("general_log_file", "Log connections and queries to given file",
...
static bool fix_slow_log_file(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  {
    char buff[FN_REFLEN];
    opt_slow_logname = my_strdup(key_memory_LOG_name, make_query_log_name(buff, QUERY_LOG_SLOW), MYF(MY_FAE + MY_WME));
    if (!opt_slow_logname)
      return true;
  }
  bool res = false;
...
static Sys_var_charptr Sys_slow_log_path("slow_query_log_file",
...
static Sys_var_have Sys_have_compress("have_compress", "have_compress", READ_ONLY GLOBAL_VAR(have_compress),
                                      NO_CMD_LINE);

static Sys_var_have Sys_have_crypt("have_crypt", "have_crypt", READ_ONLY GLOBAL_VAR(have_crypt), NO_CMD_LINE);

static Sys_var_have Sys_have_dlopen("have_dynamic_loading", "have_dynamic_loading", READ_ONLY GLOBAL_VAR(have_dlopen),
                                    NO_CMD_LINE);

static Sys_var_have Sys_have_geometry("have_geometry", "have_geometry", READ_ONLY GLOBAL_VAR(have_geometry),
                                      NO_CMD_LINE);

static Sys_var_have Sys_have_openssl("have_openssl", "have_openssl", READ_ONLY GLOBAL_VAR(have_ssl), NO_CMD_LINE);

static Sys_var_have Sys_have_profiling("have_profiling", "have_profiling", READ_ONLY GLOBAL_VAR(have_profiling),
                                       NO_CMD_LINE, NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0),
                                       DEPRECATED_VAR(""));

static Sys_var_have Sys_have_query_cache("have_query_cache",
                                         "have_query_cache. "
                                         "This variable is deprecated and will be removed in a future release.",
                                         READ_ONLY GLOBAL_VAR(have_query_cache), NO_CMD_LINE, NO_MUTEX_GUARD,
                                         NOT_IN_BINLOG, ON_CHECK(NULL), ON_UPDATE(NULL), DEPRECATED_VAR(""));

static Sys_var_have Sys_have_rtree_keys("have_rtree_keys", "have_rtree_keys", READ_ONLY GLOBAL_VAR(have_rtree_keys),
                                        NO_CMD_LINE);

static Sys_var_have Sys_have_ssl("have_ssl", "have_ssl", READ_ONLY GLOBAL_VAR(have_ssl), NO_CMD_LINE);

static Sys_var_have Sys_have_symlink("have_symlink", "have_symlink", READ_ONLY GLOBAL_VAR(have_symlink), NO_CMD_LINE);

static Sys_var_have Sys_have_statement_timeout("have_statement_timeout", "have_statement_timeout",
...
static Sys_var_mybool Sys_general_log("general_log",
                                      ...
                                      GLOBAL_VAR(opt_general_log), CMD_LINE(OPT_ARG), DEFAULT(FALSE), NO_MUTEX_GUARD,
                                      NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(fix_general_log_state));
...
static Sys_var_set Sys_log_output("log_output",
                                  ...
                                  DEFAULT(LOG_FILE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_not_empty_set),
...
static Sys_var_charptr Sys_relay_log("relay_log", "The location and name to use for relay logs",
                                     READ_ONLY GLOBAL_VAR(opt_relay_logname), CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET,
                                     DEFAULT(0));
...
*/
static Sys_var_charptr Sys_relay_log_index("relay_log_index",
                                           "The location and name to use for the file "
                                           "that keeps a list of the last relay logs",
                                           READ_ONLY GLOBAL_VAR(relay_log_index), NO_CMD_LINE, IN_FS_CHARSET,
                                           DEFAULT(0));
...
*/
static Sys_var_charptr Sys_binlog_index("log_bin_index", "File that holds the names for last binary log files.",
                                        READ_ONLY GLOBAL_VAR(log_bin_index), NO_CMD_LINE, IN_FS_CHARSET, DEFAULT(0));

static Sys_var_charptr Sys_relay_log_basename("relay_log_basename",
                                              "The full path of the relay log file names, excluding the extension.",
                                              READ_ONLY GLOBAL_VAR(relay_log_basename), NO_CMD_LINE, IN_FS_CHARSET,
                                              DEFAULT(0));

static Sys_var_charptr Sys_log_bin_basename("log_bin_basename",
                                            "The full path of the binary log file names, excluding the extension.",
                                            READ_ONLY GLOBAL_VAR(log_bin_basename), NO_CMD_LINE, IN_FS_CHARSET,
                                            DEFAULT(0));

static Sys_var_charptr Sys_relay_log_info_file("relay_log_info_file",
                                               "The location and name of the file that "
                                               "remembers where the SQL replication thread is in the relay logs",
                                               READ_ONLY GLOBAL_VAR(relay_log_info_file), CMD_LINE(REQUIRED_ARG),
                                               IN_FS_CHARSET, DEFAULT(0));

static Sys_var_mybool Sys_relay_log_purge("relay_log_purge",
...
static Sys_var_mybool Sys_relay_log_recovery("relay_log_recovery",
...
static Sys_var_mybool Sys_slave_allow_batching("slave_allow_batching", "Allow slave to batch requests",
                                               GLOBAL_VAR(opt_slave_allow_batching), CMD_LINE(OPT_ARG), DEFAULT(FALSE));

static Sys_var_charptr Sys_slave_load_tmpdir("slave_load_tmpdir",
...
static bool fix_slave_net_timeout(sys_var *self, THD *thd, enum_var_type type)
{
  ...
  for (mi_map::iterator it = channel_map.begin(); it != channel_map.end(); it++)
  {
    ...
    DBUG_PRINT("info", ("slave_net_timeout=%u mi->heartbeat_period=%.3f", slave_net_timeout,
                        (mi ? mi->heartbeat_period : 0.0)));
    if (mi != NULL && slave_net_timeout < mi->heartbeat_period)
      push_warning(thd, Sql_condition::SL_WARNING, ER_SLAVE_HEARTBEAT_VALUE_OUT_OF_RANGE_MAX,
...
static Sys_var_uint Sys_slave_net_timeout("slave_net_timeout",
  ...
  */
  if (get_gtid_mode(GTID_MODE_LOCK_NONE) == GTID_MODE_ON)
  {
    my_message(ER_SQL_SLAVE_SKIP_COUNTER_NOT_SETTABLE_IN_GTID_MODE,
               ER(ER_SQL_SLAVE_SKIP_COUNTER_NOT_SETTABLE_IN_GTID_MODE), MYF(0));
    return true;
  }
...
static Sys_var_uint Sys_slave_skip_counter("sql_slave_skip_counter", "sql_slave_skip_counter",
...
static Sys_var_uint Sys_sync_relaylog_period("sync_relay_log",
...
static Sys_var_uint Sys_sync_relayloginfo_period("sync_relay_log_info",
...
static Sys_var_uint Sys_checkpoint_mts_period("slave_checkpoint_period",
...
static Sys_var_uint Sys_checkpoint_mts_group("slave_checkpoint_group",
...
static Sys_var_uint Sys_sync_binlog_period("sync_binlog",
...
static Sys_var_uint Sys_sync_masterinfo_period("sync_master_info",
...
static Sys_var_ulong Sys_slave_parallel_workers("slave_parallel_workers",
...
static Sys_var_ulonglong Sys_mts_pending_jobs_size_max("slave_pending_jobs_size_max",
                                                       ...
                                                       DEFAULT(16 * 1024 * 1024), BLOCK_SIZE(1024), ON_CHECK(0));
#endif
...
static Sys_var_struct Sys_lc_messages("lc_messages", "Set the language used for the error messages",
                                      SESSION_VAR(lc_messages), NO_CMD_LINE, my_offsetof(MY_LOCALE, name),
...
static Sys_var_struct Sys_lc_time_names("lc_time_names",
                                        ...
                                        SESSION_VAR(lc_time_names), NO_CMD_LINE, my_offsetof(MY_LOCALE, name),
                                        DEFAULT(&my_default_lc_time_names), NO_MUTEX_GUARD, IN_BINLOG,
                                        ON_CHECK(check_locale));

static Sys_var_tz Sys_time_zone("time_zone", "time_zone", SESSION_VAR(time_zone), NO_CMD_LINE, DEFAULT(&default_tz),
                                NO_MUTEX_GUARD, IN_BINLOG);
...
static Sys_var_uint Sys_host_cache_size("host_cache_size", "How many host names should be cached to avoid resolving.",
                                        ...
                                        NOT_IN_BINLOG, ON_CHECK(NULL), ON_UPDATE(fix_host_cache_size));

static Sys_var_charptr Sys_ignore_db_dirs("ignore_db_dirs",
                                          "The list of directories to ignore when collecting database lists",
                                          READ_ONLY GLOBAL_VAR(opt_ignore_db_dirs), NO_CMD_LINE, IN_FS_CHARSET,
                                          DEFAULT(0));

const Sys_var_multi_enum::ALIAS enforce_gtid_consistency_aliases[] = {{"OFF", 0},   {"ON", 1},   {"WARN", 2},
...
static Sys_var_enforce_gtid_consistency Sys_enforce_gtid_consistency(
    ...
    GLOBAL_VAR(_gtid_consistency_mode), CMD_LINE(OPT_ARG, OPT_ENFORCE_GTID_CONSISTENCY),
    enforce_gtid_consistency_aliases, 3, DEFAULT(3 /*position of "FALSE" in enforce_gtid_consistency_aliases*/),
...
static Sys_var_mybool Sys_binlog_gtid_simple_recovery("binlog_gtid_simple_recovery",
...
static bool check_pseudo_slave_mode(sys_var *self, THD *thd, set_var *var)
{
  ...
  if (rli_fake)
  {
    if (!val)
    {
      ...
      push_warning(thd, Sql_condition::SL_WARNING, ER_WRONG_VALUE_FOR_VAR, "'pseudo_slave_mode' is already ON.");
  }
  else
  {
    if (!previous_val && !val)
...
ineffective:
  push_warning(thd, Sql_condition::SL_WARNING, ER_WRONG_VALUE_FOR_VAR, "'pseudo_slave_mode' change was ineffective.");
...
static bool check_gtid_next_list(sys_var *self, THD *thd, set_var *var)
{
  ...
  */
  if (get_gtid_mode(GTID_MODE_LOCK_NONE) == GTID_MODE_OFF && var->save_result.string_value.str != NULL)
    my_error(ER_CANT_SET_GTID_NEXT_LIST_TO_NON_NULL_WHEN_GTID_MODE_IS_OFF, MYF(0));
  DBUG_RETURN(false);
}
...
static Sys_var_gtid_set Sys_gtid_next_list("gtid_next_list",
...
static Sys_var_gtid_next Sys_gtid_next("gtid_next",
...
static Sys_var_gtid_executed Sys_gtid_executed("gtid_executed",
...
static bool check_gtid_purged(sys_var *self, THD *thd, set_var *var)
{
  ...
  if (!var->value || check_super_outside_trx_outside_sf_outside_sp(self, thd, var))
    DBUG_RETURN(true);

  if (var->value->result_type() != STRING_RESULT || !var->save_result.string_value.str)
    DBUG_RETURN(true);
...
bool Sys_var_gtid_purged::global_update(THD *thd, set_var *var)
{
  ...
  */
  thd->set_skip_readonly_check();
  char *previous_gtid_executed = NULL, *previous_gtid_purged = NULL, *current_gtid_executed = NULL,
       *current_gtid_purged = NULL;
  ...
  Gtid_set gtid_set(global_sid_map, var->save_result.string_value.str, &ret, global_sid_lock);
  ...
  sql_print_information(ER(ER_GTID_PURGED_WAS_CHANGED), previous_gtid_purged, current_gtid_purged);
  sql_print_information(ER(ER_GTID_EXECUTED_WAS_CHANGED), previous_gtid_executed, current_gtid_executed);
...
static Sys_var_gtid_purged Sys_gtid_purged("gtid_purged",
...
static Sys_var_uint Sys_gtid_executed_compression_period(
    ...
    GLOBAL_VAR(gtid_executed_compression_period), CMD_LINE(OPT_ARG), VALID_RANGE(0, UINT_MAX32), DEFAULT(1000),
    BLOCK_SIZE(1));
...
static Sys_var_mybool Sys_disconnect_on_expired_password(
    ...
    READ_ONLY GLOBAL_VAR(disconnect_on_expired_password), CMD_LINE(OPT_ARG), DEFAULT(TRUE));

#ifndef NO_EMBEDDED_ACCESS_CHECKS
static Sys_var_mybool Sys_validate_user_plugins("validate_user_plugins",
...
static Sys_var_enum Sys_block_encryption_mode("block_encryption_mode", "mode for AES_ENCRYPT/AES_DECRYPT",
...
static bool update_track_session_sys_vars(sys_var *self, THD *thd, enum_var_type type)
{
  DBUG_ENTER("check_sysvar_change_reporter");
...
static Sys_var_charptr Sys_track_session_sys_vars(
    "session_track_system_variables", "Track changes in registered system variables.", SESSION_VAR(track_sysvars_ptr),
    CMD_LINE(REQUIRED_ARG), IN_FS_CHARSET,
    DEFAULT("time_zone,autocommit,character_set_client,character_set_results,"
            "character_set_connection"),
    NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(check_track_session_sys_vars), ON_UPDATE(update_track_session_sys_vars));
...
static Sys_var_enum Sys_session_track_transaction_info(
    ...
    DEFAULT(OFF), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(update_session_track_tx_info));
...
static Sys_var_mybool Sys_offline_mode("offline_mode", "Make the server into offline mode", GLOBAL_VAR(offline_mode),
                                       CMD_LINE(OPT_ARG), DEFAULT(FALSE), &PLock_offline_mode, NOT_IN_BINLOG,
...
static Sys_var_mybool Sys_log_builtin_as_identified_by_password(
    ...
    GLOBAL_VAR(opt_log_builtin_as_identified_by_password), CMD_LINE(OPT_ARG), DEFAULT(FALSE));
...
static Sys_var_mybool Sys_avoid_temporal_upgrade(
    ...
    "This variable is deprecated and will be removed in a future release.",
    GLOBAL_VAR(avoid_temporal_upgrade), CMD_LINE(OPT_ARG, OPT_AVOID_TEMPORAL_UPGRADE), DEFAULT(FALSE), NO_MUTEX_GUARD,
    NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(0), DEPRECATED_VAR(""));

static Sys_var_mybool Sys_show_old_temporals("show_old_temporals",
...
static Sys_var_charptr Sys_disabled_storage_engines("disabled_storage_engines",
...
static Sys_var_mybool Sys_show_create_table_verbosity("show_create_table_verbosity",
                                                      "When this option is enabled, it increases the verbosity of "
                                                      "'SHOW CREATE TABLE'.",
                                                      SESSION_VAR(show_create_table_verbosity), CMD_LINE(OPT_ARG),
                                                      DEFAULT(FALSE), NO_MUTEX_GUARD, NOT_IN_BINLOG, ON_CHECK(0),
                                                      ON_UPDATE(0));
...
static bool check_keyring_access(sys_var *, THD *thd, set_var *)
{
  if (!(thd->security_context()->check_access(SUPER_ACL)))
  {
    my_error(ER_KEYRING_ACCESS_DENIED_ERROR, MYF(0), "SUPER");
    return true;
  }
  return false;
}
...
static Sys_var_mybool Sys_keyring_operations(
    ...
    GLOBAL_VAR(opt_keyring_operations), NO_CMD_LINE, DEFAULT(TRUE), &PLock_keyring_operations, NOT_IN_BINLOG,
    ON_CHECK(check_keyring_access), ON_UPDATE(0));
...
static Sys_var_mybool Sys_replication_optimize_for_static_plugin_config(
    ...
    GLOBAL_VAR(opt_replication_optimize_for_static_plugin_config), CMD_LINE(OPT_ARG), DEFAULT(FALSE), NO_MUTEX_GUARD,
    NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(handle_plugin_lock_type_change));
...
static bool handle_sender_observe_commit_change(sys_var *, THD *, enum_var_type)
{
  ...
  my_atomic_store32(&opt_atomic_replication_sender_observe_commit_only, opt_replication_sender_observe_commit_only);
  DBUG_RETURN(false);
}
...
static Sys_var_mybool Sys_replication_sender_observe_commit_only(
    ...
    "commit.",
    GLOBAL_VAR(opt_replication_sender_observe_commit_only), CMD_LINE(OPT_ARG), DEFAULT(FALSE), NO_MUTEX_GUARD,
    NOT_IN_BINLOG, ON_CHECK(0), ON_UPDATE(handle_sender_observe_commit_change));
