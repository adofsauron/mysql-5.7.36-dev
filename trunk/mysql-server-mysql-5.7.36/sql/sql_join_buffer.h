...
class JOIN_CACHE : public QEP_operation
{
  ...
  */
  ...
  ulong get_rec_length(uchar *ptr) { return get_offset(size_of_rec_len, ptr); }
  ulong get_fld_offset(uchar *ptr) { return get_offset(size_of_fld_ofs, ptr); }
  ...
  */
  ...
  bool is_key_access() const { return cache_type() & (ALG_BKA | ALG_BKA_UNIQUE); }
...
class JOIN_CACHE_BNL : public JOIN_CACHE
{
 ...
 public:
  JOIN_CACHE_BNL(JOIN *j, QEP_TAB *qep_tab_arg, JOIN_CACHE *prev) : JOIN_CACHE(j, qep_tab_arg, prev), const_cond(NULL)
  {
  }
