...
static void debug_sync_C_callback(const char *sync_point_name, size_t name_len)
{
  if (unlikely(opt_debug_sync_timeout))
...
static PSI_memory_info all_debug_sync_memory[] = {{&key_debug_THD_debug_sync_control, "THD::debug_sync_control", 0},
...
int debug_sync_init(void)
{
  ...
  if (opt_debug_sync_timeout)
  {
    ...
    if ((rc = mysql_cond_init(key_debug_sync_globals_ds_cond, &debug_sync_global.ds_cond)) ||
...
void debug_sync_end(void)
{
  ...
  if (debug_sync_C_callback_ptr)
  {
    ...
    {
      char llbuff[22];
      sql_print_information("Debug sync points hit:                   %22s", llstr(debug_sync_global.dsp_hits, llbuff));
...
void debug_sync_init_thread(THD *thd)
{
  ...
  if (opt_debug_sync_timeout)
  {
    thd->debug_sync_control = (st_debug_sync_control *)my_malloc(
        key_debug_THD_debug_sync_control, sizeof(st_debug_sync_control), MYF(MY_WME | MY_ZEROFILL));
...
static void debug_sync_action_string(char *result, uint size, st_debug_sync_action *action)
{
  ...
  assert(!action->execute || action->signal.length() || action->wait_for.length());
  ...
  */
  if (ds_control->ds_active > dsp_idx)
  {
    ...
    */
    ...
    memmove(dest, src, (ds_control->ds_active - dsp_idx) * sizeof(st_debug_sync_action));
    ...
    */
    ...
    memmove(dest, save_action, sizeof(st_debug_sync_action));
  }
...
static st_debug_sync_action *debug_sync_get_action(THD *thd, const char *dsp_name, size_t name_len)
{
  st_debug_sync_control *ds_control = thd->debug_sync_control;
  st_debug_sync_action *action;
  ...
  else
  {
    ...
    if (ds_control->ds_active > ds_control->ds_allocated)
    {
      ...
      void *new_action = my_realloc(key_debug_sync_action, ds_control->ds_action,
      ...
      void *dest = (ds_control->ds_action + dsp_idx);
      memset(dest, 0, (new_alloc - dsp_idx) * sizeof(st_debug_sync_action));
    }
  ...
  DBUG_PRINT("debug_sync", ("action: 0x%lx  array: 0x%lx  count: %u", (long)action, (long)ds_control->ds_action,
                            ds_control->ds_active));
...
bool debug_sync_update(THD *thd, char *val_str)
{
  ...
  */
  DBUG_RETURN(opt_debug_sync_timeout ? debug_sync_eval_action(thd, val_str) : FALSE);
}
...
uchar *debug_sync_value_ptr(THD *thd)
{
  ...
  if (opt_debug_sync_timeout)
  {
    ...
    for (iter = debug_sync_global.ds_signal_set.begin(); iter != debug_sync_global.ds_signal_set.end();)
    {
...
{
  return (debug_sync_global.ds_signal_set.find(*signal_name) != debug_sync_global.ds_signal_set.end());
}
  ...
  if (action->execute)
  {
    ...
    */
    ...
    if (action->signal.length())
    {
      ...
      for (std::vector<std::string>::const_iterator it = signals.begin(); it != signals.end(); ++it)
      {
    ...
    if (action->wait_for.length())
    {
      ...
      */
      ...
      DBUG_EXECUTE("debug_sync_exec",
                   { DBUG_PRINT("debug_sync_exec", ("wait for '%s'  at: '%s'", sig_wait, dsp_name)); });
      /*
      ...
      */
      while (!is_signalled(&wait_for) && !thd->killed && opt_debug_sync_timeout)
      {
        error = mysql_cond_timedwait(&debug_sync_global.ds_cond, &debug_sync_global.ds_mutex, &abstime);

        DBUG_EXECUTE("debug_sync", {
          /* Functions as DBUG_PRINT args can change keyword and line nr. */
          DBUG_PRINT("debug_sync", ("awoke from %s error: %d", sig_wait, error));
        });

        if (error == ETIMEDOUT || error == ETIME)
        {
          // We should not make the statement fail, even if in strict mode.
          push_warning(thd, Sql_condition::SL_WARNING, ER_DEBUG_SYNC_TIMEOUT, ER(ER_DEBUG_SYNC_TIMEOUT));
