...
static PSI_mutex_info all_slave_list_mutexes[] = {{&key_LOCK_slave_list, "LOCK_slave_list", PSI_FLAG_GLOBAL}};
...
void init_slave_list()
{
  ...
  my_hash_init(&slave_list, system_charset_info, SLAVE_LIST_CHUNK, 0, 0, (my_hash_get_key)slave_list_key,
               (my_hash_free_key)slave_info_free, 0, key_memory_SLAVE_INFO);
...
void unregister_slave(THD *thd, bool only_mine, bool need_lock_slave_list)
{
  if (thd->server_id && my_hash_inited(&slave_list))
  {
    ...
    SLAVE_INFO *old_si;
    if ((old_si = (SLAVE_INFO *)my_hash_search(&slave_list, (uchar *)&thd->server_id, 4)) &&
...
bool com_binlog_dump(THD *thd, char *packet, size_t packet_length)
{
  ...
  */
  ...
  query_logger.general_log_print(thd, thd->get_command(), "Log: '%s'  Pos: %ld", packet + 10, (long)pos);
...
bool com_binlog_dump_gtid(THD *thd, char *packet, size_t packet_length)
{
  ...
  */
  ...
  if (slave_gtid_executed.add_gtid_encoding(packet_position, data_size) != RETURN_STATUS_OK)
    DBUG_RETURN(true);
  ...
  kill_zombie_dump_threads(thd);
  query_logger.general_log_print(thd, thd->get_command(), "Log: '%s' Pos: %llu GTIDs: '%s'", name, pos, gtid_string);
  my_free(gtid_string);
...
class Find_zombie_dump_thread : public Find_THD_Impl
{
 public:
  ...
  virtual bool operator()(THD *thd)
  {
    ...
    if (thd != cur_thd && (thd->get_command() == COM_BINLOG_DUMP || thd->get_command() == COM_BINLOG_DUMP_GTID))
    {
      String tmp_uuid;
      ...
      {
        is_zombie_thread = (tmp_uuid.length() && !strncmp(m_slave_uuid.c_ptr(), tmp_uuid.c_ptr(), UUID_LENGTH));
      }
      else
      {
        /*
        ...
        */
        is_zombie_thread = ((thd->server_id == cur_thd->server_id) && !tmp_uuid.length());
      }
      if (is_zombie_thread)
      {
...
void kill_zombie_dump_threads(THD *thd)
{
  ...
  THD *tmp = Global_THD_manager::get_instance()->find_thd(&find_zombie_dump_thread);
  if (tmp)
  {
    /*
...
bool show_master_status(THD *thd)
{
  ...
  field_list.push_back(new Item_empty_string("Executed_Gtid_Set", gtid_set_size));
...
bool show_binlogs(THD *thd)
{
  ...
  while ((length = my_b_gets(index_file, fname, sizeof(fname))) > 1)
  {
    ...
    else
    {
      ...
      if ((file = mysql_file_open(key_file_binlog, fname, O_RDONLY | O_SHARE | O_BINARY, MYF(0))) >= 0)
      {
