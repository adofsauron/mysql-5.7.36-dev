...
Field *create_tmp_field_from_field(THD *thd, Field *org_field, const char *name, TABLE *table, Item_field *item)
{
  ...
  new_field = org_field->new_field(thd->mem_root, table, table == org_field->table);
  if (new_field)
  {
...
static Field *create_tmp_field_from_item(THD *thd, Item *item, TABLE *table, Func_ptr_array *copy_func,
                                         bool modify_item)
{
  ...
  {
    case REAL_RESULT:
      new_field = new Field_double(item->max_length, maybe_null, item->item_name.ptr(), item->decimals, TRUE);
      break;
    case INT_RESULT:
      /*
      ...
      */
      if (item->max_length >= (MY_INT32_NUM_DECIMAL_DIGITS - 1))
        new_field = new Field_longlong(item->max_length, maybe_null, item->item_name.ptr(), item->unsigned_flag);
      else
        new_field = new Field_long(item->max_length, maybe_null, item->item_name.ptr(), item->unsigned_flag);
      break;
    case STRING_RESULT:
      ...
      */
      if (item->is_temporal() || item->field_type() == MYSQL_TYPE_GEOMETRY || item->field_type() == MYSQL_TYPE_JSON)
      {
...
static Field *create_tmp_field_for_schema(THD *thd, Item *item, TABLE *table)
{
  if (item->field_type() == MYSQL_TYPE_VARCHAR)
  {
    ...
    if (item->max_length > MAX_FIELD_VARCHARLENGTH)
      field =
          new Field_blob(item->max_length, item->maybe_null, item->item_name.ptr(), item->collation.collation, false);
    else
    {
      field = new Field_varstring(item->max_length, item->maybe_null, item->item_name.ptr(), table->s,
...
Field *create_tmp_field(THD *thd, TABLE *table, Item *item, Item::Type type, Func_ptr_array *copy_func,
                        Field **from_field, Field **default_field, bool group, bool modify_item,
  ...
  switch (type)
  {
    ...
    case Item::TRIGGER_FIELD_ITEM:
    {
      ...
      */
      ...
      else if (table_cant_handle_bit_fields && field->field->type() == MYSQL_TYPE_BIT)
      {
        *from_field = field->field;
        result = create_tmp_field_from_item(thd, item, table, copy_func, modify_item);
        if (!result)
          break;
        if (modify_item)
          field->result_field = result;
      ...
      else
      {
        result = create_tmp_field_from_field(thd, (*from_field = field->field),
                                             orig_item ? orig_item->item_name.ptr() : item->item_name.ptr(), table,
                                             modify_item ? field : NULL);
        if (!result)
          break;
      }
    ...
    case Item::FUNC_ITEM:
      if (((Item_func *)item)->functype() == Item_func::FUNC_SP)
      {
        ...
        result = create_tmp_field_from_field(thd, sp_result_field, item_func_sp->item_name.ptr(), table, NULL);
        if (!result)
          break;
        if (modify_item)
    ...
    case Item::PARAM_ITEM:
      ...
      result = create_tmp_field_from_item(thd, item, table, (make_copy_field ? NULL : copy_func), modify_item);
      break;
    case Item::TYPE_HOLDER:
      result = ((Item_type_holder *)item)->make_field_by_type(table, thd->is_strict_mode());
      if (!result)
        break;
...
static void setup_tmp_table_column_bitmaps(TABLE *table, uchar *bitmaps)
{
  ...
  bitmap_init(&table->def_read_set, (my_bitmap_map *)bitmaps, field_count, FALSE);
  bitmap_init(&table->tmp_set, (my_bitmap_map *)(bitmaps + bitmap_buffer_size(field_count)), field_count, FALSE);
  bitmap_init(&table->cond_set, (my_bitmap_map *)(bitmaps + bitmap_buffer_size(field_count) * 2), field_count, FALSE);
...
void get_max_key_and_part_length(uint *max_key_length, uint *max_key_part_length)
{
  ...
  switch (internal_tmp_disk_storage_engine)
  {
    case TMP_TABLE_MYISAM:
      *max_key_length = std::min(Cache_temp_engine_properties::HEAP_MAX_KEY_LENGTH,
                                 Cache_temp_engine_properties::MYISAM_MAX_KEY_LENGTH);
      *max_key_part_length = std::min(Cache_temp_engine_properties::HEAP_MAX_KEY_PART_LENGTH,
      ...
      */
      assert(Cache_temp_engine_properties::HEAP_MAX_KEY_PARTS <= Cache_temp_engine_properties::MYISAM_MAX_KEY_PARTS);
      break;
    ...
    default:
      ...
      *max_key_part_length = std::min(Cache_temp_engine_properties::HEAP_MAX_KEY_PART_LENGTH,
      ...
      assert(Cache_temp_engine_properties::HEAP_MAX_KEY_PARTS <= Cache_temp_engine_properties::INNODB_MAX_KEY_PARTS);
      break;
  }
}
...
static void register_hidden_field(TABLE *table, Field **default_field, Field **from_field, uint *blob_field,
                                  Field *field)
{
  uint i;
...
TABLE *create_tmp_table(THD *thd, Temp_table_param *param, List<Item> &fields, ORDER *group, bool distinct,
                        bool save_sum_fields, ulonglong select_options, ha_rows rows_limit, const char *table_alias)
{
  ...
  */
  ...
  DBUG_PRINT("enter", ("distinct: %d  save_sum_fields: %d  rows_limit: %lu  group: %d", (int)distinct,
                       (int)save_sum_fields, (ulong)rows_limit, MY_TEST(group)));
  ...
  else
  {
    ...
    sprintf(path, "%s%lx_%x_%x", tmp_file_prefix, current_pid, thd->thread_id(), thd->tmp_table++);
  }
  ...
  if (group)
  {
    if (!param->quick_group)
      ...
      for (ORDER *tmp = group; tmp; tmp = tmp->next)
      {
        ...
        */
        (*tmp->item)->marker = 4;
        const uint char_len = (*tmp->item)->max_length / (*tmp->item)->collation.collation->mbmaxlen;
        /*
        ...
        */
        if (char_len > CONVERT_IF_BIGGER_TO_BLOB || (*tmp->item)->is_non_deterministic())
        {
  ...
  */
  ...
  if (!multi_alloc_root(&own_root, &table, sizeof(*table), &share, sizeof(*share), &reg_field,
                        sizeof(Field *) * (field_count + 2), &default_field, sizeof(Field *) * (field_count + 1),
                        &blob_field, sizeof(uint) * (field_count + 2), &from_field, sizeof(Field *) * (field_count + 1),
                        &param->keyinfo, sizeof(*param->keyinfo), &key_part_info,
                        sizeof(*key_part_info) * (param->group_parts + 1), &param->start_recinfo,
                        sizeof(*param->recinfo) * (field_count * 2 + 4), &tmpname, strlen(path) + 1, &group_buff,
                        (group && !using_unique_constraint ? param->group_length : 0), &bitmaps,
                        bitmap_buffer_size(field_count + 1) * 3, NullS))
  {
  ...
  while ((item = li++))
  {
    ...
    if (not_all_columns)
    {
      if (item->with_sum_func && type != Item::SUM_FUNC_ITEM)
      {
        ...
        if (type == Item::SUBSELECT_ITEM || (item->used_tables() & ~OUTER_REF_TABLE_BIT))
        {
          /*
    ...
    { /* Can't calc group yet */
      ...
      for (i = 0; i < sum_item->get_arg_count(); i++)
      {
        ...
        if (!arg->const_item())
        {
          new_field = create_tmp_field(thd, table, arg, arg->type(), copy_func, tmp_from_field, &default_field[fieldnr],
    ...
    else
    {
      ...
      */
      new_field = (param->schema_table)
                      ...
                      : create_tmp_field(thd, table, item, type, copy_func, tmp_from_field, &default_field[fieldnr],
                                         group != 0, !force_copy_fields && (not_all_columns || group != 0),
                                         /*
                                         ...
                                         */
                                         item->marker == 4 || param->bit_fields_as_long, force_copy_fields);
      ...
      if (new_field->type() == MYSQL_TYPE_VAR_STRING || new_field->type() == MYSQL_TYPE_VARCHAR)
        table->s->db_create_options |= HA_OPTION_PACK_RECORD;
  ...
  update_hidden:
    ...
    */
    if (distinct && !using_unique_constraint && hidden_field_count <= 0 && new_field)
    {
      if (new_field->flags & BLOB_FLAG)
        using_unique_constraint = true;
      else
  ...
  *blob_field = 0;  // End marker
  ...
  else if (blob_count || (thd->variables.big_tables && !(select_options & SELECT_SMALL_RESULT)))
  {
    /*
  ...
  if (!table->file)
    goto err;
  if (group && (param->group_parts > table->file->max_key_parts() || param->group_length > max_key_length))
    using_unique_constraint = true;
    ...
    */
    ...
    if (!using_unique_constraint)
    {
      ...
      if (!(key_part_info =
        ...
        goto err;
      memset(key_part_info, 0, keyinfo->user_defined_key_parts * sizeof(KEY_PART_INFO));
      table->key_info = share->key_info = keyinfo;
      keyinfo->key_part = key_part_info;
  ...
  */
  ...
  {
    using_unique_constraint = true;
    Field_longlong *field = new (&table->mem_root) Field_longlong(sizeof(ulonglong), false, "<hash_field>", true);
    if (!field)
    {
      assert(thd->is_fatal_error);
      goto err;  // Got OOM
    ...
    register_hidden_field(table, default_field, from_field, share->blob_field, field);
    // Repoint arrays
    table->field--;
    default_field--;
    from_field--;
  ...
  hidden_null_pack_length = (hidden_null_count + 7 + hidden_uneven_bit_length) / 8;
  ...
  for (i = 0, reg_field = table->field; i < field_count; i++, reg_field++, recinfo++)
  {
    ...
    if (!(field->flags & NOT_NULL_FLAG))
    {
      ...
      field->move_field(pos, null_flags + null_count / 8, (uint8)1 << (null_count & 7));
      null_count++;
    }
    else
    ...
    */
    ...
    else if (use_packed_rows && field->real_type() == MYSQL_TYPE_VARCHAR && length >= MIN_STRING_LENGTH_TO_PACK_ROWS)
  ...
  */
  ...
  if (group && !using_unique_constraint)
  {
    ...
    */
    for (; cur_group; cur_group = cur_group->next, key_part_info++)
    {
      ...
      cur_group->field = field->new_key_field(thd->mem_root, table, group_buff + MY_TEST(maybe_null));
  ...
  if (distinct && field_count != param->hidden_field_count && !using_unique_constraint)
  {
  ...
  if (using_unique_constraint)
  {
    ...
    if (!multi_alloc_root(&table->mem_root, &hash_key, sizeof(*hash_key), &hash_kpi,
                          sizeof(*hash_kpi),  // Only one key part
                          NullS))
      goto err;
  ...
  {
    if (instantiate_tmp_table(table, param->keyinfo, param->start_recinfo, &param->recinfo, select_options,
...
TABLE *create_duplicate_weedout_tmp_table(THD *thd, uint uniq_tuple_length_arg, SJ_TMP_TABLE *sjtbl)
{
  ...
  */
  ...
  init_sql_alloc(key_memory_TABLE, &own_root, TABLE_ALLOC_BLOCK_SIZE, 0);
  if (!multi_alloc_root(&own_root, &table, sizeof(*table), &share, sizeof(*share), &reg_field,
                        sizeof(Field *) * (1 + 2), &blob_field, sizeof(uint) * 3, &keyinfo, sizeof(*keyinfo),
                        &key_part_info, sizeof(*key_part_info) * 2, &start_recinfo, sizeof(*recinfo) * (1 * 2 + 2),
                        &tmpname, strlen(path) + 1, &group_buff, (!using_unique_constraint ? uniq_tuple_length_arg : 0),
  ...
  }
  {
    ...
    */
    field = new Field_varstring(uniq_tuple_length_arg, FALSE, "rowids", share, &my_charset_bin);
    if (!field)
      DBUG_RETURN(0);
    field->table = table;
  ...
  null_count = 1;
  for (i = 0, reg_field = table->field; i < share->fields; i++, reg_field++, recinfo++)
  {
    Field *field = *reg_field;
    uint length;
  ...
  if (thd->is_fatal_error)  // If end of memory
    goto err;
  share->db_record_offset = 1;
  if (instantiate_tmp_table(table, table->key_info, start_recinfo, &recinfo, 0, 0, &thd->opt_trace))
    goto err;
...
TABLE *create_virtual_tmp_table(THD *thd, List<Create_field> &field_list)
{
  ...
  if (!multi_alloc_root(thd->mem_root, &table, sizeof(*table), &share, sizeof(*share), &field,
                        (field_count + 1) * sizeof(Field *), &blob_field, (field_count + 1) * sizeof(uint), &bitmaps,
  ...
  while ((cdef = it++))
  {
    *field = make_field(share, 0, cdef->length, (uchar *)(f_maybe_null(cdef->pack_flag) ? "" : 0),
                        f_maybe_null(cdef->pack_flag) ? 1 : 0, cdef->pack_flag, cdef->sql_type, cdef->charset,
                        cdef->geom_type, cdef->unireg_check, cdef->interval, cdef->field_name);
    if (!*field)
      goto error;
    (*field)->init(table);
...
bool open_tmp_table(TABLE *table)
{
  int error;
  if ((error =
           table->file->ha_open(table, table->s->table_name.str, O_RDWR, HA_OPEN_TMP_TABLE | HA_OPEN_INTERNAL_TABLE)))
  {
...
bool create_myisam_tmp_table(TABLE *table, KEY *keyinfo, MI_COLUMNDEF *start_recinfo, MI_COLUMNDEF **recinfo,
                             ulonglong options, my_bool big_tables)
{
  ...
  if ((error = mi_create(share->table_name.str, share->keys, &keydef, (uint)(*recinfo - start_recinfo), start_recinfo,
                         0, &uniquedef, &create_info,
...
bool create_innodb_tmp_table(TABLE *table, KEY *keyinfo)
{
  ...
  create_info.options |= HA_LEX_CREATE_TMP_TABLE | HA_LEX_CREATE_INTERNAL_TMP_TABLE;
  /*
  ...
  */
  if (table->s->db_type() == innodb_hton)
  {
    ...
    {
      if ((*field)->type() == MYSQL_TYPE_STRING && (*field)->key_length() > 1024)
      {
  ...
  if ((error = table->file->create(share->table_name.str, table, &create_info)))
  {
    ...
    if (error == HA_ERR_FOUND_DUPP_KEY || error == HA_ERR_TABLESPACE_EXISTS || error == HA_ERR_TABLE_EXIST)
      table->temp_pool_slot = MY_BIT_NONE;
    table->db_stat = 0;
    DBUG_RETURN(true);
  }
...
static void trace_tmp_table(Opt_trace_context *trace, const TABLE *table)
{
  ...
  trace_tmp.add("row_length", table->s->reclength)
      .add("key_length", table->s->key_info ? table->s->key_info->key_length : 0)
  ...
  else
  {
    ...
    trace_tmp.add_alnum("location", "memory (heap)").add("row_limit_estimate", table->s->max_rows);
  }
}
...
bool instantiate_tmp_table(TABLE *table, KEY *keyinfo, MI_COLUMNDEF *start_recinfo, MI_COLUMNDEF **recinfo,
                           ulonglong options, my_bool big_tables, Opt_trace_context *trace)
{
#ifndef NDEBUG
  ...
  {
    if (create_myisam_tmp_table(table, keyinfo, start_recinfo, recinfo, options, big_tables))
      return TRUE;
    // Make empty record so random data is not written to disk
    empty_record(table);
  }
...
bool create_ondisk_from_heap(THD *thd, TABLE *table, MI_COLUMNDEF *start_recinfo, MI_COLUMNDEF **recinfo, int error,
                             bool ignore_last_dup, bool *is_duplicate)
{
  ...
  if (!(new_table.file = get_new_handler(&share, &new_table.mem_root, new_table.s->db_type())))
  ...
  {
    if (create_myisam_tmp_table(&new_table, table->s->key_info, start_recinfo, recinfo,
                                thd->lex->select_lex->active_options(), thd->variables.big_tables))
      goto err2;
  }
  ...
  */
  ...
  {
    if (!new_table.file->is_ignorable_error(write_err) || !ignore_last_dup)
      goto err;
    if (is_duplicate)
      *is_duplicate = TRUE;
  }
