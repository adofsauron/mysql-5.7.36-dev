...
*/
extern "C" int mysql_string_convert_to_char_ptr(mysql_string_handle string_handle, const char *charset_name,
                                                char *buffer, unsigned int buffer_size, int *error)
{
  String *str = (String *)string_handle;
  int len = (int)my_convert(buffer, buffer_size - 1, &my_charset_utf8_general_ci, str->ptr(), str->length(),
                            str->charset(), (uint *)error);
  buffer[len] = '\0';
  return (len);
}
...
*/
extern "C" void mysql_string_free(mysql_string_handle string_handle)
{
  String *str = (String *)string_handle;
  str->mem_free();
  delete[] str;
...
*/
extern "C" void mysql_string_iterator_free(mysql_string_iterator_handle iterator_handle)
{
  my_free((string_iterator *)iterator_handle);
}

/* This service function allocate mysql_string_iterator_handle and return it */
extern "C" mysql_string_iterator_handle mysql_string_get_iterator(mysql_string_handle string_handle)
{
  String *str = (String *)string_handle;
  string_iterator *iterator =
      (string_iterator *)my_malloc(key_memory_string_iterator, sizeof(struct st_string_iterator), MYF(0));
...
extern "C" int mysql_string_iterator_next(mysql_string_iterator_handle iterator_handle)
{
  ...
  if (iterator->iterator_ptr >= (const char *)end)
    return (0);
  char_len = (cs->cset->ctype(cs, &char_type, (uchar *)iterator->iterator_ptr, (uchar *)end));
...
*/
extern "C" int mysql_string_iterator_isupper(mysql_string_iterator_handle iterator_handle)
{
  string_iterator *iterator = (string_iterator *)iterator_handle;
...
*/
extern "C" int mysql_string_iterator_islower(mysql_string_iterator_handle iterator_handle)
{
  string_iterator *iterator = (string_iterator *)iterator_handle;
...
/*
  ...
  digit or not
*/
extern "C" int mysql_string_iterator_isdigit(mysql_string_iterator_handle iterator_handle)
{
  string_iterator *iterator = (string_iterator *)iterator_handle;
...
*/
extern "C" mysql_string_handle mysql_string_to_lowercase(mysql_string_handle string_handle)
{
  String *str = (String *)string_handle;
