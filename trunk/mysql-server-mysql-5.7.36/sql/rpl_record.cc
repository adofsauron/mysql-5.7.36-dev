...
size_t pack_row(TABLE *table, MY_BITMAP const *cols, uchar *row_data, const uchar *record)
{
  ...
  DBUG_PRINT("debug", ("null ptr: 0x%lx; row start: %p; null bytes: %d", (ulong)null_ptr, row_data, null_byte_count));
  ...
  for (; (field = *p_field); p_field++)
  {
    if (bitmap_is_set(cols, p_field - table->field))
    {
      ...
      {
        DBUG_PRINT("debug", ("Is NULL; null_mask: 0x%x; null_bits: 0x%x", null_mask, null_bits));
        offset = def_offset;
        null_bits |= null_mask;
      }
      else
...
#endif
        pack_ptr = field->pack(pack_ptr, field->ptr + offset, field->max_data_length(), TRUE);
        DBUG_PRINT("debug", ("field: %s; real_type: %d, pack_ptr: 0x%lx;"
                             " pack_ptr':0x%lx; bytes: %d",
                             field->field_name, field->real_type(), (ulong)old_pack_ptr, (ulong)pack_ptr,
...
int unpack_row(Relay_log_info const *rli, TABLE *table, uint const colcnt, uchar const *const row_data,
               MY_BITMAP const *cols, uchar const **const current_row_end, ulong *const master_reclength,
               uchar const *const row_end)
{
  ...
  for (field_ptr = begin_ptr; field_ptr < end_ptr && *field_ptr; ++field_ptr)
  {
    ...
    DBUG_PRINT("debug", ("field: %s; null mask: 0x%x; null bits: 0x%lx;"
                         ...
                         f->field_name, null_mask, (ulong)null_bits, pack_ptr, (ulong)master_null_byte_count));
    ...
    if (bitmap_is_set(cols, field_ptr - begin_ptr))
    {
      ...
      if (null_bits & null_mask)
      {
        ...
        {
          f->set_default();
          push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_BAD_NULL_ERROR, ER(ER_BAD_NULL_ERROR),
                              f->field_name);
        }
      }
      else
      {
        ...
        */
        ...
        DBUG_PRINT("debug", ("Unpacked; metadata: 0x%x;"
                             ...
                             metadata, (ulong)old_pack_ptr, (ulong)pack_ptr, (int)(pack_ptr - old_pack_ptr)));
        ...
        assert(tabledef->type(i) == MYSQL_TYPE_DECIMAL ||
               tabledef->calc_field_size(i, (uchar *)old_pack_ptr) == (uint32)(pack_ptr - old_pack_ptr));
      }
      ...
      */
      if (conv_field)
      {
        ...
        DBUG_PRINT("debug", ("Copying field '%s' of type '%s' with value '%s'", (*field_ptr)->field_name,
        ...
        DBUG_PRINT("debug", ("Value of field '%s' of type '%s' is now '%s'", (*field_ptr)->field_name,
  ...
  if (table->has_gcol())
  {
    for (Field **vfield_ptr = table->vfield; *vfield_ptr; ++vfield_ptr)
    {
      ...
      if ((vfield->is_virtual_gcol() && vfield->m_indexed) ||
          bitmap_is_overlapping(table->write_set, &vfield->gcol_info->base_columns_map))
      {
  ...
  */
  ...
  for (Field **field_ptr = table->field; *field_ptr; ++field_ptr)
  {
    ...
    if (field_index >= cols->n_bits || !bitmap_is_set(cols, field_index))
    {
      ...
      {
        f->set_default();
        push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_NO_DEFAULT_FOR_FIELD,
                            ER(ER_NO_DEFAULT_FOR_FIELD), f->field_name);
      }
