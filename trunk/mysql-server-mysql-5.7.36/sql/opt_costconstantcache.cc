...
static void report_engine_cost_warnings(const LEX_CSTRING &se_name, int storage_category, const LEX_CSTRING &cost_name,
                                        double value, cost_constant_error error)
{
  switch (error)
  {
    case UNKNOWN_COST_NAME:
    ...
    case INVALID_DEVICE_TYPE:
      sql_print_warning(
          "Invalid device type %d for \"%s\" storage engine for cost constant \"%s\" in mysql.engine_cost table\n",
          storage_category, se_name.str, cost_name.str);
      break;
    case INVALID_COST_VALUE:
      sql_print_warning(
          "Invalid value for cost constant \"%s\" for \"%s\" storage engine and device type %d in mysql.engine_cost "
          "table: %.1f\n",
...
static void read_server_cost_constants(THD *thd, TABLE *table, Cost_model_constants *cost_constants)
{
  ...
  */
  ...
  const bool ret = init_read_record(&read_record_info, thd, table, NULL, true, true, false);
  if (!ret)
  {
    ...
    while (!read_record_info.read_record(&read_record_info))
    {
      ...
      */
      if (!table->field[1]->is_null())
      {
        ...
        String cost_name(cost_name_buf, sizeof(cost_name_buf), &my_charset_utf8_general_ci);
        ...
        const cost_constant_error err = cost_constants->update_server_cost_constant(cost_constant, value);
    ...
    while (!read_record_info.read_record(&read_record_info))
    {
      ...
      */
      if (!table->field[3]->is_null())
      {
        ...
        String engine_name(engine_name_buf, sizeof(engine_name_buf), &my_charset_utf8_general_ci);
        char cost_name_buf[MAX_FIELD_WIDTH];
        String cost_name(cost_name_buf, sizeof(cost_name_buf), &my_charset_utf8_general_ci);
        ...
        const cost_constant_error err =
            cost_constants->update_engine_cost_constant(thd, engine, device_type, cost_constant, value);
        if (err != COST_CONSTANT_OK)
          report_engine_cost_warnings(engine, device_type, cost_constant, value, err);
      }
    }
...
static void read_cost_constants(Cost_model_constants *cost_constants)
{
  ...
  */
  ...
  TABLE_LIST tables[2];
  tables[0].init_one_table(C_STRING_WITH_LEN("mysql"), C_STRING_WITH_LEN("server_cost"), "server_cost", TL_READ);
  tables[1].init_one_table(C_STRING_WITH_LEN("mysql"), C_STRING_WITH_LEN("engine_cost"), "engine_cost", TL_READ);
