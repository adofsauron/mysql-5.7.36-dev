...
Rpl_info_table::Rpl_info_table(uint nparam, const char *param_schema, const char *param_table,
                               const uint param_n_pk_fields, const uint *param_pk_field_indexes)
    : Rpl_info_handler(nparam), is_transactional(FALSE)
{
  ...
  if ((str_schema.str = (char *)my_malloc(key_memory_Rpl_info_table, schema_length + 1, MYF(0))))
  {
  ...
  if ((str_table.str = (char *)my_malloc(key_memory_Rpl_info_table, table_length + 1, MYF(0))))
  {
    str_table.length = table_length;
    strmake(str_table.str, param_table, table_length);
  }

  if ((description = (char *)my_malloc(key_memory_Rpl_info_table, str_schema.length + str_table.length + 2, MYF(0))))
  {
...
int Rpl_info_table::do_init_info(enum_find_method method, uint instance)
{
  ...
  */
  if (access->open_table(thd, str_schema, str_table, get_number_info(), TL_WRITE, &table, &backup))
    goto end;
...
int Rpl_info_table::do_flush_info(const bool force)
{
  ...
  */
  if ((res = access->find_info(field_values, table)) == NOT_FOUND_ID)
  {
    ...
    */
    empty_record(table);
    if (access->store_info_values(get_number_info(), table->field, field_values))
      goto end;
  ...
  DBUG_EXECUTE_IF("mts_debug_concurrent_access", {
    while (thd->system_thread == SYSTEM_THREAD_SLAVE_WORKER && mts_debug_concurrent_access < 2 &&
           mts_debug_concurrent_access > 0)
    {
...
*/
int Rpl_info_table::do_reset_info(uint nparam, const char *param_schema, const char *param_table,
                                  const char *channel_name, uint channel_field_idx)
{
  int error = 0;
  TABLE *table = NULL;
  sql_mode_t saved_mode;
  ...
  /*
    Opens and locks the rpl_info table before accessing it.
  */
  if (info->access->open_table(thd, info->str_schema, info->str_table, info->get_number_info(), TL_WRITE, &table,
                               &backup))
  {
    error = 1;
    goto end;
  ...
  if (!(handler_error = table->file->ha_index_init(0, 1)))
  {
    ...
    */
    ...
    table->field[fieldnr]->store(channel_name, strlen(channel_name), &my_charset_bin);
    uint key_len = key_info->key_part[0].store_length;
    uchar *key_buf = table->field[fieldnr]->ptr;

    if (!(handler_error =
              table->file->ha_index_read_map(table->record[0], key_buf, (key_part_map)1, HA_READ_KEY_EXACT)))
    {
      do
      {
        if ((handler_error = table->file->ha_delete_row(table->record[0])))
          break;
      } while (!(handler_error = table->file->ha_index_next_same(table->record[0], key_buf, key_len)));
...
enum_return_check Rpl_info_table::do_check_info()
{
  ...
  /*
    Opens and locks the rpl_info table before accessing it.
  */
  if (access->open_table(thd, str_schema, str_table, get_number_info(), TL_READ, &table, &backup))
  {
    sql_print_warning(
        "Info table is not ready to be used. Table "
        "'%s.%s' cannot be opened.",
  ...
  */
  ...
  /*
    Unlocks and closes the rpl_info table.
  */
  access->close_table(thd, table, &backup, return_check == ERROR_CHECKING_REPOSITORY);
  thd->variables.sql_mode = saved_mode;
  access->drop_thd(thd);
...
bool Rpl_info_table::do_count_info(uint nparam, const char *param_schema, const char *param_table, uint *counter)
{
  int error = 1;
  TABLE *table = NULL;
  sql_mode_t saved_mode;
...
bool Rpl_info_table::do_set_info(const int pos, const uchar *value, const size_t size)
{
...
{
  return (field_values->value[pos].set_real(value, NOT_FIXED_DEC, &my_charset_bin));
}
...
bool Rpl_info_table::do_get_info(const int pos, char *value, const size_t size, const char *default_value)
{
  if (field_values->value[pos].length())
    strmake(value, field_values->value[pos].c_ptr_safe(), field_values->value[pos].length());
                                 ...
                                 const uchar *default_value MY_ATTRIBUTE((unused)))
{
  if (field_values->value[pos].length() == size)
    return (!memcpy((char *)value, field_values->value[pos].c_ptr_safe(), size));
  return TRUE;
}

bool Rpl_info_table::do_get_info(const int pos, ulong *value, const ulong default_value)
{
  if (field_values->value[pos].length())
  {
...
bool Rpl_info_table::do_get_info(const int pos, int *value, const int default_value)
{
  if (field_values->value[pos].length())
  {
...
bool Rpl_info_table::do_get_info(const int pos, float *value, const float default_value)
{
  if (field_values->value[pos].length())
  {
...
bool Rpl_info_table::do_update_is_transactional()
{
  ...
  DBUG_EXECUTE_IF("simulate_update_is_transactional_error", { DBUG_RETURN(TRUE); });
...
bool Rpl_info_table::verify_table_primary_key_fields(TABLE *table)
{
  ...
  */
  if ((error = !key_info || key_info->user_defined_key_parts == 0 ||
               (m_n_pk_fields > 0 && key_info->user_defined_key_parts != m_n_pk_fields)))
  {
    sql_print_error("Corrupted table %s.%s. Check out table definition.", str_schema.str, str_table.str);
  }
    ...
    */
    for (uint idx = 0; idx < m_n_pk_fields; idx++)
    {
      if (key_info->key_part[idx].field != table->field[m_pk_field_indexes[idx]])
      {
        ...
        sql_print_error(
            "Info table has a problem with its key field(s). "
            "Table '%s.%s' expected field #%u to be '%s' but "
            "found '%s' instead.",
            str_schema.str, str_table.str, m_pk_field_indexes[idx], key_field_name, table_field_name);
        error = true;
        break;
      }
    }
