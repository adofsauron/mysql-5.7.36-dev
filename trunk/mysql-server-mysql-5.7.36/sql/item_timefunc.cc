...
static void adjust_time_range_with_warn(MYSQL_TIME *ltime, uint8 decimals)
{
  ...
  {
    int warning = 0;
    make_truncated_value_warning(ErrConvString(ltime, decimals), MYSQL_TIMESTAMP_TIME);
...
static bool extract_date_time(Date_time_format *format, const char *val, size_t length, MYSQL_TIME *l_time,
  ...
  if (val != val_end)
  {
    do
    {
      if (!my_isspace(&my_charset_latin1, *val))
      {
        ...
        make_truncated_value_warning(ErrConvString(val_begin, length), cached_timestamp_type);
        break;
      }
    } while (++val != val_end);
  }
      ...
      switch (*++ptr)
      {
        case 'M':
          ...
          str->append(locale->month_names->type_names[l_time->month - 1],
                      strlen(locale->month_names->type_names[l_time->month - 1]), system_charset_info);
          break;
        case 'b':
          if (!l_time->month)
            return 1;
          str->append(locale->ab_month_names->type_names[l_time->month - 1],
                      strlen(locale->ab_month_names->type_names[l_time->month - 1]), system_charset_info);
          break;
        case 'W':
        ...
        case 'j':
        {
          ...
          int radix = 10;
          int diff = calc_daynr(l_time->year, l_time->month, l_time->day) - calc_daynr(l_time->year, 1, 1) + 1;
          if (diff < 0)
            radix = -10;
...
static bool get_interval_info(Item *args, String *str_value, bool *is_negative, uint count, ulonglong *values,
                              bool transform_msec)
{
  String *res;
  ...
  for (uint i = 0; i < count; i++)
  {
    ...
    if (str == end && i != count - 1)
    {
      ...
      memmove(reinterpret_cast<uchar *>(values + count) - len, reinterpret_cast<uchar *>(values + i) - len, len);
...
bool Item_temporal_func::check_precision()
{
  ...
  {
    my_error(ER_TOO_BIG_PRECISION, MYF(0), (int)decimals, func_name(), DATETIME_MAX_DECIMALS);
    return true;
  }
  return false;
}
...
type_conversion_status Item_temporal_hybrid_func::save_in_field_inner(Field *field, bool no_conversions)
{
...
my_decimal *Item_temporal_hybrid_func::val_decimal(my_decimal *decimal_value)
{
  ...
  else
  {
    ...
    return null_value                                ? 0
           : ltime.time_type == MYSQL_TIMESTAMP_TIME ? time2my_decimal(&ltime, decimal_value)
                                                     : date2my_decimal(&ltime, decimal_value);
  }
}

bool Item_temporal_hybrid_func::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date)
{
  MYSQL_TIME tm;
  ...
  if (cached_field_type == MYSQL_TYPE_TIME || tm.time_type == MYSQL_TIMESTAMP_TIME)
...
bool Item_temporal_hybrid_func::get_time(MYSQL_TIME *ltime)
{
  ...
  if (cached_field_type == MYSQL_TYPE_TIME && ltime->time_type != MYSQL_TIMESTAMP_TIME)
...
String *Item_temporal_hybrid_func::val_str_ascii(String *str)
{
  ...
  if (val_datetime(&ltime, TIME_FUZZY_DATE) ||
      (null_value = my_TIME_to_str(
           &ltime, str,
           cached_field_type == MYSQL_TYPE_STRING ? (ltime.second_part ? DATETIME_MAX_DECIMALS : 0) : decimals)))
    return (String *)0;

  /* Check that the returned timestamp type matches to the function type */
  assert((cached_field_type == MYSQL_TYPE_TIME && ltime.time_type == MYSQL_TIMESTAMP_TIME) ||
         (cached_field_type == MYSQL_TYPE_DATE && ltime.time_type == MYSQL_TIMESTAMP_DATE) ||
         (cached_field_type == MYSQL_TYPE_DATETIME && ltime.time_type == MYSQL_TIMESTAMP_DATETIME) ||
         cached_field_type == MYSQL_TYPE_STRING || ltime.time_type == MYSQL_TIMESTAMP_NONE);
  return str;
}
...
longlong Item_date_func::val_date_temporal()
{
  assert(fixed == 1);
  MYSQL_TIME ltime;
  return get_date(&ltime, TIME_FUZZY_DATE) ? 0LL : TIME_to_longlong_date_packed(&ltime);
}
...
longlong Item_datetime_func::val_date_temporal()
{
  assert(fixed == 1);
  MYSQL_TIME ltime;
  return get_date(&ltime, TIME_FUZZY_DATE) ? 0LL : TIME_to_longlong_datetime_packed(&ltime);
}

bool Item_date_literal::eq(const Item *item, bool binary_cmp) const
{
  return item->basic_const_item() && type() == item->type() && func_name() == ((Item_func *)item)->func_name() &&
...
longlong Item_func_to_seconds::val_int_endpoint(bool left_endp, bool *incl_endp)
{
  ...
  null_value = check_date(&ltime, non_zero_date(&ltime), (TIME_NO_ZERO_IN_DATE | TIME_NO_ZERO_DATE), &dummy);
  /*
...
longlong Item_func_to_days::val_int_endpoint(bool left_endp, bool *incl_endp)
{
  ...
  */
  if ((!left_endp && !(ltime.hour || ltime.minute || ltime.second || ltime.second_part)) ||
...
String *Item_func_monthname::val_str(String *str)
{
  ...
  str->copy(month_name, strlen(month_name), &my_charset_utf8_bin, collation.collation, &err);
  return str;
}
...
bool Item_func_week::itemize(Parse_context *pc, Item **res)
{
  ...
  {
    THD *thd = pc->thd;
    args[1] = new (pc->mem_root) Item_int(NAME_STRING("0"), thd->variables.default_week_format, 1);
    if (args[1] == NULL)
      return true;
  }
...
longlong Item_func_weekday::val_int()
{
  ...
  return (longlong)calc_weekday(calc_daynr(ltime.year, ltime.month, ltime.day), odbc_type) + MY_TEST(odbc_type);
}
...
String *Item_func_dayname::val_str(String *str)
{
  ...
  str->copy(day_name, strlen(day_name), &my_charset_utf8_bin, collation.collation, &err);
  return str;
}
...
{
  struct timeval tm;
  return val_timeval(&tm) ? 0 : (double)tm.tv_sec + (double)tm.tv_usec / (double)1000000;
}
...
{
  assert(fixed == 1);
  assert(arg_count == 1 && args[0]->type() == Item::FIELD_ITEM && args[0]->field_type() == MYSQL_TYPE_TIMESTAMP);
...
bool get_interval_value(Item *args, interval_type int_type, String *str_value, Interval *interval)
{
  ...
  if (int_type == INTERVAL_SECOND && args->decimals)
  {
    ...
    lldiv_t tmp;
    if (!(val = args->val_decimal(&decimal_value)) || my_decimal2lldiv_t(E_DEC_FATAL_ERROR, val, &tmp))
      return false;
...
bool Item_func_from_days::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date)
{
  ...
  if ((null_value = (fuzzy_date & TIME_NO_ZERO_DATE) && (ltime->year == 0 || ltime->month == 0 || ltime->day == 0)))
    return TRUE;
...
void MYSQL_TIME_cache::set_datetime(struct timeval tv, uint8 dec_arg, Time_zone *tz)
{
...
void MYSQL_TIME_cache::set_time(struct timeval tv, uint8 dec_arg, Time_zone *tz)
{
  tz->gmt_sec_to_TIME(&time, tv);
...
bool MYSQL_TIME_cache::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzydate) const
{
  int warnings;
  get_TIME(ltime);
...
void Item_func_curtime::fix_length_and_dec()
{
  if (check_precision())
    return;
  THD *thd = current_thd;
  cached_time.set_time(thd->query_start_timeval_trunc(decimals), decimals, time_zone());
  /*
...
void Item_func_now::fix_length_and_dec()
{
  if (check_precision())
    return;
  THD *thd = current_thd;
  cached_time.set_datetime(thd->query_start_timeval_trunc(decimals), decimals, time_zone());
...
*/
bool Item_func_sysdate_local::get_date(MYSQL_TIME *now_time, my_time_flags_t fuzzy_date MY_ATTRIBUTE((unused)))
{
  THD *thd = current_thd;
...
void Item_func_date_format::fix_length_and_dec()
{
  ...
  */
  ...
  {  // Optimize the normal case
    fixed_length = 1;
    max_length = format_length(&arg1->str_value) * collation.collation->mbmaxlen;
  }
  else
  {
    fixed_length = 0;
    max_length = min<uint32>(arg1->max_length, MAX_BLOB_WIDTH) * 10 * collation.collation->mbmaxlen;
...
bool Item_func_from_unixtime::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date MY_ATTRIBUTE((unused)))
{
  lldiv_t lld;
  if (decimals)
  {
    my_decimal *val, decimal_value;
    if (!(val = args[0]->val_decimal(&decimal_value)) || my_decimal2lldiv_t(E_DEC_FATAL_ERROR, val, &lld))
    {
      null_value = 1;
      return true;
    }
  ...
  if ((null_value = (args[0]->null_value || lld.quot > TIMESTAMP_MAX_VALUE) || lld.quot < 0 || lld.rem < 0))
    return 1;
...
{
  fix_length_and_dec_and_charset_datetime(MAX_DATETIME_WIDTH, args[0]->datetime_precision());
  maybe_null = 1;
}

bool Item_func_convert_tz::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date MY_ATTRIBUTE((unused)))
{
...
bool Item_date_add_interval::get_date_internal(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date)
{
  Interval interval;
  ...
  */
  if (cached_field_type == MYSQL_TYPE_DATE && ltime->time_type == MYSQL_TIMESTAMP_DATETIME)
    datetime_to_date(ltime);
  else if (cached_field_type == MYSQL_TYPE_DATETIME && ltime->time_type == MYSQL_TIMESTAMP_DATE)
...
bool Item_date_add_interval::get_time_internal(MYSQL_TIME *ltime)
{
  ...
  if ((null_value = args[0]->get_time(ltime) || get_interval_value(args[1], int_type, &value, &interval)))
    return true;
  ...
  longlong usec1 =
      ((((ltime->day * 24 + ltime->hour) * 60 + ltime->minute) * 60 + ltime->second) * 1000000LL + ltime->second_part) *
      (ltime->neg ? -1 : 1);
  longlong usec2 = ((((interval.day * 24 + interval.hour) * 60 + interval.minute) * 60 + interval.second) * 1000000LL +
  ...
  if ((null_value = (interval.year || interval.month || sec_to_time(seconds, ltime))))
  {
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_DATETIME_FUNCTION_OVERFLOW,
                        ER(ER_DATETIME_FUNCTION_OVERFLOW), "time");
    return true;
  }
  return false;
}

bool Item_date_add_interval::val_datetime(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date)
{
  if (cached_field_type != MYSQL_TYPE_TIME)
...
bool Item_datetime_typecast::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date)
{
...
bool Item_func_makedate::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date)
{
  ...
  if (args[0]->null_value || args[1]->null_value || year < 0 || year > 9999 || daynr <= 0)
    goto err;
...
bool Item_func_add_time::val_datetime(MYSQL_TIME *time, my_time_flags_t fuzzy_date)
{
  assert(fixed == 1);
  ...
  time->neg = calc_time_diff(&l_time1, &l_time2, -l_sign, &seconds, &microseconds);
...
bool Item_func_timediff::get_time(MYSQL_TIME *l_time3)
{
  ...
  if ((args[0]->is_temporal_with_date() && args[1]->field_type() == MYSQL_TYPE_TIME) ||
      (args[1]->is_temporal_with_date() && args[0]->field_type() == MYSQL_TYPE_TIME))
    goto null_date;  // Incompatible types

  if (args[0]->is_temporal_with_date() || args[1]->is_temporal_with_date())
  {
    if (args[0]->get_date(&l_time1, TIME_FUZZY_DATE) || args[1]->get_date(&l_time2, TIME_FUZZY_DATE))
      goto null_date;
  }
  else
  {
...
bool Item_func_maketime::get_time(MYSQL_TIME *ltime)
{
  ...
  if ((null_value = (args[0]->null_value || args[1]->null_value || args[2]->null_value ||
                     my_decimal2lldiv_t(E_DEC_FATAL_ERROR, sec, &second) || minute < 0 || minute > 59 ||
  ...
  int len = (int)(ptr - buf) + sprintf(ptr, ":%02u:%02u", (uint)minute, (uint)second.quot);
  if (second.rem)
  {
    /*
    ...
    */
    ...
    len += sprintf(buf + len, ".%0*lld", dec, second.rem / (ulong)log_10_int[9 - dec]);
  }
...
void Item_func_str_to_date::fix_from_format(const char *format, size_t length)
{
    ...
    if (*val == '%' && val + 1 != end)
    {
      ...
      if (date_part_used && frac_second_used)
      {
        ...
        */
        ...
        cached_field_type = MYSQL_TYPE_DATETIME;
        fix_length_and_dec_and_charset_datetime(MAX_DATETIME_WIDTH, DATETIME_MAX_DECIMALS);
        return;
      }
    }
  }
  ...
  if (frac_second_used) /* TIME with microseconds */
  {
    ...
    cached_field_type = MYSQL_TYPE_TIME;
    fix_length_and_dec_and_charset_datetime(MAX_TIME_FULL_WIDTH, DATETIME_MAX_DECIMALS);
  }
...
void Item_func_str_to_date::fix_length_and_dec()
{
  ...
  cached_timestamp_type = MYSQL_TIMESTAMP_DATETIME;
  fix_length_and_dec_and_charset_datetime(MAX_DATETIME_WIDTH, DATETIME_MAX_DECIMALS);
  sql_mode = current_thd->variables.sql_mode & (MODE_NO_ZERO_DATE | MODE_NO_ZERO_IN_DATE | MODE_INVALID_DATES);
...
bool Item_func_str_to_date::val_datetime(MYSQL_TIME *ltime, my_time_flags_t fuzzy_date)
{
  Date_time_format date_time_format;
  ...
  if (extract_date_time(&date_time_format, val->ptr(), val->length(), ltime, cached_timestamp_type, 0, "datetime") ||
      ((fuzzy_date & TIME_NO_ZERO_DATE) && (ltime->year == 0 || ltime->month == 0 || ltime->day == 0)))
    goto null_date;
