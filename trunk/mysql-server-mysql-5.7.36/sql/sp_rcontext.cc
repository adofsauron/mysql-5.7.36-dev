...
sp_rcontext::sp_rcontext(const sp_pcontext *root_parsing_ctx, Field *return_value_fld, bool in_sub_stmt)
...
sp_rcontext *sp_rcontext::create(THD *thd, const sp_pcontext *root_parsing_ctx, Field *return_value_fld)
{
  sp_rcontext *ctx = new (thd->mem_root) sp_rcontext(root_parsing_ctx, return_value_fld, thd->in_sub_stmt);

  if (!ctx)
    return NULL;

  if (ctx->alloc_arrays(thd) || ctx->init_var_table(thd) || ctx->init_var_items(thd))
  {
    delete ctx;
    return NULL;
  }
...
bool sp_rcontext::alloc_arrays(THD *thd)
{
...

  {
    ...
    m_case_expr_holders.reset(static_cast<Item_cache **>(thd->mem_calloc(n * sizeof(Item_cache *))), n);
  }
...
bool sp_rcontext::init_var_items(THD *thd)
{
  ...
  m_var_items.reset(static_cast<Item **>(thd->alloc(num_vars * sizeof(Item *))), num_vars);
...
bool sp_rcontext::handle_sql_condition(THD *thd, uint *ip, const sp_instr *cur_spi)
{
  ...
  */
  ...
  if (thd->is_error())
  {
    ...
    found_handler = cur_pctx->find_handler(da->returned_sqlstate(), da->mysql_errno(), Sql_condition::SL_ERROR);
      ...
      */
      if (!found_condition)
      {
        found_condition = new (callers_arena->mem_root)
            Sql_condition(callers_arena->mem_root, da->mysql_errno(), da->returned_sqlstate(), Sql_condition::SL_ERROR,
                          da->message_text());
      }
    }
  }
  else if (da->current_statement_cond_count())
  {
    ...
    */
    ...
    while ((c = it++))
    {
      if (c->severity() == Sql_condition::SL_WARNING || c->severity() == Sql_condition::SL_NOTE)
      {
        sp_pcontext *cur_pctx = cur_spi->get_parsing_ctx();

        const sp_handler *handler = cur_pctx->find_handler(c->returned_sqlstate(), c->mysql_errno(), c->severity());
        if (handler)
        {
  ...
  */
  ...
  uint continue_ip = handler_entry->handler->type == sp_handler::CONTINUE ? cur_spi->get_cont_dest() : 0;

  /* Add a frame to handler-call-stack. */
  Handler_call_frame *frame = new (std::nothrow) Handler_call_frame(found_handler, found_condition, continue_ip);
  if (!frame)
  {
    sql_alloc_error_handler();
    DBUG_RETURN(false);
...
Item_cache *sp_rcontext::create_case_expr_holder(THD *thd, const Item *item) const
{
  ...
  thd->set_n_backup_active_arena(thd->sp_runtime_ctx->callers_arena, &current_arena);
...
bool sp_cursor::fetch(THD *thd, List<sp_variable> *vars)
{
  ...
  DBUG_EXECUTE_IF("bug23032_emit_warning",
                  push_warning(thd, Sql_condition::SL_WARNING, ER_UNKNOWN_ERROR, ER(ER_UNKNOWN_ERROR)););
...
int sp_cursor::Query_fetch_into_spvars::prepare(List<Item> &fields, SELECT_LEX_UNIT *u)
{
  /*
