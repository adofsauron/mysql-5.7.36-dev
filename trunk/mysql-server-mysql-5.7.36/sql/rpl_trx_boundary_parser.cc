...
static const char *event_parser_state_names[] = {"None", "GTID", "DDL", "DML", "Error"};
#endif
...
void Transaction_boundary_parser::reset()
{
  ...
  DBUG_PRINT("info", ("transaction boundary parser is changing state "
                      "from '%s' to '%s'",
                      event_parser_state_names[current_parser_state], event_parser_state_names[EVENT_PARSER_NONE]));
                                             ...
                                             const Format_description_log_event *fd_event, bool throw_warnings)
{
  ...
  enum_event_boundary_type event_boundary_type = get_event_boundary_type(buf, length, fd_event, throw_warnings);
...
*/
Transaction_boundary_parser::enum_event_boundary_type Transaction_boundary_parser::get_event_boundary_type(
    const char *buf, size_t length, const Format_description_log_event *fd_event, bool throw_warnings)
{
  ...
  DBUG_PRINT("info", ("trx boundary parser was fed with an event of type %s", Log_event::get_type_str(event_type)));
  ...
  switch (event_type)
  {
    ...
    */
    case binary_log::QUERY_EVENT:
    {
      ...
      */
      if (!strncmp(query, "BEGIN", qlen) || !strncmp(query, STRING_WITH_LEN("XA START")))
        boundary_type = EVENT_BOUNDARY_TYPE_BEGIN_TRX;
      /*
        COMMIT and ROLLBACK are always the end of a transaction.
      */
      else if (!strncmp(query, "COMMIT", qlen) || (!native_strncasecmp(query, STRING_WITH_LEN("ROLLBACK")) &&
...
*/
bool Transaction_boundary_parser::update_state(enum_event_boundary_type event_boundary_type, bool throw_warnings)
{
        ...
        case EVENT_PARSER_DML:
          if (throw_warnings)
            sql_print_warning(
                ...
                current_parser_state == EVENT_PARSER_GTID  ? "after a GTID_LOG_EVENT or an ANONYMOUS_GTID_LOG_EVENT"
  ...
  DBUG_PRINT("info", ("transaction boundary parser is changing state "
                      "from '%s' to '%s'",
                      event_parser_state_names[current_parser_state], event_parser_state_names[new_parser_state]));
