...
Unique::Unique(qsort_cmp2 comp_func, void *comp_func_fixed_arg, uint size_arg, ulonglong max_in_memory_size_arg)
...
{
  my_b_clear(&file);
  init_tree(&tree, (ulong)(max_in_memory_size / 16), 0, size, comp_func, 0, NULL, comp_func_fixed_arg);
...
static double get_merge_buffers_cost(Unique::Imerge_cost_buf_type buff_elems, uint elem_size, uint first, uint last,
                                     const Cost_model_table *cost_model)
{
  ...
  const double io_ops = static_cast<double>(total_buf_elems * elem_size) / IO_SIZE;
  const double io_cost = cost_model->io_block_read_cost(io_ops);
  /* Using log2(n)=log(n)/log(2) formula */
  const double cpu_cost = cost_model->key_compare_cost(total_buf_elems * log((double)n_buffers) / M_LN2);
...
static double get_merge_many_buffs_cost(Unique::Imerge_cost_buf_type buffer, uint maxbuffer, uint max_n_elems,
                                        uint last_n_elems, int elem_size, const Cost_model_table *cost_model)
{
  int i;
  double total_cost = 0.0;
  Unique::Imerge_cost_buf_type buff_elems = buffer; /* #s of elements in each of merged sequences */
  ...
  */
  if (maxbuffer >= MERGEBUFF2)
  {
    while (maxbuffer >= MERGEBUFF2)
    {
      ...
      {
        total_cost += get_merge_buffers_cost(buff_elems, elem_size, i, i + MERGEBUFF - 1, cost_model);
        lastbuff++;
      }
      total_cost += get_merge_buffers_cost(buff_elems, elem_size, i, maxbuffer, cost_model);
      maxbuffer = lastbuff;
    }
  }

  /* Simulate final merge_buff call. */
  total_cost += get_merge_buffers_cost(buff_elems, elem_size, 0, maxbuffer, cost_model);
  return total_cost;
}
...
double Unique::get_use_cost(Imerge_cost_buf_type buffer, uint nkeys, uint key_size, ulonglong max_in_memory_size,
                            const Cost_model_table *cost_model)
{
  ...
  max_elements_in_tree = ((ulong)max_in_memory_size / ALIGN_SIZE(sizeof(TREE_ELEMENT) + key_size));
  ...
  */
  result +=
      cost_model->disk_seek_base_cost() * n_full_trees * ceil(((double)key_size) * max_elements_in_tree / IO_SIZE);
  result += cost_model->disk_seek_base_cost() * ceil(((double)key_size) * last_tree_elems / IO_SIZE);

  /* Cost of merge */
  double merge_cost =
      get_merge_many_buffs_cost(buffer, n_full_trees, max_elements_in_tree, last_tree_elems, key_size, cost_model);
  if (merge_cost < 0.0)
    return merge_cost;
...
static bool merge_walk(uchar *merge_buffer, size_t merge_buffer_size, size_t key_length, Merge_chunk *begin,
                       Merge_chunk *end, tree_walk_action walk_action, void *walk_action_arg, qsort_cmp2 compare,
                       const void *compare_arg, IO_CACHE *file)
{
  if (end <= begin || merge_buffer_size < (ulong)(key_length * (end - begin + 1)))
    return 1;
  ...
  Priority_queue<Merge_chunk *, std::vector<Merge_chunk *, Malloc_allocator<Merge_chunk *>>, Merge_chunk_less> queue(
      (Merge_chunk_less(compare_context)), (Malloc_allocator<Merge_chunk *>(key_memory_Unique_merge_buffer)));
...
bool Unique::walk(tree_walk_action action, void *walk_action_arg)
{
  ...
  */
  ...
  const size_t merge_buffer_size = std::max(min_merge_buffer_size, static_cast<size_t>(max_in_memory_size));

  if (!(merge_buffer = (uchar *)my_malloc(key_memory_Unique_merge_buffer, merge_buffer_size, MYF(0))))
    return 1;
  res = merge_walk(merge_buffer, merge_buffer_size, size, file_ptrs.begin(), file_ptrs.end(), action, walk_action_arg,
...
bool Unique::get(TABLE *table)
{
  ...
  if (my_b_tell(&file) == 0)
  {
    ...
    if ((record_pointers = table->sort.sorted_result =
             (uchar *)my_malloc(key_memory_Filesort_info_record_pointers, size * tree.elements_in_tree, MYF(0))))
    {
  ...
  */
  if (!table->sort.io_cache)
    outfile = table->sort.io_cache =
        (IO_CACHE *)my_malloc(key_memory_TABLE_sort_io_cache, sizeof(IO_CACHE), MYF(MY_ZEROFILL));
  ...
  sort_param.max_keys_per_buffer = (uint)(max_in_memory_size / sort_param.sort_length);
  sort_param.not_killable = 1;

  const size_t num_bytes = (sort_param.max_keys_per_buffer + 1) * sort_param.sort_length;
  if (!(sort_memory = (uchar *)my_malloc(key_memory_Unique_sort_buffer, num_bytes, MYF(0))))
    return 1;
  sort_param.unique_buff = sort_memory + (sort_param.max_keys_per_buffer * sort_param.sort_length);
  ...
  if (merge_many_buff(&sort_param, Sort_buffer(sort_memory, num_bytes),
                      Merge_chunk_array(file_ptrs.begin(), file_ptrs.size()), &num_chunks, &file))
    goto err;
  if (flush_io_cache(&file) || reinit_io_cache(&file, READ_CACHE, 0L, 0, 0))
    goto err;
  if (merge_buffers(&sort_param, &file, outfile, Sort_buffer(sort_memory, num_bytes), file_ptr,
