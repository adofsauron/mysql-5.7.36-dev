...
{
  List_iterator_fast<const sp_condition_value> li(const_cast<List<const sp_condition_value> &>(condition_values));
...
sp_pcontext::sp_pcontext(THD *thd, sp_pcontext *prev, sp_pcontext::enum_scope scope)
    : Sql_alloc(),
...
sp_variable *sp_pcontext::find_variable(LEX_STRING name, bool current_scope_only) const
{
...
sp_variable *sp_pcontext::add_variable(THD *thd, LEX_STRING name, enum enum_field_types type,
...
{
  sp_label *label = new (thd->mem_root) sp_label(name, ip, sp_label::IMPLICIT, this);
...
bool sp_pcontext::add_condition(THD *thd, LEX_STRING name, sp_condition_value *value)
{
...
sp_condition_value *sp_pcontext::find_condition(LEX_STRING name, bool current_scope_only) const
{
...
sp_handler *sp_pcontext::find_handler(const char *sql_state, uint sql_errno,
                                      Sql_condition::enum_severity_level severity) const
{
  ...
  for (size_t i = 0; i < m_handlers.size(); ++i)
  {
    ...
    while ((cv = li++))
    {
      ...
      {
        case sp_condition_value::ERROR_CODE:
          if (sql_errno == cv->mysqlerr && (!found_cv || found_cv->type > sp_condition_value::ERROR_CODE))
          {
            found_cv = cv;
            found_handler = h;
          }
        ...
        case sp_condition_value::SQLSTATE:
          if (strcmp(sql_state, cv->sql_state) == 0 && (!found_cv || found_cv->type > sp_condition_value::SQLSTATE))
          {
            found_cv = cv;
            found_handler = h;
          }
        ...
        case sp_condition_value::WARNING:
          if ((is_sqlstate_warning(sql_state) || severity == Sql_condition::SL_WARNING) && !found_cv)
          {
            found_cv = cv;
            found_handler = h;
          }
        ...
        case sp_condition_value::EXCEPTION:
          if (is_sqlstate_exception(sql_state) && severity == Sql_condition::SL_ERROR && !found_cv)
          {
            found_cv = cv;
            found_handler = h;
          }
...
bool sp_pcontext::find_cursor(LEX_STRING name, uint *poff, bool current_scope_only) const
{
  ...
  return (!current_scope_only && m_parent) ? m_parent->find_cursor(name, poff, false) : false;
}
...
{
  if (m_cursor_offset <= offset && offset < m_cursor_offset + m_cursors.size())
  {
