...
void init_sql_alloc(PSI_memory_key key, MEM_ROOT *mem_root, size_t block_size, size_t pre_alloc)
{
                               ...
                               const CHARSET_INFO *to_cs, size_t *result_length)
{
  ...
  {
    uint dummy_errors;
    new_length = copy_and_convert(pos, new_length, to_cs, str, arg_length, from_cs, &dummy_errors);
  }
  pos[new_length] = 0;
