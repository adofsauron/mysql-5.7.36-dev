...
int injector::transaction::commit()
{
  ...
  */
  ...
  if ((error == 0) && (m_thd->binlog_next_event_pos.file_name != NULL) &&
      ((m_next_pos.m_file_name = my_strdup(key_memory_binlog_pos, m_thd->binlog_next_event_pos.file_name, MYF(0))) !=
       NULL))
  {
...
int injector::transaction::use_table(server_id_type sid, table tbl)
{
  ...
  error = m_thd->binlog_write_table_map(tbl.get_table(), tbl.is_transactional(), FALSE);
  m_thd->set_server_id(save_id);
  DBUG_RETURN(error);
}

int injector::transaction::write_row(server_id_type sid, table tbl, MY_BITMAP const *cols, size_t colcnt,
                                     record_type record, const uchar *extra_row_info)
{
  ...
  error = m_thd->binlog_write_row(tbl.get_table(), tbl.is_transactional(), record, extra_row_info);
  m_thd->set_server_id(save_id);
  DBUG_RETURN(error);
}
...
int injector::transaction::delete_row(server_id_type sid, table tbl, MY_BITMAP const *cols, size_t colcnt,
                                      record_type record, const uchar *extra_row_info)
{
  ...
  m_thd->set_server_id(sid);
  table::save_sets saveset(tbl, cols, cols);
  error = m_thd->binlog_delete_row(tbl.get_table(), tbl.is_transactional(), record, extra_row_info);
  m_thd->set_server_id(save_id);
  DBUG_RETURN(error);
}
...
int injector::transaction::update_row(server_id_type sid, table tbl, MY_BITMAP const *before_cols,
                                      MY_BITMAP const *after_cols, size_t colcnt, record_type before, record_type after,
                                      const uchar *extra_row_info)
{
  ...
  error = m_thd->binlog_update_row(tbl.get_table(), tbl.is_transactional(), before, after, extra_row_info);
  m_thd->set_server_id(save_id);
  DBUG_RETURN(error);
}

int injector::transaction::update_row(server_id_type sid, table tbl, MY_BITMAP const *cols, size_t colcnt,
...
int injector::record_incident(THD *thd, binary_log::Incident_event::enum_incident incident, LEX_STRING const message)
{
  Incident_log_event ev(thd, incident, message);
  return mysql_bin_log.write_incident(&ev, thd, true /*need_lock_log=true*/, message.str);
}
