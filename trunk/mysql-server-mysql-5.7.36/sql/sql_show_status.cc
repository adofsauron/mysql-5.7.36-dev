  ...
  is built as:
    SELECT * FROM
             (SELECT VARIABLE_NAME as Variable_name, VARIABLE_VALUE as Value
              ...
              WHERE <where_clause>
*/
SELECT_LEX *build_query(const POS &pos, THD *thd, enum_sql_command command, const LEX_STRING &table_name,
                        const String *wild, Item *where_cond)
{
  ...
  */
  ...
  table_expression =
      new (thd->mem_root) PT_table_expression(table_reference_list, NULL, NULL, NULL, NULL, NULL, NULL, lock_type);

  /* ... FROM (SELECT ...) ... */
  PT_table_factor_select_sym *table_factor_select_sym;
  table_factor_select_sym =
      new (thd->mem_root) PT_table_factor_select_sym(pos, NULL, options, item_list, table_expression);
  ...
  table_factor_parenthesis =
      new (thd->mem_root) PT_table_factor_parenthesis(select_derived_union_select, &derived_table_name, pos);
  ...
  if (wild != NULL)
  {
    ...
    wild_string =
        new (thd->mem_root) PTI_text_literal_text_string(pos, false, *lex_string);  // TODO WL#6629 check is_7bit
    if (wild_string == NULL)
      return NULL;
  ...
  select_part2 = new (thd->mem_root) PT_select_part2(options_and_item_list2, NULL, /* opt_into */
...
SELECT_LEX *build_show_session_status(const POS &pos, THD *thd, const String *wild, Item *where_cond)
{
...
SELECT_LEX *build_show_global_status(const POS &pos, THD *thd, const String *wild, Item *where_cond)
{
...
SELECT_LEX *build_show_session_variables(const POS &pos, THD *thd, const String *wild, Item *where_cond)
{
...
SELECT_LEX *build_show_global_variables(const POS &pos, THD *thd, const String *wild, Item *where_cond)
{
