...
*/
LEX_CSTRING group_replication_plugin_name_str = {C_STRING_WITH_LEN("group_replication")};
...
bool is_group_replication_plugin_loaded()
{
  ...
  plugin_ref plugin = my_plugin_lock_by_name(0, group_replication_plugin_name_str, MYSQL_GROUP_REPLICATION_PLUGIN);
  if (plugin != NULL)
  {
...
int group_replication_start()
{
  ...
  if (plugin != NULL)
  {
    ...
    */
    ...
    st_mysql_group_replication *plugin_handle = (st_mysql_group_replication *)plugin_decl(plugin)->info;
...
void get_server_startup_prerequirements(Trans_context_info &requirements, bool has_lock)
{
  ...
  requirements.gtid_mode = get_gtid_mode(has_lock ? GTID_MODE_LOCK_GTID_MODE : GTID_MODE_LOCK_NONE);
  requirements.log_slave_updates = opt_log_slave_updates;
  requirements.transaction_write_set_extraction = global_system_variables.transaction_write_set_extraction;
...
bool get_server_encoded_gtid_executed(uchar **encoded_gtid_executed, size_t *length)
{
  ...
  *encoded_gtid_executed = (uchar *)my_malloc(key_memory_Gtid_set_to_string, *length, MYF(MY_WME));
