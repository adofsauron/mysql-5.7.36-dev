...
bool handle_slave_worker_stop(Slave_worker *worker, Slave_job_item *job_item)
{
  ...
  */
  group_index = (job_item->data) ? rli->gaq->get_job_group(job_item->data->mts_group_idx)->total_seqno
...
const char *info_slave_worker_fields[] = {
    ...
    /*
      ...
      ease debugging.
    */
    "group_relay_log_name", "group_relay_log_pos", "group_master_log_name", "group_master_log_pos",
    ...
    */
    "checkpoint_relay_log_name", "checkpoint_relay_log_pos", "checkpoint_master_log_name", "checkpoint_master_log_pos",
...
#ifdef HAVE_PSI_INTERFACE
                     ,
                     param_key_info_run_lock, param_key_info_data_lock, param_key_info_sleep_lock,
                     param_key_info_thd_lock, param_key_info_data_cond, param_key_info_start_cond,
      ...
      exit_incremented(false)
{
  ...
  */
  ...
  mysql_mutex_init(key_mutex_slave_parallel_worker, &jobs_lock, MY_MUTEX_INIT_FAST);
...
int Slave_worker::init_worker(Relay_log_info *rli, ulong i)
{
  ...
  if (rli_init_info(false) || DBUG_EVALUATE_IF("inject_init_worker_init_info_fault", true, false))
    DBUG_RETURN(1);
  ...
  assert(rli->current_mts_submode->get_type() == current_mts_submode->get_type());
...
int Slave_worker::rli_init_info(bool is_gaps_collecting_phase)
{
  ...
  */
  size_t num_bits = is_gaps_collecting_phase ? MTS_MAX_BITS_IN_GROUP : c_rli->checkpoint_group;
  /*
  ...
  */
  ...
  if (is_gaps_collecting_phase &&
      (DBUG_EVALUATE_IF("mts_slave_worker_init_at_gaps_fails", true, false) || read_info(handler)))
  {
...
bool Slave_worker::read_info(Rpl_info_handler *from)
{
  ...
  if (from->get_info(&temp_internal_id, 0) ||
      ...
      from->get_info(group_master_log_name, sizeof(group_master_log_name), (char *)"") ||
      from->get_info(&temp_group_master_log_pos, 0UL) ||
      from->get_info(checkpoint_relay_log_name, sizeof(checkpoint_relay_log_name), (char *)"") ||
      from->get_info(&temp_checkpoint_relay_log_pos, 0UL) ||
      from->get_info(checkpoint_master_log_name, sizeof(checkpoint_master_log_name), (char *)"") ||
      from->get_info(&temp_checkpoint_master_log_pos, 0UL) || from->get_info(&temp_checkpoint_seqno, 0UL) ||
      from->get_info(&nbytes, 0UL) || from->get_info(buffer, (size_t)nbytes, (uchar *)0) ||
...
/*
  ...
  will be used.
*/
void Slave_worker::copy_values_for_PFS(ulong worker_id, en_running_state thd_running_status, THD *worker_thd,
                                       const Error &last_error, const Gtid_specification &gtid)
{
  id = worker_id;
...
bool Slave_worker::write_info(Rpl_info_handler *to)
{
  ...
  if (to->prepare_info_for_write() || to->set_info((int)internal_id) || to->set_info(group_relay_log_name) ||
      to->set_info((ulong)group_relay_log_pos) || to->set_info(group_master_log_name) ||
      to->set_info((ulong)group_master_log_pos) || to->set_info(checkpoint_relay_log_name) ||
      to->set_info((ulong)checkpoint_relay_log_pos) || to->set_info(checkpoint_master_log_name) ||
      to->set_info((ulong)checkpoint_master_log_pos) || to->set_info((ulong)checkpoint_seqno) || to->set_info(nbytes) ||
...
const char *Slave_worker::get_master_log_name()
{
  ...
  return (ptr_g->checkpoint_log_name != NULL) ? ptr_g->checkpoint_log_name : checkpoint_master_log_name;
}
...
bool Slave_worker::commit_positions(Log_event *ev, Slave_job_group *ptr_g, bool force)
{
  ...
  */
  ...
  {
    strmake(group_master_log_name, ptr_g->group_master_log_name, sizeof(group_master_log_name) - 1);
    my_free(ptr_g->group_master_log_name);
    ptr_g->group_master_log_name = NULL;
    strmake(checkpoint_master_log_name, group_master_log_name, sizeof(checkpoint_master_log_name) - 1);
  }
  ...
  */
  ...
  {
    assert(strlen(ptr_g->group_relay_log_name) + 1 <= sizeof(group_relay_log_name));
  ...
  */
  strmake(group_master_log_name, c_rli->get_group_master_log_name(), sizeof(group_master_log_name) - 1);
  ...
  DBUG_EXECUTE_IF("mts_debug_concurrent_access", { mts_debug_concurrent_access++; };);
...
extern "C" uchar *get_key(const uchar *record, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
  DBUG_ENTER("get_key");
...
bool init_hash_workers(Relay_log_info *rli)
{
  ...
  rli->inited_hash_workers = (my_hash_init(&rli->mapping_db_to_worker, &my_charset_bin, 0, 0, 0, get_key,
                                           (my_hash_free_key)free_entry, 0, key_memory_db_worker_hash_entry) == 0);
  if (rli->inited_hash_workers)
  {
    mysql_mutex_init(key_mutex_slave_worker_hash, &rli->slave_worker_hash_lock, MY_MUTEX_INIT_FAST);
...
*/
TABLE *mts_move_temp_table_to_entry(TABLE *table, THD *thd, db_worker_hash_entry *entry)
{
                               ...
                               bool need_temp_tables, Slave_worker *last_worker)
{
  ...
  assert(!rli->last_assigned_worker || rli->last_assigned_worker == last_worker);
  ...
  // Search in CGAP
  for (db_worker_hash_entry **it = rli->curr_group_assigned_parts.begin(); it != rli->curr_group_assigned_parts.end();
       ++it)
  {
    entry = *it;
  ...
  hash_value = my_calc_hash(&rli->mapping_db_to_worker, (uchar *)dbname, dblength);

  mysql_mutex_lock(&rli->slave_worker_hash_lock);

  entry = (db_worker_hash_entry *)my_hash_search_using_hash_value(&rli->mapping_db_to_worker, hash_value,
  ...
  if (!entry)
  {
    ...
    */
    if (!(db = (char *)my_malloc(key_memory_db_worker_hash_entry, dblength + 1, MYF(0))))
      goto err;
    if (!(entry =
              (db_worker_hash_entry *)my_malloc(key_memory_db_worker_hash_entry, sizeof(db_worker_hash_entry), MYF(0))))
    {
      my_free(db);
      goto err;
    }
    ...
    */
    ...
    entry->worker = (!last_worker) ? get_least_occupied_worker(rli, workers, NULL) : last_worker;
    ...
    if (rli->mapping_db_to_worker.records > mts_partition_hash_soft_max)
    {
      ...
      */
      Prealloced_array<db_worker_hash_entry *, HASH_DYNAMIC_INIT> hash_element(key_memory_db_worker_hash_entry);
      /*
      ...
      */
      for (uint i = 0; i < rli->mapping_db_to_worker.records; i++)
      {
        ...
        db_worker_hash_entry *entry = (db_worker_hash_entry *)my_hash_element(&rli->mapping_db_to_worker, i);
  ...
  else
  {
    ...
    else
    {
      ...
      assert(last_worker != NULL && rli->curr_group_assigned_parts.size() > 0);
      ...
      do
      {
        thd->ENTER_COND(&rli->slave_worker_hash_cond, &rli->slave_worker_hash_lock,
...
void Slave_worker::slave_worker_ends_group(Log_event *ev, int error)
{
  ...
  if (!error)
  {
    ...
    */
    ...
    assert(last_group_done_index != c_rli->gaq->size || ptr_g->group_relay_log_name != NULL);
  ...
  */
...
#endif
          DBUG_PRINT("info", ("Notifying entry %p release by worker %lu", entry, this->id));
  ...
  else  // not DB-type scheduler
  {
    ...
    */
    Mts_submode_logical_clock *mts_submode = static_cast<Mts_submode_logical_clock *>(c_rli->current_mts_submode);
    int64 min_child_waited_logical_ts = my_atomic_load64(&mts_submode->min_waited_timestamp);
...
ulong Slave_committed_queue::move_queue_head(Slave_worker_array *ws)
{
  ...
  for (i = entry; i != avail && !empty(); cnt++, i = (i + 1) % size)
  {
    ...
    */
    if (ptr_g->worker_id == MTS_WORKER_UNDEF || my_atomic_load32(&ptr_g->done) == 0)
      break; /* gap at i'th */
...
*/
ulong Slave_committed_queue::find_lwm(Slave_job_group **arg_g, ulong start_index)
{
  Slave_job_group *ptr_g = NULL;
  ulong i, k, cnt;
  ...
  */
  for (i = start_index, cnt = 0; cnt < len - (start_index + size - entry) % size; i = (i + 1) % size, cnt++)
  {
    ptr_g = &m_Q[i];
...
void Slave_committed_queue::free_dynamic_items()
{
  ...
  assert((avail == size /* full */ || entry == size /* empty */) || i == avail /* all occupied are processed */);
}

void Slave_worker::do_report(loglevel level, int err_code, const char *msg, va_list args) const
{
  ...
  bool is_group_replication_applier_channel = channel_map.is_group_replication_channel_name(c_rli->get_channel(), true);
  ...
  if (level == ERROR_LEVEL &&
      (!has_temporary_error(thd, err_code) || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::SESSION)))
  {
  ...
  if (is_group_replication_applier_channel)
  {
    my_snprintf(buff_coord, sizeof(buff_coord), "Worker %u failed executing transaction '%s'", internal_id, buff_gtid);
  }
  else
  {
...
int Slave_worker::slave_worker_exec_event(Log_event *ev)
{
  ...
  if (!is_mts_db_partitioned(rli) && may_have_timestamp(ev) && !curr_group_seen_sequence_number)
  {
    ...
    */
    assert(rli->gaq->entry == ev->mts_group_idx || Mts_submode_logical_clock::clock_leq(last_committed, lwm_estimate));
...
{
  if (event.get_type_code() == binary_log::ROWS_QUERY_LOG_EVENT)  // If a `Rows_query_log_event`, let the
...
*/
bool Slave_worker::retry_transaction(uint start_relay_number, my_off_t start_relay_pos, uint end_relay_number,
                                     my_off_t end_relay_pos)
{
  ...
  do
  {
    ...
    if (found_order_commit_deadlock())
    {
      ...
      */
      ...
      if (!da->is_error() || has_temporary_error(thd, da->is_error() ? da->mysql_errno() : error, &silent))
      {
  ...
  } while (read_and_apply_events(start_relay_number, start_relay_pos, end_relay_number, end_relay_pos));
  DBUG_RETURN(false);
}
...
/**
  ...
  @return false if succeeds, otherwise returns true.
*/
bool Slave_worker::read_and_apply_events(uint start_relay_number, my_off_t start_relay_pos, uint end_relay_number,
                                         my_off_t end_relay_pos)
{
  ...
  while (!arrive_end)
  {
    ...
    if (!my_b_inited(&relay_io))
    {
      ...
      {
        sql_print_error("Failed to open relay log %s, error: %s", file_name, errmsg);
        goto end;
      }
    ...
    arrive_end = (my_b_tell(&relay_io) == end_relay_pos && file_number == end_relay_number);

    ev = Log_event::read_log_event(&relay_io, NULL, rli->get_rli_description_event(), opt_slave_sql_verify_checksum);
    if (ev != NULL)
    {
      /* It is a event belongs to the transaction */
      if (!ev->is_mts_sequential_exec(rli->current_mts_submode->get_type() == MTS_PARALLEL_TYPE_DB_NAME))
      {
        int ret = 0;
    ...
    else
    {
      ...
      */
      ...
      {
        sql_print_error(
            "Error when worker read relay log events,"
      ...
      {
        sql_print_error(
            "Failed to find next relay log when retrying the "
            "transaction, current relay log is %s",
            file_name);
        goto end;
      }
 ...
 */
static db_worker_hash_entry *find_entry_from_db_map(const char *dbname, Relay_log_info *rli)
{
  db_worker_hash_entry *entry = NULL;
  my_hash_value_type hash_value;
  uchar dblength = (uint)strlen(dbname);

  hash_value = my_calc_hash(&rli->mapping_db_to_worker, (const uchar *)dbname, dblength);
...
Slave_job_item *de_queue(Slave_jobs_queue *jobs, Slave_job_item *ret)
{
  ...
  *ret = jobs->m_Q[jobs->entry];
  ...
  assert(jobs->entry == jobs->size ||
         (jobs->len == (jobs->avail >= jobs->entry) ? (jobs->avail - jobs->entry)
                                                    : (jobs->size + jobs->avail - jobs->entry)));
...
bool append_item_to_jobs(slave_job_item *job_item, Slave_worker *worker, Relay_log_info *rli)
{
  ...
  */
         ...
         (big_event && rli->mts_pending_jobs_size != 0))
  {
    ...
    thd->ENTER_COND(&rli->pending_jobs_cond, &rli->pending_jobs_lock, &stage_slave_waiting_worker_to_free_events,
                    &old_stage);
  ...
  */
  if (rli->mts_wq_underrun_w_id == MTS_WORKER_UNDEF && worker->jobs.len > worker->underrun_level)
  {
    /*
  ...
  {
    thd->ENTER_COND(&worker->jobs_cond, &worker->jobs_lock, &stage_slave_waiting_worker_queue, &old_stage);
...
static void remove_item_from_jobs(slave_job_item *job_item, Slave_worker *worker, Relay_log_info *rli)
{
  ...
  */
  if (worker->overrun_level < worker->jobs.len)
  {
    ...
    assert(rli->workers.size() != 1 || rli->mts_wq_excess_cnt == worker->wq_overrun_cnt);
  }
...
struct slave_job_item *pop_jobs_item(Slave_worker *worker, Slave_job_item *job_item)
{
  ...
  while (!job_item->data && !thd->killed &&
         (worker->running_status == Slave_worker::RUNNING || worker->running_status == Slave_worker::STOP))
  {
    PSI_stage_info old_stage;
    ...
    if (job_item->data == NULL)
    {
      ...
      thd->ENTER_COND(&worker->jobs_cond, &worker->jobs_lock, &stage_slave_waiting_event_from_coordinator, &old_stage);
      mysql_cond_wait(&worker->jobs_cond, &worker->jobs_lock);
      mysql_mutex_unlock(&worker->jobs_lock);
      thd->EXIT_COND(&old_stage);
      mysql_mutex_lock(&worker->jobs_lock);
  ...
  */
  if (thd->is_error())
  {
    ...
    DBUG_PRINT("info", ("thd->get_stmt_da()->get_mysql_errno()=%d; "
                        ...
                        thd->get_stmt_da()->mysql_errno(), worker->last_error().number));
    ...
    if (worker->last_error().number == 0 &&
        ...
        */
        thd->get_stmt_da()->mysql_errno() != ER_SLAVE_WORKER_STOPPED_PREVIOUS_THD_ERROR)
    {
      /*
        This function is reporting an error which was not reported
        while executing exec_relay_log_event().
      */
      worker->report(ERROR_LEVEL, thd->get_stmt_da()->mysql_errno(), "%s", errmsg);
    }
  }
}
...
int slave_worker_exec_job_group(Slave_worker *worker, Relay_log_info *rli)
{
  ...
  while (1)
  {
    ...
    if (error || worker->found_order_commit_deadlock())
    {
      ...
      error =
          worker->retry_transaction(start_relay_number, start_relay_pos, job_item->relay_number, job_item->relay_pos);
      if (error)
        goto err;
    }
    /*
...
err:
  if (error)
  {
    ...
    DBUG_PRINT("info", ("Worker %lu is exiting: killed %i, error %i, "
                        "running_status %d",
                        worker->id, thd->killed, thd->is_error(), worker->running_status));
