...
static bool check_view_insertability(THD *thd, TABLE_LIST *view, const TABLE_LIST *insert_table_ref);
...
static bool check_single_table_insert(List<Item> &fields, TABLE_LIST *view, TABLE_LIST **insert_table_ref)
{
...
static bool check_insert_fields(THD *thd, TABLE_LIST *table_list, List<Item> &fields, uint value_count,
                                bool value_count_known, bool check_unique)
{
  ...
  else
  {
    ...
    */
    ...
    res = setup_fields(thd, Ref_ptr_array(), fields, INSERT_ACL, NULL, false, true);
    ...
    {
      if (check_single_table_insert(fields, table_list, &lex->insert_table_leaf))
        return true;
  ...
  if (check_key_in_view(thd, table_list, lex->insert_table_leaf) ||
      (table_list->is_view() && check_view_insertability(thd, table_list, lex->insert_table_leaf)))
  {
    my_error(ER_NON_INSERTABLE_TABLE, MYF(0), table_list->alias, "INSERT");
    return true;
  }

  assert(saved_insert_table_leaf == NULL || lex->insert_table_leaf == saved_insert_table_leaf);
...
static bool check_valid_table_refs(const TABLE_LIST *view, List<Item> &values, table_map map)
{
...
bool validate_default_values_of_unset_fields(THD *thd, TABLE *table)
{
  ...
  {
    if (!bitmap_is_set(write_set, (*field)->field_index) && !((*field)->flags & NO_DEFAULT_VALUE_FLAG))
    {
...
bool mysql_prepare_blob_values(THD *thd, List<Item> &fields, MEM_ROOT *mem_root)
{
  ...
  Prealloced_array<Field_blob *, 16, true> blob_update_field_set(PSI_NOT_INSTRUMENTED);
  ...
  while ((fld = f++))
  {
    ...
    if (lhs_field->type() == MYSQL_TYPE_BLOB || lhs_field->type() == MYSQL_TYPE_GEOMETRY)
  ...
  if (thd->lex->has_values_map())
  {
    ...
    for (iter = thd->lex->begin_values_map(); iter != thd->lex->end_values_map(); ++iter)
    {
...
bool Sql_cmd_insert::mysql_insert(THD *thd, TABLE_LIST *table_list)
{
  ...
  */
  ...
  COPY_INFO info(COPY_INFO::INSERT_OPERATION, &insert_field_list, manage_defaults, duplicates);
  COPY_INFO update(COPY_INFO::UPDATE_OPERATION, &insert_update_list, &insert_value_list);
  Name_resolution_context *context;
  Name_resolution_context_state ctx_state;
  uint num_partitions = 0;
  enum partition_info::enum_can_prune can_prune_partitions = partition_info::PRUNE_NO;
  ...
  if (duplicates == DUP_UPDATE && update.add_function_default_columns(insert_table, insert_table->write_set))
    goto exit_without_my_ok; /* purecov: inspected */
  ...
  */
  ...
  {
    if (insert_table->part_info->can_prune_insert(thd, duplicates, update, insert_update_list, insert_field_list,
                                                  !MY_TEST(values->elements), &can_prune_partitions,
    ...
    if (can_prune_partitions != partition_info::PRUNE_NO)
    {
      ...
      */
      if (insert_table->part_info->set_used_partition(insert_field_list, *values, info, prune_needs_default_values,
                                                      &used_partitions))
  ...
  {  // Statement plan is available within these braces
    Modification_plan plan(thd, (lex->sql_command == SQLCOM_INSERT) ? MT_INSERT : MT_REPLACE, insert_table, NULL, false,
                           0);
    ...
    if (can_prune_partitions != partition_info::PRUNE_NO)
    {
      ...
      */
      bitmap_intersect(&insert_table->part_info->read_partitions, &used_partitions);
      bitmap_intersect(&insert_table->part_info->lock_partitions, &used_partitions);
    }

    // Lock the tables now if not locked already.
    if (!is_locked && lock_tables(thd, table_list, lex->table_count, 0))
      DBUG_RETURN(true);
    ...
    */
    thd->count_cuted_fields =
        ((insert_many_values.elements == 1 && !lex->is_ignore()) ? CHECK_FIELD_ERROR_FOR_NULL : CHECK_FIELD_WARN);
    thd->cuted_fields = 0L;
    ...
    if (thd->slave_thread)
    {
      ...
      assert(c_rli != NULL);
      if (info.get_duplicate_handling() == DUP_UPDATE && insert_table->next_number_field != NULL &&
    ...
    if (duplicates == DUP_REPLACE && (!insert_table->triggers || !insert_table->triggers->has_delete_triggers()))
    ...
    while ((values = its++))
    {
      if (insert_field_list.elements || !value_count)
      {
        ...
        */
        ...
        if (fill_record_n_invoke_before_triggers(thd, &info, insert_field_list, *values, insert_table, TRG_EVENT_INSERT,
        ...
        res = check_that_all_fields_are_given_values(thd, insert_table, table_list);
        if (res)
        {
          assert(thd->is_error());
          error = 1;
      ...
      else
      {
        ...
        else
        {
          ...
          */
          ...
          {
            insert_table->record[0][share->null_bytes - 1] = share->default_values[share->null_bytes - 1];
          }
        }
        if (fill_record_n_invoke_before_triggers(thd, insert_table->field, *values, insert_table, TRG_EVENT_INSERT,
                                                 insert_table->s->fields))
        {
          assert(thd->is_error());
          error = 1;
  ...
  */
  {
    ...
    */
    ...
    if (error <= 0 || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT))
    {
      if (mysql_bin_log.is_open())
      {
        ...
        */
        ...
        if (thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str, thd->query().length, transactional_table, FALSE,
                              FALSE, errcode))
          error = 1;
      }
    }
    assert(transactional_table || !changed || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT));
  }
  ...
  */
  id = (thd->first_successful_insert_id_in_cur_stmt > 0)
           ...
           : (thd->arg_of_last_insert_id_function
                  ...
                  : ((insert_table->next_number_field && info.stats.copied) ? insert_table->next_number_field->val_int()
                                                                            : 0));
  ...
  if (insert_many_values.elements == 1 && (!(thd->variables.option_bits & OPTION_WARNINGS) || !thd->cuted_fields))
  {
    my_ok(thd,
          info.stats.copied + info.stats.deleted +
              (thd->get_protocol()->has_client_capability(CLIENT_FOUND_ROWS) ? info.stats.touched : info.stats.updated),
          id);
  }
  else
  {
    ...
    ha_rows updated =
        thd->get_protocol()->has_client_capability(CLIENT_FOUND_ROWS) ? info.stats.touched : info.stats.updated;
    if (lex->is_ignore())
      my_snprintf(buff, sizeof(buff), ER(ER_INSERT_INFO), (long)info.stats.records,
                  (long)(info.stats.records - info.stats.copied),
                  (long)thd->get_stmt_da()->current_statement_cond_count());
    else
      my_snprintf(buff, sizeof(buff), ER(ER_INSERT_INFO), (long)info.stats.records,
                  (long)(info.stats.deleted + updated), (long)thd->get_stmt_da()->current_statement_cond_count());
...
static bool check_view_insertability(THD *thd, TABLE_LIST *view, const TABLE_LIST *insert_table_ref)
{
  ...
  assert(view->table == NULL && table != NULL && view->field_translation != 0);
...
bool Sql_cmd_insert_base::mysql_prepare_insert_check_table(THD *thd, TABLE_LIST *table_list, List<Item> &fields,
                                                           bool select_insert)
{
  ...
  if (insert_into_view)
  {
    ...
    */
    if (!select->first_execution && table_list->is_merged() && fix_join_cond_for_insert(thd, table_list))
      DBUG_RETURN(true); /* purecov: inspected */
  }
  ...
  */
  if (select->derived_table_count && select->check_view_privileges(thd, INSERT_ACL, SELECT_ACL))
    DBUG_RETURN(true);
...
bool Sql_cmd_insert_base::mysql_prepare_insert(THD *thd, TABLE_LIST *table_list, List_item *values, bool select_insert)
{
  ...
  DBUG_PRINT("enter", ("table_list 0x%lx, view %d", (ulong)table_list, (int)insert_into_view));
  /*
  ...
  */
  ...
  if (mysql_prepare_insert_check_table(thd, table_list, insert_field_list, select_insert))
    DBUG_RETURN(true);
  ...
  if (values)
  {
    ...
    if (!res)
      res = check_insert_fields(thd, context->table_list, insert_field_list, values->elements, true, !insert_into_view);
    table_map map = 0;
    if (!res)
    ...
    while ((values = its++))
    {
      ...
      if (!res)
        res = setup_fields(thd, Ref_ptr_array(), *values, SELECT_ACL, NULL, false, false);
      if (!res)
        res = check_valid_table_refs(table_list, *values, map);

      if (!res && lex->insert_table_leaf->table->has_gcol())
        res = validate_gc_assignment(thd, &insert_field_list, values, lex->insert_table_leaf->table);
    }
    its.rewind();
    values = its++;
    ...
    if (!res && duplicates == DUP_UPDATE)
    {
      ...
      res = setup_fields(thd, Ref_ptr_array(), insert_update_list, UPDATE_ACL, NULL, false, true);
      if (!res)
      ...
      if (!res)
        res = setup_fields(thd, Ref_ptr_array(), insert_value_list, SELECT_ACL, NULL, false, false);
      ...
      if (!res && lex->insert_table_leaf->table->has_gcol())
        res = validate_gc_assignment(thd, &insert_update_list, &insert_value_list, lex->insert_table_leaf->table);
    }
  }
  else if (thd->stmt_arena->is_stmt_prepare())
  {
    ...
    */
    res = check_insert_fields(thd, context->table_list, insert_field_list, 0, false, !insert_into_view);
    table_map map = 0;
    if (!res)
      map = lex->insert_table_leaf->map();
  ...
  if (!select_insert)
  {
    TABLE_LIST *const duplicate = unique_table(thd, lex->insert_table_leaf, table_list->next_global, true);
    if (duplicate)
    {
    ...
    while ((error = table->file->ha_write_row(table->record[0])))
    {
      ...
      */
      ...
      if (!table->file->is_ignorable_error(error))
        goto err;
      is_duplicate_key_error = (error == HA_ERR_FOUND_DUPP_KEY || error == HA_ERR_FOUND_DUPP_UNIQUE);
      ...
      */
      if (duplicate_handling == DUP_REPLACE && table->next_number_field && key_nr == table->s->next_number_index &&
      ...
      */
      else if (key_nr < MAX_KEY)
      {
        ...
        */
        ...
        if ((error = (table->file->ha_index_read_idx_map(table->record[1], key_nr, (uchar *)key, HA_WHOLE_KEY,
                                                         HA_READ_KEY_EXACT))))
          goto err;
      }
      else
      ...
      if (duplicate_handling == DUP_UPDATE)
      {
        ...
        */
        if (insert_id_for_cur_row > 0)
          prev_insert_id = insert_id_for_cur_row;
        ...
        */
        if (mysql_prepare_blob_values(thd, *update->get_changed_columns(), &mem_root))
          goto before_trg_err;
        restore_record(table, record[1]);
        assert(update->get_changed_columns()->elements == update->update_values->elements);
        if (fill_record_n_invoke_before_triggers(thd, update, *update->get_changed_columns(), *update->update_values,
            ...
            (insert_id_for_cur_row > 0))
        {
          ...
          else if (table->file->auto_inc_interval_for_cur_row.in_range(auto_incr_val))
          {
            /*
        ...
        {
          const TABLE_LIST *inserted_view = table->pos_in_table_list->belong_to_view;
      ...
      else /* DUP_REPLACE */
      {
        ...
        if (view && view->replace_filter)
        {
          ...
          void *record0_saved = my_malloc(PSI_NOT_INSTRUMENTED, record_length, MYF(MY_WME));
...
int check_that_all_fields_are_given_values(THD *thd, TABLE *entry, TABLE_LIST *table_list)
{
  ...
  for (Field **field = entry->field; *field; field++)
  {
        ...
        ((*field)->real_type() != MYSQL_TYPE_ENUM))
    {
      ...
      if (view)
        (*field)->set_warning(Sql_condition::SL_WARNING, ER_NO_DEFAULT_FOR_VIEW_FIELD, 1, table_list->view_db.str,
                              table_list->view_name.str);
      else
        (*field)->set_warning(Sql_condition::SL_WARNING, ER_NO_DEFAULT_FOR_FIELD, 1);
      err = 1;
    }
  }
...
bool Sql_cmd_insert_select::mysql_insert_select_prepare(THD *thd)
{
  ...
  */
  ...
  select_lex->leaf_table_count -= lex->insert_table->is_view() ? lex->insert_table->leaf_tables_count() : 1;
  DBUG_RETURN(false);
}
...
int Query_result_insert::prepare(List<Item> &values, SELECT_LEX_UNIT *u)
{
  ...
  */
  ...
  res = check_insert_fields(thd, table_list, *fields, values.elements, true, !insert_into_view);
  if (!res)
    res = setup_fields(thd, Ref_ptr_array(), values, SELECT_ACL, NULL, false, false);

  if (!res && lex->insert_table_leaf->table->has_gcol())
    res = validate_gc_assignment(thd, fields, &values, lex->insert_table_leaf->table);
  ...
  if (duplicate_handling == DUP_UPDATE && !res)
  {
...
#endif
    if (!res)
      res = setup_fields(thd, Ref_ptr_array(), *update.get_changed_columns(), UPDATE_ACL, NULL, false, true);

    if (!res && lex->insert_table_leaf->table->has_gcol())
      res = validate_gc_assignment(thd, update.get_changed_columns(), update.update_values,
    ...
    */
    assert(!table_list->next_name_resolution_table);
    if (lex->select_lex->group_list.elements == 0 && !lex->select_lex->with_sum_func)
    {
      /*
    ...
    if (!res)
      res = setup_fields(thd, Ref_ptr_array(), *update.update_values, SELECT_ACL, NULL, false, false);
    ...
    if (!res)
    {
      ...
      */
      ...
      while ((item = li++))
      {
        item->transform(&Item::update_value_transformer, (uchar *)lex->current_select());
      }
    }
  ...
  */
  ...
  if (info.add_function_default_columns(table, table->write_set))
    DBUG_RETURN(1);
  if ((duplicate_handling == DUP_UPDATE) && update.add_function_default_columns(table, table->write_set))
    DBUG_RETURN(1);
  ...
  */
  ...
  if (duplicate_handling == DUP_REPLACE && (!table->triggers || !table->triggers->has_delete_triggers()))
...
int Query_result_insert::prepare2()
{
  ...
  if (thd->locked_tables_mode <= LTM_LOCK_TABLES && !thd->lex->describe)
  {
...
bool Query_result_insert::send_data(List<Item> &values)
{
  ...
  if (!error && (table->triggers || info.get_duplicate_handling() == DUP_UPDATE))
  {
    /*
...
void Query_result_insert::store_values(List<Item> &values)
{
  if (fields->elements)
  {
    ...
    if (!validate_default_values_of_unset_fields(thd, table))
      fill_record_n_invoke_before_triggers(thd, &info, *fields, values, table, TRG_EVENT_INSERT, table->s->fields);
  }
  else
    fill_record_n_invoke_before_triggers(thd, table->field, values, table, TRG_EVENT_INSERT, table->s->fields);
...
bool Query_result_insert::send_eof()
{
  ...
  assert(trans_table || !changed || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT));
  ...
  */
  if (mysql_bin_log.is_open() && (!error || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT)))
  {
    int errcode = 0;
    if (!error)
      thd->clear_error();
    ...
    if (thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str, thd->query().length, trans_table, false, false,
                          errcode))
    {
    ...
    */
    ...
    if (thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT))
    {
      if (mysql_bin_log.is_open())
      {
        ...
        (void)thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str, thd->query().length, transactional_table, FALSE,
                                FALSE, errcode);
      }
      if (changed)
        query_cache.invalidate(thd, table, TRUE);
...
static TABLE *create_table_from_items(THD *thd, HA_CREATE_INFO *create_info, TABLE_LIST *create_table,
                                      Alter_info *alter_info, List<Item> *items)
{
  ...
  tmp_table.s->db_low_byte_first = MY_TEST(create_info->db_type == myisam_hton || create_info->db_type == heap_hton);
  ...
  while ((item = it++))
  {
    ...
    else
    {
      ...
      tmp_table_field = create_tmp_field(thd, &tmp_table, item, item->type(), NULL, &from_field, &default_field, false,
                                         false, false, false);
    }
    ...
    switch (item->type())
    {
      ...
      */
       ...
       */
        table_field = NULL;
        if (tmp_table_field->is_temporal_with_date() && thd->is_strict_mode() && !item->maybe_null)
  ...
  */
  {
    if (!mysql_create_table_no_lock(thd, create_table->db, create_table->table_name, create_info, alter_info,
...
int Query_result_create::prepare(List<Item> &values, SELECT_LEX_UNIT *u)
{
  ...
  if (!(table = create_table_from_items(thd, create_info, create_table, alter_info, &values)))
    /* abort() deletes table */
    DBUG_RETURN(-1);
  ...
  for (Field **f = field; *f; f++)
  {
    if ((*f)->gcol_info)
    {
      /*
        ...
        SELECT statment.
      */
      my_error(ER_NON_DEFAULT_VALUE_FOR_GENERATED_COLUMN, MYF(0), (*f)->field_name, (*f)->table->s->table_name.str);
      DBUG_RETURN(true);
    }
  }
...
int Query_result_create::prepare2()
{
  ...
  {
   public:
    MY_HOOKS(Query_result_create *x, TABLE_LIST *create_table_arg, TABLE_LIST *select_tables_arg)
        : ptr(x), create_table(create_table_arg), select_tables(select_tables_arg)
    {
    }
   ...
   private:
    virtual int do_postlock(TABLE **tables, uint count)
    {
      ...
      create_table->table->set_binlog_drop_if_temp(!thd->is_current_stmt_binlog_disabled() &&
  ...
  */
  if (!(extra_lock = mysql_lock_tables(thd, &table, 1, 0)) || hooks.postlock(&table, 1))
  {
    if (extra_lock)
    {
...
{
  fill_record_n_invoke_before_triggers(thd, field, values, table, TRG_EVENT_INSERT, table->s->fields);
}
...
void Query_result_create::send_error(uint errcode, const char *err)
{
  ...
  DBUG_PRINT("info", ("Current statement %s row-based", thd->is_current_stmt_binlog_format_row() ? "is" : "is NOT"));
  DBUG_PRINT("info", ("Current table (at 0x%lu) %s a temporary (or non-existant) table", (ulong)table,
...
{
  assert(thd->lex->sql_command == SQLCOM_REPLACE || thd->lex->sql_command == SQLCOM_INSERT);
...
bool Sql_cmd_insert_select::execute(THD *thd)
{
  ...
  */
  ...
  if (!(res = open_tables_for_query(thd, all_tables, 0)))
  {
    ...
    if (!res &&
        (sel_result = new Query_result_insert(first_table, first_table->table, &insert_field_list, &insert_field_list,
                                              &insert_update_list, &insert_value_list, lex->duplicates)))
    {
      Ignore_error_handler ignore_handler;
      Strict_error_handler strict_handler;
      if (thd->lex->is_ignore())
      ...
      res = handle_query(thd, lex, sel_result,
                         ...
                         // to binary log
                         OPTION_SETUP_TABLES_DONE | SELECT_NO_UNLOCK, 0);
