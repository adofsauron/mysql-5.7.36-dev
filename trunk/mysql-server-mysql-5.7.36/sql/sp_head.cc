...
void sp_name::init_qname(THD *thd)
{
  ...
  if (!(m_qname.str = (char *)thd->alloc(m_qname.length + 1)))
    return;
  sprintf(m_qname.str, "%.*s%.*s%.*s", (int)m_db.length, (m_db.length ? m_db.str : ""), dot, ".", (int)m_name.length,
          m_name.str);
}
...
void *sp_head::operator new(size_t size) throw()
{
  ...
  init_sql_alloc(key_memory_sp_head_main_root, &own_root, MEM_ROOT_BLOCK_SIZE, MEM_ROOT_PREALLOC);
      ...
      unsafe_flags(0)
{
  ...
  */
  ...
  my_hash_init(&m_sptabs, system_charset_info, 0, 0, 0, sp_table_key, 0, 0, key_memory_sp_head_main_root);
  my_hash_init(&m_sroutines, system_charset_info, 0, 0, 0, sp_sroutine_key, 0, 0, key_memory_sp_head_main_root);
...
void sp_head::init_sp_name(THD *thd, sp_name *spname)
{
  ...
  m_name.str = strmake_root(thd->mem_root, spname->m_name.str, spname->m_name.length);
  ...
  m_qname.str = (char *)memdup_root(thd->mem_root, spname->m_qname.str, spname->m_qname.length + 1);
}
...
bool sp_head::setup_trigger_fields(THD *thd, Table_trigger_field_support *tfs, GRANT_INFO *subject_table_grant,
                                   bool need_fix_fields)
{
  for (SQL_I_List<Item_trigger_field> *trig_field_list = m_list_of_trig_fields_item_lists.first; trig_field_list;
       trig_field_list = trig_field_list->first->next_trig_field_list)
  {
    for (Item_trigger_field *f = trig_field_list->first; f; f = f->next_trg_field)
    {
...
bool sp_head::has_updated_trigger_fields(const MY_BITMAP *used_fields) const
{
       ...
       trig_field_list = trig_field_list->first->next_trig_field_list)
  {
    for (Item_trigger_field *f = trig_field_list->first; f; f = f->next_trg_field)
    {
      ...
      if (f->field_idx != (uint)-1)
      {
        if (bitmap_is_set(used_fields, f->field_idx) && f->get_settable_routine_parameter())
          return true;
      }
    }
  }
...
Field *sp_head::create_result_field(size_t field_max_length, const char *field_name, TABLE *table)
{
  size_t field_length = !m_return_field_def.length ? field_max_length : m_return_field_def.length;

  Field *field = ::make_field(table->s,     /* TABLE_SHARE ptr */
                              (uchar *)0,   /* field ptr */
                              field_length, /* field [max] length */
                              (uchar *)"",  /* null ptr */
                              0,            /* null bit */
                              m_return_field_def.pack_flag, m_return_field_def.sql_type, m_return_field_def.charset,
                              m_return_field_def.geom_type, Field::NONE, /* unreg check */
                              m_return_field_def.interval, field_name ? field_name : (const char *)m_name.str);
...
bool sp_head::execute(THD *thd, bool merge_da_on_success)
{
  ...
  */
  ...
  init_sql_alloc(key_memory_sp_head_execute_root, &execute_mem_root, MEM_ROOT_BLOCK_SIZE, 0);
  ...
  {
    DBUG_PRINT("info",
               ("first free for 0x%lx ++: 0x%lx->0x%lx  level: %lu  flags %x", (ulong)m_first_instance, (ulong)this,
                (ulong)m_next_cached_sp, m_next_cached_sp->m_recursion_level, m_next_cached_sp->m_flags));
  }
  /*
  ...
  */
  assert((m_next_cached_sp == 0 && m_first_instance->m_last_cached_sp == this) ||
  ...
  */
  if (m_db.length &&
      (err_status = mysql_opt_change_db(thd, to_lex_cstring(m_db), &saved_cur_db_name, false, &cur_db_changed)))
  {
    goto done;
  }
  ...
  */
  ...
  do
  {
    ...
    */
    ...
    thd->m_statement_psi = MYSQL_START_STATEMENT(&psi_state, psi_info->m_key, thd->db().str, thd->db().length,
    ...
    */
    if (!thd->is_fatal_error && !thd->killed_errno() && thd->sp_runtime_ctx->handle_sql_condition(thd, &ip, i))
    {
      err_status = FALSE;
    }
  ...
  if (err_status && thd->is_error() && !caller_da->is_error())
  {
    ...
    */
    caller_da->set_error_status(thd->get_stmt_da()->mysql_errno(), thd->get_stmt_da()->message_text(),
  ...
  */
  ...
  m_flags &= ~IS_INVOKED;
  DBUG_PRINT("info", ("first free for 0x%lx --: 0x%lx->0x%lx, level: %lu, flags %x", (ulong)m_first_instance,
                      (ulong)m_first_instance->m_first_free_instance, (ulong)this, m_recursion_level, m_flags));
  /*
  ...
  */
  assert((m_first_instance->m_first_free_instance == 0 && this == m_first_instance->m_last_cached_sp &&
          m_next_cached_sp == 0) ||
         (m_first_instance->m_first_free_instance != 0 && m_first_instance->m_first_free_instance == m_next_cached_sp &&
                              ...
                              GRANT_INFO *grant_info)
{
  ...
  if (m_chistics->suid != SP_IS_NOT_SUID &&
      m_security_ctx.change_security_context(thd, definer_user, definer_host, &m_db, &save_ctx))
    DBUG_RETURN(true);
  ...
  */
  ...
  if (!(grant_info->privilege & TRIGGER_ACL))
  {
    ...
    my_error(ER_TABLEACCESS_DENIED_ERROR, MYF(0), priv_desc, thd->security_context()->priv_user().str,
  ...
  */
  init_sql_alloc(key_memory_sp_head_call_root, &call_mem_root, MEM_ROOT_BLOCK_SIZE, 0);
  thd->set_n_backup_active_arena(&call_arena, &backup_arena);

  sp_rcontext *trigger_runtime_ctx = sp_rcontext::create(thd, m_root_parsing_ctx, NULL);
...
bool sp_head::execute_function(THD *thd, Item **argp, uint argcount, Field *return_value_fld)
{
  ...
  */
  if (argcount != m_root_parsing_ctx->context_var_count())
  {
    ...
    */
    my_error(ER_SP_WRONG_NO_OF_ARGS, MYF(0), "FUNCTION", m_qname.str, m_root_parsing_ctx->context_var_count(),
             argcount);
    DBUG_RETURN(true);
  }
  /*
  ...
  */
  ...
  sp_rcontext *func_runtime_ctx = sp_rcontext::create(thd, m_root_parsing_ctx, return_value_fld);
  ...
  */
  if (need_binlog_call)
  {
    ...
    for (arg_no = 0; arg_no < argcount; arg_no++)
    {
      ...
      str_value = sp_get_item_value(thd, func_runtime_ctx->get_item(arg_no), &str_value_holder);
  ...
  if (need_binlog_call)
  {
    ...
    if (thd->binlog_evt_union.unioned_events)
    {
      ...
      Query_log_event qinfo(thd, binlog_buf.ptr(), binlog_buf.length(), thd->binlog_evt_union.unioned_events_trans,
                            FALSE, FALSE, errcode);
      if (mysql_bin_log.write_event(&qinfo) && thd->binlog_evt_union.unioned_events_trans)
      {
...
err_with_cleanup:
  ...
  /*
    ...
    messages.
  */
  if (need_binlog_call && thd->sp_runtime_ctx == NULL && !thd->binlog_evt_union.do_union)
...
bool sp_head::execute_procedure(THD *thd, List<Item> *args)
{
  ...
  sp_rcontext *proc_runtime_ctx = sp_rcontext::create(thd, m_root_parsing_ctx, NULL);
  ...
  if (params > 0)
  {
    ...
    for (uint i = 0; i < params; i++)
    {
      ...
      if (spvar->mode == sp_variable::MODE_OUT)
      {
        ...
        if (!null_item || proc_runtime_ctx->set_variable(thd, i, (Item **)&null_item))
        {
          err_status = TRUE;
          break;
        }
...
bool sp_head::restore_lex(THD *thd)
{
  ...
  DBUG_PRINT("info", ("lex->get_stmt_unsafe_flags: 0x%x", thd->lex->get_stmt_unsafe_flags()));
...
void sp_head::set_info(longlong created, longlong modified, st_sp_chistics *chistics, sql_mode_t sql_mode)
{
  m_created = created;
  m_modified = modified;
...
void sp_head::set_definer(const char *definer, size_t definerlen)
{
  ...
  parse_user(definer, definerlen, user_name_holder, &user_name.length, host_name_holder, &host_name.length);

  set_definer(user_name, host_name);
}

void sp_head::set_definer(const LEX_CSTRING &user_name, const LEX_CSTRING &host_name)
{
...
{
  const char *col1_caption = (type == SP_TYPE_PROCEDURE) ? "Procedure" : "Function";

  const char *col3_caption = (type == SP_TYPE_PROCEDURE) ? "Create Procedure" : "Create Function";
...

  {
    ...
    */
    ...
    Item_empty_string *stmt_fld = new Item_empty_string(col3_caption, std::max<size_t>(m_defstr.length, 1024U));
...
bool sp_head::show_routine_code(THD *thd)
{
  ...
  field_list.push_back(new Item_empty_string("Instruction", std::max<size_t>(buffer.length(), 1024U)));
...
void sp_head::add_used_tables_to_table_list(THD *thd, TABLE_LIST ***query_tables_last_ptr, enum_sql_command sql_command,
                                            TABLE_LIST *belong_to_view)
{
  ...
  */
  ...
  for (uint i = 0; i < m_sptabs.records; i++)
  {
    ...
    char *tab_buff = static_cast<char *>(thd->alloc(ALIGN_SIZE(sizeof(TABLE_LIST)) * stab->lock_count));
    char *key_buff = static_cast<char *>(thd->memdup(stab->qname.str, stab->qname.length));
    ...
    for (uint j = 0; j < stab->lock_count; j++)
    {
      ...
      */
      ...
      if (sql_command == SQLCOM_LOCK_TABLES)
      {
        ...
        */
        mdl_lock_type = (stab->lock_type >= TL_WRITE_ALLOW_WRITE) ? MDL_SHARED_NO_READ_WRITE : MDL_SHARED_READ_ONLY;
      }
      else
      {
        /*
      ...
      table->init_one_table(key_buff, stab->db_length, key_buff + stab->db_length + 1, stab->table_name_length,
                            key_buff + stab->db_length + 1 + stab->table_name_length + 1, stab->lock_type,
                            mdl_lock_type);
...
bool sp_head::check_show_access(THD *thd, bool *full_access)
{
  ...
  *full_access =
      ((!check_table_access(thd, SELECT_ACL, &tables, false, 1, true) && (tables.grant.privilege & SELECT_ACL) != 0) ||
       (!strcmp(m_definer_user.str, thd->security_context()->priv_user().str) &&
        !strcmp(m_definer_host.str, thd->security_context()->priv_host().str)));

  return *full_access ? false : check_some_routine_access(thd, m_db.str, m_name.str, m_type == SP_TYPE_PROCEDURE);
}
...
bool sp_head::set_security_ctx(THD *thd, Security_context **save_ctx)
{
  ...
  */
  ...
  if (*save_ctx && check_routine_access(thd, EXECUTE_ACL, m_db.str, m_name.str, m_type == SP_TYPE_PROCEDURE, false))
  {
    m_security_ctx.restore_security_context(thd, *save_ctx);
    *save_ctx = NULL;
    return true;
