...
bool init_dynarray_intvar_from_file(char *buffer, size_t size, char **buffer_act, IO_CACHE *f);

Rpl_info_file::Rpl_info_file(const int nparam, const char *param_pattern_fname, const char *param_info_fname,
                             bool indexed_arg)
...
file '%s', errno %d)",
    ...
    else if (init_io_cache(&info_file, info_fd, IO_SIZE * 2, READ_CACHE, 0L, 0, MYF(MY_WME)))
    {
      sql_print_error(
...
*/
bool Rpl_info_file::do_count_info(const int nparam, const char *param_pattern, bool indexed, uint *counter)
{
  uint i = 0;
...
int Rpl_info_file::do_reset_info(const int nparam, const char *param_pattern, bool indexed)
{
  int error = false;
  uint i = 0;
  Rpl_info_file *info = NULL;
...
bool Rpl_info_file::do_set_info(const int pos, const uchar *value, const size_t size)
{
...
bool Rpl_info_file::do_set_info(const int pos, const Server_ids *value)
{
  ...
  */
  ...
  error = (my_b_printf(&info_file, "%s\n", buffer.c_ptr_safe()) > (size_t)0 ? FALSE : TRUE);
err:
  return error;
}

bool Rpl_info_file::do_get_info(const int pos, char *value, const size_t size, const char *default_value)
{
  return (init_strvar_from_file(value, size, &info_file, default_value));
}

bool Rpl_info_file::do_get_info(const int pos, uchar *value, const size_t size, const uchar *default_value)
{
  return (my_b_read(&info_file, value, size));
}

bool Rpl_info_file::do_get_info(const int pos, ulong *value, const ulong default_value)
{
  return (init_ulongvar_from_file(value, &info_file, default_value));
}

bool Rpl_info_file::do_get_info(const int pos, int *value, const int default_value)
{
  return (init_intvar_from_file(value, &info_file, (int)default_value));
}

bool Rpl_info_file::do_get_info(const int pos, float *value, const float default_value)
{
                                ...
                                const Server_ids *default_value MY_ATTRIBUTE((unused)))
{
  ...
  */
  ...
  bool error = init_dynarray_intvar_from_file(buffer, sizeof(buffer), &buffer_act, &info_file);
  if (!error)
...
bool init_dynarray_intvar_from_file(char *buffer, size_t size, char **buffer_act, IO_CACHE *f)
{
  ...
  if (read_size + 1 == size && buf[size - 2] != '\n')
  {
    ...
    */
    ...
    if (!(buf_act = (char *)my_malloc(key_memory_Rpl_info_file_buffer, max_size, MYF(MY_WME))))
      DBUG_RETURN(TRUE);
    *buffer_act = buf_act;
    memcpy(buf_act, buf, read_size);
    snd_size = my_b_gets(f, buf_act + read_size, max_size - read_size);
    if (snd_size == 0 || ((snd_size + 1 == max_size - read_size) && buf[max_size - 2] != '\n'))
    {
      /*
