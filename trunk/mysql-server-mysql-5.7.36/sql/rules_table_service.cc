...
Cursor::Cursor(MYSQL_THD mysql_thd)
    : m_thd(mysql_thd), m_table_list(NULL), m_is_finished(true), m_table_is_malformed(true)
{
  ...
  m_table_list->init_one_table(db_name, strlen(db_name), table_name, strlen(table_name), "alias", TL_WRITE_DEFAULT);
  ...
  if (m_pattern_column == ILLEGAL_COLUMN_ID || m_pattern_database_column == ILLEGAL_COLUMN_ID ||
      m_replacement_column == ILLEGAL_COLUMN_ID || m_enabled_column == ILLEGAL_COLUMN_ID)
  {
