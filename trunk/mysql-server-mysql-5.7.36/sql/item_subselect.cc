...
void Item_subselect::init(st_select_lex *select_lex, Query_result_subquery *result)
{
  /*
  ...
  */
  ...
  else
  {
    ...
    */
    parsing_place = (outer_select->in_sum_expr ? CTX_NONE : outer_select->parsing_place);
...
{
  assert(exec_method == EXEC_EXISTS_OR_MAT || exec_method == EXEC_EXISTS);
  /*
    Change
      SELECT expr1, expr2
    to
  ...
  */
  if (unit->thd->stmt_arena->is_conventional())  // not prepared stmt
  {
    ...
    for (; cnt > 0; cnt--)
      select_lex->item_list.push_back(new Item_int(NAME_STRING("Not_used"), (longlong)1, MY_INT64_NUM_DECIMAL_DIGITS));
    Opt_trace_context *const trace = &unit->thd->opt_trace;
    OPT_TRACE_TRANSFORM(trace, oto0, oto1, select_lex->select_number, "IN (SELECT)", "EXISTS (CORRELATED SELECT)");
...
bool Item_in_subselect::finalize_materialization_transform(JOIN *join)
{
  ...
  subselect_single_select_engine *old_engine_derived = static_cast<subselect_single_select_engine *>(engine);
  ...
  */
  ...
  OPT_TRACE_TRANSFORM(&thd->opt_trace, oto0, oto1, old_engine_derived->select_lex->select_number, "IN (SELECT)",
                      "materialization");
  oto1.add("chosen", true);

  subselect_hash_sj_engine *const new_engine = new subselect_hash_sj_engine(thd, this, old_engine_derived);
  if (!new_engine)
    return true;
...
/**
  ...
  processor.
*/
static bool walk_join_condition(List<TABLE_LIST> *tables, Item_processor processor, Item::enum_walk walk, uchar *arg)
{
  TABLE_LIST *table;
  ...
  while ((table = li++))
  {
    ...
    if (table->nested_join != NULL && walk_join_condition(&table->nested_join->join_list, processor, walk, arg))
      return true;
  }
  return false;
}
...
*/
bool Item_subselect::walk_body(Item_processor processor, enum_walk walk, uchar *arg)
{
  ...
  if (walk & WALK_SUBQUERY)
  {
    for (SELECT_LEX *lex = unit->first_select(); lex; lex = lex->next_select())
    {
      ...
      if (lex->join_list != NULL && walk_join_condition(lex->join_list, processor, walk, arg))
        return true;

      // @todo: Roy thinks that we should always use lex->where_cond.
      Item *const where_cond = (lex->join && lex->join->is_optimized()) ? lex->join->where_cond : lex->where_cond();

      if (where_cond && where_cond->walk(processor, walk, arg))
        return true;
...
bool Item_subselect::exec()
{
  ...
  */
  ...
  const bool disable_trace = traced_before && !trace->feature_enabled(Opt_trace_context::REPEATED_SUBSELECT);
...
void Item_subselect::fix_after_pullout(st_select_lex *parent_select, st_select_lex *removed_select)
...
bool Item_in_subselect::walk(Item_processor processor, enum_walk walk, uchar *arg)
{
...
bool Item_in_subselect::exec()
{
  ...
  assert(exec_method != EXEC_MATERIALIZATION ||
         (exec_method == EXEC_MATERIALIZATION && engine->engine_type() == subselect_engine::HASH_SJ_ENGINE));
  /*
  ...
  */
  if (need_expr_cache && !left_expr_cache && exec_method == EXEC_MATERIALIZATION && init_left_expr_cache())
    DBUG_RETURN(TRUE);
...
class Query_result_max_min_subquery : public Query_result_subquery
{
 ...
 public:
  Query_result_max_min_subquery(Item_subselect *item_arg, bool mx, bool ignore_nulls)
      : Query_result_subquery(item_arg), cache(0), fmax(mx), ignore_nulls(ignore_nulls)
  {
  }
  void cleanup();
  bool send_data(List<Item> &items);
...
bool Query_result_max_min_subquery::cmp_decimal()
{
  ...
  if (cache->null_value || maxmin->null_value)
    return (ignore_nulls) ? !(cache->null_value) : !(maxmin->null_value);
  return (fmax) ? (my_decimal_cmp(cvalue, mvalue) > 0) : (my_decimal_cmp(cvalue, mvalue) < 0);
}
    ...
    : Item_singlerow_subselect(), was_values(false)
{
  ...
  max = max_arg;
  init(select_lex, new Query_result_max_min_subquery(this, max_arg, ignore_nulls));
  max_columns = 1;
  maybe_null = 1;
  max_columns = 1;
...
*/
Item_subselect::trans_res Item_singlerow_subselect::select_transformer(SELECT_LEX *select)
{
  ...
  if (!unit->is_union() && !select->table_list.elements && select->item_list.elements == 1 &&
      ...
      */
      !(select->item_list.head()->type() == FIELD_ITEM || select->item_list.head()->type() == REF_ITEM) &&
...
bool Item_singlerow_subselect::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzydate)
{
  if (!no_rows && !exec() && !value->null_value)
  {
    null_value = false;
...
Item_exists_subselect::Item_exists_subselect(st_select_lex *select)
    : Item_subselect(), value(FALSE), exec_method(EXEC_UNSPECIFIED), sj_convert_priority(0), embedding_join_nest(NULL)
{
...
Item_in_subselect::Item_in_subselect(Item *left_exp, st_select_lex *select)
...
Item_in_subselect::Item_in_subselect(const POS &pos, Item *left_exp, PT_subselect *pt_subselect_arg)
    : super(pos),
      left_expr(left_exp),
      left_expr_cache(NULL),
      left_expr_cache_filled(false),
...
bool Item_in_subselect::itemize(Parse_context *pc, Item **res)
{
  if (skip_itemize(res))
    return false;
  if (super::itemize(pc, res) || left_expr->itemize(pc, &left_expr) || pt_subselect->contextualize(pc))
    return true;
...
Item_allany_subselect::Item_allany_subselect(Item *left_exp, chooser_compare_func_creator fc, st_select_lex *select,
                                             bool all_arg)
...
Item_subselect::trans_res Item_in_subselect::single_value_transformer(SELECT_LEX *select, Comp_creator *func)
{
  ...
  */
  ...
  /*
    ...
    union.
  */
  for (SELECT_LEX *sel = unit->first_select(); sel != NULL; sel = sel->next_select())
  {
  ...
  */
       ...
       (!left_expr->maybe_null && !subquery_maybe_null)))
  {
    ...
    Item *subs;
    if (!select->group_list.elements && !select->having_cond() && !select->with_sum_func && !(select->next_select()) &&
        select->table_list.elements && !(substype() == ALL_SUBS && subquery_maybe_null))
    {
      OPT_TRACE_TRANSFORM(&thd->opt_trace, oto0, oto1, select->select_number, "> ALL/ANY (SELECT)", "SELECT(MIN)");
      oto1.add("chosen", true);
    ...
    {
      OPT_TRACE_TRANSFORM(&thd->opt_trace, oto0, oto1, select->select_number, "> ALL/ANY (SELECT)", "MIN (SELECT)");
      oto1.add("chosen", true);
      Item_maxmin_subselect *item;
      subs = item = new Item_maxmin_subselect(thd, this, select, func->l_op(), substype() == ANY_SUBS);
      if (upper_item)
  ...
  if (!substitution)
  {
    ...
    */
    Item_direct_ref *const left = new Item_direct_ref(&select->context, (Item **)optimizer->get_cache(),
...
Item_subselect::trans_res Item_in_subselect::row_value_transformer(SELECT_LEX *select)
{
...
Item_subselect::trans_res Item_in_subselect::row_value_in_to_exists_transformer(SELECT_LEX *select)
{
  THD *const thd = unit->thd;
  Item *having_item = 0;
  uint cols_num = left_expr->cols();
  bool is_having_used =
      select->having_cond() || select->with_sum_func || select->group_list.first || !select->table_list.elements;
    ...
    */
    ...
    for (uint i = 0; i < cols_num; i++)
    {
      ...
      assert((left_expr->fixed && item_i->fixed) ||
             (item_i->type() == REF_ITEM && ((Item_ref *)(item_i))->ref_type() == Item_ref::OUTER_REF));
      if (item_i->check_cols(left_expr->element_index(i)->cols()))
        DBUG_RETURN(RES_ERROR);
      Item_ref *const left = new Item_ref(&select->context, (*optimizer->get_cache())->addr(i), (char *)"<no matter>",
      ...
      Item_bool_func *item_eq =
          new Item_func_eq(left, new Item_ref(&select->context, pitem_i, (char *)"<no matter>", (char *)"<list ref>"));
      item_eq->set_created_by_in2exists();
      Item_bool_func *item_isnull =
          new Item_func_isnull(new Item_ref(&select->context, pitem_i, (char *)"<no matter>", (char *)"<list ref>"));
      ...
      {
        if (!(col_item = new Item_func_trig_cond(col_item, get_cond_guard(i), NULL, NO_PLAN_IDX,
                                                 Item_func_trig_cond::OUTER_FIELD_IS_NOT_NULL)))
          DBUG_RETURN(RES_ERROR);
        col_item->set_created_by_in2exists();
      }
      ...
      Item_bool_func *item_nnull_test = new Item_is_not_null_test(
          this, new Item_ref(&select->context, pitem_i, (char *)"<no matter>", (char *)"<list ref>"));
      item_nnull_test->set_created_by_in2exists();
      if (!abort_on_null && left_expr->element_index(i)->maybe_null)
      {
        if (!(item_nnull_test = new Item_func_trig_cond(item_nnull_test, get_cond_guard(i), NULL, NO_PLAN_IDX,
                                                        Item_func_trig_cond::OUTER_FIELD_IS_NOT_NULL)))
          DBUG_RETURN(RES_ERROR);
        item_nnull_test->set_created_by_in2exists();
      }
  ...
  else
  {
    ...
    */
    ...
    for (uint i = 0; i < cols_num; i++)
    {
      ...
      if (item_i->check_cols(left_expr->element_index(i)->cols()))
        DBUG_RETURN(RES_ERROR);
      Item_ref *const left = new Item_direct_ref(&select->context, (*optimizer->get_cache())->addr(i),
                                                 (char *)"<no matter>", (char *)in_left_expr_name);
      if (left == NULL)
        DBUG_RETURN(RES_ERROR);
      ...
      Item_bool_func *item = new Item_func_eq(
          left, new Item_direct_ref(&select->context, pitem_i, (char *)"<no matter>", (char *)"<list ref>"));
      item->set_created_by_in2exists();
      if (!abort_on_null)
      {
        ...
        Item_bool_func *item_isnull = new Item_func_isnull(
            new Item_direct_ref(&select->context, pitem_i, (char *)"<no matter>", (char *)"<list ref>"));
        item_isnull->set_created_by_in2exists();
        ...
        */
        ...
        {
          if (!(item = new Item_func_trig_cond(item, get_cond_guard(i), NULL, NO_PLAN_IDX,
                                               Item_func_trig_cond::OUTER_FIELD_IS_NOT_NULL)))
            DBUG_RETURN(RES_ERROR);
          item->set_created_by_in2exists();
          if (!(having_col_item = new Item_func_trig_cond(having_col_item, get_cond_guard(i), NULL, NO_PLAN_IDX,
                                                          Item_func_trig_cond::OUTER_FIELD_IS_NOT_NULL)))
            DBUG_RETURN(RES_ERROR);
          having_col_item->set_created_by_in2exists();
        }
...
Item_subselect::trans_res Item_in_subselect::select_transformer(SELECT_LEX *select)
{
...
Item_subselect::trans_res Item_in_subselect::select_in_like_transformer(SELECT_LEX *select, Comp_creator *func)
{
  THD *const thd = unit->thd;
...
bool Item_in_subselect::fix_fields(THD *thd_arg, Item **ref)
{
  ...
  if ((thd_arg->lex->context_analysis_only & CONTEXT_ANALYSIS_ONLY_VIEW) && left_expr && !left_expr->fixed)
  {
...
void Item_in_subselect::fix_after_pullout(st_select_lex *parent_select, st_select_lex *removed_select)
{
...
bool Item_in_subselect::init_left_expr_cache()
{
  ...
  */
  QEP_TAB *const qep_tab = &outer_join->qep_tab[outer_join->primary_tables - 1];
  ...
  {
    Cached_item *cur_item_cache = new_Cached_item(unit->thd, left_expr->element_index(i), use_result_field);
...
{
  std::pair<SELECT_LEX *, int> *pair_object = pointer_cast<std::pair<SELECT_LEX *, int> *>(arg);
...
bool Item_subselect::clean_up_after_removal(uchar *arg)
{
  ...
  */
  if (unit->is_executed())
  {
    assert(unit->first_select()->parent_lex->sql_command == SQLCOM_SET_OPTION);
    unit->cleanup(true);
  }

  st_select_lex *root = static_cast<st_select_lex *>(static_cast<void *>(arg));

  /*
    Do not remove this subquery if it is a part of the select
    list.
  */
  if (root != NULL && (root->resolve_place != SELECT_LEX::RESOLVE_SELECT_LIST) && root->is_in_select_list(this))
    return false;
...
Item_subselect::trans_res Item_allany_subselect::select_transformer(SELECT_LEX *select)
{
...
subselect_union_engine::subselect_union_engine(st_select_lex_unit *u, Query_result_interceptor *result_arg,
                                               Item_subselect *item_arg)
...
*/
void subselect_engine::set_row(List<Item> &item_list, Item_cache **row, bool possibly_empty)
{
  assert(engine_type() == SINGLE_SELECT_ENGINE || engine_type() == UNION_ENGINE);
...
{
  return select_lex->table_list.elements == 0 && !select_lex->where_cond() && !select_lex->having_cond() &&
...
bool subselect_indexsubquery_engine::scan_table()
{
  ...
  table->file->extra_opt(HA_EXTRA_CACHE, item->unit->thd->variables.read_buff_size);
...
*/
void subselect_indexsubquery_engine::copy_ref_key(bool *require_scan, bool *convert_error)
{
  ...
  for (uint part_no = 0; part_no < tab->ref().key_parts; part_no++)
  {
    ...
    if (s_key->null_key)
    {
      ...
      */
      if (cond_guard && !*cond_guard)
      {
        assert(!(static_cast<Item_in_subselect *>(item)->is_top_level_item()));
...
bool subselect_indexsubquery_engine::exec()
{
  ...
  if (!table->file->inited && (error = table->file->ha_index_init(tab->ref().key, !unique /* sorted */)))
  {
    (void)report_handler_error(table, error);
    DBUG_RETURN(true);
  }
  ...
  error = table->file->ha_index_read_map(table->record[0], key, key_parts_map, HA_READ_KEY_EXACT);
  ...
  else
  {
    for (;;)
    {
      ...
      if (!table->status)
      {
        ...
        if (unique)
          break;
        error = table->file->ha_index_next_same(table->record[0], key, key_length);
        if (error && error != HA_ERR_END_OF_FILE)
        {
          error = report_handler_error(table, error);
          break;
...
bool subselect_single_select_engine::change_query_result(Item_subselect *si, Query_result_subquery *res)
{
  item = si;
  result = res;
...
bool subselect_union_engine::change_query_result(Item_subselect *si, Query_result_subquery *res)
{
  item = si;
...
bool subselect_indexsubquery_engine::change_query_result(Item_subselect *si, Query_result_subquery *res)
{
  assert(0);
  return TRUE;
}
...
bool subselect_hash_sj_engine::setup(List<Item> *tmp_columns)
{
  ...
  */
  if (!(tmp_result_sink = new Query_result_union))
    DBUG_RETURN(TRUE);
  THD *const thd = item->unit->thd;
  if (tmp_result_sink->create_result_table(thd, tmp_columns,
                                           true,  // Eliminate duplicates
                                           thd->variables.option_bits | TMP_TABLE_ALL_COLUMNS, "materialized-subquery",
                                           true, true))
    DBUG_RETURN(TRUE);
  ...
  */
  assert(tmp_columns->elements == tmp_table->s->fields ||
         ...
         (tmp_table->hash_field && tmp_columns->elements == (tmp_table->s->fields - 1)));
  ...
  */
  ...
  if (!(tab->ref().key_buff = (uchar *)thd->mem_calloc(key_length)) ||
      !(tab->ref().key_copy = (store_key **)thd->alloc((sizeof(store_key *) * tmp_key_parts))) ||
  ...
  */
  ...
  tmp_table_ref->init_one_table("", 0, "materialized-subquery", 21, "materialized-subquery", TL_READ);
  ...
  for (uint part_no = 0; part_no < tmp_key_parts; part_no++)
  {
    ...
    if (!(right_col_item = new Item_field(thd, context, field)) ||
        !(eq_cond = new Item_func_eq(tab->ref().items[part_no], right_col_item)) ||
    ...
    if (tmp_table->hash_field)
      tab->ref().key_copy[part_no] =
          new store_key_hash_item(thd, field, cur_ref_buff, 0, field->pack_length(), tab->ref().items[part_no], &hash);
    else
      tab->ref().key_copy[part_no] = new store_key_item(thd, field,
                                                        /* TODO:
      ...
      mat_table_has_nulls != NEX_IRRELEVANT_OR_FALSE)
  {
    ...
    */
    if (mat_table_has_nulls == NEX_UNKNOWN)  // We do not know yet
    {
      ...
      *tab->ref().null_ref_key = 1;
      if (!table->file->inited && table->file->ha_index_init(tab->ref().key, false /* sorted */))
        DBUG_RETURN(true);
      if (safe_index_read(tab) == 1)
        DBUG_RETURN(true);
      *tab->ref().null_ref_key = 0;  // prepare for next searches of non-NULL
      mat_table_has_nulls = (table->status == 0) ? NEX_TRUE : NEX_IRRELEVANT_OR_FALSE;
    }
...
void subselect_hash_sj_engine::print(String *str, enum_query_type query_type)
{
  ...
  if (tab)
    ...
    str->append(STRING_WITH_LEN("<the access method for lookups is not yet created>"));
}
