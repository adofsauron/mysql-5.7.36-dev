...
extern int HINT_PARSER_parse(THD *thd, Hint_scanner *scanner, PT_hint_list **ret);
...
*/
const char *st_select_lex::type_str[SLT_total] = {"NONE",     "PRIMARY", "SIMPLE",       "DERIVED",
...
bool lex_init(void)
{
  ...
  for (CHARSET_INFO **cs = all_charsets; cs < all_charsets + array_elements(all_charsets) - 1; cs++)
  {
...
void Lex_input_stream::body_utf8_start(THD *thd, const char *begin_ptr)
{
  ...
  size_t body_utf8_length =
      (m_buf_length / thd->variables.character_set_client->mbminlen) * my_charset_utf8_bin.mbmaxlen;
...
void Lex_input_stream::body_utf8_append_literal(THD *thd, const LEX_STRING *txt, const CHARSET_INFO *txt_cs,
                                                const char *end_ptr)
{
  ...
  {
    thd->convert_string(&utf_txt, &my_charset_utf8_general_ci, txt->str, txt->length, txt_cs);
  }
  else
  {
    utf_txt.str = txt->str;
...
{
  st_select_lex *select = new (thd->mem_root) st_select_lex(NULL, NULL, NULL, NULL, NULL, NULL);
  if (select == NULL)
...
st_select_lex *LEX::new_query(st_select_lex *curr_select)
{
  ...
  if (curr_select != NULL && curr_select->nest_level >= (int)MAX_SELECT_NESTING)
  {
  ...
  SELECT_LEX_UNIT *const sel_unit =
      new (thd->mem_root) SELECT_LEX_UNIT(curr_select ? curr_select->parsing_place : CTX_NONE);
  if (!sel_unit)
    DBUG_RETURN(NULL); /* purecov: inspected */
...
static bool consume_optimizer_hints(Lex_input_stream *lip)
{
  ...
  if (lip->yyPeekn(whitespace) == '/' && lip->yyPeekn(whitespace + 1) == '*' && lip->yyPeekn(whitespace + 2) == '+')
  {
    ...
    Hint_scanner hint_scanner(lip->m_thd, lip->yylineno, lip->get_ptr(), lip->get_end_of_query() - lip->get_ptr(),
                              lip->m_digest);
...
static int find_keyword(Lex_input_stream *lip, uint len, bool function)
{
  ...
  const SYMBOL *symbol = function ? Lex_hash::sql_keywords_and_funcs.get_hash_symbol(tok, len)
  ...
  if (symbol)
  {
    ...
    if ((symbol->tok == NOT_SYM) && (lip->m_thd->variables.sql_mode & MODE_HIGH_NOT_PRECEDENCE))
      return NOT2_SYM;
    if ((symbol->tok == OR_OR_SYM) && !(lip->m_thd->variables.sql_mode & MODE_PIPES_AS_CONCAT))
      return OR2_SYM;
...
{
  assert(name != NULL);
  return Lex_hash::sql_keywords_and_funcs.get_hash_symbol(name->str, (uint)name->length) != NULL;
}
...
static char *get_text(Lex_input_stream *lip, int pre_skip, int post_skip)
{
  ...
  while (!lip->eof())
  {
    ...
    {
      int l;
      if (use_mb(cs) && (l = my_ismbchar(cs, lip->get_ptr() - 1, lip->get_end_of_query())))
      {
        lip->skip_binary(l - 1);
        continue;
      }
    }
    if (c == '\\' && !(lip->m_thd->variables.sql_mode & MODE_NO_BACKSLASH_ESCAPES))
    {  // Escaped character
      found_escape = 1;
      if (lip->eof())
        return 0;
    ...
    else if (c == sep)
    {
      ...
      else
      {
        ...
        for (to = start; str != end; str++)
        {
          ...
          if (!(lip->m_thd->variables.sql_mode & MODE_NO_BACKSLASH_ESCAPES) && *str == '\\' && str + 1 != end)
          {
            switch (*++str)
            {
              case 'n':
...
bool consume_comment(Lex_input_stream *lip, int remaining_recursions_permitted)
{
  ...
  assert(remaining_recursions_permitted == 0 || remaining_recursions_permitted == 1);
  uchar c;
  while (!lip->eof())
  {
    c = lip->yyGet();
  ...
  for (;;)
  {
    switch (state)
    {
      ...
      case MY_LEX_SKIP:  // This should not happen
        if (c == '-' && lip->yyPeek() == '-' && (my_isspace(cs, lip->yyPeekn(1)) || my_iscntrl(cs, lip->yyPeekn(1))))
        {
      ...
      case MY_LEX_IDENT:
        ...
        if (use_mb(cs))
        {
          ...
          while (ident_map[c = lip->yyGet()])
          {
            switch (my_mbcharlen(cs, c))
            {
              ...
              default:
                int l;
                if ((l = my_ismbchar(cs, lip->get_ptr() - 1, lip->get_end_of_query())) == 0)
                  break;
                lip->skip_binary(l - 1);
            }
          }
        ...
        */
        ...
        {
          CHARSET_INFO *cs = get_charset_by_csname(yylval->lex_str.str + 1, MY_CS_PRIMARY, MYF(0));
          if (cs)
          {
            yylval->charset = cs;
      ...
      case MY_LEX_NUMBER_IDENT:  // number or ident which num-start
        ...
        if (c == 'e' || c == 'E')
        {
          ...
          if (my_isdigit(cs, lip->yyPeek()) || (c = (lip->yyGet())) == '+' || c == '-')
          {                                     // Allow 1E+10
      ...
      case MY_LEX_USER_VARIABLE_DELIMITER:  // Found quote char
      {
        ...
        for (;;)
        {
          ...
          {
            if ((var_length = my_ismbchar(cs, lip->get_ptr() - 1, lip->get_end_of_query())))
      ...
      case MY_LEX_CMP_OP:  // Incomplete comparison operator
        if (state_map[lip->yyPeek()] == MY_LEX_CMP_OP || state_map[lip->yyPeek()] == MY_LEX_LONG_CMP_OP)
          lip->yySkip();
      ...
      case MY_LEX_STRING:  // Incomplete text string
        ...
        lip->body_utf8_append_literal(thd, &yylval->lex_str, lip->m_underscore_cs ? lip->m_underscore_cs : cs,
                                      lip->m_cpp_text_end);
        ...
        if (lip->yyPeekn(2) == '!')
        {
          ...
          */
          ...
          if (my_isdigit(cs, (version_str[0] = lip->yyPeekn(0))) &&
              ...
              my_isdigit(cs, (version_str[3] = lip->yyPeekn(3))) && my_isdigit(cs, (version_str[4] = lip->yyPeekn(4))))
          {
            version_str[5] = 0;
            ulong version;
      ...
      case MY_LEX_END_LONG_COMMENT:
        if ((lip->in_comment != NO_COMMENT) && lip->yyPeek() == '/')
        {
          ...
          */
          if (!my_isspace(cs, lip->yyPeek()) && lip->get_cpp_ptr() != lip->get_cpp_buf() &&
      ...
      case MY_LEX_HOSTNAME:  // end '@' of user@hostname
        for (c = lip->yyGet(); my_isalnum(cs, c) || c == '.' || c == '_' || c == '$'; c = lip->yyGet())
          ;
        yylval->lex_str = get_token(lip, 0, lip->yyLength());
        return (LEX_HOSTNAME);
      case MY_LEX_SYSTEM_VAR:
        ...
        yylval->lex_str.length = 1;
        lip->yySkip();  // Skip '@'
        lip->next_state = (state_map[lip->yyPeek()] == MY_LEX_USER_VARIABLE_DELIMITER ? MY_LEX_OPERATOR_OR_IDENT
...
st_select_lex::st_select_lex(TABLE_LIST *table_list,
  ...
  while (sl)
  {
    ...
    for (SELECT_LEX_UNIT *u = sl->first_inner_unit(); u; u = u->next_unit())
    {
      ...
      */
      ...
      if (u->fake_select_lex && u->fake_select_lex->context.outer_context == &sl->context)
...
void st_select_lex::make_active_options(ulonglong added_options, ulonglong removed_options)
{
  m_active_options = (m_base_options | added_options | parent_lex->thd->variables.option_bits) & ~removed_options;
}
  ...
  */
  for (SELECT_LEX *s = this; s && s != last; s = s->outer_select())
  {
    if (!(s->uncacheable & UNCACHEABLE_DEPENDENT))
    {
      ...
      s->uncacheable = (s->uncacheable & ~UNCACHEABLE_UNITED) | UNCACHEABLE_DEPENDENT;
      SELECT_LEX_UNIT *munit = s->master_unit();
      munit->uncacheable = (munit->uncacheable & ~UNCACHEABLE_UNITED) | UNCACHEABLE_DEPENDENT;
      for (SELECT_LEX *sl = munit->first_select(); sl; sl = sl->next_select())
      {
        if (sl != s && !(sl->uncacheable & (UNCACHEABLE_DEPENDENT | UNCACHEABLE_UNITED)))
...
void st_select_lex_unit::set_explain_marker_from(const st_select_lex_unit *u)
{
  thd->lock_query_plan();
...
bool st_select_lex::setup_ref_array(THD *thd)
{
  ...
  if (is_distinct())
  {
    ...
    while ((item = li++))
    {
      ...
      */
      if (item->fixed && item->type() == Item::FIELD_ITEM && item->field_type() == MYSQL_TYPE_BIT)
        ++bitcount;
    }
  ...
  */
  ...
  const uint n_elems = (n_sum_items + n_child_sum_items + item_list.elements + select_n_having_items +
                        select_n_where_fields + order_group_num) *
                       5;
  DBUG_PRINT("info", ("setup_ref_array this %p %4u : %4u %4u %4u %4u %4u %4u", this,
                      n_elems,  // :
                      n_sum_items, n_child_sum_items, item_list.elements, select_n_having_items, select_n_where_fields,
                      order_group_num));
...
void st_select_lex::print_limit(THD *thd, String *str, enum_query_type query_type)
{
  ...
  if (item && unit->global_parameters() == this)
  {
    ...
    if (subs_type == Item_subselect::EXISTS_SUBS || subs_type == Item_subselect::IN_SUBS ||
...
void Index_hint::print(THD *thd, String *str)
{
  ...
  if (key_name.length)
  {
    if (thd && !my_strnncoll(system_charset_info, (const uchar *)key_name.str, key_name.length,
                             (const uchar *)primary_key_name, strlen(primary_key_name)))
...
static void print_table_array(THD *thd, String *str, TABLE_LIST **table, TABLE_LIST **end, enum_query_type query_type)
{
  ...
  for (TABLE_LIST **tbl = table + 1; tbl < end; tbl++)
  {
    ...
    Item *const cond = (curr->select_lex->join && curr->select_lex->join->is_optimized()) ? curr->join_cond_optim()
                                                                                          : curr->join_cond();
    if (cond)
    {
...
static void print_join(THD *thd, String *str, List<TABLE_LIST> *tables, enum_query_type query_type)
{
...
{
  return thd != NULL && thd->db().str != NULL && thd->db().length == db_len && !memcmp(db, thd->db().str, db_len);
}
  ...
  else
  {
    ...
    if (view_name.str)
    {
      ...
      if (!(query_type & QT_NO_DB) &&
          !((query_type & QT_NO_DEFAULT_DB) && db_is_default_db(view_db.str, view_db.length, thd)))
      {
    ...
    else
    {
      ...
      if (!(query_type & QT_NO_DB) && !((query_type & QT_NO_DEFAULT_DB) && db_is_default_db(db, db_length, thd)))
      {
...
void st_select_lex::print(THD *thd, String *str, enum_query_type query_type)
{
  ...
  */
  assert(!(master_unit()->cleaned == SELECT_LEX_UNIT::UC_CLEAN && is_explainable_query(thd->lex->sql_command)));
  ...
  */
  ...
  // Where
  Item *const cur_where = (join && join->is_optimized()) ? join->where_cond : m_where_cond;
  ...
  // having
  Item *const cur_having = (join && join->having_for_explain != (Item *)1) ? join->having_for_explain : m_having_cond;
...
bool walk_item(Item *item, Select_lex_visitor *visitor)
{
  if (item == NULL)
    return false;
  return item->walk(&Item::visitor_processor, get_walk_flags(visitor), pointer_cast<uchar *>(visitor));
}

static bool accept_for_order(SQL_I_List<ORDER> orders, Select_lex_visitor *visitor)
{
...
static bool accept_for_join(List<TABLE_LIST> *tables, Select_lex_visitor *visitor)
{
...
bool st_select_lex::accept(Select_lex_visitor *visitor)
{
  ...
  // Having clause
  Item *having_condition = join != NULL ? join->having_for_explain : m_having_cond;
...
bool st_select_lex_unit::is_mergeable() const
{
  ...
  return !select->is_grouped() && !select->having_cond() && !select->is_distinct() && select->table_list.elements > 0 &&
         !select->has_limit() && thd->lex->set_var_list.elements == 0;
}
  ...
  */
  ...
  switch (sql_command)
  {
    case SQLCOM_LOCK_TABLES:
      ...
      */
      new_trg_event_map = static_cast<uint8>(1 << static_cast<int>(TRG_EVENT_INSERT)) |
    ...
    */
    ...
    case SQLCOM_UPDATE_MULTI:
      new_trg_event_map |= static_cast<uint8>(1 << static_cast<int>(TRG_EVENT_UPDATE));
      break;
    /* Basic delete and multi-delete */
    case SQLCOM_DELETE: /* fall through */
    case SQLCOM_DELETE_MULTI:
      new_trg_event_map |= static_cast<uint8>(1 << static_cast<int>(TRG_EVENT_DELETE));
      break;
    default:
      break;
  }
  ...
  */
  ...
  while (tables)
  {
    /*
      ...
      parsing.
    */
    if (static_cast<int>(tables->lock_type) >= static_cast<int>(TL_WRITE_ALLOW_WRITE))
...
void st_select_lex::fix_prepare_information_for_order(THD *thd, SQL_I_List<ORDER> *list, Group_list_ptrs **list_ptrs)
{
...
st_select_lex::type_enum st_select_lex::type()
{
  ...
  else if (!master_unit()->outer_select() && master_unit()->first_select() == this)
  {
...
*/
void st_select_lex::include_standalone(st_select_lex_unit *outer, st_select_lex **ref)
{
  next = NULL;
  prev = ref;
  master = outer;
...
void st_select_lex::include_chain_in_global(st_select_lex **start)
{
  ...
  for (last_select = this; last_select->link_next != NULL; last_select = last_select->link_next)
  {
  }
...
static bool get_optimizable_join_conditions(THD *thd, List<TABLE_LIST> &join_list)
{
  ...
  while ((table = li++))
  {
    ...
    if (nested_join && get_optimizable_join_conditions(thd, nested_join->join_list))
      return true;
...
/**
   ...
   @returns true if OOM
*/
bool SELECT_LEX::get_optimizable_conditions(THD *thd, Item **new_where, Item **new_having)
{
  /*
...
Item_exists_subselect::enum_exec_method st_select_lex::subquery_strategy(THD *thd) const
{
  if (opt_hints_qb)
  {
    Item_exists_subselect::enum_exec_method strategy = opt_hints_qb->subquery_strategy();
...
{
  return opt_hints_qb ? opt_hints_qb->semijoin_enabled(thd) : thd->optimizer_switch_flag(OPTIMIZER_SWITCH_SEMIJOIN);
}

void st_select_lex::update_semijoin_strategies(THD *thd)
{
  uint sj_strategy_mask = OPTIMIZER_SWITCH_FIRSTMATCH | OPTIMIZER_SWITCH_LOOSE_SCAN | OPTIMIZER_SWITCH_MATERIALIZATION |
  ...
  while ((sj_nest = sj_list_it++))
  {
    ...
    */
    ...
    sj_nest->nested_join->sj_enabled_strategies =
        table->opt_hints_qb ? table->opt_hints_qb->sj_enabled_strategies(opt_switches) : opt_switches;
  }
}
...
bool st_select_lex::validate_outermost_option(LEX *lex, const char *option) const
{
...
{
  assert(!(options_arg &
           ~(SELECT_STRAIGHT_JOIN | SELECT_HIGH_PRIORITY | SELECT_DISTINCT | SELECT_ALL | SELECT_SMALL_RESULT |
             SELECT_BIG_RESULT | OPTION_BUFFER_RESULT | OPTION_FOUND_ROWS | OPTION_TO_QUERY_CACHE)));
  ...
  if (options_arg & SELECT_HIGH_PRIORITY && validate_outermost_option(lex, "HIGH_PRIORITY"))
    return true;
  if (options_arg & OPTION_BUFFER_RESULT && validate_outermost_option(lex, "SQL_BUFFER_RESULT"))
    return true;
  if (options_arg & OPTION_FOUND_ROWS && validate_outermost_option(lex, "SQL_CALC_FOUND_ROWS"))
    return true;
...
{
  return (sql_command == SQLCOM_ALTER_TABLE && (alter_info.flags == Alter_info::ALTER_ADD_PARTITION ||
...
bool LEX::accept(Select_lex_visitor *visitor)
{
  ...
  {
    List_iterator<List_item> row_it(static_cast<Sql_cmd_insert_base *>(m_sql_cmd)->insert_many_values);
...
void st_lex_master_info::initialize()
{
  ...
  ssl = ssl_verify_server_cert = heartbeat_opt = repl_ignore_server_ids_opt = retry_count_opt = auto_position =
      port_opt = LEX_MI_UNCHANGED;
