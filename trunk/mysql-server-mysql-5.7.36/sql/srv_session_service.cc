...
extern "C"
{
  ...
  */
  Srv_session *srv_session_open(srv_session_error_cb error_cb, void *plugin_ctx)
  {
    ...
    {
      if (error_cb)
        error_cb(plugin_ctx, ER_SERVER_ISNT_AVAILABLE, ER_DEFAULT(ER_SERVER_ISNT_AVAILABLE));
      DBUG_RETURN(NULL);
    }

    bool simulate_reach_max_connections = false;
    DBUG_EXECUTE_IF("simulate_reach_max_connections", simulate_reach_max_connections = true;);

    Connection_handler_manager *conn_manager = Connection_handler_manager::get_instance();

    if (simulate_reach_max_connections || !conn_manager->check_and_incr_conn_count())
    {
      if (error_cb)
