...
int fill_query_profile_statistics_info(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  const char *old = thd->lex->sql_command == SQLCOM_SHOW_PROFILE ? "SHOW PROFILE" : "INFORMATION_SCHEMA.PROFILING";
...
*/
void QUERY_PROFILE::set_query_source(const char *query_source_arg, size_t query_length_arg)
{
  ...
  {
    m_query_source.str = my_strndup(key_memory_PROFILE, query_source_arg, length, MYF(0));
...
*/
void PROFILING::status_change(const char *status_arg, const char *function_arg, const char *file_arg,
                              unsigned int line_arg)
{
...
void PROFILING::finish_current_query()
{
  ...
  if (current != NULL)
  {
    ...
    if ((enabled) &&                                              /* ON at start? */
        ...
        (current->m_query_source.str != NULL) && (!current->entries.is_empty()))
    {
...
bool PROFILING::show_profiles()
{
  ...
  field_list.push_back(new Item_return_int("Duration", TIME_FLOAT_DIGITS - 1, MYSQL_TYPE_DOUBLE));
  ...
  void *iterator;
  for (iterator = history.new_iterator(); iterator != NULL; iterator = history.iterator_next(iterator))
  {
    ...
    protocol->store((double)(query_time_usecs / (1000.0 * 1000)), (uint32)TIME_FLOAT_DIGITS - 1, &elapsed);
    if (prof->m_query_source.str != NULL)
      protocol->store(prof->m_query_source.str, prof->m_query_source.length, system_charset_info);
    else
      protocol->store_null();
