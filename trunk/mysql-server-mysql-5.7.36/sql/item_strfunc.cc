...
String *Item_str_func::val_str_from_val_str_ascii(String *str, String *str2)
{
  ...
  if ((null_value = str2->copy(res->ptr(), res->length(), &my_charset_latin1, collation.collation, &errors)))
    return 0;
...
my_decimal *Item_str_func::val_decimal(my_decimal *decimal_value)
{
  ...
  if (!res)
    return 0;
  (void)str2my_decimal(E_DEC_FATAL_ERROR, (char *)res->ptr(), res->length(), res->charset(), decimal_value);
...
String *Item_func_sha2::val_str_ascii(String *str)
{
  ...
  const unsigned char *input_ptr = pointer_cast<const unsigned char *>(input_string->ptr());
  ...
  switch (hash_length)
  {
    ...
    default:
      ...
      if (!args[1]->const_item())
        push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WRONG_PARAMETERS_TO_NATIVE_FCT,
...
#else
  push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_FEATURE_DISABLED, ER(ER_FEATURE_DISABLED), "sha2",
                      "--with-ssl");
  null_value = TRUE;
  return (String *)NULL;
...
class iv_argument
{
  ...
  */
  const unsigned char *retrieve_iv_ptr(enum my_aes_opmode aes_opmode, uint arg_count, Item **args,
                                       const char *func_name, THD *thd, my_bool *error_generated)
  {
    ...
    *error_generated = FALSE;
    ...
    else
    {
      if (arg_count == 3)
      {
        push_warning_printf(thd, Sql_condition::SL_WARNING, WARN_OPTION_IGNORED, ER(WARN_OPTION_IGNORED), "IV");
      }
    }
    return iv_str;
  }
...
String *Item_func_aes_encrypt::val_str(String *str)
{
  ...
  if (sptr && key)  // we need both arguments to be not NULL
  {
    const unsigned char *iv_str =
        iv_arg.retrieve_iv_ptr((enum my_aes_opmode)aes_opmode, arg_count, args, func_name(), thd, &null_value);
    if (null_value)
      DBUG_RETURN(NULL);

    // Calculate result length
    aes_length = my_aes_get_size(sptr->length(), (enum my_aes_opmode)aes_opmode);

    str_value.set_charset(&my_charset_bin);
    if (!str_value.alloc(aes_length))  // Ensure that memory is free
    {
      // finally encrypt directly to allocated buffer.
      if (my_aes_encrypt((unsigned char *)sptr->ptr(), sptr->length(), (unsigned char *)str_value.ptr(),
                         (unsigned char *)key->ptr(), key->length(), (enum my_aes_opmode)aes_opmode,
                         iv_str) == aes_length)
      {
...
void Item_func_aes_encrypt::fix_length_and_dec()
{
  ...
  max_length = my_aes_get_size(args[0]->max_length, (enum my_aes_opmode)aes_opmode);
}
...
String *Item_func_aes_decrypt::val_str(String *str)
{
  ...
  if (sptr && key)  // Need to have both arguments not NULL
  {
    ...
    if (!str_value.alloc(sptr->length()))  // Ensure that memory is free
    {
      ...
      int length;
      length = my_aes_decrypt((unsigned char *)sptr->ptr(), sptr->length(), (unsigned char *)str_value.ptr(),
                              (unsigned char *)key->ptr(), key->length(), (enum my_aes_opmode)aes_opmode, iv_str);
...
String *Item_func_random_bytes::val_str(String *a)
{
  ...
  {
    my_error(ER_ERROR_WHEN_EXECUTING_COMMAND, MYF(0), func_name(), "SSL library can't generate random bytes");
    null_value = TRUE;
    return NULL;
  }
...
String *Item_func_to_base64::val_str_ascii(String *str)
{
  ...
  if (!res || res->length() > (uint)base64_encode_max_arg_length() ||
      (too_long = ((length = base64_needed_encoded_length((uint64)res->length())) >
...
String *Item_func_from_base64::val_str(String *str)
{
  ...
  if (!res || res->length() > (uint)base64_decode_max_arg_length() ||
      (too_long = ((uint64)(length = base64_needed_decoded_length((uint64)res->length())) >
                   current_thd->variables.max_allowed_packet)) ||
      tmp_value.alloc((uint)length) ||
      (length = base64_decode(res->ptr(), (uint64)res->length(), (char *)tmp_value.ptr(), &end_ptr, 0)) < 0 ||
...
String *Item_func_concat::val_str(String *str)
{
  ...
  for (uint i = 0; i < arg_count; ++i)
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
...
String *Item_func_des_encrypt::val_str(String *str)
{
  ...
  */
  ...
  // Real encryption
  memset(&ivec, 0, sizeof(ivec));
  DES_ede3_cbc_encrypt((const uchar *)(tmp_arg.ptr()), (uchar *)(tmp_value.ptr() + 1), res_length, &keyschedule.ks1,
                       &keyschedule.ks2, &keyschedule.ks3, &ivec, TRUE);
  return &tmp_value;

error:
  push_warning_printf(current_thd, Sql_condition::SL_WARNING, code, ER(code), "des_encrypt");
#else
  push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_FEATURE_DISABLED, ER(ER_FEATURE_DISABLED),
...
String *Item_func_des_decrypt::val_str(String *str)
{
  ...
  if (arg_count == 1)  // If automatic uncompression
  {
    ...
    if (!(current_thd->security_context()->check_access(SUPER_ACL)) || key_number > 9)
      goto error;
...
error:
  push_warning_printf(current_thd, Sql_condition::SL_WARNING, code, ER(code), "des_decrypt");
wrong_key:
#else
  push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_FEATURE_DISABLED, ER(ER_FEATURE_DISABLED),
...
String *Item_func_concat_ws::val_str(String *str)
{
  ...
  for (i++; i < arg_count; i++)
  {
    ...
    if (tmp_value.length() + sep_str->length() + res2->length() > thd->variables.max_allowed_packet)
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                          ER(ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(), thd->variables.max_allowed_packet);
      null_value = 1;
...
String *Item_func_reverse::val_str(String *str)
{
  ...
  if (!res->length())
    return make_empty_result();
  if (tmp_value.alloced_length() < res->length() && tmp_value.mem_realloc(res->length()))
  {
    null_value = 1;
    return 0;
  }
    ...
    while (ptr < end)
    {
      if (*ptr == *search)
      {
        ...
        if (!alloced)
        {
          ...
          {
            if (tmp_value_res.alloc(res->length() + to_length) || tmp_value_res.copy(*res))
              goto null;
            res = &tmp_value_res;
          }
          else
...
String *Item_func_insert::val_str(String *str)
{
  ...
  */
  ...
  if ((ulonglong)(orig_len - length + res2->length()) > (ulonglong)current_thd->variables.max_allowed_packet)
  {
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
...
String *Item_func_trim::val_str(String *str)
{
  ...
  if (remove_length == 0 || remove_length > res->length())
    return res;
  ...
  if (ptr == res->ptr() && end == ptr + res->length())
    return res;
  tmp_value.set(*res, static_cast<uint>(ptr - res->ptr()), static_cast<uint>(end - ptr));
  return &tmp_value;
}
...
static size_t calculate_password(String *str, char *buffer)
{
  ...
  if (old_passwords == 2)
  {
    ...
    my_make_scrambled_password(buffer, str->ptr(), str->length());
...
String *Item_func_password::val_str_ascii(String *str)
{
  ...
  str->set(m_hashed_password_buffer, m_hashed_password_buffer_len, default_charset());
...
bool Item_func_user::fix_fields(THD *thd, Item **ref)
{
  return (Item_func_sysconst::fix_fields(thd, ref) ||
          init(thd->m_main_security_ctx.user().str, thd->m_main_security_ctx.host_or_ip().str));
}
  ...
  */
  for (nchars = 1;;)
  {
    ...
    if ((ch != '0') && (ch != last_ch))  // if not skipped or double
    {
      ...
      if ((rc = cs->cset->wc_mb(cs, (my_wc_t)ch, (uchar *)to, (uchar *)to_end)) <= 0)
      {
        // Extra safety - should not really happen
        assert(false);
        break;
...
{
  fix_length_and_charset(Item_func_geohash::upper_limit_output_length, default_charset());
}
...
bool Item_func_geohash::fix_fields(THD *thd, Item **ref)
{
  ...
  if (arg_count == 2)
  {
    ...
    */
    ...
    maybe_null = (args[0]->maybe_null || args[1]->maybe_null);
    if (!is_item_null(args[0]) && args[0]->field_type() != MYSQL_TYPE_GEOMETRY && args[0]->type() != PARAM_ITEM &&
  ...
  */
  ...
  bool is_binary_charset = (args[geohash_length_arg_index]->collation.collation == &my_charset_bin);
  bool is_parameter =
      (args[geohash_length_arg_index]->type() == PARAM_ITEM || args[geohash_length_arg_index]->type() == INT_ITEM);
...
bool Item_func_geohash::check_valid_latlong_type(Item *arg)
{
  ...
  */
  bool is_parameter = (arg->type() == INT_ITEM || arg->type() == DECIMAL_ITEM || arg->type() == REAL_ITEM ||
                       arg->type() == STRING_ITEM) &&
                      (arg->field_type() == MYSQL_TYPE_VARCHAR);
  bool is_parameter_marker = (arg->type() == PARAM_ITEM && arg->field_type() == MYSQL_TYPE_VARCHAR);
...
bool Item_func_geohash::is_item_null(Item *item)
{
  ...
  if (is_binary_charset && item->type() == FUNC_ITEM && item->field_type() == MYSQL_TYPE_MEDIUM_BLOB)
  {
    return true;
  }
  return false;
...
bool Item_func_geohash::fill_and_check_fields()
{
  ...
  if (arg_count == 2)
  {
    ...
    else
    {
      ...
      else if (geom->get_type() != Geometry::wkb_point || geom->get_x(&longitude) || geom->get_y(&latitude))
      {
        my_error(ER_INCORRECT_TYPE, MYF(0), "point", func_name());
        return true;
      }
  ...
  else if (arg_count == 3)
  {
    ...
    */
    ...
    if ((null_value = args[0]->null_value || args[1]->null_value || args[2]->null_value))
      return true;
  }
...
String *Item_func_geohash::val_str_ascii(String *str)
{
  ...
  for (uint i = 0; i < geohash_max_output_length; i++)
  {
    ...
    */
    ...
    for (uint bit_number = 0; bit_number < 5; bit_number++)
    {
      if (even_bit)
      {
        ...
        encode_bit(&upper_longitude, &lower_longitude, longitude, &current_char, bit_number);
      }
      else
      {
        // Encode one latitude bit.
        encode_bit(&upper_latitude, &lower_latitude, latitude, &current_char, bit_number);
      }
      even_bit = !even_bit;
    }
    ...
    */
    if (latitude == (lower_latitude + upper_latitude) / 2.0 && longitude == (lower_longitude + upper_longitude) / 2.0)
      break;
  }
  return str;
}
...
*/
void Item_func_geohash::encode_bit(double *upper_value, double *lower_value, double target_value, char *char_value,
                                   int bit_number)
{
...
MY_LOCALE *Item_func_format::get_locale(Item *item)
{
  ...
  {
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_UNKNOWN_LOCALE, ER(ER_UNKNOWN_LOCALE),
...
double Item_func_elt::val_real()
{
  ...
  uint tmp;
  null_value = 1;
  if ((tmp = (uint)args[0]->val_int()) == 0 || args[0]->null_value || tmp >= arg_count)
    return 0.0;
...
void Item_func_make_set::split_sum_func(THD *thd, Ref_ptr_array ref_pointer_array, List<Item> &fields)
{
      ...
      if (res)  // Skip nulls
      {
        ...
        else
        {
          ...
          {  // Copy data to tmp_str
            if (tmp_str.alloc((result != NULL ? result->length() : 0) + res->length() + 1) || tmp_str.copy(*result))
              return make_empty_result();
            result = &tmp_str;
          }
...
inline String *alloc_buffer(String *res, String *str, String *tmp_value, size_t length)
{
...
String *Item_func_rpad::val_str(String *str)
{
  ...
  */
  ...
  /*
    ...
    at this stage.
  */
  if (!(res = alloc_buffer(res, str, &tmp_value, static_cast<size_t>(byte_count))))
  {
    null_value = true;
    return NULL;
  }
...
bool Item_func_set_collation::itemize(Parse_context *pc, Item **res)
{
  if (skip_itemize(res))
    return false;
  THD *thd = pc->thd;
  args[1] = new (pc->mem_root) Item_string(collation_string.str, collation_string.length, thd->charset());
  if (args[1] == NULL)
    return true;
...
void Item_func_set_collation::fix_length_and_dec()
{
  ...
  collation.set(set_collation, DERIVATION_EXPLICIT, args[0]->collation.repertoire);
  max_length = args[0]->max_length;
}
...
bool Item_func_weight_string::itemize(Parse_context *pc, Item **res)
{
  ...
  if (as_binary)
  {
    if (args[0]->itemize(pc, &args[0]))
      return true;
    args[0] = new (pc->mem_root) Item_char_typecast(args[0], nweights, &my_charset_bin);
    if (args[0] == NULL)
      return true;
  }
  return super::itemize(pc, res);
...
void Item_func_weight_string::fix_length_and_dec()
{
  ...
  field = args[0]->type() == FIELD_ITEM && args[0]->is_temporal() ? ((Item_field *)(args[0]))->field : (Field *)NULL;
  /*
...
bool Item_func_weight_string::eq(const Item *item, bool binary_cmp) const
{
  ...
  if (nweights != wstr->nweights || flags != wstr->flags)
    return 0;
...
String *Item_func_weight_string::val_str(String *str)
{
  ...
  if (args[0]->result_type() != STRING_RESULT || !(res = args[0]->val_str(str)))
    goto nl;
  ...
  */
  tmp_length = field           ? field->pack_length()
               : result_length ? result_length
                               : cs->coll->strnxfrmlen(cs, cs->mbmaxlen * max<size_t>(res->length(), nweights));
  ...
  if (field)
  {
    ...
    frm_length = cs->coll->strnxfrm(cs, (uchar *)tmp_value.ptr(), tmp_length, nweights ? nweights : tmp_length,
                                    (const uchar *)res->ptr(), res->length(), flags);
...
String *Item_func_hex::val_str_ascii(String *str)
{
  ...
  if (args[0]->result_type() != STRING_RESULT)
  {
    ...
    if (args[0]->result_type() == REAL_RESULT || args[0]->result_type() == DECIMAL_RESULT)
    {
      ...
      if ((val <= (double)LLONG_MIN) || (val >= (double)(ulonglong)ULLONG_MAX))
        dec = ~(longlong)0;
      else
    ...
    if (!(ptr = longlong2str(dec, ans, 16)) || str->copy(ans, (uint32)(ptr - ans), &my_charset_numeric))
...
err:
  ...
  push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WRONG_VALUE_FOR_TYPE,
                      ER_THD(current_thd, ER_WRONG_VALUE_FOR_TYPE), "string", err.c_ptr_safe(), func_name());
...
String *Item_func_like_range::val_str(String *str)
{
  ...
  if (cs->coll->like_range(cs, res->ptr(), res->length(), '\\', '_', '%', nbytes, (char *)min_str.ptr(),
...
String *Item_char_typecast::val_str(String *str)
{
  ...
  if (cast_length >= 0 && ((unsigned)cast_length) > current_thd->variables.max_allowed_packet)
  {
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
  ...
  else
  {
    ...
    uint dummy_errors;
    if (!(res = args[0]->val_str(str)) || tmp_value.copy(res->ptr(), res->length(), from_cs, cast_cs, &dummy_errors))
    {
      null_value = 1;
      return 0;
    }
  ...
  if (cast_length >= 0)
  {
    ...
    {  // Safe even if const arg
      ...
      push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_TRUNCATED_WRONG_VALUE,
                          ER(ER_TRUNCATED_WRONG_VALUE), char_type, err.ptr());
      res->length(length);
    }
    else if (cast_cs == &my_charset_bin && res->length() < (uint)cast_length)
    {
      ...
      memset(const_cast<char *>(res->ptr() + res->length()), 0, cast_length - res->length());
...
void Item_char_typecast::fix_length_and_dec()
{
  ...
  */
  from_cs = args[0]->collation.repertoire == MY_REPERTOIRE_ASCII && my_charset_is_ascii_based(cast_cs) &&
...
String *Item_load_file::val_str(String *str)
{
  ...
  if ((file = mysql_file_open(key_file_loadfile, file_name->ptr(), O_RDONLY, MYF(0))) < 0)
    goto err;
  if (mysql_file_read(file, (uchar *)tmp_value.ptr(), stat_info.st_size, MYF(MY_NABP)))
  {
...
String *Item_func_export_set::val_str(String *str)
{
  ...
  */
  ...
  const ulonglong max_total_length = num_set_values * max(yes->length(), no->length()) + num_separators * sep->length();

  if (unlikely(max_total_length > max_allowed_packet))
  {
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_WARN_ALLOWED_PACKET_OVERFLOWED,
                        ER(ER_WARN_ALLOWED_PACKET_OVERFLOWED), func_name(), static_cast<long>(max_allowed_packet));
    null_value = true;
    return NULL;
  }
...
void Item_func_export_set::fix_length_and_dec()
{
  ...
  if (agg_arg_charsets_for_string_result(collation, args + 1, min(4U, arg_count) - 1))
    return;
...
longlong Item_func_uncompressed_length::val_int()
{
  ...
  */
  if (res->length() <= 4)
  {
    push_warning(current_thd, Sql_condition::SL_WARNING, ER_ZLIB_Z_DATA_ERROR, ER(ER_ZLIB_Z_DATA_ERROR));
    return 0;
  }
...
String *Item_func_compress::val_str(String *str)
{
  ...
  */
  ...
  if (((new_size + 5) <= res->length()) || buffer.mem_realloc(new_size + 4 + 1))
  {
    null_value = 1;
    return 0;
  }
...
String *Item_func_uncompress::val_str(String *str)
{
  ...
  {
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_TOO_BIG_FOR_UNCOMPRESS,
                        ER(ER_TOO_BIG_FOR_UNCOMPRESS), static_cast<int>(current_thd->variables.max_allowed_packet));
    goto err;
  }
  if (buffer.mem_realloc((uint32)new_size))
    goto err;

  if ((err = uncompress(pointer_cast<Byte *>(const_cast<char *>(buffer.ptr())), &new_size,
                        pointer_cast<const Bytef *>(res->ptr()) + 4, res->length() - 4)) == Z_OK)
  {
...
String *Item_func_uuid::val_str(String *str)
{
  ...
  if (!uuid_time) /* first UUID() call. initializing data */
  {
    ...
    randominit(&uuid_rand, tmp + (ulong)server_start_time, tmp + (ulong)thd->status_var.bytes_sent);
    set_clock_seq_str();
  }
  ...
  DBUG_EXECUTE_IF("force_fake_uuid", my_stpcpy(s, "a2d00942-b69c-11e4-a696-0020ff6fcbe6"););
  return str;
}
...
void Item_func_gtid_subtract::fix_length_and_dec()
{
  ...
  */
  fix_char_length_ulonglong(args[0]->max_length +
                            max<ulonglong>(args[1]->max_length - binary_log::Uuid::TEXT_LENGTH, 0) * 5 / 2);
}
...
String *Item_func_gtid_subtract::val_str_ascii(String *str)
{
  ...
  */
  if ((str1 = args[0]->val_str_ascii(&buf1)) != NULL && (charp1 = str1->c_ptr_safe()) != NULL &&
      (str2 = args[1]->val_str_ascii(&buf2)) != NULL && (charp2 = str2->c_ptr_safe()) != NULL && !args[0]->null_value &&
      !args[1]->null_value)
  {
