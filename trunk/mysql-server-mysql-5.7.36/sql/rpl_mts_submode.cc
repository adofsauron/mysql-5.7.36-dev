...
/**
 ...
 @return: 1  if  error
          0 no error
*/
int Mts_submode_database::schedule_next_event(Relay_log_info *rli, Log_event *ev)
{
  /*nothing to do here*/
  return 0;
}
...
/**
  ...
  @return: void
*/
void Mts_submode_database::attach_temp_tables(THD *thd, const Relay_log_info *rli, Query_log_event *ev)
{
  int i, parts;
  ...
  parts = ((ev->mts_accessed_dbs == OVER_MAX_DBS_IN_EVENT_MTS) ? 1 : ev->mts_accessed_dbs);
  for (i = 0; i < parts; i++)
  {
    mts_move_temp_tables_to_thd(thd, ev->mts_assigned_partitions[i]->temporary_tables);
...
int Mts_submode_database::wait_for_workers_to_finish(Relay_log_info *rli, Slave_worker *ignore)
{
  uint ret = 0;
...
/**
 ...
 @return: void
 */
void Mts_submode_database::detach_temp_tables(THD *thd, const Relay_log_info *rli, Query_log_event *ev)
{
  int i, parts;
  ...
  for (TABLE *table = thd->temporary_tables; table;)
  {
    ...
    for (i = 0; i < parts; i++)
    {
      ...
      else
      {
        ...
        if (!rpl_filter->is_rewrite_empty() && strcmp(table->s->db.str, db_name))
          continue;
        else
          break;
      }
 ...
 */
Slave_worker *Mts_submode_database::get_least_occupied_worker(Relay_log_info *rli, Slave_worker_array *ws,
                                                              Log_event *ev)
{
  long usage = LONG_MAX;
  ...
  if (DBUG_EVALUATE_IF("mts_distribute_round_robin", 1, 0))
  {
    ...
    sql_print_information(
        "Chosing worker id %lu, the following is"
        " going to be %lu",
        worker->id, static_cast<ulong>(w_rr % ws->size()));
    assert(worker != NULL);
    DBUG_RETURN(worker);
  }
#endif
...
longlong Mts_submode_logical_clock::get_lwm_timestamp(Relay_log_info *rli, bool need_lock)
{
  ...
  */
  ...
  last_lwm_index = rli->gaq->find_lwm(&ptr_g,
                                      ...
                                      */
                                      lwm_estim == SEQ_UNINIT || (is_stale = clock_leq(last_lwm_timestamp, lwm_estim))
                                          ? rli->gaq->entry
                                          : last_lwm_index);
  /*
...
*/
bool Mts_submode_logical_clock::wait_for_last_committed_trx(Relay_log_info *rli, longlong last_committed_arg,
                                                            longlong lwm_estimate_arg)
{
  ...
  */
  if ((!rli->info_thd->killed && !is_error) && !clock_leq(last_committed_arg, get_lwm_timestamp(rli, true)))
  {
    PSI_stage_info old_stage;
    ...
    thd->ENTER_COND(&rli->logical_clock_cond, &rli->mts_gaq_LOCK, &stage_worker_waiting_for_commit_parent, &old_stage);
    do
    {
      mysql_cond_wait(&rli->logical_clock_cond, &rli->mts_gaq_LOCK);
    } while ((!rli->info_thd->killed && !is_error) && !clock_leq(last_committed_arg, estimate_lwm_timestamp()));
...
int Mts_submode_logical_clock::schedule_next_event(Relay_log_info *rli, Log_event *ev)
{
  ...
  */
  switch (ev->get_type_code())
  {
    ...
    case binary_log::ANONYMOUS_GTID_LOG_EVENT:
      ...
      ptr_group->sequence_number = sequence_number = static_cast<Gtid_log_event *>(ev)->sequence_number;
      ptr_group->last_committed = last_committed = static_cast<Gtid_log_event *>(ev)->last_committed;
      break;
  ...
  DBUG_PRINT("info", ("sequence_number %lld, last_committed %lld", sequence_number, last_committed));
  ...
  {
    if (unlikely(clock_leq(sequence_number, last_committed) && last_committed != SEQ_UNINIT))
    {
      /* inconsistent (buggy) timestamps */
      sql_print_error(
          "Transaction is tagged with inconsistent logical "
          "timestamps: "
          "sequence_number (%lld) <= last_committed (%lld)",
          sequence_number, last_committed);
      DBUG_RETURN(ER_MTS_CANT_PARALLEL);
    }
    if (unlikely(clock_leq(sequence_number, last_sequence_number) && sequence_number != SEQ_UNINIT))
    {
      /* inconsistent (buggy) timestamps */
      sql_print_error(
          "Transaction's sequence number is inconsistent with that "
          "of a preceding one: "
  ...
  */
  is_new_group = (/* First event after a submode switch; */
                  first_event ||
...
/**
 ...
 @return: void
 */
void Mts_submode_logical_clock::attach_temp_tables(THD *thd, const Relay_log_info *rli, Query_log_event *ev)
{
  bool shifted = false;
  ...
  do
  {
    ...
    if (thd->server_id == st_id_pair.first && thd->variables.pseudo_thread_id == st_id_pair.second)
    {
...
/**
 ...
 @return: void
 */
void Mts_submode_logical_clock::detach_temp_tables(THD *thd, const Relay_log_info *rli, Query_log_event *ev)
{
...
Slave_worker *Mts_submode_logical_clock::get_least_occupied_worker(Relay_log_info *rli, Slave_worker_array *ws,
                                                                   Log_event *ev)
{
  ...
  if (rli->last_assigned_worker)
  {
    ...
    assert(ev->get_type_code() != binary_log::USER_VAR_EVENT || worker->id == 0 || rli->curr_group_seen_begin ||
  ...
  else
  {
    ...
    assert(ev->get_type_code() != binary_log::USER_VAR_EVENT || rli->curr_group_seen_begin ||
           rli->curr_group_seen_gtid);
    ...
    if (worker == NULL)
    {
      ...
      thd->enter_stage(&stage_slave_waiting_for_workers_to_process_queue, old_stage, __func__, __FILE__, __LINE__);
