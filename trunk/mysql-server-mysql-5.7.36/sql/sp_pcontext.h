...
class sp_variable : public Sql_alloc
{
 ...
 public:
  sp_variable(LEX_STRING _name, enum_field_types _type, enum_mode _mode, uint _offset)
      : Sql_alloc(), name(_name), type(_type), mode(_mode), offset(_offset), default_value(NULL)
  {
  }
};
...
class sp_condition_value : public Sql_alloc
{
 ...
 public:
  sp_condition_value(uint _mysqlerr) : Sql_alloc(), type(ERROR_CODE), mysqlerr(_mysqlerr) {}

  sp_condition_value(const char *_sql_state) : Sql_alloc(), type(SQLSTATE)
  {
    memcpy(sql_state, _sql_state, SQLSTATE_LENGTH);
    sql_state[SQLSTATE_LENGTH] = 0;
  }

  sp_condition_value(enum_type _type) : Sql_alloc(), type(_type) { assert(type != ERROR_CODE && type != SQLSTATE); }
...
class sp_condition : public Sql_alloc
{
 ...
 public:
  sp_condition(LEX_STRING _name, sp_condition_value *_value) : Sql_alloc(), name(_name), value(_value) {}
};
...
class sp_handler : public Sql_alloc
{
 ...
 public:
  ...
  sp_handler(enum_type _type, sp_pcontext *_scope) : Sql_alloc(), type(_type), scope(_scope) {}
...
class sp_pcontext : public Sql_alloc
{
 ...
 public:
  ...
  uint current_var_count() const { return m_var_offset + static_cast<uint>(m_vars.size()); }
