...
ulonglong find_set(TYPELIB *lib, const char *str, size_t length, const CHARSET_INFO *cs, char **err_pos, uint *err_len,
                   bool *set_warning)
{
...
uint find_type2(const TYPELIB *typelib, const char *x, size_t length, const CHARSET_INFO *cs)
{
  ...
  DBUG_PRINT("enter", ("x: '%.*s'  lib: 0x%p", static_cast<int>(length), x, typelib));
...
size_t strconvert(CHARSET_INFO *from_cs, const char *from, CHARSET_INFO *to_cs, char *to, size_t to_length,
                  uint *errors)
{
  int cnvres;
  my_wc_t wc;
  ...
  while (1)
  {
    ...
    */
    if ((cnvres = (*mb_wc)(from_cs, &wc, (uchar *)from, (uchar *)from + 10)) > 0)
    {
      if (!wc)
        break;
      from += cnvres;
...
int find_string_in_array(LEX_STRING *const haystack, LEX_STRING *const needle, CHARSET_INFO *const cs)
{
  const LEX_STRING *pos;
  for (pos = haystack; pos->str; pos++)
    if (!cs->coll->strnncollsp(cs, (uchar *)pos->str, pos->length, (uchar *)needle->str, needle->length, 0))
    {
