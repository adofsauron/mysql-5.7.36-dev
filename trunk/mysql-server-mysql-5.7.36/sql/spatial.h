...
struct MBR
{
  ...
  MBR(const point_xy &min, const point_xy &max) : xmin(min.x), ymin(min.y), xmax(max.x), ymax(max.y) {}
  ...
  */
  ...
  int overlaps(const MBR *mbr) const
  {
    ...
    */
    ...
    MBR intersection(std::max(xmin, mbr->xmin), std::max(ymin, mbr->ymin), std::min(xmax, mbr->xmax),
                     std::min(ymax, mbr->ymax));
...
{
  sz += GEOM_HEADER_SIZE;
  char *p = static_cast<char *>(my_malloc(key_memory_Geometry_objects_data, sz, MYF(MY_FAE)));
  p += GEOM_HEADER_SIZE;
  return p;
}
...
class Geometry
{
  ...
  */
  class wkb_container
  {
   ...
   public:
    wkb_container() {}
    wkb_container(const char *data, const char *data_end) { set(data, data_end); }
    ...
    */
    bool no_data(size_t data_amount) const { return (m_data + data_amount > m_data_end); }
    ...
    */
    ...
    {
      return (m_data_end < m_data || expected_points > ((m_data_end - m_data) / (POINT_DATA_SIZE + extra_point_space)));
    }
  };
  ...
  */
  class wkb_parser : public wkb_container
  {
   ...
   public:
    wkb_parser(const char *data, const char *data_end) : wkb_container(data, data_end) {}
    ...
    bool scan_non_zero_uint4(uint32 *number) { return (scan_uint4(number) || 0 == *number); }
    bool scan_n_points_and_check_data(uint32 *n_points, uint32 extra_point_space = 0)
    {
      return scan_non_zero_uint4(n_points) || not_enough_points(*n_points, extra_point_space);
    }
  ...
  virtual uint init_from_wkb(const char *wkb, uint len, wkbByteOrder bo, String *res) { return 0; }

  virtual bool get_data_as_wkt(String *txt, wkb_parser *wkb) const { return true; }
  virtual bool get_mbr(MBR *mbr, wkb_parser *wkb) const { return true; }
  ...
  wkbType get_type() const { return static_cast<Geometry::wkbType>(get_class_info()->m_type_id); }
 ...
 public:
  ...
  {
    return construct(buffer, str->ptr(), static_cast<uint32>(str->length()), has_srid);
  }
  static Geometry *create_from_wkt(Geometry_buffer *buffer, Gis_read_stream *trs, String *wkt, bool init_stream = true,
                                   bool check_trailing = true);
  static Geometry *create_from_wkb(Geometry_buffer *buffer, const char *wkb, uint32 len, String *res, bool init);
  ...
  bool is_polygon_ring() const { return m_flags.props & (POLYGON_OUTER_RING | POLYGON_INNER_RING); }

  bool is_polygon_outer_ring() const { return m_flags.props & POLYGON_OUTER_RING; }

  bool is_polygon_inner_ring() const { return m_flags.props & POLYGON_INNER_RING; }

  bool has_geom_header_space() const
  {
    return (m_flags.props & HAS_GEOM_HEADER_SPACE) || (m_flags.props & IS_BOOST_GEOMETRY_ADAPTER);
  }
  ...
  bool is_components_no_overlapped() const { return (m_flags.props & MULTIPOLYGON_NO_OVERLAPPED_COMPS); }

  void set_components_no_overlapped(bool b)
  {
    assert(get_type() == wkb_multilinestring || get_type() == wkb_multipolygon || get_type() == wkb_geometrycollection);
    if (b)
  ...
  void set_srid(srid_t id) { m_srid = id; }
 ...
 protected:
  ...
  {
    return ((type_id < wkb_first) || (type_id > wkb_last)) ? NULL : ci_collection[type_id];
  }
  ...
  */
  ...
  static bool is_valid_geotype(uint32 gtype)
  {
     ...
     */
    return ((gt >= wkb_first && gt <= wkb_geometrycollection) || gt == wkb_polygon_inner_rings);
  }
   ...
   */
  bool is_bg_adapter() const { return m_flags.props & IS_BOOST_GEOMETRY_ADAPTER; }
 ...
 protected:
   ...
   */
  bool polygon_is_wkb_form() const { return m_flags.props & POLYGON_IN_WKB_FORM; }
   ...
   */
  bool has_out_of_line_components() const { return m_flags.props & HAS_OUT_OF_LINE_COMPONENTS; }
...
inline char *write_wkb_header(void *p0, Geometry::wkbType geotype, uint32 obj_count)
{
  char *p = static_cast<char *>(p0);
...
inline char *write_geometry_header(void *p0, uint32 srid, Geometry::wkbType geotype)
{
  char *p = static_cast<char *>(p0);
  int4store(p, srid);
...
inline char *write_geometry_header(void *p0, uint32 srid, Geometry::wkbType geotype, uint32 obj_count)
{
  char *p = static_cast<char *>(p0);
  int4store(p, srid);
...
inline void write_wkb_header(String *str, Geometry::wkbType geotype, uint32 obj_count)
{
  write_wkb_header(str, geotype);
  str->q_append(obj_count);
}

inline void write_geometry_header(String *str, uint32 srid, Geometry::wkbType geotype)
{
  str->q_append(srid);
  write_wkb_header(str, geotype);
}

inline void write_geometry_header(String *str, uint32 srid, Geometry::wkbType geotype, uint32 obj_count)
{
...
class Gis_point : public Geometry
{
 public:
  ...
  explicit Gis_point(bool is_bg_adapter = true) : Geometry(NULL, 0, Flags_t(wkb_point, 0), default_srid)
  {
    set_ownmem(false);
  ...
  Gis_point(const void *ptr, size_t nbytes, const Flags_t &flags, srid_t srid) : Geometry(ptr, nbytes, flags, srid)
  {
    set_geotype(wkb_point);
    assert((ptr != NULL && get_nbytes() == SIZEOF_STORED_DOUBLE * GEOM_DIM) || (ptr == NULL && get_nbytes() == 0));
    set_ownmem(false);
    set_bg_adapter(true);
  }
  ...
  double get() const
  {
    assert(
        ...
        ((m_ptr != NULL && get_nbytes() == SIZEOF_STORED_DOUBLE * GEOM_DIM) || (m_ptr == NULL && get_nbytes() == 0)));
  ...
  bool operator==(const Gis_point &pt) const { return (get<0>() == pt.get<0>() && get<1>() == pt.get<1>()); }
...
class Gis_wkb_vector_const_iterator
{
 ...
 public:
  ...
  reference operator[](difference_type offset) const
  {
    ...
    assert(itr.m_owner != NULL && itr.m_curidx >= 0 && itr.m_curidx < static_cast<index_type>(itr.m_owner->size()));
...
class Gis_wkb_vector_iterator : public Gis_wkb_vector_const_iterator<T>
{
 ...
 public:
  ...
  Gis_wkb_vector_iterator(index_type idx, const owner_t *owner) : base(idx, owner) {}
  ...
  difference_type operator-(const self &itr) const { return base::operator-(itr); }
  ////////////////////////////////////////////////////////////////////
  ...
  reference operator[](difference_type offset) const
  {
    ...
    assert(itr.m_owner != NULL && itr.m_curidx >= 0 && itr.m_curidx < static_cast<index_type>(this->m_owner->size()));
...
class Gis_wkb_vector : public Geometry
{
 ...
 public:
  ...
  Gis_wkb_vector(const void *ptr, size_t nbytes, const Flags_t &flags, srid_t srid, bool is_bg_adapter = true);
...
class Gis_line_string : public Gis_wkb_vector<Gis_point>
{
  ...
  static const uint32 max_n_points = (uint32)(UINT_MAX32 - WKB_HEADER_SIZE - 4 /* n_points */) / POINT_DATA_SIZE;
 ...
 public:
  ...
  Gis_line_string(const void *wkb, size_t len, const Flags_t &flags, srid_t srid)
...
class Gis_polygon_ring : public Gis_wkb_vector<Gis_point>
{
 public:
  ...
  Gis_polygon_ring(const void *wkb, size_t nbytes, const Flags_t &flags, srid_t srid)
  ...
  Gis_polygon_ring() : base(NULL, 0, Flags_t(Geometry::wkb_linestring, 0), default_srid, true) {}
...
class Gis_polygon : public Geometry
{
 public:
  ...
  Gis_polygon(const void *wkb, size_t nbytes, const Flags_t &flags, srid_t srid);

  /*
    We can't require boost geometry use the 'polygon' in any particular way,
    so we have to default to true.
  */
  explicit Gis_polygon(bool isbgadapter = true) : Geometry(NULL, 0, Flags_t(Geometry::wkb_polygon, 0), default_srid)
  {
    m_inn_rings = NULL;
  ...
  inner_container_type *inner_rings() const { return m_inn_rings; }

  void set_inner_rings(inner_container_type *inns) { m_inn_rings = inns; }
...
class Gis_multi_point : public Gis_wkb_vector<Gis_point>
{
 ...
 public:
  ...
  explicit Gis_multi_point(bool is_bg_adapter = true)
      : base_type(NULL, 0, Flags_t(wkb_multipoint, 0), default_srid, is_bg_adapter)
  {
  }

  Gis_multi_point(const void *ptr, size_t nbytes, const Flags_t &flags, srid_t srid)
...
class Gis_multi_line_string : public Gis_wkb_vector<Gis_line_string>
{
 public:
  ...
  explicit Gis_multi_line_string(bool is_bg_adapter = true)
      : base(NULL, 0, Flags_t(wkb_multilinestring, 0), default_srid, is_bg_adapter)
  {
  }

  Gis_multi_line_string(const void *ptr, size_t nbytes, const Flags_t &flags, srid_t srid)
...
class Gis_multi_polygon : public Gis_wkb_vector<Gis_polygon>
{
 public:
  ...
  Gis_multi_polygon(const void *ptr, size_t nbytes, const Flags_t &flags, srid_t srid)
...
class Gis_geometry_collection : public Geometry
{
 ...
 public:
  Gis_geometry_collection() : Geometry(NULL, 0, Flags_t(wkb_geometrycollection, 0), default_srid)
  {
    set_bg_adapter(false);
  }
  Gis_geometry_collection(Geometry *geo, String *gcbuf);
  Gis_geometry_collection(srid_t srid, wkbType gtype, const String *gbuf, String *gcbuf);
  virtual ~Gis_geometry_collection() {} /* Remove gcc warning */
  bool append_geometry(const Geometry *geo, String *gcbuf);
  bool append_geometry(srid_t srid, wkbType gtype, const String *gbuf, String *gcbuf);
  uint32 get_data_size() const;
  bool init_from_wkt(Gis_read_stream *trs, String *wkb);
  uint init_from_wkb(const char *wkb, uint len, wkbByteOrder bo, String *res);
  bool get_data_as_wkt(String *txt, wkb_parser *wkb) const;
 ...
 */
struct Geometry_buffer : public my_aligned_storage<sizeof(Gis_polygon), MY_ALIGNOF(Gis_polygon)>
{
};
