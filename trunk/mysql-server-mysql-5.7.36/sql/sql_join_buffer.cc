...
static uint add_table_data_fields_to_join_cache(QEP_TAB *tab, MY_BITMAP *field_set, uint *field_cnt,
...
void JOIN_CACHE::calc_record_fields()
{
  ...
  for (; tab < qep_tab; tab++)
  {
    ...
    calc_used_field_length(join->thd, tab->table(), tab->keep_current_rowid, &used_fields, &used_fieldlength,
  ...
  if ((with_match_flag =
           (qep_tab->is_first_inner_for_outer_join() ||
            (qep_tab->first_sj_inner() == qep_tab->idx() && qep_tab->get_sj_strategy() == SJ_OPT_FIRST_MATCH))))
    flag_fields++;
  fields += flag_fields;
}
...
int JOIN_CACHE::alloc_fields(uint external_fields)
{
  ...
  field_descr = (CACHE_FIELD *)sql_alloc(fields_size + sizeof(CACHE_FIELD *) * ptr_cnt);
...
void JOIN_CACHE::create_flag_fields()
{
  ...
  if (with_match_flag)
    length += add_flag_field_to_join_cache((uchar *)&qep_tab->found, sizeof(qep_tab->found), &copy);
...
void JOIN_CACHE::set_constants()
{
  ...
  */
  uint len = length + fields * sizeof(uint) + blobs * sizeof(uchar *) +
             (prev_cache ? prev_cache->get_size_of_rec_offset() : 0) + sizeof(ulong) + aux_buffer_min_size();
...
{
  DBUG_EXECUTE_IF("jb_alloc_fail", buff = NULL; DBUG_SET("-d,jb_alloc_fail"); return true;);

  DBUG_EXECUTE_IF("jb_alloc_100MB", buff = (uchar *)my_malloc(key_memory_JOIN_CACHE, 100 * 1024 * 1024, MYF(0));
                  return buff == NULL;);
...
static void filter_gcol_for_dynamic_range_scan(QEP_TAB *const tab)
{
  ...
  for (uint key = 0; key < table->s->keys; ++key)
  {
    ...
    */
    ...
    {
      my_bitmap_map bitbuf[(bitmap_buffer_size(MAX_FIELDS) / sizeof(my_bitmap_map)) + 1];
      ...
      table->mark_columns_used_by_index_no_reset(key, &range_read_set, UINT_MAX);
...
void JOIN_CACHE::filter_virtual_gcol_base_cols()
{
  ...
  for (QEP_TAB *tab = qep_tab - tables; tab < qep_tab; tab++)
  {
    ...
    const bool cov_index = index != MAX_KEY && table->index_contains_some_virtual_gcol(index) &&
                           /*
                             There are two cases:
    ...
    if (!(cov_index || tab->dynamic_range()))
      continue;
    ...
    */
    my_bitmap_map *bitbuf = (my_bitmap_map *)alloc_root(tab->table()->in_use->mem_root, table->s->column_bitmap_size);
    MY_BITMAP *save_read_set = (MY_BITMAP *)alloc_root(tab->table()->in_use->mem_root, sizeof(MY_BITMAP));
    ...
    */
    ...
    if (cov_index)
    {
      ...
      if (table->s->primary_key != MAX_KEY)
        table->mark_columns_used_by_index_no_reset(table->s->primary_key, table->read_set);
...
void JOIN_CACHE::restore_virtual_gcol_base_cols()
{
  ...
  for (QEP_TAB *tab = qep_tab - tables; tab < qep_tab; tab++)
  {
    ...
    const bool cov_index =
        (index != MAX_KEY && table->index_contains_some_virtual_gcol(index) && table->covering_keys.is_set(index));
...
int JOIN_CACHE_BNL::init()
{
  ...
  */
  QEP_TAB *tab = prev_cache                                               ? prev_cache->qep_tab
                 : sj_is_materialize_strategy(qep_tab->get_sj_strategy()) ? &QEP_AT(qep_tab, first_sj_inner())
  ...
  if (qep_tab->condition() && qep_tab->first_inner() == NO_PLAN_IDX)
  {
    ...
    */
    ...
    Item *const tmp = make_cond_for_table(qep_tab->condition(), join->const_table_map | available, available, false);
    if (tmp)
    {
      Opt_trace_object(&join->thd->opt_trace).add("constant_condition_in_bnl", tmp);
      const_cond = tmp;
    }
  }
    ...
    for (QEP_TAB *tab = cache->qep_tab - cache->tables; tab < cache->qep_tab; tab++)
    {
      ...
      for (uint i = 0; i < ref->key_parts; i++)
      {
        ...
        if (!(tab->table_ref->map() & ref_item->used_tables()))
          continue;
        ref_item->walk(&Item::add_field_to_set_processor, Item::enum_walk(Item::WALK_POSTFIX | Item::WALK_SUBQUERY),
                       (uchar *)tab->table());
      }
  ...
  */
  for (i = 0; i < ref->key_parts; i++)
  {
    ...
    if (!key_part->field->eq_def(((Item_field *)item)->field))
      return FALSE;
    if (((Item_field *)item)->field->table->s->db_low_byte_first != table->s->db_low_byte_first)
    {
      return FALSE;
    }
...
uint JOIN_CACHE_BKA::aux_buffer_incr()
{
  ...
  */
  rec_per_key_t rec_per_key = tab->key_info[ref->key].records_per_key(ref->key_parts - 1);
...
{
  if ((t->last_sj_inner() == t->idx() && t->get_sj_strategy() == SJ_OPT_FIRST_MATCH))
    return true;
  if (t->first_inner() != NO_PLAN_IDX && QEP_AT(t, first_inner()).last_inner() == t->idx() &&
...
enum_nested_loop_state JOIN_CACHE::generate_full_extensions(uchar *rec_ptr)
{
  ...
  */
  ...
  {
    int res = 0;
    if (!qep_tab->check_weed_out_table || !(res = do_sj_dups_weedout(join->thd, qep_tab->check_weed_out_table)))
    {
...
bool JOIN_CACHE::check_match(uchar *rec_ptr)
{
  ...
  if (!((qep_tab->first_inner() != NO_PLAN_IDX && QEP_AT(qep_tab, first_inner()).last_inner() == qep_tab->idx()) ||
        (qep_tab->last_sj_inner() == qep_tab->idx() && qep_tab->get_sj_strategy() == SJ_OPT_FIRST_MATCH)))
  ...
  */
  plan_idx f_i = qep_tab->first_inner() != NO_PLAN_IDX
                     ? qep_tab->first_inner()
                     : ((qep_tab->get_sj_strategy() == SJ_OPT_FIRST_MATCH) ? qep_tab->first_sj_inner() : NO_PLAN_IDX);
  ...
  for (;;)
  {
    ...
    if (calc_check_only_first_match(first_inner) && qep_tab->first_inner() == NO_PLAN_IDX)
      return TRUE;
    /*
...
static bool bka_range_seq_skip_record(range_seq_t rseq, char *range_info, uchar *rowid)
{
...
enum_nested_loop_state JOIN_CACHE_BKA::join_matching_records(bool skip_last)
{
  ...
  while (!(error = file->multi_range_read_next((char **)&rec_ptr)))
  {
    ...
    /*
      ...
      is discarded.
    */
    if (rc == NESTED_LOOP_OK && (!check_only_first_match || !get_match_flag_by_pos(rec_ptr)))
    {
...
bool JOIN_CACHE_BKA::init_join_matching_records(RANGE_SEQ_IF *seq_funcs, uint ranges)
{
  ...
  */
  ...
  return file->multi_range_read_init(seq_funcs, (void *)this, ranges, mrr_mode, &mrr_buff);
}
      ...
      */
      if (external_key_arg_fields)
      {
        ...
        for (cache = prev_cache; key_arg_count; cache = cache->prev_cache)
        {
          ...
          while (key_arg_count && cache->read_referenced_field(*copy_ptr, rec_ptr, &len2))
          {
            copy_ptr++;
            --key_arg_count;
          }
...
int JOIN_CACHE_BKA_UNIQUE::init()
{
  ...
  for (size_of_key_ofs = 2; size_of_key_ofs <= max_size_of_key_ofs; size_of_key_ofs += 2)
  {
    ...
    */
    uint max_n = buff_size / (pack_length - length + key_entry_length + size_of_key_ofs);

    hash_entries = (uint)(n / 0.7);

    if (offset_size(max_n * key_entry_length) <= size_of_key_ofs)
      break;
  }
...
static range_seq_t bka_unique_range_seq_init(void *init_param, uint n_ranges, uint flags)
{
...
static bool bka_unique_range_seq_skip_record(range_seq_t rseq, char *range_info, uchar *rowid)
{
...
enum_nested_loop_state JOIN_CACHE_BKA_UNIQUE::join_matching_records(bool skip_last)
{
  ...
  /* Set functions to iterate over keys in the join buffer */
  RANGE_SEQ_IF seq_funcs = {bka_unique_range_seq_init, bka_unique_range_seq_next,
                            check_only_first_match && !no_association ? bka_unique_range_seq_skip_record : 0,
