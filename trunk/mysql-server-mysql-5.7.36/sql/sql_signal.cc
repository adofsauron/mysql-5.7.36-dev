...
static const LEX_STRING CONDITION_ITEM_NAMES[] = {
    {C_STRING_WITH_LEN("CLASS_ORIGIN")},         {C_STRING_WITH_LEN("SUBCLASS_ORIGIN")},
    {C_STRING_WITH_LEN("CONSTRAINT_CATALOG")},   {C_STRING_WITH_LEN("CONSTRAINT_SCHEMA")},
    {C_STRING_WITH_LEN("CONSTRAINT_NAME")},      {C_STRING_WITH_LEN("CATALOG_NAME")},
    {C_STRING_WITH_LEN("SCHEMA_NAME")},          {C_STRING_WITH_LEN("TABLE_NAME")},
    {C_STRING_WITH_LEN("COLUMN_NAME")},          {C_STRING_WITH_LEN("CURSOR_NAME")},
    {C_STRING_WITH_LEN("MESSAGE_TEXT")},         {C_STRING_WITH_LEN("MYSQL_ERRNO")},

    {C_STRING_WITH_LEN("CONDITION_IDENTIFIER")}, {C_STRING_WITH_LEN("CONDITION_NUMBER")},
    {C_STRING_WITH_LEN("CONNECTION_NAME")},      {C_STRING_WITH_LEN("MESSAGE_LENGTH")},
    {C_STRING_WITH_LEN("MESSAGE_OCTET_LENGTH")}, {C_STRING_WITH_LEN("PARAMETER_MODE")},
    {C_STRING_WITH_LEN("PARAMETER_NAME")},       {C_STRING_WITH_LEN("PARAMETER_ORDINAL_POSITION")},
    {C_STRING_WITH_LEN("RETURNED_SQLSTATE")},    {C_STRING_WITH_LEN("ROUTINE_CATALOG")},
    {C_STRING_WITH_LEN("ROUTINE_NAME")},         {C_STRING_WITH_LEN("ROUTINE_SCHEMA")},
    {C_STRING_WITH_LEN("SERVER_NAME")},          {C_STRING_WITH_LEN("SPECIFIC_NAME")},
    {C_STRING_WITH_LEN("TRIGGER_CATALOG")},      {C_STRING_WITH_LEN("TRIGGER_NAME")},
...
void Sql_cmd_common_signal::assign_defaults(Sql_condition *cond, bool set_level_code,
...
void Sql_cmd_common_signal::eval_defaults(THD *thd, Sql_condition *cond)
{
  ...
  if (is_sqlstate_warning(sqlstate))
  {
    ...
    assign_defaults(cond, set_defaults, Sql_condition::SL_WARNING, ER_SIGNAL_WARN);
  }
  else if (is_sqlstate_not_found(sqlstate))
  {
    /* SQLSTATE class "02": not found. */
    assign_defaults(cond, set_defaults, Sql_condition::SL_ERROR, ER_SIGNAL_NOT_FOUND);
  }
  else
  {
    /* other SQLSTATE classes : error. */
    assign_defaults(cond, set_defaults, Sql_condition::SL_ERROR, ER_SIGNAL_EXCEPTION);
  }
}
                                ...
                                const String *src)
{
  ...
  if (String::needs_conversion(to_copy, src_cs, dst_cs, &dummy_offset))
  {
    ...
    if (dst_str)
    {
      ...
      dst_copied = well_formed_copy_nchars(dst_cs, dst_str, dst_len, src_cs, src_str, src_len, numchars,
...
int Sql_cmd_common_signal::eval_signal_informations(THD *thd, Sql_condition *cond)
{
  ...
  static cond_item_map map[] = {{CIN_CLASS_ORIGIN, &Sql_condition::m_class_origin},
  ...
  */
  ...
  if (set != NULL)
  {
    ...
    */
    ...
    String utf8_text;
    str = set->val_str(&str_value);
    truncated = assign_fixed_string(thd->mem_root, &my_charset_utf8_bin, 128, &utf8_text, str);
    if (truncated)
    {
      if (thd->is_strict_mode())
      {
...
bool Sql_cmd_signal::execute(THD *thd)
{
  ...
  */
  ...
  assert((cond.severity() == Sql_condition::SL_WARNING) || (cond.severity() == Sql_condition::SL_ERROR));

  Sql_condition *raised =
      thd->raise_condition(cond.mysql_errno(), cond.returned_sqlstate(), cond.severity(), cond.message_text());
  if (raised)
...
bool Sql_cmd_resignal::execute(THD *thd)
{
  ...
  if (!thd->sp_runtime_ctx || !(frame = thd->sp_runtime_ctx->current_handler_frame()))
  {
  ...
  if (m_cond)
  {
    ...
    Sql_condition signaled_err(thd->mem_root, frame->sql_condition->mysql_errno(),
                               frame->sql_condition->returned_sqlstate(), frame->sql_condition->severity(),
    ...
    if (!eval_signal_informations(thd, &signaled_err))
    {
      ...
      raised = thd->raise_condition(signaled_err.mysql_errno(), signaled_err.returned_sqlstate(),
    ...
    */
    ...
    if (!eval_signal_informations(thd, raised))
    {
      if (raised->severity() == Sql_condition::SL_ERROR)
        da->set_error_status(raised->mysql_errno(), raised->message_text(), raised->returned_sqlstate());
    }
  }
  ...
  */
  ...
  if (da->is_error())
    frame->handler_da.set_error_status(da->mysql_errno(), da->message_text(), da->returned_sqlstate());
