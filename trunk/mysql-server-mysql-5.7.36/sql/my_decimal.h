...
inline uint32 my_decimal_precision_to_length_no_truncation(uint precision, uint8 scale, bool unsigned_flag)
{
  ...
  */
  ...
  uint32 retval = (uint32)(precision + (scale > 0 ? 1 : 0) + (unsigned_flag || !precision ? 0 : 1));
  return retval;
}
...
inline int binary2my_decimal(uint mask, const uchar *bin, my_decimal *d, int prec, int scale)
{
...
{
  return from->check_result(mask, decimal_round(from, to, scale, (truncate ? TRUNCATE : HALF_UP)));
}
...
inline bool str_set_decimal(const my_decimal *val, String *str, const CHARSET_INFO *cs)
{
...
{
  return d->check_result(mask, (unsigned_flag ? ulonglong2decimal((ulonglong)i, d) : longlong2decimal(i, d)));
}
...
inline bool operator<(const my_decimal &lhs, const my_decimal &rhs) { return my_decimal_cmp(&lhs, &rhs) < 0; }
