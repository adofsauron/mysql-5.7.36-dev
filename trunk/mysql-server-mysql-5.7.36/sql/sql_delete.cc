...
bool Sql_cmd_delete::mysql_delete(THD *thd, ha_rows limit)
{
  ...
  */
  ...
  if (table->all_partitions_pruned_away)
  {
    ...
    if (thd->lex->describe)
    {
      ...
      */
      Modification_plan plan(thd, MT_DELETE, table, "No matching rows after partition pruning", true, 0);
  ...
  */
  if (!using_limit && const_cond_result && !(specialflag & SPECIAL_NO_NEW_FUNC) &&
       ...
       !(table->triggers && table->triggers->has_delete_triggers())))
  {
    ...
    Modification_plan plan(thd, MT_DELETE, table, "Deleting all rows", false, maybe_deleted);
    if (thd->lex->describe)
    {
      err = explain_single_table_modification(thd, &plan, select_lex);
      goto exit_without_my_ok;
  ...
  if (conds)
  {
    ...
    if (result == Item::COND_FALSE)  // Impossible where
    {
      ...
      if (thd->lex->describe)
      {
        Modification_plan plan(thd, MT_DELETE, table, "Impossible WHERE", true, 0);
        err = explain_single_table_modification(thd, &plan, select_lex);
        goto exit_without_my_ok;
      }
    }
  ...
  {  // Enter scope for optimizer trace wrapper
    ...
    else if (conds != NULL)
    {
      ...
      QUICK_SELECT_I *qck;
      zero_rows = test_quick_select(thd, keys_to_use, 0, limit, safe_update, ORDER::ORDER_NOT_RELEVANT, &qep_tab, conds,
                                    &needed_reg_dummy, &qck, qep_tab.table()->force_index) < 0;
  ...
  if (order)
  {
    ...
    usable_index = get_index_for_order(order, &qep_tab, limit, &need_sort, &reverse);
  }

  {
    ...
    Modification_plan plan(thd, MT_DELETE, &qep_tab, usable_index, limit, false, need_sort, false, rows);
    ...
    if (table->triggers && table->triggers->has_triggers(TRG_EVENT_DELETE, TRG_ACTION_AFTER))
    {
      /*
    ...
    if ((table->file->ha_table_flags() & HA_READ_BEFORE_WRITE_REMOVAL) && !using_limit &&
        !(table->triggers && table->triggers->has_delete_triggers()) && qep_tab.quick() &&
  ...
  */
  ...
  if ((error < 0) || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT))
  {
    if (mysql_bin_log.is_open())
    {
      ...
      */
      int log_result = thd->binlog_query(query_type, thd->query().str, thd->query().length, transactional_table, FALSE,
                                         FALSE, errcode);
  ...
  assert(transactional_table || !deleted || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT));
  free_underlaid_joins(thd, select_lex);
  if (error < 0)
  {
    my_ok(thd, deleted);
...
bool Sql_cmd_delete::mysql_prepare_delete(THD *thd)
{
  ...
  if (table_list->is_view() && select->check_view_privileges(thd, DELETE_ACL, SELECT_ACL))
    DBUG_RETURN(true);
  ...
  if (select->order_list.first)
  {
    ...
    if (select->setup_ref_array(thd))
      DBUG_RETURN(true); /* purecov: inspected */
    if (setup_order(thd, select->ref_pointer_array, &tables, fields, all_fields, select->order_list.first))
      DBUG_RETURN(true);
  }
  ...
  if (select->first_execution && check_key_in_view(thd, table_list, delete_table_ref))
  {
    my_error(ER_NON_UPDATABLE_TABLE, MYF(0), table_list->alias, "DELETE");
    DBUG_RETURN(true);
  }

  TABLE_LIST *const duplicate = unique_table(thd, delete_table_ref, table_list->next_global, false);
  if (duplicate)
  {
...
int Sql_cmd_delete_multi::mysql_multi_delete_prepare(THD *thd, uint *table_count)
{
  ...
  *table_count = 0;
  ...
  for (TABLE_LIST *delete_target = lex->auxiliary_table_list.first; delete_target;
       delete_target = delete_target->next_local)
  {
    ...
    {
      my_error(ER_NON_UPDATABLE_TABLE, MYF(0), delete_target->table_name, "DELETE");
      DBUG_RETURN(true);
    }
    ...
    {
      my_error(ER_VIEW_DELETE_MERGE_VIEW, MYF(0), table_ref->view_db.str, table_ref->view_name.str);
      DBUG_RETURN(true);
    }
...
Query_result_delete::Query_result_delete(TABLE_LIST *dt, uint num_of_tables_arg)
    : delete_tables(dt),
      tempfiles(NULL),
      tables(NULL),
      deleted(0),
...
int Query_result_delete::prepare(List<Item> &values, SELECT_LEX_UNIT *u)
{
  ...
  */
  ...
  for (TABLE_LIST *walk = delete_tables; walk; walk = walk->next_local)
  {
    ...
    */
    TABLE_LIST *duplicate = unique_table(thd, ref, thd->lex->query_tables, false);
    if (duplicate)
    {
      update_non_unique_table_error(walk->correspondent_table, "DELETE", duplicate);
      DBUG_RETURN(1);
    }
  }
...
bool Query_result_delete::initialize_tables(JOIN *join)
{
  ...
  for (TABLE_LIST *walk = delete_tables; walk; walk = walk->next_local)
  {
    ...
    if (delete_while_scanning && unique_table(thd, ref, join->tables_list, false))
    {
      /*
  ...
  */
  ...
  for (uint i = 0; i < join->primary_tables; i++)
  {
    ...
    TABLE *const table = join->best_ref[i]->table();
    if (!(*tempfile++ = new Unique(refpos_order_cmp, (void *)table->file, table->file->ref_length, MEM_STRIP_BUF_SIZE)))
      DBUG_RETURN(true); /* purecov: inspected */
    *(table_ptr++) = table;
  }
...
{
  for (TABLE_LIST *tbl_ref = delete_tables; tbl_ref; tbl_ref = tbl_ref->next_local)
  {
...
static void invalidate_delete_tables(THD *thd, TABLE_LIST *delete_tables)
{
  ...
  {
    query_cache.invalidate_single(thd, tl->correspondent_table->updatable_base_table(), 1);
  }
}
...
void Query_result_delete::abort_result_set()
{
  ...
  if (error_handled || (!thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT) && !deleted))
    DBUG_VOID_RETURN;
  ...
  if (thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT))
  {
    ...
    */
    if (mysql_bin_log.is_open())
    {
      ...
      (void)thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str, thd->query().length, transactional_table_map != 0,
                              FALSE, FALSE, errcode);
    }
  }
  DBUG_VOID_RETURN;
  ...
  */
  ...
  if ((local_error == 0) || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT))
  {
    if (mysql_bin_log.is_open())
    {
      ...
      if (thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str, thd->query().length, transactional_table_map != 0,
                            FALSE, FALSE, errcode) &&
...
bool Sql_cmd_delete_multi::execute(THD *thd)
{
  ...
  if (!thd->is_fatal_error && (del_result = new Query_result_delete(aux_tables, del_table_count)))
  {
    ...
    res = handle_query(thd, lex, del_result, SELECT_NO_JOIN_CACHE | SELECT_NO_UNLOCK | OPTION_SETUP_TABLES_DONE,
