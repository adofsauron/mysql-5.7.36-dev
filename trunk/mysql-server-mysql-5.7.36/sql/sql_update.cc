...
bool compare_records(const TABLE *table)
{
  ...
  */
  ...
  {
    if (bitmap_is_set(table->write_set, (*ptr)->field_index) && (*ptr)->cmp_binary_offset(table->s->rec_buff_length))
      return TRUE;
  }
  return FALSE;
}
...
static bool check_fields(THD *thd, List<Item> &items, List<Item> *original_columns)
{
                  ...
                  enum enum_duplicates handle_duplicates, ha_rows *found_return, ha_rows *updated_return)
{
  ...
  if (mysql_prepare_update(thd, update_table_ref, &covering_keys_for_cond, values))
    DBUG_RETURN(1);
  ...
  */
  ...
  if ((table->file->ha_table_flags() & HA_PARTIAL_COLUMN_READ) != 0 && update.function_defaults_apply(table))
    /*
  ...
  if (table->part_info)
  {
    ...
    if (table->all_partitions_pruned_away)
    {
      ...
      if (thd->lex->describe)
      {
        ...
        */
        Modification_plan plan(thd, MT_UPDATE, table, "No matching rows after partition pruning", true, 0);
  ...
  if (conds)
  {
    ...
    if (table_list->check_option)
    {
      /*
        ...
        thing as Item::clone().
      */
      if (build_equal_items(thd, conds, &conds, NULL, false, select_lex->join_list, &cond_equal))
        goto exit_without_my_ok;
    ...
    if (result == Item::COND_FALSE)
    {
      ...
      if (thd->lex->describe)
      {
        Modification_plan plan(thd, MT_UPDATE, table, "Impossible WHERE", true, 0);
        error = explain_single_table_modification(thd, &plan, select_lex);
        goto exit_without_my_ok;
      }
    }
  ...
  */
  ...
  if (table->vfield && validate_gc_assignment(thd, &fields, &values, table))
    DBUG_RETURN(0);
  ...
  {  // Enter scope for optimizer trace wrapper
    ...
    else if (conds != NULL)
    {
      ...
      QUICK_SELECT_I *qck;
      impossible = test_quick_select(thd, keys_to_use, 0, limit, safe_update, ORDER::ORDER_NOT_RELEVANT, &qep_tab,
                                     conds, &needed_reg_dummy, &qck, qep_tab.table()->force_index) < 0;
  ...
  else if (qep_tab.quick())
  {
    ...
    */
    used_key_is_modified = (!qep_tab.quick()->unique_key_range() && qep_tab.quick()->is_keys_used(table->write_set));
  }
...

  {
    ...
    Modification_plan plan(thd, MT_UPDATE, &qep_tab, used_index, limit,
                           (!using_filesort && (used_key_is_modified || order)), using_filesort, used_key_is_modified,
                           rows);
    ...
    */
    ...
    if (table->triggers && table->triggers->has_triggers(TRG_EVENT_UPDATE, TRG_ACTION_AFTER))
    {
      /*
    ...
    if ((table->file->ha_table_flags() & HA_READ_BEFORE_WRITE_REMOVAL) && !thd->lex->is_ignore() && !using_limit &&
        !(table->triggers && table->triggers->has_update_triggers()) && qep_tab.quick() &&
        qep_tab.quick()->index != MAX_KEY && check_constant_expressions(values))
    ...
    while (true)
    {
      ...
      if ((!qep_tab.skip_record(thd, &skip_record) && !skip_record))
      {
        ...
        if (fill_record_n_invoke_before_triggers(thd, &update, fields, values, table, TRG_EVENT_UPDATE, 0))
          break; /* purecov: inspected */
    ...
    */
    ...
    DBUG_EXECUTE_IF("simulate_kill_bug27571", { thd->killed = THD::KILL_QUERY; };);
    error = (killed_status == THD::NOT_KILLED) ? error : 1;

    if (error && will_batch && (loc_error = table->file->exec_bulk_update(&dup_key_found)))
    /*
  ...
  */
  ...
  assert(transactional_table || !updated || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT));
  free_underlaid_joins(thd, select_lex);
  ...
  {
    char buff[MYSQL_ERRMSG_SIZE];
    my_snprintf(buff, sizeof(buff), ER(ER_UPDATE_INFO), (long)found, (long)updated,
                (long)thd->get_stmt_da()->current_statement_cond_count());
                          ...
                          List<Item> &update_value_list)
{
  ...
  if (select->setup_tables(thd, table_list, false))
    DBUG_RETURN(true);
  if (select->derived_table_count && select->check_view_privileges(thd, UPDATE_ACL, SELECT_ACL))
    DBUG_RETURN(true);
  ...
  if (select->order_list.first &&
      setup_order(thd, select->ref_pointer_array, table_list, all_fields, all_fields, select->order_list.first))
    DBUG_RETURN(true);
  ...
  *covering_keys_for_cond = update_table_ref->table->covering_keys;
...
#endif
  if (setup_fields(thd, Ref_ptr_array(), select->item_list, UPDATE_ACL, NULL, false, true))
    DBUG_RETURN(true); /* purecov: inspected */
  ...
  if (select->first_execution && check_key_in_view(thd, table_list, update_table_ref))
  {
    my_error(ER_NON_UPDATABLE_TABLE, MYF(0), table_list->alias, "UPDATE");
    DBUG_RETURN(true);
  }
  ...
  if (setup_fields(thd, Ref_ptr_array(), update_value_list, SELECT_ACL, NULL, false, false))
    DBUG_RETURN(true); /* purecov: inspected */

  thd->mark_used_columns = mark_used_columns_saved;

  // Check that table to be updated is not used in a subquery
  TABLE_LIST *const duplicate = unique_table(thd, update_table_ref, table_list->next_global, 0);
  if (duplicate)
  {
...
static bool unsafe_key_update(TABLE_LIST *leaves, table_map tables_for_update)
{
  ...
  for (tl = leaves; tl; tl = tl->next_leaf)
  {
    if (tl->map() & tables_for_update)
    {
      ...
      bool primkey_clustered = (table1->file->primary_key_is_clustered() && table1->s->primary_key != MAX_KEY);
        ...
        */
        ...
        if (tl2->map() & tables_for_update && table1->s == table2->s)
        {
              ...
              (partition_key_modified(table1, table1->write_set) || partition_key_modified(table2, table2->write_set)))
          {
            ...
            my_error(ER_MULTI_UPDATE_KEY_CONFLICT, MYF(0), tl->belong_to_view ? tl->belong_to_view->alias : tl->alias,
...
static bool multi_update_check_table_access(THD *thd, TABLE_LIST *table, table_map tables_for_update, bool updatable,
                                            bool *updated)
{
  ...
  if (table->is_view_or_derived())
  {
    ...
    */
    assert(table->merge_underlying_list || (!table->is_updatable() && !(table->map() & tables_for_update)));

    Internal_error_handler_holder<View_error_handler, TABLE_LIST> view_handler(thd, true, table->merge_underlying_list);
    for (TABLE_LIST *tbl = table->merge_underlying_list; tbl; tbl = tbl->next_local)
    {
      if (multi_update_check_table_access(thd, tbl, tables_for_update, updatable, &view_updated))
        return true;
    }
    table->set_want_privilege(view_updated ? UPDATE_ACL | SELECT_ACL : SELECT_ACL);
  ...
  else
  {
    ...
    table->set_want_privilege(base_table_updated ? UPDATE_ACL | SELECT_ACL : SELECT_ACL);
...
int Sql_cmd_update::mysql_multi_update_prepare(THD *thd)
{
  ...
  */
  if (original_multiupdate &&
      open_tables_for_query(thd, table_list, thd->stmt_arena->is_stmt_prepare() ? MYSQL_OPEN_FORCE_SHARED_MDL : 0))
    DBUG_RETURN(true);
  ...
  */
  ...
  {
    bool updated = false;
    if (multi_update_check_table_access(thd, tl, tables_for_update, true, &updated))
      DBUG_RETURN(true);
  }
  ...
  */
  if (thd->stmt_arena->is_stmt_prepare())
  {
    ...
    */
    ...
    {
      if (tr->lock_type != TL_READ && tr->lock_type != TL_READ_NO_INSERT)
      {
                        ...
                        SELECT_LEX *select_lex, Query_result_update **result)
{
  ...
  if (!(*result = new Query_result_update(select_lex->get_table_list(), select_lex->leaf_tables, fields, values,
                                          handle_duplicates)))
    DBUG_RETURN(true); /* purecov: inspected */

  assert(select_lex->having_cond() == NULL && !select_lex->order_list.elements && !select_lex->group_list.elements &&
         !select_lex->select_limit);

  res = handle_query(thd, thd->lex, *result, SELECT_NO_JOIN_CACHE | SELECT_NO_UNLOCK | OPTION_SETUP_TABLES_DONE,
...
Query_result_update::Query_result_update(TABLE_LIST *table_list, TABLE_LIST *leaves_list, List<Item> *field_list,
                                         List<Item> *value_list, enum enum_duplicates handle_duplicates_arg)
...
int Query_result_update::prepare(List<Item> &not_used_values, SELECT_LEX_UNIT *lex_unit)
{
  ...
  */
  for (TABLE_LIST *tr = leaves; tr; tr = tr->next_leaf)
  {
    ...
    if (tr->check_option && !tr->check_option->fixed && tr->check_option->fix_fields(thd, NULL))
  ...
  */
  ...
  int error = setup_fields(thd, Ref_ptr_array(), *values, SELECT_ACL, NULL, false, false);
  ...
  */
  ...
  update_operations = (COPY_INFO **)thd->mem_calloc(sizeof(COPY_INFO *) * table_count);
  ...
  for (TABLE_LIST *ref = leaves; ref != NULL; ref = ref->next_leaf)
  {
    if (tables_to_update & ref->map())
    {
      ...
      COPY_INFO *update = new (thd->mem_root) COPY_INFO(COPY_INFO::UPDATE_OPERATION, cols, vals);
      if (update == NULL || update->add_function_default_columns(table, table->write_set))
        DBUG_RETURN(1);
...
bool Query_result_update::initialize_tables(JOIN *join)
{
  ...
  */
  ...
  loop_end:
    ...
    if (table_ref->table == first_table_for_update->table && table_ref->check_option)
    {
      table_map unupdated_tables = table_ref->check_option->used_tables() & ~first_table_for_update->map();
      for (TABLE_LIST *tbl_ref = leaves; unupdated_tables && tbl_ref; tbl_ref = tbl_ref->next_leaf)
      {
    ...
    */
    ...
    do
    {
      ...
      */
      Field_varstring *field = new Field_varstring(tbl->file->ref_length, false, tbl->alias, tbl->s, &my_charset_bin);
    ...
    tmp_tables[cnt] =
        create_tmp_table(thd, tmp_param, temp_fields, group, 0, 0, TMP_TABLE_ALL_COLUMNS, HA_POS_ERROR, "");
  ...
  for (cur_table = update_tables; cur_table; cur_table = cur_table->next_local)
  {
    ...
    */
    ...
    if (table == table_to_update)
    {
      ...
      store_record(table, record[1]);
      if (fill_record_n_invoke_before_triggers(thd, update_operations[offset], *fields_for_table[offset],
      ...
      */
      ...
      if (!error && table->triggers && table->triggers->process_triggers(thd, TRG_EVENT_UPDATE, TRG_ACTION_AFTER, TRUE))
        DBUG_RETURN(1);
    }
    else
    {
      ...
      */
      ...
      do
      {
        ...
        tmp_table->visible_field_ptr()[field_num]->store(reinterpret_cast<const char *>(tbl->file->ref),
      ...
      */
      ...
      {
        for (Field **modified_fields = tmp_table->visible_field_ptr() + 1 + unupdated_check_opt_tables.elements;
      ...
      fill_record(thd, tmp_table, tmp_table->visible_field_ptr() + 1 + unupdated_check_opt_tables.elements,
      ...
      {
        if (error && (create_ondisk_from_heap(thd, tmp_table, tmp_table_param[offset].start_recinfo,
...
void Query_result_update::abort_result_set()
{
  ...
  if (error_handled || (!thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT) && !updated))
    return;
  ...
  */
  ...
  if (!trans_safe)
  {
    assert(thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT));
    ...
    */
    if (mysql_bin_log.is_open())
    {
      ...
      */
      ...
      (void)thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str, thd->query().length, transactional_tables, false,
                              false, errcode);
    }
  }
  assert(trans_safe || !updated || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT));
...
int Query_result_update::do_updates()
{
  ...
  for (cur_table = update_tables; cur_table; cur_table = cur_table->next_local)
  {
    ...
    */
    ...
    Field **field = tmp_table->visible_field_ptr() + 1 + unupdated_check_opt_tables.elements;  // Skip row pointers
    ...
    for (;;)
    {
      ...
      do
      {
        ...
        */
        ...
        if ((local_error = tbl->file->ha_rnd_pos(tbl->record[0], const_cast<uchar *>(data_ptr))))
        {
          if (table->file->is_fatal_error(local_error))
            error_flags |= ME_FATALERROR;
      ...
      if (table->vfield && update_generated_write_fields(table->write_set, table))
        goto err;

      if (table->triggers)
      {
        bool rc = table->triggers->process_triggers(thd, TRG_EVENT_UPDATE, TRG_ACTION_BEFORE, true);

        // Trigger might have changed dependencies of generated columns
        if (!rc && table->vfield && update_generated_write_fields(table->write_set, table))
          goto err;
...
bool Query_result_update::send_eof()
{
  ...
  */
  ...
  if (local_error == 0 || thd->get_transaction()->cannot_safely_rollback(Transaction_ctx::STMT))
  {
    if (mysql_bin_log.is_open())
    {
      int errcode = 0;
      if (local_error == 0)
        ...
        errcode = query_error_code(thd, killed_status == THD::NOT_KILLED);
      if (thd->binlog_query(THD::ROW_QUERY_TYPE, thd->query().str, thd->query().length, transactional_tables, FALSE,
                            FALSE, errcode))
      {
...
/**
  ...
  @return false otherwise.
*/
bool Sql_cmd_update::try_single_table_update(THD *thd, bool *switch_to_multitable)
{
  LEX *const lex = thd->lex;
  ...
  */
  ...
  if (open_tables_for_query(thd, all_tables, 0) || mysql_update_prepare_table(thd, select_lex) ||
  ...
  if (!all_tables->is_multiple_tables())
  {
    ...
    const bool res = mysql_update(thd, select_lex->item_list, update_value_list, unit->select_limit_cnt,
                                  lex->duplicates, &found, &updated);
  ...
  else
  {
    ...
    if (!thd->in_sub_stmt)
      thd->query_plan.set_query_plan(SQLCOM_UPDATE_MULTI, lex, !thd->stmt_arena->is_conventional());
...
bool Sql_cmd_update::execute_multi_table_update(THD *thd)
{
  ...
  */
  ...
  else
  {
    ...
    if (res)
      return res;
    if (check_readonly(thd, false) && some_non_temp_table_to_be_updated(thd, all_tables))
    {
      err_readonly(thd);
      return res;
    }
...
#endif
  {
    ...
    res = mysql_multi_update(thd, &select_lex->item_list, &update_value_list, lex->duplicates, select_lex, &result_obj);
...
bool Sql_cmd_update::execute(THD *thd)
{
  ...
  {
    return multi_update_precheck(thd, thd->lex->select_lex->get_table_list()) || execute_multi_table_update(thd);
  }
