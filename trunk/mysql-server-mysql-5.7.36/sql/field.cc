...
#define BLOB_PACK_LENGTH_TO_MAX_LENGH(arg) ((ulong)((1LL << MY_MIN(arg, 4) * 8) - 1LL))
...
void Field::set_tmp_null()
{
  ...
  m_count_cuted_fields_saved = table ? table->in_use->count_cuted_fields : current_thd->count_cuted_fields;
}
 ...
 */
static void push_numerical_conversion_warning(THD *thd, const char *str, uint length, const CHARSET_INFO *cs,
                                              const char *typestr, int error, const char *field_name = "UNKNOWN",
                                              ulong row_num = 0)
{
  char buf[MY_MAX(MY_MAX(DOUBLE_TO_STRING_CONVERSION_BUFFER_SIZE, LONGLONG_TO_STRING_CONVERSION_BUFFER_SIZE),
                  DECIMAL_TO_STRING_CONVERSION_BUFFER_SIZE)];

  String tmp(buf, sizeof(buf), cs);
  tmp.copy(str, length, cs);
  push_warning_printf(thd, Sql_condition::SL_WARNING, error, ER(error), typestr, tmp.c_ptr(), field_name, row_num);
}
...
*/
static void set_decimal_warning(Field_new_decimal *field, int dec_error, my_decimal *dec_value, const char *from,
                                size_t length, const CHARSET_INFO *charset_arg)
{
  switch (dec_error)
  {
    ...
    case E_DEC_BAD_NUM:
      ...
      push_warning_printf(field->table->in_use, Sql_condition::SL_WARNING, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                          ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "decimal", errmsg.ptr(), field->field_name,
...
size_t field_well_formed_copy_nchars(const CHARSET_INFO *to_cs, char *to, size_t to_length, const CHARSET_INFO *from_cs,
                                     const char *from, size_t from_length, size_t nchars,
                                     const char **well_formed_error_pos, const char **cannot_convert_error_pos,
                                     const char **from_end_pos)
{
  size_t res = well_formed_copy_nchars(to_cs, to, to_length, from_cs, from, from_length, nchars, well_formed_error_pos,
  ...
  */
  if ((to_cs->state & MY_CS_PUREASCII) && *well_formed_error_pos != NULL)
  {
    ...
    *well_formed_error_pos = NULL;
    ...
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_INVALID_CHARACTER_STRING,
...
void Field_num::prepend_zeros(String *value)
{
  ...
  if ((diff = (int)(field_length - value->length())) > 0)
  {
    ...
    if (!error)
    {
      memmove(const_cast<char *>(value->ptr()) + field_length - value->length(), value->ptr(), value->length());
...
type_conversion_status Field_num::check_int(const CHARSET_INFO *cs, const char *str, size_t length, const char *int_end,
                                            int error)
{
  ...
  if (str == int_end || error == MY_ERRNO_EDOM)
  {
    ...
    push_warning_printf(table->in_use, Sql_condition::SL_WARNING, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                        ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "integer", err.ptr(), field_name,
...
type_conversion_status Field_num::get_int(const CHARSET_INFO *cs, const char *from, size_t len, longlong *rnd,
                                          ulonglong unsigned_max, longlong signed_min, longlong signed_max)
{
  char *end;
  int error;

  *rnd = (longlong)cs->cset->strntoull10rnd(cs, from, len, unsigned_flag, &end, &error);
  if (unsigned_flag)
  {
...
*/
type_conversion_status Field_num::store_time(MYSQL_TIME *ltime, uint8 dec_arg MY_ATTRIBUTE((unused)))
{
...
String *Field::val_int_as_str(String *val_buffer, my_bool unsigned_val)
{
  ...
  if (val_buffer->alloc(MY_INT64_NUM_DECIMAL_DIGITS))
    return 0;
  length = (uint)(*cs->cset->longlong10_to_str)(cs, (char *)val_buffer->ptr(), MY_INT64_NUM_DECIMAL_DIGITS,
...
void Field::copy_data(my_ptrdiff_t src_record_offset)
{
  ...
  if (real_maybe_null())
  {
    ...
    m_null_ptr[0] = (m_null_ptr[0] & ~null_bit) | (m_null_ptr[src_record_offset] & null_bit);
  }
  else if (is_tmp_nullable())
    m_is_tmp_null = false;
}
...
*/
bool Field::compatible_field_size(uint field_metadata, Relay_log_info *rli_arg MY_ATTRIBUTE((unused)),
                                  uint16 mflags MY_ATTRIBUTE((unused)), int *order_var)
{
...
type_conversion_status Field::store(const char *to, size_t length, const CHARSET_INFO *cs,
...
/**
   ...
   little-endian.
*/
uchar *Field::pack(uchar *to, const uchar *from, uint max_length, bool low_byte_first MY_ATTRIBUTE((unused)))
{
...
*/
const uchar *Field::unpack(uchar *to, const uchar *from, uint param_data, bool low_byte_first MY_ATTRIBUTE((unused)))
{
...
void Field::make_field(Send_field *field)
{
  if (orig_table && orig_table->s->db.str && *orig_table->s->db.str)
  {
    ...
    if (orig_table->pos_in_table_list && orig_table->pos_in_table_list->schema_table)
      field->org_table_name = (orig_table->pos_in_table_list->schema_table->table_name);
    else
...
*/
longlong Field::convert_decimal2longlong(const my_decimal *val, bool unsigned_flag, bool *has_overflow)
{
  ...
  longlong val_ll;
  int conversion_error = my_decimal2int(E_DEC_ERROR & ~E_DEC_OVERFLOW & ~E_DEC_TRUNCATED, val, unsigned_flag, &val_ll);
...
bool Field::get_date(MYSQL_TIME *ltime, my_time_flags_t fuzzydate)
{
  ...
  return !(res = val_str(&tmp)) || str_to_datetime_with_warn(res, ltime, fuzzydate);
}
...
bool Field::get_timestamp(struct timeval *tm, int *warnings)
{
  MYSQL_TIME ltime;
  assert(!is_null());
  return get_date(&ltime, TIME_FUZZY_DATE) || datetime_to_timeval(current_thd, &ltime, tm, warnings);
}
...
type_conversion_status Field::store_time(MYSQL_TIME *ltime, uint8 dec_arg)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  char buff[MAX_DATE_STRING_REP_LENGTH];
  uint length = (uint)my_TIME_to_str(ltime, buff, MY_MIN(dec_arg, DATETIME_MAX_DECIMALS));
...
Field *Field::new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type MY_ATTRIBUTE((unused)))
{
...
Field *Field::new_key_field(MEM_ROOT *root, TABLE *new_table, uchar *new_ptr, uchar *new_null_ptr, uint new_null_bit)
{
  Field *tmp;
...
type_conversion_status Field_decimal::store(const char *from_arg, size_t len, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
...
double Field_decimal::val_real(void)
{
  ...
  int not_used;
  char *end_not_used;
  return my_strntod(&my_charset_bin, (char *)ptr, field_length, &end_not_used, &not_used);
}
...
longlong Field_decimal::val_int(void)
{
  ...
  int not_used;
  if (unsigned_flag)
    return my_strntoull(&my_charset_bin, (char *)ptr, field_length, 10, NULL, &not_used);
  return my_strntoll(&my_charset_bin, (char *)ptr, field_length, 10, NULL, &not_used);
}

String *Field_decimal::val_str(String *val_buffer MY_ATTRIBUTE((unused)), String *val_ptr)
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  uchar *str;
  size_t tmp_length;
...
Field_new_decimal::Field_new_decimal(uint32 len_arg, bool maybe_null_arg, const char *name, uint8 dec_arg,
                                     bool unsigned_arg)
    : Field_num((uchar *)0, len_arg, maybe_null_arg ? (uchar *)"" : 0, 0, NONE, name, dec_arg, 0, unsigned_arg)
{
  precision = my_decimal_length_to_precision(len_arg, dec_arg, unsigned_arg);
  set_if_smaller(precision, DECIMAL_MAX_PRECISION);
  assert((precision <= DECIMAL_MAX_PRECISION) && (dec <= DECIMAL_MAX_SCALE));
...
Field *Field_new_decimal::create_from_item(Item *item)
{
  ...
  */
  ...
  if (dec > 0)
  {
    ...
    const int required_length = my_decimal_precision_to_length(intg + dec, dec, item->unsigned_flag);
...
type_conversion_status Field_new_decimal::store_value(const my_decimal *decimal_value)
{
  ...
  int err = my_decimal2binary(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, decimal_value, ptr, precision, dec);
...
type_conversion_status Field_new_decimal::store(const char *from, size_t length, const CHARSET_INFO *charset_arg)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  ...
  int err =
      str2my_decimal(E_DEC_FATAL_ERROR & ~(E_DEC_OVERFLOW | E_DEC_BAD_NUM), from, length, charset_arg, &decimal_value);

  if (err != 0 && !table->in_use->lex->is_ignore() && table->in_use->is_strict_mode())
  {
    ErrConvString errmsg(from, length, charset_arg);
    const Diagnostics_area *da = table->in_use->get_stmt_da();
    push_warning_printf(table->in_use, Sql_condition::SL_WARNING, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                        ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "decimal", errmsg.ptr(), field_name,
                        da->current_row_for_condition());
...
type_conversion_status Field_new_decimal::store(double nr)
{
  ...
  int conv_err = double2my_decimal(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, nr, &decimal_value);
...
type_conversion_status Field_new_decimal::store(longlong nr, bool unsigned_val)
{
  ...
  my_decimal decimal_value;
  int conv_err = int2my_decimal(E_DEC_FATAL_ERROR & ~E_DEC_OVERFLOW, nr, unsigned_val, &decimal_value);
  DBUG_RETURN(store_internal_with_error_check(this, conv_err, &decimal_value));
}
...
type_conversion_status Field_new_decimal::store_time(MYSQL_TIME *ltime, uint8 dec_arg MY_ATTRIBUTE((unused)))
{
  my_decimal decimal_value;
...
String *Field_new_decimal::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  my_decimal decimal_value;
...
/**
   ...
   @return @c true
*/
bool Field_new_decimal::compatible_field_size(uint field_metadata, Relay_log_info *MY_ATTRIBUTE((unused)),
                                              uint16 mflags MY_ATTRIBUTE((unused)), int *order_var)
{
  uint const source_precision = (field_metadata >> 8U) & 0x00ff;
  uint const source_decimal = field_metadata & 0x00ff;
...
/**
   ...
   @return  New pointer into memory based on from + length of the data
*/
const uchar *Field_new_decimal::unpack(uchar *to, const uchar *from, uint param_data, bool low_byte_first)
{
  if (param_data == 0)
...
bool Field_new_decimal::send_binary(Protocol *protocol)
{
  ...
  if (is_null())
    return protocol->store_null();
  return protocol->store_decimal(val_decimal(&dec_value), zerofill ? precision : 0, dec);
}
...
type_conversion_status Field_tiny::store(const char *from, size_t len, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  longlong rnd;

  const type_conversion_status error = get_int(cs, from, len, &rnd, 255, -128, 127);
...
String *Field_tiny::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
...
bool Field_tiny::send_binary(Protocol *protocol)
{
  if (is_null())
    return protocol->store_null();
  return protocol->store_tiny((longlong)unsigned_flag ? (uint8)ptr[0] : (int8)ptr[0]);
}
...
type_conversion_status Field_short::store(const char *from, size_t len, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  int store_tmp;
  longlong rnd;

  const type_conversion_status error = get_int(cs, from, len, &rnd, UINT_MAX16, INT_MIN16, INT_MAX16);
...
String *Field_short::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  const CHARSET_INFO *cs = &my_charset_numeric;
  uint length;
...
type_conversion_status Field_medium::store(const char *from, size_t len, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  int store_tmp;
  longlong rnd;

  const type_conversion_status error = get_int(cs, from, len, &rnd, UINT_MAX24, INT_MIN24, INT_MAX24);
  store_tmp = unsigned_flag ? (int)(ulonglong)rnd : (int)rnd;
...
String *Field_medium::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  const CHARSET_INFO *cs = &my_charset_numeric;
  uint length;
...
type_conversion_status Field_long::store(const char *from, size_t len, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  long store_tmp;
  longlong rnd;

  const type_conversion_status error = get_int(cs, from, len, &rnd, UINT_MAX32, INT_MIN32, INT_MAX32);
...
String *Field_long::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  const CHARSET_INFO *cs = &my_charset_numeric;
  size_t length;
...
type_conversion_status Field_longlong::store(const char *from, size_t len, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  int conv_err = 0;
  type_conversion_status error = TYPE_OK;
  ...
  else if (table->in_use->count_cuted_fields && check_int(cs, from, len, end, conv_err))
    error = TYPE_WARN_OUT_OF_RANGE;
  else
    error = TYPE_OK;
...
String *Field_longlong::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  const CHARSET_INFO *cs = &my_charset_numeric;
  uint length;
...
uchar *Field_real::pack(uchar *to, const uchar *from, uint max_length, bool low_byte_first)
{
...
const uchar *Field_real::unpack(uchar *to, const uchar *from, uint param_data, bool low_byte_first)
{
...
type_conversion_status Field_real::store_time(MYSQL_TIME *ltime, uint8 dec_arg MY_ATTRIBUTE((unused)))
{
...
type_conversion_status Field_float::store(const char *from, size_t len, const CHARSET_INFO *cs)
{
  int conv_error;
  type_conversion_status err = TYPE_OK;
  char *end;
  double nr = my_strntod(cs, (char *)from, len, &end, &conv_error);
  if (conv_error || (!len || ((uint)(end - from) != len && table->in_use->count_cuted_fields)))
  {
    set_warning(Sql_condition::SL_WARNING, (conv_error ? ER_WARN_DATA_OUT_OF_RANGE : WARN_DATA_TRUNCATED), 1);
...
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  const type_conversion_status error = truncate(&nr, FLT_MAX) ? TYPE_WARN_OUT_OF_RANGE : TYPE_OK;
...
String *Field_float::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
...
type_conversion_status Field_double::store(const char *from, size_t len, const CHARSET_INFO *cs)
{
  int conv_error;
  type_conversion_status error = TYPE_OK;
  char *end;
  double nr = my_strntod(cs, (char *)from, len, &end, &conv_error);
  if ((conv_error != 0) || (!len || ((uint)(end - from) != len && table->in_use->count_cuted_fields)))
  {
    set_warning(Sql_condition::SL_WARNING, (conv_error ? ER_WARN_DATA_OUT_OF_RANGE : WARN_DATA_TRUNCATED), 1);
...
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  const type_conversion_status error = truncate(&nr, DBL_MAX) ? TYPE_WARN_OUT_OF_RANGE : TYPE_OK;
...
{
  return new_field->sql_type == real_type() && new_field->decimals == decimals();
}
...
void Field_temporal::set_warnings(ErrConvString str, int warnings)
{
  ...
  {
    set_datetime_warning(Sql_condition::SL_WARNING, WARN_DATA_TRUNCATED, str, ts_type, !cut_incremented);
    cut_incremented = 1;
  }
  if (warnings & (MYSQL_TIME_WARN_OUT_OF_RANGE | MYSQL_TIME_WARN_ZERO_DATE | MYSQL_TIME_WARN_ZERO_IN_DATE))
  {
    set_datetime_warning(Sql_condition::SL_WARNING, ER_WARN_DATA_OUT_OF_RANGE, str, ts_type, !cut_incremented);
    cut_incremented = 1;
  }
  if (warnings & MYSQL_TIME_WARN_INVALID_TIMESTAMP)
  {
    set_datetime_warning(Sql_condition::SL_WARNING, ER_WARN_INVALID_TIMESTAMP, str, ts_type, !cut_incremented);
    cut_incremented = 1;
  }
  if ((warnings & MYSQL_TIME_NOTE_TRUNCATED) && !(warnings & MYSQL_TIME_WARN_TRUNCATED))
  {
    set_datetime_warning(Sql_condition::SL_NOTE, WARN_DATA_TRUNCATED, str, ts_type, !cut_incremented);
  }
}
...
type_conversion_status Field_temporal::store(longlong nr, bool unsigned_val)
{
  ...
  int warnings = 0;
  MYSQL_TIME ltime;
  type_conversion_status error = convert_number_to_TIME(nr, unsigned_val, 0, &ltime, &warnings);
  ...
  else
  {
    ...
    if (warnings & (MYSQL_TIME_WARN_ZERO_DATE | MYSQL_TIME_WARN_ZERO_IN_DATE) && !current_thd->is_strict_mode())
...
type_conversion_status Field_temporal::store_lldiv_t(const lldiv_t *lld, int *warnings)
{
  ...
  MYSQL_TIME ltime;
  error = convert_number_to_TIME(lld->quot, 0, static_cast<int>(lld->rem), &ltime, warnings);
  if (error == TYPE_OK || error == TYPE_NOTE_TRUNCATED)
    error = store_internal_with_round(&ltime, warnings);
  else if (!*warnings)
  {
    assert(warnings != 0);  // Must be set by convert_number_to_TIME
    if (((*warnings & MYSQL_TIME_WARN_ZERO_DATE) != 0 || (*warnings & MYSQL_TIME_WARN_ZERO_IN_DATE) != 0) &&
...
/**
  ...
  @retval !TYPE_OK  Warning/error as indicated by type_conversion_status enum
                    value
*/
type_conversion_status Field_temporal::store(const char *str, size_t len, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  type_conversion_status error = TYPE_OK;
  MYSQL_TIME ltime;
  ...
  if (convert_str_to_TIME(str, len, cs, &ltime, &status))
  {
    ...
    */
    reset();
    if (status.warnings & (MYSQL_TIME_WARN_ZERO_DATE | MYSQL_TIME_WARN_ZERO_IN_DATE) && !current_thd->is_strict_mode())
      error = TYPE_NOTE_TIME_TRUNCATED;
    else
  ...
  else
  {
    ...
    const type_conversion_status tmp_error = store_internal_with_round(&ltime, &status.warnings);
...
*/
longlong Field_temporal::convert_number_to_datetime(longlong nr, bool unsigned_val, MYSQL_TIME *ltime, int *warnings)
{
  /*
...
bool Field_temporal_with_date::get_internal_check_zero(MYSQL_TIME *ltime, my_time_flags_t fuzzydate)
{
...
longlong Field_temporal_with_date::val_date_temporal()
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  MYSQL_TIME ltime;
  return get_date_internal(&ltime) ? 0 : TIME_to_longlong_datetime_packed(&ltime);
}
...
type_conversion_status Field_temporal_with_date::convert_number_to_TIME(longlong nr, bool unsigned_val, int nanoseconds,
...
type_conversion_status Field_temporal_with_date::store_time(MYSQL_TIME *ltime, uint8 dec_arg MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  type_conversion_status error;
  int warnings = 0;
  ...
  switch (ltime->time_type)  // TS-TODO: split into separate methods?
  {
    ...
    case MYSQL_TIMESTAMP_DATE:
      ...
      {
        assert(warnings & (MYSQL_TIME_WARN_OUT_OF_RANGE | MYSQL_TIME_WARN_ZERO_DATE | MYSQL_TIME_WARN_ZERO_IN_DATE));
...
bool Field_temporal_with_date::convert_str_to_TIME(const char *str, size_t len, const CHARSET_INFO *cs,
...
type_conversion_status Field_temporal_with_date::store_internal_with_round(MYSQL_TIME *ltime, int *warnings)
{
...
bool Field_temporal_with_date_and_time::convert_TIME_to_timestamp(THD *thd, const MYSQL_TIME *ltime, struct timeval *tm,
                                                                  int *warnings)
{
  /*
...
longlong Field_temporal_with_date_and_timef::val_int()
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  MYSQL_TIME ltime;
  return get_date_internal(&ltime) ? 0 : TIME_to_ulonglong_datetime_round(&ltime);
}
...
Field_timestamp::Field_timestamp(bool maybe_null_arg, const char *field_name_arg)
...
type_conversion_status Field_timestamp::store_internal(const MYSQL_TIME *ltime, int *warnings)
{
  ...
  const type_conversion_status error = time_warning_to_type_conversion_status(*warnings);
  store_timestamp_internal(&tm);
  return error;
}
...
type_conversion_status Field_timestamp::store_packed(longlong nr)
{
  ...
  assert((MY_PACKED_TIME_GET_FRAC_PART(nr) % (int)log_10_int[DATETIME_MAX_DECIMALS - decimals()]) == 0);
  MYSQL_TIME ltime;
...
****************************************************************************/
Field_timestampf::Field_timestampf(uchar *ptr_arg, uchar *null_ptr_arg, uchar null_bit_arg, enum utype unireg_check_arg,
                                   const char *field_name_arg, uint8 dec_arg)
    : Field_temporal_with_date_and_timef(ptr_arg, null_ptr_arg, null_bit_arg, unireg_check_arg, field_name_arg, dec_arg)
{
  init_timestamp_flags();
}

Field_timestampf::Field_timestampf(bool maybe_null_arg, const char *field_name_arg, uint8 dec_arg)
    : Field_temporal_with_date_and_timef((uchar *)0, maybe_null_arg ? (uchar *)"" : 0, 0, NONE, field_name_arg, dec_arg)
{
  if (unireg_check != TIMESTAMP_DN_FIELD)
    flags |= ON_UPDATE_NOW_FLAG;
}
...
void Field_timestampf::sql_type(String &res) const
{
  ...
  const CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "timestamp(%d)", dec));
}
...
bool Field_time_common::convert_str_to_TIME(const char *str, size_t len, const CHARSET_INFO *cs, MYSQL_TIME *ltime,
                                            MYSQL_TIME_STATUS *status)
{
  return str_to_time(cs, str, len, ltime, 0, status);
}

type_conversion_status Field_time_common::convert_number_to_TIME(longlong nr, bool unsigned_val, int nanoseconds,
                                                                 MYSQL_TIME *ltime, int *warnings)
{
  ...
  */
  ...
  ltime->second_part = 0;
  bool round_error = time_add_nanoseconds_with_round(ltime, nanoseconds, warnings);

  return round_error ? time_warning_to_type_conversion_status(*warnings) : TYPE_OK;
}

type_conversion_status Field_time_common::store_time(MYSQL_TIME *ltime, uint8 dec_arg MY_ATTRIBUTE((unused)))
{
  /* Check if seconds or minutes are out of range */
  if (ltime->second >= 60 || ltime->minute >= 60)
  {
    set_warnings(ErrConvString(ltime, decimals()), MYSQL_TIME_WARN_OUT_OF_RANGE);
    reset();
    return TYPE_WARN_OUT_OF_RANGE;
  }
  int warnings = 0;
...
String *Field_time_common::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  MYSQL_TIME ltime;
...
type_conversion_status Field_year::store(const char *from, size_t len, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  char *end;
  int conv_error;
  ...
  if (nr < 0 || (nr >= 100 && nr <= 1900) || nr > 2155 || conv_error == MY_ERRNO_ERANGE)
  {
    *ptr = 0;
    set_warning(Sql_condition::SL_WARNING, ER_WARN_DATA_OUT_OF_RANGE, 1);
    return TYPE_WARN_OUT_OF_RANGE;
...
type_conversion_status Field_year::store_time(MYSQL_TIME *ltime, uint8 dec_arg MY_ATTRIBUTE((unused)))
{
  if (ltime->time_type != MYSQL_TIMESTAMP_DATETIME && ltime->time_type != MYSQL_TIMESTAMP_DATE)
  {
...
String *Field_year::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
...
String *Field_newdate::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
...
{
  return get_internal_check_zero(ltime, fuzzydate) || check_fuzzy_date(ltime, fuzzydate);
}
...
type_conversion_status Field_datetime::store(longlong nr, bool unsigned_val)
{
  ...
  int warnings;
  type_conversion_status error = TYPE_OK;
  longlong tmp = convert_number_to_datetime(nr, unsigned_val, &ltime, &warnings);
  if (tmp == -1LL)
    error = TYPE_ERR_BAD_VALUE;
  else
  {
...
*/
String *Field_datetime::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  val_buffer->alloc(field_length + 1);
  val_buffer->set_charset(&my_charset_numeric);
...
void Field_datetimef::sql_type(String &res) const
{
  ...
  const CHARSET_INFO *cs = res.charset();
  res.length(cs->cset->snprintf(cs, (char *)res.ptr(), res.alloced_length(), "datetime(%d)", dec));
}
...
type_conversion_status Field_longstr::check_string_copy_error(const char *original_string,
                                                              const char *well_formed_error_pos,
                                                              const char *cannot_convert_error_pos,
                                                              const char *from_end_pos, const char *end,
                                                              bool count_spaces, const CHARSET_INFO *cs)
{
  const char *pos;
  char tmp[32];
  ...
  push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                      ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "string", tmp, field_name,
...
type_conversion_status Field_longstr::report_if_important_data(const char *pstr, const char *end, bool count_spaces)
{
...
type_conversion_status Field_string::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  size_t copy_length;
  const char *well_formed_error_pos;
  ...
  copy_length = field_well_formed_copy_nchars(field_charset, (char *)ptr, field_length, cs, from, length,
                                              field_length / field_charset->mbmaxlen, &well_formed_error_pos,
                                              &cannot_convert_error_pos, &from_end_pos);

  /* Append spaces if the string was shorter than the field. */
  if (copy_length < field_length)
    field_charset->cset->fill(field_charset, (char *)ptr + copy_length, field_length - copy_length,
                              field_charset->pad_char);

  return check_string_copy_error(from, well_formed_error_pos, cannot_convert_error_pos, from_end_pos, from + length,
                                 false, cs);
}
...
{
  return ((new_field->sql_type == real_type()) && new_field->charset == field_charset &&
...
double Field_string::val_real(void)
{
  ...
  if (!table->in_use->no_errors && (error || (field_length != (uint32)(end - (char *)ptr) &&
...
String *Field_string::val_str(String *val_buffer MY_ATTRIBUTE((unused)), String *val_ptr)
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  /* See the comment for Field_long::store(long long) */
  assert(table->in_use == current_thd);
  ...
  if (table->in_use->variables.sql_mode & MODE_PAD_CHAR_TO_FULL_LENGTH)
    length = my_charpos(field_charset, ptr, ptr + field_length, field_length / field_charset->mbmaxlen);
  else
...
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  int err = str2my_decimal(E_DEC_FATAL_ERROR, (char *)ptr, field_length, charset(), decimal_value);
...
static bool check_field_for_37426(const void *param_arg)
{
  ...
  DBUG_PRINT("debug", ("Field %s - type: %d, size: %d", param->field->field_name, param->field->real_type(),
                       param->field->row_pack_length()));
  return param->field->row_pack_length() > 255;
}
#endif

bool Field_string::compatible_field_size(uint field_metadata, Relay_log_info *rli_arg,
                                         uint16 mflags MY_ATTRIBUTE((unused)), int *order_var)
{
#ifdef HAVE_REPLICATION
  const Check_field_param check_param = {this};
  if (!is_mts_worker(rli_arg->info_thd) &&
      rpl_master_has_bug(rli_arg, 37426, TRUE, check_field_for_37426, &check_param))
...
int Field_string::cmp(const uchar *a_ptr, const uchar *b_ptr)
{
  ...
  */
  return field_charset->coll->strnncollsp(field_charset, a_ptr, a_len, b_ptr, b_len, 0);
}

void Field_string::make_sort_key(uchar *to, size_t length)
{
  size_t tmp MY_ATTRIBUTE((unused)) =
      field_charset->coll->strnxfrm(field_charset, to, length, char_length(), ptr, field_length,
...
uchar *Field_string::pack(uchar *to, const uchar *from, uint max_length, bool low_byte_first MY_ATTRIBUTE((unused)))
{
                                  ...
                                  bool low_byte_first MY_ATTRIBUTE((unused)))
{
  ...
  DBUG_PRINT("debug", ("param_data: 0x%x, field_length: %u, from_length: %u", param_data, field_length, from_length));
  /*
...
int Field_string::do_save_field_metadata(uchar *metadata_ptr)
{
  ...
  DBUG_PRINT("debug", ("field_length: %u, real_type: %u", field_length, real_type()));
...
{
  size_t bytes = my_charpos(field_charset, (char *)ptr, (char *)ptr + field_length, length / field_charset->mbmaxlen);
  memcpy(buff, ptr, bytes);
  if (bytes < length)
    field_charset->cset->fill(field_charset, (char *)buff + bytes, length - bytes, field_charset->pad_char);
  return bytes;
}
...
Field *Field_string::new_field(MEM_ROOT *root, TABLE *new_table, bool keep_type)
{
  ...
  else if ((field = new Field_varstring(field_length, maybe_null(), field_name, new_table->s, charset())))
  {
    /*
...
type_conversion_status Field_varstring::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  size_t copy_length;
  const char *well_formed_error_pos;
  ...
  copy_length = field_well_formed_copy_nchars(field_charset, (char *)ptr + length_bytes, field_length, cs, from, length,
                                              field_length / field_charset->mbmaxlen, &well_formed_error_pos,
                                              &cannot_convert_error_pos, &from_end_pos);
...
double Field_varstring::val_real(void)
{
  ...
  if (!table->in_use->no_errors && (error || (length != (uint)(end - (char *)ptr + length_bytes) &&
                                              !check_if_only_end_space(cs, end, (char *)ptr + length_bytes + length))))
  {
    push_numerical_conversion_warning(current_thd, (char *)ptr + length_bytes, length, cs, "DOUBLE",
...
longlong Field_varstring::val_int(void)
{
  ...
  uint length = length_bytes == 1 ? (uint)*ptr : uint2korr(ptr);
  longlong result = my_strntoll(cs, (char *)ptr + length_bytes, length, 10, &end, &error);

  if (!table->in_use->no_errors && (error || (length != (uint)(end - (char *)ptr + length_bytes) &&
                                              !check_if_only_end_space(cs, end, (char *)ptr + length_bytes + length))))
  {
    push_numerical_conversion_warning(current_thd, (char *)ptr + length_bytes, length, cs, "INTEGER",
                                      ER_TRUNCATED_WRONG_VALUE);
  }
  return result;
}

String *Field_varstring::val_str(String *val_buffer MY_ATTRIBUTE((unused)), String *val_ptr)
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  uint length = length_bytes == 1 ? (uint)*ptr : uint2korr(ptr);
...
my_decimal *Field_varstring::val_decimal(my_decimal *decimal_value)
{
  ...
  const CHARSET_INFO *cs = charset();
  uint length = length_bytes == 1 ? (uint)*ptr : uint2korr(ptr);
  int error = str2my_decimal(E_DEC_FATAL_ERROR, (char *)ptr + length_bytes, length, cs, decimal_value);

  if (!table->in_use->no_errors && error)
  {
    push_numerical_conversion_warning(current_thd, (char *)ptr + length_bytes, length, cs, "DECIMAL",
                                      ER_TRUNCATED_WRONG_VALUE);
  }
  return decimal_value;
}

int Field_varstring::cmp_max(const uchar *a_ptr, const uchar *b_ptr, uint max_len)
{
  ...
  diff = field_charset->coll->strnncollsp(field_charset, a_ptr + length_bytes, a_length, b_ptr + length_bytes, b_length,
                                          0);
  return diff;
}
...
int Field_varstring::key_cmp(const uchar *key_ptr, uint max_key_length)
{
  ...
  local_char_length = my_charpos(field_charset, ptr + length_bytes, ptr + length_bytes + length, local_char_length);
  set_if_smaller(length, local_char_length);
  return field_charset->coll->strnncollsp(field_charset, ptr + length_bytes, length, key_ptr + HA_KEY_BLOB_LENGTH,
                                          uint2korr(key_ptr), 0);
}
...
{
  return field_charset->coll->strnncollsp(field_charset, a + HA_KEY_BLOB_LENGTH, uint2korr(a), b + HA_KEY_BLOB_LENGTH,
                                          uint2korr(b), 0);
}
...
void Field_varstring::make_sort_key(uchar *to, size_t length)
{
  ...
  tot_length = field_charset->coll->strnxfrm(field_charset, to, length, char_length(), ptr + length_bytes, tot_length,
                                             MY_STRXFRM_PAD_WITH_SPACE | MY_STRXFRM_PAD_TO_MAXLEN);
...
{
  return length_bytes == 1 ? (uint32) * (ptr + row_offset) : uint2korr(ptr + row_offset);
}
...
uchar *Field_varstring::pack(uchar *to, const uchar *from, uint max_length, bool low_byte_first MY_ATTRIBUTE((unused)))
{
...
/**
   ...
   @return  New pointer into memory based on from + length of the data
*/
const uchar *Field_varstring::unpack(uchar *to, const uchar *from, uint param_data,
                                     bool low_byte_first MY_ATTRIBUTE((unused)))
{
  uint length;
...
size_t Field_varstring::get_key_image(uchar *buff, size_t length, imagetype type)
{
  ...
  */
  uint f_length = is_null() ? 0 : (length_bytes == 1 ? (uint)*ptr : uint2korr(ptr));
  uint local_char_length = length / field_charset->mbmaxlen;
  uchar *pos = ptr + length_bytes;
  local_char_length = my_charpos(field_charset, pos, pos + f_length, local_char_length);
...
int Field_varstring::cmp_binary(const uchar *a_ptr, const uchar *b_ptr, uint32 max_length)
{
...
{
  Field_varstring *res = (Field_varstring *)Field::new_field(root, new_table, keep_type);
  if (res)
    res->length_bytes = length_bytes;
  return res;
}

Field *Field_varstring::new_key_field(MEM_ROOT *root, TABLE *new_table, uchar *new_ptr, uchar *new_null_ptr,
                                      uint new_null_bit)
{
  Field_varstring *res;
...
{
  if (new_field->sql_type == real_type() && new_field->charset == field_charset)
  {
    if (new_field->length == max_display_length())
      return IS_EQUAL_YES;
    assert(0 == (new_field->length % field_charset->mbmaxlen));
    assert(0 == (max_display_length() % field_charset->mbmaxlen));
    if (new_field->length > max_display_length() && ((new_field->length <= 255 && max_display_length() <= 255) ||
...
void Field_blob::store_length(uchar *i_ptr, uint i_packlength, uint32 i_number, bool low_byte_first)
{
  switch (i_packlength)
  {
    case 1:
...
*/
type_conversion_status Field_blob::store_to_mem(const char *from, size_t length, const CHARSET_INFO *cs,
                                                size_t max_length, Blob_mem_storage *blob_storage)
{
  /*
  ...
  */
  ...
  if (length > max_length)
  {
    ...
    length = cs->cset->well_formed_len(cs, from, from + max_length, length, &well_formed_error);
...
type_conversion_status Field_blob::store_internal(const char *from, size_t length, const CHARSET_INFO *cs)
{
  size_t new_length;
  ...
  */
...

  {
    ...
    */
    size_t copy_length =
        field_well_formed_copy_nchars(field_charset, tmp, new_length, cs, from, length, length, &well_formed_error_pos,
                                      &cannot_convert_error_pos, &from_end_pos);
...
type_conversion_status Field_blob::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;

  if (table->blob_storage)  // GROUP_CONCAT with ORDER BY | DISTINCT
    return store_to_mem(from, length, cs, table->in_use->variables.group_concat_max_len, table->blob_storage);
...
String *Field_blob::val_str(String *val_buffer MY_ATTRIBUTE((unused)), String *val_ptr)
{
  ASSERT_COLUMN_MARKED_FOR_READ;
  char *blob;
  memcpy(&blob, ptr + packlength, sizeof(char *));
...
int Field_blob::cmp(const uchar *a, uint32 a_length, const uchar *b, uint32 b_length)
{
  return field_charset->coll->strnncollsp(field_charset, a, a_length, b, b_length, 0);
}

int Field_blob::cmp_max(const uchar *a_ptr, const uchar *b_ptr, uint max_length)
{
  uchar *blob1, *blob2;
...
int Field_blob::cmp_binary(const uchar *a_ptr, const uchar *b_ptr, uint32 max_length)
{
  char *a, *b;
  uint diff;
  uint32 a_length, b_length;
...
size_t Field_blob::get_key_image(uchar *buff, size_t length, imagetype type_arg)
{
  ...
  get_ptr(&blob);
  uint local_char_length = length / field_charset->mbmaxlen;
  local_char_length = my_charpos(field_charset, blob, blob + blob_length, local_char_length);
...
int Field_blob::key_cmp(const uchar *key_ptr, uint max_key_length)
{
  ...
  local_char_length = my_charpos(cs, blob1, blob1 + blob_length, local_char_length);
  set_if_smaller(blob_length, local_char_length);
  return Field_blob::cmp(blob1, blob_length, key_ptr + HA_KEY_BLOB_LENGTH, uint2korr(key_ptr));
}
...
void Field_blob::make_sort_key(uchar *to, size_t length)
{
  ...
  else
  {
    ...
    blob_length = field_charset->coll->strnxfrm(field_charset, to, length, length, blob, blob_length,
                                                MY_STRXFRM_PAD_WITH_SPACE | MY_STRXFRM_PAD_TO_MAXLEN);
...
uchar *Field_blob::pack(uchar *to, const uchar *from, uint max_length, bool low_byte_first)
{
  uchar *save = ptr;
  ptr = (uchar *)from;
...
/**
   ...
   @return  New pointer into memory based on from + length of the data
*/
const uchar *Field_blob::unpack(uchar *to, const uchar *from, uint param_data, bool low_byte_first)
{
  ...
  Field_blob::store(pointer_cast<const char *>(from) + master_packlength, length, field_charset);
#ifndef NDEBUG
  uchar *vptr;
  get_ptr(&vptr);
...
uint Field_blob::is_equal(Create_field *new_field)
{
  return ((new_field->sql_type == get_blob_type_from_length(max_data_length())) &&
          new_field->charset == field_charset && new_field->pack_length == pack_length());
}
...
type_conversion_status Field_geom::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
...
type_conversion_status Field_geom::store_internal(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ...
  if (length < 13 ||                                                   // 1
      ...
      !Geometry::is_well_formed(from, length,                          // 5
                                geometry_type_to_wkb_type(geom_type), Geometry::wkb_ndr))
  {
    memset(ptr, 0, Field_blob::pack_length());
    my_message(ER_CANT_CREATE_GEOMETRY_OBJECT, ER(ER_CANT_CREATE_GEOMETRY_OBJECT), MYF(0));
    return TYPE_ERR_BAD_VALUE;
...
{
  return new_field->sql_type == real_type() && new_field->geom_type == get_geometry_type() &&
         new_field->charset == field_charset && new_field->pack_length == pack_length();
}
...
*/
type_conversion_status Field_json::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
...
type_conversion_status Field_enum::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  int err = 0;
  type_conversion_status ret = TYPE_OK;
...
String *Field_enum::val_str(String *val_buffer MY_ATTRIBUTE((unused)), String *val_ptr)
{
  uint tmp = (uint)Field_enum::val_int();
  if (!tmp || tmp > typelib->count)
    val_ptr->set("", 0, field_charset);
  else
    val_ptr->set(typelib->type_names[tmp - 1], typelib->type_lengths[tmp - 1], field_charset);
  return val_ptr;
}
...
type_conversion_status Field_set::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  bool got_warning = 0;
  int err = 0;
  ...
  if (!tmp && length && length < 22)
  {
    ...
    if (err || end != from + length || (typelib->count < 64 && tmp >= (1ULL << typelib->count)))
    {
      tmp = 0;
      set_warning(Sql_condition::SL_WARNING, WARN_DATA_TRUNCATED, 1);
      ret = TYPE_WARN_TRUNCATED;
...
String *Field_set::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
...
static bool compare_type_names(const CHARSET_INFO *charset, TYPELIB *t1, TYPELIB *t2)
{
  for (uint i = 0; i < t1->count; i++)
    if (my_strnncoll(charset, (const uchar *)t1->type_names[i], t1->type_lengths[i], (const uchar *)t2->type_names[i],
                     t2->type_lengths[i]))
      return FALSE;
  return TRUE;
}
...
uchar *Field_enum::pack(uchar *to, const uchar *from, uint max_length, bool low_byte_first)
{
...
const uchar *Field_enum::unpack(uchar *to, const uchar *from, uint param_data, bool low_byte_first)
{
...
size_t Field_bit::do_last_null_byte() const
{
  ...
  */
  DBUG_PRINT("test", ("bit_ofs: %d, bit_len: %d  bit_ptr: 0x%lx", bit_ofs, bit_len, (long)bit_ptr));
  const uchar *result;
  if (bit_len == 0)
    result = get_null_ptr();
...
Field *Field_bit::new_key_field(MEM_ROOT *root, TABLE *new_table, uchar *new_ptr, uchar *new_null_ptr,
                                uint new_null_bit)
{
  Field_bit *res;
...
{
  return (new_field->sql_type == real_type() && new_field->length == max_display_length());
}

type_conversion_status Field_bit::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  int delta;
...
String *Field_bit::val_str(String *val_buffer, String *val_ptr MY_ATTRIBUTE((unused)))
{
  ASSERT_COLUMN_MARKED_FOR_READ;
...
/**
   ...
   @return @c true
*/
bool Field_bit::compatible_field_size(uint field_metadata, Relay_log_info *MY_ATTRIBUTE((unused)), uint16 mflags,
                                      int *order_var)
{
...
uchar *Field_bit::pack(uchar *to, const uchar *from, uint max_length, bool low_byte_first MY_ATTRIBUTE((unused)))
{
  assert(max_length > 0);
  uint length;
  if (bit_len > 0)
...
type_conversion_status Field_bit_as_char::store(const char *from, size_t length, const CHARSET_INFO *cs)
{
  ASSERT_COLUMN_MARKED_FOR_WRITE;
  int delta;
...
*/
void Create_field::init_for_tmp_table(enum_field_types sql_type_arg, uint32 length_arg, uint32 decimals_arg,
                                      bool maybe_null, bool is_unsigned, uint pack_length_arg)
{
  ...
  DBUG_PRINT("enter", ("sql_type: %d, length: %u, pack_length: %u", sql_type_arg, length_arg, pack_length_arg));
  ...
  switch (sql_type_arg)
  {
    ...
    case MYSQL_TYPE_DOUBLE:
      pack_flag = FIELDFLAG_NUMBER | (decimals_arg & FIELDFLAG_MAX_DEC) << FIELDFLAG_DEC_SHIFT;
      break;
  ...
  pack_flag |= (maybe_null ? FIELDFLAG_MAYBE_NULL : 0) | (is_unsigned ? 0 : FIELDFLAG_DECIMAL);

  gcol_info = 0;
  stored_in_db = TRUE;

  DBUG_PRINT("debug",
             ("pack_flag: %s%s%s%s%s%s%s, pack_type: %d", FLAGSTR(pack_flag, FIELDFLAG_BINARY),
              FLAGSTR(pack_flag, FIELDFLAG_NUMBER), FLAGSTR(pack_flag, FIELDFLAG_INTERVAL),
              FLAGSTR(pack_flag, FIELDFLAG_GEOM), FLAGSTR(pack_flag, FIELDFLAG_BLOB),
              FLAGSTR(pack_flag, FIELDFLAG_DECIMAL), FLAGSTR(pack_flag, FIELDFLAG_JSON), f_packtype(pack_flag)));
  DBUG_VOID_RETURN;
}
...
bool Create_field::init(THD *thd, const char *fld_name, enum_field_types fld_type, const char *fld_length,
                        const char *fld_decimals, uint fld_type_modifier, Item *fld_default_value,
                        Item *fld_on_update_value, LEX_STRING *fld_comment, const char *fld_change,
                        List<String> *fld_interval_list, const CHARSET_INFO *fld_charset, uint fld_geom_type,
                        Generated_column *fld_gcol_info)
{
  ...
  const bool on_update_is_function = (fld_on_update_value != NULL && fld_on_update_value->type() == Item::FUNC_ITEM);

  if (fld_default_value != NULL && fld_default_value->type() == Item::FUNC_ITEM)
  {
    // We have a function default for insertions.
    def = NULL;
    unireg_check = on_update_is_function ? Field::TIMESTAMP_DNUN_FIELD :  // for insertions and for updates.
  ...
  else
  {
    ...
    if (on_update_is_function)
      ...
      unireg_check = (fld_type_modifier & AUTO_INCREMENT_FLAG) != 0 ? Field::NEXT_NUMBER :  // Automatic increment.
                         Field::NONE;
  }
  ...
  if (is_temporal_real_type(fld_type))
  {
    ...
    {
      my_error(ER_TOO_BIG_PRECISION, MYF(0), decimals, fld_name, DATETIME_MAX_DECIMALS);
      DBUG_RETURN(TRUE);
    }
  }
  else if (decimals >= NOT_FIXED_DEC)
  {
    my_error(ER_TOO_BIG_SCALE, MYF(0), decimals, fld_name, static_cast<ulong>(NOT_FIXED_DEC - 1));
    DBUG_RETURN(TRUE);
  }
  ...
  */
  if (!fld_default_value && (fld_type_modifier & NOT_NULL_FLAG) && !(fld_type_modifier & AUTO_INCREMENT_FLAG))
  {
    /*
      TIMESTAMP columns get implicit DEFAULT value when
      explicit_defaults_for_timestamp is not set.
    */
    if (thd->variables.explicit_defaults_for_timestamp || !is_timestamp_type(fld_type))
    {
  ...
  switch (fld_type)
  {
    ...
    case MYSQL_TYPE_NEWDECIMAL:
    {
      ...
      {
        my_error(ER_TOO_BIG_PRECISION, MYF(0), static_cast<int>(length), fld_name,
    ...
    case MYSQL_TYPE_JSON:
      if (fld_default_value)
      {
        ...
        */
        ...
        {
          my_error(ER_BLOB_CANT_HAVE_DEFAULT, MYF(0), fld_name); /* purecov: inspected */
          DBUG_RETURN(TRUE);
        }
        else
        {
          ...
          */
          push_warning_printf(thd, Sql_condition::SL_WARNING, ER_BLOB_CANT_HAVE_DEFAULT, ER(ER_BLOB_CANT_HAVE_DEFAULT),
                              fld_name);
        }
        def = 0;
      }
    ...
    case MYSQL_TYPE_BIT:
    {
      ...
      {
        my_error(ER_TOO_BIG_DISPLAYWIDTH, MYF(0), fld_name, static_cast<ulong>(MAX_BIT_FIELD_LENGTH));
        DBUG_RETURN(TRUE);
      }
  ...
  if (!(flags & BLOB_FLAG) && ((length > max_field_charlength && fld_type != MYSQL_TYPE_SET &&
                                fld_type != MYSQL_TYPE_ENUM && (fld_type != MYSQL_TYPE_VARCHAR || fld_default_value)) ||
                               ((length == 0) && fld_type != MYSQL_TYPE_STRING && fld_type != MYSQL_TYPE_VARCHAR &&
                                fld_type != MYSQL_TYPE_GEOMETRY)))
  {
    my_error((fld_type == MYSQL_TYPE_VAR_STRING || fld_type == MYSQL_TYPE_VARCHAR || fld_type == MYSQL_TYPE_STRING)
...
Field *make_field(TABLE_SHARE *share, uchar *ptr, size_t field_length, uchar *null_pos, uchar null_bit, uint pack_flag,
                  enum_field_types field_type, const CHARSET_INFO *field_charset, Field::geometry_type geom_type,
                  Field::utype unireg_check, TYPELIB *interval, const char *field_name)
{
  ...
  DBUG_PRINT("debug", ("field_type: %d, field_length: %zu, "
                       ...
                       field_type, field_length, interval, FLAGSTR(pack_flag, FIELDFLAG_BINARY),
  ...
  if (f_is_alpha(pack_flag))
  {
    ...
    {
      if (field_type == MYSQL_TYPE_STRING || field_type == MYSQL_TYPE_DECIMAL ||  // 3.23 or 4.0 string
          field_type == MYSQL_TYPE_VAR_STRING)
        return new Field_string(ptr, field_length, null_pos, null_bit, unireg_check, field_name, field_charset);
    ...
    if (f_is_geom(pack_flag))
      return new Field_geom(ptr, null_pos, null_bit, unireg_check, field_name, share, pack_length, geom_type);
    if (f_is_json(pack_flag))
      return new Field_json(ptr, null_pos, null_bit, unireg_check, field_name, share, pack_length);
    if (f_is_blob(pack_flag))
      return new Field_blob(ptr, null_pos, null_bit, unireg_check, field_name, share, pack_length, field_charset);
    if (interval)
    {
      if (f_is_enum(pack_flag))
        return new Field_enum(ptr, field_length, null_pos, null_bit, unireg_check, field_name, pack_length, interval,
                              field_charset);
      else
        return new Field_set(ptr, field_length, null_pos, null_bit, unireg_check, field_name, pack_length, interval,
                             field_charset);
    }
  }
  ...
  {
    case MYSQL_TYPE_DECIMAL:
      return new Field_decimal(ptr, field_length, null_pos, null_bit, unireg_check, field_name, f_decimals(pack_flag),
                               f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_NEWDECIMAL:
      return new Field_new_decimal(ptr, field_length, null_pos, null_bit, unireg_check, field_name,
                                   f_decimals(pack_flag), f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_FLOAT:
      return new Field_float(ptr, field_length, null_pos, null_bit, unireg_check, field_name, f_decimals(pack_flag),
                             f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_DOUBLE:
      return new Field_double(ptr, field_length, null_pos, null_bit, unireg_check, field_name, f_decimals(pack_flag),
                              f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_TINY:
      return new Field_tiny(ptr, field_length, null_pos, null_bit, unireg_check, field_name,
                            f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_SHORT:
      return new Field_short(ptr, field_length, null_pos, null_bit, unireg_check, field_name,
                             f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_INT24:
      return new Field_medium(ptr, field_length, null_pos, null_bit, unireg_check, field_name,
                              f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_LONG:
      return new Field_long(ptr, field_length, null_pos, null_bit, unireg_check, field_name,
                            f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_LONGLONG:
      return new Field_longlong(ptr, field_length, null_pos, null_bit, unireg_check, field_name,
                                f_is_zerofill(pack_flag) != 0, f_is_dec(pack_flag) == 0);
    case MYSQL_TYPE_TIMESTAMP:
      return new Field_timestamp(ptr, field_length, null_pos, null_bit, unireg_check, field_name);
    case MYSQL_TYPE_TIMESTAMP2:
      return new Field_timestampf(ptr, null_pos, null_bit, unireg_check, field_name,
                                  field_length > MAX_DATETIME_WIDTH ? field_length - 1 - MAX_DATETIME_WIDTH : 0);
    case MYSQL_TYPE_YEAR:
      return new Field_year(ptr, field_length, null_pos, null_bit, unireg_check, field_name);
    case MYSQL_TYPE_NEWDATE:
    ...
    case MYSQL_TYPE_TIME2:
      return new Field_timef(ptr, null_pos, null_bit, unireg_check, field_name,
                             (field_length > MAX_TIME_WIDTH) ? field_length - 1 - MAX_TIME_WIDTH : 0);
    case MYSQL_TYPE_DATETIME:
      return new Field_datetime(ptr, null_pos, null_bit, unireg_check, field_name);
    case MYSQL_TYPE_DATETIME2:
      return new Field_datetimef(ptr, null_pos, null_bit, unireg_check, field_name,
                                 (field_length > MAX_DATETIME_WIDTH) ? field_length - 1 - MAX_DATETIME_WIDTH : 0);
    case MYSQL_TYPE_NULL:
      return new Field_null(ptr, field_length, unireg_check, field_name, field_charset);
    case MYSQL_TYPE_BIT:
      return f_bit_as_char(pack_flag)
                 ? new Field_bit_as_char(ptr, field_length, null_pos, null_bit, unireg_check, field_name)
                 : new Field_bit(ptr, field_length, null_pos, null_bit, bit_ptr, bit_offset, unireg_check, field_name);
...
bool Field_blob::copy_blob_value(MEM_ROOT *mem_root)
{
  ...
  DBUG_EXECUTE_IF("simulate_blob_memory_allocation_fail", DBUG_SET("+d,simulate_out_of_memory"););
...
bool Field::set_warning(Sql_condition::enum_severity_level level, uint code, int cut_increment,
                        const char *view_db_name, const char *view_name)
{
  /*
  ...
  */
  ...
  if (thd->lex->sql_command != SQLCOM_INSERT && thd->lex->sql_command != SQLCOM_INSERT_SELECT &&
      thd->lex->sql_command != SQLCOM_REPLACE && thd->lex->sql_command != SQLCOM_REPLACE_SELECT)
  {
    // We aggregate warnings from only INSERT and REPLACE statements.

    push_warning_printf(thd, level, code, ER(code), field_name, thd->get_stmt_da()->current_row_for_condition());
  ...
  else if (code == ER_NO_DEFAULT_FOR_VIEW_FIELD)
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, ER_NO_DEFAULT_FOR_VIEW_FIELD,
                          ER(ER_NO_DEFAULT_FOR_VIEW_FIELD), view_db_name, view_name);
...
/**
  ...
  @note
    ...
    thread.
*/
void Field_temporal::set_datetime_warning(Sql_condition::enum_severity_level level, uint code, ErrConvString val,
                                          timestamp_type ts_type, int cut_increment)
{
  THD *thd = table ? table->in_use : current_thd;
  if ((!thd->lex->is_ignore() && ((thd->variables.sql_mode & MODE_STRICT_ALL_TABLES) ||
...
{
  return thd->optimizer_switch_flag(OPTIMIZER_SWITCH_USE_INDEX_EXTENSIONS) && !(cur_index_info->flags & HA_NOSAME)
