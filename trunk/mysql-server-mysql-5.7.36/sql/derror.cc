...
static const int NUM_SECTIONS = sizeof(errmsg_section_start) / sizeof(errmsg_section_start[0]);
...
const char *MY_LOCALE_ERRMSGS::lookup(int mysql_errno)
{
  ...
  {
    if (mysql_errno >= errmsg_section_start[i] && mysql_errno < (errmsg_section_start[i] + errmsg_section_size[i]))
...
bool init_errmessage()
{
  ...
  for (int i = 0; i < NUM_SECTIONS; i++)
  {
    if (my_error_register(get_server_errmsgs, errmsg_section_start[i],
...
void deinit_errmessage()
{
  for (int i = 0; i < NUM_SECTIONS; i++)
  {
    my_error_unregister(errmsg_section_start[i], errmsg_section_start[i] + errmsg_section_size[i] - 1);
  }
}
...
bool MY_LOCALE_ERRMSGS::read_texts()
{
  ...
  if ((file = mysql_file_open(key_file_ERRMSG, fn_format(name, ERRMSG_FILE, lang_path, "", 4),
                              O_RDONLY | O_SHARE | O_BINARY, MYF(0))) < 0)
  {
    ...
    */
    if ((file = mysql_file_open(key_file_ERRMSG, fn_format(name, ERRMSG_FILE, lc_messages_dir, "", 4),
                                O_RDONLY | O_SHARE | O_BINARY, MYF(0))) < 0)
    {
      sql_print_error(
          "Can't find error-message file '%s'. Check error-message"
    ...
    sql_print_warning("Using pre 5.5 semantics to load error messages from %s.", lc_messages_dir);
  ...
  if (mysql_file_read(file, (uchar *)head, 32, MYF(MY_NABP)))
    goto read_err;
  if (head[0] != (uchar)254 || head[1] != (uchar)254 || head[2] != 3 || head[3] != 1 || head[4] != 1)
    goto read_err;
  ...
  my_free(errmsgs);
  if (!(errmsgs = (const char **)my_malloc(key_memory_errmsgs, length + no_of_errmsgs * sizeof(char *), MYF(0))))
  {
  ...
  */
  if (mysql_file_read(file, start_of_errmsgs, (size_t)no_of_errmsgs * 4, MYF(MY_NABP)))
    goto read_err_init;
