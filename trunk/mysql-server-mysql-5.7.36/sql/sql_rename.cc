...
bool mysql_rename_tables(THD *thd, TABLE_LIST *table_list, bool silent)
{
  ...
  */
  ...
  if (query_logger.is_log_table_enabled(QUERY_LOG_GENERAL) || query_logger.is_log_table_enabled(QUERY_LOG_SLOW))
  {
    /*
        ...
        */
        ...
        if (rename_log_table[log_table_rename])
        {
          ...
          else
          {
            /*
              ...
              between.
            */
            my_error(ER_CANT_RENAME_LOG_TABLE, MYF(0), ren_table->table_name, ren_table->table_name);
            goto err;
          }
        }
        else
        {
          ...
          else
          {
            ...
            rename_log_table[log_table_rename] = const_cast<char *>(ren_table->table_name);
          }
        }
      }
    }
  ...
  for (ren_table = table_list; ren_table; ren_table = ren_table->next_local)
    tdc_remove_table(thd, TDC_RT_REMOVE_ALL, ren_table->db, ren_table->table_name, FALSE);
  ...
  */
  ...
  if (!silent && !error)
  {
    binlog_error = write_bin_log(thd, true, thd->query().str, thd->query().length);
    if (!binlog_error)
      my_ok(thd);
  }
               ...
               const char *new_table_alias, bool skip_error)
{
  ...
  switch (frm_type)
  {
    case FRMTYPE_TABLE:
    {
      ...
      if (!(rc = mysql_rename_table(hton, ren_table->db, old_alias, new_db, new_alias, 0)))
      {
        if ((rc = change_trigger_table_name(thd, ren_table->db, old_alias, ren_table->table_name, new_db, new_alias)))
        {
          /*
          ...
          */
          (void)mysql_rename_table(hton, new_db, new_alias, ren_table->db, old_alias, NO_FK_CHECKS);
        }
      }
    }
    break;
    case FRMTYPE_VIEW:
      ...
      */
      if (thd->lex->sql_command != SQLCOM_ALTER_DB_UPGRADE && strcmp(ren_table->db, new_db))
    ...
    case FRMTYPE_ERROR:
    {
      ...
      my_error(ER_FILE_NOT_FOUND, MYF(0), name, my_errno(), my_strerror(errbuf, sizeof(errbuf), my_errno()));
    }
    break;
  }
...
static TABLE_LIST *rename_tables(THD *thd, TABLE_LIST *table_list, bool skip_error)
{
  ...
  for (ren_table = table_list; ren_table; ren_table = new_table->next_local)
  {
    ...
    if (do_rename(thd, ren_table, new_table->db, new_table->table_name, new_table->alias, skip_error))
