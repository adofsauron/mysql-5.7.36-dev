...
table_mapping::table_mapping() : m_free(0)
{
  ...
  */
  (void)my_hash_init(&m_table_ids, &my_charset_bin, TABLE_ID_HASH_SIZE, offsetof(entry, table_id), sizeof(ulonglong), 0,
                     0, 0, psi_key);
  /* We don't preallocate any block, this is consistent with m_free=0 above */
  init_alloc_root(psi_key, &m_mem_root, TABLE_ID_HASH_SIZE * sizeof(entry), 0);
}
...
TABLE *table_mapping::get_table(ulonglong table_id)
{
  ...
  if (e)
  {
    DBUG_PRINT("info", ("tid %llu -> table 0x%lx (%s)", table_id, (long)e->table, MAYBE_TABLE_NAME(e->table)));
    DBUG_RETURN(e->table);
  }
...
int table_mapping::set_table(ulonglong table_id, TABLE *table)
{
  ...
  DBUG_PRINT("enter", ("table_id: %llu  table: 0x%lx (%s)", table_id, (long)table, MAYBE_TABLE_NAME(table)));
  entry *e = find_entry(table_id);
  if (e == 0)
  {
    if (m_free == 0 && expand())
