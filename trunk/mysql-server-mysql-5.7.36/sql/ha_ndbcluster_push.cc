...
#define EXPLAIN_NO_PUSH(msgfmt, ...)                                                           \
  ...
  {                                                                                            \
    ...
    {                                                                                          \
      push_warning_printf(current_thd, Sql_condition::SL_NOTE, ER_YES, (msgfmt), __VA_ARGS__); \
    }                                                                                          \
  } while (0)
...
{
  return accessType == AQP::AT_PRIMARY_KEY || accessType == AQP::AT_MULTI_PRIMARY_KEY ||
...
ndb_pushed_join::ndb_pushed_join(const ndb_pushed_builder_ctx &builder, const NdbQueryDef *query_def)
    : m_query_def(query_def), m_operation_count(0), m_field_count(builder.m_fld_refs)
{
...
{
  const NdbQueryOperationDef *const root_operation = m_query_def->getQueryOperation((uint)0);
  const NdbQueryOperationDef::Type def_type = root_operation->getType();

  if (def_type != type)
  {
    DBUG_PRINT("info", ("Cannot execute push join. Root operation prepared as %s "
                        "not executable as %s",
...
NdbQuery *ndb_pushed_join::make_query_instance(NdbTransaction *trans, const NdbQueryParamValue *keyFieldParams,
                                               uint paramCnt) const
{
  ...
  DBUG_PRINT("info", ("executing chain of %d pushed joins."
                      ...
                      NdbQueryOperationDef::getTypeName(m_query_def->getQueryOperation((uint)0)->getType())));
      ...
      m_builder(NULL)
{
  ...
  if (count > 1)
  {
    for (uint i = 0; i < count; i++)
    {
      ...
      {
        DBUG_PRINT("info", ("Table '%s' not in ndb engine, not pushable", table->get_table()->alias));
        continue;
      }
      ...
      switch (table->get_access_type())
      {
        ...
        case AQP::AT_FIXED:
          EXPLAIN_NO_PUSH("Table '%s' was optimized away, or const'ified by optimizer", table->get_table()->alias);
          break;

        case AQP::AT_OTHER:
          EXPLAIN_NO_PUSH("Table '%s' is not pushable: %s", table->get_table()->alias,
        ...
        default:
          ...
          const ha_ndbcluster *handler = static_cast<ha_ndbcluster *>(table->get_table()->file);
          ...
          {
            EXPLAIN_NO_PUSH("Table '%s' is not pushable: %s", table->get_table()->alias, reason);
          }
          break;
      }  // switch
       ...
       */
      const AQP::Table_access *const firstmatch_last_skipped = table->get_firstmatch_last_skipped();
      if (firstmatch_last_skipped)
      {
        const uint last_skipped_tab = firstmatch_last_skipped->get_access_no();
        assert(last_skipped_tab <= i);
...
uint ndb_pushed_builder_ctx::get_table_no(const Item *key_item) const
{
  ...
  for (uint i = 0; i < MAX_TABLES && bitmap != 0; i++, bitmap >>= 1)
  {
    if (bitmap & 1)
    {
      assert(bitmap == 0x01);  // Only a single table in 'bitmap'
 ...
 *
 */
int ndb_pushed_builder_ctx::make_pushed_join(const AQP::Table_access *join_root, const ndb_pushed_join *&pushed_join)
{
  ...
  if (is_pushable_with_root(join_root))
  {
    ...
    DBUG_PRINT("info", ("Created pushed join with %d child operations", pushed_join->get_operation_count() - 1));
  }
  DBUG_RETURN(0);
 ...
 * it pushable .
 ****************************************************************/
bool ndb_pushed_builder_ctx::is_pushable_as_child(const AQP::Table_access *table)
{
  ...
  if (!(ndbcluster_is_lookup_operation(access_type) || access_type == AQP::AT_ORDERED_INDEX_SCAN))
  {
    EXPLAIN_NO_PUSH("Can't push table '%s' as child, 'type' must be a 'ref' access", table->get_table()->alias);
  ...
  if (access_type == AQP::AT_ORDERED_INDEX_SCAN && ndbcluster_is_lookup_operation(root_type))
  {
    EXPLAIN_NO_PUSH(
  ...
  for (uint i = tab_no; i > root_no; i--)
  {
    if (m_plan.get_table_access(i)->uses_join_cache())
    {
      EXPLAIN_NO_PUSH(
          ...
          table->get_table()->alias, m_join_root->get_table()->alias, m_plan.get_table_access(i)->get_table()->alias);
      DBUG_RETURN(false);
    }
  }
  ...
  if (unlikely(m_internal_op_count + internal_ops_needed > NDB_SPJ_MAX_TREE_NODES))
  {
    EXPLAIN_NO_PUSH(
  ...
  DBUG_PRINT("info", ("Table:%d, Checking %d REF keys", tab_no, table->get_no_of_key_fields()));
  ...
  for (uint key_part_no = 0; key_part_no < table->get_no_of_key_fields(); key_part_no++)
  {
    ...
    else if (key_item->type() == Item::FIELD_ITEM)
    {
       ...
       */
      assert(key_item == table->get_key_field(key_part_no));
      assert(key_item->type() == Item::FIELD_ITEM);
  ...
  assert(!depend_parents.is_clear_all());
  assert(!depend_parents.contain(tab_no));  // Circular dependency!
  ...
  while (true)
  {
    ...
    const ndb_table_access_map &ancestor_dependencies = m_tables[common_ancestor_no].m_depend_parents;
    const uint first_ancestor = ancestor_dependencies.last_table(common_ancestor_no - 1);
    dependencies.add(first_ancestor);
  }  // while

  const AQP::Table_access *const common_ancestor = m_plan.get_table_access(common_ancestor_no);
  ...
  for (uint ancestor_no = dependencies.last_table(tab_no - 1); ancestor_no != common_ancestor_no;
       ancestor_no = dependencies.last_table(ancestor_no - 1))
  {
     ...
     */
    if (depend_parents.contain(ancestor_no) && ancestor->get_join_type(common_ancestor) == AQP::JT_OUTER_JOIN)
    {
      /**
      ...
      if (child_dependencies.is_clear_all())
      {
         ...
         */
        assert(extend_parents.contain(common_parents));
        for (uint parent_no = extend_parents.last_table(tab_no - 1); parent_no > ancestor_no;
        ...
        assert(!extend_parents.is_clear_all());
      }
      else if (!child_dependencies.contain(ancestor_no))
      {
        ...
        EXPLAIN_NO_PUSH(
            ...
            table->get_table()->alias, m_join_root->get_table()->alias, ancestor->get_table()->alias);
        DBUG_RETURN(false);
      }
    }
  ...
  assert(m_join_scope.contain(common_parents));
  assert(m_join_scope.contain(extend_parents));
  assert(extend_parents.is_clear_all() || extend_parents.contain(common_parents));
  /**
 ...
 */
bool ndb_pushed_builder_ctx::is_field_item_pushable(const AQP::Table_access *table, const Item *key_item,
                                                    const KEY_PART_INFO *key_part, ndb_table_access_map &field_parents)
{
  ...
  DBUG_PRINT("info", ("keyPart:%d, field:%s.%s", (int)(key_item - table->get_key_field(0)),
  ...
  if (!key_item_field->field->eq_def(key_part->field))
  {
    EXPLAIN_NO_PUSH(
        ...
        "column '%s.%s'",
        table->get_table()->alias, key_part->field->field_name, key_item_field->field->table->alias,
  ...
  if (item_equal)
  {
    ...
    while ((substitute_field = equal_iter.next()) != NULL)
    {
      if (substitute_field != key_item_field)
      {
        ...
        {
          DBUG_PRINT("info",
                     (" join_items[%d] %s.%s can be replaced with %s.%s", (int)(key_item - table->get_key_field(0)),
                      get_referred_table_access_name(key_item_field), get_referred_field_name(key_item_field),
                      get_referred_table_access_name(substitute_field), get_referred_field_name(substitute_field)));
  ...
  if (m_const_scope.contain(referred_table_no))
  {
    ...
    if (ndbcluster_is_lookup_operation(m_join_root->get_access_type()))
    {
      ...
      EXPLAIN_NO_PUSH(
          ...
          table->get_table()->alias, m_join_root->get_table()->alias, get_referred_table_access_name(key_item_field),
...
bool ndb_pushed_builder_ctx::is_const_item_pushable(const Item *key_item, const KEY_PART_INFO *key_part)
{
  DBUG_ENTER("is_const_item_pushable()");
  assert(key_item->const_item());
  ...
  const int error = const_cast<Item *>(key_item)->save_in_field_no_warnings(field, true);
  if (unlikely(error))
  {
...
int ndb_pushed_builder_ctx::optimize_query_plan()
{
  ...
  for (uint tab_no = m_plan.get_access_count() - 1; tab_no > root_no; tab_no--)
  {
    if (!m_join_scope.contain(tab_no))
      continue;
    ...
    if (!table.m_depend_parents.is_clear_all())
    {
      ...
      assert(!dependency.contain(tab_no));  // Circular dependency!
    ...
    ndb_table_access_map const &parents =
        table.m_common_parents.is_clear_all() ? table.m_extend_parents : table.m_common_parents;

    assert(!parents.is_clear_all());
    assert(!parents.contain(tab_no));  // No circular dependency!
  ...
  for (uint tab_no = root_no + 1; tab_no < m_plan.get_access_count(); tab_no++)
  {
    if (m_join_scope.contain(tab_no))
    {
      pushed_tables &table = m_tables[tab_no];
      ...
      assert(table.m_ancestors.contain(table.m_depend_parents));
    }
  }
  DBUG_RETURN(0);
}  // ndb_pushed_builder_ctx::optimize_query_plan

void ndb_pushed_builder_ctx::collect_key_refs(const AQP::Table_access *table, const Item *key_refs[]) const
{
  ...
  for (uint key_part_no = 0; key_part_no < table->get_no_of_key_fields(); key_part_no++)
  {
    ...
    assert(key_item->const_item() || key_item->type() == Item::FIELD_ITEM);
    ...
    if (key_item->type() == Item::FIELD_ITEM)
    {
      ...
      if (referred_table_no != parent_no && (item_equal = table->get_item_equal(join_item)) != NULL)
      {
        ...
        while ((substitute_field = iter.next()) != NULL)
        {
          ...
          {
            DBUG_PRINT("info",
                       (" Replacing key_refs[%d] %s.%s with %s.%s (parent)", key_part_no,
                        get_referred_table_access_name(join_item), get_referred_field_name(join_item),
                        get_referred_table_access_name(substitute_field), get_referred_field_name(substitute_field)));
          ...
          else if (ancestors.contain(substitute_table_no))
          {
            ...
            {
              DBUG_PRINT("info",
                         (" Replacing key_refs[%d] %s.%s with %s.%s (grandparent)", key_part_no,
                          get_referred_table_access_name(join_item), get_referred_field_name(join_item),
                          get_referred_table_access_name(substitute_field), get_referred_field_name(substitute_field)));
        ...
        assert(referred_table_no == parent_no || !m_join_scope.contain(referred_table_no) ||  // Is a 'const' paramValue
...
int ndb_pushed_builder_ctx::build_key(const AQP::Table_access *table, const NdbQueryOperand *op_key[])
{
  ...
  else
  {
    ...
    {
      const ha_ndbcluster *handler = static_cast<ha_ndbcluster *>(table->get_table()->file);
      ndbcluster_build_key_map(handler->m_table, handler->m_index[table->get_index_no()], key, map);
    }
    else
    {
    ...
    for (uint i = 0; i < key_fields; i++, key_part++)
    {
      ...
      assert(item->const_item() == item->const_during_execution());
      if (item->const_item())
      {
        /**
         * Propagate Items constant value to Field containing the value of this
      ...
      else
      {
        ...
        if (m_join_scope.contain(referred_table_no))
        {
          ...
          op_key[map[i]] = m_builder->linkedValue(parent_op, field_item->field_name);
        }
        else
        {
  ...
  for (uint tab_no = root_no; tab_no < m_plan.get_access_count(); tab_no++)
  {
    ...
    if (ndbcluster_is_lookup_operation(access_type))
    {
      ...
      else
      {
        ...
        const NdbDictionary::Index *const index = handler->m_index[table->get_index_no()].unique_index;
        assert(index);
     ...
     */
    else if (access_type == AQP::AT_ORDERED_INDEX_SCAN || access_type == AQP::AT_MULTI_MIXED)
    {
      ...
      DBUG_PRINT("info", ("Creating scanIndex on index id:%d, name:%s", table->get_index_no(),
                          handler->m_index[table->get_index_no()].index->getName()));

      const NdbQueryIndexBound bounds(op_key);
      query_op =
          m_builder->scanIndex(handler->m_index[table->get_index_no()].index, handler->m_table, &bounds, &options);
    }
 ...
 */
void ndbcluster_build_key_map(const NDBTAB *table, const NDB_INDEX_DATA &index, const KEY *key_def, uint ix_map[])
{
  uint ix;
