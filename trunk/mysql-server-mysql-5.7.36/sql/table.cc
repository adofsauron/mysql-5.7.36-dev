...
static Item *create_view_field(THD *thd, TABLE_LIST *view, Item **field_ref, const char *name,
...
View_creation_ctx *View_creation_ctx::create(THD *thd, TABLE_LIST *view)
{
  ...
  {
    push_warning_printf(thd, Sql_condition::SL_NOTE, ER_VIEW_NO_CREATION_CTX, ER(ER_VIEW_NO_CREATION_CTX), view->db,
                        view->table_name);
  ...
  invalid_creation_ctx = resolve_charset(view->view_client_cs_name.str, system_charset_info, &ctx->m_client_cs);

  invalid_creation_ctx =
      resolve_collation(view->view_connection_cl_name.str, system_charset_info, &ctx->m_connection_cl) ||
      invalid_creation_ctx;
  ...
  if (invalid_creation_ctx)
  {
    sql_print_warning(
        ...
        view->db, view->table_name, view->view_client_cs_name.str, view->view_connection_cl_name.str);

    push_warning_printf(thd, Sql_condition::SL_NOTE, ER_VIEW_INVALID_CREATION_CTX, ER(ER_VIEW_INVALID_CREATION_CTX),
...
static uchar *get_field_name(Field **buff, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
...
static TABLE_CATEGORY get_table_category(const LEX_STRING &db, const LEX_STRING &name)
{
  assert(db.str != NULL);
      ...
      (my_strcasecmp(system_charset_info, MYSQL_SCHEMA_NAME.str, db.str) == 0))
  {
    ...
    if ((name.length == GENERAL_LOG_NAME.length) &&
        (my_strcasecmp(system_charset_info, GENERAL_LOG_NAME.str, name.str) == 0))
      return TABLE_CATEGORY_LOG;

    if ((name.length == SLOW_LOG_NAME.length) && (my_strcasecmp(system_charset_info, SLOW_LOG_NAME.str, name.str) == 0))
      return TABLE_CATEGORY_LOG;

    if ((name.length == RLI_INFO_NAME.length) && (my_strcasecmp(system_charset_info, RLI_INFO_NAME.str, name.str) == 0))
      return TABLE_CATEGORY_RPL_INFO;

    if ((name.length == MI_INFO_NAME.length) && (my_strcasecmp(system_charset_info, MI_INFO_NAME.str, name.str) == 0))
      return TABLE_CATEGORY_RPL_INFO;

    if ((name.length == WORKER_INFO_NAME.length) &&
        (my_strcasecmp(system_charset_info, WORKER_INFO_NAME.str, name.str) == 0))
      return TABLE_CATEGORY_RPL_INFO;

    if ((name.length == GTID_EXECUTED_NAME.length) &&
        (my_strcasecmp(system_charset_info, GTID_EXECUTED_NAME.str, name.str) == 0))
...
TABLE_SHARE *alloc_table_share(TABLE_LIST *table_list, const char *key, size_t key_length)
{
  MEM_ROOT mem_root;
  ...
  DBUG_PRINT("enter", ("table: '%s'.'%s'", table_list->db, table_list->table_name));
  ...
  */
  path_length = build_table_filename(path, sizeof(path) - 1 - reg_ext_length, table_list->db, table_list->table_name,
                                     "", 0, &was_truncated);
  ...
  */
  ...
  if (multi_alloc_root(&mem_root, &share, sizeof(*share), &key_buff, key_length, &path_buff, path_length + 1,
                       &cache_element_array, table_cache_instances * sizeof(*cache_element_array), NULL))
  {
    ...
    */
    ...
    memset(cache_element_array, 0, table_cache_instances * sizeof(*cache_element_array));
...
void init_tmp_table_share(THD *thd, TABLE_SHARE *share, const char *key, size_t key_length, const char *table_name,
                          const char *path)
{
...
int open_table_def(THD *thd, TABLE_SHARE *share, uint db_flags)
{
  ...
  if ((file = mysql_file_open(key_file_frm, path, O_RDONLY | O_SHARE, MYF(0))) < 0)
  {
    ...
    */
    if (has_disabled_path_chars(share->table_name.str) || has_disabled_path_chars(share->db.str) ||
        !strncmp(share->db.str, MYSQL50_TABLE_NAME_PREFIX, MYSQL50_TABLE_NAME_PREFIX_LENGTH) ||
        !strncmp(share->table_name.str, MYSQL50_TABLE_NAME_PREFIX, MYSQL50_TABLE_NAME_PREFIX_LENGTH))
      goto err_not_open;
    ...
    */
    ...
    size_t length;
    strxnmov(path, sizeof(path) - 1, mysql_data_home, "/", share->db.str, "/", share->table_name.str, reg_ext, NullS);
    ...
    */
    if (length == share->normalized_path.length ||
        ((file = mysql_file_open(key_file_frm, path, O_RDONLY | O_SHARE, MYF(0))) < 0))
      goto err_not_open;
...
void KEY_PART_INFO::init_from_field(Field *fld)
{
  ...
  if (field->type() == MYSQL_TYPE_BLOB || field->real_type() == MYSQL_TYPE_VARCHAR ||
  ...
  key_type = ((ha_base_keytype)type == HA_KEYTYPE_TEXT || (ha_base_keytype)type == HA_KEYTYPE_VARTEXT1 ||
...
static void setup_key_part_field(TABLE_SHARE *share, handler *handler_file, uint primary_key_n, KEY *keyinfo,
                                 uint key_n, uint key_part_n, uint *usable_parts, bool part_of_key_not_extended)
{
  ...
  if (key_part_n == 0 && key_n != primary_key_n)
    field->flags |= (((keyinfo->flags & HA_NOSAME) && (keyinfo->user_defined_key_parts == 1)) ? UNIQUE_KEY_FLAG
                                                                                              : MULTIPLE_KEY_FLAG);
  if (key_part_n == 0)
  ...
  if (!(key_part->key_part_flag & HA_REVERSE_SORT) && *usable_parts == key_part_n)
...
static uint add_pk_parts_to_sk(KEY *sk, uint sk_n, KEY *pk, uint pk_n, TABLE_SHARE *share, handler *handler_file,
  ...
  for (; pk_part < pk->user_defined_key_parts; pk_part++)
  {
    ...
    for (uint j = 0; j < sk->user_defined_key_parts; j++)
    {
      if (sk->key_part[j].fieldnr == pk_key_part->fieldnr &&
          share->field[pk_key_part->fieldnr - 1]->key_length() == sk->key_part[j].length)
      {
    ...
    if (!pk_field_is_in_sk)
    {
      ...
      */
      if ((sk->flags & HA_NOSAME) || !use_extended_sk)
        continue;

      *current_key_part = *pk_key_part;
      setup_key_part_field(share, handler_file, pk_n, sk, sk_n, sk->actual_key_parts, usable_parts, false);
      sk->actual_key_parts++;
      sk->unused_key_parts--;
...
class Tablespace_name_error_handler : public Internal_error_handler
{
 public:
  bool handle_condition(THD *thd, uint sql_errno, const char *sqlstate, Sql_condition::enum_severity_level *level,
                        const char *msg)
  {
    if (sql_errno == ER_WRONG_TABLESPACE_NAME || sql_errno == ER_TOO_LONG_IDENT)
      return true;
    return false;
  }
};

bool get_table_and_parts_tablespace_names(THD *thd, TABLE_LIST *table, Tablespace_hash_set *tablespace_set)
{
  ...
  // header fields.
  uchar head[64];
  if (mysql_file_read(file, head, sizeof(head), MYF(MY_NABP)) || head[0] != (uchar)254 || head[1] != 1 ||
      ...
      uint4korr(head + 51) < 50120)     // Version before 50120.
  {
    ...
    plugin_ref se_plugin = ha_lock_engine(NULL, ha_checktype(thd, db_type, false, false));
    ...
    if (se_hton->get_tablespace && !se_hton->get_tablespace(thd, schema_name, table_name, &ts_name))
    {
      ...
      if (ts_name.length == 0 || check_tablespace_name(ts_name.str) == IDENT_NAME_OK)
        tablespace_name = ts_name.str;
      thd->pop_internal_handler();
    }
    plugin_unlock(NULL, se_plugin);

    if (tablespace_name && strlen(tablespace_name) && tablespace_set->insert(const_cast<char *>(tablespace_name)))
    {
      mysql_file_close(file, MYF(MY_WME));
      return true;
    }
  ...
  uchar forminfo[288];
  uchar *extra_segment_buff = static_cast<uchar *>(my_malloc(key_memory_frm_extra_segment_buff, n_length, MYF(MY_WME)));
  const uint reclength = uint2korr(head + 16);
  const uint record_offset =
      uint2korr(head + 6) + ((uint2korr(head + 14) == 0xffff ? uint4korr(head + 47) : uint2korr(head + 14)));
  if (!mysql_file_read(file, forminfo, sizeof(forminfo), MYF(MY_NABP)) && extra_segment_buff &&
      !mysql_file_pread(file, extra_segment_buff, n_length, record_offset + reclength, MYF(MY_NABP)))
  {
    ...
    if (next_chunk + 5 < buff_end)              // Partitioning
    {
      ...
      if (partition_info_str_len)
      {
        if (!(partition_info_str =
                  (const char *)memdup_root(thd->mem_root, next_chunk + 4, partition_info_str_len + 1)))
        {
          goto err;
        }

        // Fill tablespace names used by partition into tablespace_set.
        if (get_partition_tablespace_names(thd, partition_info_str, partition_info_str_len, tablespace_set))
        {
          goto err;
        }
      }
    ...
    if (forminfo[46] == static_cast<uchar>(255) && (next_chunk + 2 < buff_end))
    ...
    if (next_chunk + format_section_header_size < buff_end)
    {
      ...
      {
        tablespace_name = thd->mem_strdup((char *)next_chunk + format_section_header_size);
...
static int make_field_from_frm(THD *thd, TABLE_SHARE *share, uint new_frm_ver, bool use_hash, uint field_idx,
                               uchar *strpos, uchar *format_section_fields, char **comment_pos, char **gcol_screen_pos,
                               uchar **null_pos, uint *null_bit_pos, int *errarg)
{
  ...
  if (new_frm_ver >= 3)
  {
    ...
    if (unireg_type & Field::GENERATED_FIELD)
    {
      ...
      */
      ...
      gcol_info->dup_expr_str(&share->mem_root, *gcol_screen_pos + (uint)FRM_GCOL_HEADER_SIZE, gcol_info_length);
  ...
  if (field_type == MYSQL_TYPE_NEWDECIMAL && !share->mysql_version)
  {
    ...
    */
    ...
    sql_print_error(
        ...
        share->fieldnames.type_names[field_idx], share->table_name.str, share->table_name.str);
  ...
  share->field[field_idx] = reg_field = make_field(
      ...
      (uint32)field_length, *null_pos, *null_bit_pos, pack_flag, field_type, charset, geom_type,
      (Field::utype)MTYP_TYPENR(unireg_type), (interval_nr ? share->intervals + interval_nr - 1 : (TYPELIB *)0),
  ...
  if (format_section_fields)
  {
    ...
    const uchar field_column_format = ((field_flags >> COLUMN_FORMAT_SHIFT) & COLUMN_FORMAT_MASK);
    DBUG_PRINT("debug",
               ("field flags: %u, storage: %u, column_format: %u", field_flags, field_storage, field_column_format));
...
static int open_binary_frm(THD *thd, TABLE_SHARE *share, uchar *head, File file)
{
  ...
  */
  ...
  if (*(head + 61) &&
      !(share->default_part_db_type = ha_checktype(thd, (enum legacy_db_type)(uint) * (head + 61), 1, 0)))
    goto err;
  ...
  */
  if (legacy_db_type > DB_TYPE_UNKNOWN && legacy_db_type < DB_TYPE_FIRST_DYNAMIC)
    share->db_plugin = ha_lock_engine(NULL, ha_checktype(thd, legacy_db_type, 0, 0));
  ...
  if (!head[32])  // New frm file in 3.23
  {
    ...
    share->table_charset = get_charset((((uint)head[41]) << 8) + (uint)head[38], MYF(0));
  ...
  use_extended_sk = ha_check_storage_engine_flag(share->db_type(), HTON_SUPPORTS_EXTENDED_KEYS);

  uint total_key_parts;
  if (use_extended_sk)
  {
    uint primary_key_parts = keys ? (new_frm_ver >= 3) ? (uint)strpos[4] : (uint)strpos[3] : 0;
  ...
  */
  if (!multi_alloc_root(&share->mem_root, &keyinfo, n_length + uint2korr(disk_buff + 4), &rec_per_key,
                        sizeof(ulong) * total_key_parts, &rec_per_key_float, sizeof(rec_per_key_t) * total_key_parts,
                        NULL))
    goto err; /* purecov: inspected */
  ...
  use_packed_keys = ha_check_storage_engine_flag(share->db_type(), HTON_SUPPORTS_PACKED_KEYS);
  ...
  for (keyinfo = share->key_info, i = 0; i < keys; i++, keyinfo++)
  {
    if (keyinfo->flags & HA_USES_COMMENT)
    {
      ...
      keyinfo->comment.str = strmake_root(&share->mem_root, (char *)strpos + 2, keyinfo->comment.length);
      strpos += 2 + keyinfo->comment.length;
    }
    assert(MY_TEST(keyinfo->flags & HA_USES_COMMENT) == (keyinfo->comment.length > 0));
  }
  ...
  if ((n_length = uint4korr(head + 55)))
  {
    ...
    if (!(extra_segment_buff = (uchar *)my_malloc(key_memory_frm_extra_segment_buff, n_length, MYF(MY_WME))))
      goto err;
    next_chunk = extra_segment_buff;
    if (mysql_file_pread(file, extra_segment_buff, n_length, record_offset + share->reclength, MYF(MY_NABP)))
    {
      goto err;
    }
    ...
    if (next_chunk + 2 < buff_end)
    {
      ...
      {
        if (legacy_db_type > DB_TYPE_UNKNOWN && legacy_db_type < DB_TYPE_FIRST_DYNAMIC &&
            legacy_db_type != ha_legacy_type(plugin_data<handlerton *>(tmp_plugin)))
        {
        ...
        */
        ...
        DBUG_PRINT("info", ("setting dbtype to '%.*s' (%d)", str_db_type_length, next_chunk + 2,
                            ha_legacy_type(share->db_type())));
      }
      else if (!tmp_plugin && str_db_type_length == 9 && !strncmp((char *)next_chunk + 2, "partition", 9))
      {
        /*
        ...
        */
        if (mysqld_server_started)
          ...
          */
          sql_print_warning(ER_DEFAULT(ER_PARTITION_ENGINE_DEPRECATED_FOR_TABLE), share->db.str, share->table_name.str);

        /* Check if the partitioning engine is ready */
        if (!ha_checktype(thd, DB_TYPE_PARTITION_DB, true, false))
        {
          error = 8;
          my_error(ER_FEATURE_NOT_AVAILABLE, MYF(0), "partitioning", "--skip-partition",
        ...
        */
        assert(is_ha_partition_handlerton(plugin_data<handlerton *>(share->db_plugin)));
        DBUG_PRINT("info", ("setting dbtype to '%.*s' (%d)", str_db_type_length, next_chunk + 2,
                            ha_legacy_type(share->db_type())));
      }
      else if (!tmp_plugin && name.length == 18 && !strncmp(name.str, "PERFORMANCE_SCHEMA", name.length))
      {
        /*
    ...
    for (i = 0; i < keys; i++, keyinfo++)
    {
      if (keyinfo->flags & HA_USES_PARSER)
      {
        ...
        LEX_CSTRING parser_name = {reinterpret_cast<char *>(next_chunk), strlen(reinterpret_cast<char *>(next_chunk))};
    ...
    if (next_chunk + format_section_header_size < buff_end)
    {
      ...
      */
      ...
      if (next_chunk + format_section_length > buff_end)
      {
        DBUG_PRINT("error", ("format section length too long: %u", format_section_length));
        goto err;
      }
      DBUG_PRINT("info",
                 ("format_section_length: %u, format_section_flags: %u", format_section_length, format_section_flags));

      share->default_storage_media = (enum ha_storage_media)(format_section_flags & 0x7);
      ...
      if (tablespace_length)
      {
        ...
        if (name_check == IDENT_NAME_OK &&
            !(share->tablespace = strmake_root(&share->mem_root, tablespace, tablespace_length + 1)))
        {
          goto err;
        }
      }
      DBUG_PRINT("info", ("tablespace: '%s'", share->tablespace ? share->tablespace : "<null>"));

      /* pointer to format section for fields */
      format_section_fields = next_chunk + format_section_header_size + tablespace_length + 1;
    ...
    if (next_chunk + 2 <= buff_end)
    {
      ...
      if (!(share->compress.str = strmake_root(&share->mem_root, (char *)next_chunk + 2, share->compress.length)))
      {
        goto err;
      }
    ...
    if (next_chunk + 2 <= buff_end)
    {
      ...
      if (!(share->encrypt_type.str =
                strmake_root(&share->mem_root, (char *)next_chunk + 2, share->encrypt_type.length)))
      {
        goto err;
      }
  ...
  if (mysql_file_pread(file, record, (size_t)share->reclength, record_offset, MYF(MY_NABP)))
    goto err; /* purecov: inspected */
  ...
  DBUG_PRINT(
      "info",
      ("i_count: %d  i_parts: %d  index: %d  n_length: %d  int_length: %d  com_length: %d  gcol_screen_length: %d",
       interval_count, interval_parts, share->keys, n_length, int_length, com_length, gcol_screen_length));
  if (!(field_ptr = (Field **)alloc_root(
            &share->mem_root, (uint)((share->fields + 1) * sizeof(Field *) + interval_count * sizeof(TYPELIB) +
  ...
  read_length =
      (uint)(share->fields * field_pack_length + pos + (uint)(n_length + int_length + com_length + gcol_screen_length));
  ...
  memcpy(names, strpos + (share->fields * field_pack_length), (uint)(n_length + int_length));
  orig_comment_pos = comment_pos = names + (n_length + int_length);
  memcpy(comment_pos, disk_buff + read_length - com_length - gcol_screen_length, com_length);
  orig_gcol_screen_pos = gcol_screen_pos = names + (n_length + int_length + com_length);
  memcpy(gcol_screen_pos, disk_buff + read_length - gcol_screen_length, gcol_screen_length);
...

  {
    ...
    for (interval = share->intervals; interval < share->intervals + interval_count; interval++)
    {
      ...
      if (!(interval->type_lengths = (uint *)alloc_root(&share->mem_root, count)))
        goto err;
  ...
  /* Allocate handler */
  if (!(handler_file = get_new_handler(share, thd->mem_root, share->db_type())))
    goto err;
  ...
  else
  {
    ...
    null_flags = null_pos = share->default_values + share->reclength - share->null_bytes;
    null_bit_pos = 0;
  }

  use_hash = share->fields >= MAX_FIELDS_BEFORE_HASH;
  if (use_hash)
    use_hash = !my_hash_init(&share->name_hash, system_charset_info, share->fields, 0, 0,
                             (my_hash_get_key)get_field_name, 0, 0, PSI_INSTRUMENT_ME);

  for (i = 0; i < share->fields; i++, strpos += field_pack_length)
  {
    if (new_frm_ver >= 3 && (strpos[10] & Field::GENERATED_FIELD) &&  // Field::unireg_check
    ...
    {
      if ((error = make_field_from_frm(thd, share, new_frm_ver, use_hash, i, strpos, format_section_fields,
                                       &comment_pos, &gcol_screen_pos, &null_pos, &null_bit_pos, &errarg)))
        goto err;
    }
  }
  ...
  if (key_parts)
  {
    const int pk_off = find_type(primary_key_name, &share->keynames, FIND_TYPE_NO_PREFIX);
    ...
    for (uint key = 0; key < share->keys; key++, keyinfo++)
    {
      ...
      if (primary_key >= MAX_KEY && (keyinfo->flags & HA_NOSAME))
      {
        ...
        */
        ...
        for (i = 0; i < keyinfo->user_defined_key_parts; i++)
        {
          ...
          */
          if (!table_field->real_maybe_null() && table_field->type() == MYSQL_TYPE_GEOMETRY &&
              table_field->get_geometry_type() == Field::GEOM_POINT && key_part[i].length == MAX_LEN_GEOM_POINT_FIELD)
            continue;
      ...
      for (i = 0; i < keyinfo->user_defined_key_parts; key_part++, i++)
      {
        ...
        if (new_field_pack_flag <= 1)
          key_part->fieldnr =
              (uint16)find_field(share->field, share->default_values, (uint)key_part->offset, (uint)key_part->length);
        if (!key_part->fieldnr)
        {
          error = 4;  // Wrong file
          goto err;
        ...
        setup_key_part_field(share, handler_file, primary_key, keyinfo, key, i, &usable_parts, true);
      ...
      if (primary_key < MAX_KEY && key != primary_key && (ha_option & HA_PRIMARY_KEY_IN_READ_INDEX))
        key_part += add_pk_parts_to_sk(keyinfo, key, share->key_info, primary_key, share, handler_file, &usable_parts,
                                       use_extended_sk);
  ...
  if (share->found_next_number_field)
  {
    ...
    if ((int)(share->next_number_index =
                  (uint)find_ref_key(share->key_info, share->keys, share->default_values, reg_field,
  ...
  */
  ...
  if (!(bitmaps = (my_bitmap_map *)alloc_root(&share->mem_root, share->column_bitmap_size)))
    goto err;
...
static bool validate_generated_expr(Field *field)
{
  ...
  if (expr->has_stored_program() ||                                  // 1)
      (expr->used_tables() & (RAND_TABLE_BIT | PARAM_TABLE_BIT)) ||  // 2)
      (expr->cols() != 1))                                           // 3)
  {
  ...
  */
  ...
  if (expr->walk(&Item::check_gcol_func_processor, Item::WALK_POSTFIX, pointer_cast<uchar *>(&args)))
  {
...
static bool fix_fields_gcol_func(THD *thd, Field *field)
{
  ...
  */
  ...
  func_expr->walk(&Item::change_context_processor, Item::WALK_POSTFIX, (uchar *)context);
...
bool Generated_column::register_base_columns(TABLE *table)
{
  ...
  my_bitmap_map *bitbuf =
      static_cast<my_bitmap_map *>(alloc_root(&table->mem_root, bitmap_buffer_size(table->s->fields)));
  ...
  expr_item->walk(&Item::mark_field_in_map, Item::WALK_PREFIX, (uchar *)&mark_fld);
  ...
  for (uint i = 0; i < table->s->fields; i++)
  {
    ...
    if (bitmap_is_set(&base_columns_map, field->field_index) && field->stored_in_db)
      num_non_virtual_base_cols++;
  }
  DBUG_RETURN(false);
}

void Generated_column::dup_expr_str(MEM_ROOT *root, const char *src, size_t len)
{
  if (!root)
...
static bool unpack_gcol_info_from_frm(THD *thd, TABLE *table, Field *field, bool is_create_table, bool *error_reported)
{
  ...
  */
  ...
  if (!(gcol_expr_str = (char *)alloc_root(&table->mem_root, gcol_expr->length + PARSE_GCOL_KEYWORD.length + 3)))
  {
    DBUG_RETURN(TRUE);
  }
  ...
  */
  ...
  Query_arena gcol_arena(&table->mem_root, Query_arena::STMT_CONVENTIONAL_EXECUTION);
...
int open_table_from_share(THD *thd, TABLE_SHARE *share, const char *alias, uint db_stat, uint prgflag,
                          uint ha_open_flags, TABLE *outparam, bool is_create_table)
{
  int error;
  ...
  DBUG_PRINT("enter", ("name: '%s.%s'  form: 0x%lx", share->db.str, share->table_name.str, (long)outparam));
  ...
  init_sql_alloc(key_memory_TABLE, &outparam->mem_root, TABLE_ALLOC_BLOCK_SIZE, 0);

  if (!(outparam->alias = my_strdup(key_memory_TABLE, alias, MYF(MY_WME))))
    goto err;
  ...
  if (share->null_field_first)
    ...
    outparam->null_flags = (record + 1 + share->reclength - share->null_bytes);
  ...
  for (i = 0; i < share->fields; i++, field_ptr++)
  {
    ...
    *field_ptr = new_field;
    ...
    new_field->move_field_offset((my_ptrdiff_t)(outparam->record[0] - outparam->s->default_values));
    /* Check if FTS_DOC_ID column is present in the table */
    if (outparam->file && (outparam->file->ha_table_flags() & HA_CAN_FULLTEXT_EXT) &&
  ...
  if (share->key_parts)
  {
    ...
    memcpy(key_part, share->key_info[0].key_part, (sizeof(*key_part) * share->key_parts));
    ...
    for (key_info_end = key_info + share->keys; key_info < key_info_end; key_info++)
    {
      ...
      for (key_part_end = key_part + key_info->actual_key_parts; key_part < key_part_end; key_part++)
      {
  ...
  if (share->vfields && outparam->file && !(outparam->file->ha_table_flags() & HA_GENERATED_COLUMNS))
  {
    my_error(ER_UNSUPPORTED_ACTION_ON_GENERATED_COLUMN, MYF(0), "Specified storage engine");
    error_reported = TRUE;
    goto err;
  }
  ...
  */
  ...
  if (!(bitmaps = (uchar *)alloc_root(&outparam->mem_root, bitmap_size * 5)))
    goto err;
  bitmap_init(&outparam->def_read_set, (my_bitmap_map *)bitmaps, share->fields, FALSE);
  bitmap_init(&outparam->def_write_set, (my_bitmap_map *)(bitmaps + bitmap_size), share->fields, FALSE);
  bitmap_init(&outparam->tmp_set, (my_bitmap_map *)(bitmaps + bitmap_size * 2), share->fields, FALSE);
  bitmap_init(&outparam->cond_set, (my_bitmap_map *)(bitmaps + bitmap_size * 3), share->fields, FALSE);
  bitmap_init(&outparam->def_fields_set_during_insert, (my_bitmap_map *)(bitmaps + bitmap_size * 4), share->fields,
              FALSE);
  ...
  */
  ...
  if (share->vfields)
  {
    if (!(vfield_ptr = (Field **)alloc_root(&outparam->mem_root, (uint)((share->vfields + 1) * sizeof(Field *)))))
      goto err;
    ...
    for (field_ptr = outparam->field; *field_ptr; field_ptr++)
    {
      ...
      {
        if (unpack_gcol_info_from_frm(thd, outparam, *field_ptr, is_create_table, &error_reported))
        {
          *vfield_ptr = NULL;
  ...
  if (db_stat)
  {
    ...
    if ((ha_err = (outparam->file->ha_open(
             outparam, share->normalized_path.str, (db_stat & HA_READ_ONLY ? O_RDONLY : O_RDWR),
                 ...
                 ha_open_flags))))
    {
      ...
      switch (ha_err)
      {
        ...
        case EMFILE:
           ...
           */
          DBUG_PRINT("error",
                     ("open file: %s failed, too many files opened (errno: %d)", share->normalized_path.str, ha_err));
          error = 1;
          set_my_errno(EMFILE);
          break;
        default:
  ...
  if ((share->table_category == TABLE_CATEGORY_LOG) || (share->table_category == TABLE_CATEGORY_RPL_INFO) ||
  ...
  else if (outparam->file)
  {
    ...
    outparam->no_replicate =
        !MY_TEST(flags & (HA_BINLOG_STMT_CAPABLE | HA_BINLOG_ROW_CAPABLE)) || MY_TEST(flags & HA_HAS_OWN_BINLOGGING);
  }
  else
  {
...
static ulong get_form_pos(File file, uchar *head)
{
  ...
  if (!(buf = (uchar *)my_malloc(key_memory_frm_form_pos, length + names * 4, MYF(MY_WME))))
    DBUG_RETURN(0);
...
int read_string(File file, uchar **to, size_t length)
{
  ...
  my_free(*to);
  if (!(*to = (uchar *)my_malloc(key_memory_frm_string, length + 1, MYF(MY_WME))) ||
...
ulong make_new_entry(File file, uchar *fileinfo, TYPELIB *formnames, const char *newname)
{
  ...
  { /* Expand file */
    ...
    while (endpos > maxlength)
    {
      ...
      if (mysql_file_read(file, buff, bufflength, MYF(MY_NABP + MY_WME)))
        DBUG_RETURN(0L);
      mysql_file_seek(file, (ulong)(endpos - bufflength + IO_SIZE), MY_SEEK_SET, MYF(0));
  ...
  if (mysql_file_write(file, buff, (size_t)length + 1, MYF(MY_NABP + MY_WME)) ||
      (names &&
       mysql_file_write(file, (uchar *)(*formnames->type_names + n_length - 1), names * 4, MYF(MY_NABP + MY_WME))) ||
...
void open_table_error(TABLE_SHARE *share, int error, int db_errno, int errarg)
{
  ...
  switch (error)
  {
    ...
    case 2:
    {
      ...
      {
        if ((file = get_new_handler(share, current_thd->mem_root, share->db_type())))
        {
          if (!(datext = *file->bas_ext()))
            datext = "";
        }
      }
      err_no = (db_errno == ENOENT) ? ER_FILE_NOT_FOUND : (db_errno == EAGAIN) ? ER_FILE_USED : ER_CANT_OPEN_FILE;
      strxmov(buff, share->normalized_path.str, datext, NullS);
      my_error(err_no, errortype, buff, db_errno, my_strerror(errbuf, sizeof(errbuf), db_errno));
      delete file;
      break;
    }
    case 5:
    ...
    case 9:
      ...
      my_error(ER_FRM_UNKNOWN_TYPE, MYF(0), share->path.str, share->view_def->type()->str);
      break;
...
File create_frm(THD *thd, const char *name, const char *db, const char *table, uint reclength, uchar *fileinfo,
                HA_CREATE_INFO *create_info, uint keys, KEY *key_info)
{
  ...
  if ((file = mysql_file_create(key_file_frm, name, CREATE_MODE, create_flags, MYF(0))) >= 0)
  {
    ...
    fileinfo[3] = (uchar)ha_legacy_type(ha_checktype(thd, ha_legacy_type(create_info->db_type), 0, 0));
    fileinfo[4] = 1;
    ...
    */
    for (i = 0; i < keys; i++)
    {
      assert(MY_TEST(key_info[i].flags & HA_USES_COMMENT) == (key_info[i].comment.length > 0));
...
enum_ident_name_check check_and_convert_db_name(LEX_STRING *org_name, bool preserve_lettercase)
{
...
enum_ident_name_check check_table_name(const char *name, size_t length, bool check_for_path_chars)
{
...
bool Table_check_intact::check(TABLE *table, const TABLE_FIELD_DEF *table_def)
{
  ...
  DBUG_PRINT("info", ("table: %s  expected_count: %d", table->alias, table_def->count));
  ...
  if (table->s->fields != table_def->count)
  {
    ...
    {
      report_error(ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE_V2, ER(ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE_V2),
                   table->s->db.str, table->alias, table_def->count, table->s->fields,
                   static_cast<int>(table->s->mysql_version), MYSQL_VERSION_ID);
      DBUG_RETURN(TRUE);
    }
    else if (MYSQL_VERSION_ID == table->s->mysql_version)
    {
      report_error(ER_COL_COUNT_DOESNT_MATCH_CORRUPTED_V2, ER(ER_COL_COUNT_DOESNT_MATCH_CORRUPTED_V2), table->s->db.str,
                   table->s->table_name.str, table_def->count, table->s->fields);
      DBUG_RETURN(TRUE);
    }
    /*
  ...
  for (i = 0; i < table_def->count; i++, field_def++)
  {
    ...
    if (i < table->s->fields)
    {
      ...
      if (strncmp(field->field_name, field_def->name.str, field_def->name.length))
      {
        /*
        ...
        */
        report_error(0,
                     ...
                     table->s->db.str, table->alias, field_def->name.str, i, field->field_name);
      }
      ...
      if (strncmp(sql_type.c_ptr_safe(), field_def->type.str, field_def->type.length - 1))
      {
        report_error(ER_CANNOT_LOAD_FROM_TABLE_V2,
                     ...
                     table->s->db.str, table->alias, field_def->name.str, i, field_def->type.str,
                     sql_type.c_ptr_safe());
        error = TRUE;
      }
      else if (field_def->cset.str && !field->has_charset())
      {
        report_error(ER_CANNOT_LOAD_FROM_TABLE_V2,
                     ...
                     table->s->db.str, table->alias, field_def->name.str, i, field_def->cset.str);
        error = TRUE;
      }
      else if (field_def->cset.str && strcmp(field->charset()->csname, field_def->cset.str))
      {
        report_error(ER_CANNOT_LOAD_FROM_TABLE_V2,
                     "Incorrect definition of "
                     "table %s.%s: expected the type of column '%s' at "
  ...
  {
    my_error(ER_MISSING_KEY, MYF(0), table->s->db.str, table->s->table_name.str);
    error = TRUE;
  }
...
void TABLE::init(THD *thd, TABLE_LIST *tl)
{
  ...
  if (thd->lex->need_correct_ident())
    alias_name_used = my_strcasecmp(table_alias_charset, s->table_name.str, tl->alias);
  /* Fix alias if table name changes. */
  if (strcmp(alias, tl->alias))
  {
    size_t length = strlen(tl->alias) + 1;
    alias = (char *)my_realloc(key_memory_TABLE, (char *)alias, length, MYF(MY_WME));
    ...
    for (Field **vfield_ptr = vfield; *vfield_ptr; vfield_ptr++)
    {
      ...
      if (!vfield->gcol_info->expr_item->fixed)
      {
        ...
        */
        Query_arena *backup_stmt_arena_ptr = thd->stmt_arena;
        Query_arena backup_arena;
        Query_arena gcol_arena(&vfield->table->mem_root, Query_arena::STMT_CONVENTIONAL_EXECUTION);
        if (!vfield->gcol_info->permanent_changes_completed && !thd->lex->is_ps_or_view_context_analysis())
        {
          thd->set_n_backup_active_arena(&gcol_arena, &backup_arena);
          thd->stmt_arena = &gcol_arena;
        }
...
TABLE_LIST *TABLE_LIST::new_nested_join(MEM_ROOT *allocator, const char *alias, TABLE_LIST *embedding,
                                        List<TABLE_LIST> *belongs_to, class st_select_lex *select)
{
  assert(belongs_to && select);

  TABLE_LIST *const join_nest = (TABLE_LIST *)alloc_root(allocator, sizeof(TABLE_LIST));
  if (join_nest == NULL)
    return NULL;
  new (join_nest) TABLE_LIST;

  join_nest->nested_join = (NESTED_JOIN *)alloc_root(allocator, sizeof(NESTED_JOIN));
...
void TABLE_LIST::calc_md5(char *buffer)
{
  ...
  compute_md5_hash((char *)digest, (const char *)select_stmt.str, select_stmt.length);
...
bool TABLE_LIST::create_field_translation(THD *thd)
{
  ...
  Field_translator *transl =
      (Field_translator *)thd->stmt_arena->alloc(select->item_list.elements * sizeof(Field_translator));
  if (!transl)
    return true; /* purecov: inspected */
...
bool TABLE_LIST::prepare_check_option(THD *thd, bool is_cascaded)
{
  ...
  */
  ...
  {
    Prepared_stmt_arena_holder ps_arena_holder(thd);
    if ((with_check || is_cascaded) && merge_join_conditions(thd, this, &check_option))
      DBUG_RETURN(true); /* purecov: inspected */

    for (TABLE_LIST *tbl = merge_underlying_list; tbl; tbl = tbl->next_local)
    {
      if (tbl->check_option && !(check_option = and_conds(check_option, tbl->check_option)))
        DBUG_RETURN(true); /* purecov: inspected */
    }
  ...
  if (check_option && !check_option->fixed)
  {
    ...
    if (check_option->fix_fields(thd, &check_option) || check_option->check_cols(1))
      DBUG_RETURN(true); /* purecov: inspected */
    thd->where = save_where;
  }
...
bool TABLE_LIST::prepare_replace_filter(THD *thd)
{
  ...
  if (replace_filter && !replace_filter->fixed)
  {
    ...
    if (replace_filter->fix_fields(thd, &replace_filter) || replace_filter->check_cols(1))
      DBUG_RETURN(true);
    thd->where = save_where;
  }
...
int TABLE_LIST::view_check_option(THD *thd) const
{
  if (check_option && check_option->val_int() == 0)
  {
    ...
    my_error(ER_VIEW_CHECK_FAILED, MYF(0), main_view->view_db.str, main_view->view_name.str);
...
{
  return (is_view_or_derived() || is_natural_join || is_join_columns_complete || !nested_join);
}
...
bool TABLE_LIST::prepare_view_securety_context(THD *thd)
{
  ...
  if (view_suid)
  {
    ...
    if (acl_getroot(view_sctx, const_cast<char *>(definer.user.str), const_cast<char *>(definer.host.str),
                    const_cast<char *>(definer.host.str), thd->db().str))
    {
      ...
      {
        push_warning_printf(thd, Sql_condition::SL_NOTE, ER_NO_SUCH_USER, ER(ER_NO_SUCH_USER), definer.user.str,
                            definer.host.str);
      }
      else
      {
        ...
        {
          if (thd->password == 2)
            my_error(ER_ACCESS_DENIED_NO_PASSWORD_ERROR, MYF(0), thd->security_context()->priv_user().str,
...
Security_context *TABLE_LIST::find_view_security_context(THD *thd)
{
  ...
  if (upper_view)
  {
    DBUG_PRINT("info", ("Securety context of view %s will be used", upper_view->alias));
...
Item *Natural_join_column::create_item(THD *thd)
{
  if (view_field)
  {
    ...
    return create_view_field(thd, table_ref, &view_field->item, view_field->name, &select->context);
  }
  return table_field;
}
...
const char *Natural_join_column::db_name()
{
  ...
  */
  assert(!strcmp(table_ref->db, table_ref->table->s->db.str) ||
         (table_ref->schema_table && is_infoschema_db(table_ref->table->s->db.str, table_ref->table->s->db.length)));
  return table_ref->db;
}
...
Item *Field_iterator_table::create_item(THD *thd)
{
  ...
  /*
    ...
    */
  if (item && !thd->lex->in_sum_func && select->resolve_place == st_select_lex::RESOLVE_SELECT_LIST)
  {
                               ...
                               Name_resolution_context *context)
{
  ...
  */
  Item *item = new Item_direct_view_ref(context, field_ref, view->alias, table_name, name, view);
  DBUG_RETURN(item);
}
...
void Field_iterator_table_ref::set_field_iterator()
{
  ...
  */
  if (table_ref->is_join_columns_complete)
  {
    ...
    assert(table_ref->is_natural_join || table_ref->nested_join ||
...
const char *Field_iterator_table_ref::get_table_name()
{
  ...
  assert(!strcmp(table_ref->table_name, table_ref->table->s->table_name.str));
...
Natural_join_column *Field_iterator_table_ref::get_or_create_column_ref(THD *thd, TABLE_LIST *parent_table_ref)
{
  ...
  if (field_it == &table_field_it)
  {
    ...
    Item_field *tmp_item = new Item_field(thd, &thd->lex->current_select()->context, tmp_field);
    if (!tmp_item)
      return NULL;
  ...
  else if (field_it == &view_field_it)
  {
    ...
    field_count = table_ref->field_translation_end - table_ref->field_translation;
  }
  else
  {
    /*
  ...
  assert(!nj_col->table_field || nj_col->table_ref->table == nj_col->table_field->field->table);
...
Natural_join_column *Field_iterator_table_ref::get_natural_column_ref()
{
  ...
  */
  nj_col = natural_join_it.column_ref();
  assert(nj_col && (!nj_col->table_field || nj_col->table_ref->table == nj_col->table_field->field->table));
  return nj_col;
}
...
void TABLE::prepare_for_position()
{
  ...
  if ((file->ha_table_flags() & HA_PRIMARY_KEY_REQUIRED_FOR_POSITION) && s->primary_key < MAX_KEY)
  {
...
void TABLE::mark_column_used(THD *thd, Field *field, enum enum_mark_columns mark)
{
...
void TABLE::mark_columns_used_by_index_no_reset(uint index, MY_BITMAP *bitmap, uint key_parts)
{
  ...
  if (file->ha_table_flags() & HA_PRIMARY_KEY_REQUIRED_FOR_DELETE)
  {
    ...
    */
    if (s->primary_key == MAX_KEY)
    {
      ...
      */
      if (!(mysql_bin_log.is_open() && in_use && in_use->is_current_stmt_binlog_format_row()))
    ...
    switch (thd->variables.binlog_row_image)
    {
      ...
      case BINLOG_ROW_IMAGE_NOBLOB:
        ...
        for (Field **ptr = field; *ptr; ptr++)
        {
           ...
           */
          if ((s->primary_key < MAX_KEY) && ((my_field->flags & PRI_KEY_FLAG) || (my_field->type() != MYSQL_TYPE_BLOB)))
...
bool TABLE::alloc_keys(uint key_count)
{
  assert(!s->keys);
  max_keys = key_count;
  if (!(key_info = s->key_info = (KEY *)alloc_root(&mem_root, sizeof(KEY) * max_keys)))
    return TRUE;
...
bool TABLE::add_tmp_key(Field_map *key_parts, char *key_name)
{
  ...
  */
  ...
  if (!multi_alloc_root(&mem_root, &key_buf, key_buf_size, &rec_per_key, sizeof(ulong) * key_part_count,
                        &rec_per_key_float, sizeof(rec_per_key_t) * key_part_count, NULL))
    return true; /* purecov: inspected */
...
void TABLE::use_index(int key_to_save)
{
  ...
  else
  {
    ...
    uint i;
    KEY_PART_INFO *kp;
    for (kp = key_info[key_to_save].key_part, i = 0; i < key_info[key_to_save].user_defined_key_parts; i++, kp++)
    {
    ...
    for (vfield_ptr = vfield; *vfield_ptr; vfield_ptr++)
    {
      ...
      */
      if ((!tmp_vfield->stored_in_db && tmp_vfield->m_indexed) ||
          bitmap_is_overlapping(write_set, &tmp_vfield->gcol_info->base_columns_map))
      {
        // The GC needs to be updated
        tmp_vfield->table->mark_column_used(in_use, tmp_vfield, MARK_COLUMNS_WRITE);
        // In order to update the new value, we have to read the old value
        tmp_vfield->table->mark_column_used(in_use, tmp_vfield, MARK_COLUMNS_READ);
        bitmap_updated = TRUE;
      }
    }
  }
...
bool TABLE::is_field_used_by_generated_columns(uint field_index)
{
  ...
  for (Field **vfield_ptr = vfield; *vfield_ptr; vfield_ptr++)
  {
    ...
    assert(tmp_vfield->gcol_info && tmp_vfield->gcol_info->expr_item);
    Mark_field mark_fld(MARK_COLUMNS_TEMP);
    tmp_vfield->gcol_info->expr_item->walk(&Item::mark_field_in_map, Item::WALK_PREFIX, (uchar *)&mark_fld);
...
bool TABLE_LIST::process_index_hints(TABLE *tbl)
{
  ...
  tbl->keys_in_use_for_query = tbl->keys_in_use_for_group_by = tbl->keys_in_use_for_order_by = tbl->s->keys_in_use;
...
size_t max_row_length(TABLE *table, const uchar *data)
{
  ...
  for (uint *ptr = beg; ptr != end; ++ptr)
  {
    ...
    length += blob->get_length((data + blob->offset(table->record[0]))) + HA_KEY_BLOB_LENGTH;
  }
  return length;
}
...
void init_mdl_requests(TABLE_LIST *table_list)
{
  for (; table_list; table_list = table_list->next_global)
    MDL_REQUEST_INIT(&table_list->mdl_request, MDL_key::TABLE, table_list->db, table_list->table_name,
                     mdl_type_for_dml(table_list->lock_type), MDL_TRANSACTION);
}
...
{
  return is_view_or_derived() && algorithm != VIEW_ALGORITHM_TEMPTABLE && derived->is_mergeable() &&
...
static bool add_derived_key(List<Derived_key> &derived_key_list, Field *field, table_map ref_by_tbl)
{
  uint key = 0;
  Derived_key *entry = 0;
...
bool TABLE_LIST::update_derived_keys(Field *field, Item **values, uint num_values)
{
  /*
    Don't bother with keys for CREATE VIEW, BLOB fields and fields with
    zero length.
  */
  if (field->table->in_use->lex->is_ps_or_view_context_analysis() || field->flags & BLOB_FLAG ||
...
static int Derived_key_comp(Derived_key *e1, Derived_key *e2, void *arg)
{
  ...
  return ((e1->referenced_by < e2->referenced_by) ? -1 : ((e1->referenced_by > e2->referenced_by) ? 1 : 0));
}
...
bool TABLE_LIST::generate_keys()
{
  ...
  while ((entry = it++))
  {
    ...
    if (table->add_tmp_key(&entry->used_fields, table->in_use->mem_strdup(buf)))
      return TRUE;
  }
  return FALSE;
}
  ...
  */
  ...
  for (Field **vfield_ptr = table->vfield; *vfield_ptr; vfield_ptr++)
  {
    ...
    /*
      ...
      read_set bitmap.
    */
    if (!vfield->stored_in_db && bitmap_is_set(table->read_set, vfield->field_index))
    {
