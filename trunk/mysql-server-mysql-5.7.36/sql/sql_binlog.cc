...
static int check_event_type(int type, Relay_log_info *rli)
{
  ...
  */
  if (fd_event && fd_event->event_type_permutation)
  {
    ...
    DBUG_PRINT("info", ("converting event type %d to %d (%s)", type, new_type, Log_event::get_type_str(new_type)));
#endif
...
void mysql_client_binlog_statement(THD *thd)
{
  ...
  */
  ...
  if (!rli)
  {
    ...
    */
    ...
    if ((rli = Rpl_info_factory::create_rli(INFO_REPOSITORY_DUMMY, FALSE, (const char *)"", true)))
    {
      thd->rli_fake = rli;
      rli->info_thd = thd;
    }
  ...
  const char *error = 0;
  char *buf = (char *)my_malloc(key_memory_binlog_statement_buffer, decoded_len, MYF(MY_WME));
  Log_event *ev = 0;
  ...
  */
  ...
  {
    char const *endptr = 0;
    int64 bytes_decoded = base64_decode(strptr, coded_len, buf, &endptr, MY_BASE64_DECODE_ALLOW_MULTIPLE_CHUNKS);

    DBUG_PRINT("info", ("bytes_decoded: %lld  strptr: 0x%lx  endptr: 0x%lx ('%c':%d)", bytes_decoded, (long)strptr,
                        (long)endptr, *endptr, *endptr));
    ...
    */
    ...
    DBUG_PRINT("info", ("binlog base64 decoded_len: %lu  bytes_decoded: %lld", (ulong)decoded_len, bytes_decoded));
    ...
    */
    for (char *bufptr = buf; bytes_decoded > 0;)
    {
      ...
      */
      ...
      DBUG_PRINT("info", ("event_len=%lu, bytes_decoded=%lld", event_len, bytes_decoded));

      if (check_event_type(bufptr[EVENT_TYPE_OFFSET], rli))
        goto end;

      ev = Log_event::read_log_event(bufptr, event_len, &error, rli->get_rli_description_event(), 0);
