...
static bool init_fields(THD *thd, TABLE_LIST *tables, struct st_find_field *find_fields, uint count)
{
  ...
  for (; count--; find_fields++)
  {
    ...
    if (!(find_fields->field = find_field_in_tables(thd, field, tables, NULL, 0, REPORT_ALL_ERRORS,
...
int search_topics(THD *thd, QEP_TAB *topics, struct st_find_field *find_fields, List<String> *names, String *name,
                  String *description, String *example)
{
  ...
  while (!read_record_info.read_record(&read_record_info))
  {
    if (!topics->condition()->val_int())  // Doesn't match like
      continue;
    memorize_variant_topic(thd, topics->table(), count, find_fields, names, name, description, example);
    count++;
  }
...
int search_keyword(THD *thd, QEP_TAB *keywords, struct st_find_field *find_fields, int *key_id)
{
  int count = 0;
  READ_RECORD read_record_info;
                           ...
                           List<String> *names, String *name, String *description, String *example)
{
  ...
  if ((iindex_topic = find_type(primary_key_name, &topics->s->keynames, FIND_TYPE_NO_PREFIX) - 1) < 0 ||
      (iindex_relations = find_type(primary_key_name, &relations->s->keynames, FIND_TYPE_NO_PREFIX) - 1) < 0)
  {
...
int search_categories(THD *thd, QEP_TAB *categories, struct st_find_field *find_fields, List<String> *names,
                      int16 *res_id)
{
...
void get_all_items_for_category(THD *thd, QEP_TAB *items, Field *pfname, List<String> *res)
{
  READ_RECORD read_record_info;
...
int send_header_2(THD *thd, bool for_category)
{
  ...
  DBUG_RETURN(thd->send_result_metadata(&field_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF));
}
...
bool prepare_simple_select(THD *thd, Item *cond, TABLE *table, QEP_TAB *tab)
{
  if (!cond->fixed)
  ...
  QUICK_SELECT_I *qck;
  const bool impossible = test_quick_select(thd, keys_to_use, 0, HA_POS_ERROR, false, ORDER::ORDER_NOT_RELEVANT, tab,
                                            cond, &needed_reg_dummy, &qck, tab->table()->force_index) < 0;
  tab->set_quick(qck);
...
bool prepare_select_for_name(THD *thd, const char *mask, size_t mlen, TABLE_LIST *tables, TABLE *table, Field *pfname,
                             QEP_TAB *tab)
{
...
bool mysqld_help(THD *thd, const char *mask)
{
  ...
  tables[0].init_one_table(C_STRING_WITH_LEN("mysql"), C_STRING_WITH_LEN("help_topic"), "help_topic", TL_READ);
  tables[1].init_one_table(C_STRING_WITH_LEN("mysql"), C_STRING_WITH_LEN("help_category"), "help_category", TL_READ);
  tables[2].init_one_table(C_STRING_WITH_LEN("mysql"), C_STRING_WITH_LEN("help_relation"), "help_relation", TL_READ);
  tables[3].init_one_table(C_STRING_WITH_LEN("mysql"), C_STRING_WITH_LEN("help_keyword"), "help_keyword", TL_READ);
  ...
  */
  select_lex->context.table_list = select_lex->context.first_name_resolution_table = &tables[0];
...

  {
    ...
    if (prepare_select_for_name(thd, mask, mlen, tables, tables[0].table, used_fields[help_topic_name].field, &tab))
      goto error;

    count_topics = search_topics(thd, &tab, used_fields, &topics_list, &name, &description, &example);
  }
  ...
  if (count_topics == 0)
  {
    ...
    if (prepare_select_for_name(thd, mask, mlen, tables, tables[3].table, used_fields[help_keyword_name].field, &tab))
      goto error;
  ...
  if (count_topics == 0)
  {
    ...
    Field *cat_cat_id = used_fields[help_category_parent_category_id].field;
    {
      ...
      if (prepare_select_for_name(thd, mask, mlen, tables, tables[1].table, used_fields[help_category_name].field,
                                  &tab))
        goto error;

      DEBUG_SYNC(thd, "before_help_record_read");

      count_categories = search_categories(thd, &tab, used_fields, &categories_list, &category_id);
    }
    if (!count_categories)
    {
    ...
    {
      if (send_header_2(thd, FALSE) || send_variant_2_list(mem_root, protocol, &categories_list, "Y", 0))
        goto error;
    }
    else
    {
...

      {
        ...
        if (prepare_simple_select(thd, cond_topic_by_cat, tables[0].table, &tab))
          goto error;
        get_all_items_for_category(thd, &tab, used_fields[help_topic_name].field, &topics_list);
      }
      {
        QEP_TAB_standalone qep_tab_st;
        QEP_TAB &tab = qep_tab_st.as_QEP_TAB();
        ...
        if (prepare_simple_select(thd, cond_cat_by_cat, tables[1].table, &tab))
          goto error;
        get_all_items_for_category(thd, &tab, used_fields[help_category_name].field, &subcategories_list);
      }
      String *cat = categories_list.head();
      if (send_header_2(thd, TRUE) || send_variant_2_list(mem_root, protocol, &topics_list, "N", cat) ||
  ...
  else
  {
    ...
    if (send_header_2(thd, FALSE) || send_variant_2_list(mem_root, protocol, &topics_list, "N", 0))
      goto error;

    QEP_TAB_standalone qep_tab_st;
    QEP_TAB &tab = qep_tab_st.as_QEP_TAB();

    if (prepare_select_for_name(thd, mask, mlen, tables, tables[1].table, used_fields[help_category_name].field, &tab))
      goto error;
