    ...
    if (!join_tab->used_fieldlength)  // Not calculated yet
    {
      ...
      */
      calc_used_field_length(thd, join_tab->table(),
                             false,  // (1)
                             &used_fields, &join_tab->used_fieldlength, &used_blobs, &used_null_fields,
...
*/
Key_use *Optimize_table_order::find_best_ref(JOIN_TAB *tab, const table_map remaining_tables, const uint idx,
  ...
  for (Key_use *keyuse = tab->keyuse(); keyuse->table_ref == tab->table_ref;)
  {
    ...
    */
    ...
    while (keyuse->table_ref == tab->table_ref && keyuse->key == key)
    {
      ...
      */
      for (; keyuse->table_ref == tab->table_ref && keyuse->key == key && keyuse->keypart == keypart; ++keyuse)
      {
        /*
        ...
        */
        ...
        const double cur_distinct_prefix_rowcount = prev_record_reads(join, idx, (table_deps | keyuse->used_tables));
    ...
    if (cur_keytype != FULLTEXT)
    {
      *found_condition |= MY_TEST(found_part);
      ...
      const bool all_key_parts_covered = (found_part == LOWER_BITS(key_part_map, actual_key_parts(keyinfo)));
      /*
      ...
      */
      ...
      if (all_key_parts_covered && !ref_or_null_part) /* use eq key */
      {
        ...
        {
          cur_read_cost = prev_record_reads(join, idx, table_deps) * table->cost_model()->page_read_cost(1.0);
          cur_fanout = 1.0;
        }
        else
        {
          ...
          else
          {
            ...
            {
              cur_fanout = keyinfo->records_per_key(actual_key_parts(keyinfo) - 1);
            }
            else
            { /* Prefer longer keys */
              assert(table->s->max_key_length > 0);
              cur_fanout = ((double)tab->records() / (double)distinct_keys_est *
            ...
            */
            if (table->quick_keys.is_set(key) &&
                ...
                table->quick_n_ranges[key] == 1 && cur_fanout > (double)table->quick_rows[key])
            {
              cur_fanout = (double)table->quick_rows[key];
            }
          }
          // Limit the number of matched rows
          const double tmp_fanout = min(cur_fanout, (double)thd->variables.max_seeks_for_key);
          if (table->covering_keys.is_set(key))
          {
            // We can use only index tree
            const Cost_estimate index_read_cost = table->file->index_scan_cost(key, 1, tmp_fanout);
            cur_read_cost = prefix_rowcount * index_read_cost.total_cost();
          }
          else if (key == table->s->primary_key && table->file->primary_key_is_clustered())
          {
            const Cost_estimate table_read_cost = table->file->read_cost(key, 1, tmp_fanout);
            ...
            cur_read_cost = prefix_rowcount * min(table->cost_model()->page_read_cost(tmp_fanout), tab->worst_seeks);
               ...
               (!(table->file->index_flags(key, 0, 0) & HA_ONLY_WHOLE_INDEX) || all_key_parts_covered))
      {
        /*
        ...
        else
        {
          ...
          else
          {
            ...
            */
            ...
            if (keyinfo->has_records_per_key(keyinfo->user_defined_key_parts - 1))
              rec_per_key = keyinfo->records_per_key(keyinfo->user_defined_key_parts - 1);
            else
              rec_per_key = rec_per_key_t(tab->records()) / distinct_keys_est + 1;
            ...
            else
            {
              ...
              if (keyinfo->user_defined_key_parts > 1)
                tmp_fanout =
                    (cur_used_keyparts * (rec_per_key - a) + a * keyinfo->user_defined_key_parts - rec_per_key) /
          ...
          */
          if (table->quick_keys.is_set(key) && table->quick_key_parts[key] <= cur_used_keyparts &&
              const_part & ((key_part_map)1 << table->quick_key_parts[key]) &&
              table->quick_n_ranges[key] == 1 + MY_TEST(ref_or_null_part & const_part) &&
              cur_fanout > (double)table->quick_rows[key])
          {
            tmp_fanout = cur_fanout = (double)table->quick_rows[key];
          }
    ...
    const double cur_ref_cost = cur_read_cost + prefix_rowcount * join->cost_model()->row_evaluate_cost(cur_fanout);
...
/**
  ...
  @return                     Cost of fetching rows from the storage
                              ...
                              by query conditions.
*/
double Optimize_table_order::calculate_scan_cost(const JOIN_TAB *tab, const uint idx, const Key_use *best_ref,
  ...
  */
  if (thd->optimizer_switch_flag(OPTIMIZER_SWITCH_COND_FANOUT_FILTER))
  {
    const float const_cond_filter =
        calculate_condition_filter(tab, NULL, 0, static_cast<double>(tab->found_records), !disable_jbuf);
  ...
  */
  ...
  else
  {
    ...
    if (table->force_index && !best_ref)  // index scan
      scan_cost = table->file->read_cost(tab->ref().key, 1, static_cast<double>(tab->records()));
    else
    ...
    if (disable_jbuf)
    {
      ...
      */
      scan_and_filter_cost = prefix_rowcount * (single_scan_read_cost +
    ...
    else
    {
      ...
      */
      const double buffer_count =
          1.0 + ((double)cache_record_length(join, idx) * prefix_rowcount / (double)thd->variables.join_buff_size);
...
void Optimize_table_order::best_access_path(JOIN_TAB *tab, const table_map remaining_tables, const uint idx,
                                            bool disable_jbuf, const double prefix_rowcount, POSITION *pos)
{
  ...
  */
  ...
  if (tab->keyuse() != NULL)
    best_ref =
        find_best_ref(tab, remaining_tables, idx, prefix_rowcount, &found_condition, &ref_depend_map, &used_key_parts);
  ...
  */
      ...
      best_read_cost <= tab->read_time)     // (1b)
  {
    ...
    trace_access_scan
        .add("cost", tab->read_time + cost_model->row_evaluate_cost(static_cast<double>(tab->found_records)))
        ...
        .add_alnum("cause", "cost");
           ...
           (tab->quick()->get_type() != QUICK_SELECT_I::QS_TYPE_GROUP_MIN_MAX))  // (2)
  {
    trace_access_scan.add_alnum("access_type", "range");
    tab->quick()->trace_quick_description(trace);
  ...
  else
  {
    ...
    */
    ...
    double scan_read_cost = calculate_scan_cost(tab, idx, best_ref, prefix_rowcount, found_condition, disable_jbuf,
    ...
    */
    const double scan_total_cost =
        scan_read_cost + cost_model->row_evaluate_cost(prefix_rowcount * rows_after_filtering);

    trace_access_scan.add("resulting_rows", rows_after_filtering);
    trace_access_scan.add("cost", scan_total_cost);

    if (best_ref == NULL ||
        (scan_total_cost < best_read_cost + cost_model->row_evaluate_cost(prefix_rowcount * rows_fetched)))
    {
      /*
      ...
      */
      ...
      if (tab->found_records)
      {
        /*
          ...
          therefore 'false'.
        */
        const float full_filter = calculate_condition_filter(tab, NULL, ~remaining_tables & ~excluded_tables,
                                                             static_cast<double>(tab->found_records), false);
        filter_effect = static_cast<float>(std::min(1.0, tab->found_records * full_filter / rows_after_filtering));
      }
      best_ref = NULL;
  ...
  */
  if (rows_fetched == 0.0 && (join->select_lex->outer_join & tab->table_ref->map()))
    rows_fetched = 1.0;
  ...
  */
  if (best_ref)
    filter_effect =
        calculate_condition_filter(tab, best_ref, ~remaining_tables & ~excluded_tables, rows_fetched, false);
...
float calculate_condition_filter(const JOIN_TAB *const tab, const Key_use *const keyuse, table_map used_tables,
                                 double fanout, bool is_join_buffering)
{
  /*
  ...
  */
  ...
  TABLE *const table = tab->table();
  const table_map remaining_tables = ~used_tables & ~tab->table_ref->map() & tab->join()->all_table_map;
  if (!(thd->optimizer_switch_flag(OPTIMIZER_SWITCH_COND_FANOUT_FILTER) &&  // 1)
  ...
  */
  if (!table->quick_keys.is_clear_all())
  {
    ...
    my_bitmap_map *bitbuf = static_cast<my_bitmap_map *>(static_cast<void *>(&buf));
    ...
    for (uint keyno = 0; keyno < table->s->keys; keyno++)
    {
      if (table->quick_keys.is_set(keyno))
      {
        ...
        for (uint i = 0; i < table->quick_key_parts[keyno]; i++)
          bitmap_set_bit(&fields_current_quick, key->key_part[i].field->field_index);
        ...
        */
        ...
        const float selectivity = static_cast<float>(table->quick_rows[keyno]) / static_cast<float>(tab->records());
  ...
  */
      ...
      !bitmap_is_subset(&table->cond_set, &table->tmp_set))  // 2)
  {
    ...
    */
    filter *= tab->join()->where_cond->get_filtering_effect(tab->table_ref->map(), used_tables, &table->tmp_set,
                                                                          ...
                                                                          POSITION *pos)
{
  ...
  const ulonglong bound_sj_equalities = get_bound_sj_equalities(tab, excluded_tables | remaining_tables);
  ...
  */
  for (Key_use *keyuse = tab->keyuse(); keyuse->table_ref == tab->table_ref;)
  {
    ...
    while (keyuse->table_ref == tab->table_ref && keyuse->key == key)
    {
      ...
      for (; keyuse->table_ref == tab->table_ref && keyuse->key == key && keyuse->keypart == keypart; ++keyuse)
      {
        /*
          ...
          equality".
        */
        if ((keyuse->sj_pred_no == UINT_MAX) || (excluded_tables & keyuse->used_tables) ||
            !(remaining_tables & keyuse->used_tables) || (keypart == FT_KEYPART) ||
            (table->key_info[key].key_part[keypart].key_part_flag & HA_PART_KEY_SEG))
          continue;
    ...
    */
    ...
    if (tab->quick() && tab->quick()->index == key && tab->quick()->get_type() == QUICK_SELECT_I::QS_TYPE_RANGE)
    {
    ...
    else if (tab->table()->covering_keys.is_set(key))
    {
      ...
      */
      ...
      const double cost = tab->table()->file->index_scan_cost(key, 1, rowcount).total_cost();
      ...
      */
      ...
      {
        const rec_per_key_t rpc = tab->table()->key_info[key].records_per_key(max_keypart);
  ...
  {
    pos->filter_effect =
        calculate_condition_filter(tab, pos->key, ~remaining_tables & ~excluded_tables, pos->rows_fetched, false);
    return true;
  }
...
bool Optimize_table_order::choose_table_order()
{
  ...
  const bool straight_join = join->select_lex->active_options() & SELECT_STRAIGHT_JOIN;
  ...
  if (emb_sjm_nest)
  {
    ...
    */
    merge_sort(join->best_ref + join->const_tables, join->best_ref + join->tables,
  ...
  else
  {
    ...
    */
    if (straight_join)
      ...
      merge_sort(join->best_ref + join->const_tables, join->best_ref + join->tables, Join_tab_compare_default());

    join_tables = join->all_table_map & ~join->const_table_map;
  }

  Opt_trace_object wrapper(&join->thd->opt_trace);
  Opt_trace_array trace_plan(&join->thd->opt_trace, "considered_execution_plans", Opt_trace_context::GREEDY_SEARCH);

  if (thd->optimizer_switch_flag(OPTIMIZER_SWITCH_COND_FANOUT_FILTER) && join->where_cond)
  {
    ...
    */
    join->where_cond->walk(&Item::add_field_to_cond_set_processor, Item::WALK_POSTFIX, NULL);
  }
...
static int semijoin_order_allows_materialization(const JOIN *join, table_map remaining_tables, const JOIN_TAB *tab,
                                                 uint idx)
{
  assert(!(remaining_tables & tab->table_ref->map()));
  /*
  ...
  */
  ...
  if (!emb_sj_nest || !emb_sj_nest->nested_join->sjm.positions || (remaining_tables & emb_sj_nest->sj_inner_tables))
    return SJ_OPT_NONE;
  ...
  */
  if ((remaining_tables & emb_sj_nest->nested_join->sj_depends_on) || !emb_sj_nest->nested_join->sjm.lookup_allowed)
  {
...
bool Optimize_table_order::greedy_search(table_map remaining_tables)
{
  ...
  do
  {
    ...
    */
    ...
    if (size_remain <= search_depth)
    {
      /*
        ...
        current partial QEP.
      */
      DBUG_EXECUTE("opt", print_plan(join, n_tables, idx ? join->best_positions[idx - 1].prefix_rowcount : 1.0,
                                     idx ? join->best_positions[idx - 1].prefix_cost : 0.0,
                                     idx ? join->best_positions[idx - 1].prefix_cost : 0.0, "optimal"););
      DBUG_RETURN(false);
    }
    ...
    */
    bool is_interleave_error MY_ATTRIBUTE((unused)) = check_interleaving_with_nj(best_table);
    ...
    */
    ...
    DBUG_EXECUTE("opt", print_plan(join, idx, join->positions[idx].prefix_rowcount, join->positions[idx].prefix_cost,
...
void Optimize_table_order::consider_plan(uint idx, Opt_trace_object *trace_obj)
{
  ...
  */
    ...
    for (uint i = join->const_tables; i <= idx && plan_uses_allowed_sj; i++)
      if (join->positions[i].sj_strategy == SJ_OPT_DUPS_WEEDOUT)
      {
        ...
        for (uint j = first; j <= i; j++)
        {
          ...
          if (emb_sj_nest && !(emb_sj_nest->nested_join->sj_enabled_strategies & OPTIMIZER_SWITCH_DUPSWEEDOUT))
  ...
  const bool chosen =
      found_plan_with_allowed_sj ? (plan_uses_allowed_sj && cheaper) : (plan_uses_allowed_sj || cheaper);
  ...
  DBUG_EXECUTE("opt", print_plan(join, idx + 1, join->positions[idx].prefix_rowcount, cost, cost, "full_plan"););
}
...
bool Optimize_table_order::best_extension_by_limited_search(table_map remaining_tables, uint idx,
                                                            uint current_search_depth)
{
  ...
  */
  ...
  DBUG_EXECUTE("opt", print_plan(join, idx, idx ? join->positions[idx - 1].prefix_rowcount : 1.0,
                                 idx ? join->positions[idx - 1].prefix_cost : 0.0,
                                 idx ? join->positions[idx - 1].prefix_cost : 0.0, "part_plan"););
  ...
  */
  ...
  for (JOIN_TAB **pos = join->best_ref + idx; *pos; pos++)
  {
    ...
    */
    ...
    if ((remaining_tables & real_table_bit) && !(eq_ref_extended & real_table_bit) &&
        !(remaining_tables & s->dependent) && (!idx || !check_interleaving_with_nj(s)))
    {
      ...
      /* Find the best access method from 's' to the current partial plan */
      best_access_path(s, remaining_tables, idx, false, idx ? (position - 1)->prefix_rowcount : 1.0, position);

      // Compute the cost of extending the plan with 's'
      position->set_prefix_join_cost(idx, cost_model);

      trace_one_table.add("condition_filtering_pct", position->filter_effect * 100)
      ...
      */
      ...
      {
        DBUG_EXECUTE("opt", print_plan(join, idx + 1, position->prefix_rowcount, position->read_cost,
                                       position->prefix_cost, "prune_by_cost"););
      ...
      */
      if (prune_level == 1)
      {
        if (best_rowcount > position->prefix_rowcount || best_cost > position->prefix_cost ||
        ...
        {
          DBUG_EXECUTE("opt", print_plan(join, idx + 1, position->prefix_rowcount, position->read_cost,
                                         position->prefix_cost, "pruned_by_heuristic"););
      ...
      if ((current_search_depth > 1) && remaining_tables_after)
      {
        ...
        */
            ...
            position->rows_fetched <= 1.0)  // 3)
        {
          ...
          */
          if (eq_ref_extended == (table_map)0)
          {
            ...
            eq_ref_extended = real_table_bit | eq_ref_extension_by_limited_search(remaining_tables_after, idx + 1,
          ...
          {
            DBUG_EXECUTE("opt", print_plan(join, idx + 1, position->prefix_rowcount, position->read_cost,
                                           position->prefix_cost, "pruned_by_eq_ref_heuristic"););
        ...
        Opt_trace_array trace_rest(trace, "rest_of_plan");
        if (best_extension_by_limited_search(remaining_tables_after, idx + 1, current_search_depth - 1))
          DBUG_RETURN(true);
      }
      else  // if ((current_search_depth > 1) && ...
      {
        ...
        */
        assert((remaining_tables_after != 0) ||
               ((cur_embedding_map == 0) && (join->positions[idx].dups_producing_tables == 0)));
      }
      backout_nj_state(remaining_tables, s);
    }
  }
...
table_map Optimize_table_order::eq_ref_extension_by_limited_search(table_map remaining_tables, uint idx,
                                                                   uint current_search_depth)
{
  ...
  for (JOIN_TAB **pos = join->best_ref + idx; (s = *pos); pos++)
  {
    ...
    */
        ...
        (!idx || !check_interleaving_with_nj(s)))  // 5)
    {
      ...
      */
      const bool added_to_eq_ref_extension = position->key &&
                                             almost_equal(position->read_cost, (position - 1)->read_cost) &&
                                             almost_equal(position->rows_fetched, (position - 1)->rows_fetched);
      trace_one_table.add("added_to_eq_ref_extension", added_to_eq_ref_extension);
      if (added_to_eq_ref_extension)
      {
        ...
        {
          DBUG_EXECUTE("opt", print_plan(join, idx + 1, position->prefix_rowcount, position->read_cost,
                                         position->prefix_cost, "EQ_REF_extension"););

          /* Recursively EQ_REF-extend the current partial plan */
          Opt_trace_array trace_rest(trace, "rest_of_plan");
          eq_ref_ext |= eq_ref_extension_by_limited_search(remaining_tables_after, idx + 1, current_search_depth - 1);
        }
        else
        {
          consider_plan(idx, &trace_one_table);
  ...
  */
  assert(!eq_ref_ext);
  if (best_extension_by_limited_search(remaining_tables, idx, current_search_depth))
...
bool Optimize_table_order::fix_semijoin_strategies()
{
  ...
  for (uint tableno = join->tables - 1; tableno != join->const_tables - 1; tableno--)
  {
    POSITION *const pos = join->best_positions + tableno;

    if ((handled_tables & pos->table->table_ref->map()) || pos->sj_strategy == SJ_OPT_NONE)
    {
    ...
    if (pos->sj_strategy == SJ_OPT_MATERIALIZE_LOOKUP)
    {
      ...
      /*
        ...
        not be read.
      */
      memcpy(pos - table_count + 1, sjm_nest->nested_join->sjm.positions, sizeof(POSITION) * table_count);
      ...
      trace_final_strategy.add_alnum("final_semijoin_strategy", "MaterializeLookup");
    }
    else if (pos->sj_strategy == SJ_OPT_MATERIALIZE_SCAN)
    {
      const uint last_inner = pos->sjm_scan_last_inner;
      TABLE_LIST *const sjm_nest = (join->best_positions + last_inner)->table->emb_sj_nest;
      const uint table_count = my_count_bits(sjm_nest->sj_inner_tables);
      ...
      memcpy(join->best_positions + first,  // stale semijoin strategy here too
             sjm_nest->nested_join->sjm.positions, sizeof(POSITION) * table_count);
      join->best_positions[first].sj_strategy = SJ_OPT_MATERIALIZE_SCAN;
      join->best_positions[first].n_sj_tables = table_count;

      Opt_trace_object trace_final_strategy(trace);
      trace_final_strategy.add_alnum("final_semijoin_strategy", "MaterializeScan");
      // Recalculate final access paths for this semi-join strategy
      double rowcount, cost;
      semijoin_mat_scan_access_paths(last_inner, tableno, remaining_tables, sjm_nest, true, &rowcount, &cost);
    }
    else if (pos->sj_strategy == SJ_OPT_FIRST_MATCH)
    {
      ...
      // Recalculate final access paths for this semi-join strategy
      double rowcount, cost;
      (void)semijoin_firstmatch_loosescan_access_paths(first, tableno, remaining_tables, false, true, &rowcount, &cost);
    }
    else if (pos->sj_strategy == SJ_OPT_LOOSE_SCAN)
    {
      ...
      // Recalculate final access paths for this semi-join strategy
      double rowcount, cost;
      (void)semijoin_firstmatch_loosescan_access_paths(first, tableno, remaining_tables, true, true, &rowcount, &cost);
    ...
    else if (pos->sj_strategy == SJ_OPT_DUPS_WEEDOUT)
    {
      ...
      */
      ...
      Opt_trace_object trace_final_strategy(trace);
      trace_final_strategy.add_alnum("final_semijoin_strategy", "DuplicateWeedout");
    }
...
bool Optimize_table_order::semijoin_firstmatch_loosescan_access_paths(uint first_tab, uint last_tab,
                                                                      table_map remaining_tables, bool loosescan,
                                                                      bool final, double *newcount, double *newcost)
{
  DBUG_ENTER("Optimize_table_order::semijoin_firstmatch_loosescan_access_paths");
  ...
  for (uint i = first_tab; i <= last_tab; i++)
  {
    ...
    */
    ...
    if (is_ls_driving_tab || positions[i].use_join_buffer)
    {
      ...
      best_access_path(tab, remaining_tables, i, i < no_jbuf_before, rowcount * inner_fanout * outer_fanout, dst_pos);
      test_all_ref_keys = false;
      if (is_ls_driving_tab)  // Use loose scan position
      {
        if (semijoin_loosescan_fill_driving_table_position(tab, remaining_tables, i, dst_pos))
        {
          dst_pos->table = tab;
          const double rows = rowcount * dst_pos->rows_fetched;
          dst_pos->set_prefix_cost(cost + dst_pos->read_cost + cost_model->row_evaluate_cost(rows),
    ...
    cost += pos->read_cost + cost_model->row_evaluate_cost(rowcount * inner_fanout * outer_fanout * pos->rows_fetched);
...
void Optimize_table_order::semijoin_mat_scan_access_paths(uint last_inner_tab, uint last_outer_tab,
                                                          ...
                                                          double *newcount, double *newcost)
{
  ...
  */
  ...
  for (uint i = last_inner_tab + 1; i <= last_outer_tab; i++)
  {
    ...
    POSITION regular_pos;
    POSITION *const dst_pos = final ? positions + i : &regular_pos;
    best_access_path(tab, remaining_tables, i, false, rowcount * inner_fanout * outer_fanout, dst_pos);
    remaining_tables &= ~tab->table_ref->map();
    outer_fanout *= dst_pos->rows_fetched;
    cost += dst_pos->read_cost + cost_model->row_evaluate_cost(rowcount * inner_fanout * outer_fanout);
...
void Optimize_table_order::semijoin_mat_lookup_access_paths(uint last_inner, TABLE_LIST *sjm_nest, double *newcount,
                                                            double *newcost)
{
...
void Optimize_table_order::semijoin_dupsweedout_access_paths(uint first_tab, uint last_tab, table_map remaining_tables,
                                                             double *newcount, double *newcost)
{
  ...
  */
  for (uint j = first_tab; j <= last_tab; j++)
  {
    ...
    cost += p->read_cost + cost_model->row_evaluate_cost(rowcount * inner_fanout * outer_fanout * p->rows_fetched);
  ...
  */
  ...
  cost += cost_model->tmptable_readwrite_cost(tmp_table_type, rowcount * outer_fanout,
...
void Optimize_table_order::advance_sj_state(table_map remaining_tables, const JOIN_TAB *new_join_tab, uint idx)
{
  Opt_trace_context *const trace = &thd->opt_trace;
  ...
  */
  ...
  else
  {
    ...
    // MaterializeScan
    pos->sjm_scan_need_tables = (pos[-1].sj_strategy == SJ_OPT_MATERIALIZE_SCAN) ? 0 : pos[-1].sjm_scan_need_tables;
  ...
  */
  if (emb_sj_nest && emb_sj_nest->nested_join->sj_enabled_strategies & OPTIMIZER_SWITCH_FIRSTMATCH)
  {
    ...
    */
    ...
    if (pos->first_firstmatch_table != MAX_TABLES)
    {
      ...
      else if (!(pos->firstmatch_need_tables & remaining_tables))
      {
        ...
        (void)semijoin_firstmatch_loosescan_access_paths(pos->first_firstmatch_table, idx, remaining_tables, false,
                                                         false, &rowcount, &cost);
        /*
  ...
  */
  {
    ...
    */
    ...
    {
      TABLE_LIST *const first_emb_sj_nest = join->positions[pos->first_loosescan_table].table->emb_sj_nest;
    ...
    */
    if (emb_sj_nest &&                                                                    // (1a)
        emb_sj_nest->nested_join->sj_enabled_strategies & OPTIMIZER_SWITCH_LOOSE_SCAN &&  // (1b)
    ...
    if ((pos->first_loosescan_table != MAX_TABLES) && !(remaining_tables & pos->loosescan_need_tables))
    {
      /*
      ...
      */
      if (semijoin_firstmatch_loosescan_access_paths(pos->first_loosescan_table, idx, remaining_tables, true, false,
                                                     &rowcount, &cost))
      {
        /*
        ...
        */
        ...
        best_rowcount = rowcount;
        trace_one_strategy.add("cost", best_cost).add("rows", best_rowcount);
        handled_by_fm_or_ls = join->positions[pos->first_loosescan_table].table->emb_sj_nest->sj_inner_tables;
      }
  ...
  const int sjm_strategy = semijoin_order_allows_materialization(join, remaining_tables, new_join_tab, idx);
  if (sjm_strategy == SJ_OPT_MATERIALIZE_SCAN)
  {
    ...
    */
    if (pos->sjm_scan_need_tables && emb_sj_nest != NULL &&
        emb_sj_nest != join->positions[pos->sjm_scan_last_inner].table->emb_sj_nest)
      /*
    ...
    {
      pos->sjm_scan_need_tables = emb_sj_nest->sj_inner_tables | emb_sj_nest->nested_join->sj_depends_on;
  ...
  */
  ...
  {
    TABLE_LIST *const sjm_nest = join->positions[pos->sjm_scan_last_inner].table->emb_sj_nest;
    ...
    semijoin_mat_scan_access_paths(pos->sjm_scan_last_inner, idx, remaining_tables, sjm_nest, false, &rowcount, &cost);
  ...
  /* Duplicate Weedout strategy handler */
  {
    ...
    */
    ...
    if (pos->dupsweedout_tables && !(remaining_tables & pos->dupsweedout_tables))
    {
      Opt_trace_object trace_one_strategy(trace);
      ...
      */
      double rowcount, cost;
      semijoin_dupsweedout_access_paths(pos->first_dupsweedout_table, idx, remaining_tables, &rowcount, &cost);
      /*
        Use the strategy if
