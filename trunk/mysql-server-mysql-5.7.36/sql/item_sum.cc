...
{
  ulonglong limitation = min(thd->variables.tmp_table_size, thd->variables.max_heap_table_size);
...
bool Item_sum::check_sum_func(THD *thd, Item **ref)
{
  ...
  */
  ...
  else if (max_arg_level >= 0 || !(allow_sum_func & ((nesting_map)1 << nest_level)))
  {
    /*
    ...
    */
    if (register_sum_func(thd, ref))
      return TRUE;
    invalid = aggr_level < 0 && !(allow_sum_func & ((nesting_map)1 << nest_level));
...
bool Item_sum::register_sum_func(THD *thd, Item **ref)
{
  ...
  for (SELECT_LEX *sl = thd->lex->current_select()->outer_select(); sl && sl->nest_level >= max_arg_level;
       sl = sl->outer_select())
  {
  ...
  if (aggr_level >= 0)
  {
    ...
    */
    for (SELECT_LEX *sl = thd->lex->current_select(); sl && sl != aggr_sel && sl->master_unit()->item;
         sl = sl->outer_select())
...
Item_sum::Item_sum(const POS &pos, PT_item_list *opt_list)
    : super(pos), next(NULL), arg_count(opt_list == NULL ? 0 : opt_list->elements()), forced_const(FALSE)
{
  if (arg_count > 0)
  {
...
bool Item_sum::eq(const Item *item, bool binary_cmp) const
{
  ...
  if (arg_count != item_sum->arg_count ||
      (my_sum_func != Item_sum::UDF_SUM_FUNC && func_name() != item_sum->func_name()) ||
      (my_sum_func == Item_sum::UDF_SUM_FUNC && my_strcasecmp(system_charset_info, func_name(), item_sum->func_name())))
    return false;
  for (uint i = 0; i < arg_count; i++)
  {
...
void Item_sum::update_used_tables()
{
  if (!forced_const)
  {
    ...
    */
    used_tables_cache |=
        aggr_level == nest_level ? ((table_map)1 << aggr_sel->leaf_table_count) - 1 : OUTER_REF_TABLE_BIT;
  }
}
  ...
  */
  ...
  if (item_sum->sum_func() == Item_sum::COUNT_FUNC || item_sum->sum_func() == Item_sum::COUNT_DISTINCT_FUNC)
  {
    ...
    */
    ...
    if (!(table = create_tmp_table(thd, tmp_table_param, list, NULL, true, false, select_lex->active_options(),
                                   HA_POS_ERROR, "")))
      return TRUE;
    ...
    if (table->s->db_type() == heap_hton)
    {
      ...
      */
      ...
      assert(tree == 0);
      tree = new Unique(compare_key, cmp_arg, tree_key_length, item_sum->ram_limitation(thd));
      /*
  ...
  else
  {
    ...
    */
    ...
    field_type = calc_tmp_field_type(arg->field_type(), arg->result_type());
    field_def.init_for_tmp_table(field_type, arg->max_length, arg->decimals, arg->maybe_null, arg->unsigned_flag);
    ...
    */
    tree = new Unique(simple_raw_key_cmp, &tree_key_length, tree_key_length, item_sum->ram_limitation(thd));
...
bool Aggregator_distinct::add()
{
  ...
  if (item_sum->sum_func() == Item_sum::COUNT_FUNC || item_sum->sum_func() == Item_sum::COUNT_DISTINCT_FUNC)
  {
    ...
    if (!check_unique_constraint(table))
      return false;
    if ((error = table->file->ha_write_row(table->record[0])) && !table->file->is_ignorable_error(error))
      return TRUE;
    return FALSE;
  }
  else
...
Field *Item_sum_hybrid::create_tmp_field(bool group, TABLE *table)
{
  ...
  if (args[0]->type() == Item::FIELD_ITEM)
  {
    ...
    if ((field = create_tmp_field_from_field(current_thd, field, item_name.ptr(), table, NULL)))
...
Field *Item_sum_avg::create_tmp_field(bool group, TABLE *table)
{
  ...
  if (group)
  {
    ...
    */
    field = new Field_string(((hybrid_type == DECIMAL_RESULT) ? dec_bin_size : sizeof(double)) + sizeof(longlong), 0,
...
void Item_sum_hybrid::min_max_update_str_field()
{
  ...
  if (!args[0]->null_value)
  {
    ...
    if (result_field->is_null() || (cmp_sign * sortcmp(res_str, &cmp->value2, collation.collation)) < 0)
...
extern "C" int group_concat_key_cmp_with_distinct(const void *arg, const void *key1, const void *key2)
{
...
extern "C" int group_concat_key_cmp_with_order(const void *arg, const void *key1, const void *key2)
{
  const Item_func_group_concat *grp_item = (Item_func_group_concat *)arg;
  const ORDER *order_item, *end;
  TABLE *table = grp_item->table;

  for (order_item = grp_item->order_array.begin(), end = grp_item->order_array.end(); order_item < end; order_item++)
  {
...
extern "C" int dump_leaf_key(void *key_arg, element_count count MY_ATTRIBUTE((unused)), void *item_arg)
{
  ...
  if (result->length() > item->max_length)
  {
    ...
    */
    add_length =
        cs->cset->well_formed_len(cs, ptr + old_length, ptr + item->max_length, result->length(), &well_formed_error);
    result->length(old_length + add_length);
    item->warning_for_row = TRUE;
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_CUT_VALUE_GROUP_CONCAT,
                        ER(ER_CUT_VALUE_GROUP_CONCAT), item->row_count);
...
Item_func_group_concat::Item_func_group_concat(const POS &pos, bool distinct_arg, PT_item_list *select_list,
                                               PT_order_list *opt_order_list, String *separator_arg)
    : super(pos),
      tmp_table_param(0),
      ...
      original(0)
{
  ...
  if (arg_count_order)
  {
    for (ORDER *order_item = opt_order_list->value.first; order_item != NULL; order_item = order_item->next)
    {
...
Field *Item_func_group_concat::make_string_field(TABLE *table_arg)
{
  ...
  */
  ...
  if (max_characters > CONVERT_IF_BIGGER_TO_BLOB)
    field = new Field_blob(max_characters * collation.collation->mbmaxlen, maybe_null, item_name.ptr(),
                           collation.collation, true);
  else
    field = new Field_varstring(max_characters * collation.collation->mbmaxlen, maybe_null, item_name.ptr(),
...
bool Item_func_group_concat::add()
{
  ...
  {
    DBUG_EXECUTE_IF("trigger_OOM_in_gconcat_add", DBUG_SET("+d,simulate_persistent_out_of_memory"););
...
bool Item_func_group_concat::fix_fields(THD *thd, Item **ref)
{
  ...
  */
  ...
  if (agg_item_charsets_for_string_result(collation, func_name(), args, arg_count - arg_count_order))
    return 1;
...
bool Item_func_group_concat::setup(THD *thd)
{
  ...
  */
  if (arg_count_order &&
      setup_order(thd, Ref_ptr_array(args, arg_count), context->table_list, list, all_fields, order_array.begin()))
    DBUG_RETURN(TRUE);
  ...
  /*
    ...
    field list.
  */
  if (!(table = create_tmp_table(thd, tmp_table_param, all_fields, NULL, false, true, select_lex->active_options(),
                                 HA_POS_ERROR, (char *)"")))
    DBUG_RETURN(TRUE);
  ...
  */
  ...
  if (arg_count_order)
  {
    ...
    /*
      ...
      create this tree.
    */
    init_tree(tree, min(static_cast<ulong>(thd->variables.max_heap_table_size), thd->variables.sortbuff_size / 16), 0,
              tree_key_length, group_concat_key_cmp_with_order, 0, NULL, (void *)this);
  }

  if (distinct)
    unique_filter = new Unique(group_concat_key_cmp_with_distinct, (void *)this, tree_key_length, ram_limitation(thd));
...
String *Item_func_group_concat::val_str(String *str)
{
  ...
  if (table && table->blob_storage && table->blob_storage->is_truncated_value())
  {
    ...
    push_warning_printf(current_thd, Sql_condition::SL_WARNING, ER_CUT_VALUE_GROUP_CONCAT,
                        ER(ER_CUT_VALUE_GROUP_CONCAT), row_count);
  }
...
bool Item_sum_json_array::add()
{
  ...
  try
  {
    ...
    // Get the value.
    if (get_atom_null_as_null(args, 0, func_name(), &m_value, &m_conversion_buffer, &value_wrapper))
      return error_json();
...
bool Item_sum_json_object::add()
{
  ...
  try
  {
    ...
    if (get_json_string(key_item, &m_tmp_key_value, &m_conversion_buffer, func_name(), &safep, &safe_length))
    {
    ...
    // value
    Json_wrapper value_wrapper;
    if (get_atom_null_as_null(args, 1, func_name(), &m_value, &m_conversion_buffer, &value_wrapper))
      return error_json();
