...
static uchar *ndb_schema_objects_get_key(NDB_SCHEMA_OBJECT *schema_object, size_t *length, my_bool)
{
...
class Ndb_schema_objects
{
 public:
  ...
  Ndb_schema_objects()
  {
    (void)my_hash_init(&m_hash, &my_charset_bin, 1, 0, 0, (my_hash_get_key)ndb_schema_objects_get_key, 0, 0,
                       PSI_INSTRUMENT_ME);
  }
...
NDB_SCHEMA_OBJECT *ndb_get_schema_object(const char *key, bool create_if_not_exists)
{
  ...
  while (!(ndb_schema_object =
               (NDB_SCHEMA_OBJECT *)my_hash_search(&ndb_schema_objects.m_hash, (const uchar *)key, length)))
  {
    ...
    if (!(ndb_schema_object = (NDB_SCHEMA_OBJECT *)my_malloc(PSI_INSTRUMENT_ME, sizeof(*ndb_schema_object) + length + 1,
    ...
    bitmap_init(&ndb_schema_object->slock_bitmap, ndb_schema_object->slock, sizeof(ndb_schema_object->slock) * 8,
                FALSE);
