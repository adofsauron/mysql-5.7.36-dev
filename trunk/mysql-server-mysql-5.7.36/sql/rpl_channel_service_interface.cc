...
int initialize_channel_service_interface()
{
  ...
  if (opt_mi_repository_id != INFO_REPOSITORY_TABLE || opt_rli_repository_id != INFO_REPOSITORY_TABLE)
  {
    sql_print_error(
        "For the creation of replication channels the master info"
  ...
  {
    sql_print_error(
        "For the creation of replication channels the server id"
...
void set_mi_settings(Master_info *mi, Channel_creation_info *channel_info)
{
  ...
  mi->rli->set_ignore_write_set_memory_limit(channel_info->m_ignore_write_set_memory_limit);
  mi->rli->set_allow_drop_write_set(channel_info->m_allow_drop_write_set);

  mi->rli->replicate_same_server_id = (channel_info->replicate_same_server_id == RPL_SERVICE_SERVER_DEFAULT)
                                          ? replicate_same_server_id
                                          : channel_info->replicate_same_server_id;

  mi->rli->opt_slave_parallel_workers = (channel_info->channel_mts_parallel_workers == RPL_SERVICE_SERVER_DEFAULT)
  ...
  mi->rli->checkpoint_group = (channel_info->channel_mts_checkpoint_group == RPL_SERVICE_SERVER_DEFAULT)
...
{
  lex_mi->ssl = (channel_ssl_info->use_ssl) ? LEX_MASTER_INFO::LEX_MI_ENABLE : LEX_MASTER_INFO::LEX_MI_DISABLE;
  ...
  lex_mi->ssl_verify_server_cert =
      (channel_ssl_info->ssl_verify_server_cert) ? LEX_MASTER_INFO::LEX_MI_ENABLE : LEX_MASTER_INFO::LEX_MI_DISABLE;
}
...
int channel_create(const char *channel, Channel_creation_info *channel_info)
{
  ...
  */
  ...
  if (channel_info->auto_position)
  {
    ...
    if ((mi && mi->is_auto_position()) || channel_info->auto_position == RPL_SERVICE_SERVER_DEFAULT)
    {
  ...
  if (mi)
  {
    ...
    if ((error = change_master(thd, mi, lex_mi, channel_info->preserve_relay_logs)))
    {
      goto err;
    }
  }
...
int channel_start(const char *channel, Channel_connection_info *connection_info, int threads_to_start,
...
int channel_stop(Master_info *mi, int threads_to_stop, long timeout)
{
  ...
  if ((threads_to_stop & CHANNEL_APPLIER_THREAD) && (server_thd_mask & SLAVE_SQL))
  {
    thread_mask |= SLAVE_SQL;
  }
  if ((threads_to_stop & CHANNEL_RECEIVER_THREAD) && (server_thd_mask & SLAVE_IO))
  {
    thread_mask |= SLAVE_IO;
  }
...
int channel_stop_all(int threads_to_stop, long timeout)
{
  ...
  for (mi_map::iterator it = channel_map.begin(); it != channel_map.end(); it++)
  {
    ...
    if (mi)
    {
      ...
      DBUG_EXECUTE_IF("group_replication_stop_all_channels_failure", { channel_error = 1; });

      if (channel_error && channel_error != RPL_CHANNEL_SERVICE_CHANNEL_DOES_NOT_EXISTS_ERROR)
      {
        error = channel_error;
        mi->report(ERROR_LEVEL, error, "Error stopping channel: %s. Got error: %d", mi->get_channel(), error);
      }
    }
  }
...
int channel_get_thread_id(const char *channel, enum_channel_thread_types thd_type, unsigned long **thread_id)
{
  ...
  switch (thd_type)
  {
    case CHANNEL_RECEIVER_THREAD:
      ...
      if (mi->info_thd != NULL)
      {
        *thread_id = (unsigned long *)my_malloc(PSI_NOT_INSTRUMENTED, sizeof(unsigned long), MYF(MY_WME));
    ...
    case CHANNEL_APPLIER_THREAD:
      if (mi->rli != NULL)
      {
        ...
        if (mi->rli->slave_parallel_workers > 0)
        {
          ...
          *thread_id =
              (unsigned long *)my_malloc(PSI_NOT_INSTRUMENTED, number_threads * sizeof(unsigned long), MYF(MY_WME));
          ...
          if (mi->rli->workers_array_initialized)
          {
            for (size_t i = 0; i < num_workers; i++, thread_id_pointer++)
            {
              ...
              if (worker != NULL)
              {
                ...
                if (worker->info_thd != NULL && worker->running_status != Slave_worker::NOT_RUNNING)
                {
...
long long channel_get_last_delivered_gno(const char *channel, int sidno)
{
  ...
  global_sid_lock->unlock();
  DBUG_PRINT("info", ("get_last_delivered_gno retrieved_set_string: %s", retrieved_gtid_set_string));
...
int channel_add_executed_gtids_to_received_gtids(const char *channel)
{
  ...
  enum_return_status return_status = mi->rli->add_gtid_set(gtid_state->get_executed_gtids());
...
int channel_wait_until_apply_queue_applied(const char *channel, double timeout)
{
  ...
  int error = mi->rli->wait_for_gtid_set(current_thd, mi->rli->get_gtid_set(), timeout);
  mi->dec_reference();
...
int channel_is_applier_waiting(const char *channel)
{
  ...
  int number_appliers = channel_get_thread_id(channel, CHANNEL_APPLIER_THREAD, &thread_ids);
