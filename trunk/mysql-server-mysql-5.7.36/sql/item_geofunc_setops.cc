...
#define BGOPCALL(GeoOutType, geom_out, bgop, GeoType1, g1, GeoType2, g2, wkbres, nullval)              \
  do                                                                                                   \
  {                                                                                                    \
    ...
    {                                                                                                  \
      ...
      (nullval) = false;                                                                               \
      if (geout->size() == 0 || (nullval = post_fix_result(&(m_ifso->bg_resbuf_mgr), *geout, wkbres))) \
      {                                                                                                \
        if (nullval)                                                                                   \
          return NULL;                                                                                 \
      }                                                                                                \
...
class BG_setop_wrapper
{
 ...
 public:
  ...
  /**
    ...
    */
  Geometry *point_intersection_point(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    Point pt1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Point pt2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  */
  Geometry *point_intersection_multipoint(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
  ...
  Geometry *point_intersection_geometry(Geometry *g1, Geometry *g2, String *result)
  {
#if !defined(NDEBUG)
    ...
    bool is_out = !Ifsr::bg_geo_relation_check<Coordsys>(g1, g2, Ifsr::SP_DISJOINT_FUNC, &null_value);

    assert(gt2 == Geometry::wkb_linestring || gt2 == Geometry::wkb_polygon || gt2 == Geometry::wkb_multilinestring ||
  ...
  Geometry *multipoint_intersection_multipoint(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
  ...
  Geometry *multipoint_intersection_geometry(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
#if !defined(NDEBUG)
    Geometry::wkbType gt2 = g2->get_type();
    ...
    for (TYPENAME Point_set::iterator i = ptset.begin(); i != ptset.end(); ++i)
    {
      ...
      if (!Ifsr::bg_geo_relation_check<Coordsys>(&pt, g2, Ifsr::SP_DISJOINT_FUNC, &null_value) && !null_value)
      {
        mpts2->push_back(pt);
      }
  ...
  Geometry *linestring_intersection_linestring(Geometry *g1, Geometry *g2, String *result)
  {
    ...
    BGOPCALL(Multilinestring, res, intersection, Linestring, g1, Linestring, g2, NULL, null_value);
  ...
  Geometry *linestring_intersection_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry::wkbType gt2 = g2->get_type();
    ...
    {
      BGOPCALL(Multilinestring, tmp1, intersection, Linestring, g1, Multipolygon, g2, NULL, null_value);
      BGOPCALL(Multipoint, tmp2, intersection, Linestring, g1, Multipolygon, g2, NULL, null_value);
    }
    else
    {
      BGOPCALL(Multilinestring, tmp1, intersection, Linestring, g1, Polygon, g2, NULL, null_value);
      BGOPCALL(Multipoint, tmp2, intersection, Linestring, g1, Polygon, g2, NULL, null_value);
    }

    // Need merge, exclude Points that are on the result Linestring.
    retgeo = m_ifso->combine_sub_results<Coordsys>(tmp1, tmp2, result);
    copy_ifso_state();

    return retgeo;
  }

  Geometry *linestring_intersection_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Multilinestring *res = NULL;
    Geometry *retgeo = NULL;
    ...
    // multilinestring. Intersection points are represented as lines
    // with the same start and end points.
    BGOPCALL(Multilinestring, res, intersection, Linestring, g1, Multilinestring, g2, NULL, null_value);
  ...
  Geometry *polygon_intersection_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL, *tmp1 = NULL;
    Multipoint *tmp2 = NULL;
    auto_ptr<Geometry> guard1;

    BGOPCALL(Multilinestring, tmp1, intersection, Polygon, g1, Multilinestring, g2, NULL, null_value);
    guard1.reset(tmp1);

    Multilinestring mlstr(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
    Multipoint mpts;
    Point_set ptset;
    ...
    Polygon plgn(data_ptr, g1->get_data_size(), g1->get_flags(), g1->get_srid());

    for (TYPENAME Multilinestring::iterator i = mlstr.begin(); i != mlstr.end(); ++i)
    {
    ...
    retgeo = m_ifso->combine_sub_results<Coordsys>(guard1.release(), guard2.release(), result);
    copy_ifso_state();
  ...
  void plgn_intersection_plgn_mls(const Polygon_type1 &plgn1, const Polygon_type2 &plgn2, const Multipolygon &mplgn,
                                  Multilinestring &mls)
  {
    if (mplgn.size() > 0)
    {
  ...
  Geometry *polygon_intersection_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry::wkbType gt2 = g2->get_type();
    Geometry *retgeo = NULL;
    ...
    {
      Multipolygon mplgn2(pg2, g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  Geometry *combine_mls_mplgn_results(Multilinestring *mls, auto_ptr<Multipolygon> mplgn_result, String *result)
  {
    ...
    if (mplgn_result->size() > 0)
    {
      if (mls->size() == 0)
      {
        ...
        null_value = post_fix_result(&(m_ifso->bg_resbuf_mgr), *mplgn_result, result);
        if (null_value)
          return NULL;
        else
      ...
      {
        String mplgn_resbuf;
        null_value = post_fix_result(&(m_ifso->bg_resbuf_mgr), *mplgn_result, &mplgn_resbuf);
        if (null_value)
          return NULL;
        if (geom->get_type() == Geometry::wkb_geometrycollection)
        {
          down_cast<Gis_geometry_collection *>(geom)->append_geometry(&(*mplgn_result), result);
          retgeo = geom;
        }
        else
        {
          ...
          Gis_geometry_collection *tmp_gc = new Gis_geometry_collection(&(*mplgn_result), result);
  ...
  Geometry *multilinestring_intersection_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Multilinestring *res = NULL;
    Geometry *retgeo = NULL;
    ...
    // multilinestring. Intersection points are represented as lines
    // with the same start and end points.
    BGOPCALL(Multilinestring, res, intersection, Multilinestring, g1, Multilinestring, g2, NULL, null_value);
  ...
  Geometry *multilinestring_intersection_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL, *tmp1 = NULL;
    Multipoint *tmp2 = NULL;

    auto_ptr<Geometry> guard1;
    BGOPCALL(Multilinestring, tmp1, intersection, Multilinestring, g1, Multipolygon, g2, NULL, null_value);
    guard1.reset(tmp1);

    Multilinestring mlstr(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Multipoint mpts;
    ...
    Multipolygon mplgn(data_ptr, g2->get_data_size(), g2->get_flags(), g2->get_srid());
    Point_set ptset;
  ...
  Geometry *multipolygon_intersection_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    ...
    Multilinestring mls;
    Multipolygon mplgn1(pg1, g1->get_data_size(), g1->get_flags(), g1->get_srid());
  ...
  Geometry *point_union_geometry(Geometry *g1, Geometry *g2, String *result)
  {
    ...
    assert(gt2 == Geometry::wkb_linestring || gt2 == Geometry::wkb_polygon || gt2 == Geometry::wkb_multilinestring ||
           gt2 == Geometry::wkb_multipolygon);
    if (Ifsr::bg_geo_relation_check<Coordsys>(g1, g2, Ifsr::SP_DISJOINT_FUNC, &null_value) && !null_value)
    {
  ...
  Geometry *linestring_union_linestring(Geometry *g1, Geometry *g2, String *result)
  {
    Linestring ls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Linestring ls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  Geometry *linestring_union_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    ...
    if (post_fix_result(&m_ifso->bg_resbuf_mgr, *linestrings, NULL) && linestrings->size() > 0)
    {
      my_error(ER_GIS_UNKNOWN_ERROR, MYF(0), m_ifso->func_name());
      null_value = TRUE;
      return NULL;
  ...
  Geometry *linestring_union_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Linestring ls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Multilinestring mls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
    auto_ptr<Multilinestring> res(new Multilinestring);
    res->set_srid(g1->get_srid());
  ...
  Geometry *linestring_union_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    const void *g2_wkb = g2->normalize_ring_order();
    if (g2_wkb == NULL)
    ...
    Linestring ls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Multipolygon mpy2(g2_wkb, g2->get_data_size(), g2->get_flags(), g2->get_srid());
    auto_ptr<Multilinestring> linestrings(new Multilinestring());
    linestrings->set_srid(g1->get_srid());
  ...
  Geometry *polygon_union_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
  ...
  Geometry *multipoint_union_multipoint(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Point_set ptset;
    Multipoint *mpts = new Multipoint();
  ...
  Geometry *multipoint_union_geometry(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
#if !defined(NDEBUG)
    Geometry::wkbType gt2 = g2->get_type();
    ...
    {
      Point &pt = const_cast<Point &>(*i);
      if (Ifsr::bg_geo_relation_check<Coordsys>(&pt, g2, Ifsr::SP_DISJOINT_FUNC, &null_value))
      {
  ...
  Geometry *multilinestring_union_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Multilinestring mls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Multilinestring mls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
    auto_ptr<Multilinestring> res(new Multilinestring());
    res->set_srid(g1->get_srid());
  ...
  Geometry *multilinestring_union_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    const void *g2_wkb = g2->normalize_ring_order();
    if (g2_wkb == NULL)
  ...
  Geometry *polygon_union_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    ...
    Polygon py1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Polygon py2(g2_wkb, g2->get_data_size(), g2->get_flags(), g2->get_srid());
    auto_ptr<Multipolygon> res(new Multipolygon());
    res->set_srid(g1->get_srid());

    boost::geometry::union_(py1, py2, *res);
    if (post_fix_result(&m_ifso->bg_resbuf_mgr, *res, result) && res->size() > 0)
    {
      my_error(ER_GIS_UNKNOWN_ERROR, MYF(0), m_ifso->func_name());
      null_value = TRUE;
      return NULL;
  ...
  Geometry *polygon_union_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, union_, Polygon, g1, Multipolygon, g2, result, null_value);
    return retgeo;
  }

  Geometry *multipolygon_union_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, union_, Multipolygon, g1, Multipolygon, g2, result, null_value);

    return retgeo;
  }

  Geometry *point_difference_geometry(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    bool is_out = Ifsr::bg_geo_relation_check<Coordsys>(g1, g2, Ifsr::SP_DISJOINT_FUNC, &null_value);
  ...
  Geometry *multipoint_difference_geometry(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Multipoint *mpts = new Multipoint();
    auto_ptr<Multipoint> guard(mpts);
    ...
    for (TYPENAME Multipoint::iterator i = mpts1.begin(); i != mpts1.end(); ++i)
    {
      if (Ifsr::bg_geo_relation_check<Coordsys>(&(*i), g2, Ifsr::SP_DISJOINT_FUNC, &null_value))
      {
        if (null_value)
          return 0;
        ptset.insert(*i);
  ...
  Geometry *linestring_difference_linestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Linestring ls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Linestring ls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
    ...
    else if (res->size() == 1)
    {
      ...
      ls->set_ptr(const_cast<char *>(result->ptr()) + GEOM_HEADER_SIZE, result->length() - GEOM_HEADER_SIZE);
      ls->set_ownmem(false);
      retgeo = ls;
    }
    else
  ...
  Geometry *linestring_difference_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multilinestring, retgeo, difference, Linestring, g1, Polygon, g2, result, null_value);
  ...
  Geometry *linestring_difference_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Linestring ls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Multilinestring mls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  Geometry *linestring_difference_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multilinestring, retgeo, difference, Linestring, g1, Multipolygon, g2, result, null_value);
  ...
  Geometry *polygon_difference_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, difference, Polygon, g1, Polygon, g2, result, null_value);
  ...
  Geometry *polygon_difference_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, difference, Polygon, g1, Multipolygon, g2, result, null_value);
  ...
  Geometry *multilinestring_difference_linestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Multilinestring mls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Linestring ls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  Geometry *multilinestring_difference_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multilinestring, retgeo, difference, Multilinestring, g1, Polygon, g2, result, null_value);
  ...
  Geometry *multilinestring_difference_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Multilinestring mls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Multilinestring mls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  Geometry *multilinestring_difference_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multilinestring, retgeo, difference, Multilinestring, g1, Multipolygon, g2, result, null_value);
  ...
  Geometry *multipolygon_difference_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, difference, Multipolygon, g1, Polygon, g2, result, null_value);
  ...
  Geometry *multipolygon_difference_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, difference, Multipolygon, g1, Multipolygon, g2, result, null_value);
  ...
  Geometry *linestring_symdifference_linestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Linestring ls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Linestring ls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  Geometry *linestring_symdifference_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Linestring ls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Multilinestring mls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  Geometry *polygon_symdifference_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, sym_difference, Polygon, g1, Polygon, g2, result, null_value);
  ...
  Geometry *polygon_symdifference_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, sym_difference, Polygon, g1, Multipolygon, g2, result, null_value);
  ...
  Geometry *multilinestring_symdifference_multilinestring(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;
    Multilinestring mls1(g1->get_data_ptr(), g1->get_data_size(), g1->get_flags(), g1->get_srid());
    Multilinestring mls2(g2->get_data_ptr(), g2->get_data_size(), g2->get_flags(), g2->get_srid());
  ...
  Geometry *multipolygon_symdifference_polygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, sym_difference, Multipolygon, g1, Polygon, g2, result, null_value);
  ...
  Geometry *multipolygon_symdifference_multipolygon(Geometry *g1, Geometry *g2, String *result)
  {
    Geometry *retgeo = NULL;

    BGOPCALL(Multipolygon, retgeo, sym_difference, Multipolygon, g1, Multipolygon, g2, result, null_value);
 ...
 */
template <typename Geom_types>
Geometry *Item_func_spatial_operation::intersection_operation(Geometry *g1, Geometry *g2, String *result)
{
  typedef typename Geom_types::Coordinate_type Coord_type;
  typedef typename Geom_types::Coordinate_system Coordsys;
 ...
 */
template <typename Geom_types>
Geometry *Item_func_spatial_operation::union_operation(Geometry *g1, Geometry *g2, String *result)
{
  typedef typename Geom_types::Coordinate_type Coord_type;
  typedef typename Geom_types::Coordinate_system Coordsys;
 ...
 */
template <typename Geom_types>
Geometry *Item_func_spatial_operation::difference_operation(Geometry *g1, Geometry *g2, String *result)
{
  BG_setop_wrapper<Geom_types> wrap(this);
  Geometry *retgeo = NULL;
  Geometry::wkbType gt1 = g1->get_type();
 ...
 */
template <typename Geom_types>
Geometry *Item_func_spatial_operation::symdifference_operation(Geometry *g1, Geometry *g2, String *result)
{
  typedef typename Geom_types::Coordinate_type Coord_type;
  typedef typename Geom_types::Coordinate_system Coordsys;
...
Geometry *Item_func_spatial_operation::bg_geo_set_op(Geometry *g1, Geometry *g2, String *result)
{
 ...
 */
template <typename Coordsys>
Geometry *Item_func_spatial_operation::combine_sub_results(Geometry *geo1, Geometry *geo2, String *result)
{
  typedef BG_models<Coordsys> Geom_types;
  typedef typename Geom_types::Multipoint Multipoint;
  Geometry *retgeo = NULL;
  ...
  assert((geo1->get_type() == Geometry::wkb_multilinestring || geo1->get_type() == Geometry::wkb_multipolygon) &&
         geo2->get_type() == Geometry::wkb_multipoint);
  Multipoint mpts(geo2->get_data_ptr(), geo2->get_data_size(), geo2->get_flags(), geo2->get_srid());
  ...
  {
    isin = !Item_func_spatial_rel::bg_geo_relation_check<Coordsys>(&(*i), geo1, SP_DISJOINT_FUNC, &had_error);
...
*/
Geometry *Item_func_spatial_operation::simplify_multilinestring(Gis_multi_line_string *mls, String *result)
{
  ...
  if (points->size() == 0 && linestrings->size() == 1)
  {
    ...
    linestringresult->set_ptr(const_cast<char *>(result->ptr()) + oldlength + GEOM_HEADER_SIZE,
  ...
  else if (points->size() == 1 && linestrings->size() == 0)
  {
    ...
    size_t newlength = result->length();
    pointresult->set_ptr(const_cast<char *>(result->ptr()) + oldlength + GEOM_HEADER_SIZE,
                         (newlength - oldlength) - GEOM_HEADER_SIZE);
...
bool simplify_multi_geometry(String *str, String *result_buffer)
{
  ...
  if (gtype == Geometry::wkb_multipoint || gtype == Geometry::wkb_multilinestring ||
  ...
  else if (gtype == Geometry::wkb_geometrycollection)
  {
    ...
    wkb_scanner(p + GEOM_HEADER_SIZE, &wkb_len, Geometry::wkb_geometrycollection, false, &ex);
...
String *Item_func_spatial_operation::val_str(String *str_value_arg)
{
  ...
  */
  try
  {
    if (g1->get_type() != Geometry::wkb_geometrycollection && g2->get_type() != Geometry::wkb_geometrycollection)
      ...
      gres = geometry_collection_set_operation<bgcs::cartesian>(g1, g2, str_value_arg);
  }
  catch (...)
  {
    had_except1 = true;
  ...
  */
  if (simplify_multi_geometry(str_value_arg, (result_is_args ? &m_result_buffer : NULL)) && result_is_args)
...
{
  return g != NULL && (g->get_type() == Geometry::wkb_polygon || g->get_type() == Geometry::wkb_multipolygon);
}
 ...
 */
template <typename Coordsys>
Geometry *Item_func_spatial_operation::geometry_collection_set_operation(Geometry *g1, Geometry *g2, String *result)
{
  Geometry *gres = NULL;
  ...
  if (empty1 || empty2)
  {
    if (spatial_op == op_intersection ||
        (empty1 && empty2 && (spatial_op == op_symdifference || spatial_op == op_union)) ||
  ...
  */
  if (spatial_op == op_union || spatial_op == op_symdifference)
  {
  ...
  if (gv1.size() == 1 && gv2.size() == 1 &&
      (spatial_op != op_symdifference || (is_areal(*(gv1.begin())) && is_areal(*(gv2.begin())))))
  {
     ...
     */
    bool do_dup = false;
    for (BG_geometry_collection::Geometry_list::iterator i = gv1.begin(); i != gv1.end(); ++i)
      if (*i == gres)
        do_dup = true;
    if (!do_dup)
      for (BG_geometry_collection::Geometry_list::iterator i = gv2.begin(); i != gv2.end(); ++i)
        if (*i == gres)
          do_dup = true;
    ...
    if (do_dup)
    {
      ...
      const void *data_start = static_cast<const char *>(tmpres.ptr()) + GEOM_HEADER_SIZE;
 ...
 */
template <typename Coordsys>
Geometry *Item_func_spatial_operation::geocol_intersection(const BG_geometry_collection &bggc1,
                                                           const BG_geometry_collection &bggc2, String *result)
{
  ...
  for (BG_geometry_collection::Geometry_list::const_iterator i = gv->begin(); i != gv->end(); ++i)
  {
    BG_box box;
    make_bg_box(*i, &box);
 ...
 */
template <typename Coordsys>
Geometry *Item_func_spatial_operation::geocol_union(const BG_geometry_collection &bggc1,
                                                    const BG_geometry_collection &bggc2, String *result)
{
  Geometry *gres = NULL;
  BG_geometry_collection bggc;
  BG_geometry_collection::Geometry_list &gv = bggc.get_geometries();
  gv.insert(gv.end(), bggc1.get_geometries().begin(), bggc1.get_geometries().end());
  gv.insert(gv.end(), bggc2.get_geometries().begin(), bggc2.get_geometries().end());
  bggc.set_srid(bggc1.get_srid());
 ...
 */
template <typename Coordsys>
Geometry *Item_func_spatial_operation::geocol_difference(const BG_geometry_collection &bggc1,
                                                         const BG_geometry_collection &bggc2, String *result)
{
  Geometry *gres = NULL;
  String *wkbres = NULL;
  ...
  for (BG_geometry_collection::Geometry_list::const_iterator i = gv1->begin(); i != gv1->end(); ++i)
  {
    ...
    */
    for (Rtree_index::const_query_iterator j = rtree.qbegin(bgi::intersects(box)); j != rtree.qend(); ++j)
    {
 ...
 */
template <typename Coordsys>
Geometry *Item_func_spatial_operation::geocol_symdifference(const BG_geometry_collection &bggc1,
                                                            const BG_geometry_collection &bggc2, String *result)
{
  Geometry *res = NULL;
  ...
  Var_resetter<op_type> var_reset(&spatial_op, op_symdifference);
  ...
  spatial_op = op_union;
  res = geometry_collection_set_operation<Coordsys>(diff12.get(), diff21.get(), result);
  if (diff12.get() == res)
  {
