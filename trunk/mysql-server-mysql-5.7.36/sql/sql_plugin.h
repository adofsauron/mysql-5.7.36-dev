...
{
 public:
  Sql_cmd_install_plugin(const LEX_STRING &comment, const LEX_STRING &ident) : m_comment(comment), m_ident(ident) {}

  virtual enum_sql_command sql_command_code() const { return SQLCOM_INSTALL_PLUGIN; }
...
{
 public:
  explicit Sql_cmd_uninstall_plugin(const LEX_STRING &comment) : m_comment(comment) {}

  virtual enum_sql_command sql_command_code() const { return SQLCOM_UNINSTALL_PLUGIN; }
...
extern bool plugin_register_dynamic_and_init_all(int *argc, char **argv, int init_flags);
...
extern bool plugin_foreach_with_mask(THD *thd, plugin_foreach_func **funcs, int type, uint state_mask, void *arg);
