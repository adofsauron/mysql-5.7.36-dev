      ...
      reset(false)
{
  ...
  my_snprintf(for_channel_str, sizeof(for_channel_str) - 1, " for channel '%s'", channel);
  my_snprintf(for_channel_uppercase_str, sizeof(for_channel_uppercase_str) - 1, " FOR CHANNEL '%s'", channel);
...
{
  return std::binary_search(ignore_server_ids->dynamic_ids.begin(), ignore_server_ids->dynamic_ids.end(), s_id);
}
...
bool Master_info::read_info(Rpl_info_handler *from)
{
  ...
  */
  ...
  if (from->prepare_info_for_read() || from->get_info(master_log_name, sizeof(master_log_name), (char *)""))
    DBUG_RETURN(true);

  lines = strtoul(master_log_name, &first_non_digit, 10);

  if (master_log_name[0] != '\0' && *first_non_digit == '\0' && lines >= LINES_IN_MASTER_INFO_WITH_SSL)
  {
  ...
  if (from->get_info(&temp_master_log_pos, (ulong)BIN_LOG_HEADER_SIZE) ||
      from->get_info(host, sizeof(host), (char *)0) || from->get_info(user, sizeof(user), (char *)"test") ||
      from->get_info(password, sizeof(password), (char *)0) || from->get_info((int *)&port, (int)MYSQL_PORT) ||
  ...
  */
  ...
  {
    if (from->get_info(&temp_ssl, 0) || from->get_info(ssl_ca, sizeof(ssl_ca), (char *)0) ||
...
bool Master_info::write_info(Rpl_info_handler *to)
{
  ...
  */
  if (to->prepare_info_for_write() || to->set_info((int)LINES_IN_MASTER_INFO) || to->set_info(master_log_name) ||
      to->set_info((ulong)master_log_pos) || to->set_info(host) || to->set_info(user) || to->set_info(password) ||
      to->set_info((int)port) || to->set_info((int)connect_retry) || to->set_info((int)ssl) || to->set_info(ssl_ca) ||
      to->set_info(ssl_capath) || to->set_info(ssl_cert) || to->set_info(ssl_cipher) || to->set_info(ssl_key) ||
      to->set_info((int)ssl_verify_server_cert) || to->set_info(heartbeat_period) || to->set_info(bind_addr) ||
      to->set_info(ignore_server_ids) || to->set_info(master_uuid) || to->set_info(retry_count) ||
      to->set_info(ssl_crl) || to->set_info(ssl_crlpath) || to->set_info((int)auto_position) || to->set_info(channel) ||
