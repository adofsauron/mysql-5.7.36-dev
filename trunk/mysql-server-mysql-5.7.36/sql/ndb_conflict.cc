...
*/
bool ExceptionsTableWriter::has_prefix_ci(const char *col_name, const char *prefix, CHARSET_INFO *cs)
{
...
/*
  ...
  without the suffix.
*/
bool ExceptionsTableWriter::has_suffix_ci(const char *col_name, const char *suffix, CHARSET_INFO *cs,
                                          char *col_name_real)
{
  uint col_len = strlen(col_name);
  ...
  if (col_len > suffix_len && my_strcasecmp(cs, col_name_endp - suffix_len, suffix) == 0)
  {
 ...
 */
bool ExceptionsTableWriter::find_column_name_ci(CHARSET_INFO *cs, const char *col_name,
                                                const NdbDictionary::Table *table, int *pos, int *key_pos)
{
...
bool ExceptionsTableWriter::check_mandatory_columns(const NdbDictionary::Table *exceptionsTable)
{
  ...
  if (/* server id */
      exceptionsTable->getColumn(0)->getType() == NDBCOL::Unsigned && exceptionsTable->getColumn(0)->getPrimaryKey() &&
      /* master_server_id */
      exceptionsTable->getColumn(1)->getType() == NDBCOL::Unsigned && exceptionsTable->getColumn(1)->getPrimaryKey() &&
      /* master_epoch */
      exceptionsTable->getColumn(2)->getType() == NDBCOL::Bigunsigned &&
      exceptionsTable->getColumn(2)->getPrimaryKey() &&
      /* count */
      exceptionsTable->getColumn(3)->getType() == NDBCOL::Unsigned && exceptionsTable->getColumn(3)->getPrimaryKey())
    DBUG_RETURN(true);
  else
    DBUG_RETURN(false);
}

bool ExceptionsTableWriter::check_pk_columns(const NdbDictionary::Table *mainTable,
                                             const NdbDictionary::Table *exceptionsTable, int &k)
{
  ...
  for (int i = k = 0; i < ncol && k < nkey; i++)
  {
    ...
    if (col->getPrimaryKey())
    {
      const NdbDictionary::Column *ex_col = exceptionsTable->getColumn(fixed_cols + k);
      if (!(ex_col != NULL && col->getType() == ex_col->getType() && col->getLength() == ex_col->getLength() &&
...
bool ExceptionsTableWriter::check_optional_columns(const NdbDictionary::Table *mainTable,
  ...
  for (i = xncol - 1; i >= 0; i--)
  {
    ...
    */
    ...
    if (has_prefix_ci(col_name, NDB_EXCEPTIONS_TABLE_COLUMN_PREFIX, cs))
    {
      has_prefix = true;
      m_extended = true;
      DBUG_PRINT("info", ("Exceptions table %s is extended with column %s", ex_tab_name, col_name));
    }
    ...
    if (my_strcasecmp(cs, col_name, NDB_EXCEPTIONS_TABLE_OP_TYPE) == 0)
    {
      ...
      {
        my_snprintf(error_details, error_details_len, "Table %s has incorrect type %u for NDB$OP_TYPE",
                    exceptionsTable->getName(), exceptionsTable->getColumn(i)->getType());
    ...
    if (my_strcasecmp(cs, col_name, NDB_EXCEPTIONS_TABLE_CONFLICT_CAUSE) == 0)
    {
      ...
      if (exceptionsTable->getColumn(i)->getType() != NDBCOL::Char &&
          exceptionsTable->getColumn(i)->getType() != NDBCOL::Unsigned)
      {
        my_snprintf(error_details, error_details_len, "Table %s has incorrect type %u for NDB$CFT_CAUSE",
                    exceptionsTable->getName(), exceptionsTable->getColumn(i)->getType());
        DBUG_PRINT("info", ("%s", error_details));
        ok = false;
        break;
    ...
    if (my_strcasecmp(cs, col_name, NDB_EXCEPTIONS_TABLE_ORIG_TRANSID) == 0)
    {
      ...
      {
        my_snprintf(error_details, error_details_len, "Table %s has incorrect type %u for NDB$ORIG_TRANSID",
                    exceptionsTable->getName(), exceptionsTable->getColumn(i)->getType());
        DBUG_PRINT("info", ("%s", error_details));
        ok = false;
        break;
    ...
    */
    if (i >= fixed_cols)
    {
      ...
      if (has_suffix_ci(col_name, NDB_EXCEPTIONS_TABLE_COLUMN_OLD_SUFFIX, cs, col_name_real))
      {
        DBUG_PRINT("info", ("Found reference to old column %s", col_name));
        column_version = OLD;
      }
      else if (has_suffix_ci(col_name, NDB_EXCEPTIONS_TABLE_COLUMN_NEW_SUFFIX, cs, col_name_real))
      {
      ...
      if (!find_column_name_ci(mcs, col_name_real, mainTable, &match, &match_k))
      {
        if (!strcmp(col_name, col_name_real))
        {
          ...
          */
          ...
          {
            DBUG_PRINT("info", ("Column %s in main table %s has an unfortunate name", col_name, mainTable->getName()));
          }
        }
      }
      /*
      ...
      */
      ...
      if (match == -1)
      {
        ...
        */
        ...
        {
          my_snprintf(error_details, error_details_len,
                      "Extra column %s in table %s is not nullable and doesn't have a default value", col->getName(),
                      exceptionsTable->getName());
          DBUG_PRINT("info", ("%s", error_details));
          ok = false;
          break;
        }
        my_snprintf(error_details, error_details_len, "Column %s in extension table %s not found in %s", col->getName(),
                    exceptionsTable->getName(), mainTable->getName());
        DBUG_PRINT("info", ("%s", error_details));
        my_snprintf(msg_buf, msg_buf_len,
      ...
      {
        DBUG_PRINT("info", ("Comparing column %s in exceptions table with column %s in main table", col->getName(),
                            mcol->getName()));
        ...
        if (!mcol->getPrimaryKey())
        {
          ...
          */
          if (col->getNullable() || col->getDefaultValue() != NULL)
          {
            DBUG_PRINT("info", ("Mapping column %s %s(%i) to %s(%i)", col->getName(), mainTable->getName(), match,
          ...
          {
            my_snprintf(error_details, error_details_len,
                        "Data column %s in table %s is not nullable and doesn't have a default value", col->getName(),
                        exceptionsTable->getName());
            DBUG_PRINT("info", ("%s", error_details));
            ok = false;
            break;
        ...
        else
        {
          ...
          {
            my_snprintf(error_details, error_details_len,
                        "Old or new values of primary key columns cannot be referenced since primary keys cannot be "
                        "updated, column %s in table %s",
                        col->getName(), exceptionsTable->getName());
            DBUG_PRINT("info", ("%s", error_details));
            ok = false;
            break;
          ...
          if (col->getNullable() == mcol->getNullable())
          {
            ...
            */
            ...
            {
              my_snprintf(error_details, error_details_len, "Multiple references to the same key column %s in table %s",
                          col->getName(), exceptionsTable->getName());
              DBUG_PRINT("info", ("%s", error_details));
              ok = false;
              break;
          ...
          else if (column_version == DEFAULT)
          {
            ...
            */
            my_snprintf(error_details, error_details_len, "Pk column %s not declared not null in both tables",
                        col->getName());
            DBUG_PRINT("info", ("%s", error_details));
            ok = false;
            break;
      ...
      else
      {
        ...
        */
        my_snprintf(error_details, error_details_len,
                    ...
                    col->getName(), mcol->getName(), mainTable->getName(), col->getType(), mcol->getType());
        DBUG_PRINT("info", ("%s", error_details));
        ok = false;
        break;
      }
...
int ExceptionsTableWriter::init(const NdbDictionary::Table *mainTable, const NdbDictionary::Table *exceptionsTable,
                                char *msg_buf, uint msg_buf_len, const char **msg)
{
  ...
  *msg_buf = '\0';
  ...
  DBUG_PRINT("info", ("Checking definition of exceptions table %s", ex_tab_name));
  /*
    Check that the table have the corrct number of columns
    and the mandatory columns.
   */

  bool ok = exceptionsTable->getNoOfColumns() >= fixed_cols && exceptionsTable->getNoOfPrimaryKeys() == 4 &&
  ...
  if (ok)
  {
    ...
    /*
      ...
      columns.
     */
    ok = check_pk_columns(mainTable, exceptionsTable, k) &&
         check_optional_columns(mainTable, exceptionsTable, msg_buf, msg_buf_len, msg, k, error_details,
                                error_details_len);
    if (ok)
    {
...
int ExceptionsTableWriter::writeRow(NdbTransaction *trans, const NdbRecord *keyRecord, const NdbRecord *dataRecord,
                                    uint32 server_id, uint32 master_server_id, uint64 master_epoch,
                                    const uchar *oldRowPtr, const uchar *newRowPtr, enum_conflicting_op_type op_type,
                                    enum_conflict_cause conflict_cause, uint64 orig_transid, const MY_BITMAP *write_set,
                                    NdbError &err)
{
  DBUG_ENTER("ExceptionsTableWriter::writeRow");
  DBUG_PRINT("info", ("op_type(pos):%u(%u), conflict_cause(pos):%u(%u), orig_transid:%llu(%u)", op_type, m_op_type_pos,
                      conflict_cause, m_conflict_cause_pos, orig_transid, m_orig_transid_pos));
  ...
  do
  {
      ...
      if (m_extended)
      {
        if (m_op_type_pos)
        {
          if (m_ex_tab->getColumn(m_op_type_pos)->getType() == NDBCOL::Char)
          {
            ...
            if (ex_op->setValue((Uint32)m_op_type_pos, (const char *)&(op_type_val)))
            {
              err = ex_op->getNdbError();
              break;
            }
        ...
        {
          if (m_ex_tab->getColumn(m_conflict_cause_pos)->getType() == NDBCOL::Char)
          {
            /* Defined as ENUM */
            char conflict_cause_val = (char)conflict_cause;
            if (ex_op->setValue((Uint32)m_conflict_cause_pos, (const char *)&(conflict_cause_val)))
            {
              err = ex_op->getNdbError();
              break;
            }
        ...
        if (m_orig_transid_pos != 0)
        {
          ...
          if (orig_transid == Ndb_binlog_extra_row_info::InvalidTransactionId && col->getNullable())
          {
          ...
          else
          {
            ...
            if (ex_op->setValue((Uint32)m_orig_transid_pos, (const char *)&(orig_transid_val)))
            {
              err = ex_op->getNdbError();
              break;
            }
    ...
    /* copy primary keys */
    {
      ...
      for (k = 0; k < nkey; k++)
      {
        ...
        {
          const uchar *data =
              (const uchar *)NdbDictionary::getValuePtr(keyRecord, (const char *)rowPtr, m_key_attrids[k]);
    ...
    if (m_extended)
    {
      ...
      for (i = 0; i < xncol; i++)
      {
        ...
        DBUG_PRINT("info",
                   ("Checking column %s(%i)%s", col->getName(), i, (default_value) ? ", has default value" : ""));
        assert(rowPtr != NULL);
        if (m_data_pos[i] != -1)
        {
          ...
          if (row_vPtr == NULL || (m_col_nullable[m_data_pos[i]] &&
                                   NdbDictionary::isNull(dataRecord, (const char *)row_vPtr, m_data_pos[i])))
          {
          ...
          else if (write_set != NULL && bitmap_is_set(write_set, m_data_pos[i]))
          {
            ...
            const uchar *data =
                (const uchar *)NdbDictionary::getValuePtr(dataRecord, (const char *)row_vPtr, m_data_pos[i]);
...
{
  assert(((trans_dependency_tracker == NULL) && (trans_conflict_apply_state == SAS_NORMAL)) ||
         ((trans_dependency_tracker != NULL) && (trans_conflict_apply_state == SAS_TRACK_TRANS_DEPENDENCIES)));
  ...
  {
    DBUG_PRINT("info", ("Max replicated epoch increases from %llu to %llu", max_rep_epoch, current_max_rep_epoch));
...

  {
    ...
    if (total_conflicts > 0)
    {
       ...
       */
      DBUG_PRINT("info", ("Last conflicted epoch increases from %llu to %llu", last_conflicted_epoch, epoch));
      hadConflict = true;
    }
    else
    {
      ...
      Uint32 current_reflect_op_apply_count = current_reflect_op_prepare_count - current_reflect_op_discard_count;
      if (current_reflect_op_apply_count > 0 || current_refresh_op_count > 0)
      {
        DBUG_PRINT("info", ("Reflected (%u) or Refresh (%u) operations applied this "
                            ...
                            current_reflect_op_apply_count, current_refresh_op_count, last_conflicted_epoch, epoch));
        hadConflict = true;
      }
    }
  ...
  DBUG_EXECUTE_IF("ndb_slave_fail_marking_epoch_committed", {
    fprintf(stderr,
            ...
            current_master_server_epoch >> 32, current_master_server_epoch & 0xffffffff, current_master_server_epoch);
    current_master_server_epoch_committed = false;
  });
}
...
*/
bool st_ndb_slave_state::verifyNextEpoch(Uint64 next_epoch, Uint32 master_server_id) const
{
  ...
  */
  ...
  DBUG_PRINT("info", ("ndb_apply_status write from upstream master."
                      ...
                      current_master_server_epoch >> 32, current_master_server_epoch & 0xffffffff,
                      current_master_server_epoch, current_master_server_epoch_committed));
  DBUG_PRINT("info", ("mi_slave_run_id=%u, ndb_slave_state_run_id=%u", ndb_mi_get_slave_run_id(), sql_run_id));
  DBUG_PRINT("info", ("First epoch since slave start : %u", first_epoch_since_slave_start));
    ...
    */
    if (next_epoch < current_master_server_epoch)
    {
      sql_print_warning(
          ...
          next_epoch >> 32, next_epoch & 0xffffffff, next_epoch, master_server_id, current_master_server_epoch >> 32,
          current_master_server_epoch & 0xffffffff, current_master_server_epoch, ndb_mi_get_group_master_log_name(),
    ...
    else if (next_epoch == current_master_server_epoch)
    {
      ...
      */
      if (current_master_server_epoch_committed)
      {
        ...
        sql_print_error(
            "NDB Slave : SQL thread stopped as attempted "
            "to reapply already committed epoch %llu/%llu (%llu) "
            "from server id %u.  "
            "Group Master Log : %s  Group Master Log Pos : %llu.",
            current_master_server_epoch >> 32, current_master_server_epoch & 0xffffffff, current_master_server_epoch,
            master_server_id, ndb_mi_get_group_master_log_name(), ndb_mi_get_group_master_log_pos());
        /* Stop the slave */
        DBUG_RETURN(false);
      }
      else
      ...
      if (!current_master_server_epoch_committed)
      {
        ...
        */
        sql_print_error(
            ...
            "Group Master Log : %s  Group Master Log Pos : %llu.",
            next_epoch >> 32, next_epoch & 0xffffffff, next_epoch, current_master_server_epoch >> 32,
            current_master_server_epoch & 0xffffffff, current_master_server_epoch, master_server_id,
...
*/
int st_ndb_slave_state::atApplyStatusWrite(Uint32 master_server_id, Uint32 row_server_id, Uint64 row_epoch,
                                           bool is_row_server_id_local)
{
  ...
  else if (is_row_server_id_local)
  {
    DBUG_PRINT("info", ("Recording application of local server %u epoch %llu "
                        ...
                        (row_epoch > current_max_rep_epoch) ? " new highest." : " older than previously applied"));
...
bool st_ndb_slave_state::checkSlaveConflictRoleChange(enum_slave_conflict_role old_role,
                                                      enum_slave_conflict_role new_role, const char **failure_cause)
{
  ...
  *failure_cause = "Internal error";
  ...
  switch (old_role)
  {
    ...
    case SCR_SECONDARY:
      ...
      break;
    case SCR_PASS:
      bad_transition = ((new_role == SCR_PRIMARY) || (new_role == SCR_SECONDARY));
      break;
    default:
      assert(false);
      return false;
...
void st_ndb_slave_state::atEndTransConflictHandling()
{
  ...
  {
    current_trans_in_conflict_count = trans_dependency_tracker->get_conflict_count();
    trans_dependency_tracker = NULL;
...
*/
int st_ndb_slave_state::atPrepareConflictDetection(const NdbDictionary::Table *table, const NdbRecord *key_rec,
  ...
  switch (trans_conflict_apply_state)
  {
    ...
    case SAS_TRACK_TRANS_DEPENDENCIES:
    {
      ...
      */
      ...
      int res = trans_dependency_tracker->track_operation(table, key_rec, row_data, transaction_id);
      if (res != 0)
      {
    ...
    case SAS_APPLY_TRANS_DEPENDENCIES:
    {
      ...
      */
      ...
      {
        DBUG_PRINT("info", ("Event for transaction %llu is conflicting.  Handling.", transaction_id));
      ...
      /*
         ...
         stage.
      */
      DBUG_PRINT("info", ("Event for transaction %llu is OK, applying", transaction_id));
      break;
    }
  }
  DBUG_RETURN(0);
...
*/
static int row_conflict_fn_old(NDB_CONFLICT_FN_SHARE *cfn_share, enum_conflicting_op_type op_type,
                               const NdbRecord *data_record, const uchar *old_data, const uchar *new_data,
                               const MY_BITMAP *bi_cols, const MY_BITMAP *ai_cols, NdbInterpretedCode *code)
{
  ...
  const uchar *field_ptr =
      (const uchar *)NdbDictionary::getValuePtr(data_record, (const char *)old_data, cfn_share->m_resolve_column);
  ...
  DBUG_PRINT("info", ("Adding interpreted filter, existing value must eq event old value"));
  /*
...
static int row_conflict_fn_max_update_only(NDB_CONFLICT_FN_SHARE *cfn_share, enum_conflicting_op_type op_type,
                                           const NdbRecord *data_record, const uchar *old_data, const uchar *new_data,
                                           const MY_BITMAP *bi_cols, const MY_BITMAP *ai_cols, NdbInterpretedCode *code)
{
  ...
  uint32 resolve_size = cfn_share->m_resolve_size;
  const uchar *field_ptr =
      (const uchar *)NdbDictionary::getValuePtr(data_record, (const char *)new_data, cfn_share->m_resolve_column);
  ...
  DBUG_PRINT("info", ("Adding interpreted filter, existing value must be lt event new"));
  /*
  ...
  };
  {
    ...
    else
    {
      ...
      DBUG_PRINT("info", ("  new_value_64: %llu", (unsigned long long)new_value_64));
    }
  }
  /*
...
*/
static int row_conflict_fn_max(NDB_CONFLICT_FN_SHARE *cfn_share, enum_conflicting_op_type op_type,
                               const NdbRecord *data_record, const uchar *old_data, const uchar *new_data,
                               const MY_BITMAP *bi_cols, const MY_BITMAP *ai_cols, NdbInterpretedCode *code)
{
  switch (op_type)
  {
    case WRITE_ROW:
      ...
      return 1;
    case UPDATE_ROW:
      return row_conflict_fn_max_update_only(cfn_share, op_type, data_record, old_data, new_data, bi_cols, ai_cols,
                                             code);
    case DELETE_ROW:
       ...
       * Use OLD instead
       */
      return row_conflict_fn_old(cfn_share, op_type, data_record, old_data, new_data, bi_cols, ai_cols, code);
    default:
      abort();
      return 1;
  }
...
static int row_conflict_fn_max_del_win(NDB_CONFLICT_FN_SHARE *cfn_share, enum_conflicting_op_type op_type,
                                       const NdbRecord *data_record, const uchar *old_data, const uchar *new_data,
                                       const MY_BITMAP *bi_cols, const MY_BITMAP *ai_cols, NdbInterpretedCode *code)
{
  switch (op_type)
...
static int row_conflict_fn_epoch(NDB_CONFLICT_FN_SHARE *cfn_share, enum_conflicting_op_type op_type,
                                 const NdbRecord *data_record, const uchar *old_data, const uchar *new_data,
                                 const MY_BITMAP *bi_cols, const MY_BITMAP *ai_cols, NdbInterpretedCode *code)
{
  ...
  switch (op_type)
  {
    ...
    {
      const uint label_0 = 0;
      const Uint32 RegAuthor = 1, RegZero = 2, RegMaxRepEpoch = 1, RegRowEpoch = 2;
      int r;
...
static int row_conflict_fn_epoch2_primary(NDB_CONFLICT_FN_SHARE *cfn_share, enum_conflicting_op_type op_type,
                                          const NdbRecord *data_record, const uchar *old_data, const uchar *new_data,
                                          const MY_BITMAP *bi_cols, const MY_BITMAP *ai_cols, NdbInterpretedCode *code)
{
  ...
  DBUG_RETURN(row_conflict_fn_epoch(cfn_share, op_type, data_record, old_data, new_data, bi_cols, ai_cols, code));
}

static int row_conflict_fn_epoch2_secondary(NDB_CONFLICT_FN_SHARE *cfn_share, enum_conflicting_op_type op_type,
                                            const NdbRecord *data_record, const uchar *old_data, const uchar *new_data,
...
static int row_conflict_fn_epoch2(NDB_CONFLICT_FN_SHARE *cfn_share, enum_conflicting_op_type op_type,
                                  const NdbRecord *data_record, const uchar *old_data, const uchar *new_data,
                                  const MY_BITMAP *bi_cols, const MY_BITMAP *ai_cols, NdbInterpretedCode *code)
{
  ...
  switch (opt_ndb_slave_conflict_role)
  {
    ...
    case SCR_PRIMARY:
      DBUG_RETURN(
          row_conflict_fn_epoch2_primary(cfn_share, op_type, data_record, old_data, new_data, bi_cols, ai_cols, code));
    case SCR_SECONDARY:
      DBUG_RETURN(row_conflict_fn_epoch2_secondary(cfn_share, op_type, data_record, old_data, new_data, bi_cols,
                                                   ai_cols, code));
    case SCR_PASS:
      /* Do nothing */
      DBUG_RETURN(0);
...
static const st_conflict_fn_def conflict_fns[] = {
    {"NDB$MAX_DELETE_WIN", CFT_NDB_MAX_DEL_WIN, &resolve_col_args[0], row_conflict_fn_max_del_win, 0},
    {"NDB$MAX", CFT_NDB_MAX, &resolve_col_args[0], row_conflict_fn_max, 0},
    {"NDB$OLD", CFT_NDB_OLD, &resolve_col_args[0], row_conflict_fn_old, 0},
    {"NDB$EPOCH2_TRANS", CFT_NDB_EPOCH2_TRANS, &epoch_fn_args[0], row_conflict_fn_epoch2,
     CF_REFLECT_SEC_OPS | CF_USE_ROLE_VAR | CF_TRANSACTIONAL},
    {"NDB$EPOCH2", CFT_NDB_EPOCH2, &epoch_fn_args[0], row_conflict_fn_epoch2, CF_REFLECT_SEC_OPS | CF_USE_ROLE_VAR},
    {"NDB$EPOCH_TRANS", CFT_NDB_EPOCH_TRANS, &epoch_fn_args[0], row_conflict_fn_epoch, CF_TRANSACTIONAL},
    {"NDB$EPOCH", CFT_NDB_EPOCH, &epoch_fn_args[0], row_conflict_fn_epoch, 0}};

static unsigned n_conflict_fns = sizeof(conflict_fns) / sizeof(struct st_conflict_fn_def);

int parse_conflict_fn_spec(const char *conflict_fn_spec, const st_conflict_fn_def **conflict_fn,
                           st_conflict_fn_arg *args, Uint32 *max_args, char *msg, uint msg_len)
{
    ...
    for (;;)
    {
      ...
      switch (type)
      {
        case CFAT_COLUMN_NAME:
        {
          ...
          memcpy(dest, start_arg, (len < (uint)NAME_CHAR_LEN ? len : NAME_CHAR_LEN));
          dest[len] = '\0';
          break;
        }
  ...
  /* parse error */
  my_snprintf(msg, msg_len, "%s, %s at '%s'", conflict_fn_spec, error_str, ptr);
...
static int slave_set_resolve_fn(Ndb *ndb, NDB_CONFLICT_FN_SHARE **ppcfn_share, const char *dbName, const char *tabName,
                                const NDBTAB *ndbtab, uint field_index, uint resolve_col_sz,
                                const st_conflict_fn_def *conflict_fn, uint8 flags)
{
  ...
  if (cfn_share == NULL)
  {
    *ppcfn_share = cfn_share = (NDB_CONFLICT_FN_SHARE *)my_malloc(PSI_INSTRUMENT_ME, sizeof(NDB_CONFLICT_FN_SHARE),
  ...
  for (int tries = 2; tries-- > 0; ex_suffix = (tries == 1) ? (const char *)NDB_EXCEPTIONS_TABLE_SUFFIX_LOWER : NullS)
  {
    ...
    if (ex_tab)
    {
      ...
      const char *msg = NULL;
      if (cfn_share->m_ex_tab_writer.init(ndbtab, ex_tab, msgBuf, sizeof(msgBuf), &msg) == 0)
      {
        /* Ok */
        ...
        {
          sql_print_information("NDB Slave: Table %s.%s logging exceptions to %s.%s", dbName, tabName, dbName,
                                ex_tab_name);
        }
      }
      else
...
bool is_exceptions_table(const char *table_name)
{
  ...
  if (len > suffixlen &&
      (strcmp(table_name + len - suffixlen,
              lower_case_table_names ? NDB_EXCEPTIONS_TABLE_SUFFIX_LOWER : NDB_EXCEPTIONS_TABLE_SUFFIX) == 0))
  {
    return true;
  }
  return false;
...
int setup_conflict_fn(Ndb *ndb, NDB_CONFLICT_FN_SHARE **ppcfn_share, const char *dbName, const char *tabName,
                      bool tableUsesBlobs, bool tableBinlogUseUpdate, const NDBTAB *ndbtab, char *msg, uint msg_len,
                      const st_conflict_fn_def *conflict_fn, const st_conflict_fn_arg *args, const Uint32 num_args)
{
  DBUG_ENTER("setup_conflict_fn");

  if (is_exceptions_table(tabName))
  {
    my_snprintf(msg, msg_len, "Ndb Slave: Table %s.%s is exceptions table: not using conflict function %s", dbName,
  ...
  switch (conflict_fn->type)
  {
    ...
    case CFT_NDB_MAX_DEL_WIN:
    {
      if (num_args != 1)
      {
        my_snprintf(msg, msg_len, "Incorrect arguments to conflict function");
        DBUG_PRINT("info", ("%s", msg));
        DBUG_RETURN(-1);
      }
      ...
      for (int j = 0; j < ndbtab->getNoOfColumns(); j++)
      {
        ...
        if (strncmp(colName, resolveColName, resolveColNameLen) == 0 && colName[resolveColNameLen] == '\0')
        {
          colNum = j;
          break;
        }
      ...
      if (colNum == -1)
      {
        my_snprintf(msg, msg_len, "Could not find resolve column %s.", resolveColName);
        DBUG_PRINT("info", ("%s", msg));
        DBUG_RETURN(-1);
      }
      ...
      if (resolve_col_sz == 0)
      {
        ...
        my_snprintf(msg, msg_len, "Column '%s' has wrong datatype", resolveColName);
        DBUG_PRINT("info", ("%s", msg));
        DBUG_RETURN(-1);
      }

      if (slave_set_resolve_fn(ndb, ppcfn_share, dbName, tabName, ndbtab, colNum, resolve_col_sz, conflict_fn,
                               CFF_NONE))
      {
        my_snprintf(msg, msg_len, "Unable to setup conflict resolution using column '%s'", resolveColName);
        DBUG_PRINT("info", ("%s", msg));
        DBUG_RETURN(-1);
      }

      /* Success, update message */
      my_snprintf(msg, msg_len, "NDB Slave: Table %s.%s using conflict_fn %s on attribute %s.", dbName, tabName,
                  conflict_fn->name, resolveColName);
      break;
    }
    case CFT_NDB_EPOCH2:
    ...
    case CFT_NDB_EPOCH_TRANS:
    {
      if (num_args > 1)
      {
        my_snprintf(msg, msg_len, "Too many arguments to conflict function");
        DBUG_PRINT("info", ("%s", msg));
        DBUG_RETURN(-1);
      }
      ...
      if (tableUsesBlobs)
      {
        my_snprintf(msg, msg_len, "Table has Blob column(s), not suitable for %s.", conflict_fn->name);
        DBUG_PRINT("info", ("%s", msg));
        DBUG_RETURN(-1);
      }
      ...
      if (ndbtab->getExtraRowGciBits() == 0)
        sql_print_information("NDB Slave: Table %s.%s : %s, low epoch resolution", dbName, tabName, conflict_fn->name);
      ...
      {
        my_snprintf(msg, msg_len, "unable to setup conflict resolution");
        DBUG_PRINT("info", ("%s", msg));
        DBUG_RETURN(-1);
      }
      /* Success, update message */
      my_snprintf(msg, msg_len, "NDB Slave: Table %s.%s using conflict_fn %s.", dbName, tabName, conflict_fn->name);
...
SHOW_VAR ndb_status_conflict_variables[] = {
    ...
    {"trans_conflict_commit_count", (char *)&g_ndb_slave_state.trans_conflict_commit_count, SHOW_LONGLONG,
     SHOW_SCOPE_GLOBAL},
...
int show_ndb_conflict_status_vars(THD *thd, struct st_mysql_show_var *var, char *buff)
{
