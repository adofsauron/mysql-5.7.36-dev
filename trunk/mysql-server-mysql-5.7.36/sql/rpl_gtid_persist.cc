...
void Gtid_table_access_context::before_open(THD *thd)
{
  ...
  */
  m_flags = (MYSQL_OPEN_IGNORE_GLOBAL_READ_LOCK | MYSQL_LOCK_IGNORE_GLOBAL_READ_ONLY | MYSQL_OPEN_IGNORE_FLUSH |
...
bool Gtid_table_access_context::init(THD **thd, TABLE **table, bool is_write)
{
  ...
  bool ret = this->open_table(*thd, DB_NAME, TABLE_NAME, Gtid_table_persistor::number_fields,
                              m_is_write ? TL_WRITE : TL_READ, table, &m_backup);

  DBUG_RETURN(ret);
}

bool Gtid_table_access_context::deinit(THD *thd, TABLE *table, bool error, bool need_commit)
{
  ...
  */
  if (err)
  {
    my_printf_error(ER_ERROR_DURING_FLUSH_LOGS, ER(ER_ERROR_DURING_FLUSH_LOGS), MYF(ME_FATALERROR), err);
...
int Gtid_table_persistor::fill_fields(Field **fields, const char *sid, rpl_gno gno_start, rpl_gno gno_end)
{
...
int Gtid_table_persistor::write_row(TABLE *table, const char *sid, rpl_gno gno_start, rpl_gno gno_end)
{
  ...
  if (DBUG_EVALUATE_IF("simulate_err_on_write_gtid_into_table", (error = -1), error))
  {
    if (error == HA_ERR_FOUND_DUPP_KEY)
    {
      /* Ignore the duplicate key error, log a warning for it. */
      sql_print_warning(
          "The transaction owned GTID is already in "
...
int Gtid_table_persistor::update_row(TABLE *table, const char *sid, rpl_gno gno_start, rpl_gno new_gno_end)
{
  ...
  if ((error = table->file->ha_index_read_map(table->record[0], user_key, HA_WHOLE_KEY, HA_READ_KEY_EXACT)))
  {
  ...
  if (DBUG_EVALUATE_IF("simulate_error_on_compress_gtid_table", (error = -1), error))
  {
    table->file->print_error(error, MYF(0));
    /*
...
int Gtid_table_persistor::save(THD *thd, const Gtid *gtid)
{
  ...
  if (error == 0 && gtid_executed_compression_period != 0)
  {
    ...
    if (count == gtid_executed_compression_period || DBUG_EVALUATE_IF("compress_gtid_table", 1, 0))
    {
...
int Gtid_table_persistor::compress_in_single_transaction(THD *thd, bool &is_complete)
{
...
int Gtid_table_persistor::compress_first_consecutive_range(TABLE *table, bool &is_complete)
{
...
void Gtid_table_persistor::get_gtid_interval(TABLE *table, string &sid, rpl_gno &gno_start, rpl_gno &gno_end)
{
...
int Gtid_table_persistor::delete_all(TABLE *table)
{
  ...
  */
  while (!(err = table->file->ha_rnd_next(table->record[0])))
  {
    ...
    if (DBUG_EVALUATE_IF("simulate_error_on_delete_gtid_from_table", (err = -1), err))
    {
      table->file->print_error(err, MYF(0));
      sql_print_error(
          "Failed to delete the row: '%s' from the gtid_executed "
          "table.",
  ...
  */
  ...
  for (;;)
  {
    ...
    if (terminate_compress_thread)
      break;
    THD_ENTER_COND(thd, &COND_compress_gtid_table, &LOCK_compress_gtid_table, &stage_suspending, NULL);
...
void create_compress_gtid_table_thread()
{
  ...
  {
    sql_print_error(
        "Failed to compress the gtid_executed table, because "
  ...
  if (DBUG_EVALUATE_IF("simulate_create_compress_thread_failure", error = 1, 0) ||
#ifndef _WIN32
      (error = pthread_attr_setscope(&attr, PTHREAD_SCOPE_SYSTEM)) ||
#endif
      (error = mysql_thread_create(key_thread_compress_gtid_table, &compress_thread_id, &attr, compress_gtid_table,
                                   (void *)thd)))
  {
    sql_print_error(
        "Can not create thread to compress gtid_executed table "
        "(errno= %d)",
        error);
...
void terminate_compress_gtid_table_thread()
{
  ...
  if (error != 0)
    sql_print_warning(
        "Could not join gtid_executed table compression thread. "
        "error:%d",
        error);
