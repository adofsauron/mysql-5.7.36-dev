...
static bool create_string(THD *thd, String *buf, enum_sp_type sp_type, const char *db, size_t dblen, const char *name,
                          size_t namelen, const char *params, size_t paramslen, const char *returns, size_t returnslen,
                          const char *body, size_t bodylen, st_sp_chistics *chistics, const LEX_CSTRING &definer_user,
                          const LEX_CSTRING &definer_host, sql_mode_t sql_mode);

static int db_load_routine(THD *thd, enum_sp_type type, sp_name *name, sp_head **sphp, sql_mode_t sql_mode,
                           const char *params, const char *returns, const char *body, st_sp_chistics &chistics,
                           const char *definer, longlong created, longlong modified,
                           Stored_program_creation_ctx *creation_ctx);

static const TABLE_FIELD_TYPE proc_table_fields[MYSQL_PROC_FIELD_COUNT] = {
    {{C_STRING_WITH_LEN("db")}, {C_STRING_WITH_LEN("char(64)")}, {C_STRING_WITH_LEN("utf8")}},
    {{C_STRING_WITH_LEN("name")}, {C_STRING_WITH_LEN("char(64)")}, {C_STRING_WITH_LEN("utf8")}},
    {{C_STRING_WITH_LEN("type")}, {C_STRING_WITH_LEN("enum('FUNCTION','PROCEDURE')")}, {NULL, 0}},
    {{C_STRING_WITH_LEN("specific_name")}, {C_STRING_WITH_LEN("char(64)")}, {C_STRING_WITH_LEN("utf8")}},
    {{C_STRING_WITH_LEN("language")}, {C_STRING_WITH_LEN("enum('SQL')")}, {NULL, 0}},
    {{C_STRING_WITH_LEN("sql_data_access")},
     {C_STRING_WITH_LEN("enum('CONTAINS_SQL','NO_SQL','READS_SQL_DATA','MODIFIES_SQL_DATA')")},
     {NULL, 0}},
    {{C_STRING_WITH_LEN("is_deterministic")}, {C_STRING_WITH_LEN("enum('YES','NO')")}, {NULL, 0}},
    {{C_STRING_WITH_LEN("security_type")}, {C_STRING_WITH_LEN("enum('INVOKER','DEFINER')")}, {NULL, 0}},
    {{C_STRING_WITH_LEN("param_list")}, {C_STRING_WITH_LEN("blob")}, {NULL, 0}},

    {{C_STRING_WITH_LEN("returns")}, {C_STRING_WITH_LEN("longblob")}, {NULL, 0}},
    {{C_STRING_WITH_LEN("body")}, {C_STRING_WITH_LEN("longblob")}, {NULL, 0}},
    {{C_STRING_WITH_LEN("definer")}, {C_STRING_WITH_LEN("char(93)")}, {C_STRING_WITH_LEN("utf8")}},
    {{C_STRING_WITH_LEN("created")}, {C_STRING_WITH_LEN("timestamp")}, {NULL, 0}},
    {{C_STRING_WITH_LEN("modified")}, {C_STRING_WITH_LEN("timestamp")}, {NULL, 0}},
    {{C_STRING_WITH_LEN("sql_mode")},
     ...
     {NULL, 0}},
    {{C_STRING_WITH_LEN("comment")}, {C_STRING_WITH_LEN("text")}, {C_STRING_WITH_LEN("utf8")}},
    {{C_STRING_WITH_LEN("character_set_client")}, {C_STRING_WITH_LEN("char(32)")}, {C_STRING_WITH_LEN("utf8")}},
    {{C_STRING_WITH_LEN("collation_connection")}, {C_STRING_WITH_LEN("char(32)")}, {C_STRING_WITH_LEN("utf8")}},
    {{C_STRING_WITH_LEN("db_collation")}, {C_STRING_WITH_LEN("char(32)")}, {C_STRING_WITH_LEN("utf8")}},
    {{C_STRING_WITH_LEN("body_utf8")}, {C_STRING_WITH_LEN("longblob")}, {NULL, 0}}};

static const TABLE_FIELD_DEF proc_table_def = {MYSQL_PROC_FIELD_COUNT, proc_table_fields};
...
Stored_routine_creation_ctx *Stored_routine_creation_ctx::load_from_db(THD *thd, const sp_name *name, TABLE *proc_tbl)
{
  ...
  if (load_charset(thd->mem_root, proc_tbl->field[MYSQL_PROC_FIELD_CHARACTER_SET_CLIENT],
  ...
  if (load_collation(thd->mem_root, proc_tbl->field[MYSQL_PROC_FIELD_COLLATION_CONNECTION],
  ...
  if (load_collation(thd->mem_root, proc_tbl->field[MYSQL_PROC_FIELD_DB_COLLATION], NULL, &db_cl))
  {
    sql_print_warning(
        "Stored routine '%s'.'%s': invalid value "
  ...
  {
    push_warning_printf(thd, Sql_condition::SL_WARNING, ER_SR_INVALID_CREATION_CTX, ER(ER_SR_INVALID_CREATION_CTX),
                        db_name, sr_name);
  }
...
my_bool Proc_table_intact::check_proc_table(TABLE *table)
{
  ...
  table->field[MYSQL_PROC_FIELD_DEFINER]->field_length =
      (USERNAME_CHAR_LENGTH + HOSTNAME_LENGTH + 1) * table->field[MYSQL_PROC_FIELD_DEFINER]->charset()->mbmaxlen;
...
static int db_find_routine_aux(THD *thd, enum_sp_type type, sp_name *name, TABLE *table)
{
  ...
  DBUG_PRINT("enter", ("type: %d  name: %.*s", type, (int)name->m_name.length, name->m_name.str));
...
static int db_find_routine(THD *thd, enum_sp_type type, sp_name *name, sp_head **sphp)
{
  ...
  *sphp = 0;  // In case of errors
  ...
  ret = db_load_routine(thd, type, name, sphp, sql_mode, params, returns, body, chistics, definer, created, modified,
                        creation_ctx);
done:
  /*
...
class Silence_deprecated_warning : public Internal_error_handler
{
 public:
  ...
  {
    if (sql_errno == ER_WARN_DEPRECATED_SYNTAX && (*level) == Sql_condition::SL_WARNING)
      return true;
...
static sp_head *sp_compile(THD *thd, String *defstr, sql_mode_t sql_mode, Stored_program_creation_ctx *creation_ctx)
{
  ...
  if (sp != NULL)
    sp->m_sp_share = MYSQL_GET_SP_SHARE(sp->m_type, sp->m_db.str, sp->m_db.length, sp->m_name.str, sp->m_name.length);
#endif
  return sp;
}
                           ...
                           Stored_program_creation_ctx *creation_ctx)
{
  ...
  parse_user(definer, strlen(definer), definer_user_name_holder, &definer_user_name.length, definer_host_name_holder,
  ...
  if (!create_string(thd, &defstr, type, NULL, 0, name->m_name.str, name->m_name.length, params, strlen(params),
                     returns, strlen(returns), body, strlen(body), &chistics, definer_user_name, definer_host_name,
                     sql_mode))
  {
  ...
  */
...

  {
    ...
    */
    ...
    if (cur_db_changed && mysql_change_db(thd, to_lex_cstring(saved_cur_db_name), true))
    {
      ret = SP_INTERNAL_ERROR;
      goto end;
    }
...
bool sp_create_routine(THD *thd, sp_head *sp)
{
  ...
  MDL_key::enum_mdl_namespace mdl_type = (sp->m_type == SP_TYPE_FUNCTION) ? MDL_key::FUNCTION : MDL_key::PROCEDURE;
  ...
  DBUG_PRINT("enter", ("type: %d  name: %.*s", sp->m_type, (int)sp->m_name.length, sp->m_name.str));
  String retstr(64);
  retstr.set_charset(system_charset_info);

  assert(sp->m_type == SP_TYPE_PROCEDURE || sp->m_type == SP_TYPE_FUNCTION);

  /* Grab an exclusive MDL lock. */
  if (lock_object_name(thd, mdl_type, sp->m_db.str, sp->m_name.str))
  {
    my_error(ER_SP_STORE_FAILED, MYF(0), SP_TYPE_STRING(thd->lex), sp->m_name.str);
    DBUG_RETURN(true);
  }
  ...
  */
  ...
  else
  {
    ...
    strxnmov(definer, sizeof(definer) - 1, thd->lex->definer->user.str, "@", thd->lex->definer->host.str, NullS);
    ...
    if (system_charset_info->cset->numchars(system_charset_info, sp->m_name.str, sp->m_name.str + sp->m_name.length) >
    ...
    store_failed = table->field[MYSQL_PROC_FIELD_DB]->store(sp->m_db.str, sp->m_db.length, system_charset_info);

    store_failed = store_failed ||
                   table->field[MYSQL_PROC_FIELD_NAME]->store(sp->m_name.str, sp->m_name.length, system_charset_info);

    store_failed = store_failed || table->field[MYSQL_PROC_MYSQL_TYPE]->store((longlong)sp->m_type, TRUE);

    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_SPECIFIC_NAME]->store(
                                       sp->m_name.str, sp->m_name.length, system_charset_info);

    if (sp->m_chistics->daccess != SP_DEFAULT_ACCESS)
    {
      store_failed =
          store_failed || table->field[MYSQL_PROC_FIELD_ACCESS]->store((longlong)sp->m_chistics->daccess, TRUE);
    }

    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_DETERMINISTIC]->store(
                                       (longlong)(sp->m_chistics->detistic ? 1 : 2), TRUE);

    if (sp->m_chistics->suid != SP_IS_DEFAULT_SUID)
    {
      store_failed =
          store_failed || table->field[MYSQL_PROC_FIELD_SECURITY_TYPE]->store((longlong)sp->m_chistics->suid, TRUE);
    }

    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_PARAM_LIST]->store(
                                       sp->m_params.str, sp->m_params.length, system_charset_info);
    ...
    bool store_definer_failed =
        table->field[MYSQL_PROC_FIELD_DEFINER]->store(definer, strlen(definer), system_charset_info);
    ...
    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_SQL_MODE]->store((longlong)saved_mode, TRUE);

    if (sp->m_chistics->comment.str)
    {
      store_failed =
          store_failed || table->field[MYSQL_PROC_FIELD_COMMENT]->store(
                              sp->m_chistics->comment.str, sp->m_chistics->comment.length, system_charset_info);
    }

    if ((sp->m_type == SP_TYPE_FUNCTION) && !trust_function_creators && mysql_bin_log.is_open())
    {
    ...
    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_CHARACTER_SET_CLIENT]->store(
                                       thd->charset()->csname, strlen(thd->charset()->csname), system_charset_info);
    ...
    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_COLLATION_CONNECTION]->store(
                                       ...
                                       strlen(thd->variables.collation_connection->name), system_charset_info);

    table->field[MYSQL_PROC_FIELD_DB_COLLATION]->set_notnull();
    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_DB_COLLATION]->store(db_cs->name, strlen(db_cs->name),
                                                                                      system_charset_info);

    table->field[MYSQL_PROC_FIELD_BODY_UTF8]->set_notnull();
    store_failed = store_failed || table->field[MYSQL_PROC_FIELD_BODY_UTF8]->store(
    ...
    if (store_definer_failed &&
        table->field[MYSQL_PROC_FIELD_DEFINER]->field_length <
            (USERNAME_CHAR_LENGTH + HOSTNAME_LENGTH + 1) * table->field[MYSQL_PROC_FIELD_DEFINER]->charset()->mbmaxlen)
    {
      my_error(ER_USER_COLUMN_OLD_LENGTH, MYF(0), table->field[MYSQL_PROC_FIELD_DEFINER]->field_name);
      goto done;
    }
    ...
    {
      my_error(ER_SP_ALREADY_EXISTS, MYF(0), SP_TYPE_STRING(thd->lex), sp->m_name.str);
      goto done;
    }
    ...
    if (mysql_bin_log.is_open())
    {
      ...
      if (!create_string(thd, &log_query, sp->m_type, (sp->m_explicit_name ? sp->m_db.str : NULL),
                         (sp->m_explicit_name ? sp->m_db.length : 0), sp->m_name.str, sp->m_name.length,
                         sp->m_params.str, sp->m_params.length, retstr.c_ptr(), retstr.length(), sp->m_body.str,
                         sp->m_body.length, sp->m_chistics, thd->lex->definer->user, thd->lex->definer->host,
                         saved_mode))
      {
        my_error(ER_SP_STORE_FAILED, MYF(0), SP_TYPE_STRING(thd->lex), sp->m_name.str);
        error = true;
      ...
      if (thd->binlog_query(THD::STMT_QUERY_TYPE, log_query.c_ptr(), log_query.length(), FALSE, FALSE, FALSE, 0))
      {
        my_error(ER_SP_STORE_FAILED, MYF(0), SP_TYPE_STRING(thd->lex), sp->m_name.str);
        error = true;
      };
...
int sp_drop_routine(THD *thd, enum_sp_type type, sp_name *name)
{
  ...
  int ret;
  bool save_binlog_row_based;
  MDL_key::enum_mdl_namespace mdl_type = (type == SP_TYPE_FUNCTION) ? MDL_key::FUNCTION : MDL_key::PROCEDURE;
  ...
  */
  ...
  if (ret == SP_OK)
  {
    ...
    */
    {
      sp_head *sp;
      sp_cache **spc = (type == SP_TYPE_FUNCTION ? &thd->sp_func_cache : &thd->sp_proc_cache);
    ...
    MYSQL_DROP_SP(type, name->m_db.str, name->m_db.length, name->m_name.str, name->m_name.length);
#endif
  }
  /* Restore the state of binlog format */
  assert(!thd->is_current_stmt_binlog_format_row());
...
int sp_update_routine(THD *thd, enum_sp_type type, sp_name *name, st_sp_chistics *chistics)
{
  TABLE *table;
  int ret;
  bool save_binlog_row_based;
...
class Lock_db_routines_error_handler : public Internal_error_handler
{
 public:
  virtual bool handle_condition(THD *thd, uint sql_errno, const char *sqlstate,
                                Sql_condition::enum_severity_level *level, const char *msg)
  {
    if (sql_errno == ER_NO_SUCH_TABLE || sql_errno == ER_CANNOT_LOAD_FROM_TABLE_V2 ||
        sql_errno == ER_COL_COUNT_DOESNT_MATCH_PLEASE_UPDATE_V2 || sql_errno == ER_COL_COUNT_DOESNT_MATCH_CORRUPTED_V2)
      return true;
    return false;
  }
};
...
bool lock_db_routines(THD *thd, const char *db)
{
  ...
  */
  ...
  if (!table->file->ha_index_read_map(table->record[0], table->field[MYSQL_PROC_FIELD_DB]->ptr, (key_part_map)1,
                                      HA_READ_KEY_EXACT))
  {
    do
    {
      ...
      MDL_REQUEST_INIT(mdl_request, sp_type == SP_TYPE_FUNCTION ? MDL_key::FUNCTION : MDL_key::PROCEDURE, db, sp_name,
                       MDL_EXCLUSIVE, MDL_TRANSACTION);
      mdl_requests.push_front(mdl_request);
    } while (
        !(nxtres = table->file->ha_index_next_same(table->record[0], table->field[MYSQL_PROC_FIELD_DB]->ptr, key_len)));
  }
  table->file->ha_index_end();
  ...
  assert(thd->mdl_context.owns_equal_or_stronger_lock(MDL_key::GLOBAL, "", "", MDL_INTENTION_EXCLUSIVE) &&
         thd->mdl_context.owns_equal_or_stronger_lock(MDL_key::SCHEMA, db, "", MDL_EXCLUSIVE));
                                      ...
                                      HA_READ_KEY_EXACT))
  {
    ...
    do
    {
      if (!table->file->ha_delete_row(table->record[0]))
      {
        ...
        /* Drop statistics for this stored program from performance schema. */
        MYSQL_DROP_SP(sp_type, db, db_name_length, sp_name, sp_name_length);
#endif
      }
      else
      {
...
bool sp_show_create_routine(THD *thd, enum_sp_type type, sp_name *name)
{
  ...
  DBUG_PRINT("enter", ("name: %.*s", (int)name->m_name.length, name->m_name.str));
  ...
  */
  ...
  if (sp == NULL || sp->show_create_routine(thd, type))
  {
    /*
      ...
      does not exist.
    */
    my_error(ER_SP_DOES_NOT_EXIST, MYF(0), type == SP_TYPE_FUNCTION ? "FUNCTION" : "PROCEDURE", name->m_name.str);
    DBUG_RETURN(TRUE);
  }
...
{
  sp_head *sp;
  ulong depth = (type == SP_TYPE_PROCEDURE ? thd->variables.max_sp_recursion_depth : 0);
  DBUG_ENTER("sp_find_routine");
  DBUG_PRINT("enter", ("name:  %.*s.%.*s  type: %d  cache only %d", (int)name->m_db.length, name->m_db.str,
                       (int)name->m_name.length, name->m_name.str, type, cache_only));
  ...
  if ((sp = sp_cache_lookup(cp, name)))
  {
    ...
    */
    ...
    {
      DBUG_PRINT("info", ("first free: 0x%lx  level: %lu  flags %x", (ulong)sp->m_first_free_instance,
    ...
    */
    ...
    strxmov(definer, sp->m_definer_user.str, "@", sp->m_definer_host.str, NullS);
    if (type == SP_TYPE_FUNCTION)
    {
      sp_returns_type(thd, retstr, sp);
      returns = retstr.ptr();
    }
    if (db_load_routine(thd, type, name, &new_sp, sp->m_sql_mode, sp->m_params.str, returns, sp->m_body.str,
                        *sp->m_chistics, definer, sp->m_created, sp->m_modified, sp->get_creation_ctx()) == SP_OK)
    {
      sp->m_last_cached_sp->m_next_cached_sp = new_sp;
      new_sp->m_recursion_level = level;
      new_sp->m_first_instance = sp;
      sp->m_last_cached_sp = sp->m_first_free_instance = new_sp;
      DBUG_PRINT("info", ("added level: 0x%lx, level: %lu, flags %x", (ulong)new_sp, new_sp->m_recursion_level,
                          new_sp->m_flags));
      DBUG_RETURN(new_sp);
    }
    DBUG_RETURN(0);
  ...
  if (!cache_only)
  {
    if (db_find_routine(thd, type, name, &sp) == SP_OK)
    {
      ...
      DBUG_PRINT("info", ("added new: 0x%lx, level: %lu, flags %x", (ulong)sp, sp->m_recursion_level, sp->m_flags));
    }
  }
  DBUG_RETURN(sp);
}
...
bool sp_exist_routines(THD *thd, TABLE_LIST *routines, bool is_proc)
{
  ...
  for (routine = routines; routine; routine = routine->next_global)
  {
    ...
    name->init_qname(thd);
    sp_object_found = is_proc ? sp_find_routine(thd, SP_TYPE_PROCEDURE, name, &thd->sp_proc_cache, FALSE) != NULL
                              : sp_find_routine(thd, SP_TYPE_FUNCTION, name, &thd->sp_func_cache, FALSE) != NULL;
    thd->get_stmt_da()->reset_condition_info(thd);
    if (!sp_object_found)
    {
      my_error(ER_SP_DOES_NOT_EXIST, MYF(0), is_proc ? "PROCEDURE" : "FUNCTION", routine->table_name);
      DBUG_RETURN(TRUE);
    }
  }
  DBUG_RETURN(FALSE);
...
{
  my_hash_init_opt(&prelocking_ctx->sroutines, system_charset_info, Query_tables_list::START_SROUTINES_HASH_SIZE, 0, 0,
                   sp_sroutine_key, 0, 0, PSI_INSTRUMENT_ME);
...
void sp_add_used_routine(Query_tables_list *prelocking_ctx, Query_arena *arena, sp_name *rt, enum_sp_type rt_type)
{
  MDL_key key((rt_type == SP_TYPE_FUNCTION) ? MDL_key::FUNCTION : MDL_key::PROCEDURE, rt->m_db.str, rt->m_name.str);
...
void sp_update_stmt_used_routines(THD *thd, Query_tables_list *prelocking_ctx, HASH *src, TABLE_LIST *belong_to_view)
{
  for (uint i = 0; i < src->records; i++)
  {
    ...
    (void)sp_add_used_routine(prelocking_ctx, thd->stmt_arena, &rt->mdl_request.key, belong_to_view);
  }
}
...
int sp_cache_routine(THD *thd, Sroutine_hash_entry *rt, bool lookup_only, sp_head **sp)
{
  char qname_buff[NAME_LEN * 2 + 1 + 1];
  sp_name name(&rt->mdl_request.key, qname_buff);
  MDL_key::enum_mdl_namespace mdl_type = rt->mdl_request.key.mdl_namespace();
  enum_sp_type type = (mdl_type == MDL_key::FUNCTION) ? SP_TYPE_FUNCTION : SP_TYPE_PROCEDURE;
...
int sp_cache_routine(THD *thd, enum_sp_type type, sp_name *name, bool lookup_only, sp_head **sp)
{
  int ret = 0;
  sp_cache **spc = (type == SP_TYPE_FUNCTION) ? &thd->sp_func_cache : &thd->sp_proc_cache;
...
*/
static bool create_string(THD *thd, String *buf, enum_sp_type type, const char *db, size_t dblen, const char *name,
                          size_t namelen, const char *params, size_t paramslen, const char *returns, size_t returnslen,
                          const char *body, size_t bodylen, st_sp_chistics *chistics, const LEX_CSTRING &definer_user,
                          const LEX_CSTRING &definer_host, sql_mode_t sql_mode)
{
  ...
  if (buf->alloc(100 + dblen + 1 + namelen + paramslen + returnslen + bodylen + chistics->comment.length +
...
sp_head *sp_load_for_information_schema(THD *thd, TABLE *proc_table, String *db, String *name, sql_mode_t sql_mode,
                                        enum_sp_type type, const char *returns, const char *params, bool *free_sp_head)
{
  const char *sp_body;
  String defstr;
  ...
  *free_sp_head = 0;
  ...
  defstr.set_charset(creation_ctx->get_client_cs());
  if (!create_string(thd, &defstr, type, sp_db_str.str, sp_db_str.length, sp_name_obj.m_name.str,
                     sp_name_obj.m_name.length, params, strlen(params), returns, strlen(returns), sp_body,
                     strlen(sp_body), &sp_chistics, definer_user, definer_host, sql_mode))
    return 0;
...
bool sp_check_name(LEX_STRING *ident)
{
  ...
  if (check_string_char_length(ident_cstr, "", NAME_CHAR_LEN, system_charset_info, 1))
  {
...
TABLE_LIST *sp_add_to_query_tables(THD *thd, LEX *lex, const char *db, const char *name)
{
  ...
  table->init_one_table(thd->strmake(db, db_length), db_length, thd->strmake(name, table_name_length),
                        table_name_length, thd->mem_strdup(name), TL_IGNORE, MDL_SHARED_NO_WRITE);
...
Item *sp_prepare_func_item(THD *thd, Item **it_addr)
{
  ...
  if (!(*it_addr)->fixed && ((*it_addr)->fix_fields(thd, it_addr) || (*it_addr)->check_cols(1)))
  {
...
{
  Item *expr_item;
  Strict_error_handler strict_handler(Strict_error_handler::ENABLE_SET_SELECT_STRICT_ERROR_HANDLER);
  enum_check_fields save_count_cuted_fields = thd->count_cuted_fields;
  unsigned int stmt_unsafe_rollback_flags = thd->get_transaction()->get_unsafe_rollback_flags(Transaction_ctx::STMT);
  ...
  */
  ...
  thd->get_transaction()->set_unsafe_rollback_flags(Transaction_ctx::STMT, stmt_unsafe_rollback_flags);
