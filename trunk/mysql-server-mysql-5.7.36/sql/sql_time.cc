...
LEX_STRING interval_type_to_name[INTERVAL_LAST] = {{C_STRING_WITH_LEN("YEAR")},
...
static uint to_ascii(const CHARSET_INFO *cs, const char *src, size_t src_length, char *dst, size_t dst_length)
{
  ...
  while (dst < dstend && (cnvres = (cs->cset->mb_wc)(cs, &wc, (const uchar *)src, (const uchar *)srcend)) > 0 &&
         wc < 128)
  {
    src += cnvres;
...
bool str_to_time(const CHARSET_INFO *cs, const char *str, size_t length, MYSQL_TIME *l_time, my_time_flags_t flags,
                 MYSQL_TIME_STATUS *status)
{
  ...
  return str_to_time(str, length, l_time, status) ||
         (!(flags & TIME_NO_NSEC_ROUNDING) &&
          time_add_nanoseconds_with_round(l_time, status->nanoseconds, &status->warnings));
}

/* Character set-aware version of str_to_datetime() */
bool str_to_datetime(const CHARSET_INFO *cs, const char *str, size_t length, MYSQL_TIME *l_time, my_time_flags_t flags,
                     MYSQL_TIME_STATUS *status)
{
  ...
  return str_to_datetime(str, length, l_time, flags, status) ||
         (!(flags & TIME_NO_NSEC_ROUNDING) &&
          datetime_add_nanoseconds_with_round(l_time, status->nanoseconds, &status->warnings));
}
...
bool datetime_add_nanoseconds_with_round(MYSQL_TIME *ltime, uint nanoseconds, int *warnings)
{
  ...
  if (check_date(ltime, non_zero_date(ltime), (TIME_NO_ZERO_IN_DATE | TIME_NO_ZERO_DATE), warnings))
    return true;
...
*/
bool datetime_with_no_zero_in_date_to_timeval(THD *thd, const MYSQL_TIME *ltime, struct timeval *tm, int *warnings)
{
...
/**
  ...
  @retval      true on error
*/
bool datetime_to_timeval(THD *thd, const MYSQL_TIME *ltime, struct timeval *tm, int *warnings)
{
...
{
  thd->variables.time_zone->gmt_sec_to_TIME(ltime2, static_cast<my_time_t>(thd->query_start()));
...
bool parse_date_time_format(timestamp_type format_type, Date_time_format *date_time_format)
{
...
Date_time_format *date_time_format_copy(THD *thd, Date_time_format *format)
{
  ...
  if (new_format)
  {
    ...
    memcpy(new_format->format.str, format->format.str, format->format.length);
...
*/
void make_time(const Date_time_format *format MY_ATTRIBUTE((unused)), const MYSQL_TIME *l_time, String *str, uint dec)
{
...
/**
  ...
  @param[out] str      String to convert to
*/
void make_date(const Date_time_format *format MY_ATTRIBUTE((unused)), const MYSQL_TIME *l_time, String *str)
{
...
/**
  ...
  @param      dec      Number of fractional digits.
*/
void make_datetime(const Date_time_format *format MY_ATTRIBUTE((unused)), const MYSQL_TIME *l_time, String *str,
                   uint dec)
{
...
void make_truncated_value_warning(THD *thd, Sql_condition::enum_severity_level level, ErrConvString val,
                                  timestamp_type time_type, const char *field_name)
{
  ...
  if (field_name)
    cs->cset->snprintf(cs, warn_buff, sizeof(warn_buff), ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), type_str, val.ptr(),
                       field_name, (long)thd->get_stmt_da()->current_row_for_condition());
  else
  {
    if (time_type > MYSQL_TIMESTAMP_ERROR)
      cs->cset->snprintf(cs, warn_buff, sizeof(warn_buff), ER(ER_TRUNCATED_WRONG_VALUE), type_str, val.ptr());
    else
      cs->cset->snprintf(cs, warn_buff, sizeof(warn_buff), ER(ER_WRONG_VALUE), type_str, val.ptr());
  }
  ...
  switch (int_type)
  {
    ...
    case INTERVAL_DAY_HOUR:
    {
      ...
      sec = ((ltime->day - 1) * 3600 * 24L + ltime->hour * 3600 + ltime->minute * 60 + ltime->second +
             sign * (longlong)(interval.day * 3600 * 24L + interval.hour * 3600LL + interval.minute * 60LL +
                               interval.second)) +
            extra_sec;
      if (microseconds < 0)
      {
...
bool calc_time_diff(const MYSQL_TIME *l_time1, const MYSQL_TIME *l_time2, int l_sign, longlong *seconds_out,
                    long *microseconds_out)
{
  ...
  */
  ...
  microseconds =
      ((longlong)days * SECONDS_IN_24H + (longlong)(l_time1->hour * 3600L + l_time1->minute * 60L + l_time1->second) -
...
{
  assert(ldate->time_type == MYSQL_TIMESTAMP_DATE || ldate->time_type == MYSQL_TIMESTAMP_DATETIME);
...
*/
longlong TIME_to_longlong_packed(const MYSQL_TIME *ltime, enum enum_field_types type)
{
  switch (type)
  {
