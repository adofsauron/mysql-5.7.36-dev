...
const char *XID_STATE::xa_state_names[] = {"NON-EXISTING", "ACTIVE", "IDLE", "PREPARED", "ROLLBACK ONLY"};
...
{
  plugin_foreach(NULL, commit ? xacommit_handlerton : xarollback_handlerton, MYSQL_STORAGE_ENGINE_PLUGIN, xid);
}
  ...
  if (hton->state == SHOW_OPTION_YES && hton->recover)
  {
    while ((got = hton->recover(hton, info->list, info->len)) > 0)
    {
      ...
      for (int i = 0; i < got; i++)
      {
        ...
        // recovery mode
        if (info->commit_list ? my_hash_search(info->commit_list, (uchar *)&x, sizeof(x)) != 0
...
int ha_recover(HASH *commit_list)
{
  ...
  info.dry_run = (info.commit_list == 0 && tc_heuristic_recover == TC_HEURISTIC_NOT_USED);
  info.list = NULL;

  /* commit_list and tc_heuristic_recover cannot be set both */
  assert(info.commit_list == 0 || tc_heuristic_recover == TC_HEURISTIC_NOT_USED);
  ...
  if (total_ha_2pc > (ulong)opt_bin_log + 1)
  {
    ...
    {
      sql_print_error(
          "--tc-heuristic-recover rollback strategy is not safe "
...
bool Sql_cmd_xa_commit::trans_xa_commit(THD *thd)
{
  ...
  assert(!thd->slave_thread || xid_state->get_xid()->is_null() || m_xa_opt == XA_ONE_PHASE);
  ...
  if (!xid_state->has_same_xid(m_xid))
  {
    ...
    */
    ...
    if (!res)
    {
      ...
      MYSQL_SET_TRANSACTION_XID(thd->m_transaction_psi, (const void *)xs->get_xid(), (int)xs->get_state());
    }
#endif
    /*
    ...
    */
    ...
    MDL_REQUEST_INIT(&mdl_request, MDL_key::COMMIT, "", "", MDL_INTENTION_EXCLUSIVE, MDL_STATEMENT);
  ...
  else if (xid_state->has_state(XID_STATE::XA_IDLE) && m_xa_opt == XA_ONE_PHASE)
  {
    int r = ha_commit_trans(thd, true);
    if ((res = MY_TEST(r)))
      my_error(r == 1 ? ER_XA_RBROLLBACK : ER_XAER_RMERR, MYF(0));
  }
  else if (xid_state->has_state(XID_STATE::XA_PREPARED) && m_xa_opt == XA_NONE)
  {
    MDL_request mdl_request;
...
bool Sql_cmd_xa_rollback::trans_xa_rollback(THD *thd)
{
  ...
  if (xid_state->has_state(XID_STATE::XA_NOTR) || xid_state->has_state(XID_STATE::XA_ACTIVE))
  {
    my_error(ER_XAER_RMFAIL, MYF(0), xid_state->state_name());
    DBUG_RETURN(true);
  }
  ...
  */
  ...
  bool gtid_error = MY_TEST(commit_owned_gtids(thd, true, &need_clear_owned_gtid));
...
bool Sql_cmd_xa_start::trans_xa_start(THD *thd)
{
  ...
  if (xid_state->has_state(XID_STATE::XA_IDLE) && m_xa_opt == XA_RESUME)
  {
    ...
    else
    {
      ...
      MYSQL_SET_TRANSACTION_XA_STATE(thd->m_transaction_psi, (int)thd->get_transaction()->xid_state()->get_state());
    }
  ...
  else if (!trans_begin(thd))
  {
    ...
    MYSQL_SET_TRANSACTION_XID(thd->m_transaction_psi, (const void *)xid_state->get_xid(), (int)xid_state->get_state());
...
bool Sql_cmd_xa_end::trans_xa_end(THD *thd)
{
  ...
  if (m_xa_opt != XA_NONE)
    my_error(ER_XAER_INVAL, MYF(0));
  else if (!xid_state->has_state(XID_STATE::XA_ACTIVE) && !xid_state->has_state(XID_STATE::XA_ROLLBACK_ONLY))
    my_error(ER_XAER_RMFAIL, MYF(0), xid_state->state_name());
...
bool Sql_cmd_xa_prepare::trans_xa_prepare(THD *thd)
{
  ...
  if (!xid_state->has_state(XID_STATE::XA_IDLE))
    ...
    my_error(ER_XAER_NOTA, MYF(0));
  else if (thd->slave_thread && is_transaction_empty(thd))  // No changes in none of the storage engine
...
bool Sql_cmd_xa_prepare::execute(THD *thd)
{
  ...
  if (!st)
  {
    if (!thd->rpl_unflag_detached_engine_ha_data() || !(st = applier_reset_xa_trans(thd)))
      my_ok(thd);
  }
...
bool Sql_cmd_xa_recover::trans_xa_recover(THD *thd)
{
  ...
  while ((transaction = (Transaction_ctx *)my_hash_element(&transaction_cache, i++)))
  {
...
void XID_STATE::store_xid_info(Protocol *protocol, bool print_xid_as_hex) const
{
  ...
  if (print_xid_as_hex)
  {
    ...
    */
    ...
    size_t xid_str_len = bin_to_hex_str(xid_buf + 2, sizeof(xid_buf) - 2, const_cast<char *>(m_xid.data),
  ...
  {
    protocol->store(m_xid.data, m_xid.gtrid_length + m_xid.bqual_length, &my_charset_bin);
  }
}
...
extern "C" uchar *transaction_get_hash_key(const uchar *ptr, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
...
bool transaction_cache_init()
{
  ...
  mysql_mutex_init(key_LOCK_transaction_cache, &LOCK_transaction_cache, MY_MUTEX_INIT_FAST);
  return my_hash_init(&transaction_cache, &my_charset_bin, 100, 0, 0, transaction_get_hash_key, transaction_free_hash,
                      0, key_memory_XID) != 0;
}
...
Transaction_ctx *transaction_cache_search(XID *xid)
{
  ...
  Transaction_ctx *res = (Transaction_ctx *)my_hash_search(&transaction_cache, xid->key(), xid->key_length());
  mysql_mutex_unlock(&LOCK_transaction_cache);
  return res;
}
...
{
  mysql_mutex_lock(&LOCK_transaction_cache);
  if (my_hash_search(&transaction_cache, xid->key(), xid->key_length()))
  {
    mysql_mutex_unlock(&LOCK_transaction_cache);
...
bool transaction_cache_detach(Transaction_ctx *transaction)
{
  ...
  assert(my_hash_search(&transaction_cache, xid.key(), xid.key_length()));
...
my_bool detach_native_trx(THD *thd, plugin_ref plugin, void *unused)
{
  ...
  if (hton->replace_native_transaction_in_thd)
  {
    ...
    hton->replace_native_transaction_in_thd(thd, NULL, thd_ha_data_backup(thd, hton));
  }
