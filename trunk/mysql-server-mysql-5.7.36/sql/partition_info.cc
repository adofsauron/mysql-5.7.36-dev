...
bool partition_info::add_named_partition(const char *part_name, size_t length)
{
  ...
  DBUG_PRINT("info",
             ("Found partition %u is_subpart %d for name %s", part_def->part_id, part_def->is_subpart, part_name));
  DBUG_RETURN(false);
}
...
bool partition_info::set_partition_bitmaps(TABLE_LIST *table_list)
{
  ...
  if (table_list && table_list->partition_names && table_list->partition_names->elements)
  {
...
bool partition_info::can_prune_insert(THD *thd, enum_duplicates duplic, COPY_INFO &update, List<Item> &update_fields,
                                      List<Item> &fields, bool empty_values, enum_can_prune *can_prune_partitions,
                                      bool *prune_needs_default_values, MY_BITMAP *used_partitions)
{
  uint32 *bitmap_buf;
  uint bitmap_bytes;
  ...
  */
  if (table->triggers)
  {
    Trigger_chain *trigger_chain = table->triggers->get_triggers(TRG_EVENT_INSERT, TRG_ACTION_BEFORE);

    if (trigger_chain && trigger_chain->has_updated_trigger_fields(&full_part_field_set))
      DBUG_RETURN(false);
  }
  ...
  */
  ...
  if (table->found_next_number_field)
  {
    ...
    */
    if (bitmap_is_set(&full_part_field_set, table->found_next_number_field->field_index))
      DBUG_RETURN(false);
  }
  ...
  */
  if (duplic == DUP_UPDATE)
  {
    ...
    */
    if (table->triggers)
    {
      Trigger_chain *trigger_chain = table->triggers->get_triggers(TRG_EVENT_UPDATE, TRG_ACTION_BEFORE);
...
bool partition_info::set_used_partition(List<Item> &fields, List<Item> &values, COPY_INFO &info,
                                        bool copy_default_values, MY_BITMAP *used_partitions)
{
  ...
  {
    if (fill_record(thd, table, table->field, values, &full_part_field_set, NULL))
      goto err;
  }
...
char *partition_info::create_default_partition_names(uint part_no, uint num_parts_arg, uint start_no)
{
...
void partition_info::set_show_version_string(String *packet)
{
  ...
  {
    if (part_expr)
      part_expr->walk(&Item::intro_version, Item::WALK_POSTFIX, (uchar *)&version);
    if (subpart_expr)
      subpart_expr->walk(&Item::intro_version, Item::WALK_POSTFIX, (uchar *)&version);
    if (version == 0)
    {
...
char *partition_info::create_default_subpartition_name(uint subpart_no, const char *part_name)
{
...
bool partition_info::set_up_default_partitions(Partition_handler *part_handler, HA_CREATE_INFO *info, uint start_no)
{
  uint i;
  char *default_name;
  bool result = TRUE;
  ...
  if (unlikely((!(default_name = create_default_partition_names(0, num_parts, start_no)))))
    goto end;
  i = 0;
  do
  {
...
bool partition_info::set_up_default_subpartitions(Partition_handler *part_handler, HA_CREATE_INFO *info)
{
  uint i, j;
  bool result = TRUE;
  ...
  do
  {
    ...
    do
    {
      ...
      {
        char *ptr = create_default_subpartition_name(j, part_elem->partition_name);
        if (!ptr)
          goto end;
...
bool partition_info::set_up_defaults_for_partitioning(Partition_handler *part_handler, HA_CREATE_INFO *info,
                                                      uint start_no)
{
...
*/
partition_element *partition_info::get_part_elem(const char *partition_name, char *file_name, uint32 *part_id)
{
  List_iterator<partition_element> part_it(partitions);
  uint i = 0;
  ...
  do
  {
    ...
    if (is_sub_partitioned())
    {
      ...
      do
      {
        ...
        {
          if (file_name)
            create_subpartition_name(file_name, "", part_elem->partition_name, partition_name, NORMAL_PART_NAME);
    ...
    {
      if (file_name)
        create_partition_name(file_name, "", partition_name, NORMAL_PART_NAME, TRUE);
      *part_id = i;
      DBUG_RETURN(part_elem);
    }
  } while (++i < num_parts);
...
static const char *get_part_name_from_elem(const char *name, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
  *length = strlen(name);
  return name;
}
...
char *partition_info::find_duplicate_name()
{
  ...
  */
  ...
  if (my_hash_init(&partition_names, system_charset_info, max_names, 0, 0, (my_hash_get_key)get_part_name_from_elem, 0,
...
bool partition_info::check_range_constants(THD *thd)
{
  ...
  DBUG_PRINT("enter", ("RANGE with %d parts, column_list = %u", num_parts, column_list));
  ...
  if (column_list)
  {
    ...
    range_col_array = (part_column_list_val *)sql_calloc(num_parts * size_entries);
    ...
    do
    {
      part_def = it++;
      {
        ...
        if (!first)
        {
          if (compare_column_values((const void *)current_largest_col_val, (const void *)col_val) >= 0)
      ...
      if (!first)
      {
        if (unlikely(current_largest > part_range_value) ||
            (unlikely(current_largest == part_range_value) &&
             (part_range_value < LLONG_MAX || i != (num_parts - 1) || !defined_max_value)))
          goto range_not_increasing_error;
      }
...
int partition_info::compare_column_values(const void *first_arg, const void *second_arg)
{
...
bool partition_info::check_list_constants(THD *thd)
{
  ...
  */
  ...
  size_entries = column_list ? (num_column_values * sizeof(part_column_list_val)) : sizeof(LIST_PART_ENTRY);
  ...
  else
  {
    ...
    /*
      ...
      partition functions.
    */
    type_add = (longlong)(part_expr->unsigned_flag ? 0x8000000000000000ULL : 0ULL);
  ...
  if (num_list_values)
  {
    ...
    */
    my_qsort((void *)list_array, num_list_values, size_entries, compare_func);

    i = 0;
    do
    {
      assert(i < num_list_values);
      curr_value = column_list ? (void *)&list_col_array[num_column_values * i] : (void *)&list_array[i];
                                          ...
                                          bool add_or_reorg_part)
{
  ...
  if (!add_or_reorg_part)
  {
    ...
    {
      assert(part_expr);
      err = part_expr->walk(&Item::check_partition_func_processor, Item::WALK_POSTFIX, NULL);
    }
    ...
    {
      assert(subpart_expr);
      err = subpart_expr->walk(&Item::check_partition_func_processor, Item::WALK_POSTFIX, NULL);
    }
  ...
  if (unlikely(is_sub_partitioned() && (!(part_type == RANGE_PARTITION || part_type == LIST_PARTITION))))
  {
    /* Only RANGE and LIST partitioning can be subpartitioned */
    my_error(ER_SUBPARTITION_ERROR, MYF(0));
    goto end;
  }
  if (unlikely(set_up_defaults_for_partitioning(file->get_partition_handler(), info, (uint)0)))
  {
    goto end;
  }
  ...
  */
  ...
  else
  {
    ...
    if (thd->lex->sql_command != SQLCOM_CREATE_TABLE)
    {
      ...
      assert(table_engine && !is_ha_partition_handlerton(table_engine));
    }
  }
    ...
    do
    {
      ...
      if (!is_sub_partitioned())
      {
        ...
        enum_ident_name_check ident_check_status =
            check_table_name(part_elem->partition_name, strlen(part_elem->partition_name), FALSE);
      ...
      else
      {
        ...
        do
        {
          ...
          enum_ident_name_check ident_check_status =
              check_table_name(sub_elem->partition_name, strlen(sub_elem->partition_name), FALSE);
          if (ident_check_status == IDENT_NAME_WRONG)
          {
            my_error(ER_WRONG_PARTITION_NAME, MYF(0));
            goto end;
  ...
  assert(table_engine == default_engine_type && !is_ha_partition_handlerton(table_engine));
  if (eng_type)
  ...
  */
  ...
  if (add_or_reorg_part)
  {
    if (unlikely((part_type == RANGE_PARTITION && check_range_constants(thd)) ||
                 (part_type == LIST_PARTITION && check_list_constants(thd))))
      goto end;
  }
  result = FALSE;
end:
...
void partition_info::print_no_partition_found(TABLE *table_arg)
{
  ...
  if (check_single_table_access(current_thd, SELECT_ACL, &table_list, TRUE))
  {
    my_message(ER_NO_PARTITION_FOR_GIVEN_VALUE, ER(ER_NO_PARTITION_FOR_GIVEN_VALUE_SILENT), MYF(0));
  }
  else
  {
    if (column_list)
...
bool partition_info::set_part_expr(char *start_token, Item *item_ptr, char *end_token, bool is_subpart)
{
...
void partition_info::report_part_expr_error(bool use_subpart_expr)
{
  ...
  if (expr->type() == Item::FIELD_ITEM)
  {
    ...
    */
    ...
    {
      my_error(ER_FIELD_TYPE_NOT_ALLOWED_AS_PARTITION_FIELD, MYF(0), item_field->item_name.ptr());
      DBUG_VOID_RETURN;
    }
  }
  if (use_subpart_expr)
...
bool partition_info::add_column_list_value(THD *thd, Item *item)
{
  ...
  if (item->walk(&Item::check_partition_func_processor, Item::WALK_POSTFIX, NULL))
  {
    my_error(ER_PARTITION_FUNCTION_IS_NOT_ALLOWED, MYF(0));
    DBUG_RETURN(TRUE);
  }
...
bool partition_info::init_column_part()
{
  ...
  if (!(list_val = (part_elem_value *)sql_calloc(sizeof(part_elem_value))) || p_elem->list_val_list.push_back(list_val))
  {
  ...
  if (!(col_val_array = (part_column_list_val *)sql_calloc(loc_num_columns * sizeof(part_column_list_val))))
  {
...
bool partition_info::fix_partition_values(THD *thd, part_elem_value *val, partition_element *part_elem, uint part_id)
{
...
bool partition_info::fix_parser_data(THD *thd)
{
  ...
  do
  {
    ...
    assert(part_type == RANGE_PARTITION ? num_elements == 1U : TRUE);
      ...
      */
        ...
        if (!use_default_subpartitions)
        {
          ...
          do
          {
            ...
            if (strcmp_null(sub_part_elem->data_file_name, new_sub_part_elem->data_file_name) ||
                ...
                strcmp_null(sub_part_elem->tablespace_name, new_sub_part_elem->tablespace_name))
              DBUG_RETURN(false);
      ...
      else
      {
        ...
        if (strcmp_null(part_elem->data_file_name, new_part_elem->data_file_name) ||
            ...
            strcmp_null(part_elem->tablespace_name, new_part_elem->tablespace_name))
          DBUG_RETURN(false);
      }
    } while (++i < num_parts);
  }
...
bool has_external_data_or_index_dir(partition_info &pi)
{
  ...
  for (partition_element *part = part_it++; part; part = part_it++)
  {
    ...
    List_iterator<partition_element> subpart_it(part->subpartitions);
    for (const partition_element *subpart = subpart_it++; subpart; subpart = subpart_it++)
    {
...
bool check_partition_tablespace_names(partition_info *part_info)
{
  ...
  while ((part_elem = part_it++))
  {
    ...
    if (part_elem->tablespace_name && check_tablespace_name(part_elem->tablespace_name))
      return true;
    ...
    {
      // Add tablespace name from sub-partition elements, if used.
      if (sub_elem->tablespace_name && check_tablespace_name(sub_elem->tablespace_name))
        return true;
    }
  }
