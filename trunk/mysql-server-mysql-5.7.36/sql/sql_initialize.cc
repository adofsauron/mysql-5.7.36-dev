...
static const char *initialization_cmds[] = {"CREATE DATABASE mysql;\n", "USE mysql;\n", NULL};
...
static const char *initialization_data[] = {"FLUSH PRIVILEGES", insert_user_buffer,
...
static const char **cmds[] = {initialization_cmds, mysql_system_tables,
                              ...
                              fill_help_tables,    session_service_initialization_data,
                              mysql_sys_schema,    NULL};

/** keep in sync with the above array */
static const char *cmd_descs[] = {"Creating the system database",
...
#define rnd_of(x) x[((int)(my_rnd_ssl(&srnd) * 100)) % (sizeof(x) - 1)]
  ...
  rand_struct srnd;
  char *ptr = password;
  bool had_upper = false, had_lower = false, had_numeric = false, had_special = false;
...
void Compiled_in_command_iterator::begin(void)
{
  ...
  else
  {
    ...
    */
    ...
    sql_print_information("A temporary password is generated for root@localhost: %s", password);
    log_error_verbosity = saved_verbosity;

    escape_string_for_mysql(&my_charset_bin, escaped_password, sizeof(escaped_password), password,
...
int Compiled_in_command_iterator::next(std::string &query, int *read_error, int *query_source)
{
  if (init_file_iter)
  ...
  if (NULL != (dir = my_dir(data_home, MYF(MY_DONT_SORT))))
  {
    ...
    */
    for (uint i = 0; i < dir->number_off_files; i++)
    {
      ...
      if (file->name[0] != '.' && !is_in_ignore_db_dirs_list(file->name))
      {
        no_files = false;
        break;
      }
    ...
    if (NULL == fn_format(path, "is_writable", data_home, "", MY_UNPACK_FILENAME | MY_SAFE_PATH))
    {
      sql_print_error(
          "--initialize specified but the data directory"
