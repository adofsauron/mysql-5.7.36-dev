...
class Cmp_splocal_locations : public std::binary_function<const Item_splocal *, const Item_splocal *, bool>
{
 public:
...
static bool subst_spvars(THD *thd, sp_instr *instr, LEX_STRING *query_str)
{
  ...
  */
  ...
  for (Item_splocal **splocal = sp_vars_uses.begin(); splocal != sp_vars_uses.end(); splocal++)
  {
    Item *val;
...
class SP_instr_error_handler : public Internal_error_handler
{
 public:
                                ...
                                const char *)
  {
    ...
    */
    if (thd->lex && thd->lex->sql_command == SQLCOM_CREATE_TABLE && thd->lex->select_lex &&
        thd->lex->select_lex->item_list.elements > 0 && sql_errno == ER_TABLE_EXISTS_ERROR)
...
bool sp_lex_instr::reset_lex_and_exec_core(THD *thd, uint *nextp, bool open_tables)
{
  ...
  */
  ...
  unsigned int parent_unsafe_rollback_flags = thd->get_transaction()->get_unsafe_rollback_flags(Transaction_ctx::STMT);
  ...
  */
  ...
  if (thd->get_protocol()->has_client_capability(CLIENT_SESSION_TRACK) && thd->session_tracker.enabled_any() &&
  ...
  if (!error)
  {
    if (open_tables)
    {
      ...
      */
      Opt_trace_start ots(thd, m_lex->query_tables, SQLCOM_SELECT, &m_lex->var_list, NULL, 0, this,
      ...
      */
      if (m_lex->query_tables)
        error = (open_temporary_tables(thd, m_lex->query_tables) ||
                 check_table_access(thd, SELECT_ACL, m_lex->query_tables, false, UINT_MAX, false));
  ...
  */
  ...
  bool reprepare_error = error && thd->is_error() && thd->get_stmt_da()->mysql_errno() == ER_NEED_REPREPARE;
  ...
  */
  ...
  thd->get_transaction()->add_unsafe_rollback_flags(Transaction_ctx::STMT, parent_unsafe_rollback_flags);
...
LEX *sp_lex_instr::parse_expr(THD *thd, sp_head *sp)
{
  ...
  init_sql_alloc(PSI_NOT_INSTRUMENTED, &m_lex_mem_root, MEM_ROOT_BLOCK_SIZE, MEM_ROOT_PREALLOC);
  ...
  if (!parsing_failed)
  {
    ...
    if (sp->m_type == SP_TYPE_TRIGGER)
    {
      ...
      */
      ...
      for (Item_trigger_field *trg_fld = sp->m_cur_instr_trig_field_items.first; trg_fld;
           trg_fld = trg_fld->next_trg_field)
      {
        trg_fld->setup_field(thd, sp->m_trg_list->get_trigger_field_support(), t->get_subject_table_grant());
      }
...
bool sp_lex_instr::validate_lex_and_execute_core(THD *thd, uint *nextp, bool open_tables)
{
  ...
  while (true)
  {
    ...
    */
    ...
    if (!m_first_execution &&
        (sql_command_flags[m_lex->sql_command] & CF_REEXECUTION_FRAGILE || m_lex->sql_command == SQLCOM_END))
    {
    ...
    */
    if (stmt_reprepare_observer && !thd->is_fatal_error && !thd->killed &&
        thd->get_stmt_da()->mysql_errno() == ER_NEED_REPREPARE && stmt_reprepare_observer->can_retry())
    {
  ...
  */
  ...
  need_subst = !((thd->variables.option_bits & OPTION_LOG_OFF) &&
                 (!(thd->variables.option_bits & OPTION_BIN_LOG) || !mysql_bin_log.is_open() ||
                  (thd->is_current_stmt_binlog_format_row() && sqlcom_can_generate_row_events(m_lex->sql_command))));
  ...
  */
  ...
  if (query_cache.send_result_to_client(thd, thd->query()) <= 0)
  {
    ...
    /*
      ...
      problem.
    */
    assert((thd->query_name_consts == 0) || (thd->rewritten_query().length() == 0));
  }
  else
    *nextp = get_ip() + 1;
...
bool sp_instr_stmt::exec_core(THD *thd, uint *nextp)
{
  MYSQL_QUERY_EXEC_START(const_cast<char *>(thd->query().str), thd->thread_id(),
                         ...
                         (char *)thd->security_context()->host_or_ip().str, 3);
...
bool sp_instr_set_trigger_field::on_after_expr_parsing(THD *thd)
{
  ...
  m_trigger_field = new (thd->mem_root)
      Item_trigger_field(thd->lex->current_context(), TRG_NEW_ROW, m_trigger_field_name.str, UPDATE_ACL, false);

  if (m_trigger_field)
  {
    /* Adding m_trigger_field to the list of all Item_trigger_field objects */
    sp_head *sp = thd->sp_runtime_ctx->sp;
    sp->m_cur_instr_trig_field_items.link_in_list(m_trigger_field, &m_trigger_field->next_trg_field);
  }
...
bool sp_instr_set_case_expr::exec_core(THD *thd, uint *nextp)
{
  *nextp = get_ip() + 1;
  ...
  if (rctx->set_case_expr(thd, m_case_expr_id, &m_expr_item) && !rctx->get_case_expr(m_case_expr_id))
  {
