...
THD::Attachable_trx::Attachable_trx(THD *thd) : m_thd(thd)
{
...
THD::Attachable_trx::~Attachable_trx()
{
  ...
  if (DBUG_EVALUATE_IF("use_attachable_trx", false, true))
  {
    m_thd->lex->restore_backup_query_tables_list(&m_trx_state.m_query_tables_list);
  }
}
...
extern "C" uchar *get_var_key(user_var_entry *entry, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
...
{
  return length == other.length && !my_strcasecmp(system_charset_info, field_name.str, other.field_name.str);
}
...
bool Foreign_key::validate(List<Create_field> &table_fields)
{
  ...
  while ((column = cols++))
  {
    ...
    if (type == KEYTYPE_FOREIGN && sql_field->gcol_info)
    {
      ...
      {
        my_error(ER_WRONG_FK_OPTION_FOR_GENERATED_COLUMN, MYF(0), "ON DELETE SET NULL");
        DBUG_RETURN(TRUE);
      }
      if (update_opt == FK_OPTION_SET_NULL)
      {
        my_error(ER_WRONG_FK_OPTION_FOR_GENERATED_COLUMN, MYF(0), "ON UPDATE SET NULL");
        DBUG_RETURN(TRUE);
      }
      if (update_opt == FK_OPTION_CASCADE)
      {
        my_error(ER_WRONG_FK_OPTION_FOR_GENERATED_COLUMN, MYF(0), "ON UPDATE CASCADE");
        DBUG_RETURN(TRUE);
      }
    }
  }
...
extern "C" void thd_binlog_pos(const THD *thd, const char **file_var, unsigned long long *pos_var)
{
                                         ...
                                         const char *calling_file, const unsigned int calling_line)
{
  ...
  set_thd_stage_info(thd_arg, &old_stage, &new_stage, calling_function, calling_file, calling_line);

  return new_stage.m_name;
}

extern "C" void set_thd_stage_info(void *opaque_thd, const PSI_stage_info *new_stage, PSI_stage_info *old_stage,
...
void THD::enter_stage(const PSI_stage_info *new_stage, PSI_stage_info *old_stage, const char *calling_func,
                      const char *calling_file, const unsigned int calling_line)
{
  DBUG_PRINT("THD::enter_stage", ("'%s' %s:%d", new_stage ? new_stage->m_name : "", calling_file, calling_line));
...
extern "C" void thd_enter_cond(void *opaque_thd, mysql_cond_t *cond, mysql_mutex_t *mutex, const PSI_stage_info *stage,
...
extern "C" void thd_exit_cond(void *opaque_thd, const PSI_stage_info *stage, const char *src_function,
...
extern "C" void **thd_ha_data(const THD *thd, const struct handlerton *hton)
{
...
extern "C" THD *thd_tx_arbitrate(THD *requestor, THD *holder)
{
...
extern "C" char *thd_security_context(THD *thd, char *buffer, size_t length, size_t max_query_len)
{
  ...
  */
  ...
  len = my_snprintf(header, sizeof(header), "MySQL thread id %u, OS thread handle %lu, query id %lu", thd->thread_id(),
                    (ulong)thd->real_id, (ulong)thd->query_id);
  str.length(0);
      ...
      is_a_srv_session_thd(false)
{
  ...
  init_sql_alloc(key_memory_thd_main_mem_root, &main_mem_root, global_system_variables.query_alloc_block_size,
...
#endif
  m_user_connect = NULL;
  my_hash_init(&user_vars, system_charset_info, USER_VARS_HASH_SIZE, 0, 0, (my_hash_get_key)get_var_key,
               (my_hash_free_key)free_user_var, 0, key_memory_user_var_entry);
  ...
  */
  ...
  {
    m_token_array = (unsigned char *)my_malloc(PSI_INSTRUMENT_ME, max_digest_length, MYF(MY_WME));
  }
}
...
bool THD::handle_condition(uint sql_errno, const char *sqlstate, Sql_condition::enum_severity_level *level,
                           const char *msg)
{
  if (!m_internal_handler)
    return false;
...
Sql_condition *THD::raise_condition(uint sql_errno, const char *sqlstate, Sql_condition::enum_severity_level level,
                                    const char *msg, bool use_condition_handler)
{
  DBUG_ENTER("THD::raise_condition");

  if (!(variables.option_bits & OPTION_SQL_NOTES) && (level == Sql_condition::SL_NOTE))
    DBUG_RETURN(NULL);
  ...
  if (use_condition_handler && handle_condition(sql_errno, sqlstate, &level, msg))
    DBUG_RETURN(NULL);
  ...
  */
  ...
  if (!(is_fatal_error && (sql_errno == EE_OUTOFMEMORY || sql_errno == ER_OUTOFMEMORY)))
  {
...
void THD::init(void)
{
  ...
  */
  ...
  insert_lock_default = (variables.low_priority_updates ? TL_WRITE_LOW_PRIORITY : TL_WRITE_CONCURRENT_INSERT);
...
void THD::init_for_queries(Relay_log_info *rli)
{
  ...
  get_transaction()->init_mem_root_defaults(variables.trans_alloc_block_size, variables.trans_prealloc_size);
    ...
    */
    if (current_cond && current_mutex)
    {
      DBUG_EXECUTE_IF("before_dump_thread_acquires_current_mutex", {
        const char act[] = "now signal dump_thread_signal wait_for go_dump_thread";
...
void THD::disconnect(bool server_shutdown)
{
  ...
  */
  ...
  if (is_classic_protocol() && get_protocol_classic()->get_vio() != vio && get_protocol_classic()->connection_alive())
  {
...
void THD::notify_shared_lock(MDL_context_owner *ctx_in_use, bool needs_thr_lock_abort)
{
...
bool THD::store_globals()
{
  ...
  */
  ...
  if (my_thread_set_THR_THD(this) || my_thread_set_THR_MALLOC(&mem_root))
    return true;
  /*
...
LEX_CSTRING *make_lex_string_root(MEM_ROOT *mem_root, LEX_CSTRING *lex_str, const char *str, size_t length,
...
LEX_STRING *make_lex_string_root(MEM_ROOT *mem_root, LEX_STRING *lex_str, const char *str, size_t length,
                                 bool allocate_lex_string)
{
  if (allocate_lex_string)
...
LEX_CSTRING *THD::make_lex_string(LEX_CSTRING *lex_str, const char *str, size_t length, bool allocate_lex_string)
{
  return make_lex_string_root(mem_root, lex_str, str, length, allocate_lex_string);
}
...
*/
LEX_STRING *THD::make_lex_string(LEX_STRING *lex_str, const char *str, size_t length, bool allocate_lex_string)
{
  return make_lex_string_root(mem_root, lex_str, str, length, allocate_lex_string);
}
...
bool THD::convert_string(LEX_STRING *to, const CHARSET_INFO *to_cs, const char *from, size_t from_length,
                         const CHARSET_INFO *from_cs)
{
  ...
  to->length = copy_and_convert(to->str, new_length, to_cs, from, from_length, from_cs, &errors);
  to->str[to->length] = 0;  // Safety
  if (errors != 0)
  {
    char printable_buff[32];
    convert_to_printable(printable_buff, sizeof(printable_buff), from, from_length, from_cs, 6);
    push_warning_printf(this, Sql_condition::SL_WARNING, ER_INVALID_CHARACTER_STRING,
...
bool THD::convert_string(String *s, const CHARSET_INFO *from_cs, const CHARSET_INFO *to_cs)
{
  uint dummy_errors;
...
int THD::send_explain_fields(Query_result *result)
{
  ...
  item->maybe_null = 1;
  field_list.push_back(item = new Item_empty_string("key_len", NAME_CHAR_LEN * MAX_KEY));
  item->maybe_null = 1;
  ...
  item->maybe_null = 1;
  field_list.push_back(item = new Item_float(NAME_STRING("filtered"), 0.1234, 2, 4));
  item->maybe_null = 1;
  field_list.push_back(new Item_empty_string("Extra", 255, cs));
  item->maybe_null = 1;
  return (result->send_result_set_metadata(field_list, Protocol::SEND_NUM_ROWS | Protocol::SEND_EOF));
}
...
void THD::replace_rollback_place(Item **new_place)
{
  ...
  while ((change = it++))
  {
    ...
    {
      DBUG_PRINT("info", ("replace_rollback_place new_value %p place %p", *new_place, new_place));
    ...
    : file_name(name), dumpfile(flag), skip_lines(0)
{
  ...
  line.line_term = filetype == FILETYPE_CSV ? &default_line_term : &default_xml_row_term;
...
static File create_file(THD *thd, char *path, sql_exchange *exchange, IO_CACHE *cache)
{
  ...
  {
    strxnmov(path, FN_REFLEN - 1, mysql_real_data_home, thd->db().str ? thd->db().str : "", NullS);
  ...
  if ((file = mysql_file_create(key_select_to_file, path, 0666, O_WRONLY | O_EXCL, MYF(MY_WME))) < 0)
    return file;
#ifdef HAVE_FCHMOD
...
int Query_result_export::prepare(List<Item> &list, SELECT_LEX_UNIT *u)
{
  ...
  if (exchange->field.escaped->numchars() > 1 || exchange->field.enclosed->numchars() > 1)
  {
    my_error(ER_WRONG_FIELD_TERMINATORS, MYF(0));
    return TRUE;
  }
  if (exchange->field.escaped->length() > 1 || exchange->field.enclosed->length() > 1 ||
      !my_isascii(exchange->field.escaped->ptr()[0]) || !my_isascii(exchange->field.enclosed->ptr()[0]) ||
      !exchange->field.field_term->is_ascii() || !exchange->line.line_term->is_ascii() ||
      !exchange->line.line_start->is_ascii())
  {
    ...
    */
    push_warning(thd, Sql_condition::SL_WARNING, WARN_NON_ASCII_SEPARATOR_NOT_IMPLEMENTED,
                 ER(WARN_NON_ASCII_SEPARATOR_NOT_IMPLEMENTED));
  }
  field_term_length = exchange->field.field_term->length();
  field_term_char = field_term_length ? (int)(uchar)(*exchange->field.field_term)[0] : INT_MAX;
  if (!exchange->line.line_term->length())
    exchange->line.line_term = exchange->field.field_term;  // Use this if it exists
  field_sep_char = (exchange->field.enclosed->length() ? (int)(uchar)(*exchange->field.enclosed)[0] : field_term_char);
  ...
  line_sep_char = (exchange->line.line_term->length() ? (int)(uchar)(*exchange->line.line_term)[0] : INT_MAX);
  ...
  if ((is_ambiguous_field_sep && exchange->field.enclosed->is_empty() && (string_results || is_unsafe_field_sep)) ||
      (exchange->field.opt_enclosed && non_string_results && field_term_length &&
       strchr(NUMERIC_CHARS, field_term_char)))
  {
    push_warning(thd, Sql_condition::SL_WARNING, ER_AMBIGUOUS_FIELD_TERM, ER(ER_AMBIGUOUS_FIELD_TERM));
...
bool Query_result_export::send_data(List<Item> &items)
{
  ...
  if (my_b_write(&cache, (uchar *)exchange->line.line_start->ptr(), exchange->line.line_start->length()))
    goto err;
  while ((item = li++))
  {
    Item_result result_type = item->result_type();
    bool enclosed =
        (exchange->field.enclosed->length() && (!exchange->field.opt_enclosed || result_type == STRING_RESULT));
    ...
    if (res && !my_charset_same(write_cs, res->charset()) && !my_charset_same(write_cs, &my_charset_bin))
    {
      ...
      bytes = well_formed_copy_nchars(write_cs, (char *)cvt_str.ptr(), cvt_str.alloced_length(), res->charset(),
                                      res->ptr(), res->length(),
      ...
      {
        char printable_buff[32];
        convert_to_printable(printable_buff, sizeof(printable_buff), error_pos, res->ptr() + res->length() - error_pos,
                             res->charset(), 6);
        push_warning_printf(thd, Sql_condition::SL_WARNING, ER_TRUNCATED_WRONG_VALUE_FOR_FIELD,
                            ER(ER_TRUNCATED_WRONG_VALUE_FOR_FIELD), "string", printable_buff, item->item_name.ptr(),
      ...
      else if (from_end_pos < res->ptr() + res->length())
      {
        ...
        */
        push_warning_printf(thd, Sql_condition::SL_WARNING, WARN_DATA_TRUNCATED, ER(WARN_DATA_TRUNCATED),
    ...
    if (res && enclosed)
    {
      if (my_b_write(&cache, (uchar *)exchange->field.enclosed->ptr(), exchange->field.enclosed->length()))
        goto err;
    }
    if (!res)
    {  // NULL
    ...
    else
    {
      ...
      if ((result_type == STRING_RESULT || is_unsafe_field_sep) && escape_char != -1)
      {
        ...
        const CHARSET_INFO *character_set_client = thd->variables.character_set_client;
        ...
        */
        assert(character_set_client->mbmaxlen == 2 || my_mbmaxlenlen(character_set_client) == 2 ||
        ...
        for (start = pos = (char *)res->ptr(), end = pos + used_length; pos != end; pos++)
        {
          ...
          */
          ...
          {
            need_escape = true;
            if (my_mbmaxlenlen(character_set_client) == 2 && my_mbcharlen_ptr(character_set_client, pos, end) == 4)
            {
          ...
          else if (check_following_byte)
          {
            ...
            if (len == 2 && pos + 1 < end && NEED_ESCAPING(pos[1]))
              need_escape = true;
            else if (len == 4 && my_mbmaxlenlen(character_set_client) == 2 && pos + 3 < end)
            {
              in_escapable_4_bytes = 4;
              escape_4_bytes = (NEED_ESCAPING(pos[1]) || NEED_ESCAPING(pos[2]) || NEED_ESCAPING(pos[3]));
              need_escape = escape_4_bytes;
            }
          }
    ...
    if (--items_left)
    {
      if (my_b_write(&cache, (uchar *)exchange->field.field_term->ptr(), field_term_length))
        goto err;
    }
  }
  if (my_b_write(&cache, (uchar *)exchange->line.line_term->ptr(), exchange->line.line_term->length()))
    goto err;
  DBUG_RETURN(0);
err:
  DBUG_RETURN(1);
...
int Query_result_dump::prepare(List<Item> &list MY_ATTRIBUTE((unused)), SELECT_LEX_UNIT *u)
{
  unit = u;
...
bool Query_result_dump::send_data(List<Item> &items)
{
  ...
  while ((item = li++))
  {
    ...
    else if (my_b_write(&cache, (uchar *)res->ptr(), res->length()))
    {
      ...
      my_error(ER_ERROR_ON_WRITE, MYF(0), path, my_errno(), my_strerror(errbuf, sizeof(errbuf), my_errno()));
      goto err;
    }
  }
  DBUG_RETURN(0);
...
Prepared_statement_map::Prepared_statement_map() : m_last_found_statement(NULL)
{
  enum
  {
  ...
  my_hash_init(&st_hash, &my_charset_bin, START_STMT_HASH_SIZE, 0, 0, get_statement_id_as_hash_key,
               delete_statement_as_hash_key, MYF(0), key_memory_prepared_statement_map);
  my_hash_init(&names_hash, system_charset_info, START_NAME_HASH_SIZE, 0, 0, (my_hash_get_key)get_stmt_name_hash_key,
...
{
  return reinterpret_cast<Prepared_statement *>(my_hash_search(&names_hash, (uchar *)name.str, name.length));
}
...
Prepared_statement *Prepared_statement_map::find(ulong id)
{
  if (m_last_found_statement == NULL || id != m_last_found_statement->id)
  {
    Prepared_statement *stmt =
        reinterpret_cast<Prepared_statement *>(my_hash_search(&st_hash, (uchar *)&id, sizeof(id)));
...
void Prepared_statement_map::reset()
{
  ...
  if (st_hash.records > 0)
  {
    ...
    {
      Prepared_statement *stmt = reinterpret_cast<Prepared_statement *>(my_hash_element(&st_hash, i));
...
bool Query_dumpvar::send_data(List<Item> &items)
{
  ...
  while ((mv = var_li++) && (item = it++))
  {
    ...
    else
    {
       ...
       */
      Item_func_set_user_var *suv = new Item_func_set_user_var(mv->name, item, true);
...
{
  if (!row_count)
    push_warning(thd, Sql_condition::SL_WARNING, ER_SP_FETCH_NO_DATA, ER(ER_SP_FETCH_NO_DATA));
  /*
    Don't send EOF if we're in error condition (which implies we've already
    sent or are sending an error)
  */
...
void THD::restore_backup_open_tables_state(Open_tables_backup *backup)
{
  ...
  */
  assert(open_tables == 0 && temporary_tables == 0 && derived_tables == 0 && lock == 0 &&
         locked_tables_mode == LTM_NONE && get_reprepare_observer() == NULL);
...
{
  if ((thd->variables.option_bits & OPTION_ALLOW_BATCH) || (thd->slave_thread && opt_slave_allow_batching))
    return 1;
  return 0;
}
...
{
  MYSQL_CALLBACK(Connection_handler_manager::event_functions, thd_wait_begin, (thd, wait_type));
}
...
{
  MYSQL_CALLBACK(Connection_handler_manager::event_functions, thd_wait_end, (thd));
}
#else
extern "C" void thd_wait_begin(MYSQL_THD thd, int wait_type)
{
...
/**
   ...
   called.
*/
extern "C" void thd_report_row_lock_wait(THD *self, THD *wait_for)
{
  DBUG_ENTER("thd_report_row_lock_wait");

  if (self != NULL && wait_for != NULL && is_mts_worker(self) && is_mts_worker(wait_for))
...
void THD::restore_sub_statement_state(Sub_statement_state *backup)
{
  ...
  */
  ...
  if (is_current_stmt_binlog_row_enabled_with_write_set_extraction())
  {
    get_transaction()->get_transaction_write_set_ctx()->restore_savepoint_list();
  }
...
#ifdef HAVE_PSI_STATEMENT_INTERFACE
  PSI_STATEMENT_CALL(inc_statement_sort_rows)(m_statement_psi, static_cast<ulong>(count));
#endif
}
...
void THD::set_next_event_pos(const char *_filename, ulonglong _pos)
{
  ...
  if (filename == NULL)
  {
    ...
    filename = (char *)my_malloc(key_memory_LOG_POS_COORD, FN_REFLEN + 1, MYF(MY_WME));
    if (filename == NULL)
      return;
  }
...
void THD::set_currently_executing_gtid_for_slave_thread()
{
  ...
  */
  if (system_thread == SYSTEM_THREAD_SLAVE_SQL || system_thread == SYSTEM_THREAD_SLAVE_WORKER)
...
void THD::Query_plan::set_query_plan(enum_sql_command sql_cmd, LEX *lex_arg, bool ps)
{
  assert(current_thd == thd);
...
{
  uint lineno = location.raw.start ? m_parser_state->m_lip.get_lineno(location.raw.start) : 1;
  const char *pos = location.raw.start ? location.raw.start : "";
  ErrConvString err(pos, variables.character_set_client);
  my_printf_error(ER_PARSE_ERROR, ER(ER_PARSE_ERROR), MYF(0), s ? s : ER(ER_SYNTAX_ERROR), err.ptr(), lineno);
}
...
bool THD::send_result_metadata(List<Item> *list, uint flags)
{
  ...
  if (m_protocol->start_result_metadata(list->elements, flags, variables.character_set_results))
    goto err;
...
void THD::send_statement_status()
{
  ...
  switch (da->status())
  {
    ...
    case Diagnostics_area::DA_OK:
      error = m_protocol->send_ok(server_status, da->last_statement_cond_count(), da->affected_rows(),
...
{
#ifdef HAVE_REPLICATION
  Relay_log_info *rli = is_binlog_applier() ? rli_fake : (slave_thread ? rli_slave : NULL);
...
void THD::rpl_reattach_engine_ha_data()
{
  ...
  if (rli)
    rli->reattach_engine_ha_data(this);
#endif
}
...
{
  return ((variables.transaction_write_set_extraction != HASH_ALGORITHM_OFF) && is_current_stmt_binlog_format_row() &&
