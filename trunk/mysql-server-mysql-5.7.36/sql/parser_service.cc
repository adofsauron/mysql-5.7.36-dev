...
class Plugin_error_handler : public Internal_error_handler
{
 ...
 public:
  ...
  virtual bool handle_condition(THD *thd, uint sql_errno_u, const char *sqlstate, Sql_condition::enum_severity_level *,
                                const char *msg)
  {
...
extern "C" void *parser_service_start_routine(void *arg)
{
...
extern "C" void mysql_parser_start_thread(THD *thd, callback_function fun, void *arg, my_thread_handle *thread_handle)
{
  ...
  mysql_thread_create(key_thread_parser_service, &handle, &attr, parser_service_start_routine, args);
...
extern "C" int mysql_parser_parse(MYSQL_THD thd, const MYSQL_LEX_STRING query, unsigned char is_prepared,
                                  sql_condition_handler_function handle_condition, void *condition_handler_state)
{
  /*
  ...
  */
  ...
  Plugin_error_handler error_handler(thd, handle_condition, condition_handler_state);
...
extern "C" int mysql_parser_get_statement_type(MYSQL_THD thd)
{
  LEX *lex = thd->lex;
...
extern "C" int mysql_parser_get_statement_digest(MYSQL_THD thd, uchar *digest)
{
...
extern "C" int mysql_parser_visit_tree(MYSQL_THD thd, parse_node_visit_function processor, unsigned char *arg)
{
  Service_visitor visitor(processor, arg);
  return thd->lex->accept(&visitor);
}

extern "C" MYSQL_LEX_STRING mysql_parser_item_string(MYSQL_ITEM item)
{
  String str;
...
extern "C" MYSQL_LEX_STRING mysql_parser_get_normalized_query(MYSQL_THD thd)
{
  String normalized_query = thd->normalized_query();

  MYSQL_LEX_STRING str = {const_cast<char *>(normalized_query.ptr()), normalized_query.length()};
  return str;
}
