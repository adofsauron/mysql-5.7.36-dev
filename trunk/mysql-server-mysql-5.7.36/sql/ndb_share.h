...
struct NDB_SHARE
{
  ...
  static NDB_SHARE *create(const char *key, struct TABLE *table);
...
inline NDB_SHARE_STATE get_ndb_share_state(NDB_SHARE *share)
{
  NDB_SHARE_STATE state;
...
inline void set_ndb_share_state(NDB_SHARE *share, NDB_SHARE_STATE state)
{
  native_mutex_lock(&share->mutex);
  share->state = state;
  native_mutex_unlock(&share->mutex);
...
#define NSF_BINLOG_USE_UPDATE                   \
  16u /* table update should be binlogged using \
        update log event */
#define NSF_BINLOG_MINIMAL_UPDATE               \
  32u /* table update should be binlogged using \
...
inline my_bool get_binlog_nologging(NDB_SHARE *share) { return (share->flags & NSF_NO_BINLOG) != 0; }
...
inline my_bool get_binlog_full(NDB_SHARE *share) { return (share->flags & NSF_BINLOG_FULL) != 0; }
...
inline my_bool get_binlog_use_update(NDB_SHARE *share) { return (share->flags & NSF_BINLOG_USE_UPDATE) != 0; }
...
NDB_SHARE *ndbcluster_get_share(const char *key, struct TABLE *table, bool create_if_not_exists, bool have_lock);
NDB_SHARE *ndbcluster_get_share(NDB_SHARE *share);
void ndbcluster_free_share(NDB_SHARE **share, bool have_lock);
void ndbcluster_real_free_share(NDB_SHARE **share);
int handle_trailing_share(THD *thd, NDB_SHARE *share);
int ndbcluster_rename_share(THD *thd, NDB_SHARE *share, struct NDB_SHARE_KEY *new_key);
void ndbcluster_mark_share_dropped(NDB_SHARE *);
inline NDB_SHARE *get_share(const char *key, struct TABLE *table, bool create_if_not_exists = TRUE,
                            bool have_lock = FALSE)
{
  return ndbcluster_get_share(key, table, create_if_not_exists, have_lock);
}

inline NDB_SHARE *get_share(NDB_SHARE *share) { return ndbcluster_get_share(share); }

inline void free_share(NDB_SHARE **share, bool have_lock = FALSE) { ndbcluster_free_share(share, have_lock); }
...
class Ndb_share_temp_ref
{
 ...
 public:
  Ndb_share_temp_ref(const char *key)
  {
    ...
    DBUG_PRINT("NDB_SHARE", ("%s temporary  use_count: %u", m_share->key_string(), m_share->use_count));
  }
  ...
  ~Ndb_share_temp_ref()
  {
    ...
    DBUG_PRINT("NDB_SHARE", ("%s temporary free  use_count: %u", m_share->key_string(), m_share->use_count));
...
#define dbug_print_share(t, s)                       \
  DBUG_LOCK_FILE;                                    \
  DBUG_EXECUTE("info", (s)->print((t), DBUG_FILE);); \
  DBUG_UNLOCK_FILE;
