...
static enum_serialization_result serialize_json_value(const Json_dom *dom, size_t type_pos, String *dest, size_t depth,
                                                      bool small_parent);
...
*/
static bool append_offset_or_size(String *dest, size_t offset_or_size, bool large)
{
  if (large)
...
/**
  ...
  @return  false on success, true on error
*/
static bool read_variable_length(const char *data, size_t data_length, size_t *length, size_t *num)
{
  /*
...
*/
static bool should_inline_value(const Json_dom *value, bool large, int32 *inlined_val, uint8 *inlined_type)
{
  switch (value->json_type())
  {
    ...
    case Json_dom::J_BOOLEAN:
      *inlined_val = (down_cast<const Json_boolean *>(value)->value()) ? JSONB_TRUE_LITERAL : JSONB_FALSE_LITERAL;
      *inlined_type = JSONB_TYPE_LITERAL;
      return true;
    case Json_dom::J_INT:
    {
...
*/
static enum_serialization_result append_value(String *dest, const Json_dom *value, size_t start_pos, size_t entry_pos,
...
/**
  ...
  @return serialization status
*/
static enum_serialization_result serialize_json_array(const Json_array *array, String *dest, bool large, size_t depth)
{
  ...
  const size_t entry_size = large ? VALUE_ENTRY_SIZE_LARGE : VALUE_ENTRY_SIZE_SMALL;
  ...
  for (uint32 i = 0; i < size; i++)
  {
    ...
    enum_serialization_result res = append_value(dest, elt, start_pos, entry_pos, large, depth + 1);
    if (res != OK)
      return res;
...
/**
  ...
  @return serialization status
*/
static enum_serialization_result serialize_json_object(const Json_object *object, String *dest, bool large,
                                                       size_t depth)
{
  const size_t start_pos = dest->length();
  ...
  const size_t key_entry_size = large ? KEY_ENTRY_SIZE_LARGE : KEY_ENTRY_SIZE_SMALL;
  const size_t value_entry_size = large ? VALUE_ENTRY_SIZE_LARGE : VALUE_ENTRY_SIZE_SMALL;
  ...
  */
  size_t offset = dest->length() + size * (key_entry_size + value_entry_size) - start_pos;
  ...
  for (Json_object::const_iterator it = object->begin(); it != object->end(); ++it)
  {
    ...
    if (append_offset_or_size(dest, offset, large) || append_int16(dest, static_cast<int16>(len)))
      return FAILURE; /* purecov: inspected */
    offset += len;
  }
  ...
  for (Json_object::const_iterator it = object->begin(); it != object->end(); ++it)
  {
    enum_serialization_result res = append_value(dest, it->second, start_pos, entry_pos, large, depth + 1);
    if (res != OK)
      return res;
                                                      ...
                                                      bool small_parent)
{
  ...
  switch (dom->json_type())
  {
    ...
    case Json_dom::J_STRING:
    {
      ...
      if (append_variable_length(dest, size) || dest->append(jstr->value().c_str(), size))
        return FAILURE; /* purecov: inspected */
    ...
    case Json_dom::J_BOOLEAN:
    {
      char c = (down_cast<const Json_boolean *>(dom)->value()) ? JSONB_TRUE_LITERAL : JSONB_FALSE_LITERAL;
      if (dest->append(c))
        return FAILURE; /* purecov: inspected */
      (*dest)[type_pos] = JSONB_TYPE_LITERAL;
      result = OK;
    ...
    case Json_dom::J_OPAQUE:
    {
      ...
      if (dest->append(static_cast<char>(o->type())) || append_variable_length(dest, o->size()) ||
  ...
  if (result == OK && dest->length() > current_thd->variables.max_allowed_packet)
  {
    my_error(ER_WARN_ALLOWED_PACKET_OVERFLOWED, MYF(0), "json_binary::serialize",
...
Value::Value(enum_type t)
    : m_type(t), m_field_type(), m_data(), m_element_count(), m_length(), m_int_value(), m_double_value(), m_large()
{
  assert(t == LITERAL_NULL || t == LITERAL_TRUE || t == LITERAL_FALSE || t == ERROR);
}

// Constructor for int and uint.
Value::Value(enum_type t, int64 val)
    : m_type(t), m_field_type(), m_data(), m_element_count(), m_length(), m_int_value(val), m_double_value(), m_large()
{
  assert(t == INT || t == UINT);
}
...
Value::Value(enum_type t, const char *data, size_t bytes, size_t element_count, bool large)
    : m_type(t),
      m_field_type(),
      m_data(data),
      ...
      for (size_t i = 0; i < element_count(); i++)
      {
        ...
        if (i > 0)
        {
          if (prev_key_len > curr_key_len)
            return false; /* purecov: inspected */
          if (prev_key_len == curr_key_len && (memcmp(prev_key, curr_key, curr_key_len) >= 0))
            return false; /* purecov: inspected */
        }
        prev_key = curr_key;
...
Value Value::element(size_t pos) const
{
  ...
  */
  size_t first_entry_offset = 2 * (m_large ? LARGE_OFFSET_SIZE : SMALL_OFFSET_SIZE);
  if (type() == OBJECT)
    first_entry_offset += m_element_count * (m_large ? KEY_ENTRY_SIZE_LARGE : KEY_ENTRY_SIZE_SMALL);

  const size_t entry_size = m_large ? VALUE_ENTRY_SIZE_LARGE : VALUE_ENTRY_SIZE_SMALL;
...
Value Value::key(size_t pos) const
{
  ...
  const size_t key_entry_size = m_large ? KEY_ENTRY_SIZE_LARGE : KEY_ENTRY_SIZE_SMALL;
  const size_t value_entry_size = m_large ? VALUE_ENTRY_SIZE_LARGE : VALUE_ENTRY_SIZE_SMALL;
  ...
  */
  if ((key_offset < entry_offset + (m_element_count - pos) * key_entry_size + m_element_count * value_entry_size) ||
...
Value Value::lookup(const char *key, size_t len) const
{
  ...
  const size_t entry_size = (m_large ? KEY_ENTRY_SIZE_LARGE : KEY_ENTRY_SIZE_SMALL);
...
bool Value::raw_binary(String *buf) const
{
  ...
  switch (m_type)
  {
    ...
    case ARRAY:
    {
      char tp = m_large ? (m_type == OBJECT ? JSONB_TYPE_LARGE_OBJECT : JSONB_TYPE_LARGE_ARRAY)
                        : (m_type == OBJECT ? JSONB_TYPE_SMALL_OBJECT : JSONB_TYPE_SMALL_ARRAY);
      return buf->append(tp) || buf->append(m_data, m_length);
    }
    case STRING:
      return buf->append(JSONB_TYPE_STRING) || append_variable_length(buf, m_length) || buf->append(m_data, m_length);
    case INT:
    {
    ...
    case OPAQUE:
      return buf->append(JSONB_TYPE_OPAQUE) || buf->append(field_type()) || append_variable_length(buf, m_length) ||
