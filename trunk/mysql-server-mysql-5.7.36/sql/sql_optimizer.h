...
class JOIN : public Sql_alloc
{
 ...
 public:
        ...
        plan_state(NO_PLAN)
  {
    ...
    for (ORDER *group = group_list; group; group = group->next) send_group_parts++;
  }
  ...
  */
  ...
  {
    return (do_send_rows && tmp_table_param.sum_func_count != 0 && group_list == NULL && !group_optimized_away &&
  ...
  */
  bool create_intermediate_table(QEP_TAB *tab, List<Item> *tmp_table_fields, ORDER_with_src &tmp_table_group,
  ...
  */
  ...
  ORDER *remove_const(ORDER *first_order, Item *cond, bool change_list, bool *simple_order, const char *clause_type);
...
bool optimize_cond(THD *thd, Item **conds, COND_EQUAL **cond_equal, List<TABLE_LIST> *join_list,
...
{
  return f->is_temporal() && !f->is_temporal_with_date() && v->is_temporal_with_date();
}
