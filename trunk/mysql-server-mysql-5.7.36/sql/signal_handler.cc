...
extern "C" void handle_fatal_signal(int sig)
{
  ...
  my_safe_printf_stderr(
      ...
      ((ulong)dflt_key_cache->key_cache_mem_size +
       (global_system_variables.read_buff_size + global_system_variables.sortbuff_size) * max_threads +
  ...
  if (!(test_flags & TEST_NO_STACKTRACE))
  {
    ...
    my_safe_printf_stderr("%s",
                          ...
                          "terribly wrong...\n");
    my_print_stacktrace(thd ? (uchar *)thd->thread_stack : NULL, my_thread_stack_size);
  }
  if (thd)
  {
