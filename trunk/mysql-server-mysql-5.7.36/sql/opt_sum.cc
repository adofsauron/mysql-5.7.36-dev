...
static bool find_key_for_maxmin(bool max_fl, TABLE_REF *ref, Item_field *item_field, Item *cond, uint *range_fl,
                                uint *key_prefix_length);
static int reckey_in_range(bool max_fl, TABLE_REF *ref, Item_field *item_field, Item *cond, uint range_fl,
                           uint prefix_len);
  ...
  */
  for (TABLE_LIST *tl = tables; tl; tl = tl->next_leaf)
  {
    ...
    */
    ...
    if ((tl->table->file->ha_table_flags() & HA_STATS_RECORDS_IS_EXACT) && table_filled)
    {
    ...
    {
      maybe_exact_count &= MY_TEST(table_filled && (tl->table->file->ha_table_flags() & HA_HAS_RECORDS));
      ...
      switch (item_sum->sum_func())
      {
        case Item_sum::COUNT_FUNC:
          ...
          */
              ...
              !force_index)
          {
            if (!is_exact_count)
            {
              ...
              */
              if (!thd->lex->describe && (count = get_exact_record_count(tables)) == ULLONG_MAX)
              {
          ...
          */
                   ...
                   (tables->table->file->ha_table_flags() & HA_CAN_FULLTEXT_EXT) &&  // 3
                   !((Item_sum_count *)item)->get_arg(0)->maybe_null)                // 4
          {
            Item_func_match *fts_item = static_cast<Item_func_match *>(conds);
            fts_item->get_master()->set_hints(NULL, FT_NO_RANKING, HA_POS_ERROR, false);
        ...
        case Item_sum::MAX_FUNC:
        {
          ...
          */
          ...
          if (expr->real_item()->type() == Item::FIELD_ITEM)
          {
            ...
            */
            if (table->file->inited || (outer_tables & item_field->table_ref->map()) ||
                !find_key_for_maxmin(is_max, &ref, item_field, conds, &range_fl, &prefix_len))
            {
              const_result = 0;
              break;
            }
            ...
            */
            ...
            table->mark_columns_used_by_index_no_reset(ref.key, table->read_set, ref.key_parts);
            ...
            */
            ...
            if (!error && reckey_in_range(is_max, &ref, item_field, conds, range_fl, prefix_len))
          ...
          item_sum->set_aggregator(item_sum->has_with_distinct() ? Aggregator::DISTINCT_AGGREGATOR
...
static bool matching_cond(bool max_fl, TABLE_REF *ref, KEY *keyinfo, KEY_PART_INFO *field_part, Item *cond,
                          table_map map, key_part_map *key_part_used, uint *range_fl, uint *prefix_len)
{
  ...
  if (cond->type() == Item::COND_ITEM)
  {
    ...
    while ((item = li++))
    {
      if (!matching_cond(max_fl, ref, keyinfo, field_part, item, map, key_part_used, range_fl, prefix_len))
        DBUG_RETURN(FALSE);
    }
    DBUG_RETURN(TRUE);
  }
      ...
      (is_field_part && (between || eq_type || max_fl == less_fl) && !cond->val_int()))
  {
    ...
    */
    ...
    else
    {
      ...
      */
      type_conversion_status retval = value->save_in_field_no_warnings(part->field, true);
                                ...
                                uint *prefix_len)
{
  ...
  for (keyinfo = table->key_info, keyinfo_end = keyinfo + table->s->keys; keyinfo != keyinfo_end; keyinfo++, idx++)
  {
    ...
    */
    ...
    uint jdx = 0;
    *prefix_len = 0;
    for (part = keyinfo->key_part, part_end = part + actual_key_parts(keyinfo); part != part_end;
         part++, jdx++, key_part_to_use = (key_part_to_use << 1) | 1)
    {
      ...
      if (field->eq(part->field))
      {
        ...
        *range_fl = NO_MIN_RANGE | NO_MAX_RANGE;
        if (matching_cond(max_fl, ref, keyinfo, part, cond, item_field->table_ref->map(), &key_part_used, range_fl,
                          prefix_len) &&
...
static int reckey_in_range(bool max_fl, TABLE_REF *ref, Item_field *item_field, Item *cond, uint range_fl,
                           uint prefix_len)
{
  if (key_cmp_if_same(item_field->field->table, ref->key_buff, ref->key, prefix_len))
    return 1;
