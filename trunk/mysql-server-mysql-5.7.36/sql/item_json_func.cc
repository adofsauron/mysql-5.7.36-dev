...
bool ensure_utf8mb4(String *val, String *buf, const char **resptr, size_t *reslength, bool require_string)
{
  ...
  if (my_charset_same(cs, &my_charset_utf8mb4_bin) || my_charset_same(cs, &my_charset_utf8_bin) ||
  ...
  {  // If not, we convert, possibly with loss (best effort).
    uint dummy_errors;
    if (buf->copy(val->ptr(), val->length(), val->charset(), &my_charset_utf8mb4_bin, &dummy_errors))
    {
...
*/
static bool parse_json(String *res, uint arg_idx, const char *func_name, Json_dom **dom, bool require_str_or_json,
                       bool *parse_error, bool preserve_neg_zero_int = false)
{
  ...
  *parse_error = false;
  ...
  if (ensure_utf8mb4(res, &utf8_res, &safep, &safe_length, require_str_or_json))
  {
    return true;
  }
  ...
  size_t err_offset;
  *dom = Json_dom::parse(safep, safe_length, &parse_err, &err_offset, preserve_neg_zero_int);
  ...
  if (*dom == NULL && parse_err != NULL)
  {
    ...
    */
    my_error(ER_INVALID_JSON_TEXT_IN_PARAM, MYF(0), arg_idx + 1, func_name, parse_err, err_offset, "");
    *parse_error = true;
  }
  return *dom == NULL;
}
...
bool get_json_string(Item *arg_item, String *value, String *utf8_res, const char *func_name, const char **safep,
                     size_t *safe_length)
{
...
*/
static bool json_is_valid(Item **args, uint arg_idx, String *value, const char *func_name, Json_dom **dom,
                          bool require_str_or_json, bool *valid, bool preserve_neg_zero_int = false)
{
  ...
  switch (get_normalized_field_type(arg_item))
  {
    ...
    case MYSQL_TYPE_TINY_BLOB:
    {
      ...
      const bool failure =
          parse_json(res, arg_idx, func_name, dom, require_str_or_json, &parse_error, preserve_neg_zero_int);
      *valid = !failure;
      return parse_error;
    }
    default:
...
*/
static bool parse_path(Item *path_expression, String *value, bool forbid_wildcards, Json_path *json_path)
{
...
*/
enum_one_or_all_type parse_one_or_all(const char *candidate, const char *func_name)
{
...
*/
enum_one_or_all_type parse_and_cache_ooa(Item *arg, String *string_value, enum_one_or_all_type *cached_ooa,
...
type_conversion_status Item_json_func::save_in_field_inner(Field *field, bool no_conversions)
{
...
*/
static bool contains_wr(const Json_wrapper &doc_wrapper, const Json_wrapper &containee_wr, bool *result)
{
  if (doc_wrapper.type() == Json_dom::J_OBJECT)
  {
    ...
    {
      for (; !d_oi.empty() && cmp(d_oi.elt().first, c_oi.elt().first); d_oi.next())
      {
      }
  ...
  if (doc_wrapper.type() == Json_dom::J_ARRAY)
  {
    ...
    for (size_t c_i = 0; c_i < c.size(); c_i++)
    {
      ...
      {
        while (doc_i < d.size() && doc_wrapper[d[doc_i]].type() < candt)
        {
          doc_i++;
        }
        ...
        */
        for (size_t tmp = doc_i; tmp < d.size() && doc_wrapper[d[tmp]].type() == Json_dom::J_ARRAY; tmp++)
        {
      ...
      else
      {
        ...
        while (tmp < d.size())
        {
          if (doc_wrapper[d[tmp]].type() == Json_dom::J_ARRAY || doc_wrapper[d[tmp]].type() == Json_dom::J_OBJECT)
          {
            if (contains_wr(doc_wrapper[d[tmp]], (*wr)[c[c_i]], result))
              return true; /* purecov: inspected */
            if (*result)
...
longlong Item_func_json_contains::val_int()
{
  ...
  try
  {
    ...
    if (get_json_wrapper(args, 0, &m_doc_value, func_name(), &doc_wrapper) || args[0]->null_value)
    {
      null_value = true;
      return 0;
    }
    ...
    if (get_json_wrapper(args, 1, &m_doc_value, func_name(), &containee_wr) || args[1]->null_value)
    {
      null_value = true;
      return 0;
    }
...
longlong Item_func_json_contains_path::val_int()
{
  ...
  {
    // arg 0 is the document
    if (get_json_wrapper(args, 0, &m_doc_value, func_name(), &wrapper) || args[0]->null_value)
    {
      null_value = true;
      return 0;
    }
    ...
    bool require_all;
    switch (parse_and_cache_ooa(args[1], &m_one_or_all_value, &m_cached_ooa, func_name()))
    {
      case ooa_all:
      {
        require_all = true;
                      ...
                      bool preserve_neg_zero_int)
{
  ...
  */
  ...
  bool valid;
  if (json_is_valid(args, arg_idx, str, func_name, &dom, true, &valid, preserve_neg_zero_int))
    return true;
...
String *Item_func_json_type::val_str(String *)
{
  ...
  {
    Json_wrapper wr;
    if (get_json_wrapper(args, 0, &m_value, func_name(), &wr) || args[0]->null_value)
    {
      null_value = true;
      return NULL;
    }
...
{
  boost::variant<boost::blank, Json_string, Json_decimal, Json_int, Json_uint, Json_double, Json_boolean, Json_null,
                 Json_datetime, Json_opaque>
      m_val;
};
...
*/
bool val_json_func_field_subselect(Item *arg, const char *calling_function, String *value, String *tmp,
                                   Json_wrapper *wr, Json_scalar_holder *scalar, bool accept_string)
{
  ...
  switch (field_type)
  {
    ...
    case MYSQL_TYPE_GEOMETRY:
    {
      ...
      bool retval = geometry_to_json(wr, arg, calling_function, INT_MAX32, false, false, false, &geometry_srid);
    ...
    case MYSQL_TYPE_YEAR:
    {
      ...
      {
        dom = new (std::nothrow) Json_opaque(field_type, oo->ptr(), oo->length());
        if (!dom)
          return true; /* purecov: inspected */
      }
      break;
    ...
    case MYSQL_TYPE_STRING:
    {
      ...
      */
      ...
      if (cs == &my_charset_bin)
      {
        ...
        {
          dom = new (std::nothrow) Json_opaque(field_type, res->ptr(), res->length());
          if (!dom)
            return true; /* purecov: inspected */
        }
        break;
  ...
  assert((scalar == NULL) || (get_json_scalar_from_holder(scalar) != NULL));
...
// see the contract for this function in item_json_func.h
bool get_json_atom_wrapper(Item **args, uint arg_idx, const char *calling_function, String *value, String *tmp,
                           Json_wrapper *wr, Json_scalar_holder *scalar, bool accept_string)
{
  bool result = false;
  ...
  try
  {
    ...
    */
    result = val_json_func_field_subselect(arg, calling_function, value, tmp, wr, scalar, accept_string);
  }
  catch (...)
  {
    /* purecov: begin inspected */
...
bool get_atom_null_as_null(Item **args, uint arg_idx, const char *calling_function, String *value, String *tmp,
                           Json_wrapper *wr)
{
  if (get_json_atom_wrapper(args, arg_idx, calling_function, value, tmp, wr, NULL, true))
    return true;
...
bool Item_json_typecast::val_json(Json_wrapper *wr)
{
  ...
  if (get_json_atom_wrapper(args, 0, func_name(), &m_value, &m_conversion_buffer, wr, NULL, true))
    return error_json();
...
bool Item_func_json_keys::val_json(Json_wrapper *wr)
{
  ...
  try
  {
    ...
    if (!res)
      return error_json(); /* purecov: inspected */
    for (Json_wrapper_object_iterator i(wrapper.object_iterator()); !i.empty(); i.next())
    {
    ...
    for (uint32 i = 1; i < arg_count; i += 2)
    {
      ...
      */
      for (Json_dom_vector::iterator it = hits.end(); it != hits.begin();)
      {
        --it;
        Json_wrapper valuew;
        if (get_atom_null_as_null(args, i + 1, func_name(), &m_value, &m_conversion_buffer, &valuew))
          return error_json();
        ...
        else
        {
          ...
          if (!arr || arr->append_clone(*it) || arr->append_alias(valuew.to_dom()))
          {
    ...
    for (uint32 i = 1; i < arg_count; i += 2)
    {
      ...
      */
      for (Json_dom_vector::iterator it = hits.end(); it != hits.begin();)
      {
        ...
        if (leg->get_type() == jpl_array_cell)
        {
          ...
          else if (leg->get_array_cell_index() > 0)
          {
            ...
            */
            ...
            if (!newarr || newarr->append_clone(a) /* auto-wrap this */ ||
                newarr->insert_clone(leg->get_array_cell_index(), valuew.to_dom()))
            {
        ...
        else if (leg->get_type() == jpl_member && (*it)->json_type() == Json_dom::J_OBJECT)
        {
  ...
  try
  {
    ...
    for (uint32 i = 1; i < arg_count; i += 2)
    {
      ...
      */
      ...
      if ((leg_count == 0) || (m_path.get_leg_at(leg_count - 1)->get_type() != jpl_array_cell))
      {
...
*/
static bool clone_without_autowrapping(Json_path *source_path, Json_path_clone *target_path, Json_dom *doc)
{
  Json_dom_vector hits(key_memory_JSON);
  ...
  for (size_t leg_idx = 0; leg_idx < leg_count; leg_idx++)
  {
    ...
    if ((path_leg->get_type() == jpl_array_cell) && (path_leg->get_array_cell_index() == 0))
    {
      /**
            ...
            else
            {
              ...
              */
              ...
              else
              {
                ...
                Json_array *newarr = new (std::nothrow) Json_array();
                if (!newarr || newarr->append_clone(a) ||
                    newarr->insert_clone(leg->get_array_cell_index(), valuew.to_dom()))
                {
                  delete newarr;       /* purecov: inspected */
                  return error_json(); /* purecov: inspected */
...
bool Item_func_json_array::val_json(Json_wrapper *wr)
{
  ...
  try
  {
    ...
    {
      Json_wrapper valuew;
      if (get_atom_null_as_null(args, i, func_name(), &m_value, &m_conversion_buffer, &valuew))
      {
        return error_json();
      }
  ...
  try
  {
    ...
    for (uint32 i = 0; i < arg_count; ++i)
    {
      ...
      */
      ...
      if (get_json_string(key_item, &tmp_key_value, &utf8_res, func_name(), &safep, &safe_length))
      {
      ...
      // value
      Json_wrapper valuew;
      if (get_atom_null_as_null(args, value_idx, func_name(), &m_value, &m_conversion_buffer, &valuew))
      {
        return error_json();
      }
...
bool Item_func_json_search::fix_fields(THD *thd, Item **items)
{
  ...
  m_like_node = new Item_func_like(m_source_string_item, like_string_item, default_escape, true);
  if (m_like_node == NULL)
    return true; /* purecov: inspected */
...
/**
   ...
   @retval true on failure
*/
static bool find_matches(const Json_wrapper &wrapper, Json_path *path, Json_dom_vector *matches, String_set *duplicates,
                         bool one_match, Item *like_node, Item_string *source_string)
{
  switch (wrapper.type())
  {
    case Json_dom::J_STRING:
    {
      ...
      if (like_node->val_int())
      {
        ...
        std::pair<String_set::iterator, bool> res = duplicates->insert_unique(string_contents);
    ...
    {
      for (Json_wrapper_object_iterator jwot(wrapper.object_iterator()); !jwot.empty(); jwot.next())
      {
        ...
        if (path->append(next_leg) ||
            find_matches(value, path, matches, duplicates, one_match, like_node, source_string))
          return true; /* purecov: inspected */
        path->pop();
...
bool Item_func_json_search::val_json(Json_wrapper *wr)
{
  ...
  try
  {
    ...
    {
      Json_path path;
      if (find_matches(docw, &path, &matches, &duplicates, one_match, m_like_node, m_source_string_item))
        return error_json(); /* purecov: inspected */
    }
      ...
      for (uint32 i = 4; i < arg_count; ++i)
      {
        ...
        */
        if (path->contains_wildcard_or_ellipsis())
        {
          ...
          for (Json_dom_vector::iterator jdvi = dom_hits.begin(); jdvi != dom_hits.end(); ++jdvi)
          {
            if (one_match && (matches.size() > 0))
            {
              break;
            ...
            if (find_matches(subdocument_wrapper, &subdocument_path, &matches, &duplicates, one_match, m_like_node,
        ...
        else  // no wildcards in the path
        {
          ...
          for (Json_wrapper_vector::iterator jwvi = hits.begin(); jwvi != hits.end(); ++jwvi)
          {
            if (one_match && (matches.size() > 0))
            {
              break;
            ...
            if (find_matches(subdocument_wrapper, path, &matches, &duplicates, one_match, m_like_node,
                             m_source_string_item))
              return error_json(); /* purecov: inspected */
          }                        // end of loop through hits
  ...
  else
  {
    ...
    if (!array)
      return error_json(); /* purecov: inspected */
    for (Json_dom_vector::iterator vsi = matches.begin(); vsi != matches.end(); ++vsi)
    {
...
String *Item_func_json_unquote::val_str(String *str)
{
  ...
  try
  {
    ...
    */
    ...
    if (res->length() < 2 || *res->ptr() != '"' || res->ptr()[res->length() - 1] != '"')
    {
      null_value = false;
