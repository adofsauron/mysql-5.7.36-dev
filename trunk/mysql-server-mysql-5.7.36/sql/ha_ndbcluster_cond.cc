...
static void ndb_serialize_cond(const Item *item, void *arg)
{
  ...
  if (context->supported)
  {
    ...
    if (rewrite_context2 && (rewrite_func_item = rewrite_context2->func_item) && rewrite_context2->count++ == 0)
    {
        ...
        case Item_func::IN_FUNC:
        {
          ...
          */
          if (context->expecting(item->type()))
          {
            ...
            {
              Item_func *func_item = (Item_func *)item;
              if ((func_item->functype() == Item_func::UNKNOWN_FUNC || func_item->functype() == Item_func::NEG_FUNC) &&
      ...
      else
      {
        ...
        switch (item->type())
        {
          case Item::FIELD_ITEM:
          {
            ...
            */
            if (context->table->s == field->table->s)
            {
              ...
              if (context->expecting(Item::FIELD_ITEM) && context->expecting_field_type(type) &&
                  context->expecting_max_length(field->field_length) &&
                  (context->expecting_field_result(field->result_type()) ||
                   // Date and year can be written as string or int
                   ((type == MYSQL_TYPE_TIME || type == MYSQL_TYPE_TIME2 || type == MYSQL_TYPE_DATE ||
                     type == MYSQL_TYPE_NEWDATE || type == MYSQL_TYPE_YEAR || type == MYSQL_TYPE_DATETIME ||
                  ...
                  type != MYSQL_TYPE_BLOB && type != MYSQL_TYPE_JSON && type != MYSQL_TYPE_GEOMETRY)
              {
                ...
                if (!context->expecting_nothing())
                {
                  ...
                  if (type == MYSQL_TYPE_TIME || type == MYSQL_TYPE_TIME2 || type == MYSQL_TYPE_DATE ||
                      type == MYSQL_TYPE_NEWDATE || type == MYSQL_TYPE_YEAR || type == MYSQL_TYPE_DATETIME ||
                      type == MYSQL_TYPE_DATETIME2)
                  {
                ...
                else
                {
                  ...
                  if ((field->result_type() == STRING_RESULT) &&
                      ...
                      type != MYSQL_TYPE_TIME2 && type != MYSQL_TYPE_DATE && type != MYSQL_TYPE_NEWDATE &&
                      type != MYSQL_TYPE_YEAR && type != MYSQL_TYPE_DATETIME && type != MYSQL_TYPE_DATETIME2)
                  {
            ...
            {
              DBUG_PRINT("info", ("Was not expecting field from table %s (%s)", context->table->s->table_name.str,
          ...
          case Item::FUNC_ITEM:
          {
            ...
            switch (func_item->functype())
            {
              ...
              case Item_func::UNKNOWN_FUNC:
              {
                 ...
                 */
                DBUG_PRINT("info", ("Function %s", func_item->const_item() ? "const" : ""));
                 ...
                 */
                if (context->rewrite_stack && context->rewrite_stack->left_hand_item &&
                    context->rewrite_stack->left_hand_item->type() != Item::FIELD_ITEM)
                {
          ...
          case Item::STRING_ITEM:
            ...
            if (context->expecting(Item::STRING_ITEM) && context->expecting_length(item->max_length))
            {
#ifndef NDEBUG
              char buff[256];
          ...
          case Item::INT_ITEM:
            ...
            {
              DBUG_PRINT("info", ("value %ld", (long)((Item_int *)item)->value));
              NDB_ITEM_QUALIFICATION q;
              q.value_type = Item::INT_ITEM;
              curr_cond->ndb_item = new Ndb_item(NDB_VALUE, q, item);
              if (!context->expecting_no_field_result())
          ...
          case Item::DECIMAL_ITEM:
            ...
            {
              DBUG_PRINT("info", ("value %f", ((Item_decimal *)item)->val_real()));
              NDB_ITEM_QUALIFICATION q;
              q.value_type = Item::DECIMAL_ITEM;
              curr_cond->ndb_item = new Ndb_item(NDB_VALUE, q, item);
              if (!context->expecting_no_field_result())
          ...
          default:
          {
            DBUG_PRINT("info", ("Found unsupported item of type %d", item->type()));
            context->supported = FALSE;
          }
        }
        if (pop)
...
/*
  Push a condition
 */
const Item *ha_ndbcluster_cond::cond_push(const Item *cond, TABLE *table, const NDBTAB *ndb_table)
{
...
bool ha_ndbcluster_cond::serialize_cond(const Item *cond, Ndb_cond_stack *ndb_cond, TABLE *table,
...
int ha_ndbcluster_cond::build_scan_filter_predicate(Ndb_cond *&cond, NdbScanFilter *filter, bool negated) const
{
    ...
    case NDB_FUNCTION:
    {
                        ...
                        : cond->ndb_item->qualification.function_type)
      {
        ...
        case NDB_LIKE_FUNC:
        {
          ...
          uint32 len =
              ((value->is_const_func() || value->is_cached()) && is_string) ? str.length() : value->pack_length();
          const char *val =
              ((value->is_const_func() || value->is_cached()) && is_string) ? str.ptr() : value->get_val();
          DBUG_PRINT("info", ("Generating LIKE filter: like(%d,%s,%d)", field->get_field_no(), val, len));
          if (filter->cmp(NdbScanFilter::COND_LIKE, field->get_field_no(), val, len) == -1)
            DBUG_RETURN(1);
          cond = cond->next->next->next;
          DBUG_RETURN(0);
        }
        case NDB_NOTLIKE_FUNC:
        {
          ...
          const char *val =
              ((value->is_const_func() || value->is_cached()) && is_string) ? str.ptr() : value->get_val();
          DBUG_PRINT("info", ("Generating NOTLIKE filter: notlike(%d,%s,%d)", field->get_field_no(),
...
int ha_ndbcluster_cond::build_scan_filter_group(Ndb_cond *&cond, NdbScanFilter *filter) const
{
  uint level = 0;
  bool negated = FALSE;
...
int ha_ndbcluster_cond::build_scan_filter(Ndb_cond *&cond, NdbScanFilter *filter) const
{
...
int ha_ndbcluster_cond::generate_scan_filter(NdbInterpretedCode *code, NdbScanOperation::ScanOptions *options) const
{
  ...
  if (m_cond_stack)
  {
    ...
    if (ret != 0)
    {
      ...
      if (err.code == NdbScanFilter::FilterTooLarge)
      {
        ...
        push_warning(current_thd, Sql_condition::SL_WARNING, err.code, err.message);
      }
      else
        DBUG_RETURN(ret);
    }
...
int ha_ndbcluster_cond::generate_scan_filter_from_cond(NdbScanFilter &filter) const
{
  ...
  for (Ndb_cond_stack *stack = m_cond_stack; (stack); stack = stack->next)
  {
...
*/
int ha_ndbcluster_cond::generate_scan_filter_from_key(NdbInterpretedCode *code, NdbScanOperation::ScanOptions *options,
                                                      ...
                                                      const key_range *end_key) const
{
  ...
  do
  {
    ...
    */
    if (start_key != 0 && start_key->flag == HA_READ_AFTER_KEY && end_key == 0 && key_info->user_defined_key_parts == 1)
    {
      const KEY_PART_INFO *key_part = key_info->key_part;
      if (key_part->null_bit != 0)  // nullable (must be)
      {
        ...
        {
          DBUG_PRINT("info", ("Generating ISNOTNULL filter for nullable %s", key_part->field->field_name));
    ...
    */
    if (start_key != 0 && start_key->flag == HA_READ_KEY_EXACT && end_key != 0 && end_key->flag == HA_READ_AFTER_KEY &&
        start_key->length == end_key->length && memcmp(start_key->key, end_key->key, start_key->length) == 0)
    {
      const KEY_PART_INFO *key_part = key_info->key_part;
      const uchar *ptr = start_key->key;
      for (uint i = 0; i < key_info->user_defined_key_parts; i++)
      {
        ...
        if (key_part->null_bit)  // nullable
        {
          if (ptr[0] != 0)  // null
          {
            DBUG_PRINT("info", ("Generating ISNULL filter for nullable %s", field->field_name));
            if (filter.isnull(key_part->fieldnr - 1) == -1)
              DBUG_RETURN(1);
          }
          else
          {
            DBUG_PRINT("info", ("Generating EQ filter for nullable %s", field->field_name));
        ...
        {
          DBUG_PRINT("info", ("Generating EQ filter for non-nullable %s", field->field_name));
          if (filter.cmp(NdbScanFilter::COND_EQ, key_part->fieldnr - 1, ptr, field->pack_length()) == -1)
            DBUG_RETURN(1);
        }
        ptr += key_part->store_length;
  ...
  if (m_cond_stack && (res = generate_scan_filter_from_cond(filter)))
    DBUG_RETURN(res);
