...
static const char *mod_type_name[] = {"", "insert", "update", "delete", "replace"};
...
static const char *list_names[SQ_total] = {
    K_SELECT_LIST_SUBQUERIES, K_UPDATE_VALUE_SUBQUERIES, K_HAVING_SUBQUERIES, K_OPTIMIZED_AWAY_SUBQUERIES, "",
    K_ORDER_BY_SUBQUERIES,    K_GROUP_BY_SUBQUERIES,
};
...
class subquery_ctx : virtual public context, public qep_row
{
 ...
 public:
  subquery_ctx(enum_parsing_context type_arg, const char *name_arg, context *parent_arg)
  ...
  */
  virtual bool cacheable() { return is_materialized_from_subquery ? is_cacheable : subquery->cacheable(); }
  virtual bool dependent() { return is_materialized_from_subquery ? is_dependent : subquery->dependent(); }
...
class table_base_ctx : virtual public context, virtual public qep_row
{
 ...
 public:
  virtual size_t id(bool hide) { return col_id.is_empty() ? 0 : col_id.value; }
...
static void print_filtered(char *buf, uint buf_len, double filtered) { my_snprintf(buf, buf_len, "%.2f", filtered); }
...
class union_result_ctx : public table_base_ctx, public unit_ctx
{
 ...
 public:
  ...
  virtual bool format_unit(Opt_trace_context *json) { return table_base_ctx::format_unit(json); }
...
class table_with_where_and_derived : public table_base_ctx
{
 public:
  ...
  table_with_where_and_derived(enum_parsing_context type_arg, const char *name_arg, context *parent_arg)
      : context(type_arg, name_arg, parent_arg), table_base_ctx(type_arg, name_arg, parent_arg)
  {
  }
...
class message_ctx : public joinable_ctx, public table_with_where_and_derived
{
 public:
  ...
  virtual size_t id(bool hide) { return table_with_where_and_derived::id(hide); }
  virtual bool cacheable() { return table_base_ctx::cacheable(); }
  virtual bool dependent() { return table_base_ctx::dependent(); }
  virtual qep_row *entry() { return table_base_ctx::entry(); }
  virtual bool format_derived(Opt_trace_context *json) { return table_with_where_and_derived::format_derived(json); }
  virtual bool format_where(Opt_trace_context *json) { return table_with_where_and_derived::format_where(json); }
...
class simple_sort_ctx : public joinable_ctx
{
 ...
 public:
  simple_sort_ctx(enum_parsing_context type_arg, const char *name_arg, context *parent_arg,
                  const Explain_format_flags *flags, Explain_sort_clause clause)
      : context(type_arg, name_arg, parent_arg),
  ...
  virtual bool find_and_set_derived(context *subquery) { return join_tab->find_and_set_derived(subquery); }
...
class simple_sort_with_subqueries_ctx : public simple_sort_ctx
{
 ...
 public:
  simple_sort_with_subqueries_ctx(enum_parsing_context type_arg, const char *name_arg, context *parent_arg,
                                  subquery_list_enum subquery_type_arg, const Explain_format_flags *flags,
 ...
 private:
  virtual bool format_body(Opt_trace_context *json, Opt_trace_object *obj)
  {
    return (simple_sort_ctx::format_body(json, obj) || (format_list(json, subqueries, list_names[subquery_type])));
  }
};
...
class join_ctx : public unit_ctx, virtual public qep_row
{
 ...
 public:
  join_ctx(enum_parsing_context type_arg, const char *name_arg, context *parent_arg)
      : context(type_arg, name_arg, parent_arg), unit_ctx(type_arg, name_arg, parent_arg), sort(0)
  {
  }

  virtual bool add_join_tab(joinable_ctx *ctx) { return join_tabs.push_back(ctx); }
...
class sort_ctx : public join_ctx
{
 ...
 public:
  sort_ctx(enum_parsing_context type_arg, const char *name_arg, context *parent_arg, const Explain_format_flags *flags,
           Explain_sort_clause clause)
      : context(type_arg, name_arg, parent_arg),
...
class sort_with_subqueries_ctx : public sort_ctx
{
 ...
 public:
  sort_with_subqueries_ctx(enum_parsing_context type_arg, const char *name_arg, context *parent_arg,
                           subquery_list_enum subquery_type_arg, const Explain_format_flags *flags,
                           Explain_sort_clause clause)
      : context(type_arg, name_arg, parent_arg),
 ...
 private:
  virtual bool format_body(Opt_trace_context *json, Opt_trace_object *obj)
  {
    return (sort_ctx::format_body(json, obj) || format_list(json, subqueries, list_names[subquery_type]));
  }
};
...
bool join_ctx::add_subquery(subquery_list_enum subquery_type, subquery_ctx *ctx)
{
  if (sort)
...
bool join_ctx::format_body(Opt_trace_context *json, Opt_trace_object *obj)
{
  ...
  if (join_tabs.elements &&
      (join_tabs.head()->get_mod_type() == MT_INSERT || join_tabs.head()->get_mod_type() == MT_REPLACE))
  {
...
bool join_ctx::format_nested_loop(Opt_trace_context *json)
{
  ...
  if (join_tabs.head()->get_mod_type() == MT_INSERT || join_tabs.head()->get_mod_type() == MT_REPLACE)
  {
    it++;
    join_tab_num--;
  }
...
bool union_result_ctx::format_body(Opt_trace_context *json, Opt_trace_object *obj)
{
  obj->add(K_USING_TMP_TABLE, true);
...
size_t join_ctx::id(bool hide) { return sort ? sort->id(hide) : get_id(join_tabs, hide); }

bool join_ctx::cacheable() { return sort ? sort->cacheable() : join_tabs.head()->cacheable(); }

bool join_ctx::dependent() { return sort ? sort->dependent() : join_tabs.head()->dependent(); }
...
class materialize_ctx : public joinable_ctx, public join_ctx, public table_base_ctx
{
 public:
  ...
  virtual bool add_join_tab(joinable_ctx *ctx) { return join_ctx::add_join_tab(ctx); }
  virtual int add_where_subquery(subquery_ctx *ctx, SELECT_LEX_UNIT *subquery)
  {
    return join_ctx::add_where_subquery(ctx, subquery);
  }
  virtual bool find_and_set_derived(context *subquery) { return join_ctx::find_and_set_derived(subquery); }
  virtual bool format_unit(Opt_trace_context *json) { return unit_ctx::format_unit(json); }
  virtual bool format_nested_loop(Opt_trace_context *json) { return join_ctx::format_nested_loop(json); }
...
class union_ctx : public unit_ctx
{
 ...
 public:
  ...
  virtual bool add_query_spec(context *ctx) { return query_specs.push_back(ctx); }
};
...
bool union_result_ctx::format(Opt_trace_context *json)
{
  ...
  if (!order_by_subqueries.is_empty() && format_list(json, order_by_subqueries, K_ORDER_BY_SUBQUERIES))
    return true; /* purecov: inspected */
...
bool Explain_format_JSON::begin_context(enum_parsing_context ctx, SELECT_LEX_UNIT *subquery,
  ...
  switch (ctx)
  {
    case CTX_JOIN:
      assert(current_context == NULL ||
             // subqueries:
             current_context->type == CTX_SELECT_LIST || current_context->type == CTX_UPDATE_VALUE_LIST ||
             current_context->type == CTX_DERIVED || current_context->type == CTX_OPTIMIZED_AWAY_SUBQUERY ||
             current_context->type == CTX_WHERE || current_context->type == CTX_HAVING ||
             current_context->type == CTX_ORDER_BY_SQ || current_context->type == CTX_GROUP_BY_SQ ||
             current_context->type == CTX_QUERY_SPEC);
      if ((current_context = new join_ctx(CTX_JOIN, K_QUERY_BLOCK, current_context)) == NULL)
        return true;
      break;
    case CTX_ORDER_BY:
    {
      assert(current_context->type == CTX_JOIN);
      sort_ctx *ctx = new sort_with_subqueries_ctx(CTX_ORDER_BY, K_ORDERING_OPERATION, current_context, SQ_ORDER_BY,
                                                   flags, ESC_ORDER_BY);
      if (ctx == NULL)
        return true;
    ...
    case CTX_GROUP_BY:
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_ORDER_BY ||
             current_context->type == CTX_DISTINCT);
      sort_ctx *ctx = new sort_with_subqueries_ctx(CTX_GROUP_BY, K_GROUPING_OPERATION, current_context, SQ_GROUP_BY,
                                                   flags, ESC_GROUP_BY);
      if (ctx == NULL)
        return true;
      current_context->set_sort(ctx);
    ...
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_ORDER_BY);
      sort_ctx *ctx = new sort_ctx(CTX_DISTINCT, K_DUPLICATES_REMOVAL, current_context, flags, ESC_DISTINCT);
      if (ctx == NULL)
        return true;
      current_context->set_sort(ctx);
      current_context = ctx;
    ...
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_ORDER_BY ||
             current_context->type == CTX_DISTINCT || current_context->type == CTX_GROUP_BY);
      sort_ctx *ctx = new sort_ctx(CTX_BUFFER_RESULT, K_BUFFER_RESULT, current_context, flags, ESC_BUFFER_RESULT);
      if (ctx == NULL)
        return true;
      current_context->set_sort(ctx);
      current_context = ctx;
    ...
    case CTX_QEP_TAB:
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_MATERIALIZATION ||
             current_context->type == CTX_DUPLICATES_WEEDOUT || current_context->type == CTX_GROUP_BY ||
             current_context->type == CTX_ORDER_BY || current_context->type == CTX_DISTINCT ||
             current_context->type == CTX_BUFFER_RESULT || current_context->type == CTX_SIMPLE_GROUP_BY ||
             current_context->type == CTX_SIMPLE_ORDER_BY || current_context->type == CTX_SIMPLE_DISTINCT);
    ...
    case CTX_SIMPLE_ORDER_BY:
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_MATERIALIZATION ||
             current_context->type == CTX_DUPLICATES_WEEDOUT || current_context->type == CTX_GROUP_BY ||
             current_context->type == CTX_ORDER_BY || current_context->type == CTX_BUFFER_RESULT ||
             current_context->type == CTX_DISTINCT);
      simple_sort_ctx *ctx = new simple_sort_with_subqueries_ctx(CTX_SIMPLE_ORDER_BY, K_ORDERING_OPERATION,
                                                                 current_context, SQ_ORDER_BY, flags, ESC_ORDER_BY);
    ...
    case CTX_SIMPLE_GROUP_BY:
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_MATERIALIZATION ||
             current_context->type == CTX_DUPLICATES_WEEDOUT || current_context->type == CTX_GROUP_BY ||
             current_context->type == CTX_ORDER_BY || current_context->type == CTX_DISTINCT ||
             current_context->type == CTX_BUFFER_RESULT || current_context->type == CTX_SIMPLE_ORDER_BY ||
             current_context->type == CTX_SIMPLE_DISTINCT);
      simple_sort_ctx *ctx = new simple_sort_with_subqueries_ctx(CTX_SIMPLE_GROUP_BY, K_GROUPING_OPERATION,
                                                                 current_context, SQ_GROUP_BY, flags, ESC_GROUP_BY);
      if (ctx == NULL || current_context->add_join_tab(ctx))
        return true;
      current_context = ctx;
      break;
    ...
    case CTX_SIMPLE_DISTINCT:
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_MATERIALIZATION ||
             ...
             current_context->type == CTX_BUFFER_RESULT || current_context->type == CTX_SIMPLE_ORDER_BY);
      simple_sort_ctx *ctx =
          new simple_sort_ctx(CTX_SIMPLE_DISTINCT, K_DUPLICATES_REMOVAL, current_context, flags, ESC_DISTINCT);
      if (ctx == NULL || current_context->add_join_tab(ctx))
        return true;
      current_context = ctx;
      break;
    ...
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_GROUP_BY ||
             current_context->type == CTX_ORDER_BY || current_context->type == CTX_DISTINCT ||
             current_context->type == CTX_BUFFER_RESULT || current_context->type == CTX_DUPLICATES_WEEDOUT);
    ...
    case CTX_DUPLICATES_WEEDOUT:
    {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_GROUP_BY ||
             current_context->type == CTX_ORDER_BY || current_context->type == CTX_DISTINCT ||
             current_context->type == CTX_BUFFER_RESULT || current_context->type == CTX_MATERIALIZATION);
      duplication_weedout_ctx *ctx = new duplication_weedout_ctx(current_context);
      if (ctx == NULL || current_context->add_join_tab(ctx))
        return true;
      current_context = ctx;
      break;
    ...
    {
      subquery_ctx *ctx = new subquery_ctx(CTX_SELECT_LIST, NULL, current_context);
    ...
    {
      subquery_ctx *ctx = new subquery_ctx(CTX_UPDATE_VALUE_LIST, NULL, current_context);
    ...
    case CTX_DERIVED:
    {
      current_context = new subquery_ctx(CTX_DERIVED, K_MATERIALIZED_FROM_SUBQUERY, current_context);
    ...
    {
      subquery_ctx *ctx = new subquery_ctx(CTX_OPTIMIZED_AWAY_SUBQUERY, NULL, current_context);
    ...
    {
      subquery_ctx *ctx = new subquery_ctx(CTX_ORDER_BY_SQ, NULL, current_context);
    ...
    {
      subquery_ctx *ctx = new subquery_ctx(CTX_GROUP_BY_SQ, NULL, current_context);
    ...
    {
      assert(current_context->type == CTX_UNION);
      subquery_ctx *ctx = new subquery_ctx(CTX_QUERY_SPEC, NULL, current_context);
...
bool Explain_format_JSON::end_context(enum_parsing_context ctx)
{
  ...
  if (current_context->parent == NULL)
  {
    ...
    if (!it.at_end())
    {
      Opt_trace_info info;
      it.get_value(&info);
      item = new Item_string(info.trace_ptr, static_cast<uint>(info.trace_length), system_charset_info);
    }
    else
#endif
      item = new Item_null();
    ...
    ret = (item == NULL || field_list.push_back(item) || output->send_data(field_list));
  }
