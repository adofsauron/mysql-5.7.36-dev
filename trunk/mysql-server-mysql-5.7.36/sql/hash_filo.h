...
class hash_filo
{
 ...
 public:
  ...
  hash_filo(PSI_memory_key psi_key, uint size, uint key_offset_arg, uint key_length_arg, my_hash_get_key get_key_arg,
  ...
  void clear(bool locked = 0)
  {
    ...
    last_link = NULL;
    my_hash_free(&cache);
    (void)my_hash_init(&cache, hash_charset, m_size, key_offset, key_length, get_key, free_element, 0, m_psi_key);
    if (!locked)
  ...
  hash_filo_element *search(uchar *key, size_t length)
  {
    ...
    hash_filo_element *entry = (hash_filo_element *)my_hash_search(&cache, key, length);
    if (entry)
    {  // Found; link it first
