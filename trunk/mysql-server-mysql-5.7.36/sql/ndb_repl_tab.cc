...
Ndb_rep_tab_key::Ndb_rep_tab_key(const char *_db, const char *_table_name, uint _server_id)
{
...
int Ndb_rep_tab_key::attempt_match(const char *keyptr, const uint keylen, const char *candidateptr,
                                   const uint candidatelen, const int exactmatchvalue)
{
  if (my_strnncoll(system_charset_info, (const uchar *)keyptr, keylen, (const uchar *)candidateptr, candidatelen) == 0)
  {
    /* Exact match */
    return exactmatchvalue;
  }
  else if (my_wildcmp(system_charset_info, keyptr, keyptr + keylen, candidateptr, candidateptr + candidatelen, '\\',
...
int Ndb_rep_tab_key::get_match_quality(const Ndb_rep_tab_key *key, const Ndb_rep_tab_key *candidate_row)
{
  /* 0= No match
     1= Loosest match
     8= Best match
  ...
  */
  ...
  int rc;
  if ((rc = attempt_match(&key->db[1], key->db[0], &candidate_row->db[1], candidate_row->db[0], EXACT_MATCH_DB)) == -1)
  {
  ...
  if ((rc = attempt_match(&key->table_name[1], key->table_name[0], &candidate_row->table_name[1],
                          candidate_row->table_name[0], EXACT_MATCH_TABLE_NAME)) == -1)
  {
    /* No match, drop out now */
    return 0;
  }
                                     ...
                                     const char **error_str)
{
  ...
  *error_str = NULL;
  ...
  const NdbDictionary::Column *col_db, *col_table_name, *col_server_id, *col_binlog_type, *col_conflict_fn;
  ...
  if (col_db == NULL || !col_db->getPrimaryKey() || col_db->getType() != NdbDictionary::Column::Varbinary)
    DBUG_RETURN(-1);
...
int Ndb_rep_tab_reader::scan_candidates(Ndb *ndb, const NdbDictionary::Table *reptab, const char *db,
                                        const char *table_name, uint server_id, Ndb_rep_tab_row &best_match)
{
  uint retries = 100;
  ...
  while (true)
  {
    ...
    if ((scanOp->readTuples(NdbScanOperation::LM_CommittedRead) != 0) ||
        ...
        (have_conflict_fn_col &&
         ((ra_conflict_fn_spec = scanOp->getValue(nrt_conflict_fn, (char *)row.conflict_fn_spec)) == NULL)))
    {
    ...
    DBUG_PRINT("info", ("Searching ndb_replication for %s.%s %u", db, table_name, server_id));
    ...
    while ((scan_rc = scanOp->nextResult(true)) == 0)
    {
      ...
      int match_quality = Ndb_rep_tab_key::get_match_quality(&searchkey, &row.key);
#ifndef NDEBUG
      {
        ...
        DBUG_PRINT("info", ("Candidate : %s.%s %u : %u %s"
                            " Match quality : %u.",
                            row.key.get_db(), row.key.get_table_name(), row.key.server_id, row.binlog_type,
      ...
      if (match_quality > 0)
      {
        if (match_quality == best_match_quality)
        {
          ...
          my_snprintf(warning_msg_buffer, sizeof(warning_msg_buffer),
                      ...
                      ndb_rep_db, ndb_replication_table, db, table_name, server_id, &best_match.key.db[1],
                      &best_match.key.table_name[1], best_match.key.server_id, &row.key.db[1], &row.key.table_name[1],
                      row.key.server_id);
                               ...
                               const char *db, const char *table_name, uint server_id)
{
  ...
  do
  {
    if (reptab == NULL)
    {
      if (dict->getNdbError().classification == NdbError::SchemaError || dict->getNdbError().code == 4009)
      {
    ...
    if ((error = check_schema(reptab, dict, &error_str)) != 0)
    {
      DBUG_PRINT("info", ("check_schema failed : %u, error_str : %s", error, error_str));
      break;
    }

    Ndb_rep_tab_row best_match_row;

    int best_match_quality = scan_candidates(ndb, reptab, db, table_name, server_id, best_match_row);
  ...
  else
  {
    ...
    {
      case -1:
        my_snprintf(warning_msg_buffer, sizeof(warning_msg_buffer), "Missing or wrong type for column '%s'", error_str);
        break;
      case -2:
  ...
  DBUG_PRINT("info", ("Rc : %d Retrieved Binlog flags : %u and function spec : %s", error, binlog_flags,
                      (conflict_fn_spec != NULL ? conflict_fn_spec : "NULL")));
