...
static bool find_db_tables_and_rm_known_files(THD *thd, MY_DIR *dirp, const char *db, const char *path,
                                              TABLE_LIST **tables, bool *found_other_files);

long mysql_rm_arc_files(THD *thd, MY_DIR *dirp, const char *org_path);
static my_bool rm_dir_w_symlink(const char *org_path, my_bool send_error);
static void mysql_change_db_impl(THD *thd, const LEX_CSTRING &new_db_name, ulong new_db_access,
...
uchar *dboptions_get_key(my_dbopt_t *opt, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
...
static inline int write_to_binlog(THD *thd, char *query, size_t q_len, const char *db, size_t db_len)
{
...
static PSI_rwlock_info all_database_names_rwlocks[] = {{&key_rwlock_LOCK_dboptions, "LOCK_dboptions", PSI_FLAG_GLOBAL}};
...
bool my_dboptions_cache_init(void)
{
  ...
  if (!dboptions_init)
  {
    ...
    */
    error = my_hash_init(&dboptions, lower_case_table_names ? system_charset_info : &my_charset_bin, 32, 0, 0,
                         (my_hash_get_key)dboptions_get_key, free_dbopt, 0, key_memory_dboptions_hash);
  }
  return error;
}
...
void my_dbopt_cleanup(void)
{
  ...
  /*
    ...
    (my_charset_bin), for 1 and 2 - case insensitive (system_charset_info).
  */
  my_hash_init(&dboptions, lower_case_table_names ? system_charset_info : &my_charset_bin, 32, 0, 0,
               (my_hash_get_key)dboptions_get_key, free_dbopt, 0, key_memory_dboptions_hash);
...
static my_bool put_dbopt(const char *dbname, HA_CREATE_INFO *create)
{
  ...
  if (!(opt = (my_dbopt_t *)my_hash_search(&dboptions, (uchar *)dbname, length)))
  {
    ...
    char *tmp_name;
    if (!my_multi_malloc(key_memory_dboptions_hash, MYF(MY_WME | MY_ZEROFILL), &opt, sizeof(*opt), &tmp_name,
                         length + 1, NullS))
    {
      error = 1;
      goto end;
...
static bool write_db_opt(THD *thd, const char *path, HA_CREATE_INFO *create)
{
  ...
  {
    ulong length;
    length = (ulong)(strxnmov(buf, sizeof(buf) - 1, "default-character-set=", create->default_table_charset->csname,
...
int mysql_create_db(THD *thd, const char *db, HA_CREATE_INFO *create_info, bool silent)
{
  ...
  /* Check directory */
  path_len = build_table_filename(path, sizeof(path) - 1, db, "", "", 0, &was_truncated);
  if (was_truncated)
  {
  ...
  if (mysql_file_stat(key_file_misc, path, &stat_info, MYF(0)))
  {
    ...
    push_warning_printf(thd, Sql_condition::SL_NOTE, ER_DB_CREATE_EXISTS, ER(ER_DB_CREATE_EXISTS), db);
    error = 0;
    goto not_silent;
  }
  else
  {
    if (my_errno() != ENOENT)
    {
      ...
      my_error(EE_STAT, MYF(0), path, my_errno(), my_strerror(errbuf, sizeof(errbuf), my_errno()));
      goto exit;
    }
...
not_silent:
  if (!silent)
  {
    ...
    {
      id_len = my_strmov_quoted_identifier(thd, (char *)db_name_quoted, db, 0);
      db_name_quoted[id_len] = '\0';
      query = tmp_query;
      query_length = static_cast<size_t>(strxmov(tmp_query, "create database ", db_name_quoted, NullS) - tmp_query);
    }
    else
    {
      query = thd->query().str;
    ...
    ha_binlog_log_query(thd, 0, LOGCOM_CREATE_DB, query, query_length, db, "");
...
bool mysql_alter_db(THD *thd, const char *db, HA_CREATE_INFO *create_info)
{
  ...
  */
  ...
  ha_binlog_log_query(thd, 0, LOGCOM_ALTER_DB, thd->query().str, thd->query().length, db, "");

  if (mysql_bin_log.is_open())
  {
    int errcode = query_error_code(thd, TRUE);
    Query_log_event qinfo(thd, thd->query().str, thd->query().length, false, true, /* suppress_use */ true, errcode);
    /*
      Write should use the database being created as the "current
      database" and not the threads current database, which is the
      default.
...
bool mysql_rm_db(THD *thd, const LEX_CSTRING &db, bool if_exists, bool silent)
{
  ...
  if (!(dirp = my_dir(path, MYF(MY_DONT_SORT))))
  {
    ...
    {
      push_warning_printf(thd, Sql_condition::SL_NOTE, ER_DB_DROP_EXISTS, ER(ER_DB_DROP_EXISTS), db.str);
      error = false;
      goto update_binlog;
    }
  }

  if (find_db_tables_and_rm_known_files(thd, dirp, db.str, path, &tables, &found_other_files))
    goto exit;
  ...
  */
  ...
  if (lock_table_names(thd, tables, NULL, thd->variables.lock_wait_timeout, 0) || lock_db_routines(thd, db.str))
    goto exit;
  ...
  if (!thd->killed && !(tables && mysql_rm_table_no_locks(thd, tables, true, false, true, true)))
  {
    /*
...
update_binlog:
  if (!silent && !error)
  {
    ...
    if (!thd->query().str)
    {
      ...
      query_length = static_cast<size_t>(strxmov(path, "DROP DATABASE ", buffer_temp, "", NullS) - path);
    }
    else
    {
      query = thd->query().str;
  ...
  else if (mysql_bin_log.is_open() && !silent)
  {
    ...
    */
    if (thd->variables.gtid_next.type == GTID_GROUP)
    {
      ...
      thd->variables.gtid_next.gtid.to_string(global_sid_map, gtid_buf, true);
      my_error(ER_CANNOT_LOG_PARTIAL_DROP_DATABASE_WITH_GTID, MYF(0), path, gtid_buf, db.str);
      error = true;
      goto exit;
    }
    ...
    for (tbl = tables; tbl; tbl = tbl->next_local)
    {
      ...
      if (query_pos + tbl_name_len + 1 >= query_end)
      {
        ...
        */
        ...
        int ret = write_to_binlog(thd, query, query_pos - 1 - query, db.str, db.length);
      ...
      id_length = my_strmov_quoted_identifier(thd, (char *)temp_identifier, tbl->table_name, 0);
    ...
    if (query_pos != query_data_start)
    {
      ...
      /*
        ...
        metadata lock on the schema.
      */
      if (write_to_binlog(thd, query, query_pos - 1 - query, db.str, db.length))
      {
        error = true;
        goto exit;
      }
  ...
  for (uint idx = 0; idx < dirp->number_off_files && !thd->killed; idx++)
  {
    ...
    if (db && !my_strcasecmp(files_charset_info, extension, reg_ext))
    {
      ...
      *extension = 0;  // Remove extension
      TABLE_LIST *table_list = (TABLE_LIST *)thd->mem_calloc(sizeof(*table_list) + strlen(db) + 1 +
                                                             MYSQL50_TABLE_NAME_PREFIX_LENGTH + strlen(file->name) + 1);

      if (!table_list)
        DBUG_RETURN(true);
      table_list->db = (char *)(table_list + 1);
      table_list->db_length = my_stpcpy(const_cast<char *>(table_list->db), db) - table_list->db;
      table_list->table_name = table_list->db + table_list->db_length + 1;
      table_list->table_name_length = filename_to_tablename(file->name, const_cast<char *>(table_list->table_name),
                                                            MYSQL50_TABLE_NAME_PREFIX_LENGTH + strlen(file->name) + 1);
      table_list->open_type = OT_BASE_ONLY;

      /* To be able to correctly look up the table in the table cache. */
      if (lower_case_table_names)
        table_list->table_name_length = my_casedn_str(files_charset_info, const_cast<char *>(table_list->table_name));

      table_list->alias = table_list->table_name;  // If lower_case_table_names=2
      table_list->internal_tmp_table = is_prefix(file->name, tmp_file_prefix);
      MDL_REQUEST_INIT(&table_list->mdl_request, MDL_key::TABLE, table_list->db, table_list->table_name, MDL_EXCLUSIVE,
                       MDL_TRANSACTION);
      /* Link into list */
    ...
    else
    {
      ...
      */
      ...
      {
        char errbuf[MYSYS_STRERROR_SIZE];
        my_error(EE_DELETE, MYF(0), filePath, my_errno(), my_strerror(errbuf, sizeof(errbuf), my_errno()));
        DBUG_RETURN(true);
      }
    }
  }
...
bool mysql_change_db(THD *thd, const LEX_CSTRING &new_db_name, bool force_switch)
{
  ...
  if (is_infoschema_db(new_db_name.str, new_db_name.length))
  {
    ...
    mysql_change_db_impl(thd, to_lex_cstring(INFORMATION_SCHEMA_NAME), SELECT_ACL, system_charset_info);
    goto done;
  }
  ...
  */
  ...
  db_access = sctx->check_access(DB_ACLS)
                  ? DB_ACLS
                  : acl_get(sctx->host().str, sctx->ip().str, sctx->priv_user().str, new_db_file_name.str, false) |
                        sctx->master_access();

  if (!force_switch && !(db_access & DB_ACLS) && check_grant_db(thd, new_db_file_name.str))
  {
    my_error(ER_DBACCESS_DENIED_ERROR, MYF(0), sctx->priv_user().str, sctx->priv_host().str, new_db_file_name.str);
  ...
  if (check_db_dir_existence(new_db_file_name.str))
  {
    if (force_switch)
    {
      ...
      push_warning_printf(thd, Sql_condition::SL_NOTE, ER_BAD_DB_ERROR, ER(ER_BAD_DB_ERROR), new_db_file_name.str);
...
bool mysql_upgrade_db(THD *thd, const LEX_CSTRING &old_db)
{
  ...
  */
  ...
  if ((dirp = my_dir(path, MYF(MY_DONT_SORT))))
  {
    ...
    for (uint idx = 0; idx < nfiles && !thd->killed; idx++)
    {
      ...
      if (my_strcasecmp(files_charset_info, (extension = fn_rext(file->name)), reg_ext))
        continue;
  ...
  if ((table_list = thd->lex->query_tables) && (error = mysql_rename_tables(thd, table_list, 1)))
  {
    /*
  ...
  */
  ...
  {
    int errcode = query_error_code(thd, TRUE);
    Query_log_event qinfo(thd, thd->query().str, thd->query().length, FALSE, TRUE, TRUE, errcode);
    thd->clear_error();
