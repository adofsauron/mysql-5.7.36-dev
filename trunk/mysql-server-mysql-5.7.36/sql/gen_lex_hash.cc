...
void insert_into_hash(hash_lex_struct *root, const char *name, int len_from_begin, int index)
{
  ...
  insert_into_hash(root->char_tails + (*name) - root->first_char, name + 1, len_from_begin + 1, index);
}
...
int main(int argc, char **argv)
{
  ...
  if (hash_map_info().print_hash_map("sql_keywords", SG_KEYWORDS | SG_HINTABLE_KEYWORDS))
    exit(1);
