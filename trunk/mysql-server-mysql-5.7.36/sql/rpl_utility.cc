...
int compare_lengths(Field *field, enum_field_types source_type, uint16 metadata)
{
  ...
  size_t const source_length = max_display_length_for_field(source_type, metadata);
  ...
  DBUG_PRINT("debug", ("source_length: %lu, source_type: %u,"
                       ...
                       (unsigned long)source_length, source_type, (unsigned long)target_length, field->real_type()));
...
static void show_sql_type(enum_field_types type, uint16 metadata, String *str, const CHARSET_INFO *field_cs)
{
  ...
  switch (type)
  {
    ...
    case MYSQL_TYPE_VARCHAR:
    {
      ...
      size_t length = cs->cset->snprintf(cs, (char *)str->ptr(), str->alloced_length(), "varchar(%u(bytes))", metadata);
      str->length(length);
    }
    break;
    ...
    {
      const CHARSET_INFO *cs = str->charset();
      size_t length = cs->cset->snprintf(cs, (char *)str->ptr(), str->alloced_length(), "decimal(%d,?)", metadata);
      str->length(length);
    }
    break;
    ...
    {
      const CHARSET_INFO *cs = str->charset();
      size_t length = cs->cset->snprintf(cs, (char *)str->ptr(), str->alloced_length(), "decimal(%d,%d)", metadata >> 8,
                                         metadata & 0xff);
      str->length(length);
    }
    break;
    ...
    case MYSQL_TYPE_STRING:
    {
      ...
      */
      ...
      size_t length = cs->cset->snprintf(cs, (char *)str->ptr(), str->alloced_length(), "char(%d(bytes))", bytes);
      str->length(length);
    }
    break;
...
bool is_conversion_ok(int order, Relay_log_info *rli)
{
  ...
  allow_non_lossy = slave_type_conversions_options & (1ULL << SLAVE_TYPE_CONVERSIONS_ALL_NON_LOSSY);
  allow_lossy = slave_type_conversions_options & (1ULL << SLAVE_TYPE_CONVERSIONS_ALL_LOSSY);
...
*/
inline bool timestamp_cross_check(enum_field_types type1, enum_field_types type2)
{
...
*/
inline bool datetime_cross_check(enum_field_types type1, enum_field_types type2)
{
...
/**
   ...
   current setting.
 */
static bool can_convert_field_to(Field *field, enum_field_types source_type, uint16 metadata, Relay_log_info *rli,
    ...
    if (field->compatible_field_size(metadata, rli, mflags, order_var))
      ...
      DBUG_RETURN(false);
           ...
           (timestamp_cross_check(field->real_type(), source_type) ||
            datetime_cross_check(field->real_type(), source_type) || time_cross_check(field->real_type(), source_type)))
  {
    /*
  ...
  switch (source_type)  // Source type (on master)
  {
    ...
    case MYSQL_TYPE_DOUBLE:
      switch (field->real_type())
      {
        ...
        case MYSQL_TYPE_DOUBLE:
        {
          if (source_type == MYSQL_TYPE_NEWDECIMAL || source_type == MYSQL_TYPE_DECIMAL)
...
*/
bool table_def::compatible_with(THD *thd, Relay_log_info *rli, TABLE *table, TABLE **conv_table_var) const
{
  /*
  ...
  */
  ...
  for (uint col = 0; col < cols_to_check; ++col)
  {
    ...
    else
    {
      ...
      if (field->has_charset() && (field->type() == MYSQL_TYPE_VARCHAR || field->type() == MYSQL_TYPE_STRING))
      {
        field_sql_type.append((field->type() == MYSQL_TYPE_VARCHAR) ? "varchar" : "char");
        const CHARSET_INFO *cs = field->charset();
        size_t length =
            cs->cset->snprintf(cs, (char *)target_type.ptr(), target_type.alloced_length(), "%s(%u(bytes) %s)",
                               field_sql_type.c_ptr_safe(), field->field_length, field->charset()->csname);
      ...
      if (report_level != INFORMATION_LEVEL)
        rli->report(report_level, ER_SLAVE_CONVERSION_FAILED, ER(ER_SLAVE_CONVERSION_FAILED), col, db_name, tbl_name,
    ...
    for (unsigned int col = 0; col < tmp_table->s->fields; ++col)
      if (tmp_table->field[col])
      {
        ...
        DBUG_PRINT("debug", ("Field %s - conversion required."
                             ...
                             tmp_table->field[col]->field_name, source_type.c_ptr_safe(), target_type.c_ptr_safe()));
      }
  }
#endif
...
TABLE *table_def::create_conversion_table(THD *thd, Relay_log_info *rli, TABLE *target_table) const
{
  ...
  */
  ...
  unsigned_flag = slave_type_conversions_options & (1ULL << SLAVE_TYPE_CONVERSIONS_ALL_UNSIGNED);

  // Check if slave_type_conversions contains ALL_SIGNED
  unsigned_flag = unsigned_flag && !(slave_type_conversions_options & (1ULL << SLAVE_TYPE_CONVERSIONS_ALL_SIGNED));

  for (uint col = 0; col < cols_to_create; ++col)
  {
    Create_field *field_def = (Create_field *)alloc_root(thd->mem_root, sizeof(Create_field));
    ...
    uint pack_length = 0;
    uint32 max_length = max_display_length_for_field(type(col), field_metadata(col));
    ...
    switch (type(col))
    {
      ...
      case MYSQL_TYPE_DECIMAL:
        sql_print_error(
            ...
            " column Name: %s.%s.%s.",
            target_table->s->db.str, target_table->s->table_name.str, target_table->field[col]->field_name);
        goto err;
    ...
    DBUG_PRINT("debug", ("sql_type: %d, target_field: '%s', max_length: %d, decimals: %d,"
                         ...
                         binlog_type(col), target_table->field[col]->field_name, max_length, decimals, TRUE,
                         unsigned_flag, pack_length));
    field_def->init_for_tmp_table(type(col), max_length, decimals,
                                  TRUE,           // maybe_null
...
err:
  if (conv_table == NULL)
  {
    ...
    if (report_level != INFORMATION_LEVEL)
      rli->report(report_level, ER_SLAVE_CANT_CREATE_CONVERSION, ER(ER_SLAVE_CANT_CREATE_CONVERSION),
...
static uchar *hash_slave_rows_get_key(const uchar *record, size_t *length, my_bool not_used MY_ATTRIBUTE((unused)))
{
  DBUG_ENTER("get_key");
...
{
  if (my_hash_init(&m_hash, &my_charset_bin,                     /* the charater set information */
                   16 /* TODO */,                                /* growth size */
                   0,                                            /* key offset */
                   0,                                            /* key length */
...
HASH_ROW_ENTRY *Hash_slave_rows::make_entry(const uchar *bi_start, const uchar *bi_ends)
{
  ...
  HASH_ROW_ENTRY *entry = (HASH_ROW_ENTRY *)my_malloc(key_memory_HASH_ROW_ENTRY, sizeof(HASH_ROW_ENTRY), MYF(0));
  HASH_ROW_PREAMBLE *preamble =
      (HASH_ROW_PREAMBLE *)my_malloc(key_memory_HASH_ROW_ENTRY, sizeof(HASH_ROW_PREAMBLE), MYF(0));
  HASH_ROW_POS *pos = (HASH_ROW_POS *)my_malloc(key_memory_HASH_ROW_ENTRY, sizeof(HASH_ROW_POS), MYF(0));

  if (!entry || !preamble || !pos || DBUG_EVALUATE_IF("fake_myalloc_failure", 1, 0))
    goto err;
...
bool Hash_slave_rows::put(TABLE *table, MY_BITMAP *cols, HASH_ROW_ENTRY *entry)
{
...
HASH_ROW_ENTRY *Hash_slave_rows::get(TABLE *table, MY_BITMAP *cols)
{
  ...
  entry = (HASH_ROW_ENTRY *)my_hash_first(&m_hash, (const uchar *)&key, sizeof(my_hash_value_type), &state);
  if (entry)
  {
...
bool Hash_slave_rows::next(HASH_ROW_ENTRY **entry)
{
   ...
   */
  *entry = (HASH_ROW_ENTRY *)my_hash_next(&m_hash, (const uchar *)&key, sizeof(my_hash_value_type), &state);
  if (*entry)
  {
...
my_hash_value_type Hash_slave_rows::make_hash_key(TABLE *table, MY_BITMAP *cols)
{
  ...
  if (table->s->null_bytes > 0)
  {
    ...
    */
    if (table->s->last_null_bit_pos > 0)
    {
      ...
      record[table->s->null_bytes - 1] |= 256U - (1U << table->s->last_null_bit_pos);
    }
  }
  ...
  for (Field **ptr = table->field; *ptr && ((*ptr)->field_index < cols->n_bits); ptr++)
  {
     ...
     */
    if (bitmap_is_set(cols, f->field_index) && !f->is_virtual_gcol() &&  // Avoid virtual generated columns on hashes
        !f->is_null())
    {
      /*
